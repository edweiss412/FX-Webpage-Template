...
- [ ] **Step 1: failing tests.** Cases (expected values derived by running the REAL `warningFingerprint` on the fixture warnings — never hardcoded hashes):
  ...
  - `normalizeStagedIgnoredWarnings`: well-formed array passes through; non-array → `[]`; entries missing a string `fingerprint` dropped; extra keys stripped is NOT required (pass-through allowed) — assert only the three required fields survive.
...
### Task 2: migration `pending_syncs.ignored_warnings`

**Files:**
- Create: supabase/migrations/<UTCstamp>_pending_syncs_ignored_warnings.sql (new)
- Modify: `supabase/__generated__/schema-manifest.json` (regenerated)
- Test: covered by the parity gate layers; plus Task 5's round-trip.
...
- [ ] **Step 1:** migration SQL:
  ...
  ```
  No CHECK (spec §2.7 matrix: single writer + read-side coercion). Apply-twice safe via `if not exists`.
- [ ] **Step 2:** apply locally (`supabase db query` against the local stack or `psql "$DATABASE_URL" -f <file>`), then `pnpm gen:schema-manifest`; commit the regenerated manifest IN THIS COMMIT.
- [ ] **Step 3:** apply surgically to the validation project: `supabase db query --linked "<the ALTER>"` then `notify pgrst, 'reload schema';` (validation-schema-parity step 3).
- [ ] **Step 4: commit** `feat(db): pending_syncs.ignored_warnings staged ignore column`.
...
- Test: `tests/admin/assembleStep3Row.test.ts` (extend the existing suite for this module; if none exists at that path, `rg -l "assembleStep3Row" tests` and extend the suite found).
...
- Modify: `components/admin/OnboardingWizard.tsx` (call after the `components/admin/OnboardingWizard.tsx:545-566` row map)
- Test: tests/lib/admin/enrichStep3WarningModels.test.ts (new)
...
// Attaches warningModel?: WizardWarningModel per spec §2.0/§2.1.
```
...
- [ ] **Step 1: failing tests** (injected loader; taxonomy per spec §2.0 — classify with `hasReviewablePreview` semantics: `parseResult` non-null object with truthy `show`):
  ...
  - reviewable `{show, warnings: null}` → `{active: [], ignored: []}`.
...
- [ ] **Step 4: green.**  **Step 5:** wire into `OnboardingWizard` (`const rows = await enrichStep3WarningModels(assembled, loadIgnoredWarnings)`), typecheck, commit `feat(admin): enrich step3 rows with wizard warning models`.
...
- [ ] **Step 1: failing tests** — mirror `tests` for `useRawStaged` (`rg -l "useRawStaged" tests` and copy its harness shape: injected deps for supabase client + withTx). Cases:
  ...
  - every Supabase await's returned-error path → `infra_error` (not thrown).
  - Lock topology assertion: the mutation executes INSIDE the `withShowLock(driveFileId, …)` callback (assert via an injected wrapper spy that the upsert ran within the callback frame), and the callback body performs NO `.rpc(` call (static assert in Task 6 pins it too).
- [ ] **Step 2: red.**  **Step 3: implement** clause-for-clause per `useRawStaged.ts` (same-origin assert, `requireAdmin` + `requireAdminIdentity`, pre-lock pairing verify, locked re-read of `parse_result` + `ignored_warnings`, `warningsOf`-style coercion, `canonicalize(admin.email)` for `ignored_by`, jsonb update, post-commit `logAdminOutcome({ code: "STAGED_WARNING_IGNORED" | "STAGED_WARNING_UNIGNORED", … })` outside the lock, only when mutated).
- [ ] **Step 4: green.**  **Step 5: commit** `feat(admin): staged warning ignore server action`.

### Task 6: mutation + lock registries for the action

**Files:**
- Modify: `tests/log/_auditableMutations.ts` (+ row `{ file: "app/admin/onboarding/_actions/stagedWarningIgnore.ts", fn: "setStagedWarningIgnore", code: "STAGED_WARNING_IGNORED" }` — add a second row for the UNIGNORED code only if the discovery meta-test requires one row per code; follow the pattern the file uses for multi-code surfaces), `tests/log/adminOutcomeBehavior.test.ts` (behavioral proof via `proveAdminOutcomeBehavior` at `tests/log/adminOutcomeBehavior.test.ts:737` — sink records only on the committed-success branch), `tests/auth/advisoryLockRpcDeadlock.test.ts` (NEW parallel arm: scan the source of every direct `withShowLock` acquirer — seed list from the Pre-verified holder sweep above — and assert the `withShowLock` callback body of the new action module contains no `.rpc(`; the existing `withShowAdvisoryLock` arm at `tests/auth/advisoryLockRpcDeadlock.test.ts:135-214` is untouched).
...
- [ ] **Step 1: failing tests.**
  - shadow round-trip unit: writer→parser preserves the entries beside `useRawDecisions`.
  - create-path proof: phase-2 apply with staged entries + a first-seen snapshot → rows exist in `public.ignored_warnings` for `snapshot.showId` with the STAGING-TIME `ignored_by`; re-apply → no duplicates (`on conflict do nothing`).
  - update-path proof THROUGH the payload chain: build the shadow payload with entries, parse it (`parseShadowPayloadForApply`), run `applyStagedCore`→phase-2 against an EXISTING show id → rows exist for that id.
  - orphaned fingerprint (no matching live warning) carries without error.
  - RED validity: each proof's production line is the phase-2 insert that does not exist yet; the tests run against the live phase-2 harness the existing phase2 suites use (`rg -l "phase2" tests/sync` and reuse that harness).
...
- [ ] **Step 1: failing tests.**
  ...
  - React keys: assert `stableWarningKeys` output over each rendered subset (derive expected via the real helper).
...
- [ ] **Step 1:** walk `components/admin/wizard/**` + `components/admin/review/ShowReviewSurface.tsx` from the filesystem; flag any line matching `warningsBySection\(|\.parseResult\??\.warnings|data\.warnings` whose `file:symbol` is NOT in the registered-site list (exactly: the two choke-point modules, `activeWarningEntries` call sites, `WarningsBreakdown` + registry closures, enrichment/stamping helpers, the attention memo's wrapper call). Include a premise: the walk SAW every registered site (a registry row whose site is missing fails — the guard-on-itself rule).
- [ ] **Step 2:** observe it red against a deliberately planted bypass in a scratch copy (do not commit the plant), then green on the real tree.
...
- [ ] Whole-diff cross-model review via codex-guard (`--stage diff --round 1`; round-1 diff brief needs the GUARD SURFACE line ONLY if a mutation-registry surface is touched — none is; state that in the brief). 4-round cap; filing + bl-orch past it.
