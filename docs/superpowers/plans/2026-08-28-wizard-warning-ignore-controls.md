...
- [ ] **Step 1 (RED declaration — the same guard-authoring shape Task 13 declares, R4 F1):** this task deliberately claims NO chronological red — a topology arm authored against a compliant tree passes immediately, and manufacturing a red would be the test-local shape the RED-validity rule forbids. Its validity evidence: (a) the arm's scan-list premise is production-anchored (it asserts every file from the holder sweep was actually read — misname one and it fails on the real tree); (b) two uncommitted hand-probes recorded verbatim in the commit message: an `.rpc("x")` call inserted into the new action's `withShowLock` callback that the arm flags, and a holder file temporarily dropped from the scan list that the premise flags. Both reverted before commit (the MEMORY-guards planted-mutant discipline).
...
- [ ] Whole-diff cross-model review via codex-guard (`--stage diff --round 1`). **CORRECTED at closeout (whole-diff R1 finding 4) — the premise this line originally carried was WRONG.** It claimed no surface enrolled in the source-mutation score registry is touched. One is: `tests/styles/controlOutlineScan.ts` is enrolled (`tests/mutation/source/registry.ts`, id `controlOutlineScan`, `scoreFloor: 1`), and this arc edits it, because the UI edits moved the line-keyed census rows it holds. Its governing probe contract is explicit that this re-triggers score-before-closeout: `docs/superpowers/specs/probes/2026-08-18-border-border-neutral-fill-census.md` §7 — "any change to `controlOutlineScan.ts` re-triggers score-before-closeout … running `pnpm mutation:guards` and stating the score plus an empty unaccepted-survivor set in the round-1 diff brief". Two enrolled SUITES are touched as well (`_metaSubtleOnInteractive.test.ts` → `interactiveScanCore`, `_metaControlOutlineResidue.test.ts` → `controlOutlineResidue`), and a suite change can move a score as surely as a source change can. What the arc actually changed in all three is census DATA and one cardinality pin — no scanner, resolver or oracle logic — but the contract keys on the file, not on the nature of the edit, and it is right to: "only data moved" is exactly the claim a score exists to check rather than accept. The score is stated with the `OPERATORS:` tail in the brief that carries it. (The `AUDITABLE_MUTATIONS` extension in Task 5 remains a DIFFERENT registry and does not trigger the line.) 4-round cap; filing + bl-orch past it.
