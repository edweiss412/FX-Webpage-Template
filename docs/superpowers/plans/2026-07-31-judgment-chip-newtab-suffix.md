...
- [ ] **Step 4: Impeccable dual-gate** — `/impeccable critique` + `/impeccable audit` on the diff (invariant 8; canonical v3 setup gates). **Disposition sink (defined):** append a `## Close-out (§12)` section to THIS plan document recording every P0-P3 finding + disposition (fixed / deferred with DEFERRED.md entry), and summarize in the PR body. **Persistence:** the close-out section, every finding-fix diff, and any DEFERRED.md entry are COMMITTED (`git add -A && git commit -m "docs: impeccable close-out dispositions"` plus `fix(admin): …` commits for code fixes) AND PUSHED in this step — nothing this gate produces may remain only in the worktree (plan r3 HIGH).
- [ ] **Step 5: Whole-diff Codex review** via codex-guard (REVIEWER ONLY, fresh-eyes, do-not-relitigate from spec §1.1) to APPROVE. Every repair round commits AND pushes before re-dispatch, so the reviewed head is always the remote head.
- [ ] **Step 6: Merge — fail-closed preconditions first.** All three MUST hold before merging: (a) `git status --porcelain` is EMPTY (any output = an uncommitted gate artifact — commit and push it, do not merge over it); (b) `git rev-list --count origin/fix/judgment-chip-newtab-suffix..HEAD` = 0 (everything pushed); (c) real CI green on the FINAL pushed head. Then `gh pr merge --merge` → `git -C /Users/ericweiss/FX-Webpage-Template pull --ff-only` → verify `git rev-list --left-right --count main...origin/main` = `0  0` → CronDelete the nudge, clear pane label. The clean-tree precondition is the structural closure of the whole leave-behind class — it fails on ANY forgotten artifact, enumerated or not.
