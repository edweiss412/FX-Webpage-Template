...
- Meta-test inventory (writing-plans rule): CREATES no registry entries; EXTENDS `tests/components/admin/wizard/modalSectionChromeClasses.test.tsx` (new status-class block) and `tests/components/a11y/newTabAnnouncementBehavior.test.tsx` (three site cases). Registry meta-tests (`_metaInfraContract`, advisory-lock, admin-alert catalog, sentinel-hiding, no-inline-email) — none applies: no Supabase calls, locks, alerts, tiles, or email surfaces are touched. BACKLOG graduation exercises `tests/docs/_metaDeferralLedgerGraduation.test.ts` (Task 5).
...
describe("chip status classes (spec 2026-07-31 §2.2)", () => {
  ...
  test("clean chip stays borderless and sunken", () => {
    ...
    expect(chip?.className).toContain("text-text-subtle");
...
Failure mode caught: a silent revert of the judgment outline, or the fix bleeding into the clean/flagged branches. The negative lookahead pins that `border-border` (hairline) is GONE from the judgment branch, not merely joined by the strong token.
...
- [ ] **Step 5: Commit** — `git commit -m "feat(admin): stripNewTabSuffix helper in the announcement copy's single home"`
...
- [ ] **Step 1: Write the failing tests** — new `describe` in `newTabAnnouncementBehavior.test.tsx` (helper `countPhrase = (s: string) => s.split("(opens in a new tab)").length - 1` local to the block):
...
describe("interpolated labels never stack the appended suffix (spec 2026-07-31 §3)", () => {
  const countPhrase = (s: string) => s.split("(opens in a new tab)").length - 1;
  ...
  test("SheetIconLink: trailing occurrence in the subject dedupes to one", () => {
    ...
    expect(countPhrase(getByRole("link").getAttribute("aria-label") ?? "")).toBe(1);
  });
  ...
  test("SheetIconLink: mid-string occurrence is preserved, exactly two total", () => {
    ...
    expect(countPhrase(getByRole("link").getAttribute("aria-label") ?? "")).toBe(2);
  });
  ...
  test("SheetTitleLink: title that strips to empty takes the no-subject fallback", () => {
    ...
    expect(getByRole("link").getAttribute("aria-label")).toBe(
...
- [ ] **Step 2: Run to verify failures** — export the two components first (mechanical `export` keyword, no behavior change), then `pnpm exec vitest run tests/components/a11y/newTabAnnouncementBehavior.test.tsx` → the four dedup/fallback cases FAIL (counts are 2/3/2/2 and the fallback label doesn't exist); the mid-string case PASSES (pins current-and-future behavior).
...
- [ ] **Step 4: Re-run** the behavior file AND `pnpm exec vitest run tests/styles/_metaNewTabAnnouncement.test.ts` (unmodified, green — every branch still carries the literal phrase) AND `pnpm exec vitest run tests/components/admin/wizard/` (no chrome/card regression).
- [ ] **Step 5: Commit** — `git commit -m "fix(admin): dedupe the appended new-tab suffix at the three interpolated labels (BL-NEWTAB-DOUBLE-ANNOUNCE-USER-DATA)"`
...
- [ ] **Step 2:** Commit — `git commit -m "docs: graduate BL-HEADER-JUDGMENT-CHIP-CONTRAST + BL-NEWTAB-DOUBLE-ANNOUNCE-USER-DATA"`.
