...
- [ ] **Steps 4-5: Gate convergence loop (fixed point on the head SHA).** Run to a fixed point, not once-through (plan r4b HIGH):
  ...
  4. **Convergence check:** if `git rev-parse HEAD` ≠ `H` (either gate produced a commit), GO TO 1 — both gates must re-evaluate the new head. The loop exits only when one full pass (impeccable green AND Codex APPROVE) completes with ZERO new commits, i.e. both approvals reference the identical SHA that will merge. UI-fix commits are never merged without an impeccable pass over them, and review repairs never without both gates.

- [ ] **Step 6: Merge — fail-closed preconditions first.** All four MUST hold: (a) `git status --porcelain` EMPTY; (b) `git rev-list --count origin/fix/judgment-chip-newtab-suffix..HEAD` = 0; (c) real CI green on the final pushed head; (d) the convergence loop's exiting pass approved exactly `git rev-parse HEAD` (record the SHA beside the approvals in `## Close-out (§12)`). Then `gh pr merge --merge` → `git -C /Users/ericweiss/FX-Webpage-Template pull --ff-only` → verify `git rev-list --left-right --count main...origin/main` = `0  0` → CronDelete the nudge, clear pane label.
