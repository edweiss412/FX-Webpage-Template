...
- [ ] **Step 2: Verify — grep-zero (spec §2.3b RENAME-SURFACE greps; NOT a blanket `git grep observability`):**
  ...
  - **COMPREHENSIVE nav-surface sweep (Codex plan-R1 MED + R2 — the URL greps miss label-only AND testid-derived staleness):** the nav id `observability`→`telemetry` flows through to the mobile-tab testid (`AdminNav.tsx:140` renders `data-testid={`admin-bottom-tab-${item.id}`}`), and the label `Activity`→`Telemetry`. Update the FULL nav surface + EVERY nav test (both `navConfig.test.ts` locations + both `AdminNav*.test.tsx`), or their assertions go tautological:
    ...
    - **Verification greps (all must return ZERO across the nav surface + BOTH nav test dirs):** `git grep -n "admin-bottom-tab-observability"`; `git grep -n "inObservability\|id === \"observability\"\|\"observability\"" -- components/admin/nav`; `git grep -n '/Activity/i\|"Activity"\|name: "Activity"' -- components/admin/nav tests/components/admin tests/e2e/developer-tier.spec.ts` (inspect each residual "Activity"; a genuinely-unrelated string is out of scope but the nav item's must be gone).
