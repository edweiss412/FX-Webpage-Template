...
- `tests/components/observability/` → `tests/components/telemetry/` (9 test files; update component import paths).
...
- [ ] **Step 2: Verify** — grep-zero gates SCOPED to live surfaces `-- app/ components/ lib/ scripts/ tests/` (excludes `docs/**`, which per spec §1 keeps its historical observability tokens — Codex plan-R4): `git grep -n "components/admin/observability" -- app/ components/ lib/ scripts/ tests/` → 0; `git grep -n "observabilityTypes" -- app/ components/ lib/ scripts/ tests/` → 0; `git grep -n "fxav.observability" -- app/ components/ lib/ scripts/ tests/` → 0 (unless the commented legacy-compat path was chosen). `pnpm typecheck` clean. Run the moved component tests: `pnpm vitest run tests/components/telemetry/ tests/admin/parseAppEventFilters.test.ts tests/observe/` → green.
- [ ] **Step 3: Commit**
...
- [ ] **Step 2: Verify — grep-zero (spec §2.3b RENAME-SURFACE greps; NOT a blanket `git grep observability`).** **CRITICAL (Codex plan-R3): every grep-zero gate MUST be scoped to LIVE surfaces with `-- app/ components/ lib/ scripts/ tests/` (which excludes `docs/**` — historical specs/plans + THIS plan intentionally retain old tokens and would make an unscoped grep never reach zero).** Apply that scope to each:
  - `git grep -n "admin/observability" -- app/ components/ lib/ scripts/ tests/` → 0
  - `git grep -n "observability-dim" -- app/ components/ lib/ scripts/ tests/` → 0
  - `git grep -n "observability-layout" -- app/ components/ lib/ scripts/ tests/` → 0
  - `git grep -n "admin-settings-observability-link" -- app/ components/ lib/ scripts/ tests/` → 0
  - `git grep -n 'id: "observability"\|"observability"' components/admin/nav/navConfig.ts` → 0
  - **COMPREHENSIVE nav-surface sweep (Codex plan-R1 MED + R2 — the URL greps miss label-only AND testid-derived staleness):** the nav id `observability`→`telemetry` flows through to the mobile-tab testid (`AdminNav.tsx:140` renders `data-testid={`admin-bottom-tab-${item.id}`}`), and the label `Activity`→`Telemetry`. Update the FULL nav surface + EVERY nav test (both `navConfig.test.ts` locations + both `AdminNav*.test.tsx`), or their assertions go tautological:
    - **`components/admin/nav/navConfig.ts`**: id union member `"observability"`→`"telemetry"` (`:5`); the entry `id/label/short/href` (`:41-44`); the `inObservability` local + `pathname === "/admin/observability"`/`.startsWith` (`:68`) → `inTelemetry` + `/admin/dev/telemetry`; `id === "observability"` (`:71`)→`"telemetry"`; comments (`:36,:47`). `AdminNav.tsx:43,52` comments "Activity/observability"→"Telemetry".
    - **`tests/components/admin/nav/navConfig.test.ts`**: the id-list `["dashboard","attention","settings","observability"]` (`:5`)→`"telemetry"`; the "observability (Activity) … href /admin/observability" case (`:9`) → Telemetry/`/admin/dev/telemetry`/`telemetry`; the `:93` comment.
    - **`tests/components/admin/navConfig.test.ts`** (the SECOND file): grep it for `observability`/`Activity` and update every assertion.
    - **`tests/components/admin/nav/AdminNav.test.tsx`** (`:82` label, `:84` `admin-bottom-tab-observability`→`admin-bottom-tab-telemetry`) and **`AdminNav-developer.test.tsx`** (`:36,:40,:50,:54,:63,:66` `/Activity/i`→`/Telemetry/i`; `:43,:56` `admin-bottom-tab-observability`→`admin-bottom-tab-telemetry`; comments `:3,:10,:41,:55`).
    - **`tests/e2e/developer-tier.spec.ts`** (`:57` `name: "Activity"`→`"Telemetry"`; describe/comments `:11,:26,:100,:172,:183`).
    - **Verification greps (SCOPED to live nav surfaces — Codex plan-R3; all must return ZERO):** `git grep -n "admin-bottom-tab-observability" -- components/ tests/`; `git grep -n "inObservability\|id === \"observability\"\|\"observability\"" -- components/admin/nav`; `git grep -n '/Activity/i\|"Activity"\|name: "Activity"' -- components/admin/nav tests/components/admin tests/e2e/developer-tier.spec.ts` (inspect each residual "Activity"; a genuinely-unrelated string is out of scope but the nav item's must be gone). None of these greps scan `docs/**`.
...
- [ ] **Step 3: Run the gate suites** — `pnpm vitest run tests/admin/build-artifact-gate.test.ts tests/cross-cutting/auth-chain-audit.test.ts tests/auth/developerGatingContract.test.ts tests/admin/telemetryRouteAudit.test.ts tests/admin/telemetry-requires-developer.test.ts tests/app/admin/telemetryPage.test.tsx tests/components/admin/nav/navConfig.test.ts tests/components/admin/navConfig.test.ts tests/components/admin/nav/AdminNav.test.tsx tests/components/admin/nav/AdminNav-developer.test.tsx tests/cross-cutting/no-raw-codes.test.ts` → green (incl. the updated `navConfig.test.ts` "Telemetry" label/href/id assertions). The renamed `tests/e2e/developer-tier.spec.ts` "Telemetry" nav assertions run in the close-out Playwright pass (Task 12/close-out) — verify they were updated in this task's sweep. Spot-check the prod artifact: `ADMIN_DEV_PANEL_ENABLED` unset `pnpm build` → confirm `.next` contains `/admin/dev/telemetry` and NOT the dev-only trio (build-artifact-gate encodes this; a manual confirm is cheap given Codex R1+R3 burned three rounds on this vector). `pnpm typecheck` + `pnpm format:check` clean.
- [ ] **Step 4: Commit**
