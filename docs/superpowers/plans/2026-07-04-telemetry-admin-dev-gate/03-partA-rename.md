...
- [ ] **Step 2: Verify — grep-zero (spec §2.3b RENAME-SURFACE greps; NOT a blanket `git grep observability`):**
  ...
  - **Stale nav-LABEL "Activity" gate (Codex plan-R1 MED — the URL greps miss label-only staleness):** the nav item's user-facing label changes `Activity` → `Telemetry`, so update every test asserting the OLD label and confirm none remain. Explicitly update: `tests/components/admin/nav/navConfig.test.ts` (`:9` "observability (Activity) is a desktopOnly destination with href /admin/observability" → Telemetry / `/admin/dev/telemetry` / id `telemetry`; also the `:93` comment), and `tests/e2e/developer-tier.spec.ts` (`:57` `getByRole("link", { name: "Activity" })` → `"Telemetry"`; the describe/comment labels at `:11,:26,:100,:172,:183`). Verification grep: `git grep -n '"Activity"' -- components/admin/nav tests/e2e/developer-tier.spec.ts tests/components/admin/nav` returns ZERO references to the renamed nav item (a bare unrelated "Activity" string elsewhere, if any, is out of scope — inspect each match).
