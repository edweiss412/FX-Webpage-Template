...
- Whole-diff R2-R4 (single vector, monitoring-summary scoping): menu-wide query -> row-scoped + group container (R2) -> document-order + row-exclusion (R3) -> nearest-common-ancestor membership proof + proper-subtree guard (R4 repair). R4's residual objection posits a "sibling footer within a monitoring-only subtree" - a structure that shares the Monitoring heading's exclusive subtree and thus satisfies every operational property of being IN the monitoring section; absent a component-side DOM identity (a components/ change, out of this branch's ratified scope and gate posture), the NCA form is the maximal membership proof. Vector CLOSED by disposition per the same-vector three-round rule (each successive patch strengthened the pin and moved the objection; negative marginal value). Recorded here so future rounds do not re-derive it. Reviewer's substantive R1 confirmations stand: classification, action resolutions, validator fit, materializability, sweep consistency.
- Local dev-build e2e stability (2026-07-22): full-spec runs on the contended dev box rotated three UNRELATED failures (documented ERR_EMPTY_RESPONSE SSR blip on a tier-1 scenario; deep-links footnote assertion, passes isolated 1.2s; auth+shell "deleteFixtureUserByEmail.listUsers failed: fetch failed" = local Supabase API blip during sibling-session DB contention). The one REAL finding — the composite's auto-opened menu intercepting the containment publish-toggle click — was fixed (Escape-close before the click) and the containment test passes post-fix. Real CI (dev-gate-e2e, quiet runner) is the close-out arbiter per the contended-box rule.
