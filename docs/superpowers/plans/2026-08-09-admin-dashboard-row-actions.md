...
The spec §3.5 inventory's rows are asserted RED-first inside the tasks that IMPLEMENT each state, where they have production defects: menu open/close/pending/error rows in Task 3's suite; held-decision rows in Task 4's; confirm-step focus rows in Task 5's; the two compound rows (row-unmount portal close; background-refresh remount close) in Task 7's Playwright spec (they need real portals). Each task's suite enumerates the conditional renders it introduces and asserts instant treatment (or the primitive default) per the inventory. The inventory table verbatim:
