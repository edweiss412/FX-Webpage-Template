...
- [ ] **Step 2:** `git worktree add -b feat/round-economy-followups-2 ../FX-worktrees/round-economy-followups-2 origin/main`, then `pnpm install` and `pnpm worktree:link-env` in the new worktree (tests run on this branch, so the env symlink is mandatory per invariant 11). Docs-only: skip ONLY `preflight`, declared in the PR body.
...
- [ ] **Step 4:** Ledger disposition: this arc closes NO ledger entries, so there are no ids to pass to `pnpm ledger:claims --check` — run nothing (the checker exits 2 on an empty id list by design). If a later edit makes this branch close an entry, the full invariant-12 claim flow re-triggers.
...
- [ ] **Step 4:** `herdr pane rename "$HERDR_PANE_ID" --clear && herdr agent rename "$HERDR_PANE_ID" --clear`; CronDelete the nudge; then remove the worktree FROM THE MAIN CHECKOUT with absolute paths (you cannot remove the directory you are standing in): `git -C /Users/ericweiss/FX-Webpage-Template worktree remove /Users/ericweiss/FX-worktrees/round-economy-followups-2`.
