...
- Produces: `mintFixtureCombos(client, fixtures): Promise<{ minted: number }>` — **mint loop ONLY, no finalize** (each combo → `client.rpc("mint_validation_fixture_atomic", {...})`, `{data,error}` destructure + throw on error; mirrors `scripts/validation-reseed.ts:102-118`). AND `finalizeFixtures(client, requiredCombos, validationTodayIso): Promise<void>` — the single `validation_finalize_all_atomic` call (`scripts/validation-reseed.ts:123-134`); the RPC REQUIRES `p_validation_today_iso` (`supabase/migrations/20260527210001_validation_finalize_all_atomic.sql:23-26`), so the caller MUST pass the SAME `validationTodayIso` used to build the fixtures (the CLI computes one before mint/finalize to avoid UTC-midnight drift — `scripts/validation-reseed.ts:193-195,123-125`).
...
- [ ] **Step 1: Write the failing real-DB tests** (`tests/db/reseedFixtures.test.ts`): pick a fixed `validationTodayIso` (e.g. `"2026-06-22"`). (a) `mintFixtureCombos(serviceClient, buildFixtures(validationTodayIso))` then `finalizeFixtures(serviceClient, allCombos, validationTodayIso)` against an empty DB → seeded show count equals the fixtures' combo count (derive from `fixtures`, not a literal) and `validation_state.combos_materialized` lists all combos; (b) **single-combo regression:** `mintFixtureCombos` for ONE combo's fixtures WITHOUT `finalizeFixtures` → that combo's shows exist AND no prune occurred (a pre-seeded other-combo show survives, `validation_state` not rewritten). Run → FAIL (module missing).
...
- [ ] **Step 2b: Run the parity check RED (TDD invariant 1 — failing check before the generated/runtime layers exist):** with ONLY the §12.4 prose rows added (Step 2), run the **direct vitest command** (NOT `pnpm test:audit:x1-catalog-parity`, which is `pnpm gen:spec-codes && vitest …` per `package.json:30` and would regenerate `spec-codes.ts` first, masking the red): `pnpm exec vitest run tests/cross-cutting/codes.test.ts tests/cross-cutting/extract-spec-codes.test.ts`. Expected: FAIL — the 4 new codes are in §12.4 prose but missing from `lib/messages/__generated__/spec-codes.ts` and `lib/messages/catalog.ts`. This proves the gate guards the lockstep.
...
- [ ] **Step 1: Write the failing tests FIRST — action test AND the meta-test extension** (TDD invariant 1):
  ...
  - **Reseed SUCCESS-PATH test (P2-F1 — proves finalize is not forgotten):** gates pass + the assert RPC stubbed to succeed → assert, in order, that the service-role client IS constructed, then **`mintFixtureCombos` is called, then `finalizeFixtures` is called** (mock both as spies; assert `mintFixtureCombos.mock.invocationCallOrder[0] < finalizeFixtures.mock.invocationCallOrder[0]` and both called exactly once), and the action returns `{ ok:true, count }`. A mint-but-no-finalize implementation MUST fail this test. Also a reset SUCCESS-PATH test: assert RPC returns `{clearedShows: N}` → action returns `{ ok:true, count:N }` and `revalidatePath` called for `/admin` + `/admin/settings`.
...
- [ ] **Step 3: Implement** (`"use server"`):
  ...
  - `reseedValidationFixturesAction`: `requireAdmin` → gate → session client `.rpc("assert_destructive_reset_enabled")` (gate-disabled → `VALIDATION_RESET_NOT_ENABLED`); on success build the service-role client, compute `validationTodayIso` ONCE (reuse the CLI's helper — `scripts/validation-reseed.ts:193-195`; do not hardcode), `const fixtures = buildFixtures(validationTodayIso)`, `await mintFixtureCombos(serviceClient, fixtures)` THEN `await finalizeFixtures(serviceClient, allCombos, validationTodayIso)` (SAME `validationTodayIso` passed to both build and finalize — UTC-midnight-drift safe; the button always seeds the full 16, so finalize IS called here, unlike a single-combo CLI run; any infra fault → `VALIDATION_RESEED_FAILED`); `revalidatePath` + `return { ok:true, count: minted }`. Construct the service-role client ONLY after the assert passes.
