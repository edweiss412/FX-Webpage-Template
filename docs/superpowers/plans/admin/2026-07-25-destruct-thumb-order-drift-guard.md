...
`tests/e2e/pendingDiscardReal.layout.spec.ts` covers D1, D2, D3, D4 and D7 across the rails `rail320`, `page358`, `band440`, `wide900` and `bigtext440`, idle and armed, alongside `pendingDiscardReflow.layout.spec.ts`'s historical set — the negative control, two fixed-panel positive tests, and a source guard keeping the transcribed markup historical.
