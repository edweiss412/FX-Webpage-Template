...
- Modify: `tests/db/viewer_version_token.test.ts:69-71` — format regexes become three-part (`/^\d+:1:(?:true|false)$/` etc.; the seeded shows there are published, so `:true`); `tests/db/viewer-version-token-grants.test.ts:102` — missing-show token becomes `/^[0-9]+:0:false$/` (the `coalesce(..., 'false')` arm)
...
- [ ] **Step 4: Run tests to verify pass** — `pnpm vitest run tests/db/unpublish_show_rpc.test.ts tests/db/viewer_version_token.test.ts tests/db/viewer-version-token-grants.test.ts` → PASS (the two companion suites updated for the three-part token shape).
...
- Test: `pnpm test:audit:x1-catalog-parity`, `pnpm test:audit:x2-no-raw-codes`, `pnpm vitest run tests/help/errors-grouping.test.tsx` (if CREW-prefixed codes land in "Other", add `"CREW"` to the **`publishing-shows`** family's `prefixes` (`app/help/errors/_families.ts:90-93` — "Putting shows live, unpublishing, and undo"), NOT sign-in; note this also re-homes `CREW_LINK_UNAVAILABLE` if it currently falls elsewhere — check the grouping test's snapshot and prefer the narrower prefix ordering if that causes a diff)
...
- Migrate surviving test consumers of the deleted legs (sweep: `rg -n "readUnpublishTokenForSlug|unpublishShow_unlocked|\\bunpublishShow\\(" tests app components lib` must end with zero hits outside `unpublishShowViaEmailedLink`/`lib/showLifecycle`): `tests/sync/unpublishShow.test.ts:93-216` — rewrite the `unpublishShow_unlocked` describe blocks against `unpublishShowViaEmailedLink_unlocked` (same fakes + a valid recipient binding via `mintIdFor`; the null-token expectation changes from CONSUMED to neutral `not_found` per the emailed-path contract `lib/sync/unpublishShow.ts:293-307`); `tests/notify/deliver-auto-publish-undo-real-db.test.ts:77-79` and `tests/notify/auto-publish-undo-live-probe-real-db.test.ts:241-244` — replace `unpublishShow({slug, token})` consume calls with `unpublishShowViaEmailedLink({slug, token, r})`, deriving `r` from the seeded admin email + stored token exactly as the notify templates do (these tests already have the recipient context in scope)
- Modify: `tests/sync/_metaInfraContract.test.ts:171` — remove the `helper: "unpublishShow"` registry row (the helper is deleted); keep the `unpublishShowViaEmailedLink` row
- Modify: `tests/db/showCacheRevalidateCoverage.test.ts` — remove the deleted `_actions/undoAutoPublish.ts` entry from `REVALIDATE_REGISTRY` (`:144-148`) AND from `lib/sync/unpublishShow.ts`'s `coveredByCallers` list (`:172-178`); the surviving callers (route + confirm action) keep that entry valid
- Modify: `tests/sync/_advisoryLockSingleHolderContract.test.ts` — remove the plain-`unpublishShow` holder row from `lockHolderRegistry` (`:57-63`) AND the coupled `expect.objectContaining({ holder: "unpublishShow", … })` assertion (`:364-368`) — the guard hard-codes both and fails if only one is removed; keep `unpublishShowViaEmailedLink` in both places, `tests/components/admin/transitionAudit.test.tsx` (in the `SERVER_RENDERED`/scanned file registry `:32-40`: REMOVE the `components/admin/UndoAutoPublishButton.tsx` row — the file is deleted and the registry `readFileSync`s every entry — and ADD `components/admin/PublishedToggle.tsx`), `tests/e2e/admin-lifecycle-transitions.spec.ts:55` surface registry (keep `PublishShowButton.tsx`, add `PublishedToggle.tsx`)
