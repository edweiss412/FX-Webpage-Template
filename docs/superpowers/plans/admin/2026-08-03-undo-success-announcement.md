...
<!-- spec-lint: ignore — new file created by this plan; not tracked until implementation -->
- [ ] Create `components/admin/AdminAnnounceProvider.tsx` (`"use client"`): holds `useAnnounceLog`, provides `UndoAnnounceContext`, renders `<AnnounceLogRegion … label="Undo updates" />` with `testId="admin-undo-status"` / `label="Undo updates"` at the layout and `testId="dialog-undo-status"` / `label="Undo updates in this dialog"` in the dialog (both identifiers distinct, so neither the e2e locator nor a screen reader navigating by region is ambiguous while both are attached) as its **always-first** child, then `{children}`.
- [ ] Mount a **second** `AdminAnnounceProvider` inside `ReviewModalShell`, wrapping the entire panel interior exactly as `PopoverHostContext` already does (`components/admin/review/ReviewModalShell.tsx:690-695`) rather than inventing a placement, with its region as the first child of that wrapper (spec §3.5.2).
- [ ] Derive its `testId` and `label` from `testIdBase` (`ReviewModalShell.tsx:583`), never hard-coded: the shell has THREE render sites (`PublishedReviewModal.tsx:687`, `ShowReviewModalSkeleton.tsx:44`, `Step3ReviewModal.tsx:373`), so a fixed id would put one identifier on three regions. Nested context resolves to the nearest provider, so no flag or prop decides which channel a button uses.
- [ ] Edit `app/admin/layout.tsx` to wrap **each of its three returns individually**, with `AdminAnnounceProvider` as the outermost element of each so it always precedes that branch's `data-inert-root` root (`app/admin/layout.tsx:90`, `layout.tsx:155`, `layout.tsx:177`), and **outside** `PageTransition` (`layout.tsx:171`).
- [ ] **Run the cold-load spike BEFORE any later task depends on this**, in the two parts spec §3.5.2 defines. **Part 1:** deep-link the modal open (`/admin?show=`), let the portal flip finish (`components/admin/review/ReviewModalShell.tsx:723`, which recreates host nodes per `tests/lib/a11y/dialogFocusReattach.test.tsx:6-13`), undo, assert the announcement lands. **Part 2:** attempt an undo initiated or resolved ACROSS the flip. **Part 3:** drive an undo across the Suspense fallback-to-real swap (`ShowReviewModalSkeleton` is the fallback at `app/admin/page.tsx:168`), which is a SECOND post-hydration replacement of the dialog region and is not covered by Part 2's hydration argument. Part 1 alone proves only the settled tree, which nobody disputes.
- [ ] Record Part 2's outcome explicitly, including "could not be constructed" — `useHasMounted` returns true on the first client render (`lib/a11y/useHasMounted.ts:22-26`), so the flip is part of hydration and a form action cannot precede it. An unconstructible Part 2 IS the evidence; passing Part 1 and declaring the vector closed is not.
- [ ] **If either part fails, stop and redesign the dialog channel** rather than continuing to Task 7.
...
- [ ] **A2** — every file rendering `<AdminAnnounceProvider` is `app/admin/layout.tsx` or a modal shell (a file also rendering `role="dialog"`); no file outside the provider module references `UndoAnnounceContext.Provider`. Planted violations: a channel on an ordinary surface; and a file rendering `<UndoAnnounceContext.Provider>`.
...
- [ ] **A4** — every file rendering `<UndoChangeButton` directly lives under `app/admin/` or `components/admin/`. Do NOT add a hand-listed wrapper set: the transitive case is explicitly unguarded per spec §5, and the list an earlier draft proposed was already wrong (it named a type, not a component) before it shipped. Planted violation: such a file outside those trees. This is the escaping mutant the earlier guard admitted, where a non-admin call site silently consumes the no-op context.
