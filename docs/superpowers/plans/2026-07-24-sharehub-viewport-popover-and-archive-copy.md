...
**Implementation.** Per spec §2.1.1-§2.1.2: `mounted` gate flipped in an effect (`HoverHelp.tsx:146-154` pattern and rationale — `useHasMounted` is wrong here because a provider's ref is still null on the first client commit); `createPortal` to `hostRef.current ?? document.body`; measure and apply per §2.1.2, mirroring `components/admin/HoverHelp.tsx:228-308`: the body-host bounds degeneration (`host === document.body` -> viewport rect, `components/admin/HoverHelp.tsx:234-239`) and the `toHostOffsets` conversion (`components/admin/HoverHelp.tsx:283-290`), which is applied to the body at `components/admin/HoverHelp.tsx:291-293` and to the caret at `components/admin/HoverHelp.tsx:306-308` so the two coordinate paths cannot drift. Remove the bespoke `caretRightPx` layout effect (`ShareHub.tsx:180-211`) and the `absolute right-0 top-full` / `max-w-[calc(100vw-2rem)]` classes (`ShareHub.tsx:487`). Set `data-popover-side`. Handle `kind:"hidden"` per §2.1.2.
...
Also file `BL-SHAREHUB-BACKDROP-COVERS-TRIGGERS` (surfaced by Task 4's `elementFromPoint` probe, 2026-07-24): with the hub open, the `fixed inset-0 z-20` backdrop swallows a tap on either trigger. The root's open-gated `z-30` elevates the whole root, backdrop included, and does not order that fixed child against its non-positioned trigger siblings — so `shareHub.test.tsx`'s class-level pin never meant what its title claimed. **Pre-existing:** the same probe fails identically against `origin/main`'s `ShareHub.tsx`, so this branch neither introduced nor worsened it. Near-invisible in use, because the backdrop's own handler closes the popover — a trigger tap still dismisses, just via the outside-click path and without focus restore. Fix shape: give the trigger group its own open-gated stacking level above the backdrop, or move the backdrop into the portal beneath the body, then re-run the Task 4 probe with the trigger assertions restored.
