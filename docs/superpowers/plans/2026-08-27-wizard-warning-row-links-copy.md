...
- **Invariant 8, UI gate.** IN SCOPE: `components/admin/PerShowActionableWarnings.tsx` changes its guidance composition (Task 5) and its correction-sentence gate, as does `NoteWarningCard.tsx` (Task 3). Task 7 runs the critique + audit pair and writes the marker line.
...
- CREATES new file synthesizeBlocksEquivalence.test.ts under `tests/drive/`: the coordinate path and the markdown path agree over the whole xlsx corpus (Task 2).
...
- [ ] **Step 4: Run green, plus the detector suites that pin `scanRowsWithOpener`**

Run: `pnpm vitest run tests/parser/rowScanCore.test.ts tests/parser/fieldNearMiss.test.ts tests/parser/fieldNearMissBaseline.test.ts`
Expected: PASS, all three. The baseline suite is the proof the shell's output did not move (65 rows, unchanged).
...
- [ ] **Step 4: Run green, plus the byte pin**

Run: `pnpm vitest run tests/drive/synthesizeBlocks.test.ts tests/drive/round-trip-fixture.test.ts tests/drive/workbookSynthesisError.test.ts tests/drive/exportSheetToMarkdown.test.ts tests/drive/exportSheetArchivedPullSheet.test.ts tests/drive/sourceAnchors.test.ts`
...
- Modify: `components/admin/NoteWarningCard.tsx:39` (`notePopoverParts` sentence gate) and `components/admin/PerShowActionableWarnings.tsx:163-166` (`followUp` gate): both become `sourceCell?.a1` non-blank (spec §2.4, T8); tests `tests/components/admin/NoteWarningCard*.test.tsx` (`notePopoverParts` table) and `tests/components/admin/perShowActionableFollowUp.test.tsx`
...
```ts
  it("the dispatching show's workbook (ria.xlsx): the three near-miss rows resolve to their own cells and the hotel row to the hotels region", async () => {
    ...
    expect(hotel!.sourceCell).toEqual(extractSourceAnchors(buffer, gids).hotels);
  });
```

(`buildSheetDeepLink` imported from `@/lib/sheet-links/buildSheetDeepLink`. The hotel assertion goes red in THIS task and green in Task 4; write it here, mark it `it.fails`-free and expected red until Task 4, and note that in both commits. If the RIA sheet's `Backdrop` occurs twice on `INFO` (the fixture also has `Backdrop (LED, Spandex)`, which does not equal `Backdrop` after trim, so the premise holds), the premise says so instead of the assertion drifting.)
...
```ts
  it("every DETAILS-family spelling anchors the Stage row: one kind function on both sides (spec 2026-08-27 §2.2)", () => {
    for (const header of ["DETAILS", "DETAILS/Room Diagram", "GS DETAILS (FOR BOTH)"]) {
      ...
      expect(cell, header).toEqual({ title: "INFO", gid: 0, a1: "A2" });
    }
  });
```
...
```ts
  it("rows after an internal blank row anchor under their OWN block's kind (the exporter splits there, and so does the detector)", async () => {
    ...
    expect(w?.sourceCell).toEqual({ title: "INFO", gid: 0, a1: "A4" });
  });
```

- [ ] **Step 2: Run it, expect red**

Run: `pnpm vitest run tests/drive/unknownFieldAnchors.test.ts tests/drive/synthesizeBlocksEquivalence.test.ts tests/parser/fieldNearMissBaseline.test.ts`
Expected: FAIL: the equivalence suite cannot import `blockRowsForScan`; the corpus case and the four new cases (`sourceCell` null where a cell is expected; the tab-level case gets `null` instead of `{title, gid}`). The retired case's replacement is also red (kind `stage sze` matches no `BLOCKS` family).
...
T8, the correction sentence follows the cell (spec §2.4). `components/admin/NoteWarningCard.tsx` `notePopoverParts`: `const sentence = w.sourceCell ? correctionLoopCopy("resync") : null;` becomes `const sentence = typeof w.sourceCell?.a1 === "string" && w.sourceCell.a1.trim().length > 0 ? correctionLoopCopy("resync") : null;`. `components/admin/PerShowActionableWarnings.tsx` `followUp` (line 163): `w.sourceCell && typeof followUpCopy === "string" ...` becomes `typeof w.sourceCell?.a1 === "string" && w.sourceCell.a1.trim().length > 0 && typeof followUpCopy === "string" ...`. Tests, in the `notePopoverParts` table suite under `tests/components/admin/` (find it with `rg -l notePopoverParts tests`) and in `tests/components/admin/perShowActionableFollowUp.test.tsx`: `{ title: "INFO", gid: 0, scope: "tab" }` → no sentence / no followUp; `{ title: "INFO", gid: 0, a1: "A2", scope: "cell" }` and `{ title: "INFO", gid: 0, a1: "A2:D5" }` → sentence / followUp as today. Red: both gates test `sourceCell` truthiness only.
...
- [ ] **Step 4: Run green, plus every suite that resolves through the scanner**

Run: `pnpm vitest run tests/drive/unknownFieldAnchors.test.ts tests/drive/unknownFieldAnchors.live.test.ts tests/components/admin/perShowActionableFollowUp.test.tsx tests/components/admin/warningCardFollowUp.test.tsx tests/sheet-links/buildSheetDeepLink.test.ts tests/sheet-links/allowlistMeta.test.ts tests/components/admin/review/sectionFreshness.test.ts tests/drive/showDayTimeAnchors.test.ts tests/parser/fieldNearMissBaseline.test.ts tests/parser/fieldNearMiss.test.ts tests/drive/synthesizeBlocksEquivalence.test.ts`
Expected: PASS except the ria hotel assertion, which stays red until Task 4 (say so in the commit). `fieldNearMissBaseline` AC-N9 (line 448) is AC-3: the east-coast `Stage`/`Storage` rows still resolve. Add `tests/drive/unknownFieldAnchors.live.test.ts` to the same run: it is not env-gated (line 52, a committed-workbook DETAILS fidelity case) and must stay green.
...
Expected: FAIL: `HOTEL_REGION_ANCHORED` not exported; the negative membership case is green on arrival (it is a pin, not a red) and stays green.
...
Run: `pnpm vitest run tests/drive/showDayTimeAnchors.test.ts tests/parser/operatorActionableWarnings.test.ts tests/parser/parseWarningDeepLinkRender.test.tsx tests/parser/waveCodesNoSourceCell.test.ts tests/parser/dataGaps.test.ts tests/parser/dataGapsClassCompleteness.test.ts tests/drive/unknownFieldAnchors.test.ts`
Expected: PASS, including Task 3's ria hotel assertion.
...
<!-- task: red=`pnpm vitest run tests/messages/_metaWarningCardCopy.test.ts tests/admin/perShowActionableRenderControls.test.tsx tests/components/step3SheetCard.test.tsx` red-state=authored red-target=`lib/messages/catalog.ts:1352` why=`UNKNOWN_FIELD.helpfulContext still ends with "Report flags it to us; Ignore hides this notice.", so the whole-catalog no-control-names assertion fails, the wizard row still renders "Report", and the card has no controlsNote to render` ac=AC-6 -->
...
- Modify: `components/admin/PerShowActionableWarnings.tsx` lines 54-67, 86-115, 147-180 and 355, `components/admin/showpage/sectionWarningExtras.tsx` (the ACTIVE-list `PerShowActionableWarnings` mount only)
- Modify: `tests/admin/perShowActionableRenderControls.test.tsx`, `tests/admin/perShowActionableTransitions.test.tsx`, `tests/components/step3SheetCard.test.tsx`, `tests/messages/_metaCatalogCopyHygiene.test.ts:189-210` (`FIELD_POLICY` row), `tests/admin/showpage/crewUnderRowCards.test.tsx` and `tests/components/admin/showpage/publishedReviewModal.test.tsx` (mount binding, T6(e))
...
What is red and why: `lib/messages/catalog.ts:1352` still carries the control sentence in `helpfulContext`, and no `controlsNote` exists. `DB-free`.
...
```ts
  it("frozen copy fixture: controlsNote matches spec 2026-08-27 §4.2 byte-for-byte, total", () => {
    expect(Object.keys(EXPECTED_CONTROLS_NOTE).sort()).toEqual(
      Object.entries(CATALOG).filter(([, e]) => typeof e?.controlsNote === "string").map(([c]) => c).sort(),
    );
  ...
  it("no helpfulContext of a card code names a card control; the sentence lives in controlsNote (spec 2026-08-27 §4)", () => {
    ...
    const withNote = Object.entries(CATALOG).filter(([, e]) => typeof e?.controlsNote === "string" && e.controlsNote.trim());
    premiseHolds("at least the three moved rows carry a note", withNote.length >= 3);
    for (const [code, e] of withNote) {
      expect(WARNING_CARD_COPY_CODES.has(code), `${code} carries controlsNote but is not a card code`).toBe(true);
      expect(/\b(Report|Ignore)\b/.test(e.controlsNote!), `${code}.controlsNote names a control`).toBe(true);
    }
  });
```
...
describe("controlsNote renders only beside the controls (spec 2026-08-27 §4.3)", () => {
  ...
  test("active list (showControlsNote + item controls): the guidance line ends with the note", () => {
  ...
  test("without item controls, the note is absent", () => {
...
`tests/admin/perShowActionableTransitions.test.tsx` (spec §11): its `vi.mock` of `@/lib/messages/lookup` (line 21) builds synthetic entries by spreading the real `UNKNOWN_FIELD` entry, so FIRST set `controlsNote: null` explicitly on `SYN_A`..`SYN_D` (they must keep today's render). Then extend `VARIANTS` (line 20-25) with `E: { code: "SYN_E", guidance: true, trigger: false, note: true }` (G2: rendered with `showControlsNote` and a `renderItemControls` node), `F: { code: "SYN_F", guidance: false, trigger: false, note: true }` (G3: `helpfulContext` null, `controlsNote` set), `G: { code: "SYN_G", guidance: true, trigger: false, instance: true, note: true }` (G4: the warning carries an `autocorrect` payload that makes `resolveGuidance` return `instance`, and the note must be suppressed). Extend `expectVariant` (line 27) to assert the guidance text ends with / lacks the note per variant, reading the note from a local literal in the mock (not the catalog), and to assert `container.querySelector('[data-testid="per-show-actionable-guidance"]')?.closest("[data-motion], [style*=\"transition\"]")` is null. The existing every-ordered-pair loop (line 64-79, `VARIANTS[x]` → `VARIANTS[y]`) then covers all ten G pairs in both directions with no new loop; run the same matrix once more with `condensed` for the C axis. No `waitFor` anywhere: every assertion is synchronous after `rerender`.
...
Run: `pnpm vitest run tests/messages/_metaWarningCardCopy.test.ts tests/admin/perShowActionableRenderControls.test.tsx tests/components/step3SheetCard.test.tsx`
Expected: FAIL: the whole-catalog assertion lists `PULL_SHEET_PARSE_PARTIAL`, `UNKNOWN_FIELD`, `UNKNOWN_SECTION_HEADER`; the wizard row test matches `Report`; the card tests fail on `NOTE` being undefined.
...
Move `const controls = renderItemControls ? renderItemControls(w, i) : null;` from line 355 up to just after `const entry = ...` (line 136), then replace line 147:

```ts
        const controlsNote =
          showControlsNote === true && controls !== null && typeof entry?.controlsNote === "string" && entry.controlsNote.trim().length > 0
...
What is red and why: `rg -q "^## BL-NEARMISS-CANDIDACY-NON-FIELD-BLOCKS" BACKLOG.md` exits 1 (spec T7). After insertion the same command exits 0 and the three ledger walkers hold the field shape (`Effort` by sizing, `Filed`/`Facing` by mint bar, flight fields by in-progress). `DB-free`.
...
- `rg -n "firstNonBlank\b|nextNonBlankAfter|DETAILS_NON_TERMINATOR_FIELDS" lib tests --glob '*.ts'` → only `lib/drive/unknownFieldAnchors.ts` (deleted by Task 3) and `lib/drive/exportSheetToMarkdown.ts` lines 210, 216 and 223 (a local `firstNonBlankCol`, untouched). No other importer.
- `rg -n "extractUnknownFieldAnchors" lib tests --glob '*.ts'` → `lib/sync/attachWarningAnchors.ts` lines 7 and 50, `tests/drive/unknownFieldAnchors.test.ts`, `tests/drive/unknownFieldAnchors.live.test.ts`, `tests/parser/fieldNearMissBaseline.test.ts` lines 436 and 445. All keep the signature Task 3 preserves.
- `rg -n "\\b(Report|Ignore)\\b" lib/messages/catalog.ts` over `helpfulContext` values → line 1352 `UNKNOWN_FIELD`, line 1734 `PULL_SHEET_PARSE_PARTIAL`, line 2104 `UNKNOWN_SECTION_HEADER` (the three Task 5 rows) plus line 2838 `TILE_SERVER_RENDER_FAILED` and line 2857 `TILE_PROJECTION_FETCH_FAILED`. The last two are alert-surface codes outside `WARNING_CARD_COPY_CODES`, rendered where a Report control exists; not this arc's copy. Task 5's assertion is therefore scoped to registry members, and this bullet is the record of why.
- `rg -n "synthesizeMarkdownFromXlsx\(" lib app scripts tests --glob '*.ts' --glob '*.tsx' --glob '*.mjs'` → every caller keeps the same signature and return shape; none needs an edit.
