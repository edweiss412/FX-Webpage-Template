...
**Status:** APPROVED 2026-08-27 by bl-orch ruling at the four-round plan cap (26 findings over four rounds, all repaired; `docs/review-rounds/fix/wizard-warning-row-links-copy/66c9857f56a5.md` plan section), on the condition that the three verification passes in §7 be run before handoff (done, results below).
...
- **The three passes the filing names, run 2026-08-27 19:50 CDT on the pushed head, as the ruling's condition.** (1) Every fixture the plan constructs, fed to the live exporter + detector (a scratch tsx script over `synthesizeMarkdownFromXlsx` + `parseSheet`): 15 checked, 0 moved (the T3 INFO/GEAR, FORM, B2-origin, duplicate and split workbooks, the blank-row and three DETAILS-family `Stage` rows, the PULL SHEET title row, the `regionA` OLD tab as archived and as included, the `renderRow` first-line expectation, and `ria.xlsx`; each emits exactly the kinds and labels the plan asserts). (2) Every suite the diff touches walked for exact-object anchor assertions (`toEqual({ title:`, `sourceCell).toEqual(`, `?.a1).toBe(`): 36 checked across 20 suites, 6 moved, all six already in Task 3 step 1 (the four `toBeNull` flips in `unknownFieldAnchors.test.ts`, `attachWarningAnchors.test.ts:134`, `unknownFieldAnchors.live.test.ts:71`); the 20 in `showDayTimeAnchors.test.ts` and 3 in `dataGaps.test.ts` build their anchors by hand and get them back by identity or through `stripLegacyUnknownFieldAnchors`, untouched. (3) `rg -c 'sourcePath: "<path>"' tests/mutation/source/registry.ts` for each of the eleven `Modify:` paths: 2 enrolled (`lib/parser/blocks/_rowScan.ts` → `rowScanOpener`, re-derived by Task 1; `lib/parser/fieldNearMiss.ts` → `fieldNearMiss`, export-only edit, row untouched), 9 not enrolled.
