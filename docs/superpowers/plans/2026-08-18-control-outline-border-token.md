...
- **No ShareHub swap.** Spec §3.5. `tests/styles/_metaControlOutlineFill.test.ts:156-164` stays byte-identical and passing.
...
| --- | --- | --- |
| `tests/styles/_metaControlOutlineFill.test.ts` | **EDITED** | census rows carry `border-text-faint`; **new:** no row carries `border-border` on any path; **new:** five dividers are non-members |
| `tests/styles/controlOutlineScan.ts` | **EDITED** | `CENSUS` 21 → 58 rows |
| `tests/styles/secondary-action-contrast.test.ts` | **EDITED** | **new:** `--color-border` as OUTLINE, four grounds, both themes |
...
| AC-4 | §3.5 | Task 2 step 2.7: `_metaControlOutlineFill.test.ts:156-164` byte-identical, passing |
...
| Original 21 failing a hypothetical `everyPathCarries` | **2** — the second (`Mi11GateActions.tsx:69`) is a ratified exemption, which is why that form was rejected |
...
- [ ] **1.2** Add the per-row assertion, mirroring the existing `border-border-strong` case at `tests/styles/_metaControlOutlineFill.test.ts:121-123`:
      ```ts
      ...
      ```
      Use the existing `carries` helper (`:46-49`) — it reads `allStrings`, which spans every render alternative, so this existential-negation IS the universal claim. **Do NOT introduce `everyPathCarries` here** (see "What this plan does NOT build").
...
- [ ] **1.5** Three fixtures, each with its own `premise("fixture parsed and produced an element", cover.length, 0)` so a fixture that fails to parse cannot pass vacuously:
      ...
      - **(b)** two ternary arms, one `border-text-faint`, one `border-border` → PASSES the pre-existing `carries(…, "border-text-faint")` check and FAILS the new one. **This is the `:178` shape and the executable proof the strengthening is not cosmetic.**
...
- [ ] **2.3** Swap class B's 8 elements. Four of them (`NeedsAttentionInbox.tsx:101/:130/:198/:224`) share ONE occurrence at `components/admin/NeedsAttentionInbox.tsx:31` — one edit moves four census rows.
...
- [ ] **2.7** Verify the untouched surfaces explicitly:
      ...
      ```
      The last is NOT empty — `AutoRefreshControl.tsx` is edited at `:119` — so inspect it and confirm the diff does **not** reach `:106`, the switch-track path.
...
- [ ] **3.1** Add the assertion FIRST: `--color-border` as an OUTLINE against all four neutral grounds, both themes, pinned to the measured figures (`bg` 1.22/1.35, `surface` 1.27/1.27, `surface-raised` 1.27/1.19, `surface-sunken` 1.15/1.38) with `toBeCloseTo(…, 2)`, in the same shape as the existing tinted-plate case at `tests/styles/secondary-action-contrast.test.ts:75-89`. This records the before-state so a future retune of `--color-border` cannot quietly reintroduce the weight this arc removed.
...
- [ ] **4.1** Replace `DESIGN.md:227-233` per spec §4.1: record the 2026-08-18 ruling and that it was taken against a rendered mockup including the crew surfaces; state that `border-border` on a control's resting outline is now the text ramp; state the divider carve-out **in both directions**; point at the ShareHub filing rather than restating its numbers.
- [ ] **4.2** Cite no line numbers for the swept population — the census is the contract, and the `:344`/`:266` anchors in the current text are exactly what drifted (spec §4.1).
...
- [ ] **5.1** `pnpm heavy pnpm mutation:guards` — `tests/styles/controlOutlineScan.ts` is enrolled at `scoreFloor: 1` with `accepted: []` (`tests/mutation/source/registry.ts:1909-1924`), and this arc edits it.
...
- [ ] **6.1** Setup gates first: `context.mjs` context load (PRODUCT.md + DESIGN.md) → register reference read (`brand.md`).
...
- [ ] **7.1** File `BL-CONTROL-OUTLINE-SHAREHUB-MOBILE-SKIN-WEIGHT` (spec §3.5). It MUST carry:
      ...
      - `**Class-sweep exception:** (b)` — a ratified scope decision fences it, naming BOTH ratifications: `spec 2026-07-24-strip-mobile-stacked-band §3 R3` and the executable pin `tests/styles/_metaControlOutlineFill.test.ts:156-164`.
...
Spec §7 enumerates every dimension relationship that could have moved and shows none does: the complete diff substitutes one colour custom property for another inside the SAME Tailwind `border` utility at every site. Border-width stays `1px` (the bare `border` utility is untouched), no padding, radius or font utility changes, no swapped control is a fixed-dimension parent, and the five dividers — the only elements whose rule position could shift — are excluded from the swap entirely. A colour class contributes nothing to layout, so the Tailwind v4 `align-items: stretch` trap is not reachable.

## Transition-audit task — EXEMPTION CLAIMED

Spec §8 enumerates all five state pairs. Every swapped site's transition is governed by a `transition-colors duration-fast` utility present before and after and not edited; the swap changes the resting colour that transition interpolates TO, not whether or how it interpolates. There is no `AnimatePresence`, no new conditional render, and no new state. The one compound case — `components/layout/ThemeToggle.tsx:91` changing outline colour while the theme itself changes — has both endpoints pinned by §1.2's four ground rows in both themes.
