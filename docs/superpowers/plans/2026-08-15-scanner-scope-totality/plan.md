...
3. `pnpm heavy pnpm mutation:guards`: record BOTH surfaces' scores; disposition every survivor (suite extension, `equivalent`, or `accepted-gap` with ledger ref); floors at-or-below achieved scores; STATE both scores + unaccepted-survivor sets in the round-1 diff brief (AGENTS.md convergence bullet 4).
...
2. Gates: `pnpm heavy pnpm test:fast`, `pnpm typecheck`, `pnpm exec eslint .`, `pnpm format:check`.
