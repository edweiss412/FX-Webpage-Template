...
| compound: pathname refetch fires while the seed promise is still pending | §3.2 interleaving table; refetch commits, late seed dropped/demoted |
| compound: layout re-render mid-stream (router.refresh) | older subscription invalidated at arrival; only newest promise reaches ingestion |
| compound: P1 pending, P2 arrives, P1 resolves first | P1 ignored; pending persists until P2 resolves or a refetch lands |
| compound: P2 arrives and HANGS after P1 invalidated | pending persists; next pathname refetch repopulates |
| compound: refresh promise pending, bell zeroed, restoring fetch commits 0, refresh resolves pre-open count | demoted to fresh fetch; stale count never painted |
