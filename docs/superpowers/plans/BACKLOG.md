...
1. **Room name / dimensions / floor split** — `lib/parser/blocks/rooms.ts`. **MULTI-PATH** (precise per-show grounding, 2026-06-23). Each room's physical name, dims, floor should be SEPARATE fields (`kind` already records gs/breakout). **Naming RESOLVED (owner, 2026-06-23):** `name` = the **venue room only** — strip the `GENERAL SESSION` / `BREAKOUT N` label + dims + floor; fall back to `General Session` for a GS with no venue. The source cell is `LABEL\nNAME[\nDIMS][\nFLOOR]` but `synthesizeMarkdownFromXlsx` flattens the newlines to SPACES, so the split is PATTERN-based: dims `/\d+\s*'\s*x/` with an optional intro prefix (`TOTAL:` / `A/B:` / `APPROXIMATELY`); floor `/\b\d+(?:st|nd|rd|th)\s+Floor\b/i`; drop literal `Dimensions`/`Floor` placeholder words. (Casing: source is ALL-CAPS; title-casing is a minor open sub-decision.)
   ...
   | show | current GS `name` | should be: name / dims / floor |
   |---|---|---|
   | redefining-fi | `General Session` (**venue/dims/floor LOST**) | `LAKEVIEW BALLROOM` / `61' x 55' x 11'` / `7th Floor` |
   | ria | `General Session` (**LOST**) | `SALON ABCD` / `41' x 73' x 13'` / — |
   | east-coast | `General Session` (**LOST**) | `MABEL 1` / `60' x 45'` / — |
   | consultants | `- GRAND BALLROOM A/B` (no dims/floor) | `GRAND BALLROOM A/B` / `A/B: 82' x 63' x 14'` / `8th Floor` |
   | fintech | `GENERAL SESSION ADLER BALLROOM 75' x 37' x 15th Floor` (fused) | `ADLER BALLROOM` / `75' x 37' x` / `15th Floor` |
   | fixed-income | `GENERAL SESSION SALON ABC 43' x 49' x 12'` (fused) | `SALON ABC` / `43' x 49' x 12'` / — |
   | rpas | `GENERAL SESSION GRAND BALLROOM A/B TOTAL: … A/B: … 8th Floor` (fused, 1st dims segment only) | `GRAND BALLROOM A/B` / both segments / `8th Floor` |

   Breakouts follow the same pattern (`BREAKOUT 1 SALON D 43' x 24' x 12'` → `SALON D` / dims; `BREAKOUT 1 DELAWARE 7th Floor` → `DELAWARE` / floor, no dims). **Two sub-fixes:** **1a — v4 fused path** (fintech, fixed-income, rpas): `parseV4RoomBlock` (`:~257`) already extracts dims (`:266`) + floor (`:270`) but sets `name` to the full header via `buildEmptyRoom(kind, headerText)` (`:263`); clean the name + widen dims extraction (prefixed / two-segment / incomplete). `isPlaceholderRoomName` (`:229`) still works on cleaned names — keep the placeholder-stub suppression (`:215`). **1b — the data-losing "General Session" path** (redefining, ria, east-coast; + consultants' `- ` artifact): these do NOT go through `parseV4RoomBlock`; find the v2/other path that names GS `General Session` and drops venue/dims/floor (start at `parseGsRoom` `:~319`, the `mabelRooms`/merge logic `:~490`) and extract venue/dims/floor there too.
...
**Why backlog:** intricate MULTI-PATH parser surgery on a 659-line file with heavy 859-test blast radius; deferred from the session that did the grounding (it deserves a fresh focused pass, which the precise per-show current→desired table above makes efficient — naming decision already resolved). Not breaking anything — the residuals are MEDIUM display-fidelity gaps, and the round-trip + parser suites are green. Suggested order: fix #1a (contained v4 PR) → #1b (the "General Session" path) → #2 (meal-room suppression) → #3 (hotels).
