...
### BL-WIZARD-RESTAGE-FETCH-BEFORE-LOCK — Drive-under-lock class: wizard restage FIXED; retrySingleFile is the remaining instance

**Filed:** 2026-06-22 from the PR-#73 follow-up investigation. **Wizard-restage instance CLOSED** in the lock-hygiene follow-up PR (the wizard revision-race restage now prepares pre-lock + stages under the lock via `prepareOnboardingFiles` + `scanOnboardingPreparedFiles`; the combined-fetch dedup landed; the advisory-lock guard was extended to follow cross-file scan calls — `runOnboardingScan` / `prepareOnboardingFiles` are now Drive-reaching markers). This entry tracks the REMAINING instance found by that guard extension.

**The remaining instance:** `retrySingleFile_unlocked` (`lib/sync/retrySingleFile.ts`) runs `(deps.runOnboardingScan ?? runOnboardingScan)(...)` — a full Drive xlsx export — AND `fetchDriveFileMetadata` while the per-show pipeline lock is held (`retrySingleFile` wraps it in `withPostgresSyncPipelineLock`; the body asserts `assertShowLockHeld`). Same lock-hygiene class as the (now-fixed) wizard restage. The extended guard flags it; it is currently exempted via `knownDriveUnderLockPaths` in `tests/sync/_advisoryLockSingleHolderContract.test.ts` with a "remove when fixed" comment.

**The fix:** mirror the wizard-restage reorder — Lock#1 preflight read (settings + pending) → release → pre-lock Drive metadata + `prepareOnboardingFiles` → Lock#2 stage + supersession-guarded delete. **Remove the `knownDriveUnderLockPaths` exemption in the same PR** so the guard then enforces the path.

**Why backlog, not deferred (it's a major restructure, not light surgery):** `retrySingleFile_unlocked` is a **contract-pinned entry point**: it is named + has documented contracts in TWO structural meta-tests — `_advisoryLockSingleHolderContract.test.ts` (`"retrySingleFile_unlocked never locks"`) and `_metaInfraContract.test.ts` (registry row). It is unit-tested directly across `retrySingleFile.test.ts` (5 call sites, 12 `runOnboardingScan` refs), `wizardSessionCasRaceDb.test.ts`, and `pendingIngestionsWizardActions.test.ts`. Its semantics are **intricate two-transaction supersession**: the scan stages in its OWN committed connection + own show-lock (no tx passed to `runOnboardingScan`), while the delete runs in the pipeline-locked tx with a currency-`EXISTS` guard that 0-rows → throws `WizardSessionSupersededRollbackError` (the `wizardSessionCasRaceDb` "R32-1" race pins exactly this: the scan's W1-scoped rows survive a concurrent supersession). Reordering it requires rewriting both structural meta-test registries + 4 test files while preserving that supersession behavior exactly — a focused, carefully-reviewed PR, not a rider on a perf change.

**Promotion prerequisite:** a dedicated PR (spec/plan + adversarial review) that reorders `retrySingleFile` per the wizard-restage pattern, migrates the two structural meta-test registries + the 4 test files, preserves the two-transaction supersession semantics, and removes the `knownDriveUnderLockPaths` exemption. Pairs naturally with BL-ONBOARDING-SCAN-TRANSIENT-THROTTLE-RETRY in a v1.x sync-robustness / lock-hygiene milestone.
