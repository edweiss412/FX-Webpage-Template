...
1. **Room name / dimensions / floor split** — `lib/parser/blocks/rooms.ts`. **MULTI-PATH** (precise per-show grounding, 2026-06-23). Each room's physical name, dims, floor should be SEPARATE fields (`kind` already records gs/breakout). **Naming RESOLVED (owner, 2026-06-23):** `name` = the **venue room only** — strip the `GENERAL SESSION` / `BREAKOUT N` label + dims + floor; fall back to `General Session` for a GS with no venue. The source cell is `LABEL\nNAME[\nDIMS][\nFLOOR]` but `synthesizeMarkdownFromXlsx` flattens the newlines to SPACES, so the split is PATTERN-based: dims `/\d+\s*'\s*x/` with an optional intro prefix (`TOTAL:` / `A/B:` / `APPROXIMATELY`); floor `/\b\d+(?:st|nd|rd|th)\s+Floor\b/i`; drop literal `Dimensions`/`Floor` placeholder words. (Casing: source is ALL-CAPS; title-casing is a minor open sub-decision.)
   ...
   | consultants | `GRAND BALLROOM A/B` / — / — (name ✅; dims/floor → 1b) | **1a ✅ / 1b** |
   | redefining-fi | `General Session` / — / — | **1b** |
   | ria | `General Session` / — / — | **1b** |
   | east-coast | `General Session` / — / — (GS); `MABEL 1` / `60' x 45'` already correct | **1b** |
   ...
   **1b — REMAINING:** the v2 shows whose venue dims/floor live in a **SEPARATE row**, NOT the GS header — `redefining-fi:327` `LAKEVIEW BALLROOM - 7th Floor ROOM DIMENSIONS: 61' x 55' x 11'`, `consultants:253` `BALLROOM A/B - 8th Floor ROOM DIMENSIONS: TOTAL: 82' x 94' x 14' A/B: 82' x 63' x 14'`, `ria:342` `SALON ABCD␊41' x 73' x 12'` (note: **12'** in the fixture, not the 13' the live-sheet read inferred). This needs a NEW parse surface (`<NAME> - <Nth Floor> ROOM DIMENSIONS: <dims>` / `<NAME>␊<dims>` row → associate to the GS room) plus east-coast's MABEL-1/GS reconciliation (the GS-prefixed rows ARE MABEL 1's setup; the mabel breakout already carries `60' x 45'`). `parseGsRoom` (`:~319`) currently names these `General Session` with null dims/floor — pinned at that state by the 1a test so 1b updates them intentionally. Open sub-decision: which name wins for consultants/redefining (GS header `GRAND BALLROOM A/B` vs dims-row `BALLROOM A/B` vs metadata `Grand Ballroom Section A/B` / `Lakeview Junior`).
