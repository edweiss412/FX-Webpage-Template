...
**Filed:** 2026-06-23. The end-to-end grounding audit (`docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/sheet-data-grounding-audit-2026-06-18.md` §"End-to-end exporter+parser validation", "Recommended fixes (ranked)") listed 9 parser-fidelity fixes against the real production renderer (`exporter-xlsx/` fixtures). **Re-assessing the CURRENT parser (2026-06-23) against the 7 fixtures, the CRITICAL/HIGH ones are already DONE:** event_details (exporter DETAILS-column-collapse fix), General Session room present, transportation populated, East-Coast `dates`, no phantom DOCUMENTS crew, agenda label, HTML-entity decode. The round-trip guard now pins all 7 fixtures (`tests/drive/round-trip-fixture.test.ts`, PRs #94/#97; activate the CI job by adding the `GOOGLE_SERVICE_ACCOUNT_JSON` repo secret per `real-drive.yml`).

**Remaining (~3 MEDIUM):**

1. **Room name / dimensions / floor split** — `lib/parser/blocks/rooms.ts`, `parseV4RoomBlock` (≈:257). It already extracts dims (`:266`) + floor (`:270`) but (a) leaves `name` = the FULL flattened header (e.g. `BREAKOUT 1 SALON D 43' x 24' x 12'`), and (b) the dims regex misses prefixed / two-segment / incomplete forms. The source cell is `LABEL\nNAME[\nDIMS][\nFLOOR]` but `synthesizeMarkdownFromXlsx` flattens the newlines to SPACES, so the split must be PATTERN-based. **Variance to handle (all 7 read via gsheets MCP):** dims-only; floor-only (`BREAKOUT 1 DELAWARE 7th Floor`); both; prefixed dims (`A/B: 82' x 63' x 14'`); two-segment (rpas `TOTAL: 82' x 94' x 14' A/B: 82' x 63' x 14'`); incomplete (`75' x 37' x`); placeholder→suppress (`BREAKOUT 1 BREAKOUT ROOM Dimensions Floor`); east-coast messy (dup `MABEL 1`, `MABEL 2 FOOD`, `APPROXIMATELY 60' x 45'`). **DESIGN Q for product:** GS room `name` = the venue room (`SALON ABC`, per audit fix #2c) vs the label `General Session`.
...
**Why backlog:** intricate parser surgery on a 659-line file with heavy 859-test blast radius + a product naming decision; deferred from the session that did the grounding (it deserves a fresh focused pass, which the captured variance above makes efficient). Not breaking anything — the residuals are MEDIUM display-fidelity gaps, and the round-trip + parser suites are green.
