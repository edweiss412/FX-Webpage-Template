...
This is **pre-existing and project-wide**, not introduced by the typography layer: `--color-accent-on-bg` is already the link/emphasis text color on StagedReviewCard, the onboarding wizard, IdentityChip, DashboardFooter, etc. The `/help` prose layer is the first surface to use it as a long-form *body-prose* link color at scale (`app/globals.css:583-587`), which is what surfaced it — but every prose link also carries an underline, so WCAG 1.4.1 (use-of-color) is satisfied and the deficit is the small 4.11-vs-4.5 contrast gap only. Links inside Callouts/asides were separately reset to `color: inherit` in that PR so they clear AA on the tinted backgrounds.

**Why backlog, not now:** the correct fix is at the token layer, not in `/help`. Darkening `--color-accent-on-bg-runtime` (light) from `#c25e00` to ~`#b35600` (≈4.6:1 on `#fafaf9`) changes the orange on **every** accent-on-bg consumer (admin + crew), requires correcting the DESIGN.md §1.1/§1.2 figures, and would shift the `/admin` screenshot baselines (which the screenshot-drift gate pins) — a much larger blast radius than the typography chunk, and a brand decision (it nudges the brand orange darker). Making `/help` links a different color than the rest of the app to dodge it would break the system's link-color consistency.
