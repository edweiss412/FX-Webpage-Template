...
- [ ] **Step 1:** Failing derivation tests (all from spec §10): mixed-warning row (non-gap warn + ambiguity gap → row judgment, section flagged); **gap-mixed row precedence** (`FIELD_UNREADABLE` + `ROOM_HEADER_SPLIT_AMBIGUOUS` on one row → needs-look (K), counted once, NOT judgment — precedence needs-look > judgment > clean); M=0 ⇒ count model's `showJudgmentBucket === false` (pure predicate; the RENDER consequence "two-state summary" is Task 11's test); blocking rows excluded; set-aside excluded; missing-preview row stays needs-look despite ambiguity; finalize-failure likewise; N+M+K === publishRows.length; ambiguity-only section → judgment; ambiguity+non-ambiguity section → flagged; FIELD_LABELS unknown-value omission. **Task 9 test scope, precise:** pure predicate/derived-value tests (`sectionStatus(...)`, `rowNeedsLook(...)`, `rowIsJudgment(...)`, `nonAmbiguityGapTotal(...)`, count triple, FIELD_LABELS lookup) PLUS the wiring-proof component tests from the Files block — which assert ONLY about EXISTING chrome (old needs-look border/button not selected for ambiguity-only rows; still selected for non-ambiguity gaps). NO assertions about NEW judgment chrome/copy — those (including the badge/chrome split: judgment chrome present while `DataQualityBadge` shows the FULL gap count) live exclusively in Task 11, preserving its red phase. Where a derivation currently lives inline in a component (`rowNeedsLook` in `Step3Review.tsx`, `needsLook` in `Step3SheetCard.tsx`), extract it to a pure exported function (component file or `lib/admin/`) so Task 9 tests target the function, and Task 11 wires chrome to it.
...
```
  ...
  (Adapt to the file's local helpers; `summarizeDataGaps` already exposes per-class counts.) Task 9 delivers a pure count model `deriveStep3Buckets(publishRows): { clean: number; judgment: number; needsLook: number; showJudgmentBucket: boolean }` plus the section/card predicates — NO JSX/copy changes anywhere in Task 9. ALL rendered output (three-count summary copy "N clean · M parsed with judgment — spot-check · K need a look", judgment card border, callout variant) is implemented AND render-tested first in Task 11, keeping Task 11's red phase intact. ("flagged" wording banned for K.)
