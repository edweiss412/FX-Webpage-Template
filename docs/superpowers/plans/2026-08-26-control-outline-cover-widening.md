...
| AC-13 | Playwright at 390px, light and dark, on the five surfaces spec §14 names, with before and after captures in the PR body. SATISFIED by TWO specs, 16 cases. `tests/e2e/control-outline-contrast.live.spec.ts` carries 14: the venue tile, crew contact icons, BellPanel config row, report textarea and row-actions trigger in both arms, plus §14 pairs 3 and 4 as real rect measurements. `tests/e2e/control-outline-pill.route.spec.ts` carries the other 2, the wizard done pill at `/admin?step=2`. The pill is a separate FILE and not merely a separate page: its route is a real Next document that loads the app's own font faces, and the shared font-fidelity fixture's oracle accepts only what `compileEntryCss` emits. Source pins are kept as the cheap layer; the 2026-08-26 ruling is that they do not satisfy this on their own. |
...
| dimensional invariants | 2 pairs measured in a real browser on production routes, 2 more measured by rect in the mounted step-3 tree (added at whole-diff round 1, which found the claim made and unsupported), 1 recorded as a documented limit with its probe; `scripts/ac15-width-parity.mts` shows 0 width differences across all 767 elements |
