...
| M70 premature summary | child emits `spec:lint <path>` then `summary:` — both ends correct, no frame | AC-36 fifth probe; without it the construction emits the summary TWICE (`emittedSummaryCopies=2`) |
| M71 vacuous embed fixture | AC-1/AC-20 fixture with zero sections, findings or details | AC-1 asserts the fixture's own class counts are non-zero first; only a report with all three kills all three deletion mutants |
| M72 row overlap | catch-all row above the well-formed row, so a valid marker matches both | design §3.4.1 first-match ordering; specific rows, then well-formed, then catch-all |
| M73 cardinality two only | `markers.length === 2` | AC-44 three-marker case |
| M74 needs a well-formed peer | `length >= 2 && some(isWellFormed)` | AC-44 both-defective pair (`RED_EMPTY` + `AC_MISSING`) |
| M75 severity downgrade | emit all ten codes as `advisory` | AC-47: CLI exits 1, renders `FAIL`, and the finding is waiver-suppressible (suppression applies only to `severity: "fail"`) |
| M76 post-inventory renderer | render `taskContract` AFTER the `INVENTORY` block | AC-25: section precedes `INVENTORY` in raw stdout AND the findings survive into the EMBEDDED block |
| M77 first-match over the whole table | apply first-match across per-extent AND per-marker rows, so `TASK_MARKER_DUPLICATE` short-circuits classification | design §3.4.1 two-group scoping; AC-44's five mixed fixtures |
| M78 early sentinel | child emits a bare `INVENTORY` or a non-final `summary:` before a later finding section | §2.3 fifth frame clause: nothing finding-shaped between `INVENTORY` and `summary:`, and `summary:` appears once |
| M79 first-id-only resolution | resolve `ids[0]` and ignore the rest | AC-13 multi-id marker: `ac=AC-1,AC-2` with only `AC-1` in prose gives `[TASK_AC_UNRESOLVED]`, the mutant `[]` |
| M80 singleton severity downgrade | emit exactly ONE of the ten codes as advisory | AC-47 catalog-driven fixture set, one plan per code — a single-finding fixture leaves nine mutants alive |
| M81 unmodelled exit status | child exits 3-255 with frame-shaped stdout | §2.3 catch-all row: any status outside 0/1/2 refuses with exit 2 |
| M82 small AC-35 fixture | pin `process.exitCode` with a report that flushes before exit | AC-35 requires a raw report >64 KB — a 305-byte fixture passes against the KNOWN-BAD `process.exit()` build |
| M83 four-clause validator retained | keep the pre-R12 frame check | AC-36 nine cases incl. early-inventory and early-summary |
| M84 discard-span denylist | reject only finding-shaped lines between `INVENTORY` and `summary:` | §2.3 fifth clause is an ALLOWLIST: every line there must be an inventory entry, so labels and `detail:` lines are protected too |
| M85 last-id-only resolution | resolve only the final id in `ac=` | AC-13 mirrored fixture (only the FIRST id unresolvable) |
| M86 dedupe-by-classification | collapse same-code markers before counting cardinality | AC-44 same-class pair — full-list assertion keeps the repeated code |
...
**M63/M72/M73/M74 — duplication is about cardinality, not classification (AC-44), and the rule quantifies over two dimensions.** Pin the matrix: two markers in one extent, the second well-formed, the first drawing each of `TASK_RED_EMPTY`, `TASK_AC_MISSING`, `TASK_MARKER_MALFORMED`, `TASK_AC_UNRESOLVED` in turn — each asserting `TASK_MARKER_DUPLICATE` **and** the defect code. The well-formed pair is the base case, not one of the four. Then close both dimensions the rule states ("two or more", "any class") with two further cases, verified against the reference implementation: **three markers** yields `[TASK_MARKER_DUPLICATE]`, killing `markers.length === 2`; **two defective markers** (`RED_EMPTY` + `AC_MISSING`) yields `[TASK_AC_MISSING, TASK_MARKER_DUPLICATE, TASK_RED_EMPTY]`, killing `length >= 2 && some(isWellFormed)`. **M86**: add a SAME-class pair — `RED_EMPTY` + `RED_EMPTY` yields `[TASK_MARKER_DUPLICATE, TASK_RED_EMPTY, TASK_RED_EMPTY]`, killing an implementation that deduplicates by classification before counting. The repeated code is the assertion, so compare full lists, never sets. Also assert a valid marker draws no code (M72) — with the catch-all row ordered above the well-formed row, every valid marker classified as `TASK_MARKER_MALFORMED`. **M77** is the same matrix read from the other side: the five mixed fixtures must each report the defect code **alongside** `TASK_MARKER_DUPLICATE`, which is what fails if an implementation applies first-match across the per-extent rows as well as the per-marker ones and stops at the duplicate. A mutant counting only well-formed markers toward duplication passes AC-8, AC-27 and AC-29 and silently omits duplication in every mixed extent.
...
**M79/M85 — resolution covers every id in the marker, both endpoints.** Two mirrored fixtures with `ac=AC-1,AC-2`: prose resolving only `AC-1` kills first-id-only, prose resolving only `AC-2` kills last-id-only. Each asserts `[TASK_AC_UNRESOLVED]`; one fixture alone leaves the opposite endpoint mutant alive. No other resolution case uses a comma-separated list, so nothing else can see the difference.
...
<!-- task: red=`pnpm vitest run tests/specLint/run.test.ts tests/specLint/cli.test.ts` ac=AC-10,AC-25,AC-40,AC-47 -->

**RED.** Cover M21 first: assert the CLI's stdout carries a `taskContract:` section with code, line, and message (AC-25) **and that the section precedes the `INVENTORY` block**, plus a second assertion that the findings survive into the embedded block (M76). Position is not decoration: §2.2.2's transformation removes everything from `INVENTORY` to `summary:`, so a renderer appending the new section after the inventory passes a presence-only check and AC-1's sequence comparison while the review prompt contains none of the new findings — the whole feature failing silently with every criterion green.

**Pin severity behaviorally, per code (M75, M80, AC-47).** Drive the fixture set from the §3.4 catalog — one minimal plan per code, ten in all — each asserting exit 1 and a `FAIL` render for that code, plus one waiver case. A single-finding fixture kills only the mutant that downgrades every code and leaves the nine singleton mutants alive; deriving from the catalog also covers an eleventh code by construction. Each fixture must make the CLI exit 1, render `FAIL` rather than `ADVISORY`, and be suppressible by a `spec-lint: ignore` waiver — suppression applies only to `severity: "fail"` findings (`lib/specLint/run.ts:93`), so the waiver case doubles as the severity assertion. A mutant emitting all ten codes as advisory preserves every code, anchor, message and finding list the other criteria assert, and flips the gate to exit 0. `CHECK_ORDER` does not drive the renderer — `scripts/spec-lint.ts:46` iterates its own closed literal list — so a run can exit 1 while the embedded report shows only an aggregate count, which is exactly what a reviewer would receive under design §2. Then extend `tests/specLint/run.test.ts` with a plan-kind document producing a `taskContract` finding, and a spec-kind document with identical text producing none (AC-10, closure family M12 — the identical-text pairing is what makes it a leakage test rather than two unrelated fixtures). Extend `tests/specLint/cli.test.ts` asserting `taskContract` findings render under their own heading in the documented check order.
...
- **M82 — the AC-35 fixture must be big enough to lose the race.** Pipe truncation is a race with process exit; a small report wins it and flushes. Assert the fixture's raw report exceeds 64 KB **before** comparing buffers, so a corpus change that shrinks it fails loudly. Measured: a 305-byte report gives `bufferEqual=true summary=true` against the KNOWN-BAD `process.exit()` build, while this spec's 56 KB report truncates to 16384.

- **M83/M84 — the frame check has five clauses and nine AC-36 cases.** Include early-inventory and early-summary reports, a check-section label or `detail:` line inside the discard span, and a child exiting 3.

- **M78/M81 — the frame check covers the discard and the exit status.** Assert a child emitting a bare `INVENTORY` before a later finding section is refused (raw has the finding, the embedded block would not, and the surviving summary would still count it), same for a non-final `summary:`; and assert a child exiting 3 with frame-shaped stdout is refused rather than dispatched.

- **M70/M71 — the embed fixture must not be vacuous, and the frame must be validated.** Choose the AC-1/AC-20 fixture by asserting its class counts first: `sections>=1, findings>=1, details>=1`, plus an `INVENTORY` block. `detail:` lines are the scarcest class, so a fixture picked for findings alone will usually have none and the drop-detail mutant survives. Separately assert the shape check rejects a child emitting only `spec:lint <path>` + `summary:` — both ends correct, no `kind:` line, no blank — which the construction would otherwise transform into a block carrying the summary twice.
