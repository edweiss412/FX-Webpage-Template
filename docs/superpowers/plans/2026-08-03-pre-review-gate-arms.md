...
**M79 — resolution covers every id in the marker.** One fixture with `ac=AC-1,AC-2` whose prose resolves only `AC-1`, asserting `[TASK_AC_UNRESOLVED]`; a first-id-only implementation reports `[]`. No other resolution case uses a comma-separated list, so nothing else can see the difference.
