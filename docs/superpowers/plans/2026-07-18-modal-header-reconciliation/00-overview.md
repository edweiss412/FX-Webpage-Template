...
| M4 | 8 | §4.5's collapse trades height for WIDTH, and the plan costed only the height. At 390px the wider single-line status pushes the strip onto a second wrapped row: the idle strip measures **44px → 80px** (`sm:flex-nowrap` means this is mobile-only; ≥sm is unaffected). This is inherent to the ratified §4.5 delta stacked on Task 7's Re-sync trigger, not a defect in the collapse — but it was not anticipated anywhere in the spec or plan. | Implemented §4.5 as specified and **reported rather than absorbed**. Knock-on: it silently broke `statusStripToggleLayout.spec.ts` invariant (b), whose card-vs-inline delta fell 20+px → ~11px. (b) claims a TOGGLE-WEIGHT compaction, and `cardShort` is a hand-rolled row with nothing to wrap, so at 390px the assertion had quietly become a wrap-count comparison. **Re-measured at ≥sm (800px) rather than widening the threshold** — precedent: invariant (a)'s CI-1b clause already measures at 800px in the same describe. Flagged for the close-out impeccable gate: whether an 80px two-row mobile strip is acceptable is a DESIGN call, not a test one. |

| M5 | 9 | Assertion E's stated rationale — "both are a single control row whose height is driven by the band's own `py-2` plus a ~24px child" — holds at ≥sm but **not at 390px**, where `flex-wrap` is live and the loaded strip wraps to THREE rows inside the panel: `[toggle, live]` / `[status, Re-sync]` / `[copy]`, a **149px** band against the skeleton's 73px. The gap is NOT closable by the lever the plan nominates (bar heights): the wrap point is a function of RENDERED DATA — the status line's width depends on its relative-time strings, so "Synced 1 hour ago" and "Synced 3 days ago" wrap differently. | **Did not widen the tolerance.** E is asserted at its full 4px strictness at ≥sm, where its premise is true (**measured: 61px vs 61px, delta 0.00px**). At 390px it is replaced by an honest weaker clause — the skeleton band reserves ≥ one full tap row + `py-2`, and never exceeds the loaded band — which still catches the regression that matters (a band collapsing because its row lost `min-h-tap-min`). Sizing the placeholders to reproduce this fixture's 3-row wrap was rejected as overfitting: it would assert nothing about any real show while looking green. **The root cause is the M4 finding**, and it is a DESIGN call for the close-out impeccable gate, not a test-tolerance one. D was unaffected — it holds at **0.30px at both viewports**. |
| M6 | 9 | Assertion C ("same number of header text rows") is **NOT red against the pre-change tree**, contrary to the plan's implicit framing. The old skeleton header had two rendered rows — title + STRIP — so the count coincidentally matched the loaded title + subline. | Declared rather than contorted. C's red is **conditional on the fix**: verified by executing the "moved the strip out but forgot the subline" state, where C fails 2-vs-1 and D fails at 22.30px. That is the exact regression C exists to catch, so it earns its place as a guard even though the original tree did not trip it. The genuine pre-change red for this task came from A (no `-subheader` element) and D (45.70px seam gap at 390px). |
