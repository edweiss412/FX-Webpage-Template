...
- **Real browser**, 5 cases, desktop-chromium, theme storage key blocked surgically (this count is safe to write down BECAUSE it is anchored: `scripts/check-crew-e2e-executed.mjs` carries it as an executed-count floor and the wiring guard reds if the two disagree, which is how the stale 4 was caught in the first place): a blocked write applies the theme and renders no note in the help header and the admin nav at 320px; the toggle keeps a 44px-plus target and the page does not overflow; the help header's trailing link still sits clear of the toggle. That last case is the wrapper-removal regression check.
...
- **Class sweep on round-1 finding 1 (the non-discriminating assertion), recorded rather than assumed.** Every OTHER case this arc added was written RED against the unchanged tree and only then made green, and a RED-first cycle IS the discrimination proof: the assertion demonstrably failed while the production line was absent. The one case that had no such cycle was the OS-change case, because it was a rewrite of an already-passing test rather than a new one, and that is precisely the case the reviewer caught. The repair was verified the other way instead, by planting: deleting the handler's dataset write reds it, deleting the handler's `setState` reds it. The lesson generalises past this arc, so it is written here: **a rewritten test inherits none of the original's discrimination, and needs its own proof.**
- The mechanical UI guards run green: em-dash copy, tap-target floor, control-outline fill, canonical class callee, and live-region mounting. The case count is deliberately not written down — it is a property of those suites, not of this change, and round-1 finding 3 was exactly that mistake. Re-derive with `pnpm exec vitest run tests/styles/_metaEmDashCopy.test.ts tests/styles/_metaTapTargetFloor.test.ts tests/styles/_metaControlOutlineFill.test.ts tests/specLint/canonicalClassCallee.test.ts tests/components/_metaLiveRegionMounting.test.ts`.
...
2. **`pnpm spec:lint` reports `CITATION_SYMBOL_ABSENT` advisories** on the spec's retired sections, which necessarily name symbols the code no longer has. **Exit code stays 0**, and that is the whole claim. An earlier draft of this row asserted "two" and was wrong — the diff review's round-1 finding 3 probed six of that class and eleven advisories overall. The count is a property of the document's current text, so it is derived rather than recorded; the spec banner carries the one-line command. **Re-file trigger:** `pnpm spec:lint` on that spec exiting non-zero, which is the only outcome that would actually mean something is broken.
