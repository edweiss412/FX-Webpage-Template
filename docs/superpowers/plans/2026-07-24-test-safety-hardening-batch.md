...
- [ ] **Step 1 (RED)** (spec §5 test 11): `synthesizeMarkdownFromXlsx(new ArrayBuffer(8))` (not a ZIP) throws `WorkbookSynthesisError`, `.cause` is the original error, and the message does not lose the original text. A VALID workbook fixture still returns markdown unchanged *(catches a wrapper that swallows the success path)*.
...
- [ ] **Step 2b:** add `BL-CRON-WORKBOOK-FAULT-MISCLASSIFIED` (medium / TELEMETRY GRANULARITY) per spec §7: the cron sync path funnels a `synthesizeMarkdownFromXlsx` throw (`lib/sync/runScheduledCronSync.ts:3118,3144`) into `drive_error` → `DRIVE_FETCH_FAILED` (`:207-213`, `:2660-2668`), so a corrupt workbook reads as a Drive failure there too. Deferred because that path owns a different classification, persistence, and alert-resolution contract; the `WorkbookSynthesisError` type landed in Task 6 is the hook a later fix keys on.
