...
- [ ] **Step 4: run to pass.** **Step 5: `pnpm tsc --noEmit`.** **Step 6: commit** `feat(crew-page): per-block WrappedSection error boundaries (crew:<section>:<block>, TILE_SERVER_RENDER_FAILED preserved)`.

---

### Task 10: Wire the real sections into `CrewShell`'s dispatcher (R8-HIGH-2 — the route still renders placeholders without this)
