...
**Goal:** Land the four no-UI data-layer changes that the rest of the redesign sits on: (1) the dates-parser `loadIn` capture into the existing `dates` jsonb (no DDL), (2) the shared `resolveKeyTimes(show, rooms)` helper with deterministic GS-room selection + `dates.loadIn` precedence + embedded-sentinel guard, (3) `buildRightNowContext` re-sourced from `rooms` (new signature, drop `contacts`, drop the always-empty `event_details` time reads), (4) `ProjectedRoomRow` projection passthrough (`ShowForViewer.rooms` typed `RoomRow & { id }`, `dates.loadIn` passes through). Covers §9 tests **3, 4, 20**. **No UI is touched in this phase** — `_ShowBody.tsx` is NOT modified (the `buildRightNowContext` call-site swap is Phase 2's `_ShowBody`→`_CrewShell` work).
...
- Do **not** modify `_ShowBody.tsx`. It still imports the **old** `buildRightNowContext` signature; changing the signature here will break its call site (`_ShowBody.tsx:122-127`), so the signature-change task below uses a `// not-subject-to-meta` shim discipline: see Task 3, step "Reconcile the existing call site."
