...
> `CrewSubNav` is `'use client'`: builds the next URL from a FRESH `URLSearchParams` carrying only the allow-listed `s` + `gate` (R13-MEDIUM-1 — NOT a clone of all current params), `router.push(url, { scroll: false })`. `CrewSectionTransition` wraps the server-rendered active section as `children`, keyed by section id, framer crossfade `initial={false}`, reduced-motion-safe (§4.10). Full nav addressability + back-button + gate-preservation are Playwright-asserted in Phase 4 (test 13/28); this task pins the unit-level URL-building + structure.

- [ ] Write `tests/components/crew/crewSubNav.test.tsx` (jsdom; mock `next/navigation` `useRouter`/`usePathname`/`useSearchParams`). Assert:
  - With `useSearchParams` = `gate=skip`, clicking the "venue" tab calls `router.push` with a URL whose query has BOTH `s=venue` AND `gate=skip` and `{ scroll: false }`. _Failure mode: tab-click clobbers `gate` (the test-13 tab-click case)._
  - **Allow-list (R13-MEDIUM-1):** with `useSearchParams` = `gate=skip&evil=1&token=secret`, clicking a tab → the pushed URL has **ONLY** `s` and `gate` (= `skip`); `evil`/`token` are **NOT** carried. _Failure mode: cloning all current params preserves stale/sensitive query keys into browser history + every tab URL — contradicting the §4.1a/wp-26 allow-list the auth-boundary work (Task 12) otherwise enforces._
...
**Files:** `lib/adminAlerts/upsertAdminAlert.ts` (modify), `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` §12.4 (modify), `lib/messages/catalog.ts` (modify), `lib/messages/__generated__/spec-codes.ts` (regenerated), `tests/messages/_metaAdminAlertCatalog.test.ts` (modify), **`components/admin/PerShowAlertSection.tsx` (modify — render `context.failedKeys`, R5-HIGH-1)**, `tests/components/admin/perShowAlertFailedKeys.test.tsx` (new), **`app/show/[slug]/[shareToken]/_CrewShell.tsx` (CREATE — minimal producer)**, `tests/components/crew/crewShellAlert.test.tsx` (new)
...
- [ ] **Layer 2 — §12.4 prose:** add a row to the master-spec §12.4 table (under the **Tile error boundaries (server-side)** group, after `TILE_SERVER_RENDER_FAILED` at `:2934`). Columns: code | trigger | dougFacing | crewFacing | followUp. Pattern-match the `TILE_SERVER_RENDER_FAILED` row. No em-dash (use commas/colons/parentheses, §4.18). Example row (adapt copy, keep `*<sheet-name>*` interpolation, crewFacing null since crew never sees it):
  - **The copy is DOMAIN-NEUTRAL — do NOT enumerate the specific data domains (R12-MEDIUM):** the failed sources live in `context.failedKeys` (rendered in the alert detail, R5-HIGH-1), and Phase 2 adds `run_of_show` as a 6th `failedKeys` domain on the explicit premise that this copy is **domain-agnostic** (so Phase 2 needs no §12.4 change). Enumerating "rooms, hotel, contacts, transportation, financials" here would make that premise false and leave stale copy when `run_of_show` (or any future domain) is added. So the row + helpfulContext say "one or more data sources" and point to the alert detail, never a fixed list.
  - `| `TILE_PROJECTION_FETCH_FAILED` | the crew-page projection (`getShowForViewer`) reported one or more data-source fetch failures (`tileErrors`); the page rendered, but at least one data source could not load (the specific sources are in the alert's failed-sources detail). | "*<sheet-name>*: one or more crew-page data sources couldn't load (the failed sources are listed in the alert detail). The page rendered with the rest of the data; refresh in a minute. Tell the developer if this keeps happening." | — | Doug → refresh / Report; Eric → investigate |`
  - Also add the matching entry to the §12.4 helpfulContext appendix map (near `:3117`, after `TILE_SERVER_RENDER_FAILED`): `TILE_PROJECTION_FETCH_FAILED: "The crew page loaded, but one or more of its data sources failed to fetch from the server. The page rendered with the data that did load. The specific failed sources are listed in the alert detail. Refresh in a minute; if this keeps happening, click 'Report' so the developer can investigate."` (No enumerated domain list — domain-neutral, R12.)
...
- [ ] **Layer 4 — `catalog.ts` row:** add a `TILE_PROJECTION_FETCH_FAILED` entry to `MESSAGE_CATALOG` (pattern-match `TILE_SERVER_RENDER_FAILED` `:1690-1702` — `code`, `dougFacing` with `*<sheet-name>*`, `crewFacing: null`, `followUp`, `helpfulContext`, `title`, `longExplanation`, `helpHref: "/help/errors#TILE_PROJECTION_FETCH_FAILED"`). Copy must match the §12.4 prose verbatim (x1 parity compares them). No em-dash. **Do NOT** add a `<failed-keys>` placeholder to `dougFacing` — `failedKeys` is an array `safeDougFacing`'s `messageFor` interpolation does not enumerate, and an unresolved `<…>` placeholder makes `safeDougFacing` return `null` (`PerShowAlertSection.tsx:64`). The failed-domains list is rendered separately (next step).
- [ ] **Render `context.failedKeys` in the admin alert detail (R5-HIGH-1 — the observability surface).** The `message` is a viewer-independent constant (R3) and the `dougFacing` is a static template, so **nothing renders the failed domains today** — Doug would see only a generic "one or more sources failed". Fix: in `components/admin/PerShowAlertSection.tsx` (the per-show admin alert list — verify the exact alert-row render site), when `alert.code === "TILE_PROJECTION_FETCH_FAILED"` and `Array.isArray(alert.context?.failedKeys)`, render a small detail line under the alert: `Failed sources: ${(alert.context.failedKeys as string[]).join(", ")}` (sentinel/escape-safe, no raw error text — `failedKeys` is the curated domain list, never pg error strings). Mirror it in `components/admin/AlertBanner.tsx` only if the global banner surfaces this code (it is per-show, so PerShowAlertSection is the primary). This is admin UI (Opus). Write the failing test first: `tests/components/admin/perShowAlertFailedKeys.test.tsx` — an `admin_alerts` row `{ code: "TILE_PROJECTION_FETCH_FAILED", context: { sheet_name: "Acme", failedKeys: ["hotel","rooms"] } }` → the rendered alert detail contains the text `hotel, rooms` (the union'd domains visible to Doug); a row WITHOUT `failedKeys` (or another code) → no "Failed sources" line; assert NO raw error string and NO em-dash. _Failure mode: the union-merged `failedKeys` payload being stored but invisible to Doug — the new alert is useless without the domain list rendered (the R3 viewer-independent message moved the detail into `failedKeys`, which must therefore be rendered)._
- [ ] **Create the MINIMAL `_CrewShell.tsx` producer** (Task 11 extends it). Write `tests/components/crew/crewShellAlert.test.tsx` first: render `CrewShell` (async Server Component) with `showId="show-abc"` and `data.tileErrors = { hotel, rooms, contacts }` populated → EXACTLY ONE `upsertAdminAlert` call with **`showId: "show-abc"`** (the prop, NOT `null` and NOT `data.show.id` — R7-HIGH; assert the exact value), `code: "TILE_PROJECTION_FETCH_FAILED"`, `context.failedKeys` = sorted `['contacts','hotel','rooms']` (derived from the fixture's `tileErrors` keys, not hardcoded), `context.sheet_name === data.show.title`, `context.tileId === 'crew:projection-alert'`, NO `signature`/`viewerVersionToken` key; a HEALTHY projection (`tileErrors={}`) → NO call; the upsert mock rejecting → still renders (fail-quiet). Then implement the minimal shell:
  ```tsx
  // app/show/[slug]/[shareToken]/_CrewShell.tsx  (MINIMAL — Task 11 adds Header/nav/section/Footer/guard)
  import { upsertAdminAlert } from "@/lib/adminAlerts/upsertAdminAlert";
  export async function CrewShell({ data, viewer, showId, rawSection, slug, shareToken }: CrewShellProps) {
    const failedKeys = Object.keys(data.tileErrors).sort();
    if (failedKeys.length > 0) {
      try {
        await upsertAdminAlert({ showId, code: "TILE_PROJECTION_FETCH_FAILED", // R7-HIGH: the showId PROP, NOT data.show.id (ShowRow has no id)
...
EXACT props (00-overview): `CrewShellProps = { data: ShowForViewer; viewer: Viewer; showId: string; rawSection: string | undefined; slug: string; shareToken?: string; identityChip?: { name: string; role: string; shareToken: string } | null }`. **`showId` is a SEPARATE prop (R7-HIGH — `ShowRow` has no `id`)** threaded into `upsertAdminAlert({ showId })`, `ShowRealtimeBridge showId={showId}` (test 23), and any wrapped-block alert; assert tests use the exact `showId`, never `data.show.id` or `null`. **`identityChip` is ported from `ShowBody` (R4-MEDIUM-2)** — the crew route derives it from the resolved crew row (`page.tsx:171` `{ name: crew.name, role: crew.role, shareToken }`), admin + preview-as pass `null` (`page.tsx:134`); `CrewShell` threads it into `Header` exactly as `_ShowBody.tsx:455-462` does. Add a shell test: a `crew` viewer with `identityChip` set → the Header renders the chip; an `admin` viewer (`identityChip={null}`) and the malformed-projection `TerminalFailure` path render NO chip. **CrewShell resolves `activeSection` ITSELF** (R2-HIGH-1): `const activeSection = resolveActiveSection(rawSection, { budgetVisible: financialsVisible(ctx.viewerFlags, ctx.isAdmin) })` — after the fail-closed `resolveViewerContext` — so the Budget gate (tab + section + direct-URL) has a single authority and preview-as cannot enter `?s=budget` for a non-LEAD previewed crew.
...
- [ ] Implement:
  ...
  - `admin` arm (`:128`) + `resolved` arm (`:165`): swap `<ShowBody .../>` → `<CrewShell data viewer showId={result.showId} rawSection={s} slug shareToken={shareToken} identityChip={...} />`, passing **`showId={result.showId}` verbatim as today** (R7-HIGH — `ShowBody` already receives `result.showId` at `page.tsx:131`/`:168`; `CrewShell` needs it for the alert + realtime) and `identityChip` **verbatim as today** (R4-MEDIUM-2): admin arm `identityChip={null}` (`page.tsx:134`); resolved/crew arm `identityChip={crew ? { name: crew.name, role: crew.role, shareToken } : null}` (`page.tsx:171`). `CrewShell` threads it into `Header` (Task 11). (admin arm passes `shareToken` too; preview-as omits both in Task 13 — `identityChip` defaults to `null`/undefined there, matching the admin view.)
