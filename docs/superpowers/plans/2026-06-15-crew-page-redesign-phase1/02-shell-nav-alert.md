...
**CRITICAL ORDERING (§4.13 / §10 sequencing — non-negotiable):** the server-side projection-alert upsert (Task 11), the `upsert_admin_alert` migration (Task 9), the `TILE_PROJECTION_FETCH_FAILED` four-part lockstep (Task 8), and the `_metaAdminAlertCatalog` / `_metaSentinelHidingContract` meta-test extensions (Tasks 4, 8) must **all be GREEN in Phase 2** — *before* Phase 4 deletes the `_ShowBody.tsx` always-rendered `notes-tile` alert path. The section-independent observability must be live before its render-bound predecessor is removed. This is restated in **Phase exit criteria**.
...
### Task 8: `TILE_PROJECTION_FETCH_FAILED` four-part lockstep + EXTEND `_metaAdminAlertCatalog` (tests 16/29/36 catalog half, meta-test #2)

**Files:** `lib/adminAlerts/upsertAdminAlert.ts` (modify), `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` §12.4 (modify), `lib/messages/catalog.ts` (modify), `lib/messages/__generated__/spec-codes.ts` (regenerated), `tests/messages/_metaAdminAlertCatalog.test.ts` (modify)

> The §5 four-part lockstep (master-spec prose + `gen:spec-codes` + `catalog.ts` row + `AdminAlertCode` union) lands in ONE commit. `x1-catalog-parity` enforces three of the four; the extended meta-test enforces the fourth (registry⊆union). No DB change — `admin_alerts.code` is unconstrained `text`. The producer (the `CrewShell` server-side upsert) lands in Task 11; this task registers it ahead so the write-site assertion is ready (and Task 11 is the green-up). To keep all gates green within THIS commit, register the producer write-site pointing at `_CrewShell.tsx` — so Tasks 8→11 must be developed together OR Task 8's producer test temporarily exempted; **simplest: do Task 11's `_CrewShell` scaffold + upsert first within this same task branch, then land Task 8's lockstep referencing it.** (The plan keeps them as separate commits for review legibility but the write-site assertion requires the producer file to exist.)
...
- [ ] Run to pass: `pnpm vitest run tests/messages/_metaAdminAlertCatalog.test.ts tests/cross-cutting/codes.test.ts` (x1-catalog-parity). `pnpm tsc --noEmit`. (Requires `_CrewShell.tsx` to exist with the upsert — see Task 11 note above.)
- [ ] Commit `feat(messages): TILE_PROJECTION_FETCH_FAILED four-part lockstep + registry⊆union meta-assertion`.
...
**Files:** `app/show/[slug]/[shareToken]/_CrewShell.tsx` (new), `tests/components/crew/crewShell.test.tsx` (new)

> Replaces `_ShowBody` as the body. Renders inside `data-testid="crew-shell"`: `Header` → `CrewSubNav` → `ShowRealtimeBridge` → `CrewSectionTransition`(active section) → `Footer`. Today leads with `RightNowHero`. During render it performs the §4.13 section-independent server-side projection-alert upsert. Ports verbatim: the malformed-projection fail-closed guard (`_ShowBody.tsx:113-121`), the Footer report-prop contract (`:509-539`), `ShowRealtimeBridge renderVersion={data.viewerVersionToken}` (`:469`), `nowDate()` server clock (`:133`), `sheetName=data.show.title`. Section components are Phase 3 — use a minimal placeholder `<section data-testid={\`section-${activeSection}\`}>` that renders the section id + (Today only) `RightNowHero` so the shell is testable now; Phase 3 fills the real sections.

EXACT props (00-overview): `CrewShellProps = { data: ShowForViewer; viewer: Viewer; activeSection: SectionId; slug: string; shareToken?: string }`.
...
- [ ] Implement `app/show/[slug]/[shareToken]/_CrewShell.tsx` (Server Component, `async`):
  ...
  - **Projection alert (§4.13, server-side, fail-quiet):** compute `failedKeys = Object.keys(data.tileErrors).sort()`; iff non-empty, `try { await upsertAdminAlert({ showId, code: "TILE_PROJECTION_FETCH_FAILED", context: { sheet_name: data.show.title, tileId: "crew:projection-alert", message: \`${failedKeys.length} crew-page data sources failed to load: ${failedKeys.join(", ")}\`, failedKeys } }) } catch (e) { console.warn(...); /* fail-quiet — never break render */ }`. Carry an inline `// not-subject-to-meta: best-effort observability write, fail-quiet` note OR add to the call-boundary audit registry if one walks `app/show/**` (decide by checking which audit covers the file — invariant 9 / 00-overview meta-inventory #7). Destructure `{ data, error }` is inside `upsertAdminAlert` already; the shell's contract is the try/catch.
  - Render `data-testid="crew-shell"` wrapper: `Header` (with status pill + identity chip — the chip prop comes from the caller as today via `identityChip`; preserve) → `CrewSubNav activeSection={activeSection} budgetVisible={financialsVisible(ctx.viewerFlags, ctx.isAdmin)}` → `ShowRealtimeBridge showId slug renderVersion={data.viewerVersionToken}` → `CrewSectionTransition sectionId={activeSection}`(placeholder active section; Today leads with `RightNowHero context={buildRightNowContext({ show, dateRestriction: ctx.dateRestriction, hotelReservations: data.hotelReservations, rooms: data.rooms })}`) → `Footer` with the verbatim per-viewer-kind report props (`reportSurfaceOverride`, conditional `reportSurfaceIdOverride`, `reportAutocapture.crewPreview` only for `admin_preview`, `lastSyncedAt`/`lastSyncStatus`).
...
**Files:** `app/show/[slug]/[shareToken]/page.tsx` (modify), `app/show/[slug]/[shareToken]/_PickerInterstitial.tsx` (modify), `app/show/[slug]/[shareToken]/_SignInOrSkipGate.tsx` (modify), `tests/components/crew/showPageRouting.test.tsx` (new)

> Widen `searchParams` to `{ gate?; s? }`; resolve `activeSection` via `resolveActiveSection(s, { budgetVisible })`; pass to `CrewShell`; preserve the validated `s` (+ existing `gate`) through ALL the redirect builders that return to `/show/[slug]/[shareToken]`: picker-bootstrap, sign-in `returnTo`, `gate=skip` CTA, stale-cleanup, and the picker `selectIdentityFormAction` selection. Only the allow-listed `s` (validated against the section-id set) and `gate` are carried (§4.1 / wp-24 / wp-26). The access guards stay unchanged (§4.14) — this adds a query param to existing builders, NOT an auth rewrite.
...
- [ ] Run to fail: `pnpm vitest run tests/components/crew/showPageRouting.test.tsx`.
- [ ] Implement:
  ...
  - `page.tsx:71` → `searchParams: Promise<{ gate?: string; s?: string }>`; `:74` await `s`; compute `const activeSection = resolveActiveSection(s, { budgetVisible: financialsVisible(...) })` for the `admin`/`resolved` arms (budgetVisible per the viewer; for `admin` it is `true` via `isAdmin`).
  - `admin` arm (`:128`) + `resolved` arm (`:165`): swap `<ShowBody .../>` → `<CrewShell data viewer activeSection slug shareToken={shareToken} />` (admin arm passes `shareToken` too; preview-as omits it in Task 13). Keep `identityChip` threading (pass through to `CrewShell` Header).
...
- [ ] Run to pass; `pnpm tsc --noEmit`. Commit `feat(crew-page): widen searchParams to {gate,s}; preserve s+gate through picker/sign-in/gate=skip/stale redirects`.
...
- [ ] Write `tests/components/crew/previewAsRoute.test.tsx`: mock `requireAdmin`, `getShowForViewer`, the show/crew lookups, and `nowDate`; render the page with `searchParams` resolving `{ s: "venue" }` → assert `CrewShell` is rendered (`data-testid="crew-shell"`) with `viewer.kind === "admin_preview"`, `activeSection === "venue"`, and `shareToken` undefined; default (no `s`) → `activeSection === "today"`. _Failure mode: preview-as left on the old flat-grid `ShowBody`; preview not reading its own `?s=`._
- [ ] Run to fail: `pnpm vitest run tests/components/crew/previewAsRoute.test.tsx`.
- [ ] Implement: widen `PageProps` (`:51-53`) to `{ params: Promise<{ slug; crewId }>; searchParams: Promise<{ s?: string }> }`; await `s`; `const activeSection = resolveActiveSection(s, { budgetVisible: true })` (admin_preview adopts the previewed viewer's gate — but Budget visibility derives from the previewed crew's flags inside `CrewShell`; pass `budgetVisible` consistently with the §4.1 single-predicate rule by resolving it the same way `CrewShell` will, i.e. via `financialsVisible` on the resolved viewer — to avoid divergence, resolve `activeSection` with `budgetVisible: true` here and let `CrewShell`'s tab/section gate be the authority; a non-entitled `?s=budget` still falls back inside `CrewShell`). Swap `<ShowBody slug showId viewer data />` (`:233-238`) → `<CrewShell data viewer={{ kind: "admin_preview", crewMemberId: crewId }} activeSection slug={slug} />` (no `shareToken`). Keep `<PreviewBanner />` above.
