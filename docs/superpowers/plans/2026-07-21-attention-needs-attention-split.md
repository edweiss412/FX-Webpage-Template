...
3. **Focus mechanism (§6a probe verdict; REVISED in whole-diff R1):** the rAF-deferred close FAILED 3/9 probe cells; the first ratified mechanism was a DERIVED open state (`menuOpen && interactive`, same-render unmount) + sync focus rescue + next-frame rAF flag cleanup. Whole-diff R1 found a rebound race in the rAF cleanup (1-frame interactivity blip cancels the pending close; §5.2 auto-open one-shot also re-armed). Final mechanism: derived open state + RENDER-PHASE flag reconciliation (React's sanctioned derived-state adjustment — `if (menuOpen && !interactive) setMenuOpen(false)` during render) + post-commit focus-rescue effect keyed on the derived-open drop + auto-open one-shot consumed whenever the menu is already open. Probe re-ratified 11/11 + §11.9 nav green; §6a's outcome contract unchanged (it is mechanism-agnostic by design).
...
## §13 Whole-diff adversarial review record (2026-07-22)

Two scoped Codex dispatches (split tight-scope per AGENTS.md default). Lib scope needed the inlined no-tool fallback (3x no_o_file on the tool-using dispatch).

**R1 verdicts:** UI scope NEEDS-ATTENTION (7 findings, all CONFIRMED against source); lib scope NEEDS-ATTENTION (1 P2, 2 P3).

**Repaired (commit 2516e77ed):** scroll boundary wraps all menu groups; rebound race (render-phase flag reconciliation + auto-open one-shot consumption — deeper root cause found in repro); §3.3 `!actionable` guard on both selfHeal filters; composite monitoring hollow dot; 99+ caps + sr-only exact counts on every pill count path; real " · " separator text nodes; e2e/jsdom focus assertions tightened to dialog root; publishedPill matrix rewritten to exact visible-text + per-row trigger type; driveFileId end-to-end integration pin + gallery no-id path; pageTransitions re-pins (PublishedReviewModal 8 -> 11).

**Dispositions (no change):** anti-tautology XOR truth-table test kept (P3 "test noise" — harmless documentation; the universe/parity tests carry the real protection, as the reviewer noted); "ALL 11 linked codes" stale comment corrected to 10 (P3).

**R2 (2026-07-22):** lib scope **APPROVE** (inlined dispatch: "All R1 findings are closed"). UI scope: both the tool-using AND inlined R2 dispatches died silently (no_o_file x3 each — prompt echoed, zero assistant output, no error; same wedge class as the plan-review rounds). Per the codex-guard no_verdict ladder, the UI scope is closed by SELF-CERTIFICATION: every R1 finding has a dedicated regression test in the repair commit 2516e77ed (scroll-boundary containment test; rebound-race test; root-focus assertions tightened in BOTH tiers and probe-verified; three 99+ cap tests; two hollow-dot tests; two mistagged-item guard tests; exact visible-text matrix rewrite), the real-browser probe is 11/11 with the tightened assertions, and the full suite is 16272 passed / 0 failed. Real CI green remains the final external gate before merge.
