...
**CORRECTED 2026-08-30 by `test/e2e-spec-discovery-wiring`: that spec HAD been executing, on every PR, for four weeks.** This paragraph claimed it had never run and matched no project regex until this arc wired it. It matched `tests/e2e/standalone.config.ts` from 2026-08-02 (`434deaf7f`), and `.github/workflows/standalone-e2e.yml` has run that whole config on every pull request since 2026-07-26 (`c7c5625c2`). The reds this arc measured were themselves measured under the standalone config. What this arc newly added was membership in `desktop-chromium`, which no workflow selects, and which has since been reverted. The census behind the claim measured one config of four; see `LIM-E2E-SPEC-DISCOVERY-GAP`. What the discovery gap recorded as `LIM-E2E-SPEC-DISCOVERY-GAP` had been hiding is therefore not a shipping defect, as this paragraph first claimed, but a defective oracle: one that would have gone red for whoever next moved that layout past a .5 remainder, whether or not they broke anything, and that meanwhile accepted a genuinely misfitted scroller. The defective oracle below is real and is what the arc actually found; the reading that a DARK spec had rotted is not, since this spec was never dark. The slug was filed on a count, 118 spec files on disk against 70 discovered, which is the kind of number that reads as housekeeping. This is what it actually buys.
