...
Seed a fake `sql` tag that records the query text + params and returns canned rows. Assert ONLY rows matching `source='auto_apply' AND status='applied' AND acknowledged_at IS NULL AND change_kind ∈ STRIP_KINDS AND occurred_at > windowStart` appear, grouped by show. Include (in the canned set) an acked row, a pre-window row, a non-`auto_apply` row, and an off-list `change_kind`; assert each is excluded. Because the fake `sql` returns whatever we give it, this test asserts the QUERY TEXT contains each predicate + the windowStart bind, and that the grouping/mapping of returned rows is correct.
