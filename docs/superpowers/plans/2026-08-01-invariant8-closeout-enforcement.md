...
- M1 mutants: (a) `walkPlansTree` filters out directories whose name is undated (kills category traversal) — T1 evidence: category-shape fixture plant red; T2 RE-RUNS this mutant and records the §4.1.5 canary assertion red (the live tie-in belongs to T2; plan r2 F2); (b) delete the closeout-attach branch — attach plant red (T1); (c) partition keys on the DEEPEST dated segment — no-sub-unit-reopen plant red (T1); (d) `walkPlansTree` reads a hardcoded path list — T1 evidence: novel-unit VISIBILITY assertion red (the walker fails to see a fresh tmpdir unit); the registry-coupled "novel declaring unit with no row reds the guard" half is T2's M5-adjacent evidence, recorded there (plan r2 F2).
...
- M3 mutants (one per reject-table row): drop trailing `$` anchor — trailing-text plants red; widen the half-status enum to `\w+` — `critique=SKIPPED` plant red; make `audit=` optional — missing-field plant red; delete cross-check — both cross-check plants red; widen int to `\d+` — leading-zero plant red; drop `trimStart` — indented plants red; widen the N/A dash to `[—-]` — ASCII-hyphen plant red; accept TEMPLATE form everywhere — TEMPLATE-outside plant red. (Two mutants added per plan r2 F2 — the row list is now mutant-complete against the 17-case table.)
