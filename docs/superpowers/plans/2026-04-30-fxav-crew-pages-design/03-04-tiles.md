...
export function lightingScopeVisible(flags: RoleFlag[]): boolean {
  // §8.1 amended 2026-05-13 — LEAD now reads-in to Lighting scope,
  // symmetric with Audio/Video. See spec Amendment §8.1-A1.
  return flags.includes("L1") || flags.includes("LEAD");
}
```

LEAD-only viewers see Financials AND Audio AND Video AND Lighting scope tiles (unconditional on the presence of audio/video/lighting crew on the show — LEAD has scope visibility regardless). Compound viewers like `['LEAD','A1']` see Audio (twice-true via either predicate) + Video + Lighting + Financials.
...
- [ ] **Step 1:** failing tests assert tiles render per the canonical rule:
  ...
  - `['LEAD']` viewer → Audio AND Video AND Lighting visible (§8.1 amendment 2026-05-13).
  - `['LEAD','A1']` viewer → Audio AND Video AND Lighting visible.
  - `['LEAD','L1']` viewer → Audio AND Video AND Lighting visible (Lighting unlocked by either branch).
...
- [ ] **Step 4: Crew-page visibility-mode transitions over `role_flags[]` capability set** — beyond RightNow states, the crew page's tile-visibility logic is driven by the **`role_flags[]` capability array** (§6.6), NOT a single role enum. Earlier draft used `viewerRole ∈ { A1, V1, L1, LEAD, admin }` — but `L1` isn't even a canonical flag, and a crew member can carry multiple flags simultaneously (`['LEAD', 'A1']`, `['A1', 'BO']`, etc.). The corrected audit drives transitions over capability predicates against the canonical §6.6 flag set:
  ...
  - **Capability predicates** that drive tile visibility:
    - `hasLead = flags.includes('LEAD')` → unlocks `FinancialsTile`. **Per the canonical `SCOPE_TILE_VISIBILITY_RULE` defined in spec §8.1 (amended 2026-05-13) and implemented in `lib/visibility/scopeTiles.ts`**, `hasLead` ALSO unlocks Audio scope tile AND Video scope tile AND Lighting scope tile. The shared predicates are: `audioScopeVisible = hasA1 || hasLead`, `videoScopeVisible = hasV1 || hasLead`, `lightingScopeVisible = hasL1 || hasLead`. LEAD-only viewers (`['LEAD']`) see Financials + Audio + Video + Lighting. This matrix MUST import the predicates from `lib/visibility/scopeTiles.ts` — no inline rule restatement.
    ...
    - `hasL1 = flags.includes('L1')` is the atomic-membership predicate. LightingScopeTile renders when `lightingScopeVisible(flags) = hasL1 || hasLead` (post-§8.1 amendment 2026-05-13). (`L1` is a canonical atomic flag in the v4 role-master per fixture `2026-04-asset-mgmt-cfo-coo-waldorf.md:718-743`.)
    - Each tile-visibility predicate (`audioScopeVisible`, `videoScopeVisible`, `lightingScopeVisible`, `financialsVisible = hasLead || isAdmin`) is imported from `lib/visibility/scopeTiles.ts`. Compound viewers like `['LEAD', 'A1']` get FinancialsTile AND AudioScopeTile (both predicates true) AND VideoScopeTile (LEAD branch) AND LightingScopeTile (LEAD branch). LEAD-only viewers (`['LEAD']`) get all four gated tiles.
  ...
  - **Compound transitions**: include at least 3 cases where two predicates flip simultaneously in one render cycle. **Worked examples under the canonical SCOPE_TILE_VISIBILITY_RULE (post-§8.1 amendment 2026-05-13)**: - `['LEAD','A1'] → ['A1']`: `hasLead` flips false. Tile-level: FinancialsTile disappears AND VideoScopeTile disappears AND LightingScopeTile disappears (all three were unlocked by LEAD branch). AudioScopeTile stays visible (hasA1 still true).
    - `['LEAD','A1'] → ['V1']`: `hasLead` flips false, `hasA1` flips false, `hasV1` flips true. Tile-level: FinancialsTile disappears, AudioScopeTile disappears (no LEAD, no A1), LightingScopeTile disappears (no LEAD, no L1), VideoScopeTile stays visible (now via hasV1 branch instead of LEAD branch — net visibility unchanged but the _reason_ shifted; assert tile renders without flicker).
    - `['LEAD'] → ['L1']`: `hasLead` flips false, `hasL1` flips true. Tile-level: FinancialsTile disappears, AudioScopeTile disappears, VideoScopeTile disappears, LightingScopeTile stays visible (now via hasL1 branch instead of LEAD branch — net visibility unchanged but the _reason_ shifted; assert tile renders without flicker). Largest single-render visibility delta in the matrix.
