...
**Note on milestone numbering:** the planned FXAV crew-pages milestone set in THIS plan tree is M0-M10 + X.\* (cross-cutting tasks in `11-cross-cutting.md` — the "11" is the file number, NOT an M11 milestone). **M12 (user-facing docs / `/help`) IS planned** but lives in a sibling plan tree at `docs/superpowers/plans/2026-05-12-user-facing-docs/` with its own spec at `docs/superpowers/specs/2026-05-12-user-facing-docs-design.md`; M12 depends only on M10 (already closed) and is independent of X.\*. **M11 (post-v1 ops-hardening) is NOT planned** — items currently routed to "M11 ops-hardening" or "M11+ post-v1" are routed to an UNSCHEDULED future milestone bucket. When picking up any such item, the first step is "scope and plan the M11 milestone" (write a spec + plan tree analogous to the M12 one), not "implement the deferral." This is itself a planned-future-work deferral (the planning is the blocker), but the wording should be explicit so the deferral doesn't become permanent shelving.
...
**Suggested home:** UNSCHEDULED post-v1 ops-hardening milestone (informally referenced as "M11" but not yet planned — see DEFERRED.md header "Note on milestone numbering"). Originally re-deferred to "M11 ops-hardening" at M10 close-out (2026-05-19); on later pre-X.\* sweep this was reclassified to acknowledge the milestone hasn't been planned. Same home cluster as M5-D9/D10/D11 (operator-log producers) and any forthcoming Sentry/Bug-pipeline integration. M10 runtime is acceptable without this — the Doug-facing copy was softened in Phase 1 R1 (commit `94f7c31`) to avoid falsely claiming notification was sent; the operator-error path still surfaces an actionable message to Doug inline. Picking this up requires first scoping the ops-hardening milestone (Sentry integration + admin-banner producer surface design) before the producer wiring lands.
...
**Suggested home:** UNSCHEDULED post-v1 milestone (informally referenced as "M11+" or "post-v1" but not yet planned — see DEFERRED.md header "Note on milestone numbering"). Needs (a) the notification-design-memo discharged into a spec amendment, (b) real Doug-workflow data from v1 operation, (c) an email-provider integration decision (Resend / Postmark / SES). Picking this up requires first scoping the push-notification milestone — surface is too cross-cutting (schema, routes, email provider, report-pipeline integration, action-token signing) to retrofit into an existing milestone.
...
**Suggested home:** UNSCHEDULED post-v1 ops-hardening milestone (informally referenced as "M11" but not yet planned — see DEFERRED.md header "Note on milestone numbering"). Picking this up requires first scoping + planning the ops-hardening milestone (operator-log sink design, schema, ingestion, surfaces); the producer wiring is a downstream task within that milestone. M8 kickoff verified M6 / M6.5 / M7 did not land `lib/operatorLog/`, and M8 chose not to introduce the sink because §13.2.3 report-pipeline lease/recovery work is already the milestone's convergence risk; landing operator-log storage plus auth-route producers would double the review surface. When picking up: add the OAUTH_REDIRECT_INVALID and OAUTH_STATE_INVALID emit calls in callback/route.ts:32-50 alongside the sink, with regression tests covering invalid next, missing PKCE, and exchange failure paths.
...
**Suggested home:** UNSCHEDULED post-v1 ops-hardening milestone (informally referenced as "M11" but not yet planned — see DEFERRED.md header "Note on milestone numbering"). Picking this up requires first scoping + planning the ops-hardening milestone (operator-log sink design, schema, ingestion, surfaces); the producer wiring is a downstream task within that milestone. M8 kickoff verified M6 / M6.5 / M7 did not land `lib/operatorLog/`, and M8 chose not to introduce the sink because §13.2.3 report-pipeline lease/recovery work is already the milestone's convergence risk; landing operator-log storage plus redeem-link producers would double the review surface. When picking up: add emit calls in `app/api/auth/redeem-link/route.ts` for every producer code listed above, with tests for replay, key rotation, nonce expiry, no-crew/version/revocation outcomes, and `ADMIN_SESSION_LOOKUP_FAILED` infrastructure paths.
...
**Suggested home:** UNSCHEDULED post-v1 ops-hardening milestone (informally referenced as "M11" but not yet planned — see DEFERRED.md header "Note on milestone numbering"). Picking this up requires first scoping + planning the ops-hardening milestone (operator-log sink design, schema, ingestion, surfaces); the producer wiring is a downstream task within that milestone. M8 kickoff verified M6 / M6.5 / M7 did not land `lib/operatorLog/`, and M8 chose not to introduce the sink because §13.2.3 report-pipeline lease/recovery work is already the milestone's convergence risk; landing operator-log storage plus sign-out producers would double the review surface. When picking up: add emit calls in `app/auth/sign-out/route.ts` for the two failure branches (link_sessions delete error, Supabase signOut error), with regression tests asserting an operator-log entry is written before the 500 response.
