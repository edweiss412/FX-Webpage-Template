...
**Note on milestone numbering:** the planned milestone set is M0-M10 + X.\* (cross-cutting tasks in `11-cross-cutting.md` — the "11" is the file number, NOT an M11 milestone). There is no M11, M12, or beyond in the canonical plan. Items currently routed to "M11 ops-hardening" or "M11+ post-v1" are routed to an UNSCHEDULED future milestone bucket that has not been planned. When picking up any such item, the first step is "scope and plan the milestone," not "implement the deferral." This is itself a planned-future-work deferral (the planning is the blocker), but the wording should be explicit so the deferral doesn't become permanent shelving.
