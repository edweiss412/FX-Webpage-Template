...
### M9-D-C4-1 — `useFormStatus` hardening for Resolve failure path

**Source:** M9 C4 R3 adversarial review (Codex), 2026-05-15 — MEDIUM finding from APPROVE verdict.
**Description:** `components/admin/ResolveAlertButton.tsx:74-112` — `onConfirmClick` unconditionally sets `setUi("resolving")` which disables both Confirm and Cancel buttons. This assumes the parent Server Action always completes by revalidating + remounting the banner. On the happy path that's true (`resolveAdminAlertFormAction` calls `revalidatePath`). On a failure path (Supabase / RLS / network error), the action returns without revalidation and Doug is left staring at a disabled "Resolving…" control with no retry or cancel until reload. Suggested fix: derive disabled + label from `useFormStatus()` instead of local `ui === 'resolving'` state. Add a failure-path test that simulates the action returning without revalidation and asserts the control becomes retryable.
