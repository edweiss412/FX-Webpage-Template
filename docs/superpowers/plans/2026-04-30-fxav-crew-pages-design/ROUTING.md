...
When a milestone splits this way, the handoff doc (per `HANDOFF-TEMPLATE.md`) lists which tasks belong to which implementer. Don't co-mingle UI and backend work in a single subagent dispatch — keep them separated so each runs in its specialist's harness with the correct skill stack (**impeccable v3** for Opus UI work — superseded the legacy `frontend-design` skill; AGENTS.md discipline for Codex backend work).

The cross-model adversarial review still pairs across milestones, not across split-tasks: M5's reviewer is the opposing harness for whichever side ran _more_ of the milestone's task count.
...
For frontend / UI-design work the benchmark gives no direct evidence (both repos were libraries, not UIs). Default to Opus + the project's **impeccable v3** skill (canonical UI workflow with its own preflight gates: PRODUCT.md → DESIGN.md → register identification → preflight signal). Impeccable v3 superseded the legacy `frontend-design` skill — do NOT load `frontend-design` for new UI work.
...
| **M4 — Crew page tiles** (Tasks 4.1–4.16)                    | Frontend / mixed                    | Opus 4.7 / Claude Code                                                     | GPT-5.5 / Codex        | Each tile (Lodging, Venue, Schedule, Audio/Video/Lighting Scope, Crew, Contacts, Transport, etc.) is largely local. Plus mandatory **impeccable v3** skill (canonical UI workflow) + dimensional-invariant Playwright assertions cover the integration risk. **Task 4.1 (DESIGN.md token extraction) is the gate** — assign explicitly to Opus + `/impeccable shape`. |
...
| **M9 — Stale-data UX, polish** (Tasks 9.1–9.4)               | Zod-shaped + UI                     | Opus 4.7 / Claude Code                                                     | GPT-5.5 / Codex        | All-Opus: per-task UI polish, narrow scope. Mandatory **impeccable v3** subcommands as appropriate (`/impeccable polish`, `/impeccable harden`, `/impeccable shape` for any reorder).                                                                                                                                                  |
...
- **Skill loadout per task.** Continues to follow CLAUDE.md (this session) / AGENTS.md (Codex sessions) and the skill `description:` matchers — **impeccable v3** (canonical UI workflow; superseded `frontend-design`), `subagent-driven-development`, etc. dispatch as usual.
