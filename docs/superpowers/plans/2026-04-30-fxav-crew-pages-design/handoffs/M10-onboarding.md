...
- [ ] `pnpm test:e2e --project=mobile-safari` exits 0.
- [ ] `pnpm test:e2e --project=desktop-chromium tests/e2e/admin-phase2-surfaces.spec.ts` exits 0 (close-out R1 F3: original reference was to `admin-dashboard.spec.ts` which was never authored — Phase 2 §B shipped `admin-phase2-surfaces.spec.ts` covering Dashboard / ActiveShowsPanel / PendingPanel / AdminAlertsBanner / re-entry surfaces; full DB-state e2e scenarios — 24h auto-rotate, multi-batch finalize re-entry, race-row re-Apply — were intentionally deferred per the convergence-log Phase 2 entry).
