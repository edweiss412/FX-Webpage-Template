...
- `purgeAndRotateIfStale` / `purgeAndRotateOnboardingSession` / `cleanupAbandonedFinalize` / `retrySingleFile` — exported from `lib/onboarding/sessionLifecycle.ts` (the first three) and `lib/sync/retrySingleFile.ts` (the last). Shapes per Task 10.1 §A + Task 10.4 §A above.
...
- **2026-05-17, R0 (handoff doc only):** Codex CLI targeted critique on §0 (split + Pin-stops) only. Verdict: needs-attention. **3 HIGH findings** resolved before kickoff:
