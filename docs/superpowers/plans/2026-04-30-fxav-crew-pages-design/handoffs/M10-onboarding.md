...
- [x] **No raw error codes in user-visible UI** (§1.5). **FULLY ACTIVE for §B.** The wizard is a high-stakes multi-step surface; spec §9.0 governs every step's microcopy. Every error rendering routes through `lib/messages/lookup.ts`. New ONBOARDING_*/WIZARD_*/FINALIZE_*/CLEANUP_*/PENDING_INGESTION_*/ALERT_*/LIVE_ROW_*/DRIVE_FETCH_*/FOLDER_*/OPERATOR_ERROR_* codes land in `lib/messages/catalog.ts` in the same commit that emits them. Static-grep regression at M10 close (expanded after Codex R4 finding B1): `! rg "(WIZARD_|ONBOARDING_|FINALIZE_|CLEANUP_|STAGED_PARSE_|PENDING_INGESTION_|ALERT_REQUIRES_|ADMIN_ALERT_|LIVE_ROW_|DRIVE_FETCH_|FOLDER_|OPERATOR_ERROR_|SLUG_COLLISION_|CONCURRENT_SYNC_|LOCK_OWNERSHIP_|MISSING_PENDING_INGESTION_|STALE_DISCARD_|WIZARD_REVIEWER_CHOICES_)[A-Z_]+" components/admin app/admin | rg -v "messageFor\\(|catalog\\.ts|test|spec"` returns zero. **Or** (preferred — replaces the prefix-grep with structural coverage per Codex R4 B1 recommendation): a Vitest meta-test that derives the prefix list from `lib/messages/catalog.ts` keys + asserts no rendered-component file contains a message-code literal outside lookup/catalog/test/spec contexts. §B (or §A whichever lands first) authors this meta-test at `tests/messages/_metaNoRawCodesInUI.test.ts` during M10 if the prefix-grep proves too brittle.
...
- **Cross-model adversarial review (Codex)** — two rounds run so far against `4b3b372..HEAD` (Pin-2 base). Both returned needs-attention; §B-owned findings closed each round, §A-owned findings routed below.
  - **R1** (review-mpbwa1da-x2vj0x, 3m 26s): 1 CRITICAL + 1 HIGH + 1 MEDIUM. **F-Codex-1 CRITICAL** wizard step 2/3 dead-code (Step2Verify / Step3Review / FinalizeButton had no non-test imports) — **§B FIXED at 553869f**. **F-Codex-2 HIGH** Dashboard crew_members error silently swallowed — **§B FIXED at 553869f**. **F-Codex-3 MEDIUM** final_cas_done dispatcher branch returned wizard instead of Dashboard — **§B FIXED at 553869f**.
  - **R2** (review-mpbx7a21-avr2l9, 3m 0s): 1 CRITICAL + 1 HIGH. **F-Codex-R2-2 HIGH** FinalizeButton ignored per_row failures on `batch_complete` responses (only checked `all_batches_complete`) — **§B FIXED in this commit**: filter `per_row` for non-OK codes BEFORE branching on status; same fix to ResumeFinalizeButton; new regression test pins the contract. **F-Codex-R2-1 CRITICAL (§A-OWNED, ROUTED)** `app/api/admin/onboarding/finalize-cas/route.ts:429-435` — when an existing-show shadow update fails Phase D CAS, the route computes `blocked` but still DELETEs all `shows_pending_changes`, publishes wizard shows, promotes the watched folder, and marks the checkpoint `final_cas_done` — making `STAGED_PARSE_OUTDATED_AT_PHASE_D` unrecoverable. §A action required: treat any non-OK shadow result as blocking — return 409 or keep the checkpoint/session active, preserve failed shadow rows, do not delete shadows or mark `final_cas_done` while `blocked.length > 0`. Test coverage: add assertion that a Phase D run with a blocked shadow row leaves `shows_pending_changes` intact, `final_cas_done` UNSET, and `published` unchanged on the blocked rows.
  - Next round (R3) will fire after §A patches finalize-cas; §B's R2 fixes are independent and ship now.
