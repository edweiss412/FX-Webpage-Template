...
**Adversarial reviewer:** Pair-symmetric per ROUTING.md — §A reviewer = Opus 4.7 / Claude Code; §B reviewer = GPT-5.5 / Codex CLI. Whole-milestone reviewer for the close-out APPROVE = GPT-5.5 / Codex CLI (pairs with §B which is the larger surface, mirroring M5/M6/M8 convention).
...
- **Task 10.4 §A — extension to `app/api/admin/onboarding/scan/route.ts`:** persist the per-session scan manifest (one row per Drive item per `(wizard_session_id, drive_file_id)`). The `onboarding_scan_manifest` table already exists (from M2/M6); §A just writes/updates rows per the scan outcome.
...
- **Task 10.10 §B — first-seen staged review surface** (`app/admin/show/staged/[stagedId]/page.tsx`). **DISTINCT from the wizard-scoped re-apply route in Task 10.1 §B** — this is the LIVE first-seen path (scopes `WHERE wizard_session_id IS NULL`); the wizard route is `WHERE wizard_session_id = $sessionId AND wizard_approved = FALSE`. Routing the wrong one is the failure mode plan Task 10.1's "Wizard-scoped per-row re-apply route" subsection warns against. The §A side of Task 10.10 (apply/discard routes for the LIVE partition) also lives in this §B's milestone-mate work; see §A's separate list above.
...
- `purgeAndRotateIfStale` / `purgeAndRotateOnboardingSession` / `cleanupAbandonedFinalize` — exported from `lib/onboarding/sessionLifecycle.ts` with the `{ settings, rotated, suppressed? }` shape per Task 10.1 §A.

After Pin-2 clears, §B's wizard step 2 (Step2Verify), step 3 (Step3Review), finalize buttons (Task 10.1 §B components), wizard-scoped re-apply page (Task 10.1 §B), and the dashboard + per-show parse panel (Tasks 10.6 / 10.7) all unblock in parallel. §A's remaining work after Pin-2 is the `runOnboardingScan` manifest-persistence extension for Task 10.4, plus any admin_alerts catalog rows added during Phase A/B/C/D implementation (these are append-only and don't affect §B's contract surface).
