...
3. **M9-D-9.3-1** (AC-9.2 empty-state reachability e2e spec) — **RE-DEFERRED at M9 final-review R8**. `tests/e2e/empty-state-reachability.spec.ts` exists with all four §8.3 scenarios but is `test.describe.skip()` pending an auth-fixture migration (the spec was written against the retired `?crew=<id>` mock; M5 OAuth gate is now authoritative). Unit + sentinel-hiding meta-tests + manual smoke pin the visibility logic; the e2e gap is screenshot-baseline + actual-DOM-render reachability. Suggested home: earliest of a future M-task that lands signed-in-crew e2e fixtures, OR a dedicated e2e-fixture-migration milestone, OR when a real-world reachability bug forces the issue.
