...
- **Amendment text MUST specify**:
  ...
  - Initial seed: the SQL migration writes `INSERT INTO admin_emails (email, added_by, added_at) VALUES ('dlarson@fxav.net', null, now), ('edweiss412@gmail.com', null, now) ON CONFLICT (email) DO NOTHING` — preserves identical post-migration semantics with zero-downtime cutover.
  - **Bootstrap-from-env contract (corrected at R2 repair, codex finding 4):** SQL migrations can NOT read deploy-time `process.env` directly. The `INITIAL_ADMIN_EMAIL` env var (NEW) is consumed by `supabase/seed.ts` (TypeScript seed step), NOT by the SQL migration. At seed time: if `admin_emails` is empty AND `INITIAL_ADMIN_EMAIL` is set, the TS seed canonicalizes the env var via `lib/email/canonicalize.ts` and INSERTs it. The migration's literal seed (`dlarson, edweiss`) runs only on first migration application; the TS seed is the deploy-time bootstrap path for net-new environments. On existing deployments (where `admin_emails` already contains rows from the migration's literal seed), the TS seed is a no-op. The TS seed step must be idempotent (re-running across deploys doesn't duplicate rows; uses `ON CONFLICT (email) DO NOTHING`).
  ...
  - The `ADMIN_EMAILS` env var listed at §14.3:3290 is **deleted from the spec** (retiring zombie config per AGENTS.md "Flag lifecycle table" guidance). `INITIAL_ADMIN_EMAIL` is added to §14.3 in its place, with an explicit contract row: "Consumed ONCE at deploy time by `supabase/seed.ts` to bootstrap the first admin into the `admin_emails` table. Has no effect after the table is populated. Operator must rotate via `/admin/settings/admins` afterward."
