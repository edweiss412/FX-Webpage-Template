...
| R10 | needs-attention | M1-r10 R9 row lacks SHA; M2-r10 PARSE_ERROR_LAST_GOOD crewFacing drifts from §12.4 (spec apparent typo `_<time>\*.` propagated to catalog per spec-canonical rule) | (in progress) | Two final narrow MEDIUM cleanups. The spec typo at line 2721 (`_<time>\*.` instead of `_<time>_.`) is propagated to the catalog rather than silently fixed; flagged for spec-amendment follow-up. |
