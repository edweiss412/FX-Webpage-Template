...
2. **M9-D-C6c-1** (Pinch-zoom discoverability hint) — **DECLINED with rationale at C6c `/impeccable critique`** (HIGH-1 from the LLM design review). Reviewer recommended a session-scoped one-shot chip ("Pinch to zoom · double-tap to reset") or persistent low-contrast hint. Declined: pinch-zoom is gesture-universal on mobile, the "stuck while zoomed" failure mode is already handled by the Reset chip (which is visible by definition when scale > 1). Adding a persistent hint chip would compete for header chrome on 390px viewports; session-scoped one-shot adds localStorage state machinery + dismiss surface. No user-research signal that discoverability is an actual barrier. Re-open trigger: FXAV venue-floor crew explicitly identifies pinch-discovery friction in a future round. See DEFERRED.md M9-D-C6c-1.
