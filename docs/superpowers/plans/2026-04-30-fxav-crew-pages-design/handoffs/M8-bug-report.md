...
- Operator-log structured sink — M5-D9 / M5-D10 / M5-D11 candidates (see §11(d) below). **Decision required at kickoff:** did M6 land the sink? Grep `lib/operatorLog/` at kickoff. M7 did not ship it. If absent at M8 kickoff, M8 either lands the sink as part of `lib/reports/**`'s `admin_alerts` emission contract OR stubs behind the flag.
...
4. Each round's findings are routed by file path:
   - Backend (`lib/reports/**`, `lib/github/**`, `app/api/report/**`, `app/api/cron/report-reaper/**`, `lib/messages/catalog.ts`, `tests/reports/**`, `tests/github/**`, `scripts/**`, migrations) → Codex via `/codex:rescue --resume-last` (per `feedback_adversarial_review_repair_routing.md` — `--resume-last` continues the review thread, NOT a fresh spawn).
...
**(d) M5-D9 / M5-D10 / M5-D11 — operator-log structured sink.** Three M5 deferrals route the structured operator-log sink to M6 OR M8 — whichever ships the sink first. **M6 did NOT ship the sink** (per `git log lib/operatorLog/` empty); **M7 did NOT ship it either** (no entries in `lib/operatorLog/` after M7 close). **M8 kickoff decision:** does M8 land the sink as part of `lib/reports/**`'s `admin_alerts` emission path, or stub behind the flag?

> **Recommended disposition:** Grep `lib/operatorLog/` at M8 kickoff. If absent, **decide explicitly at kickoff** whether M8 owns the sink:
>
> - **Option A — M8 lands the sink** alongside `admin_alerts.upsert` (`lib/operatorLog/emit.ts` writing to a structured table `operator_log` with `code`, `payload`, `severity`, `raised_at`). M5-D9 / M5-D10 / M5-D11 close at M8 with `app/auth/callback/route.ts`, `app/api/auth/redeem-link/route.ts`, `app/auth/sign-out/route.ts` emit-call wiring.
> - **Option B — defer to M9 / M10 / M11.** M8 ships only the `admin_alerts` emissions (per spec §13.2.3 + plan); M5-D9..D11 stay deferred. Update `DEFERRED.md` with the new home.
>
> The M8 plan as written does NOT explicitly require the operator-log sink — it focuses on `admin_alerts` emissions. Option B is the lower-scope choice and matches the plan literally; Option A is the broader-scope choice that fulfills the M5 deferrals. Surface the decision to the orchestrator at kickoff; do not silently land the sink without confirmation.
