...
> **Mandatory kickoff decision (record the chosen option in this section before Task 8.1 starts):**
>
> - **Option A — M8 lands the sink and resolves M5-D9 / M5-D10 / M5-D11.** Ship `lib/operatorLog/emit.ts` alongside `admin_alerts.upsert` (writing to a structured `operator_log` table with `code`, `payload`, `severity`, `raised_at`). Wire emit-calls into `app/auth/callback/route.ts` (OAUTH_REDIRECT_INVALID, OAUTH_STATE_INVALID), `app/api/auth/redeem-link/route.ts` (the 10 producer codes enumerated in M5-D10), `app/auth/sign-out/route.ts` (the 2 failure branches in M5-D11). Add regression tests asserting an operator-log entry is written before each non-2xx response on those routes. Update `DEFERRED.md`: move M5-D9 / M5-D10 / M5-D11 from "Open" to "Resolved" with the M8 close-out SHA.
> - **Option B — push to a specific later milestone with explicit rationale.** Update `DEFERRED.md` to rename the suggested home (e.g., to M9 polish or a new M11 ops-hardening milestone) and add a one-line "why not M8" justification. The justification CANNOT be "out of scope" alone — every M5-Dn deferral is by definition out of the originating milestone's scope; the rationale must name a concrete constraint (e.g., "M8 spec §13.2.3 makes no reference to the sink; landing it here doubles the M8 review surface and risks the §13.2.3 amendments' convergence loop").
>
> Surface the decision to the orchestrator at kickoff. Record the chosen option HERE inline (overwrite this `>`-block with the decision + date + chosen option) before Task 8.1 starts. The M8 plan as written does NOT require the operator-log sink — it focuses on `admin_alerts` emissions — so Option B is the lower-scope literal-plan-following choice; Option A is the broader-scope discipline-completing choice.
