...
- `isModalVisible = (s) => deriveScenarioAttention(s).length > 0 || (s.warnings?.length ?? 0) > 0 || s.degraded === true || (s.alerts.length === 0 && s.holds.length === 0);` — the last clause admits the intentional-empty baseline (T2_EMPTY = clean modal). A scenario that DECLARES attention (alerts/holds) yet derives nothing produces an EMPTY modal. 28/45 tier-1 alert codes are `DOUG_EXCLUDED_CODES` cut from the published surface (the real modal cuts them too), so they are excluded with `reason:"cut"` rather than shown blank (founding "no states that don't occur" principle).

- [ ] **Step 1: Failing test** — (a) **synthetic `isModalExpressible` truth table `[plan-R2 §8]`** — 4 minimal `AttentionScenario`s: `{sectionAvailable}`→false; `{crewKeyRendered}`→false; both→false; `undefined`/only-`anchorAvailable`→true (independent of catalog); (b) **`isModalVisible` directly** — a scenario with a surfacing alert → true; a synthetic scenario with only a `DOUG_EXCLUDED_CODES` code → false; a warnings-only scenario → true; a `degraded:true` scenario → true; (c) catalog `[cutaxis-1,2]`: `excluded.filter(e=>e.reason==="structural").map(e=>e.id).sort()` equals `[T2_SECTION_ABSENT, T2_OVERVIEW_ABSENT, T2_CREW_ROW_ABSENT].sort()` AND all three are `isModalVisible(s)===true` (structural-only, no hidden overlap); `excluded.filter(e=>e.reason==="cut").map(e=>e.id).sort()` equals an **exact checked-in `EXPECTED_CUT_IDS` fixture** (the current 28) — a new/removed cut scenario fails the pin, not silently adapts; (d) no tier-3 id in `rendered`; **integration visibility `[cutaxis-4]`** (not a re-call of `isModalVisible`): a warnings-only rendered scenario's `buildScenarioModalData(s).bySection` has a non-empty entry; a degraded rendered scenario's `.alertsDegraded===true`; an alert rendered scenario's `.attentionItems.length>0` — proving no blank rendered modal; (e) every `rendered` scenario's `data` `structuredClone`s clean; (f) `resolveInitialScenario` `[§3.3, §4.4, plan-R2 §9]`: valid scalar → id; unknown → null; excluded (either reason) id → null; tier-3 id → null; empty string → null; `undefined` → null; `[id,"x"]` → id (first wins); `["unknown", id]` → null; `[]` → null.
- [ ] **Step 2: Verify fail.**
...
- **Auth `[§5.2]`:** mint a developer session via the test-only session minter `app/api/test-auth/set-session` in `test.beforeAll` (the `developer-tier.spec.ts:15` pattern — confirmed grounding). Assert the landed URL is `/admin/dev/attention-gallery` (not a sign-in redirect) before any modal assertion.
