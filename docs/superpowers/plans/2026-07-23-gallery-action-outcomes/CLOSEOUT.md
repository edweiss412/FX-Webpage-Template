...
- **R2: dispatched twice (wholediff-r2-out, wholediff-r2b-out) — both `no_verdict` (the same intermittent killed-exec wedge documented in the spec TRIAGE ladder; R1 succeeded between wedges). Closed by self-certify enhanced review:** each R1 finding re-audited against the repaired diff — F1 validator rule + e2e mount click both pin the fix; F2 hold-only regression test pins the alert-only predicate; F3 no-throw + union tests pin the payload guards; F5 marker asserted; F6 recorded. Repair-introduced risk swept: the malformed-skip flag only suppresses reachability WHEN validation is already failing loud (never a silent pass); the alert-only predicate was re-validated against the roster (green). Per the ladder, the next cross-model opportunity (any future Codex round on this surface) re-covers it.
