...
- **AC-9's second property, that no already-held target is re-observed.** Round 1 found this half unproved anywhere: it is invisible to apply counts and to deliverability, and only the observe log sees it. Asserted at EVERY reconcile in the suite, through one shared helper, `expectObservedExactly(log, before, added)`, so the property is covered by where the helper is called rather than by a list someone has to keep current. That phrasing is deliberate: an earlier draft of this bullet claimed every task-3 arrangement checked the log, and whole-diff review round 2 showed the ADDITION paths did not. Four cases retained one target while adding another and asserted only membership, deliverability and apply counts, all three of which are blind to a redundant `observe()`. Plants: re-observe an already-held target (plant-1, which reds the unchanged, swap and removal paths); and re-observe every desired target whenever the set GREW (plant-10, which reds exactly the four addition paths and nothing else).
