...
Tasks 2 and 3 attach Playwright, so the three mandatory declarations. (Round 2 collapsed the
seven-task shape into five and this sentence kept the old numbers; the browser oracles live in
the tasks whose production change makes them red, which is 2 for AC-7 and 3 for AC-7b.)

- **(a) Server boot.** Both specs run against the existing `tests/e2e/_step3ReviewModalHarness.tsx` bundle under the committed Playwright config, `--project=standalone-chromium`, which is the project whose `testMatch` actually collects these
  two specs. V6 above is the record: `desktop-chromium --list` reports `Total: 0 tests in 0 files`
  and `standalone-chromium --list` reports `Total: 44 tests in 1 file`. No new server, no new port.
...
| `tests/e2e/step3-review-modal.interactions.spec.ts` | AC-7b's `load-failed` arm, which needs hydration; authored and green in Task 3 | 3 |
...
One production change, the caption moving out of the box, and everything it turns green. AC-7's
browser oracle lives HERE and not in a later task for the reason round 2 gave: after this change the
box is already the thing under measurement, so an AC-7 authored later could never be red. AC-7b is
NOT authored here — step 4 below gives the arithmetic — and belongs to Task 3, whose ratified copy is
what makes its first clause reachable.
