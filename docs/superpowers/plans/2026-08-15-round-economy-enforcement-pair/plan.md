...
- RED: extend `tests/reviewRounds/filing.test.ts` with cases for: block extent (ids in Judgment text do not land in `mechanizable.citedIds`; ids in a trailing NONCANONICAL bold-run field such as `**Carry-forward:**` do not either — spec R5 finding 2); `isNone` on `none`, `none.`, `none — prose`; NOT-none on any other value; structural `hasDecline` on a `declined: reason` line inside a multi-line block, and `hasDecline: false` on the R5 probe pair (`not declined: …`; a prose mention of `declined: <reason>`); `mechanizable === null` when the marker is absent (including the colon-less `**Mechanizable** —` spelling, spec §3.1/§5.5).
- GREEN: implement per spec §3.1 — block from the marker line to the next line matching `/^\s*\*\*[^*]+:\*\*/` (any bold-run field) or section end; `isNone` = trimmed remainder matches `/^none\b/i`; `hasDecline` = block contains a line matching `/^\s*(?:[-*]\s*|\d+\.\s*)?declined:\s*\S/im`; `mechanizable.citedIds` = `CITED_ID` matches within the block only.
