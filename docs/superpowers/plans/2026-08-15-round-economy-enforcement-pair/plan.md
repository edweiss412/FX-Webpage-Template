...
- **EXTENDS:** `tests/docs/_metaReviewRoundEconomy.test.ts` (new problem-kind fixtures), `tests/reviewRounds/filing.test.ts` (parse contract), `tests/mutation/source/registry.ts` (new `reviewRoundFiling` row), `tests/mutation/guardSurfaces.gate.test.ts` (`EXPECTED_LEDGER_KINDS` entry — its key-equality assertion fails a newly enrolled surface by default, plan R2 finding 2), `tests/mutation/_metaPremiseContract.test.ts` (`EXPECTED_ENV_TOUCHING` row for the newly enrolled suite; update the meta-test's own row counts in the same commit if the new fixture cases shift its declared number).
...
<!-- spec-lint: ignore — file created by this plan's implementation; not yet tracked -->
**What is red and why:** the new fixture case (non-none Mechanizable, no id, no decline, non-grandfathered path) asserts `problems` contains kind `mechanizable_untracked`; `checkCorpus` (`lib/reviewRounds/corpus.ts:171`) has no such kind, so the assertion fails. The grandfather module (`lib/reviewRounds/mechanizableGrandfather.ts`) is DATA, not behavior — it is authored WITH the RED step (plan R1 finding 1: a RED that dies on an unresolved import never executes its assertion), so the red= failure is the missing problem kind, never module resolution.

<!-- spec-lint: ignore — file created by this plan's implementation; not yet tracked -->
- RED: extend the meta-test with fixture cases (existing `check(files)` helper, meta-test `check` at line ~97): fail case; block-id pass; outside-block id still fails; `declined:` pass; `none` pass; every spec §6.2 fixture family (R6 duplicate-marker orderings, R7 marker-line decline + stale-none accepting fixture, R8 non-rendered matrix + list-marker pair, R9 AST-visible-field malformed forms + heading leak pair, R10 decoded-id `unresolved_id` fixtures, R11 strikethrough retractions, R12 nested-marker malformed across five forms — each nested-marker fixture ALSO carries a conforming top-level `**Judgment:**` line so the legacy raw `hasDisposition` path passes and only the nesting check can produce the failure, and each asserts the nesting-specific message, per plan R1 finding 7); grandfathered pass — plant a filing at a REAL grandfathered path (`refactor/classname-array-join-cn/61281c23e8ce`) with violating content and assert clean, with a `premiseHolds` (from `tests/_shared/premise.ts`) immediately above asserting `MECHANIZABLE_GRANDFATHERED.has("docs/review-rounds/refactor/classname-array-join-cn/61281c23e8ce.md")` — the exemption case discriminates only while the frozen set actually contains the planted path.
<!-- spec-lint: ignore — file created by this plan's implementation; not yet tracked -->
- GREEN: add `lib/reviewRounds/mechanizableGrandfather.ts` (frozen 55-path set generated from the live corpus, header contract per spec §3.3) and the `mechanizable_untracked` check in `checkCorpus` per spec §3.2. The live-corpus case (meta-test line ~649) must stay green with zero content edits to any existing filing.
...
<!-- spec-lint: ignore — file created by this plan's implementation; not yet tracked -->
- RED: author `tests/codexGuard/guardSurfaceGate.test.ts` on the existing harness (`mkRun` writes `run.briefPath`; the test overwrites it per case, writes a verdict scenario via `writeScenario(run, …)` — the fake codex reads the scenario file before recording a call (`tests/codexGuard/fixtures/fake-codex.mjs`), so accepting cases need one (plan R1 finding 2) — then `runGuard(run, ["--stage", "diff", "--round", "1"])`): one case per AC-A1..A6 including the spec §6.1 probe-pair fixtures (`mixed_missing_score`, `two_enrolled_one_score`, `nonempty_survivor_set`) AND the two remaining §6.1 discriminators — a score line lacking the marker-adjacent fraction (`MUTATION SCORE: pending, 0 unaccepted survivors`) and one lacking the zero-unaccepted-survivors phrase (`MUTATION SCORE: 1/1`), each exit 2 (plan R2 finding 1) — asserting exit code and (for AC-A1/A6) that stderr enumerates every nonconforming `GUARD SURFACE:` line. Accepting cases assert the dispatch reached the fake codex (`readCalls(run)` non-empty); EVERY exit-2 case asserts `readCalls(run)` is EMPTY — rejection strictly precedes any child dispatch, pinning spec §2.2's before-any-tokens-burn contract executably (plan R1 finding 8).
...
Add the `reviewRoundFiling` row (`sourcePath: "lib/reviewRounds/filing.ts"`, `suitePaths: ["tests/reviewRounds/filing.test.ts", "tests/docs/_metaReviewRoundEconomy.test.ts"]`, `operators: [...OPERATOR_NAMES]`, `scoreFloor: 1`, a `control` mutant flipping the Mechanizable block's `isNone` regex bound or the decline test) + `EXPECTED_ENV_TOUCHING["tests/reviewRounds/filing.test.ts"] = 0` in `tests/mutation/_metaPremiseContract.test.ts` + the matching `EXPECTED_LEDGER_KINDS` entry in `tests/mutation/guardSurfaces.gate.test.ts`, whose key-equality assertion fails every new surface by default (plan R1 finding 3). Run the SCOPED gate under the heavy wrapper — `python3 scripts/with-heavy-slot.py -- pnpm exec vitest run --project mutation tests/mutation/guardSurfaces.gate.test.ts -t reviewRound` — and disposition every survivor (kill by strengthening the suite, or an `accepted` row with `kind` + reason per the registry's existing rows). AC-B6's criterion is the three reviewRound surfaces green at scoreFloor with 0 unaccepted survivors (plan R1 finding 4: the WHOLE gate is not attainable in-arc — this arc's baseline run measured 78/79 with the sole failure on `interactionTimingScan`, an unrelated surface with zero source diffs on this branch; verify that failure reproduces at the merge base per the pre-existing-failures discipline, record the result in the PR body, and leave the surface to its owning arc — class-sweep exception (c)). The PR's own CI mutation-harness job remains the whole-gate authority. Score + unaccepted-survivor set go verbatim into the round-1 diff review brief (spec §6.3).

## Task 5 (procedural): backfill dispositions

Five `BL-` rows in `BACKLOG.md`, each at the ledger filing bar with `**Reachability:**` citing the originating filing (probe evidence already recorded there): candidates 1, 3, 4, 5, 6 of spec §4. Candidate 2 is DECLINED — probe (this arc, 2026-08-15): `tests/ci/_metaSpecRegistration.test.ts` "spec registration detector (spec §3.1)" already asserts every test-shaped file under `tests/e2e` is resolved by some config or dark-allowlisted with a backlog ref, which is the filing candidate's exact ask; the decline is recorded in the plan closeout AND in the graduation note written by Task 7's archive step (plan R1 finding 9: graduation is an explicit step, not a conditional reference). Run `pnpm ledger:mass` / the ledger meta-tests (`pnpm exec vitest run tests/docs/_metaLedgerInProgress.test.ts tests/docs/_metaReviewRoundEconomy.test.ts`) to confirm the new rows parse and resolve.
...
<!-- spec-lint: ignore — file created by this plan's implementation; not yet tracked -->
Ordered so review covers what merges (plan R1 finding 10): (1) full local gates under the heavy wrapper (`python3 scripts/with-heavy-slot.py -- pnpm test:fast`, then `pnpm typecheck`, `pnpm exec eslint .`, `pnpm format:check` unwrapped); (2) the FINAL-SHAPE commit — graduate BOTH source rows (`BL-CODEX-GUARD-ENROLMENT-PRECEDES-DISPATCH`, `BL-FILING-MECHANIZABLE-LEDGER-PARITY`) to `BACKLOG-archive.md` with graduation notes including the candidate-2 decline, remove the in-progress markers (invariant 12, plan R1 finding 9), and write the arc's own review-round filing (`docs/review-rounds/chore/round-economy-enforcement-pair/fafa354accf0.md` — a NEW filing subject to this arc's own Half B gate); (3) whole-diff codex review to APPROVE pinned to that head (round-1 diff brief carries the §6.3 per-line `GUARD SURFACE:` block — the arc dogfoods its own gate); (4) any post-review repair re-lands step 2's content as the new last commit and the final round re-examines the new head; (5) push, real CI green, `gh pr merge --merge`, fast-forward main to `0  0`, clear pane/agent labels, `CronDelete` the nudge.
