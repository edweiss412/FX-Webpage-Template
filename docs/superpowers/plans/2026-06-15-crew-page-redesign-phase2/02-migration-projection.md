...
- EDIT `lib/sync/runScheduledCronSync.ts` for the **FIX-3 dual-channel sync_log wiring** (D-7): the applied `ProcessOneFileResult` shape + the `result` built at `:2469-2474` (carry `parseWarnings` from `phase2.parseWarnings`); `emitSuccessfulPhase2Tail` (`:1724-1751`) — pass the warnings into the `logSync` call at `:1750` (the applied/success row logs HERE, NOT at `:2309` which is the SKIP branch); the `logSync` builder (`:1608-1623` — accept + set `entry.parseWarnings` for the applied outcome only); the `SyncLogEntry` type (`:187-192`, add optional `parseWarnings`); the `insertSyncLog` impl (`:794-808` — union `entry.parseWarnings` into the `$5` `parse_warnings` array). See the FIX-3 re-analysis below for the exact cross-boundary thread + escalation gate.
- NEW `tests/sync/runOfShowConfirmedReplace.test.ts` (apply-core: `shows_internal.parse_warnings` + the R6 live-snapshot plumbing).
- NEW `tests/sync/runOfShowSyncLogChannel.test.ts` (the D-7 `sync_log` channel — driven through `emitSuccessfulPhase2Tail`/`processOneFile`, the REAL logging surface; + a focused `insertSyncLog` structural pin). See the retargeted tests below.
...
**Correct wiring (the full cross-boundary thread — additive, applied-outcome only):**
  ...
  2. **Thread through the tail:** carry `phase2.parseWarnings` onto the `result` object built at `:2469-2474` (e.g. `if (phase2.parseWarnings) result.parseWarnings = phase2.parseWarnings;` — extend the applied `ProcessOneFileResult` shape with an optional `parseWarnings`), then in `emitSuccessfulPhase2Tail` pass it to `logSync` — change the `:1750` call to `logSync(args.deps, args.driveFileId, args.result, undefined, args.result.parseWarnings)` (or read from `args.parseResult.warnings` if step 1a keeps the tail's `pipeline.parseResult` in sync — but DO NOT rely on that given the rebind; thread it through `result`).
  ...
  **Scope caution + escalation gate (flag in the commit body; this is the 3rd vector touch).** `Phase2Result`, `ProcessOneFileResult`, `SyncLogEntry`, `logSync`, `emitSuccessfulPhase2Tail`, `insertSyncLog` are shared cron/push/manual plumbing. The thread MUST be additive (new optional fields, populated only on the applied outcome) and MUST NOT alter skip/error/stale/stage shapes. **If carrying `parseWarnings` through `Phase2Result` → `ProcessOneFileResult` → the tail proves to widen the blast radius materially (e.g. forces signature churn across push/manual callers of `emitSuccessfulPhase2Tail`/`processOneFile`), STOP and escalate to the orchestrator** — `shows_internal.parse_warnings` already carries the warning (the primary admin observability surface, rendered on `/admin/dev`), so this `sync_log.parse_warnings` channel is the SECONDARY D-7 channel. The fallback, if the cross-boundary thread is deemed too invasive for Phase 2, is to **explicitly scope `sync_log.parse_warnings` to a follow-up** (DEFERRED.md, citing D-7) rather than ship a half-wired thread — but the default is to wire it per steps 1-4.
...
- [ ] **D-7 sync_log channel (R7):** `AGENDA_DAY_EMPTIED` reaches `sync_log.parse_warnings` on the applied-success path via the cross-boundary thread `Phase2Result.applied.parseWarnings` → `ProcessOneFileResult`/`result` (`:2469-2474`) → `emitSuccessfulPhase2Tail`→`logSync` (`:1750`) → `insertSyncLog` `$5` union (`:794-808`). The PART-C test drives `emitSuccessfulPhase2Tail` (the REAL logging surface, NOT the apply core), the `insertSyncLog` structural pin is green, the apply-core test is RELABELED as CHANNEL-1-only, and both PART-C negative-regression checks were run. NO warning wiring at `:2309` (the SKIP branch). _(If the cross-boundary thread was escalated + scoped to DEFERRED.md per the escalation gate, this item reads "sync_log channel DEFERRED — shows_internal channel ships; DEFERRED.md entry cites D-7" instead.)_
