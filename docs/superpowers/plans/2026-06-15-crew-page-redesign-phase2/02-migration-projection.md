...
- EDIT `lib/sync/applyStaged.ts` (R16 — 3rd `emitSuccessfulPhase2Tail` caller): the inline applied result passed to the tail at `:1279` (`result: { outcome: "applied", showId: coreResult.showId }`) must add `parseWarnings` (sourced from the staged-apply's `coreResult`/`runPhase2` warnings). With the required field, `tsc` FORCES it. Verify the staged path carries the apply-time `parseResult.warnings` out of its `applyStagedCore`/`runPhase2` to `coreResult`; if it does not surface them, thread `parseWarnings` onto `coreResult` (the staged analogue of `Phase2Result.applied.parseWarnings`) — state what you found in the commit body.
- NEW `tests/sync/runOfShowConfirmedReplace.test.ts` (apply-core: `shows_internal.parse_warnings` + the R6 live-snapshot plumbing).
- NEW `tests/sync/runOfShowSyncLogChannel.test.ts` (the D-7 `sync_log` channel — driven through `emitSuccessfulPhase2Tail`/`processOneFile`, the REAL logging surface; + a focused `insertSyncLog` structural pin; **+ the R16 manual-first-seen-path test**). See the retargeted tests below.
...
**R16 caller-surface — `emitSuccessfulPhase2Tail` has THREE applied-result callers; ship a tsc-enforced structural defense, not a 4th point-add (4th touch of this vector — R3/R6/R7/R16).** The R7 thread wired only the cron caller; the tail is shared. **Verified callers** (each builds its OWN applied `ProcessOneFileResult` that reaches the tail):
  ...
  3. `applyStaged.ts:1279` (staged apply, first-published path) — passes inline `result: { outcome: "applied", showId: coreResult.showId }`, no `parseWarnings` today.

  **Structural choice = REQUIRED field (the robust default; chosen over centralizing).** Making `parseWarnings` REQUIRED on the applied `ProcessOneFileResult` variant means a caller that omits it FAILS `pnpm typecheck` — so all three current callers MUST be fixed in this milestone AND a future 4th caller cannot silently drop the sync_log channel. (Centralizing inside `emitSuccessfulPhase2Tail`/`logSync` from a single shared source was the alternative, but the tail receives `parseResult` as a *separate* arg from `result` and — per the `runPhase2` rebind blocker above — the warnings the applied result must carry are NOT reliably on the tail's `parseResult`, so there is no single trustworthy shared source inside the tail; the required-field-on-result approach is the one tsc can actually enforce.) The three callers each set `parseWarnings` from their own apply outcome (`phase2.parseWarnings ?? []` for cron/manual; the staged `coreResult` warnings for `applyStaged`). **This structural defense ships in THIS round's repair commit** (per the project's structural-defense-calibration rule — do not wait for another adversarial round to confirm the multi-caller analysis).
...
- [ ] **D-7 sync_log channel (R7) + multi-caller structural defense (R16):** `parseWarnings` is a **REQUIRED** field on the applied `ProcessOneFileResult` variant (`:165-170`) — `pnpm typecheck` PASSES only because ALL THREE `emitSuccessfulPhase2Tail` callers supply it: `runScheduledCronSync.ts:2469-2474` (cron), `runManualStageForFirstSeen.ts:113-116` (manual first-seen), `applyStaged.ts:1279` (staged). `AGENDA_DAY_EMPTIED` reaches `sync_log.parse_warnings` via the thread → `logSync` (`:1750`) → `insertSyncLog` `$5` union (`:794-808`). The PART-C test drives `emitSuccessfulPhase2Tail` (the REAL logging surface), the R16 manual-first-seen-path test asserts caller #2 threads it, the `insertSyncLog` structural pin is green, the apply-core test is RELABELED CHANNEL-1-only, and all negative-regression checks were run. A `grep "outcome: \"applied\"" lib/sync` confirms no construction site omits `parseWarnings`. NO warning wiring at `:2309` (the SKIP branch). _(If the required-field change proved too invasive for a non-Phase2 applied construction, this item instead reads "optional `parseWarnings` + CI grep-guard asserting all 3 tail callers set it" per the escalation-gate fallback; or "sync_log channel DEFERRED to DEFERRED.md citing D-7" if the whole thread was scoped out.)_
