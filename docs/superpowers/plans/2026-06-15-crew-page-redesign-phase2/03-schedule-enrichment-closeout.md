...
- **NO other meta-test changes here.** `RPC_GATED_TABLES`, `internal-code-enums`, `validation-schema-parity`, the `failedKeys` 6th-domain test — all owned by §01/§02. The Supabase call-boundary contract for the `run_of_show` read is §02's (per 00-overview's CORRECTION: `_metaInfraContract` is auth-domain-scoped and does NOT cover `lib/data`, so no registration/waiver is needed).
