...
**Execution order is strict** (01 → 02 → 03): the projection field depends on the parser type; the UI depends on the projection field. Each file's tasks are independently red→green→commit. The §03 close-out runs in this exact order (AGENTS.md invariant 8 — the impeccable dual-gate is the pre-review blocker, BEFORE the cross-model review): **self-review → impeccable dual-gate (critique + audit; fix or DEFERRED.md every HIGH/CRITICAL) → cross-model adversarial review (Codex, iterate to APPROVE; any UI-touching fix re-runs impeccable) → real CI green → merge.**
