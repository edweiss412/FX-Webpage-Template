...
function md(opts: {
  ...
  const {
    ...
    formEmail = "", formPhone = "", formAv = "", trailingStrayEmail,
  } = opts;
  const lines: string[] = [];
  if (clientBlock) {
    lines.push(
...
describe("FORM-tab AV contact fallback", () => {
  ...
  it("does not regress the venue contact from the FORM Hotel Contact Information label", () => {
    // VENUE_LABEL_RE already matches "Hotel Contact Information"; verify the AV change is inert here.
    const withVenue = "| Hotel Contact Information | kurt.ashcraft@hyatt.com |\n";
    const contacts = parseContacts(md({ formAv: "chris.mercado@encoreglobal.com" }) + "\n" + withVenue, "v4");
    expect(contacts.some((c) => c.kind === "venue" && c.email === "kurt.ashcraft@hyatt.com")).toBe(true);
  });
});
```
...
describe("FORM-tab client email/phone fallback", () => {
  it("fills client email + phone from the FORM block when INFO cells are empty", () => {
    const { client_contact } = parseClient(
      md({ formEmail: "ashley.morgan@institutionalinvestor.com", formPhone: "8452701900" }),
      "v4",
    );
  ...
  it("keeps the INFO client email when populated (INFO wins)", () => {
    const { client_contact } = parseClient(
      md({ infoEmail: "real@info.com", formEmail: "other@form.com" }),
      "v4",
    );
    expect(client_contact!.email).toBe("real@info.com");
  });
  ...
  it("extracts only the email substring from a wrapped FORM value", () => {
    const { client_contact } = parseClient(
      md({ formEmail: "Ashley Morgan <ashley.morgan@institutionalinvestor.com>" }),
      "v4",
    );
    expect(client_contact!.email).toBe("ashley.morgan@institutionalinvestor.com");
  });

  it("fills only the empty field on partial INFO data", () => {
    // INFO email present, INFO phone empty → phone filled, email kept.
    const a = parseClient(md({ infoEmail: "keep@info.com", formPhone: "8452701900" }), "v4");
    expect(a.client_contact!.email).toBe("keep@info.com");
    expect(a.client_contact!.phone).toBe("8452701900");
    // INFO phone present, INFO email empty → email filled, phone kept.
    const b = parseClient(md({ infoCell: "111-222-3333", formEmail: "fill@form.com" }), "v4");
    expect(b.client_contact!.phone).toBe("111-222-3333");
    expect(b.client_contact!.email).toBe("fill@form.com");
  });
...
- [ ] **Step 1: Write the failing (should-pass) regression test**
...
// inert here — INFO stays authoritative. Values derived from the fixture's own INFO cells.
const RAW = readFileSync("fixtures/shows/raw/2025-10-fixed-income-trading-summit.md", "utf8");

describe("FORM fallback does not alter an INFO-populated show", () => {
  it("keeps INFO-sourced AV contacts (Chris Mercado + Danilo), not the FORM Onsite AV Contact", () => {
    const av = parseContacts(RAW, "v4").filter((c) => c.kind === "in_house_av");
    expect(av.map((c) => c.email)).toEqual([
      "chris.mercado@encoreglobal.com",
      "danilo.scekic@encoreglobal.com",
    ]);
  });

  it("keeps the INFO client email + phone (not overridden by the FORM block)", () => {
    const { client_contact } = parseClient(RAW, "v4");
    expect(client_contact).toMatchObject({
      name: "Ashley Morgan",
      email: "ashley.morgan@institutionalinvestor.com",
      phone: "845-270-1900",
  ...
  it("keeps the INFO venue contact (Kurt Ashcraft with name + phone)", () => {
    const venue = parseContacts(RAW, "v4").find((c) => c.kind === "venue");
    expect(venue).toMatchObject({ name: "Kurt Ashcraft", email: "kurt.ashcraft@hyatt.com" });
  });
});
```
...
Expected: PASS (3 tests). If any fail, the fallback wrongly altered a populated show — STOP and fix Task 1/2.
