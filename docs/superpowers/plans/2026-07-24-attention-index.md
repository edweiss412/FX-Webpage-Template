...
- **testMatch wiring.** `desktop-chromium`'s `testMatch` is an explicit allow-list; the config states that a spec absent from it "runs NOWHERE and silently proves nothing" (`playwright.config.ts:87-88`). **Two** specs this plan touches are CI-dark, not one (plan R9 F1): `attention-pill-focus.spec.ts` (Task 7 disposes of it) and `published-show-attention.spec.ts` (`tests/ci/_metaE2eWorkflowCoverage.test.ts:91` = `UNSEEN`; no workflow invokes it, and Task 5 must therefore run it locally rather than rely on CI). `published-review-modal.layout.spec.ts` and the other `published-review-modal.*` specs ARE wired, via `.github/workflows/published-modal-e2e.yml:146`. No new spec file is created, so no new `testMatch` entry is needed.
...
**Depends on Tasks 3, 8b, and 9** — every assertion edit to `tests/e2e/attention-pill-focus.spec.ts` happens in those tasks (Task 3 repairs the three stale `<a>` sites, 8b adds the geometry block, 9 adds the transition block). **This task makes no further edits to `attention-pill-focus.spec.ts`** — every assertion in it is already repaired by Tasks 3, 8b, and 9. It does add its own structural wiring test (below), which is its failing-test-first step. It is last so the job it switches on carries the complete suite.
...
Lines 19-32 of the since-retired attention-anchor-e2e workflow were the structural template, but **they did not demonstrate every class** — its own live-bundle spec also passes `tsconfig.json` and its path filter omits it too (plan R11 F1). Copy its shape, not its gaps. Any omission leaves the new job dark for changes to that input — which is the precise failure this task exists to remove.
...
- **7 is last** (plan R2 F1, plan R3 F1). It adds no assertion to `attention-pill-focus.spec.ts` — only the workflow, the coverage-registry classification, and its own structural wiring test — and runs after every task that adds to that suite, so CI turns on with complete coverage rather than a partial one.
