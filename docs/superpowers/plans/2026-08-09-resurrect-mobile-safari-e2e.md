...
<!-- task: red=`gh run list --workflow=crew-e2e.yml --branch test/resurrect-mobile-safari-e2e -e workflow_dispatch --json conclusion,status,headSha | HEAD=$(git rev-parse HEAD) jq -e '[.[] | select(.status=="completed")] | .[0:5] | length == 5 and all(.conclusion=="success" and .headSha == env.HEAD)'` ac=AC-2,AC-5,AC-6 -->
