...
4. **Scoped closeout review:** dispatch a tight-scope codex-guard round on the closeout commit's diff alone (docs-only). If it demands repairs, they are docs-only commits followed by re-dispatch of this scoped round — the whole-diff APPROVE from step 2 remains valid for the code tree, and the merged diff is the union both rounds covered.
