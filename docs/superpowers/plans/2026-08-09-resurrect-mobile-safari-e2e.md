...
<!-- task: red=`BAR=$(gh run list --workflow=crew-e2e.yml --branch test/resurrect-mobile-safari-e2e -e workflow_dispatch --json conclusion,status,headSha | jq -re '[.[] | select(.status=="completed")][0:5] | if length == 5 and all(.conclusion=="success") and ([.[].headSha] | unique | length) == 1 then .[0].headSha else halt_error end') && test -z "$(git diff --name-only $BAR..HEAD | grep -vE '^(docs/|BACKLOG|DEFERRED)')"` ac=AC-2,AC-5,AC-6 -->
