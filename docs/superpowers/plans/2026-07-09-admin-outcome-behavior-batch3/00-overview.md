...
- **SQL script (verified from live source — `extractAgendaLease.ts:56-118` + route `:240-455`):**
  ...
  - **begin#3 (tx#2 persist):**
    ...
    10. **TIGHT matcher (Codex plan-R1 HIGH — the loose `/UPDATE…RETURNING true AS ok/` would false-green a regression that strips the lease/active-session fences).** Statement #10's expectation requires the SQL text to contain **all** of: `UPDATE public.pending_syncs`, `staged_id`, `staged_modified_time`, `EXISTS (` + `public.app_settings` + `pending_wizard_session_id` (active-session fence), `EXISTS (` + `public.agenda_extract_leases` + `owner` + `expires_at > now()` (lease-owned guard), and `RETURNING true AS ok`. **AND** value-binds: the fake's tagged-template receives `values` (postgres.js `LeaseTx` = `(strings, ...values)`); the expectation asserts `values` includes the expected `wizardSessionId`, `driveFileId`, `stagedId`, and `owner`. → `[{ok: true}]`. A regression dropping any fence (e.g. the lease-owner EXISTS) or persisting under the wrong owner/session fails to match → RED, so the emit is provably downstream of the full owner-scoped generation-fenced persist.
    11. `/DELETE FROM public\.agenda_extract_leases[\s\S]*owner\s*=/` (releaseExtractLease) — plus value-bind assert `values` includes the expected `owner` (owner-scoped release, not a blanket delete) → `[]`
- **Value-bind rigor** applies also to #5 (INSERT claim binds `owner`) and #8 (`hashtext('show:'||${driveFileId})` — assert the bound `driveFileId`), so the proof pins WHO/WHICH-SHOW, not just statement shape.
...
- **Verify:** green; teeth-check — remove statement #10 (UPDATE) from the script → success leg throws "unexpected/unconsumed" → RED; restore. Drop the `sql` inject → poison ECONNREFUSED → RED; restore. **`@/lib/log` unmocked.**
...
- **success:** `serviceRoleClientImpl.current = () => makeClient({from:{data:{drive_file_id:"df-1"},error:null}})` (`.maybeSingle()`); `runManualSyncForShowMock → {outcome:"applied", showId:"show-1"}` → emits `SHOW_SYNCED_MANUAL` (`route.ts:133`).
