...
- **SQL script (verified from live source — `extractAgendaLease.ts:56-118` + route `:240-455`):**
  ...
  - **begin#3 (tx#2 persist):**
    ...
    10. `/UPDATE public\.pending_syncs[\s\S]*RETURNING true AS ok/` → `[{ok: true}]`
    11. `/DELETE FROM public\.agenda_extract_leases[\s\S]*owner\s*=/` (releaseExtractLease) → `[]`
- **Mandatory-consumed set** (end-of-leg assert): #1, #5, #6, #8, #10, #11 (both advisory locks, tx#1a claim INSERT, tx#1b staged read, tx#2 owner-scoped UPDATE, release DELETE).
...
- **failure:** `applyStagedMock` sets `mark.hit=true`, returns `{outcome:"x", code:"PENDING_SYNC_NOT_FOUND"}` → 404. `failureExpect: {status: 404}` (body key is `error`, not `code` → no `bodyCode`).
...
- **failure:** `runManualSyncForShowMock` sets `mark.hit=true`, returns `{outcome:"blocked", code:FINALIZE_OWNED_SHOW}` (real const via spread) → 409. `failureExpect: {status: 409}`.
...
- **failure:** `repairSnapshotRollbackMock` sets `mark.hit=true`, returns `{outcome:"not_stuck"}` → 409. `failureExpect: {status: 409}` (body key `error`).
...
- **failure:** `discardStagedMock` sets `mark.hit=true`, returns `{outcome:"x", code:"PENDING_SYNC_NOT_FOUND"}` → 404. `failureExpect: {status: 404}`.
