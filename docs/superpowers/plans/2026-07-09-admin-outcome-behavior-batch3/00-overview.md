...
- **Verify:** green; teeth-checks (each must turn the row RED, then restore): (a) remove statement #10 from the script → "unconsumed expectation"; (b) drop the `sql` inject → poison ECONNREFUSED; (c) **wrong-row carry** — make #10's staged read (#6) return a DIFFERENT `wizardSessionId` than #5's claim so the carry assertion sees a claim-for-A / persist-for-B mismatch → RED; (d) **positional** — swap #10's v1↔v6 expected binds (primary fence vs lease EXISTS) → deep-equal fails even though `.includes` would pass. **`@/lib/log` unmocked.**
