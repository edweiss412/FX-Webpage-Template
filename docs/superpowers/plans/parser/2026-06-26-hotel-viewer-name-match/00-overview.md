...
Spec: `docs/superpowers/specs/parser/2026-06-26-hotel-viewer-name-match.md` (Codex-APPROVED). User-ratified: matcher = LENIENT (surname-only multi-token), slash-split INCLUDED, ship autonomously. UX-not-security per owner determination (`PRODUCT.md:69-73`, master spec `:7-10`).
...
- **Test first** (`tests/data/nameMatch.test.ts`): the §1 oracle matrix as a data table (roster, guest, expected). Rows: every oracle pair (east-coast Doug Larson↔Doug, Carl Fenton↔Carl, Eric Weiss↔Eric W; ria Doug Larson↔Doug, Eric Weiss↔Eric; rpas Doug Larson↔Douglas Larson, John Carleo↔John Carleo, Eric Weiss↔Eric Weiss; consultants Alex Rodrigues↔Alexandre Rodrigues, John Clark↔John Clark; fixed-income DJ Johnson↔David Johnson, Jeffrey Justice↔Jeffrey Justice; fintech John Carleo, Eric Weiss). **Over-match exclusions** (false): Eric Carroll↮Eric Weiss, Eric Weiss↮Eric Carroll, Calvin Saller↮Carlos Pineda, John Carleo↮Carlos Pineda. **Nickname/accent:** Bill Werner↔William Werner, Bill Werner↔William Werner Jr, José Núñez (precomposed)↔Jose Nunez↔José Núñez (decomposed via `́` combining acute). **Legacy `/`-merged persisted row:** `namesRefer("David Johnson / Jeffrey Justice","DJ Johnson")===true`, `…↔"Jeffrey Justice"===true`, `…↔"Eric Weiss"===false`. **Edges:** ""↮anything, "   "↮anything, single-token both sides, hyphenated `Smith-Jones`↔`Smith`. **Symmetry**: assert `namesRefer(a,b)===namesRefer(b,a)` for every row. *Failure mode:* substring-only relapse (fails the broken-show rows), over-broad (fails distinct-surname exclusions), first-name-gate reintroduction (fails Bill↔William), OR token-merge of a `/`-row (fails the legacy DJ Johnson row — surname would be `justice`).
- **Impl:** `namesRefer` exactly per spec §3.1 — `refersSingle` (`toks` with NFD+strip-marks+comma→space+suffix-strip; `tokCompat`; single-token → first/last; multi-token → surname-only) wrapped by `namesRefer` which **splits each side on `/` into people** and returns `aSubs.some(pa => bSubs.some(pb => refersSingle(pa, pb)))`. Pure, no deps.
...
- **Test first** (`tests/data/hotelVisibility.test.ts`):
  ...
  - (d) **Seeded `getShowForViewer` regression (truest proof, Codex plan R2/R3)** — extend the live-DB `tests/data/getShowForViewer.test.ts`: seed a show + crew `Carl Fenton` + a `hotel_reservations` row `names:["Carl"]`; assert `getShowForViewer(showId, {kind:"crew", crewMemberId}).hotelReservations` includes it; a `Eric Weiss` crew does NOT see a `names:["Eric Carroll"]` row. **Legacy-persisted-merged case (Codex plan R3):** seed crew `DJ Johnson` + a `hotel_reservations` row with the UN-split persisted shape `names:["David Johnson / Jeffrey Justice"]`; assert `DJ Johnson` sees it — proving the match-time `/`-split handles already-persisted rows with NO re-ingestion. (Harness verified: `seedShow`/`seedCrew` exist + cleanup cascades `hotel_reservations`; add a direct `hotel_reservations` insert.)
