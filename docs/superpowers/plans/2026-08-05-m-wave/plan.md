...
2. **Unit branches claim FIRST:** create the three unit branches off `origin/main` (`git worktree add -b feat/m-wave-guards ../FX-worktrees/m-wave-guards origin/main`, same for `-parity`, `-ui`). Run each `pnpm ledger:claims --check <unit ids>` FROM THE `FX-worktrees/m-wave` WORKTREE (it has `node_modules`; a fresh sibling has no `tsx` binary until its own install — the check reads origin branches, not the cwd branch, so where it runs is immaterial), EXPECTING exit 1 naming `feat/m-wave` and ONLY it (the planned-handoff signature; any other branch named = real collision, stop). Then in each new worktree: add markers `**Status:** IN PROGRESS · **Branch:** <unit branch>` (plain text edit, no toolchain needed), commit `--no-verify` (pre-install), push `-u`. Entries briefly dual-declared by design.
...
1. RED (executable, mutant-first): plant the gap's mutant in the harness — a harness-only mode that animates the header height from JS (`requestAnimationFrame` loop writing `style.height`, attaching NO CSS transition) on a prop change under a stable key. Run the EXISTING static-harness Part 1+2 cases against it: they stay GREEN (the recorded gap — that green is the red). 
...
3. GREEN: the new spec's assertions (one mounted node owns both heights across the reconcile; height driven by pill presence, not fixed `min-height`) turn RED against the step-1 mutant and GREEN against the real component. Both prior mechanisms survive as separate assertions (Part 1 computed-style transition scan; Part 2 fixed-min-height detection).
...
2. Implement the oracle: `fontBuiltArtifact.test.ts` reads every `*.css` under `FONT_ARTIFACT_DIR` (CI: the `.next` output of dev-gate-e2e's existing build; the test FAILS LOUD if the env var is set but the dir is empty/missing — premise rule) and asserts the shipped font-face universe against the emitted CSS. Unit rows feed the reader scratch directories containing the two fixture stylesheets and assert the reader SEES their font-faces (each escape demonstrably visible to the oracle; no per-fixture Next build needed).
3. Wire: a step in `dev-gate-e2e.yml` after its build: `FONT_ARTIFACT_DIR=.next pnpm vitest run tests/styles/fontBuiltArtifact.test.ts`. Without the env var the artifact-dir case skips-by-premise (named, not silent) so `unit-suite.yml` stays build-free; the fixture unit rows run everywhere.
