...
4. Wire: the new spec file joins `lifecycle-layout-e2e.yml` or `section-header-visual.yml` (whichever workflow already runs the sibling; name the exact file list edit in the commit). Commit `test(e2e): hydrated React reconciliation harness for section-header`.
