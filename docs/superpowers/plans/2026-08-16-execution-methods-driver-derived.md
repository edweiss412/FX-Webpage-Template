...
- EXTENDS: two further HAND-KEYED registries that enrolment drags in, each red until its row lands and neither derivable from the `GuardSurface` row (plan review R2 finding 1; Task 6 steps 2-3 carry the observed reds and the edits): `EXPECTED_ENV_TOUCHING` in `tests/mutation/_metaPremiseContract.test.ts`, which must declare a count for every enrolled suitePath, and `EXPECTED_LEDGER_KINDS` in `tests/mutation/guardSurfaces.gate.test.ts`, which must declare a kind-count row for every `GuardSurface.id`. Listed here because this inventory, not the task body, is the mandatory declaration.
