...
| **Reduced motion plus the non-stream path plus CAS entry reads as hung.** The shimmer is static under `prefers-reduced-motion`; `empty:hidden` removes the phase label until the first event; on the non-stream path `settledTotal` is 0 so no receipt renders; and in the panel composition the trigger is replaced, so there is no spinner. Net: a heading and a motionless bar, indefinitely. | P1 | Emergent from two of this arc's changes plus one pre-existing path, and the repair the reviewer proposes (render all three sub-steps as a static list with the current one marked) is new design on a surface the ruling already settled. The intersection is narrow but reachable, and narrow is not a defence on this screen. |
...
| **The CAS receipt string was never measured at 375px.** The ladder measured the BATCH string; the CAS receipt is about nine characters longer, and its row has the same unshrinking-heading-beside-a-shrink-0-count shape. | P2 | The fleet hold forbids the browser run that would settle it. Owed, and stated as owed rather than assumed safe. |
...
The four P1s are the critique's three (the bar rewinding at the phase boundary, the reduced-motion plus non-stream screen that reads as hung, the reassurance being sighted-only) and the audit's one (the settled count racing the first phase event into a polite region). One was fixed here; the other three are dispositioned above and carried to bl-orch as unfixed peers, because each repair either contradicts a ruled design decision or needs a verification the fleet-wide heavy-phase hold forbade.
