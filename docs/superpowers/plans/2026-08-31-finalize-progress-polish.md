...
**SUPPRESSED means the BOX, never the node, and the word is pinned here so nothing downstream restates it loosely.** Both renderers attach the phase element unconditionally. `empty:hidden` is `display: none`, so the element stays in the DOM and stops occupying space: measured at CAS entry as `display: "none"` with a 0px box on a span `querySelector` still returns. That is what lets `tests/components/admin/finalizeTransitionAudit.test.tsx` reach it by selector at all, which an absent node would make impossible. Three claims in this plan said "no element is rendered" and were wrong; whole-diff R3 caught the last of them.
...
- AC-7 — In the CAS phase the settled batch count persists as a receipt, an indeterminate bar reads as working, and the phase-label element occupies no space while its label is empty (suppressed, per the definition above: the box goes, the node stays).
...
| --- | --- | --- |
| `wizard-step3-footer-center` (`min-h-12 w-full max-w-md`) to `wizard-step3-tracking` | **Corrected 2026-09-01, whole-diff R1 finding 1 — this row was false as written, and measurement is what settled it.** The tracking block DOES exceed the reserve whenever a phase line is present: 54.59px against a 48px `min-h-12`, so the footer is NOT the same height running as idle. Measured at 375px in the real composition: idle 48px, batch 54.59px, CAS before the first phase event 48px (tracking 30.3px, inside the reserve), CAS with a phase 54.59px. What the reserve actually buys is the CAS-entry state, which it absorbs completely; what it does not buy is invariance across idle and running | The `min-h-12` reserve absorbs any tracking block at or under 48px; above that the footer grows with it. Every line inside the block is exactly one line, which is what bounds the growth to a single 54.59px step rather than an unbounded one |
...
| Input | Rendered |
| --- | --- |
| `casPhase: null` | The phase-label element renders and occupies nothing (`element:empty` matches, `empty:hidden` applies, task 7) — suppressed, not absent. The receipt and the bar still render. |
...
| `prefers-reduced-motion: reduce` | Indeterminate track paints a static centred accent hint, no animation, in every engine. **Updated 2026-09-01:** this is now the resting state in every engine at every setting, not only under the reduced-motion query. Chromium and WebKit never ran the animation on the `-webkit-progress-bar` pseudo-element at all, so the base position IS what they paint; it moved from `-40%` (off-canvas, nothing visible) to `50%`. The reduced-motion block still carries `animation: none`, which remains load-bearing for Firefox, the one engine that does animate. |
...
## Task 7 — the phase element occupies no space while its label is empty
...
| **A dip at CAS entry.** `empty:hidden` drops the phase slot before the first event, so the tracking column is 30.3px and then 54.59px once a phase lands. | P3 | **Measured 2026-09-01 rather than estimated, and the estimate was optimistic in one direction and pessimistic in the other.** The TRACKING dip is 24.3px, not 6px, and it is absorbed COMPLETELY rather than largely: 30.3px sits inside the 48px `min-h-12` reserve, so nothing of it reaches the footer. What does reach the footer is the step on either side of it, because 54.59px exceeds the reserve: the footer reads 54.59px in the batch phase, 48px at CAS entry, 54.59px again once a phase lands. So the visible artefact is a 6.59px footer dip and recovery, which is close to the 6px the critique guessed, by a different mechanism than the one it named. Still P3, and still the tension this row was filed to record: §7a is correctly applied, and the arc's height-invariance argument holds across COUNTS and not across phases, which §3.3 now says in those words. |
