...
| A1 | P1 | Audit: in the avatar menu the region unmounts with the popover, so a re-open mounts the container with its text already present and announces nothing to AT. | **FIXED in-branch.** The first full-suite run settled it: the repo already carries a class guard for exactly this shape (`tests/components/_metaLiveRegionMounting.test.ts`, `BL-ANNOUNCE-REGION-UNMOUNT-CLASS`) and it failed on `AvatarMenu.tsx:353`. The live region is now an always-mounted `sr-only` announcer at the component root, outside the popover's conditional render; the in-popover note is plain visible text with no live-region role, so nothing is announced twice. A deferral was drafted and withdrawn — a guard the repo already ships is a fix, not a limit. |
