...
- **Extract `driveErrorStatus` to a leaf module first** (spec §3.1.4): importing it from `lib/drive/fetch.ts` would drag `xlsx` into the hourly watch cron via `synthesizeMarkdownFromXlsx`. Move it, re-export from `fetch.ts` so existing callers are untouched, import the leaf in `watch.ts`.
