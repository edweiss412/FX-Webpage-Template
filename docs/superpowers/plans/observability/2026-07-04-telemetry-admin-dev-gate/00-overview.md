...
**Spec:** `docs/superpowers/specs/observability/2026-07-04-telemetry-rename-admin-dev-gate.md` (canonical; cross-model-APPROVE'd, 7 Codex rounds). Every task cites the spec section it implements. **The spec is canonical** — anywhere the plan and spec disagree, the spec wins (open a question; do not silently "fix").
