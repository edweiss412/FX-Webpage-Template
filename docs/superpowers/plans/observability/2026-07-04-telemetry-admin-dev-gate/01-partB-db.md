...
```bash
git add supabase/migrations/20260704000000_admin_mgmt_requires_developer.sql tests/db/admin-mgmt-requires-developer.test.ts tests/db/admin-emails.test.ts supabase/__generated__/schema-manifest.json docs/superpowers/specs/admin/2026-07-03-developer-tier.md
...
- [ ] **Step 1: Write the test** — model exactly on `tests/db/set-admin-developer-concurrency.test.ts` (the deterministic advisory-lock rendezvous: A holds the `admin_emails` lock ~2s while B passes its PRE-lock check under READ COMMITTED then parks on the lock; when A commits, B's POST-lock re-check re-reads committed state and rejects). Scenario for Part B: seed A and B as active developers (committed). **Session A** demotes B's developer bit via `set_admin_developer_rpc('${b}', false)` (acquires the `admin_emails` advisory lock inside its txn), then `pg_sleep(2)`, `commit`. After a 700ms head start, **Session B** calls `upsert_admin_email_rpc('${target}', null, false)` — B passes its pre-lock developer check (still a developer in its snapshot), blocks on the advisory lock, and after A commits is rejected by the POST-lock re-check (`42501` → `ok:false`). Assert: A → `ok`; B → `ok:false` + `/42501|requires developer/`; and a safety invariant `select count(*) … is_developer` shows ≥1 developer remains. 15000ms per-test timeout (the 2s hold exceeds vitest's 5s default). Clean up seeded rows.
  ...
  **A second case is MANDATORY (Codex plan-R1 HIGH — spec §3.2 pins the post-lock re-check for BOTH RPCs):** the same file MUST also drive B via `revoke_admin_email_rpc('${target}')` (B parks on the lock after passing its pre-lock developer check; A commits B's demotion; B's revoke is rejected by revoke's POST-lock re-check → `ok:false` + `/42501|requires developer/`). Both the upsert case and the revoke case are required; each independently proves its RPC's post-lock guard.

- [ ] **Step 2: Prove fails-first by DELETING the defense (both RPCs, not just reasoning).** For EACH RPC in turn: temporarily remove ONLY that RPC's post-lock re-check block from the migration (the second `if not exists( … is_developer ) then raise … '42501'` immediately after `perform pg_advisory_xact_lock(…)`), re-apply locally (`psql "$TEST_DATABASE_URL" -f …`), run `pnpm vitest run tests/db/admin-mgmt-developer-concurrency.test.ts` → the corresponding case FAILS (B's mutation succeeds with stale auth; `rb.ok` becomes true). (a) Delete `upsert_admin_email_rpc`'s post-lock check → the upsert case fails; restore. (b) Delete `revoke_admin_email_rpc`'s post-lock check → the revoke case fails; restore. Re-apply the fully-restored migration → both cases PASS. Note all four outcomes (upsert-deleted-fails, revoke-deleted-fails, both-restored-pass) in the commit body — this is the anti-tautology proof that a pre-lock-only impl of EITHER RPC cannot escape the race test.
