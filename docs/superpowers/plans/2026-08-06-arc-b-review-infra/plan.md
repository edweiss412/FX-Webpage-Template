...
- Corpus (probe of record, committed in this directory: fence-gate-probe.mjs + fence-gate-probe-2026-08-06.txt): 395 plan files with fences, 5,434 fences, 3,585 module-ish code fences; raw hits UNIMPORTED_IDENTIFIER 325 / DUPLICATE_IMPORT 5 / MANGLED_TEMPLATE 14 / UNCHECKED_INDEX 145 (candidates) / FENCE_EM_DASH 1,378 (all-fence scope; ships code-fence-scoped).
...
1. **G1a — read-core, TDD per rule:** RED — unit suite for `lib/planFences/` with per-rule fixture strings: each of the five rules gets a positive case (the violation fires with file/fence-line/rule named) and boundary negatives (non-code fence skipped by eligibility; unattributed fence still checked per-fence but exempt from DUPLICATE_IMPORT; waivered fence suppressed WITH the waiver's reason surfaced; empty-reason waiver rejected — reusing `lib/specLint` parse semantics, not re-implementing them). The UNCHECKED_INDEX final pattern is fixed HERE: sample the probe's 145 candidates (committed output), classify a ≥20-hit sample as true/false positives, and pin the shipped pattern + named exclusions in the rule header with that sample table in the commit message. GREEN — implement extraction, attribution (spec accept-set: nearest preceding non-blank prose line in a bounded window with exactly one backticked source-path token), the five rules, the known-API registry (closed list). Commit `feat(infra): plan-fence read-core with the five decidable shapes`.
2. **G1b — the gate + baseline:** RED — the meta-test walks BOTH the real plans tree and the planted fixture tree; against the planted tree it must fail naming every planted violation (premise: executes unconditionally, one planted plan per rule); against the real tree, its first run's hit list IS the baseline-generation input. GREEN — commit the generated baseline (rows `{path, fenceLine, rule}`); the meta-test passes baseline rows, fails non-baseline hits, and fails STALE baseline rows (shrink-only: a row whose hit no longer exists must be removed — same contract as the mutation harness's `staleRows`). Record the baseline row count in the task record. Commit `feat(infra): blocking plan-fence meta-test with shrink-only legacy baseline`.
...
1. **G2a — feature probes become REDs:** for each of the six grammar features (fences-relative-indent [already shipped — regression only], indented blocks, list-container stack, block quotes, lazy continuation, HTML blocks), write a CLI-harness fixture asserting the CORRECT classification; run against the SHIPPED recognizer; record which fail (genuine misses — expected: block quotes, dedent-pop, lazy continuation, HTML blocks per the entry's paraphrase — the probe run is the evidence, not the paraphrase). A feature whose fixture already passes is a regression pin, not a miss. Also port the entry's shipped probes as named regression fixtures (indented closers 4/4 shape, indented openers 18/18 shape, nested marker-line 2/2). Commit `test(infra): commonmark grammar fixtures — misses red, shipped fixes pinned`.
2. **G2b — the parser:** replace `stripCodeBlocks` with the vendored block-level pass (inline in `scripts/codex-guard.mjs`; container stack with content-column arithmetic; open-at-EOF strips nothing preserved; callers `parseVerdict`/`parseFindingCount` untouched). All G2a fixtures green; all existing `tests/codexGuard/` suites green. Any of the six features NOT covered lands instead as a documented-limit rewrite WITH its still-red fixture converted to a pinned-limit probe (spec §2.2 disposition menu — both outcomes compliant per feature). Commit `feat(infra): vendor a block-level commonmark pass into codex-guard, retiring the recognizer`.
3. **G2c — contract pins:** the import-surface assertion (codex-guard's import lines are node builtins + named siblings only — a static read of the script's head, failing on any `node_modules` specifier). Commit `test(infra): pin codex-guard's dependency-free import surface`.
