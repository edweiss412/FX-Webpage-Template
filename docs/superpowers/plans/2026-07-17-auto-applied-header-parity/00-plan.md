...
- **Verify:** the new adjacency test GREEN under `--project=help-docs-desktop` (was RED pre-fix) and correctly SKIPPED under `--project=help-docs` (run BOTH projects to confirm the guard, Codex R8); `Dashboard.test.tsx` still green; the deep-link walker suite still green in both projects.
