...
- Registries: `_metaInfraContract` / `_metaMutationSurfaceObservability` do not apply (no Supabase call sites, no mutation surfaces added). The advisory-lock planning contract DOES apply to A5 — see the topology section below (plan R2 F4).

## Advisory-lock holder topology (invariant 2 planning contract, for A5)

A5 rewires DSN resolution for two helpers whose SQL already holds per-show advisory locks; the lock topology is UNTOUCHED and this section pins it so the structural guard's single-holder rule stays satisfied:
...
Mech spec §2.2. Write the guard first: parse `app/globals.css` `@theme` for declared token names EXCLUDING the `--breakpoint-*` namespace (spec §4 limit 6) (premise: ≥30 parsed, via `tests/_shared/premise.ts`); scan tracked `.ts`/`.tsx` under `app/ components/ lib/` for string-literal arrow uses of covered tokens; report file:line; comment-only lines excluded. **Executable self-test fixtures land IN the guard file, in the same commit (plan R1 F4 — the scanner's claims are planted as positive AND negative shapes, since zero live breakpoint arrows exist to exercise the exclusion):** positive fixtures SPAN the scan space (plan R2 F3): covered-token arrows from THREE distinct namespaces (a `--shadow-*`, a `--spacing-*`, a `--color-*` token) each flagged, planted in BOTH a `.ts` and a `.tsx` fixture path so dropping either extension or narrowing the parser to one namespace loses a fixture; a `--breakpoint-sm` arrow → NOT flagged; a non-theme variable arrow → NOT flagged; a comment-only occurrence → NOT flagged; a canonical utility → NOT flagged. Mutation families for this guard, enumerated for its review with their killing fixture: token-parse namespace narrowing (killed by the three-namespace positive set — the ≥30 premise alone would NOT catch it, since 105 declarations survive a broad namespace loss); extension-set narrowing (killed by the .ts+.tsx positive pair); namespace-exclusion removal (killed by the breakpoint negative); comment-stripping removal (killed by the comment-only negative). Observe RED on the live tree (the shadow-arrow census; record it in the commit). Sweep every census hit to its canonical utility; rewrite the `app/globals.css` enforcement claim to the MEASURED mechanism (spec §2.2 part 2). Guard green; `pnpm lint` + `pnpm typecheck` green; `tests/specLint/canonicalTokenIdentity.test.ts` untouched and green. Commit `refactor: ban @theme-token arrow forms via derived guard; canonicalize hits`.
...
UI spec §2.3. RED: jsdom suite — avatar trigger (initials + `avatarColor` swatch, `aria-haspopup`/`aria-expanded`; accessible name CONSTRUCTED per the spec's rule — [name-or-"Crew member" fallback, role if non-empty, "account menu"] joined with ", " (plan R2 F6: role-only yields "Crew member, Lead, account menu", NEVER "Lead, account menu"; both-blank yields "Crew member, account menu") — all four partial-identity cases asserted with those exact strings; both-blank switches the menu to `aria-label="Account menu"` with the header omitted); popover with identity header outside the item list + `aria-labelledby`; theme row `menuitemcheckbox` + `aria-checked` flip without closing (dataset/localStorage handshake asserted); person row = the shipped server-action FORM (hidden `slug`/`shareToken`/`showId` pinned) with `role="menuitem"` submit; full keyboard map; **tap-floor REDs (plan R2 F5): jsdom class assertions that the avatar trigger and BOTH menu rows carry the tap-floor token classes (`min-h-tap-min`/`min-w-tap-min` or `size-tap-min`)**. **Executable transition audit in the same suite (plan R1 F12), inventory table from spec §2.3 included in the task body at execution:** open applies the `duration-fast` enter treatment (transition/animation properties asserted), close applies the reverse, `motion-reduce` emulation renders both instant, and the compound case (theme flip while open, then Escape) closes cleanly with focus returned. **In the SAME commit** (plan R1 F8): rewrite the four identity-chip suites (`IdentityChip.test.tsx`, `identityChipSrSeparator.test.tsx`, `_metaPickerRoleChipContract.test.ts`, `Header.test.tsx` — contracts carried over), re-target `picker-flow.spec.ts`'s closed-menu identity assertions (:151 + siblings) and `crew-page.spec.ts` :1099, and re-capture the six WebPs again (the header changed). Identity-less Header keeps the standalone toggle. Commit `feat(crew-page): header avatar menu — theme switch + person switch`.
...
Red is heading-anchored (`^## <id> — `; bare ids occur in archive prose — plan R2 F1); exits 1 today, 0 after the archive heading lands. Archive both crew-footer entries with markers stripped in the same commit; whole-diff cross-model review to APPROVE; real CI green; `gh pr merge --merge` same turn; ff-sync to `0  0`.
...
UI spec §3. RED: tripwire flips to the BAND assertion (width >0 ∧ ≤60, height 1) at `?step=1`, `?step=2`, AND `?step=3`, each at 390px and 900px (URL step hint via `pickStep`; verify the indicator renders at the higher steps under the suite fixture — if gated blank, seed `maxReachedStep` for the constructed case), **plus the state-color oracle (plan R1 F11, state corrected per plan R2 F2): at `?step=2` — the ONLY step where the two connectors differ (`isDone = n < step`, connectors render for n=1,2: at step 2 connector 1 is done, connector 2 is ahead; at step 3 BOTH are done) — connector 1 computes `background-color` equal to resolved `--color-border-strong` and connector 2 equals resolved `--color-border`, read via `getComputedStyle` against the live stylesheet's token values, the two REQUIRED to differ; at `?step=3` assert both compute border-strong**. Fails against shipped 0-width. Implement: `flex-1 min-w-0` on the StepIndicator nav (sole child — probed). Baseline: `canonical-dimensions.json` updates through its NATIVE numeric path (`lifecycle-layout-e2e.yml:11`; no Docker step). Update the :91-95 tripwire comment + the three rationale-rot sites (`step3-review-page.layout.spec.ts` nav transcription, `tap-target-floor.layout.spec.ts` 0px rationale, `canonicalTokenIdentity.test.ts` browser-proof rationale). Commit `feat(admin): render the wizard step connector hairline`.
