...
1b. **Harness readiness (writing-plans e2e rule, complete; R2 F2/F3 repairs in place):** (a) server boot — the shared `webServer` entry on `E2E_PORT` (`playwright.config.ts` :245-255): CI runs a cold prod `pnpm build && pnpm start -H 127.0.0.1`, local runs `pnpm dev` with `reuseExistingServer`; the case adds nothing to boot config. (b) Readiness gate — `waitForRowHydration` is MODULE-LOCAL to the reopen spec (:46), so the realtime spec gets its OWN copy (replicate the helper into the realtime spec file, or lift it into `tests/e2e/helpers/` and import from both — implementer picks; either way the helper body is the reopen spec's verbatim), awaited before the FIRST click AND again before the aborting same-row re-click; never `networkidle` alone. (c) Harness inheritance — the new test does NOT bypass the file's realtime protections: it reuses the file's own subscription-observed gate, cold-start warm-up, and quiescence helpers exactly as `runScenario` sequences them (same helper calls, not a reimplementation), and sets its own `test.setTimeout(240_000)` like the sibling test — a stimulus fired without the subscription gate can be dropped (the file's own comments document lost first broadcasts). (d) Detach-safety — freshness observation via the file's `page.evaluate`-installed `MutationObserver` with its explicit disconnect teardown (:357-384/:541-545 — the file's live-install pattern, NOT an init script; the new case replicates install-before-stimulus and disconnect-after); no `locator.evaluate` on nodes that can unmount mid-transition; clicks use the file's established locator drives.
