...
1. **RED:** flip the false-loss pin at `tests/sync/capabilityLossReachability.probe.test.ts` :258-263 to the correct expectation — `reported: false`, `reportedFlags` expectation updated to match (the row no longer reports, so the flags field asserts the fixed shape the suite's row type requires; exact literal pinned against the row's `Outcome` type at execution). Run `pnpm vitest run tests/sync/capabilityLossReachability.probe.test.ts` — observe the flipped row FAIL against the unfixed tree (the executable RED), the other three rows still green.
2. **GREEN:** add the retain to the `crew_email` branch of `applyUndoOverrideToMaps`, mirroring the sibling restore branch: `maps.retainRows.set(hold.entity_key, rowFromHeldValue(held));` placed before the branch's `return`. Nothing else changes (arm (c) untouched per spec §1.1 item 1; `protectedNames`/`pinnedIdentity` untouched). Rerun: all four rows green — the tombstone row still asserting `reported: true` is the overreach counterweight. Run the wider sync suites (`pnpm vitest run tests/sync/`) to catch any suite pinning the old false-loss notice output; any such pin updates in this commit with its reason (class-sweep at round 0). Commit `fix(sync): retain the surviving crew_email hold row so a live LEAD is not reported as a capability loss`.
3. Archive the entry (archive RED) with the probe table updated to post-fix behaviour and the M→S resize + reachability history preserved. Commit `docs(backlog): archive BL-CAPABILITY-LOSS-SURVIVING-ROW-FALSE-POSITIVE — retainRows symmetry restored`.
