...
1. **RED:** flip the false-loss pin at `tests/sync/capabilityLossReachability.probe.test.ts` :258-263 to the correct expectation — `reported: false`, `reportedFlags` expectation updated to match (exact literal pinned against the row's `Outcome` type at execution) — AND add the staleness assertion (spec §2.1 / R2, oracle divergence FORCED per R3 F1): seed the live row with an overriding `phone: "555-NEW"` (the testkit default and `heldRow` otherwise SHARE `"555-OLD"` — `_holdAwareTestkit.ts` :31-41, probe :146-155 — which would make the oracle vacuous), state the premise executably (`premiseHolds`: live phone ≠ held phone, immediately above the assertion), then assert via `readCrew` (:203) that the post-apply row carries `"555-NEW"` — a frozen-snapshot retain fails by name. Run `pnpm vitest run tests/sync/capabilityLossReachability.probe.test.ts` — observe the flipped row FAIL against the unfixed tree (the executable RED), the other three rows still green.
