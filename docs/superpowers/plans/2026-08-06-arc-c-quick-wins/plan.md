...
1b. **Harness readiness (writing-plans e2e rule, complete):** (a) server boot — the shared `webServer` entry on `E2E_PORT` (`playwright.config.ts` :245-255): CI runs a cold prod `pnpm build && pnpm start -H 127.0.0.1`, local runs `pnpm dev` with `reuseExistingServer`; the case adds nothing to boot config. (b) Readiness gate — `waitForRowHydration(page, slug)` (the reopen spec's own helper, :46, awaited before every drive at :82/:99) runs before the FIRST click AND again before the aborting same-row re-click; never `networkidle` alone. (c) Detach-safety — freshness observation via the init-script `MutationObserver` recording (synchronous inside the callback); no `locator.evaluate` on nodes that can unmount mid-transition; clicks on the scrim/row use the file's established locator drives.
...
2. Author `closeout.md` in this plan directory IN THE PR (pre-merge, AC-C3's artifact): the `impeccable-gate: N/A — no UI surface` marker line, the observed-RED record reference, and the AC-C1..C3 checklist. Commit `docs(plan): arc C closeout`.
3. Merge `origin/main` (ledger contention: sibling arcs + L-wave units; per-entry resolution, both sides preserved); strip any surviving marker in the last pre-merge commit (both entries archive in Q1/Q2, shedding markers in their moves — terminal check: `grep -c 'Branch:\*\* feat/backlog-quick-wins' BACKLOG.md DEFERRED.md` returns 0). PR (body notes preflight ran and links the observed-RED record); real CI green including `published-modal-e2e.yml`; `gh pr merge --merge` same turn; ff main `0 0`.
