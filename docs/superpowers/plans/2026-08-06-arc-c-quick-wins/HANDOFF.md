...
3. The implementation worktree ALREADY EXISTS with claim markers pushed (`../FX-worktrees/backlog-quick-wins`, branch `feat/backlog-quick-wins`, created by the authoring session per spec §3). Verify the arc C spec/plan PR is MERGED into origin/main (if not, STOP — launched early). In the worktree: `pnpm install && pnpm worktree:link-env && pnpm preflight`, then `git merge origin/main` (arc A just landed; ledger conflicts resolve per-entry, both sides preserved).
...
- Q2 needs the dev server + browsers under `MODAL_REALTIME_E2E=1`; jsdom re-attempts are refuted (spec §1.1 item 3).
...
Q2's timing probes (reduced-motion drive, case placement) are NOT ambiguities — all outcomes are pre-ratified as compliant; run the probe and follow its result. Only a genuinely NEW question stops the pipeline.

## Arc completion

PR merged; `0 0`; both entries archived to `BACKLOG-archive.md`; AC-C1..C3 satisfied; `closeout.md` in this directory with the `impeccable-gate: N/A — no UI surface` marker line and the observed-RED record reference; then gate-check arc B's handoff and transition (or, if arc B's plan has not merged yet, continue per the pane announcements — the authoring session announces each handoff as it merges).
