...
- EXTENDS: `tests/mutation/source/registry.ts` — Task 3 widens the existing `taskContract` row's `suitePaths`; Task 6 adds two rows. Enrolled suites fall under `tests/mutation/_metaPremiseContract.test.ts` automatically.
...
- [ ] **Step 1: Write the failing suite.** Cases (expected values derived from fixture construction): boundary vs substring BOTH directions and PER CONSUMER — for each of window, enclosing-name, whole-file, and relocation, one case where substring semantics flips the result (e.g. window contains only `depsWithStart`, id `deps` → no hit; relocation peer contains only `fooXbar`, id `foo.bar` → no hint) so a consumer that keeps `includes` while others use `idPatterns` fails its own case (AC-3 "in any of the four"); escaping (`foo.bar` NOT matching `fooXbar`; `$var` literal); segment floor (`a.of` contributes no `of` pattern); enclosing-name shapes (`export async function NAME`, `const NAME`, `interface NAME`, SQL `create or replace function public.NAME`, `## Heading`, none-by-line-1 → null); window edges BOTH sides crossed both ways (`start-5` hits / `start-6` misses AND `end+5` hits / `end+6` misses — an asymmetric window bound fails); enclosing rescue (id only in decl name 40 lines up → clean); file-hit → `"unmatched"`; nowhere → `"absent"`; relocation (found in peer, cap at 3, null-lines peer skipped, empty peers → []).
...
- Test: tests/specLint/citationIntentWiring.test.ts, tests/specLint/citationIntentCorpus.test.ts, fixtures under tests/specLint/fixtures/citationIntent/ (in-memory + committed CLI fixture docs)
...
- [ ] **Step 1: Failing suite.** `red-contract` attribute recognized; bare form unchanged; `red-contract=1` and misspellings → `TASK_ENROLL_MALFORMED`; every v2 field combination well-formed via `parseMarker` AND through `checkTaskContract` (no `TASK_MARKER_MALFORMED`); reordered fields → malformed; v1 semantics ON v2 forms — empty v2 `red=` → `TASK_RED_EMPTY`, absent `ac=` → `TASK_AC_MISSING` incl. double-defect precedence (mirror `tests/specLint/taskContract.test.ts:460-462`), v2 `ac=` ids resolve with self-resolution exclusion, v2 markers count for duplicate/cardinality; `parseMarker` unit rows (v1 → nulls; each field captured with its exact column; blank captures returned raw); byte-identity corpus regression: BEFORE editing the grammar, generate and COMMIT a JSON snapshot of `runLint(...)`'s `check: "taskContract"` findings for both fixtures/legacyPlans/ files (shipped code produces it — this step's first commit); the test then asserts deep equality against the snapshot. Coordinate contract on every new-finding assertion.
...
- [ ] **Step 5: Registry: widen `taskContract.suitePaths`** with tests/specLint/taskContractV2Grammar.test.ts (the suite that sees the new grammar and topology — review-R1 F4); run `pnpm heavy pnpm mutation:guards`; re-anchor moved accepted rows 1:1 (operator + `from>to` preserved, registry precedent `tests/mutation/source/registry.ts:170-180`); score ≥0.95, unaccepted empty.
...
- [ ] **Step 4: Green** + wiring extensions: excluded `red-target=` span draws no citation finding and cannot anchor; citation-shaped spans in `red=`/`why=`/gate `cmd=`/gate `probed=` and on spec-kind marker lines still draw `CITATION_FILE_MISSING` (pin review-R3/R4 probe outputs as expected); **real-CLI fixture docs** driven through `tsx scripts/spec-lint.ts` asserting one §4.3 hard code and one gate code appear in the text report under `taskContract:` with exit 1 (review-R1 F8). Run `_metaPureCore`.
...
// scripts/spec-lint.ts -- exported pure helpers
...
- [ ] **Step 4: CLI adapter cases** (committed fixture plans, trivial commands only): `--exec-red` on spec → 2; duplicate → 2; `exit 0` → exit 1 + `RED_ALREADY_GREEN`; `exit 1` → exit 0; `exit 126` / `exit 127` → advisory each; `SPEC_LINT_EXEC_TIMEOUT_SECS=1` + `sleep 5` → `RED_EXEC_TIMEOUT`; env `0`/`-1`/`abc`/empty → exit 2 each; `pwd > "$FIXTURE_SCRATCH/cwd.txt"` proves repo-root cwd; `exit $((125+2))` proves `sh -c`; >200-char stderr before exit 127 → trimmed tail; `kill -TERM $$` → `RED_EXEC_ERROR` naming SIGTERM; a plan whose only live markers sit outside contract regions → nothing executes (spy: zero subprocess calls). Unit-test `classifySpawnResult`: ENOENT, SIGTERM, `{status: 0}`, and the review-R4 hybrid `{status: 0, error: {code: "ETIMEDOUT"}}` → timeout.
...
- [ ] **Step 2: Add both rows; `pnpm heavy pnpm mutation:guards`.** Scores ≥0.95; every survivor killed by a strengthened test (preferred) or argued into `accepted` with its reason row; unaccepted set EMPTY.
