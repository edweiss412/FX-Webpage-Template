...
- EXTENDS: `tests/mutation/source/registry.ts` — Task 3 widens the existing `taskContract` row's `suitePaths`; Task 6 adds two rows. Enrolled suites fall under `tests/mutation/_metaPremiseContract.test.ts`, whose `EXPECTED_ENV_TOUCHING` map (`tests/mutation/_metaPremiseContract.test.ts:31`) exact-matches the registry's suite set (`:92`) — so EVERY enrolment step here also adds a declared env-touching count row: Task 3 declares taskContractV2Grammar (expected 0 — fixture reads are not provenance); Task 6 declares the five other suites — citationIntent 0, citationIntentCorpus 0, redContract 0, redExec 0, and citationIntentWiring at its honest spawn count (its real-CLI cases spawn `tsx` via child_process; the spawning helper goes at MODULE scope, never inside a describe body — the premiseScan nested-helper blindness is filed as BL-PREMISESCAN-NESTED-HELPER-SCOPE and documented at the phantomGapExecuted row).
...
- [ ] **Step 4: CLI adapter cases** (committed fixture plans, trivial commands only): `--exec-red` on spec → 2; duplicate → 2; `exit 0` → exit 1 + `RED_ALREADY_GREEN`; `exit 1` → exit 0; `exit 126` / `exit 127` → advisory each; `SPEC_LINT_EXEC_TIMEOUT_SECS=1` + `sleep 5` → `RED_EXEC_TIMEOUT`; env `0`/`-1`/`abc`/empty → exit 2 each, stderr NAMING `SPEC_LINT_EXEC_TIMEOUT_SECS`, and NOTHING executed (the fixture plan's live command writes a sentinel file; assert it does not exist after the failed invocation); `pwd > "$FIXTURE_SCRATCH/cwd.txt"` proves repo-root cwd; `exit $((125+2))` proves `sh -c`; >200-char stderr before exit 127 → trimmed tail; `kill -TERM $$` → `RED_EXEC_ERROR` naming SIGTERM; a plan whose only live markers sit outside contract regions → nothing executes (spy: zero subprocess calls). Unit-test `classifySpawnResult`: ENOENT, SIGTERM, `{status: 0}`, and the review-R4 hybrid `{status: 0, error: {code: "ETIMEDOUT"}}` → timeout.
