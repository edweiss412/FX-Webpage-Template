...
- [ ] **Step 4: Green** + wiring extensions: excluded `red-target=` span draws no citation finding and cannot anchor; citation-shaped spans in `red=`/`why=`/gate `cmd=`/gate `probed=` and on spec-kind marker lines still draw `CITATION_FILE_MISSING` (pin review-R3/R4 probe outputs as expected); **real-CLI fixture docs** (under the same fixtures/.../docs/superpowers/plans/ shape) driven through `tsx scripts/spec-lint.ts` asserting one §4.3 hard code and one gate code appear in the text report under `taskContract:` with exit 1, AND the span exclusion at adapter level: a fixture marker whose `red-target=` names a missing file yields `RED_TARGET_INVALID` in the report and NO `CITATION_FILE_MISSING` for that span (review-R1 F8; spec §6 wiring includes the exclusion surface). Run `_metaPureCore`.
...
//   spawn(command: string, timeoutMs: number): {
//     status: number | null; signal: string | null; error?: { code?: string; message?: string }; stderr: string;
//   }
// The entry-point implementation wraps node:child_process spawnSync("sh", ["-c", command], ...).
```
