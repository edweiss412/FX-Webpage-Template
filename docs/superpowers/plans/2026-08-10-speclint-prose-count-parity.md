...
5. Green: suite passes; `pnpm spec:lint` on this plan and the spec stays 0 hard (self-application); the live-corpus run emits advisories only (spec §6 corpus regression), with the survivor records committed as the arc's measurement records (spec §3 layer 3).

## Task 2 — Mutation enrollment + classifications

<!-- task: red=`pnpm mutation:guards` ac=AC-4,AC-3 -->

Red semantics: before the registry row exists, the gate does not cover the surface — the observed cycle is the ROW addition making `pnpm mutation:guards` run the new surface, with the recorded score and an EMPTY unaccepted-survivor set as the green (survivors triaged: killed, `equivalent`, or `accepted-gap` rows, never silent).
...
Red now (plan-time transcript above). PREPARE the graduation content UNCOMMITTED (archive move + `BACKLOG_GRADUATED` registry row + marker removal), include it in the whole-diff review's scope, and after APPROVE commit it as the PR's LAST commit. Completion checks: entry present in `BACKLOG-archive.md`, registry row present, `pnpm vitest run tests/docs` green. Then real CI green, `gh pr merge --merge`, fast-forward main, `0  0`.
