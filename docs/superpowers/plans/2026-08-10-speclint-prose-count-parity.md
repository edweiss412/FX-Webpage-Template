...
6. Green: suite passes; `pnpm spec:lint` on this plan and the spec stays 0 hard (self-application); the corpus regression per the executable enumeration below (R1 F2, corrected R4): `for f in $(git ls-files 'docs/*.md' 'docs/**/*.md' BACKLOG.md DEFERRED.md); do pnpm spec:lint --kind spec "$f"; done` — the extra `docs/*.md` glob picks up the two top-level files (`docs/CLAUDE.md`, `docs/vision.md` — the instrument's population includes `docs/vision.md:82`; R4 F2), and `--kind spec` is REQUIRED because kind inference only works under `/specs/` and `/plans/` (59 corpus files exit 2 without it — R4 F1; the flag affects section checks only, and the three numeric arms run identically under either kind, stated as the wrapper's contract) — with the wrapper asserting zero HARD findings and collecting every advisory of the three new codes into the committed survivor records; the classification commit carries a reconciliation count (emitted survivors == classified rows, diffed mechanically and pasted).
