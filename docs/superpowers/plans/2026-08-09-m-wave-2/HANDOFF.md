...
3. The five unit worktrees exist with claims already pushed (plan Task D3): `../FX-worktrees/m2-payload-hygiene`, `m2-sync-fault-codes`, `m2-e2e-infra`, `m2-guard-precision`, `m2-ui-cluster`. Work ONLY in them (invariant 11). Per worktree before tests: `pnpm install && pnpm worktree:link-env && pnpm preflight`.
4. Write `<active worktree>/.claude/ship-state.json` with YOUR sessionId (scratchpad path segment), branch, stage, next; register your 10-minute cron nudge (off-minute `7,17,27,37,47,57 * * * *`; prompt: date first, supersession check on sessionId, blockedOn semantics, resume `next`, never re-ask authorization); label pane AND agent: `herdr pane rename "$HERDR_PANE_ID" "<branch>"` + `herdr agent rename "$HERDR_PANE_ID" "<branch>"` (skip silently if unset; NEVER rename the workspace).
5. Begin W-PARSE Task P1.
