...
1. **RED first:** the band guard — walks `app/**`+`components/**` for numeric z in BOTH idioms (utility `z-<n>` in className context AND inline `zIndex:` numerics), plus a planted-fixture premise row.
   Run: `pnpm vitest run tests/styles/_metaZIndexBands.test.ts` — Expected: FAIL listing the census (61 className-context utilities incl. `z-60`, plus the 1 inline site `zIndex: 100` at `components/admin/PreviewBanner.tsx:69`; the `z-100` comment at `components/admin/PreviewBanner.tsx:66` is outside the comment-stripped recognition universe and appears in no expected list).
