...
- **Z-index utilities** — command: `rg -o '\bz-[0-9]+\b' app components --glob '*.tsx' | grep -oE 'z-[0-9]+' | sort | uniq -c` → `z-0`×1, `z-10`×12, `z-20`×12, `z-30`×10, `z-40`×9, `z-50`×15, `z-60`×2, `z-100`×1 = **62 utility occurrences (comments included — the guard counts className-context sites only)**, plus inline `zIndex: 100` at `components/admin/PreviewBanner.tsx:69`. `z-60` lives at `components/admin/dev/SwitcherControls.tsx:78`.
...
2. Implement: extract `stripZeroWidth` to `lib/parser/zeroWidth.ts`; `lib/parser/index.ts` + `clean()` + `hotelConfTokens.ts` import it; strip at the five Drive-string sites. **Structural uniqueness proof (spec AC-M2):** the guard file adds a source-scan assertion — `rg` over `lib/sync/**`, `lib/drive/**`, `lib/parser/index.ts`, `lib/parser/blocks/_helpers.ts`, `lib/parser/blocks/hotelConfTokens.ts` for `200B` finds ONLY `lib/parser/zeroWidth.ts` — executable, not prose. Class-sweep: `rg -n 'sheetTab|\.alt|headerPreviews|tabName' lib/sync/enrichWithDrivePins.ts lib/sync/pullSheetOverride.ts lib/drive/exportSheetToMarkdown.ts`; new Drive-string entry points join same-commit.
...
1. **RED first:** plant a `WorkbookSynthesisError` through the per-file catch (anchor: the `classifySyncFailure(error)` call near `lib/sync/runScheduledCronSync.ts:3915-3925`); assert recorded code `PARSE_ERROR_LAST_GOOD`, **outcome stays `parse_error`-family, and the show's last-good payload row is UNCHANGED byte-wise** (spec F6 proof). Negative row: non-synthesis throw still classifies via `classifySyncFailure`.
   Run: `pnpm vitest run tests/sync/ -t "workbook synthesis"` — Expected: FAIL (records `SYNC_FILE_FAILED` today).
2. Implement the `instanceof` arm. Run again — Expected: PASS. No new §12.4 row.
...
**Files:** create `supabase/migrations/<ts>_source_anchors_modified_time.sql`, `lib/sheet-links/freshSourceAnchors.ts` + its test; modify `lib/data/getShowForViewer.ts`, `components/admin/review/publishedAdapter.ts`, `lib/sync/runScheduledCronSync.ts` (coalesce sites at lines 3073 and 1527), `lib/sync/runOnboardingScan.ts` (line 1350), `lib/onboarding/sessionLifecycle.ts` (both finalize flows), `pending_syncs` staging carry-through, `scripts/backfill-validation-source-anchors.ts`, `supabase/__generated__/schema-manifest.json` (regen).
1. **RED first (helper, pure):** `freshSourceAnchors(anchors, stamp, lastSeen)` unit rows — match → map; mismatch → empty; NULL → empty. Run: `pnpm vitest run tests/sheet-links/` — Expected: FAIL (module absent). Implement; PASS.
2. Migration: column + legacy backfill (stamp = `last_seen_modified_time` where `source_anchors` non-empty) inside a per-row `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))` loop. **Executable lock proof:** a migration-text test row (or closeout grep, recorded): `grep -c "pg_advisory_xact_lock" supabase/migrations/<ts>_source_anchors_modified_time.sql` ≥ 1 within the backfill loop. Apply local; `pnpm gen:schema-manifest`; surgical validation apply + `notify pgrst, 'reload schema'`.
3. **RED next (integration set), then implement writers/readers:** crew-page pair (mismatch → `#gid=0`; match → deep link); admin published-review demote row; **pre-migration-populated row proof** (seed a row with non-empty anchors + NULL stamp, run the backfill statement, assert the deep link renders identically post-migration — spec F6); writer rows (fresh-write stamps Drive modifiedTime; preserve keeps old stamp); validation-backfill rows (unraced: stamp=watermark deep link; raced W1≠W2: NULL stamp + warning printed).
...
4. Class-sweep: `rg -n 'source_anchors' lib/ components/ app/ scripts/` — every consumer applies the helper or carries an inline fresh-by-construction reason (`components/admin/wizard/step3ReviewSections.tsx:912` expected in the latter class).
...
5. GREEN + wiring: un-skip; enroll in `crew-e2e.yml` AND both exact-set registries (`check-crew-e2e-executed.mjs` counts; `picker-flow-e2e-ci-wiring.test.ts` parity) — Run: `node scripts/check-crew-e2e-executed.mjs` + `pnpm vitest run tests/cross-cutting/picker-flow-e2e-ci-wiring.test.ts` — Expected: PASS with the suite counted. Flip-test output in the commit body.
...
2. **RED (deterministic reproduction, post-diagnosis):** a fixture row forcing the identified condition (e.g., sampling before `document.fonts.ready` resolves) — Expected: FAIL reproducing the flake's exact message deterministically.
...
2. For each of the **9** jobs: a smoke observing the job's side effect (sync path's existing smoke = template) OR a named documented-limit row (consequence bound: proven or named, never silent).
3. Wire into `pg-cron-validation-parity` (`x-audits.yml`); Run: `gh workflow run x-audits.yml --ref feat/m2-e2e-infra`, await green, record URL.
4. Archive; commit `test(db): per-job pg-cron firing smokes with named limits`.
...
**Files:** modify `tests/help/_metaUiLabelCrosswalk.test.ts`; create the third-party carve + indirect-copy registries beside it.
...
**Files:** modify `components/admin/wizard/step3ReviewSections.tsx` / `Step3Review.tsx` row-slot chrome; tests beside their existing suites.
...
1. **RED first:** class assertions on the four targets expecting the action-affordance class. Run: their suites — Expected: FAIL on `text-text-subtle`.
...
1. Probe (committed output): widened-universe scan (`.ts`/`.tsx`/`.mdx` under `app/`+`components/` + CSS `content:` declarations) × full InterVariable face → partition (a) Inter-carries / (b) Inter-lacks. Known adds: U+22EE (five help MDX pages), U+2303/U+2304 (`app/globals.css:785` and `app/globals.css:788`).
2. **RED:** a coverage row (in `fontFeatureAvailability.test.ts` or the manifest suite) asserting every partition-(a) codepoint ∈ the shipped woff2 cmap.
