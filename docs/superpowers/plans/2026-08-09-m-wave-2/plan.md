...
Live holders, probed 2026-08-09 (`rg -n "pg_advisory_xact_lock|pg_try_advisory_xact_lock" lib app scripts --glob '*.ts'` — `app/**` included; plan-review r3 F1):
...
| finalize writers `app/api/admin/onboarding/finalize/route.ts:231`, `app/api/admin/onboarding/finalize-cas/route.ts:181` and `app/api/admin/onboarding/finalize-cas/route.ts:691` (the routes hold their own `show:` locks) | JS-side (in-route) | S3 stamp joins these locked writes |
| onboarding cleanup/reap `lib/onboarding/sessionLifecycle.ts:223` and `lib/onboarding/sessionLifecycle.ts:763` (+ `finalize:` keys at `lib/onboarding/sessionLifecycle.ts:421` and `lib/onboarding/sessionLifecycle.ts:782`) | JS-side | untouched (no anchor writes; r3 corrected the earlier "finalize flows" label) |
...
| agenda extract `app/api/admin/onboarding/extract-agenda/[wizardSessionId]/[driveFileId]/route.ts:406` | JS-side (in-route) | untouched (no anchor writes; enumerated per r4 F1) |
| resolve-blocker `app/api/admin/onboarding/resolve-blocker/route.ts:422` | JS-side (in-route) | untouched (no anchor writes; enumerated per r4 F1) |
...
- **Z-index utilities** — seed command (RAW, comments included — a locator, not the census): `rg -o '\bz-[0-9]+\b' app components --glob '*.tsx' | sort | uniq -c` → 62 raw matches. The AUTHORITATIVE census is className-context only, comment-free (plan-review r4 F2 — the raw grep counts comments: `z-100` and `z-0` and one `z-60` hit are comments): a TypeScript-AST walk over string/template literals in className positions reports **38 utility sites** (`z-10`×10, `z-20`×6, `z-30`×4, `z-40`×5, `z-50`×12, `z-60`×1 — the live `z-60` is `components/admin/dev/SwitcherControls.tsx:78`) plus **1 inline site** (`zIndex: 100` at `components/admin/PreviewBanner.tsx:69`). The guard IS the deriving instrument — its first RED output is the census of record, and the exemption registry is authored FROM that output, never pre-listed.
- **Band set, FIXED HERE (spec §2.6 requires it at plan time); values = current numerics so the sweep is name-substitution with zero stacking change:** `--z-raised: 10` · `--z-dropdown: 20` · `--z-nav: 30` · `--z-banner: 40` · `--z-overlay: 50` · `--z-dev-controls: 60` · `--z-sticky-banner: 100` (the inline banner site's value). Per-site band mapping is mechanical (same numeral → same band), authored from the guard's first output.
