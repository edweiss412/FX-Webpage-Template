...
3. The worktree `../FX-worktrees/local-harness-false-failures` ALREADY EXISTS with claim markers pushed (authoring session, spec §3). Verify the authoring PR is MERGED with an ANCHORED origin-state check, never bounded log output: `git fetch origin && git show origin/main:docs/superpowers/plans/2026-08-15-local-harness-false-failures/plan.md | grep -q "plan-APPROVED"` — exit 0 means merged and gated; non-zero, STOP (launched early). In the worktree: `pnpm install && pnpm worktree:link-env && pnpm preflight` (this branch runs suites and needs the local stack — NOT docs-only), then `git merge origin/main`.
