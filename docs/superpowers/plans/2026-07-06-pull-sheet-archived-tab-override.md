...
### Step 6.2 — Failing content-changed integration test (staged DB/envelope outcome)
...
Run → **FAILS**, then after Step 6.5 impl (cron wiring) → **PASSES**. Concrete failure mode: a published show staying sticky on a stale fingerprint after the archived tab changed/vanished.

### Step 6.4 — Failing `rescanWizardSheet` persistence tests (the accept/refresh re-scan path, Codex plan-R6-1)
...
Run → **FAILS** (rescan path unwired), then after Step 6.5 impl (`rescanWizardSheet` wiring) → **PASSES**. Concrete failure mode caught: accept/stale-refresh triggering a re-scan that persists a preview with NO `archivedPullSheetTabs` (breaking S2/S4 + the R5-1 dead-loop fix) — the exact gap R6-1 flagged.

### Step 6.5 — Impl helper + wire into scan/rescan/cron/apply (all four test steps above are RED first — TDD)

Only now, with Steps 6.1–6.4 written and failing, implement:
...
- **`applyRescanDecisionUnderLock.ts`:** at Phase-D apply, under the `show:` lock, write `payload.pullSheetOverride` to `shows.pull_sheet_override` (**propagation only**). The **5.8 consistency gate** that guards this write — refuse when `payload.pullSheetOverrideApplied` is not deep-equal to `overrideSnapshot(payload.pullSheetOverride)` (payload-internal, NOT vs stale durable `shows`) — is **implemented in Task 11**, where its fail-first mismatch/pass tests live (TDD: the gate's refusal behavior gets its RED tests before its implementation). Task 9 proves only that a **consistent** payload propagates (its test 9.1 uses `applied === overrideSnapshot(override)`); Task 11 adds the guard that refuses an inconsistent one.

Run → **PASSES** (consistent-payload propagation only; the refusal gate is Task 11). Commit: `feat(onboarding): propagate pull_sheet_override to shows on both finalize flows (payload-carried for Flow B)`
