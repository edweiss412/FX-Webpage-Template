...
**Spec:** `docs/superpowers/specs/2026-07-24-share-link-chrome-backlog-design.md` — 8 adversarial rounds, ~48 findings, all verified true and repaired. Its §9.5 records the cited decision to resolve the oracle-prose vector by construction here rather than in a 9th prose round. **The plan is where that resolution happens**, so Task 7's adversary matrix is not optional polish.

## What the spec makes normative

Read §9.1 (N0-N7), §9.2 (coverage obligations), §9.3 (the live harness and its four wiring points), §9.4 (completion obligations K1-K9), and §6.1 (the three-branch transition rule). Those are the contract. §9.1.1's adversary register is worked examples for Task 7, never a gate.
...
- **UI quality gate (invariant 8):** `/impeccable critique` AND `/impeccable audit` on the diff, P0/P1 fixed or `DEFERRED.md`-logged, BEFORE cross-model review. Task 9.
...
| `components/admin/showpage/StatusStrip.tsx:18-19`, `components/admin/showpage/StatusStrip.tsx:99` | describes a strip copy-link; the file has ZERO `ShareLinkCopyButton` imports — verified `grep -c` = 0 |
...
- [ ] RED: source-scan test under `tests/components/admin/showpage/` asserting `app/globals.css` contains the spec §3.4 block and that `ShareHub.tsx` declares no `@keyframes`. Template: `tests/components/admin/wizard/step3ReviewModal.transitions.test.tsx:723-741`.
- [ ] Write the CSS, then **run `pnpm format`, THEN author the regex against the formatted bytes** — ours is a two-name shorthand prettier reflows across three lines, unlike the single-line template. Authoring the regex first is a guaranteed false red.
- [ ] Export `SHARE_LINK_FLASH_MS = 1600` from `ShareHub.tsx`; assert the VALUE (N0), not merely equality with the CSS.
- [ ] GREEN. Commit `feat(admin): add the share-link flash keyframes`.

### Task 3 — The cue state (§6.1 rule, N2-N5, N7)

- [ ] RED: jsdom test — rotate through the real two-tap confirm; the URL block carries `data-share-link-flash=""`.
- [ ] Implement per spec §3.2, placing the state block AFTER `linkActive`.
- [ ] GREEN. Commit `feat(admin): cue the crew-URL block when the share-token changes`.

### Task 4 — jsdom coverage of the transition rule

One test per branch of §6.1, each with its positive precondition:

- [ ] Branch 2: cue fires; the URL block's element identity DIFFERS across the change; the Copy button, row wrapper and panel identities are UNCHANGED (N4).
- [ ] Branch 3 persistence: with a cue live, force unrelated re-renders (busy flip, banner mount) and assert the attribute PERSISTS. This is the round-6 correctness bug.
- [ ] Timing: present immediately, present at `SHARE_LINK_FLASH_MS - 1`, absent at `SHARE_LINK_FLASH_MS` — the middle checkpoint is what rejects a wrong constant.
- [ ] Expiry identity: no element identity changes, including the URL block (N5) — this is what separates `key={token}` from `key={flash}`.
- [ ] Branch 1, each cause: closed panel; token to null; **pure unpublish with a child held BUSY so the panel cannot auto-close**, then busy clears, then republish inside the window, asserting the target is PRESENT before asserting the attribute absent.
- [ ] Strictly-lower-epoch rejection: URL unchanged AND attribute absent.
- [ ] Cleanup: `vi.getTimerCount()` > 0 with a cue live, exactly 0 after `unmount()`. Idiom: `tests/devcapture/useDevCapture.test.tsx:350-352`.
- [ ] Commit `test(admin): pin the share-link cue transition rule`.

### Task 5 — Contrast rows (§9.2 item 5)

- [ ] Add rows to `tests/styles/status-token-contrast.test.ts` covering all five uncovered pairs: `accent-edge` against `accent-tint` (both themes), against `surface` (dark), against `surface-sunken` (both themes). Helper API verified: `MODES`, `tokenIn`, `contrast`, `DOT_FLOOR`.
- [ ] Commit `test(styles): pin the flash ring contrast floors`.

### Task 6 — The live browser spec and its FOUR wiring points (§9.3)

- [ ] Write the spec and its `_*LiveEntry.tsx`, modelled on `tests/e2e/hoverhelp-geometry.spec.ts` — a portaled popover, same shape. Version-pin esbuild (`tests/e2e/hoverhelp-geometry.spec.ts:58`); compile real `app/globals.css` through the Tailwind CLI with an `@source` for `components/admin/showpage/ShareHub.tsx` (`tests/e2e/hoverhelp-geometry.spec.ts:76`, `tests/e2e/hoverhelp-geometry.spec.ts:80-82`) — without it the stylesheet omits the classes the cue paints over and the spec measures a bare box while reporting green.
- [ ] Assertions per §9.2: both `background-color` AND `box-shadow`, in the motion and reduced-motion arms; restart via node REPLACEMENT with the attribute already present, not attribute toggling; no element in the panel carries a resolved `animation-name` other than the cue's two.
- [ ] Wiring 1: the spec file. Wiring 2: the `testMatch` allow-list at `tests/e2e/standalone.config.ts:35`. Wiring 3: a dedicated workflow with `workflow_dispatch:`, path filter covering source AND runtime inputs (`package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `postcss.config.mjs`, `tsconfig.json`, `.github/actions/setup/**`) per `.github/workflows/phantom-gap-e2e.yml:71-73`, `.github/workflows/phantom-gap-e2e.yml:79`, `.github/workflows/phantom-gap-e2e.yml:84-85`. Wiring 4: a `PATH_GATED` row in `tests/ci/_metaE2eWorkflowCoverage.test.ts` — it fails by default for new dark specs (`tests/ci/_metaE2eWorkflowCoverage.test.ts:7`), precedent row at `tests/ci/_metaE2eWorkflowCoverage.test.ts:81`.
- [ ] Commit `test(admin): add the real-browser share-link cue spec`.

### Task 7 — The executable adversary matrix (spec §9.0; the oracle resolution)

- [ ] **Commit first.** Reverting an injected mutation with `git checkout --` discards uncommitted work in that file.
- [ ] For each adversary in spec §9.1.1, build it, run the suite, record which rows red.
- [ ] Every adversary must red at least one row. Every row must red for at least one adversary, except the contrast pins and the typecheck gate, which are regression guards and declared exempt.
- [ ] A row that reds for nothing is deleted or strengthened before review, never shipped.
- [ ] Put the matrix in the PR body.
- [ ] Commit `test(admin): record the share-link cue adversary matrix`.

### Task 8 — Docs and backlog (K5-K7)
...
- [ ] Task 3 — cue state
- [ ] Task 4 — transition-rule coverage
- [ ] Task 5 — contrast rows
- [ ] Task 6 — live browser spec + four wiring points
- [ ] Task 7 — adversary matrix
- [ ] Task 8 — docs + backlog
- [ ] Self-review
- [ ] Adversarial review (cross-model)
- [ ] Task 9 — gates, CI, merge
