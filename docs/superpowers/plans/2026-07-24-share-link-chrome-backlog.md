...
### Sweep B — stale topology prose (K4): a COMMAND plus a triage obligation, not a list

Two successive review rounds found this list incomplete — the plan named 7 sites, the reviewer added 11, and running the query exhaustively returns **56 hits across 23 files**. Enumeration in a document is not going to converge here, for the same reason it did not converge in the spec. So the list stops being the obligation.

**The obligation is this command and a disposition for every hit:**
...
At plan time: 56 hits, 23 files. Task 1 triages EVERY hit into exactly one of:

- **STALE** — describes the retired three-consumer fan-out or the removed strip copy-link. Corrected.
- **LEGITIMATE** — describes a copy affordance that still exists (the ShareHub Copy button, help-page prose about copying a link, unrelated `copy-link` matches in `app/globals.css` or the help tree). Left alone, with the reason recorded.

Known STALE at plan time, verified: `app/admin/show/[slug]/ShareTokenContext.tsx:6-8` and `app/admin/show/[slug]/ShareTokenContext.tsx:72-74`; `app/admin/show/[slug]/RotateShareTokenButton.tsx:14-17` and `app/admin/show/[slug]/RotateShareTokenButton.tsx:161-164`; `components/admin/showpage/StatusStrip.tsx:7-9`, `components/admin/showpage/StatusStrip.tsx:18-24`, `components/admin/showpage/StatusStrip.tsx:27`, `components/admin/showpage/StatusStrip.tsx:39`, `components/admin/showpage/StatusStrip.tsx:99`, `components/admin/showpage/StatusStrip.tsx:196`; `components/admin/showpage/ShowReviewModalSkeleton.tsx:123`; `tests/components/RotateShareTokenButton.test.tsx:4-6`, `tests/components/RotateShareTokenButton.test.tsx:71-73`; `tests/app/admin/rotateShareToken.test.tsx:9`, `tests/app/admin/rotateShareToken.test.tsx:10`, `tests/app/admin/rotateShareToken.test.tsx:73`; `tests/components/admin/showpage/statusStrip.test.tsx:5`, `tests/components/admin/showpage/statusStrip.test.tsx:373`; `tests/components/admin/showpage/publishedReviewModal.test.tsx:28`, `tests/components/admin/showpage/publishedReviewModal.test.tsx:766`; `tests/components/admin/showpage/pageTransitions.test.tsx:59`; `tests/e2e/_statusStripToggleHarness.tsx:27`; `tests/e2e/_publishedReviewModalHarness.tsx:18`, `tests/e2e/_publishedReviewModalHarness.tsx:88`; `tests/e2e/_skeletonParityHarness.tsx:126`.

That list is a starting point, NOT the definition. The triage record — every hit, its disposition, its reason — goes in the PR body, and K4 is met when the record covers all 56, not when the named sites are fixed.
...
- [ ] The workflow path filter lists BOTH the runtime inputs AND the concrete sources (plan-review finding 9): the new spec, its entry file, the bundle builder, `tests/e2e/_nextNavigationStub.ts` and any other harness stub it imports (`.github/workflows/hoverhelp-geometry-e2e.yml:19` lists exactly this class), `tests/e2e/standalone.config.ts`, the workflow itself, `components/admin/showpage/ShareHub.tsx`, `app/globals.css`, the ancestor sources named above, plus `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `postcss.config.mjs`, `tsconfig.json`, `.github/actions/setup/**`. Model: `.github/workflows/hoverhelp-geometry-e2e.yml:15-30`.
