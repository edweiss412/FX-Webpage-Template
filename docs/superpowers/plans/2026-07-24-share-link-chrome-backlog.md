...
- **No change to user-visible PRODUCT copy**, so the em-dash ban and the §12.4 catalog gates have no surface here. The rotate banner copy at `app/admin/show/[slug]/RotateShareTokenButton.tsx:278-281` is unchanged. Sweep B is deliberately scoped to code comments and test docs for this reason; the one stale user-facing claim it surfaces (`app/help/admin/per-show-panel/page.mdx:30`) is pre-existing debt from the share-hub consolidation and is FILED, not edited here.
...
- [ ] The workflow path filter lists BOTH the runtime inputs AND the concrete sources (plan-review finding 9): the new spec, its entry file, the bundle builder, `tests/e2e/_nextNavigationStub.ts` and any other harness stub it imports (`.github/workflows/hoverhelp-geometry-e2e.yml:19` lists exactly this class), `tests/e2e/standalone.config.ts`, the workflow itself, `components/admin/showpage/ShareHub.tsx`, `app/globals.css`, the ancestor sources named above, AND the production files the demonstrated browser path actually depends on — `app/admin/show/[slug]/RotateShareTokenButton.tsx` (the override seam at `app/admin/show/[slug]/RotateShareTokenButton.tsx:80`), `components/admin/dev/actionOverrideContext.tsx` (the context at `components/admin/dev/actionOverrideContext.tsx:21`), `app/admin/show/[slug]/ShareTokenContext.tsx` (the epoch gate at `app/admin/show/[slug]/ShareTokenContext.tsx:47`), and `components/admin/HoverHelp.tsx` (the portal-host context at `components/admin/HoverHelp.tsx:80`). A PR touching only the seam, the gate, the rotate driver or the portal context would otherwise break the real-browser proof while its path-gated job skips, violating §9.3 and K8 (round-3 review, HIGH). Plus `package.json`, `pnpm-lock.yaml`, `pnpm-workspace.yaml`, `postcss.config.mjs`, `tsconfig.json`, `.github/actions/setup/**`. Model: `.github/workflows/hoverhelp-geometry-e2e.yml:15-30`.
...
- [ ] `BACKLOG.md`: remove the whole section; file `BL-SHAREHUB-REMOTE-ROTATE-ANNOUNCE`; file `BL-HELP-STRIP-COPYLINK-STALE` for the stale help prose at `app/help/admin/per-show-panel/page.mdx:30` (the Re-sync button is described as sitting beside a copy-link button that the share-hub consolidation removed) — out of scope here because correcting it changes shipped user copy and pulls in the help screenshot surface.
