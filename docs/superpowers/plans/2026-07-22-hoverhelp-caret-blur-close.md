...
- Test wiring (verified): new/extended jsdom files under `tests/components/admin/` auto-run via `BASE_INCLUDE` (`vitest.projects.ts:34`) + parallel glob (`vitest.projects.ts:65`); `tests/lib/popover/position.test.ts` already included; the since-retired hoverhelp-geometry-e2e workflow path-triggered on `lib/popover/position.ts`, `components/admin/HoverHelp.tsx`, `tests/e2e/hoverhelp-geometry.spec.ts`, and `tests/e2e/_hoverHelpGeometryLiveEntry.tsx` (workflow lines 15-22) so the e2e gate fired on that PR unchanged.
