...
**IMPLEMENTED IN TASK 8 (the Stage-B core — testable at the sync/RPC layer that exists now):**
- `tests/sync/commitOverrideSideEffects.test.ts` — pure dispatch: each variant routes to the correct port method (refresh never reaches deactivate, and vice-versa); reason code preserved; port throw propagates (aborts the locked tx, never swallowed).
- `tests/sync/commitOverrideSideEffectsDb.test.ts` — the LOAD-BEARING DB proof (drives the REAL `PostgresPipelineTx` executors via `makeSyncPipelineTx`): (a) refresh does NOT bump `version` (R30, before/after = 5/5); (b) deactivate DOES bump + sets `active=false` + reason (5→6); (c) deactivate on an already-inactive row is a `and active` no-op (no second bump, original reason kept); (d) an OBJECT `sheet_value` refresh stores `jsonb_typeof='object'` — NOT a double-encoded string scalar (feedback_postgres_js_jsonb_param_double_encode; pinned by `tests/sync/jsonbBoundaryRepresentation.meta.test.ts`); (e) a STRING `sheet_value` stores `jsonb_typeof='string'` matching the RPC's `to_jsonb`; (f) null refresh stores jsonb/SQL null.
- `tests/sync/overrideShowHotelWiring.test.ts` (STRENGTHENED) — the Stage-B commit is proven to actually RUN inside the locked tx (the refresh port records the show-dates side-effect), not merely carried on the applied result.

**RE-ALLOCATED (their surfaces land in later tasks — writing them here would ship a knowingly-RED test across a task boundary, invariant 1):**
- `overrideHoldOrdering.test.ts` (§3.4 hold matrix — disposition follows RAW parse name / removal-suppression retains WITHOUT deactivate (R10) / genuine removal deletes + deactivates): the hold-retain + deactivate-on-genuine-removal behavior is already exercised end-to-end in **Task 7** `tests/sync/overrideApply.test.ts` (POST-HOLD retain) + the Task-8 DB deactivation proof above. No separate file needed; the §3.4 matrix is covered across those two.
- `matchKeyDurability.test.ts` (§8.2a/§8.1 — role/hotel_address durability under an active sibling name override; content-disambiguator matchKeys; R26/R27; raw-loader `expectedCurrentValue`): the durability CONTRACT is proven in **Task 4** `tests/overrides/setFieldOverride.test.ts` (matchKey durability + CAS-B raw-loader matrix) + **Task 6** `tests/sync/overrideShowHotel.test.ts` (R26/R27 FINAL-name composition) + the Task-8 sheet_value-refresh proof (survives-next-sync). No separate file.
- `wizardLiveSource.test.ts` (R18 — the widget reads LIVE values) → **Task 15** (the review-wizard override widget; the surface does not exist until then).
- `deactivationReason.test.ts` (R12/R30): the "`deactivation_code` set IN the locked tx" half IS proven by the Task-8 DB deactivate test above (code=`target_missing`/`name_conflict`). The needs-attention RENDERING (correct reason even when `upsertAdminAlert` throws) → **Task 10** (the inactive-row needs-attention stream); the alert-lifecycle coherence (collapse/resolve) → **Task 11** (the two alert codes + auto-resolve). Both surfaces land there.

Run → FAIL (module absent / version bumped on benign refresh).

### Step 8.2 — Implement `commitOverrideSideEffects` + wire at the applied-path Stage-B slot in `phase2.ts` (past the stale short-circuit; two write ports `refreshOverrideSheetValue`/`deactivateOverride` on `Phase2Tx` + concrete `PostgresPipelineTx` impls). Version-bump rules exactly per R30 (bump on deactivate + mutation, NOT on benign sheet_value refresh). Run → PASS.

**Deliverable:** Stage B side-effects committed atomically on the applied path; version semantics + jsonb representation green. Hold-ordering / matchKey-durability coverage cross-referenced to Tasks 4/6/7; wizard-live-source → Task 15; deactivation-reason rendering + alert lifecycle → Tasks 10/11 (see the re-allocation note above).
...
**Spec:** §10, §11, §12. **Deliverables:** ADD 4 `AUDITABLE_MUTATIONS` rows `{ file:"app/admin/show/[slug]/_actions/overrides.ts", fn:"setFieldOverrideAction", code }` for `FIELD_OVERRIDE_SET`/`REVERTED`/`REPOINTED`/`DISCARDED` + register them in the forensic code-set (`SANCTIONED_CODES:312` / `NEW_FORENSIC_CODES:390`). Forensic codes are NOT §12.4 rows / NOT in `catalog.ts` (precedent `archive.ts` `SHOW_ARCHIVED`).

**⚠️ FOLDED INTO TASK 14 (structural infeasibility of a standalone pre-registration — VERIFIED empirically 2026-07-09).** The plan originally staged Task 12 as a standalone RED→GREEN static pre-registration (rows + code-set) landing BEFORE the action, claiming `_metaAdminOutcomeContract` goes green with the action file absent. **That is false.** `_metaAdminOutcomeContract` **Assertion 1** (`tests/log/_metaAdminOutcomeContract.test.ts:24-34`) iterates `AUDITABLE_MUTATIONS` and calls `read(file)` = `readFileSync(file)` (`:21`, NO existence guard) on EVERY registry row, then asserts the file imports + `await`s `logAdminOutcome` and carries the code. A row pointing at the not-yet-existent `app/admin/show/[slug]/_actions/overrides.ts` makes `readFileSync` throw **ENOENT** → Assertion 1 fails hard (confirmed: `node -e readFileSync(...)` → `THROWS: ENOENT`). **Assertion 3** (`:57-66`) independently requires every `SANCTIONED_CODES` member be *used by ≥1 registry row* — so the code-set entries cannot precede the rows either. The rows ⟺ code-set ⟺ action-file-exists are bidirectionally bound; there is NO ordering that leaves both assertions green while the action is absent. (Only `_metaMutationSurfaceObservability`'s filesystem WALK is inert on a missing file — but `_metaAdminOutcomeContract`'s registry-ITERATION is not, which the original plan conflated.)

**Resolution:** all four forensic-code surfaces — the 4 `AUDITABLE_MUTATIONS` rows, the `SANCTIONED_CODES`/`NEW_FORENSIC_CODES` entries, AND the executable behavioral spy (`tests/log/adminOutcomeBehavior.test.ts`) — land TOGETHER in **Task 14**, in the SAME commit as `app/admin/show/[slug]/_actions/overrides.ts` (which emits `await logAdminOutcome(...)` with the per-op code on each committed-success branch). Then Assertion 1 reads a REAL file that imports+awaits+carries each code (green), Assertion 3 sees each sanctioned code used by its row (green), and `_metaMutationSurfaceObservability` discovers the new admin surface and finds it registered + behaviorally proven (green). This preserves invariant 1 (no knowingly-RED test across a task boundary) — which the standalone Task 12 would have VIOLATED (a committed ENOENT-throwing Assertion 1). Task 12 is therefore a no-op marker; see Task 14's forensic-code step.

**Commit:** none (folded into Task 14).
...
- `tests/log/_auditableMutations.ts` (**FOLDED FROM TASK 12** — ADD the 4 rows `{ file:"app/admin/show/[slug]/_actions/overrides.ts", fn:"setFieldOverrideAction", code }` for `FIELD_OVERRIDE_SET`/`REVERTED`/`REPOINTED`/`DISCARDED` AND add those 4 to the forensic code-set `SANCTIONED_CODES:312`/`NEW_FORENSIC_CODES:390`, IN THIS COMMIT — the action file now exists so `_metaAdminOutcomeContract` Assertion 1 reads a real file that imports+awaits+carries each code (green) and Assertion 3 sees each sanctioned code used by its row (green). See Task 12 for why this cannot precede the action.)
