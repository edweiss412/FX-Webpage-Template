...
| --- | --- | --- |
| `tests/components/admin/showpage/sectionWarningControls.test.tsx:424` | the correction-loop callout renders inside the PUBLISHED warnings section | Task 4 updates it to assert absence. |
...
**Interfaces:**
- Produces: `visibleWarningRows(warnings, routedWarningsRenderElsewhere)`, consumed by the panel body and the rail count, both of which land in Task 3. One predicate, two readers, per spec §3.2.
...
<!-- spec-lint: ignore — file created by this plan -->
- Create: `tests/components/admin/review/routedWarningsGate.test.tsx`
...
**This task changes NO rendered output.** Plan review R1a finding 2 correctly caught an earlier ordering in which the rail count switched to the info-only value while the body still rendered every row, so the task's own equality assertion could not pass. The rail-count VALUE and the body filter now land together in Task 3, because they are one behavior.

<!-- spec-lint: ignore — file created by this plan -->
- [ ] **Step 1: Write the failing plumbing test** in `tests/components/admin/review/routedWarningsGate.test.tsx`. Plan review R2a finding 2 correctly caught that an earlier draft named no test file and could observe the gate ONLY through `railCount`, so an implementation that passed the right gate to the rail while omitting or corrupting the context values would pass and defer its own verification to Task 3.
...
| Fixture | info rows | `here` | `elsewhere` | Expected |
| --- | --- | --- | --- | --- |
| (a) List | 3 | 2 | 2 | the list; NEITHER line |
| (b) Silent | 0 | 2 | 0 | no body content at all |
| (c) Elsewhere | 0 | 0 | 2 | the Elsewhere line |
| (d) Clean, no warnings | 0 | 0 | 0 | the Clean line |
| (e) Clean, all ignored | 0 | 0 | 0 | the Clean line |
| (f) Silent precedence | 0 | 2 | 2 | no body content; NOT the Elsewhere line |

Fixture (a) carries `here > 0` AND `elsewhere > 0` so List provably outranks every body-empty row. Fixture (f) is the case R2a finding 3 named: with both counts positive, Silent must win, and without it the branch order is untested. Fixtures (d) and (e) differ only in whether the ignored warnings exist, which is the pair a wrong active/ignored predicate confuses.

Five states, six fixtures:
  - (a) List with `here > 0` AND rows elsewhere: the list renders, NEITHER line testid is present;
  - (b) Silent: the body container's `childNodes` is empty while the extras below are non-empty; same assertion for (f);
...
- [ ] **Step 2b: Guard the seam against production use.** Add an AST-based assertion (the pattern `tests/log` already uses for log-code stamps, not a regex) that walks every non-test source file, finds each `CallExpression` whose callee resolves to `deriveAttentionItems`, and asserts no argument object literal carries an `excludedCodes` property. Plan review R1b correctly noted an exported optional parameter is externally usable production API.

  **Stated limit** (plan review R2b finding 5): an AST walk over call sites cannot see a value forwarded through a spread of a preconstructed object, an alias, or a wrapper function. The guard detects the direct form, which is the form a future caller would actually write; it is a strong signal, not a proof, and is described at that strength.
...
- [ ] **Step 2: Drive the REAL feed route, not only a mocked response.** Plan review R4b finding 1 correctly caught that a mocked panel plus an import guard leaves the actual path untested: the route could filter either code by literal comparison or through a third helper, and both assertions would still pass. Invoke the feed route handler against seeded `admin_alerts` rows carrying both cut codes and assert both appear as entries in its JSON response. Together with Step 1 this covers both halves of the path, the route that produces entries and the panel that renders them.
- [ ] **Step 3: Add the source-scan guard** that neither `components/admin/BellPanel.tsx` nor `app/api/admin/alerts/bell/feed/route.ts` nor its transitive builder imports `deriveAttentionItems` or `DOUG_EXCLUDED_CODES`. Verified absent at plan time; the guard keeps it that way. It is a cheap third signal, not the proof, which Steps 1 and 2 now carry.
- [ ] **Step 4: Run, commit** `test(admin): pin bell inclusion of the codes cut from the modal`
...
| Elsewhere to Clean | instant, no animation needed |
  ...
  **Every one of the six pairs is EXERCISED**, not four of them argued away. Plan review R2b finding 3 correctly rejected an earlier draft that exercised one pair fully, gestured at a second, and left four to a source audit: "wholesale re-render from new props" is a claim about React reconciliation, which a static audit cannot establish, and distinct conditional branches are not equivalent merely because they share a trigger.
  ...
  | --- | --- | --- |
  | List to Silent | (a) | (b) |
  | List to Elsewhere | (a) | (c) |
  | List to Clean | (a) | (d) |
  | Silent to Elsewhere | (b) | (c) |
  | Silent to Clean | (b) | (d) |
  | Elsewhere to Clean | (c) | (d) |

  Silent to Clean additionally carries the node-identity contract in Step 2, because it is the only pair that also changes extras membership while a disclosure is open.
...
<!-- spec-lint: ignore — file created by this plan -->
<!-- spec-lint: ignore — file created by this plan -->
- [ ] **Step 4: Write `docs/superpowers/plans/2026-07-20-warning-surface-trim/handoff.md` with numbered sections through §12, and record the findings and dispositions in its §12**, exactly as invariant 8 words the requirement. Plan review R2b finding 1 and R3b both rejected substituting a differently-named document, on the correct grounds that an existing convention is not a ratified exception; writing the document with the section the invariant names costs one file and removes the question entirely.

  Borrow the finding-table FORMAT from `docs/superpowers/plans/2026-07-19-published-show-alerts/impeccable.md`, and nothing else from it: a run header naming the impeccable version and the setup gates, critique and audit sections, and a severity / finding / disposition table where every P0 and P1 reads FIXED with its commit sha or carries an explicit deferral. That file is a precedent for presentation only, never an authority for where findings live. Plan review R4b finding 4 and R5b finding 2 both caught an earlier draft that kept the superseded rationale in this same step.

- [ ] **Step 5: Commit the handoff document and every gate repair.** Plan review R4b finding 3 correctly caught that an earlier draft created a tracked file with no step committing it, leaving the per-task commit invariant unsatisfied and the artifact uncommitted at push. Commit message: `docs(admin): impeccable dual-gate record for the warning surface trim`.
...
- [ ] **Step 6b: Loop the WHOLE gate sequence to a fixpoint.** Plan review R4b finding 5 correctly caught that an earlier loop restarted only on a Step 6 repair, so a P1 found by the reran dual gate, or a failure repaired during Steps 1 through 5, could reach push having never been re-verified.

  The rule is one sentence with no exceptions: **any repair, made at any point in Task 9 or Task 10, restarts this task at Step 1 and reruns Task 9's dual gate if the repair touched a UI surface.** The sequence is complete only when one full pass of Task 9 plus Steps 1 through 6 produces zero repairs. Do not push before that pass. Plan review R2b finding 2 correctly caught an earlier draft that reran only for UI repairs, which let a test, fixture, or config repair reach push and CI with no local verification. Verification that precedes its own repairs has not verified them.
