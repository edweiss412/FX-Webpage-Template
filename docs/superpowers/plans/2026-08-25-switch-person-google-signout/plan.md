...
- Server boot: the `webServer` in `playwright.config.ts`, project `desktop-chromium`. Locally `reuseExistingServer` is on (`playwright.config.ts:269`), so a live sibling-worktree dev server on :3000 would be reused and serve the WRONG code (`playwright.config.ts:4-8`). Every local run in this plan therefore relocates the server with `E2E_PORT=3107` (the config's escape hatch, `playwright.config.ts:8`) AND `PICKER_E2E_BASE_URL=http://127.0.0.1:3107`, because picker-flow computes its own base URL from that variable (`tests/e2e/picker-flow.spec.ts:49`) rather than from `E2E_PORT`. Before each run, `lsof -nP -iTCP:3107 -sTCP:LISTEN` must print nothing; Playwright then boots the server from THIS worktree and tears it down when the run ends, so RED and GREEN each get a fresh server built from the file state at that moment. CI is unaffected: `reuseExistingServer` is false there and `.github/workflows/crew-e2e.yml:188` runs on :3000. Local command: `E2E_PORT=3107 PICKER_E2E_BASE_URL=http://127.0.0.1:3107 pnpm heavy pnpm exec playwright test --project=desktop-chromium tests/e2e/picker-flow.spec.ts`.
...
<!-- task: red=`E2E_PORT=3107 PICKER_E2E_BASE_URL=http://127.0.0.1:3107 pnpm heavy pnpm exec playwright test --project=desktop-chromium tests/e2e/picker-flow.spec.ts` red-state=authored red-target=`lib/auth/picker/clearIdentity.ts:86` why=`on the pre-Task-1 file the tap clears the entry but the session survives, the re-render returns needs_picker_bootstrap and re-mints the same identity, so the gate never mounts and the new case times out on sign-in-or-skip-gate; RED is observed with lib/auth/picker/clearIdentity.ts checked out from origin/main over the committed Task 1 version, GREEN with the committed version restored` ac=AC-8,AC-9 -->

**What is red and why.** The new case asserts the first-contact gate after the tap. Before Task 1 the resolve chain re-mints the identity (`lib/auth/picker/resolveShowPageAccess.ts:246-252`), so `crew-shell` renders again and `sign-in-or-skip-gate` never appears. Task 1 is already committed when this task runs, so a stash cannot remove it. Observe RED by checking the pre-Task-1 file out of `origin/main` over the committed one, running the case, then restoring the committed version: `git checkout origin/main -- lib/auth/picker/clearIdentity.ts`, confirm `lsof -nP -iTCP:3107 -sTCP:LISTEN` prints nothing, run the command, `git checkout HEAD -- lib/auth/picker/clearIdentity.ts`, confirm `git status --short lib/auth/picker/clearIdentity.ts` prints nothing. Then confirm the port is free again and run the same command on the branch tree for GREEN. Both runs boot their own server from this worktree on :3107 (harness-readiness section above); a server left on :3000 by another worktree cannot be reused. Record both runs' summary lines in the commit body. (Plan review round 1: `git stash push <path>` stashes only uncommitted differences from `HEAD`, so the earlier procedure would have run RED with the fix still present.)
...
**GREEN.** Task 1 is the implementation. Run the spec on the branch tree: `E2E_PORT=3107 PICKER_E2E_BASE_URL=http://127.0.0.1:3107 pnpm heavy pnpm exec playwright test --project=desktop-chromium tests/e2e/picker-flow.spec.ts`; the new case passes and the existing six still pass.
