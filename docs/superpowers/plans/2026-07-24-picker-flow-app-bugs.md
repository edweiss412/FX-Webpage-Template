...
| HIGH — `DOCS_ONLY` still accepted `docs/**`, a documented build input through `pretest-gen.mjs` | Pattern narrowed to root-level markdown, issue templates, the licence | Rejects all five probe inputs — the `docs/**` glob, a nested docs markdown path, the `app/**` glob, a library TypeScript path, and the recursive `**/*.md` glob — and still accepts the four real docs paths |
