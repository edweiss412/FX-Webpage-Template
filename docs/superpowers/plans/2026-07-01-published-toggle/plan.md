...
- Modify: `tests/db/showCacheRevalidateCoverage.test.ts` — remove the deleted `_actions/undoAutoPublish.ts` entry from `REVALIDATE_REGISTRY` (`:218-223`) AND from `lib/sync/unpublishShow.ts`'s `coveredByCallers` list (`:169-178`); the surviving callers (route + confirm action) keep that entry valid
