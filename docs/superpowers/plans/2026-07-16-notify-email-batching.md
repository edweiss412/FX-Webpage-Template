...
13. **Truncation is display-only at the DELIVERY layer:** 21 same-group candidates → ONE send whose `idempotencyKey === baseKey(kind, combinedDedupKey(all 21 keys), recipient)`, 21 sent ledger rows land, and the sent body renders 20 items + overflow line (assert member 21's identifying string absent from the body but present in a ledger row's `dedup_key`). Failure mode caught: slicing members before key derivation/ledger writes, orphaning member 21 into a next-tick resend.
14. **Post-reissue conflict → retryLater, zero writes:** batch of 2, BOTH sends return `{ok:false, kind:"idempotency_conflict"}` → counts `{retryLater:2}`, zero sent/failed rows, zero alerts (spec §2.3 arm).
