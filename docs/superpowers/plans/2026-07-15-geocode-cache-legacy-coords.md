 ...
 *      the past; a coords-bearing sibling keeps its future expiry.
 *   4. miss-predicate proof: the expired row no longer satisfies the read filter
 *      `expires_at > now()` that readGeocodeCache applies (lib/geocoding/cache.ts:61
 *      — `.gt("expires_at", new Date().toISOString())`); the sibling still does.
 *      (SQL mirror of the PostgREST filter: vitest has no Supabase env for a
 *      supabase-js round-trip; the predicate IS the contract under test.)
 */
...
describe("20260715000000 one-shot coord-less expiry DO block", () => {
  ...
  test("fuse: >1000 fresh coord-less rows abort with zero mutation", async () => {
    // 1001 coord-less rows trip the 1000-row fuse (spec §3.1 R5).
    await sql.unsafe(`
  ...
  test("expiry: both coord-less shapes expire into the past; coords-bearing sibling untouched", async () => {
    await sql`
      insert into public.geocode_cache (query_hash, venue_name, city, lat, lng, expires_at)
      values
    ...
    `;
    await sql.unsafe(expiryDoBlock());
    const rows = await sql<{ query_hash: string; expired: boolean; future: boolean }[]>`
      select query_hash,
  ...
  test("miss-predicate proof: expired row fails the readGeocodeCache filter; sibling still hits", async () => {
    await sql`
      insert into public.geocode_cache (query_hash, venue_name, city, lat, lng, expires_at)
      values
        (${PREFIX + "miss"}, 'Miss Venue', 'Chicago', null, null, now() + interval '10 days'),
        (${PREFIX + "hit"}, 'Hit Venue', 'Chicago', 41.88, -87.63, now() + interval '10 days')
    `;
    await sql.unsafe(expiryDoBlock());
    // Mirror of lib/geocoding/cache.ts:61 `.gt("expires_at", new Date().toISOString())` —
    // the JS-side now is interchangeable with DB now() only because the expiry lands a
    // full day in the past (R8).
    const jsNow = new Date().toISOString();
    const visible = await sql<{ query_hash: string }[]>`
      select query_hash from public.geocode_cache
       where query_hash like ${PREFIX + "%"} and expires_at > ${jsNow}
    `;
    const hashes = visible.map((r) => r.query_hash);
    expect(hashes, "expired coord-less row must read as a MISS").not.toContain(PREFIX + "miss");
    expect(hashes, "coords-bearing sibling must still read as a HIT").toContain(PREFIX + "hit");
  });
});
```
