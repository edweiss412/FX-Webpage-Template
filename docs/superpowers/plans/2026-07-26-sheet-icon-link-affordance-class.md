...
- `components/admin/showpage/ShowReviewModalSkeleton.tsx:83` — skeleton's `size-tap-min` sheetlink placeholder mirrors the old box → becomes the 20px-box-in-44px-row shape in Task 4, with `tests/e2e/_skeletonParityHarness.tsx` lines 54-56 and 165-168 expectations updated in the same task.
...
- Modify: `components/admin/showpage/ShowReviewModalSkeleton.tsx:83` (placeholder mirrors the new shape: 20px box inside the 44px-floored row)
...
- [ ] **Step 1 (RED):** Rewrite the rider: anti-inflation twin (visible rect `< TAP_MIN` on BOTH axes — spec §7.1) + hit-tested target ≥44 both axes; add title-row floor assertion (row ≥ 44); add the rect-intersection set {title box, subline, state pill `PublishedReviewModal.tsx:879-901`, close button} + one elementFromPoint spot check, on the new saturated-title page.
  ...
  - jsdom RED: `pnpm exec vitest run tests/components/admin/showpage/publishedReviewModal.test.tsx` — expected failure: the reshaped line 362 case (asserts the overlay-idiom token set on the anchor) fails against the boxed markup.
...
- Modify: `tests/e2e/step3-review-modal.layout.spec.ts` (hit-tested 44×44 + row floor ≥ 44 + rect-intersection set {title box, eyebrow, subline, state chip `Step3ReviewModal.tsx:443-465`, close button} + spot check, saturated long title per `tests/e2e/step3-review-modal.layout.spec.ts:613-639` precedent)
...
- [ ] **Step 1 (RED):** Reshape the line 310 className pin to the shared-idiom token set; add the e2e cases; land the guard.
  - RED command A: `pnpm exec vitest run tests/components/admin/wizard/Step3ReviewModal.test.tsx tests/components/admin/sheetIconLinkContainment.test.ts` — expected failures: the token-set case (boxed markup) and the guard (counts 3/2 vs pinned 0/1 for the two modal files).
  - RED command B: `pnpm exec playwright test --config=tests/e2e/standalone.config.ts tests/e2e/step3-review-modal.layout.spec.ts` — expected failure: none of the new cases exists yet pre-edit; after the edit, the hit-test and floor pass only with Step 2 (declared: these are GREEN-step proofs, the jsdom cases are this task's red edge).
- [ ] **Step 2 (GREEN):** Swap the anchor (`className="mr-0.5"`); row `gap-1` → `gap-2.5` + `min-h-tap-min`; reword comments line 26/line 27/line 390 (guard counts comments — the label literal leaves line 27); drop `ExternalLink` import; regenerate the byte baseline. All commands green, incl. `pnpm exec vitest run tests/components/admin/review/reviewModalShell.test.tsx`.
