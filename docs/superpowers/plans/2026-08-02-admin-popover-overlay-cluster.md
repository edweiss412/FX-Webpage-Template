...
- **Sync pre-step (FIRST action of T1's session — review R9 F5):** the branch is ~156 commits behind `origin/main`, and main has since changed the very surfaces this plan edits (`tests/e2e/standalone.config.ts`, `tests/e2e/standalone-baseline.json`, a ~700-line extension of `tests/ci/_metaSpecRegistration.test.ts`, and both backlog ledgers). Ordered steps (review R10 F1–F3):
  1. `git fetch origin && git rebase origin/main`. Expected conflict surface: the `test(infra)` allowlist commit `7c684de20` vs main's `standalone.config.ts` edit — resolve by UNION OVER ACTIVE REGISTRATIONS after applying upstream retirements (review R10 F4): main deleted `agendaBreakdown.layout.spec.ts` and replaced its token with `step3-review-modal.agenda` — keep main's replacement, do NOT restore the retired token; this branch's `popover-clip-fit` token is added later by T3, not here.
  2. Remove the stale untracked probe artifacts BEFORE any gate runs (review R10 F2 — `timerLeakProbe.test.tsx` fails ESLint `no-require-imports`): `rm -f tests/e2e/popover-probe.spec.ts tests/components/admin/showpage/timerLeakProbe.test.tsx`; `git status --porcelain` → only intended files. (This performs the hygiene pre-step's removal, below.)
  3. Re-run the §0 code-verification pass for every citation into the four moved surfaces (line numbers WILL have shifted; fix citations in this plan document, then COMMIT them — review R10 F3): `docs(plan): post-rebase citation refresh`.
  4. `pnpm install && pnpm worktree:link-env && pnpm preflight` (invariant-11 sequence, `AGENTS.md:25` — review R13 F3; link-env is idempotent when the symlink already exists).
  5. Gate sanity (review R10 F1 — full-`pnpm test` green is IMPOSSIBLE pre-T11 by the §0 transient-marker design): `pnpm typecheck && pnpm lint` green; `pnpm test` green EXCEPT exactly one expected failure, `tests/docs/_metaInvariant8Closeout.test.ts` on this plan's TEMPLATE marker — verify the failure set is exactly that (e.g. `pnpm test 2>&1 | grep -c "FAIL"` = 1 and the line names that file). No `--exclude` alternative: repo Vitest ignores CLI `--exclude` under project-level exclusions (`vitest.config.ts:18`, review R12 F1).
  A final pre-push checkpoint (after T12, below) repeats the fetch/reconcile with its own commit path.
- **Worktree hygiene pre-step (before T1):** the clean standalone allowlist is COMMITTED (the accidental `popover-probe` token was dropped in `test(infra)` commit `7c684de20`; `rg popover-probe tests/e2e/standalone.config.ts` → no match at plan-final time). Remaining artifacts: `rm -f tests/e2e/popover-probe.spec.ts tests/components/admin/showpage/timerLeakProbe.test.tsx` (both untracked) — PERFORMED at sync step 2 above, before any gate runs (review R10 F2). Verify `git status --porcelain` shows only intended files. Registration lifecycle for the NEW spec (review R8 F1): T2b registers `popover-clip-fit` TEMPORARILY (working tree) for the RED observation and reverts it (`git checkout -- tests/e2e/standalone.config.ts`); T3 registers it PERMANENTLY (first committed case block) and regenerates the baseline in the same commit; T4 and T7 each regenerate the baseline when their case blocks land. Transient census note: between T2b and T3 the authored spec file is untracked and unregistered, so a LOCAL run of `tests/ci/_metaSpecRegistration.test.ts` (which walks the disk) reds during that working-tree window — no committed state is ever census-red.
...
- [ ] RED 1: write `tests/lib/popover/rafCoalescer.test.ts (new)` (path corrected at implementation time from `tests/popover/` — every other `lib/popover/*` unit test lives at `tests/lib/popover/`, and `tests/lib/**` is the glob that puts them in the PARALLEL vitest project, `vitest.projects.ts:98`; `tests/popover/**` matches no parallel glob and would have filed the test alone in the serial phase):
...
```
  ...
  Adopt in `components/admin/showpage/ShareHub.tsx:379-386` and `components/admin/HoverHelp.tsx:309-316` (open-gate `if (!open ...) return` stays at the call site wrapping `schedule()`); delete both local coalescers + local marker comments. Cleanup contract, BOTH consumers (review R14 F1): the coalescer instance is EFFECT-SCOPED and the effect cleanup calls the shared instance's `cancel()` — ShareHub's cleanup currently calls `cancelAnimationFrame` directly (`components/admin/showpage/ShareHub.tsx:423`), HoverHelp's cancels its pending frame at `components/admin/HoverHelp.tsx:350`; both become `instance.cancel()` (satisfying RED 2's rule (v); the compile-time consumer pin authored in RED 1 goes green with the module).
- [ ] Behavioral cleanup pin (ShareHub pending-frame — review R14 F1; HoverHelp's equivalent already lives at `tests/components/admin/hoverHelpLifecycle.test.tsx:113`): add to `tests/components/admin/showpage/shareHubVisualViewport.test.tsx` a case stubbing `requestAnimationFrame`/`cancelAnimationFrame`, driving one `schedule()`-provoking event while open, then unmounting with the frame still pending → `cancelAnimationFrame` called with the pending id. Pins existing behavior, so first-try-green is expected — answer with a deliberate reverted mutation (drop the cleanup `cancel()`, observe the failure, restore; quote both lines in the commit body).
...
- [ ] VERIFY: `pnpm vitest run tests/lib/popover/rafCoalescer.test.ts tests/components/admin/showpage/_metaSharedHelperAdoption.test.ts tests/components/admin/showpage/shareHubVisualViewport.test.tsx` → green (T-S8 unchanged); plus `pnpm vitest run tests/components/admin/hoverHelpVisualViewport.test.tsx tests/components/admin/hoverHelpLifecycle.test.tsx` (the open/closed scheduling coverage AND the close/unmount pending-frame regression suite at `tests/components/admin/hoverHelpLifecycle.test.tsx:113` — review R14 F1; both verified paths at plan time).
...
- [ ] RED: write `tests/components/admin/useFitWithinClip.test.tsx (new)` (jsdom pragma `// @vitest-environment jsdom`). Harness: TWO distinct ancestor nodes (review R5 F3 — collapsing them makes case (d) tautological, since the hook already observes the clip ancestor): an OUTER clipping ancestor div (mocked `getComputedStyle` `overflow: clip`; rect bottom 560) containing an INNER positioned div (the stubbed `offsetParent`), containing the fitted element (rect top 230; declared CSS cap 384). **jsdom returns `offsetParent === null`, so cases (d)/(e) stub it with the repo precedent** (`tests/components/admin/wizard/Step3ReviewModal.test.tsx:453-461`): `Object.defineProperty(HTMLElement.prototype, "offsetParent", { get() { return (this as HTMLElement).parentElement; }, configurable: true })`, restored from the saved descriptor in `finally` — under this stub the fitted element's `offsetParent` is the INNER div and the clip ancestor stays a distinct outer node. Cases:
  ...
  - (f) no `ResizeObserver` global (spec §4.4 degrade path, review R11 F1) → renders without throwing AND still performs the measure-once fitted write (assert the initial `style.maxHeight` under the mocked rects); then change the mocked rects and dispatch `window.dispatchEvent(new Event("resize"))` → fresh recalculated `style.maxHeight` write (an extraction that drops the window-resize fallback listener fails this).
...
- [ ] GREEN: create `components/admin/useFitWithinClip.ts (new)` (`"use client"`): body from `components/admin/ReSyncButton.tsx:79-146` + extensions — ResizeObserver additionally observes `nodeRef.current.offsetParent` (Element-typed guard); `transitionend` listener on the `offsetParent` re-runs `apply`; exact signature (review R13 F2): `export function useFitWithinClip(reapplyKey?: unknown): React.RefCallback<HTMLElement>` — the optional `reapplyKey` participates in the effect deps, and consumers attach the returned ref (`const fitRef = useFitWithinClip(entered); … ref={fitRef}`); `reapplyKey` is a HOOK ARGUMENT, never a DOM prop. `findClippingAncestor` moves as module-private. `ReSyncButton.tsx` imports the hook; both local definitions deleted.
...
- [ ] Split the authored spec into owning-task case blocks (exact handoff paths — review R15 F3): `cp tests/e2e/popover-clip-fit.spec.ts .claude/t2b-handoff/popover-clip-fit.full.spec.txt` (`.txt` so no test runner ever collects it), then reduce the working-tree `tests/e2e/popover-clip-fit.spec.ts` to the shared boot scaffolding plus NOTHING. Each owning task restores its block by copying it from `.claude/t2b-handoff/popover-clip-fit.full.spec.txt` into the committed spec (T3 the menu block, T4 the toggle block + live entry, T7 the mutual-exclusion block) and quotes its REDs from `.claude/t2b-handoff/red-run.log`. Revert the temporary testMatch registration (`git checkout -- tests/e2e/standalone.config.ts`); LEAVE the lifecycle-layout edits in the working tree for T6.
...
- [ ] GREEN: `components/admin/showpage/AttentionMenu.tsx:147` scroller gains `role="group"`, `aria-label="Show issues"`, `tabIndex={0}`, and the fit ref via `const fitRef = useFitWithinClip(entered); … ref={fitRef}` (hook argument carries the re-apply key — review R13 F2; no DOM prop).
...
- [ ] Adoption-row RED (review R15 F1): with the row added and the component not yet importing, run `pnpm vitest run tests/components/admin/showpage/_metaSharedHelperAdoption.test.ts` → observe the PublishedToggle row fail; quote the line in the commit body.
- [ ] GREEN step 2: banner takes the fit ref + `tabIndex={0}` + `aria-label="Publish error details"` (turns the adoption row green).
...
- [ ] RED (unit, `tests/components/admin/showpage/shareHub.test.tsx`; reuse the `maxZLevel` helper at `tests/components/admin/showpage/shareHub.test.tsx:280` — lift it to file scope):
  ...
  - Busy-case hygiene (review R15 F2; live-suite warning `tests/components/admin/showpage/shareHub.test.tsx:846-850`): EVERY case that holds a busy promise open (open+busy, each settle case, `busyStuck`) settles its resolver inside `finally` — resolve the deferred action, restore real timers, flush React (`act`) — before the test exits; a timed-out gate does NOT settle the promise on its own.
  - Run → expected failing line: open+idle z assertion (`expected 0 to be at least 21`).
- [ ] RED (composed, `tests/components/admin/showpage/publishedReviewModal.test.tsx`; public surfaces only): render the modal with attention items present and a DEFERRED archive action (promise held open by the test — the T6 resolver pattern); FIRST await the actionable-item AUTO-OPEN of the menu (rAF path, `components/admin/showpage/PublishedReviewModal.tsx:453`; existing pin `tests/components/admin/showpage/publishedReviewModal.test.tsx:483`) and CLOSE it via the pill, asserting the menu is closed (review R13 F1 — without this, the later pill click toggles the already-open menu CLOSED and inverts the rest of the walk, leaving the test red on a correct implementation); THEN open hub via `share-hub-primary` → arm+confirm Archive so `lifecycleBusy` is in flight → open menu via the pill (`published-show-review-alert-pill`) → resolve the deferred action (settle) → triggers `< 20`; close the menu via its PUBLIC pill toggle (click the pill again — NOT Escape: both surfaces hold capture-phase document Escape listeners and ShareHub's would close the hub too, review R2 F4) → triggers `≥ 21`. Injecting the prop directly here is FORBIDDEN (tautology — bypasses both hops). Run → fails.
- [ ] Pre-code mechanical gate (invariant-8 checklist, BEFORE writing the UI change): confirm the change introduces no em-dash in user-visible copy, curly apostrophes/quotes only (U+2019/U+201C/U+201D), ≥44px tap targets preserved (`min-h-tap-min` companions untouched), canonical type/token classes only.
...
- [ ] RED (unit): AttentionMenu cases in `tests/components/admin/showpage/attentionMenu.test.tsx` — `focusin` on outside element → `onClose` called; on panel descendant → not called; on the pill → not called. ShareHub cases in `tests/components/admin/showpage/shareHub.test.tsx` — open idle: `focusin` outside all four → closes AND `document.activeElement` NOT restored to a trigger; on each inside-set member (four separate cases) → stays open; busy: outside `focusin` → stays open (settle the held busy promise in `finally` with timer restoration + React flush before test exit — review R15 F2, `tests/components/admin/showpage/shareHub.test.tsx:846-850`). Window-blur negatives (spec §3.4/§10 ratified exception, review R6 F3), BOTH surfaces: dispatch `window.blur()` / a `focusout` with `relatedTarget: null` and NO subsequent in-document `focusin` → surface stays open (an implementation dismissing on blur fails these).
...
**Pre-push reconciliation checkpoint (review R9 F5, execution contract per R10 F3/F4):** `git fetch origin`; if `origin/main` moved since the pre-T1 sync, `git rebase origin/main` again — resolve `tests/e2e/standalone.config.ts` by UNION OVER ACTIVE REGISTRATIONS after upstream retirements (never restore a token main retired; keep `popover-clip-fit`), and reconcile BOTH backlog ledgers (keep both sides' ACTIVE rows; graduated entries stay archive-only). Then the baseline cycle: `node scripts/check-standalone-baseline.mjs --list-check` → observe the drift RED if membership changed; REGENERATE from the reconciled tree (`node scripts/check-standalone-baseline.mjs --write` — never hand-merge baseline JSON); re-run `--list-check` → green. Re-run the affected gates (`pnpm vitest run tests/ci/_metaSpecRegistration.test.ts tests/docs/_metaDeferralLedgerGraduation.test.ts` + full `pnpm test && pnpm typecheck && pnpm lint` + full standalone config). Any tracked-file change this checkpoint produced (config resolution, ledger reconcile, regenerated baseline) COMMITS here as `test(infra): pre-push reconciliation — config union, ledger merge, baseline regen`; finish with `git status --porcelain` → empty. Only a clean, reconciled, green, fully committed tree proceeds.
