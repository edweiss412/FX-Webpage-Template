...
```
  ...
  The unit test imports BOTH exports (`import { createRafCoalescer, type RafCoalescer } from "@/lib/popover/rafCoalescer"` and types a local `const c: RafCoalescer` — the compile-time consumer pin). Adopt in `components/admin/showpage/ShareHub.tsx:379-386` and `components/admin/HoverHelp.tsx:309-316` (open-gate `if (!open ...) return` stays at the call site wrapping `schedule()`); delete both local coalescers + local marker comments. Cleanup contract, BOTH consumers (review R14 F1): the coalescer instance is EFFECT-SCOPED and the effect cleanup calls the shared instance's `cancel()` — ShareHub's cleanup currently calls `cancelAnimationFrame` directly (`components/admin/showpage/ShareHub.tsx:423`), HoverHelp's cancels its pending frame at `components/admin/HoverHelp.tsx:350`; both become `instance.cancel()`. Adoption proof: extend the meta-test rows with (v) per consumer, a `.cancel` property-access CALL whose receiver symbol's declaration is a variable initialized from the resolved `createRafCoalescer` import's call result — pre-implementation this fails alongside (i)–(ii) (module absent), and post-GREEN a consumer that adopts `schedule()` but keeps a raw `cancelAnimationFrame` cleanup fails (v).
