...
- [ ] Author the T-BACKDROP trigger assertions in `tests/e2e/admin-lifecycle-layout.spec.ts:623-666` (working tree) and run `node_modules/.bin/playwright test tests/e2e/admin-lifecycle-layout.spec.ts --project=mobile-safari` → OBSERVE the trigger `elementFromPoint` assertions fail against production (backdrop wins). Record the lines.
...
| Compound: lifecycle change while open | Existing immediate-or-busy-deferred close machinery untouched; elevation unmounts with the close | Existing deferred-close coverage (`tests/components/admin/showpage/shareHub.test.tsx:846-894` region); T6 VERIFY green |
...
- [ ] VERIFY: `pnpm vitest run tests/components/admin/showpage/` → green (broadest-blast-radius task: whole directory); re-run the T2b lifecycle-layout spec → T-BACKDROP trigger assertions now GREEN (quote the T2b RED line + this green in the commit body).
...
- [ ] RED: append to `tests/components/admin/showpage/shareHubFlashState.test.tsx` (a) a behavioral pin making the draft-time probe permanent — fake timers, render + open + unmount, `vi.getTimerCount()` delta across open→unmount is EXACTLY 1 (the jsdom Selection timer; a future real component leak makes it ≥2 and fails); (b) a source-contract assertion (`readFileSync` on the test file itself) that the documentation string `Selection._associateRange` is present near the delta baseline. Run → (b) fails (root cause not yet documented), (a) passes (pins current truth).
