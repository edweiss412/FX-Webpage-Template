...
**The `tests/mutation/guardSurfaces.gate.test.ts` citations above move too, and Task 0 re-derives them with the rest.** They are cited there by LINE, and those lines already differ between this branch and `origin/fix/scanner-scope-totality` because #830 landed above them. Re-derive by SYMBOL after the merge — `grep -n "EXPECTED_LEDGER_KINDS\|interactionTimingScan: {" tests/mutation/guardSurfaces.gate.test.ts` — and update the three citations in the same commit as the merge. Recorded because they were added at implementation time and would otherwise sit outside the anchor sweep the next paragraph defines. (Written without bare line numbers on purpose: `spec:lint` raises CITATION_MALFORMED for a colon-prefixed line number carrying no path, which is how this paragraph failed its own gate on first draft — twice, since the explanation of the rule tripped it too.)
...
- **ALSO EXTENDS — `tests/mutation/guardSurfaces.gate.test.ts`, the companion this inventory omitted until implementation time.** Its `EXPECTED_LEDGER_KINDS` map (`tests/mutation/guardSurfaces.gate.test.ts:34`, the `interactionTimingScan` entry at `tests/mutation/guardSurfaces.gate.test.ts:80`, today `{ equivalent: 8 }`) is asserted by the gate's AC-13 case: `surface.accepted` is reduced to per-kind counts and compared with `toEqual`. So the moment Task 4's re-derivation changes the accepted count — which this plan already anticipates, and which probe P17 predicts (roughly six new `equivalent` rows for the resolver's defensive and memoization guards) — that entry must move in the SAME commit or the gate fails on a surface whose score is fine. A second assertion at `tests/mutation/guardSurfaces.gate.test.ts:199` pins `Object.keys(EXPECTED_LEDGER_KINDS)` to the registry's surface ids, so the file is coupled to the registry in both directions. Declaring it here is the point: the sentence below says any undeclared delta is a defect, and an implementer following the old list would have had to choose between a red gate and an undeclared edit.
