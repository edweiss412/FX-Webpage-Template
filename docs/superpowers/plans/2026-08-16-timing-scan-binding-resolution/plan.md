...
**The `tests/mutation/guardSurfaces.gate.test.ts` line citations above move too, and Task 0 re-derives them with the rest.** They are `:34` (`EXPECTED_LEDGER_KINDS`), `:80` (the `interactionTimingScan` entry) and `:199` (the keys-match-the-registry assertion) as tracked on THIS branch — and already `:89` for the middle one on `origin/fix/scanner-scope-totality`, because #830 landed above them. Re-derive with `grep -n "EXPECTED_LEDGER_KINDS\|interactionTimingScan: {" tests/mutation/guardSurfaces.gate.test.ts` after the merge rather than trusting these numbers. Recorded because they were added at implementation time and would otherwise sit outside the anchor sweep the next paragraph defines.
