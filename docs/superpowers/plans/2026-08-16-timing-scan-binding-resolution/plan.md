...
- **EXTENDS:** `scripts/scan-interaction-timings.ts` (`TimingSite.refPos` AND `TimingSite.declPos`, the resolver, `scanRepo`'s resolution step, the header paragraph documenting the hole); `tests/mutation/source/registry.ts` (`interactionTimingScan` accepted set, re-derived). Two new fields, not one — an undeclared field on a registered surface is the shape this inventory exists to catch.
...
3. **Rewrite the scanner header in THIS commit** (spec AC-9): the TOTALITY IS PER-FORM paragraph whose text wraps as "… with one / hole: an identifier delay resolves by NAME, not by binding …" and its `BL-TIMING-SCAN-NAME-VS-BINDING` pointer now state the closed contract. It lands here, with the behaviour change, because every later commit to this file shifts the line-keyed `siteId`s that Task 4 re-derives — deferring it would invalidate the very score the round-1 diff brief declares. Verify with `rg -U 'resolves by NAME' scripts/scan-interaction-timings.ts` finding nothing.
