...
**Date:** 2026-08-16 · **Spec:** `docs/superpowers/specs/ci/2026-08-16-timing-scan-binding-resolution-design.md` · **Status:** plan-APPROVED after 3 substitute rounds — 32 findings across four independent fresh-eyes reviewers plus two verification rounds, all repaired in-branch; codex-guard returned no_verdict throughout (OpenAI usage limit to 2026-08-22), so no OPPOSING MODEL has read this. Spec §7 is the honest account; the implementer's round-1 diff brief carries the cross-model gate for the design as well as the diff.
...
- **Whole-repo `scanRepo` call count, ON BOTH TREES** (the §2.4 cost claim): on `origin/fix/scanner-scope-totality`, the post-#827 tree Task 1 actually runs against, six `scanRepo(REPO_ROOT)` in `tests/docs/_metaInteractionTimingInventory.test.ts` plus one `scanRepo(process.cwd())` in `tests/docs/interactionTimingScan.test.ts` = SEVEN; on this branch's pre-#827 base, three plus one = FOUR (round-3 finding 1 — both are right for their tree, and only the label was missing). Task 0 re-derives after the merge. The remaining calls in the scan test take synthetic roots holding a handful of files (~10 ms each, probe P11).
- **Mutation harness entry points:** `pnpm mutation:guards` now runs FIVE files — four shards plus a corpus-wide gates file (`VITEST_INCLUDE_MUTATION_HARNESS=1 vitest run --project mutation tests/mutation/guardSurfaces.shard0.test.ts ... shard3 ... tests/mutation/guardSurfaces.gates.test.ts`, `package.json`). The single gate file this plan was written against — guardSurfaces.gate.test.ts, formerly beside the shards under `tests/mutation/` — NO LONGER EXISTS (written as plain prose rather than as a citation on purpose: the file is no longer tracked, so citing it fails `spec:lint`): the gate was sharded on `main` while this arc was blocked on #827 (`docs/superpowers/specs/ci/2026-08-16-mutation-gate-wallclock-design.md`). The gate has NO env-based surface filter, so a scoped run goes through the gate's own code path: `runSurface(root, surface)` (`tests/mutation/source/runner.ts:230`) + `evaluateGate` (`tests/mutation/source/gate.ts:36`) + `score` (`tests/mutation/source/ledger.ts:79`). The overlay is in-memory (`tests/mutation/source/overlay.ts`), so a scoped run does not dirty the tree.
...
| **AC-8** | mutation gate green at floor 0.95 with the accepted set re-derived, both numbers in the round-1 diff brief | Task 4 — **and Task 2's SHORTHAND-RESOLVES case, which is the twelfth assertion's AC home.** It discharges no behavioural AC (no AC names the shorthand position); it exists because AC-8 demands an EMPTY unaccepted-survivor set, and probe P17 measured that without it the mutants reverting the shorthand path to `getSymbolAtLocation` survive — the live `{ duration }` does not kill them either. Recorded here so the assertion traces to a criterion rather than floating outside the map. |
| **AC-9** | the scanner header's documented hole is rewritten to the closed contract | **Task 2 step 3** (Task 5 step 1 only VERIFIES it; the rewrite lands with the behaviour change so Task 4's re-derived siteIds stay valid) |
...
| **AC-12** | a binding declared on the same line as a covered constant does not inherit its coverage | Task 1's same-line-neighbour REGRESSION PIN — green today (P16), red only against the rejected line-keyed design (P10); it discharges AC-12 without being one of Task 1's REDs |

**The mutation-gate citations above did not merely move — the file they named was RETIRED, and Task 0 retargeted them.** `main` sharded the source-mutation gate while this arc was blocked on #827, so guardSurfaces.gate.test.ts no longer exists. Its contents now sit in three places: `EXPECTED_LEDGER_KINDS` in `tests/mutation/source/expectedLedgerKinds.ts`, the per-surface ledger-kind assertion in `tests/mutation/source/surfaceCases.ts` (shared by all four shards), and the registry-completeness assertion in `tests/mutation/guardSurfaces.gates.test.ts` (plural). Re-derive by SYMBOL, never by line — `bash .review/impl-probes/task0-anchors.sh` does it, or `grep -n "EXPECTED_LEDGER_KINDS\|interactionTimingScan: {" tests/mutation/source/expectedLedgerKinds.ts`. Recorded because these were added at implementation time and would otherwise sit outside the anchor sweep the next paragraph defines. (Written without bare line numbers on purpose: `spec:lint` raises CITATION_MALFORMED for a colon-prefixed line number carrying no path, which is how this paragraph failed its own gate on first draft — twice, since the explanation of the rule tripped it too.)
...
- **CREATES:** twelve synthetic-universe assertions in `tests/docs/interactionTimingScan.test.ts`, derived from the Task 1 and Task 2 tables rather than counted by hand — **five FIRES** (an assertion that fails before the repair: module-level shadow, inner-scope shadow, parameter shadow, aliased import, and Task 2's property-value shadow), **one REGRESSION PIN** (the same-line neighbour, green today per probe P16 and red only against the rejected line-keyed design — kept to stop a future refactor re-adopting a key that aliases, never counted as a RED), and **six STAYS-QUIET** (passes before and after: the unshadowed peer in the shadowing file, a legit same-file constant, a direct import, a barrel re-export, Task 2's live-shaped `ttlMs` pass-through, and Task 2's SHORTHAND-RESOLVES case). **The twelfth is a declared addition on measured evidence (probe P17), not a silent one:** with eleven the resolver's shorthand branch is unpinned — the mutants reverting it to `getSymbolAtLocation` survive, and the live `{ duration }` does not kill them — so P12's probed API choice would ship untested. It widens no recognizer (form 3c and a §2.3 `refPos` producer already), so it sits inside §1.1 item 2's fence. The aliased import is a FIRES case rather than a quiet one because it reports `unclassified` under today's name filter and resolves after — the one place this arc removes a residual instead of adding one. One structural test pins the resolver's alias assumption against `tsconfig.json`.

  **Twelve ASSERTIONS, eleven `test(` blocks — the counts differ on purpose and a reviewer should not read the gap as a missing case.** The inner-scope shadow case carries TWO of the twelve, because its whole point is the pair: the shadowed call reports AND the unshadowed call in the same file stays resolved. Splitting them would need two fixtures where one file is the discriminator, and a repair that reported both would then pass the first block while failing the second — the precision claim is only meaningful when both are asserted against the SAME universe. So: 8 Task 1 cases + 3 Task 2 cases = 11 blocks, plus case 2's second half = 12 assertions, plus 1 structural alias pin outside the twelve.
- **EXTENDS:** `scripts/scan-interaction-timings.ts` (`TimingSite.refPos` AND `TimingSite.declPos`, the resolver, `scanRepo`'s resolution step, the header paragraph documenting the hole, and the exported alias-mapping constant Task 3 pins against `tsconfig.json`); `tests/mutation/source/registry.ts` (`interactionTimingScan` accepted set, re-derived). Two new fields AND one new export — an undeclared addition to a registered surface is the shape this inventory exists to catch, and round 3 found this very list omitting the export one line under that sentence.
- **ALSO EXTENDS — the sharded mutation gate, the companion this inventory omitted until implementation time.** `EXPECTED_LEDGER_KINDS` (`tests/mutation/source/expectedLedgerKinds.ts`, the `interactionTimingScan` entry, today `{ equivalent: 8 }`) is asserted per surface by `tests/mutation/source/surfaceCases.ts`: `surface.accepted` is reduced to per-kind counts and compared with `toEqual`. That case is shared by all four shards, so it runs wherever this surface is packed. The moment Task 4's re-derivation changes the accepted count — which this plan already anticipates, and which probe P17 predicts (roughly six new `equivalent` rows for the resolver's defensive and memoization guards) — that entry must move in the SAME commit or the gate fails on a surface whose score is fine. A second assertion, in `tests/mutation/guardSurfaces.gates.test.ts`, pins `Object.keys(EXPECTED_LEDGER_KINDS)` to the registry's surface ids, so the map is coupled to the registry in both directions and a surface missing a row fails by default. Declaring it here is the point: the sentence below says any undeclared delta is a defect, and an implementer following the old single-file list would have found no such file at all.
...
| same-line neighbour | `const CLOSE_DELAY_MS = 220, other = readConfig();` with `setTimeout(fn, other)` | `other` appears in `unclassified` | **regression pin** — GREEN on the current tree (probe P16), red only against the rejected line-keyed design (P10). Not one of this task's REDs. |
...
**Four pre-dispatch mutants per fires case — and two of the six fires cases assert an ABSENCE, so they take their own set (round-2 finding 3).** For the aliased-import case, whose assertion is "no residual": (a) alias the import to a name no file declares — the residual must APPEAR, proving the assertion tracks resolution and not silence; (b) delete the covered export — the residual must APPEAR; (c) comment out the `setTimeout` — nothing appears, proving the row comes from the call site; (d) rename the alias — the residual must stay absent, proving the LOCAL spelling is not what resolves it. For the same-line-neighbour REGRESSION PIN, whose assertion is "`other` reports": (a) move `other` to its own line — it must still report, since the repair keys on offsets and not lines; (b) rename `other` to `otherMs` AND give it a numeric literal initializer — it becomes its own covered row and the residual goes. **Both halves are required, and the draft asked for only the second (verified by running the protocol before Task 1 wrote a line).** `pushNamed` gates on `TIMING_NAME` before it looks at the initializer, and `other` does not match that pattern, so `const CLOSE_DELAY_MS = 220, other = 500;` produces NO `named-constant` row and the delay STILL reports — the mutant fails while the code is correct. Measured: `other = 500` leaves `residual=["other"]` with `named-constants=["CLOSE_DELAY_MS=220"]`, whereas `otherMs = 500` gives `residual=[]` with `otherMs=500` inventoried. The control that separates the two gates is `otherMs = readConfig()`, which reports under its own name; (c) comment out the `setTimeout` — the residual goes; (d) rename the covered constant on that line — `other` must still report, proving nothing about the neighbour's spelling is load-bearing.

**The four shadow-shaped fires cases take the original set** (the string-presence discipline applied to the `name` assertions): (a) the shadow's initializer emptied to a numeric literal — the case must go quiet, proving the assertion tracks non-literal-ness and not the file's existence; (b) the shadow identifier suffixed (`COPY_FEEDBACK_RESET_MS2`) — the residual must still appear but under the new name, proving the assertion reads the site and not a constant; (c) the `setTimeout` call commented out — the residual must vanish, proving it comes from the call site rather than the declaration; (d) the covered export deleted from the scanned file — the residual must remain, proving the case does not depend on the covered row existing.
...
3. A new module-local `createBindingResolver(repoRoot, sources)` takes the `(path, text)` pairs `scanRepo` ALREADY READ — not a path list — and serves them through a `ts.CompilerHost`, so the resolver never touches the filesystem. Two consequences the plan depends on: `refPos` offsets are guaranteed to index the same text the sites were computed from (spec §2.3), and the existing unreadable-file case (a universe file chmod'd `0o000`) keeps behaving exactly as today, because a file `scanRepo` could not read simply is not in the pairs. It returns `(file, pos, name) => readonly declarationKey[]` — **EVERY** declaration key of the resolved symbol, empty when nothing resolves — where `declarationKey` is `${file}:${startOffset}` of that declaration's name node via `ts.getNameOfDeclaration(decl) ?? decl`, alias-followed with a guarded `getAliasedSymbol`, and `getShorthandAssignmentValueSymbol` on the shorthand path.

    **Two corrections to the draft signature, both reconciling this step with the spec rather than deciding anything new.** (i) It said `(file, pos) => declarationKey | null`, a SINGLE key — which cannot express what step 4 one line below already demands ("suppresses a delay site only when SOME declaration of the resolved symbol is in that set") and which spec §2.2 rule 2 states normatively. A single-key return has to pick one declaration, and on probe P8's shape (`export const TTL_MS = 500` beside `export type TTL_MS = number`, ONE symbol with a `VariableDeclaration` and a `TypeAliasDeclaration`) picking the type would report a covered constant as `unclassified` — the exact regression P8 was run to prevent. (ii) The draft omitted the `name` parameter, which spec §2.5 requires: the resolver checks that the node at `refPos` IS an `Identifier` whose text equals the site's `name`, and treats a mismatch as unresolved, because a mis-anchored token is the one path that could resolve a DIFFERENT binding. The spec is canonical (invariant 7); this is the plan being brought into line with it, not a design change.
...
<!-- task: red=`pnpm exec vitest run tests/docs/interactionTimingScan.test.ts` red-state=authored red-target=`scripts/scan-interaction-timings.ts:639` why=`the property-shadow case asserts a residual for a ttlMs property whose value is a local non-literal binding sharing a covered constant's spelling; Task 1 leaves that path on the name set, so the case fails until the set is deleted` ac=AC-4,AC-6 -->

**What is red and why:** one new fires case (`{ ttlMs: SHADOW }` in a file that shadows a covered constant) plus TWO stays-quiet cases (the live `ttlMs: ANNOUNCE_LOG_TTL_MS` shape reproduced in the synthetic universe, and the shorthand-resolves case below). The first fails while the property path still consults `coveredNames`.

**The shorthand-resolves case (stays-quiet, probe P17).** `const duration = 300; const opts = { duration };` in one scanned file, asserting NO residual. Its premise assertion is the same shape as every other quiet case. It is not a RED and must not be counted as one — it passes before and after — but it is REQUIRED rather than decorative: it is the only assertion that kills the mutants reverting the shorthand path from `getShorthandAssignmentValueSymbol` to `getSymbolAtLocation`, and the live `{ duration }` at `components/crew/CrewSectionTransition.tsx` does not kill them. Without it, the API choice probe P12 established ships unpinned and the mutation gate carries two survivors nothing accounts for.
...
3. **Rewrite the scanner header in THIS commit** (spec AC-9): the TOTALITY IS PER-FORM paragraph whose text wraps as "… with one / hole: an identifier delay resolves by NAME, not by binding …" and its `BL-TIMING-SCAN-NAME-VS-BINDING` pointer now state the closed contract. It lands here, with the behaviour change, because every later commit to this file shifts the line-keyed `siteId`s that Task 4 re-derives — deferring it would invalidate the very score the round-1 diff brief declares. Verify with TWO greps: `rg -U 'resolves by NAME' scripts/scan-interaction-timings.ts` finds nothing, AND a positive grep for the new contract text finds it. The absence check alone is satisfied by DELETING the paragraph, which is not a rewrite (round-3 finding 5).
4. Live-tree assertions: `scanRepo(REPO_ROOT).unclassified` stays empty and `inventoryRows` still matches §5.5 — the existing meta-test asserts both directions, so a lost resolution and a wrongly-gained one each fail it.
...
2b. **Update `EXPECTED_LEDGER_KINDS.interactionTimingScan` in `tests/mutation/source/expectedLedgerKinds.ts` to the new per-kind counts, in this same commit.** The shared per-surface case in `tests/mutation/source/surfaceCases.ts` reduces `surface.accepted` by `kind` and compares with `toEqual`, so a re-derivation that changes the count reds the gate on an otherwise healthy surface. Today `{ equivalent: 8 }`; probe P17 predicts roughly six new `equivalent` rows. Prefer `pnpm mutation:sites scripts/scan-interaction-timings.ts` for step 1 over a hand-rolled `enumerateSites` call — it reads the registry from disk, prints each row's current id, and ends with a resolve/stale verdict.
...
1. The header rewrite is NOT here — spec AC-9 puts it in the same commit as the behavior change, and Task 2 lands it. This step only verifies it happened, and with both greps: `rg -U 'resolves by NAME'` must find nothing AND the new contract text must be present. **Why the ordering is load-bearing:** `siteId`s are `<operator>:<line>:<col>:…`, so ANY edit above an accepted row shifts it. Rewriting the header here would re-staleify the set Task 4 just re-derived, and the round-1 diff brief's `MUTATION SCORE` would be computed against an accepted set a later commit invalidates. **Task 3 is the last edit this arc makes to `scripts/scan-interaction-timings.ts`** (it exports the alias mapping as a named constant), and Task 4 touches only `tests/mutation/source/registry.ts` — so the accepted set Task 4 re-derives is derived from the scanner's final text. Task 4 must not edit the scanner.
2. Record the measured cost (AC-10): the two suites' wall clock and the `mutation:guards` wall clock, before and after. **Either budget breaching lands the §2.4 memo fallback** — the suites gaining more than 2 s, OR the mutation gate gaining more than 25% — with its own before/after measurement, never a weaker resolver.
...
3. Record the BEFORE numbers BOTH of AC-10's budgets compare against: `time pnpm exec vitest run tests/docs/interactionTimingScan.test.ts tests/docs/_metaInteractionTimingInventory.test.ts` for budget (a), and for budget (b) the pre-change `pnpm heavy pnpm mutation:guards` **wall clock as well as its score** — the score alone leaves (b) with nothing to compare against, and an implementation whose harness gains 60% while the suites gain 1.4 s would pass every step of this plan while violating AC-10 (round-3 finding 2).
