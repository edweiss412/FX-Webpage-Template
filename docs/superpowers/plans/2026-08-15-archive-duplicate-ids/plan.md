...
6. **Survivor verification (plan R2 F1; artifacts authored, run, and committed AT
   PLAN TIME per plan R3 F2/F3):** the expectations table
   `docs/superpowers/plans/2026-08-15-archive-duplicate-ids/survivor-expectations.json`
   (43 rows: `{ id, file, survivorHeadingText, demotedHeadingText }`, generated by
   the committed
   `docs/superpowers/plans/2026-08-15-archive-duplicate-ids/gen-survivor-expectations.mts`,
   which self-validates every row against the live tree and the spec §2.1 direction
   column) and the verifier
   `docs/superpowers/plans/2026-08-15-archive-duplicate-ids/verify-survivors.mts`
   both ALREADY EXIST on this branch. The verifier asserts per row: survivor text
...
3. Pre-push gates: `pnpm heavy pnpm test:fast` (full suite under the slot
   semaphore), `pnpm typecheck`, `pnpm exec eslint .`, `pnpm format:check`, AND
   `pnpm exec tsx docs/superpowers/plans/2026-08-15-archive-duplicate-ids/verify-survivors.mts`
   green — re-run after EVERY later `origin/main` merge that touches a ledger file
   (plan R3 F1: a conflict-resolution edit reversing one pair passes the docs
   suites; only the verifier discriminates direction).
