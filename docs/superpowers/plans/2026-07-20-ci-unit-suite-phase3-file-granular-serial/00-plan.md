...
**Goal:** Move the spike-verified DB-free files (515 (final, after the spec §2.6 eligibility amendment), measured per spec §2) out of the serial vitest project via a new `PARALLEL_EXTRA_FILES` list, proven by a resolved-config partition test. Automating regeneration is descoped to `BL-CI-SERIAL-AUDIT-SCRIPT` (spec §1.0).
