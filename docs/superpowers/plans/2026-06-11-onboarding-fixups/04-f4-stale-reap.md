...
- `tests/db/postgrest-dml-lockdown.test.ts` — **VERIFY-ONLY here (R61-1): the EXTEND (registry rows) + REVOKE migration are F1 Task 1.3 deliverables — they land BEFORE any created_show_id consumer. Task 4.7 only re-runs the suite.** (Historical rationale, R14 HIGH:) The earlier rationale ("all mutated tables already flow through server-side SQL") conflated *how our code mutates* with *what PostgREST exposes*: `onboarding_scan_manifest`, `wizard_finalize_checkpoints`, and `shows_pending_changes` still `GRANT insert, update, delete` to `anon, authenticated` (`supabase/migrations/20260501002000_rls_policies.sql:179/:203/:211`), so every guard this milestone builds (finalize/show lock ordering, 24h freshness, `created_show_id` provenance, F5 currency predicates) is bypassable by a direct `from('<table>')` write that never enters the guarded SQL paths. Task 4.7 ships the REVOKE migration + registry rows in the same commit per the lockdown discipline.
      ...
      `
        delete from public.shows s
         ...
         where m.wizard_session_id = $1::uuid
           ...
           and m.drive_file_id = s.drive_file_id  -- R48-2: provenance binding
           and s.wizard_created_session_id = m.wizard_session_id  -- R57-1: show-side discriminator
