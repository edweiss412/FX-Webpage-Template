...
async function seed() {
  ...
  const [created] = (await db.unsafe(
    `insert into public.shows (drive_file_id, title, slug, published, client_label, template_version, wizard_created_session_id)
     values ($1, 'Wizard interim', 'f4-interim', false, 'Fixture Client', 'v2', $2::uuid)  -- $2 = SESSION
     -- R63-1: session-created shows MUST carry the show-side discriminator; the cleanup/reap SQL joins on it.
     -- (extend the insert column list with wizard_created_session_id and pass SESSION as a parameter)
     returning id`,
