...
- `tests/db/postgrest-dml-lockdown.test.ts` — **VERIFY-ONLY (R64-1): migration + registry rows are F1 Task 1.3 deliverables, landing before any created_show_id consumer. Task 4.7 only re-runs the suite and confirms 42501 on all three tables; it ships nothing.** The original R14 rationale (PostgREST exposure bypasses every F4/F5 guard) is why the lockdown exists — it is implemented in F1.
