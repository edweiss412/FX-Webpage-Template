...
**Choice-aware feed inputs (plan R24-2):** Phase D must NOT pass raw `parsed.triggeredReviewItems` as `notableItems` — the live feed writer derives `crew_renamed` rows from every MI-12/13/14 item, which mislabels an `independent` choice (independent = remove+add, not a rename; the matrix marks this distinction load-bearing). The choice-semantics dispatch (R11) ALSO filters/transforms the feed inputs: items resolved `reject` are excluded entirely; items resolved `independent` are passed in their remove+add form (or the feed writer API is extended to accept validated choices — pick whichever matches the live writer's contract after reading `lib/sync/changeLog/writeAutoApplyChanges.ts`). Required DB regression: MI-13 and MI-14 `independent` wizard shadows → `show_change_log` contains NO `crew_renamed` row for those names and the emitted rows match the applied remove+add semantics. **R33-2 pinning:** this is the g2 regression's REQUIRED assertion set — g2 seeds BOTH MI-13 and MI-14 independent choices and asserts (i) zero `crew_renamed` rows in `show_change_log` for those names, (ii) the remove/add-appropriate rows ARE present, (iii) crew + audit derived_side_effects as before; the implementation step names the exact `choiceAwareFeedItems(items, validatedChoices)` signature inside `applyStagedCore` (post-validation) as the only feed-input constructor; concrete failure mode: the feed/undo surface displays a rename the operator explicitly declined to treat as one.
...
7. `runPhase2(applyTx, { driveFileId, mode: "manual", fileMeta, parseResult, skipDiagramsWrite, snapshotAssetsForApply?, autoPublishFirstSeen?, firstSeenPublished?, …(args.wizardCreatedSessionId ? { wizardCreatedSessionId: args.wizardCreatedSessionId } : {}) /* R65-1 */, verifyReelOnApply: false, …(mi11Items.length > 0 ? { mi11Items } : {}), …(feedItems.length > 0 ? { notableItems: feedItems } : {}) /* R36-1: feedItems = args.feedPolicy.kind === "choice_aware" ? choiceAwareFeedItems(items, validatedChoices) : [] — computed INSIDE the core after step 3's validation; callers never pass notableItems */, binding: { bindingToken: stagedModifiedTime, modifiedTime: stagedModifiedTime } })`. P2-F6 (`phase2.ts:205`) remains the structural guard against an MI-11 apply with no hold port.
8. **`firstSeenPublished` AND `wizardCreatedSessionId` threading (R30-1 + R65-1, both in THIS task):** `Phase2Args`, `Phase2Tx.applyShowSnapshot` args, `runPhase2`'s forwarding, and `PostgresPipelineTx.applyShowSnapshot`'s first-seen INSERT all gain BOTH optional fields; when `wizardCreatedSessionId` is set the INSERT column list includes `wizard_created_session_id` with the value, byte-identical SQL when absent; SQL-string unit checks cover set/absent for BOTH fields. Original `firstSeenPublished` spec: `Phase2Args` (`phase2.ts:58-89`) and `Phase2Tx.applyShowSnapshot` args (`:33-54`) gain `firstSeenPublished?: false`; `runPhase2` forwards it into the `applyShowSnapshot` call (`:253-263`); `PostgresPipelineTx.applyShowSnapshot` first-seen arm (`runScheduledCronSync.ts:1074-1097`) uses an INSERT variant adding the `published` column with literal `false` when the flag is set, byte-identical SQL when absent. Unit test in THIS task: flag set → insert includes `published=false`; flag absent → SQL unchanged (string-compare against the live SQL). Task 1.1's commit typechecks standalone.
8. `stale` → `stale_write`.
...
- [ ] **Implementation:**
  ...
  4. **`processApprovedRow` first-seen branch** (`:597-606`): parse items via `parseTriggeredReviewItems(row.triggered_review_items)` (`lib/staging/triggeredReviewItems.ts` — parseable by construction for approved rows per `applyStaged.ts:1229-1231`; on `!ok` THROW `new Error("approved row has corrupt triggered_review_items")` → the route's typed-500 wrapper). Then replace the `applyFirstSeenDraft` + `insertFinalizeAudit` pair with:
     ...
     const core = await applyStagedCore(lockedTx, {
       ...
       // asserts shows.wizard_created_session_id = SESSION (R65-1: a hard `expect(show.wizard_created_session_id).toBe(SESSION)` line in the first-seen DB test — not prose), and the Phase D publish-flip regression seeds its session-created row WITH the discriminator (or uses a real Phase-B-created row). (R60-1: the SELECT above now includes the column; assert strictly equal to the seeded session id, not just non-null.)
...
- [ ] **Validation project apply (post-migration checklist, same PR — R61-1: BOTH F1 migrations):** `supabase db query --linked "$(cat supabase/migrations/20260611000000_onboarding_manifest_created_show_id.sql)"` then `supabase db query --linked "$(cat supabase/migrations/20260611000002_lockdown_wizard_staging_tables.sql)"` then `supabase db query --linked "notify pgrst, 'reload schema';"`. F1 exit criteria require schema-manifest regen + validation parity covering BOTH migrations, and `pnpm vitest run tests/db/postgrest-dml-lockdown.test.ts` green against validation.
...
- [ ] **Implementation:**
  ...
  2. Rewrite `applyShadow` (`:241-306`):
     ...
     async function applyShadow(tx, pipelineTx, row): Promise<ShadowApplyResult> {
       ...
       const core = await applyStagedCore(lockedTx, {
         ...
         // its reviewer-choice validation (the validated/normalized choices are not in scope here, and
         // validating twice would drift). The core computes notableItems = choiceAwareFeedItems(items,
         // validatedChoices) itself: reject-resolved items excluded; independent-resolved items in
         // remove+add form so writeAutoApplyChanges (phase2.ts:340-372) cannot derive crew_renamed for
         // a choice the operator declined. applyShadow passes ONLY the raw payload fields; tests cover
         // invalid choices (typed refusal) + reject/independent feed inputs. D-2
         skipDiagramsWrite: false,                                                    // payload diagrams already canonical (spec §3.4)
       });
  ...
  4. Narrow `publishAppliedWizardShows` (`:338-356`):
     ...
     **Publish-flip lock topology (plan R49-2 — invariant 2):** the flip is a `shows` mutation and must run under per-show advisory locks. Implementation: before the joined UPDATE, the Phase D tail SELECTs the session-created manifest drive ids (`status='applied' and created_show_id is not null`), sorts them, and acquires `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))` for each within the same outer transaction (which per R16-1 already holds finalize: → app_settings — per-show locks acquired LAST preserves the global order; the per-row apply loop's locks were released when its row transactions committed, so the tail re-acquires). Extend the advisory-lock topology meta-test to pin `publishAppliedWizardShows` as a lock-held surface — the second-copy tripwire allowlist entry is NOT a lock exemption. Regression: concurrent manual sync for one of the flipped shows serializes against the flip (no interleaved visibility write; no 40P01 — same total order). R50-1 regression: insert an extra matching manifest row AFTER the lock-set SELECT (side connection) → the UPDATE does NOT publish that drive id (set-bound), and a follow-up Phase D run (or finalize re-fire) handles it under its own locks; structural test asserts the UPDATE carries the any($2) bound (SQL-shape scan).
     ```
