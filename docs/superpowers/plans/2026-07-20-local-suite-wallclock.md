...
- [ ] **Step 4c: AC-2 CI arm + lever-B keep/drop decision.** After the PR is open, confirm all 8 `unit-suite` legs are green AND compare max leg duration against recent `main` runs; keep `pool: "threads"` only on a >=15s improvement, else revert that one line in-PR (spec 4.2 decision procedure) (`gh pr checks <PR#> --watch`, then `gh pr view <PR#> --json mergeStateStatus` = CLEAN). Local green is NOT sufficient for this AC.
