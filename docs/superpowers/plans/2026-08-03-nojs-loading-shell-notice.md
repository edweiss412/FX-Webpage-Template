...
|---|---|---|---|---|
| 1 | critique | **P1** | The notice inherited no page padding. Seven of the nine `loading.tsx` routes put their layout padding on a child of `data-loading-shell-content` — the half the rule hides — so the card ran flush to both edges of a 390px phone on the crew route (`app/show/[slug]/layout.tsx:37` adds none) and stretched past 1500px on wide admin. | **Fixed** (`9475a1657`). The notice carries its own `mx-auto w-full max-w-2xl px-4 py-8 sm:px-8` gutter, pinned by a component assertion. |
...
| 4 | audit | P2 | Type scale one to two steps below every comparable dead-end screen — `ShowUnavailable.tsx:33-38`, `app/admin/layout.tsx:95-96`, `app/admin/layout.tsx:95-96` all pair `text-2xl` with `text-base`, while the notice carried the page's only instruction in the caption token. | **Fixed.** Now `text-2xl` heading + `text-base` body. Same job, same vocabulary. |
