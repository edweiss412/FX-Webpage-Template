...
- CREATES a new suite waveCodeAnchors.test.ts under `tests/drive/` (Task 3: replay equals parse over the corpus and six constructed variants; the four refusals; the dispatching workbook's five cells) and a new suite waveCodeAnchorsSeams.test.ts beside it (Task 6: the parse-side sequence equals the replay on workbooks where a seam demonstrably fired).
...
- NEW suite waveCodeAnchors.test.ts under `tests/drive/`; NEW suite waveCodeAnchorsSeams.test.ts under `tests/drive/` (Task 6); NEW suite waveCodesSourceCell.test.ts under `tests/parser/`; DELETE `tests/parser/waveCodesNoSourceCell.test.ts`; EDIT the suites listed in §2.
...
<!-- task: red=`rg -q "^impeccable-gate: critique=RAN audit=RAN" docs/superpowers/plans/2026-08-29-ref-error-cell-anchors-closeout.md` red-state=live red-target=`tests/docs/_invariant8Closeout.ts:45` why=`the stem-named sibling closeout file does not exist, so rg exits 2 (no such file); it exits 0 once this task writes the sibling carrying the RAN/RAN marker, whose form the walker's MARKER regex at that line then holds through the closeout guard. This plan itself deliberately never names both gate halves (the walker's declaresGate fold needs both, so naming them here before the marker exists would red the guard); the sibling is what makes the unit declare` ac=AC-5,AC-6,AC-7 -->
...
- [ ] **Step 1: RED.** The task command (`rg -q` on the sibling) exits 2: no such file. `pnpm vitest run tests/docs/_metaInvariant8Closeout.test.ts` is GREEN here and stays green, because this plan does not declare the gate (see the marker's `why=`); it becomes the sibling's form check once the sibling exists.
