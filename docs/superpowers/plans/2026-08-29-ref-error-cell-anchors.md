...
- [ ] **Step 1: RED.** The task command (`test -f <sibling> && rg -q ...`) exits 1: no sibling. `pnpm vitest run tests/docs/_metaInvariant8Closeout.test.ts` is GREEN here and stays green, because this plan does not declare the gate (see the marker's `why=`); it becomes the sibling's form check once the sibling exists.
