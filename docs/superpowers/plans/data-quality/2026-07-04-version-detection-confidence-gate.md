...
**Spec:** `docs/superpowers/specs/data-quality/2026-07-04-version-detection-confidence-gate-design.md` (Codex-APPROVED, 7 rounds).
...
| `lib/sync/runScheduledCronSync.ts` | `updateShowParseError` | Persist `` `${code}: ${message}` `` into `shows.last_sync_error` (was code-only) so the existing-show diagnostic survives. |
| `tests/sync/runScheduledCronSync.test.ts` | source-pin | Assert the `updateShowParseError` SQL persists the message (fake-vs-prod divergence guard). |
...
- Modify: `lib/sync/runScheduledCronSync.ts` (`updateShowParseError` — Step 5b)
- Regenerate: `lib/messages/__generated__/spec-codes.ts`, `lib/messages/__generated__/internal-code-enums.ts`
- Test: `tests/parser/parseSheet.test.ts`, `tests/invariants/mi.test.ts`, `tests/sync/runScheduledCronSync.test.ts`
...
- [ ] **Step 5b: Persist the scored diagnostic on the existing-show path** (spec §4.3 "Existing-show persistence fix")

The existing-show hard-fail branch (`phase1.ts:296-299`) calls `updateShowParseError(driveFileId, { code, message })`, but the production impl (`runScheduledCronSync.ts:809-828`) writes `shows.last_sync_error = $2` with only `error.code`, discarding the message. The phase1 test fake (`phase1.test.ts:212`) already writes `` `${error.code}: ${error.message}` `` — production must match it. In `runScheduledCronSync.ts`, change the `updateShowParseError` query params from:
...
Do NOT touch the other two `last_sync_error = $2` writers (`:855` sheet_unavailable, `:881` drive_error) — different error paths, out of scope.

Add a structural source-pin so the fake cannot silently diverge again — append to the existing SQL-source test in `tests/sync/runScheduledCronSync.test.ts` (near the `last_sync_error = $2` assertion at `:617`):
...
Run:
```bash
pnpm vitest run tests/parser/parseSheet.test.ts tests/invariants/mi.test.ts tests/sync/runScheduledCronSync.test.ts
pnpm test:audit:x1-catalog-parity
pnpm test:audit:x2-no-raw-codes
pnpm vitest run tests/parser/schema.test.ts
```
Expected: PASS all. x1 confirms catalog↔spec↔scenarios lockstep (keys + 4-field parity); x2 confirms the producer is in the regenerated internal-code-enums; the runScheduledCronSync source-pin (Step 5b) confirms production persists code+message.
...
git add docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md \
  ...
  lib/parser/index.ts lib/parser/invariants.ts lib/sync/runScheduledCronSync.ts \
  tests/parser/parseSheet.test.ts tests/invariants/mi.test.ts tests/sync/runScheduledCronSync.test.ts
...
it("existing-show ambiguous parse hard-fails, retains last-good, and persists the scored diagnostic", async () => {
  ...
  // `${code}: ${message}`, matching the production fix in Task 2 Step 5b).
