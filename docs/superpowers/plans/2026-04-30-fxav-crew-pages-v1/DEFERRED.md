...
**2026-06-22 partial re-add (owner-directed).** A real incident exposed the cost of zero protection: PR #51 auto-merged over a RED `quality` check (the lint/type/test/format gate), landing a prettier-unformatted file on `main` (fixed by hotfix PR #61). Per owner decision, MINIMAL branch protection was re-PUT on `main` requiring ONLY the `quality` status check (`strict: true`), with `required_approving_review_count = 0`, `enforce_admins = false`, and force-push + deletion blocked — i.e. CI-gated merges WITHOUT the team-workflow review/admin contract R4 flagged as solo-incompatible. This deliberately sidesteps the deadlock above: with no review requirement, solo self-merge still works, but a red `quality` can no longer merge. It is NOT a full X6-D-1 reopen — the audit checks (`x1`–`x6`, `traceability-audit`, `verify-branch-protection-status`) remain ADVISORY (not in the required set), and both `verify-branch-protection` + `verify-branch-protection-status` jobs stay `if: false` (the drift-detector still hardcodes the team contract; enabling it against this solo config would exit 1 / deadlock per R4). When X6-D-1 fully reopens, the re-PUT should ADD the audit checks to the required set alongside `quality` and re-enable the jobs per the M12.1 note above.
...
## M12.5-DEF-1 — self-revoke refusal at the RPC (DB) boundary, not only the Server Action — **RESOLVED 2026-06-21**

**Status: Resolved 2026-06-21** on branch `deferred/m12.5-def1-self-revoke-rpc`. New migration `supabase/migrations/20260621000000_revoke_admin_refuse_self_revoke.sql` redefines `revoke_admin_email_rpc` to return `self_revoke_forbidden` whenever `v_canonical = v_actor_canonical` (peer-count branch dropped). Added the `SELF_REVOKE_FORBIDDEN` §12.4 code via the three-lockstep (spec prose + helpfulContext appendix, `pnpm gen:spec-codes`, `lib/messages/catalog.ts`); x1-catalog-parity green. Mapped the status through `lib/data/adminEmails.ts` (new outcome kind, transitional acceptance of BOTH `last_admin_lockout` and `self_revoke_forbidden`) + `revokeAdminAction` + `RevokeRowButton` (renders the dedicated message). `tests/db/admin-emails.test.ts` flipped to assert self-revoke is refused even with peers. Orchestrator close-out: surgical apply to validation project `vzakgrxqwcalbmagufjh` + cross-model adversarial review before merge.
...
- **A3 — east-coast "Hotel Stays" guest-name split.** REVERTED: the cell `Four Seasons Fort Lauderdale Doug--- 103317 …` is token-shape-ambiguous — `"Lauderdale Doug"` (hotel-word + name) is indistinguishable from consultants' `"Doug Larson"` (name + name) by any regex, and the corpus mixes 1-word (east-coast) and 2-word (consultants) name conventions. Needs a name/place dictionary or a structured source. Names collapse to 1 + glued hotel_name on east-coast remains (one v1 show; low impact). **Update (conf#-privacy branch):** the confirmation NUMBERS are now stripped from `hotel_name`/`names` (privacy — `stripConfTokens`, dash + bare 6+ digit), so e.g. east-coast `hotel_name` is `"Four Seasons Fort Lauderdale Doug Carl Eric W"` (no conf#) vs `origin/main`'s `"…Doug--- 103317 Carl –- 103316 Eric W--- 110525"`. The remaining glued guest NAMES are NOT a privacy leak (those names are already in the show-wide-readable `names[]`); removing them is the same A3 ambiguity (an aggressive strip would drop hotel words like "Fort Lauderdale"). Owner decision 2026-06-19: ship the conf#/entity fix, keep the guest-name residue deferred here. Re-enable needs the name/place dictionary or a structured Names-on-Reservation source.
...
- ~~**D1 — fail-loud `SECTION_HEADER_NO_FIELDS` warning.**~~ **RESOLVED (owner go 2026-06-19).** `emitEmptySection(agg, section)` (`lib/parser/warnings.ts`) emits a `severity:"warn"` ParseWarning code `SECTION_HEADER_NO_FIELDS` when a block parser recognizes a section header but extracts zero fields, wired into all 6 block parsers (event/transport/hotels/rooms/dates/contacts) — each fires only on header-present AND empty-result, silent when the section is absent. **NO §12.4 catalog code needed** (design fan-out's admin-surfacing trace was decisive: parser warnings render from `.message` in StagedReviewCard / `/admin/dev`, never through `lib/messages/lookup.ts`, so invariant 5 / x1-catalog-parity doesn't govern them). Code emitted as a string LITERAL so `extract-internal-code-enums.ts` records it (x2 no-raw-codes coverage — AR R1). Admin-log-only — does NOT block apply (warning channel, not hardError). Detectors match the parser's own anchored/exact-cell headers, NOT substrings (AR R2 caught `Get Hotel Reservations | FALSE` control-row false-positives). Zero spurious warnings on the 7 exporter shows. Codex AR APPROVE R3. Tests: `tests/parser/d1-empty-section-warning.test.ts`.
...
- **#4 sub-item — hotel confirmation numbers.** **DEFERRED (AR R1/R2/R4, privacy — `confirmation_no` is parsed-but-NOT-persisted; always `null`).** The split landed the safe half: `parseGuestCell` splits the (`&#10;`- OR space-delimited) "Names on Reservation" cell into clean per-guest names with the `<dash> #?<digits>` conf# stripped OUT (fixes the rpas `&#10;`-in-names leak + glued names — see #8). The conf# **value** is NOT stored, for three compounding reasons surfaced across the adversarial review:
  - **R1** — a row-level `confirmation_no` reaches EVERY listed guest (`getShowForViewer` filters reservations by name; `LodgingTile.tsx:131` renders it), so per-guest conf#s on a shared row leak across guests.
  - **R2** — guests are sometimes space-delimited (raw RPAS `Douglas Larson - #2069854 John Carleo - #2069855`), so a "single-guest" heuristic is unreliable.
  - **R4 (decisive)** — even a *correctly* single-guest conf# is unsafe: `hotel_reservations` is **show-wide crew-readable** (RLS `crew_read` = `public.can_read_show(show_id)`, `grant select … to authenticated`, `20260501002000_rls_policies.sql:272-288`), and crew hold a real Google Supabase session, so any crew member on the show can `select … from hotel_reservations` directly via PostgREST and read every row's `confirmation_no`, bypassing the projection's name filter.
  **Trigger to re-enable:** (a) a per-guest schema (`names: {name, confirmation_no}[]`) so a conf# is bound to its guest, AND (b) a per-viewer read boundary — either RLS that restricts `confirmation_no` to rows whose `names` match the caller, or an RPC that returns only the viewer's conf# — with a same-show non-owner RLS/runtime test proving another crew member cannot select it via PostgREST. Until then the parser keeps `confirmation_no: null`. Regression: `confirmation_no` is null for ALL reservations across all 7 shows; names still split correctly (rpas → `["Douglas Larson","John Carleo"]`).
