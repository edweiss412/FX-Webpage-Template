...
- ~~**#4 sub-item — hotel per-guest confirmation numbers dropped (`confirmation_no` null 7/7).**~~ **RESOLVED.** Table path: `parseGuestCell` splits the `&#10;`-separated guest cell into clean names + lifts each `<dash> #?<digits>` conf#. Inline path: a separate conf# scan in `buildInlineHotel` (does not touch the AR-hardened name regexes). Per-reservation conf#s join into `confirmation_no` (consultants `2035940, 2035941, 2035938 | 2035937`; rpas/fintech/ria/east-coast likewise); names stay clean for crew↔name matching. fixed-income (`David Johnson / Jeffrey Justice`, slash-separated, no conf#) + redefining (no conf# in cell) correctly stay null. Regressions cover table + inline paths.
