...
**Plan file:** `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/07-asset.md`
...
- [x] **Shared Drive support across file-level Drive APIs** — **CREATE `tests/sync/_sharedDriveSupportContract.test.ts`** (introduced for full-M7 R3 HIGH; narrowed at R3.1 after R2's revisions.* finding proved a false positive — see convergence log R3.1 entry). Static-analysis test scans `lib/sync/**`, `lib/drive/**`, and `app/api/**` for every `.files.(get|list)` call shape. Every `files.get`/`files.list` must include `supportsAllDrives: true`; every `files.list` must additionally include `includeItemsFromAllDrives: true`. **The test deliberately does NOT cover `revisions.*` calls** — those don't accept the flags (Drive v3 typings reject them; the REST API silently ignores them; revisions inherit access from the parent file's grant). Violations name the offending `file:line`.
...
**Final UI-session HEAD.** R26 attests through commit `4808576` (this convergence-log entry); the last code-changing fix landed at `68bfa48`. The R20-R26 chain lives across `3b8efca`, `752260e`, `9945396`, `51ba0c8`, `02dda36`, `68bfa48` + `c069893` (AGENTS.md rule codification). The full-M7 adversarial review against `ae6f0b8..HEAD` converged at R7 APPROVED — see the section below.

#### Full-M7 adversarial review — R1 → R7 APPROVED
...
| R5 | `review-mp1wyu1z-zexvxr` | TWO HIGHs. Finding 2: reel `revisions.get` was passed `supportsAllDrives`, which Drive v3 revisions APIs do not accept (R3.1 contract violation that slipped past the narrowed meta-test because the test had no NEGATIVE assertion against `revisions.*` carrying the flag). Finding 1: HEAD/GET Range parity still broke when upstream size was unknown; HEAD returned 200/no `Content-Range`, while GET forwarded Range and could return 206. R5 anchored to `ae6f0b8..HEAD` per the full-milestone-scope rule. | F2: `fcd6588` (drop flag from reel revisions.get + tighten local ReelDriveClient type + add negative-assertion meta-test). F1: `c7fbf20` (GETs strip Range when size unknown to match HEAD's 200) + `0511ae7` (per-route regression tests pinning HEAD/GET parity for unknown-size Range across diagram + agenda + reel Pattern A + reel Pattern B) + `f9a4877` (handoff doc) + `7f6d297` (test typecheck-narrowing widener). |
| R7 | `review-mp2bkzhk-sc4bnl` | **APPROVED.** "I could not support a P0/P1 blocker from the full ae6f0b8..HEAD milestone diff. Prior legitimate fixes appear structurally preserved: Phase 1 bridges the four sync-layer warning codes, files.* calls carry Shared Drive flags while revisions.* does not, and HEAD/GET Range parity is pinned for known-size and unknown-size branches across diagram, agenda, and reel including reel Pattern B." Zero findings. 12m 7s audit at full milestone scope. | this doc commit (close-out) |
...
**R2 resolution (later determined false positive — see R3.1).** Initial class sweep used `rg -n "drive\\.revisions\\.(get|list)\\(|drive\\.files\\.(get|list)\\(" lib app` as canonical. The already-correct proxy and `lib/drive` surfaces were left unchanged; sync-layer `drive.revisions.get/list` calls had `supportsAllDrives: true` added (this turned out to be technically incorrect — see R3.1). `tests/sync/_sharedDriveSupportContract.test.ts` was created to pin Shared Drive flags across sync, drive helpers, and asset proxy routes — broadened too aggressively in R3 to include `revisions.*`, then narrowed correctly at R3.1.

**R3 resolution.** Class sweep used `rg -n "\\.(files|revisions)\\.(get|list)\\s*\\(" lib app` and found the inline verifier gap. `verifyReelOnApply` now opts into Shared Drive metadata lookup on its `files.get` call. The `_sharedDriveSupportContract` meta-test now scans `lib/sync/**`, `lib/drive/**`, and `app/api/**` for `files.(get|list)` shapes with a live-source regression for `getDriveClient().files.get(...)`. The R4 broadening that asserted the flag on `revisions.*` was reverted at R3.1.

**R3.1 resolution (orchestrator-driven cleanup after `pnpm typecheck` regression).** `@googleapis/drive` types reject `supportsAllDrives` and `includeItemsFromAllDrives` on `Params$Resource$Revisions$Get` and `Params$Resource$Revisions$List`; the Drive v3 REST spec does not include those params on `revisions.*`. R2's "fix" therefore added invalid parameters that broke typecheck in 6 places (4 sync files × multiple call sites). Reverted those flags from `lib/sync/{applyStaged,assetRecovery,defaultSnapshotAssetsForApply,runScheduledCronSync}.ts` while keeping R3's legitimate `verifyReelOnApply` `files.get` fix. Narrowed `_sharedDriveSupportContract.test.ts` to enforce the flag only on `files.(get|list)` shapes (with `includeItemsFromAllDrives` required on `files.list`); `revisions.*` is deliberately excluded with an inline comment explaining why. Also fixed a pre-existing `_scopeCheckContract.test.ts` failure: the agenda + reel asset routes' `drive.files.get` calls were refactored into helper functions (`authorizeAgendaRequest`, `authorizeReelRequest`) before the M7 work began, but the meta-test's `INTENTIONAL_EXCEPTIONS` map still keyed only the old `GET` function names. Registered both helpers with the same non-sheet-admission rationale. Final state after R3.1: `pnpm typecheck` clean, `pnpm test` 171 files / 2503 tests pass, `pnpm lint` 0 errors.

**R5 resolution.** Two HIGH findings, both fixed without burning further review rounds on the false-positive class.
- *Finding 2 (cosmetic flag on reel revisions.get).* The pre-existing `supportsAllDrives: true` on `app/api/asset/reel/[show]/route.ts:489` had been called out as cosmetic during R3.1 but not cleaned up; Codex correctly insisted the R3.1 contract be enforced symmetrically. `fcd6588` removes the flag, tightens the local `ReelDriveClient` type so the field can no longer be passed (any re-add fails typecheck), and adds a NEGATIVE assertion to `_sharedDriveSupportContract` that scans `lib/sync/**`, `lib/drive/**`, and `app/api/**` for any `revisions.(get|list)` call object carrying `supportsAllDrives` or `includeItemsFromAllDrives`. The R3.1 contract is now structurally enforced in both directions: presence required on `files.*`, absence required on `revisions.*`.
- *Finding 1 (HEAD/GET Range parity when size unknown).* A residual gap on the same vector R20-R25 exhausted: HEAD handlers gated `Content-Range` computation on `Number.isFinite(declaredSize|reportedSize)` and fell through to 200 when size was unknown, while GETs forwarded Range to upstream and could observe 206. `c7fbf20` makes each GET strip Range from its upstream request when size is unknown, so both HEAD and GET converge on 200 (full body within cap) for the size-unknown case. `0511ae7` adds per-route regression tests pinning the contract across diagram, agenda, reel Pattern A, and reel Pattern B (`154 tests passed`). Anti-tautology: the new tests assert (a) HEAD status === GET status, (b) both are 200, (c) neither has `Content-Range`, (d) full body delivered, AND (e) upstream call options received no Range header — a stashed fix would fail (e) immediately. `f9a4877` records the convergence; `7f6d297` widens TS null-narrowing in the new tests so `pnpm typecheck` stays green.

**R7 resolution (APPROVED, no material findings).** Cross-model adversarial review fired against `ae6f0b8..HEAD` from the main runtime after the R5 fixes landed. 12m 7s audit traversed: bare-UUID rev URLs, storage-write surface containment, streaming-hash invariant, reel 4-column atomic-NULL, advisory-lock topology (`show:` + `promote:` families), Supabase call-boundary destructuring, `admin_alerts` catalog completeness, URL-strip preservation, TOCTOU drift gates, `pending` sub-payload invisibility, Phase 1 warning bridge (R1), Shared Drive support across all Drive member-call shapes (R3 + R3.1 + R5 F2), HEAD/GET parity uniformity across all three asset proxies including reel Pattern B (R5 F1), and the AC-7.1..AC-7.25 coverage table. Codex returned `verdict: approve`, `findings: []`. Loop closed.

**Final M7 HEAD.** This convergence-log close-out commit. The R3.1→R7 chain lives across `88683fc`, `6c4a70f`, `88ee146`, `fcd6588`, `c7fbf20`, `0511ae7`, `f9a4877`, `7f6d297` + this doc commit. M7 milestone is APPROVED and ready to close.
