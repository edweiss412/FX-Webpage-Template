...
**Plan file:** `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/01-foundation.md`
...
- [x] Specific files present:
  ...
  - [x] `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/` populated with `00-overview.md`, `01-foundation.md`, …, `ROUTING.md`, `HANDOFF-TEMPLATE.md`.
...
- [x] **Commit per task** — applies as a non-negotiable invariant per AGENTS.md §1.6: every task in the plan ships with its own primary implementation commit (no batching, no merging tasks into one commit). Per-task commits for M0: Task 0.1 → `5b1cf8c`; Task 0.2 → `6e8bfff`; Task 0.3 → `b0a9425`; Task 0.4 → `be4f7bc`; Task 0.5 → `a9fb181`; Task 0.6 → `d215927`. The total commit count exceeds 6 because review-loop fix commits are additive (not substitutive) — they layer on top of the per-task primary commit. M0 commit composition: 6 task primary commits + ~7 in-flight review fix commits (across the spec-compliance + code-quality reviewer loops) + ~5 plan/spec/handoff correction commits + 3 deferred-Step-4 + env-var-migration commits + adversarial-review fix commits (3 from round 1, 2 from round 2, plus any round-3+ fixes). For the authoritative live count at any point, run `git rev-list --count dcfd2cd..HEAD`. M1+ scope-meaningful commits (parser, db, etc.) should use the wrapped `feat(<scope>):` / `test(<scope>):` form per AGENTS.md §1.6; `infra:` short form remains the convention for tooling/scaffolding/config.
...
- [x] **Working tree clean before declaring done.** After running local verification commands (`pnpm test:e2e --project=mobile-safari` in particular), `next-env.d.ts` may show a working-tree modification (dev-mode path). This is the documented flap (see "Known local-dev warts"). Run `git checkout -- next-env.d.ts` before declaring the milestone done; `git status` must show "nothing to commit, working tree clean." Convention extends to all future milestones.
...
- **Task 0.4 Step 4 — `pnpm dlx supabase@latest start` boot smoke test (resolved 2026-05-02).** Originally deferred 2026-05-02 (Docker not installed). Re-ran on 2026-05-02 with Docker version 29.4.1 and supabase CLI 2.98.0. Boot succeeded; printed `Project URL = http://127.0.0.1:54321`, `Studio URL = http://127.0.0.1:54323`, Publishable key (`sb_publishable_ACJWl…` — redacted), Secret key (`sb_secret_N7UND0…` — redacted), DB URL `postgresql://postgres:postgres@127.0.0.1:54322/postgres`, S3 Storage URL `http://127.0.0.1:54321/storage/v1/s3`. Note: CLI 2.98.0 outputs `Project URL` + `Publishable`/`Secret` keys in place of the older `API URL` + `anon key`/`service_role key`/`JWT secret` fields — `.env.local.example` and the spec at §14.3 have been updated to the current Supabase recommended naming (`NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` / `SUPABASE_SECRET_KEY`) — see commit `a46fa28`. The new key format is what `supabase start` 2.98.0 prints; legacy JWT names are deprecated per Supabase docs (https://supabase.com/docs/guides/getting-started/api-keys). HTTP smoke against `http://127.0.0.1:54321/rest/v1/` returned 200. `supabase stop` cleaned up cleanly; `docker ps | grep supabase` returned empty. M2 can now use either local Docker-backed iteration or the previously-documented `--db-url` remote path.

## Resolved local-dev warts

- **`next-env.d.ts` mode flap (mitigated 2026-05-02 by re-tracking in build-mode form).** Next 16 writes routes-types to either `.next/types/` (build mode) or `.next/dev/types/` (dev mode), and the `next-env.d.ts` import line tracks whichever mode last ran. We initially gitignored the file (commit `b913e61`) but cross-model adversarial review correctly flagged that this breaks `pnpm typecheck` reproducibility on a cold clone (`tsconfig.json` includes the file but a fresh clone has no Next artifacts until the first `next dev`/`next build`). The file is now tracked again in its build-mode form (`./.next/types/routes.d.ts`); CI's `pnpm build && pnpm start` webServer command (Task 0.3) keeps CI on the build-mode path, so CI runs never produce a diff. Local devs running `pnpm dev` or `pnpm test:e2e` will see the file mutate to `./.next/dev/types/routes.d.ts` in their working tree — run `git checkout -- next-env.d.ts` before committing to revert. `.gitattributes` marks the file `linguist-generated=true` so PR diffs collapse it. `tsconfig.json` globs both `.next/types/**` and `.next/dev/types/**` so route-segment types resolve regardless of mode.
