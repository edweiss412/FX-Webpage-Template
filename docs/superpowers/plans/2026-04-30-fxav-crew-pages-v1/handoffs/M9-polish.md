...
**Plan file:** `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/09-10-admin.md` (§M9 only; §M10 is a separate milestone)
...
- [x] **Previous milestones committed**: M0, M1, M2, M3, M4, M5, M6, M6.5, M7, M8 closed. `git rev-parse --short HEAD` at handoff authoring is `69ed38a docs(handoff): M8 milestone converged at R4 APPROVED`. Working tree clean except `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.{html,md}` (unrelated user-facing docs draft, untracked).
...
**Files:** modify `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/handoffs/M9-polish.md` (this file's §A). Modify `DEFERRED.md` to reflect any Suggested-home changes for deferrals re-deferred out of M9.
...
- **Output**: a written shape brief (committed to `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-12-crew-page-IA.md`) documenting:
...
- **Output**: two brief files at `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-12-{me-page-anchor,bootstrap-liveness}.md`.
...
- **Output**: brief at `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-12-alert-banner.md`.
...
- **Output**: brief at `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-12-admin-allowlist-UI.md`.
...
**Shape session:** `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-13-pinch-zoom-lightbox.md` (confirmed by user 2026-05-13). 14 design decisions baked: scale=4× max, single-finger pan disables Embla swipe at scale>1, reset-on-diagram-change (per-diagram zoom context), Reset chip neutral chrome centered above figure, double-tap toggles 1↔2, full desktop parity (trackpad/cmd-scroll/drag), reduced-motion = no momentum/interpolation but pinch works, etc.
...
Both gates pass with all HIGH/CRITICAL/P1 findings either fixed or explicitly dispositioned. Files touched: `components/diagrams/GalleryLightbox.tsx`, `tests/components/diagrams/GalleryLightboxPinchZoom.test.tsx`, `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/shape-sessions/2026-05-13-pinch-zoom-lightbox.md`, `package.json` + `pnpm-lock.yaml` (added `react-zoom-pan-pinch@4.0.3`).
...
4. **Whole-M9 cross-model adversarial review** ran across seven rounds:
   - **R1** (`f669e18`): HIGH spec-integration drift — C9 amendment shipped but never folded into canonical §14.3 or 00-overview ratified-amendments index; MEDIUM admin error boundary missing. Fixed: §14.3 row retired with cross-ref; 00-overview incremented to two ratified amendments with full C9 entry; new route-segment `error.tsx` + `ADMIN_EMAIL_LIST_FAILED` catalog row + 4 regression tests.
   - **R2** (`8d8a183`): HIGH Task 9.C9.0.5 runtime RLS behavioral-parity probe never built (M2-D2 closure mechanism). Fixed: new `tests/db/admin-rls-runtime.test.ts` + `admin-rls-runtime.baseline.json`. M2-D2 moved to RESOLVED.
   - **R3** (`69d4c6f`): HIGH R2 probe false-passed on NOT NULL constraints (DEFAULT VALUES INSERT bypass) + MEDIUM DEFERRED.md protocol drift. Fixed: replaced INSERT behavioral with structural qual + with_check + predicate-equivalence pinning; DEFERRED.md protocol codified the de facto in-place RESOLVED practice; M2-D1 marked RESOLVED.
   - **R4** (`d22ea75`): HIGH M9-routed DEFERRED entries not marked RESOLVED + MEDIUM C9.0.5 documentation accuracy. Fixed: 15-entry sweep marking M4-D2..D6 / M5-D1..D6 / M5-D8 / M7-D1/D2/D3/D5 RESOLVED with cluster cross-references; test file header rewritten to describe actual R3 contract.
   - **R5** (`e005c03`): HIGH M7-D3 falsely marked RESOLVED at R4 (body explicitly said STAYS DEFERRED) + MEDIUM M2-D2 status sentence still overstated. Fixed: M7-D3 corrected to RE-DEFERRED with full context; M2-D2 status rewritten.
   - **R6** (`07fa01d`): HIGH handoff §11 close-out summary still carried stale pre-final-review claims contradicting §13. Fixed: §11 C4/C9 bullets updated with RESOLVED status; tracked-residuals paragraph aligned with §13.
   - **R7** (`1842cc6`): HIGH final close-out undercounted M9-D-C6c-1 (declined HIGH from C6c critique) + MEDIUM chronology stale. Fixed: tracked-residuals count corrected to 2.
   - **R8** (`5fdd3bd`): HIGH AC-9.2 empty-state reachability e2e spec is `test.describe.skip()` (skip predates R8 but never explicitly re-deferred). Fixed: new DEFERRED.md entry M9-D-9.3-1; tracked-residuals count corrected to 3.
   - **R9** (this commit): HIGH SignInButton legacy test asserted `textContent === "Sign in with Google"` but M9 C5 commit `6be8a1d` replaced the text node with Google's official sign-in `<img>` (accessible name now via `aria-label` + `<img alt>`). Fixed: test updated to assert the accessible-name contract via `getByRole("button", { name: /sign in with google/i })`. HIGH duplicate close-out stanza below also reconciled.

**M9 final status:** see the "M9 close-out summary" section above (§"M9 close-out summary"). That section is the single authoritative residuals + status record; this §13 chronology is the action log that produced it.
