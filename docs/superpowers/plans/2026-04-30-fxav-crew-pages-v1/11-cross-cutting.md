...
<!-- coverage: section-12-4-row-sheet-unavailable-removed-from-folder -->

- [ ] **Step 1: Build a §12.4 extractor** — `scripts/extract-spec-codes.ts` parses the canonical messages section in `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` and emits a typed manifest with active codes AND retired codes separately classified.
...
- [x] **Step 2: Failing semantic-audit test** via `ts-morph`:
  ...
  // // Source: docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md §4.3 admin-only.
  ...
  const ADMIN_BOOTSTRAP_NAMES = [
    ...
    'email_deliveries', 'deferred_ingestions', 'admin_alerts', 'sync_audit', 'drive_watch_channels',
  ...
  const PROTECTED_SINKS = [
    ...
    // wizard_finalize_checkpoints, shows_pending_changes, recovery_drift_cooldowns):
    ...ADMIN_FROM_REGEXES,
...
- [ ] **Step 1: Authored allowlist** — enumerate every email-bearing path the spec calls out (§4.1.1, §7.2.2, §13.2):
  ...
  | DB write     | `lib/notify/deliver.ts` → `email_deliveries.recipient`                                                                                                 | `email_deliveries.recipient` canonicalize before insert (via `lib/email/canonicalize.ts` before the ledger INSERT)                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
  ...
  | DB write     | `lib/admin/alerts.ts` → `admin_alerts.resolved_by`                                                                                                     | spec §4.6 records `admin_alerts.resolved_by`, the admin who resolved an alert. canonicalize before the resolution UPDATE. Test: resolve alert as mixed-case admin; assert canonical persisted email.                                                                                                                                                                                                                                                                                                                                                                            |
  | DB write     | `admin_alert_reads.admin_email` (bell per-admin read state; PostgREST-locked, written only via SECURITY DEFINER RPC `bell_mark_read`) | `admin_alert_reads.admin_email` stores the canonical admin identity keying per-admin bell read state. Writes flow only through the `bell_mark_read` RPC, whose `p_admin_email` argument is the server-derived canonical identity from `requireAdminIdentity` (admin_emails is canonical at rest); the schema CHECK `admin_alert_reads_admin_email_canonical` is the safety net per §4.1.1. |
  | DB write     | `role_token_mappings.decided_by` (per-show and settings-surface role-token decisions) | `role_token_mappings.decided_by` stores the canonical admin email of the operator who accepted or rejected a role-token mapping. BOTH write paths MUST `canonicalize(email)` before the write — `app/admin/show/[slug]/_actions/roleToken.ts` and `app/admin/settings/_actions/roleTokenMappings.ts`; the schema CHECK `role_token_mappings_decided_by_canonical` (`supabase/migrations/20260716000000_role_token_mappings.sql:8`) is the safety net per §4.1.1, not the primary mechanism. Both paths are named because covering one would let the gate report the column as policed while half its writes stayed unchecked. Registered 2026-08-04 (BL-X5-ROLE-TOKEN-DECIDED-BY-BOUNDARY): both already canonicalized, but the boundary was absent from this inventory, so deleting either call failed no gate — only the DB CHECK caught it, and only at write time. |
  | DB write     | `admin_bell_state.admin_email` (bell per-admin opened watermark; PostgREST-locked, written only via SECURITY DEFINER RPC `bell_mark_opened`) | `admin_bell_state.admin_email` stores the canonical admin identity keying the per-admin bell opened watermark. Writes flow only through the `bell_mark_opened` RPC, whose `p_admin_email` argument is the server-derived canonical identity from `requireAdminIdentity`; the schema CHECK `admin_bell_state_admin_email_canonical` is the safety net per §4.1.1. |
  ...
  | Schema       | `crew_members.email`, `transportation.driver_email`, `transportation.loadout_email`, `contacts.email`, `client_contact.email` JSONB extracted via CHECK if reachable                   | every one has a `*_email_canonical` CHECK constraint per §4.1.1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
- [ ] **Step 2: Failing test** — six positive assertions across the boundary layers:
  ...
  3. **DB schema CHECK exact-expression match.** SQL introspection: for each email-bearing column, assert `pg_get_constraintdef` returns the normalized form `CHECK ((<column> = lower(btrim(<column>)) AND <column> <> ''))` (or the nullable/conditional equivalent). Catches a CHECK with the right name but a wrong/weakened body (e.g., missing non-empty rejection, or a regex check instead of the canonical-form check).
...
<!-- coverage: §16, AC-X.6, section-6-8-derivation-table, section-6-8-2-auth-side-effects-derivation, section-13-2-3-lease-holder-protocol, -40-reports-lease-amendment, -parsedsheet-parseresult-split, -immutable-pin-amendment, -cookie-session-validator-rewrite -->
      ...
      steps:
        ...
        - name: Verify generated admin tables file is fresh
          ...
          run: |
            ...
            git diff --exit-code lib/audit/admin-tables.generated.ts \
              || (echo "::error::admin-tables.generated.ts is stale - run 'pnpm gen:admin-tables' locally and commit"; exit 1)
        - run: pnpm test:audit:x5-email-canonicalization
        - uses: actions/upload-artifact@v4
          if: always
