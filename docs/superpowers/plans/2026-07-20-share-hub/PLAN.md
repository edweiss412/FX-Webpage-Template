...
### T4a — Rotate thrown-action guard (prerequisite, pre-existing bug)
Test FIRST, in the rotate control's own test file: make `rotateShareToken` reject; assert the control settles to its refused banner and leaves `resolving` (today it strands there forever — `RotateShareTokenButton.tsx:140-155` has no try/catch, and the file has zero `catch` blocks). Then add the guard, mirroring `PickerResetControl.tsx:139-145` and `CrewRowActions.tsx:203-206`. Failure mode caught: under T4b's busy contract a thrown rotate would wedge the popover permanently — `busy` never clears, so all four dismissal paths stay inert. Ships in the same commit as T4b.

### T4b — Busy contract (`onBusyChange`) — the child-component change
Tests, in `rotateShareTokenButton.test.tsx` + `pickerResetControl.test.tsx`: each control calls `onBusyChange(true)` when entering `resolving` and `onBusyChange(false)` when it settles — success AND returned-error AND thrown-action paths (the thrown path is only reachable once T4a lands); omitting the prop preserves today's behavior exactly (pins that `step3ReviewSections.tsx`, which passes nothing, is unaffected). Then in `shareHub.test.tsx`: with a child busy, ALL FOUR dismissal paths are inert — primary trigger, kebab, Escape, backdrop — and Escape still calls `stopPropagation` (assert the document-level spy does not fire, else the whole review modal closes). Failure mode caught: a trigger click unmounting a rotate mid-flight, invalidating the crew's link with no observable confirmation.
...
Enumerate every conditional render in `ShareHub.tsx`; assert each is instant (no `AnimatePresence`, no transition classes on mount/unmount) per spec §9. Then make spec §9's rules R1–R4 EXECUTABLE — they replaced a hand-enumerated compound table that four review rounds could not keep correct, so tests are now the authority: R1 (arming a control's confirm clears ITS OWN banner, success and error; banner+confirm unreachable within one control, reachable across the two); R2 (rotate's banner clears on re-arm only — its auto-revert timer must NOT clear it; reset's success banner clears on re-arm OR timer, reset's error banner on re-arm only); R3 (error banners persist and are role="alert"; successes are role="status", reset announcing via its sr-only region with the visible banner aria-hidden); R4 (busy freezes no timer and disables no confirm row — a confirm stays interactive and auto-reverts while the sibling resolves). Plus: both=confirm, both=resolving (busy ≥1 until BOTH settle), both=banner (no cross-clear), publish toggle while busy (close DEFERRED, not dropped), backdrop click while an auto-revert timer is pending, rotate success during an open Copy 2s window.
