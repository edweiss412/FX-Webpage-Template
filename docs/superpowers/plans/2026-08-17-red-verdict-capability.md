...
- **Line anchors below are as of the PLAN-TIME tree (`d2d602588`), and this arc's own diff has since shifted them.** They are a record of what was verified when the plan was written, not a live index, so they are deliberately NOT rewritten to post-diff lines — a draft-time record pointing at lines that did not exist at draft time would assert something false. `spec:lint` reports the drift as `CITATION_SYMBOL_UNMATCHED` advisories on this section; that is the expected steady state.
