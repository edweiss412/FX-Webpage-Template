...
- [ ] **Step 3: implement** — in `scripts/observe/args.ts` add `"reveal-email": { type: "boolean", default: false }` to the options and `revealEmail: boolean` to the parsed shape; in `scripts/observe.ts` alerts handler pass `includePii: args.revealEmail` into the `queryAlerts` filters + stderr notice when set; in `scripts/observe/format.ts` `formatAlerts`, append `describeAlert(row.identity)` per row (both table and `--json` shapes use the pre-serialized `row.identity`). Format-only — no second resolve. Add flag to the USAGE/help string.
- [ ] **Step 4: run → PASS**.
...
- [ ] **Step 2: run → FAIL**.
- [ ] **Step 3: implement** — extend the SELECT to `shows(slug, title)` + `occurrence_count`; `projectIdentityContext(alert.context, { includePii: true })` → `resolveAlertIdentities([row], supabase, { includePii: true })` (web is admin-only → PII allowed); on `infra_error` log via `lib/log` + render partial; render the identity with `data-testid="admin-alert-identity"` in the expanded panel (above helpful-context) AND as a collapsed line associated with the summary. jsdom (this task) verifies content/presence only; the exact collapsed-line placement geometry (grid-sibling row-2, not inside the 44px summary flex) is driven by Task 12's real-browser gate. Suppress when `describeAlert` → null. Add `data-testid="admin-alert-summary"` to the `<summary>` for Task 12's rect assertions.
