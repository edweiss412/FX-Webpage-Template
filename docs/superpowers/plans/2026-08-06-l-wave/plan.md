...
Sweeps RUN 2026-08-06 (plan R1 F1 repair); raw output committed at `docs/superpowers/plans/2026-08-06-l-wave/l1-coverage-sweep-2026-08-06.txt`. Three enumeration commands (all three are re-run at execution and the table below re-verified against their output):

1. Unconditional suites: `grep -rln "describe\.skip(\|test\.fixme(\|test\.skip(" tests/` plus the multiline-chain form `grep -rn -B1 "^\s*\.skip(" tests/` → 21 files with unconditional `describe.skip(` (20 in `tests/e2e/` + `tests/time/rightNowTransitions.test.ts`... CLASSIFIED: `tests/cross-cutting/picker-flow-e2e-ci-wiring.test.ts` hits are string/regex mentions in a wiring guard, NOT suites (excluded); `tests/time/rightNowTransitions.test.ts:8` "test.fixme" is a COMMENT, not a call (the file's suites run — its hit joins the prose sweep, not the suite set); `tests/e2e/crew-page.spec.ts:892` is a live multiline chain (test.describe on one line, .skip( on the next) — the §4.10 compound block is dead. `test.skip(` sites inside e2e files are case-level skips within already-enumerated files.
...
1. RED: guard suite (working name _metaEmDashCopy.test.ts under `tests/styles/`) covering surfaces (a) catalog string literals + (b) MDX prose (fenced code elided), with the `--` rule and per-surface planted-dash premise fixtures (`tests/_shared/premise.ts`, unconditional execution). Fails on the 20 MDX hits; catalog clean.
...
1. **Screenshot regen (decided at plan time, R1 F4):** repaired copy RENDERS on captured surfaces — `scheduleBookends` titles on crew-preview-schedule/today, `ShowsTable` status strings on dashboard-overview, `fieldChanges` labels on dashboard-overview (RecentAutoAppliedStrip), `AttentionMenu` copy on needs-attention. Regenerate ALL affected baselines FROM the pinned Playwright Docker image with `--platform linux/amd64`; byte-diff records which of the 14 WebPs actually changed; help MDX prose renders in no product capture (the captures are product-surface screenshots embedded in help), so MDX-only changes ripple nothing. After any local verification capture: `git restore public/help/screenshots/`.
2. `DESIGN.md` §9 gains one sentence naming the guard as enforcement.
