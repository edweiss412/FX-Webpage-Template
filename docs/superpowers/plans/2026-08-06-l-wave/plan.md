...
1. Write the guard suite (working name _metaEmDashCopy.test.ts under `tests/styles/`): it scans the spec §2.3 accept-set —
   ...
   (c) EVERY executable string literal and JSX text node in `components/**`, `app/**` excluding `app/api/**` (excluded with reason: invariant-5 catalog mediation), and ALL of `lib/**` (comment-stripped; covers exported copy constants, return literals, and string expressions rendered as JSX children — seed inventory per spec §2.3: ~25 component/app sites, the `NotifBell.tsx` template fragment, and the 7 lib files from the R3 probe incl. `lib/parser/warnings.ts`, `lib/sync/changeLog/fieldChanges.ts`, `lib/parser/blocks/scheduleBookends.ts`). NODE KINDS per spec §2.3 item 3: `StringLiteral`, `NoSubstitutionTemplateLiteral`, `TemplateHead`/`TemplateMiddle`/`TemplateTail`, `JsxText`, JSX attribute strings — NOT bare `isStringLiteralLike`, which excludes template fragments and silently misses live covered sites (probed, spec R2 F3). Non-copy literals the wide scan sweeps in get per-hit dispositions: repair if user-visible, exemption row with reason if internal (directory rows legal for wholly-internal trees like `lib/dev/**`);
   (d) (subsumed into (c) — `lib/notify/**` is part of `lib/**`; its 2 known copy hits are TemplateMiddle fragments).
...
2. Run it: the failure list is the sweep list, committed as the task record. Expected from the census: ~20 MDX + the ~25-site component/app seed + the R3 lib probe's 66 executable literals (19 live copy to repair, ~47 internal to disposition) + whatever else the scan finds (the seed is a floor, not a bound — the guard is the enumerator); catalog expected clean, so the §12.4 lockstep is expected NOT to fire.
