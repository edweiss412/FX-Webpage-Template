...
7. Exit-window repopulation blocked (AC-2b, spec R3 F1): close → demote DURING the exit window (the retained slide failing per the `gallery.failedItem.test.tsx:850` shape) → re-open (real parent increments `openNonce`) → no chip, asserted BOTH mid-exit (the gate's own observable) and after the re-open. The announce entry's routing after a cancelled exit is the parent's contract, covered by `Gallery — a lightbox failure inside the exit window is not lost`, and is not re-asserted here (spec §1.1 item 3 keeps the announce channel out of scope).
...
9. POSITIVE re-entry ordering (spec R4 F1; plan R1 F1): close → re-entry COMMITS (via `flushSync`, so passive effects are still pending) → the retained slide's PENDING original failure fires → the chip RENDERS (an implementation that cleared a closing flag in an effect is still suppressing at that instant and fails here; it is the executable pin of "render-time, not effect").
...
GREEN (as shipped; spec §2.1's 2026-08-16 amendments are the authority, and the retired `closingRef` / `lastNonceRef` / render-time-reset wording is gone from this plan with them): the session-stamped `demotedNotice: { id, nonce }` state, rendered only while `nonce === openNonce`; the `closedAtNonce !== openNonce` set-gate recorded at the close initiators; the `openNonce` prop (parent increments it per closed-to-open transition in `components/diagrams/Gallery.tsx`); the timer with all four clear conditions, cleared at every event-handler and effect path that ends the chip's life (both close and BOTH slide-error branches, active and inactive) with an id-guarded callback as the backstop; `relative` on the slide figure; and chip markup per spec §2.2 (`inset-x-0 bottom-2`, Reset-chip class family minus interactivity).
