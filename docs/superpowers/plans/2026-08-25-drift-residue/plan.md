...
- AC-3.5: each entry's KEY is verified, not trusted, and the binding is to the flag's DECLARATION rather than to any occurrence of its text. The node at the entry's line must DECLARE the identifier named by `flag` — a variable, parameter or property declaration whose name matches — so the coordinate is unique per flag per file.

**Why a declaration and not a text match.** Plan review round 1 caught the line being trusted at all; round 2 caught the first repair still being too weak. A flag's text occurs at several live lines that all satisfy both `unreached-no-ternary` predicates: `unavailable` at lines 101 and 125 of `components/admin/telemetry/TelemetryOverviewStrip.tsx`, `isInfra` at lines 230 and 233 of that same file, and `inOnboarding` at lines 130, 135 and 139 of `app/admin/layout.tsx` — each group a declaration plus assignments or a non-returning `if`. A line typo landing on any of them passed the text check. A declaration is unique where an occurrence is not.

- AC-3.6: the derivation's two predicates are EXPORTED SEPARATELY and each conjunct is pinned by its own control, all three DERIVED from `scanCandidates()` rather than named as literals. Asserting only the combined function's output cannot distinguish a missing conjunct from a lucky branch order.
