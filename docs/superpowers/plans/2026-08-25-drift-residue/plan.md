...
- AC-3.5: each entry's KEY is verified, not trusted. The `flag` component of `file:line:flag` must appear in the source text of the node at that line, so a line typo that happens to land on another qualifying node fails instead of deriving the same cause. Plan review round 1 enumerated the exposure: `Dashboard.tsx` holds 29 null-classified ternaries, `OnboardingWizard.tsx` 30, and `IgnoredSheetsDisclosure.tsx` two, so every ternary entry could point at a different qualifying line; and the three non-ternary entries could point at line 1 and still derive `unreached-no-ternary`.
- AC-3.6: each conjunct of `unreached-no-ternary` is pinned INDEPENDENTLY by a control, so deleting either one fails. Both controls are DERIVED from `scanCandidates()`, never named as literals: the candidate-absence conjunct is pinned by any candidate the scanner reports at a line where no `ConditionalExpression` begins (an `IfStatement`/`CaseClause`/`CatchClause` candidate such as `components/admin/settings/AdministratorsSection.tsx:54`), which must NOT derive `unreached-no-ternary`; the `classifyExpression === null` conjunct is pinned by any candidate whose form is not `unknown` and which originates at a `ConditionalExpression`, which must NOT derive `unreached-ternary`. Deriving both from the live candidate set keeps the controls correct as the corpus moves, which a literal site list would not.
...
## Task 5 — archive the three rows and take the in-progress markers off (AFTER the whole-diff review converges)
...
**This task runs LAST, after the whole-diff review reaches APPROVE, and that ordering is forced rather than stylistic.** Invariant 12 puts the marker removal in the PR's last commit, and archives categorically reject in-progress entries — so a graduating row's marker comes off in the very commit that archives it. Archiving therefore CANNOT precede the diff review: plan review round 1 observed that any accepted review finding creates commits after the claims disappeared, which is exactly what invariant 12 forbids. Tasks 1 to 4 are the reviewable diff; this task is the closeout commit on top of it.
