...
- `components/admin/telemetry/HealthAlertsPanel.tsx` (NEW) — dev detail rows + action links + resolve.
- `components/admin/telemetry/HealthAlertResolveButton.tsx` (NEW) — client button bound to the Server Action.
...
- `app/admin/page.tsx`, `app/admin/dev/telemetry/page.tsx` — render `AppHealthPanel` / `HealthAlertsPanel`.
...
- [ ] **Step 1b: Add fail-first §7 copy-edit assertions (plan-R10 finding 2 — AC14/§7 must be test-driven, not implementation-only).** In the same (or a sibling) meta-test, assert against `MESSAGE_CATALOG`: `EMAIL_NOT_CONFIGURED.followUp === "Eric → configure email env (provider key / sending address / site address) on the deployment"` and `EMAIL_DELIVERY_FAILED.followUp === "Eric → check provider key / verified sending domain"`; BOTH `EMAIL_*` `followUp` match `/Eric/` and do NOT match `/Doug/`; both `EMAIL_*` `dougFacing` do NOT contain "Check that" / a Doug configuration instruction. For `WATCH_CHANNEL_ORPHANED`: `audience === "doug"` AND its `dougFacing` retains the reassurance clause `/sync(s)? automatically/` (its urgency-demotion tone is additionally validated by the impeccable gate, Task 11). These FAIL before the Step-3 copy reconciliation.
- [ ] **Step 2: Run to verify FAIL** — `pnpm vitest run tests/messages/_metaAlertAudienceContract.test.ts` → FAIL (fields + copy absent).
...
- Produces: `AppHealthPanel` (async server component) doing its OWN `fetchHealthRollup()` read; renders a `StatusIndicator`-style row: ok → "All systems normal" (positive), else the worst-active state + count; Doug → popover trigger, dev → "View details →" `/admin/dev/telemetry#health`.
...
**Files:**
- Create: `lib/admin/healthAlerts.ts` (loader), `components/admin/telemetry/HealthAlertsPanel.tsx`
- Modify: `app/admin/dev/telemetry/page.tsx` (render above `CronHealthHeader`, `:20` already `requireDeveloperIdentity()`), `tests/admin/_metaInfraContract.test.ts` + `tests/admin/_metaBoundedReads.test.ts` (register the loader — already declared Task 3 inventory; add here if not).
...
- Create: `components/admin/telemetry/HealthAlertResolveButton.tsx` (client button bound to the action; wire into `HealthAlertsPanel`)
