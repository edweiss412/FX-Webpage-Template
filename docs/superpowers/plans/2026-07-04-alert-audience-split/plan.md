...
- [ ] **Step 1: Failing tests** — (a) `alertCount` excludes a seeded `WEBHOOK_TOKEN_INVALID` row from the count but still counts a `SHEET_UNAVAILABLE` row AND an uncataloged `TOTALLY_UNKNOWN_CODE` row (exclusion-not-allowlist, AC10). (b) `AlertBanner` does not render a top `WEBHOOK_TOKEN_INVALID` but renders `SHEET_UNAVAILABLE`; a seeded uncataloged code still renders. (c) `PerShowAlertSection` filters a show-scoped `TILE_PROJECTION_FETCH_FAILED` out but **still renders a show-scoped `SHOW_FIRST_PUBLISHED`** (info, NOT health — regression guard) AND a `PARSE_ERROR_LAST_GOOD` AND an unknown code.
- [ ] **Step 2: Run → FAIL.**
