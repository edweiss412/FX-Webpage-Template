...
**Spec:** `docs/superpowers/specs/alerts/2026-07-04-alert-resolve-truthing.md` (APPROVED, 3 adversarial rounds).
...
- **Modify** `docs/superpowers/specs/alerts/2026-07-03-admin-alert-auto-resolution.md` — §3 row + counts.
...
In `docs/superpowers/specs/alerts/2026-07-03-admin-alert-auto-resolution.md`: move the `GITHUB_BOT_LOGIN_MISSING` §3 row (`:94`) from `DEFER` to auto (cite the two resolve sites); update §3 line 51 counts `7 AUTO · 14 NEW · 18 EVENT · 3 DEFER` → `7 AUTO · 15 NEW · 18 EVENT · 2 DEFER`; add a one-line note that `2026-07-04-alert-resolve-truthing` supersedes this row. Do NOT touch the two `BRANCH_PROTECTION_*` rows. Edit surgically (no prettier).

- [ ] **Step 10: Commit**

```bash
git add lib/messages/catalog.ts lib/adminAlerts/audience.ts tests/messages/_metaAdminAlertCatalog.test.ts tests/adminAlerts/autoResolving.test.ts docs/superpowers/specs/alerts/2026-07-03-admin-alert-auto-resolution.md
...
Run: `rg -n "22|20|15 NEW|2 DEFER|BRANCH_PROTECTION" docs/superpowers/specs/alerts/2026-07-04-alert-resolve-truthing.md DEFERRED.md` and confirm consistency.
...
**Files:**
- Create: `docs/superpowers/plans/alerts/2026-07-04-alert-resolve-truthing/CLOSEOUT.md` (invariant-8 disposition record — this standalone feature has no milestone handoff doc, so its §Impeccable record lives here — R4 H2)
...
`docs/superpowers/plans/alerts/2026-07-04-alert-resolve-truthing/CLOSEOUT.md`** with an `## Impeccable dual-gate`
...
```bash
git add docs/superpowers/plans/alerts/2026-07-04-alert-resolve-truthing/CLOSEOUT.md
