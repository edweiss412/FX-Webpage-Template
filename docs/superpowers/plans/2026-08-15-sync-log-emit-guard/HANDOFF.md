...
3. The worktree `../FX-worktrees/sync-log-emit-guard` ALREADY EXISTS with the claim marker pushed (created by the authoring session). Verify the authoring PR is MERGED (`git log --oneline origin/main | head -5` shows the docs/sync-log-emit-guard-spec merge; if not merged, STOP — you were launched early). In the worktree: `pnpm install && pnpm worktree:link-env && pnpm preflight`, then `git merge origin/main`.
4. Overwrite `/Users/ericweiss/FX-worktrees/sync-log-emit-guard/.claude/ship-state.json`: `{branch: "fix/sync-log-emit-guard", stage: "implement", tasksRemaining: "T1-T6", next: "Task T1", blockedOn: "", cronJobId: <yours>, sessionId: <YOUR session UUID — the directory segment of your scratchpad path>}`.
