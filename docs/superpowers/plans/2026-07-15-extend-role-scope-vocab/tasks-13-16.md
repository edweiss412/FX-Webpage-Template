...
- [ ] **Step 0: Extract the Doug-facing strings module** — `components/admin/roleRecognizeCopy.ts` exporting every §9-pinned string as named constants (both components + the settings row import from it; single source). Then EXTEND the hygiene sweep (`tests/messages/_metaCatalogCopyHygiene.test.ts` D7 block from Task 9) with a second loop importing this module and sweeping every exported string with the same banned-regex + placeholder-stripping (Codex plan-R1 F6). **Plus the inline-copy convention guard (plan-R2 F7):** the same sweep reads the SOURCE of the four new component files and asserts they contain no raw JSX text nodes (regex over `>[^<>{}\n]*[A-Za-z]{3,}[^<>{}\n]*<` matches outside `{…}` expressions) — every user-visible string must flow through `roleRecognizeCopy`, so an inline "sync failed" literal can never bypass the sweep. Whitelist punctuation-only/single-glyph nodes (`✓`, `⌄`, `·`).
