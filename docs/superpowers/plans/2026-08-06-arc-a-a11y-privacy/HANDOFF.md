...
2. GATE CHECK, from the MAIN checkout `/Users/ericweiss/FX-Webpage-Template` (post-merge by definition): `git -C /Users/ericweiss/FX-Webpage-Template pull --ff-only`; verify the docs/arc-a-spec merge appears in `git log --oneline origin/main | head -5` AND the plan's **Status** line there reads plan-APPROVED. Either failing = STOP (launched early).
