...
You are the Opus implementer session for arc A of the ABC batch (a11y/privacy cluster). This file is self-contained: everything you need is here or one `Read` away. THE GATE CHECK is Step 0.2 below — after Step 0.1's `date`, from the MAIN checkout: the plan's **Status** line must read plan-APPROVED or plan-CONVERGED with a codex-guard round reference; if it says DRAFT or pending, STOP and do nothing (launched early; the authoring session owns the gate). Both user review gates were WAIVED by the user's 2026-08-06 autonomy grant. Do not re-ask any ratified decision — they are enumerated in the spec §1.1.
...
2. GATE CHECK, from the MAIN checkout `/Users/ericweiss/FX-Webpage-Template` (post-merge by definition): `git -C /Users/ericweiss/FX-Webpage-Template pull --ff-only`, then `git -C /Users/ericweiss/FX-Webpage-Template show origin/main:docs/superpowers/plans/2026-08-06-arc-a-a11y-privacy/plan.md | grep -E "\*\*Status:\*\* plan-(APPROVED|CONVERGED)"` — the file existing ON origin/main with that Status IS the merged-and-approved proof, immune to log depth. Empty grep or missing file = STOP (launched early).
3. In the worktree (`../FX-worktrees/a11y-privacy-cluster`, branch `feat/a11y-privacy-cluster`, already existing with claim markers pushed per spec §3): `git fetch origin` → `git merge origin/main` FIRST (mechanical; the parent PR only adds spec/plan/handoff and removes its own markers) → THEN `pnpm install && pnpm worktree:link-env && pnpm preflight`. NOW read, in full, from the post-merge worktree: `AGENTS.md`, the arc A spec, this directory's `plan.md`, `docs/agents/writing-plans.md`, `docs/agents/spec-self-review.md`.
4. Write `/Users/ericweiss/FX-worktrees/a11y-privacy-cluster/.claude/ship-state.json` fresh: `{branch: "feat/a11y-privacy-cluster", stage: "arc-a-impl", tasksRemaining, next: "Task T1", blockedOn: "", cronJobId: "", sessionId: <YOUR session UUID — the directory segment of your scratchpad path>}` — `cronJobId` EMPTY at this step; Step 0.5 fills it (write order fixed per plan review R1 F5).
5. Register YOUR 10-minute cron nudge (off-minutes `7,17,27,37,47,57 * * * *`, local time). The prompt MUST: run `date` first; re-read the marker and stand down via supersession check if `sessionId` is not yours (CronDelete own job, clear pane + agent labels, silent); skip silently if `blockedOn` non-empty; otherwise resume the marker's `next` immediately; explicitly discard stale blocked/waiting framing. THEN write the returned job id into the marker's `cronJobId`.
...
Plan order is binding: T1 (travel-date suppression) → T2a–T2e (live regions) → T3a–T3e (channel wire+strip+rename) → T4 (scroll cue + transition audit) → T5 (dual gate + closeout, merge origin/main + marker strip, FINAL-diff review to APPROVE, then PR, CI, merge, ff `0 0`). One worktree, one branch, one PR.
