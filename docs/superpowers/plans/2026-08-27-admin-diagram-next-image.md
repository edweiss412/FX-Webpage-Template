...
| failed → failed, across an unrelated PARENT re-render | No transition, and that is the point. A breakdown re-render hands every tile a fresh loader closure; a failure must survive it and change only when the SOURCE does. Task 2 case 5, added when mutant (i) proved nothing observed this. |
...
- The anchor gains `relative aspect-4/3 w-full overflow-hidden`, and NOTHING else. `relative` and the aspect box are required: without a positioned ancestor a `fill` image resolves against the modal panel, which IS positioned, and one thumbnail covers the whole dialog.

  **The tile's chrome STAYS on the image, and an earlier draft of this plan was wrong twice about it.** It first claimed moving `rounded-md border border-text-faint bg-surface-sunken` onto the anchor was "required rather than cosmetic", because a `fill` image insets against the padding box so an image-side border "would no longer bound the tile". Mutant (b) measured that false: with no border on the anchor its padding box IS its border box, both arrangements render identically, and all 44 e2e pass either way. The reason was then restated as consistency with the crew gallery — true, and still not this arc's to act on. `BL-ADMIN-DIAGRAM-NEXT-IMAGE` is a next/image adoption; the image change demonstrably does not require the move; and the move takes the tile's `<img>` out of the painted-child family that `docs/superpowers/specs/2026-08-26-control-outline-cover-widening-design.md` §15 table 3 counts, which is a ratified design claim spent on a preference a perf arc picked up in passing. Filed instead as `BL-DIAGRAM-TILE-CHROME-CONSISTENCY`.
...
- (b) Move the border onto the anchor: **44 passed — ORPHANED**, which disproved the mechanism claim above and, once the reason was restated as consistency, decided the scope question too. The move is reverted; the border stays on the image where §15 counts it. The suite carries a non-zero IMAGE-border assertion on its own merits — removing the border from a bordered grid of thumbnails is a visible regression nothing else here would notice — and its own mutant, (b′) removing the border from the image entirely, fails it at every mode.
...
**Mutant record.** All RUN, and the third one was orphaned as written — the same shape as Task 1's (b), and dispositioned by the same rule.

- (g) Delete the reconciliation branch: 4 failed of 13, exactly the four reconcile cases, staged suite untouched.
- (h) Reconcile on `hasPreviewSource` alone: 2 failed of 13, cases 3 and 4 — the two whose source moves while availability does not.
- (i) Reconcile on the loader function identity: **13 passed as originally aimed — ORPHANED.** The plan predicted Task 1's staged error case would fail because "every render clears the failure". It does not: `setFailed(true)` re-renders the TILE, not the breakdown, so the loader prop identity is unchanged and the failure sticks. The claim was right about the mechanism and wrong about which render exposes it — only a PARENT re-render mints a fresh loader closure.

  The suite gains a fifth case for that, justified on its own merits: **a failed tile stays failed across an unrelated parent re-render.** "A failure survives until its SOURCE changes" is the actual contract, and nothing else here observed it; without it a tile that failed its fetch would silently retry on every unrelated render of the breakdown, flickering, with no state change that explains it. Measured: passes clean (5 of 5), and under (i) it is the only failure (1 of 5). The guard got stronger; the mutant was not reworded to fit.
...
Both halves ran with the canonical v3 setup gates: the skill's context.mjs with --target (PRODUCT.md + DESIGN.md), the command reference, and the skill's product register reference — this is an admin surface where design SERVES the task. Critique ran DUAL-AGENT, two isolated sub-agents that could not see each other's output, so this run carries no degraded banner. Audit ran inline, which its reference permits; only critique carries the dual-agent hard invariant.

**Critique: 25/40.** AI-slop verdict PASSES the product register — no invented affordances, no gratuitous motion, standard grid vocabulary. The lowest heuristic is 9, Diagnose and Recover, at 1/4, and that is the honest read: the surface has one failure string for two states.

**Audit: 17/20.** Accessibility 3 (AA met once the focus ring landed; the placeholder's border misses 1.4.11 non-text contrast). Performance 3 (the point of the arc; one inert `srcset` clone finding). Theming 4 (every colour a token, zero hard-coded values in the diff). Responsive 4 (tap targets 97px at 390 and 170px on desktop, zero horizontal scroll measured). Anti-patterns 4 (no tells).

### Findings and dispositions

| Tier | Finding | Disposition |
|---|---|---|
| P1 | No focus-visible ring on the tile anchor. WCAG 2.4.7 AA, and all nine sibling links in this file carry the recipe. | FIXED in-branch. The arc rewrote that exact className line, so it owed the ring. Ground is `surface`, matching the file's dominant recipe; `overflow-hidden` does not clip it, since an element's own overflow never clips its own ring. |
| P1 | A grid of failed tiles read as N identical grey boxes: the name was in hand as `alt` and discarded, so the reviewer could not tell WHICH diagram was missing on the surface that gates publishing. | FIXED in-branch. The placeholder now renders the truncated name with the full string as `title`. Two assertions, plus an empty-name guard driven through `DiagramTile` directly — the level that owns it, because an empty `stub.alt` falls back upstream to a sheet-tab name and the breakdown path can never reach the guard. |
| P1 | "Preview unavailable" collapses two states the component distinguishes internally: not in the snapshot (the `useState(!hasPreviewSource)` seed) and failed to load (`onError`). On a pre-publish confirmation surface, an absent diagram looks like a slow one. | DEFERRED as `DIAGRAMTILE-FAILURE-STATE-COPY-1`. Ruled by bl-orch: class-sweep exception (a) — splitting the states needs new Doug-facing copy for a state that has never had words, a product decision this PR cannot settle. Line 3896 is untouched by the diff, verified against the hunks, so shipping is no worse than yesterday's main. |
| P2 | The placeholder's `border-border` is roughly 1.22–1.38:1 on `bg-surface-sunken` against the live tile's `border-text-faint` at 3.02:1 — the state that most needs noticing has the faintest edge, and misses WCAG 1.4.11's 3:1 for non-text UI. | FOLDED into the P1 deferral, per bl-orch: same placeholder surface, and whoever splits the states restyles it once. The row also records that this arc's reconciliation made the presentation DYNAMIC — a tile can now flip between the two edges live, without a remount, which it could not do before. |
| P2 | `onError` removes the anchor, so the one case where the URL is fine has no recovery path. | RECORDED, not fixed. Keeping the anchor on the error branch is a design change to the failure affordance, not a defect repair, and it belongs with the copy split that gives the failure words. |
| P3 | On the STAGED path the loader is width-independent, yet `next/image` still emits a 15-candidate, 1679-byte `srcset` whose URLs are byte-identical — roughly 20KB of duplicated markup per staged modal at cap. | RECORDED. Measured by Assessment B, and behaviourally inert: one fetch, correct box, correct tier. Published is unaffected. Removing it would mean `unoptimized`, which also removes the real srcset the published path depends on, so the cure is worse. |
| FALSE POSITIVE ×2 | the detector reports `broken-image` at `components/admin/wizard/step3ReviewSections.tsx:3831` (in-diff) and `components/admin/wizard/step3ReviewSections.tsx:3904` (pre-existing). | REFUTED with the cause named: the detector's text matcher, detect-text.mjs line 165, matches `/<img\b…>/gi` over raw text with no comment stripping, so it cannot tell markup from prose. Both hits are `<img>` inside code comments, and `grep -nE "^\s*<img"` returns NOTHING — removing the last real one is what this arc did. Note the direction: the false positive gets LOUDER the better the change is documented. |

No P0. Two of three P1s fixed in-branch; the third deferred by explicit ruling with a `DEFERRED.md` entry carrying the impeccable finding as evidence, which is invariant 8's fixed-or-deferred contract.

**One defect the gate caught in its own repair, recorded because it is the more useful artifact.** The P1b fix first gave the name node `data-testid={`${testId}-name`}`. The tile-count assertion selects with `[data-testid^="…-diagram-tile-"]`, a PREFIX match, so the new node counted AS a tile and the cap read 24 where 12 is correct, at all three breakpoints. The name node now carries no testid and both suites handle it by `title`; the component comment says why, so nobody re-adds one.

**Real-browser evidence, not inferred.** Assessment B rebuilt the standalone harness outside Playwright with no dev server, no DB and no heavy slot. At 1280px the image box equals the anchor box to 0.000px on both axes, all 12 tiles share one identical box against 15 stubs, network shows zero requests to `/_next/image`, and the console carries no missing-loader-width warning — which is the runtime confirmation that the `href`/`sourceKey` split does what its comment claims.

**What the marker claims and what it cannot.** It records a gate RESULT. It does not prove the gates ran; `tests/docs/_invariant8Closeout.ts:11-16` says so about itself and assigns a fabricated marker to reviewer territory. The table above is the evidence.

**Why this marker could not be written earlier, kept because the constraint was real and shaped the branch.** `tests/docs/_metaInvariant8Closeout.test.ts` accepts only a well-formed marker line, and nothing truthful could go in that slot before the gates ran. A placeholder value there is a MALFORMED marker, which reds §4.1.2 as well as §4.1.1 — probed both ways. `PRE_GUARD_DEBT` is not the escape either: it is scoped to pre-guard history, and a row for a live arc reds on its own rule the moment the real marker lands.

So the plan was a declaring unit with no marker until this task, and **the branch was not pushed between the plan commit and this one**. The ledger's IN PROGRESS marker went to origin at Stage 0, which is what invariant 12 actually requires; the review dispatches read the live worktree rather than origin; and CI's twelve are a close-out gate, so nothing was lost by holding.

A smaller thing worth recording, because it cost a debugging cycle: an earlier draft of this note QUOTED the placeholder literal in order to explain what fails, and the gate's own `grep -qi` matched the quotation. Use versus mention, in the gate I had just hardened against exactly that. The note now names the shape without spelling it.

### Pre-code checklist result

Run against the diff rather than recalled, with the command for each. Clean, and nothing to fix.

- **Em dash ban and apostrophe literals in user-visible copy.** `git diff origin/main -- components/ app/ | grep '^+' | grep -E '—|'` returns matches only inside CODE COMMENTS. The diff adds no user-visible string at all: the sole rendered copy on this surface, "Preview unavailable", is unchanged from before the arc. Nothing owed.
- **44px tap targets.** `min-h-tap-min` occurrences in `components/admin/wizard/step3ReviewSections.tsx` are 21 before the arc and 21 after, so nothing lost one. The tile's interactive element is the anchor, whose box IS the 4:3 tile: measured 97px tall at the 390px sheet and 170px on desktop, far past the floor. Asserted in a real browser rather than assumed — `tests/e2e/step3-review-modal.layout.spec.ts` measures the anchor at 390/800/1280.
- **Canonical type and token classes.** The only color classes on added lines are `bg-surface-sunken` and `border-text-faint`, both already in the system and both already on this element before the arc. No type-scale class changes.
- **No new color token, so no new contrast pin is owed.** The diff introduces no `@theme` entry and no arbitrary color value.

One pre-existing condition is named here rather than discovered by the gate: the tile anchor carries no focus-visible ring, so a keyboard user gets only the browser default on a surface this arc touches. Not introduced by this diff. The mechanical UI checklist runs BEFORE the first UI edit, because the invariant-8 pair verifies and does not discover: em-dash ban and apostrophe literals in user-visible copy (this diff adds no user-visible string — the placeholder copy "Preview unavailable" is unchanged); 44px tap targets (the anchor is the interactive element and its box is the 4:3 tile, far past the floor at every measured breakpoint — asserted in Task 1's e2e); canonical type and token classes (no type or color class changes, and the two branches keep their distinct border tokens); no new color token, so no new contrast pin is owed.
