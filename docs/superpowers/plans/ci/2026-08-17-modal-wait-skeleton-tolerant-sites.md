...
- Edited: `tests/e2e/helpers/openShowReviewModal.ts`, `tests/e2e/published-review-modal.deeplink.spec.ts` (one wait site), `tests/e2e/published-review-modal.realtime.spec.ts` (one wait site + one comment block), `tests/ci/modalWaitHelper/scan.ts` (`HELPER_CALL` only), `tests/ci/modalWaitHelper/disposition.ts`, `tests/ci/_metaModalWaitHelper.test.ts`, `tests/mutation/source/registry.ts` (+ `tests/mutation/source/expectedLedgerKinds.ts` and `tests/mutation/_metaPremiseContract.test.ts` if the helper enrolls), `BACKLOG.md`/`BACKLOG-archive.md`, `docs/superpowers/plans/ci/README.md`.
...
<!-- task: red=`pnpm vitest run tests/e2e/helpers/openShowReviewFrame.unit.test.ts` red-state=authored red-target=`tests/e2e/helpers/openShowReviewModal.ts:43` why=`the module today exports only the loaded-only surface (LOADED_REVIEW_MODAL at :43 and the three shipped entry points; plan-time probe: 0 hits for the frame API names) — the sibling suite imports the frame API as missing named exports, which vitest/esbuild leaves undefined at load (spliced probe: a void-reference PASSES), so every case RUNS and fails the moment it calls the API (spliced probe: TypeError awaitReviewFrameOrRecover is not a function, 1 failed) until this task green half lands the exports` ac=AC-1 -->
...
3. **Skeleton return + watchdog arm:** skeleton visible, loaded not → `{ frame: "skeleton" }`, unscoped `SKELETON_REVIEW_MODAL` locator, AND a recorded `waitFor` on `[data-testid="admin-route-error-boundary"]` (the watchdog's arm — its fire path is spec limit 2's dynamic-import shadow, not unit-reachable). Catches: a watchdog that never arms, which would silently revert the site to today's no-annotation behavior.
4. **Loaded-priority tiebreak:** BOTH shapes visible (the streaming-swap overlap window) → `{ frame: "loaded" }`. Catches: a skeleton-first check that reports "skeleton" during the swap and makes the deeplink test's frame report lie in the common §2.2 path.
5. **Twin-frame cardinality** (parent AC-1 pattern): with a bare skeleton `…-modal` and a loaded `…-modal:has(…-title)` both present, `SKELETON_REVIEW_MODAL` and `LOADED_REVIEW_MODAL` resolve one DISTINCT node each — selector disjointness is what makes `frame` truthful.
...
Anti-tautology: each case's discriminating premise is the FakePage `visible` callback the case itself configures — expected values derive from the case's own fixture wiring, never from the module under test; case 5's two nodes are distinct fake locators so the assertion cannot be satisfied by one node matching twice.

Observe red: `pnpm vitest run tests/e2e/helpers/openShowReviewFrame.unit.test.ts` fails (unresolved import), while `pnpm vitest run tests/e2e/helpers/openShowReviewModal.unit.test.ts` stays 15 passed.
...
test("skeleton visible and loaded absent returns the skeleton frame and arms the watchdog", async () => {
  ...
  const boundaryWaits = fake.waitForCalls.filter((c) =>
    c.selector.includes("admin-route-error-boundary"),
  );
  expect(boundaryWaits.length).toBeGreaterThan(0);
});
```
...
<!-- task: red=`pnpm vitest run tests/ci/_metaModalWaitHelper.test.ts` red-state=authored red-target=`tests/ci/modalWaitHelper/scan.ts:135` why=`HELPER_CALL at :135 does not recognize openShowReviewFrameAt, and PINNED_EXEMPTIONS still pins two entries — so the moment this task rewrites the deeplink site (marker deleted, goto folded into the helper) the exemption-inventory assertion observes 1 exemption against a 2-row pin and fails; the same command passes once the census vocabulary (HELPER_CALL alternations, U-frame rule, derived arithmetic, one-row pin) lands in this task's green half` ac=AC-2,AC-4,AC-6 -->
...
1. RED: rewrite the deeplink site — delete the `modal-wait-exempt` marker and the goto+wait lines, insert `openShowReviewFrameAt` with label `"deeplink-esc:any-frame"`, extend the helper import. Observe `pnpm vitest run tests/ci/_metaModalWaitHelper.test.ts` red on the exemption-inventory assertion (found 1, pinned 2).
2. GREEN: `HELPER_CALL` gains both alternations (spec §4.6-1); `f/member-shape-U-frame` rule added (both names, shape "U", count 1, spec §4.6-2); `PINNED_EXEMPTIONS` drops the `/skeleton-tolerant/` row; the arithmetic test rewritten to the DERIVED sum over all origin-(f) member rules with per-id pins retained and total 52 at this task's end (53 lands with Task 3's N row; state the intermediate number in the commit body — the derivation makes the update mechanical).
...
3. **Same-commit mutation ledger refresh** (the registry NOTE, `tests/mutation/source/registry.ts:415-421`): the `scan.ts` edit relocates both accepted equivalent ids. Scoped shard per the lessons-file recipe (temporary `guardSurfaces.shardX.test.ts` filtered to `modal-wait-helper-scan`, `VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm heavy vitest run --project mutation tests/mutation/guardSurfaces.shardX.test.ts`, DELETE the temp file before commit — `_metaSourceShardIntegrity` pins the shard set). Refresh both siteIds, repay or per-row-accept any new survivor, record score + survivor set in the commit body.
4. Verify: meta suite green, candidate-v2 suite 15 passed (its fixtures reference neither new name), `pnpm typecheck`.

Commit: `fix(e2e): deeplink Esc site adopts the frame-reporting wait; census learns the frame entry (scan ledger refreshed same-commit)`.

### Task 3: Realtime adoption + N-wait registry row (red → green on the meta suite)

<!-- task: red=`pnpm vitest run tests/ci/_metaModalWaitHelper.test.ts` red-state=authored red-target=`tests/ci/modalWaitHelper/disposition.ts:110` why=`N_WAIT_SITES at :110 holds 12 rows and d/skeleton-tolerant-click still expects its one noWaitAfter site — so the moment this task rewrites the realtime wait into awaitReviewModalOrRecover the census observes a 13th N-wait with no registry row (exact-match assertion reds naming the triple) and the retired rule's count drifts 1 to 0; the same command passes once the registry row, the rule retirement, and the row-activation re-claim land in this task's green half` ac=AC-3,AC-4,AC-6 -->

**Files:** `tests/e2e/published-review-modal.realtime.spec.ts` (site rewrite + the `published-review-modal.realtime.spec.ts:54-58` comment rewrite per spec §4.4), `tests/ci/modalWaitHelper/disposition.ts` (retire `d/skeleton-tolerant-click`; drop the `!/noWaitAfter/` arm from `d/member-row-activation`, count 8 → 9; `N_WAIT_SITES` +1), `tests/ci/_metaModalWaitHelper.test.ts` (derived total 52 → 53).

1. RED: rewrite the realtime wait to `awaitReviewModalOrRecover(page, { timeoutMs: MODAL_OPEN_TIMEOUT_MS, label: "reopen:aborted-close" })` and rewrite the stale `MODAL_ANY` rationale comment to the spec §4.4 text (probe-cited). Observe the meta suite red on the registry exact-match (undeclared 13th triple named).
...
4. disposition.ts changed in Tasks 2+3 with `accepted: []` — no ids to relocate; the full re-score lands once, Task 4 (score-before-closeout, lessons file).

Commit: `fix(e2e): realtime aborted-close reopen adopts the loaded-only wait; N-wait registry gains its row`.

### Task 4: Mutation duties — disposition re-score + helper enrolment decision
...
<!-- task: red=`VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm heavy vitest run --project mutation tests/mutation/guardSurfaces.shardX.test.ts` red-state=authored red-target=`tests/mutation/source/expectedLedgerKinds.ts:280` why=`on the enrolment branch, adding the helper GUARD_SURFACES row leaves EXPECTED_LEDGER_KINDS (:280 block) without the new key and the gates suite pins key-set equality, so the scoped run is OBSERVED red the moment the row lands and green once the key, any accepted rows, and EXPECTED_ENV_TOUCHING are reconciled; on the not-expressible branch the probe row is removed again and the same command completes the same red-then-green cycle` ac=AC-6 -->

**Files:** `tests/mutation/source/registry.ts`, `tests/mutation/source/expectedLedgerKinds.ts`, `tests/mutation/_metaPremiseContract.test.ts` (enrolment branch only); temporary shard file (created, DELETED before commit).

1. **`modal-wait-disposition` re-score** (edited by Tasks 2+3; `accepted: []` — any survivor is repaid with a deciding case or per-row accepted, never silently): scoped shard run, score recorded. This is the score-before-closeout run; the Task 2 scan score and this one are both stated in the implementation round-1 diff brief (AGENTS.md criterion 4 + the codex-guard `GUARD SURFACE:` line).
2. **Helper enrolment probe** (spec §4.6-7): add the candidate `GuardSurface` row for `tests/e2e/helpers/openShowReviewModal.ts` (suitePaths = both unit suites; `operators: [...OPERATOR_NAMES]`; provisional floor; a `control` anchored in the frame race — flipping the loaded/skeleton priority check is the candidate, since unit case 4 notices; `accepted: []`). Observe the gates suite red (missing ledger-kinds key). Run the scoped gate and decide:
   ...
   - **Not expressible / floor unreachable:** remove the row, record the probe-backed disposition as a dated comment at the registry insertion point (step3 precedent — honest, never symbolic).
3. Delete the temp shard; gates suite green; `pnpm heavy pnpm test:fast` for collateral.

Commit: `test(mutation): disposition re-score + helper enrolment disposition (probe-backed)`.
...
- Diff-review briefs: guard surface — every brief carries the spec §6 consequence bound, `PROBE DOMAIN:` line, threat fence; round 1 adds `GUARD SURFACE:` + `MUTATION SCORE:` lines (Tasks 2 + 4 recorded runs). The convergence-gate hook additionally requires the literal acceptance-posture phrasing ("correct or signaled, never silently wrong") in the brief body.
