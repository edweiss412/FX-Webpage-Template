...
1. **Measure first, unconditionally.** Run `legfix <PR run id>`, `legfix <baseline run id>`, and `legwall` on both (baseline chosen by spec §7.1's rule). Record in the PR body: both run IDs, both leg counts, both medians, and both **max legs** — max leg is TOTAL job wall clock from `legwall`, NOT the max fixed overhead `legfix` reports; the authority records 91s fixed overhead against a 245s max leg on the same run and requires the latter. `legfix` prints its per-leg values and a median/max-fixed-overhead line even when the leg count is not 8, then exits 1 — that non-zero exit is DATA, not a tooling failure: it means fewer than eight legs completed, which is a classification input for step 2, and the figures it printed still go in the PR body labelled as a partial run.
