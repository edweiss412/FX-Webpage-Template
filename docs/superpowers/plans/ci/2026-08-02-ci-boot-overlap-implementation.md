...
2. **Classify the run by the conclusion of every REQUIRED job** — the 8 `unit-suite-db` legs, the 3 `unit-suite-nodb` legs, and the `unit-suite` aggregator (`gh run view <id> --json jobs --jq '.jobs[] | {name, conclusion}'`). Exactly one branch applies:
   - **Every required job's conclusion is exactly `success`** → go to step 3.
   - **Anything else** → not accepted. The branch is defined by the COMPLEMENT, not by an enumeration: GitHub's conclusion domain also carries `action_required`, `neutral` and `stale`, and a job can be ABSENT from the list entirely when the matrix did not fully expand, so any enumerated list would leave outcomes unclassified. `cancelled` and `skipped` most often mean a superseding push or a `needs` short-circuit rather than a defect. Proceed as follows.

     **First, the run-level check, because some non-`success` outcomes have no failing STEP to reason about.** If any required job is ABSENT from the job list, or concluded non-`success` without any step reaching a `failure` conclusion (a job cancelled before its first step, `neutral`, `stale`, `action_required`), the run is INCOMPLETE rather than failing: there is no signal in it about this branch's code at all. Re-dispatch it (`gh run rerun` on that run, or `gh workflow run unit-suite.yml --ref <branch>`) and return to step 1 with the new run. If a SECOND consecutive dispatch on the identical tree lands in the same shape, stop re-dispatching — that is a GitHub-side condition, not a code signal; record it in the PR body with both run IDs and escalate to the user.

     **Then, for every required job that DOES carry a failing step, classify PER CAUSE.** Both matrices set `fail-fast: false` (`.github/workflows/unit-suite.yml:106`, `.github/workflows/unit-suite.yml:140`), so one completed run can carry several independent failures — a combined-step failure alongside an unrelated `unit-suite-nodb` failure, or two elsewhere-failures of which one is a flake and the other deterministic. A per-RUN classification would have two sub-branches claiming the same run. So enumerate every failing required job with its failing step, attribute NARROWLY (the cause counts as the overlap's only if the COMBINED STEP ITSELF failed — `gh api .../jobs --jq '.jobs[] | select(.name|startswith("unit-suite-db")) | {job: .name, step: (.steps[] | select(.name|startswith("Boot local Supabase")) | {name, conclusion})}'`; "never reached vitest" is NOT the predicate, since `checkout`, `setup-cli`, the psql guard and the pnpm/node pair all precede the combined step), and give each cause one of:

     - **This cause IS the combined step** → item 1 is reverted (step 4). **Precedence:** if ANY cause is the combined step, the revert happens FIRST and the remaining causes are re-evaluated on the reverted tree — several may disappear with it, and the rest are cheaper to diagnose without the overlap in the picture.
     - **This cause is elsewhere and does NOT reproduce** on one re-run of the identical tree → a flake. Record it; no further action.
     - **This cause is elsewhere and DOES reproduce** on that re-run (same job, same step) → deterministic, and re-running again is an infinite loop. Reverting is also wrong: backing out the overlap cannot fix a `unit-suite-nodb`, prerequisite, aggregator or post-boot vitest regression. So DIAGNOSE it. If it reproduces on a `main` run of the same period, it is pre-existing and not this branch's to fix — record it in the PR body with the `main` run ID and escalate to the user, because merging while a required context is red on `main` is their call, not the run's. If it does NOT reproduce on `main`, it is this branch's regression: fix it as a new commit, which re-enters the push → green-CI → final-review loop of ship steps 7-8, then return to step 1 with the run that fix produced.

     **The revert is STICKY and TERMINAL for item 1.** Once step 4 has run, item 1 no longer exists on this branch, so step 3 is CLOSED for the remainder of it: a later all-green run does not re-open the accept path, cannot graduate `BL-CI-OVERLAP-BOOT-WITH-SETUP`, and must not be re-measured against the ≥8s gate — there is nothing left to measure, and runner noise on a reverted tree could otherwise "clear" a threshold for a change that is gone. After a revert, this task's remaining job is only to disposition any other causes and get the branch to green so item 2 can ship. Record the revert in the PR body as the decision of record.

     **Completion rule:** the run is dispositioned only when every enumerated cause has one of the three, and step 3 is reachable only from a subsequent run in which every required job concluded `success` AND no revert has occurred. A run with a mix — say one flake and one branch-only regression — takes the union of the actions and then re-enters at step 1. Say in the PR body which causes were found and what each got.
...
**Commit:** one of three, matching the branch actually taken —
...
- revert on a combined-step failure: `revert(infra): back out the boot/install overlap — the combined step failed in CI`. This case usually has `median=NA` (no vitest step ran, so no leg produced a fixed-overhead value), so the measured-`<n>s` template would state something false; the body carries `legfix`'s partial output and the failing job/step instead.
