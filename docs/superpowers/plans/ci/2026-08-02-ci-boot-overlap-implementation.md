...
2. **Classify the run by the conclusion of every REQUIRED job** — the 8 `unit-suite-db` legs, the 3 `unit-suite-nodb` legs, and the `unit-suite` aggregator (`gh run view <id> --json jobs --jq '.jobs[] | {name, conclusion}'`). Exactly one branch applies:
   ...
   - **Anything else** → not accepted. The branch is defined by the COMPLEMENT, not by an enumeration: GitHub's conclusion domain also carries `action_required`, `neutral` and `stale`, and a job can be ABSENT from the list entirely when the matrix did not fully expand, so any enumerated list would leave outcomes unclassified. `cancelled` and `skipped` most often mean a superseding push or a `needs` short-circuit rather than a defect. Attribute before deciding, and attribute NARROWLY: the cause counts as the overlap's only if the COMBINED STEP ITSELF failed — check the step-level conclusion (`gh api .../jobs --jq '.jobs[] | select(.name|startswith("unit-suite-db")) | {job: .name, step: (.steps[] | select(.name|startswith("Boot local Supabase")) | {name, conclusion})}'`). "Never reached vitest" is NOT the predicate: `checkout`, `setup-cli`, the psql guard and the pnpm/node pair all precede the combined step, and a failure in any of them also stops vitest while having nothing to do with this change. Then, exhaustively:
     **Classify PER CAUSE, not per run.** Both matrices set `fail-fast: false` (`.github/workflows/unit-suite.yml:106`, `.github/workflows/unit-suite.yml:140`), so one completed run can carry several independent failures — a combined-step failure alongside an unrelated `unit-suite-nodb` failure, or two elsewhere-failures of which one is a flake and the other deterministic. A per-RUN classification would have two sub-branches claiming the same run, which is what an earlier draft of this tree did. So: enumerate every failing required job with its failing step, and give each its own disposition from the three below. Then apply them under one precedence rule and one completion rule.

     - **This cause IS the combined step** → item 1 is reverted (step 4). **Precedence:** if ANY cause is the combined step, the revert happens FIRST and the remaining causes are re-evaluated on the reverted tree — several of them may simply disappear with it, and the ones that do not are cheaper to diagnose without the overlap in the picture.
     - **This cause is elsewhere and does NOT reproduce** on one re-run of the identical tree → a flake. It needs no action beyond being recorded.
     ...
     **Completion rule:** the run is dispositioned only when EVERY enumerated cause has one of the three, and step 3 is reachable only from a subsequent run in which every required job concluded `success`. A run with a mix — say one flake and one branch-only regression — takes the union of the actions (record the flake, fix the regression) and then re-enters at step 1. Say in the PR body which causes were found and what each got.
