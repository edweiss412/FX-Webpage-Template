...
2. **Classify the run by the conclusion of every REQUIRED job** — the 8 `unit-suite-db` legs, the 3 `unit-suite-nodb` legs, and the `unit-suite` aggregator (`gh run view <id> --json jobs --jq '.jobs[] | {name, conclusion}'`). Exactly one branch applies:
   ...
   - **Anything else** → not accepted. The branch is defined by the COMPLEMENT, not by an enumeration: GitHub's conclusion domain also carries `action_required`, `neutral` and `stale`, and a job can be ABSENT from the list entirely when the matrix did not fully expand, so any enumerated list would leave outcomes unclassified. `cancelled` and `skipped` most often mean a superseding push or a `needs` short-circuit rather than a defect. Attribute before deciding, and attribute NARROWLY: the cause counts as the overlap's only if the COMBINED STEP ITSELF failed — check the step-level conclusion (`gh api .../jobs --jq '.jobs[] | select(.name|startswith("unit-suite-db")) | {job: .name, step: (.steps[] | select(.name|startswith("Boot local Supabase")) | {name, conclusion})}'`). "Never reached vitest" is NOT the predicate: `checkout`, `setup-cli`, the psql guard and the pnpm/node pair all precede the combined step, and a failure in any of them also stops vitest while having nothing to do with this change. Then, exhaustively:
     - **The combined step itself failed** → go to step 4 (revert).
     - **The failure is elsewhere and does NOT reproduce** on one re-run of the identical tree → it was a flake. Return to step 1 with the new run.
     - **The failure is elsewhere and DOES reproduce** on that re-run (same job, same step) → it is deterministic, and re-running again is an infinite loop, which is what an earlier draft of this tree did here. Reverting is also wrong: backing out the overlap cannot fix a `unit-suite-nodb`, prerequisite, aggregator or post-boot vitest regression, and would discard a change the evidence does not implicate. So DIAGNOSE it. If it reproduces on a `main` run of the same period, it is pre-existing and not this branch's to fix — record that in the PR body with the `main` run ID and escalate to the user, because merging while a required context is red on `main` is their call, not the run's. If it does NOT reproduce on `main`, it is this branch's regression: fix it as a new commit, which re-enters the push → green-CI → final-review loop of ship steps 7-8 like any other repair, then return to step 1 with the run that fix produced.

     Exactly one of the three applies to every non-`success` outcome, and each terminates: revert, re-measure, or a bounded repair. Say in the PR body which happened.
