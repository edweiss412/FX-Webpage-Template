...
| …and on x86_64 Linux, from a FRESH install | `git archive HEAD` piped into `--platform linux/amd64 node:20-bookworm`, `corepack enable`, `pnpm install --frozen-lockfile`, with a `sha256sum` manifest of `supabase/` diffed before and after | x86_64, Debian 12 (NOT Ubuntu 24.04 — the shared axis is linux/x64/glibc, which is what these install scripts branch on; the distro gap is documented in spec §3.2); the same 4 build scripts + `prepare` ran; 126-file manifest IDENTICAL, so no adds, deletes or content changes; one Linux-only candidate `@parcel/watcher` 2.6.0 correctly IGNORED (spec §3.2 probe B) |
...
Scheduled after the run COMPLETES, not after it goes green — the revert branch exists precisely for the not-green case, and gating the task on green would make it unreachable.

1. **If any required leg is red** — any of the 8 `unit-suite-db` legs, the 3 `unit-suite-nodb` legs, or the `unit-suite` aggregator — first attribute the failure. If it is attributable to the overlap (a boot or install failure inside the combined step, or a leg that never reached vitest), item 1 is REVERTED, go to step 4. If it is an unrelated flake, re-run and continue. `legfix` exiting 1 with "FAIL: n legs, expected 8" is itself this signal, not a tooling problem: fewer than eight legs reported means fewer than eight completed.
2. **If everything required is green,** run `legfix <PR run id>` and `legfix <baseline run id>`, the baseline chosen by spec §7.1's rule, and record in the PR body: both run IDs, both leg counts, both medians, **and both max legs** (`gh run view <id> --json jobs`, or the same `--jq` with `max` instead of the median step) — spec §7.4 requires median AND max whichever way the decision goes.
3. **Accept** if the median drops by ≥8s AND every required leg is green. Graduating `BL-CI-OVERLAP-BOOT-WITH-SETUP` has a real red-first hook, so it is done TDD-shaped: FIRST add its row to `BACKLOG_GRADUATED` in `tests/docs/_metaDeferralLedgerGraduation.test.ts:90` (`{ id: "BL-CI-OVERLAP-BOOT-WITH-SETUP", provenance: "chore/ci-boot-overlap-and-popover-flake" }`) — red, because the entry is still in `BACKLOG.md` and absent from the archive — THEN move the entry to `BACKLOG-archive.md` with the two figures and a section naming that branch, and re-run to green. Without the registry row the move alone would satisfy only the no-overlap assertion; the archive-only and section-scoped provenance assertions (`tests/docs/_metaDeferralLedgerGraduation.test.ts:351` and `tests/docs/_metaDeferralLedgerGraduation.test.ts:357`) run only for registered ids.
4. **Revert** otherwise. The revert is ONE operation over the PAIR of item-1 files — `.github/workflows/unit-suite.yml` AND the `describe` block added to `tests/cross-cutting/unit-suite-shard-topology.test.ts` — because that block pins the combined step's existence; reverting only the workflow leaves the suite red (spec §7). T2's work and its guard are untouched by this path and still ship. `BL-CI-OVERLAP-BOOT-WITH-SETUP` stays OPEN, updated with the measurement that forced the revert, so the next attempt starts from data. Then re-run the pre-push ladder and push; the revert commit needs its own green CI before merge.
