...
- F1/F3 negatives: catch over-poisoning (a guard that reds clean jobs forces reason-free rows — precision is load-bearing, not cosmetic). **F6 honesty (plan-review R4):** the CENSUS comment fixture is mutation-sensitive — deleting its full-line-`#` filter makes a comment-only mention poison. The SCANNER's comment-glue fixture is NOT: since R6, `canonicalYaml` parses and re-emits the YAML, so the parser has already dropped comments before any filter runs. It stays as a behavioural regression pin (comment prose must never poison), not as a mutation probe for those filters.
...
- F7 + F8 (added at spec R1 via live escaping mutants, refined R7-R10): catch map-presence-treated-as-modeled and recursion deletion — in-memory mutants of each produce `covered=true, reasons=[]`. Both are pinned at direct AND nested sites in both layers (the nested F7 fixtures were added at plan-review R4, which correctly found them direct-only); the R8-R10 additions (typed validator, narrow accept profile, uses classifier) each carry their own shape tables.
...
**Latest run (HEAD `f9e55566e`, after the R19/R20 schema work):** typecheck, lint, and format:check green; `pnpm test` = 1696 passed / 1 failed, the failure again `tests/scripts/runExcludedTest.test.ts` and again green in isolation. Five consecutive ladders have now produced the same shape, which is what makes the contention diagnosis a measurement rather than an excuse.
