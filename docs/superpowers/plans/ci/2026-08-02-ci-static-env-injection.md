...
- S1 POSITION matrix (added at final review (a) R3/R4): dirty SECOND workflow file, dirty SECOND job, dirty SECOND run-step, dirty SECOND `uses:`-step, dirty LATER composite sibling (incl. dirt behind the SECOND `uses:` one level down), and a claim at a LATER job step — every iterated collection carries a late-position positive, plus a clean-late-sibling twin;
...
- S8 (folded in at spec R3–R5, extended at final review (a) R2/R4/R5/R6/R7 — incl. multi-value action scope with a last-value-only negative, a guarded invocation and guarded composite step conferring nothing, composite `uses:`-step env at both depths, and additive action-vs-direct credit with a direct-only negative): governance twins through pure `envPairGovernance`/`governanceViolations` — relocation, prose-park, silent gain, and path-gated duplicate each red; faithful + alias-resolved pass; the value-SWAP twin (pair-keyed governance); a positive per CREDIT SITE (workflow-root / job / run-step / whole-config) and per POSITION (later job, later step); CARDINALITY cells (two-member whole-config, two claiming steps) each with a TRUNCATED-ROW negative; PRECEDENCE twins (root<job, root<step, job<step — the shadowed value governs nothing and the swap reds); and ACTION-SCOPED pairs governing later claims at EVERY action-scoped site (uses-invocation env, composite run-step env, composite `uses:`-step env, nested run and uses env), with a relocation twin, an ADDITIVE-credit twin (an action value and a same-key direct value BOTH govern, with a direct-only row that reds — action-scoped credit is never suppressed by direct scope), and guarded-invocation / guarded-composite-step / guarded-parent cells that confer nothing while the claim stays covered.
...
- S3 clean twins at EVERY modeled scope: root/job/run-step, `uses:`-step (remote and local refs), all four composite cells, and a clean LATER composite sibling (a missing clean cell lets a coarsening mutant escape at exactly that scope);
- POSITION cells mirrored census-side: dirty SECOND job, dirty SECOND run-step, dirty SECOND `uses:`-step, dirty LATER composite sibling;
...
- S1 matrix — TRAVERSAL and POSITION only; every governance axis pins under S8 below, not here (incl. all composite cells, remote AND local refs, + census standalone): a mutant deleting the check at any single scope, or narrowing the composite walk by depth or step kind — including narrowing by `usesKind` — reverts that cell to probe behavior (`covered=true` / `poisoned=false`) and fails exactly that fixture. The R1 probe demonstrated all three composite escape cells live; the final review (a) probe demonstrated the LOCAL-invocation cell, whose fixtures resolve a CLEAN manifest so only the invoking step's env can produce the refusal. Final review (a) R3 then demonstrated the sibling-POSITION axis (first-sibling-only traversal, and recursion into only the first `uses:`), so every axis now carries a late-position positive with a clean earlier sibling, plus the swept workflow-file and job-step axes. R4 then demonstrated the JOB and workflow-STEP source axes and the governance truncation axes (first job / first step / first whole-config member / first accumulated claim), closed with late-position positives plus truncated-row negatives. R5 demonstrated two derivation-fidelity axes: scope PRECEDENCE (a shadowed value kept the governance of the effective one) and ACTION-SCOPED pairs (uses-invocation and composite env conferred nothing), both now credited.
