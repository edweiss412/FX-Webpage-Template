...
6. `pnpm typecheck` + both deciding suites green + `VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm heavy vitest run --project mutation tests/mutation/guardSurfaces.gates.test.ts` (the suite lives ONLY in the opt-in mutation project — `vitest.projects.ts:90`; the bare unwrapped form collects nothing and exits 1, R2 finding 2; heavy-wrapped because every `--project mutation` run is heavy by invocation shape, AGENTS.md semaphore rule — R3 finding 3) + `pnpm heavy pnpm test:fast` for collateral (full-suite vitest is likewise always wrapped). Record the score + unaccepted-survivor set (must be empty) in the commit body — the number the round-1 diff brief states.
...
1. Add the COMPLETE candidate row — the `GuardSurface` type requires all seven fields (`tests/mutation/source/registry.ts:12-37`, and the gate validates the floor and the control anchor): `id: "modal-wait-disposition"`, `sourcePath: "tests/ci/modalWaitHelper/disposition.ts"`, `suitePaths` = both deciding suites, `operators: [...OPERATOR_NAMES]`, provisional `scoreFloor: 0.9` (raised to the observed tier once the run reports), a `control` whose `from` anchors uniquely in the post-Task-2 file and changes behavior (candidate shape: one member rule's `expectedCount` literal flipped — the drift assertion notices; the implementer picks the exact anchor and the gate's anchor validation is the check), and `accepted: []`. Observe the gates suite red (missing `EXPECTED_LEDGER_KINDS` key).
2. Run the scoped gate (temp shard filtered to the new id). Decide from the run — either branch leaves a TRACKED artifact so the task commit is never empty (plan review R3 finding 2):
   - **Keep:** survivors dominated by equivalent mutants of `reason`/`protects` prose strings → accept per-row with reasons, set `scoreFloor` to the observed tier, add the `EXPECTED_LEDGER_KINDS` entry. The commit body AND Task 5's round-1 brief then carry BOTH surfaces' scores.
   - **Not expressible:** remove the row and record the probe-backed disposition as a dated comment block at the registry insertion point in `tests/mutation/source/registry.ts` (the step3-a11y precedent: re-dispositioned honestly with the probe that shows it, never enrolled symbolically) — a tracked-file change, so the commit exists; the gates suite returns green on removal and step 1's red still completed the cycle.

   Decided BEFORE the first diff-review dispatch (AGENTS.md criterion 4).
3. Delete the temp shard file; `git status` clean of it; `VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm heavy vitest run --project mutation tests/mutation/guardSurfaces.gates.test.ts` green.
