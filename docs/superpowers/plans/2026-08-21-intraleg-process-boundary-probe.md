...
**Acceptance:** the dated probe record this task creates under the probes directory (the exact path is the red command's test target) exists, carrying per-arm eligible-versus-planned counts, seeds, commands and raw outputs; adjudication text quotes spec §3 VERBATIM (no post-hoc readings); the record contains the production isolation evidence — the campaign directory's record count (nonzero) AND byte-identical before/after listings of the default `.mutation-records` across the campaign window, both pasted (AC-6 on the real run, not only in-process); every gate command in the campaign driver was probed against a constructed failing input and observed exiting non-zero (transcript pasted in the record).
