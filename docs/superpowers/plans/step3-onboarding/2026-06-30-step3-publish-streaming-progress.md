...
**Spec:** `docs/superpowers/specs/step3-onboarding/2026-06-30-step3-publish-streaming-progress.md` (Codex-APPROVED). Read it — this plan implements it and does not restate every rationale.
...
6. **Two-batch `listed` reconciliation (spec §12 server test):** inject `deps.batchCap = 2` and seed 3 finishable clean rows. Drive `handleOnboardingFinalizeStream` twice (batch 1, then batch 2 — the client-driven loop is simulated by two calls against a fake whose consumed rows carry over). Assert: batch-1 stream has `listed.total === 3` and 2 `row` events; batch-1 terminal `remaining_count === 1`; batch-2 stream has `listed.total === 1` and 1 `row` event; and the reconciliation identity holds: `2 (rows processed in batch 1) + 1 (listed.total batch 2) === 3 (listed.total batch 1)`. **Catches:** a `listed` total that is per-batch-relative (would break the client's cross-batch grandTotal) or an off-by-one in the remaining count.

> **Non-streaming byte-identity (spec §12 "pinned snapshot"):** rather than a brittle single snapshot, the byte-identity of the non-streaming path is proven by the EXISTING `tests/onboarding/finalize.test.ts` / `finalize-cas.test.ts` / `finalizeRevalidate.test.ts` / `finalizeApprovalRace.test.ts` suites passing UNCHANGED after the refactor (Step 5) — they assert exact bodies and HTTP statuses across many scenarios, which subsumes a one-shot snapshot. Case 1 additionally cross-checks that the STREAMING terminal body deep-equals the non-streaming body for the same fake.
