...
**Goal:** Fix four shipped Step-3 review-modal defects per `docs/superpowers/specs/step3-onboarding/2026-07-04-step3-modal-followups-b2.md` (the spec; canonical): XLSX-media diagram previews (§A), footer pending-label inversion (§B), rescan false-positive copy (§C), report-form progressive disclosure (§D).
...
Branch at `:1122` uses `showCheckedSlot`; UPDATE the stale marker comment above it (`:1121` currently documents "slot follows the checked prop; spec §C2/§H N5") to the amended contract — e.g. `{/* §11 N5: instant — deliberate (slot follows the operation in flight while pending, else the checked prop; spec 2026-07-04 §B2 amendment) */}` — keeping the literal tokens `§11` and `instant — deliberate` so the source-marker classifier (`step3ReviewModal.transitions.test.tsx:1027-1034`) still classifies it; both buttons use `disabled={isPending}` / `aria-busy={isPending || undefined}`; labels: `publishLabel = pendingOp === "publish" ? "Selecting…" : "Publish this show"` and checked-slot `{pendingOp === "unpublish" ? "Removing…" : "Unpublish"}`; error note condition stays `publishState === "error"`. Every `publishState === "pending"` comparison in the file updates (grep the file — includes any scroll/drag guards if present).
...
- [ ] **Step 1: failing tests (spec T-C1/T-C2)** — extend the existing `toEqual` shapes. Branch mapping (verified against the tests' actual scenarios, NOT their line order): `demoted: true` for every result produced by the DIRTY branch (`rescanWizardSheet.ts:386-398`) — the MI-regression cases at `:401,428` AND the corrupt-prior fail-closed case at `:594-619` (it reaches the dirty branch via `prior.priorReady && prior.priorParse === null`, `rescanWizardSheet.ts:384`); `demoted: false` for the clean re-stamp at `:360` AND the clean-unapproved/concurrent-unapprove case at `:550-569` (it returns from the `:445` branch). Route test asserts `mapResult` forwards `demoted`. Run the Global-Constraints sweep (`rg 'status: "updated", needsReview' tests/ lib/`) and update EVERY hit including Flow-B.
...
- [ ] **Step 1: failing structural test** — add a describe to `step3ReviewModal.transitions.test.tsx` that reads the `ReportIssueSection` source region (slice `step3ReviewSections.tsx` from `"export function ReportIssueSection"` to the next top-level `export`), runs the file's existing `findConditionalLines` over it, and asserts every conditional-render hit carries a deliberate-instant marker on the line above. The existing `isClassified` is keyed on `§11` (`:1027-1034`) — do NOT reuse it as-is; define a local `isClassifiedD2` with the same shape keyed on `§D2` + `instant — deliberate`. Run → FAIL (Task 7's new `{expanded ? (` conditional has no marker).
