...
**Spec:** `docs/superpowers/specs/step3-onboarding/2026-06-29-per-sheet-rescan-wizard.md` (APPROVED, Codex 4 rounds). The spec is the behavioral source of truth; this plan is the build sequence. Cite the spec section for exhaustive behavior; cite `file:line` for every live API.
...
- **CREATES** `tests/onboarding/_metaRescanDecisionInvariants.test.ts` — pins the decision-requiring-invariant set (`{MI-11, MI-12, MI-13, MI-14}`, asserted ⊇ the multi-action `allowedActions().size>1` set) so a future gated invariant can't silently bypass the clean rule (spec §6 / §11 T-M).
...
```bash
git add docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md lib/messages/catalog.ts lib/messages/__generated__/spec-codes.ts app/api/admin/onboarding/finalize/route.ts
...
- Produces: `type RescanResult` (spec §5.4) and `async function rescanWizardSheet(driveFileId: string, wizardSessionId: string, deps: RescanDeps = {}): Promise<RescanResult>`. `RescanDeps` injects `fetchDriveFileMetadata`, `prepareOnboardingFiles`, `afterDriveRead?` (TOCTOU test seam), and **`withTx`** — a runtime yielding a **RAW postgres.js transaction** (`PostgresTransaction`, exposing `.unsafe(sql, params)`), so `PostgresOnboardingScanTx` can be constructed directly on it (mirrors the `.db.test.ts` harness: `sql.begin(async (rawTx) => { new PostgresOnboardingScanTx(rawTx, …); await rawTx.unsafe('select pg_advisory_xact_lock(…)') })`). The default runtime opens a `postgres()` connection and `sql.begin(fn)`. **NOT** a finalize-style `.query()`-only adapter (finding r2-3: `PostgresOnboardingScanTx` calls `tx.unsafe`).

- [ ] **Step 1: Write the failing real-DB tests** (`tests/onboarding/rescanWizardSheet.db.test.ts`) — copy the harness shape from `tests/onboarding/finalizeCasReonboardBaseline.db.test.ts` (seed app_settings pending session+folder, stage via `runPhase1`+`PostgresOnboardingScanTx`). Cases (spec §11): **T-A1** clean+approved → refreshed parse, `wizard_approved=true`, `wizard_approved_by_email` non-null + `wizard_approved_at` refreshed (CHECK passes), `last_finalize_failure_code=null`, choices regenerated to NEW item ids (derive from row), `needsReview=false`. **T-A2** email change → `wizard_approved=false`, `last_finalize_failure_code='RESCAN_REVIEW_REQUIRED'`, `triggered_review_items` includes the MI-11 item, manifest `'staged'`; then `handleOnboardingFinalize` does NOT consume it + `unresolvedManifestCount` blocks. **T-A3** gap-count regress → blocked; remove-gap stays clean. **T-A4** hard-fail → `needs_attention` + concrete `pending_ingestions.last_error_code`, manifest `hard_failed`. **T-SCOPE2** metadata.parents lacks `pending_folder_id` → `STAGED_PARSE_SOURCE_OUT_OF_SCOPE`, no mutation. **T-LOCK(i)** hold `finalize:<session>` concurrently → `CONCURRENT_FINALIZE_IN_FLIGHT`, no mutation. **T-LOCK(iv)** run `unapprove` during the pre-lock Drive seam → under-lock re-read sees `priorReady=false` (no resurrected approval). **T-MANIFEST (finding r2-1)** a `driveFileId` in the pending folder but with NO `onboarding_scan_manifest` row for the session → `{status:"not_found"}`, zero mutation (no `pending_syncs`/manifest/shadow written). **T-DEMOTED-CLEAN (finding r2-2)** a previously-demoted pending row (`wizard_approved=false`, `last_finalize_failure_code` non-null) rescanned with NO crew change / gap regression → `last_finalize_failure_code` cleared to `null` and the row no longer counts in `unresolvedManifestCount` (un-blocked).
...
- [ ] **Step 3: Implement `rescanWizardSheet`** per spec §5:
  ...
  - **§5.3 locked tx — `await deps.withTx(async (rawTx) => { … })`:** acquire `await rawTx.unsafe("select pg_try_advisory_xact_lock(hashtext('finalize:'||$1)) as locked", [wizardSessionId])` → `locked===false` ⇒ `{status:"busy", code:"CONCURRENT_FINALIZE_IN_FLIGHT"}`; `rawTx.unsafe("select pending_wizard_session_id from app_settings where id='default' for update")` → mismatch ⇒ `superseded`; **manifest-membership guard (finding r2-1):** `rawTx.unsafe("select 1 from onboarding_scan_manifest where wizard_session_id=$1 and drive_file_id=$2", [wizardSessionId, driveFileId])` → no row ⇒ `{status:"not_found"}` (NO mutation); then `rawTx.unsafe("select pg_advisory_xact_lock(hashtext('show:'||$1))", [driveFileId])`.
  - **Step 2.0 capture prior state UNDER lock** (read `pending_syncs` OR shadow per spec §5.3 via `rawTx.unsafe`; `priorReady`, `priorApprovedByEmail`, `priorParse` (shadow via `parseShadowPayloadForApply`, corrupt → null), `priorChoices`, `priorDataGaps`, plus the prior `staged_modified_time` for `changed`).
  - **(a) re-stage (raw-tx adapter, mirroring the `.db.test.ts` harness, NOT holdPort):** `const scanTx = new PostgresOnboardingScanTx(rawTx, folderId, wizardSessionId)` (rawTx already exposes `.unsafe`), then `const res = await scanOnboardingPreparedFiles(folderId, wizardSessionId, [prepared], { tx: scanTx, withShowLock: async (_id, fn) => fn(scanTx) })` — the passthrough reuses the held `show:` lock (no second acquire; staging is wizard-scoped and atomic in this tx). If `res.processed[0].outcome === "hard_failed"` → `DELETE` orphan shadow, read `pending_ingestions.last_error_code` (just written), return `{status:"needs_attention", code:<that>}`.
