...
Coverage closure (so an omitted family cannot pass silently): a reusable **grain-walker assertion** `assertShowForViewerGrain(data)` (test-local helper: walks the output asserting every field's runtime type matches the projection grain) run on the output of EVERY malformed-input case, PLUS a **generative malformation sweep** over every LEAF PATH the fixture exercises: (a) every top-level `ParseResult` field; (b) every field of the first entry of each ARRAY family (crew, hotels, rooms, contacts, pull-sheet, agenda_links, ScheduleDay.entries); (c) every field of each non-array OBJECT family — `transportation` (e.g. `parking: 42` reaches `.trim()` consumers via a shallow copy), `show.venue`, `show.client_contact`, `show.event_details`, `show.schedule_phases`, the remaining `show` scalars, and each `runOfShow` day value's fields. Each case replaces the leaf with a wrong-typed scalar (`42`) and asserts the adapter either returns `decode_error` (permitted only for `show.dates`) or returns `ok` whose output passes the grain walker AND (for the Task 3 integration fixture's happy path) renders without throwing — a shallow-copy adapter fails on the first unnormalized leaf. The sweep is code-generated from `Object.keys` walks of the fixture, not a hand list, so a fixture field added later is swept by default.
...
**Registry-count reconciliation (authored AND run):** this plan adds exactly 1 `infraRegistry` row (Task 5), 1 e2e allowlist `UNSEEN` row (Task 8), 0 rows to every other registry — verified at plan time by `grep -c "path:" tests/admin/_metaInfraContract.test.ts` before/after being +1 in Task 5's Step 3 (the tests-before-helper step), and the allowlist meta's own fail-by-default walk being Task 8's red.
