...
**Meta-test inventory (writing-plans rule):** this plan EXTENDS `tests/admin/_metaInfraContract.test.ts` (one registry row, Task 5), the fail-closed route audit `PROTECTED_ROUTES` (one `requireAdmin` row for the new page, Task 5 — the completeness walk at `lib/audit/protectedRoutes.ts:27-33` rejects any tracked `app/admin/**/page.tsx` without a row, enforced by `tests/cross-cutting/auth-chain-audit.test.ts`), and the e2e coverage allowlist `tests/ci/_metaE2eWorkflowCoverage.test.ts` (one `UNSEEN` row, Task 8). No other registry applies: no admin_alerts catalog change (no new alert code), no advisory-lock topology change, no sentinel-hiding surface, no email normalization, and no mutation-surface observability row (the page is a GET-only render with no server action or mutating route, so the filesystem walk in `tests/log/_metaMutationSurfaceObservability.test.ts` discovers nothing new).
...
Coverage closure (so an omitted family cannot pass silently): a reusable **grain-walker assertion** `assertShowForViewerGrain(data)` (test-local helper: walks the output asserting every field's runtime type matches the projection grain) run on the output of EVERY malformed-input case, PLUS a **generative malformation sweep** over every LEAF PATH of the fixture, enumerated by a RECURSIVE walk `enumerateLeafPaths(fixture)` — no per-family hand list at any depth: objects recurse into every key; arrays recurse into EVERY index (never only index 0 — the fixture's restricted members sit at DIFFERENT crew indices, and the URL-only `agenda_links` entry is the second element, so an index-0-only walk could never reach the stage-restricted member's `stage_restriction.stages` leaves or the url-only entry's `url` leaf); a primitive array element is itself a leaf (so every `show.dates.showDays` element, every `transportation.assigned_names` element, each crew member's `date_restriction.days` and `stage_restriction.stages` elements (`lib/parser/types.ts:120-126`), and every `show.schedule_phases[date]` element are swept), and nested families like each `pullSheet[i].items[j].cat`, each `transportation.schedule[i].stage`, and `show.client_contact.secondary` leaves fall out of the recursion rather than any enumeration. (Sweep size stays trivial: the fixture is a hand-built object of a few hundred leaves, each case one adapter call plus one grain walk.) Each case replaces the leaf with a wrong-typed scalar (`42` for non-number leaves, `"x"` for number leaves, so the mutation is always wrong-typed) and asserts the adapter either returns `decode_error` (permitted only for the `show.dates` family) or returns `ok` whose output passes the grain walker AND (for the Task 3 integration fixture's happy path) renders without throwing. This depth matters because `asParseResult` validates containers, never their elements (`lib/db/coerceJsonbObject.ts:133`), and element-grain garbage crashes real consumers: `showDays: [42]` reaches `a.localeCompare(b)` (`lib/crew/agendaDisplay.ts:129`); a numeric `transportation.schedule[0].stage` reaches `shouldHideGenericOptional`'s `.trim()` (`components/crew/sections/TravelSection.tsx:324`); a numeric `pullSheet[0].items[0].cat` reaches the same predicate (`components/crew/sections/GearSection.tsx:403`). NON-VACUOUSNESS PREMISE (a guard states its premise executably): the sweep only visits paths the fixture populates, so Step 1's fixture MUST carry every optional nested family non-empty — `show.dates.showDays`, `show.schedule_phases` (at least one date key with at least one phase), `show.client_contact` with a populated `secondary`, `transportation.schedule` (at least one leg with stage/date/time populated) plus `assigned_names`, at least one `pullSheet` case with at least one item carrying `cat`/`subCat`, and crew restrictions carrying a non-empty `date_restriction.days` (the explicit-M/D member) and a non-empty `stage_restriction.stages` (the stage-restricted member) — and the test asserts the enumerated path set CONTAINS a sentinel list of the deepest known paths (at minimum: `show.dates.showDays.0`, one `show.schedule_phases.<date>.0`, one `show.client_contact.secondary` leaf, `transportation.schedule.0.stage`, `transportation.assigned_names.0`, `pullSheet.0.items.0.cat`, one crew `date_restriction.days` element, one crew `stage_restriction.stages` element), so a later fixture edit that empties a family fails the premise loudly instead of silently shrinking the sweep.
...
- [ ] **Step 1: Write the failing test file** — cases (derive expectations from the fixture object built in-test, never hardcoded literals; the fixture is a hand-built `ParseResult` with 4 crew members [one LEAD with explicit `6/24` M/D date restriction, one `ONLY***`-style stage-restricted, one unrestricted member with neither entitlement flag, one whose ONLY capability flag is FINANCIALS], 2 hotel reservations naming different members, 2 rooms with the SAME name, agenda_links with one fileId entry + one url-only entry, po/proposal/invoice/invoice_notes populated, runOfShow spanning restricted + unrestricted days):
  ...
  3. date normalization: the explicit `M/D` viewer's `dateRestriction.days` deep-equals the in-test derived ISO normalization of the fixture's `M/D` tokens against `show.dates` (equality, not subset; premise: the derived set is non-empty, so a `days: []` projection fails);
...
- [ ] **Step 1: Write the failing test** — harness pattern of `tests/components/crew/crewShellSections.test.tsx` (hoisted `upsertAdminAlert` mock, pinned `nowDate`), fixture via `makeShowForViewer`:
  ...
  10. RENDERED schedule equality for BOTH restricted viewers: the stage-restricted viewer's schedule-body day set deep-equals the in-test derived worked-day set from `effectiveViewerDateRestriction`, and the explicit `M/D` viewer's schedule-body day set deep-equals their normalized ISO restriction days ∩ show days (equality, not subset; premise: each derived set non-empty — an adapter projecting `days: []` while gating runOfShow from private state fails here).
...
- Modify: `tests/admin/_metaInfraContract.test.ts` (one `infraRegistry` row for `lookupStagedRow` PLUS a bespoke behavioral `describe` block — the registry rows are grep/static coverage only; the behavioral proofs are the file's separate `describe` blocks, so this task adds one with SIX arms. Four yield `{ kind: "infra_error" }`: a client whose `.from()` throws synchronously; a builder that rejects mid-await; a builder that RESOLVES `{ data: null, error: <object> }` (returned-error path — a helper mapping returned errors to `not_found` fails this arm); and `createSupabaseServerClient` itself throwing at construction. Two prove the REAL helper's non-infra semantics, because the route test reaches `not_found` only through its module mock: a builder resolving `{ data: null, error: null }` (a healthy `maybeSingle()` miss) returns `{ kind: "not_found" }` — an implementation mapping healthy nulls to `infra_error`, or throwing on them, fails this arm and would turn AC-4's required 404 into an error surface; and a builder resolving `{ data: <row>, error: null }` returns `{ kind: "found", row }` with the snake_case columns mapped to the Produces field names)
- Modify: `lib/audit/trustDomains.ts` (one `PROTECTED_ROUTES` row for the new page — the fail-closed completeness audit walks every tracked `app/admin/**/page.tsx` via `git ls-files` and fails any page without a row, `lib/audit/protectedRoutes.ts:6-33`)
<!-- spec-lint: ignore — file created by this task; not yet tracked -->
...
- [ ] **Step 3: Write the helper's tests BEFORE the helper** — add to `tests/admin/_metaInfraContract.test.ts` the `infraRegistry` row (`helper`/`path`/`contract` shape, near line 170) AND the bespoke `describe("lookupStagedRow infra contract")` with the six arms from the Files block (four infra arms; healthy-null `not_found`; found-row column mapping). Run `pnpm vitest run tests/admin/_metaInfraContract.test.ts` — Expected: FAIL (grep-visibility: the helper module does not exist). Record the red.
<!-- spec-lint: ignore — helper module created by this task; not yet tracked -->
- [ ] **Step 4: Implement `lib/admin/lookupStagedRow.ts`, page, and error.tsx** (page exports `metadata` per case 9; the helper distinguishes returned-error from thrown per invariant 9). Then the route-audit red, observed BEFORE its repair: `git add` the new page files, run `pnpm vitest run tests/cross-cutting/auth-chain-audit.test.ts` — Expected: FAIL with `app/admin/wizard/preview/[stagedId]/page.tsx is not classified in TRUST_DOMAINS` (the completeness walk, `lib/audit/protectedRoutes.ts:27-33`). Repair: add `{ path: "app/admin/wizard/preview/[stagedId]/page.tsx", chain: ["requireAdmin"] }` to `PROTECTED_ROUTES` (`lib/audit/trustDomains.ts:30`), matching the `preview/[crewId]` precedent row and the page's actual `requireAdmin()`-first chain. Record the red in the commit message. (`error.tsx` needs no row: the audit's git-ls arm globs only `page.tsx`, and `classifyTrustDomain` already classifies admin-tree `error.tsx` as `admin`, `lib/audit/trustDomains.ts:270-283`.)
- [ ] **Step 5: Run to verify green** — three suites: the Step 1 route suite, `tests/admin/_metaInfraContract.test.ts`, and `tests/cross-cutting/auth-chain-audit.test.ts`.
...
- [ ] **Step 5: Marker removal is the branch's LAST commit, sequenced AFTER review and CI** — `BL-STEP3-FULL-CREW-PREVIEW`'s `**Status:** IN PROGRESS · **Branch:** feat/admin-ui-surfaces` line comes off (the entry graduates to the archive as SHIPPED in the same commit) only once the whole-diff adversarial review has converged APPROVE and CI is green, immediately before `gh pr merge` (invariant 12's Stage 4.4 sequencing). If any review or CI repair commit lands after this commit, redo this step so the marker-removal commit is again last.

<!-- tasks: end -->

## §12 Closeout

The invariant-8 dual gate runs in Task 9 (both arms on the affected diff, before adversarial diff review); its findings + dispositions land HERE, and Task 9 Step 2a appends the machine-checkable gate marker line in the §3.3 RAN-form grammar with the real counts.

impeccable-gate: critique=RAN-DEGRADED audit=RAN p0=0 p1=2 dispositions=recorded

**Provenance — why critique is RAN-DEGRADED, not RAN.** The critique reference mandates two ISOLATED sub-agents (A design review, B detector evidence) and requires a degraded run to say so rather than claim dual-agent provenance. Both agents were dispatched per that shape and **neither returned any content** in a ~20-minute window; `TaskStop` ended them. This is the measured failure recorded on PR #809 (`feedback_critique_subagents_idle_without_returning`): an in-process sub-agent that ends its turn without messaging back produces nothing the parent can read, and its idle ping is not a result. Both halves were then run inline. The degradation is real rather than cosmetic: running `detect.mjs` in this context means the design review was anchored by detector output, which is exactly the contamination the isolation rule exists to prevent. Everything that did not depend on the agents was drained first (the mechanical checklist, the style meta-tests, the fixes below, the suites).

**Pre-code mechanical checklist** (AGENTS.md cross-cutting list), over `StagedPreviewBanner.tsx`, `page.tsx`, `error.tsx` and the modal footer link: em-dash in user-visible copy — none (comments excluded, since a comment naming the ban is a mention); apostrophes — none in copy; `min-h-tap-min` on every interactive target — 5 of 5 (`TARGET_BASE` covers the picker link, the current-entry chip and the exit; the disclosure `summary` and both failure-surface links carry it directly); canonical type/token classes only — no raw hex, no arbitrary bracket values, no `text-[10px]`/`text-text-faint` pairing. No NEW token is introduced, so no DESIGN.md contrast row is added.

**Deterministic detector.** `detect.mjs --json` over all four files: `[]`, exit 0 (clean). Browser visualization was deliberately skipped: this machine's heavy-phase semaphore was saturated by nine concurrent arcs, and starting a dev server for the overlay would have taken a slot from a queued merge. The real-browser evidence this surface actually needs is Task 8's Playwright spec, which is stronger than an overlay pass.

### Findings and dispositions

| # | Sev | Finding | Disposition |
| --- | --- | --- | --- |
| 1 | P1 | The banner's "Back to setup" link wore `bg-warning-text text-warning-bg`. DESIGN.md §14 reserves that inverted-amber pair as the **confirm-go recipe for DESTRUCTIVE mutations only**; a plain navigation wearing it is the twin-confusion the recipe exists to prevent. Caught fail-by-default by `tests/styles/_metaDestructiveConfirm.test.ts` ("UNREGISTERED DESTRUCTIVE CONFIRM components/admin/StagedPreviewBanner.tsx:109"). | **FIXED.** Re-rendered as the same underlined `text-text-strong` affordance the §2.7 failure surfaces use, so "Back to setup" now reads identically at all four sites in this arc. Contrast on `warning-bg` is 17.4:1 light / 12.0:1 dark (DESIGN.md §1.2), AAA. No registry row is needed, because the recipe is no longer used. |
| 2 | P1 | The step-3 footer link opens a new tab (`target="_blank"`) without announcing it: screen-reader users heard only the destination. Caught fail-by-default by `tests/styles/_metaNewTabAnnouncement.test.ts` (`Step3ReviewModal.tsx:566 external link does not announce that it opens a new tab`). | **FIXED.** Uses the shared `<NewTabHint />` (the ONE definition of the phrase, `components/shared/NewTabHint.tsx`) rather than a hand-written `aria-label`, which also keeps the §6.8 census green. The accessible name is now "Open crew preview (opens in a new tab)"; the suite asserts that exact name AND the sr-only span, so dropping the hint fails. |
| 3 | P2 | The viewing-as picker was an unlabelled run of person-name links: no landmark, no stated purpose. | **FIXED.** Wrapped in `<nav aria-label="Preview as another crew member">`. |
| 4 | P3 | The banner sits inside a `role="status"` live region that contains interactive links. | **No change, deliberate.** It is the `PreviewBanner` recipe the spec mandates verbatim (§2.5), and every picker activation is a FULL navigation, so the region never mutates in place. |

**Audit dimensions.** Accessibility 4 (labelled landmark, `aria-current` on the current entry, AAA contrast, 44px targets, no keyboard trap — the surface adds no focus management because it adds no client state); Performance 4 (Server Components, zero client JS added, no animation, no layout-property work); Responsive 4 (flex-wrap chrome, no fixed dimensions, verified at 390px and 1280px by Task 8's e2e); Theming 4 (tokens only, both themes exercised in the e2e); Anti-patterns 4 (no gradient text, no glassmorphism, no hero-metric block, no identical-card grid, no tracked-uppercase eyebrow). **20/20.**
...
**Registry-count reconciliation (authored AND run):** this plan adds exactly 1 `infraRegistry` row (Task 5 Step 3, the tests-before-helper step), 1 `PROTECTED_ROUTES` row (Task 5 Step 4, red observed first via the auth-chain audit's fail-by-default walk), 1 e2e allowlist `UNSEEN` row (Task 8), 0 rows to every other registry — verified at plan time by `grep -c "path:" tests/admin/_metaInfraContract.test.ts` before/after being +1 in Task 5's Step 3, the auth-chain audit red/green pair recorded in Task 5's commit message, and the allowlist meta's own fail-by-default walk being Task 8's red.
