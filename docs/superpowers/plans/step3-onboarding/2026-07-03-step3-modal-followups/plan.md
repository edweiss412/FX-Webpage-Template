...
**Goal:** Ship the seven user-reported Step-3 review-modal follow-ups per the APPROVE'd spec `docs/superpowers/specs/step3-onboarding/2026-07-03-step3-modal-followups.md` (10 adversarial rounds; the spec is the canonical contract — this plan cites it as §A–§O and copies exact values; where plan and spec disagree, the spec wins). Items: A nav race + sliding indicator; B diagrams section + new admin-only staged-preview route; C footer Unpublish + demoted gate; D report-an-issue rail entry + report-API null-show loosening; E flag callouts + warning jump-links; F rooms notes separation; G rescan result overlay.
