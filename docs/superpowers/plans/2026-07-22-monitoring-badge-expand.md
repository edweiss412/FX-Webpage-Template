...
- [ ] **Step 1.5: NEGATIVE CONTROL (prove-the-probe-can-fail — the post-hoc equivalent of TDD red, plan R3 F2)**: before the real run, temporarily invert ONE load-bearing assertion in each new probe family — stays-open cell asserts `aria-expanded="false"`; treatment probe inverts the REAL predicate — assert `!(property === "none" || duration === "0s")`, i.e. the element HAS an active transition (an untransitioned element computes `property: all` with `duration: 0s`, so a bare `property !== "none"` check would pass vacuously — the negation must be of the full disjunction); node-identity asserts `dataset.pin === undefined` — run the spec, VERIFY each inverted cell FAILS (recording that the harness actually exercises the surface), then restore the correct assertions. A probe whose inversion passes is vacuous — fix the probe before proceeding.
...
**Tests-scope review train — APPROVE at R4:** R1 (6 findings: summary-pin sweep, jsdom vacuities, matrix counts), R2 (4: detachment proof, iterate-validated-arrays, CSS-duration repetition, exact-token classes), R3 (7: removed-SET absence by count, effective-duration, purity predicate, deferred-steal control), R4 APPROVE. All test-adequacy; every finding converged the same class (assertions derive from ground truth — node identity, set counts, effective duration, exhaustive predicates). Negative controls re-run each round (detachment + oracle inversions FAIL before restore).
