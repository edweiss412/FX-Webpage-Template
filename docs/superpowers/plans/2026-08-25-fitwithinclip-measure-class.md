...
| M13 | Return `undefined` from the ref callback instead of the teardown | **MEASURED in Task 1: kills (g) and (g3)**, not (h3). R1 claimed (h13), R4 corrected that to (h3); the run says both predictions were wrong and the case that actually sees it is (g3), the pending-frame cancel, plus (g)'s count. (h3) passes under it. Recorded as measured rather than as predicted — a mutant table that states what it expects instead of what it observed is the defect this arc keeps finding. (h21), authored in Task 3, is expected to add a third — with no cleanup returned, React has nothing to call and `observer.disconnect()` never runs. Plan review R1 disproved the earlier claim against (h13): Strict Mode yields two renders and two applies with or without a returned cleanup, so only a CLEANUP-counting case can see this. `_metaSharedHelperAdoption` also does not catch it — it checks `.cancel()` EXISTS, not that the closure is returned | Task 3 |
...
| M22 | Wire the observer and listeners at hook body level rather than on attach | **NOT EXPRESSIBLE, measured.** Wiring an observer at hook body level needs a node, and on the never-attached path there is none — so every spelling of this mutant degenerates to M16 (`apply()` at body level), which kills (g) and (h) and leaves (h18) passing, because `apply()` returns early on the null node. **(h18) therefore has no killing mutant**, like (h3). It is a pin against a future hook that subscribes before it attaches, and is recorded as such rather than paired with a mutant that does not kill it | Task 3 |
...
3. Add case (h3): after `view.unmount()`, fire a `window` resize and flush frames; assert `applyCount` did not move. **Measured in Task 1: no mutant in this table kills (h3).** M3 does not (§5's ACCEPTED GAP row), and M13 — which R4 predicted would — turns (g) and (g3) red instead while (h3) passes. (h3) is therefore a regression pin whose red condition is a future teardown that stops cancelling, and it is recorded as such rather than paired with a mutant that does not kill it.
...
| AC-1b | Each shipped lifecycle of spec §0.1 has a case in the BARE render, and §0.1a's entrance table is asserted as two snapshots. **NOT every cell, and NOT both modes** — diff review round 3 corrected this: the suite's only `<StrictMode>` case is (h13), covering `ReSyncButton`, so `PublishedToggle`'s Strict counts and both `AttentionMenu` Strict snapshots are unexecuted and recorded as such in spec §3.1a. The bare-render assignment is: `ReSyncButton` by (h15), `PublishedToggle` by (h16), `AttentionMenuPanel` by (h17), with (h17) asserting the attach and post-entrance snapshots separately. (h13) and (h14) pin the replay and the minimal-shape headline; (g) and (h) pin the always-present harness; (h8) and (h9) pin §2.2's two re-render rows. Plan review R3 charged the earlier version for crediting the generic cases with per-consumer rows they do not assert. |
| AC-2 | One attach is one ancestor walk, pinned by (h) with a derived expectation, and mutants M2/M7 turn a named case red. |
| AC-3 | Every pair and compound row that spec §3.1 claims as executed has a case that ACTUALLY PERFORMS it. **FOUR rows do not, and are recorded as unexecuted in spec §3.1a rather than claimed** — diff review round 3 charged the earlier version of this criterion for asserting all of them. The remaining rows each have a case that performs the transition — not one that merely cites a nearby test. Spec review R6 found `N to F` citing a clipped-to-clipped case for six rounds; (h19) closes it and every other row was re-verified against what its cited case does. Mutants M8, M10 and M18 each turn a named one red. **M9 does not, and is an accepted gap** — measured, not assumed: planted, the suite stayed 33/33 green. |
