...
10. **Task 4 lands now, as the PR's LAST commit** (invariant 12; spec §2.5): the graduation, its registry row, the marker strip. **Push it**, because CI can only read a sha that is on origin (plan review round 3). Then a SCOPED closeout review of that one commit alone (`--stage diff`, the next round number, brief scoped to `BACKLOG.md`, `BACKLOG-archive.md`, `tests/docs/_metaDeferralLedgerGraduation.test.ts`; precedent: the resurrect-mobile-safari arc's rounds 8 and 9, `docs/review-rounds/test/resurrect-mobile-safari-e2e/b2aca7b02547.md`), dispatched concurrently with the sha-keyed CI read on that pushed final head. Both must be green: the scoped APPROVE and the twelve checks on the final sha. **A defect in the closeout commit is never repaired by a further commit**, because that would make the marker-strip commit penultimate: the closeout commit is redone as ONE new last commit (`git reset --soft HEAD~1`, fix, commit, `git push --force-with-lease`, the superseded final sha recorded for the readiness report), and the scoped review and the CI read are repeated on the new final head. The marker-strip commit is always the last commit, and nothing merges unreviewed or unread.
11. Readiness report to `bl-orch` naming the FINAL head sha (and any superseded final sha from item 10), the scoped APPROVE, and the twelve checks on that sha; the merge word is the orchestrator's.
