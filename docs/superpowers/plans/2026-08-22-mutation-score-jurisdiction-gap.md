...
| `docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.mts`, `docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.out.txt`          | the committed census (spec §1.0); output below                                                                                                                                      |

Census output, pasted from `pnpm exec tsx docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.mts` at `9102025c5` (byte-identical to the committed `.out.txt`):
...
Sequence: Task 1 (code + every grammar copy, one commit) → Task 2 (harness-spec limit) → Task 4 (graduation + registry row + marker strip, ONE commit, before any review) → closeout §5 (gates, push, the whole-diff review of the full tree, CI on the final head, absent-at-HEAD, readiness). Task 3, the index row, is already landed on the spec branch (see below). Task 2 is docs-only and may be authored before Task 1's review lands, but it commits after it. This is the ruled reading of invariant 12 (Task 4 quotes the ruling): the marker is gone at the graduation commit and therefore at every later commit, the whole-diff review and CI cover the tree that merges, and the corpus rows every review dispatch writes land as corpus-only commits after it without displacing anything.
...
with `import { HEADING_CORPUS } from "./fixtures/guardSurfaceHeadingCorpus";` at the top. **Every rejecting case writes an APPROVE scenario first** (`writeScenario(run, [APPROVE_STEP])`): the fake codex (`tests/codexGuard/fixtures/fake-codex.mjs`) reads its scenario before recording a call and exits 1 without one, so a rejecting case with no scenario would see `readCalls` empty EVEN IF the gate had dispatched, and its zero-call assertion would be vacuous; with the scenario in place a dispatch records one call and the wrapper exits 0, which is exactly the observable RED this task predicts (plan review round 3). The same line is added to the file's ten pre-existing rejecting cases in the same commit (class sweep, same file, same shape, derived at plan time by scanning the file for `it(` blocks that assert `toHaveLength(0)` and never call `writeScenario`: the cases opening at lines 39, 77, 96, 114, 134, 144, 169, 177, 232, 249), so their zero-call halves discriminate too; their `res.code` assertions are unchanged. And the derived cover, so the class cannot return with the next case: `readCalls` in `tests/codexGuard/harness.ts` throws when the run's scenario was never written (the symbol `writeScenario` is what creates it), so a zero-call assertion on a scenario-less run fails loudly instead of passing vacuously. Three spawns for the replay, not 24: the enumeration assertion is the per-line classification (exact set equality on line numbers, so a conforming line listed or a nonconforming line omitted both fail), and the two dispatching briefs are its positive twins, so "accepted" is never proven by absence alone.

Run `pnpm vitest run tests/codexGuard/guardSurfaceGate.test.ts` (the full file, the same command the GREEN criterion names) and record the observed output in the commit message. Expected at RED: exactly the five new rejecting cases fail and every pre-existing case passes (the pre-existing rejecting cases, now with a scenario, still exit 2 on the live gate for the reasons they always did), namely: the plain no-tail case, the heading no-tail case, the empty-tail case, the score-plus-cannot-express case and the non-canonical-plus-cannot-express case fail with `expected 0 to be 2` and, on the recorded call, `expected 1 to be 0` (the live gate dispatches and the scenario records it); the replay case fails on `resAll.code` (the shipped trigger reads none of the 24 lines, so the brief dispatches: the incident in executable form); the with-tail case passes on the live tree, which is the stays-green direction and is expected.
...
**Edit 1.** Append to the §7 table, after the L-10 row (`docs/superpowers/specs/ci/2026-08-04-source-mutation-guard-gate.md:399`), one TABLE ROW in the table's existing three-column shape, `| L-11 | <limit> | <consequence> |`, whose limit and consequence cells are the text spec §2.4 gives for them, verbatim (the spec renders that text as a blockquote for readability; what lands here is the row, so the `| L-11 |` grep of this task's red goes green; plan review round 3). The `**L-11**` bold and the `|` separators inside the blockquote are the row's cell boundaries, not prose to copy.
...
### Task 4: graduation, registry row and marker strip (ONE commit, landed BEFORE the whole-diff review; the ruled reading of invariant 12)
...
**What is red and why.** The grep exits 1 at `9102025c5` (run at plan time); 0 after the move. `pnpm vitest run tests/docs/_metaDeferralLedgerGraduation.test.ts tests/docs/_metaLedgerInProgress.test.ts` is the GREEN criterion beside it (archive-only plus provenance for the registered id; no in-flight archive row). Field preservation is NOT proved by any suite: `_metaLedgerMintBar.test.ts` scans open ledgers only (its `openLedgers` filter excludes the archive ledgers; plan review round 3), so it is proved by a gate authored for this task: a script that reads the row body from the pre-move tree (`git show HEAD~1:BACKLOG.md`, the lines after the meta line through the "Close condition" paragraph), asserts that exact text is a substring of the archive section under the new heading, and asserts the archive section's meta line contains each of `**Filed:**`, `**Severity:**`, `**Class:**`, `**Effort:**`, `**Facing:** process`, `**Class-sweep exception:**`, `**Reachability:**`, `**Incident:**`, exiting 1 on any miss. Proved able to fail before it is trusted: run once against a scratch copy of the archive with `**Incident:**` deleted from the section and observe exit 1.
...
- Meta line: replace `**Status:** IN PROGRESS · **Branch:** docs/mutation-score-jurisdiction-gap` with `**Status:** CLOSED · **Resolution:** re-scoped 2026-08-22 (orchestrator ruling, wave-6 arc brief) to admit a documented-limit close, then closed as documented limit (harness spec §7 L-11) plus the OPERATORS: disclosure arm on the GUARD SURFACE score line, the heading-form trigger, and the bullet-4 admissibility correction · **Shipped by:** docs/mutation-score-jurisdiction-gap`; keep every other field (`Filed`, `Severity`, `Class`, `Effort`, `Facing`, `Class-sweep exception`, `Reachability`, `Incident`) verbatim.
...
This commit lands BEFORE the whole-diff review is dispatched, as ONE commit carrying the whole ledger change. That is the orchestrator's ruled reading of invariant 12 (ruling of 2026-08-18, arc F, recorded in the wave lessons file and quoted here so this plan is self-contained): "The marker comes off in the PR's last commit" is the MECHANISM; "so it never reaches main" is the PROPERTY. They come apart when a post-review repair displaces the ledger commit from last position, and a literal last-commit reading also conflicts with the final-diff-ordering rule, since a commit made after whole-diff review is unreviewed. Ruling: do the whole ledger change, peer rows filed, graduating rows archived, marker removed, as ONE commit BEFORE whole-diff review. Absence is then guaranteed rather than maintained (gone at commit N is gone at every commit after N), and review plus CI cover exactly what merges. Plan review rounds 2 and 4 measured why the literal reading cannot hold on this repo: every `codex-guard review` dispatch appends a row to the tracked corpus file `docs/review-rounds/docs/mutation-score-jurisdiction-gap/50ca72a566b0.jsonl` inside the reviewed worktree, so a review dispatched after the ledger commit necessarily changes the tree (the repo's own precedent: commit `2075eb719`, "the round-3 corpus row, which the wrapper wrote after the last commit"), and a fourth counted diff round adds the sibling filing too. Under the ruled ordering those rows land as corpus-only commits after the graduation, the marker is already gone at every one of them, and absent-at-HEAD is verified immediately before the readiness report (§5). Spec §2.5's "the PR's last commit before merge" wording is amended to this ruled form (a post-approval wording amendment, disclosed in the round-4 brief and the round-economy filing).
...
## 5. Closeout: gates, the freeze diff, the whole-diff review of the full tree, CI on the final head, absent-at-HEAD (not a task marker; every command here is a gate that passes, proved able to fail)
...
4. `pnpm mutation:sites psqlStartupScan` prints zero stale rows (spec §2.6's checkable consequence); `pnpm exec tsx docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.mts` diffed against the committed `.out.txt` is byte-identical, because no enrolled input moved.
5. **The freeze diff (AC-7), run after Task 4 and again on the final head:** `test -z "$(git diff --name-only origin/main...HEAD -- tests/cross-cutting/psqlStartupFiles/scan.ts tests/cross-cutting/psqlStartupFileSuppression.test.ts tests/mutation/source/registry.ts tests/mutation/source/shardPartition.ts tests/mutation/source/operators.ts)"` exits 0. Proved able to fail at plan time by a CONSTRUCTED failing input: the identical command with `docs/superpowers/specs/ci/2026-08-22-mutation-score-jurisdiction-gap.md` appended to the path list prints that path and exits 1 (run 2026-08-22 at `48ad0ad30`: `gate rc=0`, `constructed rc=1`). `git diff --name-only origin/main...HEAD` is exactly the union of the twelve `**Files:**` entries and the eight paths the branch already carries at `7708d8e1b` (`BACKLOG.md`, `docs/superpowers/specs/ci/README.md`, the spec, this plan, `docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.mts`, `docs/superpowers/specs/ci/probes/2026-08-22-jurisdiction-census.out.txt`, and the two corpus files under `docs/review-rounds/docs/mutation-score-jurisdiction-gap/`): two overlap (`BACKLOG.md`, the README), so the union is 18 distinct paths, derived at plan time by `git diff --name-only origin/main...HEAD` plus the `**Files:**` list and re-derived at closeout by the same command (plan review round 2: the earlier "eleven plus the fixture" double-counted the fixture and omitted the branch's own artifacts).
6. Re-run the §1 grep pair; the only new hits are Task 1's files, the fixture module, and this plan.
7. **Round-1 `--stage diff` brief**, dispatched against the head that holds Tasks 1, 2 and 4 (one scope; that diff is the 18-path union of item 5; no split needed): REVIEWER ONLY; consequence bound, PROBE DOMAIN and threat fence copied from spec §3, each on ONE line (the convergence hook greps per line); `GUARD SURFACE: scripts/codex-guard.mjs` with `CANNOT-EXPRESS: spawn-only surface, no Vitest import edge (tests/codexGuard/harness.ts)` on that same line; the do-not-relitigate list from spec §1.1 R1 to R7 plus "presence not membership (§4 L-A)" and "the heading-form widening is bounded by the sweep (§4 L-E)" and "marker precedence: a score-shaped line is never rescued (§2.2 matrix)"; the four pre-dispatch string-presence mutants for the new cases' `toContain("OPERATORS:")` (value emptied; suffix appended; the word present only inside a fence; the `OPERATORS` regex varied) run against the wrapper by editing a SCRATCH brief, never the tree, and recorded in the brief. **Dispatch through the MAIN checkout's wrapper**, `~/.claude/bin/codex-guard` (it execs `$HOME/FX-Webpage-Template/scripts/codex-guard.mjs`), so the file under review is not the file running the review; dry-run the convergence hook first; `run_in_background`; fresh timestamped `--out`. Cap 4; a fourth counted diff round files the round-economy record in the arc's corpus directory.
8. If a review round demands a repair, the repair lands, its corpus row is committed, the SAME whole-diff review is re-dispatched against the new head, and the round count continues; the APPROVE is on the head that merges, and every commit after Task 4 is either a review-demanded repair that is itself re-reviewed or a corpus-only commit (`docs/review-rounds/**`), which is checked: `git diff --name-only <task-4 sha>..HEAD` lists only corpus files and re-reviewed repairs.
9. Push the final head (the APPROVE's corpus row committed); real CI green BY NAME on that sha (the twelve required contexts in both vocabularies, sha-keyed; Vercel FAILURE is the deploy rate limit and is not required; a rollup PENDING with every named check green is suppressed).
10. **Absent-at-HEAD**, immediately before the readiness report: `git grep -c -F 'Branch:** docs/mutation-score-jurisdiction-gap' -- BACKLOG.md BACKLOG-archive.md` exits 1 (no marker anywhere), and `pnpm vitest run tests/docs/_metaLedgerInProgress.test.ts tests/docs/_metaDeferralLedgerGraduation.test.ts` is green on the final head. Proved able to fail at plan time: the same grep on the current tree exits 0 with a count of 1 (the live marker).
11. Readiness report to `bl-orch` naming the final head sha, the whole-diff APPROVE on it, and the twelve checks on that sha; the merge word is the orchestrator's.
...
- **Task 4 lands as one commit before the whole-diff review (the ruled reading of invariant 12, quoted in Task 4).** The marker is gone at that commit and at every commit after it; review and CI cover the tree that merges; after Task 4 only corpus-only commits and re-reviewed repairs land; absent-at-HEAD is verified before readiness (§5 item 10).
