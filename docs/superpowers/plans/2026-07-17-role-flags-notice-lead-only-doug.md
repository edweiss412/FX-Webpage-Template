...
- [ ] **Step 3: Implement (ONE commit — §12.4 lockstep):**
  ...
  - **§4 item 7 — reconcile EVERY stale LEAD-only financials-entitlement claim (GREP-DRIVEN, comprehensive — Codex plan-review, same-vector structural close).** This is pre-existing `2026-07-15-extend-role-scope-vocab` debt (that spec added `FINANCIALS` but did not reconcile the master spec's financials prose); corrected here so the canon is internally consistent with §6.8's LEAD ∪ FINANCIALS capability model (invariant 7). **Do NOT hand-enumerate lines** (per-line lists kept missing hits across review rounds). Instead run the grep and fix EVERY financials-entitlement hit:
    ...
    rg -n "financ|shows_internal|FinancialsTile|financialsEntitled|financialsVisible|Proposal|Invoice|PO#|COI" \
      docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md | rg -i "LEAD" | rg -vi "FINANCIALS"
    ```
