...
- [ ] **Step 1: Write failing tests** in `tests/sync/phase2.test.ts` — add these cases to the `roleFlagsNotice` block (derive expected from fixture flags, anti-tautology per spec §7):
  ...
  - **Flip the EXISTING stale same-file assertions (Codex plan-review — do NOT defer to Task 4).** The narrowing breaks pre-existing `phase2.test.ts` cases that assert a scope-tile-only delta produces `ROLE_FLAGS_NOTICE` (e.g. `:594` "auto-applied non-LEAD role flag changes return deferred ROLE_FLAGS_NOTICE", `:606`). Inspect every `roleFlagsNotice` assertion in `phase2.test.ts`: flip any that uses a PURE scope-tile fixture (A1→V1, +BO) to assert NO notice; KEEP capability fixtures asserting the notice. This lands in THIS task so `phase2.test.ts` is fully green at Task-1 commit (Task 4 covers OTHER files: runScheduledCronSync/applyStaged/emitLeadRoleApplied).
...
- [ ] **Step 4: Run tests, verify pass** — run the **WHOLE file** `pnpm vitest run tests/sync/phase2.test.ts` → PASS (not just `-t roleFlagsNotice` — the whole file must be green after the stale-assertion flips). Then typecheck: `pnpm tsc --noEmit` (or `pnpm typecheck`).
...
- [ ] **Step 4: Run, verify ALL pass** — `pnpm vitest run tests/sync/roleChangeLog.test.ts tests/sync/roleChangeLog.db.test.ts tests/sync/roleChangeLogCoverageParity.test.ts tests/sync/runScheduledCronSync.test.ts tests/sync/applyStaged.test.ts` → PASS (unit writer + DB supersession + DB-path wiring). Run the topology meta-test: `pnpm vitest run tests/sync/_metaLeadRoleAppliedTopology.test.ts` → PASS (unchanged). `pnpm tsc --noEmit`.

- [ ] **Step 5: Commit** — `git add lib/sync/changeLog/writeRoleChangeLogRows.ts lib/sync/phase2.ts lib/sync/changeLog/writeAutoApplyChanges.ts "tests/sync/roleChangeLog.test.ts" "tests/sync/roleChangeLog.db.test.ts" "tests/sync/roleChangeLogCoverageParity.test.ts" tests/sync/runScheduledCronSync.test.ts tests/sync/applyStaged.test.ts && git commit --no-verify -m "feat(sync): identifiable role change-log rows via shared runPhase2 writer; drop MI-9 arm from writeAutoApplyChanges"`
...
- [ ] **Step 1: Residual sweep** — the main same-file flips already landed in their tasks (`phase2.test.ts` in Task 1, `emitLeadRoleApplied.test.ts` in Task 2, `runScheduledCronSync.test.ts`/`applyStaged.test.ts` in Task 3). This task is the SAFETY-NET sweep for any straggler: `rg -n "ROLE_FLAGS_NOTICE" tests/` and inspect each fixture's flags. For every remaining test asserting a PURE scope-tile delta (A1→V1, +BO) emits the notice, flip it to assert no-notice. Verify each flipped fixture holds no `LEAD`/`FINANCIALS` transition (leave capability cases asserting notice). If the sweep finds nothing left to flip (all handled in tasks 1-3), this task is a no-op commit-wise — skip the commit and note it.
...
- Modify (master spec, NEVER prettier): `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` §6.8 MI-9 `:1624`, MI-10 `:1629`, §6.8.2 `:1713`, §12.4 `:2863`, help `:3157`, PLUS the stale LEAD-only financials-gate claims `:23`, `:191-194`, `:220`, `:638`, `:651`, `:1484` (see §4 item 7)
...
- Test: `tests/messages/_metaAlertAudienceContract.test.ts`, `tests/messages/codes.test.ts` (x1), `tests/cross-cutting/codes.test.ts`, resolve-route + BellPanel render tests, AND the health-contract reconciliations `tests/admin/healthAlerts.test.ts`, `tests/components/healthAlertsPanel.test.tsx`, `tests/components/PerShowAlertSection.test.tsx`
...
- [ ] **Step 1: Write failing tests:**
  ...
  - **Reconcile ALL tests encoding the OLD health contract for `ROLE_FLAGS_NOTICE` (Codex plan-review — sweep beyond messages).** First `rg -rln "ROLE_FLAGS_NOTICE" tests/` and inspect every hit for a health-audience assumption; reconcile each so it reflects `audience:"doug"`. Known surfaces (verify + fix each; they will FAIL after the catalog reclassify, TDD-guiding the fix):
    - `tests/admin/healthAlerts.test.ts:274` — the `(b) ROLE_FLAGS_NOTICE health row → identityText…` case uses `ROLE_FLAGS_NOTICE` as a **health** fixture. After reclassify it is NOT health, so it won't load via the health-alerts loader. Reconcile: either swap the fixture to a still-health notice-weight code (e.g. `OAUTH_IDENTITY_CLAIMED`) if the case tests the health loader's generic identity projection, OR move the `ROLE_FLAGS_NOTICE`-specific identity-projection assertion to a doug/bell-surface test (the bell feed still projects its `changes[]` → crew names + count via the same `ALERT_IDENTITY_MAP.ROLE_FLAGS_NOTICE`). Preserve the identity-projection coverage on whichever surface now carries it.
    - `tests/components/healthAlertsPanel.test.tsx:111` — `row({ code:"ROLE_FLAGS_NOTICE" })` as a health-panel fixture → swap to a still-health code (health panel no longer shows `ROLE_FLAGS_NOTICE`).
    - `tests/components/PerShowAlertSection.test.tsx:162,195` — comments/asserts treat `ROLE_FLAGS_NOTICE` as `audience:"health"` (excluded from `PerShowAlertSection` via `HEALTH_CODES`). After reclassify it is `doug` → **included**; flip the assertion to expect it rendered in `PerShowAlertSection`, and fix the `:162` comment. (This is the per-show dismissible surface, spec §2.2.)
    - `tests/messages/adminAlertsRegistry.ts` — `ROLE_FLAGS_NOTICE` STAYS in `ADMIN_ALERTS_CODES` (still an admin-alert code; only its audience changes) — no removal; confirm.
...
- [ ] **Step 3: Implement (ONE commit — §12.4 lockstep):**
  ...
  - **§4 item 7 — reconcile EVERY stale LEAD-only financials-entitlement claim (GREP-DRIVEN, comprehensive — Codex plan-review, same-vector structural close).** This is pre-existing `2026-07-15-extend-role-scope-vocab` debt (that spec added `FINANCIALS` but did not reconcile the master spec's financials prose); corrected here so the canon is internally consistent with §6.8's LEAD ∪ FINANCIALS capability model (invariant 7). **Do NOT hand-enumerate lines** (per-line lists kept missing hits across review rounds). Instead run the grep and fix EVERY financials-entitlement hit:
    ```bash
    rg -n "financ|shows_internal|FinancialsTile|financialsEntitled|financialsVisible|Proposal|Invoice|PO#|COI" \
      docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md | rg -i "LEAD" | rg -v "FINANCIALS"
    ```
    (The final `rg -v "FINANCIALS"` is **case-SENSITIVE by design** — it drops only lines that already name the all-caps `FINANCIALS` role-flag token, i.e. already-reconciled lines. It must NOT be `-vi`, which would wrongly drop the target lines about lowercase `financials` / `FinancialsTile`.)
    Inspect EVERY hit. **The rule:** any claim that ties **reading `shows_internal.financials` / the FinancialsTile / financial fields (PO#/Proposal/Invoice)** to `LEAD` MUST be widened to **`LEAD` OR `FINANCIALS`** (or admin), matching live `getShowForViewer.ts:380` (`financialsEntitled = isAdmin || includes("LEAD") || includes("FINANCIALS")`) + `:712-714` (the `shows_internal` JOIN gated on `financialsEntitled`) + `scopeTiles.ts:141`. Also ADD `"FINANCIALS"` to the `role_flags` permitted-values comment (`~:220`). Known hits at time of writing include `:23, :191-194, :197, :220, :630, :638, :651, :1484, :2369, :2448, :2457, :3704, :3705, :3724, :3782, :3808, :3809` — but the GREP is authoritative; fix whatever it returns.
    **Legitimate exclusions (do NOT touch — confirm each is one of these):** RLS admin-only denial claims (e.g. `:650, :3769`, and any `shows_internal`-is-admin-only-via-RLS statement — TRUE and unchanged; FINANCIALS crew read via the service-role render path, NOT via RLS); `raw_unrecognized` / `parse_warnings` access (`:1415` etc. — admin/LEAD-only; `FINANCIALS` grants ONLY the `financials` column, verified `getShowForViewer.ts:378` "financial-specific" + `financialsEntitled` gating only the financials read); and non-financials `isLead` consumers (LEAD-specific tile/scope logic that FINANCIALS does not affect). When in doubt, a claim is in-scope iff it gates FINANCIAL DATA reading.
...
- [ ] **Step 4: Run, verify pass** — `pnpm vitest run tests/messages tests/cross-cutting/codes.test.ts tests/admin/healthAlerts.test.ts tests/components/healthAlertsPanel.test.tsx tests/components/PerShowAlertSection.test.tsx` + the resolve/render tests → PASS. **Sweep-verify no stale health assumption survives:** `rg -rln "ROLE_FLAGS_NOTICE" tests/` and confirm each hit is either audience-agnostic or now treats it as `doug`. Then the **grep-verification** (§4) — check BOTH orderings AND the dept-swap example rows on the canonical surfaces (`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` + `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/00-overview.md`):
    ```bash
    rg -n "non-LEAD.*ROLE_FLAGS_NOTICE|ROLE_FLAGS_NOTICE.*non-LEAD|AUTO-?APPLIES.*ROLE_FLAGS_NOTICE|ROLE_FLAGS_NOTICE.*(department|dept|scope|A1 → V1|→ V1|BO)|(department|dept|scope|A1 → V1|→ V1|additive).*ROLE_FLAGS_NOTICE" \
    ...
    ```
    Manually inspect each hit: ZERO may assert that a scope-tile/department/non-capability change emits `ROLE_FLAGS_NOTICE`. Every §6.8/§6.8.2/§12.4/help dept-swap example must read "change-log row, no bell alert" (a mention of "department" NEAR ROLE_FLAGS_NOTICE is only OK if it explicitly says the dept change gets a change-log row, NOT the alert). Also grep the **financials-gate** staleness (§4 item 7) with a BROADER pattern that catches `shows_internal`-join prose too: `rg -n "financ|shows_internal|financialsEntitled|FinancialsTile" docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md | rg -i "LEAD" | rg -v "FINANCIALS"` → **manually inspect each hit**: every hit that ties **financials / `shows_internal.financials` access** to LEAD must now name LEAD **∪ FINANCIALS** (or admin). ZERO surviving "LEAD-only ⇒ financials/shows_internal-join" claims. (Legitimate exclusions that may remain: `:650`/`:3769` RLS admin-only denial; `:1415` `raw_unrecognized` admin/LEAD-only — those are NOT financials-entitlement claims. Confirm each surviving hit is one of these, not a stale financials gate.) (Historical docs 06-drive-sync.md / handoffs / doug-validation-questions.md / html-plans are NOT edited — §4 classifies them historical.)

- [ ] **Step 5: Commit** — stage ALL of this task's changes including the Step-1 test files (resolve-route, BellPanel render, legacy-copy). Use `git add -A` OR the explicit list: `git add lib/messages/catalog.ts lib/messages/__generated__/spec-codes.ts tests/messages/ tests/admin/ tests/components/ tests/api/ tests/cross-cutting/ docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md docs/superpowers/specs/alerts/2026-07-04-alert-audience-split.md docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/00-overview.md docs/superpowers/plans/2026-07-17-role-flags-notice-lead-only-doug.md` (whichever dirs the Step-1 tests actually live in — `git status` first to confirm nothing is left unstaged), then `git commit --no-verify -m "feat(admin): reclassify ROLE_FLAGS_NOTICE audience health→doug + capability master-spec reconciliation (§12.4 lockstep)"`. Verify `git status` is clean after the commit.
