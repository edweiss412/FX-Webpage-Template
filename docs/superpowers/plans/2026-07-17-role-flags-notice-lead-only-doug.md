...
- [ ] **Step 3: Implement** in `lib/log/emitLeadRoleApplied.ts`:
  ...
  - **Invariant 9 — convert the touched call site to `{ ok, error }` destructuring (Codex plan-review P0).** The live code (`:41-63`) uses `const result = await persistAppEventStrict(...)` then `result.ok`/`result.error`. Since this task edits that call, make it invariant-9-compliant: `const { ok, error } = await persistAppEventStrict({...});` then `if (!ok) { await log.error(..., { ..., error: serializeError(error) }); }`. Keep the loud failure escalation. (This is the ONLY behavioral change to the failure path — the escalation itself is unchanged.)
...
- Modify (master spec, NEVER prettier): `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` §6.8 MI-9 `:1624`, MI-10 `:1629`, §6.8.2 `:1713`, §12.4 `:2863`, help `:3157` (the ROLE_FLAGS_NOTICE audit-trigger surfaces only). **Broad master-spec financials-gate reconciliation is OUT of scope** — pre-existing 2026-07-15-extend-role-scope-vocab debt, filed as `BL-MASTERSPEC-FINANCIALS-VOCAB` (Task 6). The only capability-model touch here is the §6.8 MI-9 "LEAD is the only capability element" sentence (§4 item 1), corrected because that row IS the audit trigger I'm editing.
...
- [ ] **Step 3: Implement (ONE commit — §12.4 lockstep):**
  ...
  - **Broad master-spec financials-gate reconciliation: OUT of scope** (owner decision 2026-07-17 — pre-existing 2026-07-15-extend-role-scope-vocab debt, filed as `BL-MASTERSPEC-FINANCIALS-VOCAB`, Task 6). Do NOT sweep the whole master spec's financials prose here. The §6.8 MI-9 "LEAD is the only capability element" sentence IS corrected (§4 item 1) because that row is the audit trigger being edited — but do not chase the ~15 other financials-access mentions in this change.
...
- [ ] **Step 4: Run, verify pass** — `pnpm vitest run tests/messages tests/cross-cutting/codes.test.ts tests/admin/healthAlerts.test.ts tests/components/healthAlertsPanel.test.tsx tests/components/PerShowAlertSection.test.tsx` + the resolve/render tests → PASS. **Sweep-verify no stale health assumption survives:** `rg -rln "ROLE_FLAGS_NOTICE" tests/` and confirm each hit is either audience-agnostic or now treats it as `doug`. Then the **grep-verification** (§4) — check BOTH orderings AND the dept-swap example rows on the canonical surfaces (`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` + `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/00-overview.md`):
    ...
    ```
    Manually inspect each hit: ZERO may assert that a scope-tile/department/non-capability change emits `ROLE_FLAGS_NOTICE`. Every §6.8/§6.8.2/§12.4/help dept-swap example must read "change-log row, no bell alert" (a mention of "department" NEAR ROLE_FLAGS_NOTICE is only OK if it explicitly says the dept change gets a change-log row, NOT the alert). (The broad financials-gate reconciliation is OUT of scope — `BL-MASTERSPEC-FINANCIALS-VOCAB`, Task 6.) (Historical docs 06-drive-sync.md / handoffs / doug-validation-questions.md / html-plans are NOT edited — §4 classifies them historical.)
...
**Files:**
**NON-TDD close-out task (Codex plan-review P0).** This task is explicitly a **docs + whole-diff verification** step — it adds `BACKLOG.md` rows (documentation, no code-under-test) and runs the full suite as a gate. There is no red/green cycle because there is no product code to exercise; TDD invariant 1 governs implementation tasks (1-5), and this is a documentation/verification close-out. (If a subagent framework requires a "test" per task, the "test" here is the full-suite gate in Step 2 that MUST be green before the commit.)

**Files:** Modify `BACKLOG.md`

- [ ] **Step 1** — add three `BACKLOG.md` rows: (a) `BL-BELLPANEL-ROWTONE-NOTICE-WEIGHT` — the `rowTone` `isHealth → critical` short-circuit (`BellPanel.tsx:129`) still renders OTHER notice-weight health codes red; fix to `DEGRADED_HEALTH_CODES.includes(code) ? "critical" : "notice"` (spec §5, deferred to keep this change non-UI); (b) `BL-STAGED-IDENTITYLINK-RENAME-IDENTITY` — the dashboard staged path applies identity-link renames as remove+add (R33-2); if identity-preservation on that path is ever wanted, thread `identityLinkRenames` (out of scope; capability AUDIT is already complete via arms b/c); (c) `BL-MASTERSPEC-FINANCIALS-VOCAB` — the master spec `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` has ~15 stale "LEAD-only ⇒ financials/`shows_internal`/FinancialsTile" prose claims (pre-existing `2026-07-15-extend-role-scope-vocab` debt; that spec added the `FINANCIALS` flag but didn't reconcile the financials-access prose). Reconcile them all to `LEAD ∪ FINANCIALS` (or admin) per live `getShowForViewer.ts:380`/`scopeTiles.ts:141`. Grep seed: `rg -n "financ|shows_internal|FinancialsTile|financialsEntitled|Proposal|Invoice|PO#" <masterspec> | rg -i "LEAD" | rg -v "FINANCIALS"` (case-sensitive exclude); exclude RLS-admin-only and `raw_unrecognized` lines.
...
- [ ] **Step 3: Commit** — `git add BACKLOG.md && git commit --no-verify -m "docs(backlog): file BL-BELLPANEL-ROWTONE-NOTICE-WEIGHT + BL-STAGED-IDENTITYLINK-RENAME-IDENTITY + BL-MASTERSPEC-FINANCIALS-VOCAB"`
