...
| W8 | category bar accepts any non-blank reason | the twelve AC-4 refusals | §1.5 |
...
| W24 | projection keyed on every paint-prefixed spelling, compiled or not (a typo beside a live recipe reds as unregistered plus stale, spec r6) | AC-13 co-location: `border-borde`, `bg-bogus`, `border-border-strong!!` appended to PublishedToggle's OFF alternative keep the key; `bg-surface` and `border-2` appended change it | §3.3 |
...
- [ ] **1.7** Paired fixtures (spec §5.4, §5.5, W3, W4, W10, W11): a local `scanFixture(source)` harness (the same six-line shape as `_metaControlOutlineFill.test.ts`, replicated on purpose); every fixture that expects an element NOT to be residue also contains an element that IS residue, one variable away, with `premise("fixture produced both elements", found.length, 1)` inside the case for every fixture whose two elements are both in scope (the tag fixture below states its own); fixtures for: `border-borde` (one character short of the stem) beside `border-border-strong/50` (spec §5.5(b), both edges of the prefix); token in a comment beside a live one; token in a `data-x` attribute beside a `className`; a `<div>` beside a `<button>`, both carrying `border border-border-strong` (this fixture's premise is on its OWN source, `premise("fixture source carries both tags", (source.match(/<(div|button) /g) ?? []).length, 1)`, and its claim is `found.length === 1` with `found[0].tag === "button"` by equality: the scanner admits the button and omits the div by design, `isInScope` at `tests/styles/interactiveScanCore.ts:865`, so the two-element premise of the other fixtures does not apply here; probed at plan time: one element, `button`); a `false && "border-border-strong"` branch (residue, conservative direction, asserted); `sm:border-border-strong` beside bare; an unresolvable `className={cls}` whose readable string carries the token (residue); two identical weak buttons in one file (multiset count 2).
- [ ] **1.7a** Grammar pin (AC-13, W14): import `normalizeToken` from `./_childlessGrowableScan` IN THE SUITE ONLY (never in the module) and assert `utilityOf(t) === normalizeToken(t)` for every whitespace-split token of every live residue element; then thirty-two scratch roots, one per form of spec §3.2's two tables, every root through ONE local helper `scratchRoot(file, anchor, form)` that copies the corpus, replaces the anchor exactly once (`occurrences === 1`, else throw), scans, finds the target element, and asserts the CASE-OWN premise `premiseHolds("the form reached the element's readable strings", allStrings(el).some((s) => s.split(/\s+/).includes(form)))` before returning `{ el, paint }` (the scanner reads class strings whether or not they compile, so non-compiling forms satisfy it too); every negative case therefore proves its own input landed and cannot pass by an edit that did not (spec §5 self-proof 4); the same helper serves the co-location, key-membership and selector roots with `PublishedToggle.tsx` and the ON/OFF anchors; then five roots for the co-location case (AC-13's fourth `it`: three non-compiling paint-prefixed tokens keep PublishedToggle's key by equality, `bg-surface` AND `border-2`, the two compiling tokens of spec §3.3's first table, change it), then eleven roots for §3.3's key-membership probe, ONE PER TOKEN so no token masks another (a fifth `it`, and a sixth with ten roots for the selector rule, W26: `![border-color:…]`, `[border-color:…]`, `[background:red]`, `[border-top-color:#cfcdc7]`, `[border:1px_solid_var(--color-border)]`, `sr-only` change the key; `rounded-full`, `divide-border`, `ring-border`, `outline-border`, `shadow-sm` do not; by equality on `keyChanged`) (twenty-eight residue forms expected residue, the four controls `border-borde`, `border-text-faint/50`, `border-transparent`, `border-warning-text/60` expected NOT residue, each asserted by equality on `isResidue`), plus a third `it` asserting `classifyValue` on each table's stated `value=` string equals its stated `class=` by equality. Every one of the thirty-two roots substitutes one form for `border-text-faint` in `components/admin/UnignoreButton.tsx` (anchor exactly once) and asserts `isResidue(el, paint)` BY EQUALITY against the table's stated value: `true` for the twenty-eight residue forms, `false` for the four controls (`border-borde`, `border-text-faint/50`, `border-transparent`, `border-warning-text/60`); for the residue forms that compile (every one but `border-borde`) it also asserts the raw form appears in the element's projection.
- [ ] **1.7c** Planted defect, different producer (AC-16, W16, W20): write `css.replace(/^\s*--color-border-strong:.*$/m, "")` to a file under `mkdtemp` (the oracle loads from a path; `premise("the declaration was removed", css.length - broken.length, 1)` guards against a silent no-op replace), `const blind = await loadOracle(brokenPath)`; assert `classify(blind, ["border-border-strong"]).get("border-border-strong")` is `null`; assert the residue of `ROOT` under `blind`, as sorted `file:line` strings, equals the seven `border-border` elements of §1.4.3's population table BY EQUALITY (the premise that the live scan produced 362 elements, 1.2, already rules out the absent-scan "seven"); and assert that `validateCensus(RESIDUE_CENSUS, residueUnderBlind, blind, css, ledger)` names exactly the five `border-border-strong` rows stale. Pair it in 1.12 with `_metaControlOutlineFill` in the same invocation so the old path is observed green while this case exercises the defect.
...
- [ ] **1.15** Observed-red for the floor cases: neutralise the RESIDUE DECISION, `weakSides`, to `return [];` (a condition, never an excision; `classify` and `isWeakValue` stay intact, so AC-15's module-scope premise on the canonical token's sides still holds and collection proceeds; plan round 4 showed that neutralising `isWeakValue` trips that premise and aborts collection before any floor case runs) (a condition, never an excision, so the module stays collectable), run the suite, observe the TEN inequality floor cases, the four cascade cases, AC-15 and the census-equality case red for the asserted reason (value assertions: `expected true to be false` on inequality, twelve stale rows on equality, `expected [] to equal [...]` on the cascade winners) while the two equality CONTROLS stay green (an empty residue equals an empty residue; they are controls for the key, not instruments for this mutant, and their staying green is recorded too), `git checkout -- tests/styles/controlOutlineResidue.ts`, re-run green. Record the observed failure lines in the commit message of the NEXT commit (Task 2), not by amending.
...
- [ ] **2.2** Add `controlOutlineResidue: {},` to `tests/mutation/source/expectedLedgerKinds.ts` beside `controlOutlineScan: {}`. Run `VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm exec vitest run --project mutation tests/mutation/guardSurfaces.gates.test.ts` (the gates suite only collects under the mutation project, the root vitest config, line 137): green. (`_metaSourceShardIntegrity` pins the shard FILE set; the registry growth repacks the partition and is expected to pass.)
...
git rev-parse --verify -q origin/main^{commit} >/dev/null || { echo FAIL base-ref-unresolvable; exit 1; }   # every git gate below fails CLOSED: an unresolvable ref makes command substitution empty and test -z would pass (plan round 4 probe)
files=$(git diff --name-only origin/main...HEAD -- app components DESIGN.md tailwind.config.ts) || { echo FAIL git-diff; exit 1; }
test -z "$files" || { echo FAIL ui-surface-touched; exit 1; }
# the shipped pins are byte-identical
pins=$(git diff --name-only origin/main...HEAD -- tests/styles/controlOutlineScan.ts tests/styles/_metaControlOutlineFill.test.ts tests/styles/interactiveScanCore.ts) || { echo FAIL git-diff; exit 1; }
test -z "$pins" || { echo FAIL shipped-pin-edited; exit 1; }
# no scratch shard on any commit
shas=$(git log --format=%H origin/main..HEAD) || { echo FAIL git-log; exit 1; }
shard=$(printf '%s\n' "$shas" | xargs -I{} git ls-tree -r --name-only {} | grep shardTMP || true)
test -z "$shard" || { echo FAIL scratch-shard-committed; exit 1; }
...
pnpm exec vitest run tests/styles tests/mutation/_metaSourceShardIntegrity.test.ts tests/docs
VITEST_INCLUDE_MUTATION_HARNESS=1 pnpm exec vitest run --project mutation tests/mutation/guardSurfaces.gates.test.ts   # the gates suite is a MUTATION_TEST_GLOBS member excluded from the default projects (vitest.projects.ts:91, :100; vitest.config.ts:137); named bare it collects nothing and reports green
echo PASS closeout
```
...
| AC-13 normaliser pin; twenty-eight residue forms, four controls not; value classes by equality; typo co-location keeps the key; §3.3's key-membership and selector probes | 1.7a | the five `it`s of spec AC-13 plus a sixth for W25 (the eleven key-membership roots, not an AC case); thirty-two plus five plus eleven plus ten scratch roots |
...
| AC-17 Tailwind major pinned; census names its version; upgrade reds the census | 1.2, 1.11 | `premise` plus `expect(major).toBe(4)`; the header comment above `RESIDUE_CENSUS` |
