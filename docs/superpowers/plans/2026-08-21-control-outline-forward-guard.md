...
# W17: no spelling predicate of any kind in the module (grep -q exits 1 on no match; negated, so a match fails the gate)
! grep -qE 'startsWith\("border|=== "border-border|includes\("-border' tests/styles/controlOutlineResidue.ts || { echo FAIL spelling-predicate; exit 1; }
# W22: no hex literal in the module (the value classes compare nothing)
! grep -qE '#[0-9a-fA-F]{6}' tests/styles/controlOutlineResidue.ts || { echo FAIL hex-literal; exit 1; }
# W26: exactly one ownDeclarations: its definition and its two call sites (classify, paintProjection)
test "$(grep -c 'ownDeclarations(' tests/styles/controlOutlineResidue.ts)" = 3 || { echo FAIL own-declarations-sites; exit 1; }
...
At plan time the first two lines PASS (the diff touches docs only), the three module gates FAIL because the module does not exist yet (grep on a missing file exits 2, which the negation turns into a gate failure, and `grep -c` prints nothing, which is not 3), the marker line FAILS for the asserted reason (`**Status:** IN PROGRESS` is present until Task 3), and the suite line passes. The three module gates were probed against CONSTRUCTED inputs at plan time (a file carrying a stem predicate and a hex literal; a file with three `ownDeclarations(` occurrences):

```
=== gate probes (constructed inputs)
spelling gate: exit 1 on the constructed spelling predicate (correct)
hex gate: exit 1 on the constructed hex literal (correct)
ownDeclarations gate: PASS on 3 occurrences (correct)
ownDeclarations gate: exit 1 on 0 occurrences (correct)
```
