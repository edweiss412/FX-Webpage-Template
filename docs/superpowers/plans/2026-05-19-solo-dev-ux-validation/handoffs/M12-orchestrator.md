...
- **JWT_SIGNING_SECRET orphan (filed 2026-05-26 amendment implementer):** the M12 amendment implementer's live-code citation pass surfaced that `.env.local.example:12` still carries `JWT_SIGNING_SECRET=` (empty placeholder) post-M11.5 G3 cutover, but the only production code reference is one M11-era playwright test fixture (`tests/help/playwright-config.test.ts:82`). The implementer did not touch `.env.local.example` (out of amendment scope — spec/plan-markdown only per dispatch §0). Orchestrator decision needed: (a) file as a BACKLOG.md item ("remove JWT_SIGNING_SECRET from .env.local.example + retire the playwright-config.test.ts:82 reference"); OR (b) fold into M12 close-out tidying (alongside the test-migration coda decision). Recommend (b) — small mechanical cleanup that pairs naturally with close-out's other "M11.5 supersession cleanup that M11.5 deliberately didn't do" decisions. Decision NOT taken at amendment time; orchestrator triages at next available turn.
