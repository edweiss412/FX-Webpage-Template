...
| [08-auth-integration.md](08-auth-integration.md) | **H** | Anchor resolver test (test #1); auth-gating + AdminInfraError mapping test (test #3); MDX smoke test (test #4); mobile-layout Playwright (test #6); no-placeholder lint (test #7); live-catalog biconditional assertion (test #2's H.6 deferral from B.4) | 6 (H.1–H.6) |
...
**Total:** 63 tasks across 9 phase files (A=7 + B=5 + C=4 + D=7 + E=13 + F=11 + G=7 + H=6 + I=3). Sub-skill: [`superpowers:subagent-driven-development`](../../../../.claude/skills/superpowers/subagent-driven-development) (recommended) or [`superpowers:executing-plans`](../../../../.claude/skills/superpowers/executing-plans).
