...
| [07-affordance-retrofit.md](07-affordance-retrofit.md) | **G** | `affordanceMatrix.ts` registry; render-side gate with preview-as-crew exception; `Learn more →` link wiring via `messageFor().helpHref`; deep-link affordance walker (test #13, three row-class split); error-renderer gate (test #12); retrofit `data-testid` to M3/M9/M10 source surfaces | 6 (G.1–G.6) |
| [08-auth-integration.md](08-auth-integration.md) | **H** | Anchor resolver test (test #1, deferred from Phase A per dependency on Phase B + E); auth-gating + AdminInfraError mapping test (test #3); MDX smoke test (test #4); mobile-layout Playwright (test #6); no-placeholder lint (test #7) | 5 (H.1–H.5) |
| [09-close-out.md](09-close-out.md) | **I** | `/impeccable critique` + `/impeccable audit` per page; cross-model adversarial review of plan execution; M12 handoff doc update | 3 (I.1–I.3) |
...
**Total:** 61 tasks across 9 phase files (A=7 + B=5 + C=4 + D=7 + E=13 + F=11 + G=6 + H=5 + I=3). Sub-skill: [`superpowers:subagent-driven-development`](../../../../.claude/skills/superpowers/subagent-driven-development) (recommended) or [`superpowers:executing-plans`](../../../../.claude/skills/superpowers/executing-plans).
