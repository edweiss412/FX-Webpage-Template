...
| **D — MDX components** (Tasks D.1–D.7) | `<Callout>`, `<Step>`, `<ScreenshotPlaceholder>`, `<Screenshot>`, `<RefAnchor>`, `<TipFromSheets>` + `mdx-components.tsx` registration | **Opus / Claude Code** | Codex | All UI components. Heavy `impeccable` workflow territory (component visual design + a11y). All-Opus per UI hard rule. |
