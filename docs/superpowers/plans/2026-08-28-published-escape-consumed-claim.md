...
**Mutation-family closure:** this plan ships ONE structural guard and states its families, because an earlier draft declared N/A and whole-diff review round 2 was right that the declaration was false.

The guard is the consumed-key decision, extracted to `lib/admin/escapeClaim.ts` as a pure function precisely so it can be exercised rather than described. Its families are the ones that change an OUTCOME a user would notice: a state misclassified (the panel's Escape treated as a defer, or the reverse), a decision that consumes without acting, and an inverted `consumesKey`. All three were run against the shipped guard and all three die.

It is deliberately NOT enrolled in `tests/mutation/source/registry.ts`. The registry's operators are relational, equality, connective, literal, quantifier and statement-removal mutations over a scored surface; this is a four-state decision whose whole input space the suite already enumerates, so a score would range over the same four cases the tests do and report a number rather than a fact. Enrolling it would be the symbolic enrolment the source-mutation spec warns against. The suite it adds is a behavioral contract over a React component, which the registry cannot express (the step3-a11y precedent, `docs/superpowers/specs/2026-08-09-quick-wins-2-mech.md` §1.1.4).
