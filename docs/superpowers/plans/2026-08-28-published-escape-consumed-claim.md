...
- [ ] Add the pre-listener case, spec §6.2 case 11: deliver Escape while the panel is mounted with its claim acquired and its passive listener not yet installed, assert the PANEL is dismissed and the modal stays, then that a second Escape closes the modal. Stage it by rendering OUTSIDE `act` and `flushSync` and advancing to a macrotask boundary, which spec §6.2 records as probed: a `flushSync` mount runs both effect phases before returning on React 19.2.4, so that route leaves the frame's listener live and the case would pass against unrepaired code.
