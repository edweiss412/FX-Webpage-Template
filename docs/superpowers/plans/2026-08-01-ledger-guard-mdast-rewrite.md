...
RED: rewrite the two terminal-status `it()` bodies + `shoutyIds` call sites in `tests/docs/_metaDeferralLedgerGraduation.test.ts` to consume `_ledgerMdast.ts` (eight `it()` names unchanged) — the rewritten bodies are RED against the not-yet-wired evaluator (import/wiring absent). GREEN: wire `entryTerminal`. Live ledgers must stay green (spec §5 live-clean criterion). Regex lanes, `normalizeSection`, `WRAP`, `AFTER`, `CONTAINER` deleted. NO plants land here (r6 P0 — single fixture owner is T3).
