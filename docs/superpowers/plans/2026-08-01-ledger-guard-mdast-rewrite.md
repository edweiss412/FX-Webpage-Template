...
Port the ~230-line r15–r40 plants verbatim from `a1cfce98d` (annotations kept), re-targeted at walker verdicts; add ALL spec §5 plants (P1–P7, fenced-code, table/link/html silents, setext, 3-space heading, entity, autolink, lazy-continuation, `Resolution`, `Not CLOSED`, ✅ pair, the FULL r2–r10 review-round shape set — including M9's six formatted-id rejections, five formatted-prefix mints, plain/struck controls, and the DEFERRED levels-mask plant (plan-r1 F2: M9's permanent pins are owned HERE, since the migration census dies with the legacy regex) — M2's per-word hit plants, M7 quoted-heading plant). Each plant states its mutation family (M1–M9) in a comment. RED edge (fixture-port task, spec §1.1.14): per lane family, run the lane-deletion mutant (comment out the wiring) — family reds; restore — family greens. Both runs recorded in the commit body.
...
TDD RED edge (plan-r1 F1): add the `BACKLOG_GRADUATED` registry row FIRST, with the entry still sitting in the active queue — the guard's own "every graduated id is archive-only" assertion reds (id present in `BACKLOG.md`, absent from the archive). GREEN: move the entry to `BACKLOG-archive.md` with provenance `test/ledger-guard-mdast-rewrite` and layer the reconciliation note. The new walker validates its own graduation row — the guard polices this commit.

Reference sweep RUN 2026-08-01, exact command `rg -n "BL-LEDGER-GUARD-MDAST-REWRITE" --no-heading | cut -d: -f1,2` — EIGHT hits, per-hit dispositions (plan-r1 F3):
- root `BACKLOG.md` line 7 — reconciliation-note prose recording the 2026-07-31 split: KEEP (historical record; the graduation layers its own note per house style).
- root `BACKLOG.md` line 11 — the entry heading: MOVES to archive (the RED/GREEN above).
- design spec lines 5 and 164, this plan (title/charter lines and this task), and the probe sibling `2026-08-01-ledger-guard-mdast-rewrite-probes.mjs` line 5 — this arc's own artifacts naming their charter: KEEP ALL (provenance citations; graduation never purges historical references, and the archived entry itself carries the id).
`BL-SOUND-REDIRECT-GUARD` does NOT cite the id (an earlier draft assumed it did — refuted by this sweep); the r30 header citation exists only in the `a1cfce98d` snapshot file, which this branch's rewrite replaces.
