...
- [ ] **Step 1: Read the current registry.** Open `tests/log/_metaAdminOutcomeContract.test.ts`; copy the `AUDITABLE_MUTATIONS` array, `SANCTIONED_CODES`, `NEW_FORENSIC_CODES` verbatim.

- [ ] **Step 2: Create `tests/log/_auditableMutations.ts`** with the copied constants, adding `fn` to every row (`fn: "POST"` for every existing route row; the exported action name for action rows — e.g. `app/admin/show/[slug]/_actions/archive.ts` → `fn: "archiveShowAction"`, `setPublished.ts` → `fn: "setShowPublishedAction"`, `feed.ts` → three rows `mi11ApproveAction`/`mi11RejectAction`/`undoChangeAction`, `unarchive.ts` → `unarchiveShowAction`). For route files the fn is always `"POST"`.
...
- [ ] **Step 1: Failing tests** — add to `enumerate.test.ts`: a `"use server"` module with 2 exported async fns yields 2 module-action units (fn names correct); an `export { mutate }` list export is collected; a route file with `export async function POST` yields one route unit (`fn:"POST"`); a route with `POST`+`DELETE` → `routeMutatingMethods` length 2; a `"use server"` module with `export default` → `moduleDefaultExports` true; admin classification: a module action calling `requireAdmin` → `admin:true`, a route under `app/api/admin/**` → `admin:true`, `app/api/report/route.ts`-style path → `admin:false`.
...
- `manifest/…/ignore` route → `MANIFEST_SHEET_IGNORED` on the committed manifest-transition branch (NOT CAS-miss rollback); actor from its `requireAdminIdentity()`.
- `reap-stale-sessions` route → `STALE_SESSIONS_REAPED`, `result: "reaped_" + count`, on the successful reap; actor from `requireDeveloperIdentity()`.
Emit in the route file, post-commit, before the JSON response. Commit: `feat(admin): observe manifest-ignore + stale-session reap`.
