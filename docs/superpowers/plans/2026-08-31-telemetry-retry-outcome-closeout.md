...
- Whole-diff review round 1 returned three findings, all repaired: the intent copy was only ever asserted against its own constant, the three canonical documents still bound the deleted parity design in seven places, and this closeout claimed a misattribution fix the code does not implement.
