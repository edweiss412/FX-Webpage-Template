...
- EXTENDS `tests/parser/ambiguityCodes.test.ts` — five→seven, both codes in the sorted enumeration (T4 row p).
...
- NEW oracle: `INTERNAL_CODE_ENUMS` membership for both codes, source `parse_warnings.code` (T4 row j).
...
- ambiguityCodes sweep: `grep -n "exactly the" tests/parser/ambiguityCodes.test.ts` — hit :16 ("exactly the five ratified members") — edited by T4 row p.
- Card-copy doc sweep (banked 2026-07-30, plan R1 f8): `grep -n "\b42\b\|\b29\b" docs/superpowers/specs/2026-07-20-warning-card-copy-restore.md` — hits :44, :53, :58 (×2), :60, :64, :103, :148, :161, :165, :207 (×2). Edited by T4 row v: :44, :53, :60 (29→31 + names), :64, :103, :161 region (+rows 43/44), :165 (+provenance note), :207 ("× 42 in the current universe"→44). NON-edits with dispositions: :58 "42 excluded codes" (a measured exclusion-set size, not the registry count), :148 "| 29 |" (a §4.2 table row number), :207 "(13 in the current universe)" (sync-code count, unchanged by this feature).
