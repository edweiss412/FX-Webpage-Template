...
- **`tests/log/_metaMutationSurfaceObservability.test.ts` — NO change, verified not assumed.** `evaluateUnit` checks for a coded emit FIRST and consults `noTelemetryExempt` only if none is found (`tests/log/_metaMutationSurfaceObservability.test.ts:178-179`), and the walk carries no unused-exemption rule, so a `no-telemetry:`-exempt wrapper that gains a coded `log.warn` still passes — it simply passes on the `coded emit` branch instead of the `no-telemetry` one. The conclusion is unchanged; the mechanism is the reverse of what an earlier draft of this plan asserted. No `AUDITABLE_MUTATIONS`, `ADMIN_SURFACE_EXEMPTIONS`, or `KNOWN_UNINSTRUMENTED` row changes: the gate adds a BRANCH to existing surfaces, not a surface. The origin walk READS `ADMIN_SURFACE_EXEMPTIONS` (Task 2) but never writes it.
...
- **Invariant-10 non-interference** — read of `evaluateUnit`'s ordering at `tests/log/_metaMutationSurfaceObservability.test.ts:178-179` (coded emit first, `noTelemetryExempt` second), plus `grep -n "both\|redundant\|conflict\|unused exemption\|unnecessary"` over that file, which returns no unused-exemption rule. Either branch passes, so no registry row changes.
...
<!-- task: red=`pnpm vitest run tests/mutation/_metaPremiseContract.test.ts` red-state=authored red-target=`tests/mutation/_metaPremiseContract.test.ts:30` why=`EXPECTED_ENV_TOUCHING has no key for tests/auth/sameOriginServerAction.ts, and the contract asserts exact equality between its keys and the enrolled suite paths, so adding the registry row reds it` ac=AC-10 -->

**What is red and why:** enrolment is not a free registry row. `tests/mutation/_metaPremiseContract.test.ts:141` asserts `Object.keys(EXPECTED_ENV_TOUCHING).sort()` equals the enrolled suite paths exactly, so the moment `tests/auth/sameOriginServerAction.test.ts` becomes an enrolled suite the contract fails for want of a key at `tests/mutation/_metaPremiseContract.test.ts:30`. The count assertion at `tests/mutation/_metaPremiseContract.test.ts:154` then requires the declared number to match what `classifyTests` finds, and the suite has environment-touching cases carrying neither a premise nor an exemption. Both halves land in this one command.
...
- [ ] **Step 1 (RED).** Add the `GuardSurface` row to `tests/mutation/source/registry.ts`: `id: "sameOriginServerAction"`, `sourcePath: "lib/auth/sameOriginServerAction.ts"`, `suitePaths: ["tests/auth/sameOriginServerAction.test.ts"]`, `operators: [...OPERATOR_NAMES]`, `scoreFloor: 0.95` (the value every shipped row uses), `control: { from: 'secFetchSite === "same-origin"', to: 'secFetchSite !== "same-origin"' }`, `accepted: []`.
  ...
  **Registry reconciliation, run at plan time and pasted rather than described** (`docs/agents/writing-plans.md:27`): `grep -c '^    id: "' tests/mutation/source/registry.ts` reports **16** rows on the live tree; this task's single addition takes it to **17**, and no row is removed or renamed. The task's own check is that same command reporting 17 after step 1.

- [ ] **Step 2 (GREEN).** Add the `EXPECTED_ENV_TOUCHING` key for the suite with its real count, and give each environment-touching case in `tests/auth/sameOriginServerAction.test.ts` either a `premise` / `premiseHolds` line proven on that case's OWN inputs, or an explicit `no-premise` justification where the environment IS the fixture. Do not pick the count by running the scanner and copying the number — derive it from which cases actually touch the environment, then confirm the scanner agrees; a count read off the tool is the degenerate-source shape the premise rules exist to stop.
- [ ] **Step 3.** `pnpm vitest run tests/mutation/_metaPremiseContract.test.ts` green; `pnpm typecheck`; commit `test(auth): enrol the same-origin helper in the source-mutation guard gate`.

<!-- tasks: end -->

## Mutation-guard acceptance (not a TDD task)

Running the harness is a measurement, not a behavior change, so it carries no `red=` — the same reason backlog reconciliation below carries none. It runs at closeout step 3 and its acceptance is stated here:
...
5. **Backlog archive — the PR's LAST and ONLY post-review commit.** Archive the entry to `BACKLOG-archive.md` with a resolution note, removing its IN PROGRESS marker in the SAME commit (archives reject in-progress entries), then re-run `pnpm vitest run tests/docs/_metaLedgerInProgress.test.ts tests/docs/_metaLedgerReferentialIntegrity.test.ts`.

   **Why this does not violate review-covers-what-merges** (`docs/agents/writing-plans.md:28`, lint shape (i)). That rule and invariant 12 pull in opposite directions here: invariant 12 REQUIRES the marker to come off "in the PR's last commit, before the merge", because a marker that reaches `main` names a branch the merge just deleted and reds `_metaLedgerInProgress` on `main` until somebody clears it. The reconciliation, which is the shipped precedent from `fix/auth-picker-hardening`, is that the reviewed CODE equals the merged CODE: this commit contains an entry move, a marker removal, and a one-line resolution note — no source file, no test, no behavior, and nothing the review could have found. It is the narrow invariant-mandated exception, and it is the only commit permitted after step 4. If any repair is needed from step 4, it lands BEFORE this commit and is re-reviewed.
...
- [ ] **Enrolment's downstream contract is in the plan, not discovered at closeout.** Adding a `GuardSurface` row makes `tests/mutation/_metaPremiseContract.test.ts` require a matching `EXPECTED_ENV_TOUCHING` key and per-case premises; Task 11 is that cycle, and the registry-array reconciliation (16 → 17) is run at plan time and pasted.
...
- [ ] **Invariant 12 + review-covers-what-merges:** marker filled before the whole-diff review; the archive + marker removal is the PR's last and only post-review commit, carries only ledger-status changes, and the closeout states the reconciliation with `docs/agents/writing-plans.md:28` rather than asserting the exception.
