...
2. **Fill the machine closeout marker, BEFORE review.** After step 1 has actually run, add the bare-anchored marker line to this section: a line reading `impeccable-gate: critique=RAN audit=RAN p0=<n> p1=<n> dispositions=<recorded|none>` with the REAL counts (`RAN-DEGRADED` if a half degraded; `dispositions=recorded` iff `p0+p1>0`). **Never fabricated** — the gate has not run in the spec+plan arc, so no `RAN` claim is honest yet, and `tests/docs/_metaInvariant8Closeout.test.ts` reds on this unmerged branch by design until the implementation arc fills it.
   ...
   **Then commit, here, before step 3.** Steps 1 and 2 both write tracked text into this document — the findings-and-dispositions record and the marker line — and there is no other commit boundary between them and the post-review ledger commit. Without this one, the closeout edits either ride into step 5 (which would make the "ledger-only" post-review commit carry plan changes, defeating its own justification) or sit uncommitted and risk never reaching the pushed branch at all. Commit them as `docs(plan): record the invariant-8 dual-gate result and fill the closeout marker`. This is the last commit before review, so step 4 reviews it along with everything else.
