...
- The score and the unaccepted-survivor set are stated in the whole-diff review brief (closeout step 5).
...
- **Archived at CLOSEOUT step 4, the last commit authored and still inside the reviewed diff:** the entry moves to `BACKLOG-archive.md` with a resolution note, its IN PROGRESS marker removed in the SAME commit — archives reject in-progress entries, and a marker that reaches `main` names a branch the merge just deleted, which reds `tests/docs/_metaLedgerInProgress.test.ts` on `main` until somebody clears it. It sits after the local gates and before the whole-diff review, which is the one position that satisfies invariant 12 and `docs/agents/writing-plans.md:28` at the same time; the closeout preamble fences that ordering in both directions.
...
Ordered; each item gates the next. Two project rules bear on the tail of this list and appear to pull opposite ways, so the ordering below is the one that satisfies both LITERALLY, and it is fenced in both directions because two consecutive review rounds pushed it each way (plan R7 #2, plan R8 #2).

- `docs/agents/writing-plans.md:28` shape (i): **the diff the final review round examined must be the diff that merges.** Its wording is about the diff, not about executable lines, so "the post-review commit carries no code" does NOT exempt anything. An earlier draft argued exactly that and was refuted.
- Invariant 12 (`AGENTS.md`): the ledger marker **comes off in the PR's last commit, before the merge, so it never reaches `main`**, and the origin-visible claim lives as long as the work does. An intermediate draft moved the archive ahead of the review, which satisfied the first rule by giving up the second, and left the row reading unclaimed through gates, review, and CI.

**The ordering that satisfies both:** the backlog archive is the LAST COMMIT AUTHORED (step 4), and the whole-diff review (step 5) runs AFTER it, over the complete diff including that commit. Nothing is committed after the review on the terminal path, so the reviewed diff is the merged diff; and the marker comes off in the last commit, so the claim stands until the work is done. If the review demands a repair, that repair commits and the review runs AGAIN over everything — the marker is still off before the merge, which is the purpose invariant 12 states for itself, and the re-run restores the first property. Neither rule is waived and no exception is claimed. **Do not re-propose either the archive-after-review ordering or the archive-before-gates ordering**; each was tried, each broke one of the two rules, and this is the resolution.
...
4. **Backlog archive — the LAST COMMIT AUTHORED, and it is still reviewed.** Archive `BL-SERVER-ACTION-ORIGIN-GATE-SWEEP` to `BACKLOG-archive.md` with a resolution note, removing its IN PROGRESS marker in the SAME commit (archives reject in-progress entries), then `pnpm vitest run tests/docs/_metaLedgerInProgress.test.ts tests/docs/_metaLedgerReferentialIntegrity.test.ts` and `pnpm format:check`. This commit lands AFTER step 3's gates and BEFORE step 5's review, which is what makes it simultaneously the last authored commit (invariant 12) and part of the reviewed diff (`docs/agents/writing-plans.md:28`). Its only cover from the full suite is step 6's real CI, which runs everything on the merged tree — the two named ledger tests are the local proof, and that is deliberate rather than an omission.
5. **Whole-diff cross-model review to APPROVE — the last thing that happens before the merge, and it covers step 4's commit.** The brief states the mutation score and the unaccepted-survivor set (AGENTS.md convergence bullet 4), the consequence bound, the `PROBE DOMAIN:`, and the threat fence, and carries the spec's §1.1 do-not-relitigate list — in particular Resolved scope #9r and 12c-12g, which cost four spec rounds to settle. **If the diff stage reaches `ROUND_THRESHOLD` counted rounds** (`4`, `lib/reviewRounds/constants.ts:11`), the arc owes a `## diff — <n> rounds` section in the same round-economy filing the spec and plan stages already write, `docs/review-rounds/fix/server-action-origin-sweep/119895a7c756.md`, or in the sibling named for a newer merge-base if the branch is rebased; it lands with the repair commits of the round that requires it and is therefore itself reviewed, never appended after the final APPROVE. Any repair this review demands lands as its own commit and the review runs again; the run ends on an APPROVE with nothing uncommitted and nothing to commit.
6. **Real CI green**, then `gh pr merge --merge`, then fast-forward local `main` to `0  0`. No commit lands between step 5's APPROVE and the merge.
...
- [ ] **Invariant 12 + review-covers-what-merges:** the impeccable marker is filled and committed at closeout step 2; the archive + ledger-marker removal is closeout step 4, the LAST commit authored, and the whole-diff review at step 5 runs over it. No commit follows the review on the terminal path. Neither rule is waived and no exception is claimed — earlier drafts tried the archive after the review and then before the gates, and each gave up one of the two rules; the closeout preamble fences both.
