...
- [ ] **Step 1 (RED).** Add the `GuardSurface` row to `tests/mutation/source/registry.ts`: `id: "sameOriginServerAction"`, `sourcePath: "lib/auth/sameOriginServerAction.ts"`, `suitePaths: ["tests/auth/sameOriginServerAction.test.ts"]`, `operators: [...OPERATOR_NAMES]`, `scoreFloor: 0.95` (the modal value among the sixteen shipped rows — seven use 0.95, six use 0.9, three use 1.0), `control: { from: 'secFetchSite === "same-origin"', to: 'secFetchSite !== "same-origin"' }`, `accepted: []`.
  ...
  **Registry reconciliation, run at plan time and pasted rather than described** (`docs/agents/writing-plans.md:27`). Three arrays move, and all three counts are live-tree readings:
  ...
  The key counts differ because a surface may declare more than one suite path; the target suite is absent from both tables today (probed). The task's own checks are those three commands reporting 17 / 17 / 25 after step 1.
...
- **Archived at CLOSEOUT step 3, BEFORE the whole-diff review:** the entry moves to `BACKLOG-archive.md` with a resolution note, its IN PROGRESS marker removed in the SAME commit — archives reject in-progress entries, and a marker that reaches `main` names a branch the merge just deleted, which reds `tests/docs/_metaLedgerInProgress.test.ts` on `main` until somebody clears it. It is deliberately NOT the last commit: invariant 12 wants the marker off before the merge, which this satisfies, and putting it ahead of the review is what makes the reviewed diff the merged diff (closeout preamble).
...
Ordered; each item gates the next. The ordering exists to make one property literally true rather than argued for: **the diff the final review round examines IS the diff that merges** (`docs/agents/writing-plans.md:28`, lint shape (i)). Every tracked edit — the impeccable record and marker, the ledger archive, any round-economy filing the arc owes — is committed BEFORE the review, and **no commit is permitted after step 5 except a repair the review itself demands, which is re-reviewed.** An earlier draft of this plan kept the ledger archive as a post-review commit and argued it was exempt because it carried no code; that argument was wrong twice over — the rule's wording is about the diff, not about executable lines, and invariant 12 does not need the exception, because its purpose ("so it never reaches `main`") is satisfied by any pre-merge commit. Ordering the archive before the review costs nothing and removes the exception entirely.
...
3. **Backlog archive, BEFORE review, not after.** Archive `BL-SERVER-ACTION-ORIGIN-GATE-SWEEP` to `BACKLOG-archive.md` with a resolution note, removing its IN PROGRESS marker in the SAME commit (archives reject in-progress entries). Invariant 12 requires the marker off before the merge so it never reaches `main`; it does not require the removal to be the last commit for its own sake, and doing it here is what lets step 5 review the merged diff exactly. One consequence to know rather than discover: `pnpm ledger:claims` stops reporting this branch's claim from this commit onward, so the row reads unclaimed for the short window between here and the merge.
4. **Full local gates before push:** `pnpm heavy pnpm test` (no exception now — step 2 filled the invariant-8 marker, so `tests/docs/_metaInvariant8Closeout.test.ts` is green here, unlike at Task 10), `pnpm typecheck`, `pnpm exec eslint .`, `pnpm format:check`, `pnpm heavy pnpm mutation:guards`, and `pnpm vitest run tests/docs/_metaLedgerInProgress.test.ts tests/docs/_metaLedgerReferentialIntegrity.test.ts` for the archive move.
5. **Whole-diff cross-model review to APPROVE — the last thing that happens before the merge.** The brief states the mutation score and the unaccepted-survivor set (AGENTS.md convergence bullet 4), the consequence bound, the `PROBE DOMAIN:`, and the threat fence, and carries the spec's §1.1 do-not-relitigate list — in particular Resolved scope #9r and 12c-12g, which cost four spec rounds to settle. **If the diff stage reaches `ROUND_THRESHOLD` counted rounds** (`4`, `lib/reviewRounds/constants.ts:11`), the arc owes a `## diff — <n> rounds` section in the same round-economy filing the spec and plan stages already write, `docs/review-rounds/fix/server-action-origin-sweep/119895a7c756.md`, or in the sibling named for a newer merge-base if the branch is rebased; it lands with the repair commits of the round that requires it and is therefore itself reviewed, never appended after the final APPROVE. Any repair this review demands lands as its own commit and the review runs again; the run ends on an APPROVE with nothing uncommitted and nothing to commit.
