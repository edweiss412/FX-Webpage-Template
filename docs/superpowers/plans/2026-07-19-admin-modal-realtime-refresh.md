...
1. Vacuous-pass test (Step 3): temporarily move the `<ShowRealtimeBridge …/>` line BEFORE `<PublishedReviewModal …/>` in the loader → rerun the child-order-stability + "bridge rendered LAST" tests → both must FAIL → revert.
2. Forensic-code row (Step 5): temporarily delete the `"ADMIN_SHOW_VERSION_TOKEN_READ_FAILED"` row from `NEW_FORENSIC_CODES` → run `pnpm vitest run tests/log` → the code-scanning meta-test that consumes the set must FAIL on the loader's new emit (identify the exact failing test in the run output — that is the row's enforcement proof) → revert.
...
- Files: Create `tests/e2e/_realtimeBridgeLiveEntry.tsx` (client entry rendering `<ShowRealtimeBridge showId="harness-show" slug="harness" renderVersion="v0"/>`) + `tests/e2e/realtime-bridge-conduction.spec.ts` (spec); Modify `tests/e2e/_step3ReviewModalBundle.mjs`-STYLE bundler — the VERIFIED live-bundle precedent is the `_step3ReviewModalBundle.mjs` esbuild script with its `*LiveEntry.tsx` peers (`_blockedRowResolverLiveEntry.tsx`, `_packListRescanLiveEntry.tsx`, …, all in `tests/e2e/`), which already carries the use-server-elision esbuild plugin — add the new entry to that bundler (or a sibling `.mjs` copied from it) with TWO aliases: `@/lib/supabase/browser` and `next/navigation`; Modify `playwright.config.ts` (register `realtime-bridge-conduction` in the desktop-chromium testMatch INSTEAD of `published-review-modal\.realtime`); Modify `.github/workflows/published-modal-e2e.yml` (run line + path filters reference the new spec + entry instead of the realtime spec); Modify `tests/e2e/published-review-modal.interactions.spec.ts` (the appended reconcile-invariants test — in the fallback File list explicitly, with its OWN TDD cycle below).
...
- Conduction spec TDD: write `realtime-bridge-conduction.spec.ts` FIRST; run against the entry WITHOUT the aliases wired → FAIL (no `__driveInvalidation` defined); wire; PASS. Assertions: (a) `__driveInvalidation` burst of 8 within 50ms → exactly ONE `__refreshCount` increment, ≥100ms after the last call (REAL bridge debounce conducted it — never call refresh directly); (b) two invalidations 300ms apart → two increments (negative regression, mirrors the bridge's pinned debounce tests).
- Reconcile-invariants test TDD: append to `published-review-modal.interactions.spec.ts` a test driving a REAL app-owned reconcile (the ignore-warning server action that spec already exercises) with the §4.4 1–4 + skeleton + geometry oracles; red phase = the same oracle-precondition assertions run against a deliberately broken oracle setup first is NOT meaningful here — instead its red phase is the standard interactions-spec pattern: write the assertions, run once, and if green on first run perform ONE negative verification (temporarily key crew rows by index-only in a scratch edit is NOT allowed — instead scroll-reset the modal programmatically mid-test in a throwaway variant to prove the scrollTop oracle can fail); commit separately.
...
7. Assert (spec §8.4 order): **a post-mutation frame satisfying `isInvalidationFrame(text, showId)` was RECEIVED, and the content swap follows it** (positive proof of the broadcast→debounce→refresh chain — without this the swap could ride an unrelated refresh; the spike-pinned predicate is load-bearing here, not just in the quiescence gate); NEW role appears within the target row locator (URL unchanged); NO `/version` request occurred post-mutation (unless a recorded socket close/reconnect happened → retry per protocol below); skeleton recorder empty; geometry stable (`scrollHeight`, `offsetTop`, `offsetHeight` ±1px — else fail INCONCLUSIVE with a distinct message); `scrollTop` unchanged ±1px; popover still open; `document.activeElement` is the `data-probe` node; attention menu still closed.
...
- [ ] **Step 4: Red-phase proof (the e2e is an acceptance test — prove it can fail on the defect it exists to catch).** Task 1's mount is COMMITTED, so stashing does nothing — instead make a temporary uncommitted breakage: edit `app/admin/_showReviewModal.tsx` and delete the `<ShowRealtimeBridge …/>` JSX line (keep everything else), rebuild, run the spec:

```bash
# (manual edit: remove the ShowRealtimeBridge JSX line from the loader return)
...
Expected: FAIL (no bridge → no subscription → gate 2 times out waiting for the join reply). Restore with `git checkout -- app/admin/_showReviewModal.tsx` (the breakage was never staged or committed).
