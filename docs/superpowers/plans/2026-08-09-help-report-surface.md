...
- [ ] **Step 2: Run to verify failure.** Run: `pnpm vitest run tests/components/report/ReportModal.test.tsx tests/components/report/ReportButton.test.tsx`. Expected: RUNTIME failures (vitest strips types — the plan's own closeout note): the help-label assertion fails on `DEFAULT_LABEL` lacking a `help` row and the github-link/success cases fail on the admin-only gates — those records and gate lines are the failing production lines. The crew-facing-fallback case (test 5b) is NOT a live baseline RED: today's `copyForCode` sends a runtime `"help"` down its crew arm and coincidentally renders the expected crewFacing copy (plan R5 F2); it guards a PARTIAL implementation — flipping `copyForCode` (line 154) without flipping `oppositeSurface` (line 442) makes it fail — so it stays in the suite with that stated failure mode, not as a Step 2 RED. The `"help"`/`null` TYPE violations surface separately in `pnpm typecheck` (run it here too; expected: errors at `components/shared/ReportModal.tsx:40` and `components/shared/ReportModal.tsx:61` until Step 3).
...
// 8. ring wiring (plan R3 F1): the rendered CTA button's className contains
//    "focus-visible:ring-offset-info-bg", proving HelpReportCta passes
//    ringOffset="info-bg" (omitting the prop silently selects "surface",
//    ReportButton.tsx:92 default).
```
...
<!-- task: red=`rg -q 'Recurring-error report on .?/help/errors' docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md && rg -q 'non-show recurrence report' docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md && rg -q '2026-08-09-help-report-surface-design' docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md` ac=AC-6 -->

- [ ] **Step 1: RED** — all three `rg` probes above fail before the edit (verified 2026-08-09: `non-show recurrence report` has zero hits in the master spec, and the required §13.1 surface-5 sentence does not contain that phrase (it appears only in the §13.2.1 note's required copy), so the note probe discriminates that edit specifically — an earlier `helpCode` probe collided with §13.1's own `fieldRef.helpCode` text (plan R4 F1); the surface-5 heading phrase appears nowhere in the master spec, and this design-spec's path appears nowhere in the AC-11.11 doc — unlike a bare `r12` token, which that doc already contains at :158/:292 for unrelated amendments). Mutant check: each probe anchors on a phrase unique to ITS required edit, so omitting either edit fails its own probe.
...
- [ ] **Step 3: GREEN** — probes pass (probe 3's match — the design-spec path — must be inside the AC-11.11 row of the user-facing-docs doc, verified by eye at edit time; probe 2's phrase lands in the master spec's §13.2.1 note); run `pnpm vitest run tests/help/spec-citation-integrity.test.ts tests/cross-cutting/codes.test.ts` (catalog untouched — x1 must stay green).
...
- [ ] **Step 2:** M11-I-D-1: append a resolution line (closed by this branch; AC-11.11 r12; spec cross-ref `2026-08-09-help-report-surface-design`). Follow that DEFERRED.md's local resolution convention (read neighboring resolved entries first). Executable gate (plan R3 F2): `rg -q '2026-08-09-help-report-surface' docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/DEFERRED.md` — fails before this step (verified 2026-08-09: zero hits), passes after.
...
| 3 | `/help/errors` newly downloads the 4052-line message catalog client-side through `ReportButton` → `ReportModal` → `HelpAffordance` | P1 | DEFERRED — `DEFERRED.md` `HELPREPORT-MODAL-STATIC-IMPORT-CATALOG-1` (reason (c): the `next/dynamic` repair changes the mount contract for all four hardened surfaces). |
| 4 | A mid-open `hashchange` unmounts the dialog with no cue; focus falls to `document.body` | P1 | DEFERRED — `DEFERRED.md` `HELPREPORT-CTA-REMOUNT-FOCUS-1` (reason (a): the two candidate repairs contradict either the R5-ratified remount or ordinary reading behavior; that is a product decision). |
...
| 6 | Esc does not close the modal (pre-existing, all four surfaces) | P2 | DEFERRED — `DEFERRED.md` `HELPREPORT-MODAL-NO-ESCAPE-1` (reason (b): spec §1.1 item 9 fences existing-surface behavior in this PR). |
