...
**Interfaces:**
- Produces: the string literal `'RESCAN_REVIEW_REQUIRED'` available to `rescanWizardSheet` (Task 3) as a cataloged code.
...
- Produces: `DECISION_REQUIRING_INVARIANTS: ReadonlySet<TriggeredReviewItem["invariant"]>` (currently `new Set(["MI-11"])`) and `computeRescanDecision(priorParse: ParseResult|null, refreshedParse: ParseResult, priorDataGaps: DataGapsSummary|null): { dirty: boolean; decisionItems: TriggeredReviewItem[] }`.
...
- [ ] **Step 3: Implement `rescanWizardSheet`** per spec §5:
  ...
  - **(a) re-stage:** `const res = await scanOnboardingPreparedFiles(folderId, wizardSessionId, [prepared], { withShowLock: (_id, fn) => fn(scanTx) })`; if `res.processed[0].outcome === "hard_failed"` → `DELETE` orphan shadow, read `pending_ingestions.last_error_code`, return `{status:"needs_attention", code:<that>}`.
...
- [ ] **Step 1: Decision-invariant meta-test** — assert `DECISION_REQUIRING_INVARIANTS` equals the set the live `runInvariants` partition treats as gated (`{MI-11}`, per `phase1.ts:312-319`); concrete failure mode: a future gated invariant added to the reviewer-choice surface but not to the rescan decision set → CLEAN bypass. Negative-regression: mutate the set to `{}` → a fixture email-change rescan must flip to CLEAN (proving the test bites).
