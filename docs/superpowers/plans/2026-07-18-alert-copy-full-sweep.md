...
- Produces: `deriveAlertMessageParams(code, context, identity)` additionally resolves params `crew-name`, `email`, `crew-row-count`, `failed-sheet-names`, `crew-count`, `show-date` with priority identity > context > fallback. ALSO exports `IDENTITY_PARAM_TOKENS: ReadonlySet<string>` — the full set of identity-derived param tokens (sheet-name, show-name, repo, file-name, role-changes, crew-name, email, crew-row-count, failed-sheet-names) — consumed by Task 5's token inventory so the two can't drift. Fallbacks (spec §3): crew-name "a crew member", email "an email address", crew-row-count "two or more crew rows", failed-sheet-names "some sheets", crew-count "some", show-date "an upcoming date".
...
- [ ] **Step 3: Implement** — resolved identity segments arrive LABEL-LOSSY (`resolveAlertIdentities.ts:295-298` emits `{label: null, value}` for count/email segments), so label-based mapping cannot work for them. Instead import `ALERT_IDENTITY_MAP` from `lib/adminAlerts/alertIdentityMap.ts` and map by ORDERED WALK WITH SKIPS — `resolveAlertIdentities` only PUSHES segments it could build, so absent earlier specs shift the array and a naive zip mis-maps. Correct algorithm: iterate the code's declared `SegmentSpec[]` keeping a cursor into `identity.segments`; for each spec, check whether the segment AT THE CURSOR is kind-compatible (labeled specs must match the expected label; `email` kind must match `/@/` shape with `label === null`; `count` kind must match a numeric-phrase shape with `label === null`); on match, consume it (advance cursor) and derive the param; on mismatch, treat the spec's segment as absent (do NOT advance) and fall through to context/fallback for that param. Add a unit test where an earlier segment is absent (e.g. AMBIGUOUS_EMAIL_BINDING with no Show segment but email+count present) proving no mis-mapping and derive the param name from the spec kind: `sheetName`→`sheet-name`, `showName`→`show-name`, `crewName`→`crew-name`, `email`→`email`, `count`→its per-code param (`crew-row-count` for AMBIGUOUS_EMAIL_BINDING), `contextField`→its per-code param (`failed-sheet-names` for ONBOARDING_SHEET_UNREADABLE). Reuse `resolveNamedParam` (identity-quoted > context > fallback) for every mapped param; counts render unquoted (numeric phrase, not a name — quote only name-like kinds sheetName/showName/crewName). `crew-count`/`show-date` (SHOW_FIRST_PUBLISHED) have no identity segments — plain `context ?? fallback`. Respect pii: when a segment is `pii: true`, emit the fallback phrase, never the raw value (grep existing pii handling in `resolveAlertIdentities` first and mirror it).
...
- Test: `tests/adminAlerts/_metaInlineIdentityContract.test.ts` — its identity-token inventory at :14 recognizes only `<sheet-name>`, `<show-name>`, `<repo>`, `<file-name>`, `<role-changes>`; replace its hardcoded token list with the Task 1 `IDENTITY_PARAM_TOKENS` export (single source); update the pinned set size to 30.
...
- Modify: `lib/messages/catalogDocsValidator.ts:5-7` — the SHARED renderable/predicate helper still excludes `severity === "info"`; drop that clause here (`_metaErrorCatalogDocs` consumes it, but `app/help/errors/page.tsx:23-31` has its OWN LOCAL `isRenderable` — Task 9 MUST replace that local helper with an import of the shared `catalogDocsValidator` predicate so the two can never drift).
