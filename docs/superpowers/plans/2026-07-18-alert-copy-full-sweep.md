...
- [ ] **Step 3: Implement** — extend the label→param map (`:31-39` area) with `Crew → crew-name`; add email detection for label-less segments (`label === null && value.includes("@")` → `email`); route contextField segment labels through a per-code map entry for `failed-sheet-names` (ONBOARDING_SHEET_UNREADABLE's Sheet-labeled contextField); reuse `resolveNamedParam` (identity-quoted > context > fallback) for every new param; plain `context ?? fallback` for `crew-row-count`/`crew-count`/`show-date` (no identity tier — counts/dates aren't identity segments; AMBIGUOUS_EMAIL_BINDING's count segment maps identity-first via its declared key if present). Respect pii: never emit a raw email param when the segment is `pii: true` and the caller context is identity-less — fallback instead (mirror existing pii handling in `resolveAlertIdentities`; grep before implementing).
...
- [ ] **Step 4:** `npx vitest run tests/messages/fullSweepCopy.test.ts && pnpm test:audit:x1-catalog-parity` → PASS; record interim-red counts (same three known suites as Task 2).
...
- Test: `tests/adminAlerts/_metaInlineIdentityContract.test.ts` (bidirectional contract auto-covers the growth — update its pinned set size if hardcoded)
