...
- No lock-topology change: the retry route stays the single advisory-lock holder via `withRowTryLock`; callees assert (`assertShowLockHeld`). `prepareProcessOneFile` acquires nothing. No edit to `tests/auth/advisoryLockRpcDeadlock.test.ts` — Task 9 verifies it green, unchanged.
...
- **Extends:** none of the named registries. Rationale: no new Supabase call boundary (`writeSyncLog` is postgres.js — `tests/auth/_metaInfraContract.test.ts` N/A); no admin_alert code added (`tests/messages/_metaAdminAlertCatalog.test.ts` N/A); no new mutation surface and no emit removed (`tests/log/_metaMutationSurfaceObservability.test.ts`, `tests/log/_auditableMutations.ts` untouched — Task 9 verifies green); no email path (no-inline-email N/A); no advisory-lock layer change (see Global Constraints).
...
- [ ] **Step 1: Write the failing tests.** New file tests/sync/runManualStageForFirstSeenEmission.test.ts. Drive the REAL `runManualStageForFirstSeen` with injected `runPhase1`/`runPhase2` and a locked-tx double (copy the tx double shape from the existing `tests/sync/runManualStageForFirstSeen.test.ts`); a `logSync` spy records entries. Cases, expected values derived from the injected fixtures (anti-tautology):
  ...
  - dedupe pin: `runPhase2` applied path where an injected `evaluateQualityRegression`-adjacent step throws AFTER the tail wrote its row — assert exactly ONE row total and the throw propagates. (Inject via `deps.upsertAdminAlert` throwing on the `evaluateQualityRegression_unlocked` call — it is the first post-tail await at `lib/sync/runManualStageForFirstSeen.ts:177`.)
...
- [ ] **Step 3: Implement.** In `lib/sync/runScheduledCronSync.ts`: change `async function logSync(` to `export async function logSync(`. In `lib/sync/runManualStageForFirstSeen.ts`:
  ...
  - In `toResult`, replace the `if`-chain with an exhaustive `switch (result.outcome)` over ALL `Phase1Result` variants; `auto_apply_with_holds`/`shrink_held` → `return { outcome: "parsed", degenerate: "unexpected_phase1_outcome" }`; default arm `const _exhaustive: never = result;`. Delete the `?? { outcome: "parsed" }` coalesce at the call site (`lib/sync/runManualStageForFirstSeen.ts:252-254`) — `toResult` now always returns.
...
- [ ] **Step 5: Commit.** `git commit -m "feat(sync): record escaped manual-sync throws in sync_log"`
...
- Produces: `runManualSyncForShow_unlocked(tx, driveFileId, mode, fileMeta, deps, prepared)` — sixth parameter **required**, forwarded as the sixth argument to `runUnlocked`. `RunManualSyncForShowDeps.processOneFile_unlocked` gains `prepared: PreparedProcessOneFile` as its sixth (required) parameter. Task 4's route call site depends on both.

<!-- task: red=`pnpm vitest run tests/sync/runManualSyncForShowThrowEmission.test.ts -t "unlocked"` ac=AC-3,AC-5 -->

- [ ] **Step 1: Failing tests.** In the Task-2 test file, an `"unlocked"` describe: (a) six-argument spy — inject `processOneFile_unlocked` recording `args.length === 6` and the `prepared` value passed; call `runManualSyncForShow_unlocked(txDouble, id, "manual", fileMeta, { processOneFile_unlocked: spy, processDeps: { logSync: sinkSpy } }, preparedFixture)` and assert the spy received `preparedFixture` verbatim; (b) throw-before-sink ⇒ one `parse_error` row + original rethrow; (c) throw-after-tracked-sink-call ⇒ no second row. RED validity: the production forward at `lib/sync/runManualSyncForShow.ts:286` passes five arguments today (the defective line), and the signature has no sixth parameter — the six-arg test cannot typecheck-then-pass until both land.
...
- [ ] **Step 4: Census class (c).** Update `tests/sync/runManualSyncForShow.test.ts:210`/`244` to pass a `prepared` fixture; `tests/sync/runManualSyncForShow.test.ts:228`'s `toHaveBeenCalledWith` gains the sixth value; `tests/admin/pendingIngestionsLiveActions.test.ts:186` likewise; rewrite the `tests/sync/runManualSyncForShow.test.ts:690` probe to pass SIX arguments with only the tx type wrong so its `@ts-expect-error` keeps its intended meaning (a five-arg call would satisfy it vacuously).
- [ ] **Step 5: GREEN + typecheck.** `pnpm vitest run tests/sync/runManualSyncForShowThrowEmission.test.ts tests/sync/runManualSyncForShow.test.ts tests/admin/pendingIngestionsLiveActions.test.ts && pnpm typecheck`.
- [ ] **Step 6: Commit.** `git commit -m "feat(sync): require prepared data at the unlocked manual-sync seam"`
...
- Produces: `LivePendingIngestionRouteDeps` gains `prepareProcessOneFile?: typeof prepareProcessOneFile` and `logSyncSink?: typeof writeSyncLog`, both defaulted in `depsWithDefaults`. The route's `runManualSyncForShowUnlocked` injectable signature gains the sixth `prepared` parameter.

<!-- task: red=`pnpm vitest run tests/api/admin/pendingIngestionRetryPrepare.test.ts` ac=AC-1,AC-3,AC-8 -->

- [ ] **Step 1: Failing tests.** New file, driving `handleLivePendingIngestionRetry` with the route's existing injection pattern (copy the deps scaffold from `tests/api/admin/pendingIngestionRetryPostCommitIsolation.test.ts`): (a) existing-show path calls the injected `prepareProcessOneFile` with `(driveFileId, "manual", metadata, depsObj)` and forwards its return as the sixth argument to the injected `runManualSyncForShowUnlocked` (spy asserts identity); (b) throwing `prepareProcessOneFile` ⇒ `logSyncSink` spy records one `{outcome:"parse_error", code: classifySyncFailure(err)}` entry, response is the outer guard's rethrow → 500 path, `PENDING_INGESTION_RETRY_FAILED` breadcrumb fired; (c) first-seen `prepareFirstSeenStage` throwing `FirstSeenStagePrepareError("STAGED_PARSE_FAILED", cause)` ⇒ `logSyncSink` records `{outcome:"parse_error", code:"STAGED_PARSE_FAILED"}` AND the response is still the typed 409 (AC-8 — the typed response outranks recording); (d) sink failure in (c) ⇒ typed response STILL returned, `SYNC_LOG_EMIT_FAILED` escalation fired.
- [ ] **Step 2: RED** — the route has no `prepareProcessOneFile`/`logSyncSink` seams and passes no sixth argument (`app/api/admin/pending-ingestions/[id]/retry/route.ts:435-444` is the defective site).
- [ ] **Step 3: Implement.** Add both deps + defaults. Existing-show branch after the watched-folder check:
...
- [ ] **Step 6: Commit.** `git commit -m "fix(sync): prepare existing-show pending-ingestion retries and record prepare failures"`
...
- Test: tests/sync/fetchFailurePartition.test.ts (new), plus a directed override-skip case in `tests/sync/runScheduledCronSync.test.ts`

**Interfaces:**
- Consumes: existing `ManualRecoveryTx.insertSyncLog` tx member; exported `logSync`; `errorMessage` (module-local, in scope).
...
```
  ...
  Both no-show arms get the same `insertSyncLog({driveFileId, outcome, code, payload:{driveFileId, message: errorMessage(error)}})` for the value they return (`SYNC_FILE_FAILED` on the carve; `result` on the generic arm). The override-TOCTOU skip (`lib/sync/runScheduledCronSync.ts:3514-3518`) gets `await logSync(txDeps, driveFileId, result);` before its return.
- [ ] **Step 4: GREEN** + directed override-skip case (inject `readShowPullSheetOverride_unlocked` mismatch via the tx double; assert the `skipped`/`pull_sheet_override_changed_under_lock` row with duration present).
- [ ] **Step 5: Commit.** `git commit -m "fix(sync): record every fetch-failure terminal arm and the override TOCTOU skip"`
...
- [ ] **Step 1: Failing tests.** Drive REAL `applyStaged` (live scope) with the fixture scaffold from `tests/sync/applyStaged.test.ts`, injected sink + `now`: (a) applied (existing-show) ⇒ exactly one row `{outcome:"applied", showId}` whose `parseWarnings` equal the staged fixture's warnings (source-derived) and `durationMs` equals the injected clock delta; (b) applied first-seen (autoPublishFirstSeen path) ⇒ still exactly ONE row (tail sinkless); (c) smuggled sink — `firstPublishedTailDeps` cast through the wider type WITH a `logSync` spy ⇒ tail spy NEVER called, exactly one post-commit row (R7 F1 pin); (d) sink-throw ⇒ response still the applied result, `SYNC_LOG_EMIT_FAILED` escalation with that exact code asserted; (e) thrown live apply (injected `runPhase2` rejecting) ⇒ one `{outcome:"parse_error"}` row via the resolved sink, original error rethrown (R8 F1).
...
- [ ] **Step 3: Implement.**
  ...
  - Tail: narrow both `firstPublishedTailDeps` declarations with `Omit<Parameters<typeof emitSuccessfulPhase2Tail>[0]["deps"], "logSync">`; at `lib/sync/applyStaged.ts:1492`, `const { logSync: _stripped, ...suppliedTailDeps } = (deps.firstPublishedTailDeps ?? {}) as Record<string, unknown> & { logSync?: unknown };` then pass `suppliedTailDeps`-or-default (keep the tx-bound `upsertAdminAlert` default EXACTLY as today when no injection).
...
- [ ] **Step 6: Commit.** `git commit -m "feat(sync): staged-apply attempts write sync_log at the post-commit chokepoint"`
...
- [ ] **Step 3: Commit.** `git commit -m "docs(sync): retire the sync-log emission gap markers"`

### Task 8: Env-bound live-DB proof (AC-1 live probe + post-commit attribution)

**Files:**
- Test: tests/sync/pendingIngestionRetryLive.db.test.ts (new)

<!-- task: red=`TEST_DATABASE_URL=postgresql://postgres:postgres@127.0.0.1:54322/postgres pnpm vitest run tests/sync/pendingIngestionRetryLive.db.test.ts` ac=AC-1,AC-4 -->

- [ ] **Step 1: Write the gated live test.** Follow `tests/sync/resyncShrinkHold.db.test.ts`'s conventions exactly: `test.skipIf(!shouldRun)` where `shouldRun = Boolean(process.env.TEST_DATABASE_URL) && dbUp` (liveness probe), unique `driveFileId` prefix + afterAll cleanup by prefix. Case A (AC-1): seed a `shows` row + `pending_ingestions` row for the same `drive_file_id`; call the REAL `handleLivePendingIngestionRetry` with only `fetchDriveFileMetadata`/`prepareProcessOneFile`-level Drive collaborators stubbed to fixture data (everything DB-ward real); assert the response status is NOT 500, no `SyncInfraError` escaped, and ≥1 `sync_log` row exists for the file. Case B (AC-4 attribution): drive a real live staged apply (seed pending_syncs + show fixtures per `tests/sync/applyStaged.test.ts`'s DB variants if present, else via `applyStaged` with the real default sink against the local DB) and assert the applied row's `show_id` column is NON-NULL (post-commit resolution — the property the in-tx cron row cannot have).
- [ ] **Step 2: RED before Task 4 lands / GREEN after.** On the pre-change tree case A throws `SyncInfraError` (`lib/sync/runScheduledCronSync.ts:3452-3458` via the five-argument call) — if this task is executed after Task 4 (normal order), demonstrate RED by `git stash`-free means: run case A once with `deps.prepareProcessOneFile` FORCED undefined? NO — the seam defaults. Instead the RED for this task is the test file's absence of green on the PRE-TASK-4 base: record in the commit body the base-SHA run (`git worktree`-local `git stash` of Tasks 4's diff is NOT required — cite the plan-time probe in BL-PENDING-RETRY-EXISTING-SHOW-THROWS as the observed red, and assert the test currently passes). The test's discriminating power premise: case A first asserts the seeded show EXISTS (premise) so the existing-show branch is the one exercised.
- [ ] **Step 3: Commit.** `git commit -m "test(sync): live-DB retry and attribution proof"`

### Task 9: Gates, ledger graduation, close-out
...
- [ ] **Step 3: Ledger graduation.** Archive both entries (archives reject in-progress rows — the graduation commit both removes the markers and archives, as the PR's LAST commit before merge). Delete nothing else.
- [ ] **Step 4: Whole-diff cross-model review to APPROVE, real CI green, `gh pr merge --merge`, fast-forward main to `0 0`** — per the autonomous-ship pipeline the implementing session runs.
...
- **Spec coverage:** §3.1 → Tasks 3+4; §3.2 → Task 1; §3.3 → Task 2; §3.4 (emission, strip, catch) → Task 6; §3.5 → Task 5; §5 census (a)–(e) → Tasks 6, 3, 4; AC-1/AC-4 live proofs → Task 8; markers → Task 7; gates/graduation → Task 9. §6/§7 impose no tasks (limits/out-of-scope).
...
- **RED validity:** every new-case RED names its absent production line inline (Task 1 step 2, Task 2 step 2, Task 3 step 1, Task 4 step 2, Task 5 step 2, Task 6 step 2); Task 7's and Task 9's `red=` are negated greps observed FAILING at plan time (markers present) and passing only when their task removes the markers — same command both sides; Task 8's RED is the ledger's live probe (documented, not re-run against a stashed tree).
