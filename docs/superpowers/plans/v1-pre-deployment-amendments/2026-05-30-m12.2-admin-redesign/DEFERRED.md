...
**Status:** ✅ **RESOLVED — owner chose option (i), 2026-06-01.** `sync_unknown` surfaces `SYNC_STATUS_UNKNOWN`'s specific developer-attention copy via `getRequiredDougFacing(health.code)` (split out of the generic group line). §3.1 amended (owner-ratified amendment 2026-06-01). **Implementation pending in a B1 follow-up; PR #4 held until it lands + re-greens, then merge.**
