...
2. **Per-entry action affordances** (client components, one per `action` discriminant):
   ...
   - `action: "approve_reject"` → **[Approve] [Reject]** (each form submits `entry.gate.holdId` + `entry.gate.baseModifiedTime` — the feed-rendered PF40 staleness token, resolution #26) → server actions → delegate to the Phase 3 `approveMi11Hold(holdId, expectedBaseModifiedTime)` / `rejectMi11Hold(holdId, expectedBaseModifiedTime)` helpers (which resolve the show's `drive_file_id` FROM the hold and own the Drive-`modifiedTime`-first F13 orchestration + the lock-taking RPCs). **PF23: the page never binds `showId`/`driveFileId` — only `holdId` + the opaque `baseModifiedTime` token cross the boundary; the token is the CLIENT-rendered value echoed back, never re-read server-side (a fresh read makes the PF40 guard vacuous).** **PF17: the swap/collision outcome is NOT pre-rendered. The Approve button is always a single `[Approve]`; on submit, the action's typed result drives the rendering — a `{ok:false, code:'IDENTITY_WOULD_COLLIDE'}` shows the conflict copy via `lib/messages`, an approved (possibly group/swap) success flips the entry status. No pre-computed `[Approve group]` / conflict-no-button state.**
...
- [ ] **Impl** `app/admin/show/[slug]/page.tsx` — `await readShowChangeFeed(show.id)`; render `<ChangesFeed>` (passing the three thin server actions + `now`). **PF14: each entry already carries its own action payload from Phase 5 — `entry.gate = {holdId, disposition, baseModifiedTime}` for `approve_reject` (the `baseModifiedTime` is the resolution #26/PF40 staleness token the Approve/Reject forms submit back), `entry.changeLogId` for `undo` — so the page does NO second query for hold/disposition data; it just forwards `entries` to `<ChangesFeed>`.** **Remove EXACTLY the per-show LIVE whole-parse review mount and nothing else:** delete (a) the `<section data-testid="admin-show-parse-warnings-section">` block that renders `<ParsePanel rows={rows} showId={show.id} readOnly={archived} />` (page.tsx:625–639); (b) the `pending_syncs` query (`supabase.from("pending_syncs")…` at :190–212) and the `rows: StagedRow[]` derivation that feeds it (:214–228); (c) the now-unused imports (`ParsePanel`, `StagedRow` type, `parseTriggeredReviewItems`, `deriveParseSummary`, `safeStringField`). **Do NOT touch** the `StagedReviewCard` / `ParsePanel` components themselves — they remain in use by the wizard `wizard_failed_reapply` route (`app/admin/onboarding/staged/[wizardSessionId]/[driveFileId]/page.tsx`) and the live `first_seen` route (`app/admin/show/staged/[stagedId]/page.tsx`), confirmed by importer grep. First-seen approval is **unchanged** in this phase: it stays governed by `auto_publish_clean_first_seen` and its dedicated `/admin/show/staged/[stagedId]` route; Phase 6 deletes no first-seen surface. Keep the wrapping `requireAdmin` + try/catch boundary discipline.
- [ ] **DEPENDENCY ORDERING (PF31).** The per-show LIVE `pending_syncs` mount removal in this task MUST land **AFTER the Phase 2 cutover** that clears legacy live `pending_syncs` (`wizard_session_id IS NULL` rows). Otherwise a pre-existing live `pending_sync` would point at a removed review mount and strand the operator with no way to apply/discard it. Sequencing: Phase 2 cutover (clears live rows) → THEN this mount removal. The **wizard** `pending_syncs` path (`first_seen` + `wizard_failed_reapply`, `wizard_session_id IS NOT NULL`) is untouched — only the live `wizard_session_id IS NULL` review path retires.
- [ ] Commit `feat(admin): mount changes feed, wire gate/undo actions, drop whole-parse review`.

### T6.7b — Prove no legacy `existing_staged` inbox route can strand the operator (PF31)

> `components/admin/NeedsAttentionInbox.tsx:100-114` renders an `existing_staged` card whose "Open show →" Link targets `/admin/show/{slug}` — the per-show page that (after T6.7) no longer mounts the live whole-parse review. The new model never creates live `pending_syncs` and the Phase 2 cutover clears any pre-existing ones, so `existing_staged` items should be **0** in practice. This task proves a legacy row can't leave a dangling "review me" route to a mountless surface.

- [ ] **Test** `tests/components/admin/needsAttentionInbox.existingStaged.test.tsx`. Failure mode it catches: *after cutover, a stale live `pending_sync` still surfaces an `existing_staged` "Open show" route pointing at the removed review mount, stranding the operator on a page with no apply/discard.* Assert: with the post-cutover inbox feed shape (the inbox source query no longer emits `existing_staged` items — Phase 2 cutover + the new model never writing live `pending_syncs`), `screen.queryByTestId(/needs-attention-item-existing-/)` is `null` and no `needs-attention-link-*` points at a per-show route expecting a review mount. Derive the feed from the inbox's actual data source (the cutover-cleared `pending_syncs`), NOT a hand-built `existing_staged` fixture (anti-tautology — a hardcoded `existing_staged` item would force a render the live source can no longer produce).
- [ ] **Code disposition (test-first, code only if a hypothetical row would still break):** the per-show route `/admin/show/{slug}` STILL EXISTS and renders after T6.7 (only the parse-warnings section is removed), so an `existing_staged` link is NOT a 404 — it lands on a valid page that simply no longer shows the staged review. Minimal disposition: **leave the dead `existing_staged` branch in place** (the inbox source never emits it post-cutover) and let the guard test pin the emptiness. **Only if** the inbox source could still emit such a row, repoint the card to the changes-feed anchor on the show page (or hide the card) rather than promising a "review" that isn't there. Do NOT rip out the branch — keep the change minimal. (So: **likely TEST-ONLY**; a code change is the fallback, not the expectation.)
- [ ] Commit `test(admin): pin no legacy existing_staged route strands the operator (PF31)`.
