...
- [ ] **Test** `tests/sync/applyParseResult.holdAware.undoOverride.test.ts` (DB-backed, real-PG, next-sync semantics — seed the `undo_override` hold directly, then run a later `applyParseResult`):
  ...
  - **(b) Direction-A held-present undo (removal/rename) is retained — `crew_identity` held-present** — seed `{kind:'undo_override', domain:'crew_identity', entity_key:'Alice', held_value:<prior Alice row>}`; sheet still omits Alice (or still lists the rename target). Assert Alice's restored row still exists after apply AND the conflicting added/rename row is suppressed. **Failure mode:** restored crew member re-removed (or the rename re-applied) next sync.
  ...
  - **(d) release-on-reconcile for each** — three sub-cases: `crew_email` override releases when the sheet email == `held_value.email` (sheet reverted) → hold deleted, no pin needed; held-present override releases when the sheet's value for that entity CHANGES (sheet moved on) → hold deleted and the new sheet value applies; tombstone releases when the sheet DROPS or changes `Zed` → hold deleted and (if re-added later) the add flows normally. Assert each `sync_holds` row is deleted and the new value applied. **Failure mode:** stuck override pins forever after the sheet has moved on (whack-a-mole / permanent ghost).
...
- [ ] **Impl** — extend the hold-aware apply (Task 2.4 logic) and `lib/sync/holds/reEvaluateHolds.ts` (Task 2.8):
  - Read **ALL** open holds at the top of `applyParseResult` (`tx.readOpenHolds(showId)` already returns every row — drop the `kind='mi11_pending'`-only filter when building the apply maps). Dispatch per `(kind, domain, held_value shape)`:
    ...
    - `undo_override` + `crew_identity` HELD-PRESENT (`held_value` is a prior crew row, no `absent` flag): re-insert/retain that row (synthesize an upsert from `held_value` if the parse would drop it), exclude `entity_key` from delete, and suppress any conflicting added row (same email/name as the held identity).
  ...
  - **Release (§4.3), extend `reEvaluateHolds` to cover `undo_override`:** for each open `undo_override`, after computing the sheet's current state for `entity_key` — `crew_email`: if sheet email == `held_value.email` → `DELETE` (reconciled). HELD-PRESENT: if the sheet's value for that entity now differs from `held_value` (source moved on) → `DELETE` (the new value then applies this same pass). TOMBSTONE: if the sheet no longer lists `held_value.name` OR lists it with a different identity → `DELETE`. One row per `(show_id, domain, entity_key)` throughout (the `UNIQUE` constraint holds for both kinds).
  - Ordering inside the locked txn: read holds → fold (2.5/2.6) → apply pins/suppressions (mi11_pending AND undo_override) → reconcile reconcile-release (2.8 + this task) → write change-log (2.9). Document this order in the impl so a later edit can't reintroduce a post-state read.
