...
- [ ] **Test** `tests/sync/phase1.decision-rule.test.ts`:
  - `routes a parse with only FYI items (MI-9 LEAD toggle + asset drift) to auto-apply, not stage` — build `args`/`tx` whose warnings trigger BOTH a `DIAGRAMS_LINKED_FOLDER_DRIFT_PENDING` AND a `REEL_DRIFT_PENDING` asset-drift item via `syncLayerReviewItems` (`lib/sync/phase1.ts:221-239`), plus an MI-9 LEAD toggle; assert `runPhase1(...)` returns `outcome:'pass'` (existing show) or `'auto_publish_ready'` (first-seen), and assert `tx.upsertLivePendingSync` was **NOT** called (spy) for EITHER asset-drift item — i.e. asset drift auto-applies (spec line 40) and becomes a notification-only feed row in Phase 5 (`action='none'`, §6 lines 195-197), never a stage. Derive the expectation from the items the fixture actually triggers (read the `syncLayerReviewItems`/`runInvariants` output in the test, don't hardcode "MI-9"). **Failure mode (PF34):** asset drift still routes to `upsertLivePendingSync`/`outcome:'stage'`, which Phase 6 (T6.7) strands on the removed per-show review mount.
...
- [ ] **Impl** in `lib/sync/phase1.ts`:
  ...
  - After `reviewItems` is built (`:298`) and the MI-8 debounce check (`:299-300`), partition: `const mi11Items = reviewItems.filter(i => i.invariant === 'MI-11')`. Keep the existing hard-fail and debounce branches exactly as-is (do NOT change `hasLead`/MI-10 safety-net or `mi8DebounceReason`). **Keep computing `reviewItems` (incl. `withLeadToggleSafetyNet` and `syncLayerReviewItems`)** — it still feeds the `mi8DebounceReason(args, reviewItems)` early-return (`:299-300`) AND Phase-2 / Phase-5 feed-row derivation. ONLY the question of *which* items route to `upsertLivePendingSync` changes.
  - **The staging branch (`upsertLivePendingSync`, `:322-349`) is now reserved for SENTINELS + hard-fail ONLY.** Build the `triggeredReviewItems` that feed `upsertLivePendingSync` from `sentinel` (the `onboarding_scan` `ONBOARDING_SCAN_REVIEW`, `:244-249`) PLUS the clean-first-seen `FIRST_SEEN_REVIEW` injection (`:309-320`) and **NOTHING ELSE**. BOTH `invariantItems` (MI-6…MI-14 except MI-11) AND `syncLayerItems` (asset drift) are dropped from the set routed to `upsertLivePendingSync`.
  - New rule: if `mi11Items.length > 0`, return `{ outcome: 'auto_apply_with_holds', mi11Items }` (the rest of the parse still applies in Phase 2; only MI-11 emails hold). Otherwise, with no sentinel/first-seen injection, **all other previously-staging invariants — MI-6…MI-14 (except MI-11) AND asset drift — now fall through to `pass`** (existing show) / `auto_publish_ready` (first-seen): they are notifications, written as feed rows in Phase 2 apply / Phase 5, never staged.
  - **PF34 correction (cite):** this REPLACES the prior note that said "asset-drift / onboarding sentinels and the first-seen-clean flag branch keep their existing staging behavior." That note was the bug — it contradicted the spec (asset drift → auto-apply, design spec line 40; asset drift = notification-only feed row `action='none'`, §6 lines 195-197) AND the Task 2.1 asset-drift test. Asset drift MUST auto-apply. Only the `onboarding_scan` sentinel + the clean-first-seen `FIRST_SEEN_REVIEW` injection still stage.
  - Guard: an `onboarding_scan` or unresolved `FIRST_SEEN_REVIEW` sentinel still stages via the existing `triggeredReviewItems.length > 0` branch (`:322`) — now reachable ONLY from those two sources; MI-11 on a first-seen sheet is impossible so the two never conflict.
...
- [ ] **Test** `tests/sync/applyParseResult.holdAware.undoOverride.test.ts` (DB-backed, real-PG, next-sync semantics — seed the `undo_override` hold directly, then run a later `applyParseResult`):
  ...
  - **(f) REJECT-produced undo_override sticks across the next sync (PF30, per 00-overview resolution #20)** — Phase 3's `mi11_reject_hold` converts a rejected FOLDED rename/removal `mi11_pending` into an `undo_override` `crew_identity` carrying `held_value.baseline` (the SAME contract as undo); Task 2.8b's apply + release must treat a reject-produced hold IDENTICALLY to an undo-produced one (the `held_value.baseline` shape is identical). Two next-sync sub-cases:
    - **Rejected folded RENAME** — produce the hold via `mi11_reject_hold` on a folded-rename `mi11_pending` (or seed the equivalent `undo_override` `crew_identity`, `held_value.baseline={kind:'rename',suppressed_added:{name,email}}`); run a NEXT UNCHANGED sync where the sheet STILL has the rename (old name absent, new/suppressed name present). Assert the OLD row is RETAINED and the replacement (`held_value.baseline.suppressed_added`) is NOT added (no duplicate). Then run a sync where the sheet RECONCILES (old name present / rename gone) → assert the override RELEASES same-sync. **Failure mode (PF30):** rejected rename re-applies next sync (old row dropped, replacement re-added) — the reject didn't stick.
    - **Rejected REMOVAL** — produce the hold via `mi11_reject_hold` on a removal `mi11_pending` (`held_value.baseline={kind:'removal'}`); run a NEXT UNCHANGED sync where the sheet STILL omits the crew member. Assert the row is KEPT (NOT re-removed). Then run a sync where the sheet RE-ADDS them → assert the override RELEASES and the member flows normally. **Failure mode (PF30):** rejected removal re-removes the crew member next sync.
    - These exercise the SAME `undo_override` machinery as (b)/(d)/(e); assert the reject-produced hold's `held_value->'baseline'` is read identically to an undo-produced hold's (cross-phase parity pin with Phase 3).
...
- [ ] **Test** `tests/sync/writeChangeLog.autoApply.test.ts` (DB-backed):
  ...
  - `auto-applied crew rename writes a crew_renamed row whose entity_ref is the PRIOR name (PF28)` — prior `[Alice(a@x)]`, next `[Dana(a@x)]` (MI-12 same-email rename, no MI-11). After apply assert the `show_change_log` row has `change_kind='crew_renamed'` and `entity_ref='Alice'` (the PRIOR/OLD name undo keys its restore on), NOT `'Dana'`. **Failure mode (PF28):** entity_ref stores the new name → supersession cleanup protects the wrong row and undo (which re-inserts on the old name) is left unguarded / mis-superseded.
...
- [ ] **Impl** `lib/sync/changeLog/writeAutoApplyChanges.ts`:
  ...
  - **`entity_ref` canonical rule (PF28, per 00-overview resolution #19): `entity_ref` = the name `undo_change` keys its crew-row restore on.** `crew_added` → the ADDED name; `crew_removed` → the REMOVED name; `crew_renamed` (Alice→Dana) → the **PRIOR/OLD name** (`'Alice'`), NOT the new name — undo re-inserts via `ON CONFLICT (show_id, name='Alice')`, and the suppressed NEW name (`'Dana'`) lives in `held_value.baseline.suppressed_added`, never in `entity_ref`. This makes supersession cleanup (keyed on `entity_ref`) protect exactly the row undo would touch.
...
### Task 2.10b — Cutover: retire the LIVE whole-parse `pending_syncs` staging path (PF31)
...
**Deployment-ordering precondition (PF35 — the load-bearing fix; mirror in `00-overview.md` resolution #21).** No migration snapshot can be complete while an OLD (pre-deploy) code path is still inserting live `pending_syncs`: a new live row created for a show that had NONE at snapshot time is never revisited by the migration, and the per-show advisory lock does NOT close this (an old in-flight sync that acquires the lock AFTER the migration's per-show delete still inserts). The fix is **deployment ordering, not migration cleverness**:
1. **Deploy the new decision-rule code (Phase 1 + Phase 2)** so NO code path writes a live `pending_sync` (`wizard_session_id IS NULL`) anymore — Task 2.1's "never inserts a live pending_sync" test pins this at the source.
2. **Quiesce the sync cron / drain in-flight syncs** (no old-code sync running).
3. **THEN run the cutover migration as a residue sweep.**
The migration is a **residue-sweep that runs AFTER the new code is the only writer** — it is NOT concurrency control against old writers (deployment ordering is). The per-show advisory lock stays as **defense-in-depth** (serializes against any in-flight NEW sync, which won't write live `pending_syncs` anyway).
...
- [ ] **Test** `tests/sync/cutover.retireLivePendingSyncs.test.ts` (DB-backed):
  ...
  - `no-row-at-snapshot vector is closed AT THE SOURCE, not by the migration (PF35)` — this replaces the implicit "migration alone is race-proof" claim. Two parts: **(a)** under the NEW decision-rule code, a fresh sync of a show that had NO live `pending_sync` at any prior moment does NOT create one — drive a full MI-11 sync AND an FYI/asset-drift sync for such a show and assert `pending_syncs WHERE drive_file_id=<X> AND wizard_session_id IS NULL` stays empty (the source of the race — old code inserting for an unseen show — cannot occur once new code is the only writer, per the deployment-ordering precondition). **(b)** Re-running the cutover migration AFTER that sync finds zero new live rows → idempotent residue-sweep (run the migration twice around the sync; assert the second run enumerates zero shows and mutates nothing). **Failure mode (PF35):** new code still writes a live `pending_sync`, so a show with no row at the first snapshot acquires one later and strands on the removed Phase-6 mount.
