...
- [ ] **Test** `tests/sync/applyParseResult.holdAware.undoOverride.test.ts` (DB-backed, real-PG, next-sync semantics — seed the `undo_override` hold directly, then run a later `applyParseResult`):
  ...
  - **(d) release-on-reconcile for each — SAME-SYNC release + apply (PF12)** — three sub-cases, each running **one** later sync whose sheet meets the release condition, then asserting the released entity's NEW sheet value is live **after that single sync** (NOT two): `crew_email` override releases when the sheet email == `held_value.email` (sheet reverted) → hold deleted, the row's email is the sheet value this same sync; held-present override releases when the sheet's value for that entity CHANGES (sheet moved on) → hold deleted AND the new sheet value is applied this same sync; tombstone releases when the sheet DROPS or changes `Zed` → hold deleted AND (if the sheet now lists `Zed` with a new identity) that add flows through this same sync. Assert each `sync_holds` row is deleted AND the new sheet value is live in `crew_members` after the single sync. **Failure mode (PF12):** the apply pins/suppresses using the stale override and THEN deletes the hold, so release happens but the new value only appears a sync LATER (two-sync lag) — or the override pins forever (whack-a-mole / permanent ghost).
...
- [ ] **Impl** — extend the hold-aware apply (Task 2.4 logic) and `lib/sync/holds/reEvaluateHolds.ts` (Task 2.8):
  ...
  - **PF12 — RELEASE EVALUATION IS A PRE-APPLY PASS.** Before building the active pin/suppress maps, evaluate each open hold's release condition against the **incoming sheet/parse**:
    - `undo_override` + `crew_email`: release if sheet email == `held_value.email` (sheet reverted).
    - `undo_override` + `crew_identity` HELD-PRESENT: release if the sheet's value for that entity now differs from `held_value` (source moved on).
    - `undo_override` + `crew_identity` TOMBSTONE: release if the sheet no longer lists `held_value.name` OR lists it with a different identity.
