...
**18. Undo idempotency — the undone source row goes non-actionable (resolves PF16).** When `undo_change` succeeds it **updates the ORIGINAL `show_change_log` row's `status` to `'undone'`** (under the same advisory lock), in addition to inserting the `source='undo'` row (`undo_of=<orig>`). Supersession: `undo_change` first checks the target row's `status` — if it is **not** `'applied'` (already undone/superseded) → return `{ok:false,code:'UNDO_SUPERSEDED'}` (no mutation). The feed's undo predicate is already `change_kind ∈ {crew_added,crew_removed,crew_renamed} AND status='applied'`, so the original row drops to `action='none'` immediately after undo — no Undo button on a refreshed feed, and a double-submit returns `UNDO_SUPERSEDED`. Phase 4 test: undo twice on the same row → 2nd returns `UNDO_SUPERSEDED`, exactly one undo row + one hold exist; the original row's `status` is `'undone'`.
