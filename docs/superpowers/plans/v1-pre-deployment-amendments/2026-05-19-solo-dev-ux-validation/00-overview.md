...
- Master spec: `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` (amended atomically in Phase 0 per §3.3.2 (α + γ-footnote) hybrid)
- Picker pivot spec (canonical for auth-model facts post-M11.5): `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-23-crew-auth-pivot-show-link-picker.md`
- M12 canonical spec: `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` (25-round adversarial-reviewed; rebased 2026-05-26 per §15.26)
...
**M12 starts only after M11 closes** (spec §10). M11 (`/help` docs at `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/`) ships 13 `/help` pages that are load-bearing on the cold-start pass (§6) AND part of the matrix's surface band D (§4.2). If M11 has not closed when M12 is ready to start, the dev WAITS — no soft-start.
...
docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md  # Phase 0.B (modify) — §4.3 line 610 (verified 2026-05-26): 21→22 prose-track tables + add picker-pivot drift footnote per §3.3.2 (α + γ-footnote) hybrid; §4.1 schema-section: CREATE TABLE validation_state inserted as final §4.1 block before §4.2 line 596; AC-2.5 line 3536: 21→22 / 84→88 + cross-reference footnote
...
docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation/SIGN-OFF.md  # Final sign-off (create) — single required exercise output per spec §8.1
```
...
3. **Spec is canonical** (AGENTS.md #7) — M12 spec is at `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` after 25 rounds of adversarial review.
...
| `tests/cross-cutting/email-canonicalization.test.ts` | 0.C | **Task 0.C.9** (per DEFERRED `M12-PHASE0C-EMAIL-CANON-EXT`) | The existing email-boundary audit WILL be extended in Phase 0.C to flag `lower(...)` / `trim(...)` in `scripts/validation-*.ts` files unless adjacent to a `canonicalize()` call from `lib/email/canonicalize.ts`. **R11 audit (2026-05-26): the R5 narrative previously framed this extension as already-landed, but `auditLiveEmailCanonicalization()` at `lib/audit/emailCanonicalization.ts:693-705` walks `lib/parser`/`lib/sync`/`lib/reports`/`lib/auth`/`lib/data`/`lib/adminAlerts`/`app/api/admin` only — `scripts/validation-*.ts` is absent.** R13 commit 29 (per R12 F12 finding + (A) phantom-trigger audit) scheduled this as concrete Task 0.C.9 in `03-phase0-tooling-reseed.md`. |

This milestone WILL CREATE the following structural meta-tests:

| Meta-test (planned) | Phase | Created in task | Purpose |
| --- | --- | --- | --- |
| `tests/db/postgrest-dml-lockdown.test.ts` | 0.B | **Task 0.B.2 Step 8** (per R17 commit 38 F15 structural defense — AGENTS.md cross-cutting #1 plan-time checklist) | Project-wide PostgREST DML lockdown invariant. For every table in the `LOCKED_TABLES` registry, asserts that `anon` + `authenticated` Supabase clients receive a permission error (PostgREST `42501 permission denied`) on direct INSERT/UPDATE/DELETE. Initial registry at Phase 0.B completion (post-R67 F55 amendment): `crew_members` (M9.5 R6 close), `validation_state` (M12 R17 F15 close). `crew_member_auth` (M9.5 R5 close) was originally proposed for the registry but is NOT registered because the M11.5 G3 cutover migration (`supabase/migrations/20260523000099_cutover_drop_m9_5.sql:26`) dropped the table; `has_table_privilege` on a non-existent relation would fail Layer 1 and PostgREST would surface a relation-missing error rather than 42501. `crew_member_auth` retirement is independently validated by `tests/db/cutover-drop-m9-5.test.ts`. New RPC-gated tables MUST register here. The alternative is an inline `// not-subject-to-meta: <reason>` comment when the table intentionally permits PostgREST DML. Test source embedded verbatim in `02-phase0-validation-state.md` Task 0.B.2 Step 8. RED→GREEN: assertions for `validation_state` FAIL before the R17 REVOKE block lands, PASS after; the `crew_members` row stays GREEN throughout as regression baseline. |
| `tests/cross-cutting/validation-tooling-tz-pin.test.ts` | 0.C | **Task 0.C.8** (per DEFERRED `M12-PHASE0C-TZ-PIN-METATEST`) | Greps every `.sql` migration AND every `.ts` script in `scripts/validation-*.ts` for the string `current_date` (lowercase, plain match). Each match MUST be either (a) inside the bounded-skew sanity check (`abs(DATE_TEXT::date - current_date) > 1` — integer day comparison; the earlier R5 narrative cited `abs(extract(epoch from ...::date - current_date)) > 86400` but PostgreSQL returns INTEGER days from `date - date` arithmetic and `extract(epoch from integer)` is invalid; corrected per R11 F9 fix at `03-phase0-tooling-reseed.md`), OR (b) carry an inline `// not-validation-today-iso: <reason>` waiver. The default is "TZ-pinned `validationTodayIso` wins; `current_date` is for skew-check only." Catches the recurring class where a future M12 plan amendment slips a `current_date` back into a seed/finalize path. **R11 audit (2026-05-26): this test was framed as already-landed in the R5 narrative but does NOT exist in git history (verified via `git log --all -- 'tests/cross-cutting/validation-tooling-tz-pin.test.ts'` → empty).** R13 commit 29 (per R12 F12 finding + (A) phantom-trigger audit) scheduled this as concrete Task 0.C.8 in `03-phase0-tooling-reseed.md`. |

**Per AGENTS.md "Meta-test inventory" rule** + the R5 "vector unresolved" declaration: these structural defenses are SPECIFIED here and AUTHORED in Phase 0.C Tasks 0.C.8 + 0.C.9 (per the DEFERRED entries above). Until they land, drift in either email canonicalization or TZ-pinned dates is caught by per-instance adversarial review on validation tooling, not by CI structural defense. Phase 0.F close-out gate (Task 0.F.8) REQUIRES both meta-tests committed and green before Phase 0 is marked done.
...
| ~~Re-seed cleans `revoked_links WHERE revoked_reason LIKE 'validation:%'`~~ | — | **RETIRED 2026-05-26 picker-pivot rebase** — `revoked_links` table dropped at M11.5 G3 cutover. Re-seed has no equivalent cleanup; `--combo all` is the structural reset for OAuth-claim state via the mint RPC's `ON CONFLICT (show_id, name) DO UPDATE SET ..., claimed_via_oauth_at = NULL` clause, which **preserves the stable `crew_members.id`** while resetting `claimed_via_oauth_at` to NULL (per R13 commit 31 F11 amendment + R23 commit 51 F23-class adjacent-peer amendment; see spec §3.3 Cleanup contract for the row-stability invariant the J3 walk relies on) |
