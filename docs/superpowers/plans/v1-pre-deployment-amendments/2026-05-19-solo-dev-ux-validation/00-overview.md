...
| `tests/cross-cutting/email-canonicalization.test.ts` | 0.C | Task 0.C.4 (planned per DEFERRED `M12-PHASE0C-EMAIL-CANON-EXT`) | The existing email-boundary audit WILL be extended in Phase 0.C to flag `lower(...)` / `trim(...)` in `scripts/validation-*.ts` files unless adjacent to a `canonicalize()` call from `lib/email/canonicalize.ts`. **R11 audit (2026-05-26): the R5 narrative previously framed this extension as already-landed, but `auditLiveEmailCanonicalization()` at `lib/audit/emailCanonicalization.ts:693-705` walks `lib/parser`/`lib/sync`/`lib/reports`/`lib/auth`/`lib/data`/`lib/adminAlerts`/`app/api/admin` only — `scripts/validation-*.ts` is absent.** The extension is deferred to Phase 0.C `scripts/validation-reseed.ts` authoring per `DEFERRED.md` entry. |
...
| --- | --- | --- | --- |
| `tests/cross-cutting/validation-tooling-tz-pin.test.ts` | 0.C | Task 0.C.4 (planned per DEFERRED `M12-PHASE0C-TZ-PIN-METATEST`) | Greps every `.sql` migration AND every `.ts` script in `scripts/validation-*.ts` for the string `current_date` (lowercase, plain match). Each match MUST be either (a) inside the bounded-skew sanity check (`abs(DATE_TEXT::date - current_date) > 1` — integer day comparison; the earlier R5 narrative cited `abs(extract(epoch from ...::date - current_date)) > 86400` but PostgreSQL returns INTEGER days from `date - date` arithmetic and `extract(epoch from integer)` is invalid; corrected per R11 F9 fix at `03-phase0-tooling-reseed.md`), OR (b) carry an inline `// not-validation-today-iso: <reason>` waiver. The default is "TZ-pinned `validationTodayIso` wins; `current_date` is for skew-check only." Catches the recurring class where a future M12 plan amendment slips a `current_date` back into a seed/finalize path. **R11 audit (2026-05-26): this test was framed as already-landed in the R5 narrative but does NOT exist in git history (verified via `git log --all -- 'tests/cross-cutting/validation-tooling-tz-pin.test.ts'` → empty).** Authoring is deferred to Phase 0.C per `DEFERRED.md` entry. |

**Per AGENTS.md "Meta-test inventory" rule** + the R5 "vector unresolved" declaration: these structural defenses are SPECIFIED here and AUTHORED in Phase 0.C (per the DEFERRED entries above). Until they land, drift in either email canonicalization or TZ-pinned dates is caught by per-instance adversarial review on validation tooling, not by CI structural defense. Phase 0.C close-out gate REQUIRES both meta-tests committed and green before Phase 0.C is marked done.
