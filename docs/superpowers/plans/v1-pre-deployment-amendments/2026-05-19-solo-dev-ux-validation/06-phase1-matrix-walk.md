...
This is the load-bearing identity-exclusivity walk. It requires (i) a Google account whose email matches `alias_5a_lead`'s fixture email — **per R13 commit 30 amendment + R15 commit 35 canonical rejected-domain set extension** combo R1's `alias_5a_lead.email` is the value of env var `VALIDATION_J3_CLAIM_EMAIL` (the dev's REAL Google account email, set during Phase 0.A.5 per spec §3.3 step 5 R13-amendment paragraph). Google OAuth cannot authenticate against the **canonical rejected set** (RFC 2606 `example.com`/`.org`/`.net`; RFC 6761 `*.test`/`*.invalid`/`*.localhost`/bare `localhost`; mDNS RFC 6762 + project-conventional `*.local`/`dev.local`), so the previously-documented `validation+5a@example.com` convention is unusable for the live OAuth-claim walk; the dev signs in as themselves per spec §1.5 "solo-dev IS the validation". AND (ii) a separate "Bob" Google account whose email matches NEITHER R1's `alias_5a_lead` (= `$VALIDATION_J3_CLAIM_EMAIL`) NOR R1's `alias_6a_a1` (= synthesized `validation+R1-alias_6a_a1@example.com`) — Bob is any non-fixture Google account the dev controls (a personal-alt or test account; the point is that Bob's email is NOT in the validation crew_members table for this show). Step 4 precondition for the Mode-B walk.
