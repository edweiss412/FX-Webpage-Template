...
**STATUS: RESOLVED 2026-06-21** (test-hardening bundle). Authored the sibling audit `tests/cross-cutting/no-m9-5-surfaces-in-m12-docs.test.ts`, which scans `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation/**/*.md` + the design spec for the 18-term forbidden list (NOT 14 — the live `tests/cross-cutting/no-m9-5-surfaces.test.ts` array has grown to 18; the sibling pins its copy to the canonical array via a lockstep guard test so the two cannot drift). Historical-context exclusions implemented: `~~strikethrough~~`, RETIRED/DELETED/DROPPED/REMOVED/cutover/historical/precedent/legacy/picker-pivot keyword within ±3 lines, §15 audit-trail section detection, plan-wide-invariant-2 advisory-lock table-list lines, and the M9.5-grants / M11.5-cutover migration-filename citation anchors. An anti-self-satisfying guard asserts no exclusion keyword is a substring of any forbidden term. RED→GREEN verified: a stale `crew_member_auth` ref injected at the neutral plan-02 H1 fires the assertion; all current historical references pass clean. Original entry preserved below.
