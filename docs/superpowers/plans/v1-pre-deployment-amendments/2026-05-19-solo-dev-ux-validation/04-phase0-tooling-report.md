...
- `report_rate_limits` rows → no jsonb column; instead the `identity` field uses a reserved prefix `validation:m12-fixture-<outcome>:` so the cleanup pattern is `WHERE identity LIKE 'validation:m12-fixture-%'`. For the rate-limit-admin outcome where the harness needs to actually deny admin POSTs, the dev's real admin email is used (NOT the prefix); cleanup for this outcome uses the **R35 commit 73 F34 snapshot+restore contract** (not blanket deletion): the harness records the prior `(kind='admin', identity=canonicalize(email), hour_bucket=<recorded>)` row state before seeding and the `--cleanup --include-admin-email <email>` flag scopes its `report_rate_limits` write strictly to the recorded `hour_bucket` (restoring the prior count, or DELETEing the row if no prior row existed). The cleanup recipe **never** touches cross-hour buckets and **never** issues a `WHERE kind='admin' AND identity=canonicalize(<email>)` predicate without the `AND hour_bucket=<recorded>` clause — see F34 contract below.
...
4. **Snapshot persistence.** The `(snapshot_prior_count, recorded_hour_bucket, email_canonical_form)` tuple must survive across the harness's seed → exercise → cleanup invocation boundary (CLI invocations are separate processes). Two acceptable storage strategies:
   - **(a) File-backed snapshot** at a known path (e.g., `.validation-state/rate-limit-admin-snapshot.json`, gitignored) — recipe persists / reads / removes the file across invocations.
   - **(b) `validation_state` table row** with a known key (e.g., `validation_state.key = 'rate_limit_admin_snapshot'`) — recipe upserts / reads / clears the row across invocations.

   The plan executor (Phase 0.E.1 Step 4 implementer) picks ONE strategy; document it in the harness's `--help` output and the rate-limit-admin test fixture (Step 1).

5. **Cleanup defense-in-depth.** If `--cleanup --include-admin-email <email>` is invoked WITHOUT a snapshot record (e.g., orphaned cleanup after a crash), the harness MUST refuse with stderr `no rate-limit-admin snapshot found; cleanup aborted to avoid touching prod buckets` (exit 1). A `--force-cleanup-without-snapshot --hour-bucket <ISO timestamp>` flag is available for the explicit emergency case where the dev knows the recorded bucket value but the snapshot file/row was lost — this flag DELETEs only the exact named bucket (still never cross-hour).
...
- [ ] **Step 4: Implement** `scripts/validation-report-fixtures.ts` using `assertProdEquivalentTarget` guard. Per-outcome dispatch with the table-write recipe from the map above. Service-role client (`createSupabaseServiceRoleClient`) for INSERT/UPSERT; advisory lock unnecessary (none of these writes mutates `shows`, `crew_members`, `crew_member_auth`, `pending_syncs`, or `pending_ingestions` per plan-wide invariant 2). **For rate-limit-admin specifically:** wire the R35 commit 73 F34 snapshot+restore lifecycle — (a) `SELECT count FROM report_rate_limits WHERE kind='admin' AND identity=canonicalize($VALIDATION_ADMIN_EMAIL) AND hour_bucket=date_trunc('hour', now())` before seeding; (b) persist `(snapshot_prior_count, recorded_hour_bucket, email_canonical_form)` to the chosen storage (file at `.validation-state/rate-limit-admin-snapshot.json` OR `validation_state.key='rate_limit_admin_snapshot'` row); (c) UPSERT the seed at the recorded bucket; (d) cleanup reads the snapshot, scopes its restore-or-delete to the exact recorded bucket, and removes the snapshot record on success. Without a snapshot record, cleanup refuses (exit 1) absent `--force-cleanup-without-snapshot --hour-bucket <ISO>` — see the F34 contract block above.
...
- [ ] **Step 5: Tag every harness write** per the tagging convention above. Specifically:
  ...
  - `report_rate_limits.identity` ← `'validation:m12-fixture-' || $outcome || ':' || gen_random_uuid()` (except for `rate-limit-admin`, which MUST use `canonicalize(process.env.VALIDATION_ADMIN_EMAIL!)` per R33 commit 69 F32 fix — see Step 1 above; the harness must read the env var and apply `lib/email/canonicalize.ts` before the UPSERT so the seeded bucket matches what live `enforceQuota` writes on a real admin POST). The rate-limit-admin UPSERT MUST be scoped to the snapshot-recorded `hour_bucket` per the R35 commit 73 F34 contract — never spans multiple buckets.
...
- [ ] **Step 2: Verify cleanup works:** `pnpm validation:report-fixtures --cleanup --include-admin-email $VALIDATION_ADMIN_EMAIL`. Confirm DELETE removed ALL three tables' validation-tagged rows in order — query each table directly to verify zero `validation:m12-fixture-%` matches remain. Per the R35 commit 73 F34 snapshot+restore contract, verify additionally that the rate-limit-admin recorded `hour_bucket` row was restored to its pre-seed `count` (or removed if no row existed pre-seed) — query `report_rate_limits` for the canonical admin identity in the recorded bucket AND in every adjacent hour bucket; assert no cross-hour deletion occurred.
