...
- [ ] **Step 3: Create the skeleton** `scripts/validation-reseed.ts` with `parseArgs` from `node:util`, printing the usage block on `--help`. **Required env vars: per the canonical CLI command-by-command env-var contract at spec §9.1.2** (R27 commit 58 F10-class Option D refactor — §9.1.2 is the SOLE source of truth for the literal env-var names; this plan task no longer inline-restates them). The reseed row in §9.1.2 names the exact required set (currently 4 vars including the R13 commit 30 J3-claim-email addition); the skeleton's `--help` text + env-validation block read FROM that row, MUST name every var in it verbatim (do NOT inherit a subset by shorthand), and MUST abort with a predicate-(k)-shaped diagnostic if the J3-claim-email var (named in §9.1.2 row) is unset OR matches the canonical rejected-domain set, per Task 0.C.3 fixture-build TS guard + Task 0.C.4 mint RPC defense-in-depth. See also plan §0.A.5 `.env.local.example` template for the literal-value placeholders the dev fills in locally.
...
BEGIN
  ...
  --
  -- R57 commit 95 F49 amendment — mint RPC NEVER writes last_seed_date; only
  -- validation_finalize_all_atomic writes it (per R55 Option b + R57 F49 fix).
  -- The column is omitted from the INSERT column-list AND value-list entirely:
  -- the singleton's first creation leaves last_seed_date NULL (per plan 02 DDL
  -- R57 nullable amendment). Predicate (b) at check-seed treats NULL as stale.
  -- The ON CONFLICT DO UPDATE SET clause likewise omits last_seed_date (this
  -- was already the contract pre-R57; verified). Rationale: F49 closed the
  -- INSERT-side bypass where the initial singleton creation by a single-combo
  -- reseed would have stamped last_seed_date = $validationTodayIso without the
  -- finalizer ever running, falsely PASSing check-seed predicate (b) under
  -- --combo all dispatch + bypassing the F47 CAS contract scoped to the
  -- finalizer. The two-RPC contract is now: mint writes everything EXCEPT
  -- last_seed_date; finalizer writes last_seed_date EXCLUSIVELY.
  ...
  VALUES (
    'validation_seed',
    -- last_seed_date deliberately omitted — NULL on initial INSERT per R57 F49 fix.
    ARRAY[p_combo],
  ...
  ON CONFLICT (key) DO UPDATE SET
    ...
    -- (R57 commit 95 F49 amendment re-confirms the original two-RPC contract —
    -- the ON CONFLICT side already complied; the gap was the initial INSERT side).
...
- [ ] **Step 6: Modify `scripts/validation-reseed.ts`** to call ONLY these RPCs (no direct PostgREST mutation of shows / crew_members). The script:
  ...
  - **For `--combo <single>` (R55 commit 94 F48 amendment — single-combo dispatch lockstep):** the script invokes `mint_validation_fixture_atomic` for the single requested combo and does NOT call `validation_finalize_all_atomic`. The mint RPC stamps `combos_seeded_dates[<single>] = $validationTodayIso` per its section 4 ON CONFLICT block; `last_seed_date` is deliberately NOT touched (it remains the "all-combos completion stamp" — see spec §3.3.2 predicate (b) scope rationale). Single-combo check-seed (`pnpm validation:check-seed --combo <single>`) dispatches to predicate (b'), which reads the per-combo stamp directly — closing the F48 gap that would otherwise block smoke 6's `reseed --combo R1` → `check-seed --combo R1` prerequisite on a fresh/next-day stack. **F47 CAS interaction (R53 commit 93):** Option (b)'s design leaves the F47 compare-and-swap pattern unchanged — CAS still scopes to `validation_finalize_all_atomic` exclusively (called only on `--combo all`). Single-combo reseed inherits the mint RPC's per-show advisory lock (AGENTS.md invariant 2) for its writes; there is no TOCTOU surface on `last_seed_date` for the single-combo path because that column is never written by mint.
...
Per spec §3.3.2 singleton write semantics. **12 predicates (a, b, b', c-g, i, k, l, m, n)** — the picker-fixture lockstep is simpler than the pre-M11.5 contract (no per-crew JWT versioning + no revoked-link table to police); R13 commit 30 adds predicate (k); R13 commit 31 adds predicate (l); R17 commit 39 adds predicate (m); R27 commit 57 adds predicate (n); **R55 commit 94 adds predicate (b') and re-scopes predicate (b) to `--combo all` only (F48 single-combo vs all-combo `last_seed_date` semantic gap); R57 commit 95 extends predicate (b) to treat `last_seed_date IS NULL` as stale (F49 INSERT-side bypass closure — mint RPC's initial INSERT now omits `last_seed_date` per R57 nullable DDL, so first-singleton-creation state is NULL and predicate (b) must accept that as a stale signal to remain the "finalizer-exclusive" gate)**. **Dispatch logic:** the check-seed CLI parses its `--combo` flag once at startup. If the value equals `all` (the canonical multi-combo form per `scripts/validation-reseed.ts` Step 6 contract), the predicate evaluator runs (a, b, c, d, e, f, g, i, k, l, m, n) — predicate (b) fires on stale `last_seed_date`; predicate (b') is skipped. If the value is any single combo enum (`R1`, `R2`, ..., `R8b`, `SW-PRE_TRAVEL`, ..., `SW-POST_SHOW`), the evaluator runs (a, b', c, d, e, f, g, i, k, l, m, n) — predicate (b) is skipped; predicate (b') fires on stale `combos_seeded_dates[<single>]`. Predicate (i) runs in both branches over the requested combo set (single-element set for `--combo <single>`; 16-element set for `--combo all`).

- (a) `validation_state` row missing (zero rows for `key='validation_seed'`)
- (b) **(R55 commit 94 F48 amendment — `--combo all` only; R57 commit 95 F49 amendment — NULL handling)** Applies ONLY when invoked as `check-seed --combo all`: `last_seed_date IS NULL OR last_seed_date != $VALIDATION_TODAY_ISO` (where `$VALIDATION_TODAY_ISO` is the canonical UTC YYYY-MM-DD value the script computes, NOT Postgres `current_date`). **NULL means `validation_finalize_all_atomic` has never run (initial singleton state per R57 F49 fix — mint RPC never stamps last_seed_date); treat as stale.** **Scope rationale:** `last_seed_date` is the "all-combos completion stamp" — written exclusively by `validation_finalize_all_atomic` after every per-combo mint succeeds. Single-combo reseed (`--combo R1` / `--combo SW-POST_SHOW` / etc.) does NOT call the finalizer (per Step 6 above) and does NOT touch `last_seed_date`, so applying predicate (b) to a single-combo check-seed would falsely fire on a fresh/next-day stack (F48). Predicate (b') below handles single-combo dispatch. **Diagnostic when NULL:** "validation_state.last_seed_date IS NULL — `validation_finalize_all_atomic` has never executed; run `pnpm validation:reseed --combo all` to perform the full all-combos reseed which calls the finalizer." **Diagnostic when stale:** "validation_state.last_seed_date = <stamp> != $VALIDATION_TODAY_ISO — re-run `pnpm validation:reseed --combo all` to refresh."
- **(b') (R55 commit 94 F48 amendment — single-combo freshness)** Applies ONLY when invoked as `check-seed --combo <single>` where `<single>` is any non-`all` combo enum value (e.g., `R1`, `R7b`, `SW-POST_SHOW`): `combos_seeded_dates[<single>] != $VALIDATION_TODAY_ISO`. Reads the per-combo stamp written by `mint_validation_fixture_atomic`'s section 4 ON CONFLICT DO UPDATE block. Catches the F48 failure mode where smoke 6's prerequisite (`reseed --combo R1` then `check-seed --combo R1`) would otherwise block on stale `last_seed_date` from an earlier day's all-combos run (or a never-run finalizer). Diagnostic: "validation_state.combos_seeded_dates['<single>'] = <stamp> != $VALIDATION_TODAY_ISO — re-run `pnpm validation:reseed --combo <single>` to refresh the per-combo stamp." Note: predicate (i) below ALSO compares `combos_seeded_dates[combo]` to `$VALIDATION_TODAY_ISO` but its scope is partial-`--combo all` (iterates over a requested set); predicate (b') is the single-combo dispatch surface — the implementations may share helper code but the diagnostics name the predicate that fired.
...
- [ ] **Step 1: Write failing test:** check-seed returns exit 0 immediately after a fresh `--combo all` reseed; returns exit 1 if `VALIDATION_SUPABASE_PROJECT_REF` env var is set to a wrong value; returns exit 1 if a `show_share_tokens` row is manually deleted for one of the seeded shows (predicate g); returns exit 1 if `VALIDATION_J3_CLAIM_EMAIL` is unset OR matches a placeholder reserved domain (predicate k, per R13 commit 30); returns exit 1 if combo R1's alias_5a_lead row in `crew_members` has a placeholder email (predicate k DB-side check); returns exit 1 if ANY baseline picker alias has `claimed_via_oauth_at IS NOT NULL` post-reseed (predicate l, per R13 commit 31 — test this by manually `UPDATE public.crew_members SET claimed_via_oauth_at = now() WHERE ...` then re-running check-seed); returns exit 1 if a manually-INSERTed stale `crew_members` row exists for a seeded validation show whose `name` is not enumerated in `validation_state.alias_map[combo]` (predicate m, per R17 commit 39 + R19 commit 42 F18 case-normalization fix — test this by manually `INSERT INTO public.crew_members (show_id, name, role, ...) VALUES ((SELECT id FROM shows WHERE drive_file_id='validation_R1'), 'orphan_stale_lead', 'LEAD', ...)` — note **UPPERCASE** `R1` matches the mint RPC's `'validation_' || p_combo` formula at Step 2 above — then re-running check-seed; the predicate (m) DIAG should name the orphan row). **R27 commit 57 F27 regression — predicate (n) shows baseline-eligibility guard:** returns exit 1 if a seeded validation show has `archived = true` (manually-archived during exercise) — test this by manually `UPDATE public.shows SET archived = true WHERE drive_file_id = 'validation_R1'` then re-running check-seed BEFORE reseed; expect exit 1 with predicate (n) DIAG. Then run `pnpm validation:reseed --combo R1` and re-run check-seed; expect exit 0 with `archived` restored to false (the R27 SET-clause amendment refreshes archived + published on every reseed). Symmetric test for `published = false`. Additionally: a `pnpm validation:reseed --combo R1` immediately followed by `pnpm validation:check-seed --combo R1` MUST resolve a non-null R1 show_id via predicate (m)'s `'validation_' || C` lookup and PASS — this exercises the F18 case-normalization round-trip end-to-end (mint writes `validation_R1`; check-seed predicate (m) reads `'validation_' || 'R1'`; the two must match). **R55 commit 94 F48 regression — single-combo dispatch on stale `last_seed_date` stack:** test fixture-sets `last_seed_date` to a stale value (e.g., yesterday's ISO via service-role `UPDATE public.validation_state SET last_seed_date = (current_date - interval '1 day')::date WHERE key = 'validation_seed'` after a baseline `--combo all` reseed succeeded today and was then reverted by the fixture); runs `pnpm validation:reseed --combo R1` (which stamps `combos_seeded_dates['R1'] = $VALIDATION_TODAY_ISO` per the mint RPC's section 4 ON CONFLICT block but does NOT call the finalizer, so `last_seed_date` stays at yesterday's value); runs `pnpm validation:check-seed --combo R1` and asserts exit 0 (predicate (b') reads `combos_seeded_dates['R1']` and matches today; predicate (b) is skipped because dispatch is single-combo). Symmetric negative: runs `pnpm validation:check-seed --combo all` against the same stale-`last_seed_date` stack and asserts exit 1 with predicate (b) diagnostic naming `last_seed_date != $VALIDATION_TODAY_ISO` (predicate (b) fires under `--combo all` dispatch; predicate (b') is skipped). **Concrete failure mode the F48 regression catches:** a future amendment collapses the dispatch logic back to "predicate (b) applies to any requested set" → smoke 6's `check-seed --combo R1` falls back to the global predicate (b) reading the all-combos-scoped `last_seed_date` → fresh/next-day single-combo reseeds block until a full `--combo all` reseed runs, contradicting the spec §3.3.2 R55-amendment single-combo dispatch contract. The test's invariant is "predicate (b) and predicate (b') are mutually exclusive at dispatch time and read different singleton columns" — both halves (`--combo R1` PASSes with stale `last_seed_date`; `--combo all` FAILs with the same stale stack) MUST hold; if either branch flips, the dispatch is broken. **R57 commit 95 F49 regression — mint RPC initial INSERT MUST NOT stamp `last_seed_date` (F48-class round 2 same-vector closure):** test fixture starts from a singleton-deleted state (`DELETE FROM public.validation_state WHERE key = 'validation_seed'` via service-role psql — wiping the validation_state row entirely so the next mint RPC call hits the INITIAL INSERT path, not the ON CONFLICT DO UPDATE path); runs `pnpm validation:reseed --combo R1` (mint RPC's INSERT must omit `last_seed_date` from its column-list per R57 fix); asserts `SELECT last_seed_date FROM public.validation_state WHERE key = 'validation_seed'` returns NULL (proves mint RPC did NOT stamp the all-combos completion column on initial singleton creation); asserts `SELECT combos_seeded_dates->>'R1' FROM public.validation_state WHERE key = 'validation_seed'` returns today's `$VALIDATION_TODAY_ISO` value (proves single-combo data WAS written by mint per its section 4 ON CONFLICT block, just NOT the last_seed_date column); runs `pnpm validation:check-seed --combo R1` and asserts exit 0 (predicate (b') reads `combos_seeded_dates['R1']` which matches today; predicate (b) is skipped under single-combo dispatch); runs `pnpm validation:check-seed --combo all` and asserts exit 1 with predicate (b) diagnostic naming `last_seed_date IS NULL — validation_finalize_all_atomic has never executed` (predicate (b) fires under `--combo all` dispatch on NULL `last_seed_date`); then runs `pnpm validation:reseed --combo all` (full all-combos path which calls `validation_finalize_all_atomic` per Step 6 contract); asserts `SELECT last_seed_date FROM public.validation_state WHERE key = 'validation_seed'` returns today's date (proves the finalizer stamped post-`--combo all`); asserts `pnpm validation:check-seed --combo all` now exits 0 (predicate (b) PASSes). **Concrete failure mode the F49 regression catches:** a future amendment re-introduces `last_seed_date` into the mint RPC's initial INSERT column-list (e.g., `INSERT INTO public.validation_state (key, last_seed_date, ...) VALUES ('validation_seed', v_validation_today_iso::date, ...)`) → first singleton creation by a single-combo reseed stamps the all-combos completion column without the finalizer ever running → `check-seed --combo all` PASSes with NO `validation_finalize_all_atomic` invocation → F47 CAS contract is bypassed because CAS scopes exclusively to the finalizer → walk-session gate falsely advances on a partial seed. The test's invariant is "the mint RPC's initial INSERT path produces a singleton with `last_seed_date IS NULL`; only `validation_finalize_all_atomic` ever writes a non-NULL value into that column." If the assertion `last_seed_date IS NULL` post-`reseed --combo R1` from a wiped state flips to `last_seed_date = today`, the bypass is reopen. The test also covers the symmetric "wiped + reseed --combo all" path to confirm the finalizer DOES stamp (positive control); both halves must hold for the F49 invariant. Commit alongside the F47/F48 regressions.

- [ ] **Step 2: Run — expect FAIL** (no implementation).

- [ ] **Step 3: Implement** all 12 predicates (a, b, b', c-g, i, k, l, m, n) with the R55 commit 94 dispatch logic: `--combo all` evaluates (a, b, c-g, i, k, l, m, n); `--combo <single>` evaluates (a, b', c-g, i, k, l, m, n). Stdout on success: `OK: seed matches today (combos: R1,R2,...,SW-POST_SHOW)`. Stderr + exit 1 on failure: human-readable diagnostic naming the failed predicate (including `(b)` vs `(b')` so the dispatch surface that fired is unambiguous in logs).

- [ ] **Step 4: Run — expect PASS.** Commit:

```bash
git add scripts/validation-check-seed.ts tests/scripts/validation-check-seed.test.ts
git commit -m "feat(validation): implement check-seed with 12 picker-fixture predicates (a, b, b', c-g, i, k, l, m, n incl R17 full-replace orphan guard + R27 baseline-eligibility guard + R55 single-combo b' dispatch)"
```
...
**Authoring contract (from `DEFERRED.md M12-PHASE0C-TZ-PIN-METATEST`; R27 commit 56 F26 scope extension):** grep every `.sql` migration that lands in this Phase 0.C (`supabase/migrations/*mint_validation_fixture_atomic.sql` + `*validation_finalize_all_atomic.sql` + any other Phase-0.C-authored migration) AND every `.ts` script in `scripts/validation-*.ts` for the lowercase string `current_date`. Each match MUST be either (a) inside the bounded-skew sanity check (`abs(DATE_TEXT::date - current_date) > 1` — integer day comparison; the R5 narrative cited `abs(extract(epoch from ...::date - current_date)) > 86400` but that's invalid SQL — `date - date` returns INTEGER not interval; corrected per R11 F9 fix above in this file), OR (b) carry an inline `// not-validation-today-iso: <reason>` / `-- not-validation-today-iso: <reason>` waiver comment. Default: "TZ-pinned `validationTodayIso` wins; `current_date` is for skew-check only." **R27 commit 56 F26 extension — check-seed contract assertion.** Additionally scan `scripts/validation-check-seed.ts` for any predicate body comparing `last_seed_date` (predicate (b)) or `combos_seeded_dates[...]` (predicate (i)) against `current_date` rather than `$VALIDATION_TODAY_ISO`. Spec §3.3.2 predicate (b) was authored against `current_date` pre-R27; R27 commit 56 retired that framing in favor of `$VALIDATION_TODAY_ISO` to align with predicate (i)'s TZ-pin discipline. Each check-seed predicate that compares a date column against `current_date` MUST carry the same waiver semantics OR be rewritten against `$VALIDATION_TODAY_ISO`. The scan target now covers BOTH live `.sql` + `.ts` files AND the spec/plan markdown that names the predicate bodies (catches drift where a future amendment slips `current_date` back into a check-seed predicate while leaving the live script aligned).
...
- **crew_members UPSERT fails on email canonicalization CHECK.** Master spec X.5 requires email canonicalization (AGENTS.md invariant 3); the canonicalize helper at `lib/email/canonicalize.ts:2-6` performs `raw.trim().toLowerCase()` ONLY — it does NOT strip plus-aliases or perform any other transformation. So `validation+5a@example.com` canonicalizes to ITSELF (already lowercase + no outer whitespace); the `+` and the `5a` alias segment are preserved. The reseed script canonicalizes in TS via this helper before sending to the RPC, so the CHECK should pass for any well-formed synthesized alias email. If the CHECK does fire, the failure mode is one of: (i) a payload assembly path that bypassed `canonicalize()` and supplied a raw value with mixed case or stray whitespace; (ii) a malformed alias value injected by the fixture-build (e.g., embedded whitespace from a env-var read); (iii) the schema CHECK constraint shape itself drifted from `email = canonicalize(email)`-equivalent — re-read the `crew_members` table DDL and confirm the constraint matches `email = lower(trim(email))` (the safety-net DDL form per AGENTS.md invariant 3). It is NOT a "the helper changed shape and started stripping plus-aliases" failure — the helper has never stripped plus-aliases. **R49 commit 88 F44 amendment + R51 F46 amendment:** this entry previously claimed `validation+5a@example.com` canonicalized to `validation@example.com` via strip-plus — that claim was FALSE and is corrected here; cross-reference `01-phase0-infra.md:30,48` for the parallel R49 fix on the admin-bootstrap procedure.
