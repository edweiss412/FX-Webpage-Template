...
- **Containment:** the broadened guard makes the debt SHRINK-ONLY — any NEW e2e file (or any new mutation site in a clean file) fails CI immediately, and the stale-exemption assertion forces list removal the moment a file is cleaned up. New locked-table fixture mutations must go through `lockedCrewRestriction.ts` or a sibling locked-psql helper.
