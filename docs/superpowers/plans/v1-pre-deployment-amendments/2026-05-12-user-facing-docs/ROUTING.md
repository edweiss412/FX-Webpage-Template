...
**Inputs:** M11 spec at `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md`; plan phases A–I in this dir; project-wide routing rules from the FXAV crew-pages plan ([`../../2026-04-30-fxav-crew-pages-v1/ROUTING.md`](../../2026-04-30-fxav-crew-pages-v1/ROUTING.md)); UI hard rule from `AGENTS.md`; lessons from M0–M10 + M6.5.
...
| **E — Content authoring** (Tasks E.1–E.13) — **CLOSED 2026-05-20** at SHA `ba2ae36` ([handoff](handoffs/E-content.md)) | Author 13 MDX pages: 4 adoption-track + 7 capability-reference + `/help/tour` + `/help/errors` | **Opus / Claude Code** | Codex | All content + `.mdx` files. Heavy `impeccable` `clarify` / `shape` / `polish` territory. Doug is the audience; voice + IA matter. All-Opus. **Convergence:** 13 per-page external impeccable §1.8 dual-gates (all PASS) + Codex 11 rounds (R1 + R3 + R5-R11 fresh-eyes; R11 explicit Phase-E-own-surface APPROVE-equivalent). 5 DEFERRED entries (M11-E-D1..D5) with concrete M9.5 / Phase F.10 / BACKLOG re-open triggers. 5 structural meta-tests landed: anchor-resolver, UI-label crosswalk (heuristic + 34-entry declaration registry), screenshot asset existence (auto-activates with Phase F), catalog biconditional + target-class routing, catalog copy hygiene. Cross-CLI adversarial-review fresh-eyes-framing hook installed at `~/.claude/hooks/check-codex-adversarial-review-fresh-eyes.sh` mid-phase. **Close-out structural coverage:** `tests/e2e/help-pages.spec.ts` (added 2026-05-20) navigates all 13 `/help/*` URLs as admin under `mobile-safari` and asserts 200 + documented H1 + sidebar chrome + zero `pageerror` events, retiring the prior manual user-review smoke gate per the M11/M12 boundary split (M11 owns structural correctness; M12 owns experiential UX validation). |
...
Both phases have a single pin-stop where the backend signature stabilizes before the UI/co-located work consumes it. Pattern matches the M5/M8/M10 single-pin-stop rule documented in `../../2026-04-30-fxav-crew-pages-v1/HANDOFF-TEMPLATE.md` §0:
