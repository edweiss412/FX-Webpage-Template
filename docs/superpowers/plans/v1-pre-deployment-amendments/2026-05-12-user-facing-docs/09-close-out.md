...
**Scope:** The required quality gates before M11 ships. `/impeccable critique` + `/impeccable audit` on every `/help/*` page (AGENTS.md invariant #8). Cross-CLI adversarial review of the assembled M11 work (per AGENTS.md writing-plans additions). Final M11 handoff doc capturing findings + dispositions.
...
- Modify: `handoffs/M11-help.md` (Phase I.3 creates this; Task I.1 logs findings + dispositions in §12 per the project's handoff convention).
...
For M11, the relevant pages and components:
...
- The §5.6 affordance-matrix retrofits (testids + Learn-more links inside existing M3/M9/M10 components — those should already have impeccable run for their owning milestones, but the M11 retrofit may introduce new visual elements that warrant a re-pass)
...
Scope: the diff produced by Phases A – H. Cache the critique output in `handoffs/M11-help.md` §12 — list findings by severity.
...
Same scope. Cache the audit output in `handoffs/M11-help.md` §12 alongside the critique findings.
...
- **OR explicitly defer via `DEFERRED.md`** (matching the existing crew-pages plan's `DEFERRED.md` pattern at `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/DEFERRED.md`). For M11, the canonical deferral file path is `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/DEFERRED.md` (mirror the crew-pages pattern under the M11 plan directory). Acceptable for findings that are real but better-scoped to a follow-up milestone.
...
```bash
git add docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/handoffs/M11-help.md docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/DEFERRED.md
git commit -m "docs(handoff): M11 impeccable critique + audit findings (Task I.1)"
```
...
**Files:**
- Modify: `handoffs/M11-help.md` (record convergence log)

Per AGENTS.md writing-plans additions: cross-CLI adversarial review is **mandatory** between self-review and execution handoff. For this plan, the execution-handoff has already happened (Phase A – H is the execution); I.2 retroactively reviews the assembled M11 work against the canonical spec.

**Invocation (r2 fix per I-r1 finding 2 — pin the milestone base SHA so the final APPROVE attests to the full M11 diff, not just the latest fix):**

```bash
# 1. Capture the M11 base SHA — the commit BEFORE M11's first plan scaffold
#    landed. The first M11 commit is `b30f522` ("M11 plan scaffold — README
#    + overview + Phase A foundation"), so the base is its parent:
M12_BASE=$(git rev-parse b30f522^)
echo "M11 base: $M12_BASE"
...
#    M11 diff (not the previous-round fix-base, which would hide drift
...
/codex:adversarial-review --wait --base "$M12_BASE" --scope branch \
  "M11 Phase I final fresh-eyes review: audit the entire M11 plan + implementation against spec r14, all 17 §7.1 tests, AC-11.1 through AC-11.39, and every plan-wide invariant from AGENTS.md. Return verdict + findings."

# 3. Each retry MUST keep the same --base so the cumulative APPROVE attests
#    to the complete milestone. If M11's first commit SHA changes (e.g.,
#    history rewrite), update `b30f522` above to the new first-M11-commit
...
The reviewer should anchor on the spec's watchpoints + the 9-round spec review history. Specific watchpoints for the M11 *implementation* review:
...
Follow the project's canonical adversarial-review invocation. Capture output to `handoffs/M11-help.md` §10 (Convergence Log).
...
```bash
git add docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/handoffs/M11-help.md
git commit -m "docs(handoff): M11 adversarial-review round N findings + fixes (Task I.2)"
```
...
### Task I.3: M11 handoff doc

**Files:**
- Create or finalize: `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/handoffs/M11-help.md`

The handoff doc is the canonical record of M11's execution. Mirror the existing handoff pattern at `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/handoffs/`.

**r3 fix per I-r2 finding 2 (MEDIUM):** when copying `HANDOFF-TEMPLATE.md` to `handoffs/M11-help.md`, IMMEDIATELY update the template's Spec version line. The template (post-r3) shows r14 + the r11-r14 amendment lineage but still requires the implementer to fill in the actual r14 commit SHA via:

```bash
SPEC_SHA=$(git log -n1 --format="%h" -- docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md)
echo "Spec r14 commit: $SPEC_SHA"
# Then edit handoffs/M11-help.md's Spec version line to replace the
...
1. **Spec sections in scope** — list every §X reference the M11 work touched
2. **AC list** — every AC-11.* with PASS / DEFERRED / N/A annotation
3. **Amendments in scope** — none for M11 (spec r14 is canonical (incorporating r11/r12/r13/r14 amendments: STALE_MANUAL_REPLAY_ABORTED classification, parser canonical-derivation, predicate single source of truth, Screenshot `name` prop). Plan-amendments documented inline per task; aggregate list in 00-overview if needed)
...
9. **Performance & bundle impact** — `pnpm build` artifact size delta vs. pre-M11 baseline; any concern with the new @next/mdx + sharp deps
10. **Convergence log** — round-by-round (from Task I.2)
11. **Routing decision update** — confirm M11 is in `ROUTING.md` with Opus as implementer + Codex as reviewer
...
# state proves AC-11.1 — every page compiles to RSC chunks. Phase A's stub-
...
Expected: all green; `pnpm build` completes without errors. This is the AC-11.1 post-content build proof, distinct from Phase A.1's stub-era build.

- [ ] **Step 2: Write `handoffs/M11-help.md`**

Use HANDOFF-TEMPLATE.md (in the plan dir) as a template. Fill in every section.

- [ ] **Step 3: Update `ROUTING.md`**

Add the M11 row to the project's main `ROUTING.md`:

```
| M11 | User-facing /help docs | Opus / Claude Code | Codex (cross-CLI) | UI surface — invariant #8 (impeccable v3 gate); UI-always-Opus rule |
```

- [ ] **Step 4: Commit + close-out announcement**

```bash
git add docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-12-user-facing-docs/handoffs/M11-help.md docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/ROUTING.md
git commit -m "docs(handoff): M11 final handoff doc + routing update (Task I.3 — milestone close-out)"
```
...
- [ ] `handoffs/M11-help.md` is complete with all 12 sections filled
- [ ] `ROUTING.md` includes M11
- [ ] **M11 ships.**
...
- [ ] **`pnpm build` succeeds against the post-content state** (AC-11.1 final proof; r10 — distinct from Phase A.1's stub-era build)
...
- [ ] M11 handoff doc complete + routing updated
