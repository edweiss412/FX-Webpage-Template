...
### M11-G-D-6: M12.2 dashboard redesign orphaned two §5.6 matrix testids + the deep-link walker is not in CI (filed 2026-06-10) — ✅ RESOLVED 2026-06-11 (M12.12)

**Resolution (2026-06-11, M12.12 affordance-matrix realignment):** full reconciliation shipped. The §5.6 matrix is now a 19-concrete-row table with a `visibleAt` field (spec amendment ratified at `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md` §5.6, design at `docs/superpowers/specs/v1-pre-deployment-amendments/2026-06-11-affordance-matrix-realignment-design.md`); all six broken rows were re-pointed onto the live surfaces (five renames + the needs-attention desktop/page two-row split); 7 previously-unregistered tooltips were registered with Learn-more links; the deep-link walker runs green locally at both viewports (registration unconditional, runtime skip by project); and two CI gates now enforce the contract — the parity meta-test (`tests/help/_metaAffordanceMatrixParity.test.ts`, x-audits job `affordance-matrix-parity`, every PR) and the `help-affordances.yml` two-viewport walker workflow (path-filtered PRs + dispatch). Shipped in PR #24 (merge 5d60638e). Original entry preserved below.
...
### M11-G-D-1: §5.6 matrix row `help-affordance--dashboard-restage-badge--tooltip` defers UI delivery — **UPDATED 2026-06-10: folded into M11-G-D-6 cluster** — ✅ RESOLVED 2026-06-11 (M12.12)

**Resolution (2026-06-11, M12.12 affordance-matrix realignment):** shipped as part of the M11-G-D-6 cluster reconciliation. The restage affordance ships as the **conditional legend under ShowsTable** (owner-selected option C): the matrix row was renamed `help-affordance--dashboard-restage-badge--tooltip` → `help-affordance--dashboard-restage--legend` with an affordance-kind change from `?` tooltip to legend link (a button-based tooltip cannot legally nest inside ShowsTable's whole-row `<Link>`), targeting `/help/admin/review-queues#re-stage`, walked green at both viewports and enforced by the same parity meta-test + `help-affordances.yml` CI gates. Spec §5.6 amendment ratified alongside. Shipped in PR #24 (merge 5d60638e). Original entry preserved below.
...
- **"Revoke all links" + "Issue new link"** — v1-blocking (security/recovery). ✅ **M9.5 SUBSET RESOLVED 2026-05-21 (tag `m9.5-completed` at SHA `ad4826e`).** Both affordances shipped via M9.5 (handoff at [`../../2026-04-30-fxav-crew-pages-v1/handoffs/M9.5-signed-link-controls.md`](../../2026-04-30-fxav-crew-pages-v1/handoffs/M9.5-signed-link-controls.md); plan at [`../../2026-04-30-fxav-crew-pages-v1/handoffs/M9.5-plan.md`](../../2026-04-30-fxav-crew-pages-v1/handoffs/M9.5-plan.md)). AC-9.5-1 through AC-9.5-6 met. 8-round Codex adversarial review converged APPROVE at R8; impeccable v3 dual-gate APPROVED in 2 rounds. The `/help/admin/sharing-links` spec-vs-shipped gap closed for these two labels.
...
- **Symptom:** Page documents UI controls per FXAV master spec §7.2 / §5.2: "Issue first link", "Issue new link", "Revoke all links", "Copy share link". These labels are extensively documented in `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` at lines 239, 241, 374, 1091, 1100, 1110, 1953, 1959, 1971, 1975. Grep of `app/` + `components/` for these labels returns **zero matches** — M9 was supposed to ship the controls but the labels are not in shipped code.
...
**STATUS: RESOLVED 2026-06-21** (test-hardening bundle). The runtime DOM crawl in `tests/e2e/no-raw-codes.spec.ts` is the structural guard that supersedes a static MDX AST pass: MDX compiles to ordinary HTML text nodes + user-visible attributes, which `collectRawCodeLeaksInPage` already walks. Two things landed: (1) the underlying coverage bug was still live — `discoverStaticAppRoutePaths()` called `walkSourceFiles(["app"])` with the default `.ts`/`.tsx` extensions, so the `endsWith("/page.mdx")` filter was dead code and the 13 `/help/**/page.mdx` routes had NEVER been crawled; fixed by passing `{ extensions: [".tsx", ".mdx"] }` (completes fix path (b)). (2) Added MDX fixtures (`tests/cross-cutting/fixtures/no-raw-codes/mdx/{bad,good}-help-page.mdx`) + a `test.describe("M11-A-D3 ...")` block that compiles them through the same `@mdx-js/mdx` pipeline `@next/mdx` uses, renders to DOM, and asserts the crawl catches a raw `SHEET_UNAVAILABLE` in BOTH the compiled `<p>` text node and the `<abbr title=...>` attribute, plus a clean-MDX negative control. The route-discovery test pins `.mdx` inclusion so a regression dropping it fails. A static MDX AST pass is therefore unnecessary — the runtime crawl covers the surface. Original entry preserved below.
...
- **Cross-reference:** `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/handoffs/X2-no-raw-codes.md` — the X.2 implementer should add this finding to its scope.
