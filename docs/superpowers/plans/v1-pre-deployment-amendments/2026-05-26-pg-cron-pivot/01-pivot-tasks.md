...
- [ ] **Step 0: TDD red — author T5's dedicated test file at the T5 commit boundary (R7 F17 + R8 F20 + R9 F23 fix).** **Create new test file** `tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts` (NOT in pg-cron-pivot-doc-guard — that ships in T4 before T5 edits exist; commingling causes T4-commit CI failure per R9 F23). The test contains 9 positive assertions covering BOTH M12 plan files T5 modifies.
  ...
  Run `pnpm test tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts` against HEAD `ac752d9` → expect all 18 assertions (A-R) to FAIL (the M12 plan tree + M12 spec + master spec haven't been amended yet). This is the TDD-red phase that authorizes the T5 doc edits to land. Per AGENTS.md invariant 1.
...
- [ ] **Step 5a: Extend pg-cron-pivot-amendment doc-guard with assertions J + K + L + M + N + O + P + Q + R (R11 F30 + R18 F39 + R29 F57 + R31 F60 fix).** Add 9 assertions to T5's `tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts`:
  ...
  - Assertion R (R31 F60 structural defense — full-surface sweep): master spec contains zero matches for `/seven required|SEVEN required|all seven|the seven/` outside HISTORICAL audit-trail contexts. Closes the X6-D-1-reopen-time regression vector at the master-spec-prose layer.
  
  All 9 FAIL at HEAD (stale text present + M12 spec section not yet added + master spec inventory not yet amended + master spec branch-protection-surface not yet swept); PASS after step 4a + step 4b + step 4c + step 4c.1 edits. T5 doc-guard now has 18 assertions (was 9: A-I, now A-R with J/K/L/M added in R11/R18, N/O/P added in R29, Q/R added in R31).
