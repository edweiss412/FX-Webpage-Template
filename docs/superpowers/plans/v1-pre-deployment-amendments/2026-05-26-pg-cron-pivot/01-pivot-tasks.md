...
- [ ] **Step 3: Migration idempotency check (R20 F42 + R21 F44 fix — execute SQL directly with ON_ERROR_STOP).** R3 F8 established that `npx supabase db push` only applies PENDING migrations; re-running it after T2.1 is tracked produces a no-op at the Supabase CLI layer, NOT a SQL re-execution. R21 F44: psql `-f` without `-v ON_ERROR_STOP=1` doesn't fail the process on SQL errors — the script prints errors but still exits 0, giving false-green idempotency. The repo's existing psql helpers all use `ON_ERROR_STOP=1` (verified at `tests/cross-cutting/email-canonicalization.test.ts` + `tests/reports/quota.test.ts`). Execute migration SQL directly + fail-loud on error: `psql "$TEST_DATABASE_URL" -v ON_ERROR_STOP=1 -f supabase/migrations/<ts1>_enable_pg_net.sql`. Non-zero exit blocks the task; only exit 0 + the post-check below count as passing. The `if not exists` clause in the migration body should make this a no-op at the PostgreSQL layer — re-running the SQL produces no errors and no state change. Confirm via `select extname from pg_extension where extname = 'pg_net';` still returns one row.
- [ ] **Step 4: Commit.**
...
- [ ] **Step 1: Finalize pg-cron-coverage.test.ts (R10 F25 fix — incremental ownership).** The test file exists from T2.1 with Layer 0a, was extended in T2.2 with Layer 0b, and extended in T3 with the 7-job assertion. T4.2 finalizes by (a) refactoring the T3 inline JOB_TABLE to read from the new `pg-cron-jobs.json` canonical source, and (b) adding the anti-tautology negative-regression assertion + step 3 procedure below. Final state asserts:
  ...
  3. For each row: jobname is in JOB_TABLE; schedule matches byte-for-byte; command contains the matching `/api/cron/<route>` substring AND contains `vault.decrypted_secrets` AND **contains `net.http_get(` (NOT `net.http_post(`)** AND **(R21 F45 fix)** contains the auth-header shape pattern — specifically: command contains both `headers := jsonb_build_object(` (the named-arg form of pg_net's headers param) AND `'Authorization'` (the literal header name) AND `'Bearer '` (the literal scheme + space prefix) AND `vault.decrypted_secrets` (the secret source) all within the SAME command. A command that reads vault.decrypted_secrets into `params` instead of `headers`, or misspells `Authorization`, or omits the `Bearer ` prefix, or uses a different secret-source would satisfy the route + vault + http_get assertions while every Vercel cron route returns 401. The auth-header-shape assertion catches that class. The http_get assertion is load-bearing: R1 F1 (HIGH, conf 0.97) caught a draft-state bug where the T3 SQL used http_post against GET-only handlers — this assertion pins the verb contract structurally so the class cannot recur silently. The `vault.decrypted_secrets` assertion (NOT `supabase_vault.decrypted_secrets`) is also load-bearing: R2 F4 (HIGH, conf 0.95) caught a draft-state bug where the migrations referenced the extension name instead of the schema name — this assertion pins the Vault-schema contract structurally.
  ...
  5. **Non-fxav cron preservation (R25 F49 amended):** the assertion is NOT "count > 0" (that's tautologically met by the orphaned `cleanup-bootstrap-nonces` cron T3 now also cleans up); instead, capture the BASELINE non-fxav cron set BEFORE T3 applies (snapshot via `select array_agg(jobname order by jobname) from cron.job where jobname not like 'fxav\_cron\_%' escape '\' and jobname != 'cleanup-bootstrap-nonces';` saved to a constant), then assert the SAME set is present AFTER T3 applies. Equality on the snapshot proves T3's cron.unschedule LIKE clause didn't accidentally hit jobs outside its scope, regardless of whether the orphan was present or absent. The `cleanup-bootstrap-nonces` orphan is EXPLICITLY excluded from the snapshot — T3 step 5 unschedules it as a side-effect cleanup (R25 F49), so any post-T3 check of "non-fxav crons unchanged" would otherwise fail if the snapshot captured the pre-T3 state including the orphan.
  6. **Orphan cleanup (R25 F49):** `select count(*) from cron.job where jobname = 'cleanup-bootstrap-nonces';` returns 0 post-T3 (the orphan is removed). M11.5 G3 cutover dropped the cron's target function + table but didn't unschedule the cron itself; T3 step 5 now does that cleanup as a side-effect.
- [ ] **Step 2: Verify it passes post-T3.** `pnpm test tests/cross-cutting/pg-cron-coverage.test.ts`.
- [ ] **Step 3: Anti-tautology verification (negative-regression discipline) + R20 F43 active-gate regression + R21 F45 auth-header regression.** Per `feedback_negative_regression_verification`: a passing test alone doesn't prove the contract; only a failing test against a known-broken state does. **R32 F61 fix — pg_cron parameter name is `job_id`, NOT `jobid`.** Per upstream `citusdata/pg_cron` README (https://github.com/citusdata/pg_cron#altering-a-cron-job), the `cron.alter_job` function signature is `cron.alter_job(job_id bigint, schedule text default null, ...)`. PostgreSQL named-arg syntax requires the exact parameter name. The `cron.job` TABLE column IS named `jobid` (no underscore — selectable via `select jobid from cron.job ...`); the FUNCTION parameter is `job_id` (with underscore). Confusingly distinct. R0-R31 drafts used `jobid := ...` which would error "no such parameter jobid" and silently break the active/auth negative-regression — defeating the exact R20 F43 + R21 F45 structural defense it was meant to verify. **R20 F43 added negative-regression for the active-gate assertion (step 2a):** in the SAME local DB, run `select cron.alter_job(job_id := (select jobid from cron.job where jobname = 'fxav_cron_sync'), active := false);` to flip one fxav_cron_* row to active=false WITHOUT removing it (count stays at 7; jobname/schedule/command match JOB_TABLE; only `active` changed). Run the meta-test → expect step 2a assertion to FAIL. Restore via `cron.alter_job(job_id := ..., active := true);`; re-run → expect PASS. **R21 F45 negative-regression for the auth-header shape:** use `cron.alter_job(job_id := ..., command := <new-command-without-Bearer>);` to replace one fxav_cron_* command with a variant that omits `'Bearer '` from the headers (or moves the secret to `params` instead of `headers`, or misspells `'Authorization'`). Run meta-test → expect step 3 auth-header assertion to FAIL while count + route + vault + http_get assertions still pass. Restore via `cron.alter_job(job_id := ..., command := <original-command>);` (capture the original command before mutation via `select command from cron.job where jobname = 'fxav_cron_sync'` and save to a shell variable). Re-run → expect PASS. Proves the auth-header assertion catches the class of "secret read but not passed correctly to handler." **R3 F8 + R20 F42 fix (conf 0.9):** the original draft used `supabase db push` to "re-apply" the edited migration — but `db push` only applies PENDING migrations (those not in `supabase_migrations.schema_migrations`); already-applied migrations are NOT re-run on push. The corrected procedure uses **live cron-state mutation via `cron.unschedule()`** (no migration file edit) so the live DB enters a known-broken state without touching the migration system:
...
- [ ] **Step 1: Author the prose-guard test.** Walks (a) `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-26-pg-cron-pivot-design.md` (M12.1 delta spec), (b) `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-26-pg-cron-pivot/**/*.md` (M12.1 plan tree), AND (c) `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` (the **parent M12 spec** that T5 amends per spec §2.1 + §2.2 + §2.3 — R27 F53 fix; 11th hit of structural-defense scope class). Asserts NONE of the 8 forbidden patterns appear in any of these surfaces outside allowlisted finding-history contexts.
  
  **R27 F53 fix — parent M12 spec inclusion rationale.** Earlier T4.3 drafts walked only M12.1 surfaces; T5 amends `2026-05-19-solo-dev-ux-validation-design.md` §9.1 / §9.2 / new §9.1.3, making that file a canonical surface future executors read. Without doc-guard coverage there, a T5 edit could reintroduce any of Patterns 1-8 (HTTP POST verb drift, un-joined job_run_details shape, secret-bearer SELECT outside cron body, etc.) while x6 CI still passes. The 8 patterns are all SQL-shape patterns; the parent M12 spec at HEAD `ac752d9` does NOT contain any of them (verified pre-T4-commit via a one-shot rg sweep — see T4.3 step 3), so adding the parent spec to the walked surface is a no-op at T4 commit boundary and a forward-looking defense for the §9.1.3 insertion T5 lands. If the parent spec's existing line 1017 HISTORICAL audit-trail row trips any pattern (it should not — it's Vercel-Cron prose, not SQL-shape drift), allowlist it via the existing inline-waiver mechanism documented in step 2.
  ...
  **R22 F46 fix — patterns-definition table lives in the test file, not in walked plan markdown.** Patterns 4 (non-existent pg_cron columns) and 7 (double-backslash ESCAPE) have regex sources that match their own literal in this plan file — 7th hit of structural-defense-self-inconsistency class (R4 F9, R6 F15, R10 F26, R13 F33, R14 F35, R15 F36, R22 F46). Genuine class closure: ALL patterns + regexes + fixtures live in the self-excluded test file `tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts` as TypeScript const arrays. Plan markdown describes the class semantically only — semantic name + class + why-forbidden in prose, NO regex source, NO literal forbidden patterns.
  ...
  Semantic pattern inventory (regex source lives in test file; see `tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts` `FORBIDDEN_PATTERNS` const):
  
  - **Pattern 1 (Vault schema drift):** flags SQL function-call / table-access against the extension-name schema instead of the correct `vault.*` schema (per Supabase Vault docs: functions/tables live in the `vault` schema; the extension name is only valid inside `create extension` contexts). Caught: R2 F4. Refined for false-positive scoping (must be a function-call/table-access, not bare prose): R13 F33.
  - **Pattern 2 (HTTP POST verb drift):** flags the SQL function-call form of pg_net's POST helper. Cron handlers export GET only; POST → 405. Caught: R1 F1. Refined for false-positive scoping (prose mentions of the verb name allowed; only the `(`-suffixed function-call form fires): R13 F33.
  - **Pattern 3 (jobname-on-job_run_details column-shape drift):** flags pg_cron run-history queries that filter by `jobname` without joining to `cron.job` (the jobname column lives on `cron.job`, not on `cron.job_run_details`). Caught: R3 F7.
  - **Pattern 4 (non-existent pg_cron column-names drift):** flags column names that don't exist on `cron.job_run_details` (the actual columns are `start_time` + `end_time`). Caught: R2 F5.
  ...
  - **Pattern 6 (unescaped LIKE wildcard):** flags PostgreSQL `LIKE` clauses targeting underscore-containing jobname prefixes without an `ESCAPE` clause. PostgreSQL LIKE treats `_` as single-char wildcard; queries need an escape clause to scope literally. Caught: R4 F10. Refined lookahead direction: R10 F26.
  ...
  - **Pattern 8 (secret-bearer reveal via decrypted-secrets SELECT — R28 F55 rewording):** flags SQL SELECT statements that read the decrypted secret column from the Vault decrypted-secrets view outside the cron schedule body context (the schedule body legitimately reads the secret to construct the auth header; any OTHER use exposes the bearer in SQL editor / browser / session history — same exposure class as R23 F47 SQL-statement leak). Allowed contexts: inside a `cron.schedule()` body's `format($body$ ... $body$, ...)` block; inside the `tests/cross-cutting/pg-cron-coverage.test.ts` test file (self-excluded from walk); inside finding-history paragraphs. Caught: R24 F48. Regex source + literal forbidden SQL phrase live in the test file (FORBIDDEN_PATTERNS const array, position 8) — NOT in this plan markdown (per R28 F55: the prior draft embedded the literal SELECT phrase here, which would self-match Pattern 8; 12th hit of structural-defense-self-inconsistency class).
...
- [ ] **Step 2: Allowlist mechanism for finding-history paragraphs.** The spec's R1/R2/R3/R4/R5/R6/R7 finding-history paragraphs INTENTIONALLY cite forbidden patterns as "what was wrong." Allowlist these via:
  ...
  **Self-exclusion (R6 F15 fix, same shape as no-vercel-cron):** the test file itself (`tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts`) is excluded from the walk entirely — its regex definitions necessarily contain the forbidden patterns. Implement via `if (filePath === __filename) continue;` at the walker OR via `SELF_EXEMPT` constant. Walked surfaces (R27 F53 extended): `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-26-pg-cron-pivot-design.md` + `docs/superpowers/plans/v1-pre-deployment-amendments/2026-05-26-pg-cron-pivot/**/*.md` + `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` (parent M12 spec — covers T5's §9.1 / §9.2 / new §9.1.3 amendments). NOT walked: `tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts` itself.

- [ ] **Step 3: Verify it passes at HEAD post-R3-fix (extended for R27 F53).** `pnpm test tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts`. If any of the R1/R2/R3 finding paragraphs trip the assertion despite the allowlist, refine the allowlist regex until clean. **R27 F53 verification:** confirm the parent M12 spec at `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` is in the test's walked-files list (e.g., assert via `expect(walkedFiles).toContain(...)` inside the test) AND that no Pattern 1-8 match fires inside that file at HEAD `ac752d9`. If a pattern fires (unexpected — pre-pivot M12 spec is pre-pg_cron prose and should not contain SQL-shape drift), inspect the offending line; either add an inline waiver per step 2 (HISTORICAL audit-trail row only) or treat as a real finding requiring spec correction before T4 commits.
...
- [ ] **Step 4: Anti-tautology / negative-regression + positive-regression verification (R13 F33 structural-defense calibration).** Same-vector recurrence tracker: "structural defense regex catches its own walked surface" has now hit 4 times (R4 F9 db push regex too broad; R6 F15 self-scan; R10 F26 LIKE/ESCAPE lookahead direction; R13 F33 net.http_post / supabase_vault. bare references in inverse assertions). Comprehensive structural defense: every forbidden pattern must have BOTH a negative-regression test (introduce a violation → expect FAIL) AND a positive-regression test (introduce a legitimate near-violation in a non-finding-history non-SQL-call context → expect PASS). The bidirectional regression-set proves the pattern catches real drift WITHOUT false-positiving on legitimate prose.
  ...
  **R14 F35 fix — regression fixtures live in the test file, NOT in this plan markdown.** Earlier R13-fix inlined paired-case examples here, but this file is in the doc-guard's walked surface; embedding the forbidden patterns as fixtures (even labeled as "negative case") triggers the doc-guard against itself (5th hit of structural-defense-self-inconsistency class — same shape as R4 F9 / R6 F15 / R10 F26 / R13 F33). Comprehensive structural defense for the class: ALL regression fixtures live in the self-excluded test file (`tests/cross-cutting/pg-cron-pivot-doc-guard.test.ts`). Plan markdown describes the discipline; test file owns the literal cases.
  ...
  - **Pattern 8 (secret-bearer reveal via decrypted-secrets SELECT — R24 F48 added, R26 F52 fixture-inventory closure, R28 F55 literal-removal):** negative case = a bare bearer-revealing SELECT (literal fixture in the test file) outside any cron-schedule body (operator diagnostic / spec prose / plan diagnostic ladder — the exposure class); positive case 1 = the same SELECT inside a `cron.schedule('fxav_cron_*', ...)` `format($body$ ... $body$, ...)` body (legitimate auth-header construction); positive case 2 = semantic prose describing the sourcing without the literal SQL phrase (e.g., "reads from Supabase Vault decrypted-secrets view, keyed by fxav_cron_secret name"); positive case 3 = the SELECT inside a finding-history paragraph block (R23/R24/R25/R26 narrative — allowlisted via the same finding-history mechanism as patterns 1-7). All literal fixtures live in the test file per R14 F35 + R28 F55 (this plan markdown is in the doc-guard's walked surface and must not contain the literal forbidden phrase).
...
- Modify: `.github/workflows/x-audits.yml` (add new job `x6-pg-cron-pivot` — naming convention per R28 F56: existing jobs are `x1-catalog-parity` through `x5-email-canonicalization`, NO `audit-` prefix; the traceability inventory parser at `scripts/generate-traceability.ts:193` recognizes required-audit jobs via regex `^x[1-6]-`, so the `audit-` prefix would render the job invisible to the inventory tooling)
...
- [ ] **Step 0: TDD red — author T5's dedicated test file at the T5 commit boundary (R7 F17 + R8 F20 + R9 F23 fix).** **Create new test file** `tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts` (NOT in pg-cron-pivot-doc-guard — that ships in T4 before T5 edits exist; commingling causes T4-commit CI failure per R9 F23). The test contains 9 positive assertions covering BOTH M12 plan files T5 modifies.
  ...
  - Assertion S (R33 F62): M12 spec contains ZERO matches for the regex `/seven canonical|seven CI gates|all seven|the seven|required-blocking/` outside HISTORICAL audit-trail contexts (lines ~1064 + ~1077 in the M12 spec — allowlist via line-range exclusion OR per-line annotation). Closes the 8th hit of CI-gating-enforcement recurrence at the M12-spec-prose layer (mirrors assertion R's coverage on the master spec).
  
  Against `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` (R29 F57 + R31 F60 fix — master spec required-check inventory + full surface sweep):
  ...
  - Assertion Q (R31 F60): `BRANCH_PROTECTION_DRIFT` catalog row (master spec line ~2844) contains backtick literal `x6-pg-cron-pivot` AND does NOT contain "SEVEN required" (case-sensitive — the catalog row's specific phrasing). Sweep the 4-surface master-spec branch-protection copy.
  - Assertion R (R31 F60): master spec contains zero matches for the regex `/seven required|SEVEN required|all seven|the seven/` outside HISTORICAL audit-trail contexts. (The audit-trail HISTORICAL row count for the master spec is small; allowlist via per-line annotation if any historical rows trip.)
  
  Run `pnpm test tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts` against HEAD `ac752d9` → expect all 19 assertions (A-S) to FAIL (the M12 plan tree + M12 spec + master spec haven't been amended yet). This is the TDD-red phase that authorizes the T5 doc edits to land. Per AGENTS.md invariant 1.
...
- [ ] **Step 4b (R18 F39 fix): Amend the parent M12 spec.** Edit `docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md`:
  ...
  - **Full M12 spec required-blocking surface sweep (R33 F62 fix — 8th hit of CI-gating-enforcement recurrence).** The M12 spec was R70 APPROVED before X6-D-1 deferral (2026-05-20 deletion of branch protection); active "required-blocking" / "seven canonical" / "Phase 0 verifies branch-protection" prose at multiple §9.1 / §9.1.1 / §12 surfaces is now stale. M12.1 sweeps these as adjacent surfaces to the gate-#8 amendment (without sweeping, the M12 spec internally contradicts T5 Step 8's X6-D-1 dormancy contract; an executor reading the M12 spec would attempt branch-protection setup that's already deleted). Each surface gets a parenthetical X6-D-1 dormancy note plus 7→8 bump:
    1. **Line ~803** (§9.1 CI gates row): current text "The seven canonical CI gates from `.github/workflows/x-audits.yml` (per master spec X.* lineage) are required-blocking ... Phase 0 verifies branch-protection has all seven set as required checks. The seven gates are enumerated in §9.1.1 below." Update: "The eight canonical CI gates from `.github/workflows/x-audits.yml` (per master spec X.* lineage) WERE required-blocking; per DEFERRED.md §X6-D-1 (2026-05-20), branch protection on `main` was deleted and both `verify-branch-protection` + `verify-branch-protection-status` are guarded `if: false` in the workflow. The eight gates STILL run on every PR + push and surface red checks in the GitHub UI; they no longer block merges until X6-D-1 reopens. The eight gates are enumerated in §9.1.1 below. M12.1 (2026-05-27) added gate #8 `x6-pg-cron-pivot`."
    2. **Line ~808** (§9.1.1 opening sentence): current "The 'seven CI gates' referenced in §9.1 are concrete jobs in `.github/workflows/x-audits.yml`. Each is independently required-blocking on the branch protection rule ... Phase 0 verifies each is required and each fires correctly". Update: "The 'eight CI gates' referenced in §9.1 are concrete jobs in `.github/workflows/x-audits.yml`. Each WAS independently required-blocking on the branch protection rule; per X6-D-1, branch protection is deleted (2026-05-20). Phase 0 no longer verifies branch-protection settings; it verifies each gate is wired in the workflow and fires correctly. M12.1 added gate #8 `x6-pg-cron-pivot` (2026-05-27)."
    3. **Line ~818** (§9.1.1 table row 7 description for `verify-branch-protection-status`): current "Required-blocking PR check ... This is the gate that actually blocks merges." Update: "(X6-D-1 dormant) PR check that WOULD read the latest privileged `verify-branch-protection` artifact ... — DORMANT per DEFERRED.md §X6-D-1, currently `if: false`."
    4. **Line ~822** (Phase 0 sub-task): current "Phase 0 sub-task: confirm the production-target Vercel deployment branch has all seven set as required-blocking checks in the GitHub branch-protection rule." Update: "Phase 0 sub-task — per X6-D-1, branch protection on `main` is DELETED and not part of Phase 0 verification. Confirm each of the eight gate jobs is wired in `.github/workflows/x-audits.yml` and runs on PR + push. When X6-D-1 reopens, this sub-task is reactivated to re-PUT branch protection with all eight gate names per the master spec AC-X.6 inventory + the BRANCH_PROTECTION_DRIFT resolution prose."
    5. **Line ~952** (§12 self-consistency sweep): current "**7 canonical CI gates (§9.1.1 — added R6; R7 corrected gate #7 name to `verify-branch-protection-status`)**". Update: "**8 canonical CI gates (§9.1.1 — added R6; R7 corrected gate #7 name; M12.1 added gate #8 `x6-pg-cron-pivot` per the master spec AC-X.6 inventory amendment; X6-D-1 marks the required-blocking enforcement dormant pending team workflow)**".
    6. **Line ~954** (§12 Build-vs-runtime): current "The seven CI gates (§9.1.1) are PR-time + main-branch build-time gates". Update: "The eight CI gates (§9.1.1) are PR-time + main-branch build-time gates (X6-D-1 dormancy applies to the merge-blocking layer only; the gates still run continuously)."
    7. **DO NOT edit** lines ~1064, ~1077 (M12 spec audit-trail HISTORICAL rows per M12.1 spec §2.4).
  
  Pre-edit grep verification: `rg -n 'seven canonical|seven CI gates|all seven|the seven|required-blocking|required.blocking' docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-19-solo-dev-ux-validation-design.md` — should match the surfaces above (typically 9-10 hits including audit-trail rows). Post-edit grep should show zero matches outside HISTORICAL audit-trail contexts (lines 1064, 1077).
...
- [ ] **Step 4c (R29 F57 fix — master spec AC-X.6 required-check inventory amendment).** Edit the **master spec** at `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md`. The required-check contract that branch-protection enforces is parsed by `scripts/generate-traceability.ts:189-194` from the master spec's AC-X.6 paragraph (between the literal `AC-X.6` anchor and `### 17.2.1`). The parser scrapes all backtick-quoted strings in that range matching regex `/^x[1-6]-|^traceability-audit$|^verify-branch-protection-status$/`. Without amending the master spec, `x6-pg-cron-pivot` will NOT appear in `loadRequiredChecksFromSpec()` → branch protection does NOT require it → later PRs can merge with the M12.1 cross-cutting guards failing or absent, defeating the structural defense. **5th hit of CI-gating-inventory recurrence class (R5 F11 + R8 F22 + R27 F54 + R28 F56 + R29 F57)**; per AGENTS.md "Structural-defense calibration", ship the structural defense in this commit (the meta-test asserting `loadRequiredChecksFromSpec()` returns `x6-pg-cron-pivot` — added as assertion N in T5 step 0 below).
  ...
  **Operator follow-up (R30 F58 + R31 F59 correction): NONE at M12.1 close-out.** Per X6-D-1 deferral (`docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/DEFERRED.md`), branch protection on `main` was deleted on 2026-05-20 and both `verify-branch-protection` + `verify-branch-protection-status` jobs in `.github/workflows/x-audits.yml:268-345` are guarded with `if: false`. The M12.1 amendment to master spec AC-X.6 is prep work for when X6-D-1 reopens; the privileged drift detector is dormant and will NOT alert. NO GitHub branch-protection settings update is required at M12.1 close-out. See T5 Step 8 below for the full dormancy contract + the DEFERRED.md §X6-D-1 cross-reference that captures the reopen-time obligation.

- [ ] **Step 4c.1 (R31 F60 fix — full master spec branch-protection surface sweep).** AC-X.6 amendment alone is insufficient; the master spec has 4 other surfaces that enumerate "seven required" / "SEVEN required" / "all seven" / "7th required" + the 7-name verbatim list. These surfaces are operational copy that an incident-responder reads when `BRANCH_PROTECTION_DRIFT` fires; if X6-D-1 reopens with stale 7-name copy in the master spec, the responder will re-PUT branch protection with only the pre-M12.1 7-gate set, regressing the M12.1 inventory amendment. Apply the following targeted edits (line numbers approximate; grep-confirm at edit time):
  
  1. **Line ~2844** — `BRANCH_PROTECTION_DRIFT` catalog row (§12.4 admin-alerts catalog). Current text: "found that one or more of the SEVEN required X.* status checks (`traceability-audit`, ... , `verify-branch-protection-status`)". Update to "EIGHT required X.* status checks" + append `, \`x6-pg-cron-pivot\`` to the named-verbatim list. The resolution-text "The 7th required check is `verify-branch-protection-status`..." stays historically accurate (verify-branch-protection-status WAS introduced as the 7th); add a parenthetical: "(M12.1 added `x6-pg-cron-pivot` as the 8th)".
  
  2. **Line ~3087** — `BRANCH_PROTECTION_DRIFT` message field (the operator-facing copy emitted to admin_alerts). Current text: "one of the seven required checks is missing". Update to: "one of the eight required checks is missing". Keep the rest unchanged.
  
  3. **Line ~3744** — §17.2.1 opening sentence "verify-branch-protection-status as a 7th PR-required status check (per AC-X.6)". This describes the introduction of verify-branch-protection-status historically; leave as-is (the "7th" is the original ordinal at introduction time; M12.1's addition is the 8th).
  
  4. **Line ~3746** — §17.2.1 `BRANCH_PROTECTION_DRIFT` resolution prose. Current text: "Settings → Branches → Branch protection rules → `main`, all seven check names verbatim — the 7th name is `verify-branch-protection-status`, NOT `verify-branch-protection`". Update: "all eight check names verbatim — the 7th name is `verify-branch-protection-status` (NOT `verify-branch-protection`); the 8th name is `x6-pg-cron-pivot` (M12.1 pg_cron pivot defenses, added 2026-05-27)."
  
  Pre-edit grep verification: `rg -n 'seven required|SEVEN required|all seven|the seven|7 required check' docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` — confirm exactly 4 matches (the 4 above). If more surface, audit each before bulk-editing; if fewer, the line-numbers shifted (re-grep for context).
...
- [ ] **Step 5a: Extend pg-cron-pivot-amendment doc-guard with assertions J + K + L + M + N + O + P + Q + R + S (R11 F30 + R18 F39 + R29 F57 + R31 F60 + R33 F62 fix).** Add 10 assertions to T5's `tests/cross-cutting/m12-plan-pg-cron-pivot-amendment.test.ts`:
  ...
  - Assertion P (R29 F57 structural defense): `loadRequiredChecksFromSpec()` returned array contains `x6-pg-cron-pivot` (dynamic import of `scripts/generate-traceability.ts` named export). Closes the CI-gating-inventory recurrence at the parser layer.
  - Assertion Q (R31 F60): master spec `BRANCH_PROTECTION_DRIFT` catalog row (line ~2844) contains backtick literal `x6-pg-cron-pivot`.
  - Assertion R (R31 F60 structural defense — full-surface sweep, master spec): master spec contains zero matches for `/seven required|SEVEN required|all seven|the seven/` outside HISTORICAL audit-trail contexts. Closes the X6-D-1-reopen-time regression vector at the master-spec-prose layer.
  - Assertion S (R33 F62 structural defense — full-surface sweep, M12 spec): M12 spec contains zero matches for `/seven canonical|seven CI gates|all seven|the seven|required-blocking/` outside HISTORICAL audit-trail contexts (lines ~1064 + ~1077). Closes the 8th hit of CI-gating-enforcement recurrence at the M12-spec-prose layer.
  
  All 10 FAIL at HEAD (stale text present + M12 spec section not yet added + master spec inventory not yet amended + master spec branch-protection-surface not yet swept + M12 spec required-blocking surfaces not yet swept); PASS after step 4a + step 4b (now extended with R33 F62 sweep) + step 4c + step 4c.1 edits. T5 doc-guard now has 19 assertions (was 9: A-I, now A-S with J/K/L/M added in R11/R18, N/O/P added in R29, Q/R added in R31, S added in R33).
...
- [ ] **Step 7: Verify the FINAL x6 audit script in real GitHub Actions (R27 F54 fix).** T4.4 step 3a previously verified the x6 job in real CI when the script ran only `no-vercel-cron.test.ts` + `pg-cron-pivot-doc-guard.test.ts` (2 tests). T5 step 0a extended the script to include `m12-plan-pg-cron-pivot-amendment.test.ts` (3 tests total). Without a fresh real-CI run AFTER T5 commit lands, a CI-only failure mode in the new third test (jsdom-vs-node, fs-walk path resolution, test-runner config mismatch, etc.) would slip past close-out and only surface when an unrelated PR triggers the workflow.
  
  Procedure (run AFTER T5 commit lands on a remote branch):
  
  1. `git push origin <branch>` — push the T5 commit (and all M12.1 commits) to GitHub. `gh workflow run` requires the ref to exist on the remote.
  2. `gh workflow run x-audits.yml --ref <branch>` — manually trigger the workflow against the branch.
  3. `gh run watch` — follow the latest run's execution; confirm the `x6-pg-cron-pivot` job completes with status `success`.
  4. **Verify the run executed all 3 tests** (not just 2 — confirms the T5 step 0a script extension actually took effect in the CI environment). Pull the run's log via `gh run view --log --job=<x6-job-id>` and grep for the three test file names: `no-vercel-cron`, `pg-cron-pivot-doc-guard`, `m12-plan-pg-cron-pivot-amendment`. All three must appear in the vitest output. If any are missing, the script extension is broken (verify `package.json`'s `test:audit:x6-pg-cron-pivot` value).
  5. If the job fails in CI but the local `pnpm test:audit:x6-pg-cron-pivot` passes, that's the "local-passes-CI-fails" class (AGENTS.md cross-cutting #4) — surface to orchestrator before declaring M12.1 done.
...
**Risk class:** low — markdown documentation edit; no executable code. The risk is conceptual: (a) missing the Vault populate step means every cron firing 401s in validation; (b) missing the smoke-file sweep means executor follows stale debugging guidance; (c) per X6-D-1 deferral, no GitHub branch-protection settings update is in scope at M12.1 close-out — the spec inventory amendment is prep work for when X6-D-1 reopens; the workflow-level x6 job runs continuously (red surfaces, no merge block per X6-D-1); the meta-test pins inventory at CI time.
