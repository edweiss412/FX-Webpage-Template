...
- [ ] **Step 10.0 (failing test — R19):** re-point the pure-helper tests FIRST: change `tests/components/admin/formatDateRange.test.ts` (+ `class-sweep-now-utility.test.ts` if it imports the panel) to import from `@/lib/admin/showDisplay`, and add to `formatDateRange.test.ts` a source-level assertion that NO file under `components/`+`app/` imports from `components/admin/ActiveShowsPanel` or `components/admin/PendingPanel` (walk the tree, grep import specifiers). Run: `pnpm vitest run tests/components/admin/formatDateRange.test.ts` → FAIL (module `@/lib/admin/showDisplay` does not exist; six live importers still reference the panels). Concrete failure mode: helpers duplicated instead of moved, or a dead-panel import surviving.
...
- [ ] **Step 13.2:** Expected: ALL rows pass at their viewports — 17 live rows = **16 `both` + 1 `desktop`** (row 2; R19 count fix). Mobile project: 16 passed + 1 skipped; desktop project: 17 passed. Derive the totals in the run summary from `allWalkableRows.length` (17) and the per-viewport partition, NOT from hardcoded literals — a report missing any live row must fail close-out. Legend/archived/alerts/drive-health rows render from Task-12 fixtures. Triage every failure to its task; re-run until green. This step is the milestone's core acceptance gate — do not weaken row assertions to pass; fix surfaces or fixtures.
