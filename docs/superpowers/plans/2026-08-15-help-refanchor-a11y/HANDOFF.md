...
3. The implementation worktree ALREADY EXISTS with the claim marker pushed: `/Users/ericweiss/FX-worktrees/help-refanchor-a11y`, branch `fix/help-refanchor-a11y`. Verify the authoring PR is MERGED (`git log --oneline origin/main | head -5` shows the docs/help-refanchor-a11y-spec merge; if not merged, STOP — you were launched early). In the worktree: `pnpm install && pnpm worktree:link-env && pnpm preflight`, then `git merge origin/main` (mechanical — the parent PR only adds spec/plan/handoff and strips its own marker).
