...
5. Four pre-dispatch mutants for the string-presence assertions (writing-plans rule), run before the diff review dispatch and recorded in the commit: (a) announcement value emptied; (b) `Link copied` plus an appended suffix; (c) the string present but not live (behind a false condition); (d) the discriminating parameter varied (a different `id` must change the aria-label). Each must fail the suite.
6. Commit `fix(help): per-code copy-link names + copy announcement in RefAnchor`.
...
1. **Tests first**, in `tests/help/page-errors.test.tsx`:
   - Premise (executable, `tests/_shared/premise.ts`): `premise("renderable catalog entries make the 219-stop problem reachable", renderableCodes.length, 1)` — the file already computes `renderableCodes` from the live catalog; below 2 entries the DOM-order assertion proves nothing.
   - Skip link is the FIRST page-contributed focusable (spec §3.5(a) / R3 F2): among ALL anchors/tabbables in the rendered fragment, the skip link is index 0 — not merely before the first copy-link (the jump-list nav contributes seven-plus family anchors that a loose ordering assertion would let precede it). Catches: the shipped tree (no anchor) and a skip link parked after the nav.
...
5. Commit `fix(help): skip link past the error catalog to the report CTA`.
...
Red now and until the gate runs: `closeout.md` does not exist, so the grep exits 2; it exits 0 once the marker line lands in the §3.3 grammar. (This red is RUN at plan time — see the plan-time validation record below.)

1. Run the spec §7 dual gate on the arc diff (both halves, in order, with the skill's canonical setup gates — context load of PRODUCT.md + DESIGN.md, then the register reference read). The diff is `app/help/**` UI — Opus implements and gates it directly per the routing hard rule.
