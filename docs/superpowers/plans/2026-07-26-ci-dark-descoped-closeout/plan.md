...
2. **Every "mark ✅ RESOLVED in BACKLOG.md" step in this plan is superseded by graduation — PR-A's here, and B4's + C5's when those PRs land.** Main's `tests/docs/_metaDeferralLedgerGraduation.test.ts` (merged into this branch at eb4f282ce) forbids terminal-status entries sitting in BACKLOG.md, whatever branch writes them. For PR-A, `BL-CI-UNREGISTERED-SELF-CONTAINED-SPEC` and `BL-CI-ENV-DEPENDENT-CONFIG-NARROWING` moved to `BACKLOG-archive.md` (substance intact, provenance `feat/ci-dark-descoped-guards`) and are registered in the test's `BACKLOG_GRADUATED` list. B4 (`BL-CI-VITEST-EXCLUSION-COVERAGE`) and C5 (the two resolver entries) MUST follow the same protocol — archive move + `BACKLOG_GRADUATED` row with the resolving branch as provenance — not the in-place status flip their task bodies below describe.

3. **Every "three configs" / "trio" count in this plan (architecture line, A4 task bodies, execution-cost note) is three-at-spec-time.** Main's section-header visual gate added a fourth Playwright config (`tests/e2e/visual.config.ts`), which joined the detector's `CONFIGS` set at the eb4f282ce merge — forced by the config-set tripwire itself. The shipped detector unions and probes FOUR configs; executed task-body text and quoted commit messages stay as the historical record.
