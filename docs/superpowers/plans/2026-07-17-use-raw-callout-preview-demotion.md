...
- **(c) ⚠️ STANDALONE control tests — DO NOT TOUCH.** `tests/components/UseRawControl.test.tsx` (e.g. `:830-855`) renders `<UseRawControl … site="callout">` **directly** to test the #454 site-scoping mechanism. Spec §2 (`docs/superpowers/specs/2026-07-17-...md:30`) explicitly RETAINS the `"callout"` enum value and this test — the standalone control still accepts `site="callout"`. It is NOT the wizard callout mount and must stay unchanged. (Codex plan-R2: the invert rule must not rewrite it.) NOTE: `tests/components/RoleRecognizeControl.test.tsx` has **no** `site="callout"` usage at all — its standalone suite uses `site="showpage"` (`:410`), so it will not appear in the `site="callout"` grep and is outside this sweep entirely (Codex plan-R3 correction).
