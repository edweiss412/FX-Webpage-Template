...
- [ ] **Step 2: run — FAIL** (unit pins + serverProps grouping assertions + e2e all red). **Step 3: implement scenarios.** **Step 4: run — PASS** (unit + serverProps; e2e goes green in Task 7's full run; group walk stays `GROUP_ORDER`-sorted). **Step 5: commit** — `feat(admin): 36 modal-state scenarios (changes feed, lifecycle, sync, empty, caps, ignored)`
...
- [ ] `/impeccable critique` + `/impeccable audit` on the affected diff (invariant 8; the two UI files from Task 5 + any copy), with the canonical v3 setup: `node .claude/skills/impeccable/scripts/context.mjs` context load (PRODUCT.md + DESIGN.md) then register reference read (the skill's product register reference — admin tool register). P0/P1 fixed or DEFERRED.md. Findings + dispositions recorded in this plan's Review record (no milestone handoff doc exists; the plan is the §12-equivalent).
