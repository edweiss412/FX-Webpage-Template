...
- CREATES tests/sync/feed/shapeChangeFeed.test.ts (new file), tests/dev/publishedModalFixtureKnobs.test.ts (new file) (incl. the §3.2 parity-table walker).
...
- [ ] **Step 4: run — PASS**, plus `tests/dev/attentionScenariosIndex.test.ts` extended: no tier-1/3 scenario carries any of the five fields (walk `ALL_SCENARIOS`).
...
- Test: tests/dev/publishedModalFixtureKnobs.test.ts (new file) (create)
...
- Test: `tests/dev/deriveScenarioAttention.test.ts` (extend), `tests/dev/buildScenarioModalData.test.ts` (extend if present; else assertions live in serverProps test)
...
- Test: `tests/dev/attentionScenariosTier2.test.ts` (extend)
...
- [ ] `/impeccable critique` + `/impeccable audit` on the affected diff (invariant 8; the two UI files from Task 5 + any copy). P0/P1 fixed or DEFERRED.md.
