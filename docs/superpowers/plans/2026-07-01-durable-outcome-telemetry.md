...
- [ ] **Task 9 — finalize-cas** (`app/api/admin/onboarding/finalize-cas/route.ts`): `code: "SHOW_FINALIZED"`, `result: "final_cas"`, `source: "api.admin.onboarding.finalize-cas"`. Cover the streaming success branch AND :795. **Placement nuance (Codex plan-R3 HIGH):** finalize-cas commits per-row via each row's OWN `withRowTx` and then does POST-COMMIT work (revalidate every committed show at :790-792, `subscribeToWatchedFolder`, response). Because the applies are ALREADY durable at that point, the outcome log MUST fire for committed finalizations EVEN IF a post-commit path (revalidate/subscribe/response) later throws — losing that telemetry would defeat the audit trail. So place the ref/log so it reflects the committed state as soon as the per-row commits are durable (do NOT gate it behind the post-commit revalidate/response). Tests: (a) streamed terminal success logs; (b) non-streamed (:795) logs; (c) **commit-failure negative (correct fixture):** the per-row commit dep REJECTS before durability (nothing committed) → assert 500 AND no `SHOW_FINALIZED` log; (c2) **committed-then-post-commit-throw POSITIVE:** the per-row applies COMMIT, then a post-commit path (e.g. `subscribeToWatchedFolder`) throws → assert the outer catch still returns its error BUT the `SHOW_FINALIZED` log DID fire for the committed shows (durable telemetry not lost); (d) a per-row 409 for a blocked sibling in a MIXED batch → the outcome log reflects only the committed applies (assert `result`/count excludes the blocked one, per :790-795 mixed-batch semantics).
