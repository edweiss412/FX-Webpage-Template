...
**Spec:** `docs/superpowers/specs/nav-perf/2026-06-23-nav-perf-tag-caching.md` (Codex APPROVE R5).
...
**Files:** Create `docs/superpowers/plans/nav-perf/2026-06-23-nav-perf-tag-caching/01-write-site-registry.md` (working doc).
...
**Cache profile (Codex plan-R3 MED + Next-16 API finding):** `unstable_cache(fn, key, { tags:[showCacheTag(id)], revalidate: 300 })` — `revalidate: 300` is the periodic TTL BACKSTOP (spec §4.3). The on-demand bust is `revalidateShow(id)` = `revalidateTag(showCacheTag(id), { expire: 0 })`. **CRITICAL (Next 16):** `revalidateTag`'s 2nd `profile` arg is now MANDATORY; `profile="max"` / `{expire:N>0}` is STALE-WHILE-REVALIDATE (a stale window) which DEFEATS near-zero staleness (with the live token, a stale read → bridge match → page stuck stale). `{ expire: 0 }` = IMMEDIATE (next read is a blocking cache miss = fresh) — the Next 16 docs prescribe it for route-handler/webhook immediate invalidation. `updateTag` is immediate too but Server-Action-ONLY (can't be used in the sync route handlers), so we use `{expire:0}` uniformly. The per-site tests (Tasks 5-9) assert `revalidateTag` is called with `{ expire: 0 }` (not just "called").
