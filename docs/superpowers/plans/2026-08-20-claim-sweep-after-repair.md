...
| Task 4, not-found | emit not-found whenever the REPORTED occurrence list is empty | **that is NOT the rule, and measuring is what showed it.** `PublishedReviewModal.tsx:978` occurs EXACTLY ONCE in the frozen `c272ebed3` blob, entirely INSIDE the repair's hunks: it reports nothing and it WAS FOUND, so calling it not-found puts a false statement about the corpus in the arm's own finding text. The predicate counts EXACT occurrences BEFORE the span filter, never the reported list, and TWO mutants (the predicate keyed on `findings.length`, and the counter moved after the span filter) prove the PLACEMENT is load-bearing rather than the expression. The pair one variable apart (truncated vs untruncated identifier) is what makes the clean half attributable |
