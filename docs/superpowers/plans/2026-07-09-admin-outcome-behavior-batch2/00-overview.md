...
- **Deterministic DB/Drive/client-free enforcement (Codex plan-R8/R9) — closes all THREE default-infra channels:** (i) Batch-2 `beforeAll` poisons `process.env.TEST_DATABASE_URL`/`DATABASE_URL` → `postgresql://poison:poison@127.0.0.1:1/none` (port 1) + `delete`s `GOOGLE_SERVICE_ACCOUNT_JSON` (saved; `afterAll` restores) → any un-injected `postgres()`/Drive default throws. (ii) A Batch-2-scoped **`beforeEach`** (runs AFTER the file-level `beforeEach:425` → wins) sets `serverClientImpl.current`/`serviceRoleClientImpl.current` to throwing stubs → any un-injected default that reaches the mocked Supabase client throws (no restore needed; the file-level `beforeEach` re-benigns for later tests). (iii) The one poison-proof default — `readCurrentWizardSessionIdBestEffort` (swallows its own error → null) — is reached only on #14's rollback branch, which is DISALLOWED (see below). Deterministic on any runner regardless of CI booting local Supabase (`unit-suite.yml:80`). Replaces the earlier false "CI has no DB" claim.
...
- **#14** records 3 codes under the one file key: `handleWizardPendingIngestionRetry` → `PENDING_INGESTION_RETRIED`; `handleWizardPendingIngestionAction(ctx, deps, "defer_until_modified"|"permanent_ignore")` → `PENDING_INGESTION_DEFERRED`/`PENDING_INGESTION_IGNORED`. Failure drives use the **MANDATED callback-invoking pre-mutation 404 refusal** per spec §3.3 (faked tx → locked-row read null → `requireCurrentWizardRow` 404, `failureExpect={status:404,code:"PENDING_INGESTION_NOT_FOUND"}`, `mark.hit` set in the faked read). The rollback-branch failure is **DISALLOWED** (reaches `readCurrentWizardSessionIdBestEffort` which swallows errors → evades every poison). Never drive the sibling delegator routes.
...
- Negative-regression (spec §5) — each mutation leaves an EXECUTABLE assertion in place that must fail (no "delete the assertion" checks); restore after each:
  ...
  - (f) **omit one `postgres()`/Drive seam injection from a row (success or failure)** → env-poison → `127.0.0.1:1` ECONNREFUSED / missing-cred throw → success drive rethrows via `observeSuccessCodes` (RED) / failure `thrown` defined → RED (Codex plan-R7/R8).
  - (g) **omit a seam whose default reaches the Supabase client** → the Batch-2 `beforeEach` throwing `serverClientImpl`/`serviceRoleClientImpl` stub throws → RED (Codex plan-R9). (Confirm both poisons are load-bearing: temporarily drop each and re-run — a missed seam then silently passes; restore.)
