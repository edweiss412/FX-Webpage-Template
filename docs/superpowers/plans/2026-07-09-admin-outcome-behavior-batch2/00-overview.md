...
- Add a local **`proveAdminOutcomeBehavior({ file, fn, code, success, failure })`** helper (Codex plan-R2/R6) that STRUCTURALLY ties every `recordAdminOutcomeBehavior` call to a paired, PROVEN-REAL failure proof (full contract in spec §3.3): `success: () => Promise<unknown>`; `failure: (mark: {hit:boolean}) => Promise<unknown>`. It (1) `const ok = await observeSuccessCodes(success); expect(ok).toContain(code)`, (2) `const mark={hit:false}; const bad = await observeCodes(() => failure(mark)); expect(bad).not.toContain(code)`, (3) **`expect(mark.hit).toBe(true)`** — the anti-hollow guard: each row's `failure` sets `mark.hit=true` INSIDE the injected mutation/tx seam it drives to refusal, so a no-op failure leaves it false → RED, (4) ONLY then `recordAdminOutcomeBehavior`. `success`/`failure` also do the #18 drain and the #14 DB-free callback-invoking refusal. The helper is the ONLY place that calls `observeSuccessCodes`/`observeCodes`/`recordAdminOutcomeBehavior`; no Batch-2 test body calls them directly.
- **Structural guard (Codex plan-R3, CI-enforced):** wrap the Batch-2 block in sentinel comments `// >>> BATCH-2 PROOF BLOCK START` / `// <<< BATCH-2 PROOF BLOCK END`, and add a `test("Batch 2 rows record only via the paired-proof helper")` that reads this test file's own source (repo-relative path via `fs.readFileSync`), slices between the sentinels, and asserts the slice matches none of `/\brecordAdminOutcomeBehavior\s*\(/`, `/\bobserveSuccessCodes\s*\(/`, `/\bobserveCodes\s*\(/` (only `proveAdminOutcomeBehavior(` allowed). Batch-2-scoped — Batch-1's direct-`record` rows (outside the sentinels) are untouched. This makes "record directly / skip the failure drive" a CI failure, not a convention.
...
**Inject EVERY DB/Drive/lock seam per spec §3.5 (Codex plan-R4).** Each `routeDeps` MUST inject every seam in the §3.5 inventory — all `withTx`/`withRowTx`/`withRowTryLock` as callback-invoking passthroughs `(…, fn) => fn(fakeTx)`, plus every `read*`/`verifyFolder`/`upsertAdminAlert`/mutation dep — so no default Postgres/advisory-lock/Drive impl is reached. **Backstop (accurate mechanism, spec §3.5):** the defaults do NOT throw on unset env — `databaseUrl()` falls back to local Supabase `…54322`. The real enforcement is **CI unit-suite** (no Postgres on 54322, no Drive creds): an un-injected seam is connection-refused → the drive throws → the row can't go green. **Local green is NOT authoritative** (a missed seam silently hits local Supabase); copy each cited driver's deps-builder verbatim and treat CI unit-suite green as the gate. Notes:
