...
**Inject EVERY DB/Drive/lock seam per spec §3.5 (Codex plan-R4).** Each `routeDeps` MUST inject every seam in the §3.5 inventory — all `withTx`/`withRowTx`/`withRowTryLock` as callback-invoking passthroughs `(…, fn) => fn(fakeTx)`, plus every `read*`/`verifyFolder`/`upsertAdminAlert`/mutation dep — so no default Postgres/advisory-lock/Drive impl is reached. **Structural backstop:** this test file configures NO DB connection, so any seam left defaulted throws at run time (loud failure, not a silent real-DB hit) — but §3.5 is the primary defense; copy each cited driver's deps-builder verbatim and vary only the outcome. Notes:
