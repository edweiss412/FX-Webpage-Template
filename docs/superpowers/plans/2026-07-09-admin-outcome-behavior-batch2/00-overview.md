...
- Negative-regression (spec §5) — each mutation leaves an EXECUTABLE assertion in place that must fail (no "delete the assertion" checks); restore after each:
  ...
  - (f) **omit one seam injection from a row's `failure` so it falls through to a default/un-injected seam** → either `thrown` is defined OR `result.status` is the generic 500 (`SYNC_INFRA_ERROR`) ≠ the declared `failureExpect.status` → helper RED. Proves the failure cannot certify absence via a swallowed infra/default-seam error (Codex plan-R7).
