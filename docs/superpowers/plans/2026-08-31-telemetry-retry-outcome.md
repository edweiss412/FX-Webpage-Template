...
1. `telemetryRetryButton.test.tsx` — new cases, rendered-text assertions per the file's own anti-tautology posture:
   ...
   - **the two OUTCOME renders in that sequence differ from EACH OTHER** — AC-6. Adjacent-pair inequality alone is satisfied by alternating intent and outcome copy, so it cannot see an outcome setter that leaves `seq` unchanged: with that mutant both outcomes inherit the parity of the tap that preceded them (odd both times) and render byte-identical, which is silence for the second one. Captured as the two outcome strings and compared directly, so the assertion fails on exactly that edit;
...
Both inventories are read off the shipped implementation at mutation time, so a site added later is covered by re-running the derivation rather than by remembering a list. Record each mutant, the case that redded, and the revert in the commit message. No production change; any surviving mutant is repaired test-side in this task.
