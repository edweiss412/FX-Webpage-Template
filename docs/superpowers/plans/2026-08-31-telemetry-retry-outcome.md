...
1. `telemetryRetryButton.test.tsx` — new cases, rendered-text assertions per the file's own anti-tautology posture:
   ...
   - **the adjacency property itself, walked over a sequence, not asserted as parity arithmetic** — AC-6. Drive tap, tap, change, tap, change, change through one mounted control and assert that every non-empty region text differs from the one rendered immediately before it. That is the property spec §3.6 states; a parity claim about any particular pair is arithmetic about the implementation, and round 3 caught the round-2 version of this bullet asserting a pair that the stated implementation renders identically (outcomes land at even `seq` both times). The walk cannot go stale that way, because it reads what rendered rather than predicting it;
   - **parity is the non-breaking space specifically, pinned by code point.** Assert the parity-suffixed render's text ends with ` ` (and that the unsuffixed one does not), not merely that consecutive renders differ: an ordinary trailing space is textually distinct and would pass an inequality-only check while abandoning the mechanism the sibling precedent uses (`components/admin/ShowRowActions.tsx:608`) — AC-6;
...
- **Every boolean operand:** `Number.isFinite(renderedAt)` at the tap, `baseline !== null`, `Number.isFinite(renderedAt)` at settlement, `renderedAt !== baseline`, `seq % 2`. Mutate each to the ordinary authoring substitutes for its type: a truthiness test for a finite/null check, `!Number.isNaN` for `Number.isFinite`, `>` and `<` for `!==`, an ordinary space for the U+00A0 suffix.
- **Every state write:** the tap's `seq + 1`, the tap's text, the tap's `baseline = renderedAt`, the settlement's `seq + 1`, the settlement's text, the settlement's `baseline = null`. Mutate each to the ordinary substitutes for a write: dropped (the value carried through unchanged), duplicated, or written with the sibling branch's value.
...
**One write is expected to survive, and it is recorded as EQUIVALENT rather than chased.** Dropping the settlement's `seq + 1` changes no reachable rendering, because two consecutive announcements can share their text only if both are intents (the outcome clears the baseline, so a second outcome needs an intervening tap, and AC-9 pins the two strings apart), and the tap-side increment is what separates that pair. So the parity suffix is load-bearing at the tap and inert at the settlement. It is written on both sides anyway, because a reader who sees it on one side asks why, but the mutation record carries the reachability argument and does not claim a red the suite cannot produce. Rounds 2 and 3 were spent here: round 2 asked for the red, round 2's repair asserted a pair that cannot differ, and round 3 did the arithmetic that shows why. Record each mutant, the case that redded, and the revert in the commit message. No production change; any surviving mutant is repaired test-side in this task.
