...
- Mutation-operator families for the new guard surface, enumerated up front (closure set): (a) memoized clock (module-level cache in `nowDate`); (b) baseline never recorded (tap handler drops the record); (c) baseline never cleared (outcome repeats on every render); (d) **any one-edit substitution of a boolean operand in the announcement path** — the family is derived from the operand inventory in Task 3, not from a fixed list, and covers comparator inversion (`!==` → `===`), comparator ordering (`!==` → `>`/`<`), truthiness for finite/null checks, and `!Number.isNaN` for `Number.isFinite`; (e) outcome string emptied / suffixed / not-live (the four string mutants), plus the parity suffix downgraded from ` ` to an ordinary space; (f) prop unthreaded at one site (census). A reviewer-proposed NEW family needs a live escaping mutant.
...
Run all four string mutants (emptied / suffixed / not-live / parameter-varied) plus family (c) baseline-never-cleared and family (d) comparison-or-guard-substituted from the closure set. The guard/comparator arm is derived rather than listed: enumerate every boolean operand in the announcement path (`Number.isFinite(renderedAt)` at the tap, `baseline !== null`, `Number.isFinite(renderedAt)` at settlement, `renderedAt !== baseline`, `seq % 2`) and mutate each in turn to the ordinary authoring substitutes for its type — a truthiness test for a finite/null check, `!Number.isNaN` for `Number.isFinite`, `>` and `<` for `!==`, ` ` for ` ` — so a sixth operand added later is covered by re-running the derivation, not by remembering a list. Record each mutant, the case that redded, and the revert in the commit message. No production change; any surviving mutant is repaired test-side in this task.
