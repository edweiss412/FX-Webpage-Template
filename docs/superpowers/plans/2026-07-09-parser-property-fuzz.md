...
2. Role vocab: fixed clean list, e.g. `["Load In / Set / Strike", "Video Engineer", "Audio A2", "LED Tech", "Camera Op"]` — screened: no `ONLY`, no `***`, no `\d{1,2}/\d{1,2}`, no stage words, no parens (spec §3.1; grammar cites `personalization.ts:58-62`).
...
- **ROOMS:** header cell `<kind> <name> <w>' x <d>' Floor 1` (dims via the dial's format: `50' x 40'`, `50 x 40`, `50′ × 40′` — `DIMS_FULL_SRC` `_dimsToken.ts:40-47`), followed by the structured bare-label rows (`SETUP`, `SET TIME`, `SHOW TIME`, `STRIKE TIME`, `AUDIO`, `VIDEO`, `LIGHTING`, `SCENIC`, `POWER`, `DIGITAL SIGNAGE`, `OTHER`, `NOTES`) each as `| <LABEL> | |`.
