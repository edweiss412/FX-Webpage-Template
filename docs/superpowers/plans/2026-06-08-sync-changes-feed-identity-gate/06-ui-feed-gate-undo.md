...
- [ ] **Impl** `app/admin/show/[slug]/_actions.ts` — add `undoChangeAction`, `mi11ApproveAction`, `mi11RejectAction` (`"use server"`). Each: `await requireAdmin()`; read `FormData`; canonicalize the admin email for `created_by`/audit (invariant 3 via `lib/email/canonicalize`); call the Phase 3/4 RPC through the service-role client destructuring `{ data, error }` (invariant 9). **Approve orchestration (F13/F15):** read the current Drive `modifiedTime` for `show.drive_file_id` FIRST; on returned-error OR thrown → return a typed `lib/messages` result and **do not** call the RPC, leave hold pending; else pass the observed time as `p_observed_modified_time`. `revalidatePath('/admin/show/[slug]', 'page')` on success. Map every RPC `{ok:false, code}` to a `lib/messages` code (no raw codes — invariant 5).
- [ ] **Impl** `app/admin/show/[slug]/page.tsx` — `await readShowChangeFeed(show.id)`; render `<ChangesFeed>` (passing the three actions bound to `show`, `now`, and per-entry `gate` data the page derives from open holds). **Remove** the `<ParsePanel>` / `StagedReviewCard` whole-parse review section (lines ~621–639) and its `pending_syncs` query + `rows` derivation (the staged-review feed is superseded; first-seen review, if still needed, stays out of scope here — confirm in open questions). Keep the wrapping `requireAdmin` + try/catch boundary discipline.
...
- **Removing the whole-parse review** from `page.tsx`: `ParsePanel` + `StagedReviewCard` may still be imported by other surfaces (wizard `wizard_failed_reapply` / `first_seen` modes). Do **not** delete those components blindly — grep their importers first (`components/admin/onboarding/**`, wizard routes) and only remove the **per-show live whole-parse mount**. Confirm scope in the open question below.
- All new copy renders through `lib/messages` (`ErrorExplainer`/`messageFor`) for any code-derived string; hard-coded English only for absence-of-failure states (empty feed). No raw error codes in the DOM (invariant 5).
...
## Open questions (carry to handoff / resolve before execution)

1. **First-seen review:** the whole-parse removal also removes the per-show `first_seen` `StagedReviewCard` mount. Does first-seen approval still need a per-show surface in v1, or is it fully governed by `auto_publish_clean_first_seen` + the feed? (Spec §2 says first-seen has no prior snapshot so MI-11 can't fire — implies the feed covers it, but the apply/publish affordance for a held first-seen show needs confirmation.)
2. **Gate `detail` derivation:** the page derives the per-entry `gate.detail` disposition string from the open `sync_holds` row. Confirm the catalog string for a rename-folded-into-MI-11 hold (spec §11 open question — "email change + rename" combined pending entry needs a `lib/messages` code).
3. **Feed cap value:** spec proposes 50 (§6.2); confirm before wiring the truncation copy.
