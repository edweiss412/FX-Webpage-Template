...
- [ ] **Impl** `lib/sync/changeLog/writeAutoApplyChanges.ts`:
  - Compute notable changes from `(previousCrewMembers, parseResult.crewMembers, triggeredItems)`: crew add (`crew_added`), crew remove (`crew_removed`), rename (`MI-12`/`MI-13`/`MI-14`), and the asset-drift / section / field invariants that fire (mapped to `change_kind`). Exclude held entities (their feed entry comes from `sync_holds`).
