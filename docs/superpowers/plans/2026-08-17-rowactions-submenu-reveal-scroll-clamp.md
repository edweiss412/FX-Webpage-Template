...
**Known branch reds (docs gates) — the live set, each with the task that clears it:** `pnpm vitest run tests/docs/` reds exactly the two files named here, both temporary by design; a third red is a real defect, not a count to update. (1) `tests/docs/_metaInvariant8Closeout.test.ts` — the §12 closeout marker is deliberately unfilled until Task 5 (see §12). (2) `tests/docs/_metaLedgerReferentialIntegrity.test.ts` — `BL-SCREENSHOTS-DRIFT-SINGLE-FAILURE-UNEXPLAINED` is cited by the spec (§7) and by Task 6 before its BACKLOG.md row exists; Task 6 step 2 files the row and clears it. Two gates that DID red on this branch are already cleared and must not be re-counted: `specsReadmeIndexParity` (the spec had no `docs/superpowers/specs/ci/README.md` index row and no task added one — untimed, so the row landed in the plan-R3 repair commit) and `_metaReviewRoundEconomy` (the plan stage reached the four-round threshold at R4 — the filing landed in the plan-R4 repair commit). Task 6 step 4 re-runs `pnpm vitest run tests/docs/` and requires ZERO reds before merge.
...
Every fenced test artifact in this plan was drafted, spliced into the live tree, and RUN at plan time; transcripts follow each task. No fixed artifact count is restated here — the set grew across review rounds, and Appendix B's diff headers are the authoritative enumeration of it. The temporary helper used for the green-reachability check was deleted afterward — the implementation re-lands it via TDD — and every spliced test file was likewise reverted after capture; the fenced blocks in Tasks 2-3 and Appendix B are their exact validated content (typecheck: `pnpm typecheck` exits 0 with every spliced artifact present; the one strict-mode violation found at plan time — an `HTMLElement`-to-record cast in the spy cleanup — is already repaired in the embedded content, which uses `Reflect.deleteProperty`).
...
**RED half — author EVERY test this task ships, then observe the red.** Same rule as Task 3 and for the same reason (plan-R4 F1, swept to this task at plan-R5 F1): no production edit happens until every test that discriminates it exists, or the §4.5 filter could be written before the assertion that catches its absence. Three edits, all here:
...
Add the family-C second phase to the same e2e case — the exact body is in Appendix B, the `tests/e2e/rowactions-geometry.spec.ts` block (validated: typecheck clean; red pre-fix by design). Then, still in the RED half, append the family-B and family-C jsdom describes to `tests/components/admin/rowActions/anchoredPortal.test.tsx` — the exact body is in Appendix B, the `tests/components/admin/rowActions/anchoredPortal.test.tsx` block (the suite's `test` import gains `vi`). Observe that second red too: `pnpm vitest run tests/components/admin/rowActions/anchoredPortal.test.tsx` fails at `panel-origin scroll is ignored (self-origin filter): expected 1 to be +0` until this task's guard lands, while the family-B case is green here by design (a regression pin on behavior the current code already has). Do NOT commit in this half — the task commits once, after GREEN, with both observed red-to-green transcripts in the message.

**GREEN half — the AnchoredPortal rewrite:**

Rewrite `measureAndApply` (`components/admin/AnchoredPortal.tsx:127-187`) per spec §4.3 row 1: the clear/measure/restore block becomes `withNaturalSize(panel, (probe) => …)`; `wrappedHeightAt: probe.heightAtWidth`; the held-style locals go away; `placeWithinVisibleViewport` inputs and the `commit` logic are unchanged. In the SAME task, add the §4.5 self-origin guard to `onScrollCapture` (`AnchoredPortal.tsx:204-215`): an event whose target is the panel or a descendant returns before the re-place branch; document-scroll dismissal and ancestor re-placement unchanged. Without the guard the restore's own scroll events loop the coalescer at one forced measurement per frame (spec §4.2/§4.5, R5 F1). No test is authored in this half — every one already exists and two of them are red. GREEN, on the SAME command the marker's `red=` names: `E2E_PORT=3107 pnpm heavy pnpm exec playwright test --project=desktop-chromium tests/e2e/rowactions-geometry.spec.ts -g "CAPPED submenu" --repeat-each=10` passes 10/10. Two further gates in the same half: the full-file form `E2E_PORT=3107 pnpm heavy pnpm exec playwright test --project=desktop-chromium tests/e2e/rowactions-geometry.spec.ts --repeat-each=2` passes 10/10 — five cases times two repetitions, which is what Playwright enumerates and reports as `Total: 10 tests in 1 file`, so the rewrite broke no sibling, and `pnpm vitest run tests/components/admin/rowActions/anchoredPortal.test.tsx` — red in the RED half at its family-C assertion — passes in full. Family-B and family-C mutant validation for this surface happens here (capped measurement; dropped filter — each observed red, reverted, recorded). Commit (`fix(admin):`) — the strengthened e2e case, the call-site rewrite, the filter, and the jsdom cases land together, red-then-green inside this one task.

## Task 3 — derived-cover meta-test + all peer pins RED → peer call sites → GREEN

<!-- task: red=`pnpm vitest run tests/components/_metaScrollNeutralMeasurement.test.ts tests/components/admin/hoverHelpBlurClose.test.tsx tests/components/admin/hoverHelpPlacement.test.tsx tests/components/admin/showpage/shareHubVisualViewport.test.tsx tests/components/admin/useFitWithinClip.test.tsx` red-state=authored red-target=`components/admin/HoverHelp.tsx:220` why=`this task authors the meta-test and all three peer pin suites before any production edit; on that tree the command is red twice over — the meta-test names HoverHelp.tsx, ShareHub.tsx and useFitWithinClip.ts as bare-clear offenders (after Task 2 removes AnchoredPortal), and the newly authored hoverHelpPlacement family-C case fails at the missing §4.5 self-origin filter. The SAME command goes green once all three route their measurement through withNaturalSize and HoverHelp's capture listener gains the filter; hoverHelpBlurClose and the family A/B pins ride along in the command to pin that the rewrites preserved existing behavior` ac=AC-4,AC-7 -->

**RED half — author the meta-test AND every pin suite this task ships, then observe the red.** No production edit happens until all of them exist: the §4.5 self-origin guard must not be implementable before the `hoverHelpPlacement` family-C assertion that discriminates it is authored (plan-R4 F1).
...
Observe RED on the EXACT command the marker names, which is also this task's GREEN command — `pnpm vitest run tests/components/_metaScrollNeutralMeasurement.test.ts tests/components/admin/hoverHelpBlurClose.test.tsx tests/components/admin/hoverHelpPlacement.test.tsx tests/components/admin/showpage/shareHubVisualViewport.test.tsx tests/components/admin/useFitWithinClip.test.tsx`. Two independent reds are expected and both must be present: the meta-test's offender list, which after Task 2 is exactly the three peers (plan-time validation observed all four on the pre-repair tree; Task 2 removes AnchoredPortal — the shrink is itself evidence Task 2's rewrite landed), and `hoverHelpPlacement`'s family-C assertion, red at `body-origin scroll is ignored (self-origin filter): expected 1 to be +0` until this task's §4.5 guard lands. The family A/B pins are green here by design — they are regression pins on behavior the current code already has (spec §5.4). Do NOT commit here: this task commits ONCE, after its GREEN half, with the observed red-to-green transcript on that one command in the message.

**Site-transition pins (spec §5.4, AC-7), in the same task, authored HERE in the RED half — THREE mutant families per the spec's closure matrix (A cap-STATE, B capped-measurement, C dropped-filter; AC-7 and spec §5.4 enumerate all three, and HoverHelp carries all three):**
...
Family B and C case bodies are in Appendix B, all validated executably at plan time (family A/B green on the live tree — current code measures naturally and applies both branches; family C red at exactly the missing-filter assertion. Family C exists at the TWO scroll-listening surfaces only (spec §5.4, AC-7): AnchoredPortal, whose red is Task 2's, and HoverHelp, whose red is this task's. ShareHub and useFitWithinClip carry families A and B only, and their pins are green here):
...
**GREEN half — the three peer rewrites. No test is authored here:** every pin above already exists. Of them only HoverHelp's family-C case is red (this task's authored red); every family A and B pin, at all three sites, is green by design — they are regression pins on behavior the current code already has.
...
3. **Whole-diff cross-model review** (codex-guard, `--stage diff`) of the pushed head — the diff under review IS the diff that merges. If the review forces changes, step 2 cannot simply be repeated: its entries are already archived and its markers already gone, so there is no closeout change left to commit last and the marker removal would stay stranded in an earlier commit, breaking invariant 12 (plan-R6 F1). The retry is therefore three commits, in this order: (a) `git revert --no-edit <closeout-sha>`, then **push it immediately, before anything else in this step** — the entries return to the open ledger and both `**Status:** IN PROGRESS · **Branch:** …` markers come back, which is also the honest state, since the work is demonstrably not finished. The push is not optional and not deferrable to the end of the step (plan-R7 F1): `pnpm ledger:claims` resolves claims from live branches on ORIGIN (`scripts/ledger-claims.ts`), so a marker that exists only in this worktree is invisible to every other session. Between the local revert and its push, origin still advertises both rows as unclaimed while this branch holds them — and the retry window spans the forced changes, a UI dual gate, and a nine-dispatch AC-5 probe, which is long enough for another session's Stage 0 to claim a row this branch is actively working. Immediate publication is exactly what invariant 12 requires of any marker write; (b) the forced changes, in their own commit(s); (c) step 2 again, now genuinely last. Between (b) and (c): if any changed file is a UI surface (invariant 8's definition), RE-RUN Task 5's dual gate on the new diff and re-fill the marker; RE-RUN step 1's nine-dispatch AC-5 probe at the new production sha (the acceptance evidence must belong to the sha that merges — a docs-only forced change may keep step 1's evidence, stated explicitly in the PR body since `app/**`+`components/**`+`lib/**` are byte-identical). Then re-review the new head, and repeat this whole step for as many forced-change rounds as it takes.
