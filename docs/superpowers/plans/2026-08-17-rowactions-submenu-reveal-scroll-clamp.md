...
Family B and C case bodies are in Appendix B, all validated executably at plan time (family A/B green on the live tree — current code measures naturally and applies both branches; family C red at exactly the missing-filter assertion — Task 2's authored red for AnchoredPortal, this task's for the other three sites):
...
**GREEN half — the three peer rewrites. No test is authored here:** every pin above already exists, family C red and families A/B green by design (regression pins on behavior the current code already has).
