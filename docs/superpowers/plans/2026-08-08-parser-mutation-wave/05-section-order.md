...
- [ ] **Step 3 (retro F3 — spec §7.2(b) says ANY adjacent swap, not ten):** add the EXHAUSTIVE sweep as a second describe block in the same file, gated with the harness (`if (!process.env["VITEST_INCLUDE_MUTATION_HARNESS"]) describe.skip(...)`): for EVERY fixture, for EVERY adjacent block pair i/i+1 (corpus total 497 swaps), assert the post-fix signal multiset equals baseline. The 10 named cases stay ungated as the fast-suite regression; the exhaustive block is the spec-letter proof and runs with the harness project + in branch close-out. The mutation harness alone is NOT equivalent (its oracle accepts a changed multiset when a stronger signal fires).
