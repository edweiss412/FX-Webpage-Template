...
- **audit — ⚠️ DEGRADED: single-context.** Two sub-agent attempts each did the work and neither returned a report through the message channel. Rather than block the gate, the audit was completed in this context from directly measured evidence (the probes listed below, all run here). Declared rather than hidden.
...
| - | --- | ------ | ----- | ------- | ----------- |
| 1 | P1 | audit + critique | PRE-EXISTING | The `md:grid-cols-3` grid (`app/help/tour/page.mdx:53`) sits inside `.help-prose`'s `max-width: 70ch` (`app/globals.css:1148`), so the reading measure is divided three ways. Probed live: 768px → body 104.7px = **10.5ch**, card 811.3px tall; 900px → 14.9ch; 1024px and above → **18.2ch** (the cap binds, not the viewport). `DESIGN.md` §2.5 caps measure at 65–75ch. Mobile (390px, one column) is unaffected. | DEFERRED — `BL-HELP-TOUR-CARD-GRID-MEASURE` (BACKLOG.md), filed with the probe table. Disposition exception (b): spec §1.1 fences copy and layout changes out of this arc, which converts text children only. The repair is a choice between `md:grid-cols-2` and lifting both grids out of the prose cap — the second touches `.help-prose`, which every `/help/*` page renders through. |
| 2 | P1 | critique | PRE-EXISTING | "Below is every admin screen" (`app/help/tour/page.mdx:3`) is false by one: `app/help/_nav.ts:22-29` declares eight `admin-surface` entries and the page renders seven cards — `/help/admin/settings` has none. | DEFERRED — `BL-HELP-TOUR-SETTINGS-CARD-MISSING` (BACKLOG.md). Exceptions (a) and (b): adding the card means choosing its group and changing the page's rhythm, softening the intro gives up the completeness promise; both defensible, so it is the page author's call, and §1.1 fences copy changes out regardless. |
