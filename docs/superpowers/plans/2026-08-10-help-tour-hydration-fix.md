...
<!-- task: red=`true` ac=AC-5 -->

Not a red/green cycle — a gate: run BOTH halves of the invariant-8 dual gate (`/impeccable`, critique mode then audit mode) on the diff with the canonical v3 setup (context.mjs load then register read). Expected visual delta nil-to-minimal (spec §6); P0/P1 findings fixed or DEFERRED.md-logged. On completion, fill the §12 marker line with the real values (`critique=RAN audit=RAN p0=<n> p1=<n> dispositions=<recorded|none>`) — the closeout guard accepts only the filled form, which is why this task precedes the docs-suite green in Task 4.

## Task 4 — Parent-entry reconciliation + graduation

<!-- task: red=`pnpm vitest run tests/docs` ac=AC-6 -->

Red is written by this task (invariant-1 shape): archiving `BL-HELP-TOUR-HYDRATION-MISMATCH` without its graduation bookkeeping fails the docs guards — the production lines are the missing `BACKLOG_GRADUATED`-style archive entry and the parent entry's census restate (`tests/docs` ledger suites enforce archive/provenance shape; verified the entry is currently OPEN/IN PROGRESS on the live tree).

1. `BL-E2E-APP-DEPENDENT-SPECS-CI-DARK` (`BACKLOG.md:634` region): heading population 25 → 24 (count the allowlist, per its own rule), rewrite the "Ninth member deferred, not wired" paragraph (`BACKLOG.md:679` region) to record the promotion. Dated historical records untouched.
...
3. Green: `pnpm vitest run tests/docs` (ledger, graduation, review-round-economy, and — with Task 3's marker filled — the invariant-8 closeout suite, all green).

## Task 5 — Five-green wiring bar + merge sequence

<!-- task: red=`gh run list --workflow=app-e2e.yml --limit 5` ac=AC-6 -->

Gate commands, mutant-validated at plan time: the acceptance check is NOT a bare `gh run list` (which exits 0 on anything); the recorded evidence command is `gh run list --workflow=app-e2e.yml --branch fix/help-tour-hydration --json conclusion,headSha,event --jq '[.[] | select(.event=="pull_request")] | .[0:5] | all(.conclusion=="success")'` asserted `true`, five run links pasted in the PR body. Constructed-failure probe: with fewer than five successes the jq emits `false`, and `grep -qx true` on it exits 1 (verified against a constructed two-run JSON locally at plan time).
...
2. Five consecutive green `app-e2e` pull_request runs, zero retries; links in the PR body.
...
4. Whole-diff cross-model review to APPROVE; real CI green (all workflows); ledger marker off in the last commit (invariant 12); `gh pr merge --merge`; fast-forward main; `git rev-list --left-right --count main...origin/main` == `0  0`.
