...
2. Prettier stability proof (R7 F1 — snapshot before comparing): COMMIT the MDX edit first, THEN run the repo formatter over the file and assert `git diff --exit-code app/help/tour/page.mdx` (formatting changed nothing against the committed form), and `pnpm format:check` green.
