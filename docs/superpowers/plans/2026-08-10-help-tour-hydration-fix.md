...
- *"The live page still has `p p`=3 / `span p`=6, and the three fixed cards' body text renders in the subtle token while the other four render in the base token."* Refuted: that is the PRE-fix render. On this tree the live DOM returns `p p`=0, `span p`=0, `p div`=0, zero pageerrors, and all seven cards at `rgb(26,27,31)`. The compile probe agrees (one `_components.p`, none nested, before and after Prettier).
...
- *Per-card body-color drift.* Refuted with the same measurement as the first item.
