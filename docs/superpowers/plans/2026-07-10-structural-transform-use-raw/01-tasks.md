...
- [ ] **Step 2: Run to fail.**
- [ ] **Step 3: Implement** — thread the param; `applyStagedCore` reads `pending_syncs.use_raw_decisions`, `runManualSyncForShow` reads `shows_internal.use_raw_decisions`; call the overlay; persist/change-log(via `writeUseRawStaleChanges`)/GC.
...
**Interfaces — Produces:** `setStagedUseRawDecisionAction(wizardSessionId, warningRef, useRaw)` — same `WarningRef` contract as T7. Loads `pending_syncs` by `wizard_session_id` server-side, reads `drive_file_id`, locks `show:<driveFileId>`. Under the lock: toggle-ON upserts `{preference:"raw", applied:false}`, toggle-OFF hard-deletes (pre-create has no applied entity rows — no `clear-pending`). Does NOT re-apply (no show yet). **`warningRef` server-validation against that row's `parse_result.warnings`:** same three-branch check as T7 (matched by `code`+`blockRef`; reject absent/`resolvable:false`; reject `observedContentHash` mismatch = stale UI). Stored `contentHash`/`target` from the LIVE staged warning. Post-commit `logAdminOutcome`.
...
- **Per-show** (`app/admin/show/[slug]/page.tsx`): add `use_raw_decisions` to the `shows_internal` select (currently `parse_warnings` only, `:336`); validate/normalize the jsonb (guard non-array → `[]`, drop out-of-scope `code`); for each rendered warning, find the matching decision by `(code, warning.resolution.contentHash)` and pass it as `decision`.
- **Wizard** (`OnboardingWizard.tsx`): add `use_raw_decisions` to the `pending_syncs` select (alongside `parse_result`, `:427-429`); same normalize + `(code, contentHash)` mapping.
