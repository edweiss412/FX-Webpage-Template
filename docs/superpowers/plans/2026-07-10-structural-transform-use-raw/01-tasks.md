...
- [ ] **Step 2: Run to fail.**
- [ ] **Step 3: Implement** — add `useRawDecisions` to `Phase2Args`; thread it into the single `applyParseResult` call (`phase2.ts:369`); the FINALIZE `Phase2Args` builder (`applyStagedCore.ts:564`) reads STAGED `pending_syncs.use_raw_decisions`; the RE-SYNC `Phase2Args` builder (`runScheduledCronSync` `processOneFile` — the path `runManualSyncForShow` delegates into, NOT `runManualSyncForShow` itself) reads PERSISTED `shows_internal.use_raw_decisions`; call the overlay; persist `kept`; ONE ungated `writeUseRawStaleChanges` branch in `phase2`; GC `reverted`.
