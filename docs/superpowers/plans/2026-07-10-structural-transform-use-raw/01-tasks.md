...
**Interfaces — Consumes:** `applyUseRawDecisions` (T3), `USE_RAW_DECISION_STALE` (T4), the columns (T5). **Produces:** `applyParseResult`'s `args` gains an **OPTIONAL** `useRawDecisions?: UseRawDecision[]` (default `[]`) and returns `useRawInvalidated: UseRawDecision[]` on its outcome. **Optional is load-bearing: ~15+ existing tests call `applyParseResult(tx, {…})` directly** (e.g. `tests/sync/applyParseResult.holdAware.*.test.ts`, `applyParseResultScheduleDay.test.ts`, `holds.releaseEval.test.ts`) — a required param would break them all. Defaulting to `[]` means an omitted-decisions call is a no-op overlay (`kept/invalidated/reverted` all empty), so every existing caller and test is unaffected; only the two `runPhase2`-caller builders (T6) pass a non-empty array. Likewise `Phase2Args.useRawDecisions?` is optional (default `[]`). It runs the overlay BEFORE the full-replace writes, and persists the new `use_raw_decisions` = `kept` each `applied:true` (drop `invalidated`+`reverted`) via the SAME `tx.upsertShowsInternal(...)` that already writes `parse_warnings` (:207-214). It does NOT write change-log (keeping its current write-scope) — it RETURNS `useRawInvalidated`.
