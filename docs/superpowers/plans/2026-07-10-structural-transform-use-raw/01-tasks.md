...
- [ ] **Step 3b: Transition-audit (static).** Enumerate every `AnimatePresence`, ternary, and conditional block in `UseRawControl.tsx`; assert (in a test or a documented audit block in the PR) each transition-inventory cell above is either animated with the right props or explicitly instant. Test the compound cases: `pending` overlaying each steady state; toggling while a prior toggle is mid-`pending`.
