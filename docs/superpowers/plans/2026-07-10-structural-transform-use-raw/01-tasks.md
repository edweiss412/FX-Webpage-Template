...
- [ ] **Step 1: Failing tests** (derive expected from fixture raw, never hardcode hashes — recompute in-test with the same helper):
...
**Interfaces — Consumes:** `applyUseRawDecisions` (T3), `USE_RAW_DECISION_STALE` (T4), the columns (T5). **Produces:** `applyParseResult` gains one param `useRawDecisions: UseRawDecision[]` and returns `useRawInvalidated: UseRawDecision[]` on its outcome. It runs the overlay BEFORE the full-replace writes, and persists the new `use_raw_decisions` = `kept` each `applied:true` (drop `invalidated`+`reverted`) via the SAME `tx.upsertShowsInternal(...)` that already writes `parse_warnings` (:207-214). It does NOT write change-log (keeping its current write-scope). **The caller writes the `USE_RAW_DECISION_STALE` change-log rows** — `phase2.ts` already owns change-log writes for this apply (`writeAutoApplyChanges` at `:389`, inside the same locked txn, right after the `applyParseResult` call at `:368`); extend that path (or add a sibling `writeUseRawStaleChanges(port, showId, invalidated)`) to emit one doug-facing `USE_RAW_DECISION_STALE` row per `invalidated` decision, naming `target`. `reverted` rows drop with NO change-log row. (Finalize goes through the same `applyParseResult`→phase2 machinery via `applyStagedCore`; the two paths differ only in which `use_raw_decisions` column supplies the decisions.)
...
- [ ] **Step 1: Failing tests:**
  ...
  - **Staged→persisted finalize migration:** a staged `pending_syncs.use_raw_decisions` decision applies at finalize and lands in `shows_internal.use_raw_decisions`; a re-ingestion that regenerated the staged parse (new hash) → `invalidated` at finalize (surfaced). Assert against persisted `shows_internal` + change-log.
...
**Files:**
- Create: a `"use server"` module (e.g. `app/admin/onboarding/_actions/useRawStaged.ts`)
...
- [ ] **Step 1: Failing tests:** decision written to `pending_syncs.use_raw_decisions` under `withShowLock`; toggle-off deletes; `driveFileId`/lock key server-derived from the `pending_syncs` row (not client arg); NO re-apply; behavioral proof of the forensic emit; infra-fault typed result; `warningRef` validated against `parse_result`.
- [ ] **Step 2: Run to fail.**
...
- Modify: `components/admin/PerShowActionableWarnings.tsx` (pass via `renderItemControls` :31/:103) + `components/admin/show/[slug]/page.tsx` wiring; `components/admin/wizard/step3ReviewSections.tsx` (judgment callout :505-517) + `components/admin/OnboardingWizard.tsx`
...
**Transition Inventory (from spec §8) — the transition-audit task MUST cover:** `transform-active → apply-pending → raw-active`; `raw-active → clear-pending → transform-active`; `apply-pending → transform-active` (toggle-off = delete); `clear-pending → raw-active` (immediate, applied:true); each pending state SURVIVING a reload after a failed re-sync; the optimistic-`pending` compound. Plus the guard states `disabled` / `legacy-unavailable` (distinct copy).

- [ ] **Step 1: Failing tests (jsdom logic):** state derivation for all four persisted shapes + the three guard branches; `current` reads `resolution.parsed` (assert it does NOT read entity rows — pass an overlaid entity row and a differing `resolution.parsed`, assert the parsed value renders). Anti-tautology: clone+strip sibling nodes before scanning for a label.
- [ ] **Step 2: Run to fail.**
