...
**Meta-test inventory (writing-plans rule):** this plan EXTENDS `tests/mutation/source/registry.ts` (one `GuardSurface` row), `tests/mutation/source/expectedLedgerKinds.ts` (one row), `tests/mutation/_metaPremiseContract.test.ts` (THREE suite-path declaration rows — one per enrolled deciding suite; the enforcing equality is one `EXPECTED_ENV_TOUCHING` key per distinct suite path, `tests/mutation/_metaPremiseContract.test.ts:535`), and `tests/docs/_metaDeferralLedgerGraduation.test.ts` (`BACKLOG_GRADUATED` — one row). It CREATES no new meta-test. `tests/specLint/_metaPureCore.test.ts` covers the new module automatically (recursive walk over `lib/specLint/`). No Supabase call boundary, no admin alert, no tile, no advisory lock registry applies.
...
- [ ] Commit-order rule (plan review R2 F1, writing-plans RED-validity): the module SKELETON lands FIRST — `checkExpectN` exported with the spec §6 signature, returning `[]` — so every import resolves and the observed RED is BEHAVIORAL: the fixture cases fail on their assertions (`expected 1 finding, got 0`), never on module loading.
- [ ] Observe RED: both suites fail on assertions against the skeleton (zero findings emitted; corpus fire-set empty while the premise proves the walk non-empty).
- [ ] GREEN: implement `checkExpectN` exactly as spec §4.1 — the end-anchored pattern, rule 2, the CLOSED opener list. Nothing else.
- [ ] Pre-dispatch mutants (recorded in the commit message). String-presence quartet on the §4.5 message/code/detail assertions: (a) the message string emptied, AND the detail sentence emptied, AND `detail` dropped entirely — the detail-presence assertion must fail; (b) the expected message plus an appended suffix, likewise the detail sentence; (c) the code present in the module but not live on the emit path (behind a false condition) — the fixture asserting the emitted finding carries `EXPECT_N_UNENFORCED` must fail; (d) each discriminating parameter varied in turn — the anchored line, the `#` column, the captured N, and the trimmed command swapped for the untrimmed line. Grammar mutants on top: (e) end-anchor removed — the corpus suite must fail with the two §4.2 sites appearing in the fire-set; (f) opener list emptied — the opener fixture must fail; (g) `\d+` swapped to `.+` — the non-integer fixtures (`green`, `empty`) must fail by FIRING (probed at plan review R1: `\d*` is NOT killed by them — `[ \t]+` still refuses the non-integer tail — so `.+` is the discriminating mutant). Run each against the suites; all must go red; revert.
...
- [ ] Commit-order rule (R2 F1): the three Arm B export skeletons land FIRST (`playwrightCollectionPlan` returning `[]`, `configsToProbe` returning `[]`, `synthesizeCollectionVerdicts` returning `[]`), so the observed RED is behavioral: extraction fixtures assert candidates the skeleton does not produce; corpus Arm B assertions fail on the empty candidate set while the premise proves the corpus non-empty.
- [ ] Observe RED on the skeletons.
- [ ] GREEN: implement per spec §5.1–§5.3.
...
<!-- task: red=`pnpm vitest run tests/specLint/expectContractCli.test.ts` red-state=authored red-target=`lib/specLint/run.ts:102` why=`the task lands the buildCollectedSets skeleton (empty map) and the accepted-but-unused runLint parameter stub FIRST, so the observed RED is behavioral: adapter unit cases assert normalized keys and unavailable records the empty map does not hold, the spawn recorder asserts two spawns while none are issued, and the render cases assert EXPECT_N_UNENFORCED and the Arm B codes that never appear - green arrives when the parameter is consumed, the arms are called, and the --exec-red block spawns, builds and injects` ac=AC-5 -->
...
- Modify: scripts/spec-lint.ts — TWO pieces, both TESTED (plan review R1 F4; the tested-adapter precedent is `classifySpawnResult` (`scripts/spec-lint.ts:135`), `runCli` (`scripts/spec-lint.ts:455`) and `nodeDeps` (`scripts/spec-lint.ts:838`), imported by `tests/specLint/redExec.test.ts:6`):
  - a new EXPORTED adapter function `buildCollectedSets(results, repoRoot)` — input one record per distinct config `{ config, outcome, stdout, stderrTail }` (the tail captured by the spawn seam, mirroring the existing separate-tail channel beside `ExecOutcome`, `lib/specLint/types.ts:183`); parses reporter JSON, reads that report's own `config.rootDir`, joins + repo-relativizes each `file` into §5.1 token spelling, returns the `ReadonlyMap<string, ReadonlySet<string> | { unavailable: string }>` the pure core consumes; a non-exit-0 or unparseable outcome becomes `{ unavailable }` carrying the reason AND the stderr tail (spec §5.3 requires the advisory to carry both; reusing `ExecOutcome` classification, `lib/specLint/types.ts:173`);
...
- [ ] RED (render): a fixture plan doc with an Arm A site renders `EXPECT_N_UNENFORCED` in the default (no-flag) run; the same doc with a Playwright candidate renders NO Arm B code without `--exec-red`; with `--exec-red` and the recorder deps returning the fixture report, the absent-file case renders `PLAYWRIGHT_COLLECTS_NOTHING` and the unavailable case renders `PLAYWRIGHT_COLLECTION_UNVERIFIED` WITH the stderr tail visible in its rendered detail (spec §5.3). Silent-drop of the codes in the render path is the `CHECK_ORDER` incident class (`lib/specLint/types.ts:14-33`) — both arms report under `taskContract`, so no renderer edit is needed and the test proves it.
- [ ] Commit-order rule (R2 F1): the `buildCollectedSets` export skeleton (returning an empty map) and the appended `runLint` parameter STUB (accepted, unused) land FIRST, so the observed RED is behavioral: the adapter unit cases assert normalized keys and `{ unavailable }` records an empty map does not hold; the spawn-recorder case asserts two spawns while none are issued; the render cases assert codes that never appear.
- [ ] Observe RED on the skeletons.
...
<!-- task: red=`pnpm vitest run tests/mutation/_metaPremiseContract.test.ts tests/mutation/_metaGuardSurfaceRegistry.test.ts` red-state=authored red-target=`tests/mutation/_metaPremiseContract.test.ts:32` why=`adding the specLintExpectContract registry row WITHOUT its suite-path declarations reds the premise-contract walk (fail-by-default on newly enrolled suites; one EXPECTED_ENV_TOUCHING key per distinct suite path is the enforcing equality at :535, so THREE rows are owed) - red observed after the registry edit and before the declarations; the SAME command greens with all three rows` ac=AC-7 -->
...
- Modify: tests/mutation/_metaPremiseContract.test.ts (THREE suite-path declaration rows — one `EXPECTED_ENV_TOUCHING` key per enrolled deciding suite: expectContract.test.ts, expectPlaywright.test.ts, expectContractCorpus.test.ts — the enforcing equality at `tests/mutation/_metaPremiseContract.test.ts:535`)

**Steps:**

- [ ] Registry row first; observe RED on the premise-contract walk (three missing declarations, one per enrolled suite path); add all THREE declaration rows; GREEN.
