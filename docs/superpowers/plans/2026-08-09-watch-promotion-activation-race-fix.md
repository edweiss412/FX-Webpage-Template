...
- [ ] **Step 1: Write the failing tests** (fake-tx suite, spec §5 rows 2+4). Using the Task 1 fake with `settingsRow` set to `{ watched_folder_id: <a different folder than the subscribed one> }`, drive `subscribeToWatchedFolder(folderId, { recordAttempt: true, watchFolder: stub, tx: fake, ... })` and assert:
  ...
  - `classifyWatchError` recorded zero calls on this path — executable seam (plan review R3 finding 2: `SubscribeDeps` has no classifier member, `lib/drive/watch.ts:157-177`): a partial module mock at the top of the test file, `vi.mock("@/lib/drive/watchErrors", async (importOriginal) => { const real = await importOriginal(); return { ...real, classifyWatchError: vi.fn(real.classifyWatchError) }; })`, then assert `vi.mocked(classifyWatchError).mock.calls.length === 0` in this case (and leave every other case's behavior untouched since the wrapper delegates)
