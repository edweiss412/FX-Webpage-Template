...
- [ ] **Step 5: Commit.** `git commit -m "feat(sync): folder_changed consumer branches in renewal + reconcile + retry pin (AC-3, AC-4, AC-5)"`
...
- [ ] **Step 1: Write the test.** Two real connections, loopback-guarded. State captured and restored in `finally`: `app_settings` `'default'` row, `drive_watch_channels` fixture rows, AND the global `WATCH_CHANNEL_ORPHANED` `admin_alerts` row — the real mismatch path upserts it via `markWatchOrphanedWithTx` (`lib/drive/watch.ts:810-833`; plan review R1 finding 5). Port S1–S4 from the probe with its phase acknowledgements and `settledWithin` still-pending checks (spec §5 test 1: sleeps are not synchronization). Every assertion states FIXED behavior: S1-schedule case → 0 stale rows (assert via the probe's predicate `status='active' and watched_folder_id is distinct from settings` against the DB) + `folder_changed` outcome; S2 → subscriber pending while promo holds lock, then abort + 0 stale; S3 → activation then promotion supersede, 0 stale; S4 (instrumented-connection seam per the Interfaces block) → promotion pending while share held, both complete, 0 stale, no deadlock (vitest timeout is the bound). Premise guards: fixture folder ids differ (assert), seeds wrote 1 row (assert).
- [ ] **Step 2: Run — mutant-RED protocol.** This task executes the spec's RED phase (spec §5 test 1: "red-first by failing on current code") as a controlled mutant, because in plan order the mechanism already landed in Tasks 1–3: (i) `pnpm vitest run tests/db/watchActivationRace.db.test.ts` → PASS; (ii) comment out the guard select inside `PostgresWatchTx.activatePending` (a local, uncommitted edit reproducing the pre-arc code exactly) → the S1-schedule case MUST FAIL with 1 stale row — the spec-named failure — and S2 must fail its still-pending assertion; (iii) restore, re-run → PASS. Record all three runs in the commit message. The probe's committed S1 measurement is the corroborating pre-implementation evidence.
...
- [ ] **Step 2: Verify.** `pnpm vitest run tests/docs` → PASS (the BACKLOG entry is still present and still carries its claim marker at this point — the ledger meta-suite accepts an in-progress entry on a live branch, which this is). `grep -rn "narrow concurrent-promotion schedule" docs/` → zero hits.
- [ ] **Step 3: Commit.** `git commit -m "docs: restore AC-6.18 absolute, close the documented race window (AC-7)"`
...
- [ ] **Step 2: Graduation commit (the PR's last commit).** Apply the pre-declared content above. Run `pnpm vitest run tests/docs` → PASS (the ledger meta-suite verifies: archive holds no in-flight entry, BACKLOG.md no longer lists it). `git commit -m "docs: graduate BL-WATCH-PROMOTION-ACTIVATION-RACE (AC-7)"`
- [ ] **Step 3: Ship.** Push → open PR (body: conventional summary + `impeccable-gate: N/A — no UI surface`; preflight was green, not skipped) → real CI green → `gh pr merge --merge` in the SAME turn → sync main checkout (`git -C /Users/ericweiss/FX-Webpage-Template pull --ff-only`, verify `git rev-list --left-right --count main...origin/main` = `0  0`).
- [ ] **Step 4: Stage 4.4.** `CronDelete` the session's nudge job; `herdr pane rename "$HERDR_PANE_ID" --clear`; `herdr agent rename "$HERDR_PANE_ID" --clear`; set ship-state `stage: "done"`.
