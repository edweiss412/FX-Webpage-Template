...
- [ ] **Step 1: Write the failing tests — fake rows AND real-adapter rows (both RED, plan review R2 finding 1).** (a) Fake level (spec §5 test 2): in `tests/drive/watch.test.ts`, add four guard-rule rows — absent / NULL / match / mismatch — driving the FakeTx's `activatePending` and asserting the discriminated result shape; these FAIL against the current scalar-returning fakes (`tests/drive/watch.test.ts:63-69`). (b) Real adapter: in `tests/db/watchLifecycle.db.test.ts`, add a `describe("activation guard")` with the four §3.1 guard-rule rows, run through `createPostgresWatchTx` with the file's existing connection/cleanup idiom (`tests/db/watchLifecycle.db.test.ts:514-548` is the structural template — note those cases run ordinary `sql.begin` and observe COMMITTED state; there is no rollback isolation, so restoration is explicit). Settings isolation for the WHOLE `activatePending` describe-block population, new AND pre-existing cases alike (plan review R3 finding 1): a describe-level `beforeAll` captures the current `app_settings` `'default'` row, each case seeds its own shape (the pre-existing cases seed the row to their own fixture folder so their original subject is preserved), and `afterAll`/`finally` restores the captured row. Premise guards (`tests/_shared/premise.ts`), exact per case — never an OR that a failed setup can satisfy vacuously: seeded cases assert `seedCount === 1`; the row-absent case asserts a post-delete `select count(*)` of 0. Cases (derive folder ids from one fixture constant, not literals repeated inline):
...
then the existing two UPDATEs unchanged, returning `{ promoted: promoted.length, abortedFolderMismatch: false }`. Update the `WatchTx` type. Update both fakes (`tests/drive/watch.test.ts:63-69` FakeTx; `tests/drive/watchExpiration.test.ts:54-69` minimal tx): add a public field `settingsRow: { watched_folder_id: string | null } | null` whose `activatePending` applies the same four-row rule in memory (null field = row absent) and returns the new shape. This implements the spec §3.2 test-port contract (aligned again at plan review R5 finding 2: `FakeWatchTx` receives no folder at construction, `tests/drive/watch.test.ts:40-46`, and existing bare instances subscribe with different folders — `"folder-1"` at `tests/drive/watch.test.ts:410`, `"folder-lifecycle"` at `tests/drive/watch.test.ts:708-711` — so a matched-folder default is not derivable). Default: `settingsRow = null` (row absent), which the four-row rule maps to PROCEED — behaviorally identical for every existing case, no per-case wiring. The four guard-row cases set `settingsRow` explicitly. The step-1a fake rows go green here. Update EVERY `activatePending` override site to the new result shape (comprehensive-audit enumeration, so none is left returning a bare number): `tests/drive/watch.test.ts:2060` (zero-rows case → `{ promoted: 0, abortedFolderMismatch: false }`); `tests/drive/watch.test.ts:556-564` (throwing override — only its `Promise<number>` annotation changes to `Promise<ActivatePendingResult>`); `tests/drive/watch.test.ts:2709` and `tests/drive/watch.test.ts:2719` (both throw — `Promise<never>` is assignable to the new return type, so NO change needed; listed to close the enumeration); `tests/drive/watchExpiration.test.ts:156-159` and `tests/drive/watchExpiration.test.ts:177-180` (each `return 1` → `return { promoted: 1, abortedFolderMismatch: false }`), plus the minimal tx's `activatePending: async () => 1` at `tests/drive/watchExpiration.test.ts:69`. **Update the EXISTING real-adapter cases** at `tests/db/watchLifecycle.db.test.ts:514-548` (plan review R1 finding 1): their assertions read the scalar count today — change to `.promoted` on the new shape, and have their setup seed the `app_settings` `'default'` row to each case's own folder — matched-folder port cases per spec §3.2, never the delete/absent variant (plan review R4 finding 2) — so the guard's match row applies and the cases keep their original subject. Extend the structural source test (`tests/drive/watch.test.ts:382-385`) to also assert the guard select string (`for share`) appears inside `activatePending` before the supersede UPDATE.
...
- [ ] **Step 1: Write the failing tests** (fake-tx suite, spec §5 rows 2+4). Using the Task 1 fake with `settingsRow` set to `{ watched_folder_id: <a different folder than the subscribed one> }`, drive `subscribeToWatchedFolder(folderId, { recordAttempt: true, watchFolder: stub, tx: fake, ... })` and assert:
  ...
  - the fake tx recorded ZERO `recordAttemptFailure` (and zero `recordAttemptSuccess`) operations despite `recordAttempt: true` — those are the exact `WatchTx` methods `recordAttemptSafe` drives (`lib/drive/watch.ts:844-850`)
  - the log capture — via the sanctioned `setLogSink` seam already used file-wide in this suite (`tests/drive/watch.test.ts:10-25`, persist-free) — recorded exactly ONE entry with `code === "DRIVE_WATCH_ACTIVATION_FOLDER_CHANGED"` (match the code field, not message text) and no `webhookSecret` in its payload
  - `classifyWatchError` recorded zero calls on this path — executable seam (plan review R3 finding 2: `SubscribeDeps` has no classifier member, `lib/drive/watch.ts:157-177`): a TYPED partial module mock at the top of the test file, following the repo pattern at `tests/reports/unknownOutcome.test.ts:16-22`: `vi.mock("@/lib/drive/watchErrors", async (importOriginal) => { const actual = await importOriginal<typeof import("@/lib/drive/watchErrors")>(); return { ...actual, classifyWatchError: vi.fn(actual.classifyWatchError) }; })` — and ADD `classifyWatchError` to the test file's imports from `@/lib/drive/watchErrors` (it is not currently imported, `tests/drive/watch.test.ts:4-9`; plan review R5 finding 3). Assert `vi.mocked(classifyWatchError).mock.calls.length === 0` in this case (every other case is untouched since the wrapper delegates)
...
- [ ] **Step 3: Implement.** `activateWithTx`: on `result.abortedFolderMismatch` → `throw new WatchFolderChangedDuringActivationError(result.configuredFolderId)`; on `result.promoted === 0` → existing `DriveWatchInfraError` path unchanged. In `subscribeToWatchedFolder`'s activation try/catch, FIRST branch: `if (err instanceof WatchFolderChangedDuringActivationError)` → run `markWatchOrphanedWithTx` with the full watch payload (reason `"folder_changed_during_activation"`, `watch.resourceId` passed) → awaited fail-open `log.warn("drive watch activation aborted: folder changed", { source: "drive.watch", code: "DRIVE_WATCH_ACTIVATION_FOLDER_CHANGED", watchedFolderId: folderId, configuredFolderId: err.configuredFolderId, channelId: watch.id })` (bindings are the live scope's identifiers — `folderId` the function parameter, `watch.id` the Drive channel, `err` the caught error; plan review R3 finding 2) → `return { outcome: "folder_changed", channelId: watch.id, configuredFolderId: err.configuredFolderId }`. Add the `folder_changed` union member to `SubscribeResult`. Do NOT widen `SubscribeOrphanReason` (plan review R5 finding 1): that union exclusively types the `reason` field of the `"orphaned"` result member (`lib/drive/watch.ts:138`, `lib/drive/watch.ts:151`), and the ratified `folder_changed` member carries no `reason`. The `"folder_changed_during_activation"` string appears ONLY in the orphan context payload passed to `markWatchOrphanedWithTx`, whose `context` parameter is `Record<string, unknown>` (`lib/drive/watch.ts:813`) — no type change required.
...
<!-- task: red=`pnpm vitest run tests/drive/watch.test.ts tests/admin/retryWatchAction.test.ts -t "folder_changed consumer"` ac=AC-3,AC-4,AC-5 -->
...
- Test: `tests/drive/watch.test.ts`; `tests/admin/retryWatchAction.test.ts` (existing suite for the action)
...
- [ ] **Step 3: Implement.** Consumer coverage note, declared so the consequence bound maps every §3.2 consumer (plan review R4 finding 4): the two finalize-cas await-and-discard sites (`app/api/admin/onboarding/finalize-cas/route.ts:1190`, `app/api/admin/onboarding/finalize-cas/route.ts:1290`) receive NO code change per spec §3.2's ratified disposition — a `folder_changed` there is recorded by the §3.1 warn and superseded by the newer promotion's own subscribe; the union addition compiles through them untouched (they never narrow the result). Renewal loop: before the existing orphan-reason handling, `if (result.outcome === "folder_changed") { continue; }` (after neither pushing nor emitting — the §3.1 warn already recorded it). Reconcile: in the still-orphaned branch, `if (result.outcome === "folder_changed") { outcome = "folder_changed"; }` BEFORE the `attempt === null` check, skipping it and the alert resolves; add the union member with the never-persisted comment mirroring `lib/drive/watch.ts:1407-1408`; leave the escalation condition list untouched.
- [ ] **Step 4: GREEN + mutant check + suites.** `pnpm vitest run tests/drive/ tests/admin/retryWatchAction.test.ts tests/api/cron-sync.test.ts tests/db/watchReconcileState.test.ts` → PASS. Retry-pin mutant (four-mutant discipline variant d): temporarily invert the action's branch to `result.outcome !== "active"` locally, run `pnpm vitest run tests/admin/retryWatchAction.test.ts -t "folder_changed consumer"`, confirm it FAILS, restore, re-run PASS; record both runs in the commit message.
...
- [ ] **Step 1: Write the test.** Two real connections, loopback-guarded. State captured and restored in `finally`: `app_settings` `'default'` row, `drive_watch_channels` fixture rows, AND the global `WATCH_CHANNEL_ORPHANED` `admin_alerts` row — the real mismatch path upserts it via `markWatchOrphanedWithTx` (`lib/drive/watch.ts:810-833`; plan review R1 finding 5). The mismatch path's persistent warn would insert `app_events` rows (`lib/log/logger.ts:57-82` via `lib/log/persist.ts:12-26`; plan review R2 finding 3) — prevent rather than clean: install a silent persist-free sink for the suite via the sanctioned `setLogSink` seam, exactly the idiom at `tests/drive/watch.test.ts:10-25`, restoring it in `afterAll`. No `app_events` writes occur, so no restoration query is needed. Port S1–S4 from the probe with its phase acknowledgements and `settledWithin` still-pending checks (spec §5 test 1: sleeps are not synchronization). Every assertion states FIXED behavior: S1-schedule case → 0 stale rows (assert via the probe's predicate `status='active' and watched_folder_id is distinct from settings` against the DB) + `folder_changed` outcome; S2 → subscriber pending while promo holds lock, then abort + 0 stale; S3 → activation then promotion supersede, 0 stale; S4 (instrumented-connection seam per the Interfaces block) → promotion pending while share held, both complete, 0 stale, no deadlock (vitest timeout is the bound). Premise guards: fixture folder ids differ (assert), seeds wrote 1 row (assert).
- [ ] **Step 2: Run — mutant-RED protocol.** This task executes the spec's RED phase (spec §5 test 1: "red-first by failing on current code") as a controlled mutant, because in plan order the mechanism already landed in Tasks 1–3: (i) `pnpm vitest run tests/db/watchActivationRace.db.test.ts` → PASS; (ii) apply the single-token mutant `if (false && settings.length > 0 …)` to the guard condition in `PostgresWatchTx.activatePending` (compiles, disables exactly the abort rule — plan review R2 finding 2 rejected the comment-out form, whose dangling `settings` reference would not build) → the S1-schedule case MUST FAIL with 1 stale row (the spec-named failure) and S2's OUTCOME assertion MUST FAIL with result `"orphaned"` in place of `"folder_changed"` (plan review R3 finding 4: under the disabled guard, promotion's in-tx orphan UPDATE has already converted the pending row, `app/api/admin/onboarding/finalize-cas/route.ts:871-878`, so activation matches zero `pending` rows, `lib/drive/watch.ts:270-282`, and routes down the existing zero-rows infra path to the generic orphaned outcome, `lib/drive/watch.ts:1009-1035` — a different wrong answer than `folder_changed`, and the assertion discriminates it; S2's still-pending assertion is NOT expected to fail, since pre-arc code still blocks on the channel-row lock); (iii) restore, re-run → PASS. Record all three runs in the commit message. The probe's committed S1 measurement is the corroborating pre-implementation evidence.
