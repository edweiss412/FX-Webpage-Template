...
**Concrete failure mode caught:** a future route layout adding its own `Inter()` call, which re-registers a second `@font-face` set under the same family name. The probe already observed **seven** `Inter` faces on the crew page from the single existing loader; a second loader compounds that silently, and the only other thing that would notice is the runtime pair below, and only when the two loaders' faces differ. Class-sweep discipline: filesystem walk, not a named-file scan.
