...
**Concrete failure mode caught:** a future route layout adding its own loader call, which re-registers a second `@font-face` set under the same family name. The probe observed **seven** `Inter` faces on the crew page from the single existing loader — a count that was true of `next/font/google`, which emits one face per `unicode-range` slice, and became **one** when `BL-INTER-NUMERAL-DISAMBIGUATION` moved to a single self-hosted file on 2026-08-03; a second loader compounds that silently. The only other thing that would notice is the runtime pair below, and only when the two loaders' faces differ AND the module is reached by one of the three routes that suite visits. Class-sweep discipline: filesystem walk, not a named-file scan.
...
**EDIT `app/globals.css`.** `--font-sans` becomes `var(--font-inter, "Inter", "Inter Fallback"), ui-sans-serif, …`. Naming the literal `"Inter"` skips next/font's generated metric-matched `Inter Fallback` face — built with `size-adjust`/`ascent-override` so the `display: "swap"` window reflows far less (**not zero** — corrected 2026-08-03 by `BL-INTER-NUMERAL-DISAMBIGUATION`: metric overrides narrow the mismatch but cannot equalise per-string advances; measured residual 0.2983em on this suite's own probe string) — so first paint renders a system font at native metrics and then snaps ~10% on every React-root route. The `var()` **fallback list** is what makes this safe: where the token is undefined (the standalone harnesses, which compile this file with no Next runtime) it substitutes the literal pair rather than leaving the declaration invalid at computed-value time, which would drop `font-family` entirely.
...
| **P3** | `font-feature-settings: "tnum" 1, "cv11" 1` was host-font-dependent outside the crew tree and now deterministically activates Inter's alternates on admin/auth/help for the first time. | **ACCEPTED as written, and the second half was WRONG — corrected 2026-08-03 by `BL-INTER-NUMERAL-DISAMBIGUATION`.** The `tnum` half stands: figure alignment was host-dependent outside the crew tree and is now deterministic. The alternates half was false. No alternate was activated on any tree, because the Inter build Google Fonts serves has the character variants stripped — probed live, the latin subset carries only `calt ccmp dnom frac kern locl mark mkmk numr pnum tnum`. `cv11` had rendered nothing since `78662acb5` (2026-05-03) and went on rendering nothing after this change. The row is left standing rather than rewritten, because what a review believed at the time is part of the record; the correction is that binding a family cannot activate features the served file does not contain. Reaching them required self-hosting the upstream release. |
