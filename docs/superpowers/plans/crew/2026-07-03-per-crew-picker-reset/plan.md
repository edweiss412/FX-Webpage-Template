...
**Spec:** `docs/superpowers/specs/crew/2026-07-03-per-crew-picker-reset.md` (APPROVED, Codex 6 rounds). Every task cites the spec section it implements. Read the spec's §2 Resolved Decisions before starting.
...
- [ ] **Step 1: Write the invariant test:** set `selections_reset_at` on a crew row, run the sync UPSERT (`upsertCrewMembers` in `runScheduledCronSync.ts:1306-1338`) with the **same name** and a changed **non-name** field (e.g. `role`), assert `selections_reset_at` is unchanged. **Note (discovered during impl):** the sync UPSERT is `on conflict (show_id, name) do update set email/phone/role/role_flags/date_restriction/stage_restriction/flight_info` — column-scoped, so a same-name update preserves `selections_reset_at` (and `claimed_via_oauth_at`). A **name change** is delete+insert (marker lost), which is acceptable and identical to `claimed_via_oauth_at`'s behavior (a rename re-stages the member). So the invariant is specifically about same-name field updates.
- [ ] **Step 2: Run.** Expected: **PASS** (the `do update set` list omits `selections_reset_at`). This guards against a future edit adding `selections_reset_at = excluded.selections_reset_at` (or the column) to the UPSERT. If it **FAILS**, that's a real bug — the fix is to keep `selections_reset_at` out of the sync UPSERT column list.
