...
**Spec:** `docs/superpowers/specs/crew/2026-06-23-crew-client-section-toggle.md` (Codex-APPROVED round 1).
...
- [ ] **Step 1 (failing test):** update `crewShellSections.test.tsx`. The contract is "all entitled bodies are RENDERED SERVER-SIDE and HANDED to the controller" — NOT "all in the DOM" (the controller mounts only the active one via `AnimatePresence mode="wait"`, T2). So `vi.mock("@/components/crew/CrewSections")` with a capturing stub and assert `_CrewShell` calls it with `sectionNodes` whose keys === the entitled set: for a LEAD viewer `[...BASE_SECTION_IDS, "budget"]` (7); for a NON-lead exactly `BASE_SECTION_IDS` (6) with **no `budget` key**; and `initialSection` === the server-resolved active (a non-lead `?s=budget` → `today`). **Per-id correctness (anti-tautology — keys alone can pass with a mis-wired body):** `vi.mock` each section component (`TodaySection`…`BudgetSection`) to emit a distinct marker `<div data-testid={"section-"+id} />`, then for EVERY entitled id assert `render(sectionNodes[id]!)` shows `section-<id>` — proving `today`→TodaySection, `schedule`→ScheduleSection, …, not a swapped wiring. Keep a small integration assertion (unmocked) that the INITIAL active body renders + `data-testid="crew-shell"` is present. Update `crewShellAlert.test.tsx` to confirm `upsertAdminAlert` still fires ONCE when `tileErrors` non-empty (now once per load, section-independent). Run → FAIL.
- [ ] **Step 2:** run, verify fail.
...
- [ ] **Step 2 (failing perf-budget test) — payload AND first-contentful (spec §9 line 108):** capture BOTH baselines first on `origin/main` (pre-change), loading the content-heavy lead fixture, into committed constants (each commented with the fixture + the `origin/main` commit they were measured at):
  - `CREW_PAYLOAD_BASELINE_BYTES` — crew-page document + RSC transfer bytes (deterministic). Post-change assert ≤ `× 1.25` (HARD).
  - `CREW_FCP_BASELINE_MS` — first-contentful paint via `performance.getEntriesByName("first-contentful-paint")` (the spec's first-contentful-render budget). FCP is run-to-run noisy, so the test takes the **median of 5 cold loads** and asserts the median ≤ `CREW_FCP_BASELINE_MS * 1.25 + 150ms` (a margin generous enough to ignore CI noise but catch a GROSS first-paint regression from rendering all bodies).
  Either assertion failing → **STOP and surface** (the all-sections-render tradeoff needs the lazy/cache-backed variant; do not silently ship a regression). (The "0 network requests on a section tap" hard assertion lives in T5.)
