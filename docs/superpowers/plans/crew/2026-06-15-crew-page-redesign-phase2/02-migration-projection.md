...
**Live-wiring pinning table (R20 — close the "fake passes, live producer unpinned" class for ALL of §02).** The recurring failure mode (R7/R17/R20) is a fake/mock test that passes while a LIVE producer or read is never wired. Every §02 live-wiring point is pinned by **tsc (required field)** OR a **source-scan guard** — NOT a fake/mock alone. A mock-only test is acceptable only where the mock keys off the live target name (so a wrong target fails the mock), and that is noted:

| Live-wiring point | Live surface | Pinning mechanism | Catches the dead-producer? |
|---|---|---|---|
| `priorRunOfShow` reaches `applyParseResult` | `applyShowSnapshot` RETURN (`phase2.ts:49-59`) | **tsc required field** (step 2) — every impl/fake must populate it | partial (forces a value, not the read) |
| Postgres `select run_of_show from shows_internal` | `PostgresPipelineTx.applyShowSnapshot` (`runScheduledCronSync.ts:938`) | **source-scan guard** (step 6) — asserts the SQL substring + `priorRunOfShow:` return wiring | **YES** (the load-bearing producer pin) |
| Sync `upsertShowsInternal.run_of_show` upsert | `PostgresPipelineTx.upsertShowsInternal` SQL (`:1324-1331`) | **source-scan guard (ADD, R20)** — `_insertParamsArityContract.test.ts` only scans `applyShowSnapshot` (verified, `:50`/`:83`), NOT `upsertShowsInternal`; add a `classMethodSource("PostgresPipelineTx","upsertShowsInternal")` substring assert for `run_of_show`, `$5::jsonb`, and `run_of_show = excluded.run_of_show` in the conflict clause (so the column is actually written, not just SELECT-listed) | **YES** |
| 3 tail callers' `parseWarnings` sourcing | cron `:2469-2474` / manual `:113-116` / staged `applyStaged.ts:1280` | **tsc required field** (R16, applied `ProcessOneFileResult`) **+ a runtime-spy test EACH** (PART-C cron, R16 manual, R17 staged — each asserts the warning reaches `logSync`/the tail, each with a stash-to-`[]`→red negative-regression) | **YES** (required field = floor; runtime tests = correct sourcing) |
| `insertSyncLog` `$5` union | `PostgresPipelineTx.insertSyncLog` (`:794-808`) | **runtime structural pin** (PART-C suite 2, via `makeSyncPipelineTx` + fake `unsafe` capturing `$5`) — independent of the tail | **YES** |
| Sync READS `parseResult.runOfShow` | the §01-owned parser field, read in `applyParseResult` | the executable `runOfShowConfirmedReplace` suite drives the REAL `runPhase2`→`applyParseResult` with `parseResult.runOfShow` set (NOT a stubbed reader) — the apply consumes the real field; §01 pins `parseAgenda` producing it | **YES** (real consumer path, not fake-only) |
| Projection `shows_internal.run_of_show` service-role read | `getShowForViewer.ts` read block (Task 02.5) | **source-scan guard (ADD, R20)** — `tests/data/getShowForViewerRunOfShow.test.ts` mocks key off the table NAME `shows_internal` (a wrong target → mock default → assertions fail), but add a structural assert reading `getShowForViewer.ts` source for `.from("shows_internal")` + `.select("run_of_show")` so the read TARGET is pinned, not only the mock | **YES** (with the added source-scan) |

> **THREE NEW source-scan guards (R20)** — co-locate them in one `tests/sync/runOfShowLiveWiring.meta.test.ts` (or fold into the relevant test files): (1) `applyShowSnapshot` reads `shows_internal.run_of_show` → `priorRunOfShow` (step 6); (2) `upsertShowsInternal` writes `run_of_show` (`$5::jsonb` + `excluded.run_of_show`); (3) `getShowForViewer` reads `.from("shows_internal").select("run_of_show")` (Task 02.5). Each lifts `classMethodSource` (or `readFileSync` for the non-class `getShowForViewer` function) and asserts the live substring, with a stash-the-line→confirm-RED negative-regression. The rest are covered by the required field or runtime spies per the table.
...
2. **Producer interface — `priorRunOfShow` is REQUIRED (R20 producer-side defense, mirrors the R16 consumer required-field):** extend the `Phase2Tx.applyShowSnapshot` return `"updated"` branch (`phase2.ts:49-59`) with **required** `priorRunOfShow: Record<string, AgendaEntry[]> | null` (NOT optional; first-seen = `null`, never absent) — sibling of `previousCrewNames`/`previousCrewMembers`. **Required, because:** if it were optional, an impl that extends the type + `applyParseResult` but FORGETS the live `select run_of_show` from `shows_internal` (step 3) still TYPECHECKS and passes every fake-seeded test, while production silently never emits `AGENDA_DAY_EMPTIED` (the exact R20 dead-producer class). Required forces EVERY `applyShowSnapshot` impl (the Postgres one AND every fake) to populate it — a return without it fails `pnpm typecheck`. The `snapshot` built at `phase2.ts:264` carries it, flowing into `applyParseResult(tx, { …, snapshot })` (`phase2.ts:343-350`) as `args.snapshot.priorRunOfShow`. (Note: `ApplyParseResultSnapshot.priorRunOfShow` in step 1 can stay OPTIONAL — it tolerates other snapshot sources — but the `applyShowSnapshot` RETURN, the actual live producer, is required.)
...
```
> **Negative-regression check (mandatory):** temporarily make the fake `applyShowSnapshot` return `priorRunOfShow: null` UNCONDITIONALLY (it stays type-valid since the field is required — `null` is the "nothing prior" value, modeling the R20 dead-producer that forgot the live `select`) and confirm cases (iii)/(iv)/CHANNEL-1/R6-cross-boundary FAIL — proving the suite exercises the prior-value plumbing, not a hand-built snapshot. Restore, then commit. (R6 class: a test green with the plumbing dead is worse than no test. Note: returning `undefined` is NOT a valid stash here — the required field rejects it at tsc; `null` is the realistic dead-producer shape.)
