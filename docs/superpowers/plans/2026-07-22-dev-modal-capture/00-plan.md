...
- [ ] **Step 1:** `pnpm add -D html-to-image modern-screenshot html2canvas pngjs @types/pngjs` (candidates as devDeps — winner promoted to `dependencies` in Task 7, losers removed; `pngjs` + typings stay as devDeps for the spike scan and the Task 9 e2e decoder, satisfying the strict-TS gate).
<!-- spec-lint: ignore — file created by this plan; not yet tracked -->
- [ ] **Step 2:** Write `scripts/devcapture-spike.mts` — Playwright chromium script that (a) boots `pnpm dev` (reuse the port/readiness pattern from `playwright.config.ts` webServer), (b) calls `deleteFixtureUsers()` FIRST (the set-session endpoint is create-only — `signInAs.ts:12` requires the wipe before each sign-in) then signs in via the `signInAs` helper (POST `/api/test-auth/set-session`, `ENABLE_TEST_AUTH=true`), (c) seeds a published show via the existing e2e seed helper (`tests/e2e/helpers/seedShowWithCrew.ts`), (d) opens the published review modal and prepares the §3.4 measurement: seed/overflow BOTH inner panes and append the two sentinel divs (rail `-review-rail`, content scroller `-review-content`) exactly as the eventual e2e will, (e) for each candidate library: `page.addScriptTag` its IIFE/dist build — and when a candidate ships no injectable browser artifact (ESM-only), bundle one first with `pnpm exec esbuild node_modules/<lib>/dist/index.js --bundle --format=iife --global-name=<libGlobal> --outfile=scratch/spike/<lib>.iife.js` (precedent: `tests/e2e/_step3ReviewModalBundle.mjs`) — evaluate a capture of `[data-review-modal-panel]` twice — once as-is, once with clone-side overrides lifting `max-height`/`overflow` on the panel and the two inner panes (`ShowReviewSurface` rail + main pane) — and save PNGs to `scratch/spike/<lib>-{plain,expanded}.png`, then programmatically pixel-scan each expanded PNG for BOTH sentinel colors (pngjs, installed here as a devDep alongside the candidates) and print a per-candidate PASS/FAIL line — expansion capability is judged by the scan, not by eye; fidelity items (shadows, corners) remain visual.
<!-- spec-lint: ignore — file created by this plan; not yet tracked -->
...
- [ ] **Step 4:** Commit: `git add docs/superpowers/plans/2026-07-22-dev-modal-capture/SPIKE.md scripts/devcapture-spike.mts package.json pnpm-lock.yaml && git commit --no-verify -m "chore(admin): dev-modal-capture library spike findings"`
...
- `useDevCapture(opts: { target: () => HTMLElement | null; request: CaptureTelemetryRequest; clientSnapshot: () => unknown; filenameSeed: string; preCapture?: () => Promise<void> }): { state: "idle" | "busy" | "error"; run: () => void; busyRef: React.RefObject<boolean> }` — `busyRef` is the SYNCHRONOUS lockout signal (true from the first line of `run()`, before React commits `state: "busy"`; false at settle). Host toggle handlers gate on `busyRef.current`, NOT on rendered state — rendered `state` drives visuals (`aria-disabled`, spinner) only. Closes the same-tick race where a toggle click lands between `run()` and the busy render — `run()` enters `busy` SYNCHRONOUSLY (inFlight ref + setState before any await; spec §2.2 amendment #2), then awaits `preCapture` (ShareHub passes popover-close + two-rAF here) INSIDE the busy window, then captures — §7 machine: concurrent capture+action, §4.5 reason classification, screenshot-reject ⇒ error, telemetry-reject ⇒ proceed, mounted-ref guard, 6 s error auto-clear with cleanup, busy no-reentry.
...
- Modify: `playwright.config.ts` — add `dev-capture` to the desktop-chromium project's `testMatch` alternation (the config is an explicit allowlist; an unlisted spec runs NOWHERE — add it in the same commit as the skeleton, and verify with `pnpm exec playwright test --list | grep dev-capture`)
<!-- spec-lint: ignore — file created by this plan; not yet tracked -->
...
- [ ] **Step 1b: Failing jsdom tests** (queries by testid only; §9.8):
  ...
  - activation-interval attack (the R2 P0, tightened per R5): inside ONE `act()` block dispatch capture-row click AND kebab click AND share-link click before any rerender flush, then flush — popover never appears (the synchronous `busyRef` gate, not rendered state, is what blocks); repeat with the second click after one `requestAnimationFrame` — same result;
...
- [ ] **Step 2:** Run — FAIL. **Step 3:** Implement all four files (row markup mirrors the mailto-row classes `ShareHub.tsx:455`; activation is BUSY-FIRST per spec §2.2 amendment #2: the row's onClick calls `run()` directly; BOTH ShareHub toggle handlers add `if (devCapture.busyRef.current) return;` as their FIRST line (synchronous ref gate — rendered busy state lags a commit and cannot close the same-tick race); ShareHub supplies `preCapture: () => { setOpen(false); return new Promise((r) => requestAnimationFrame(() => requestAnimationFrame(() => r()))); }` so the popover close + two settle frames happen inside the busy lockout).
...
- (No package change: `pngjs` + `@types/pngjs` already installed as devDeps in Task 1 Step 1.)
...
<!-- spec-lint: ignore — file created by this plan; not yet tracked -->
- [ ] **Step 1:** Flesh out the spec authored in Task 8 Step 1. Every session-establishing case calls `deleteFixtureUsers()` before `signInAs` (create-only endpoint, `signInAs.ts:12`) — spike, developer cases, and the non-developer case alike. (a) developer session (`ADMIN_FIXTURE` — exported at `fixtures.ts:25` — via `signInAs`), published modal open in the REAL app (copy the flow from `tests/e2e/published-review-modal.crew-actions.spec.ts:11-12` — it signs in via `signInAs` + seeds via `seedShowWithCrew` and opens the real modal; the `.layout` spec is a standalone harness and is NOT the pattern here), seed content so both inner panes overflow, inject two sentinel divs (distinct colors, `page.evaluate` appending to the rail scroller `-review-rail` (`ShowReviewSurface.tsx:841-842`) and the CONTENT scroller `-review-content` (`ShowReviewSurface.tsx:996-997` — the `overflow-y-auto` pane; `-review-main` is the outer flex container and would NOT prove inner-scroll expansion), click kebab → `share-hub-dev-capture`, await Playwright `download` event, unzip in-test (fflate), decode PNG with `pngjs`, assert ≥1 pixel of EACH sentinel color, assert `telemetry.json` parses with the three top-level keys and `modalKind: "published"`; (b) staged modal in the REAL app: seed a first-seen `pending_syncs` staged row directly via the supabase admin client — copy the insert pattern from `tests/e2e/admin-parse-panel.spec.ts:76-93` (the `step3-review-modal.interactions` spec is a bundled component harness, NOT an app flow) — then open the wizard Step3 review modal from the dashboard, seed/overflow BOTH staged inner panes, inject the SAME two sentinels (rail + `-review-content` scroller — identical testid pattern, shared `ShowReviewSurface`), capture, unzip, pixel-scan for BOTH colors (full §3.4 acceptance, not just `modalKind: "staged"`); (c) non-developer session (normal-admin fixture) → capture testids absent on both surfaces; (d) telemetry JSON string contains neither the seeded crew email nor the show's REAL 64-hex share token (`seedShowWithCrew` returns `shareToken` — `seedShowWithCrew.ts:86`).
