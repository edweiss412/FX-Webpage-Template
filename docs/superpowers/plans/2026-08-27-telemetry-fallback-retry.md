...
| a repeated identical string needs a seq toggle to re-announce | `components/admin/ShowRowActions.tsx:610` | `announcement.seq % 2 === 0 ? text : text + " "` |
...
2. No guard would have caught that. `tests/styles/_metaControlOutlineFill.test.ts:7-12` states its own threat fence: it pins the 57 census rows two arcs swapped and "does NOT defend against a contributor adding a NEW control at either token". The forward guard was cut after five review rounds (`docs/superpowers/specs/2026-08-16-control-outline-surface-fills-design.md` §5.2). So this is a policy the plan has to honour by reading, not a red the suite would have produced.
...
| The retry fails again | The server re-renders the same fallback: copy unchanged, control still mounted, counter incremented, region re-announces | The counter's parity toggle appends ` ` on alternate attempts so a screen reader does not swallow the repeated identical string (`components/admin/ShowRowActions.tsx:610`). Without it the second failed attempt is silent, which is the same silence the row exists to fix. |
...
Guards that walk a population this diff adds to, each named so a red is attributed rather than guessed at:

| Guard | Why it sees this diff | Expected |
| --- | --- | --- |
| `tests/styles/_metaTapTargetFloor.test.ts` | walks `app` + `components`, so the new button is scanned on arrival | CLEAR: `min-h-tap-min` is unconditional in the class string and no defeater accompanies it |
| `tests/styles/_metaEmDashCopy.test.ts` | parses string literals in changed files | no em dash and no `--` in any new string |
| `tests/styles/_metaControlOutlineFill.test.ts` | census of 57 rows | UNCHANGED. The new element is not a census row and the guard states it does not cover new controls |
| `tests/components/telemetry/transitionAudit.test.tsx` | its population is a literal list today, which is why Task 4 derives it | once Task 4 lands, a new file is covered with no edit |
| `tests/styles/_metaUndoAnnounceProvider.test.ts` | guards providers, not consumers | UNCHANGED. This diff adds no `AdminAnnounceProvider` and no raw context provider |
| `tests/log/_metaMutationSurfaceObservability.test.ts` | filesystem-walked, fails new surfaces by default | UNCHANGED. No mutating route handler and no `"use server"` action is added |
...
- [ ] **1.4** Assert `loadCronHealth` has been called once. Click. Assert it has been called exactly twice, and that the count is `renders + 1` where `renders` is the number of times the harness invoked `Page`, never a bare literal.
...
- [ ] **1.6** The consequence bound, written as assertions: `push` and `replace` were never called, and `window.location.reload` (spied) was never called. "Re-invokes the loader and never navigates or reloads" is then a test rather than a sentence.
...
- [ ] **3.3** Case 2: after one click the region's text contains `CRON_HEALTH_RETRY_ANNOUNCEMENT`, imported from the component rather than retyped.
...
<!-- task: red=`pnpm exec prettier --check docs/superpowers/plans/2026-08-27-telemetry-fallback-retry.md` red-state=authored red-target=`docs/superpowers/plans/2026-08-27-telemetry-fallback-retry.md` why=`the sibling closeout file does not exist yet, so its marker line is absent and tests/docs/_metaInvariant8Closeout.test.ts has nothing to read for this plan; the task writes it` ac=AC-8 -->
...
| AC-2 | One tap re-invokes the health loader, count 1 to 2 | Task 1, asserted on the `loadCronHealth` spy |
| AC-3 | The retry never navigates and never reloads | Task 1, `push`/`replace`/`location.reload` all asserted uncalled |
