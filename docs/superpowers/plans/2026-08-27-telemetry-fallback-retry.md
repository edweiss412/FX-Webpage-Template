...
- **Success is not announced.** §3.2 again: the control unmounts with the branch it lives in, which `app/admin/dev/telemetry/page.tsx:77` shows in one line. A finding that success needs an announcement is a proposal to hoist the announce channel above the branch, which is a different arc.
...
| the branch that replaces the fallback on a successful re-read | `app/admin/dev/telemetry/page.tsx:77` | `{health.kind === "ok" ? (` — one ternary, so a successful read replaces the whole fallback subtree, the button and its status region included |
...
| `tests/docs/_metaInteractionTimingInventory.test.ts` | compares a derived scan of `app/**` + `components/**` against the DESIGN.md §5.5 table in BOTH directions, so a new timer delay or any numeric binding named ms / delay / duration / timeout owes a row there | UNCHANGED, and this is a constraint on the control rather than a coincidence: it owns NO timer, the same decision as having no pending state (§3.2). A reviewer proposing a debounce or an auto-retry would owe a §5.5 row and a `DESIGN.md` edit |
...
- [ ] **5.4** Record the one behavioural limit this control has: on a successful retry the focused button unmounts with the branch (`app/admin/dev/telemetry/page.tsx:77`), so focus falls to the document body. **Grounded on this page's own ternary, and on nothing else.** Two earlier drafts cited `HealthAlertResolveButton` and `PerShowAlertResolveButton` as precedent; both citations were wrong in the same way, because neither file's cited line establishes an unmount. Removal there needs the server action's update, the revalidation, the unresolved-only query and the panel's row rendering, or in the second case an `onResolved` callback in a different component. The claim this plan actually needs is one ternary in one file. Recorded in the closeout, named in the PR body under "Unfixed peers", not filed.
