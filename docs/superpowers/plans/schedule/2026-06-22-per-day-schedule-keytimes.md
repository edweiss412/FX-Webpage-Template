...
**Spec:** `docs/superpowers/specs/schedule/2026-06-22-per-day-schedule-keytimes-design.md` (APPROVED — 13 adversarial rounds, 19 findings fixed). Section refs below (§N) point there.
...
- [ ] Run the RightNowHero + buildRightNowContext suites (client-component surfaces): `pnpm vitest run tests/components/crew/rightNowHero.test.tsx tests/components/buildRightNowContext.test.ts` → all green. (The structural transition audit `tests/crew/transitionAudit.test.ts` is created later in Task 18 — do NOT reference it here; it does not exist yet at this point in the sequence.)
