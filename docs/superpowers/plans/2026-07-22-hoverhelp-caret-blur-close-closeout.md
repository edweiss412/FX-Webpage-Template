...
## Stale review dispatch refutation (triage record)

A duplicate plan-review dispatch (`plan-r3b-out`, launched as a retry before `plan-r4-out` superseded it) returned BLOCKING at 18:40 CDT with a single finding: "T-E5 does not use the scrolly fixture / scrollY===1260 / style.top stability." REFUTED as stale: the finding reviewed the pre-repair brief snapshot; the shipped `tests/e2e/hoverhelp-geometry.spec.ts` T-E5 (commit 1e7a2d3e6 plan text; implemented in the Task 2/3 commits) is exactly the demanded scrolly style-stability variant (asserts `window.scrollY === 1260`, before/after `style.top` delta <= TOL, plus live-rect abutment). The authoritative plan verdict is `plan-r4-out`: APPROVE, "T-E5 repair is sound... no additional issues." Recorded so no future round re-raises it.
