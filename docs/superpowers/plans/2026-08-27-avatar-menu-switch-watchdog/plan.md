...
tests/components/auth/avatarMenuTransitionAudit.test.ts, in the shape `tests/show/claimedRowTransitionAudit.test.ts:24` already uses on the sibling, because a second shape for the same job is a second thing to learn. It reads the component through the TypeScript AST and holds a `DECLARED` list: one row per conditional, each with an `id`, what it is, a regex `marker` matched against the NODE TEXT, and its §4.6 treatment. The check is a BIJECTION rather than a count. Every conditional the AST finds must be matched by some row, and every row must match at least one conditional, so the failure names WHICH conditional nobody declared, with its line and its text.
...
The population is 30 sites: 15 `if`, 12 ternaries, 1 `switch`, and 2 `&&` guards whose right operand is JSX. A `&&` inside a boolean condition is NOT counted, because it is part of one branch rather than a branch of its own. Proved reachable rather than assumed: planting `switchBusy ? 0 : 1`, a shape the old regex could never have seen, reds the suite with `components/auth/AvatarMenu.tsx:132 [ternary] switchBusy ? 0 : 1`.
