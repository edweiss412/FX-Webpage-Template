...
| **Render-site dry run** (retires the class R8 BLOCKING-2 came from) | `rg -n --fixed-strings "<RightNowCard\|<PerShowCrewSection\|<ResolveAlertButton\|<RunFinalCASButton" tests app components` | Every render site of the four is inside a file this plan already deletes or retargets, with exactly two cross-file exceptions, both now handled: `tests/components/admin/FinalizeReentry.test.tsx:55` and `:68` (the `RunFinalCASButton` describe Task 8 removes; the `CleanupAbandonedFinalizeButton` describe at `:80` stays) and `tests/components/admin/RescanSheetButton.test.tsx:460` (the whole test Task 8 deletes). A JSX reference to a deleted component is a build break, so this is checked by RENDER SITE, not by import |
