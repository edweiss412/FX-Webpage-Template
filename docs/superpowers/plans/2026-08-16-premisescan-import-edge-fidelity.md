...
**AC-1's declared movement is unchanged by every repair above** — still exactly the sixteen tests Task 0 measured, which is what `_metaPremiseContract`'s exact-count equality re-checks on every run. That is NOT the same as "AC-1 holds": the criterion as written in spec §6 asks for ZERO movement, and this arc's headline is that it does not hold. §4's AC-1 amendment states the ratified exception; the invariant these repairs preserve is that they add nothing to it.
