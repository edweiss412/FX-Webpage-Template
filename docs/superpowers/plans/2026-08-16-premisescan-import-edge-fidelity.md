...
- [ ] **Step 4: Update `EXPECTED_LEDGER_KINDS`.** `tests/mutation/source/expectedLedgerKinds.ts:33` declared `premiseScan: { equivalent: 3, "accepted-gap": 1 }` when this step was written and asserts it with `toEqual`, so retiring the row reds it until the declaration matches the ledger. **Shipped value: `{ equivalent: 2 }`** — the retirement happened and one further row came off with the withdrawal, so the step is DONE and the number here is the pre-implementation prediction, kept for the reader who compares them.
...
| spec AC | proven by |
| --- | --- |
| AC-1 verdict-neutral — AMENDED | Task 6 Step 6 (mechanical, merge-base keyed, discrimination proven) + the `_metaPremiseContract` run ending every task. The criterion is now exact-count equality against the ratified sixteen-test movement, not zero movement; spec §6 AC-1 carries the amendment and its cause |
...
**Rows 3, 3b, 3c, 5 and 5b are gone from this table because those repairs are WITHDRAWN**, not because they were never made — diff round 2 found six of its eight findings in them, and the user chose ship-and-fence. They are §4 limits 16 and 17 now, and the round-2 section below records the decision. The commits stay in the history; what they added does not ship.

§2.4b gained the row finding 4 added, so the shipped rule and its statement do not drift. The row finding 5 added and §2.6's module-load item were removed again with the repairs they described.

**AC-1's declared movement is unchanged by every repair above** — still exactly the sixteen tests Task 0 measured, which is what `_metaPremiseContract`'s exact-count equality re-checks on every run. That is NOT the same as "AC-1 holds": the criterion as ORIGINALLY written asked for ZERO movement, and spec §6 now carries the amendment that replaced it, and this arc's headline is that it does not hold. spec §6's amended AC-1 states the ratified exception; the invariant these repairs preserve is that they add nothing to it.

**AC-14 is measured rather than argued:** the contract suite runs at 29.9-34.7s on this head against 33.9s / 34.1s measured twice on the pre-repair scanner (`a28ef9f9c`'s `premiseScan.ts` dropped into this tree). The traversal findings 3 and 5 added is withdrawn, so that measurement now describes a tree without it and the short-circuit budget is untouched by what ships.

### Invariant 1 was broken mid-arc, and it is recorded rather than repaired (diff round 3 finding 3)

Round 3 established, with `git log` evidence, that the contract suite was RED across a run of commits rather than green at each boundary. `9a2a03cb2` already classified the two suites as 10 and 13 while their declarations still read 7 and 0, so `_metaPremiseContract`'s exact-count assertion necessarily failed there; it stayed failing through `1d0524deb`, `571f08c9f`, `1969eabbc` and `0d9778067`, and became consistent only at `a28ef9f9c`. Invariant 1 asks for a passing test before each commit, and five commits crossed their boundaries without one.

**It is recorded, not repaired.** The repair would be rewriting five commits' history on a branch whose review rounds cite those SHAs — round 3's own finding cites them — so the fix costs more truth than it buys. What is worth carrying forward is the cause, because it is not carelessness: the barrel reach was DISCOVERED during implementation, and the declarations could not be corrected until the user had decided between premises, exemptions and a follow-up PR. The correct shape would have been to stop at the discovery, take the decision, and land the declaration change FIRST — a decision gate inside a task, which the plan did not anticipate because the spec predicted no movement at all.
