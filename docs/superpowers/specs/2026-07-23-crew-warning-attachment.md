...
2. **Model test** (extend existing sectionWarningModel coverage): a `FIELD_UNREADABLE` warning with crew blockRef lands in `warningsByCrewKey` under `canonicalCrewKey(stripDayRestrictionParen(name))` (R4-F1: same key expression as the production helper — a raw day-restriction fixture name must land under the STRIPPED key); conservation — the same item filtered from `activeGroups` render path only when its key is rendered.
