...
**Design mock:** `docs/superpowers/specs/step3-onboarding/2026-07-04-step3-review-page-variant-b-mock/` (snapshot of Claude Design project `33ee8c30-4eaa-48b3-9e3e-8fa642f7f3cd`, file `Step 3 Review - Publish (B).html`).
...
- `publishCount` NaN — impossible (integer filter counts); `selectableTotal === 0` → bar shows "0 of 0"; the Publish button keeps its existing `disabled={!finishable}` gate (NOT tied to selectable count) — enabled when finishable so "finish setup with nothing published" stays reachable; disabled only when a blocking row makes `finishable` false. Unchanged behavior.
