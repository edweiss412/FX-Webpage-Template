...
The populated section (`:443-446`) currently carries `aria-label="Recently auto-applied changes"` AND a same-text heading. Replace the `aria-label` with `aria-labelledby` pointing at the heading's `id`. Apply to the populated branch; the `infra_error` branch section (`:426`) has no `aria-label` today — give it the same `aria-labelledby` treatment for consistency. After the change BOTH branches expose the accessible name "Recently auto-applied" (identical to each other, sourced from their heading); relative to today the populated branch's name drops the "changes" suffix — that IS the FLOW4-7 rename (see §4 / §8), applied in every render context.

**Hook-ordering (mandatory):** call `useId()` UNCONDITIONALLY at the top of `RecentAutoAppliedStrip`, before any early return (`RecentAutoAppliedStrip.tsx` returns early for `infra_error` `:421`, for `groups.length === 0` `:440`, then populated). Never call `useId()` inside a conditional branch — hook order must be stable across `data.kind` / `groups.length` transitions (React rules-of-hooks). Compute `const headingId = useId();` once, reference it in whichever branch renders (`aria-labelledby={headingId}` on the section + `id={headingId}` on the `SectionHeading`). The `groups.length === 0` branch returns `null` and uses neither — fine, the hook still ran.
...
| `headingLevel` omitted | defaults to `4` — dashboard heading LEVELS unchanged (h4/h5 as today). NB: the D4 `aria-label`→`aria-labelledby` change is GLOBAL (every strip render context, dashboard included), so on the dashboard too the section's accessible name goes from "Recently auto-applied changes" (old `aria-label`) to "Recently auto-applied" (heading) — the intended FLOW4-7 fix, not a page-only change. "Byte-identical dashboard markup" is NOT claimed; only heading-level stability is. |
...
3. **`tests/components/admin/RecentAutoAppliedStrip.test.tsx`** (extend): default renders `h4`/`h5` (regression pin); `headingLevel={2}` renders `h2`/`h3` in populated AND infra_error branches; populated + degraded sections' accessible name comes from the heading via `aria-labelledby`. Scope the accessible-name assertion to the single strip under test — each `render()` mounts exactly one strip, so `getByRole("region", { name: "Recently auto-applied" })` resolves uniquely; assert the resolved element has NO `aria-label` attribute (`.not.toHaveAttribute("aria-label")`). Do NOT use a bare container query that could match a co-rendered sibling; if a test ever renders two strips, switch to `getAllByRole("region", { name: ... })` and index. (`region` role requires the accessible name — an unnamed `<section>` has no implicit role, so the query also proves the name wiring.)
...
- `BACKLOG.md`: no edit — verified no `BL-FLOW4-*` row exists in either `BACKLOG.md` or `BACKLOG.md` (the DEFERRED refs were forward-looking ids that were never filed; same finding as the destructive-confirm arc). **Stale as of 2026-08-02:** the dangling-citation filing pass wrote those never-filed ids into `BACKLOG-archive.md` at their already-resolved state, so rows now exist. Accurate when written; the "no edit" outcome for this spec is unchanged.
