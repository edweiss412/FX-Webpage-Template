...
Date: 2026-07-04. Parent: PR #295 (`docs/superpowers/specs/step3-onboarding/2026-07-03-step3-modal-followups.md`, merged `5d7f5386`). Four operator-reported defects/changes on the shipped Step-3 review modal. Autonomous-ship pipeline; spec/plan user-review gates waived.
...
Real-browser (Playwright, existing step3-review-modal e2e harness): extend the interactions spec with ONE scenario — publish-pending label assertion under a real deferred network stub — only if the harness already stubs publish-intent; otherwise T-B1's jsdom deferred-promise coverage is sufficient (no layout/scroll physics involved; §K11-class frame sampling not required for a label swap). K14 footer no-shift and all other existing e2e pins stay green unmodified, with ONE required adaptation: the §D disclosure hides the report submit button by default, and the layout spec's tap-target audit (`tests/e2e/step3-review-modal.layout.spec.ts:400-418`) currently measures it. The layout harness is STATIC markup (react-dom/server via tsx — no interactivity), so that audit row changes to the always-present disclosure TOGGLE (`…-report-toggle`, itself a new ≥44px target), and the submit-button ≥44px measurement moves to the LIVE interactions spec behind an expand step. Net tap-target coverage is preserved for both the toggle and the submit button.
