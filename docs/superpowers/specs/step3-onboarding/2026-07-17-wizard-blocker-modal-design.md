...
- **Cleanup ORDER (focus continuity) — CORRECTED at implementation (whole-diff review R1):** React runs effect cleanups in **DECLARATION order** (top-to-bottom, verified empirically), NOT reverse. There is also a second trap: applying `inert` to an ancestor **blurs** the focused review element, so any `document.activeElement` snapshot taken AFTER inert (including `useDialogFocus`'s own save) captures `<body>`. Both are resolved by having the **inert effect OWN the outer focus save/restore**: it is declared BEFORE `useDialogFocus`, snapshots `document.activeElement` BEFORE inerting, and in its cleanup un-inerts THEN restores focus to that snapshot. On unmount (declaration-order cleanup) the inert-cleanup runs before `useDialogFocus`'s cleanup, so un-inert + restore happen first; `useDialogFocus`'s restore then targets `<body>` (a no-op — body is not focusable) and never disturbs the restored focus. This ordering + ownership is load-bearing.
- This compound is exercised by a component test (§10.11), which asserts (a) both dialog nodes exist in the DOM, (b) the review modal's body node carries `inert`/`aria-hidden` while the blocker is open and loses it on close, (c) Escape does not close the review modal, and (d) after the blocker closes, `document.activeElement` is a node INSIDE the review modal. jsdom does NOT enforce `inert` focus-blocking, so the DEFINITIVE focus-continuity proof is a **real-browser** Playwright test (`tests/e2e/wizard-blocker-modal.layout.spec.ts`, "focus continuity"): an app-root button is focused, the blocker opens (real `inert` blurs it), and on dismiss focus returns to it.
