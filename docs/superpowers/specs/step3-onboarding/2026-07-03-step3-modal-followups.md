...
Parent spec (contracts still binding): `docs/superpowers/specs/step3-onboarding/2026-07-02-step3-review-modal-redesign.md`. Design-mock snapshot: `docs/superpowers/specs/step3-onboarding/2026-07-02-step3-review-modal-mock/` (the mock's only diagrams treatment is a one-line field row — `data.jsx:32` — so Item B's grid is additive design in the modal's established visual language, not a mock-fidelity port).
...
- The scroll-spy `evaluate()` (L192-216): while `spySuppressedRef` is true it does NOT call `setActive`; instead it checks release conditions and, when released, falls through to normal derivation on the same frame. Release when ANY of:
  ...
  3. **Timeout:** the `NAV_SCROLL_SETTLE_TIMEOUT_MS` fallback fired (covers zero-event and interrupted-glide cases). **[Ratified in-branch amendment, Task 14 / commit `b85b425d`]:** the fallback timer RESTARTS on every suppressed scroll-progress frame — a healthy long glide (~1s for a full-pane scroll in Chromium) is neither a zero-event nor an interrupted case, and a single-shot 700ms timer released mid-flight, reproducing the exact §H-N1 flicker (§K11 frame sampler evidence). Zero-event glides still release at exactly 700ms (no events → no restarts). Additionally, the settled/clamp release completes only when `activeSectionFor` at the current position equals its value at the destination (arrival-consistency guard) — the 2px settle epsilon is wider than the derivation's 1px bottom-clamp tolerance, so an unguarded ε-zone release could derive the second-to-last section for 1–2 frames. User-input releases (condition 4) bypass the guard and stay instant.
...
- **Row lookup:** mirror the unapprove route's guard exactly (unapprove route L80-92): select `parse_result` from `pending_syncs` where `drive_file_id = $driveFileId` AND `wizard_session_id = $wizardSessionId::uuid` AND the session is the active one per `app_settings.pending_wizard_session_id` (`pending_syncs.wizard_session_id` — migration `20260501001000_internal_and_admin.sql:150`; `app_settings.pending_wizard_session_id` — same file L243; there is NO `pending_wizard_session_id` column on `pending_syncs`). 0 rows (superseded/absent/inactive session) → 404. **`parse_result` is untrusted JSONB** — this codebase already guards legacy double-encoded/corrupt rows before dereferencing (`lib/sync/applyStaged.ts:443-459`): the route validates shape at BOTH levels — container (object, `diagrams` object, `embeddedImages` array) AND element (a stub is addressable/renderable only if it is a non-null object with `typeof objectId === "string"`, `typeof mimeType === "string"`, AND `typeof sheetTab === "string"` (required by the type, `lib/parser/types.ts:249`, and dereferenced by the UI's alt fallback), `contentUrl` absent/null/string, and `alt` absent/undefined/string — the predicate covers EVERY field either consumer dereferences; the existing coercion guard proves only array-ness, `lib/db/coerceJsonbObject.ts:165-172`, so element checks are the route's job). Malformed elements are skipped during objectId matching (an unaddressable stub is a 404 like any unknown objectId); any malformed/null `parse_result` container → 404, never a 500.
- **Ordering:** auth runs FIRST, mirroring the sibling route (`requireAdminIdentity` itself performs Supabase-backed session checks — `lib/auth/requireAdmin.ts:154-225` — so "before any DB query" below means before any **route-owned `pending_syncs` query**, not before the auth gate).
- **Param validation (all three, after auth, before any route-owned DB query):** `wizardSessionId` must match the strict UUID pattern (the columns are `uuid` and the sibling SQL casts `$2::uuid` — a malformed param must be a controlled 404, never a Postgres invalid-UUID 500); `driveFileId` must match `/^[A-Za-z0-9_-]{1,128}$/`; `objectId` decoded via `decodeURIComponent`, must match `/^[A-Za-z0-9_-]{1,STAGED_DIAGRAM_OBJECT_ID_MAX}$/` else 400. Match against `stub.objectId` (a `string` — `lib/parser/types.ts:250`) over `parse_result.diagrams.embeddedImages`; first match wins; no match → 404.
- **Shared element predicate:** the stub-validity rule above is exported once as `isRenderableDiagramStub(x: unknown): x is EmbeddedImageStub`-shaped predicate from a small shared module `lib/admin/stagedDiagramGuards.ts` (client-safe, no server imports) and consumed by BOTH this route and `DiagramsBreakdown` (§B3) — the two surfaces must never disagree on what "valid stub" means.
- **URL trust boundary (load-bearing security guard):** `snapshotFetchEmbeddedImageBytesTimed` sends the Drive **bearer token** in `Authorization` to whatever `entry.contentUrl` says (`defaultSnapshotAssetsForApply.ts:60-66`). Because `parse_result` is untrusted, a `typeof === "string"` check is NOT sufficient — a corrupt row could point `contentUrl` at an attacker origin and exfiltrate the token (SSRF class). Before fetching, the route parses `contentUrl` with `new URL(...)` and requires: parse succeeds, `protocol === "https:"`, and hostname is `googleusercontent.com`/`google.com` or a dot-boundary subdomain of either (`h === d || h.endsWith("." + d)` — suffix-spoofs like `google.com.evil.net` fail). Any other URL → stub treated as unaddressable → 404, zero network calls.
...
- **Folder row:** when `linkedFolder != null`, an external-link anchor "Open diagrams folder in Drive", `target="_blank" rel="noopener noreferrer"`, plus "N files" when `linkedFolderItems.length > 0`. The parser captures `drive.google.com/.../folders/...` URLs in EITHER scheme — `FOLDER_URL_RE` accepts `https?://` (`lib/parser/diagrams.ts:26-42`) — and the modal consumes persisted untrusted JSONB besides; so the component revalidates before `href`: `new URL(driveFolderUrl)` succeeds AND hostname is exactly `drive.google.com` AND protocol is `https:` or `http:` (an `http:` URL — live-reachable via the regex — is upgraded to `https:` before rendering, since the exact-host constraint makes the upgrade safe); anything else renders the counts text with NO link. Tested with a hostile-origin fixture (no anchor) and the `http://drive.google.com/...` case (anchor rendered with the upgraded `https:` href). Linked-folder items get NO thumbnails (out of scope, §10).
- Guards: `pr.diagrams` null → section not rendered (registry gate); all-empty diagrams object → not rendered; `embeddedImages` empty but folder present → folder row only, no grid. **Element-level (mirrors §B1):** the modal renders the same persisted untrusted JSONB the route reads (the card only root-guards `parseResult`, `Step3SheetCard.tsx:325-327`; `arr()` proves array-ness only, `step3ReviewSections.tsx:154-158`) — `DiagramsBreakdown` filters `embeddedImages` through the shared `isRenderableDiagramStub` predicate (§B1, `lib/admin/stagedDiagramGuards.ts`) BEFORE any dereference; invalid elements are excluded from tiles, counts, and cap math (never a crash, never a `[object Object]`/`undefined` URL); `alt`/`sheetTab` used only when strings, else the generic alt fallback. Testids: `-section-diagrams` body, `-diagram-tile-${i}`, `-diagram-folder-link`.
...
- **§D3a — active-section plumbing:** `active` is `Step3ReviewModal` local state (`Step3ReviewModal.tsx:155-157`) and section bodies receive only `SectionData` (`step3ReviewSections.tsx:1577-1601`). The modal extends the per-section `Step3SectionChromeContext` provider value (current shape `{ Icon, label, flagged }`, `step3ReviewSections.tsx:242-250`; providers already pass a fresh inline object each render, `Step3ReviewModal.tsx:756-758` — that existing behavior is unchanged) with `getActiveSection: () => SectionId`, a stable-identity function (useCallback reading a modal-held ref kept in sync with `active`). Its purpose is a stale-free read at submit time — NOT render optimization; no render-stability contract is claimed. `ReportIssueSection` calls it at submit time; outside the modal chrome context the field is omitted. The §K4 test changes the active section before submitting and asserts the submitted `viewerVisibleSection` follows it (a hardcoded `"report"` value must fail the test).
- Idempotency semantics mirror ReportModal's contract in lean form (NOT a reuse of the full ReportModal component; deliberate divergence): mint `crypto.randomUUID()` per attempt, persist under `sessionStorage["fxav-report-attempt-wizard-" + wizardSessionId + "-" + driveFileId]` so a retry of a failed attempt reuses the key; the key is scoped to the **wizard session AND drive file** (both are in `SectionData`, `step3ReviewSections.tsx:1577-1583`) because the report context embeds `wizardSessionId` — a later wizard session for the same file is a different report, and must not be swallowed as a duplicate of a stale attempt (mirrors ReportModal's surfaceId-validated reuse, `ReportModal.tsx:110-124`); rotate (delete) only on terminal success (`ReportModal.tsx:110-133,327` is the reference pattern). `REPORT_HORIZON_EXPIRED` (410) → treat as terminal: rotate the key and show its dougFacing copy.
...
- The lease layer already tolerates null (`ReportLeaseDb`/`acquireReportLease` take `showId: string | null` — `lib/reports/leaseProtocol.ts:10`; `claimed.show_id: string | null`, `submit.ts:586`; `admin_alerts` upsert uses `coalesce(show_id::text,'')`, L645-647). The show-context lookup does NOT: `readReportShowContext(showId: string)` (`submit.ts:309`) is called unconditionally with `body.show_id` at BOTH call sites (`submit.ts:1035` expired-retry path and `submit.ts:1039` normal path). **Do NOT widen it or route null through it** — its not-found shape is `{ state: "missing" }` (`submit.ts:301`), which `showContextLine` renders as "(deleted)" (`submit.ts:278-282`); a staged wizard report must never be labeled a deleted show. Required change instead: `readReportShowContext` stays string-only; BOTH call sites are guarded so it is not called when `body.show_id == null`, and `showContextLine` gets a first branch `body.show_id == null → showLine(body)` (the staged fallback path, never "(deleted)"). Enumerate and test both call-site guards.
- **Formatter changes are REQUIRED (they do not exist today):** `showLine` (`submit.ts:263-270`) currently ends with `return body.show_id` — with `show_id: string | null` that is a typecheck break and, if silenced, renders `null` into issue text. Required: every `body.show_id` interpolation site handles null — `showLine` (L263-270: title/slug branches interpolate `— ${body.show_id}` only when non-null; terminal fallback when all of title/slug/show_id are null → the literal string `"staged wizard sheet (no show record)"`), `showContextLine` (L278-282, falls through to `showLine`), `issueSummaryLine` (L361-388, considers only `body.showTitle` — already fed by the §D3 payload; must not emit `null` when both are null), AND `buildCrewIssueBody` (interpolates `body.show_id` at `submit.ts:447`) — the crew route path still forbids null `show_id`, but once `RequestBody.show_id` widens, this exported formatter must be null-hardened too (same fallback family), not left relying on a caller-side invariant. Because the §D3 payload sets top-level `showTitle`, the normal wizard path exercises the existing title branches; the null-null-null terminal is the hardening case. §K6 asserts, for BOTH fixtures — (a) showTitle set + show_id null, (b) all three null — that title and body contain no `"null"` literal AND no "(deleted)" label (the Show line must take the staged fallback path, §D4 above).
...
6. `submitReport` with null show: the issue BODY (summary/Show lines — the GitHub issue title is always `Bug report: <surface>`, `submit.ts:471-484`, and stays that way) contains the fieldRef fallbacks and no `"null"` literal (derive expectations from the fixture's fieldRef, not hardcoded strings).
7. Staged-diagram route: unauth 403; malformed `wizardSessionId` (non-UUID) AND malformed `driveFileId` (charset violation, e.g. `a/b` or empty) → 404 with zero route-owned `pending_syncs` queries (spy on the route's sql; auth is mocked and its internal checks are out of scope); superseded session 404; unknown objectId 404; hostile `contentUrl` fixtures — `http:` scheme, `https://evil.example/x`, and the suffix-spoof `https://google.com.evil.net/x` — → 404 with ZERO fetch calls (spy asserts no token ever leaves); null-contentUrl stub 404 with zero Drive calls (spy); happy path returns bytes + exact header set with the helper stubbed to return the realistic `BoundedByteResult` wrapped shape `{ bytes, sha256Base64Url, md5Hex }` (NOT a raw `Uint8Array` fixture — catches un-normalized union handling); helper returning raw `Uint8Array` also served; helper THROWING → 404 not 500; non-allowlisted mime 404; malformed `parse_result` fixtures at container level (JSON string double-encode, null, missing `diagrams`, non-array `embeddedImages`) AND element level (`embeddedImages: [null]`, `[{ objectId: 123 }]`, contentUrl non-string, missing `sheetTab`, non-string `alt`) → 404 not 500 (catches: SVG passthrough, session-guard bypass, 500s for stale Drive fetches or corrupt staged rows).
8. Diagrams body: cap at `DIAGRAM_TILE_CAP` with "+N more" note derived from fixture length (fixture must exceed the cap); null-contentUrl tile renders placeholder without an `<img src>` fetch; folder-only renders link and no grid; hostile `driveFolderUrl` fixture renders counts with no `<a>`; malformed-element fixture `embeddedImages: [null, { objectId: 123 }, { objectId: "x", mimeType: "image/png", contentUrl: null } /* missing sheetTab */, { ...valid, alt: 7 }, <valid stub>]` renders exactly one tile, count "1", no crash (catches: unvalidated href, client-side dereference of corrupt staged JSON incl. the alt-fallback `sheetTab` read).
