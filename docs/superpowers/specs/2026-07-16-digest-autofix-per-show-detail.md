...
- Rebuild the digest model's `autofix` signal per show from **all** in-window applied `sync_log` rows, **fingerprint-deduped** per show (the same notice reported once regardless of how many syncs repeat it; distinct notices retained to the granularity persisted identity supports — §3), carrying show title/slug and the autofix warning **messages**.
...
- **Fingerprint dedupe per show (source-aware; complete identity ladder):** within a show (keyed by `drive_file_id`), an item is kept only the first time its fingerprint is seen. The fingerprint is `` `${code}|${anchor ?? ""}|${item}` `` where `anchor` is `` `${sourceCell.gid}!${sourceCell.a1}` `` when the warning carries a resolved `sourceCell` with an `a1`, else absent (`ParseWarning.sourceCell?: SourceAnchor`, `lib/parser/types.ts:83`; `SourceAnchor = { title, gid, a1? }`, `lib/sheet-links/buildSheetDeepLink.ts:3`).
  **Anchor granularity is whatever attachment produced — this is the finest identity that exists** (audited end-to-end, adversarial R9):
  - `STAGE_WORD_AUTOCORRECTED` / `ROLE_TOKEN_AUTOCORRECTED`: per-row CELL anchor resolved by crew NAME — **unique-name-only**: the resolver returns null on zero or MULTIPLE name matches (`lib/drive/showDayTimeAnchors.ts:133-138`), so duplicate-name crew rows degrade to UNANCHORED and byte-identical notices from those rows collapse (residual a below — acceptable, no numeric claim rides on it). Byte-identical notices in different cells stay distinct only when both anchors resolve (unique names).
  ...
  This mirrors the existing anchored-dedup precedent (`(code, resolved-anchor-A1)` key, `lib/parser/dataGaps.ts:332-344`), extended with the item text so same-anchor DIFFERENT corrections never collapse.
  **What the fingerprint IS (ratified, R10 structural closure): notice spam-suppression, NOT correction counting.** A kept item is a verbatim parser NOTICE, not a claimed one-to-one correction: one `STAGE_WORD_AUTOCORRECTED` notice can describe several corrected pairs in one message (`stageNorm.corrections.map(...).join(", ")`, `lib/parser/blocks/crew.ts:346-348`), and a notice's text shifts when unrelated text in the same quoted cell changes. Deriving a true per-correction identity would require parsing `'X' as 'Y'` pairs back out of display strings — brittle, and still block-ambiguous for region-anchored codes. Instead the EMAIL MAKES NO NUMERIC PER-CORRECTION CLAIM (§5): the intro counts SHOWS (well-defined), each bullet is a notice, and dedupe merely prevents the same notice text from repeating. The entire under/over-count class is closed by removing the claim, not by sharpening an identity that persisted data cannot support. `item` is the UNCAPPED normalized message (§4); the 200-char display cap is applied to kept items AFTER dedupe, inside `computeAutofixShows`, so two distinct long corrections sharing a 200-char prefix both survive (they may render identical capped lines — accepted residual, the count stays honest). Repeated syncs re-emitting the same warning from the same cell collapse to one item (the reported inflation bug — same gid/a1/text across syncs); distinct corrections across different in-window rows ALL survive (a 09:00 correction still reports even if the 10:00 sync no longer carries it — the section is "applied since your last digest", an event digest). **Count contract:** `total` = distinct kept notices. It is TELEMETRY-AND-GATE ONLY (`monitor_totals.autofixTotal`, the model empty-gate) and is NEVER rendered as a correction count in the email (§5 — the only rendered number is the show count). Notice-level residuals — (a) byte-identical unanchored notices collapse, (b) byte-identical notices sharing one region-range anchor collapse, (c) an anchor or quoted-cell text that shifts between syncs re-emits as a new notice — are display noise with no numeric claim attached.
- **Ordering (deterministic under caps):** rows arrive `occurred_at desc, drive_file_id asc, id asc` (query `ORDER BY` above — fully deterministic for a fixed row set, including same-show timestamp ties). Shows are grouped in the stream order of their FIRST KEPT NOTICE (a clean zero-autofix row never seeds a show's position — otherwise a show's latest ordinary sync would outrank another show's newer actual notice under the 12-show render cap) — most recent notice first. Items within a show keep stream order (newest row's warnings first, in-array order preserved), so the 5-item cap is likewise deterministic.
...
- **No count capping in the model:** `computeAutofixShows` preserves EVERY show and EVERY kept item (only the per-item 200-char TEXT cap applies here). The `DIGEST_MAX_SHOWS` / `DIGEST_MAX_ITEMS_PER_SHOW` count caps and `+N more` overflow notes are applied exclusively by `renderMonitorSection` from SOURCE totals, exactly as sub-block 1 does (`lib/notify/templates/digest.ts:38-63`) — the intro's show count and overflow math need the uncapped model.
- `total` = sum of `items.length` across ALL shows (pre-cap SOURCE total; = count of distinct kept notices; telemetry/gate only, never rendered — §3 count contract, §5).
...
**Semantics change (intentional, the point of the fix):** `autofix.total` now means "distinct kept notices in the window" (telemetry/gate only), not "sum over every applied row," and the email renders no per-correction number at all — only the show count and the notices themselves. Repetition across syncs no longer inflates anything; no distinct notice is dropped.
...
- **Normalization (bounded items; messages quote arbitrary sheet cells):** before the raw-code-shaped check, the message is normalized with the same code-point contract as `sanitizeIdentityString` step 1 (`lib/adminAlerts/sanitizeIdentityString.ts:1-33,45`): zero-width/bidi format chars (U+200B-200D, U+FEFF, U+202A-202E, U+2066-2069) REMOVED; C0/C1 controls incl. `\n`/`\t` → space; whitespace collapsed; trimmed. **Step order (load-bearing, adversarial R13):** (1) normalize as above; (2) the empty / `message === code` / SHOUTY-token shape check runs on the normalized UNREDACTED string → label fallback on match (a raw catalog code like `STAGE_WORD_AUTOCORRECTED` is itself a 24-char token — redacting first would render `[redacted-token]` instead of the human label); (3) only ACCEPTED prose messages then get substring redaction on the full un-capped string (cap-order rationale per `sanitizeIdentityString.ts:38-48`): token-like (`/[A-Za-z0-9+/_-]{24,}/`) → `[redacted-token]`, email-like (`/\S+@\S+/`) → `[redacted-email]`, both ALWAYS — unlike sub-block 1's curated `show_change_log.summary` copy, parser messages quote ARBITRARY sheet cells, which can carry pasted secrets or emails (adversarial R12); nothing in an autofix notice legitimately needs either; (4) the 200-char display cap last (§3). The helper returns the UNCAPPED normalized item — identity and display are separated: `computeAutofixShows` dedupes on the §3 fingerprint (`code`/`anchor`/uncapped `item`), then length-caps each KEPT item at 200 chars with `…` appended for display (cap after both the shape check and dedupe, so truncation can neither dodge the shape check nor collapse distinct corrections). Whether implementation shares code with `sanitizeIdentityString` or replicates the two range tables is a plan-level choice; the ranges above are the contract.
...
**Intro line (`<p>` / text):** `We applied automatic corrections to N shows:` (`1 show` when N === 1; N = `autofix.shows.length`, the SOURCE show total, pre-cap). **The email renders NO per-correction count** — one notice can describe several corrected pairs and notice text shifts with unrelated cell edits (§3), so any per-correction number would be unsupportable; the show count is exactly well-defined. The per-class parenthetical is dropped — the per-show messages carry strictly more information.
...
| message contains token-like substring (≥24 base64ish chars) | `[redacted-token]` substituted upstream (§4), always |
| message contains email-like substring | `[redacted-email]` substituted upstream (§4), always |
...
| N === 1 | `1 show` singular in the intro line |
| two identical notices, different ANCHORED cells (cell-anchored codes) | stay distinct (fingerprint includes `gid!a1` — §3); both render |
| two identical notices, SAME region-range anchor (region-anchored codes) | collapse to one item (§3 residual b); no numeric claim affected |
| two identical notices, UNANCHORED (no resolved `sourceCell.a1`) | collapse to one item (§3 residual a); no numeric claim affected |
| one notice describing several corrected pairs | renders as its one verbatim message; no per-correction count exists to disagree with (§3 R10 closure) |
...
1. `tests/notify/monitorDigest.autofix.test.ts` — rewrite for `computeAutofixShows` + `listAutoFixItems`. Cases: payload-object skip; non-autofix-code skip; info-severity skip; item fallback to label (message absent / empty / non-string / raw-code-shaped — incl. `message === code` and an unrelated SHOUTY token, asserting the EXACT class label is emitted (e.g. `corrected stage word`), not merely the absence of `_AUTOCORRECTED` — a `[redacted-token]` output must FAIL these cases, pinning the shape-check-before-redaction order); normalization + redaction (multiline message → single line; zero-width/bidi chars removed; 500-char message → 200 chars + `…`; overlong SHOUTY-token message → label fallback, proving the shape check runs pre-cap; truncation-collision: two same-code messages sharing a >200-char prefix with different suffixes → 2 items, `total === 2`, proving dedupe keys on the uncapped message; message quoting a 32-char token → `[redacted-token]`; message quoting an email → `[redacted-email]`; a token straddling the 200-char boundary is redacted whole, proving redaction runs pre-cap); property `listAutoFixItems(w).length === summarizeAutoFixes(w).total` over the fixture set; fingerprint dedupe (same code+anchor+message across two rows of one show → 1 item; two DIFFERENT messages across two rows → 2 items, both retained; same message text under two different codes → 2 items; IDENTICAL message + code in two different anchored cells (`gid!a1` differs) → 2 items; identical message + code both UNANCHORED → 1 item; identical message + code sharing one RANGE anchor (`a1` contains `:`) → 1 item (§3 residual b); identical message + code in two DIFFERENT range anchors → 2 items; same anchor with two DIFFERENT messages → 2 items); show ordering WITHOUT count capping (13 shows with distinct `occurred_at` → ALL 13 preserved, most-recent-first order derived from fixture timestamps; >5 items on one show → all preserved — count caps are render-only, §3). *Catches: helper counting/emitting divergence, fallback regressions, raw-code leak (invariant 5), dedupe collapse of distinct notices, event-semantics drop, model-side cap leak (which would break render overflow math).*
2. `tests/notify/monitorDigest.autofix.db.test.ts` — currently seeds 2 in-window applied rows for ONE show and expects `total === 2` (`:73-74`), i.e. it PINS the inflation bug. Rewrite: same-show two rows carrying the SAME autofix warning → `total === 1`; same-show two rows carrying DIFFERENT warnings → both survive (`total === 2`) even though only one is the latest row; second show grouped separately with title/slug; rows returned newest-first (assert show order from fixture `occurred_at`); same-show rows with IDENTICAL `occurred_at`, FIXED seeded uuids, and >5 distinct items → assert ALL items present in the MODEL in the EXACT order implied by `occurred_at desc, drive_file_id asc, id asc` over the seeded uuids (an exact-order proof — two-consecutive-calls stability is NOT sufficient, Postgres heap order can repeat while still unpinned; the 5-item cap is render-only and asserted in test 3). *Catches: dedupe regression to sum-over-rows (the exact reported bug), latest-row-only regression (dropping real notices), title/slug drop, tied-row order nondeterminism that would make the render-time cap unstable.*
...
6. Live anchor-attachment path (extends the existing `tests/drive/showDayTimeAnchors.test.ts` surface): two DUPLICATE-NAME crew rows carrying the same role typo → the resolver anchors NEITHER (null on multiple name matches), both notices arrive unanchored, and `computeAutofixShows` collapses the byte-identical pair to one item — pins the unique-name-only contract through the REAL resolver, not injected `sourceCell` fixtures. A sibling unique-name pair in the same fixture anchors both and stays distinct. *Catches: spec/live divergence on anchor granularity (the R11 class — fixture-injected anchors can't see resolver nulls).*
...
- **The dedupe-identity vector is CLOSED STRUCTURALLY (R5-R10)** — after comprehensive re-analysis, the §3 ladder (code → anchor at native granularity → notice text) is the audited maximum identity in persisted warnings, AND the email no longer makes any per-correction numeric claim it would need to defend: the fingerprint is notice spam-suppression, the only rendered number is the show count, `total` is telemetry/gate-only. Under/over-count findings against the dedupe key are out of scope BY CONSTRUCTION — there is no rendered count for them to falsify. Do not relitigate unless naming a rendered numeric claim that is actually wrong.
