...
- **Normalization (bounded items; messages quote arbitrary sheet cells):** before the raw-code-shaped check, the message is normalized with the same code-point contract as `sanitizeIdentityString` step 1 (`lib/adminAlerts/sanitizeIdentityString.ts:1-33,45`): zero-width/bidi format chars (U+200B-200D, U+FEFF, U+202A-202E, U+2066-2069) REMOVED; C0/C1 controls incl. `\n`/`\t` → space; whitespace collapsed; trimmed; THEN redaction on the full un-capped string (order load-bearing, `sanitizeIdentityString.ts:38-48`): token-like substrings (`/[A-Za-z0-9+/_-]{24,}/`) → `[redacted-token]` and email-like substrings (`/\S+@\S+/`) → `[redacted-email]`, both ALWAYS — unlike sub-block 1's curated `show_change_log.summary` copy, parser messages quote ARBITRARY sheet cells, which can carry pasted secrets or emails (adversarial R12); nothing in an autofix notice legitimately needs either. If the normalized+redacted result is empty or raw-code-shaped → label fallback. The helper returns the UNCAPPED normalized item — identity and display are separated: `computeAutofixShows` dedupes on the §3 fingerprint (`code`/`anchor`/uncapped `item`), then length-caps each KEPT item at 200 chars with `…` appended for display (cap after both the shape check and dedupe, so truncation can neither dodge the shape check nor collapse distinct corrections). Whether implementation shares code with `sanitizeIdentityString` or replicates the two range tables is a plan-level choice; the ranges above are the contract.
