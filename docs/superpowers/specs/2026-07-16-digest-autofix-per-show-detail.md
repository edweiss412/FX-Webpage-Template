...
- **Fingerprint dedupe per show (source-aware):** within a show (keyed by `drive_file_id`), an item is kept only the first time its fingerprint is seen. The fingerprint is:
  - `` `${code}|${sourceCell.gid}!${sourceCell.a1}|${item}` `` when the warning carries a resolved `sourceCell` anchor with an `a1` (`ParseWarning.sourceCell?: SourceAnchor`, `lib/parser/types.ts:83`; `SourceAnchor = { title, gid, a1? }`, `lib/sheet-links/buildSheetDeepLink.ts:3`) — all five autofix codes are anchor-eligible (`OPERATOR_ACTIONABLE_ANCHORED`, `lib/parser/dataGaps.ts:314-318`), so two byte-identical corrections in DIFFERENT cells stay distinct;
  - `` `${code}|${item}` `` when no resolved anchor exists (`sourceCell` absent/null or `a1` missing) — unanchored repeats still collapse, otherwise the reported inflation bug returns for exactly the unanchored class.
  This mirrors the existing anchored-dedup precedent (`(code, resolved-anchor-A1)` key, `lib/parser/dataGaps.ts:332-344`), extended with the item text so same-cell DIFFERENT corrections never collapse. `item` is the UNCAPPED normalized message (§4); the 200-char display cap is applied to kept items AFTER dedupe, inside `computeAutofixShows`, so two distinct long corrections sharing a 200-char prefix both survive (they may render identical capped lines — accepted residual, the count stays honest). Repeated syncs re-emitting the same warning from the same cell collapse to one item (the reported inflation bug — same gid/a1/text across syncs); distinct corrections across different in-window rows ALL survive (a 09:00 correction still reports even if the 10:00 sync no longer carries it — the section is "applied since your last digest", an event digest). **Count contract:** `total` counts **distinct corrections** (distinct fingerprints) and the rendered copy says exactly that (§5). Residuals, accepted: byte-identical UNANCHORED corrections in different sheet locations collapse to one; a cell whose A1 shifts between syncs (row inserted above) counts once per location. Both are edge noise; the unit "distinct corrections" stays honest.
...
`anchor` is `` `${sourceCell.gid}!${sourceCell.a1}` `` when the warning carries a `sourceCell` with a numeric `gid` and a non-empty string `a1`, else `null` (jsonb-defensive on both fields). The caller's fingerprint (§3) is `` `${code}|${anchor}|${item}` `` with `anchor ?? ""`.
...
| two identical corrections, different ANCHORED cells | stay distinct (fingerprint includes `gid!a1` — §3); both render, count 2 |
| two identical corrections, UNANCHORED (no resolved `sourceCell.a1`) | collapse to one item; count unit "distinct corrections" stays accurate (§3 residual) |
