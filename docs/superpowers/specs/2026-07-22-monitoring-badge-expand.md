...
- **Focus rescue for removed rows while the menu stays open (NEW mechanism — none exists today):** the current rescue effect fires only when `menuEffectivelyOpen` flips false (`PublishedReviewModal.tsx:353-366`). When the focused actionable/needs-look row unmounts while the menu remains open, the browser drops focus to `<body>` (standard DOM behavior on removing the focused element), breaking the modal focus trap. Mechanism: extend the post-commit rescue effect to ALSO run while `menuEffectivelyOpen` is true — if `document.activeElement` is `<body>` (or no longer inside the dialog), refocus the pill button (still mounted, still the menu's trigger; `aria-expanded` stays true so AT context is preserved). Ratification is EMPIRICAL, not prose: the real-browser Playwright cell below (focus a row, drive `__setItems(0,0,1)`, assert menu open + focus on the pill + never `<body>`) is the probe that ratifies the mechanism; jsdom focus semantics on element removal differ from browsers, so the e2e cell is the authoritative proof and the jsdom twin is corroborating only.
...
1. `tests/components/admin/showpage/publishedPill.test.tsx:75` — `(0,0,1)` row flips `interactive` to `true`. Keep `publishedPill.test.tsx:103` (hollow dot, no solid dot), `publishedPill.test.tsx:109-140` (sr-only expansion, caps, `title`) — now against a `<button>`. NEW: monitoring-only pill has `aria-expanded` and opens the menu on click; quiet palette asserted structurally — `bg-surface-sunken` on the button AND zero elements matching `[class*="warning"]` inside it (catches amber leakage on the segment span and chevron, §3.1); middot never leads the pill (assert visible text for `(0,0,2)` is exactly "2 monitoring").
...
5. `tests/components/admin/showpage/pillFocusReconcile.test.tsx:52-63` — EXIT matrix drops B; ENTRY matrix gains the monitoring-only shape (`[0,0]` with selfHeal=1). Cell-count pin becomes **8** (4 entries × 2 exits). NEW tests: (a) open `[a,n]` → `(0,0,1)`: menu STAYS open, Monitoring group rows visible, `aria-expanded="true"` retained, focus not on `<body>`; (b) monitoring-only entry: click quiet pill opens the menu (`aria-expanded` flips, panel mounts).
6. `tests/e2e/attention-pill-focus.spec.ts:131-141` — same matrix change (**8** cells, monitoring-only entry added) + the stays-open case + the AUTHORITATIVE focus-rescue probe (§3.3): boot `(1,0,1)`, open menu, focus the actionable row, drive `__setItems(0,0,1,false)`; assert menu still mounted, `aria-expanded="true"`, `document.activeElement` is the pill button, never `<body>` (real browser — jsdom removal-focus semantics are not authoritative).
