...
| S6 | `sheets → app` fix guidance in the SUSPECTED copy names exactly ONE sheet edit: one booking per row — the only edit that clears the warning for every guest-name shape. The full-postal-address alternative was REMOVED in Codex R8: guard (c) keeps 3+-base-word first guests (`Mary Ann Smith`) in tier 2 even with a postal-complete address, so advertising it created a remediation loop. Copy states what to CHECK and what to DO, never how the algorithm decided (parent spec §7.0 copy discipline). | User requirement, 2026-07-27 conversation; parent spec §7.0; Codex R8 |
...
- `[0, addressEnd)` — the kept hotel text (D4 match, postal-terminated per S8);
- `[addressEnd, prefixEnd)` — the pre-guest residual, bounded by the residual-tail guard's three arms;
- `[prefixEnd, end)` — everything after the prefix (delimiter, conf, dates, and any GLUED later material), which must scan NEGATIVE for hotel evidence. The post-prefix scan has TWO arms (R12 finding 2; R13 finding 2): (i) street/postal — `" " + s2.slice(prefixEnd)` matches neither `STREET_ADDRESS_RE` nor `STREET_ADDRESS_ZIP_RE`, and `s2.slice(prefixEnd)` lacks the postal-evidence regex; (ii) post-marker conf delimiter — the region AFTER the segment's first `Check In` match's end contains no confirmation delimiter (D3's discriminator: `[-–—]{1,3}\s*#?\s*\d{4,}\b`, `looksLikeStreetStart` false, not a ZIP+4 hyphen). Arm (ii) is post-marker ONLY: a clean multi-guest booking legitimately carries several conf delimiters BEFORE its marker (probe B4, and the two-guest own-hotel keep pinned in §8.1), but after `Check In` comes only date/checkout material, so a conf delimiter there proves a glued later booking even when it carries no address (probe 2026-07-28: `… Jane Doe - 1002 Check In: 3/3/26 Hilton Midtown Bob Roe - 1003 Check Out: 3/6/26` parses today to names `["Jane Doe","Hilton Midtown Bob Roe"]`). For a zero-marker tail the post-marker region is empty and arm (ii) is vacuously negative;
...
**D3 — prefix.** Scan `s2` for the first **non-street confirmation delimiter**: `[-–—]{1,3}\s*#?\s*(\d{4,})\b` where `looksLikeStreetStart(" " + s2.slice(numStart))` is false — the same discriminator the no-Check-In branch uses (`lib/parser/blocks/hotels.ts:953-957`). **ZIP+4 exclusion (Codex R3 finding 2):** a candidate is also NOT a delimiter when it is a true ZIP+4 hyphen by the established `stripConfTokens` predicate (`lib/parser/blocks/hotelConfTokens.ts:46-53`): a single `-` with no separator, exactly 4 digits, immediately preceded by a word-boundary 5-digit run. Without this, `… Chicago IL 60601-1234 Jane Doe - 1002 …` cuts the prefix at `-1234` and the persisted address loses its ZIP+4 (probe-verified: the delimiter scan yields `-1234`, precededBy5Digits, exactly-4). Also locate the first `/check\s+in/i` match. `prefixEnd` = the smallest of: the delimiter's match start, the Check In match start, `s2.length`. `prefix` = `s2.slice(0, prefixEnd)`, trimmed. The delimiter's preceding guest NAME is inside `prefix` — the address anchor is what separates it from hotel text (D4); without an anchor no split of `prefix` is attempted (S1: that boundary is exactly the fact nothing evidences).
...
| ---- | --------- | ------ |
| 1 | segment has at most one `Check In` match (zero for a tail segment; >= 2 is degraded, R11/R12) AND D4 matched AND `hotelText` is postal-terminated (S8: trimmed `s2.slice(0, addressEnd)` matches `/(?:\d{5}(?:-\d{4})?|[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d)$/u`) AND the residual-tail guard passes AND the D6 rebuild succeeds AND the post-prefix scan is negative on BOTH arms (S9, R12-R13: no street/postal evidence in `s2.slice(prefixEnd)`, no conf delimiter after the first `Check In` match) | keep own hotel (D6); stash `HOTEL_INLINE_GROUP_OWN_HOTEL` |
| 2 | (D4 matched but the postal terminator is absent, the residual-tail guard fired, or D6 aborted) OR (D4 did not match AND `baseWords(prefix) >= 4`) OR post-prefix evidence (below — street/postal anywhere in `postGuest` or a post-marker conf delimiter, arm ii; including on an otherwise tier-1-qualifying segment, R12 finding 2, R13 finding 2) OR a degraded segment (>= 2 `Check In` matches) whose D3-D6 outcome would otherwise be tier 1 (R11 demotion) | inherit as today; stash `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` |
| 3 | otherwise | inherit as today, no stash |

**Post-guest address evidence = the post-prefix scan (Codex R5 finding 2; promoted to a tier-1 gate by R12 finding 2).** For EVERY later segment — tier-1 candidates included — evaluate `postGuest = s2.slice(prefixEnd)` (delimiter, conf, later guests, dates, and any glued later material — everything after the prefix): if `" " + postGuest` matches `STREET_ADDRESS_RE` or `STREET_ADDRESS_ZIP_RE`, or `postGuest` matches the postal-evidence regex, or the post-marker region carries a conf delimiter (arm ii, R13-R14), the segment carries booking material OUTSIDE the S9 partition — either a hotel listed after its guest (`Jane Doe - 1002 Marriott Downtown 200 Oak Ave …`, the R5 shape) or a GLUED later booking whose own `Check In` is missing (the R12-finding-2 shape, where the marker count alone cannot reveal the glue). Warn (tier 2), never auto-correct — a positive scan on an otherwise tier-1-qualifying segment demotes it: keeping the prefix hotel would hand it the glued booking's guests and dates. False-positive surface is small by construction — the match needs a street number + suffix or a `ST ZIP` tail; conf numbers, `Check In: 3/3` dates, and bare guest names have neither (consultants' later group and B1z's bare-guest group 2 probe negative). Date-window edge: a street number followed by a suffix word can only come from address text, not from the `Check In/Out` grammar.
...
| n | `tests/parser/dataGaps.test.ts` — the test NAME at line 43 ("has exactly 34 entries…"), `expect(GAP_CLASSES).toHaveLength(34)` at line 44, `DATA_GAP_CODES.size` at line 45, the enumeration at line 75, and the `// 34` comment at line 157 (R12 finding 4: all five 34-literals in the file) | 34 → **36** in every listed location; add both codes |
| o | `tests/parser/dataGapsClassCompleteness.test.ts:205`, `tests/parser/dataGapsClassCompleteness.test.ts:209`, doc comment `tests/parser/dataGapsClassCompleteness.test.ts:36`, `tests/parser/dataGapsClassCompleteness.test.ts:68`, test name `tests/parser/dataGapsClassCompleteness.test.ts:204` | 34 → **36**; `ALL_PERSISTED_WARNING_CODES.size` 54 → **56**; the :68 doc comment "(51)" is ALREADY STALE on main (live assertion at :209 is 54) — rewrite it to "(56)" in the same edit (R13 finding 6) |
...
| C-OWN-5 | catalog `helpfulContext` AND the master-spec §12.4 helpfulContext appendix entry, byte-identical (x1 parity via `gen:spec-codes`; R13 finding 1) | `One hotel line seems to book more than one hotel, so this reservation was given its own hotel instead of sharing the line's first one. Check this reservation's hotel name, address, guests, and dates against your sheet. To avoid this warning, give each hotel booking its own row in the sheet.` |
| C-OWN-6 | catalog `longExplanation` (catalog-ONLY — the master spec carries helpfulContext, not longExplanation; R13 finding 1) | `One hotel line seems to book more than one hotel, so this reservation was given its own hotel instead of sharing the line's first one. Spot-check this reservation's hotel name, address, guests, and dates. This cannot be fixed in the app: if the hotel is wrong, edit the sheet so each hotel booking has its own row, and the next sync will pick it up.` |
...
| C-SUS-1 | `ParseWarning.message` | `Hotel line "<raw, collapsed>" may put this reservation under the wrong hotel; double-check it. To fix it, edit the sheet so each hotel booking has its own row.` |
| C-SUS-2 | catalog `title` | `A reservation may show the wrong hotel` |
| C-SUS-3 | catalog `dougFacing` | `A hotel line in _<sheet-name>_ may show a reservation under the wrong hotel; check it against your sheet. Giving each booking its own row fixes this.` |
| C-SUS-4 | catalog `triggerContext` | `Appears when a reservation on a shared hotel line may be under the wrong hotel.` |
| C-SUS-5 | catalog `helpfulContext` AND the master-spec §12.4 helpfulContext appendix entry, byte-identical (R13 finding 1) | `A reservation on a shared hotel line may be under the wrong hotel. Check it against your sheet. This cannot be fixed in the app: edit the sheet so each hotel booking has its own row, and the next sync will pick it up.` |
| C-SUS-6 | catalog `longExplanation` (catalog-ONLY; R13 finding 1) | `A reservation on a shared hotel line may be showing the wrong hotel. Spot-check it against your sheet. This cannot be fixed in the app: edit the sheet so each hotel booking has its own row, and the next sync will pick it up.` |
...
| Advertised remediation clears the warning (R8) | the "Guard-(c) conservatism" bookings re-authored per the C-SUS instruction as the structured HOTEL table (probe-verified 2026-07-28, exact literal, R13 f3 pin): header `\| HOTEL \| RESERVATION \#1 \|  \| RESERVATION \#2 \|  \|`, then alignment row, then rows `Hotel Name / Address` / `Hyatt Regency 100 Main St Chicago, IL 60601` + `Marriott Downtown 200 Oak Ave, Chicago, IL 60601`, `Names on Reservation` / `John Smith` + `Mary Ann Smith`, `Check In Date \| Check Out Date \| Check In Date \| Check Out Date` / `3/1/26 \| 3/2/26 \| 3/3/26 \| 3/4/26` | two clean reservations `{Hyatt Regency, 100 Main St Chicago, IL 60601, [John Smith], 2026-03-01..02}` and `{Marriott Downtown, 200 Oak Ave, Chicago, IL 60601, [Mary Ann Smith], 2026-03-03..04}`, ZERO new-code warnings — proves the single advertised fix works for the exact 3-word-guest shape the removed address-fix advice could not clear |
...
- AUTO-CORRECTING hotels listed AFTER their guests within a segment (`Jane Doe - 1002 Marriott Downtown 200 Oak Ave …`): guest-first ordering leaves no provable hotel span, so no tier-1 path exists for it. When the post-guest text carries street/postal evidence or a post-marker conf delimiter, it is NOT silent — the post-prefix scan (§3 D5, Codex R5 finding 2; R13 finding 2) stashes `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED`, whose copy carries the sheet-fix guidance. A hotel identified by BARE NAME ONLY with no address and no post-marker conf delimiter (`Jane Doe - 1002 Marriott Downtown Check In: …`, probe 2026-07-28: parses to a clean inherited row) is indistinguishable from note text at the raw-fragment level (S1) and stays tier-3 silent — a documented detection limit, pinned by the §8.1 bare-name negative, not a defect.
