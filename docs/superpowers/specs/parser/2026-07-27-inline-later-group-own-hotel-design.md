...
At most one SUSPECTED stash per row regardless of how many arms fire. Envelope for scope-(A)/(B) stashes: `blockRef.index` = the surviving row's final index, `blockRef.name` = its resolved hotel name, `rawSnippet` = the row's raw (its segment on the multi-row path; the whole cell for the fallback survivor, matching the existing single-path `rawCells`). The operator learns the cell may book another hotel even though the parser could not split it.
...
| 2 | (D4 matched but the postal terminator is absent, the residual-tail guard fired, or D6 aborted) OR (D4 did not match AND `baseWords(prefix) >= 4`) OR post-guest address evidence (below) | inherit as today; stash `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` |
...
| `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` | tier-2 row; any row inheriting from a tier-1 group (§4); any row whose segment holds >= 2 `Check In` markers with a positive post-first-marker evidence scan (§3 scope A, R9+R10); or the fallback survivor with tier-1/2 evidence (§3 scope B, R7). Max once per row | `warn` | **absent** (S2) |
...
| Full envelope, code OWN | cell B1z | exactly one `HOTEL_INLINE_GROUP_OWN_HOTEL`: `severity "warn"`, `blockRef {kind:"hotels", index:1, field:"address", name:"Marriott Downtown"}`, `rawSnippet` === segment 2 exactly, `resolution` key ABSENT, `message` === C-OWN-1 with the collapsed raw substituted |
| Full envelope, code SUSPECTED | the "Tier 2, word arm" input | exactly one `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED`: `severity "warn"`, `blockRef {kind:"hotels", index:1, field:"address", name:"Hyatt Regency 100"}` (the inherited hotel), `rawSnippet` === its segment exactly, `resolution` key ABSENT, `message` === C-SUS-1 with the collapsed raw substituted |
...
- Tiers = **3**; tier-2 word threshold = **4** base words; tier-1 postal terminator = `/(?:\d{5}(?:-\d{4})?|[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d)$/u` (S8); residual-tail guard arms = **3** (a/b/c); D4b tail arms = **3**; tier-2 entry arms = **4** (no-terminator / guard-or-D6 / word-count / post-guest evidence)
