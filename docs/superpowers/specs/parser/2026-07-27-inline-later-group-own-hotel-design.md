...
Degradation is handled at TWO scopes, and neither auto-corrects — auto-correct exists only for cleanly-segmented later groups, where "cleanly-segmented" means the segment contains **exactly one `Check In` match** (the degraded-segment gate, Codex R11 finding 1):
...
**D5 — tier decision.** Precondition for tier 1: the segment contains exactly ONE `Check In` match. A segment with two or more is degraded (scope A above): D3-D6 still evaluate, but a tier-1-qualifying outcome demotes to tier 2, and the scope-A post-first-marker evidence scan also applies — at most one SUSPECTED per row regardless.

| Tier | Condition | Action |
| ---- | --------- | ------ |
| 1 | segment has exactly one `Check In` match (R11 degraded gate) AND D4 matched AND `hotelText` is postal-terminated (S8: trimmed `s2.slice(0, addressEnd)` matches `/(?:\d{5}(?:-\d{4})?|[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d)$/u`) AND the residual-tail guard passes AND the D6 rebuild succeeds | keep own hotel (D6); stash `HOTEL_INLINE_GROUP_OWN_HOTEL` |
| 2 | (D4 matched but the postal terminator is absent, the residual-tail guard fired, or D6 aborted) OR (D4 did not match AND `baseWords(prefix) >= 4`) OR post-guest address evidence (below) OR a degraded segment (>= 2 `Check In` matches) whose D3-D6 outcome would otherwise be tier 1 (R11 demotion) | inherit as today; stash `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` |
| 3 | otherwise | inherit as today, no stash |

**Post-guest address evidence (Codex R5 finding 2).** For any segment NOT kept by tier 1, evaluate `postGuest = s2.slice(prefixEnd)` (delimiter, conf, later guests, dates — everything after the prefix): if `" " + postGuest` matches `STREET_ADDRESS_RE` or `STREET_ADDRESS_ZIP_RE`, or `postGuest` matches the postal-evidence regex, the segment lists hotel/address text AFTER its guest (`Jane Doe - 1002 Marriott Downtown 200 Oak Ave …`) — the ordering the prefix scan cannot reach. Warn (tier 2), never auto-correct: guest-first ordering leaves no provable hotel span. False-positive surface is small by construction — the match needs a street number + suffix or a `ST ZIP` tail; conf numbers, `Check In: 3/3` dates, and bare guest names have neither (consultants' later group and B1z's bare-guest group 2 probe negative). Date-window edge: a street number followed by a suffix word can only come from address text, not from the `Check In/Out` grammar.
...
| Full envelope, code OWN | cell B1z | exactly one `HOTEL_INLINE_GROUP_OWN_HOTEL`: `severity "warn"`, `blockRef {kind:"hotels", index:1, field:"address", name:"Marriott Downtown"}`, `rawSnippet` === the RAW segment 2 exactly (pre-D1: whitespace runs and any `&#10;` intact, R11 finding 3), `resolution` key ABSENT, `message` === C-OWN-1 with the collapsed raw substituted |
| Full envelope, code SUSPECTED | the "Tier 2, word arm" input | exactly one `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED`: `severity "warn"`, `blockRef {kind:"hotels", index:1, field:"address", name:"Hyatt Regency 100"}` (the inherited hotel), `rawSnippet` === its RAW segment exactly (pre-D1, R11 finding 3), `resolution` key ABSENT, `message` === C-SUS-1 with the collapsed raw substituted |
...
- Tiers = **3**; tier-2 word threshold = **4** base words; tier-1 postal terminator = `/(?:\d{5}(?:-\d{4})?|[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d)$/u` (S8); tier-1 additionally requires exactly **1** `Check In` match in the segment (R11 degraded gate); residual-tail guard arms = **3** (a/b/c); D4b tail arms = **3**; tier-2 entry arms = **5** (no-terminator / guard-or-D6 / word-count / post-guest evidence / degraded demotion, R11)
