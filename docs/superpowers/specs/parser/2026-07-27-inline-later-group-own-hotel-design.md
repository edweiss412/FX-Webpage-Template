...
On EVERY single-reservation collapse of a multi-`Check In` cell (`checkInCount >= 2`, one surviving reservation, regardless of cause), the rows are discarded as today — **no auto-correct survives the fallback**, because it exists precisely when grouping/attribution failed — but the wrong-hotel signal is preserved: append exactly ONE `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` stash to the surviving reservation when EITHER

- (a) any computed later-segment outcome was tier 1 or tier 2 (including a D6 abort), OR
- (b) the **degraded-segmentation evidence scan** fires: the region after the FIRST `Check In` match's end (which can only contain later bookings' material — group 0's own hotel text precedes its `Check In`) matches `STREET_ADDRESS_RE` or `STREET_ADDRESS_ZIP_RE` (padded read) or the postal-evidence regex. Probe-verified: true on the missing-checkout Marriott shape above; false on `Hyatt Regency John Smith - 1001 Check In: 3/1 Jane Doe - 1002 Check In: 3/3 Check Out: 3/4` (no address after the first marker).

Envelope for the attributed stash: `blockRef.index` = the surviving row's final index, `blockRef.name` = its resolved hotel name, `rawSnippet` = the whole cell (the single reservation's raw, matching the existing single-path `rawCells`). The operator learns the cell may book another hotel even though the parser could not split it.
...
| `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` | tier-2 row; any row inheriting from a tier-1 group (§4); or ONCE on the surviving reservation of a multi-`Check In` single-reservation collapse with tier-1/2 evidence or a positive degraded-segmentation evidence scan (§3 ordering note, R7+R9) | `warn` | **absent** (S2) |
