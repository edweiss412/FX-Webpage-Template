...
- `[prefixEnd, end)` — everything after the prefix (delimiter, conf, dates, and any GLUED later material), which must scan NEGATIVE for hotel evidence (the post-prefix scan: `" " + s2.slice(prefixEnd)` matches neither `STREET_ADDRESS_RE` nor `STREET_ADDRESS_ZIP_RE`, and `s2.slice(prefixEnd)` lacks the postal-evidence regex);
...
| C-SUS-1 | `ParseWarning.message` | `Hotel line "<raw, collapsed>" may name another hotel for this reservation that we could not read, so it may show the wrong hotel; double-check it. To fix it, edit the sheet so each hotel booking has its own row.` |
...
| C-SUS-5 | catalog `helpfulContext` | `Part of a hotel line may name a hotel we could not read, so this reservation may show the wrong hotel. Check it against your sheet. This cannot be fixed in the app: edit the sheet so each hotel booking has its own row, and the next sync will pick it up.` |
| C-SUS-6 | master-spec + catalog `longExplanation` | `Part of a hotel line may name a hotel we could not read, so this reservation may be showing the wrong hotel. Spot-check it against your sheet. This cannot be fixed in the app: edit the sheet so each hotel booking has its own row, and the next sync will pick it up.` |
...
| Advertised remediation clears the warning (R8) | the "Guard-(c) conservatism" bookings re-authored per the C-SUS instruction: each booking in its OWN structured HOTEL table row (Hyatt+John, Marriott+Mary Ann Smith) | two clean reservations with the correct hotels, ZERO new-code warnings — proves the single advertised fix works for the exact 3-word-guest shape the removed address-fix advice could not clear |
...
| Partial degradation, multi-row survival (R10) | the 3-booking cell of §3 scope A (`Hyatt … Check In: 3/1 Marriott Downtown 200 Oak Ave, Chicago, IL 60601 Jane Doe - 1002 Check In: 3/3 Check Out: 3/4 Bob Roe - 1003 Check In: 3/5 Check Out: 3/6`) | TWO rows byte-equal to today's parse, PLUS exactly one SUSPECTED at index 0 (segment 0 has 2 `Check In` markers + positive evidence). NO warning on Bob's row (tier 3, single marker). INTEGRATION test — kills the R10 partial-degradation bypass |
