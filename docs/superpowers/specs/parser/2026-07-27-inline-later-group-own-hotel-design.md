...
Probe-verified (2026-07-27, all through the real `splitHotelNameAddress`): `… 200 Oak Ave, Chicago, IL 60601 Jane Doe` → hotelText spans the full address, split `{Marriott Downtown, 200 Oak Ave, Chicago, IL 60601}`; `… 200 Oak Ave Suite 400 Jane Doe` and `… Suite 400, Chicago, IL 60601 Jane Doe` extend correctly; `… 200 Oak Ave, IL 60601 Jane Doe` (no city) extends via the optional-city arm.

**D5 — tier decision.**

| Tier | Condition | Action |
| ---- | --------- | ------ |
| 1 | D4 matched AND the residual-comma guard passes AND the D6 rebuild succeeds | keep own hotel (D6); stash `HOTEL_INLINE_GROUP_OWN_HOTEL` |
| 2 | (D4 matched but the residual-comma guard or D6 aborted) OR (D4 did not match AND `baseWords(prefix) >= 4`) | inherit as today; stash `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` |
...
**Residual-comma guard (Codex R1 finding 1).** After D4b, if `s2.slice(addressEnd)` begins (after optional whitespace) with a comma, the address tail was NOT fully classified — e.g. `200 Oak Ave, Chicago Jane Doe`, a ZIP-less city tail neither tail arm may consume. Keeping tier 1 would persist a truncated address; downgrade to tier 2. Probe-verified: that input downgrades with hotelText intact through the suffix only.
...
| Address tail, comma-less city | B1 variant, later hotel `Marriott Downtown 200 Oak Ave Chicago, IL 60601` | tier 1 via the longer-match tie-break (D4); full address persists. Kills an implementation whose tie-break prefers the suffix match |
| Unit tail | B1 variant, later hotel `Marriott Downtown 200 Oak Ave Suite 400` | tier 1; `hotel_address === "200 Oak Ave Suite 400"` |
| ZIP-less city tail downgrades | B1 variant, later segment `Marriott Downtown 200 Oak Ave, Chicago Jane Doe - 1002 Check In: 3/3 Check Out: 3/4` | tier 2 (residual-comma guard): inherited hotel, `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED`, and NO truncated `"200 Oak Ave"` address anywhere on the row |
