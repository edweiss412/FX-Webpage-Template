...
**D4 — address anchor.** Evaluate BOTH regexes against `" " + prefix` (position-0 normalization, parent spec §3.1 "P3 position-0"). If either matches, take the match with the smaller index (tie: the longer match); `addressEnd` = match index + match length − 1 (unpadding the 1-char pad), an index into `s2`. Both regexes are non-global singletons consumed read-only; the detector never adds flags to them (parent spec R4-finding-5 discipline: never mutate the shared regex).

**D5 — tier decision.**

| Tier | Condition | Action |
| ---- | --------- | ------ |
| 1 | D4 matched AND the D6 rebuild succeeds | keep own hotel (D6); stash `HOTEL_INLINE_GROUP_OWN_HOTEL` |
| 2 | (D4 matched but D6 aborted) OR (D4 did not match AND `baseWords(prefix) >= 4`) | inherit as today; stash `HOTEL_INLINE_GROUP_HOTEL_SUSPECTED` |
