...
### 7.4 The ratified rows — reclassification mechanics

**Amended 2026-08-16 by measurement (branch-5 implementation).** This section was written as "the 72 ratified rows (58 payload-reorder + 14 signal-reorder)", derived from the ledger probe's split of the 82 `section-reorder` rows into 10 real-loss and 72 ratified. The collected harness run refutes the split: the content-keyed detector closes **24** of them, not 10, so **59** remain — and all 59 are of kind `wrong`, because the operator's entire `signal_loss` and `text_drift` population was what the positional sweep produced. The reclassification MECHANICS below are unaffected; only the counts are. Evidence: `probes/2026-08-16-newhole-mechanism.md`.

They move to the documented-limit register (§11). `OPERATOR_FINDING_MAP["section-reorder"]` (`knownHoles.ts:88`) KEEPS the literal value `"BL-MUTATION-SECTION-ORDER"` — the documented-finding FORM here is the archived-row form, not a new string: `knownHoles.test.ts:153` validates shape only, and the umbrella precedent (`BL-MUTATION-HARNESS-OPEN-HOLES`) is that an archived row keeps its id resolvable. What changes in the same commit as the backlog row's archive: the map's inline comment (→ "documented: source order ratified, spec §7; archived row") and the suite's pointer comment. (Amended 2026-08-09, retro plan review F4 — the earlier "re-maps to a new ref string" wording contradicted the plan's pinned disposition.) The ratified rows STAY in `RAW_HOLES` as documented-class rows (the audit-#5/#10 precedent — documented classes keep their rows); only the venue-hardening rows surface as `fixedHoles` and are deleted. Counts predicted 72 staying and 10 deleted; measured 59 and 24 (§7.4 amendment). Probe precondition SATISFIED: §13.2 confirms zero of the 58 `wrong` rows touch a VENUE block and zero lose signals, and the 14 have byte-identical signal multisets.
...
5. For section-reorder only: rows split by disposition — the venue-hardening rows surface as `fixedHoles` and are deleted (measured 24, not the predicted 10 — §7.4 amendment); the ratified remainder (measured 59) STAYS as documented-class rows under the `OPERATOR_FINDING_MAP` re-map (§7.4), landing in the same commit; `knownHoles.test.ts` keeps every remaining row resolvable.
