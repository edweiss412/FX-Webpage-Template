...
- **AC-11.** The form-dump arm compares NORMALIZED openers, not raw strings. Every corpus form dump spells its opener exactly `Timestamp`, so an implementation writing `opener === "Timestamp"` satisfies AC-5, AC-9 and AC-10 while violating §3.1, and no corpus input distinguishes the two. The criterion is discharged against one-edit spelling variants of a real corpus form-dump block: `TIMESTAMP`, `timestamp`, `Timestamp:` and a space-padded form all normalize to `timestamp` and must be excluded exactly as `Timestamp` is (TABLE-N: 4 of the 5 spellings separate the rule from the impostor). The test carries a premise that each variant differs from the corpus block in the opener spelling ALONE, so it cannot pass by testing an unrelated document, and it is stated as a case an exact-string implementation FAILS rather than as a restatement of §3.1.
