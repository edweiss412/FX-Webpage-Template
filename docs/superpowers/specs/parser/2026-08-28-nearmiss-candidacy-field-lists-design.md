...
- **AC-11.** The form-dump arm compares NORMALIZED openers, not raw strings. Every corpus form dump spells its opener exactly `Timestamp`, so an implementation writing `opener === "Timestamp"` satisfies AC-5, AC-9 and AC-10 while violating §3.1, and no corpus input distinguishes the two. The criterion is discharged by REBUILDING a real corpus form-dump block with its opener cell replaced, one ordinary sheet edit at a time, and requiring the rule's verdict where an exact-string implementation gives the opposite: `TIMESTAMP`, `timestamp` and `Timestamp:` all normalize to `timestamp` and must be excluded, and the real detector emits `Backdrop` and `Room Diagram` from that block whenever it is admitted, which is the consequence the impostor ships (TABLE-N: 3 of 6 spellings separate the rule from the impostor). The test asserts shape invariance on each rebuild — the opener changed and the row count and minimum value cells did not — so it cannot pass by having perturbed something else, which is the round-3 defect applied to this table. Whitespace padding is deliberately NOT among the discriminating cases: `scanRowsWithOpener` cleans the opener before either comparison sees it, so ` Timestamp ` is read as `Timestamp` by rule and impostor alike.
...
5. **A form dump whose opener is spelled with an internal space.** `Time stamp` normalizes to `time stamp`, which is not `timestamp`, so the form-dump arm does NOT exclude it and its rows become candidates again. Measured in TABLE-N, where it is the one spelling on which the rule and an exact-string impostor agree. This is the inverse of limit 1: that one is a field list wrongly excluded, this one is a form dump wrongly admitted, and both follow from keying the arm on a single normalized token. Zero corpus instances; a live sheet spelling it that way is the re-file trigger.
