...
- **Form dump.** `normalizeV3(opener) === "timestamp"`. A Google Form response export opens on the `Timestamp` column; its rows are questions, not fields. The comparison is on the NORMALIZED form and that is load-bearing rather than incidental: every corpus instance happens to spell it `Timestamp`, so an exact-string implementation is indistinguishable from this rule on the corpus as it stands, and diverges the moment a sheet is edited to `TIMESTAMP` or `Timestamp:`. AC-11 is the criterion that separates them.
...
`joann | Diagrams?` is the one surviving row whose home is arguably not a field list, and the shape is nameable: a QUESTION CHECKLIST addressed to a person. The opener is a name (`JOANN`) and every one of its four other rows ends in a question mark: `Details?`, `Setup Time?`, `Diagrams?`, `Budget?`. Three of those are already suppressed by the existing §3.1 label guards (`Details?` is named in the predecessor spec's §3.2 as guard-suppressed), which is why exactly one row reaches the baseline. On the merits the card is as wrong there as in the other two shapes: telling Doug to rename `Diagrams?` inside someone's to-do list is not useful advice. It survives because the ratification names exactly two non-home shapes, and this block is neither. Excluding it would need a third arm keyed on something this spec has not measured and the ratification did not authorize, which is the forbidden direction for this arc (§1.1.8). It is recorded as a documented limit (§6) with a re-file trigger, not carried as a hidden decision.
...
**The injected row must not change what the rule classifies.** Spec round 3 found that it did. The first version appended a row with ONE value cell to every block, which drops an inventory matrix's MINIMUM to 1, so by §3.1 the mutated block is no longer a matrix and a correct detector must ADMIT it, while the table still expected exclusion. AC-10's matrix half was unsatisfiable without violating §3.1, and a conforming implementation would have finished at 18 disagreements rather than 0. The blast radius of that naive append is kept measurable rather than only described (TABLE-M): **151 of 514 blocks had their minimum changed, and 18 of those crossed the exclusion boundary** — exactly the 18 inventory-matrix blocks, which is the count round 3 named. The `Timestamp` half was unaffected, because appending a row does not change an opener.

The injected row is now padded to the block's OWN minimum value-cell count, which preserves the minimum exactly and leaves the opener untouched. Verified across all 514 corpus blocks: zero change to `(opener, minValueCells)`.

**Three controls, each asserted rather than assumed**, because this probe has now been wrong three times and every one of those defects made it report agreement or disagreement for a reason it had not established:

| control | required | catches |
| --- | --- | --- |
| positive | the injected label emits somewhere (514 of 514 today) | an emission check that matches nothing, so every block reads "not emitted". The first draft read a `detail` field `emitUnknownField` does not set. |
| ambiguity | zero blocks already contain the injected label | attributing a pre-existing row's emission to the injected one. `Address:` failed this in 9 `client` blocks; `Equipment Storage:` appears nowhere in the corpus. |
| shape invariance | zero blocks whose `(opener, minValueCells)` the injection changed | the round-3 defect itself: a probe measuring a block the rule would classify differently from the one it computed its expectation from. |

At the merge base the probe reports **33 disagreements out of 514 blocks** — exactly the 33 the rule excludes, because today the detector has no candidacy gate and emits in all of them. The implementation drives that to 0. A three-key hardcode leaves all 33 standing.
...
- **AC-9.** The block classification census (§3.5) is pinned as a committed artifact keyed PER BLOCK, asserted at CANDIDACY level rather than through emissions. The test walks every corpus block, applies the candidacy predicate, and compares the full per-block verdict set, each row identified by `fixture#ordinal` so a swap within one namespace cannot leave the artifact unchanged. It carries a premise that the walk actually saw the families it claims to cover (21 `venue`, 13 `details`, 4 `console`, each derivable from TABLE-K rather than stated only here), so a scoping bug that selects nothing cannot pass it vacuously, and it asserts BOTH directions: every `venue` and `details` block admitted, every `console` block refused. A pin that only asserted admission would pass against a predicate that admits everything, which is the pre-change detector.
- **AC-10.** The detector is bound to the predicate. For every corpus block, injecting one row whose label is a known near-miss and is not one of the three known keys makes `detectFieldNearMisses` emit if and only if the predicate admits that block. RED at the merge base with exactly 33 disagreements, the 33 excluded blocks; GREEN at 0 after implementation. This is the criterion that fails an implementation exposing a correct predicate for AC-9 while suppressing the three known keys directly in the detector, which every other criterion in this spec accepts. The test asserts all three controls of §3.5 — positive, ambiguity, and shape invariance — as assertions rather than properties of the chosen string, so a later edit to the injected label or its padding fails loudly instead of quietly making the binding unsatisfiable.
- **AC-11.** The form-dump arm compares NORMALIZED openers, not raw strings. Every corpus form dump spells its opener exactly `Timestamp`, so an implementation writing `opener === "Timestamp"` satisfies AC-5, AC-9 and AC-10 while violating §3.1, and no corpus input distinguishes the two. **The criterion is discharged by an EXECUTED suite case, not by a probe table.** The case rebuilds a real corpus form-dump block with its opener cell replaced, requires the candidacy predicate to exclude every spelling that normalizes to `timestamp` (`TIMESTAMP`, `timestamp`, `Timestamp:`), and states two premises on its own inputs: that the reparsed opener DIFFERS from the source opener, which is what makes a rebuild a rebuild, and that the row count and minimum value-cell count did not move. Whitespace padding is deliberately excluded from the discriminating set: `scanRowsWithOpener` cleans the opener, so ` Timestamp ` is reparsed as `Timestamp` and separates nothing. The case lives inside the `fieldNearMiss` source-mutation surface, so an exact-string implementation is killed by the harness rather than caught by a reader. TABLE-N illustrates the same shapes and is explicitly not evidence (§6.7).
...
- `tests/parser/fieldNearMiss.test.ts` gains the per-class cases: AC-4, AC-5, AC-6, AC-7, AC-9, AC-10 and AC-11. AC-10 pins the detector to the predicate (it emits if and only if the predicate admits); AC-11 pins the normalized-opener comparison against rebuilt blocks. Both are named here because both are EXECUTED cases, and AC-11 in particular carries the whole weight of its criterion after TABLE-N's demotion (§6.7). Each shape case states its premise executably via `tests/_shared/premise.ts` and asserts the exclusion, never merely the absence of output. A test that only shows "nothing was emitted" would pass against a detector that emits nothing at all, which is exactly the mutant the registry's `control` row plants.
...
3. **The question-checklist shape.** §3.4. A block whose opener is a person's name and whose every other row ends in a question mark is not a field list, but it is a THIRD shape and excluding it is outside this arc's ratification, which named exactly two. One row survives from it. The re-file trigger is a second corpus instance of a question-checklist block, or an owner decision on that shape.
...
5. **A form dump whose opener is spelled with an internal space.** `Time stamp` normalizes to `time stamp`, which is not `timestamp`, so the form-dump arm does NOT exclude it and its rows become candidates again. This is the inverse of limit 1: that one is a field list wrongly excluded, this one is a form dump wrongly admitted, and both follow from keying the arm on a single normalized token. It is not the only spelling on which the rule and an exact-string comparison agree — `Timestamp` agrees because both exclude it, and ` Timestamp ` agrees because the scanner cleans it to `Timestamp` before either comparison runs — but it is the only one where they agree by both ADMITTING. Zero corpus instances; a live sheet spelling it that way is the re-file trigger.
6. **Fixtures are the corpus, not the workbooks.** The markdown fixtures concatenate tabs, so the block boundaries measured here are the fixture's pipe runs. The three named rows were probed against the live RIA workbook by the filing arc; this spec's tables are fixture-derived by construction, and the two agree on all three rows.
7. **A probe table asked to carry an acceptance criterion.** TABLE-N was written to prove AC-11, and spec rounds 4, 5 and 6 each found a defect in it or its predecessor: no criterion separated the rule from an impostor; the table added to fix that compared two local expressions and never rebuilt a block; the rebuilt version's shape check compared the reparsed opener against the spelling it had just requested rather than against the source opener, so it reported "shape held" for two spellings whose opener never changed. The class is stable rather than accidental — a table is read, never executed, so nothing but a reviewer's attention stands between a wrong table and its acceptance, and each repair yields another unreviewed table. Retired by moving AC-11's proof into the suite, where the runner executes it and the mutation harness attacks it. Recorded here rather than filed as a row: the repair already shipped. Re-file trigger: another criterion in this repo resting on a probe table rather than an executed case.
