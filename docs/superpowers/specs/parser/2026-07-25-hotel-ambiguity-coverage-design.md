...
**Status:** Draft → self-review → adversarial review R1–R8 (all BLOCKING, all repaired) → R9 pending
...
| # | Surface | Site | Exact text |
| - | ------- | ---- | ---------- |
| C1 | Inline guest `ParseWarning.message` | new inline emitter | `Hotel line "<raw, collapsed>" holds the hotel and the people staying in one cell, so we had to work out where each part starts; double-check who is on this reservation.` |
...
| C18 | Disabled reason, P1 | same | `This hotel line holds the hotel and the people staying all together, so there's nothing safe to swap in.` |
...
| **`contentHash` is derived, not fixed** (R8 finding 5) | every resolvable emit test | assert `resolution.contentHash === contentHashForRawSnippet(warning.rawSnippet)` computed independently in the test, AND that two warnings with different `rawSnippet` have different hashes. A fixed valid 64-hex constant passes an undo test that builds its decision from the emitted hash, but then unrelated address warnings share decisions and edits stop invalidating them |
