...
2. **Per-segment comparison form.** For each content segment, derive the comparison token: trim, uppercase, then **peel a trailing restriction marker** — a trailing `ONLY` (optionally `ONLY***`) or a bare `***` (exactly three), `/(\s*\bONLY\b(?:\s*\*{3})?\s*|\s*\*{3}\s*)$/i` — so a stage word followed by `ONLY`/`***` (e.g. `"STRKE ONLY"`) is compared as `"STRKE"`. (Deliberately **not** 1–2 stars — `***`-count tolerance is deferred, §9; `"ONLY*"` is left unpeeled and falls through to existing behavior.) This mirrors how the `…ONLY` regexes treat `ONLY` as a separate trailing marker, so the normalizer's token model **matches the grammar it feeds** (closes the HIGH-class divergence: without the peel, `"STRKE ONLY"` is distance > 1 from `STRIKE` and the gate silently fails on the 2-word ONLY cells).
...
| `Lod In/Set/Strke/Load Ot - V1` (3 typos, each Damerau ≤ 1) | 4 (1 exact `SET` / 3 near: `LOD IN`+A, `STRKE`-I, `LOAD OT`+U) | yes (1) | **fires** | all 3 corrected → `Load In/Set/Strike/Load Out - V1`; `V1` parses; 1 drift note. (Note: a word with **two** typos like `Lod Ot` is Damerau 2 — beyond `maxDistance=1` — and is **not** corrected; two typos in one stage word is not confidently that stage word.) |
