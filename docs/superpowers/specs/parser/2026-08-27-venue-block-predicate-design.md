...
- **helpfulContext:** `A row's label in your sheet is misspelled. We matched it to the field name it stands for, so it wasn't listed as a row we didn't recognize. This is a record for us; there is nothing for you to fix.`
...
**Three drafts, three different false claims, all of which cleared the mechanical gates.** Draft one said the row still shows on the crew page (false about publication); draft two invited a sheet edit (contradicting the not-actionable classification); draft three said we read the row (false about storage). The banned-vocabulary regex and the caps passed every time. `.probe/copy-check4` now asserts the absence of all three semantic classes alongside the mechanical ones, and it runs before the copy enters the catalog.
