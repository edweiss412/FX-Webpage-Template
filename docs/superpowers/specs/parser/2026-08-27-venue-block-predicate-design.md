...
**The anchor repair reaches a code that renders a real link.** `UNKNOWN_FIELD` IS a member of `OPERATOR_ACTIONABLE_ANCHORED` (`lib/parser/dataGaps.ts:414`), which is what `CELL_ANCHORED_CODES` aliases, so a v4 venue-table near-miss row that resolves to a cell renders an "Open in Sheet" link on its card. §4 is therefore not a mechanical tidy-up: it is the difference between an operator seeing a row they cannot locate and one they can click through to. That is done-condition 5.
...
**The copy does not say "misspelled", and the reason is in the alias table's own comment.** `TYPO_ALIASES` holds four entries, and one of them is `"diagrams"` — annotated in place as a "capitalisation variant", with the set's header saying it covers "true misspellings (not capitalization variants like `DIagrams` which is a corpus capitalization oddity but not a user typo)" and that it is listed only because the master spec §1.10 names it. `resolveAliasFull` lowercases before testing membership (`lib/parser/aliases.ts:178-181`), so EVERY casing resolves through it: `DIagrams`, `Diagrams`, `DIAGRAMS` and `diagrams` all emit `TYPO_NORMALIZED`, including the perfectly correct spelling. Copy calling that a misspelling would be false on the most ordinary casing of one of the four codes it describes.

What is true of all four is narrower and is what the copy now says: the label matched an alternate spelling the alias table keeps for a field, so it was not reported as unrecognized.

**The copy claims RECOGNITION and nothing beyond it, which is all the parser actually does.** A third draft said "we read the row as intended" and "a record of what we read". Both imply the row's VALUE was taken, and it is not: probed across all four registered typo aliases in a v4 venue table, the value appears nowhere in the parse result (`valueStoredAnywhere=false` for every one), `show.event_details` is `{}` and `contacts` is `[]`. `venue.contact_info` in particular is a field `parseVenue` never writes. What the alias resolution genuinely buys is that the row is not reported as unrecognized — `raw_unrecognized` is empty — and that is exactly what the copy now says.

**Four drafts, four different false claims, every one of which cleared the mechanical gates.** Draft one said the row still shows on the crew page (false about publication); draft two invited a sheet edit (contradicting the not-actionable classification); draft three said we read the row (false about storage); draft four called the label misspelled (false for `Diagrams`, whose every casing resolves through the typo set). The banned-vocabulary regex and the caps passed all four times, because they lint vocabulary and none of these is a vocabulary defect. `.probe/copy-check5` asserts the absence of all four semantic classes alongside the mechanical ones, and it runs before the copy enters the catalog. The lesson is an ordering one: probe what the code does, then write only what the probe supports — never write the sentence first and look for confirmation.
...
**`TYPO_NORMALIZED` becomes the FIRST info-severity member of `WARNING_CARD_COPY_CODES`, and that is in scope rather than a stretch.** Measured: all 47 current members are warn-severity, and neither info code in `INFO_CODE_ACTIONABILITY` is a member today. The registry's own criterion is not severity — §3.1 of `docs/superpowers/specs/2026-07-20-warning-card-copy-restore.md` defines it as "every parse-warning code the cards can render", and this code renders on a note card, which is what the `CARD_SURFACED_LOG_ONLY` carve-out exists to arrange. Two consequences worth stating so neither is discovered late:

- The corpus oracle (`tests/messages/_metaWarningCardCopy.test.ts:160-173`) asserts warn-severity-emitted ⟹ registered, ONE direction only. It never asserts registered ⟹ warn-severity, so an info member neither breaks it nor is covered by it. **Nor does it cover this code at all**, and no other gate did either: an earlier draft of this section claimed "layer two, the static membership check" covered it, and that check does not exist. Probed: `CARD_SURFACED_LOG_ONLY` is imported by `_metaCatalogAdminLogOnlyAlignment`, `catalog.test`, `catalog-schema-extension` and `catalogDocsValidator`, and by none of them to assert registry membership; `rg CARD_SURFACED_LOG_ONLY tests/messages/_metaWarningCardCopy.test.ts tests/messages/warningCardCopyRegistry.ts` returns nothing.
- All three existing `CARD_SURFACED_LOG_ONLY` members happen to be registry members, but **nothing enforced that**, so it was a coincidence this spec was reading as a pattern. A contributor could drop `TYPO_NORMALIZED` from the registry and its expected-copy maps while leaving it card-surfaced, and its copy would then escape the banned-vocabulary regex, the caps and the byte-fidelity freeze entirely, while still rendering to Doug.

**So this arc ships the missing implication as an assertion rather than asserting it in prose (AC-V11).** `CARD_SURFACED_LOG_ONLY ⊆ WARNING_CARD_COPY_CODES`, in `tests/messages/_metaWarningCardCopy.test.ts`: a code the cards surface must carry card copy, which is the whole premise of the carve-out. It is green on the three current members and stays green on the fourth, so it pins today's truth rather than demanding new work. This is the structural-defense calibration rule taken at FIRST occurrence: the class is nameable now, so the guard lands in this arc's repair rather than after a recurrence proves it.
...
- **AC-V11 — the carve-out's implication is enforced, not assumed.** `tests/messages/_metaWarningCardCopy.test.ts` asserts `CARD_SURFACED_LOG_ONLY` is a subset of `WARNING_CARD_COPY_CODES`. Its red is CONSTRUCTED rather than waited for: dropping any one member from the registry must fail it. Green on the three current members before the change and on four after, so it pins today's truth rather than demanding new work.
...
**A probe must not import from the test tree.** `tests/components/admin/sheetIconLinkContainment.test.ts` walks the FILESYSTEM and fails any non-exempt file importing from `tests/`, and `.probe/` is untracked but still on disk — the same "on disk whether or not it is TRACKED" hazard `.gitignore` records for the mutation shard scratch. An early version of these probes imported the harness fixture module and turned the whole suite red locally while the tree looked clean. They now derive the fixture list themselves (`.probe/_fixtures`, globbing the two fixture directories and excluding that directory's `README.md` — an early version swept it in and reported 18 fixtures where the harness has 17). The guard is NOT exempted: editing a gate so scratch passes is the forbidden direction.
...
8. **probe 8, `payload-truth`** — §5's copy claims, all of them. For every registered typo alias in a v4 venue table it prints `show.venue`, `show.event_details`, `contacts`, the `raw_unrecognized` length, and whether the row's value appears ANYWHERE in the result. All four report `valueStoredAnywhere=false`, `event_details={}`, `contacts=[]`, `raw_unrecognized=0`.

   It **replaces** `finding-repairs`, which read `r.venue` — a key the parse result does not have (the path is `r.show.venue`) — and so printed `undefined` and verified nothing while §5 quoted it as evidence. The replacement asserts the key it reads before reading it, so the same vacuity cannot recur silently. The probe-mini-review rule paying for itself: a probe feeding a spec is a spec input.
9. **probe 9, `copy-check5`** — the copy gate. Asserts the mechanical rules (banned vocabulary, caps) AND the absence of all four semantic classes the review rounds found: an outcome claim, an action invitation, a read-the-row claim, and a misspelling claim.
10. **probe 10, `copy-claim-sweep`** — §5.4's derived cover. Scans the copy-restore spec, its meta-test and its registry for every line whose count the registry's size or membership determines, so the dependent-claim inventory is a rerunnable scan rather than a list that goes stale. Rerun after the edit: a surviving `47` in a claim position is a miss. Every registered typo alias under a v4 venue table yields `contacts=[]` and no event output, which is what refuted the "still shows on the crew page" draft.
...
- **An opener carrying a newline entity between its words** (`VENUE&#10;NAME`) is not the venue block. BOTH arms reject it, together, which is the parity §2.3 chose over permissiveness; and the parser surfaces `UNKNOWN_FIELD` on such an opener row anyway (probed: `raw_unrecognized` gains `{block: "venue name", key: "VENUE&#10;NAME"}`), so the shape is conservative-plus-surfaced rather than silent. An earlier draft named `FIELD_LABEL_AUTOCORRECTED` here, which is the wrong code; the disposition is unchanged, the fact was not. Closing it would mean wrapping arm 2 in `decodeEntities`, which arm 1 does not do — making the arms disagree again in the opposite direction.
