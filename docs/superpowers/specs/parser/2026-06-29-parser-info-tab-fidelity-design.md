...
**Scope:** **three** parser-only (non-UI) fixes surfaced by the INFO-tab data-fidelity audit (`docs/audits/info-tab-fidelity-audit-2026-06-29.md`): H1 DRESS capture, H2 lunch-room dedup, M3 title banner-preference. No migrations, no UI files, no `app/`, no `components/`. Commit style `feat|fix|test(parser):`. **(A fourth fix — M1 GS-dimension capture — was DROPPED during spec review; see Resolved Decision 5.)**
...
Wire into `index.ts` orchestrator after `parseEventDetails`, merging through the **existing sentinel-aware precedence** (round-6 fix, Codex spec-R6) — NOT a blunt overwrite. Reuse the rule in `event.ts:314 writeField` (export it, or apply the identical 3 lines): a **sentinel** dress block (`shouldHideGenericOptional(dress)` true — `''`/`TBD`/`N/A`/`TBA`) must NOT clobber an existing **real** `eventDetails.dress_code`; otherwise the dress block wins (last-write-wins). Cases:
- DRESS block real, existing absent/sentinel → block wins (the common case: standalone DRESS before DETAILS, existing absent). ✓
- DRESS block real, existing real → block wins (the dedicated full-block capture supersedes the in-DETAILS single-row alias). ✓
- **DRESS block sentinel (`N/A`), existing real (e.g. DETAILS `Attire = Black tie`) → keep the real value, drop the sentinel.** ✓ (the regression Codex spec-R6 flagged — a blunt overwrite would have hidden dress.)
- DRESS block sentinel, existing absent/sentinel → block (sentinel) stored; UI hides it (unchanged outcome). ✓

The typo-tolerant alias path (`atire`→dress_code) is untouched: `parseDress` matches only exact normalized `DRESS`, so fuzzy-only labels stay with `parseEventDetails`.
...
**Acceptance predicate — use the parser's OWN canonical recognizer (structural convergence, Codex spec-R5→R9).** After five rounds of per-instance regex patching on the room-header edge (bare → suffixed → `BREAKOUTS` → no-separator), the robust fix is to stop hand-rolling a room-header matcher and instead reject anything the parser already recognizes as a section/room header. Refactor the existing #6 guard (`index.ts:202-209`) into a shared helper `isAcceptableTitleCell(cell)` used by BOTH #0 and #6 (so they can't drift). `isAcceptableTitleCell` returns **false** (reject) when any of:
- empty;
- `isKnownNonTitle(cell)`;
- `cell.toUpperCase().startsWith("CLIENT")`;
- `startsWith("NO_HEADER")`;
- `cell === "\\#NUM\\!"` or `/^\\#/.test(cell)` (escaped error cells);
- **`isKnownSectionHeader(cell)`** — the canonical recognizer from `knownSections.ts`. It exact-matches bare headers (`CLIENT`, `DETAILS`, `DOCUMENT FOLDER LINK`, `VENUE`, `DRESS`, `HOTEL`, …) AND **whole-token-prefix-matches the `PREFIX_SECTION_FAMILIES`** (`GENERAL SESSION`, `BREAKOUT`, `BREAKOUTS`, `ADDITIONAL ROOM`, `LUNCH ROOM`, `LUNCH SESSION`). Because it prefix-matches, it rejects **every** room-header shape uniformly — separator or not, numbered or not: `GENERAL SESSION - GRAND BALLROOM A/B`, `GENERAL SESSION SALON ABC`, `BREAKOUT 1 DELAWARE`, `BREAKOUT 2 - SALON C`, `ADDITIONAL ROOM 2`, `LUNCH ROOM BALLROOM C`. This is the SAME predicate the parser uses to decide a row is a section header, so the title guard and the parser cannot disagree about what a room header is, and a future registry addition is covered automatically.

**Accepted tradeoff (explicit):** because `isKnownSectionHeader` prefix-matches the families, a hypothetical title that literally begins with a room-family token (e.g. "Breakout Innovation Summit", "General Session Networking Expo") would be rejected and fall to the filename fallback (#7). This is intentional: (a) NO show in the live corpus has such a title (all are `II - …`/`AII/III - …`/`East Coast …` forms — see the table above and the 10 fixtures); (b) single-cell disambiguation of "BREAKOUT <words>" (title) vs "BREAKOUT <room>" (header) is not reliably possible without the surrounding-row context the parser has and a title cell does not; (c) the failure mode is a degraded title (filename-derived), never wrong *data*. This reverses the earlier round-6/7 attempt to preserve "Breakout Innovation Summit" via exact-only matching, which left the no-separator room-header hole (Codex spec-R9). **The shared helper means BOTH #0 and #6 reject these (round-6 fix).**

So a no-banner sheet whose first row is a duplicated **section header** — bare (`| CLIENT | CLIENT |`, `| DOCUMENT FOLDER LINK | … |`, `| \#NUM\! | … |`) OR any room header (`| GENERAL SESSION SALON ABC | … |`, `| BREAKOUT 1 DELAWARE | … |`) — is rejected by BOTH #0 and #6 and falls through to the filename fallback (#7). Only after passing all guards is a duplicated `cells[0]` returned. This signature:
...
## Live-sheet validation (REQUIRED close-out gate — Codex spec-R7)
...
- **DRESS (H1)** — live `INFO` rows 27-28: `["DRESS","Set/Strike: Black Pants, Black Polo Shirt, Black Footwear"]` then `["","Show: Black Pants, Black Long Sleeve Button Down Shirt, Black Footwear"]`. Confirms the header+continuation block exists with both labeled lines; the exporter renders a `| :---: | :---: |` separator between them (→ parseDress must skip it). ✓ matches Fix 1.
- **Lunch dedup (H2)** — live `INFO` row 110 `LUNCH ROOM / BALLROOM C`; live `GEAR` row 87 `LUNCH SESSION - GRAND BALLROOM C`. Confirms the kind/name mismatch (INFO breakout `BALLROOM C` vs GEAR `GRAND BALLROOM C`) the dedup targets. ✓ matches Fix 2.
- **Title (M3)** — live `INFO` row 1 `AII/III - Consultants Roundtable 2025` (proper case, year); live `GEAR` `Event Name:` = `AII/III - CONSULTANTS ROUNDTABLE` (uppercase, year dropped). Confirms banner-vs-EventName. ✓ matches Fix 3. (The exporter column-duplicates row 1 → the banner signature holds in production markdown.)
- **GS dims (dropped Fix 4 confirmation)** — live `INFO` row 77 GS header is the multi-line cell `GENERAL SESSION / GRAND BALLROOM A/B / A/B: 82' x 63' x 14' / 8th Floor` → dims are **inline** → `splitRoomHeader` already captures them → no live parse drop → Fix 4 correctly dropped. ✓

**Note on what the parser actually consumes:** production input is exporter-generated **markdown** (sheet → exporter → markdown → parse), so structural assertions (separator rows, column-duplication) are validated by the exporter fixture format, while the live-sheet check above confirms the **content** assumptions still hold. The implementation's TDD tests run in CI (no gsheets access); this live-sheet check is the manual close-out artifact that catches stale-fixture skew.
...
2. **Dress on real fixture** (`parseSheet`/`exporterFixtures`): assert `parseSheet(consultants).show.event_details.dress_code` contains both `Set/Strike` and `Show`. Negative-regression: mutate `parseDress` to return only the header row → test fails (proves it captures continuations).
   - **Sentinel-precedence mixed-source tests (Codex spec-R6):** (a) top-level `DRESS | N/A` + DETAILS `Attire | Black tie` → final `dress_code === "Black tie"` (sentinel block must NOT clobber the real DETAILS value); (b) top-level `DRESS | <real>` + DETAILS `Dress Code | N/A` → final `dress_code === <real>` (real block wins over sentinel); (c) both real → block wins. Negative-regression: wire `parseDress` as a blunt overwrite → case (a) regresses to `N/A` → test fails.
...
4. **Title banner** (`tests/parser/...`): exact assertions — consultants → `AII/III - Consultants Roundtable 2025`; fintech/fixed-income/rpas → proper-cased banners (not uppercase). Unchanged assertions: east-coast, ria, redefining-fi titles equal their current values (snapshot the current value first, assert preserved). Generic guards (parseSheet.test.ts:44/54) still pass. Negative: revert priority #0 → consultants title reverts to uppercase. **Duplicated-section-header negatives (Codex spec-R5→R9):** inline no-banner fixtures whose first table row is `| CLIENT | CLIENT |`, `| DOCUMENT FOLDER LINK | DOCUMENT FOLDER LINK |`, `| \#NUM\! | \#NUM\! |`, and room headers in BOTH shapes — separator (`| GENERAL SESSION - GRAND BALLROOM A/B | … |`, `| BREAKOUT 2 - SALON C | … |`) AND **no-separator** (`| GENERAL SESSION SALON ABC | … |`, `| BREAKOUT 1 DELAWARE | … |`, `| ADDITIONAL ROOM 2 | … |`, `| LUNCH ROOM BALLROOM C | … |`) → assert the **final `show.title`** (via `parseSheet`, not just `#0`) is NOT that header — it falls through to the filename fallback (#7). This proves BOTH #0 and the shared #6 reject every room-header shape via `isKnownSectionHeader`. Positive coverage: the real banner titles (consultants/fintech/fixed-income/rpas exact assertions above) confirm legitimate titles are still accepted — note a title literally beginning with a room-family token is intentionally NOT supported (degraded to filename fallback; none exist in the corpus).
5. **Meta-test** (`_metaKnownSectionsRegistry`): `DRESS` registered (passes with the registry as-is).
6. **Structural title-guard test (class-level defense, Codex spec-R5→R9 convergence):** `it.each` over **every** `KNOWN_SECTION_HEADERS` entry (bare) AND every `PREFIX_SECTION_FAMILIES` entry rendered in MULTIPLE suffix shapes — separator (`${fam} - SYNTHETIC ROOM`), numbered (`${fam} 2 - SYNTHETIC`), and **no-separator** (`${fam} SYNTHETIC ROOM`) → build a minimal no-banner sheet whose first table row is that header **column-duplicated** → assert `parseSheet(...).show.title` is NEVER that header (falls through to filename fallback). Both the guard and this test import the SAME `isKnownSectionHeader`/registry from `knownSections.ts`, so they cannot diverge and a future registry addition is automatically covered. Companion positive (must still pass): the real exporter banner titles (e.g. `II - FinTech Forum CTO Summit 2026`) ARE accepted — they are not section headers.
