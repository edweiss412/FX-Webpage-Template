...
- **`TYPO_NORMALIZED` is re-gated, not orphaned.** The scope flag §2.1 removes has a second reader: the `TYPO_NORMALIZED` info emission at `venue.ts:135`, deliberately scope-gated by a prior review decision (`venue.ts:149-155`). Its gate re-keys from the positional window to the STORE EVENT: emit iff the row's typo alias resolved to a `venue.*` canonical AND `parseVenue` stores that field from this row (content-keyed, so swap-invariant; a typo-alias-shaped row in a foreign block, e.g. `Virtaul Audience` in a `DETAIL CHECKLIST` block, stays silent because `parseVenue` stores nothing from it). Expected corpus census: 0 today (review probe docs/superpowers/specs/parser/probes/2026-08-15-rev-probe-typo.ts), 0 after — pinned alongside the baseline (AC-N8). `FIELD_LABEL_AUTOCORRECTED` (the emission immediately following, venue.ts:156) is already row-local-proven and ungated — untouched.
...
The `ADDITIONAL ROOM&#10;Dimensions&#10;Floor` case is a real section header defeated by encoding, not an operator mistake. THREE repair sites, per the calibration's diagnosis (its §3 miss table) and the review's site verification — `_sectionHeaderMatch.ts` is NOT one of them (`rooms.ts` never imports it; its additional-room matcher is a raw allowlisted regex, `rooms.ts:100-107`):

1. `parseAdditionalRoom`'s own opener matching (`lib/parser/blocks/rooms.ts:535`, regex defined near `rooms.ts:100-107`): decode entities in the col-0 it tests, so the block PARSES as `rooms.additional` payload (AC-N4).
2. `isKnownSectionHeader` (`lib/parser/knownSections.ts:202`): decode entities before the exact/prefix check, closing the sequencing gap where the raw `&#10;` text prefix-matched through `&`.
3. `canonicalSectionKind` (`lib/parser/sectionKind.ts:82`): same decode, same reason — these two are the detector's candidate-exclusion checks, and they must agree with the detector's own v3 normalization about what `&#10;` means.

Payload-affecting repair with its own fixture-backed test; after it, those 2 rows stop being near-miss candidates (they resolve). Calibration table reflects post-repair state.
...
- **AC-N8:** `TYPO_NORMALIZED` census is 0 on the unreordered corpus (pinned alongside the baseline) and its re-keyed store-event gate is unit-tested both directions: a constructed typo venue-field row inside the venue block fires exactly once at the store; the corpus's `Virtaul Audience` row (foreign block) stays silent under every adjacent swap (the 497-sweep covers it via `signalKeys`).
...
- **Catalog (`lib/messages/catalog.ts:1307-1321`) — ALL SIX copy strings move to near-miss framing, not two:** `title` ("Unrecognized row in sheet"), `dougFacing`, `crewFacing`, `followUp`, `helpfulContext` ("Your sheet has a row we didn't recognize…"), `triggerContext` ("…doesn't match anything we know how to show."), and `longExplanation` (the `/help/errors` body: "…doesn't match any section we read…") — each currently asserts the retired "we don't recognize this" framing, which is now false by construction (the row DOES nearly match). Example dougFacing direction: "A row labeled 'Stage' looks like it was meant to be 'Stage Size', so it isn't showing on the crew page. If it should show, rename the row in the sheet." Exact copy drafted at plan time under the copy rules (no raw codes, no em-dashes, `'` apostrophes, `STAGE_WORD_AUTOCORRECTED` tone template at `catalog.ts:1428-1443`).
