...
- Composition of the 72 (sums exactly; probe §7.2/§8/§9): **7 audited true positives** (Stage ×2, Storage ×2, Address:, Phone:, Client:/Contact:) + **25 same-shape near-misses in additional fixtures** (colon-suffixed contact fields incl. the 9 recovered `E-mail:` rows) + **15 `Backdrop`** (confirmed unconsumed near-miss of `event.scenic`'s "Backdrop / Scenic") + **21 `Room Diagram` rows in `Timestamp` blocks** (Google-Forms echo text; the 3 `Room Diagram` rows in DETAILS blocks are the consumption-excluded ones) + **`Speaker` ×2** + **`Notes` ×1** (roundtable's empty-value instance) + **`Diagrams?` ×1**. `Details?`/`Contact`/`Contact:` are SUPPRESSED by the adopted guard and are NOT in the baseline. All residuals conservative and surfaced, pinned in the baseline JSON.
...
The naive "payload changed ⇒ consumed" rule would have dropped Stage/Storage — `parseEvent`'s fallback stores them under self-slug keys while today also emitting `UNKNOWN_FIELD`. The normative definition is DELETION-DIFF-EQUIVALENT, stated at the write site: **a row is consumed iff a block parser resolves its label to a curated canonical key (exact map hit, alias hit, regex path, or gated fuzzy correction — `gatedVocabCorrect`, `lib/parser/blocks/event.ts:200-212` and event.ts:292-298) AND actually writes a payload value from it** (a `presence()`-filtered or sentinel-suppressed write consumes nothing — the ledger mark sits after the filter, at the write). Fallback self-slug writes (`toCanonicalKey`) are not curated and never mark. Verified against the real `CANONICAL_KEY_MAP` including the coincidence case (a curated slug equal to the self-slug: "room diagram", "notes" — curated wins, row excluded).

**Derived cover for ledger call sites (plan-time walk, not a two-item list):** every module under `lib/parser/blocks/` that accepts a `ParseAggregator` (11 files) is audited for internal resolution vocabularies at plan execution; the known members at spec time are `event.ts` (`CANONICAL_KEY_MAP` + `gatedVocabCorrect`), `contacts.ts` (regex resolution), `transport.ts` (lib/parser/blocks/transport.ts:217 label regex). The arbiter is executable, not the list: the committed 72-row baseline equals the deletion-diff measurement, so a missed ledger site surfaces as a baseline mismatch, not silence.
...
- **AC-N8:** `TYPO_NORMALIZED` census is 0 on the unreordered corpus (pinned alongside the baseline) and its re-keyed venue-block-membership gate is unit-tested both directions: a constructed fixture with a typo-alias row (`Hotal Contact Info`) INSIDE the venue block fires exactly once; the corpus's `Hotal Contact Info` rows (hotel blocks) and `Virtaul Audience` row (DETAIL CHECKLIST block) stay silent, including under every adjacent swap (the 497-sweep covers it via `signalKeys`).
...
- **Four labels surfaced today go silent — an enumerated, ratified loss, in two distinct categories** (review probe docs/superpowers/specs/parser/probes/2026-08-15-rev-probe-regression.ts: all four emit as `UNKNOWN_FIELD` today, one row each, and are absent from the 72-row baseline; part of the ratified 394→72 quieting):
