...
16. **Server-side time-call grep guard** (`tests/help/_metaServerTimeGuard.test.ts`) — new in r8
    - Greps every server-side `.ts`/`.tsx` under route directories reachable from the screenshot manifest (`app/show/`, `app/admin/`, plus any added in future).
    - For each match of `new Date()` / `Date.now()`: asserts EITHER the containing file imports from `lib/time/now.ts` AND uses the utility for render-side time, OR the matched line carries `// not-render-side: <reason>` waiver comment.
    - Catches new render-side time call sites added without migration to the utility — they would drift captured screenshots even when `lib/time/now.ts` is wired correctly.
    - Anti-tautology: the test reads source files directly (filesystem), not the rendered output; bypassing via test mocks is impossible.
...
| **AC-12.12** | All 15 unit/integration tests in §7.1 (items 1–10 plus 12 + 13 + 14 + 15 + 16) pass; nav-consistency, anchor-resolver, screenshot-coverage, manifest-integrity, `<picture>`-contract, error-renderer-gate, deep-link-walker, fixture-range-parser, `lib/time/now.ts`-gate, and server-time grep-guard tests are red on the conditions they guard. CI drift gate (§7.1 item 11) is wired and fails on uncommitted screenshot drift. |
...
| **AC-12.35** | M12 catalog-alignment subtask (r8 — reformulated from r5–r7 flag concept): `lib/messages/catalog.ts` is reconciled with master-spec §12.4 admin-log-only contract (master-spec line 2691). For every code master-spec line 2691 names as admin-log-only, set `dougFacing: null`, `crewFacing: null`, and `helpfulContext: null`. Named codes (master-spec line 2691, non-exhaustive list): `STALE_WRITE_ABORTED`, `STALE_PUSH_ABORTED`, `WEBHOOK_NOOP_ALREADY_SYNCED`, `CONCURRENT_SYNC_SKIPPED`, `WIZARD_SESSION_SUPERSEDED_DURING_SCAN`, `LOCK_OWNERSHIP_ASSERTION_FAILED`, `STAGED_PARSE_REVISION_RACE`, `LINK_CROSS_SHOW_REUSE`, `UNEXPECTED_PARENT`, `DIAGRAMS_TAB_MISSING`, `TYPO_NORMALIZED`. Implementation walks the full live catalog against master-spec line 2691's enumeration AND adjacent admin-log-only patterns (asset-recovery / cooldown variants: `STALE_MANUAL_REPLAY_ABORTED`, `STAGED_PARSE_REVISION_RACE_COOLDOWN`, `ASSET_RECOVERY_REVISION_DRIFT`, `ASSET_RECOVERY_DRIFT_COOLDOWN`). Behavior change: AlertBanner stops surfacing these to Doug — which is master-spec-correct. Tests that exercised drifted behavior need updating; M12 owns those updates as part of the catalog-alignment subtask. The full canonical list lives in the catalog source after r8; the spec does not re-enumerate every code (the source IS the source). |
