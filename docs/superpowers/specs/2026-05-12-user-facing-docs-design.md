...
- Examples of codes the parser **does NOT** derive (so Phase B does NOT add stubs):
  - `LINK_CROSS_SHOW_REUSE` — master-spec line 2860 (drifted from original wording `:2846` as master spec grew) Doug cell starts with `(operator log only`, which is NON-canonical per master-spec line 2702. M11 does not amend master-spec for this; if/when master-spec is amended to use `(admin log only`, the parser will derive it on the next run and the next milestone's alignment pass will null-stub it. Until then, M11 explicitly does not add or modify this entry.
