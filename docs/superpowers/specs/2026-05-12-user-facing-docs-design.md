...
| **Fixed clock (r4 → r5 → r6 server-side pinning)** | All time-dependent rendering is captured at a fixed instant. **`frozenClockInstant` is REQUIRED per manifest entry** — no project-wide default. Each entry picks an instant inside its fixture's operational window. Mechanism: (a) browser — Playwright's `context.clock.install({ time: frozenClockInstant })` pins `Date`/`Date.now`; (b) **server — a single test-only env var `SCREENSHOT_FROZEN_NOW=<ISO>`** is set per-entry by `globalSetup` (in addition to fixture-seeded timestamps relative to `frozenClockInstant`). The env is consumed by exactly ONE server-side time utility (a new `lib/time/now.ts` or equivalent) that returns the frozen instant when set, real `Date.now()` otherwise. The utility's read is guarded: `process.env.SCREENSHOT_FROZEN_NOW` is honored only when `process.env.ENABLE_TEST_AUTH === "true"` — same gating contract as `tests/e2e/helpers/signInAs.ts:1-23`. Production builds with `ENABLE_TEST_AUTH` unset ignore the env. Without server-side pinning, "12 min ago" computed against real `Date.now()` would drift between captures by wall-clock delta — caught in round 4. r5's "no env var" attempt was incorrect; r6 reverses with explicit gating. | `tests/e2e/right-now.spec.ts:87-114`; `tests/e2e/helpers/signInAs.ts:1-23` gating pattern |
...
| **(template-family row, r6)** Any error message rendered through `messageFor(code)` in `/admin/*` (excludes `adminLogOnly === true`) | `Learn more →` | Testid family `help-affordance--error-message--<code>--learn-more` where `<code>` is a lowercase-kebab transform of the catalog code. **Walked by family rule in test #13, NOT by AC-12.36's single-testid regex** — implementation iterates the catalog and asserts presence per-code. | `/help/errors#<code>` via `messageFor(code).helpHref` | M9 / M10 |
...
13. **Deep-link affordance walker** (`tests/help/deep-link-walker.test.ts`) — r4, discovery-by-testid r5, row-class split r6
    - **Discovery mechanism:** the test reads §5.6's matrix (as a typed `affordanceMatrix.ts` import). Three row classes are handled differently:
       - **Concrete testid rows** (the majority): walks each row by its documented `data-testid`. Asserts the affordance is present in the rendered admin route and the link's `href` matches the matrix target.
       - **Template-family row** (error-message): iterates the catalog (entries matching the AC-12.6 predicate), constructs the per-code testid via `help-affordance--error-message--<lowercase-kebab(code)>--learn-more`, navigates to a fixture surface that renders that code, asserts the link's `href` matches `messageFor(code).helpHref`.
       - **Negative-assertion row** (crew): navigates as a signed-link viewer to `/show/<slug>`, asserts no `data-testid^="help-affordance--"` element exists in the rendered DOM.
    - **Reverse-direction check:** the test ALSO greps the codebase for any `data-testid="help-affordance--*"` literal and asserts each is enumerated in `affordanceMatrix.ts` (concrete rows) OR matches the template-family pattern. Catches affordances added without matrix rows.
...
| **Catalog predicate consistency with `adminLogOnly`** | AC-12.6 / AC-12.11 / test #2 all explicitly include `adminLogOnly !== true` in the predicate (not just §5.1 prose). **Per round-4 finding 1.** Test #12 explicitly mocks an `adminLogOnly: true` entry with `helpHref` non-null and asserts no link renders. | AC-12.6, AC-12.11, §7.1 test 2, §7.1 test 12 |
| **`adminLogOnly` enumeration is a derivation rule, not a fixed list** | AC-12.35's 9-code list is the set known at write time; the rule is "any code master-spec §12.4 classifies as admin-log-only OR alert-feed-only-with-no-help-destination." Implementation walks the entire live catalog against §12.4 and flags every match. **Per round-4 finding 2** (added 3 codes: WEBHOOK_NOOP_ALREADY_SYNCED, ASSET_RECOVERY_REVISION_DRIFT, ASSET_RECOVERY_DRIFT_COOLDOWN). | AC-12.35 |
...
| **AC-12.6** | Every §12.4 catalog entry where `severity !== "info"` AND `dougFacing != null` AND `adminLogOnly !== true` has `longExplanation` (non-null, non-empty) AND `helpHref` (non-null, `^/help/...`) populated; meta-test (§7.1 test 2) enforces. Three exclusion bands: (a) info-tier entries opt out by `severity === "info"`; (b) crew-only entries (`dougFacing == null`) stay `null` in v1, fill in phase 2; (c) admin-log-only entries (`adminLogOnly === true`, AC-12.35) stay `null` because their `dougFacing` is self-contained. |
...
| **AC-12.11** | `/help/errors` iterates the catalog and renders one anchored section per entry matching the AC-12.6 predicate — `severity !== "info"` AND `dougFacing != null` AND `adminLogOnly !== true`. |
...
| **AC-12.16** | No new boolean flags, no new Supabase tables. **One** new test-only env var introduced (r6): `SCREENSHOT_FROZEN_NOW` (ISO timestamp string). Production builds ignore it unless `ENABLE_TEST_AUTH === "true"` — same gating contract as the existing test-auth env. AC-12.37 enforces production-build rejection. |
...
| **AC-12.32** | Screenshot harness pins a fixed clock per §3.6.2 row "Fixed clock": `frozenClockInstant` is required per manifest entry (no project-wide default). Browser side: `context.clock.install({ time: frozenClockInstant })`. Server side: `SCREENSHOT_FROZEN_NOW=<ISO>` env var set per-entry by `globalSetup`, consumed by `lib/time/now.ts` (or equivalent single utility), gated to honor only when `ENABLE_TEST_AUTH === "true"`. Plus fixture seed timestamps anchored relative to `frozenClockInstant` so seeded data stays consistent with the pinned now. |
...
| **AC-12.35** | Catalog entries flagged `adminLogOnly: true` per master-spec §12.4 admin-log-only contract (r6 sweep). Known set at write time: `STALE_WRITE_ABORTED`, `STALE_PUSH_ABORTED`, `STALE_MANUAL_REPLAY_ABORTED`, `CONCURRENT_SYNC_SKIPPED`, `STAGED_PARSE_REVISION_RACE`, `STAGED_PARSE_REVISION_RACE_COOLDOWN`, `WEBHOOK_NOOP_ALREADY_SYNCED`, `ASSET_RECOVERY_REVISION_DRIFT`, `ASSET_RECOVERY_DRIFT_COOLDOWN`. **Derivation rule (binding for implementation):** any catalog code that master-spec §12.4 classifies as admin-log-only OR alert-feed-only-with-no-help-destination (i.e., `dougFacing` is its own self-contained action and no `/help/errors` page would add useful guidance) MUST be flagged. Implementation walks the entire live catalog against master §12.4 and flags anything matching, not only the names listed here. These entries' `longExplanation` and `helpHref` stay `null`; the renderer-gate (test #12) verifies no `Learn more →` is emitted even if `helpHref` is mocked non-null. |
...
| **AC-12.37** | `lib/time/now.ts` (or equivalent) returns the frozen instant ONLY when `process.env.SCREENSHOT_FROZEN_NOW` is set AND `process.env.ENABLE_TEST_AUTH === "true"`. A production-mode unit test asserts: with `ENABLE_TEST_AUTH` unset, setting `SCREENSHOT_FROZEN_NOW` does NOT alter the utility's output. A capture-boundary test runs two consecutive `pnpm screenshot:help` invocations 60+ seconds apart and asserts byte-identical output (proves wall-clock minute crossings don't drift the bytes). |
