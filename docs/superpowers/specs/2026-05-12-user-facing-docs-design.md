...
**Render-side guard (r5 → r8 simplified):** The shared error renderer adds `Learn more →` only when **both** of these hold: (a) `helpHref` is non-null, AND (b) the rendering context is admin (`/admin/*` or `/help/admin/*`). Crew-facing surfaces (`/show/<slug>`) MUST NOT emit admin-gated `/help` links. (r5/r6/r7's third conjunct `selfContainedAction !== true` is removed — once the catalog is aligned with master spec, the codes formerly flagged with that field have `dougFacing == null` and never render to Doug at all, so the gate is unnecessary.) Enforced by `tests/messages/_metaErrorRendererGate.test.ts` — see §7.1 test 12.
...
| **AC-12.11** | `/help/errors` iterates the catalog and renders one anchored section per entry matching the AC-12.6 predicate — `severity !== "info"` AND `dougFacing != null`. Each section's visible heading is `entry.title` (NOT the code — invariant #5). The body is `entry.longExplanation`. The trailing link is `Learn more →` to `entry.helpHref` only when the user's auth context is admin (per the §5.2 render-side gate). |
| **AC-12.12** | All 16 unit/integration tests in §7.1 (items 1–10 plus 12 + 13 + 14 + 15 + 16 + 17) pass; nav-consistency, anchor-resolver, screenshot-coverage, manifest-integrity, `<picture>`-contract, error-renderer-gate, deep-link-walker, fixture-range-parser, `lib/time/now.ts`-gate, server-time grep-guard, and catalog-alignment meta-tests are red on the conditions they guard. CI drift gate (§7.1 item 11) is wired and fails on uncommitted screenshot drift. |
