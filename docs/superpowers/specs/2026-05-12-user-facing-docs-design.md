...
**Required-when predicate (r8 — simplified):** `severity !== "info"` AND `dougFacing != null` AND `title != null` AND `longExplanation != null` AND `helpHref != null` (last three are the M12-added fields themselves; the assertion is structural — entries that need docs have all three populated; entries that don't need docs have all three `null`).
