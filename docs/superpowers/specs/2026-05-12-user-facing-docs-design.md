...
| **Server-side `Date.now()` / `new Date()` migration inventory (r8)** | The browser clock pin doesn't reach server-rendered timestamps. Every server component reachable from a screenshot manifest route that calls `new Date()` or `Date.now()` for **render-side** output MUST migrate to `lib/time/now.ts`. Known render-side call site at r8-write time: `app/show/[slug]/page.tsx:646` (`const today = new Date()` — schedule rendering on the crew page, captured indirectly via the `/admin/show/<slug>/preview/<crew-id>` impersonation route in §5.6). Mutation-side call sites are NOT required to migrate (they set write timestamps; they don't appear in rendered output): `app/admin/actions.ts:81` (`resolved_at`), `app/show/[slug]/p/actions.ts:142` (`issuedAt`), `app/admin/dev/actions.ts:135` + `:194` (dev-only mutation timestamps). New grep guard test #16 (added in r8) catches future additions. | `app/show/[slug]/page.tsx:646` is the named render-side site |
...
`messageFor` keeps its signature; the **return type gains three new fields (r5 + r8)**:
...
export type MessageCatalogEntry = {
  ...
  followUp: string | null;
  helpfulContext: string | null;
  title: string | null;             // NEW in M12 (r8) — short heading for /help/errors#<code>
...
**`selfContainedAction` removed in r8 — catalog drift is M12's responsibility.** r5/r6/r7 invented a `selfContainedAction` opt-out for codes like `STALE_WRITE_ABORTED`, `CONCURRENT_SYNC_SKIPPED`, etc. Round 6 surfaced that these codes are canonically *master-spec admin-log-only* per master-spec line 2691: "a row whose Doug-facing message cell is `(admin log only ...)` / `—` and Crew-facing message cell is `—` is admin-log-only — the code is emitted to structured logs and sync_log for operator/Eric debugging, but is NEVER rendered to Doug's UI or crew's UI. The X.1 extractor normalizes the row's dougFacing and crewFacing to null." The runtime catalog has DRIFTED from master spec — it currently has non-null `dougFacing` for these codes. Per AGENTS.md invariant #7 (spec is canonical), the catalog must be reconciled, not blessed.
...
**Required-when predicate (r8 → r13 — single source of truth):** `predicate(entry) := severity !== "info" AND dougFacing != null`. The M12-added fields (`title`, `longExplanation`, `helpHref`) are the **consequent** — entries satisfying the predicate must have all three populated AND `helpHref` matching `/^\/help\/.+/`; entries NOT satisfying the predicate must have all three exactly `null`. The validator module `lib/messages/catalogDocsValidator.ts` (plan Task B.4) implements this contract as `contractViolations(entry)`; both forced fixtures (B.4) and the live-catalog assertion (E.13) import from it. **The earlier r8 wording that included the M12 fields inside the predicate definition was self-referential and is retracted in r13** — the M12 fields are what the predicate's truth value determines must be populated, not part of the predicate itself.

A tighter formulation: the meta-test asserts the **biconditional** that for every catalog entry where `severity !== "info"` AND `dougFacing != null`, all three M12 fields are non-null AND match shape (`helpHref` matches `/^\/help\/.+/`); for every entry where `dougFacing == null` OR `severity === "info"`, all three M12 fields ARE `null`. Crew-only entries (`dougFacing == null`, `crewFacing != null`) have all three `null` in v1; phase 2 widens the predicate and adds the fields then.
...
Info-tier entries: not required. Crew-only entries (`dougFacing == null`, `crewFacing != null`): all M12 fields stay `null` in v1; phase 2 fills them. Master-spec admin-log-only entries: post-r8 catalog alignment, those have `dougFacing == null` and naturally fall into the "not required" bucket — no separate flag needed.
...
| **(template-family row, r6 — r8 catalog-aligned)** Any error message rendered through `messageFor(code)` in `/admin/*` where `helpHref != null` (i.e., catalog entries matching the AC-12.6 predicate, naturally excluding the admin-log-only codes after r8 alignment) | `Learn more →` | Testid family `help-affordance--error-message--<code>--learn-more` where `<code>` is a lowercase-kebab transform of the catalog code. **Walked by family rule in test #13, NOT by AC-12.36's single-testid regex** — implementation iterates the catalog and asserts presence per-code. | `/help/errors#<code>` via `messageFor(code).helpHref` | M9 / M10 |
...
2. **Catalog meta-test** (`tests/messages/_metaErrorCatalogDocs.test.ts`) — r8 simplified
   ...
   - **Forced fixture cases (anti-tautology):**
     - synthetic entry with `severity: "warning"`, `dougFacing: "..."`, all M12 fields non-null → must PASS
     - synthetic entry with `severity: "warning"`, `dougFacing: "..."`, `helpHref: null` (one M12 field missing) → must FAIL
     - synthetic entry with `severity: "info"`, `dougFacing: "..."`, all M12 fields `null` → must PASS (info-tier)
     - synthetic entry with `dougFacing: null`, `crewFacing: "..."`, all M12 fields `null` → must PASS (crew-only)
     - synthetic entry with `dougFacing: null`, `crewFacing: "..."`, `helpHref: "/help/errors#X"` → must FAIL (biconditional violation — admin-log-only / crew-only must have null M12 fields)
   - Predicate covers both `severity === "warning"` and entries with unset severity (per `components/admin/AlertBanner.tsx:39-50` default-warning rule)
...
17. **Catalog-alignment meta-test** (`tests/messages/_metaCatalogAdminLogOnlyAlignment.test.ts`) — new in r9
    - Derives the canonical admin-log-only set by running `scripts/extract-admin-log-only-codes.ts` against `docs/superpowers/specs/2026-04-30-fxav-crew-pages-design.md` §12.4
    - For each code in the derived set, asserts the live `MESSAGE_CATALOG` entry has `dougFacing: null` AND `crewFacing: null` AND `helpfulContext: null` AND (new M12 fields) `title: null` AND `longExplanation: null` AND `helpHref: null`
    ...
    - **Parser normalization (r10 — clarified per round-8 finding 4):** the extractor classifies a row as admin-log-only when the `Doug-facing message` cell — after trimming whitespace — is EITHER (a) the literal em-dash `—` (U+2014), (b) an empty cell, OR (c) a parenthetical starting with `(admin log only` (with optional `— short hint)` suffix per master-spec line 2692). Other null-shaped sentinels (`null`, `none`, `n/a`) are NOT recognized — they would force a master-spec edit instead, matching the line-2692 em-dash-normalization rule. Forced parser fixtures cover all three accepted shapes.
    ...
    - This is the structural guard that round-7 finding 1 surfaced — without it, M12 could ship with admin-log-only codes still rendering to Doug via drifted catalog state.

18. **End-to-end clock-pipeline proof** (`tests/playwright/help-screenshots-clock-pipeline.spec.ts`) — new in r10 per round-8 finding 2
    ...
    - This is the missing test home for AC-12.39 (round-8 finding 2); without it, AC-12.39's "end-to-end proof" claim had no concrete implementation.
...
| **Catalog drift is M12's responsibility (r8 final)** | r6 found master-spec line 2691 names codes like STALE_WRITE_ABORTED / CONCURRENT_SYNC_SKIPPED as admin-log-only with `dougFacing: null`; live catalog has non-null `dougFacing` — drift. r5/r6/r7 invented `adminLogOnly`/`selfContainedAction` flags to dodge this; r8 instead **reconciles the catalog** (M12 catalog-alignment subtask sets `dougFacing: null` on all master-spec admin-log-only codes per AC-12.35). After alignment, the simple predicate `severity !== "info" AND dougFacing != null` suffices — no opt-out flag, no third conjunct. | AC-12.35, §5.1 |
...
| **Predicate-consistency + flag-enumeration history (collapsed r8)** | Rounds 4 and 5 surfaced individual omissions in r5/r6/r7's `selfContainedAction` opt-out (predicate inconsistency across AC/test, incomplete enumeration list). All resolved in r8 by removing the flag entirely; the catalog-alignment subtask in AC-12.35 covers the same surface canonically. | AC-12.35 |
...
| **Fixture path shape is flat single-file, not nested directories (r10)** | r5–r9 referenced `fixtures/shows/raw/<show>/INFO.md` (nested). Actual corpus is flat `fixtures/shows/raw/<fixture>.md` plus pdf-only split `fixtures/shows/pdf-only/<fixture>__INFO.md`. **Per round-8 finding 1.** | §3.6.2 row, AC-12.34 |
| **AC-12.39 has a named test home: §7.1 item #18 (r10)** | AC-12.39 mandated an end-to-end clock-pipeline proof but had no test home. r10 adds test #18 (E2E clock-pipeline proof, dedicated Playwright project, runs daily not per-PR). **Per round-8 finding 2.** | §7.1 test 18 |
...
| **Admin-log-only parser handles all three null-cell shapes (r10)** | The line-2691 normalization rule accepts: (a) literal em-dash `—`, (b) empty cell, (c) parenthetical starting `(admin log only`. r9's parser-spec mentioned (a) and (c); r10 adds (b) explicitly. Forced parser fixtures cover all three. Other sentinels (`null`, `none`, `n/a`) are NOT recognized — forcing master-spec edits to use the canonical shapes. **Per round-8 finding 4.** | §7.1 test 17 normalization clause |
| **/help/errors trailing CTA is "tell Eric", not self-linking "Learn more"** | §4.3 said each section ends with "If this keeps happening, tell Eric →". AC-12.11 said the trailing link is `Learn more → entry.helpHref` — but on `/help/errors` itself, `helpHref` points back to the same page. r10 aligns AC-12.11 with §4.3: trailing CTA is the bug-report link; `Learn more →` lives on source surfaces (admin pages where the error renders) per §5.6 matrix, NOT on the destination page. **Per round-8 finding 5.** | §4.3, AC-12.11 |
...
| **AC-12.5** | `MessageCatalogEntry` (declared in `lib/messages/catalog.ts:1-8`) is extended with three new fields: `title: string \| null`, `longExplanation: string \| null`, `helpHref: string \| null` (per §5.1, r8). `messageFor` signature unchanged; all existing callers continue to compile. |
| **AC-12.6** | Every §12.4 catalog entry where `severity !== "info"` AND `dougFacing != null` has `title`, `longExplanation`, AND `helpHref` all non-null (with `helpHref` matching `/^\/help\/.+/`). Conversely, every entry where `dougFacing == null` OR `severity === "info"` has all three M12 fields `null`. Meta-test (§7.1 test 2) enforces both directions as a biconditional. Exclusion bands: (a) info-tier (severity); (b) crew-only entries — phase-2 backfill; (c) master-spec admin-log-only entries — AC-12.35 aligns the catalog to have `dougFacing: null` for those, so they fall into the second arm naturally. |
...
| **AC-12.35** | M12 catalog-alignment subtask (r8 — reformulated from r5–r7 flag concept; sourcing tightened r9). `lib/messages/catalog.ts` is reconciled with master-spec §12.4 admin-log-only contract. **Derivation source (r9):** the admin-log-only set is derived **mechanically from the master-spec §12.4 markdown** — every table row whose `Doug-facing message` cell renders as `(admin log only ...)` / `—` per master-spec line 2691 normalization rule. The set is NOT the prose-named line-2691 examples list, which is explicitly non-exhaustive ("Examples (non-exhaustive)"). A new parser `scripts/extract-admin-log-only-codes.ts` reads `docs/superpowers/specs/2026-04-30-fxav-crew-pages-design.md` §12.4, scans every table row, classifies admin-log-only rows by the line-2691 normalization rule, and emits the canonical set. **Known misses in r8's manual enumeration (round-7 finding 1):** `DIAGRAMS_EMBEDDED_CAP_EXCEEDED` (master-spec line 2795), `PENDING_SNAPSHOT_ROLLBACK_STUCK` (line 2821), `PENDING_SNAPSHOT_PROMOTE_STUCK` (line 2822). The r9 derivation rule catches these and any future additions automatically. For every code in the derived set, the catalog-alignment subtask sets `dougFacing: null`, `crewFacing: null`, AND `helpfulContext: null` (per master-spec line 2691 normalization). M12 also sets the new M12 fields (`title`, `longExplanation`, `helpHref`) to `null` for these. New meta-test (§7.1 test 17) asserts the live catalog matches the derived set: every master-spec-admin-log-only row has all user-facing fields (`dougFacing` / `crewFacing` / `helpfulContext` / `title` / `longExplanation` / `helpHref`) set to `null`. |
...
| **AC-12.38** | Server-side render-time call sites reachable from the screenshot manifest are migrated to `lib/time/now.ts` per §3.6.2 row "Server-side `Date.now()` / `new Date()` migration inventory". At write time, `app/show/[slug]/page.tsx:646` is the named site. Mutation-path sites in `app/admin/actions.ts`, `app/show/[slug]/p/actions.ts`, `app/admin/dev/actions.ts` carry inline `// not-render-side: <reason>` waivers. Grep guard (test #16) catches future violations. |
| **AC-12.39** | One representative end-to-end screenshot capture proves the request-scoped header pipeline actually changes a rendered page (not just the utility in isolation). The manifest entry for `/admin/show/<slug>/preview/<crew-id>` (impersonation surface) captures the crew page with a `frozenClockInstant` that would render "today" as a fixture-relative date. Test asserts the captured WebP differs byte-wise when captured with a different `frozenClockInstant` — proving the header reaches the server component's render. |
