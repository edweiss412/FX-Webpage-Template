...
There are **24** `CardHeaderActions` call sites (grep `<CardHeaderActions` over `components/`+`app/`, excluding tests). **23 are `SectionCard` `action=`-slot hosts** (contexts A/B → `hitDirection="up"`); **exactly one is the bare schedule header** (`ScheduleSection.tsx:253` → `hitDirection="down"`). The `app/admin/dev/source-link-dim/page.tsx:124` harness site is a SectionCard host.

| Context | Host | Band height | Space ABOVE (nearest neighbor) | Space BELOW | Below-neighbor interactive? |
| --- | --- | --- | --- | --- | --- |
| **A. Icon SectionCard** | `SectionCard` with `icon`+`title` (e.g. `TodaySection.tsx:495`, `GearSection.tsx:449`, `VenueSection.tsx:233`) | ≥28px (`size-7` icon, `SectionCard.tsx:51`) | ≥`gap-3` 12px (tightest inter-card stack: `today-quick-cards` `flex flex-col gap-3`, `TodaySection.tsx:362`; grids use `gap-4` 16px, `CrewSection.tsx:153`; distinct sections use `gap-section-gap` 32px, `CrewSections.tsx:115`) + the upper sibling's own `p-tile-pad` 20px inside its card | `gap-3` 12px (`SectionCard.tsx:39`) | **Yes** — body rows below can be interactive. |
| **B. Title-only SectionCard** | `SectionCard` with `title` only, NO icon (`BudgetSection.tsx:102`; Dress card `TodaySection.tsx:514`) | ~16px (`text-xs` title) | same as A (≥12px gap + upper card's 20px pad) | `gap-3` 12px | **Yes** — `BudgetSection` renders `KeyValueRows`, whose tel/mailto values are `<a … min-h-tap-min>` 44px tap rows (`KeyValue.tsx:119-123`). |
| **C. Bare schedule header** | NOT a `SectionCard` — a flush `<div class="mb-2 flex justify-end">` inside a `min-w-0` grid column (`ScheduleSection.tsx:240-253`) | ~16px | `gap-4` 16px to the agenda area above (`ScheduleSection.tsx:145-160`; may contain links) | `mb-2` 8px | **No** — `DayCard` (`components/crew/primitives/DayCard.tsx`) renders no `<a>`/`<button>`/`onClick`; the day list is non-interactive. |

**The key asymmetry:** in contexts A/B the interactive neighbor is BELOW (`gap-3` 12px) and the non-interactive space is ABOVE; in context C the non-interactive neighbor is BELOW (day cards) and the possibly-interactive neighbor (agenda) is ABOVE. The safe direction to grow a hit target is therefore **opposite** between {A,B} and C. Safety is not asserted from a single spacing constant — it is proved against the *tightest* current spacing (§3.1) and enforced behaviorally per direction (§4).
...
- **`"up"` (default — contexts A, B):** overlay bottom edge is flush with the affordance box bottom; it grows 44px upward. `before:bottom-0 before:h-tap-min` (+ `before:top-auto`). **Zero downward overhang** → it can never intersect the interactive rows below (constraint 4 for A/B). *Upward-overhang bound:* the affordance box bottom sits at `p-tile-pad`(20px) + band(≥16px) ≈ ≥36px below the card's top border, so a 44px overlay's top is ≤8px above the card top border (an icon band ≥28px yields **zero** overhang — the 44px fits inside pad+band). The tightest gap above ANY actions-bearing SectionCard is `gap-3` = **12px** (`TodaySection.tsx:362`). **8px < 12px**, so the overlay never even exits the gap into the sibling above — no interactive intersection is geometrically possible, independent of the sibling's content. (Belt-and-suspenders: the §4.1 up-harness stacks an interactive `tel:` row *above* the actions card at `gap-3` and probes it stays hittable.)
- **`"down"` (context C — the bare schedule header):** overlay top edge is flush with the affordance box top; it grows 44px downward. `before:top-0 before:h-tap-min` (+ `before:bottom-auto`). **Zero upward overhang** → it can never reach the (possibly-interactive) agenda area above — proven behaviorally by the §4.1 down-harness, which renders an interactive agenda-style link above and probes it stays hittable. The ~28px downward overhang lands in the `mb-2` 8px gap + the non-interactive `DayCard` list.
...
- **CardReportTrigger** overlay is a fixed 44px wide, horizontally centered on its 14px box: `before:left-1/2 before:-translate-x-1/2 before:w-tap-min`. Symmetric ±(44−14)/2 = ±15px. Its leftward 15px lands inside the 16px `gap-4` (15 < 16 → ≥1px clearance from SourceLink's right box edge, constraint 3). Its rightward 15px lands in non-interactive space in every context: contexts A/B in the SectionCard's trailing `p-tile-pad` (20px, `SectionCard.tsx:39`); context C — the trigger is the right-most, `justify-end` element in a `min-w-0` grid column with NO local right padding, so the 15px lands in the grid `gap-4` (16px, `ScheduleSection.tsx:232`) at ≥720px or the page gutter (`px-4 sm:px-8` = 16/32px, `CREW_PAGE_CONTAINER`, `lib/crew/pageContainer.ts:17`) at <720px. 15 < 16 in both, so it clears the adjacent right column / viewport edge.
...
2. **No overhang on the interactive side (constraint 4 — boundary probe, not center).** Probe just past the affordance's anchored edge on the interactive side and assert it does NOT resolve to the affordance:
   - `"up"` contexts (A/B): `(cx, boxBottom+2)` must NOT `.closest` the affordance, AND the first interactive neighbor below (a seeded `tel:` `KeyValue` row) must be hittable at its own box — probe its TOP edge `(rowCx, rowTop+1)` and assert it resolves to the row's link, proving the overlay does not shave the row's top strip. Additionally, an interactive `tel:` row stacked ABOVE the actions card at `gap-3` (the tightest real spacing) must stay hittable at its BOTTOM edge `(rowCx, rowBottom−1)` — proving the upward overhang never reaches the sibling above.
   - `"down"` context (C): mirror — `(cx, boxTop−2)` must NOT resolve to the affordance, AND an interactive agenda-style `<a>` link seeded ABOVE the bare header must be hittable at its BOTTOM edge `(linkCx, linkBottom−1)`, proving the downward overlay steals nothing from the agenda above.
...
- **`card-actions-up`** — an interactive `tel:` `KeyValueRows` row (`data-testid="dim-tel-above"`) stacked at `flex flex-col gap-3` ABOVE an icon+title `SectionCard` whose `CardHeaderActions` uses the default `"up"` and whose body's FIRST child is an interactive `tel:` `KeyValueRows` row (`data-testid="dim-tel-below"`). This exercises interactive neighbors on BOTH sides at the tightest real spacing, so probe 2 proves zero downward bleed AND zero upward reach.
- **`card-actions-down`** — a replica of the bare schedule header: an interactive agenda-style `<a href>` link (`data-testid="dim-agenda-above"`) then, at `gap-4`, `<div class="mb-2 flex justify-end"><div data-slot="section-card-action">…CardHeaderActions hitDirection="down"…</div></div>` above a non-interactive `DayCard`-style stub. Probe 2 proves the down overlay steals nothing from the agenda link above; probe 3 exercises sibling non-overlap on the down path.
