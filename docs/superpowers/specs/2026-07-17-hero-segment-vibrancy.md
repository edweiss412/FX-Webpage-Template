...
**Surface caveat — the segment can render on `bg-stale-tint`, not only `bg-surface`.** Progress normally renders on `bg-surface` (`#ffffff` light / `#16171c` dark). But during a `morph-to-last-good` transition the hero holds the last-good `show_day_n` body — progress bar included — while `isStale` paints the section `bg-stale-tint` (`#f4ece0` / `#26221b`). `showProgress` requires `!isHeroDegraded(renderState.kind)` (`RightNowHero.tsx:440`); during the stale morph `renderState.kind` is the held `show_day_n`, so `isHeroDegraded(renderState.kind)` is **false** and progress still renders — while `isStale` independently drives `surfaceClass` to `bg-stale-tint` (`:410,:431,:432`). So the edge must clear 3:1 on BOTH surfaces; the tables cover both.
...
Each active segment is a bounded object whose edge clears 3:1 against whichever surface it renders on — so "which segments are filled" clears 3:1 by the edge alone, independent of the fill's own sub-3:1 vs surface (`#ff8c1a` = 2.33:1 vs `#ffffff`, 1.99:1 vs `#f4ece0`; the 2.23:1 quoted in §1/§10 is vs the page `bg` `#fafaf9`). The vibrant `#ff8c1a` fill carries no contrast burden.
...
Every literal in this spec: `#ff8c1a` (fill, both themes), `#7a3d00` (light edge), `#ffa047` (dark edge), `#a65000` (the OLD darkened fill being replaced), `#e5e4e0`/`#2a2b30` (inactive track light/dark), `#ffffff`/`#16171c` (surface light/dark), `#f4ece0`/`#26221b` (stale-tint light/dark), `#cfcdc7` (border-strong, only in §1's mis-cite note), `6px`/`h-1.5`, `1px`/`border`, `gap-1.5`. Contrast literals — DESIGN.md-anchored (`8.06`, `3.61`, `8.16`, `2.23`) cited verbatim from DESIGN.md:35,62; edge-vs-surface derivations `8.42`/`7.18` (light: surface/stale-tint), `8.84`/`7.82` (dark), fill-vs-surface `7.69`/`6.80` (dark), raw-fill sub-3:1 `2.33` (vs `#ffffff` surface) / `1.99` (vs stale-tint) / `2.23` (vs page bg `#fafaf9`) / `1.83` (vs bg-border) / `1.46` (vs border-strong, §1 note), track-vs-surface `1.27`/`1.09` (ratified sub-3:1, unchanged). Every edge/fill boundary value ≥3.0 on every surface it can render on. Occurrence index `1` (active segment), `0` (live-dot). All consistent across §2/§3/§5/§6.
