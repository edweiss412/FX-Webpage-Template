...
1. A producer writing `admin_alerts` through a path that is neither a `upsertAdminAlert` callee nor an `upsert_admin_alert(` SQL call is not discovered.
...
What the guard therefore does and does not promise: a NEW call site fails by default; a new code from an EXISTING dynamic site does not. Gap 2 is bounded by enumerating every dynamic site in the plan (there are few, and each is read in full), and by a lint-level rule that a dynamic site's registry row carries the `file:line` of each branch that supplies its code, so review has a checklist. Closing it fully would need a runtime oracle over `admin_alerts.code`, which is out of scope for this feature and recorded as the honest limit rather than papered over.
