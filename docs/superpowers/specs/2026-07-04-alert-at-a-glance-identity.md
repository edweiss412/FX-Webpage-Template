...
- **`sanitizeIdentityString` is the single chokepoint** for every group-(B) display string; the §8.3 redaction meta-test asserts no display string bypasses it, and that group-(A) resolution IDs never appear in the serialized CLI/`--json` output.
