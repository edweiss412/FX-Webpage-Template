...
- **`sanitizeIdentityString` is the single chokepoint** for every string that leaves `projectIdentityContext`; the §8.3 redaction meta-test asserts no returned string field bypasses it.
...
- **CREATE `tests/observe/_metaAlertsRedactionContract.test.ts`** (resolves Codex F15) — a structural guard pinning the new `queryAlerts` redaction boundary, since selecting `admin_alerts.context` reverses a hard privacy convention and example-level tests won't stop a `AlertRow.context = rawContext` regression. It asserts: (a) `lib/observe/query/alerts.ts` never returns a raw `context`/`identityContext` field that isn't the projection output (the returned shape's keys ⊆ `IDENTITY_CONTEXT_KEYS`); (b) `projectIdentityContext` is the only path from `admin_alerts` `context` to a returned field; (c) email keys appear in the projection output ONLY when `includePii` is set; (d) **in-string PII/secrets scrubbed (Codex F19 + F21 + F22)** — a default `queryAlerts` over a row whose `file_name`/`sheet_name`/`repo`/`changes[].crew_name` contains an email or long token substring returns those fields with the substring redacted; token substrings stay redacted even with `includePii: true` (only email is revealed); redaction runs before length-cap (a boundary-straddling token is fully redacted); every string value is free of Unicode control/format/bidi chars and ≤120 chars. The meta-test iterates the projection's string-field set so a newly added string key that skips `sanitizeIdentityString` fails CI (the structural closure for this vector). Pattern: source-scan `alerts.ts` for a raw `context` passthrough (mirrors the existing `_metaReadOnlyQueryCore` source-scan style) + behavioral assertions with planted arbitrary/PII-bearing context.
