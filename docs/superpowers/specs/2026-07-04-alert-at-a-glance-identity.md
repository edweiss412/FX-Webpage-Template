...
- **Every string field is sanitized, not just gated (resolves Codex F19).** An allowlisted key name is not a safety guarantee — a producer-controlled `file_name`/`sheet_name`/`repo`/`crew_name` can itself contain an email, a token, a customer name, a newline, or a copied provider diagnostic. `projectIdentityContext` therefore passes **every** string value (and every element of `role_change_crew_names`) through a `sanitizeIdentityString`:
  1. **Always** (both surfaces, any `includePii`): coerce to string; strip ASCII control chars incl. newlines/tabs (prevents terminal-injection / log-spoofing in the CLI); collapse runs of whitespace; **length-cap to 120 chars** (truncate + `…`).
  ...
  3. **Always — token redaction is NEVER revealed (resolves Codex F21):** redact long **token-like** substrings (hex/base64 runs ≥ 24 chars) to `[redacted-token]` inside every string field, regardless of `includePii`. Secrets are not part of any opt-in; `--reveal-email` must not expose them.
  4. **Email redaction, gated by `includePii`:** redact **email-like** substrings (`\S+@\S+`) inside `file_name`/`sheet_name`/`repo`/`role_change_crew_names` to `[redacted-email]` when `includePii` is false (CLI default). When `includePii` is true (web, or CLI `--reveal-email`) the embedded email is preserved — matching the dedicated email field's gate, so the flag's consent surface is exactly "email," nothing more.
- **`includePii` gates email and only email:** the dedicated `email`/`user_email` fields (withheld entirely when false) AND in-string email substrings (rule 4). Token redaction (rule 3), control-strip, length-cap, and the action enum are **always on** and unaffected by the flag. Web passes `true`; the CLI defaults `false`, `--reveal-email` sets `true`.
...
- **Wizard race sheet name at raise time (Codex F18 + F20):** producer tests for **all four** `WIZARD_SESSION_SUPERSEDED_RACE` emitters (retry, staged-apply, staged-discard, manifest-ignore) assert the emitted context carries a sanitized `file_name` (via `error.context.driveFileName`); a resolver/describe test with that context and **no `shows` row** still renders `Sheet <file_name> · <attempted_action>` (proving the sheet segment does not depend on a `shows` lookup). A structural check enumerates the emitter set so a future 5th emitter is caught.
