...
2. Issue **at most three batched, bounded `.select().in(...).limit(...)` reads**: `crew_members(id,name)`, `shows(id,title,slug)`, `shows(drive_file_id,title,slug)`. Skip any query whose id-set is empty.
...
|---|------|------------------------------|--------|-----------------|
| 1 | AMBIGUOUS_EMAIL_BINDING | Show · email · "N crew rows" | →show, `ctx:email`, count `ctx:crew_member_ids` | none |
| 2 | **OAUTH_IDENTITY_CLAIMED** | Crew · email · Show | →crew `ctx:crew_member_id`, `ctx:user_email`, →show | **add `user_email` (canonical)** |
| 3 | PICKER_BOOTSTRAP_RPC_FAILED | Show (if present) · reason `rpc_error_code` | →show, `ctx:rpc_error_code` | none |
| 4 | PICKER_BOOTSTRAP_RESOLVE_SHOW_FAILED | reason `rpc_error_code` | `ctx:rpc_error_code` | none |
| 5 | CALLBACK_CLAIM_THREW | error `error_name` | `ctx:error_name` | none |
...
| 11 | WATCH_CHANNEL_ORPHANED | Sheet · error `error_message` (already shown) | →show, `ctx:error_message` | none |
| 12 | WEBHOOK_TOKEN_INVALID | reason `reason` · channel `channel_id` | `ctx:reason`, `ctx:channel_id` | none |
...
| 17 | PARSE_ERROR_LAST_GOOD | *(already SPECIFIC — sheet in copy)* | n/a | none |
| 18 | SHEET_UNAVAILABLE | *(already SPECIFIC — sheet in copy)* | n/a | none |
...
| 22 | SHOW_FIRST_PUBLISHED | *(already SPECIFIC — sheet/crew/date in copy; info-severity)* | n/a | none |
| 23 | SHOW_UNPUBLISHED | *(already SPECIFIC — sheet in copy)* | n/a | none |
...
| 31 | REPORT_ORPHANED_LOST_LEASE | Show · issue `orphan_url` | →show, `ctx:orphan_url` | none |
...
| 35 | REPORT_OPEN_ORPHAN_LABEL | Show · issue `orphan_url` (if present) | →show, `ctx:orphan_url` | none |
| 36 | REPORT_LEASE_THRASHING | Show | →show | none |
| 37 | STALE_ORPHAN_REPORT | Show · report `report_id` | →show, `ctx:report_id` | none |
| 38 | TILE_SERVER_RENDER_FAILED | *(already SPECIFIC — sheet in copy)* · section `tileId` | `ctx:tileId` (add as complement) | none |
| 39 | TILE_PROJECTION_FETCH_FAILED | *(already SPECIFIC — sheet in copy)* | n/a | none |
...
**Producer changes required: exactly one** — `OAUTH_IDENTITY_CLAIMED` gains `user_email`. Every other identifier is either ID-resolvable at render time or already in `context`. This is the payoff of Approach A: the 36-code sweep costs one identity-map file + one resolver + one producer edit, not 36 scattered raise-site edits.

**Codes with copy already interpolating an entity** (17, 18, 22, 23, 38, 39): the identity map still lists them (so completeness holds) but the surface renders the identity line only for segments the copy does NOT already carry (e.g. `TILE_SERVER_RENDER_FAILED` adds the `tileId` section; `PARSE_ERROR_LAST_GOOD` adds nothing → identity line suppressed). See §6.4.
...
- **SELECT change:** add `context` to the read-core `SELECT` (currently omitted, `alerts.ts:8`). Extend `RawAlert` + `AlertRow` (`lib/observe/query/types.ts`) with `context`.
- **PII-scrub-by-default in the read-core:** `queryAlerts` gains an `includePii?: boolean` option (default **false**). When false, after fetching it deletes the known email keys (`PII_EMAIL_KEYS = { user_email, email, matchedEmail }` — the canonical set from the email-canonicalization boundary list) from each row's returned `context`. This keeps `queryAlerts` **redaction-guaranteed by default** even though it now selects `context`: raw email never reaches the CLI's table output OR `--json` unless `includePii` is set. (`admin_alerts.context` has no row-image PII beyond email; other keys are ids/counts/reasons/hashes — safe to surface.)
- **CLI adapter:** `scripts/observe.ts` gains a `--reveal-email` flag → passes `includePii: true` and prints a one-line stderr notice ("showing raw email — PII"). The adapter calls `resolveAlertIdentities` + `describeAlert(identity, { includePii })` to render an identity line per alert (crew/show/sheet/reason always; email only with the flag). Without the flag the email segment is absent because the read-core already scrubbed it.
...
For codes whose `dougFacing` already interpolates an entity (the 6 SPECIFIC codes), the identity line renders **only the segments not already in the copy**:
- `PARSE_ERROR_LAST_GOOD`, `SHEET_UNAVAILABLE`, `SHOW_UNPUBLISHED`, `TILE_PROJECTION_FETCH_FAILED`, `SHOW_FIRST_PUBLISHED` → sheet already in copy, no extra segment → identity line suppressed (`describeAlert` → `null`).
- `TILE_SERVER_RENDER_FAILED` → adds the `tileId` (which page section) segment only.

The map encodes this by simply not listing the already-in-copy segment for those codes (e.g. their entry is `{ kind: "global" }` or a single non-redundant segment). The rule: **the identity map never restates what the copy already says.**
...
1. `lib/observe/query/alerts.ts` — select `context`; add the `includePii` default-false scrub; replace the NOTE comment with the new posture (selects context; PII email scrubbed unless `includePii`; rationale = CLI identity enrichment + opt-in raw email).
2. `AGENTS.md` "Telemetry access → Redaction posture" bullet — amend to: `queryAlerts` selects `admin_alerts.context` but scrubs email keys (`user_email`/`email`/`matchedEmail`) unless `includePii` is set (via the `--reveal-email` CLI flag); `queryChangeLog` image exclusions unchanged. Also add `--reveal-email` to the `alerts` row of the AGENTS.md observe command table.
3. Master spec §3.3/§5 redaction note (`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md`) — add an amendment line recording the carve-out (invariant 7: spec is canonical; changes are recorded, not silent). **Never run prettier on the master spec** (`feedback_never_prettier_the_master_spec`).
4. `tests/observe/queryAlerts.test.ts` — extend: default query scrubs email keys from returned `context`; `includePii: true` preserves them; non-email context keys always present.

**Redaction-guaranteed-by-default preserved:** `queryChangeLog` still excludes `before_image`/`after_image`; `queryAlerts` now returns `context` but with email scrubbed unless explicitly opted in. The default posture is unchanged in effect (no raw PII from the read-core); only an intentional flag widens it.
...
- **Invariant 5 (no raw error codes in UI):** identity segments render **names/values**, never alert codes; `reason`/`rpc_error_code` values are producer-supplied human tokens already used in existing panels — confirm none is a bare MessageCode. (`error_name` is a JS error class name, not a MessageCode.)
...
- **CLI:** `queryAlerts` returns `context` with email scrubbed by default; `scripts/observe.ts` shows `describeAlert(…, { includePii:false })` per row (no email), and `--reveal-email` shows it. A `--json` snapshot asserts no `user_email` key by default, present with the flag.
...
**Edited:** `app/auth/callback/route.ts` (one field), `components/admin/AlertBanner.tsx` (select `shows(slug,title)` + render identity), `components/admin/PerShowAlertSection.tsx` (render identity), `lib/observe/query/alerts.ts` (select context + `includePii` scrub + comment), `lib/observe/query/types.ts` (`AlertRow.context`, `AlertFilters.includePii`), `scripts/observe.ts` (`--reveal-email` flag + identity line), `AGENTS.md` (redaction posture + observe command table), `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` (redaction amendment note), `tests/observe/queryAlerts.test.ts` (PII-scrub tests), possibly `tests/cross-cutting/email-canonicalization.test.ts` (only if the new field needs a fixture — expected: no change, guard already accepts canonicalized).
