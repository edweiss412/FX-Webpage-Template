...
2. `PICKER_BOOTSTRAP_RPC_FAILED` (`app/api/auth/picker-bootstrap/route.ts`) — `emitClaimFailure` writes no show id, even though `targetShowId` is resolved (route.ts:171) and non-null past the `if (!targetShowId) return` guard (route.ts:176) at both call sites (192, 197). Add `show_id: targetShowId` to the **`context`** object (NOT the `showId` column — the row stays null-scoped, §5b) so `→show` resolves via `identityContext.show_id` with no lifecycle change.
