...
Batch idempotency keys derive from membership (§2.2), so two concurrent delivery passes could compute DIFFERENT provider keys for the SAME member (run 1 sends `{A}`, run 2 sends `{A,B}`) — a regression against today's stable per-candidate key. Batched delivery is therefore **single-flight**: `deliverRealtimeCandidates` opens a DEDICATED lock client (`postgres(databaseUrl(), {max:1})`, separate from the work connection) and wraps the entire delivery pass in `lockSql.begin(...)` whose first statement is `select pg_try_advisory_xact_lock(hashtext('notify:realtime-delivery'))`. **Transaction-scoped, not session-scoped, deliberately:** a transaction is pinned to one Postgres backend under EVERY pooling mode (direct, session pooler, Supavisor transaction pooler — the deployed `DATABASE_URL`/`TEST_DATABASE_URL` may be a pooler URL, e.g. the non-loopback pooler `pnpm preflight` warns about), so acquire → held-across-sends → release cannot split across backends the way `pg_advisory_lock`/`pg_advisory_unlock` statements can. The lock releases automatically at commit/rollback/connection-drop; there is no unlock statement to mis-route. Work SQL (eligibility, ledger writes) stays on the EXISTING work connection with today's per-statement autocommit — ledger rows persist immediately, never held hostage by the lock transaction. If the try-lock returns false, the pass returns `{kind:"ok", sent:0, failed:0, skipped:0, retryLater:0, lockSkipped:true}` — no sends, no ledger writes; the next tick delivers.

**Lock-liveness contract (amended in whole-diff review R1):** the lock transaction never sits idle across the whole pass — the pass issues a heartbeat statement (`select 1`) on the lock transaction immediately BEFORE each recipient's active check, BEFORE each candidate's eligibility queries (currentness + ledger reads), AND BEFORE each batch send (per recipient × group) — every work-connection read inside the pass is heartbeat-fenced. Heartbeat semantics: (a) this bounds the lock transaction's idle interval to at most ONE eligibility check (two indexed single-row reads) or one provider send + one batch's ledger writes (single HTTP round-trip + a handful of single-row upserts — seconds), including sends-free passes where every candidate skips, far below any plausible `idle_in_transaction_session_timeout`/pooler idle-recycle threshold (minutes where configured at all); (b) a FAILED heartbeat (thrown/errored — the lock connection died, so the xact lock is already released) ABORTS the pass immediately: no further sends, result `{kind:"infra_error"}`. Members already sent keep their ledger rows (work-connection autocommit, unaffected by the lock client); unsent members retry next tick under whatever pass next wins the lock. The exposure window is therefore a single in-flight batch send, and a duplicate additionally requires a concurrent pass AND a membership change — folded into the residual below. `lockSkipped` is a new OPTIONAL field on the ok arm of `DeliveryResult` (`lib/notify/deliver.ts:22-24`); `runNotify` already forwards the whole result as `detail` (`lib/notify/runNotify.ts:371`), so no caller change.
...
**Residual (documented, accepted):** if the provider accepts a batch but NOT all member `upsertSent` rows persist — process crash, DB timeout/connection loss mid-writes, or a thrown `upsertSent` at member k (members 1..k-1's rows persist via per-statement autocommit; the pass then returns `infra_error` per `deliver.ts:452-453`) — the row-less members re-enter the next tick's batch, and if membership ALSO changed by then, the new batch key differs and the provider cannot dedup them. Today's path has the same post-provider-accept failure window but a stable per-candidate key, so the provider (Resend, 24h idempotency TTL) absorbs the retry. Post-batching, the consequence is a duplicate informational email for those members (undo links remain valid — same token; problem emails are stateless), only when a post-accept persistence failure **and** a membership change coincide. The same residual class covers a lock-connection drop DURING a single in-flight send (the heartbeat contract catches the drop before the NEXT send, so exposure is one batch): a concurrent pass could win the lock mid-send, and a duplicate again requires a membership change on top. Closing either leg fully would need a durable pre-send claim (a `sending` ledger status = CHECK widening + migration + reconciliation changes in `emailDeliveryFailed.ts`) — deliberately out of scope; revisit only if observed.
...
- Lock-liveness (§2.1b): heartbeat fault-injection — lock connection dies after batch 1's send (fake lock sql: first heartbeat ok, second throws) with batches 2..n pending → pass returns `infra_error`, sendEmail called exactly once, batch 1's ledger rows persisted, none for later batches. Failure mode caught: pass sailing on unserialized after silent lock loss (the R3 mid-pass release scenario). Plus heartbeat-cadence assertion: fake lock sql records one heartbeat per attempted batch send.
