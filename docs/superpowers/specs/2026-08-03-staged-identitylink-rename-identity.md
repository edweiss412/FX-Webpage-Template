...
`phase2.ts` and `applyParseResult.ts` are code-untouched. The behavior flip makes a set of shipped comments stale; this is the COMPLETE reconciliation inventory (reviews R2/R5 — the sweep is grep-driven: `rg -n "delete\+insert|remove\+add|version-bound" lib/sync/ tests/`, all staged-rename-relevant hits enumerated):
...
8. `tests/sync/selectionsResetAtPreserved.test.ts` header comment ("A NAME change is delete+insert and loses the marker") — reword: an UNLINKED name change is delete+insert and loses the marker; an identity-link rename (cron, or staged rename-choice per this spec) updates in place and PRESERVES `selections_reset_at`. Found by review R5's tests-wide sweep — the reconciliation grep scope is `rg -n "delete\+insert|remove\+add|version-bound" lib/sync/ tests/`, not lib/sync/ alone.
...
2. Code-comment reconciliation per the COMPLETE §3.3 inventory (8 sites: applyStagedCore step-3, phase2 arm-(c), Phase2Args field doc, applyParseResult arg doc + loop comment, identityLinkRenames header vouch doc, phase2.test.ts arm-(c) test comment, selectionsResetAtPreserved.test.ts header comment).
...
- **TDD (invariant 1):** every CODE task in the plan is failing-test-first; the plan's docs-only and verification tasks declare their exemption explicitly (ratified review R4).
