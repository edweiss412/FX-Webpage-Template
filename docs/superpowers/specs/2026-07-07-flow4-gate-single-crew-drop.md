...
| **accept path bypasses the hold** | `published=true`, single drop, `acceptShrink=true`, `expectedModifiedTime === binding.modifiedTime` | `runPhase1` falls through (outcome `pass`/`auto_apply_with_holds`, not `shrink_held`; no `updateShowShrinkHeld` write) | accept path broken by the new branch; confirm the reused fall-through is intact. **Layer note:** the `show_change_log` orphan-remove row is written by the phase2/scheduled-sync apply path (`lib/sync/runScheduledCronSync.ts:3313+`), NOT by `runPhase1` — that path is untouched by this change and already covered by existing phase2/producer tests, so a `runPhase1` unit test cannot (and need not) assert the feed row. |
