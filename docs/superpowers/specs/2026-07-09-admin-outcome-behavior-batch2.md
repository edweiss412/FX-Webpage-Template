...
**The atomic helper is the SOLE recording path (Codex plan-R2/R3).** Every Batch-2 row calls **only** `proveAdminOutcomeBehavior({ file, fn: "POST", code, success, failure })`. The helper — and ONLY the helper — calls `observeSuccessCodes`, `observeCodes`, and `recordAdminOutcomeBehavior`; it (1) runs `success`, asserts the `code` observed, (2) runs `failure`, asserts the `code` ABSENT, (3) then records. A row therefore cannot graduate with a success-only proof. **No Batch-2 test body calls `observeSuccessCodes`, `observeCodes`, or `recordAdminOutcomeBehavior` directly** — the `success`/`failure` callbacks only build `routeDeps`/`context`/`request` and invoke the handler (plus route-specific setup like the #18 body drain).
...
**Structural closure of the DB-free vector (accurate mechanism).** §3.5 (the exhaustive `deps.*` sweep) plus **copying each cited driver's deps-builder verbatim** is the PRIMARY defense — the drivers already inject the full working set. The runtime backstop is the **CI unit-suite environment**, and its mechanism must be stated precisely (the defaults do NOT throw on an unset connection): each `defaultWith{Tx,RowTx,RowTryLock}` calls `postgres(databaseUrl())`, and `databaseUrl()` falls back to the LOCAL Supabase DSN `postgresql://postgres:postgres@127.0.0.1:54322/postgres` when `TEST_DATABASE_URL`/`DATABASE_URL` are unset; Drive defaults (`defaultVerifyFolder`/`defaultFetchDriveFileMetadata`) require Drive credentials. In **CI unit-suite** (`VITEST_EXCLUDE_ENV_BOUND=1`, no local Postgres listening on 54322, no Drive creds) any un-injected DB/Drive seam gets **connection-refused / credential-missing → the drive throws → `observeSuccessCodes` rethrows → the helper never records → Task 18 stays RED.** So **false DB-free coverage cannot survive CI** (the ship gate): green-in-CI ⟺ every seam on the driven path was injected. **Local caveat (do NOT rely on local green alone):** locally a missed seam falls back to the running local Supabase at 54322 and could silently hit it (passing for the wrong reason, or causing cross-worktree contention). Therefore the local verification MUST treat CI unit-suite green as authoritative, and the implementer copies the driver deps verbatim rather than trusting a local pass. (Codex plan-R2→R5 iterated this vector; the closure is §3.5 completeness + the CI no-DB/no-Drive environment, not a per-route inspection ritual.)
