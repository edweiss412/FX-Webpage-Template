...
- **Failure (emit absent) — pick ONE, both explicitly DB-free:**
  1. **Pre-commit refusal (preferred):** inject `withRowTx = async () => <non-committing Response>` that resolves WITHOUT invoking the callback, so `committedAction` is never set → no emit, and the `WizardSessionSupersededRollbackError` catch (and its alert deps) is never reached. Fully DB-free with no alert-dep injection needed.
  2. **Real rollback branch:** inject `withRowTx` whose callback drives a 0-row outcome that throws `WizardSessionSupersededRollbackError` **and** inject `upsertAdminAlert: async () => null` + `readCurrentWizardSessionId: async () => null` (matching the existing telemetry driver) so the catch stays DB-free while exercising the real no-emit rollback path.

Do NOT use a bare `withRowTx` no-op that leaves `upsertAdminAlert`/`readCurrentWizardSessionId` at their Supabase defaults while hitting the rollback catch — that either touches real Supabase or gives a shallow, non-proving absence.
