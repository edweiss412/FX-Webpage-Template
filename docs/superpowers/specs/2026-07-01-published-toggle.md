...
**Version-token durability (R6):** `viewer_version_token` (`supabase/migrations/20260523000006_viewer_version_token_rewrite.sql:5-23`) is built from `last_synced_at` / crew `last_changed_at` / `picker_epoch(_bumped_at)` — a pure `published` flip changes NONE of them (D1 forbids the `picker_epoch` bump that makes archive visible to catch-up). A crew session that misses the realtime broadcast (SSR-to-hydrate gap, offline, reconnect) would compare an UNCHANGED token and skip `router.refresh()`, keeping stale show data on screen after unpublish. Fix in the SAME migration: redefine `viewer_version_token` to append a publication component — `|| ':' || coalesce((select published::text from public.shows where id = p_show_id), 'false')`. Every consumer compares tokens for EQUALITY only (`ShowRealtimeBridge.tsx:648` "synchronous refresh on version mismatch"; the SSR fence in `lib/data/getShowForViewer.ts:750-766`; `app/api/show/[slug]/version/route.ts`), so appending a component is shape-safe, and any publish/unpublish flip now produces a mismatch → durable catch-up even without the broadcast.
...
- Freshness both directions: unpublish → `publish_show_invalidation` broadcasts (fast path) AND the `published` component added to `viewer_version_token` (§3.1, R6) guarantees the missed-broadcast catch-up sees a mismatch (durable path) → live crew sessions refresh into the unavailable page; republish → the bridge is NOT mounted on the unavailable page (it renders outside `_CrewShell`), so recovery is a manual reload — acceptable (D5's page says "check back soon"). No polling added (YAGNI).
...
2. **Version token (R6)**: DB test — `viewer_version_token(show)` changes across `unpublish_show` and across `publish_show` (token inequality asserted, not specific format); bridge-level test — the existing catch-up/mismatch harness path proves a version mismatch triggers `router.refresh()` with a flipped-published token from the version route (missed-broadcast simulation: no broadcast delivered, reconnect catch-up only).
