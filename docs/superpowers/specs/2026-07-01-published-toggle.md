...
- **Deleted:** `_actions/publish.ts` (`publishShowAction`), `_actions/undoAutoPublish.ts` (`undoAutoPublishAction` + its barrel exports). `readUnpublishTokenForSlug` (`lib/sync/unpublishShow.ts:341-352`) loses its only caller and is deleted too.
...
5. **Action**: `setShowPublishedAction` — requireAdmin ordering, infra_error vs SHOW_NOT_FOUND resolution, direction dispatch, post-commit revalidation calls (mirror `tests/app/admin/` publish/undo action tests; delete the undo ones).
