...
A non-archived show is **drift-eligible** when either holds:
...
|---|---|---|---|
| CREATE mapping | Warning persisted (token was unmapped) | (b) | Grants applied, warning consumed, stamp gains entry |
...
- `parse_warnings` **null/malformed/entries without `roleToken`**: those entries simply never match direction (b); no eligibility.
...
- No change to watermark semantics (`shows.last_seen_modified_time` meaning, advancement, or any AC-X.4 symbol).
