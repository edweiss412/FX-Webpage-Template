...
| Locked apply (Phase 2) | `driftResync` marker → `less_than_or_equal` stale guard for drift-rescued cron runs only (`phase2.ts:217-219`); in-lock live-`pending_syncs` re-verification → benign skip |
