...
- **Carrier — a route-scope accumulator closed over by the row loop.** `processApprovedRow` returns `PerRowResult` (`app/api/admin/onboarding/finalize/route.ts:1339`), which is serialized verbatim into the public `per_row` response (`app/api/admin/onboarding/finalize/route.ts:757-763`), so the notice must not ride it. `core.roleFlagsNotice` exists only inside `processApprovedRow` (`app/api/admin/onboarding/finalize/route.ts:1266`), and its caller receives only `PerRowResult` — so there must be an explicit owner outside both. That owner is an array declared in the route scope, **pushed to inside `processApprovedRow` via closure**, exactly the mechanism the existing `affectedShowIds` accumulator already uses on this route. Naming the mechanism matters: "a local handed to the emitter" (an earlier revision) is not implementable across that function boundary.
...
**finalize-cas takes the same treatment for the same reason.** Its per-row loop is likewise inside `deps.withTx` → `runFinalizeCas` holding `tryFinalizeLock` (`app/api/admin/onboarding/finalize-cas/route.ts:905`) and a `FOR UPDATE` session row (`app/api/admin/onboarding/finalize-cas/route.ts:911`), so the post-`withRowTx` point named earlier in this section is inside that lock too. Both routes accumulate and flush in a `finally` after their outer transaction. This applies to Unit B's `unlandedRenames` emit on these routes as well — same accumulator, same flush.
