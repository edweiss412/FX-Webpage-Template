...
- **`SHOW_FINALIZED` MUST stay where it is.** Its suppression on outer-commit failure is deliberate and pinned: `tests/onboarding/finalize.test.ts:864` asserts it is NOT emitted when the outer commit fails. Moving it into the `finally` would invert exactly that contract. Its semantics differ from a capability notice — it reports the *session* finalizing, which genuinely did not happen if the outer transaction rolled back, whereas a capability change committed in its own row transaction did happen regardless.
...
| A | `renamedAway` survival split: an unlanded pair whose `removedName` IS in `deleteKeepNames` produces **no** loss notice; one whose `removedName` is NOT produces one (capability-flagged prior) | The §4 item 10 / item 11 pair. A single-set implementation cannot satisfy both, so this test is what forces the split. |
...
| A | `field_changed` attribution follows landed pairs: an unlanded pair emits no `field_changed` row mapped through the prior name | §4 item 3. The rename/add/remove assertions all pass while this row is still mis-attributed. |
