...
- **Payload:** `{ showId, driveFileId, removedName, addedName, reason, sourceSurvived }` — `showId`/`driveFileId` as top-level event fields, the other four as the `context` object. **`sourceSurvived` is a deliberate amendment to this line, not drift.** It did not exist when this payload was first written; it was added to `UnlandedRename` at spec review round 9 (`lib/sync/applyParseResult.ts:127-137`) and is carried into the event because it is the ONLY field that distinguishes a held pair whose source row SURVIVED from one whose row was deleted — precisely the distinction §2.1 A3 took four rounds to get right, and one that `reason` alone cannot recover (`name_held` covers both cases). Without it a forensic reader cannot tell whether the crew member is still on the roster. Redaction-safe: crew names, a closed-union reason token, and a boolean — no email/phone/token. `persistAppEventStrict` also runs `sanitizeContext`. The `context` key set is **exact and closed**: `tests/log/emitIdentityLinkRenameUnlanded.test.ts` asserts the literal key set rather than `expect.objectContaining`, so any added, removed, or renamed key fails there instead of drifting past this line unnoticed.
...
**The two PRE-EXISTING emits at that position do NOT move — and that is a decision, not an oversight.** `logAdminOutcome({ code: "SHOW_FINALIZED", … })` (`app/api/admin/onboarding/finalize-cas/route.ts:982-989`) and `ONBOARDING_SHADOW_REBUILD_EXHAUSTED` (`app/api/admin/onboarding/finalize-cas/route.ts:1025-1038`) both sit inside `runFinalizeCas`, i.e. inside the outer `tryFinalizeLock`. Having just declared that position invalid for the new emits, this spec has to say what happens to them, or an implementer will reasonably relocate them and break things:

- **`SHOW_FINALIZED` MUST stay where it is** — and the two finalize routes deliberately DIFFER on it, so neither can be reasoned from the other. Ordinary finalize suppresses it on outer-commit failure (`tests/onboarding/finalize.test.ts:864` asserts `logAdminOutcome` is not called). finalize-cas does the opposite and **preserves** it for a row that committed before a later throw (`tests/onboarding/finalize-cas.test.ts:685-686`). Both behaviors are pinned by existing tests, so relocating either emit breaks one of them. This branch touches neither.
- **`ONBOARDING_SHADOW_REBUILD_EXHAUSTED` is the genuinely arguable one** — it describes an independently committed row mutation, so it has the same durability exposure the new emits do. It is **out of scope here** and filed as `BL-SHADOW-REBUILD-EXHAUSTED-EMIT-PLACEMENT` under class-sweep exception (a): whether that event should survive an outer rollback is a product question about what the operator should be told, not a mechanical relocation, and this branch has no basis to settle it.

So: **three** accumulator-and-flush sites, not two — ordinary finalize, non-streaming finalize-cas, and streaming finalize-cas — each flushing in the `finally` of its own outer transaction, and each carrying ONLY this branch's new emits. §7's "both finalize routes" rows mean all three handlers; the durability and lock-ordering tests run against the **streaming** finalize-cas handler as well, since it is the one in production use.
...
2. **The dashboard roster-shift badge changes with them.** `roster_shift_counts` tallies `crew_added` / `crew_removed` / `crew_renamed` straight off the change log (`supabase/migrations/20260706130000_show_change_log_acknowledged.sql:32-43`), and those three counts are rendered into the badge's operator-facing label ("Roster changed since last review: N added, N removed, N renamed"). So every row in the item-1 table also moves a badge count and its wording — a false `renamed` becomes a truthful `removed` or `added`. **No UI file is in this diff**: the badge's numbers change because its data source is corrected, not because the component changed, so R4's no-UI-surface finding (and the invariant-8 exemption resting on it) still holds. §7 pins the counts at the data layer.

3. **The monitor digest's wording changes with them.** Its "Applied automatically" section selects summaries for the same change kinds (`lib/notify/monitorDigest.ts:23`), so a digest that would have read "Crew member Old renamed to New" now reads the corrected added/removed summary. No implementation work — the digest reads the corrected rows — but it is operator-visible text and belongs in this list.

4. **`field_changed` rows change too.** `renames` has a third use inside the writer beyond the two loops above — it maps prior names while deriving structured field changes (`lib/sync/changeLog/writeAutoApplyChanges.ts:145-170`). A requested-but-unlanded pair currently emits a `field_changed` row attributing the successor's field deltas to the prior member (measured: `field_changed: Role — New, A1 → V1` alongside the `crew_renamed` row). Sourcing from landed pairs drops that attribution with the rename. §7 pins this mapping explicitly.
5. **An unaccepted cron MI-13 or MI-14 stops producing a `crew_renamed` row** (replaced per the item 1 table). `computeIdentityLinkRenames` gates those two invariants on `acceptedThisVersion` (`lib/sync/identityLinkRenames.ts:20-23`); `renamePairs` had no gate at all. This is the R7 fix. MI-12 is ungated in both, so it is unaffected.
6. **A staged `independent` decision is NOT affected** — correcting an earlier revision of this list, which claimed it as a change. `independent` is already excluded upstream of the feed (`lib/sync/applyStagedCore.ts:227-231` returns `false` for it, and `choiceAwareFeedItems` yields `[]`), so it contributes no rename input today and none after. Listed explicitly because the neighbouring cron cases DO change and the symmetry is misleading.
7. **First-seen shows are unaffected.** `runManualStageForFirstSeen.ts:125` passes `[]`, and a first-seen show has no prior roster, so it produced no legitimate rename rows before and produces none now.
8. **A suppressed rename no longer appears in the capability notice**, so a `ROLE_FLAGS_NOTICE` that would have been raised solely by a phantom rename is not raised at all.
9. **Undo restores `selections_reset_at`.** A picker cookie invalidated before the undone change stays invalidated afterward.
10. **Three more paths now raise the bell alert and the durable event on a LEAD/FINANCIALS change**, matching the dashboard and cron paths: the Phase D wizard apply (finalize-cas), the ordinary onboarding finalize, and the pending-ingestion retry. All three are operator-visible additions — an alert appears where none did before. This is the whole of Unit C's user-facing effect.
11. **A capability loss that is silently suppressed today now reports.** When a rename's target is hold-suppressed (P2-F4) or the update no-ops, the source row is not delete-protected, so `deleteCrewMembersNotIn` removes it and a LEAD/FINANCIALS capability genuinely disappears. Today `renamedAway` holds the requested `removedName` and suppresses arm (c), so no loss notice fires. Under §2.1 A3 it fires. This is a **new** `ROLE_FLAGS_NOTICE` in a case that previously produced none — an addition to the operator's bell, not a removal, and the opposite direction from items 1–5.
12. **Pairs whose source row SURVIVED continue to produce no loss notice.** Membership in `deleteKeepNames` (`lib/sync/applyParseResult.ts:152`) is the test, so §2.1 A3 keeps them in `renamedAway`. Called out because a naive reading of item 11 would predict otherwise. Note this is narrower than "held": a `name_held` pair whose hold kind did not delete-protect it falls under item 11, not here.
...
| A | `renamedAway` survival split: an unlanded pair whose `removedName` IS in `deleteKeepNames` produces **no** loss notice; one whose `removedName` is NOT produces one (capability-flagged prior) | The §4 item 11 / item 12 pair. A single-set implementation cannot satisfy both, so this test is what forces the split. |
...
| A | `field_changed` attribution follows landed pairs: an unlanded pair emits no `field_changed` row mapped through the prior name | §4 item 4. The rename/add/remove assertions all pass while this row is still mis-attributed. |
| A | **db test:** for an unlanded pair, `roster_shift_counts` reports the `removed`/`added` tally and **zero** `renamed` | §4 item 2. The change-log assertions check row existence; this checks the aggregate the operator actually reads. A writer that emitted both a rename AND a removal would pass the former and fail this. |
...
- **A REPLACED-shape `crew_renamed` row can no longer be produced by any live apply.** Discovered during implementation, and a direct consequence of Unit A rather than a new decision. After Unit A the only source of a `crew_renamed` row is a **landed** pair, and landing means `renameCrewMember`'s in-place `update … set name = $3` succeeded — which preserves `crew_members.id` by construction. So the "successor carries a distinct id" shape, which the old `triggeredItems` re-derivation produced whenever a rename degraded to delete+insert, is now unreachable from production. That is the intended correction: a delete+insert IS a removal plus an addition, and §4 item 1 already says so.

  **The consequence for `undo_change`:** its Direction A REPLACED-shape branch is not dead code — it must keep working for rows already in the change log — but it is now **historical-only**, in the same sense as the two `before_image` limits below. Regression coverage for that branch therefore seeds its feed row directly rather than driving a live apply, because no live apply can create one. Pinned by `tests/db/undo-change-direction-a.test.ts` ("REPLACED-shape rename undo regression, spec test 16").
