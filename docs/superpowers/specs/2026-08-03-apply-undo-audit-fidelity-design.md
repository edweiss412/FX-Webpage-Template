...
- **Carrier — out-of-band, NOT the per-row result.** `processApprovedRow` returns `PerRowResult` (`app/api/admin/onboarding/finalize/route.ts:1339`), which is pushed verbatim into `perRow` (`app/api/admin/onboarding/finalize/route.ts:1545`) and serialized straight into the public `per_row` response (`app/api/admin/onboarding/finalize/route.ts:757-763`). Hanging the notice off that type would leak crew names and capability flags. The notice therefore rides a collection alongside the loop — the same shape as the existing `affectedShowIds` accumulator — and never touches `PerRowResult`.
- **Trigger — after the OUTER `withTx`, not per row.** Each row commits in its own `runtime.withRowTx` (`app/api/admin/onboarding/finalize/route.ts:1507`), but the route's post-commit region begins only once `runtime.withTx` resolves (`app/api/admin/onboarding/finalize/route.ts:1592`). Emitting per row would fire inside the outer transaction and violate invariant 10. All collected notices are emitted in that post-commit region, in row order.
