...
- **Carrier — out-of-band, NOT the per-row result.** `processApprovedRow` returns `PerRowResult` (`app/api/admin/onboarding/finalize/route.ts:1339`), which is pushed verbatim into `perRow` (`app/api/admin/onboarding/finalize/route.ts:1545`) and serialized straight into the public `per_row` response (`app/api/admin/onboarding/finalize/route.ts:757-763`). Hanging the notice off that type would leak crew names and capability flags. The notice therefore rides out-of-band and never touches `PerRowResult`. With the per-row trigger below, the carrier is a local handed straight to the emitter — no cross-row accumulator is needed.
- **Trigger — immediately after each row's own `withRowTx` resolves. NOT after the outer `withTx`.** An earlier revision of this spec said the opposite, on the assumption that a per-row emit would fire inside the outer transaction. It would not: `defaultWithRowTx` opens its **own** postgres connection and its own `sql.begin` (`app/api/admin/onboarding/finalize/route.ts:199-205`), so each row's mutation commits independently of the outer `withTx`. Deferring the flush to `app/api/admin/onboarding/finalize/route.ts:1592` would mean a later row's failure, or an outer-commit failure, jumps to the catch and **permanently skips the notice for a capability change that already committed** — the durability inversion invariant 10 exists to prevent. The existing regression at `tests/onboarding/finalize.test.ts:864` already pins that post-wrapper emissions are unreachable on that path. Emitting per row, after that row's own commit, is both post-commit and outside every lock.
- **Failure policy — fail-open, same as finalize-cas.** `upsertAdminAlert` throws, and a throw escaping the post-commit region would abort work already durably committed. The emit is wrapped and escalates on failure rather than propagating, matching §2.3's finalize-cas treatment and the route's own established posture.
