...
| 5 | `performed` maps fail-closed: `data === true`. During any transitional window where a void-returning RPC is still deployed, one emission is SUPPRESSED, never duplicated | Design S2 approval |
...
- `lib/showLifecycle/_shared.ts`: `LifecycleResult` ok arm becomes `{ ok: true; performed: boolean }` (required field — the compiler forces every producer through the decision; `exactOptionalPropertyTypes` posture). `callLifecycleRpc` continues returning `{ result, data }`; each caller derives `performed: data === true` (the existing `unarchiveShow` R8 idiom generalized). Refusal arm unchanged.
...
5. **Type-level:** `pnpm typecheck` sweeps every `LifecycleResult` consumer (required-field forcing function).
...
- Transitional emission suppression (§1.1 row 5): between migration apply and app deploy, a performed archive whose RPC still returns void emits nothing. Bounded by same-PR deploy ordering; forensic gap of minutes, fail-closed by choice.
...
Probe file (a Playwright spec named `probe-admin-lifecycle-transitions`, deliberately untracked, retained in the worktree until merge): 5 cases — A forced-hold via `page.route` POST interception (+3s), A2 tap-attempt, B ×8 natural, C stale-tab via subscriber-token abort, D cross-tab armed. Full timelines in §2. Recorder: MutationObserver over `[archive-show-confirm-button, archive-show-button, unarchive-show-button-*, share-hub-popover]` presence/disabled/aria-busy, consecutive-dedup, `performance.now()` stamps.
