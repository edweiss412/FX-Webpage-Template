...
Running the parser on the committed fixture of that show (`fixtures/shows/raw/2025-06-ria-investment-forum.md`, `parseSheet` from `lib/parser/index.ts`) reproduces all four rows:
...
- **`anchorNamespace` stays the single kind definition.** It is exported from `lib/parser/fieldNearMiss.ts` and imported by the scanner. Spec §2.2's amendment of 2026-08-15 ("the two DETAILS families are deliberately NOT the same set") is RETIRED by construction, not contradicted: after this arc there is one family because there is one function. The `fieldNearMissBaseline` AC-N9 rows stay anchored (§5, AC-3).
...
For every `grid` block whose `sheetName` is in `SOURCE_LINK_ALLOWLIST` and has a gid in `titleToGid`: feed the block's rows to the shared opener/alignment core (§2.3), take `kind = anchorNamespace(opener)`, and for every non-alignment row with a non-blank `cells[0]` and a non-null `absRow`, push `{ kind, label: normalizeCellKey(cells[0]), value: normalizeCellKey(cells[1] ?? ""), anchor: { title: sheetName, gid, a1: encode_cell({ r: absRow, c: absCol0 }) } }`. `BLOCKS`, `TERMINATORS`, `DETAILS_NON_TERMINATOR_FIELDS`, `firstNonBlank`, `nextNonBlankAfter` are deleted; nothing else imports them (verify with `rg` at plan time).
...
This is the equivalence the whole design rests on, and it is pinned executably rather than trusted (§5, T1): for every `fixtures/shows/exporter-xlsx/*.xlsx`, the `(opener, cells)` sequence from the coordinate path over all grid blocks, in order, equals the `(opener, cells)` sequence `scanRowsWithOpener` yields over `synthesizeMarkdownFromXlsx(buffer).markdown`. Both sequences come from the same buffer with default options, so `OLD` tabs are absent from both.

### 2.4 Tab-level fallback

When the join finds zero or several cells, the warning still knows its block kind, and the scanner knows which tab every block of that kind lives on. `resolveUnknownFieldCell` gains a second return path: if no unique cell matched and every anchor with this `kind` shares one `(title, gid)`, return `{ title, gid }` with NO `a1`. `buildSheetDeepLink` renders that as `#gid=<gid>` (the `if (anchor.a1)` branch is skipped), so the link opens the right tab. If the kind spans several tabs, or matches no anchor at all, return `null` as today.
...
- `correctionLoopCopy("resync")` on the note card and the published card is gated on `sourceCell` non-null; a tab-level anchor now shows the "Fixed it in the sheet? ... re-scan" sentence, which is true at tab grain too.
- `lib/reports/submit.ts` `sourceCellFrom` forwards whatever is there; `a1` optional in `SourceAnchor` already (`buildSheetDeepLink.ts`, `a1?: string`).

### 2.5 Guards on the input

- A block on a tab outside `SOURCE_LINK_ALLOWLIST` produces no anchors. This is load-bearing rather than tidy: `buildSheetDeepLink` maps a non-allowlisted title to `#gid=0`, so an anchor there would render an `INFO!A1` link that claims to be a row link.
...
`tests/parser/waveCodesNoSourceCell.test.ts` asserts the ABSENCE of a per-code arm for the three wave codes only and is unaffected; the new arm is code-gated to the hotel set. The hotel spec rows cc and w are amended in place with a dated note pointing here (they are records; the note says what changed and where the decision lives).
...
| `controls` null | guidance alone, note never appended |
...
| --- | --- |
| `lib/messages/catalog.ts` three `helpfulContext` values + three new `controlsNote` values | `tests/cross-cutting/codes.test.ts` (x1 parity, `helpfulContext` vs `SPEC_CODES`) |
...
- **T1, equivalence of the two paths (a NEW suite under `tests/drive/`, named synthesizeBlocksEquivalence).** For each of the seven `fixtures/shows/exporter-xlsx/*.xlsx`: `(opener, cells)` from `scanBlockCells` over every `grid` block of `synthesizeBlocksFromXlsx(buffer)` equals `(opener, cells)` from `scanRowsWithOpener(synthesizeMarkdownFromXlsx(buffer).markdown)`. Premise stated executably: each fixture yields more than one block and at least one block with more than two rows. Red on the live tree: `synthesizeBlocksFromXlsx` does not exist. Catches: a coordinate path that trims, skips, or escapes differently from the markdown path.
...
- **T3, the four fixture rows resolve (new cases in `tests/drive/unknownFieldAnchors.test.ts`).** Build a workbook fixture containing (i) a `Timestamp`-opened block on `INFO` with `Room Diagram` and `Backdrop` rows, (ii) a `Console`-opened block on `GEAR` with a `Speaker | QSC KLA` row, (iii) a `VENUE NAME` block and a `DETAILS/Room Diagram` block (the AC-N9 shapes). Run `synthesizeMarkdownFromXlsx` → `parseSheet` → `attachWarningAnchors` with a `titleToGid` for both tabs, and assert through `resolveUnknownFieldCell` (never by reading `kind`) that each `UNKNOWN_FIELD` warning resolves to the row's own label cell on the right tab. Red on the live tree: kinds `timestamp` and `console` match no `BLOCKS` entry. Catches: a scanner that anchors only its old two families.
- **T4, tab-level fallback and its refusals.** Same suite: a kind whose `(label, value)` appears twice on one tab resolves to `{ title, gid }` with no `a1`; a kind present on two tabs resolves `null`; a block on a non-allowlisted tab resolves `null`; a row with `absRow: null` (pull-sheet synthesized title) yields no anchor. Red: `resolveUnknownFieldCell` returns only cell-or-null today.
...
- **T6, copy placement.** (a) `tests/messages/_metaWarningCardCopy.test.ts`: the banned-vocabulary sweep covers `controlsNote`; every code with a `controlsNote` is in `WARNING_CARD_COPY_CODES`; no `helpfulContext` in the catalog contains `Report` or `Ignore` as a control name (`/\b(Report|Ignore)\b/`), asserted over the WHOLE catalog so the class cannot regrow (derived cover, not a three-code list). (b) `tests/admin/perShowActionableWarnings*.test.ts`: with `renderItemControls` returning a node, the guidance element's text ends with the `controlsNote`; with `renderItemControls` absent, the guidance element's text does not contain it; clone-and-strip the controls node before reading text (anti-tautology: the Report button's own label must not satisfy the assertion). (c) `tests/components/step3SheetCard.test.tsx`: the wizard row for an `UNKNOWN_FIELD` warning contains neither "Report" nor "Ignore" in its text. Red for (c) on the live tree: the row renders the old `helpfulContext`.
- **T7, wizard row link on a non-family kind.** `tests/components/step3SheetCard.test.tsx`: an `UNKNOWN_FIELD` warning with `sourceCell: { title: "GEAR", gid: 7, a1: "A12" }` renders `wizard-step3-card-<dfid>-warning-<i>-open` with `href` ending `#gid=7&range=A12`; one with `sourceCell: { title: "INFO", gid: 0 }` (tab-level) renders it ending `#gid=0` and nothing after. Red for the second case: no production change is needed for the first (it is a render of existing props), so the second is the RED, and it is red because `buildSheetDeepLink` already handles it; state that plainly in the task rather than inventing a red. The GREEN value of this test is regression cover for the wizard's link site.
...
- **Rows on tabs outside `SOURCE_LINK_ALLOWLIST`** stay link-less by design; widening the allowlist is a separate decision with its own wrong-tab hazards.
...
None. No component gains a visual state; the note is present or absent at render time and never animates.

## 12. Acceptance criteria

- **AC-1** For the RIA fixture synthesized from its xlsx (`fixtures/shows/exporter-xlsx/` has no RIA workbook; T3's constructed workbook stands in, and the plan's closeout runs the live `II - RIA Investment Forum` sheet through the onboarding scan on the validation project once the DB slot is granted), every `UNKNOWN_FIELD` warning whose `(kind, label, value)` is unique on its tab resolves to its own label cell, and the wizard row renders `Open in Sheet` with `#gid=<tab>&range=<cell>`.
...
- **AC-3** The four `fieldNearMissBaseline` AC-N9 rows still resolve non-null through `resolveUnknownFieldCell` (that suite is unedited and green).
- **AC-4** A non-unique `(kind, label, value)` on a single tab resolves to a tab-level anchor rendering `#gid=<tab>`; on two tabs, `null`; on a non-allowlisted tab, `null`.
