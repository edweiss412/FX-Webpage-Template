...
For every `grid` block whose `sheetName` is in `SOURCE_LINK_ALLOWLIST` and has a gid in `titleToGid`: feed `blockRowsForScan(block)` to the shared opener/alignment core (§2.3), take `kind = anchorNamespace(opener)`, and for every non-alignment row with a non-blank `cells[0]` and a non-null `absRow`, push `{ kind, label: normalizeCellKey(cells[0]), value: normalizeCellKey(cells[1] ?? ""), anchor: { title: sheetName, gid, a1: encode_cell({ r: absRow, c: absCol0 }) } }`. `BLOCKS`, `TERMINATORS`, `DETAILS_NON_TERMINATOR_FIELDS`, `firstNonBlank`, `nextNonBlankAfter` are deleted; nothing else imports them (verify with `rg` at plan time).
...
Every pair among G0..G4 is **instant, no animation**: the element is a text node inside the card's existing flex column, there is no `AnimatePresence`, no motion wrapper, and no height transition on it; a state change is a synchronous text swap on rerender. G1..G3 ↔ C is likewise instant (the string is routed to a different slot; the popover's own open/close behaviour is unchanged and outside this arc). Compound: `showControlsNote` or `renderItemControls` toggling while the condensed popover is open re-renders the popover body synchronously; toggling while guidance is `instance` is a no-op by the G4 rule. Pinned in `tests/admin/perShowActionableTransitions.test.tsx` (the existing "every pair instant, both directions" suite): a new case rerenders through G1 → G2 → G3 → G1 by toggling `showControlsNote` and the catalog entry's guidance, asserting the text swaps on the same tick and that no element with a motion/`data-motion` attribute or an `exit`-bearing wrapper appears around the guidance span.

## 12. Acceptance criteria

- **AC-1** For the dispatching show's committed workbook `fixtures/shows/exporter-xlsx/ria.xlsx` (the RIA Investment Forum row of `fixtures/shows/exporter-xlsx/README.md`), synthesized, parsed, and anchored through `attachWarningAnchors` (T3 corpus case): the three `UNKNOWN_FIELD` warnings resolve to their own label cells (`Room Diagram` and `Backdrop` on `INFO`, `Speaker` on `GEAR`), the hotel warning resolves to the `hotels` region, and the wizard row renders `Open in Sheet` with `#gid=<tab>&range=<cell>`. The plan's closeout repeats the check on the live sheet once the DB slot is granted.
