...
- Worker count (Codex spec-R1 #3, verified in the installed vitest 4.1.5): default pool is **forks** (child processes — `resolved.pool ??= "forks"`), and run-mode default `maxWorkers` is **`availableParallelism() − 1`** (default computed at `node_modules/vitest/dist/chunks/cli-api.Cjt90eJu.js:2343-2344`, applied to scheduling via `resolveMaxWorkers` at `:3748-3753`) → **3 workers on the 4-vCPU `ubuntu-latest` runner**, so 8 shards run in ~3 waves with dynamic file pickup. No override — local machines get `cores − 1` automatically.
...
- **AC-3:** Union of per-shard reconciliations ≡ full reconciliation — pinned by the fast partition meta-tests (a)-(d) PLUS the corpus-scale gates checks (f)-(g) (assignment covers all live pairs; ledger slices disjoint-exhaustive), not by re-running the corpus twice.
