...
- **Site-scoped testid, present:** render with `site="list"`; assert `getByTestId("use-raw-control-list")` exists and `queryByTestId("use-raw-control")` is null. Failure caught: a `tid()` that ignores `site`.
- **Bare testid, absent:** render with no `site`; assert `getByTestId("use-raw-control")` exists and `queryByTestId("use-raw-control-list")` is null. Failure caught: an always-on suffix that would break every unit mount.
