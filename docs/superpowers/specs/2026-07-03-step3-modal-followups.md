...
- **Row lookup:** select `parse_result` from `pending_syncs` where `drive_file_id = $driveFileId` AND `pending_wizard_session_id = $wizardSessionId` (the same active-session guard semantics as the unapprove route's UPDATE guard, unapprove route L72-92). 0 rows (superseded/absent) → 404.
...
- **Byte fetch:** stub `contentUrl` null (XLSX-media entry) → 404 without any Drive call. Otherwise `snapshotFetchEmbeddedImageBytesTimed(stub)` (no `fetchXlsxBytes` dep) → `null` → 404.
...
- Status line (`role="status" aria-live="polite"`, instant text swaps): pending "Sending…"; success "Sent — thanks. The developer will take a look." (draft cleared; no GitHub link exposed); errors render **copy, never codes** (plan-wide invariant 5): coded body → `isMessageCode(code) ? messageFor(code).dougFacing : generic` (`lib/messages/lookup.ts:91,95`), fetch/network failure → `messageFor("NETWORK_UNREACHABLE").dougFacing`, anything else → the `REPORT_PIPELINE_FAILED` dougFacing copy. Duplicate (200 `status:"duplicate"|"recovered"`) counts as success.
- Payload: `{ surface: "admin", show_id: null, idempotency_key, message, reporterUrl: window.location.href, viewerVisibleSection: <current active section id>, userAgent: navigator.userAgent, parseWarnings: warnings.slice(0, REPORT_PARSE_WARNINGS_CAP), fieldRef: { kind: "wizard-step3", driveFileId, wizardSessionId, driveFileName: row.driveFileName ?? null, stagedShowTitle: row.stagedShowTitle ?? null } }`.
...
- `submitReport` internals already tolerate null (`claimed.show_id: string | null`, L586; `admin_alerts` upsert uses `coalesce(show_id::text,'')`, L645-647). Audit every `body.show_id` interpolation for the null case: `showLine` (L263-270) and `buildAdminIssueBody` (L396-434) fall back to `fieldRef.stagedShowTitle` / "wizard sheet `<driveFileId>`" / "unknown show" so no issue title/body ever renders the literal `null`.
...
4. Report form: submit posts the §D3 payload shape (assert against the mocked `fetch` body, not the DOM); key persistence across a failed→retry sequence; rotation on success and on 410; rate-limit 429 renders `REPORT_RATE_LIMITED_ADMIN` dougFacing copy and NEVER the literal code string (catches: raw-code leak, key churn making duplicates unlinkable).
...
7. Staged-diagram route: unauth 403; superseded session 404; unknown objectId 404; null-contentUrl stub 404 with zero Drive calls (spy); happy path returns bytes + exact header set; non-allowlisted mime 404 (catches: SVG passthrough, session-guard bypass).
