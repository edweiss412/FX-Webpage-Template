...
- Audit every `body.show_id` interpolation for the null case: `showLine` (L263-270) and `buildAdminIssueBody` (L396-434) fall back to `fieldRef.stagedShowTitle` / "wizard sheet `<driveFileId>`" / "unknown show" so no issue title/body ever renders the literal `null`.
