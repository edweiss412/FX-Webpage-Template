...
8. Diagrams body: cap at `DIAGRAM_TILE_CAP` with "+N more" note derived from fixture length (fixture must exceed the cap); null-contentUrl tile renders placeholder without an `<img src>` fetch; folder-only renders link and no grid; hostile `driveFolderUrl` fixture renders counts with no `<a>` (catches: unvalidated href).
