...
- **Folder row:** when `linkedFolder != null`, an external-link anchor "Open diagrams folder in Drive", `target="_blank" rel="noopener noreferrer"`, plus "N files" when `linkedFolderItems.length > 0`. The parser only ever captures `https://drive.google.com/.../folders/...` URLs (`lib/parser/diagrams.ts:27-42`), but the modal consumes persisted untrusted JSONB — so the component revalidates before `href`: `new URL(driveFolderUrl)` succeeds, `protocol === "https:"`, hostname `drive.google.com` (exact); otherwise render the counts text with NO link (tested with a hostile-URL fixture). Linked-folder items get NO thumbnails (out of scope, §10).
