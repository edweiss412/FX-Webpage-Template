...
**Worked examples of the dual-read rule:** `25/3/2026` — `mdyIso` null (skipped by scan (a); the row keeps its existing unreadable-date warning, independent signal of the same root cause), `dmyIso` valid (participates in (b)) — a fully-DMY sheet gets both signals. `3/25/2026` — `mdyIso` valid, `dmyIso` null — its presence kills the DMY hypothesis outright, so a normal US sheet with a typo'd out-of-order date does NOT emit this code.
