...
`blockRef: { kind: "hotels", name?: <hotel name>, field: "guests" }`, `rawSnippet` = the raw cell. **Emit ownership:** `parseGuestCell` stays pure (it has no hotel context and no aggregator) — it returns split metadata (which branch fired) alongside `names`/`confs`, and the CALLER emits. `name` is the caller's parsed `hotel_name` when already resolved at emit time (structured left/right-slot path, `hotels.ts:348-430`); on inline paths where `hotel_name` is derived after guest extraction, or for null/inherited-name cases, `name` is OMITTED — `kind: "hotels"` alone routes the warning correctly (`KIND_TO_SECTION`), `name` only sharpens the callout text.
...
Instead, a **block-level sequence check** over the DATES section's dates **in sheet encounter order, captured BEFORE the `result.showDays.sort()` calls** (`lib/parser/blocks/dates.ts:183`, `:281` — both parser versions sort before return, erasing row order). The sequence is **token-level**, produced by a NEW dedicated pure collector — `collectDateTokens(rows): Array<{ raw: string; mdyIso: string | null; dmyIso: string | null }>` — because the existing `extractAllDates` (`dates.ts:322`) cannot prove this contract: it returns normalized ISO strings (no raw tokens), scans per regex family rather than in true within-cell offset order, and has no numeric-dash family. The collector runs ONE combined-alternation regex pass per cell (numeric slash, numeric dash, ISO, longform MDY, longform DMY) so matches come out in genuine within-cell offset order; rows are visited in DATES-block encounter order (ALL row kinds — travel-in / set / show / travel-out), in BOTH parser-version walkers, on the same raw `(label, value)` rows they already iterate, before any sort. Per token: `mdyIso` = the current-rules reading (`normalizeDate` semantics), `dmyIso` = the swapped reading for numeric slash/dash tokens where both fields ≤ 12, else `dmyIso = mdyIso` (fixed points: ISO, longform, unambiguous numerics). Calendar-invalid readings are null and skipped by the monotonicity scan. The check: if the `mdyIso` sequence is non-monotonic (strictly decreasing at any adjacent non-null pair) AND the `dmyIso` sequence is non-decreasing, emit. The collector is read-only relative to parsing — parsed values still come from the existing code paths; it feeds ONLY this check. `rawSnippet` at a violation = the `raw` token at the first out-of-order position: if that encounter-order sequence is non-monotonic (strictly decreasing at any adjacent pair) under the MDY reading, AND re-interpreting every numeric slash/dash date as DMY yields a non-decreasing sequence, emit ONE warning: `blockRef: { kind: "dates", field: "order" }`, `rawSnippet` = the first out-of-order raw date. This fires exactly in the real mis-read scenario (coordinator wrote DMY) and never on a well-ordered US sheet.
...
**MDY-invalid tokens (explicit):** a DMY-written token with day > 12 (e.g. `25/3/2026`) has `mdyIso = null` (skipped by the MDY monotonicity scan — those rows keep their existing unreadable-date warnings, independent signal of the same root cause) but a valid `dmyIso`, so it DOES participate in the DMY-rescue sequence — a fully-DMY sheet gets both signals. (This supersedes the earlier round's "MDY-valid only" narrowing: the collector's per-token dual reading makes inclusion free.)
