...
| `lib/parser/dataGaps.ts:125-133` (`hasRecoveredToBaseline` — per-class ≤ baseline check) | alert auto-recovery gate | NO partition — symmetric with the regression gate above (ambiguity classes count toward regression, so they must count toward recovery, else an alert opened by an ambiguity jump could auto-close while the jump persists — or never close). New codes participate automatically via the `GAP_CLASSES` iteration; a test pins an ambiguity-class regression blocking recovery |
| `lib/parser/dataGaps.ts` (`DATA_GAP_CLASS_LABELS`, built from `GAP_CLASSES`) | plain-language class labels | NO partition — each new `GAP_CLASSES` entry supplies its `label` (invariant-5 plain language), covered by the extended `dataGapsClassCompleteness` test |
...
- the no-token fallback (`hotels.ts:147` — whole segment treated as one guest name) fires on a segment whose shape suggests multiple glued guests. Exact predicate, on the trimmed segment the fallback consumed: (i) **≥ 4 name-like tokens** — a name-like token is a maximal run matching `/^[\p{L}][\p{L}\p{M}.'-]*$/u` after splitting the segment on whitespace (same character class as the existing `tokenRe` name grammar at `hotels.ts:138`, minus the space; pure-punctuation/digit fragments are not name-like); OR (ii) **an interior conf-shaped digit run** — a `/\d{4,}/` match (the `tokenRe` conf grammar: 4+ digits) whose match neither starts at index 0 nor ends at the segment end. Negative examples (no warn): "Mary St. Claire" (3 name-like tokens — `St.` counts as one under the stated regex), "José Núñez-Marín" (2), a segment that IS just a 5-digit number at either boundary. Accepted false positive, explicit: a 4-token single-person name ("Mary Anne St. Claire") DOES warn — the threshold stays at 4 because the dominant glued-guest shape is First Last First Last (exactly 4 tokens); raising to 5 would miss it. Cost is one spot-check glance on a publishable row, accepted;
- a trailing un-numbered tail was appended as a guest (the `tail` branch) — the tail may be a guest or may be trailing noise.

**Emission cardinality: exactly one warning per guest CELL** with ≥1 triggering condition — segments/branches are reasons, not warnings. If several segments (or both the fallback and tail branches) trigger within one cell, the single warning's message names the count of triggering segments and `rawSnippet` stays the whole raw cell; the gap-class count increments by 1 per cell. Tests: one multi-segment cell (both branches trigger) asserts exactly one warning; two ambiguous cells assert exactly two.

**Site scope (strict):** this site covers the STRUCTURED guest-cell path only — `parseGuestCell`'s two call sites at `hotels.ts:446` and `:449`. The inline shapes (v1 "Hotel Stays" / v2 inline "Hotel Reservations" rows) parse guest lines through separate code and are NOT in v1 scope; those paths get a `deferred:BL-PARSER-HOTEL-INLINE-AMBIGUITY` walker exemption (§6).
...
Walker asserts: (1) export present on every file (no allowlist analog to `NO_SECTION_OPENER` — a block with zero transform sites exports `[]`); (2) every `code` value exists in the catalog (`isMessageCode`, `lib/messages/lookup.ts:91`) — catalog `severity` is optional and uses `"warning"` values, so the walker makes NO catalog-severity assertion; the emitted `ParseWarning.severity === "warn"` is proven by each site's emit unit tests (§10), not the walker; (3) RETIRED (R8) — the entry shape has no independent "ambiguity-class" claim field, so a forward assertion would be circular; class membership is derived solely from `AMBIGUITY_CODES` and enforced by the inverse assertion (5); (4) the five required declarations exist — `crew.ts`: `CREW_COLUMN_POSITIONAL_FALLBACK`; `rooms.ts`: `ROOM_HEADER_SPLIT_AMBIGUOUS`; `hotels.ts`: `HOTEL_GUEST_SPLIT_AMBIGUOUS` AND `HOTEL_CARDINALITY_EXCEEDED`; `_helpers.ts` or `dates.ts` (plan decides the collector's home): `DATE_ORDER_SUGGESTS_DMY` (pins this spec's deliverable against silent removal); (5) **inverse completeness** — every member of `AMBIGUITY_CODES` appears as some file's declared `TRANSFORM_SITES` code (so the retroactive `CREW_COLUMN_POSITIONAL_FALLBACK` must be declared in `crew.ts`'s inventory; a registry entry with no declared site fails).
...
Deferred-exemption seeds (documented at plan time after per-file enumeration): expected entries include stage-clause extraction (`personalization.ts` — already warns via `UNKNOWN_STAGE_RESTRICTION` family), dims normalization (`_dimsToken.ts` consumers — deterministic), name canonicalization sites, address parsing (`hotels.ts` — `deferred:BL-PARSER-ADDRESS-SPLIT-AMBIGUITY`). Deferred refs are concrete `BL-*` IDs with matching `BACKLOG.md` rows; the walker validates the pattern and the BACKLOG.md presence (plan R5 hardening).
...
Guard conditions: M=0 renders the existing two-state summary (no empty "0 parsed with judgment" chrome). Zero rows is the existing empty-wizard state, unchanged. Row classification, precise: only non-ambiguity **GAP** warnings (plus the preserved operational branches) force needs-look — a row with a non-ambiguity gap warning + ambiguity warnings is **needs-look** (K), counted once; a row with only benign NON-gap warns + ambiguity gaps is **judgment** (`rowNeedsLook` consumes `summarizeDataGaps`, which never counted non-gap warns — §7.1's universe note). The row-level judgment predicate: NOT needs-look AND ≥1 ambiguity-class GAP warning (i.e. `isAmbiguityCode(code) && DATA_GAP_CODES.has(code)` — same universe as `rowNeedsLook`). Supporting invariant, enforced by a unit test: **`AMBIGUITY_CODES ⊆ GAP_CLASSES` codes** — every ambiguity code is a gap class (all four `AMBIGUITY_CODES` members satisfy it — `HOTEL_CARDINALITY_EXCEEDED` in §9 is a gap class but deliberately NOT an ambiguity code, so it is outside this subset check; a future ambiguity code outside `GAP_CLASSES` would silently drop out of row judgment, so the test makes that impossible).

### 7.3a Card-face chrome (Step3SheetCard)

The compact card independently derives `needsLook = summarizeDataGaps(warnings).total > 0` (`Step3SheetCard.tsx:470-471`) — partitioned per §3.4: the needs-look treatment — warn border (`border-border-strong`) + Review button (the selectable card's actual chrome today; the only literal chip is the demoted "Needs another look" branch — no new chip is added in v1) — appears only for non-ambiguity gaps (or operational branches). An ambiguity-only row gets a **judgment card variant**: distinct border/affordance treatment (impeccable-gate deliverable), still opens the same review modal. `DataQualityBadge` on the title keeps the FULL gap count (no partition — §3.4 dashboard rule). Three card states (plain / judgment / needs-look) mirror the §7.2 row buckets; a test pins each derivation.

### 7.3 Section chrome

`SectionFlagCallout` (`components/admin/wizard/step3ReviewSections.tsx:469-514`): judgment sections get a visually distinct callout variant — "We made a judgment call reading this — worth a glance" + the entry list (existing `CALLOUT_MAX_ENTRIES` cap at `:480` applies unchanged) with its existing per-entry "View details" affordance. The "In sheet ↗" deep-link (`:548-559`) lives in `ModalSectionChrome` at the section heading, NOT inside the callout — it stays there, unchanged, and serves as the judgment state's spot-check affordance. No new per-entry sheet link in v1 (a per-entry link is an impeccable-gate option, not a spec requirement). Entry text includes the `blockRef.field` when present ("dims for ROOM A") — rendered through a small FIELD_LABELS map (`dims` → "dimensions", `name` → "room name", `guests` → "guest list", `order` → "date order"); an empty/unknown/unmapped `field` value falls back to omitting the field phrase entirely (persisted historical warnings may carry values this build doesn't know — raw internal tokens never leak into UI copy). Visual treatment (color/token choice distinct from flagged amber and from clean) is an impeccable-gate deliverable within existing `DESIGN.md` tokens.
...
- **Flag lifecycle table:** no boolean config flags introduced. The nearest analog, `AMBIGUITY_CODES` membership: storage = `lib/parser/ambiguityCodes.ts` literal | write = parser emit sites | read = `step3SectionStatus` section derivation (§7.1), `Step3Review` row summary (§7.2), `Step3SheetCard` card chrome (§7.3a), `rescanDecision` dirty partition (§3.4), walker + subset meta-tests (§6, §7.2) | effect = wizard third state at all three grains + rescan-dirty exemption. No zombie columns.
...
TDD per task (invariant 1). Per site: fixture-derived ambiguous inputs (a multi-dims room header; a glued guest cell with interior digit run; a DMY-ordered numeric date sequence) asserting against the **warnings array + blockRef.field** (anti-tautology: never scan a container that renders both the value and the flag; expected values derived from the fixture, not hardcoded). Negative tests: well-formed inputs at each site emit nothing; both-readings-unordered dates emit nothing; a DMY-invalid numeric anywhere in the block suppresses the date code. **Cardinality-overflow direct test (§4.2b):** a fixture with > 4 hotels through `parseHotels(..., agg)` asserts the truncation warning lands in the AGGREGATOR (`warnings[]` entry with `code: "HOTEL_CARDINALITY_EXCEEDED"`, `severity: "warn"`, `blockRef: { kind: "hotels" }`) — not merely the log line — plus gap-class routing (code counted by `summarizeDataGaps`). **Date-collector placement proof (required):** tests for BOTH the v1 walker and the v2/v4 walker using an encounter-order fixture whose dates SORT clean but ENCOUNTER dirty (e.g. rows 4/3, 3/4 — sorted ascending masks the violation) — proving the collector runs pre-`showDays.sort()` in each version; a post-sort implementation fails these tests. Guard/NaN/empty inputs per §4.3 guards. Walker meta-test per §6. Wizard: derivation unit tests for §7.1/§7.2 including the mixed-warning and M=0 guards, PLUS the load-bearing scope assertions: blocking rows excluded from the three counts; set-aside rows excluded; missing-preview row stays needs-look with ambiguity warnings present; finalize-failure row stays needs-look likewise; N+M+K === `publishRows.length`. Class-sweep before push: warning-shape `toEqual` assertions (§5), `pnpm test` full suite + typecheck + eslint + format:check (memory: scoped gates miss regressions; vitest strips types; canonical Tailwind; --no-verify bypasses prettier).
