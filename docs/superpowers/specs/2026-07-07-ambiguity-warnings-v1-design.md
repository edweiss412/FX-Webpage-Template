...
**Worked examples of the dual-read rule:** `25/3/2026` — `mdyIso` null (skipped by scan (a); the row keeps its existing unreadable-date warning, independent signal of the same root cause), `dmyIso` valid (participates in (b)) — so a DMY sheet whose remaining MDY-valid readings decrease somewhere gets BOTH signals (unreadable + this code). A DMY sheet where every numeric token is MDY-invalid, or whose MDY-valid subset happens to be non-decreasing, emits only the unreadable-date warnings — accepted: condition (a) is unsatisfiable there, and the unreadable warnings already point at every affected row. `3/25/2026` — `mdyIso` valid, `dmyIso` null — its presence kills the DMY hypothesis outright, so a normal US sheet with a typo'd out-of-order date does NOT emit this code.
