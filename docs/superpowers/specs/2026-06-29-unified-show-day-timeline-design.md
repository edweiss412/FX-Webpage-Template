...
```
- `start` is the `normClock` output from the sheet's SHOW DAY TIME column (`lib/parser/blocks/scheduleTimes.ts:40-53`): `[H]:?[MM]? [AM/PM]`, no leading-zero hour, **minutes optional** (bare hour `"9 AM"` is valid), AM/PM uppercased. Examples: `"4:00 AM"`, `"9 AM"`, `"12:00 PM"`, `"2:00 PM"`.
...
**Activation rule (blast-radius minimization):** `agendaSessionsForToday` returns **only placeable** sessions — it drops any session whose `time` is unparseable (`clockToMinutes(time) === null`) *before* returning (see §3.2 step 4). So `agendaToday.length > 0` is a faithful "≥1 interleavable agenda session" gate. The merge path (`ShowDayTimelineList` via `buildShowDayTimeline`) activates **iff `agendaToday.length > 0`**; otherwise TodaySection renders the **existing `RunOfShowList(todays)` unchanged**. Two consequences, both intentional:
...
2. Map agenda sessions → `{source:"agenda", session, minutes: clockToMinutes(session.time)}`. The caller (`agendaSessionsForToday` step 4) already filtered unplaceable sessions, so `minutes` is non-null here; defensively drop any `minutes === null` anyway (a no-op for the normal path — keeps `buildShowDayTimeline` correct if called directly in a test).
...
- **Capping (borrows `RunOfShowList`'s EXEMPTION rule, `:128-138`, but NOT its partition render-order):** `RunOfShowList` exempts **synthetic** crew rows (`kind` `strike`/`loadout` — few, load-bearing milestones) from the cap, then renders the (capped) agenda group **followed by** the synthetic group (`:141-145` — a partition, acceptable there because that list is sheet-ordered, not chronological). The merged timeline must stay **chronological**, so it adopts only the **exemption**, not the partition: **synthetic crew rows are never capped AND stay in their sorted time position.** Algorithm on the sorted+deduped `items`: (1) keep every synthetic-crew item; (2) of the **non-synthetic content** (crew `kind∈{undefined,"agenda"}` **+ all PDF-agenda sessions**) keep the **first `RUN_OF_SHOW_DISPLAY_CAP`** (`lib/crew/agendaDisplay.ts:16`, =20) in timeline order, drop the rest; (3) render the union of (1)+(2) **in chronological order** (a strike at 11 PM renders at the bottom, in time order — NOT appended after the agenda group); (4) if any non-synthetic was dropped, render one `+N more` overflow stub (`data-testid="timeline-agenda-overflow"`) at the end. This prevents the `>20`-row mobile regression while keeping strike/loadout both always-visible **and** chronologically placed.
...
- **`ShowDayTimelineList` (`tests/components/crew/primitives/…`):** scope every assertion to `container.querySelector('[data-testid="show-day-timeline-<iso>"]')`. Crew items → `data-testid="agenda-entry"` (existing treatment); agenda items → `data-testid="timeline-agenda-session"` with the full `session.time` string, muted tone + event marker, room when present, **no** track/drift text; null-title agenda → time-only row. **Capping (synthetic-exempt AND chronological):** fixture = 22 crew `kind:"agenda"` items (early times) + 1 PDF agenda session + 1 crew `kind:"strike"` at the **latest** time (e.g. `"11:00 PM"`). Assert: exactly `RUN_OF_SHOW_DISPLAY_CAP`(20) non-synthetic rows + one `timeline-agenda-overflow` stub (dropped `23 − 20 = 3`); the **strike row is present AND is the LAST row** (its sorted position — proving synthetic stays chronologically placed, NOT partitioned to a separate group). *Failure mode catches both:* a 30-row mobile dump, **and** an implementation that copies `RunOfShowList`'s partition (strike rendered out of time order). A second case: strike at an **early** time (e.g. `"6:00 AM"`, before the agenda) asserts the strike renders **first**, not after the agenda group.
