...
So the anchor is the measurement, not the navigation: **await immediately before each measurement, after the content being measured is present.** Across the 21 that is 82 navigation sites' worth of surface, but the count that matters is measurements, not navigations.
...
| **Runtime check, not static:** on a route census that includes `/help`, the loaded `document.fonts` family set equals exactly `{Inter, Inter Fallback}`. | **The ninth escaping mutant, and the one that shows where the static census ends.** A client component can call `new FontFace(...)` and `document.fonts.add(...)`, or compose `<style>` text at runtime, or use CSSOM `insertRule`/`replaceSync` — registering a face with the word `@font-face` appearing nowhere in any source literal. Splitting `"@font-" + "face"` defeats a text census outright. These are real registration mechanisms, not exotica. No amount of source scanning closes this class, so the closure is a runtime family-set assertion: anything registered by any mechanism shows up in `document.fonts`, and the set is small and known. The route census must include `/help` — `tests/e2e/font-binding.spec.ts` visits `/admin`, `/auth/sign-in` and a crew route only, which is how mutants six, seven and nine all reach a user-visible surface no runtime gate looks at. |
...
Source-text assertions, DB-free unit suite — **except** the two rows explicitly marked as runtime checks (the `document.fonts` family set, and the harness face in §4.2). Those two exist because a text census provably cannot see runtime font registration, and pretending otherwise is what mutant nine punishes.
