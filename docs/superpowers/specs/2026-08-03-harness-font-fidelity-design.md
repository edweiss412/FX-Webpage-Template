...
2. **The 24 unsynchronized callers await `document.fonts.ready` once per measured document** — the invariant stated below; not once per file, not per navigation, and not per geometry read. `font-display: block` makes the race vanishingly unlikely; awaiting makes it impossible, and it is the guarantee the specification actually offers. The list is enumerated in §5 so the work is countable rather than discovered.
...
**And per-navigation is still not the right anchor, which round 6 established.** Nine of the 24 — `attention-pill-focus`, `bulk-ignore-eyebrow.layout`, `collapse-panel-morph`, `compact-alert-card-layout`, `hoverhelp-geometry`, `popover-clip-fit`, `step3-review-modal.agenda`, `step3-review-modal.interactions`, `wizard-blocker-modal.layout` — navigate to markup that is just `<div id="root"></div>` and only then hydrate. A wait placed right after that navigation settles against a document with no text in it, so it triggers no font load and guarantees nothing about the text React renders next. CSS Font Loading Module Level 3 is explicit that further fonts may load after `ready` fulfills, and that a measurement is safe only while nothing subsequently changes the document.
...
> In each of the 24 callers, every document that is measured is awaited: after the content under measurement is present and before the first geometry read **of that document**.
...
Naming only the first would leave a timing-dependent guarantee; naming only the second would leave 24 files each able to regress independently.

Per invariant 1 the wait edits are not a bulk mechanical pass: the §4.1 row above is written first and must fail against the current tree — where all 24 have zero waits, confirmed by probe — before any wait is added.
...
- **The measurement reads the route's ACTUAL rendered text, not a synthetic probe.** `tests/e2e/font-binding.spec.ts:71-79` appends its probe span to `document.body`, which cannot see a descendant override — mutant ten, `.help-prose { font-family: Arial }` on the wrapper at `app/help/layout.tsx:66`. Round 8's repair moved the probe into "the deepest content wrapper", and round 9 refuted that too: on `/me` a direct `fontFamily: "Arial"` on the visible `<h1>` (`app/me/page.tsx:156`) renders that heading in Arial while a synthetic sibling inside `<main>` (`app/me/page.tsx:154`) keeps inheriting Inter and passes. There is no singular deepest wrapper — pages have multiple sibling content branches, and any probe measures its own branch.
...
**The fontkit dependency is named, because round 11 showed the spec did not determine one buildable diff.** `fontkit` does not resolve in this repo today; the spike reached it through Next's private compiled path, which is not something to build a permanent guard on. The implementation adds `fontkit` as an explicit devDependency and imports it normally. That is a lockfile change and is called out here so it is reviewed rather than discovered — the alternatives (depend on a Next internal, or hand-parse woff2) are both worse and both would have been legitimate readings of the earlier wording.
  ...
  **The expectation must be instanced at the element's own weight.** Inter is a variable font with a `wght` axis (100-900, default 400), and advance widths differ by weight: for the probe string at 16px, 400 gives 196.30px, 600 gives 200.30px, 700 gives 202.33px — each far outside the 0.5px tolerance. The tree has live bold and semibold text. So the oracle must account for weight; a fixed-400 expectation would fail on legitimate bold Inter, which is a guard that cries wolf and gets deleted.

**How it accounts for weight was probed, and the obvious answer does not work.** Instancing the variable font at the element's computed weight is the natural fix and is what an earlier draft specified. `getVariation({ wght: w })` **throws** — on the Next-vendored fontkit *and* on a real `fontkit@2.0.4` devDependency, identically, and even at the default weight (`Cannot read properties of undefined (reading 'tables')`). The axis is readable; instancing from this WOFF2 is not available in either build.

**Resolution: the width oracle runs on default-weight text, as a documented limit.** `font.layout(probe).advanceWidth` on the base font works and is exactly what produces the 194.133px expectation the spike matched to 0.008px — instancing was only ever needed for non-default weights. Body text is weight 400 — `app/globals.css:143` defines `--font-weight-regular: 400` and `app/globals.css:676-681` applies it to `body` — so effectively every surface carries an element the width check can land on. Verified rather than assumed, since it is the premise the limit rests on. Bold and semibold elements are still walked for computed family; they simply do not carry the byte-derived width assertion, and a surface with no weight-400 text is listed as computed-family-only.

The discriminating property is preserved — an impostor still fails on any default-weight text — and only the coverage narrows, visibly.

**The probe element must neutralise CSS spacing.** `app/globals.css` sets `letter-spacing: -0.005em` on `body`, which any probe inherits, and fontkit's `advanceWidth` is pure glyph advance that knows nothing about it — at 16px over a 22-character string that is ≈1.76px, more than 3× the tolerance. The end-to-end spike hit exactly this and read as "the font isn't loading". So the measured element carries `letter-spacing: normal; word-spacing: normal`; with that, delta is 0.008px. A guard that fails on every surface is as useless as one that passes on every surface, and this is the difference between the two.
