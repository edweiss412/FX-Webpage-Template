...
The oracle therefore runs **from the close path**: the fixture proxies `page.close()` and `context.close()` so the check completes on every live page *before* the underlying close proceeds, and the `page`-fixture branch also runs it after the test body for documents the caller never closed. That point is the only one guaranteed to be both after the document is final and before it is destroyed. The working prototype is tracked beside the guard, at `docs/superpowers/specs/spikes/2026-08-03-harness-font-fidelity/fixture-prototype.ts`; it builds two caller-owned contexts, closes both before teardown in the shape `agendaScheduleLayout` uses, and observes both.
