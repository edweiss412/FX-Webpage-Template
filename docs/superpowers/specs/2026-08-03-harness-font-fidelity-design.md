...
So Kind B is closed by **one general oracle**, not a list: walk every visible text-bearing element on every rendered surface, classify it by the family its cascade selects, and assert that family — for sans text, a normalised child probe against the byte-derived expectation (§4.2); for the app's monospace text, the mono chain. Computed-family resolution alone is NOT the oracle — it was measured identical across a real face and two impostors. It has no privileged place to look within the elements it can reach. **Completeness has three dimensions — rounds 15 and 18 each found one the previous statement had missed.** One is the *surface census*, which §4.2's census derives from the framework's own config rather than from a hand list. The second is the *element class*: two classes cannot host a probe at all, and §4.2 closes them by a different mechanism rather than by pretending they do not exist.

The third is **glyph repertoire, and it is OPEN.** The child probe is ASCII, so it exercises only the latin face; a face aliased as `Inter` but scoped to another range — `new FontFace("Inter", "local('Arial')", { unicodeRange: "U+0370-03FF" })` — renders the parent's Greek from the impostor while the ASCII probe measures the genuine latin face and passes. Six subsets are unexercised, and the signal is large (Inter Greek 86.92px against Arial Greek 76.02px); nothing is looking at it.

The obvious fix, one probe per subset, is not yet specified because two attempts to build it failed for different reasons: natural probe strings pull in the latin face through spaces and ASCII, so the baseline fires with everything genuine; and a face without its `unicode-range` claims all codepoints, cascading fires into unrelated subsets. The requirements are now known — strings strictly inside each range, glyphs actually present in that subset file, faces carrying their real ranges — but a mechanism that has not been made to run does not go in this document as though it had. That is what produced round 13's BLOCKING verdict, and §4.2 records the same posture for the weight question.
...
| Descriptor parsing is **case-insensitive**, matching CSS. | **Round 18's mutant:** `SRC: local("Arial")` appended after a correct lowercase `src`. CSS treats the two as the same descriptor and applies the last; a lowercase-only parser reads the first and passes. It affects all 48 pinned descriptor slots — six descriptors across seven Inter faces plus six on the fallback — and a non-Latin face could therefore render Arial with the ASCII runtime probe green. |
