...
- **Routes** — every route the help navigation registry enumerates, derived from it rather than hand-listed so a new help page joins the census automatically, plus the three `tests/e2e/font-binding.spec.ts` already covers (`/admin`, `/auth/sign-in`, a crew route). Hand-listing is refused precisely because the 13 leaves beyond `/help` are what mutants nine and ten exploit.
