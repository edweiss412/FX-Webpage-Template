...
The fifth is **viewport**, and round 27 found it. The census as specified would land in the project that already runs `font-binding`, which is `desktop-chromium` at 1280×800 — so a rule as ordinary as `@media (max-width: 639px) { html { font-family: Arial } }` introduces no `@font-face`, leaves every static row intact, passes the entire census, and renders every mobile page wrong. That is the product's **primary** viewport, not an edge case: the app is mobile-first, `playwright.config.ts:68` pins the mobile project at 390×844, and there are text-bearing roots that only mount there. So the census runs at **both** viewports — 390×844 and 1280×800 — and the mobile pass is not optional, because a mobile-first product whose font guard only checks desktop has the guarantee backwards. **Round 28 then showed two viewport samples do not close it**, and the right answer is not more samples. `app/globals.css` branches on four environment axes today — `max-width: 480px`, `min-width: 640px`, `prefers-color-scheme: dark`, `prefers-reduced-motion: reduce` — and any of them can re-point `font-family` without touching a single `@font-face`. Sampling that matrix at runtime is combinatorial and still only a sample. So the class is closed **statically**: a guard row forbids `font-family` (and the `font` shorthand) inside **any** conditional at-rule, at any nesting depth, and the mobile-only and dark-mode override mutants both die on it. The two runtime viewport passes remain, because layout-dependent surfaces genuinely only mount at one width or the other — but they are no longer what stands between the product and an environment-scoped override.

The sixth is **document-tree boundaries**, and round 29 found it. A `TreeWalker` does not cross a shadow boundary and a page is not its frames, so text inside an open shadow root or a child frame was invisible to every vantage while the main-document walk and the registered face set both stayed green. The walk now descends into open shadow roots, and every sweep enumerates `page.frames()` rather than only `context.pages()` — including on `page.close()`, which is a distinct ending from `context.close()` and was the gap the frame case actually fell through. A **closed** shadow root is unreachable by construction and is a stated limit rather than an oversight; the tree has no `attachShadow` today, so this is preventive.

Round 29 also corrected the fifth dimension's own guard, which had claimed more than it did: the row registered a `media` visitor only, read only the fonts stylesheet, and ignored indirection, so `@supports`, `@container`, `app/globals.css`, and a conditionally-redefined `--font-*` token all walked through prose that said "any conditional at-rule at any nesting depth". It now scans the rule tree of every shipped stylesheet for `font-family`, the `font` shorthand, **and any `--font-*` token**, because an unconditional `font-family` reading a conditionally-redefined token is exactly as conditional. Verified against the real `app/globals.css`: it passes today and fails on an injected dark-mode token override.
...
**The fontkit dependency is named, because round 11 showed the spec did not determine one buildable diff.** `fontkit` does not resolve in this repo today; the spike reached it through Next's private compiled path, which is not something to build a permanent guard on. The implementation adds `fontkit` as an explicit devDependency and imports it normally. That is a lockfile change and is called out here so it is reviewed rather than discovered — the alternatives (depend on a Next internal, or hand-parse woff2) are both worse and both would have been legitimate readings of the earlier wording.
  ...
  **The walk partitions by the family the MANIFEST expects, because the app ships two.** `DESIGN.md` §2.1 commits to one *sans* family and says nothing about monospace, but the tree renders mono in monospace in a number of places that is **derivation-sensitive**, which rounds 28 and 29 both demonstrated by disagreeing about it — and the disagreement is the useful result, not the number. Counting `font-mono` utilities agrees at **9**. Everything else moves with the method: whether "the dev subtree" means `app/admin/dev` alone or also `components/admin/dev` (4 of the 9 utilities sit in the wider reading, 1 in the narrower), whether semantic elements are counted by grep or by TypeScript AST (7 versus 8 outside that subtree), and whether MDX inline code is counted as authored backticks or as compiled `<code>` elements (**16** either way, today).

So the spec does not pin a total, because a pinned total is a claim that must be re-derived every time the tree moves and was wrong in two consecutive rounds. What it pins is the **rule**: the manifest is seeded from `font-mono` utilities plus semantic `code` / `kbd` / `samp` / `pre` elements including those MDX compiles to, with any container that carries the utility for a region entered once as `scope: "subtree"` instead of one entry per descendant. The seed is then verified by the freshness assertion — every entry must match at least one element on its route, and every element rendering mono must match an entry — which is what makes an off-by-one in the seed fail loudly instead of persisting as a silently over-broad exemption. That assertion, not a number in this document, is the thing that keeps the manifest honest.. `/help/errors` is an always-reachable member of the 32-route census, and `/admin/dev` puts `font-mono` on its entire `<main>` — so this is not a fringe case behind an interaction or an exclusion.
...
- **Config** — add to **both** the mobile and desktop projects' `testMatch` explicitly — round 28 caught that "the project that already runs `font-binding`" selects `desktop-chromium` alone, and naming both projects on the CI command line does not override a project's own `testMatch`, alongside it.
