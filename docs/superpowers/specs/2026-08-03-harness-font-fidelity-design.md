...
Re-validated end to end after the port, and the port found two genuinely weak assertions of its own that the earlier instrument had also carried: a URL check that accepted a greek URL nested one directory deeper (a "sub" segment inserted before the filename) because it only tested a `/fonts/` prefix, and a `--font-inter` check that accepted a trailing `, Arial` because it matched by substring. Both are now exact — full path equality, and an exact token sequence. **30/30 mutants killed, 15/15 rows passing, no browser.**

**What the spike instrument covers, stated so its results are not over-read.** Two different things in this spec are called spiked, and they cover different halves. The *mechanism* — below — is spiked end to end, through the real `compileEntryCss` path, and that is what establishes the harness half works at all. The *static guard* is a separate instrument: it parses the app stylesheet with the browser's own CSSOM and is validated by mutation, currently **30 mutants, all killed, across 14 assertions**. Every mutant it has killed was aimed at the app stylesheet. The harness-side rows in §4.1 — emitted-CSS descriptor equality, the bare-sibling URL resolution, the copied-artifact hashes, `font-display: block` — are specified with their escaping mutants named, but they are **not** among the 30, because exercising them requires the target-state toolchain the plan builds. So "30/30" is a statement about the app-side rows and should be read as one. The plan carries them as implementation tasks with the same mutation obligation, which is the point at which that number should grow rather than be quoted.
...
- **The 31 harness documents are reached by a shared fixture, not by a census spec — round 20's finding.** An earlier draft said each of the 31 gets the same visible-text walk "because the documents are already being rendered". They are, but not by anything the census file can see: all 31 import the base `@playwright/test` fixture directly and run under their own standalone and visual configurations, while the census spec this design adds (font-rendering-census, not yet created) is assigned to the crew project. A spec cannot inspect pages owned by another spec's process. As written, the caller-local `font-family: Arial` mutant survives the entire guard.
  ...
  **The mechanism was checked against the callers before being specified.** All 31 obtain their page from the standard `page` fixture — none calls `browser.newPage()` or `newContext()`, so an auto-fixture keyed on `page` reaches every one, with no caller left needing a bespoke hook. Their import lines take seven distinct shapes (`{ test, expect }`, `{ test, expect, type Page }`, `{ expect, test, type Locator, type Page }` and so on), so the shared module must re-export `expect` and the type names as well as the extended `test`; the edit is then one module specifier per file. Worth noting for the plan: the repo has **no** existing `test.extend` fixture, so this introduces the pattern rather than following one, and the meta-test below is what stops it from being a pattern only these 31 files happen to follow.
...
| The 25 geometry harnesses | Playwright | Expected to move where a figure is font-dependent; the suite run is the census. |
