...
- **Interaction-only surfaces are in the census too.** Eleven dialog roots across ten components (`ReportModal`, `ReviewModalShell`, `ShareHub`, `BellPanel`, `HelpSheet`, `AgendaEmbed`, `GalleryLightbox`, `AppHealthPopover`, `FinalizeButton` with two, `CleanupAbandonedFinalizeButton`) mount only on interaction — **plus two more round 11 found**, the reset and reseed confirmations in `components/admin/MaintenanceResetButtons.tsx:263` and `components/admin/MaintenanceResetButtons.tsx:317`. Those two carry a further constraint worth stating: `destructiveResetAllowed()` gates them on the validation project ref plus `ALLOW_DESTRUCTIVE_RESET=true`, and the crew-e2e workflow sets neither, so they are unreachable in that environment. They are covered where they are reachable, or listed as an explicit exclusion with that reason — the same visible-exclusion rule the route census uses. Thirteen roots in total — `components/shared/ReportModal.tsx:247-253` returns `null` when closed. A walk of "currently visible text" after navigation never reaches them, which is §4.0's own "a state no navigation reaches" category. Each is opened and walked.
