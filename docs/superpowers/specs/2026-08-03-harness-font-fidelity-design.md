...
| Every `.woff2` `compileEntryCss` copies into a harness output directory hash-matches the `public/fonts/` original of the **same name**. | **The eighth escaping mutant:** copy the latin file's bytes out under all seven names. The `public/fonts/` originals keep their correct hashes, the emitted CSS keeps every correct filename-and-range pair, source equality passes, and a rendered-metric probe on ASCII text loads only `inter-latin.woff2` — so nothing notices. Greek, Cyrillic, Vietnamese and Latin-extended harness text then falls through to a host face while every other guard is green. The §3.3 hashes constrain `public/fonts/` only; the copied artifacts are a distinct surface, and this row is the only assertion that reaches it. |
...
- **The measurement is taken inside the route's own content, not on `document.body`.** `tests/e2e/font-binding.spec.ts:71-79` appends its probe span directly to `document.body`; copying that exactly cannot see a descendant override, which is exactly what mutant ten is (`.help-prose { font-family: Arial }` on the wrapper at `app/help/layout.tsx:66`). The probe is inserted **within the deepest content wrapper of the route under test**, so the cascade it measures is the cascade the user gets.
