...
| D10 | **Concurrency — wholesale wipe uses TABLE-level locks, not per-show advisory locks (comprehensive R4 re-analysis of the 3-round concurrency vector):** per-show advisory locks serialize *known* keys but **cannot prevent a concurrent cron/onboarding writer from INSERTing a brand-new `drive_file_id` mid-reset** (the phantom-row race) — so they are the wrong mechanism for a wholesale wipe. The correct, strictly-stronger mechanism: as the FIRST statement of the transaction, `LOCK TABLE <every drive-keyed table this RPC mutates> IN SHARE ROW EXCLUSIVE MODE` (fixed alphabetical order). `SHARE ROW EXCLUSIVE` conflicts with `ROW EXCLUSIVE` (every INSERT/UPDATE/DELETE) so **no concurrent writer — including an insert of a new key — can proceed until the reset commits** (phantom race closed); it does NOT conflict with `ACCESS SHARE`, so reads continue. The reset takes **no advisory locks**, so it never waits on a sync's `pg_advisory_xact_lock` (no deadlock-by-double-holding against single-show holders). A rare reset-vs-concurrent-multi-table-writer deadlock is caught by Postgres's detector and surfaced as a transient retryable `reset_busy` (operator re-runs). This SUPERSEDES the round-2/3 per-show-advisory-loop design and SUBSUMES the per-key lock union (no key set to drift). Pinned by the advisory-lock topology test (reset = documented table-lock serializer, asserts it holds `share row exclusive` and contains NO `pg_advisory*` call) + a phantom-row concurrency test. |
...
**Immediately after the guard passes, BEFORE any delete**, the RPC acquires the table-level `SHARE ROW EXCLUSIVE` locks (D10) inside the same transaction, so no concurrent writer can insert a phantom row during the wipe and the gate + serialization cannot be separated.
...
- `create or replace function public.reset_validation_data() returns jsonb` — SECURITY DEFINER, admin+marker gate (§4.4), then in one transaction:
  1. **Serialize via table locks (D10 — closes the phantom-row race):** as the FIRST statement, `lock table public.shows, public.reports, public.pending_syncs, public.pending_ingestions, public.deferred_ingestions, public.validation_state, <every other drive-keyed table this RPC clears, alphabetical> in share row exclusive mode;` — blocks all concurrent writers (so no new key can be inserted during the wipe), allows reads, takes no advisory locks. (The exact table list is the `clear` set from the drive-keyed audit registry, §8.)
  ...
  5. explicit `delete` of drive-keyed staging/suppression tables NOT FK'd to shows (`pending_syncs`, `pending_ingestions`, **`deferred_ingestions`**, onboarding scan manifest, wizard sessions/checkpoints — plus any others the mandated drive-keyed audit surfaces);
...
- `resetValidationDataAction()`: after gates, calls `reset_validation_data()` via the admin **session** client; maps a marker-disabled raise to `reset_not_enabled` and a serialization/deadlock failure (SQLSTATE `40P01`/`40001`) to the transient `reset_busy` (operator re-runs).
...
- New codes: `reset_not_allowed` (guard closed), `reset_not_enabled` (DB marker `false`), `reset_busy` (transient — table-lock deadlock/contention, re-runnable), and success summaries are plain counts (not error codes). Follow the three-part §12.4 lockstep (AGENTS.md): master spec §12.4 prose + `pnpm gen:spec-codes` (`scripts/extract-spec-codes.ts`) + `lib/messages/catalog.ts` row, all in one commit; the x1 gate `pnpm test:audit:x1-catalog-parity` (`package.json:30` → `tests/cross-cutting/codes.test.ts:76` + `tests/cross-cutting/extract-spec-codes.test.ts`) enforces.
...
- **Drive-keyed cleanup audit (structural)** (`tests/db/resetValidationDataDriveKeyedAudit.test.ts`): query `information_schema.columns` for every `public` table with a `drive_file_id` column (currently the 11: `shows`, `pending_syncs`, `pending_ingestions`, `deferred_ingestions`, `onboarding_scan_manifest`, `pending_snapshot_uploads`, `revision_race_cooldowns`, `shows_pending_changes`, `sync_audit`, `sync_log`, `wizard_finalize_checkpoints`). Assert each appears in a maintained registry with an explicit `clear` | `preserve(reason)` disposition, and that every `clear` table is in BOTH the RPC's `LOCK TABLE` set AND its delete set. Fails when a future migration adds a drive-keyed table with no disposition. Seeds `revision_race_cooldowns` (a transient cooldown) as a clean-reset regression. Default dispositions: all `pending_*` / `deferred_ingestions` / `*_scan_manifest` / `shows_pending_changes` / `pending_snapshot_uploads` / `revision_race_cooldowns` / `wizard_finalize_checkpoints` → `clear`; `sync_log` / `sync_audit` → the plan decides (default `clear` for a true empty-state, or `preserve` with rationale recorded in the registry).
- **Phantom-row concurrency test** (`tests/db/resetValidationDataConcurrency.test.ts`): open the reset in one connection holding its `SHARE ROW EXCLUSIVE` locks; from a second connection, an INSERT into `shows`/`pending_ingestions` BLOCKS until the reset commits; after commit the table is empty (no phantom survivor). Proves the table-lock mechanism (D10) actually closes the race.
...
- **Advisory-lock topology test** (extend `tests/auth/advisoryLockRpcDeadlock.test.ts`): pin `reset_validation_data()` as a documented **table-lock serializer** — assert its body contains `lock table ... in share row exclusive mode` and contains **NO** `pg_advisory` call (so it cannot deadlock-by-double-holding against the advisory-lock sync paths). This is the structural defense that closes the 3-round concurrency vector (per AGENTS.md "structural-defense calibration" — shipped in the same round the vector was re-analyzed, not deferred).
...
- **Reset serializes via TABLE-level `SHARE ROW EXCLUSIVE` locks, deliberately NOT per-show advisory locks (D10, R4 comprehensive re-analysis)** — for a *wholesale wipe*, per-show advisory locks cannot stop a concurrent writer inserting a brand-new key (phantom-row race), so the table lock is the strictly-stronger correct mechanism; it is deadlock-free against the advisory-lock sync paths because the reset takes no advisory locks. Do not relitigate "must use `pg_advisory_xact_lock` per invariant 2" — that invariant's intent (serialize concurrent mutations) is *exceeded* by the table lock, and combining both would reintroduce a deadlock. This is pinned structurally by the topology test asserting the reset holds `share row exclusive` and NO `pg_advisory*`. (Rounds 1-3 iterated toward this; R4 settled it.)
