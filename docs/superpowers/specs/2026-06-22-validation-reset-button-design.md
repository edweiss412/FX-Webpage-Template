...
- **Gate `enabled=false`** (validation env but DB gate not provisioned out-of-band) → server action proceeds to RPC; RPC raises `destructive reset not enabled`; action maps to a typed `reset_not_enabled` code with calm copy. Buttons may render (env says validation) but the action fails safely with a clear message.
...
- **Gate-table prod-safety test (real-DB)** (`tests/db/destructiveResetGate.test.ts`): as an `authenticated` admin session (JWT with `is_admin()` true), assert a direct PostgREST `update`/`select`/`insert` on `public.destructive_reset_gate` is REJECTED (no DML grant) — i.e. an admin CANNOT flip `enabled=true` through a runtime session; only a service-role/out-of-band path can. Then assert `reset_validation_data()` raises while the gate is `false`. This is the regression that proves the R7 critical (admin-mutable marker) is closed.
