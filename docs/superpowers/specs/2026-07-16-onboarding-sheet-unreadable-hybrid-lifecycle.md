...
2. **`resolveUnreadableAlertIfHealed(input)`** — cron observer, called from the tick epilogue (between the file-loop end at `runScheduledCronSync.ts:~3881` and `finishCompletedRun` at `:3883`), fail-open (an `infra_error` or thrown dependency NEVER fails the tick; forensic `log.warn` only). Evaluation:
   ...
   d. Else, for EVERY `id` in `context.failed_drive_file_ids`, at least one of:
      - **removed** — `id ∉ listedDriveFileIds` (the tick's own listing, `runScheduledCronSync.ts:3733`);
      - **registered** — a `public.shows` row with `drive_file_id = id` exists;
      - **staged** — a `public.pending_syncs` row with `drive_file_id = id` exists (row existence IS the open-staged signal; the table has no status column — rows are deleted on apply/discard, DDL `supabase/migrations/20260501001000_internal_and_admin.sql:138-179`).
      All satisfied → resolve. Any id still present-in-folder AND unregistered AND unstaged → leave open (still genuinely failing).
   e. The resolve UPDATE is CAS-guarded against a concurrent scan replacing the context: `... AND resolved_at IS NULL AND context->'failed_drive_file_ids' = $observed::jsonb`. A lost CAS is a no-op (next tick re-evaluates fresh truth).
...
| `context.folder_id` missing/malformed (not a string) | Treat as mismatch → resolve (row is unidentifiable — stale by definition; forensic `log.warn` with code). |
| `context.failed_drive_file_ids` missing, not an array, or empty array | Do NOT resolve; forensic `log.warn` (fail-open toward keeping the alert visible; an empty list on an open row is a producer bug, not proof of health). |
...
| Any DB/list dependency throws | `infra_error` / forensic log; tick and scan response unaffected. |
| CAS conflict on UPDATE | No-op; next tick re-evaluates. |

**Telemetry (invariant 10 posture):** each successful resolve emits a forensic durable `log.info` with `code: "ONBOARDING_ALERT_AUTO_RESOLVED"` (`source`: `"admin.onboarding.scan"` or `"cron/sync"`), post-commit, best-effort. Forensic app_events-only code, NOT a §12.4 row (precedent: `ADMIN_ALERT_RESOLVE_FAILED` at `app/api/admin/admin-alerts/[id]/resolve/route.ts:158`, "Plain structural API code (not a §12.4 row)"). Not `REPORT_*`-prefixed (report-code namespace scanner). The scan route is already a registered `AUDITABLE_MUTATIONS` surface (`tests/log/_auditableMutations.ts:158`); the cron library path is not a mutation-surface unit (not a route/action file) — the `log.info` code satisfies the non-admin instrumentation rule for the new write.
...
Regens: `pnpm gen:spec-codes` (copy edit). `pnpm gen:internal-code-enums` + commit (`x2` gate) — required because the emit call's context shape changes are irrelevant to it but the NEW forensic code `ONBOARDING_ALERT_AUTO_RESOLVED` and any extractor-visible changes to the `upsertAdminAlert` call site must land regenerated. No new §12.4 code rows → no x-gate family fan-out beyond x1/x2.
...
3. **Cron predicate matrix** (DB-backed, each id state derived from seeded rows): all-removed / all-registered / all-staged / mixed-satisfied → resolved; one-still-failing → NOT resolved; folder-mismatch → resolved regardless of ids; wizard-pending → untouched even when all ids satisfied; empty/missing ids array → untouched; CAS conflict (context swapped between read and update) → untouched. Assert on `resolved_at` in the DB row, never on "helper was called".
