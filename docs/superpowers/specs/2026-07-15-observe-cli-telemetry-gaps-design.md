...
- Ordering: none guaranteed by table; order by `show_id` for determinism. Warning-free rows are excluded DB-side and BEFORE the row cap: `.not("parse_warnings->0", "is", null)` (same first-element-exists predicate as §2.1 — excludes empty arrays, NULLs, scalars, and objects before the cap; false-empty-page and malformed-row-cap-consumption both closed).
...
- Output row: `occurred_at  drive_file_id  status  warning_count  duration_ms  message(sanitized, truncated)`. `sync_log.status` is unconstrained text in the DDL → routed through `sanitizeIdentityString` (§5.2 class C — lossless for real values like `watermark`/`deferred_permanent`, token-proof for garbage; no enum exists for this column).
...
1b. The mapped-credential assignment is one exported chokepoint function — `applyResolvedTarget(target)` in `scripts/observe/env.ts` — which assigns `process.env.SUPABASE_URL` / `process.env.SUPABASE_SECRET_KEY` from the resolved pair (no-op when the target carries no mapped pair). BOTH query-issuing paths call it after `resolveTarget` and before their first query: the normal command path AND `runTailFollow` (`scripts/observe.ts:177` — a separate entry-loop outside `runObserve` that would otherwise silently skip the mapping and read the wrong target under a validation label, Codex R5 F2).
2. `resolveTarget` gains validation auto-mapping: when `--env validation` and `ambient.VALIDATION_SUPABASE_URL` is set (non-loopback) and `ambient.VALIDATION_SUPABASE_SECRET_KEY` is set, resolve to those values — but ONLY after the pair passes the existing validation target binding (Codex R2 F1: a stale/mistargeted `.env.local` pair must not silently read another hosted project while presenting as validation). Specifically, the auto-map path calls `assertProdEquivalentTarget(url, false)` (`scripts/lib/validation-target.ts:38` — refuses local, empty, plain-`http`, branch-preview `<ref>--<branch>.supabase.co` hosts) and `assertSupabaseTargetMatchesProjectRef(url, ambient.VALIDATION_SUPABASE_PROJECT_REF)` (`scripts/lib/validation-target.ts:83` — requires `VALIDATION_SUPABASE_PROJECT_REF` and that the URL host's project ref matches it; the var already exists in `.env.local`). Both throw; `resolveTarget` catches and converts to `{ kind: "error", message }`. On pass, the return type gains the resolved pair: `{ kind: "ok"; envName; url?: string; key?: string }`; the entry assigns them to `process.env.SUPABASE_URL` / `process.env.SUPABASE_SECRET_KEY` before any query runs (`createSupabaseServiceRoleClient` reads `process.env` at call time, `lib/supabase/server.ts:79-80`). A binding failure is an ERROR (exit 1), NOT a silent fall-through to the ambient path — fail loud over fail wrong-target.
3. `--env validation` is `VALIDATION_*`-ONLY — there is NO ambient fallback (Codex R3 F2: an ambient fallback lets a stale exported prod/branch `SUPABASE_URL` masquerade as validation, and with `--reveal-email` that is a wrong-project PII read presented under a validation label). The full triple (`VALIDATION_SUPABASE_URL` hosted-https non-branch-preview, `VALIDATION_SUPABASE_SECRET_KEY`, `VALIDATION_SUPABASE_PROJECT_REF` matching the URL) is required; ANY missing or invalid member is a hard error naming the exact failure. Ambient-remote targeting remains available exclusively via `--env prod` (unchanged). This is a deliberate behavior change: today `--env validation` accepts any non-loopback ambient `SUPABASE_URL` (`scripts/observe/env.ts:28-38`); after this change that invocation shape errors with instructions to either populate `VALIDATION_*` in `.env.local` or use `--env prod` for an explicit ambient target. The AGENTS.md observe section is updated accordingly (§8).
...
Guard conditions (`--env validation`; every failure is a hard error, exit 1, NO fall-through of any kind): `VALIDATION_SUPABASE_URL` absent → error `--env validation requires VALIDATION_SUPABASE_URL in .env.local (use --env prod for an explicit ambient remote target)`. Set but loopback → error (a loopback validation target is a misconfiguration, not a signal to try something else). `VALIDATION_SUPABASE_SECRET_KEY` absent → error naming it. `VALIDATION_SUPABASE_PROJECT_REF` absent or not matching the URL host → error naming it. Branch-preview host or plain-`http` URL → error (via `assertProdEquivalentTarget`).
...
### 5.0 Emitted-column trust classification (exhaustive)

Structural closure of the recurring redaction vector (R2–R5 each found one more unclassified emission): EVERY column any of the six commands emits is classified below; the classification is the implementation contract and each class has a fixed treatment. A column not in this table must not be emitted.

| Class | Treatment | Columns |
| --- | --- | --- |
...
One serializer, `serializeParseWarning(raw: unknown, opts: { includePii: boolean })` in `lib/observe/query/`, is the ONLY way a warning element reaches any output path (table, `--full`, `--json`, every command). It ALLOWLISTS, with runtime shape VALIDATION on every pass-through field (Codex R2 F2: jsonb is untrusted — a corrupted element could carry an email/token/control chars in `code` or `field`, bypassing the sanitizer):

- `severity` → emitted raw ONLY if strictly `"info"` or `"warn"` (`ParseWarning` enum, `lib/parser/types.ts:8`); anything else → `""`
- `code` → emitted raw ONLY if it is a member of `INTERNAL_CODE_ENUMS` whose payload `source` includes `parse_warnings.code` (`lib/messages/__generated__/internal-code-enums.ts:7` — the generated, CI-enforced finite enum; 41 entries carry that source). Membership alone is too broad: the enum also carries `admin_alerts.code` / `pending_ingestions.last_error_code` / `shows.last_sync_status` entries, and a corrupted warning element must not render an unrelated domain's code as a parse warning (Codex R7 F1). Anything else → `""`. Regression test: an enum member from another source (e.g. an admin-alert-only code) in a warning's `code` field is emptied, not emitted. Finite-enum membership, not a shape regex — a regex like `/^[A-Z][A-Z0-9_]{0,63}$/` still admits 24+-char token-shaped strings (Codex R3 F1), and legit long codes (e.g. `STAGED_PARSE_OUTDATED_AT_PHASE_D`, 32 chars) themselves match the TOKEN hazard pattern, so routing codes through the sanitizer would corrupt them — enum membership is the only validator that is simultaneously token-proof and lossless for real codes.
- `message` → `sanitizeIdentityString` (unconditional — free text by contract)
- `iso` → emitted ONLY if it matches `/^\d{4}-\d{2}-\d{2}$/` (fixed 10-char shape — structurally below the 24-char TOKEN floor); else omitted
- `field` → emitted ONLY if it matches `/^[a-z][a-zA-Z0-9_.-]{0,22}$/` (max length 23, strictly below the TOKEN pattern's 24-char floor — a passing value structurally cannot be token-shaped; real fields are short: `"dims"`, `"name"`, `"guests"`); else omitted
...
| `--since` not one of `1h\|24h\|7d\|all` (new commands) | CLI error, exit 1: `--since must be 1h\|24h\|7d\|all` — the existing `sinceToHours` maps unknown values to 24h (`scripts/observe/args.ts:37`), which on a forensic command silently narrows `--since 30d` to a 24h window (false negatives during incident response — Codex R5 F1). Existing commands keep the 24h-fallback posture unchanged. |
...
- **Args tests**: comma-list split for `--code`/`--source` (single → singular field, multi → plural, empties dropped); new command flags; `--reveal-email` plumbing into staged/deferred filters; **fail-closed regression cases (Codex R4 F1)**: `staged --session not-a-uuid` (with and without `--reveal-email`), `warnings --show not-a-uuid`, `synclog --file ""`, `failures --code <201 chars>`, `staged --since 30d` (invalid `--since` token — Codex R5 F1) — each exits 1 with a flag-naming error and NO query executes (assert the query function is never called); existing `events --show not-a-uuid` and `events --since 30d` still silently fall back (posture boundary pinned).
- **Entry-boundary env tests (Codex R5 F2)**: beyond `resolveTarget` unit tests, assert the mapping is APPLIED at the query boundary — inject a fake query dep that records `process.env.SUPABASE_URL`/`SUPABASE_SECRET_KEY` at call time and assert both equal the `VALIDATION_*` values before the first query for (a) a normal command path and (b) the `tail --follow` loop path (both must call `applyResolvedTarget`; a structural assertion that `runTailFollow`'s source calls it before its first `collectEvents`/`queryEvents` is acceptable in lieu of executing the infinite loop).
...
- AGENTS.md "Telemetry access (observe CLI)" command table updated in the same PR (it enumerates commands/flags and currently lists 6 commands; after this change 12), AND its "`--env` guardrail" paragraph rewritten for the new `--env validation` contract (`VALIDATION_*`-only, no ambient fallback; ambient remote = `--env prod` only).
