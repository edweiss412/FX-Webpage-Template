...
Guard conditions: `VALIDATION_SUPABASE_URL` set but loopback → treated as absent (falls through to ambient path — loopback means the operator's `.env.local` doesn't carry a hosted validation target). `VALIDATION_SUPABASE_URL` absent, or `VALIDATION_SUPABASE_SECRET_KEY` absent → falls through to ambient path (and the ambient path's existing error message fires if ambient is also unusable; error text extended to mention the `VALIDATION_*` option: `--env validation requires VALIDATION_SUPABASE_URL + VALIDATION_SUPABASE_SECRET_KEY (+ matching VALIDATION_SUPABASE_PROJECT_REF) in .env.local, or a non-local SUPABASE_URL`). `VALIDATION_SUPABASE_URL` set, non-loopback, but binding fails (missing/mismatched `VALIDATION_SUPABASE_PROJECT_REF`, branch-preview host, plain http) → hard error, no fall-through.
...
- **Env tests** (`resolveTarget`): validation auto-map matrix — full valid triple (URL + secret + matching ref, hosted https) → ok+mapped, wins over ambient; URL/secret missing → ambient fallback; both missing + ambient loopback → error mentioning `VALIDATION_*`; **binding failures are hard errors, never fall-through**: missing `VALIDATION_SUPABASE_PROJECT_REF`, mismatched ref, branch-preview host (`<ref>--<branch>.supabase.co`), plain-`http` URL; prod path unchanged; local path unchanged (non-loopback ambient still refused without `--env`).
