...
| `tests/docs/_metaCapabilityProseClaims.test.ts` | **New.** Prose pins giving the documentation-only tasks a genuine RED — one case per false sentence (instances B, C, D, E) |
...
| AC-5a | Instance C's false mechanism claim is gone and pinned | `tests/docs/_metaCapabilityProseClaims.test.ts` asserts `lib/visibility/capabilityTransitions.ts` no longer promises a "TypeScript error if the matrix is incomplete"; RED before the edit |
