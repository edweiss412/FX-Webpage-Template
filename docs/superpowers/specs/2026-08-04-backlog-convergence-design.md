...
3. Unsized entries are invisible to mass (reported by count only). 42 at spec time (before the DEFERRED re-heading surfaces three more); the grandfather registry makes the number ratchet-only from its capture moment.
...
- **AC-B2:** `tests/docs/_metaLedgerSizing.test.ts` fails on a planted unsized new entry in a scratch ledger fixture, names it by id, and passes on the real tree with the implementation-time grandfather registry (§3.3 capture rule; 42 ids at spec time before the DEFERRED re-heading).
