...
### 5c. Manual-accept semantics (what "re-sync" actually applies)

Manual re-sync applies the sheet's **current** Drive state — NOT a byte-frozen snapshot of the reviewed parse. This design deliberately does not persist the held parse (that is the deferred staged-review UI, §13), so there is nothing to "apply exactly." This is the **pre-existing** `runManualSyncForShow` contract (`mode: "manual"` re-fetches + applies current, overriding the cron hold). Three things make this safe and honest:

1. **The alert `detail` is kept fresh by cron.** Each cron pass re-parses; if the sheet still materially shrinks, it re-upserts the alert (dedupe replaces `context` → `detail` reflects the latest evaluation). So the summary the admin reads is at most one cron interval stale.
2. **A still-broken current sheet still fails closed.** Manual mode skips only the *shrink* hold, NOT the hard-fail path — an unparseable/`MI-1..5b` current sheet still retains last-good (`hard_fail`), so "accept" can never apply garbage. The only thing manual override applies is a *materially-shrunk-but-valid* parse, which is exactly what "accept this shrink" means.
3. **Honest framing.** Alert copy and the `ReSyncButton` post-sync result summary describe applying the *current* sheet, not "the flagged bytes." Context stores `held_modified_time` (the `modifiedTime` that triggered the hold) for provenance/observability.

Byte-exact "apply exactly the version I reviewed" requires persisting the parse + a diff review surface — that is `BL-RESYNC-STAGED-REVIEW-UI` (§13), deliberately deferred by the owner.
...
8b. **Manual accept applies current + fails closed on hard-fail.** (a) hold a show; then manual re-sync with a still-shrunk current sheet → applies (accept), alert resolves. (b) hold a show; then manual re-sync with a *hard-fail* (MI-1) current sheet → `hard_fail` retain-last-good (NOT applied), alert path unchanged. Catches: manual "accept" applying garbage (R8-1).
