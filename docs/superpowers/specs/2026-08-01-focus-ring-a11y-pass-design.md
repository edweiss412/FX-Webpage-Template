...
1. **Valid literal companion, same variant chain:** a `ring-offset-<token>` on the same line whose variant prefix matches the offset's and whose `<token>` is a member of the guard's **backdrop allowlist** — a named constant in the test file enumerating the backdrop-family tokens (`surface`, `surface-raised`, `surface-sunken`, `bg`, `warning-bg`, `info-bg`, `stale-tint`, `accent-tint`, `danger-bg`; extend by adding a row). NOT the whole `@theme` `--color-*` namespace: `ring-offset-text` compiles but is a text token, not a backdrop — membership in the full color namespace does not establish a backdrop (round-2 probe). `ring-offset-garbage` fails the same way. Arbitrary values `ring-offset-[…]` are accepted ONLY when the bracket content is color-shaped (`#…`, `rgb`/`oklch` functional, or `var(--color-…)`): Tailwind emits `--tw-ring-offset-color:garbage` verbatim for `ring-offset-[garbage]` (round-2 probe), so bracket syntax alone proves nothing.
...
| Offset sweep | planned tests/styles/noBareRingOffset.test.ts (§4.3): backdrop-allowlist companion + variant-chain match + color-shaped arbitrary values + file-scoped indirection registry, filesystem-walked | New bare `ring-offset-2`; non-backdrop token (`ring-offset-text`) or non-emitting spelling (`ring-offset-garbage`); non-color arbitrary value (`ring-offset-[garbage]`); companion under the wrong variant prefix; new unregistered indirection file |
