...
| The info-bg conflict resolves by **nudging the light info-bg token** (`#EEEAE3` → `#F1EDE7`, §3.1) — not by re-skinning the ReSync success overlay, not by changing the ring, not by excluding the token. Fenced both directions: do not propose overlay re-skins, and do not re-open the exclusion. | §3.1 measurement + consumer audit; round-4 review probe |
...
A second, companion token change ships with it: the **light** `--color-info-bg-runtime` nudges `#EEEAE3` → `#F1EDE7` so the ring clears the floor on info-bg surfaces (§3.1 carries the measurement, rationale, and consumer audit). The DESIGN.md §1.1 info-bg row updates in the same commit.
...
### 3.1 The info-bg resolution (round-3/round-4 review arc)
