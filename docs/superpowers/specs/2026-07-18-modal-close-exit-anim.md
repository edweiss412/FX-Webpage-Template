...
**Ownership, minimally.** The deterministic window is short, but a slow transition still permits the user to open another surface. `restoreOverlay` is therefore skipped if **another visible `role="dialog"`/overlay root is present** — one narrow check, not the R24/R25 activity machinery. Crucially there is **no user-action flag**, so neither `releaseOverlay`'s programmatic focus restore (R25 medium) nor a user's retry click (R25 high) can suppress recovery.
