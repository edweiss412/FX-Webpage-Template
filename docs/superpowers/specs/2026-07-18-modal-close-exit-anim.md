...
- **`restoreOverlay()` — the exact inverse of the five `releaseOverlay` duties (§3.1a):** clear `dialogRef.style.display`; re-apply the body scroll lock; re-apply background `inert`/`aria-hidden` via the saved `inertPrevRef` snapshot; un-gate the document Esc handler; and reset `dismissingRef`/`overlayReleasedRef` to `false` so the modal is fully interactive and closable again. Focus returns to the panel's initial-focus target, as on a normal open.
...
| **Skeleton (`closeBehavior="none"`): scrim tap / Esc / grab-tap / drag** | nothing happens — `requestClose` returns immediately, no `beginDismiss`, no `inert`, no animation, no `onClose`. Byte-identical to `main`. Without this the frame would animate off-screen and go inert while scroll stays locked (§3.1a; Codex R22 high) |
...
5. **Real-browser (`published-review-modal.interactions.spec.ts`)** — groups:
   ...
   (i) **failed-close bounded rollback** (R21, §3.1d) — three cases on the loaded Published path: (a) the `/admin` close response is **aborted/errored**; (b) it is left **indefinitely pending**; (c) the browser is **offline**. In each, after `CLOSE_NAV_TIMEOUT_MS` assert `restoreOverlay()` fully reversed the release: modal visible again, body scroll re-locked, `[data-inert-root]` re-inerted, Esc handler live, and **dismissing again works** (retries the navigation). Anti-tautology: assert the URL still reads `show=<slug>` at rollback time — if the close actually committed, the failure path was never exercised; and assert the modal is genuinely interactive (activate a control), not merely present in the DOM.
...
| tests (§9) | shell unit, ModalCloseButton unit, skeleton-unchanged regression, interactions spec (13 groups: a–i, i2 + j1–j4), transition-audit |
