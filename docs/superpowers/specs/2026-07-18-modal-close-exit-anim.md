...
| footer action resolves AFTER exit-end (shell unmounted, `closeApiRef` cleared) | `closeApiRef.current?.()` is a no-op — a resolution arriving after the modal is gone closes nothing. No second `onClose`, no duplicate `router.push` (§3.1a, §7.5(h)) |
...
5. **Real-browser (`published-review-modal.interactions.spec.ts`)**:
   ...
   (h) **resolution arrives AFTER exit-end (§3.1a, Codex R5)** — the slower-than-exit case: start Publish (and separately Approve & apply / Ignore) with a resolution delayed past the full exit duration, dismiss, let the exit complete and the shell unmount, *then* let the action resolve. Assert `onClose` fired **exactly once** (at exit-end) and that the late resolution produces **no second close** — on Published specifically, assert no second `router.push`, since that is what a resurrected fallback would emit. This is the case the withdrawn `?? onClose` fallback would have failed while (g) still passed, so it is the regression pin for that fallback ever returning.
