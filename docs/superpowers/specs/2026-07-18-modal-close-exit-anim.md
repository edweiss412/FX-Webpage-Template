...
| File | Change |
|------|--------|
| `components/admin/review/ReviewModalShell.tsx` | `requestClose` (+ drag-cancel), `closeBehavior?: "unmount"\|"navigate"\|"none"` prop, `scrimRef` + `dialogRef`, `releaseOverlay()` (body-scroll + background-un-inert + dialog-root `display:none` + Esc-gate + focus) + `overlayReleasedRef`, lift `previouslyFocusedRef`/`scrollLockPrevRef`/`inertPrevRef` from effect locals to refs + `restoreBackgroundInert()` helper, document-Esc `onKeyDown` early-returns on `overlayReleasedRef`, mode-aware close-commit shared by `requestClose` AND the drag-past-threshold branch (§3.1b), `ReviewModalCloseContext` + `useReviewModalClose`, provider wrap; scrim/Esc/grab → `requestClose`; `handleGrabPointerEnd` early-return on `dismissingRef` |
