...
**Comprehensive close-state inverse table (Codex R21–R24 — four consecutive rollback defects).** R21 (deferral that was really a regression), R22 (missing ownership check), R23 (incomplete state reversal), and R24 (ownership check too narrow) were all defects in this one contract, each found by patching the previous one. Per AGENTS.md the vector gets enumerated rather than patched again. **Every** piece of state the close path mutates, and rollback's obligation for each — this table is the contract; any future edit re-derives it:

| # | State mutated by close | Mutated by | Rollback obligation |
|---|------------------------|-----------|---------------------|
| 1 | `dismissingRef` → `true` | `beginDismiss` | reset to `false` |
| 2 | `dialogRef.inert` → `true` | `beginDismiss` | **clear** (missed in R23) |
| 3 | panel `animation`/`transform`/`opacity`/`transition` | step 5 | clear to open-state rest (missed in R23) |
| 4 | scrim `animation`/`transition`/`opacity` | step 5 / §3.2 | clear to open-state rest (missed in R23) |
| 5 | `dragRef` → `null` | step 2 | none — correct as-is (no drag in flight) |
| 6 | `settleTimerRef` cleared | step 2 | none |
| 7 | exit-end fallback timer scheduled | step 5 | already fired or cleared on release |
| 8 | `onClose()` → `router.push` fired | §3.1a (navigate) | **NOT reversible** — the push already went out. This is precisely why rollback exists: the push failed, so the URL still reads `?show=`, and restoring the modal reconverges UI with URL |
| 9 | `overlayReleasedRef` → `true` | `releaseOverlay` | reset to `false` |
| 10 | `document.body.style.overflow` restored | `releaseOverlay` | re-apply the scroll lock |
| 11 | `[data-inert-root]` inert + `aria-hidden` restored | `releaseOverlay` | re-apply from `inertPrevRef` |
| 12 | `dialogRef.style.display` → `"none"` | `releaseOverlay` | clear |
| 13 | document Esc handler gated off | `releaseOverlay` (via #9) | live again once #9 resets |
| 14 | focus moved to trigger | `releaseOverlay` | move to the panel's initial-focus target |
| 15 | `CLOSE_NAV_TIMEOUT` timer scheduled | §3.1d | fired; clear the handle |

**Rollback must not reclaim what another surface now owns (Codex R22/R24).** The released window is exactly when the user can open **another** modal or lightbox (§3.1b, §4) — that surface then owns `document.body.style.overflow` and `[data-inert-root]`. If the close ALSO stalls, an unconditional `restoreOverlay()` would redisplay the stale review modal, re-lock scroll, re-inert the background, and steal focus **on top of the newer overlay**. That compounds two in-scope cases and breaks latest-user-action ownership. Contract — an ownership check, extending the R11 single-token principle to the rollback direction:

A globals-only check is **too narrow (Codex R24)**: real surfaces in this repo — `AppHealthPopover`, `BellPanel`, `ReportModal` — present dialog/overlay UI via `useDialogFocus` **without** touching `document.body.style.overflow` or `[data-inert-root]`. Close Published, open one of those in the released window, let the timeout elapse, and the globals still match, so rollback would redisplay and focus the stale review modal over the surface the user is actively using. The guard must therefore test *ownership of the screen*, not ownership of two specific globals. **Rollback proceeds only if ALL of these hold:**

1. **Globals unchanged** — `document.body.style.overflow` and the `[data-inert-root]` inert/`aria-hidden` state still match what `releaseOverlay` left them as.
2. **No other overlay is present** — no other visible `role="dialog"` (or known overlay root) exists in the document.
3. **The user has not acted since release** — no `pointerdown`, `keydown`, or `focusin` has occurred after `releaseOverlay` ran. This is the general form the first two are special cases of: if the user has done anything at all, the screen is theirs.

If any check fails, **skip the rollback entirely** and stay hidden. Condition 3 is deliberately the broad, cheap catch-all — it does not require enumerating overlay implementations, so a surface added later is covered by default rather than becoming R25.

**This is deliberately conservative, and that is the correct bias.** It means a user who idly clicks the dashboard during a stalled close does not get the modal back, and is left with a stale `?show=` until the next navigation or refresh — an inert, self-healing inconvenience on a page that remains fully usable. The alternative bias (always recover) risks yanking focus and re-locking scroll out from under someone actively working in another surface, which is a far worse failure. Rollback exists for the *unattended* stall — user dismisses, nothing lands, nothing else happens — and that is exactly the case condition 3 preserves.
...
| **Published (`navigate`): close stalls and the user has done ANYTHING since release** (opened any overlay — including ones that do not touch body-scroll/inert like `AppHealthPopover`/`BellPanel`/`ReportModal` — or merely clicked/typed/focused) | rollback is SKIPPED. Ownership requires all three: globals unchanged, no other overlay present, AND no user action since release. Deliberately conservative — never steal the screen from an active user; the stale `?show=` self-heals on next navigation (§3.1d; Codex R24 high) |
...
5. **Real-browser (`published-review-modal.interactions.spec.ts`)** — groups:
   ...
   (i2) **compound: stalled close + newer overlay → rollback SKIPPED** (R22, §3.1d) — with the route delayed, close Published and wait for `releaseOverlay`; open ANOTHER modal/lightbox in the released window; then stall the close past `CLOSE_NAV_TIMEOUT_MS`. Assert `restoreOverlay` did NOT run: the stale review modal does not reappear, the newer overlay keeps its scroll lock / `[data-inert-root]` inert / focus, and nothing is stolen. Anti-tautology: assert the rollback timer genuinely elapsed (advance past 8000ms and confirm the shell is still mounted), so the case cannot pass merely by the timeout never firing. This is the compound of (h) and (i) — neither alone catches it. **Run it for TWO surface classes (R24):** (i2a) a surface that DOES take the globals (another review modal / lightbox), and **(i2b) one that does NOT** — `AppHealthPopover`, `BellPanel`, or `ReportModal`, which present overlay UI via `useDialogFocus` without touching body overflow or `[data-inert-root]`. (i2b) is the case a globals-only ownership check passes while stealing the screen; (i2a) alone would not catch it. Additionally **(i2c)**: no other overlay at all, but the user simply **clicks somewhere on the dashboard** during the stall — assert rollback is skipped (ownership condition 3), proving the guard is user-action-based and not merely overlay-detection.
