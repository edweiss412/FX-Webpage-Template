...
None changed. The panel's `max-h`/`max-w` and internal flex column (`ReviewModalShell.tsx:400-457`) are untouched; exit sets only `transform`/`opacity`/`transition`/`animation` inline on the panel + scrim, and `releaseOverlay` sets `display:none` on the dialog root — all cleared by unmount.
...
| tests (§9) | shell unit, ModalCloseButton unit, skeleton close, interactions spec (7 groups a–g), transition-audit |
