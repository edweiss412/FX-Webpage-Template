...
The shell's `onClose` prop is unchanged: consumers still pass their raw close (`close` / parent-unmount `onClose`). The shell derives `requestClose` from `onClose` internally. `PublishedReviewModal` passes `closeBehavior="navigate"` (§3.1a); `Step3ReviewModal` passes nothing (defaults `"unmount"`); `ShowReviewModalSkeleton` also passes nothing — it unmounts instantly via `onDismissStart` + `ShowReviewModalSlot`, so it never reaches an exit-end and `closeBehavior` is moot (§3.4). All three render a `ModalCloseButton`.
...
- The skeleton is already a client component, so it calls `useShowModalNav()` itself and passes `onClose={close}` — its Esc/scrim/grab/X now close the modal (strip `?show`), so a slow-streaming load is never a trap.
...
|---------------|----------|
| skeleton Esc/scrim/grab/X during a slow Suspense load | closes the modal (`useShowModalNav().close`); `onDismissStart` → `ShowReviewModalSlot` renders `null` → **instant** unmount, shell unmount cleanups restore scroll/inert/focus synchronously — no stuck-loading trap, no lingering window (§3.4; resolves MODAL-SKELETON-CLOSE-1) |
...
5. **Real-browser (`published-review-modal.interactions.spec.ts`)** — groups:
   ...
   (g) **skeleton close during slow load** (R5 high, §3.4) — open the modal so the skeleton mounts (delayed loader), press Esc / tap scrim / click the skeleton's X; assert the frame unmounts immediately, `?show` is stripped, and scroll/inert/focus are restored — the loading frame is never a trap.
...
| `components/admin/showpage/ShowReviewModalSkeleton.tsx` | real close: `useShowModalNav()` + `onClose={close}` + `onDismissStart={reportDismissed}` (from `ShowModalDismissContext`; no `closeBehavior` — instant unmount via the slot, §3.4); placeholder close block → `<ModalCloseButton>` on a `closeRef`/`initialFocusRef`; **restructure the title row so the real close button is NOT under the `aria-hidden="true"` wrapper (`:48`) — only decorative loading blocks stay `aria-hidden`** (Codex R7 medium) (resolves MODAL-SKELETON-CLOSE-1, §3.4) |
| `components/admin/showpage/ShowReviewModalSlot.tsx` | **new** client component above the Suspense boundary: `dismissed` state + `ShowModalDismissContext` (`reportDismissed`); renders `null` in place of the whole subtree once dismissed, so a late-resolving `ShowReviewModal` never mounts (Codex R8 high, §3.4) |
