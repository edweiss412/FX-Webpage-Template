...
6. **Repeats distinguishable:** any two consecutive announcements, identical text or not, must be perceivable as separate utterances by a screen reader. Mechanism, revised at the impeccable gate: the repo's shared append channel (`components/admin/announceLog.tsx`, `role="log"` with keyed additions), which DESIGN.md §15 mandates whenever the same message text can legitimately recur and which this diff's two-message-kinds region is exactly the trigger for. That replaces the sequence counter and trailing-U+00A0 parity suffix an earlier draft of this spec described. The channel is pruned only at a cycle boundary (no retry in flight), so taps inside one cycle stay distinct nodes while a settled cycle does not accumulate for the visit. The existing pinned contract "a second activation is distinguishable from the first" (`telemetryRetryButton.test.tsx`, case 3) must keep passing. Where the parity is load-bearing: only between two announcements that share their text, which for this control means two intents in a row (a double tap). An outcome clears the baseline, so a second outcome needs an intervening tap, and §4 pins the intent and outcome strings apart; the counter is therefore written on both sides for symmetry while doing its work at the tap.
