...
6. **Repeats distinguishable:** any two consecutive announcements, identical text or not, must be perceivable as separate utterances by a screen reader. Mechanism, revised at the impeccable gate: the repo's shared APPEND channel (`components/admin/announceLog.tsx`, `role="log"` with one keyed child per message), which DESIGN.md §15 mandates whenever the same message text can legitimately recur, and this region's two message kinds are exactly that case. An identical text change may not re-announce; an identical addition always does. This replaces the sequence counter and trailing-U+00A0 parity suffix an earlier draft of this spec specified, whose behaviour varies by assistive technology. The channel is pruned only at a CYCLE BOUNDARY (a tap with no retry in flight), so taps inside one cycle stay separate nodes while a settled cycle does not accumulate for the visit. The existing pinned contract "a second activation is distinguishable from the first" (`telemetryRetryButton.test.tsx`, case 3) must keep passing.
7. **Tap while an outcome is pending** (double-tap before the refresh lands): announce intent again (a second keyed addition, which is what makes the repeat audible), and re-record `baseline` from the current `renderedAt`. The cadence is one outcome per settled CYCLE, not one per tap and not one per re-read: the first differing render answers the armed baseline and disarms it, so if a double tap provokes two server responses the second is silent, having nothing outstanding to answer. Its answer would be the same answer.
...
Region states, by what the log holds: {empty, intent last, outcome last}. N=3, so three pairs:
...
2. **A re-read that never lands is silent, and stays silent.** The announcement fires on a NEW server render; if `router.refresh()` never produces one (the network or the server itself is down, which is the failure most likely to co-occur with this surface), no outcome is ever announced. Raised as a P1 at the impeccable gate with a timer as the proposed repair, and declined for the reason the row's own probes already give: announcing "still couldn't load" on a timeout asserts an outcome nobody observed, which is the same dishonesty as a timer-driven `aria-busy`. The worst case here is exactly today's shipped behaviour, never a wrong announcement. The related misattribution risk is NARROWED, not closed, and the earlier draft of this line overclaimed it (whole-diff review round 1, finding 3). The baseline stays armed for as long as the fallback is mounted, so an auto-refresh landing minutes after a tap does append an outcome to that tap's intent. What that outcome says remains TRUE at the moment it is said, since a server re-read did complete and this branch is still failing, which is the whole of the claim; what is not bounded is how long the user has to hold the tap in mind for the pairing to read naturally. Cycle-boundary pruning bounds the region's accumulated TEXT (§3.7), never the baseline's lifetime.
3. **Frozen-clock capture environment:** under `ENABLE_TEST_AUTH` with a valid frozen-now header (`lib/time/now.ts:36-48`), `renderedAt` is stable across renders and the outcome never fires. Screenshot-capture-only environment, gated by a bearer secret; the feature is inert there by construction.
4. **Outcome text does not say WHY.** The failure cause is the plate's own paragraph; the announcement settles the outcome, not the diagnosis. Sibling row `DIAGRAM-PLACEHOLDER` (DEFERRED.md) owns the why-half on its surface.
5. **No pending/`aria-busy` state.** Deliberate (§1.1). The baseline mechanism would support an honest one; whether to add it is a future product call, not this row.
6. **Indirect component usage** stays outside the census, unchanged (its header's documented limit and re-file trigger).
7. **Success remains silent** (unmount, §1.1).
...
- AC-6: Consecutive announcements are pairwise perceivable, each being its own keyed addition to the log region; the existing "second activation is distinguishable" case keeps passing.
