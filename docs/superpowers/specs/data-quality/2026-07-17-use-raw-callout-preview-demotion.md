...
**Core behavioral proof — TWO fixtures are mandatory (one per removed mount).** The two callout mounts gate on **different** warning codes, so a single fixture cannot prove both are gone:
- `UseRawControlBoundary` always renders `<UseRawControl>` (`components/admin/UseRawControlBoundary.tsx:85`); the inner `UseRawControl` self-hides (renders null) unless the warning is an in-scope structural-transform code (e.g. `roomSplitWarning` → `ROOM_HEADER_SPLIT_AMBIGUOUS`, fixture `:395`). Either way, removing the callout's `UseRawControlBoundary` mount removes the whole subtree, so the `use-raw-control-callout` testid disappears.
- `RoleRecognizeControlBoundary` returns `null` unless `warning.code === "UNKNOWN_ROLE_TOKEN"` with a non-blank `roleToken` (`components/admin/RoleRecognizeControlBoundary.tsx:51`); fixture `roleWarning(token)` (`:75`).

So a use-raw fixture ALONE (the room-split fixture the existing `:369-408` test uses) would leave the recognize-role callout mount unproven — the boundary was already `null` for it. The proof MUST render the callout host **twice** (or with both entries), JSDOM (see note), asserting:
- **Use-raw fixture** (`roomSplitWarning`): callout box (`wizard-step3-card-<dfid>-section-<sectionId>-flag-callout`) contains **zero** `use-raw-control-callout`; the list (`renderBreakdown`) still mounts `use-raw-control-list`. Failure mode caught: use-raw callout mount re-added.
- **Role-token fixture** (`roleWarning("SLED DRIVER")`, `UNKNOWN_ROLE_TOKEN`): callout box contains **zero** `role-recognize-trigger-callout` (and no `role-recognize-*-callout`); the list still mounts `role-recognize-trigger-list`. Failure mode caught: **recognize-role callout mount re-added** (the mode a use-raw-only fixture silently misses — Codex R2).
- Both fixtures: the callout still renders the entry title text + a "View details" jump button. Failure mode caught: over-stripping the preview (Option B regression).

Assertions scope to the **callout container** vs the **list container** separately (`within`), never a shared ancestor — the two sites emit site-scoped testids (`-callout` / `-list`, #454), so a container-scoped query cannot pass by matching the sibling.
...
- `:294-321` (callout role-recognize panel expand — renders a `roleWarning` `UNKNOWN_ROLE_TOKEN` at the callout and mounts `role-recognize-trigger-callout`/`role-recognize-panel-callout`) — **breaks on removal**. **Rework into the role-token half of the core proof**: assert the callout box contains **no** `role-recognize-trigger-callout`, while the list still mounts `role-recognize-trigger-list` for the same `roleWarning`. This is the mandatory role-token fixture from the core proof above — the panel-state behavior it previously proved is unchanged in the list (`RoleRecognizeControl.test.tsx:155-170,:279` covers the panel + stale/conflict notices at the control level).
...
- `:369-408` ("cross-site testid distinctness", room-split fixture) — **the use-raw half of the core proof.** `localCalloutHost` (`:372-392`) mounts the callout via chrome context WITH `useRawDecisions: []` + `wizardSessionId`, and `:405` asserts `within(box).getByTestId("use-raw-control-callout")` is truthy. After demotion the callout mounts **no** control → **invert `:405` to expect `queryByTestId("use-raw-control-callout")` to be null**, keep the list assertion (`:399-400`). NOTE this fixture is `roomSplitWarning` (use-raw only) — it does NOT by itself prove the recognize-role mount is gone (that boundary was already `null` for a room-split code); the role-token proof lives in the reworked `:294-321` above. The `localCalloutHost` chrome value at `:383-384` still passes `wizardSessionId`/`useRawDecisions` — after §4.5 removes those chrome fields, drop them from this fixture too (they no longer exist on `Step3SectionChrome`).
- `:147` ("every in-scope warning gets a use-raw control — beyond the callout cap") — about the **list**; unchanged.

**Other callout tests to sweep** (grep `flag-callout` / callout control testids): component — `tests/components/admin/wizard/step3ReviewSections.test.tsx`, `rawUnrecognizedCallout.test.tsx`, `Step3ReviewModal.test.tsx`, `step3ReviewModal.transitions.test.tsx`, `publishedNoStagedTraffic.test.tsx`; **e2e (Playwright)** — `tests/e2e/step3-review-modal.interactions.spec.ts:615` (targets the `flag-callout` container) and `tests/e2e/step3-review-modal.layout.spec.ts:421` (targets `flag-callout button` — the FIRST callout button; likely still "View details" and valid, but a button-index/count shift when the control buttons go must be checked, not assumed). The plan's pre-draft pass enumerates which assert callout controls (update to preview-only) vs. merely render the callout / the jump button (no change). Note: e2e specs are env-bound and excluded from `pnpm test` — run them explicitly in the plan's verification.
