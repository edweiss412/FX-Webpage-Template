...
| D3 | **The ambiguous path parses the sheet best-effort under the best-guess version and *attaches* the `VERSION_AMBIGUOUS` hardError — mirroring the MI-2..MI-5b convention, NOT MI-1.** Only the truly-unparseable `not_a_sheet` case returns the MI-1 minimal stub. | This matches the established `parseSheet` contract: for MI-2 (empty title), MI-3 (no dates), MI-4 (no crew), MI-5 (no rooms) the parser returns a **full best-effort parse** and `runInvariants` maps the code to hard_fail — it does NOT stub. An ambiguous sheet is *parseable* (like MI-2..5), unlike a non-sheet (MI-1). Returning a stub for it violated the convention and broke every block-parser unit test that feeds a minimal (v1-shaped) fixture — those fixtures already parsed as v1 under the old `detectVersion`, and best-guess-parse reproduces that exactly while adding the hardError. Fail-closed is preserved: `runInvariants` still hard_fails on the attached hardError, so the parse is held for review and never applied; the best-guess parse is discarded on the hard_fail path (and surfaced only as a diagnostic on the dev panel). The hardError *message* names the best-guess version + per-version marker scores. See §7.1 for the resolution path. |
...
| D7 | **No DB migration; the only `app/` touch is a one-token taxonomy addition.** | The code is a new text value in the existing `pending_ingestions.last_error_code` column, rendered by existing surfaces via `messageFor()`. The `/help/errors` index requires every renderable code to map to a named family (`tests/help/errors-grouping.test.tsx` asserts the "Other" bucket is empty), so `VERSION_AMBIGUOUS`'s `VERSION` prefix is added to the same `crew-schedule` family as its sibling `MI-1_VERSION_DETECTION_FAILED` in `app/help/errors/_families.ts`. This is a data/taxonomy edit (one string in a prefix array), not a design change: no `@theme`/component/`DESIGN.md`/layout/JSX/styling change, and the help page renders the new code with the identical list-item treatment used for all existing codes. The impeccable dual-gate (invariant 8) targets visual/UX quality; no rendered design is altered, so it is not meaningfully engaged — disposition recorded in the milestone handoff. No schema change → no `validation-schema-parity` step. |
...
- `ambiguous` → push `{ code: "VERSION_AMBIGUOUS", message }` to `hardErrors`, set `version = verdict.bestGuess`, and **continue the normal parse** (the block parsers run under the best-guess version). Mirrors the MI-2..MI-5b convention — parse best-effort + attach the code — NOT MI-1 (D3). The success-path return (`index.ts:~676`) already includes `hardErrors`, so the flag rides out with a real parse.
...
Add, immediately after the MI-1 block (so `VERSION_AMBIGUOUS` sorts to `failedCodes[0]` — it remains the routed/rendered code even when an ambiguous sheet is *also* otherwise-invalid and trips MI-2..5). **Forward the parser's original hardError message** (which carries the best guess + both marker scores) rather than a generic string — the cron path persists `invariant.messages.join("; ")` (`phase1.ts:289-290`), so this is the only way the diagnostic reaches `last_error_message` / `last_sync_error` (Codex R5 HIGH):
```ts
...
| Table, best version has <2 markers (e.g. novel template, 0/0) | `ambiguous` (bestGuess `v1`) | best-effort parse + `VERSION_AMBIGUOUS` | hard_fail → retain last-good (existing) / wizard ingestion (first-seen) |
| Table, v4=1 & v2=1 (margin <2) | `ambiguous` | best-effort parse + flag | hard_fail |
| Table, v2=2 but from **1 block** (`GS SET TIME`+`GS SETUP`) | `ambiguous` (blocks <2) | best-effort parse + flag | hard_fail — block-diversity clause; same-block markers are weak evidence (Codex R6) |
...
| Table, only the rental block present (v4=2, 1 block) | `ambiguous` (blocks <2) | best-effort parse + flag | hard_fail — single surviving block is not enough |
...
- **A genuine legacy-v1 sheet:** none exist in the corpus; it would flag ambiguous and require one of the two actions above. An admin "force-classify as v1" override is **not** built, and will not be (`BL-VERSION-AMBIGUOUS-V1-OVERRIDE`, resolved won't-build 2026-08-03, §10), because building an approve-ambiguous path is precisely what the gate is designed to avoid — and because such a sheet, once actually seen, is registrable by the bullet above like any other unregistered template.
...
- **`parseSheet` integration** — feed an ambiguous markdown → `hardErrors` contains `VERSION_AMBIGUOUS` with the scored message, and `template_version` is the best guess. Feed an ambiguous markdown that *contains crew* → the block parsers still run (`crewMembers.length > 0`), proving best-effort parse (MI-2..5b convention), not a stub. Feed a confident v4 markdown → no `VERSION_AMBIGUOUS`, fully-populated parse, `template_version==="v4"`. *Catches:* the flag firing on clean sheets, or the ambiguous path stubbing instead of parsing.
- **`runInvariants` routing** — a `ParseResult` carrying a `VERSION_AMBIGUOUS` hardError (with empty crew/rooms to also trip MI-4/5) → `outcome==="hard_fail"`, `failedCodes[0]==="VERSION_AMBIGUOUS"` (ahead of the co-firing MI-4/5). *Catches:* the cron-path gap (flag not reaching hard_fail via invariants) **and** wrong `failedCodes` ordering.
...
- Positive v1 detection / an admin "force-classify as v1" override for a genuine legacy-v1 sheet — no such sheet exists in the corpus. Closed won't-build on 2026-08-03 as **BL-VERSION-AMBIGUOUS-V1-OVERRIDE**: a real legacy sheet is registrable via resolution #2 above, so the override adds nothing the two live resolutions do not already cover. Re-opens only if a real legacy sheet surfaces AND marker registration proves impossible (no stable column-0 labels spanning ≥2 blocks) — both halves required.
...
- **"The gate should let the operator approve the ambiguous parse in-app."** Deliberately not built — approving a parse we are not confident about defeats the gate (§7.1). Resolution is making the sheet recognizable again (the MI-1 pattern), not rubber-stamping an untrusted parse. A force-classify override for a genuinely-new template is resolved won't-build (2026-08-03, `BL-VERSION-AMBIGUOUS-V1-OVERRIDE`) — it serves no occupant of the ambiguous bucket better than that occupant's existing resolution. The hard_fail route also cannot persist a best-guess parse for approval anyway (`pending_ingestions` has no `parse_result` column) — Codex R1 confirmed this.
- **"Existing v1 shows will re-flag on every sync (noise)."** Only if a pure-v1 published show exists — none do (corpus evidence). Documented risk; the override is closed won't-build (§10). Not a regression of any real show.
