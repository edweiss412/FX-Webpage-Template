...
`mutants.mjs` applies 34 mutations to a known-good stylesheet and asserts the
...
mutants, all killed. Together the two instruments are 28 rows and 45 mutants.
...
Expected: `18/18 rows passing` for the target state, `34/34 mutants killed`.
