...
`mutants.mjs` applies 37 mutations to a known-good stylesheet and asserts the
...
mutants, all killed. Together the two instruments are 28 rows and 48 mutants.
...
Expected: `18/18 rows passing` for the target state, `37/37 mutants killed`.
