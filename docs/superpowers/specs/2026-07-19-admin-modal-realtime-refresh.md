...
| --- | --- |
| Realtime fails open (mint/subscribe failure → no live updates, never a broken surface) | `ShowRealtimeBridge.tsx:84-86`, `:707-725` (M4) |
| Token-first read order (token sampled before data reads) | `lib/data/getShowForViewer.ts:920-935` (audit idx19) |
| Reuse the bridge; no fork; bridge internals out of scope | §3 of this spec |
| Bounded-backoff of the INITIAL subscribe is v2 | `ShowRealtimeBridge.tsx:85-86` |
| `crew_member_auth` subscribe-gap residual accepted (status-quo parity with crew page) | §4.1a |
| Actions × refresh overlap: no new mechanism, no compound-race test | §4.5, citing prefetch spec §6(b)/§3.4 empirics |
| Refresh × refresh overlap: serialized by the App Router's single action queue (ratified empiric) + M4 crew-page precedent; no new behavioral test | §4.5 |
| Advisory-lock invariant untouched; e2e fixture writes follow `seedShowWithCrew` precedent | §9 |
| E2E oracle CONSTANTS (frame predicates, quiesce windows, timeouts) are pinned by the plan's spike task from measured local behavior — the spec pins REQUIREMENTS only (attribution, geometry-proof, non-tautology). Further prose refinement of oracle constants is out of scope per the three-round cap. | §8.4 + AGENTS.md empirical-spike rule |
...
| Manual stage / unpublish (`lib/sync/runManualStageForFirstSeen.ts:56`, `lib/sync/unpublishShow.ts:183` → same helper) | yes (explicit helper) | NO — verified: neither file writes `last_synced_at` (grep, this session) | broadcast covers the subscribed steady-state; the subscribe-gap residual for these paths is the same ACCEPTED status-quo class as the `crew_member_auth` row below (identical on the crew page today) |
| `crew_members` insert/update (statement triggers, `supabase/migrations/20260501001000_internal_and_admin.sql:95-105`) | yes | yes — `bump_last_changed_at()` BEFORE-trigger stamps `last_changed_at` on every update (`20260501001000_internal_and_admin.sql:34-57`), which `viewer_version_token` maxes over | broadcast; gap covered by token catch-up |
...
2. the loader renders **without** the bridge for THAT RENDER PASS ONLY (everything else unchanged) — not "for the whole open": any later refresh in the same open (the modal's own revalidate-on-open, or an admin action's revalidation) re-runs the loader, and if the read recovers the bridge mounts on that reconcile (the recovery path below).
...
2. document focus is unchanged by NODE IDENTITY: the test records `document.activeElement` before the mutation (the pinned target: the untouched row's ⋮ trigger, testid `crew-row-menu-button-<crewId>`, `components/admin/wizard/CrewRowActions.tsx:241`, or a button inside its open popover — the popover renders INLINE in the row subtree, no portal, so it is modal content) and asserts the SAME node is still `document.activeElement` after the swap;
...
**Refresh × refresh overlap (resolved — App Router action queue + M4 precedent):** concurrent `router.refresh()` calls (open-refresh, catch-up, broadcast-driven, spaced broadcasts >100ms apart) cannot interleave or apply out of order: every `router.refresh()` is a router ACTION dispatched through the App Router's single serial action queue — the same queue whose serialization the ratified prefetch spec measured empirically (`docs/superpowers/specs/2026-07-19-show-modal-prefetch.md` §3.4 empirics / §6(b) "the router's action serialization makes release-after-commit unconstructible"). Each refresh's RSC payload applies in dispatch order; the last-dispatched wins; every payload is a full fresh server render (no stale-response hazard — a "stale" payload is at worst momentarily older data, immediately superseded by the queued later refresh, which is the convergence guarantee). Precedent: the crew page has run this exact configuration — bridge broadcasts concurrent with catch-up and navigation renders — since M4 with no ordering defect. No new behavioral test; listed in §1.1.
...
1. **Loader unit tests** (extend `tests/app/admin/showReviewModalLoader.test.tsx` — the existing loader suite, which already asserts on log-spy codes, e.g. `ADMIN_SHOW_LOOKUP_FAILED` at `:352-354`):
   - token read ok → rendered tree contains `ShowRealtimeBridge` with `{ showId, slug, renderVersion: <token> }`, AND the token RPC mock records being called with EXACTLY `("viewer_version_token", { p_show_id: <the resolved showId> })` — the argument binding is asserted, not just the RPC name (an args-agnostic mock would let a missing/wrong `p_show_id` pass);
   ...
   **Recovery-transition note (absent→present bridge across a fault recovery):** the claim that appending the trailing bridge sibling on a recovery render preserves `PublishedReviewModal` state rests on React's DOCUMENTED positional reconciliation of children (earlier siblings keep identity when a later sibling is appended) — not on undocumented framework behavior, so no spike is required. The child-order unit test pins the precondition (bridge strictly last, both renders); the preservation itself is React contract.
...
4. **Real-browser e2e** (Playwright — extend the modal's existing e2e surface, `tests/e2e/published-review-modal.prefetch.spec.ts` patterns): open `/admin?show=<slug>`, commit a content change so the statement trigger broadcasts on the local Supabase Realtime stack, assert:
   **Pre-mutation gates (anti-tautology, BOTH mandatory before the DB change is committed):**
   ...
   3. **Full quiescence:** after `SUBSCRIBED` the bridge runs its version catch-up (`refreshSyncIfMismatch`, one GET `/api/show/<slug>/version`), and a mismatch there initiates its own `router.refresh()` whose RSC round-trip can still be in flight. Waiting for the `/version` response alone is NOT enough. The gate is quiescence over BOTH request classes: no in-flight `?show=` RSC request AND no in-flight `/version` request, sustained for a quiet window whose duration MUST exceed the bridge's 100ms debounce plus a dispatch-latency margin (floor 250ms; the plan's spike may raise it from measured local timing, never lower) — otherwise an invalidation already sitting in the debounce timer pre-mutation could fire its refresh after the mutation and be misattributed to the broadcast.

   Only after ALL THREE gates does the test commit the DB change. **Positive attribution assertion:** the broadcast path never fetches `/version` (broadcasts refresh unconditionally, `ShowRealtimeBridge.tsx:744-754`; only catch-up paths fetch it). So the test additionally asserts NO further `/api/show/<slug>/version` request occurs between the mutation and the content swap — the swap is then attributable ONLY to a received broadcast traversing debounce → `router.refresh()`. **Reconnect flake guard:** a genuine `system.reconnected` during the window would legitimately fetch `/version`; the test observes the socket frames it is already watching (gate 2), and if a disconnect/re-join occurred in the observation window it RETRIES the scenario once (bounded) instead of failing the attribution assertion — an environmental reconnect must read as flake-retry, not as a broadcast-path failure. **Retry protocol:** the retry is a FULL scenario re-run against a FRESHLY seeded show (`seedShowWithCrew`'s create-or-replace under a new `driveFileId`/slug — `tests/e2e/helpers/seedShowWithCrew.ts:106-113`), never an in-place retry — the first attempt's mutation already changed the roster, so re-running in place would pass vacuously against already-updated content. The retry also runs in a FRESH browser context/page: websocket + request listeners, counters, the skeleton `MutationObserver`, and captured node references from attempt one are all attempt-scoped and must not leak into attempt two.
   **The stimulus (pinned):** an UPDATE to `crew_members.role` on ONE rendered crew row of the seeded show. This is the only stimulus class that satisfies every constraint at once: it broadcasts via the statement trigger (`20260501001000_internal_and_admin.sql:95-99` — show-field edits like title/venue do NOT broadcast without the explicit helper), it advances the token (`bump_last_changed_at()` BEFORE-trigger, `20260501001000_internal_and_admin.sql:34-57` → `max(crew_members.last_changed_at)`), it changes content the crew section actually renders, and it is key-stable — crew rows are keyed `` key={`${m.name}-${i}`} `` (`components/admin/wizard/step3ReviewSections.tsx:1394`), so a ROLE swap (never a rename, never an insert/remove/reorder) leaves every row's key and index intact — and role plays NO part in row ordering: the single crew display sort is `sortCrew`, ordering by `(name, id)` only, with no role-based grouping or filtering anywhere in the section (`components/admin/review/publishedAdapter.tsx:228-232`, single-sort contract at `:50-53`).

   **Assertions after the mutation:**
   - **row-scoped content swap** (not a whole-modal text search): fixture roles are UNIQUE across the roster; preconditions asserted BEFORE mutating — the target row's locator shows the OLD role and the NEW role string appears nowhere in the modal; post-mutation, the NEW role appears WITHIN the target row's locator, without navigation (URL unchanged);
   ...
   - **scroll oracle precondition (anti-tautology):** before recording the baseline, the test PROVES the modal is scrollable (`scroller.scrollHeight > scroller.clientHeight` — the fixture roster is sized to guarantee it) and sets `scrollTop` to a material mid-position (≥ 100px and below max). A reset-to-zero regression is then detectable; starting at 0 would make the §4.4 inv-3 assertion vacuous;
   - **objective geometry invariant, scroll-independent (replaces "similar length" hand-waving):** equal-length strings can still wrap or change row height, so the test PROVES geometry stability instead of assuming it — using measurements that do NOT depend on `scrollTop` (a `getBoundingClientRect` check would be circular: a scroll-reset regression would shift the rect and mask itself as "inconclusive"). Pinned pre→post: the modal scroller's `scrollHeight` and the anchored row's `offsetTop` + `offsetHeight` (layout-relative, scroll-independent) unchanged ±1px. Only a delta in THESE fails as INCONCLUSIVE (fixture problem); the `scrollTop` invariant (§4.4 inv 3) is asserted SEPARATELY and a violation there always fails the test proper;
