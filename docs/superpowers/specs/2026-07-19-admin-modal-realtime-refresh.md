...
- Placement inside vs. outside `ShareTokenProvider` is behaviorally irrelevant (the bridge renders `null` and reads no context); it sits NEXT TO `PublishedReviewModal` inside the provider for locality. `key={showId}` on the provider (`_showReviewModal.tsx:372`) means a different show remounts the bridge cleanly — the bridge's own effect deps (`[showId, slug, …]`, `ShowRealtimeBridge.tsx:876`) already handle that.
