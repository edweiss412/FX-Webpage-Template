...
| Role chip (`data-testid="picker-role-chip"`) | `role` if present, else absent | text `Signing in…` (R4: renders even when `role` is the empty string), composed from `chipBaseClassName` with `bg-accent-tint`, `text-accent-on-bg` and `border border-accent-on-bg`. The fill AND the border are both required: the idle chip is 1.00:1 against the row, and the fill alone is 1.02:1, so the border is what makes it a container (invariant-8 P1) |
| `<button>` | interactive | `aria-disabled="true"`, `aria-busy="true"`, `onClick` guard returns early; **NOT** the `disabled` attribute |
| Live region | rendered, empty | `sr-only role="status" aria-live="polite"` carrying `Signing in as <name>`. Rendered OUTSIDE the `<button>`: ARIA lets AT ignore descendant changes while an ancestor is `aria-busy`, and the button is that ancestor |
...
| P10 | A screen reader announces the pending transition | **PARTLY SETTLED, and the design DOES depend on it.** No harness here can settle the spoken text, so that remains unproven. But `aria-busy` alone was ruled insufficient by the invariant-8 gate (WCAG 2.2 SC 4.1.3), so an `sr-only role="status" aria-live="polite"` region is a SHIPPED requirement, not a nicety — and it lives OUTSIDE the button, because ARIA lets AT ignore descendant changes while an ancestor is `aria-busy`. Its presence, placement and content are asserted in §8.2 | Playwright/jsdom for presence and placement; spoken text unproven |
