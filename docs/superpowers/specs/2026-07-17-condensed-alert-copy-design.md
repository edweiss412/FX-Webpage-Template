...
`BellEntry.action` (`lib/admin/bellFeed.ts:27-47`) becomes `actions: AlertActionLink[]` (empty array when none); `BellPanel.ActionCell` (`BellPanel.tsx:244-296`) renders the list in order with existing `LINK_CTA` styling. Other codes' single actions become one-element lists — no per-code behavior change. The per-show card renders **no** show-page link (it lives on that page); its existing action handling is unchanged.
