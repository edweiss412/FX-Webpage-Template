...
- **EXTENDS:**
  ...
  - `tests/messages/_metaAdminAlertCatalog.test.ts` — **register the SECOND `PICKER_EPOCH_RESET` producer** (finding #2). The new server action `resetCrewMemberSelection.ts` is an additional `upsertAdminAlert` write-site for the existing code `PICKER_EPOCH_RESET`. Convert that code's registry entry (currently a single object `path: "lib/auth/picker/resetPickerEpoch.ts"`, `:135-138`) into a **two-element `[{path,pattern}, …]` array** covering both `resetPickerEpoch.ts` and `resetCrewMemberSelection.ts`. The test normalizes `Array.isArray(writeSite) ? writeSite : [writeSite]` and iterates each (`:325-331`); the canonical array-form precedent is `WIZARD_SESSION_SUPERSEDED_RACE` (`:275`, a two-producer array — NOT `SHOW_UNPUBLISHED`, which is a single-site object whose second SQL producer is pinned by a separate dedicated test). This does **not** add a §12.4 catalog code; it registers a second producer of an existing one.
