...
3. **ONLY present.** Let `body` = text between the leading dash and the ONLY marker; `tail` = text after the marker. Split `body` on `/` into segments (trimmed). Classify the WHOLE body:
   ...
   - **Pure-role:** body has NO stage segment AND every segment resolves to a recognized `ROLE_NORMALIZATIONS` token (incl. multi-word roles like `SHOW CALLER`) — i.e. it is `<role(s)> ONLY`. → `stages = []`, `hasOnly = true`, `unrecognizedRestriction = false`, `cleaned = roleCell` UNCHANGED (so `extractRoleFlags` yields the role + `ONLY` flag). Covers `SHOW CALLER ONLY`, `A1 ONLY`.
