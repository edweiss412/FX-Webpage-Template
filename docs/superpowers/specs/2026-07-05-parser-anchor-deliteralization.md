...
- **`extractGsBlock` terminator (`:687`)** and **`NEXT_ROOM_HEADER_RE` (`:880-881`)**: keep the structural keywords (`GENERAL SESSION|BREAKOUT|ADDITIONAL ROOM|LUNCH ROOM|DETAILS`) and REPLACE the `MABEL|LAUDERDALE` alternatives with a **strong-evidence terminator predicate**, NOT bare name-shape (adversarial-review R2 finding 1 — a name-shape-only terminator would let any all-caps note / equipment / value row inside a GS/BO block truncate the block silently, with no warning since it is only a terminator, not a failed discovery). The terminator fires at row `i` iff:
  ```
  roomHeaderNameShape(col0(i)) && ( dimsInHeader(col0(i)) || hasRoomFieldEvidence(<next 1–2 non-blank table rows after i>) )
  ```
  This is the SAME name-shape + strong-evidence gate as discovery, but with **bounded 1–2-row lookahead** instead of a whole-block scan — no recursion into the block being bounded. `extractBoBlock`/`extractGsBlock` already iterate lines by index, so the lookahead is a cheap peek at the following non-blank row(s). Consequence: an all-caps note (`IMPORTANT NOTES`) with no dims and no room-field row after it does NOT terminate; `MABEL 1\nDAY 1 & 2` (no dims, but BO fields immediately after) DOES terminate. The **byte-identical east-coast golden test** (§2.4) plus an explicit **negative terminator test** (§8: an all-caps non-room row sitting inside a GS/BO block before later fields → extraction does NOT stop there) pin this — the single highest-risk decision in the PR, now bounded by an evidence gate rather than asserted blind.
...
4. **`cleaned` is the input with the stage clause removed ONLY when `hasOnly && stages.length > 0`** (a genuine restriction was consumed). **Otherwise `cleaned = roleCell` UNCHANGED** (adversarial-review R2 finding 2). This is load-bearing: a partial stage subset WITHOUT `ONLY` — e.g. `Set / Strike` — must NOT be silently stripped, because on `origin/main` `extractRoleFlags` tokenizes it into `UNKNOWN_ROLE_TOKEN` signals (`SET`, `STRIKE`). Leaving `cleaned` unchanged preserves that existing signal (crew sees whole show + a warning, never silently). The descriptive **full** stage list without ONLY (`Load In / Set / Strike / Load Out - LEAD`, the "works all phases" case) continues to be prefix-stripped by the EXISTING `FULL_STAGE_PATTERN` logic retained inside `extractRoleFlags` (see §3.2 consumer contract) — that is not a duplicated restriction pattern, so it stays.
