...
// (b) PURE — dims token rides in the (possibly multi-line) header cell. VALUE + admit signal.
function dimsInHeader(col0Raw: string): boolean       // /\d+\s*'\s*x/i across any header line
// (b') PURE — a SECOND-line DAY marker ("NAME&#10;DAY 1 & 2"), the other structural signature of a
//      real v1/v2 room header (the packed day-usage line). NOT "any &#10;": an arbitrary multi-line
//      in-room note (adversarial-review R19 f1) is excluded because its extra line is not a DAY row.
function headerDayMarker(col0Raw: string): boolean    // /\bDAYS?\s+\d/i across the header cell's lines
...
//      weak; a single-line in-room note before a normal Audio/Notes field would fabricate a room).
function hasRoomFieldEvidence(nextRowsText: string): boolean
// (d) ADMIT predicate — strong HEADER-CELL evidence: a DIMS token OR a DAY marker in the cell.
//     Independent of fields beneath. NOT "any multi-line" (R19 f1: excludes multi-line notes).
export function isRoomHeaderShape(col0Raw: string): boolean
//   = roomHeaderNameShape(col0Raw) && (dimsInHeader(col0Raw) || headerDayMarker(col0Raw))
...
**Design note — comprehensive re-analysis of the room-discovery false-positive vector (adversarial-review R2 f1, R3 f2, R4 f1, R6 f1, R7 f1, R10 f1, R11 f1, R12 f1, R15 f1, R17 f2 , R18 f1, R19 f1 — a recurring vector; per the AGENTS.md "comprehensive re-analysis + structural defense" rule the model is now anchored end-to-end against the LIVE extraction code, not point-patched: admit/emit/terminate are three distinct gates, ADMIT requires a DIMS token OR a `DAY N` marker in the header cell — the actual structural signature of a v1/v2 room header — so neither field-beneath (R18 f1) NOR a bare multi-line note (R19 f1) can fabricate a room, per-member independent extraction + `mergeBoFields` is the origin/main-identical merge, and the corpus-wide no-op test is the mechanism-agnostic guard).** The literal `MABEL|LAUDERDALE` loop was safe because it matched two specific names; a shape scan is inherently more permissive. The resolution is a **two-stage admit-then-emit** model that separates "worth extracting a block for" from "actually a room":

1. **ADMIT** a candidate iff `roomHeaderNameShape && (dimsInHeader || headerDayMarker)` — strong HEADER-CELL evidence: a DIMS token OR a `DAY N` marker line in the (multi-line) cell. This is the actual structural signature of a v1/v2 room header — they pack `NAME&#10;DIMS` (e.g. `SALON ABCD&#10;60' x 45'`) or `NAME&#10;DAY 1 & 2` (e.g. `MABEL 1&#10;DAY 1 & 2`). **Neither field-evidence-beneath NOR bare multi-line is an admit signal (adversarial-review R18 f1 + R19 f1).** Field-beneath fabricates a room from a single-line in-room note (`SPECIAL NOTE`) before a normal `Audio` field; "any `&#10;`" fabricates one from a MULTI-LINE in-room note (`FOH POSITION&#10;Downstage`). Requiring a dims token OR a `DAY N` line structurally excludes BOTH note shapes (a note's extra line is neither a dims token nor a `DAY N` marker), while admitting every real room header. This ADMITS the dims-only-first block (`SALON ABCD&#10;60' x 45'`, dims — so its dims are captured for merge, R7 f1) AND the day block (`MABEL 1&#10;DAY 1 & 2`, DAY marker) AND the equipment row (`5' X 9' …`, dims); it does NOT admit a single-line OR multi-line note.
...
3. **EMIT** a room for a key-group iff **≥1 admitted block in that group has `hasRoomFieldEvidence`** (an ACTUAL immediate room-field row) — NOT merely dims/multi-line. A group whose every member is dims/multi-line-only (an equipment/asset row with a unique key and no field-bearing sibling) is DROPPED. This is the load-bearing gate: `roomHasContent` alone is INSUFFICIENT because it counts `dimensions` as content (`rooms.ts:424-441`), so a dims-only equipment row would pass it — the emit gate requires a real *field*-bearing block in the group, which equipment never has.

| False-positive / correctness class | Example | Closed by |
|-------|---------|-----------|
| All-caps NOTE inside a block (note text beneath) | `SPECIAL NOTE` then note text | not admitted (single-line: no dims, no `&#10;`) |
| Single-line NOTE followed by a REAL FIELD (R18 f1) | `SPECIAL NOTE` then `Audio`/`Notes`/`Digital Signage` (fields of the OPEN room) | not admitted — no dims, no `DAY N`, and field-beneath is NOT an admit signal → neither emits NOR enters `ROOM_HEADER_LINES` |
| MULTI-LINE NOTE followed by a REAL FIELD (R19 f1) | `FOH POSITION&#10;Downstage` then `BO Audio` (field of the OPEN room) | not admitted — the extra line is neither a dims token nor a `DAY N` marker, so `headerDayMarker`/`dimsInHeader` are both false → not a room, does not truncate the open room |
...
**In-room notes are structurally closed (R18 f1 + R19 f1), NOT backlogged.** The admit rule requires a DIMS token OR a `DAY N` marker in the header cell. A single-line note (`SPECIAL NOTE`) has neither; a MULTI-LINE note (`FOH POSITION&#10;Downstage`, `SPECIAL NOTE&#10;see below`) has a second line that is neither a dims token nor a `DAY N` marker — so both are rejected at admit, statelessly, without an "is-inside-an-open-room" positional tracker (added parser state this PR deliberately avoids). The only residual would be an in-room note whose extra line literally reads like `DAY 2` or carries a `NN' x NN'` dims token AND which sits immediately before a real field — a shape indistinguishable from a room header by construction and absent from the corpus (the corpus-wide rooms no-op test is the backstop). Regression tests cover single-line-note-then-field (R18 f1) and multi-line-note-then-field (R19 f1).
...
| Mixed-case novel venue name (`Salon ABCD&#10;60' x 45'`) + `BO Setup` beneath | ADMITTED (dims token in header) + emit-eligible (field beneath) → EMITTED (R10 f1; case-insensitive) |
...
| MULTI-LINE note, no dims, no `DAY N` (`FOH POSITION&#10;Downstage`) — even with a field beneath | NOT admitted (`headerDayMarker`+`dimsInHeader` both false) → no room, no terminate (R19 f1) |
...
- **Novel-venue recognition (mixed-case, R10 f1):** synthetic `| Salon ABCD&#10;60' x 45' |` (Title Case header cell WITH dims + `&#10;`) and a `BO Setup` field row immediately beneath → assert one breakout room `{name:'Salon ABCD', dimensions:"60' x 45'"}` appears (was dropped on `origin/main`; proves the shape gate is case-insensitive). Admitted via `dimsInHeader` (the `60' x 45'` token), emit-eligible via the field beneath. Failure mode: all-caps-only shape gate rejects a real Title-Case venue name.
- **Novel-venue recognition (no-dim, DAY-marker header, R18 f1 + R19 f1):** synthetic `| GRAND FOYER&#10;DAY 1 & 2 |` (NO dims token, but a `DAY N` marker line) followed immediately by a recognized room-field row (`BO Setup`) → assert the room is ADMITTED via `headerDayMarker` (the `DAY 1` line, NOT bare multi-line, NOT field-beneath) and EMITTED (field-bearing). Exercises the `MABEL 1&#10;DAY 1 & 2` merge shape. Failure mode: admit rule can't see the DAY-marker signal, or (regression) admits on field-beneath / bare `&#10;`.
...
- **Room ADMIT requires a DIMS token OR a `DAY N` marker in the header cell (R18 f1 + R19 f1).** This is the actual structural signature of a v1/v2 room header (`NAME&#10;DIMS` or `NAME&#10;DAY 1 & 2`) — NOT field-beneath (would fabricate from a single-line note) and NOT bare multi-line (would fabricate from a multi-line note `FOH POSITION&#10;Downstage`). Field-evidence-beneath is the EMIT signal (drops equipment), never the ADMIT signal. This structurally closes the in-room-note class WITHOUT stateful open-room tracking; the corpus-wide no-op test backstops. Do not relitigate admitting on field-beneath OR bare `&#10;`, and do not ask for the multi-line-note case to be backlogged — it is closed by the `DAY N`/dims discriminator (R19 f1).
