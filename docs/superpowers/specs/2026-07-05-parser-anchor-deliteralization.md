...
// (b') PURE — the header cell is MULTI-LINE (packs "NAME&#10;DIMS" / "NAME&#10;DAY 1 & 2"), the
//      structural signature of a real v1/v2 room header. Mirrors the strong-evidence rule of the
//      shipping precedent findGsBlockVenueHeader (:543-581): in-cell &#10; OR a dims token.
function multiLineHeader(col0Raw: string): boolean    // raw col0 (pre-first-line) contains &#10; / \n
...
// (d) ADMIT predicate — strong HEADER-CELL evidence (dims OR multi-line), independent of fields beneath.
export function isRoomHeaderShape(col0Raw: string): boolean
//   = roomHeaderNameShape(col0Raw) && (dimsInHeader(col0Raw) || multiLineHeader(col0Raw))
...
**Design note — comprehensive re-analysis of the room-discovery false-positive vector (adversarial-review R2 f1, R3 f2, R4 f1, R6 f1, R7 f1, R10 f1, R11 f1, R12 f1, R15 f1, R17 f2 , R18 f1 — a recurring vector; per the AGENTS.md "comprehensive re-analysis + structural defense" rule the model is now anchored end-to-end against the LIVE extraction code, not point-patched: admit/emit/terminate are three distinct gates, ADMIT uses strong header-cell evidence ONLY (dims OR multi-line — the same rule the shipping `findGsBlockVenueHeader` uses; field-beneath is emit-only, R18 f1), per-member independent extraction + `mergeBoFields` is the origin/main-identical merge, and the corpus-wide no-op test is the mechanism-agnostic guard).** The literal `MABEL|LAUDERDALE` loop was safe because it matched two specific names; a shape scan is inherently more permissive. The resolution is a **two-stage admit-then-emit** model that separates "worth extracting a block for" from "actually a room":

1. **ADMIT** a candidate iff `roomHeaderNameShape && (dimsInHeader || multiLineHeader)` — strong HEADER-CELL evidence only, mirroring the shipping `findGsBlockVenueHeader` precedent (in-cell `&#10;` OR dims token). **Field-evidence-beneath is NOT an admit signal (adversarial-review R18 f1)** — admitting on "a room field appears in the next rows" fabricates a room from any single-line in-room note/subheading (`SPECIAL NOTE`, `FOH POSITION`) that happens to precede a normal `Audio`/`Notes`/`Digital Signage` field of the CURRENTLY-OPEN room. Real v1/v2 room headers pack `NAME&#10;DIMS` or `NAME&#10;DAY 1 & 2` (multi-line) or carry a dims token; a single-line proper-name note has neither, so it is never admitted. This ADMITS the dims-only-first block (`SALON ABCD&#10;60' x 45'`, dims+multi-line, so its dims are captured for merge — R7 f1) AND the day block (`MABEL 1&#10;DAY 1 & 2`, multi-line) AND the equipment row (`5' X 9' …`, dims); it does NOT admit a single-line note.
...
| All-caps NOTE inside a block followed by a REAL FIELD (R18 f1) | `SPECIAL NOTE` / `FOH POSITION` then `Audio`/`Notes`/`Digital Signage` (fields of the OPEN room) | not admitted — admit needs dims OR multi-line header-cell evidence, and field-beneath is NOT an admit signal, so the note neither emits NOR enters `ROOM_HEADER_LINES` (cannot truncate the open room) |
...
1. **Non-empty, label-less proper name — CASE-INSENSITIVE (adversarial-review R10 finding 1).** After flattening `&#10;`→newline, taking the first line, trimmed, and **uppercasing**, it matches the proper-name shape `^[A-Z0-9][A-Z0-9 &'./-]*$` (letters/digits/spaces + a small punctuation set; at least one letter). The match is case-INSENSITIVE (the literal `mabelRe` it replaces used `/gim`), so a mixed-case novel venue name like `Salon ABCD` / `Grand Foyer` IS admitted — that is the feature's whole point. Mixed-case *field labels* (`Breakout Room Setup Date / Time`) are NOT excluded by casing; they are excluded by item 2 (`BREAKOUT`/`ADDITIONAL`/… banner) and item 3 (field-label), both of which uppercase before comparing. The header-cell-evidence admit gate (dims OR multi-line `&#10;`) + the emit gate (§2.2) are the real discriminator for anything else; the corpus no-op test pins that no mixed-case non-room row in the 15 fixtures becomes a room.
...
**Known limitation (R18 finding 1 residual, backlog):** the admit rule requires strong HEADER-CELL evidence (dims token OR `&#10;`), which structurally closes the single-line-note-before-a-field false positive (the R18 f1 class). A residual edge — a proper-name note whose header cell ITSELF packs a `&#10;` AND sits immediately before a real room field — could still be admitted+emitted, but (a) no such row exists in the corpus (the corpus-wide rooms no-op test is the backstop — a fabricated room on ANY real fixture fails it), and (b) it is a far narrower shape than the single-line note. Documented as unsupported/backlog rather than adding an "is-inside-an-open-room" positional tracker (added parser state this PR deliberately avoids); the corpus no-op test bounds the blast radius.
...
| Mixed-case novel venue name (`Salon ABCD&#10;60' x 45'`) + `BO Setup` beneath | ADMITTED (dims/multi-line header) + emit-eligible (field beneath) → EMITTED (R10 f1; case-insensitive) |
...
| Single-line proper name, no dims, no `&#10;` (stray note) — even with a field beneath | NOT admitted (no header-cell evidence) → no room, no terminate (R18 f1) |
...
| Dims-only block sharing a key with a field-bearing block (`SALON ABCD&#10;60' x 45'`) | ADMITTED (dims/multi-line) → merged → EMITTED with dims (via the field-bearing sibling) |
| Multi-line proper name (`MABEL 1&#10;DAY 1 & 2`) + `BO Setup` beneath (east-coast) | ADMITTED (multi-line `&#10;`) + emit-eligible (field beneath) → EMITTED (regression anchor) |
...
2. **Full-4-list lenient-star carve-out FIRST (backward-compat, adversarial-review R17 finding 1).** Before the strict marker search, test the retained live `FULL_STAGE_ONLY_PATTERN` — `/Load\s+In\s*\/\s*Set\s*\/\s*Strike\s*\/\s*Load\s+Out\s+ONLY\*{0,3}/i` (`personalization.ts:53-54`, the EXACT-ORDER full physical-4 list with a LENIENT `ONLY\*{0,3}` marker). If it matches → `stages = [Load In, Set, Strike, Load Out]`, `hasOnly = true`, `unrecognizedRestriction = false`, `consumedOnlyClause = true`. **`cleaned` PRESERVES any recognized leading-role prefix (adversarial-review R18 f2):** the pattern is UNANCHORED (mirroring live `FULL_STAGE_ONLY_PATTERN`), so it can match a full-stage substring AFTER a leading role (`A1 / Load In / Set / Strike / Load Out ONLY*`). Let `prefix` = text BEFORE the matched span, `tail` = text after it; `cleaned = <the ROLE segments of `prefix` (split on '/', kept via the SAME `segmentHasKnownRole` recognizer as the some-stage branch), joined by ' / '> + tail`. For `A1 / Load In / … ONLY*` → `prefix = "A1 /"` → ROLE seg `A1` → `cleaned = "A1"`, so `extractRoleFlags` keeps `A1` — EXACTLY origin/main (whose anchored `extractRoleFlags` strip left `A1` intact while `FULL_STAGE_ONLY_PATTERN` produced the 4-stage restriction). For a pure `Load In / … ONLY*` (no prefix) → `cleaned = tail`. This PRESERVES origin/main behavior EXACTLY: a full-4-list row with `ONLY*` / `ONLY**` yields an explicit `[Load In,Set,Strike,Load Out]` restriction (which EXCLUDES pure `Show` days — now that `Show` is a distinct WorkPhase, dropping this to whole-show would EXPOSE Show days, a real expansion) AND keeps any leading role. The lenient-star tolerance is scoped to THIS exact full-4 pattern only; it does NOT generalize to subsets.
...
| `A1 / Load In / Set / Strike / Load Out ONLY*` (LEADING role + full-4) | [Load In,Set,Strike,Load Out] | true | false | explicit 4-stage AND ROLE `A1` preserved (`cleaned="A1"`) — carve-out keeps the leading-role prefix (R18 f2); matches origin/main |
...
- **Novel-venue recognition (mixed-case, R10 f1):** synthetic `| Salon ABCD&#10;60' x 45' |` (Title Case header cell WITH dims + `&#10;`) and a `BO Setup` field row immediately beneath → assert one breakout room `{name:'Salon ABCD', dimensions:"60' x 45'"}` appears (was dropped on `origin/main`; proves the shape gate is case-insensitive). Admitted via `dimsInHeader`/`multiLineHeader`, emit-eligible via the field beneath. Failure mode: all-caps-only shape gate rejects a real Title-Case venue name.
- **Novel-venue recognition (no-dim, MULTI-LINE header, R18 f1):** synthetic `| GRAND FOYER&#10;DAY 1 & 2 |` (NO dims token, but MULTI-LINE via `&#10;`) followed immediately by a recognized room-field row (`BO Setup`) → assert the room is ADMITTED via `multiLineHeader` (the `&#10;`, NOT field-beneath) and EMITTED (field-bearing). Exercises the `MABEL 1&#10;DAY 1 & 2` merge shape. Failure mode: admit rule can't see the multi-line header signal, or (regression) admits on field-beneath alone.
...
- **Negative (single-line note FOLLOWED BY A REAL FIELD is not a room, R18 finding 1):** inside an open room block, a single-line proper-name note row `| SPECIAL NOTE |` (NO dims, NO `&#10;`) IMMEDIATELY followed by a genuine room-field row `| BO Audio | L-Acoustics |` (a field of the OPEN room) → assert (a) NO room named `SPECIAL NOTE` is created, and (b) the `BO Audio` value lands on the OPEN room (the note is NOT admitted, so its line is NOT in `ROOM_HEADER_LINES` and it does NOT terminate the open room). Also test `| FOH POSITION |` + `| Notes | … |`. This is the exact false-positive the field-beneath admit rule would have caused — closed by requiring dims/multi-line header evidence to admit. Failure mode: a proper-name note before a normal field fabricates a room and steals the field.
...
- **Full-4 carve-out preserves a LEADING role (R18 finding 2):** `A1 / Load In / Set / Strike / Load Out ONLY`, `... ONLY*`, `... ONLY**` → `extractStageRestriction` returns explicit `[Load In,Set,Strike,Load Out]` AND `extractRoleFlags(parseStageClause(...).cleaned)` yields role `A1`. Assert BOTH the 4-stage restriction AND the `A1` flag are present, for all three marker forms — proving the unanchored carve-out did not drop the `A1 /` prefix. Failure mode: the carve-out sets `cleaned` to text after the matched full-stage span, silently dropping the leading role that origin/main preserved.
...
- **Marker tolerance is NOT widened, and full-4 compat is NOT dropped (R14 f1 + R17 f1).** For the GENERALIZED grammar the valid ONLY-marker forms are exactly `ONLY` and `ONLY***` — identical to the live `STAGE_TRAILING_MARKER_RE` (`personalization.ts:183-187`); `ONLY*`/`ONLY**` are invalid on PARTIAL subsets (fail-safe to whole-show), and origin/main never narrowed a partial on a starred marker (its partial narrowing used the star-free `LOAD_IN_SET_ONLY_PATTERN`/`LOAD_OUT_STRIKE_ONLY_PATTERN`). SEPARATELY, the EXACT full-4 list keeps origin/main's lenient `FULL_STAGE_ONLY_PATTERN` `ONLY\*{0,3}` (step 2 carve-out) → `ONLY*`/`ONLY**` on the full-4 list stays an explicit 4-stage restriction (excludes Show days). Both directions mirror origin/main exactly; neither the generalized-marker strictness NOR the full-4 lenience is a behavior change. Do not relitigate `***`-count tolerance for partial subsets (deferred), and do not relitigate demoting the full-4 lenient star (it would EXPOSE Show days — R17 f1).
...
- **Room ADMIT uses header-cell evidence ONLY (dims OR multi-line `&#10;`), NOT field-beneath (R18 f1).** This mirrors the shipping `findGsBlockVenueHeader` precedent (`:543-581`) exactly — it is not a novel gate. Field-evidence-beneath is the EMIT signal (drops equipment), never the ADMIT signal (which would fabricate a room from a single-line in-room note before a normal field). The corpus-wide rooms no-op test backstops any residual. Do not relitigate admitting on field-beneath; it is the exact false-positive R18 f1 closed.
