...
2. **Full-4-list lenient-star carve-out FIRST (backward-compat, adversarial-review R17 finding 1).** Before the strict marker search, test the retained live `FULL_STAGE_ONLY_PATTERN` — `/Load\s+In\s*\/\s*Set\s*\/\s*Strike\s*\/\s*Load\s+Out\s+ONLY\*{0,3}/i` (`personalization.ts:53-54`, the EXACT-ORDER full physical-4 list with a LENIENT `ONLY\*{0,3}` marker). If it matches → `stages = [Load In, Set, Strike, Load Out]`, `hasOnly = true`, `unrecognizedRestriction = false`, `consumedOnlyClause = true`, `cleaned = <text after the matched pattern>`. This PRESERVES origin/main behavior EXACTLY: a full-4-list row with `ONLY*` / `ONLY**` currently yields an explicit `[Load In,Set,Strike,Load Out]` restriction (which EXCLUDES pure `Show` days — now that `Show` is a distinct WorkPhase, dropping this to whole-show would EXPOSE Show days, a real expansion). The lenient-star tolerance is scoped to THIS exact full-4 pattern only; it does NOT generalize to subsets.
