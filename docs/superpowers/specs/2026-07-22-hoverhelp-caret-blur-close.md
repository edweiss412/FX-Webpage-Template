...
- **T-J5** close FROM the suppressed state: the caret's inline `visibility` is reset by cleanup (a lingering `hidden` would survive into the next open), and its `data-popover-side` is absent — cleared earlier by the `caret:null` branch, NOT by cleanup. Cleanup deliberately does not strip the side (§3.4): a close from a PLACED-VISIBLE state retains it through the exit fade, which T-A1 pins. Catches: visibility leaking into the next open; a cleanup that re-introduces the fade-breaking strip.
