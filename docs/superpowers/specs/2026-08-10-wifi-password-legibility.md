...
- The copy control's hit target is exactly `size-tap-min` (44×44, `--spacing-tap-min`, `app/globals.css:179`); its margin box is 36×28 (`-my-2 -ml-2`); its painted tile is `size-7`; the target rect stays within the row rect (right edge pinned by the zero right margin) — real-browser `getBoundingClientRect()` assertions, not jsdom.
...
States: idle, copied (2 s), back to idle. Pairs: idle→copied — instant glyph swap (copy→check), no animation (matches ShareLinkCopyButton); copied→idle — instant swap back on timeout. Compound: re-tap while copied — timeout resets (clearReset pattern), state stays copied, and a fresh keyed "Copied." entry APPENDS to the `role="log"` region, which re-announces identical text by construction (the reason log was adopted, §4.2). No other visual states; no animation anywhere (`prefers-reduced-motion` moot).
