...
- **DOM shape (R2 F1):** `<dd>` is a `flex-col` (`FactRows.tsx:85`), so the control cannot be a bare sibling of the value span. When `copyLabel` is present, the value span and the island render inside a NEW inline row wrapper `<div className="flex min-w-0 items-center justify-end gap-2.5">` as the dd's first child (the `sub` line, when present, stays a following sibling); when `copyLabel` is absent the dd renders EXACTLY today's children — the wrapper exists only on the opted-in row, which is what keeps control-free consumers byte-identical. `gap-2.5` (10px) exceeds the 8px reach of the button's `-m-2`, and the 44px hit target extends 8px past the margin box — landing exactly at the gap's far edge — so the target and the selectable value text do not intersect (asserted, Dimensional Invariants).
- `<button type="button">` inside that wrapper, with the step3-a11y **Class B recipe verbatim** (`docs/superpowers/specs/2026-08-07-step3-a11y-cluster.md`, "Recipe, empirically selected"): button = `-m-2 inline-flex size-tap-min shrink-0 items-center justify-center rounded-md` (44×44 target, 28×28 margin box; the radius sits on BOTH target and visual per the Class B ownership rule, so the focus ring follows the tile's shape), inner visual `<span>` = `grid size-7 place-items-center rounded-md bg-surface-sunken text-text-subtle` (the exact `dt` icon-tile treatment, `FactRows.tsx:76`) holding the repo's standard copy/check glyph pair at `size-3.5`. Exact classes stated because wildcard `-m-*` is not a spec (R1 F3). The 28px margin box raises the row's flex cross-size from ~21px (bare `text-sm` value) to 28px — the SAME content height every icon-bearing row already has, so the row lands on an existing design size rather than a novel one.
...
- **AC-2:** Copy control present on exactly that row, 44×44 target; the row's height equals an icon-bearing row's height (the stated +7px over a bare-text row — the Dimensional Invariants numbers, R2 F2); copies the exact password bytes (including trailing punctuation); a keyed entry appears in the rendered `role="log"` region (`venue-wifi-copy-log`) per copy, including repeat-identical copies.
