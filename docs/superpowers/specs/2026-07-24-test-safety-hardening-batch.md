...
- **WI-2 residual bypasses, each pinned by a KNOWN-LIMITS test** (`tests/admin/stagedPageRefScan.test.ts`) so the boundary is executable rather than assumed: a path assembled across module boundaries; `href` via `{...props}`; a PARAMETERIZED helper (`joinPath("/admin/onboarding/", "staged/")`) — resolving it needs interprocedural argument binding; a REASSIGNED `let`/`var` binding; and a BLOCK-SCOPED constant, since the walker reads module scope only (name-keyed resolution across scopes produces both false negatives and false positives, which is worse than a documented gap).
