...
- **Zoom intent** = the first of: committed pinch (the existing commitment detection around `GalleryLightbox.tsx:79`, which already distinguishes pointer-down from actual zooming), keyboard `+` / zoom-in control, or double-tap zoom — i.e., every path that changes scale. Intent flips a per-slide `wantsOriginal` state to `true`, and it **stays true for that slide for the lightbox session** (matching the ratified per-diagram zoom context, `GalleryLightbox.tsx:190` region), so de-zooming does not re-downgrade and re-zooming never re-fetches.
...
- **Announcement:** the failure emits one message through the shared announce implementation (`components/admin/announceLog.tsx`, `role="log"` shape per DESIGN.md — identical text CAN legitimately recur when multiple thumbnails fail): "Diagram <n> could not be loaded." where <n> is the 1-based visible position (the existing `aria-label` naming scheme, `Gallery.tsx:157`). The sr-only fallback text in the replacement cell stays (browse-time discoverability), the announcement adds the event-time signal.
...
- **Loader/unit (red first):** active slide passes no `pinOriginal` before intent and `pinOriginal: true` after; intent flips on each of the three input paths; state persists across de-zoom; variant-less entries produce identical URLs before/after intent. Anti-tautology: assert on the URL the loader RETURNS (the data source), not on a container attribute that renders both; derive expected tier URLs from fixture variant ladders, never hardcode.
- **Component (Gallery):** focus relocation to next/prev/show-more/list in each availability configuration, exercised by firing `onError` while focus is on the failing button (jsdom `document.activeElement` assertions); announcement sink-spy records exactly one message per failure with the correct index; no-focus failure relocates nothing.
- **Real-browser (Playwright, mobile-safari):** open seeded lightbox under CDP throttle — assert the active slide's `<img>` currentSrc is the 1024 tier within 1 s and does NOT hit the original URL (network log) before a pinch; after a programmatic zoom, the original request fires and `currentSrc` upgrades. This is the §2 probe turned into the arc's regression gate.
...
- **AC-1:** Lightbox open paints the clamped tier; the original is fetched only after zoom intent (network-log assertion), on all three intent paths.
