...
- **Component (Gallery):** focus relocation to next/prev/show-more/list in each availability configuration, exercised by firing `onError` while focus is on the failing button (jsdom `document.activeElement` assertions); announcement asserted on the RENDERED region — after a failure, the element with `role="log"` (located by `testId` `gallery-announce-log`) contains the message text as an appended child (R1 F3: a sink-spy alone can pass while the accessibility tree stays silent); exactly one entry per failure with the correct index; no-focus failure relocates nothing.
