...
| thumbnail fails WHILE the lightbox is open | announcement routes to the lightbox-local region (R3 F1); if the failed thumbnail is the saved trigger, the close-restore target is retargeted (R3 F2). Instant, no animation. |
