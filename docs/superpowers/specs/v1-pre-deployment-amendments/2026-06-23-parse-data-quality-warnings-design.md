...
**Where — the apply orchestrator, NOT Phase 1 (corrected in spec adversarial R4 F1):** the comparator needs BOTH `priorParseResult` and the next `parseResult`, AND its `dataGapItems` output must reach `Phase2Args.dataGapItems` (§6.3). `Phase1Result` is a discriminated *outcome* union (`lib/sync/phase1.ts:79`) that carries no parse data out, so computing in Phase 1 would strand the descriptors. Ownership therefore sits in each **recurring apply orchestrator** that reconstructs `priorParseResult` AND constructs the Phase-2 args — `lib/sync/runScheduledCronSync.ts` (priorParseResult reconstructed `:587-623`, includes `crewMembers`/`hotelReservations`/`rooms`/`transportation`/`contacts`), plus `lib/sync/runManualStageForFirstSeen.ts` and `lib/sync/retrySingleFile.ts` (manual re-sync / retry). A shared `detectBlockDisappearance(prior, next): { warnings: ParseWarning[]; dataGapItems: DataGapItem[] }` (`lib/sync/blockDisappearance.ts`) is invoked at that single point in each orchestrator; first-seen (`prior == null`) → returns empty (class C cannot fire first-seen). The orchestrator then (a) appends `warnings` to `parseResult.warnings` **before** the apply call that persists `shows_internal.parse_warnings`, and (b) sets `Phase2Args.dataGapItems` from the same output.
...
1. The class-C comparator (§5.3) — invoked by the apply orchestrator (`runScheduledCronSync` / manual re-sync / retry, per §5.3) — returns `dataGapItems: { blockKey, priorCount }[]` alongside the warnings.
2. The orchestrator sets `Phase2Args.dataGapItems`; `phase2` forwards them through a **new `dataGapItems` field** on `WriteAutoApplyChangesArgs` (additive, optional, defaults `[]`) at the `phase2.ts:387` call.
...
- **Class C lives in the apply orchestrator, not the parser or Phase 1** (R4 F1) — the parser is stateless; `Phase1Result` carries no parse data out; `priorParseResult` + `Phase2Args` coexist only in the orchestrators (`runScheduledCronSync.ts:587-623` + manual re-sync + retry). Do not relocate it to Phase 1.
...
- **Class C end-to-end** (`tests/sync/` real-DB cron, R4 F1): an existing-show cron re-sync where a block goes prior>0 → next 0 writes a `show_change_log` row with `change_kind = 'section_emptied'` (authoritative proof the descriptor survives orchestrator→phase2→writer, not just unit tests) AND appends `BLOCK_DISAPPEARED` to `shows_internal.parse_warnings`. **Catches:** descriptors dying at the Phase1/Phase2 boundary (R4 F1).
