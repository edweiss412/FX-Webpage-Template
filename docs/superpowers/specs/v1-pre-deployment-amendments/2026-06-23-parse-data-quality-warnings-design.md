...
**Where — the shared `processOneFile_unlocked` chokepoint, NOT Phase 1 (corrected R4 F1 + R5 F2):** the comparator needs BOTH `priorParseResult` and the next `parseResult`, AND its `dataGapItems` must reach `Phase2Args.dataGapItems` (§6.3). `Phase1Result` is a discriminated *outcome* union (`phase1.ts:79`) that carries no parse data out, so Phase 1 would strand the descriptors. The correct single home is **`processOneFile_unlocked`** (`runScheduledCronSync.ts:2416`) — the shared per-file processor that reconstructs `priorParseResult` (`:587-623`, includes `crewMembers`/`hotelReservations`/`rooms`/`transportation`/`contacts`), already runs `runInvariants(priorShow.priorParseResult, pipeline.parseResult)` (`:2555`), and per its own comment is **"Shared by cron / push / manual (all route their apply through [it])"** — so cron, push, AND manual existing-show re-sync all flow through this one point. (The first draft named `runManualStageForFirstSeen`/`retrySingleFile` — those are first-seen/onboarding-retry stagers, NOT the recurring existing-show path — R5 F2.) A shared `detectBlockDisappearance(prior, next): { warnings: ParseWarning[]; dataGapItems: DataGapItem[] }` (`lib/sync/blockDisappearance.ts`) is invoked there alongside `runInvariants`; first-seen (`prior == null`) → returns empty (class C cannot fire first-seen). `processOneFile_unlocked` then (a) appends `warnings` to `parseResult.warnings` **before** the apply call that persists `shows_internal.parse_warnings`, and (b) sets `Phase2Args.dataGapItems` from the same output.

**Per-block cardinality map (R4 F2 — `transportation` is `TransportationRow | null`, not an array):** the comparator must NOT assume `.length` for every block. `crewMembers` / `hotelReservations` / `rooms` / `contacts` are arrays (`types.ts:341-345`) → count = `.length`. `transportation` is `TransportationRow | null` → count = `transportation == null ? 0 : 1`. "Disappeared" = `priorCount > 0 && nextCount === 0`. Tests cover **every** block, including `transportation: object → null`.

**Mechanism:** `detectBlockDisappearance(prior, next)` (new, `lib/sync/blockDisappearance.ts`), invoked by the apply orchestrators above. For each stateful block, if `priorCount > 0 && nextCount === 0` (per the cardinality map) AND no `SECTION_HEADER_NO_FIELDS` warning already covers that block (header-present-but-empty is a different, already-signaled event), it emits both a `BLOCK_DISAPPEARED` `ParseWarning` and a `DataGapItem { blockKey, priorCount }`. The output is used **two ways** (both non-blocking; it is **never** a `TriggeredReviewItem` — those stage/block, violating §4):
1. **Persistence** — the orchestrator appends the `ParseWarning`s to `parseResult.warnings` **before** the apply call (flows to `shows_internal.parse_warnings` + `sync_log.parse_warnings` → the per-show Data-Quality panel, §6.5). The comparator itself is in `lib/sync/`, explicitly **not** in `lib/parser/warnings.ts` (stateless/parser-internal).
2. **Change feed** — the orchestrator sets `Phase2Args.dataGapItems` from the same output; see the §6.3 threading contract (the data flow R3 F1 flagged + R4 F1 located at the orchestrator boundary).
...
Recurring existing-show applies write change-log rows via `lib/sync/changeLog/writeAutoApplyChanges.ts` (Task 2.9), rendered by `components/admin/ChangesFeed.tsx` (data via `lib/sync/feed/readShowChangeFeed.ts`, mounted per-show `app/admin/show/[slug]/page.tsx:51`).

**Threading contract (R3 F1 — the data flow that did not exist):** today `writeAutoApplyChanges` (`WriteAutoApplyChangesArgs`, `writeAutoApplyChanges.ts:28-37`) is driven ONLY by crew diffs + `triggeredItems: TriggeredReviewItem[]` and has **no parse-warning input**; it is called once from `phase2.ts:387` (gated on `args.notableItems !== undefined`). To get `BLOCK_DISAPPEARED` into the feed without making it a `TriggeredReviewItem` (which would stage/block — forbidden by §4):
1. The class-C comparator (§5.3) — invoked inside `processOneFile_unlocked` (the shared cron/push/manual chokepoint, per §5.3) — returns `dataGapItems: { blockKey, priorCount }[]` alongside the warnings.
2. `processOneFile_unlocked` sets `Phase2Args.dataGapItems`; `phase2` forwards them through a **new `dataGapItems` field** on `WriteAutoApplyChangesArgs` (additive, optional, defaults `[]`) at the `phase2.ts:387` call.
3. `writeAutoApplyChanges` writes one change-log row per data-gap with a **new non-crew `change_kind`** (`section_emptied`). Per the writer's own contract (`writeAutoApplyChanges.ts:11-14`: non-crew kinds get `before_image = null`, notification-only, `change_kind` is STRUCTURAL never MI-*), `section_emptied` fits as a notification-only structural kind — it does NOT reuse `section_shrunk` (which carries crew-diff semantics) to avoid conflating "shrunk by N" with "emptied to 0", and to keep the de-dup against `SECTION_HEADER_NO_FIELDS` clean.
4. `AutoApplyChangeKind` union (`writeAutoApplyChanges.ts:20-26`) gains `section_emptied`; the plan confirms whether the change-kind set is itself catalog-governed (§9) and updates the `ChangeFeedEntry` render mapping for the new kind.

**Test the boundary, not just the render:** assert that given `dataGapItems`, `writeAutoApplyChanges` emits the `section_emptied` change-log row(s) (R3 F1's explicit ask) — in addition to a `ChangesFeed` render test. This replaces the first draft's broken "augment `SHOW_FIRST_PUBLISHED`" recurring plan (that alert never fires recurring — R1 F2).
...
| `section_emptied` change kind (P4) | `AutoApplyChangeKind` union (`writeAutoApplyChanges.ts:20-26`) + `ChangeFeedEntry` render mapping; verify whether the change-kind set is itself catalog-governed | add `section_emptied` (non-crew, notification-only, `before_image=null`); add the optional `dataGapItems` field to `WriteAutoApplyChangesArgs`; thread from `phase2.ts:387`; render the new kind in `ChangeFeedEntry` |
...
- **`transportation` cardinality is `null`-based, not `.length`** (R4 F2) — the per-block cardinality map (§5.3) is required; do not assume arrays.
...
- **Class C comparator** (`tests/sync/` unit on `detectBlockDisappearance`): **every** stateful block, prior>0 → next 0 → one `BLOCK_DISAPPEARED` + one `DataGapItem`. **Mandatory per-block coverage incl. `transportation: object → null`** (R4 F2 — not an array; the cardinality map must handle it). Negatives: first-seen (prior null) → empty; header-present-empty → `SECTION_HEADER_NO_FIELDS` not `BLOCK_DISAPPEARED` (no double-fire); both empty → empty. **Catches:** vanished hotel block (VB10), transportation null-cardinality bug, double-fire.
- **Class C end-to-end** (`tests/sync/` real-DB, R4 F1 + R5 F2): drive a block `prior>0 → next 0` through `processOneFile_unlocked` via **both** the cron path **and** the manual existing-show re-sync path → each writes a `show_change_log` row with `change_kind = 'section_emptied'` (authoritative proof the descriptor survives `processOneFile_unlocked`→phase2→writer, not just unit tests) AND appends `BLOCK_DISAPPEARED` to `shows_internal.parse_warnings`. **Catches:** descriptors dying at the boundary (R4 F1) + the manual existing-show path being missed (R5 F2).
...
- **P4 threading boundary** (`tests/sync/`, R3 F1): given `dataGapItems` input, `writeAutoApplyChanges` emits a `section_emptied` change-log row (assert at the writer boundary, not just the rendered feed) — AND the class-C comparator produces `dataGapItems` for a `prior>0 → next 0` block. Plus a `ChangeFeedEntry`/`ChangesFeed` render test for the new kind. **Catches:** the missing parse-warning→change-log data flow (R3 F1) and recurring drops being invisible because `SHOW_FIRST_PUBLISHED` never fires recurring (R1 F2).
