...
**Mechanism:** a comparator (new, `lib/sync/blockDisappearance.ts`) run in the sync layer after parse. For each stateful block, if `prior.<block>.length > 0 && next.<block>.length === 0` AND no `SECTION_HEADER_NO_FIELDS` warning already covers that block (header-present-but-empty is a different, already-signaled event), it produces a `BLOCK_DISAPPEARED` descriptor. The comparator output is used **two ways** (both non-blocking; it is **never** a `TriggeredReviewItem` — those stage/block, violating §4):
1. **Persistence** — append a `BLOCK_DISAPPEARED` `ParseWarning` to `parseResult.warnings` **before** persistence (flows to `shows_internal.parse_warnings` + `sync_log.parse_warnings` → the per-show Data-Quality panel, §6.5). Invoked from the phase-1 flow where `priorParseResult` is available — explicitly **not** in `lib/parser/warnings.ts` (stateless/parser-internal).
2. **Change feed** — the same descriptors are threaded to the change-log writer for the recurring-edit operator surface; see the §6.3 threading contract (this is the load-bearing data flow R3 F1 flagged as unspecified).
...
1. The class-C comparator (§5.3) returns `dataGapItems: { blockKey, priorCount }[]` alongside the warnings.
2. `phase2` passes them through a **new `dataGapItems` field** on `WriteAutoApplyChangesArgs` (additive, optional, defaults `[]`).
