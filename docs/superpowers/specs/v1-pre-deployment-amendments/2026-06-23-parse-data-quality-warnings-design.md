...
**De-dup guarantee:** because class C emits only a `parse_warning` (never a feed row) and reuses MI-7's items, there is exactly ONE Changes-feed row per disappearance (MI-7's `section_shrunk`) and ONE parse-warning (for the panel) — no `section_shrunk` + `section_emptied` duplication. The `SECTION_HEADER_NO_FIELDS` case (header present, rows cleared) is a *parser* warning that CAN co-occur with MI-7 on a recurring edit (header kept, rows emptied, prior>0). The helper's `existingWarnings` parameter (above) makes the suppression real: it skips emitting `BLOCK_DISAPPEARED` for any block whose `kind` already has a `SECTION_HEADER_NO_FIELDS` warning, so the persisted `shows_internal.parse_warnings` carries exactly one warning for that block (the more specific `SECTION_HEADER_NO_FIELDS`).
