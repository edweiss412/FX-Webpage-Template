...
| **Validation-env meta-test apply (analog of local-passes-CI-fails for DB)** | AGENTS.md cross-cutting #4 + project Vitest live-DB convention (TEST_DATABASE_URL env var with localhost fallback; .env.local is NOT auto-loaded) + mode-gated test discipline (R16 F37) | Task 0.A.4.5 step 5a runs pg-cron-coverage.test.ts against the validation project after migration apply, with `PG_CRON_COVERAGE_TARGET=validation` env var enabling the non-local URL-shape guard | Plan T5 step 1 Task 0.A.4.5 step 5a (R12 F31 + R13 F34 + R16 F37 fix) | ✅ R12 F31 + R13 F34 + R16 F37 fixed — local-dev TDD red/green (R11 F29) is necessary but not sufficient; the same contract must pass at validation-env apply time. **R16 F37 self-contradiction fix:** the R13 F34 unconditional guard (test errors if TEST_DATABASE_URL isn't validation) would break T2/T3 local red/green cycles (those legitimately run against local Supabase). Resolved via mode-gated env var: `PG_CRON_COVERAGE_TARGET=local` (default) skips the URL-shape guard for TDD cycles; `PG_CRON_COVERAGE_TARGET=validation` (set explicitly at Task 0.A.4.5 step 5a) enables the guard. Both modes run the same Layer 0a/0b/7-job assertions |
| **Doc-guard regex bidirectional regression (negative + positive paired cases)** | Same-vector recurrence calibration (R4 F9 + R6 F15 + R10 F26 + R13 F33 = 4 hits of "structural defense regex too broad") | T4.3 Step 4 expanded: every forbidden pattern has BOTH a negative-regression case (introduce a violation → expect FAIL) AND a positive-regression case (legitimate near-violation in prose/inverse/diagnostic context → expect PASS) | Plan T4.3 step 4 (R13 F33 structural-defense calibration) | ✅ R13 F33 fixed (4th hit of class) — comprehensive bidirectional regression-set proves each pattern catches real drift WITHOUT false-positiving on legitimate prose. Pattern 1 (supabase_vault.SQL-call) + Pattern 2 (net.http_post SQL-call) regexes refined to require `(` follow-context (function-call only); prose mentions allowed. 7 patterns × 2 regression cases = 14+ test cases pin the bidirectional contract |
