...
- Should `net.http_get()` calls have a timeout configured (default is 5s per pg_net)? Resolved in T3 SQL: 30s per call via `timeout_milliseconds := 30000`. Matches the longest expected sync run; surfaces in plan T3.
