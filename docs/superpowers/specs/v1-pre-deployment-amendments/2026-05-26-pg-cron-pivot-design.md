...
**Branch-protection enforcement is DORMANT per X6-D-1 (R30 F58 correction).** At HEAD `ac752d9`, GitHub branch-protection on `main` was deleted on 2026-05-20 per `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/DEFERRED.md` §X6-D-1, and both the privileged `verify-branch-protection` job + the PR-required reader `verify-branch-protection-status` in `.github/workflows/x-audits.yml:268-345` are guarded with `if: false`. M12.1 does NOT update GitHub branch-protection settings — that's out of scope until X6-D-1 reopens (re-enable triggers in X6-D-1 §"Re-enable triggers"). The M12.1 amendment to master spec AC-X.6 is **prep work** that lands now so the inventory is correct when X6-D-1 reopens; a future re-PUT of branch protection per X6-D-1's reopen procedure will read the current spec inventory via `loadRequiredChecksFromSpec()` and naturally include `x6-pg-cron-pivot`. Plan T5 Step 8 + DEFERRED.md §X6-D-1 cross-reference document the deferred-enforcement contract.

6th hit of CI-gating-inventory recurrence (R5 F11 + R8 F22 + R27 F54 + R28 F56 + R29 F57 + R30 F58). Structural defense per AGENTS.md "Structural-defense calibration": (a) the m12-plan-pg-cron-pivot-amendment.test.ts meta-test asserts `loadRequiredChecksFromSpec()` contains `x6-pg-cron-pivot` (assertion P) — catches inventory regression at CI time; (b) DEFERRED.md §X6-D-1 cross-reference (added per T5 Step 8) ensures the X6-D-1 reopener doesn't accidentally re-PUT branch protection with the stale pre-M12.1 7-gate set. Combined, the defenses are dormant-enforcement-aware: the spec amendment is load-bearing at reopen time, the workflow-level x6 job runs continuously now, and the meta-test pins the inventory.
