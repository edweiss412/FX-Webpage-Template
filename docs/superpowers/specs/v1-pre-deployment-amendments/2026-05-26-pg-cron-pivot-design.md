...
| `vault.update_secret(secret_id uuid, new_secret text default null, new_name text default null, new_description text default null) returns void` — but **DO NOT call from SQL editor with literal secret values** per R23 F47 | Supabase Vault docs + https://github.com/supabase/vault#turning-off-statement-logging | Plan T5 Task 0.A.4.5 step 3 (Vault populate) MUST use Vault Dashboard UI, NOT SQL editor | Plan T5 step 1 (R2 F4 + R23 F47 fix) | ✅ R2 F4 + R23 F47 fixed — R23 F47 caught that pasting the bearer token into SQL editor's `vault.update_secret(<id>, '<token>', ...)` exposes the secret in PostgreSQL statement logs (per Supabase Vault docs warning). The fix mandates Vault UI for real secret entry; SQL `vault.update_secret(...)` is reserved for cases where the secret value is sourced from a non-logged channel (e.g., another vault function output). All M12.1 cron-secret-population goes via UI |
