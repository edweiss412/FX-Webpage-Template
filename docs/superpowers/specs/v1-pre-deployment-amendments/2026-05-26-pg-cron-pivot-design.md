...
**Branch-protection enforcement is DORMANT per X6-D-1 (R30 F58 correction).** At HEAD `ac752d9`, GitHub branch-protection on `main` was deleted on 2026-05-20 per `docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/DEFERRED.md` §X6-D-1, and both the privileged `verify-branch-protection` job + the PR-required reader `verify-branch-protection-status` in `.github/workflows/x-audits.yml:268-345` are guarded with `if: false`. M12.1 does NOT update GitHub branch-protection settings — that's out of scope until X6-D-1 reopens (re-enable triggers in X6-D-1 §"Re-enable triggers"). The M12.1 amendment to master spec AC-X.6 + the full-surface master-spec sweep (R31 F60) is **prep work** that lands now so the inventory + operator-facing copy is correct when X6-D-1 reopens; a future re-PUT of branch protection per X6-D-1's reopen procedure will read the current spec inventory via `loadRequiredChecksFromSpec()` and naturally include `x6-pg-cron-pivot`, and the operator/incident-responder reading `BRANCH_PROTECTION_DRIFT` catalog + message copy will see the correct 8-name list. Plan T5 Step 4c + Step 4c.1 + Step 8 + DEFERRED.md §X6-D-1 cross-reference document the deferred-enforcement contract.

**Master spec surfaces touched by M12.1 (R31 F60 — full sweep, not just AC-X.6).** The R29 F57 fix amended AC-X.6 alone, but 4 other master-spec surfaces enumerate "seven required" / "SEVEN required" / "all seven" / "7th required" + the 7-name verbatim list and are operational copy at X6-D-1 reopen time:
- Line ~2844: §12.4 `BRANCH_PROTECTION_DRIFT` catalog row — "SEVEN required" → "EIGHT required"; append `x6-pg-cron-pivot` to verbatim list.
- Line ~3087: §12.4 `BRANCH_PROTECTION_DRIFT` message field — "seven required" → "eight required".
- Line ~3744: §17.2.1 opening "verify-branch-protection-status as a 7th PR-required" — unchanged (historically accurate at introduction time).
- Line ~3746: §17.2.1 `BRANCH_PROTECTION_DRIFT` resolution — "all seven check names" → "all eight check names"; append `x6-pg-cron-pivot` to verbatim list.

**M12 spec surfaces touched by M12.1 (R33 F62 — same pattern as R31 F60 on the master spec).** The M12 spec was R70 APPROVED before X6-D-1 deferral (2026-05-20 deletion of branch protection); active "seven canonical" / "all seven" / "required-blocking" / "Phase 0 verifies branch-protection" prose at multiple §9.1 / §9.1.1 / §12 surfaces is now stale and contradicts T5 Step 8's X6-D-1 dormancy contract. M12.1 sweeps these as adjacent surfaces to the gate-#8 amendment:
- Line ~803 (§9.1 CI gates row): 7→8 bump + X6-D-1 dormancy note (branch protection deleted; gates still run, no longer block).
- Line ~808 (§9.1.1 opening): same dormancy reframe.
- Line ~818 (§9.1.1 table row 7): "Required-blocking PR check" → "(X6-D-1 dormant) PR check that WOULD ...".
- Line ~822 (Phase 0 sub-task): "confirm all seven set as required-blocking" → "branch protection is DELETED per X6-D-1; not part of Phase 0 verification".
- Line ~952 (§12 self-consistency): "7 canonical CI gates" → "8 canonical CI gates" + X6-D-1 dormancy note.
- Line ~954 (§12 Build-vs-runtime): "seven CI gates" → "eight CI gates" + X6-D-1 dormancy note.

8th hit of CI-gating-enforcement recurrence (R5 F11 + R8 F22 + R27 F54 + R28 F56 + R29 F57 + R30 F58 + R31 F60 + R33 F62). Structural defense per AGENTS.md "Structural-defense calibration": (a) assertion P (`loadRequiredChecksFromSpec()` returns `x6-pg-cron-pivot`) catches AC-X.6 regression at CI time; (b) assertion Q (BRANCH_PROTECTION_DRIFT catalog row contains backtick literal) catches catalog-row regression; (c) assertion R (master spec zero matches for "seven required" / etc. outside HISTORICAL) catches operator-copy drift across the full master-spec surface; (d) assertion S (M12 spec zero matches for "seven canonical" / "all seven" / "required-blocking" / etc. outside HISTORICAL) catches the same class on the M12 spec; (e) DEFERRED.md §X6-D-1 cross-reference ensures the reopener doesn't accidentally re-PUT branch protection with stale gate copy. Combined, the defenses cover (a) parser layer, (b) catalog row, (c-d) full prose surface on BOTH spec files, (e) operator-handoff narrative — dormant-enforcement-aware at every layer M12.1 touches.
...
| **CI gate wiring (.github/workflows/x-audits.yml + package.json test:audit:xN script naming)** | M11 + M12 x-audits convention | T4.4 step 1-4 + new test:audit:x6-pg-cron-pivot script + new x6-pg-cron-pivot CI job (R28 F56 — no `audit-` prefix per existing x1-x5 jobname pattern + `scripts/generate-traceability.ts:193` regex `^x[1-6]-`) | Plan T4.4 | ✅ R5 F11 fixed — CI-safe tests (no-vercel-cron + pg-cron-pivot-doc-guard) wired into x-audits.yml; pg-cron-coverage marked local-only (live DB required, out of CI scope). R28 F56 rename: `x6-pg-cron-pivot` (not `audit-x6-pg-cron-pivot`) so the traceability inventory parser recognizes it as a required-audit job. |
