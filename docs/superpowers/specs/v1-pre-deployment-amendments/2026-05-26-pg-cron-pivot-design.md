...
| **Validation-env meta-test apply (analog of local-passes-CI-fails for DB)** | AGENTS.md cross-cutting #4 + project Vitest live-DB convention (TEST_DATABASE_URL env var with localhost fallback; .env.local is NOT auto-loaded) | Task 0.A.4.5 step 5a runs pg-cron-coverage.test.ts against the validation project after migration apply, with TEST_DATABASE_URL set to validation pooler URL + in-test guard rejecting localhost/127.0.0.1/:54322 targets | Plan T5 step 1 Task 0.A.4.5 step 5a (R12 F31 + R13 F34 fix) | ✅ R12 F31 + R13 F34 fixed — local-dev TDD red/green (R11 F29) is necessary but not sufficient; the same contract must pass at validation-env apply time. R13 F34 caught that the original Step 5a relied on `.env.local` (which Vitest doesn't auto-load); corrected to explicit `TEST_DATABASE_URL=<pooler-URL> pnpm test ...` + test-side guard asserting the target is not local. Without the guard, operator could think they ran the validation gate while silently hitting local Supabase |
