...
| **Validation-env meta-test apply (analog of local-passes-CI-fails for DB)** | AGENTS.md cross-cutting #4 (extended to validation Supabase project as a separate env gate from local Supabase dev) | Task 0.A.4.5 step 5a runs pg-cron-coverage.test.ts against the validation project after migration apply | Plan T5 step 1 Task 0.A.4.5 step 5a (R12 F31 fix) | ✅ R12 F31 fixed — local-dev TDD red/green (R11 F29) is necessary but not sufficient; the same contract must also pass at validation-env apply time to catch env-specific drift (baked URL, Vault path, route URLs). The Task 0.A.4.5 step 5a addition is the validation-env equivalent of CI green vs local green |
