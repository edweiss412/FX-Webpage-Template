...
7th hit of CI-gating-enforcement recurrence (R5 F11 + R8 F22 + R27 F54 + R28 F56 + R29 F57 + R30 F58 + R31 F60). Structural defense per AGENTS.md "Structural-defense calibration": (a) assertion P (`loadRequiredChecksFromSpec()` returns `x6-pg-cron-pivot`) catches AC-X.6 regression at CI time; (b) assertion Q (BRANCH_PROTECTION_DRIFT catalog row contains backtick literal) catches catalog-row regression; (c) assertion R (master spec zero matches for "seven required" / etc. outside HISTORICAL) catches operator-copy drift across the full master-spec surface; (d) DEFERRED.md §X6-D-1 cross-reference ensures the reopener doesn't accidentally re-PUT branch protection with stale 7-gate copy. Combined, the defenses cover (a) parser layer, (b) catalog row, (c) full prose surface, (d) operator-handoff narrative — dormant-enforcement-aware at every layer.
