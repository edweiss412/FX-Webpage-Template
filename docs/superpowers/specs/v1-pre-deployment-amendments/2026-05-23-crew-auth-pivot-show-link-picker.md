...
│        → TERMINAL: return `no_auth` (P-R27 Fix-1).           │
│        SignInOrSkipGate renders "signed in as someone else". │
│        **DO NOT fall through to step 5** — that would let a  │
│        stale picker cookie from a prior user on a shared     │
│        device resolve as that prior identity. CRITICAL       │
│        identity-leak vector closed in P-R27.                 │
│                                                              │
...
│         wire outcomes (R41-R35 simplified; P-R29/P-R30      │
│         amendment — identity_invalidated.reason now accepts │
│         BOTH 'claimed_after_pick' AND 'session_mismatch'    │
│         but both reasons share the same wire outcome:       │
│           - { kind: 'resolved', crewMemberId }               │
│           - { kind: 'no_selection' }                         │
│           - { kind: 'epoch_stale' }                          │
│           - { kind: 'removed_from_roster' }                  │
│           - { kind: 'identity_invalidated', reason:          │
│               'claimed_after_pick' | 'session_mismatch' }    │
│             (R41-R8 base; R41-R35 simplified; P-R29/P-R30    │
│             added 'session_mismatch' for shared-device       │
│             API defense; see §6.1 case 9)                    │
...
- `app/api/auth/picker-bootstrap/route.ts` (new, R41-R2 amendment; R41-R3 atomic claim-and-mint; R41-R5 intent-token CSRF; R41-R6 one-show write only; **R41-R7 fail-closed on RPC failure**; **R41 P-R22 + P-R23 flow restructured**). Route Handler that legally mints `__Host-fxav_picker` for a Google-signed-in user. **Flow:** (1) read `next` AND `t` (intent token); validate `next` against the `validateNextParam.ts` allowlist (`^/show/[a-z0-9-]+/[0-9a-f]{64}$` per P-R12 Fix-1); **(1.5) verify the intent token** — `t = base64url(payload) + '.' + base64url(HMAC-SHA256(payload, PICKER_COOKIE_SIGNING_KEY))` where `payload = JSON({slug, shareToken, exp})`. Decoder rejects: missing `t`, malformed format, expired (`exp < now`), HMAC mismatch, OR `{slug, shareToken}` not matching the `next` URL's parsed slug+token. **A failed intent check returns 403** (NOT 302). The token's 60s expiry bounds replay; **(2) resolve `target_show_id` (P-R22/P-R23 amendment — moved BEFORE the session check because the live `validateGoogleSession` helper at `lib/auth/validateGoogleSession.ts:48` requires a `showId` context arg)**: parse `next` to extract `(slug, shareToken)` → call `serviceRole.rpc('resolve_show_by_slug_and_token', { p_slug, p_share_token })` with `{ data, error }` destructure + try/catch (AGENTS.md invariant 9). On thrown OR returned-error → fail-closed 502 with cataloged terminal HTML (crew-facing copy is shared across both bootstrap failure surfaces — same "Couldn't sign you in." page) AND best-effort admin_alert emission with code **`PICKER_BOOTSTRAP_RESOLVE_SHOW_FAILED`** (P-R24 amendment — distinct from the post-session `PICKER_BOOTSTRAP_RPC_FAILED` code; at this point `viewer.email` is not yet available, so the context is email-less `{ stage: 'resolve_show', slug, rpc_error_code, rpc_error_message, route }`). On `data: null` (slug/token mismatch — schema integrity bug post-intent-token, or concurrent rotation between intent-token mint and bootstrap arrival) → 403 with cataloged terminal HTML, NO alert (this is a user/token mismatch, not infra fault); (3) `validateGoogleSession(req, { showId: target_show_id })` — three-arm result: `terminal_failure` → cataloged terminal HTML; `continue` (no session OR `GOOGLE_NO_CREW_MATCH`) → 302 to `next` with NO cookie set (user proceeds to page which renders SignInOrSkipGate or picker); `success` with `viewer: { email, showId, crewMemberId }` → continue to step 4 using `viewer.email` (already canonicalized by the helper — do NOT re-canonicalize); (4) invoke `claim_oauth_identity(viewer.email)` via service-role client. **(4a) RPC infra failure — R41-R7 fail-closed contract**: if the RPC returns an error or throws, the handler MUST NOT 302 back to `next` (that would re-trigger the same detection → infinite loop). Instead, the handler returns an HTTP 502 with the cataloged terminal-failure UI rendered as HTML (same template as the page-route `infra_error` branch). The user sees a "Couldn't sign you in — please try again in a moment." page with a manual retry link. No cookie set. Structured log carries `PICKER_BOOTSTRAP_RPC_FAILED` operator code. Bounded recovery: user reload retries. The redirect-chain depth is structurally capped at 1 hop (page → bootstrap → 502 terminal page); (4b) on success, the RPC returns `{ claimed_count, claimed_rows, shows, mint_safe_t_millis }` (R41-R22/R41-R23: the `mint_safe_t_millis` field is the DB-side `clock_timestamp()`-derived value strictly greater than every `claimed_via_oauth_at` on the user's rows); (5) **one-show write contract (R41-R6 lost-update repair; R41-R24 timestamp source pinned)**: `target_show_id` was resolved in step 2; reuse it. Find the entry for `target_show_id` in `result.shows`. If present, read the request's picker envelope, modify ONLY that show's entry to `{ id: crew_member_id, e: picker_epoch, t: result.mint_safe_t_millis }` (NOT `Date.now()` or any JS clock — R41-R24 pin: the bootstrap MUST use the DB-side value from the RPC's response; app-server/DB-server clock skew or same-millisecond races would otherwise produce a cookie.t that fails the resolver's `cookie.t > claim_epoch_millis` check, causing the legitimate OAuth user to be wrongly rejected as claimed_after_pick). Leave every other entry untouched, write via `cookies().set('__Host-fxav_picker', signEnvelope(envelope), PICKER_COOKIE_OPTIONS)`. If `target_show_id` is NOT in `result.shows` (user not on this show's roster), write NO cookie. **Structural defense (R41-R24)**: the §10.1 picker-cookie meta-test grep-asserts the bootstrap route handler does NOT call `Date.now()`, `new Date()`, `performance.now()`, or any other JS clock source as the cookie's `t` value; the only legal source is `result.mint_safe_t_millis` returned by the RPC. The handler never touches entries for shows other than `target_show_id`; (6) 302 to `next`. **No redirect loop is possible across ALL paths** (P-R27/P-R34 hardened): (a) RPC success + target in shows → mint + 302 → page step 6 catches and renders `_ShowBody`; (b) (R41-R35 removed — ambiguous branch no longer exists); (c) RPC success + Google-session user not on target show's roster (GOOGLE_NO_CREW_MATCH at the page) → bootstrap returns `continue` arm → 302 with NO mint → page **step 4(e) is TERMINAL `no_auth` with `reason: 'google_mismatch'` per P-R27 Fix-1** — renders `<SignInOrSkipGate>` Mode B (signed-in-as-someone-else); **DOES NOT fall through to step 5 picker-cookie path** (closes the shared-device identity-leak vector where Alice's stale cookie would otherwise resolve Bob's session as Alice); (d) RPC failure → 502 terminal page, NO 302 (R41-R7 fail-closed); (e) no Google session → 302 with no mint → page step 4(a) falls through to step 5 (existing picker cookie if any) OR terminal `no_auth/first_contact` if no cookie either (gate or picker, no re-redirect); (f) invalid intent token → 403, no redirect. **This Route Handler IS in the cookie-mutator allowlist** alongside `selectIdentity`, `clearIdentity`, `cleanupStaleEntry`; `/auth/callback/route.ts` is NOT in the allowlist (R41-R6 — callback does not write picker cookies). **Anti-overwrite contract**: handler MUST NOT modify entries for shows other than `target_show_id`. The §10.2 regression test asserts a pre-existing entry for Show-Y persists byte-identical across a picker-bootstrap call for Show-X.
- `app/show/[slug]/[shareToken]/_PickerInterstitial.tsx` (new, Server Component) — renders the picker. Reads the show's roster from `crew_members` via service-role client. Renders the `<form action={selectIdentity}>` markup. No client-side JavaScript needed for the picker itself. When the resolver returns **any of `epoch_stale` | `removed_from_roster` | `identity_invalidated`** (R41-R15 base; R41-R35 + P-R29/P-R30 amendment — `identity_invalidated.reason` accepts BOTH `'claimed_after_pick'` and `'session_mismatch'`, both sharing the same cleanup flow + banner copy), also mounts `<StaleCleanupAutoSubmit>` (below) once with the resolver's `expectedEpoch` and `expectedCrewMemberId`. The compare-and-delete contract is identical across all THREE stale-state kinds AND both `identity_invalidated` reasons; the banner copy differs per the §4.1 step-8 routing table.
...
- `lib/auth/picker/resolveShowPageAccess.ts` (new, R41 Fix-3; R41-R3 pure-resolver; R41-R7: union enumerated; **R41-R12: unpublished-precedence corrected; R41-R13: identity_invalidated arm added; R41-R14: kind/outcome wording harmonized; P-R29/P-R30 Fix-1: identity_invalidated.reason carries 'claimed_after_pick' | 'session_mismatch'**). **Page-route-only auth chain helper.** Called exclusively by `app/show/[slug]/[shareToken]/page.tsx`. Encapsulates the resolver chain **in this exact order**: archived → admin precedence → **unpublished (R41-R10 step 3.5 — MUST come before any Google-session branch to prevent the bootstrap-loop class)** → Google-session-matching-crew-row → existing picker cookie. The helper is defense-in-depth alongside the page-route's own ordered chain; the page can also pre-check published before calling the helper, but the helper MUST also enforce the ordering internally. Imports `validateGoogleSession`. **The helper is PURE — it never encodes cookies and never calls `cookies().set()`.** Returns a discriminated union of **EXACTLY ELEVEN discriminant `kind` values** — kind count, not reason count. The `identity_invalidated` arm carries two reasons (`'claimed_after_pick'` + `'session_mismatch'`) but is ONE kind (one wire outcome), so 11 kinds = 11 wire outcomes; caller exhaustiveness checks (`assertNever`) fail if any kind is missing. The eleven kinds (the `ambiguous_email` slot at position 4 is vacated per R41-R35 and the numbering preserved to avoid renumbering downstream citations):
  ...
  10. `{ kind: 'identity_invalidated', expectedEpoch, expectedCrewMemberId, reason: 'claimed_after_pick' | 'session_mismatch' }` **(R41-R13; R41-R35 simplified; P-R29/P-R30 Fix-1 added 'session_mismatch')** — cookie's identity is no longer valid. `'claimed_after_pick'` fires when the cookie pre-dates an OAuth claim on the same crew row; `'session_mismatch'` fires when an active Supabase session's canonical email differs from the cookie's crew_members.email (shared-device API defense). Both reasons render the picker with `PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER` + cleanupStaleEntry form. Surfaced by §6.1 resolver chain step 9.
...
3. **Picker block** — left/right padded 16px on a 390px viewport.
   ...
   - **Optional banner row** (R41-R17 base; R41-R35 simplified; P-R29/P-R30 amendment — present when resolver returns ANY of `epoch_stale`, `removed_from_roster`, OR `identity_invalidated` (with EITHER reason `'claimed_after_pick'` OR `'session_mismatch'`) from §6.1) — one-line copy in a 12px medium-weight inline note, FXAV-orange-tinted background (`bg-orange-100` / `bg-orange-900/30` in dark mode). 8px above the list. Banner copy selected per the §7.4 mode table: epoch-stale, removed-from-roster, or identity-claimed (three modes — both `identity_invalidated` reasons share the same banner code PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER and same copy, since the user-facing recovery action is identical).
...
- `PICKER_EPOCH_RESET` (P-R18 + P-R19 amendment): emitted by `lib/auth/picker/resetPickerEpoch.ts` (Task B6) when `reset_picker_epoch_atomic` RPC succeeds. **Producer**: `upsertAdminAlert({ showId, code: 'PICKER_EPOCH_RESET', context })` in that file ONLY. **Context shape**: `{ show_id: uuid, new_epoch: int, admin_email_hash: text }`. **NEVER raw `admin_email`** — P-R10 hashed-email contract; the helper `hashForLog(canonicalEmail)` from `lib/email/hashForLog.ts` (Task A6.5) is the single source of truth. `show_slug` is NOT in the context (P-R18 amendment: H7 grep-asserts the slug is not duplicated into JSONB context since it can drift from the live `shows.slug` and admin tools already index by `show_id`). Best-effort emission: failure does NOT roll back the reset. Informational.
- `PICKER_SELECTION_RACE` (P-R18 + P-R19 amendment): emitted by `lib/auth/picker/cleanupStaleEntry.ts` (Task B5) ONLY when the "cleaned" code path materializes a stale-cookie deletion. **NOT emitted by `selectIdentity`** — the pre-P-R18 spec wording said `selectIdentity` committed a stale-epoch write; that pattern is OBSOLETE. The P-R8/P-R10 RPC-based atomic selection (`select_identity_atomic`) re-validates inside the per-show advisory lock and rejects stale-epoch writes at the RPC layer — there's no cookie write under stale epoch to alert about. The race that DOES still materialize is: cookie was written before an admin Reset → resolver later detects `cookie.e < shows.picker_epoch` → page renders `<StaleCleanupAutoSubmit>` → POSTs to cleanupStaleEntry → the compare-and-delete deletes the stale entry. That's the PICKER_SELECTION_RACE signal. **Producer**: `upsertAdminAlert({ showId, code: 'PICKER_SELECTION_RACE', context })` in `lib/auth/picker/cleanupStaleEntry.ts` ONLY, on the "cleaned" branch (NOT the "noop" race-resolved-itself branch). **Context shape**: `{ show_id: uuid, stale_epoch: int, stale_crew_member_id: uuid }`. **NO email/PII fields** — this is a cleanup-side race signal, not an identity event. Best-effort emission. The alert is the operator's window into how often this happens in practice. If frequent, follow-up work could pre-emptively invalidate cookies on epoch bump rather than waiting for the resolver pass.
...
- **`PICKER_BOOTSTRAP_RPC_FAILED` (R41-R7 Fix-2; P-R10 Fix-3; P-R26 scope-correction):** emitted from `/api/auth/picker-bootstrap` step 4 when `claim_oauth_identity` returns an error or throws (POST-session — distinct from the pre-session `PICKER_BOOTSTRAP_RESOLVE_SHOW_FAILED`). Crew-facing copy: "Couldn't sign you in. Please try again in a moment." Operator-facing context: `{ attempted_email_hash, rpc_error_code, rpc_error_message, route }`. **NEVER raw `user_email`** — the hashed form via `hashForLog()` (Task A6.5) is mandatory FOR THIS CODE. On render: the handler returns 502 with the cataloged terminal-failure UI (HTML response, NOT a redirect — R41-R7 fail-closed contract prevents the infinite redirect loop the reviewer flagged). Cataloged in `lib/messages/catalog.ts`. ALSO emitted to `admin_alerts` with the same code so operators can investigate persistent failures.

**R41 P-R26 amendment — email-posture matrix across all 6 R41 admin_alerts producers.** The earlier wording "hashed form is mandatory across every R41 admin_alerts producer" was too broad — only email-bearing producers carry a hashed field. The canonical per-producer email contract:

| Code | Email field | Source | H7 guard |
|---|---|---|---|
| `OAUTH_IDENTITY_CLAIMED` | `user_email_hash` | `hashForLog(canonicalEmail)` from C7 callback | grep-asserts NO raw `user_email` |
| `PICKER_BOOTSTRAP_RPC_FAILED` | `attempted_email_hash` | `hashForLog(canonicalEmail)` from C6 step 4 | grep-asserts NO raw `user_email` |
| `PICKER_EPOCH_RESET` | `admin_email_hash` | `hashForLog(adminCtx.email)` from B6 | grep-asserts NO raw `admin_email` |
| `PICKER_BOOTSTRAP_RESOLVE_SHOW_FAILED` | NONE (email-less; pre-session) | n/a — `viewer.email` doesn't exist yet at C6 step 3 | grep-asserts NO `attempted_email_hash`/`user_email`/`share_token` |
| `CALLBACK_CLAIM_THREW` | NONE (no PII; only `error_name`) | n/a — outer catch in C7 has no email context (the `viewer` may not have resolved) | grep-asserts NO email fields |
| `PICKER_SELECTION_RACE` | NONE (cleanup-side race signal; no PII) | n/a — B5 cleanupStaleEntry has no email context | grep-asserts NO email fields |

The hash-mandatory rule applies to the THREE email-bearing producers ONLY. The THREE no-PII producers are structurally email-less; H7 grep-asserts each email field's ABSENCE in those emission sites.
...
- `OAUTH_IDENTITY_CLAIMED`: emitted to `admin_alerts` when `claim_oauth_identity()` updates 1+ crew rows (§4.8). **R41 P-R9 Fix-3**: emitted PER-ROW (one alert per claimed row, scoped to that row's `show_id` — NOT one aggregate alert with `show_id=NULL`). Context: `{ crew_member_id: uuid, show_id: uuid, claimed_at_millis: bigint, user_email_hash: text }`. **NEVER raw `user_email`** — this email-bearing producer uses `user_email_hash` per the P-R26 email-posture matrix above. The `_metaAdminAlertCatalog` registry asserts this code is cataloged AND that no `user_email` (raw) or `claimed_count` field appears in any OAUTH_IDENTITY_CLAIMED emission site (these would be aggregate-shape regressions). Informational.
- `PICKER_BOOTSTRAP_RPC_FAILED`: emitted to `admin_alerts` when `claim_oauth_identity` returns error OR throws inside picker-bootstrap (§4.7 R41-R7 fail-closed). Context: `{ attempted_email_hash: text, rpc_error_code: text, rpc_error_message: text, route: text }`. NEVER raw email. Emitted AFTER `validateGoogleSession` succeeds, so `viewer.email` is available for hashing.
- **R41 P-R24 Fix-1 — `PICKER_BOOTSTRAP_RESOLVE_SHOW_FAILED`**: emitted to `admin_alerts` when `resolve_show_by_slug_and_token` returns error OR throws inside picker-bootstrap step 3 (R41 P-R22 amendment moved show resolution BEFORE session validation). This is the **PRE-SESSION** counterpart to PICKER_BOOTSTRAP_RPC_FAILED — at this point in the flow `validateGoogleSession` has NOT yet run, so there is no `viewer.email` to hash. Context (email-less): `{ stage: 'resolve_show', slug: text, rpc_error_code: text, rpc_error_message: text, route: text }`. NEVER `attempted_email_hash` / raw email / share_token / share_token_hash — operator triage uses occurrence_count + slug + rpc_error_code. Crew-facing copy is SHARED with PICKER_BOOTSTRAP_RPC_FAILED — the user sees the same "Couldn't sign you in. Please try again in a moment." 502 terminal page either way; the operator codes differ only for triage. **NOT emitted** when the RPC returns `data: null` with no error (slug/token mismatch — a user/token signal that returns 403 instead of 502, no alert).
...
- **Google-session auto-resolution (R41-R2 revised — redirect-bootstrap flow)** — two sub-cases:
  ...
  - **Negative test (P-R27 Fix-1 hardened)**: Google session whose email does NOT match any crew row for this show renders the SignInOrSkipGate (NOT the picker, NOT a stale picker cookie's identity). Step 4(e) is TERMINAL no_auth — does NOT fall through to step 5. Redirect-bootstrap NOT triggered. **R41 P-R27 Fix-1 shared-device identity-leak regression**: seed a valid `__Host-fxav_picker` envelope with Alice's entry for Show-X (Alice has a real crew row, cookie is signed correctly). THEN simulate Bob signing in via Google (Bob's email matches NO crew row on Show-X). Bob visits `/show/<slug-X>/<token-X>`. Pre-Fix-1: step 4(e) falls through to step 5; the resolver reads Alice's still-valid picker cookie; Bob's request renders as Alice — Bob sees Alice's identity. Post-Fix-1: step 4(e) is terminal; the resolver returns `no_auth` (NOT `resolved`); the page renders SignInOrSkipGate with a "signed in as someone else / sign out to continue" UI. Bob CANNOT see Alice's identity. Tests assert: (a) the page response renders SignInOrSkipGate; (b) the rendered HTML does NOT contain Alice's name or crew-row data; (c) the API consumer matrix for this request returns 401 (no picker cookie consumed); (d) NO Set-Cookie clearing Alice's entry (clearing requires explicit Server Action — the page route helper cannot mutate cookies; the gate's "Continue as guest" button is the user-driven mutation path).
...
- **Stale-credential mapping across all API consumers (R41-R8 base; R41-R35 simplified; P-R29/P-R30 amendment)** — for every route in §6 that calls `resolvePickerSelection` (subscriber-token, version, asset/{diagram,reel,agenda}, report), the test matrix asserts: (a) `kind: 'epoch_stale'` → 401 with no resource body; (b) `kind: 'removed_from_roster'` → 401 with no resource body; (c) `kind: 'identity_invalidated'` → **HTTP 410** (stale-but-knowable; the cookie's identity exists but is no longer valid). The matrix MUST parameterize this row over BOTH reason values `'claimed_after_pick'` and `'session_mismatch'` — both produce 410 (the shared-device API defense per P-R30 Fix-1 requires this); a regression that returns 401 for either reason fails CI. (d) `kind: 'show_unavailable'` → 410; (e) `kind: 'infra_error'` → 500; (f) `kind: 'no_selection'` → 401 (no credential at all). Each status is distinguishable on the wire so `ShowRealtimeBridge` can treat 401 as `forceRefresh` and 410 as "this show is gone or your identity is no longer valid."
- **Pre-claim picker cookie is invalidated after OAuth claim (R41-R8 HIGH regression)** — three sub-scenarios:
  - **Sub-A: a user picked bypass identity, then a different user signs in via OAuth for that identity**. Fixture: T0: Mallory picks Alice's crew row via selectIdentity; Alice's `claimed_via_oauth_at` is NULL, so the pick succeeds; Mallory's cookie has `{Show-X: {id: Alice's crew id, e: 1, t: 100}}`. T1 (later): Alice signs in via Google OAuth; callback fires `claim_oauth_identity`; Alice's row's `claimed_via_oauth_at = 200` (later than 100). T2: Mallory's browser makes a request to `/show/<slug-X>/<token-X>` or any API consumer for Show-X with her T0 cookie. Tests assert: (a) `resolvePickerSelection` returns `{ kind: 'identity_invalidated', reason: 'claimed_after_pick' }` — derived from `cookie.t (100) < extract(epoch from claimed_via_oauth_at) (200)`; (b) page route renders the picker with the `PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER` banner; (c) Mallory's row for Alice (deactivated per §7.2 R41-R6 because `claimed_via_oauth_at IS NOT NULL`) cannot be re-picked; (d) any API consumer returns **HTTP 410** (P-R29/P-R30 amendment — identity_invalidated maps to 410 stale-but-knowable; pre-P-R29 wording said 401); (e) the `cleanupStaleEntry` Server Action receives the right `expectedEpoch + expectedCrewMemberId` and compare-and-deletes Mallory's stale entry on auto-submit.
...
| Parent spec amendment block-quote | `2026-04-30-fxav-crew-pages-v1.md:7-10` | Intro |
| Parent spec §7.2 signed-link format (obsolete) | `2026-04-30-fxav-crew-pages-v1.md:1926` | §6 |
| Parent spec §7.4 role-derivation contract (kept) | `2026-04-30-fxav-crew-pages-v1.md:2307-2316` | §1, §4.1 |
| Parent spec §7.3 routing table | `2026-04-30-fxav-crew-pages-v1.md:2288-2306` | §6 |
| Parent spec §8 Realtime contract | `2026-04-30-fxav-crew-pages-v1.md:2328-2345` | §4.6 |
| Parent spec cookie-name growth reasoning | `2026-04-30-fxav-crew-pages-v1.md:1949` | Resolved Decision 4 |
