...
## §3 — §12.4 warning codes (the 5-part lockstep, ×3)
...
- **`tests/cross-cutting/codes.test.ts` (orphan-codes / `x1-catalog-parity`) — EXTENDED.** ALL THREE new `code:` literals (`TRAVEL_FLIGHT_NAME_UNMATCHED`, `TRAVEL_FLIGHT_UNPARSEABLE`, `TRAVEL_FLIGHT_AMBIGUOUS_TABLE`) in `travelFlightWarnings.ts` are producers the orphan gate scans; each MUST appear in §12.4 + the catalog (the §3 5-part lockstep per code). This is the gate that fails if a code is added without the catalog rows.
- **`tests/cross-cutting/no-raw-codes.test.ts` (`x2-no-raw-codes`) — EXTENDED via regeneration.** The 3 codes are `lib/parser` literals, so `pnpm gen:internal-code-enums` must regenerate `lib/messages/__generated__/internal-code-enums.ts` to include them (beside the `AGENDA_*`); CI checks the manifest with `git diff --exit-code`, so a stale manifest fails x2. Both generated manifests (`spec-codes.ts`, `internal-code-enums.ts`) are regenerated + staged in the codes commit.
...
9. **All three `code:` literals** (`TRAVEL_FLIGHT_NAME_UNMATCHED`, `TRAVEL_FLIGHT_UNPARSEABLE`, `TRAVEL_FLIGHT_AMBIGUOUS_TABLE`) are present in §12.4 + `catalog.ts` (`x1` passes) AND in the regenerated `internal-code-enums.ts` (`x2`/`no-raw-codes` passes, `git diff --exit-code` clean). Catches: the missing 5-part lockstep (incl. both generated manifests).
...
- §12.4 lockstep gates: `tests/cross-cutting/codes.test.ts` orphan-codes + `x1-catalog-parity`; `tests/cross-cutting/no-raw-codes.test.ts` `x2-no-raw-codes`; `lib/messages/catalog.ts`; `lib/messages/__generated__/spec-codes.ts` (`pnpm gen:spec-codes`) + `lib/messages/__generated__/internal-code-enums.ts` (`pnpm gen:internal-code-enums`, `tsx scripts/extract-internal-code-enums.ts`).
