...
- **Advisory-lock posture (R2 fix — invariant 2 applies to fixtures):** `shows` is a locked table; any fixture write to it must hold `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))`. The compliant design: the show-state fixtures (`pending_review`, archived) are seeded **through `supabase/seed.ts`**, whose `seedSql` transaction already acquires the per-show advisory lock for every seeded `drive_file_id` (`supabase/seed.ts:517-523`) — and `help-docs-setup` already runs `pnpm db:seed` (`help-docs-setup.ts:16-19`), so the walker gets these states for free, deterministically, with the lock held at exactly one layer (the seed transaction — single-holder rule; no new holder is introduced). Per-test direct service-role writes remain ONLY for tables outside the invariant-2 locked list (`app_settings`, `admin_alerts`) — precedent: `admin-banner.spec.ts:355-357` explicitly avoids mutating `shows` for the same reason.
