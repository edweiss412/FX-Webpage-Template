...
- **Advisory-lock posture (R2 fix — invariant 2 applies to fixtures):** `shows` is a locked table; any fixture write to it must hold `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))`. The compliant design: ALL walker-only fixture rows on locked tables — the three show-state fixtures (`pending_review`, archived, `drive_error`) AND the relocated first-seen `pending_syncs` row — are seeded by a **walker-only seed extension**, a standalone script (`supabase/seedWalkerFixtures.ts`; `seed.ts` itself runs `main()` at import so the extension cannot import it) that reuses `seedSql`'s locked-transaction pattern (`begin; select pg_advisory_xact_lock(hashtext('show:' || <drive_file_id>)); …` — `supabase/seed.ts:517-523`) over its four sorted ids, invoked by `help-docs-setup` AFTER its existing `pnpm db:seed` step (`help-docs-setup.ts:16-19`). Nothing walker-only goes into base `db:seed`. The lock is held at exactly one layer (that transaction — single-holder rule; no new holder is introduced). **Why an extension and not base `db:seed` (R3 interaction):** the screenshot captures seed from the same base `db:seed`; putting a `pending_review` show into the base seed would add a visible row + the §4.3 legend to the `dashboard-overview` capture and silently churn its baseline. The extension keeps capture DB state byte-identical. Its fixture rows use the seed-prefix naming convention (`seedDrivePrefix = "seed-fixture:"` — `supabase/seed.ts:16`) so any subsequent `pnpm db:seed` (which every capture setup runs first) clears them — no cross-contamination on a shared local stack. **Cleanup lock (R6 CRITICAL fix):** `seed.ts`'s cleanup deletes `shows` by wildcard prefix (`seed.ts:530-537`) while its lock acquisition covers only the enumerated base fixture ids (`:517-523`) — once walker rows exist, that wildcard delete would mutate `shows` rows whose locks it does not hold. The base `seedSql` therefore gains, BEFORE its wildcard deletes and in the same transaction, a **lock-and-delete-by-the-same-id-set** pattern (R11 union + R15 race fix): the transaction materializes the prefix-matching ids from the UNION of every locked table the cleanup touches (`shows`, `pending_syncs`, `pending_ingestions` — R11: a `pending_syncs`-only fixture id has no `shows` row) into a `create temporary table _locked_seed_ids on commit drop`, acquires `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))` over that table `order by drive_file_id`, and the locked-table DELETEs filter `where drive_file_id in (select drive_file_id from _locked_seed_ids)` — NOT by naked `like` wildcard. Under READ COMMITTED a row committed by a concurrent extension run AFTER the sweep would otherwise be wildcard-deletable without its lock (R15); deleting by the locked snapshot makes that row simply survive this cleanup (correct — its lock isn't held; the extension's own idempotent delete owns it). The non-locked `sync_audit` delete may stay a plain wildcard. Still single-holder (one transaction). **Lock ordering is deterministic everywhere (R7 CRITICAL):** the sweep carries `order by drive_file_id`, and the walker extension acquires its per-id locks in the same `drive_file_id`-sorted order before any delete/insert — two transactions taking overlapping lock sets in different orders is the textbook advisory-lock deadlock (concurrent `db:seed` cleanup vs extension re-run). Per-test direct service-role writes remain ONLY for tables outside the invariant-2 locked list (`app_settings`, `admin_alerts`) — precedent: `admin-banner.spec.ts:355-357` explicitly avoids mutating `shows` for the same reason. **The walker becomes read-only on every locked table (R10 CRITICAL):** the existing `firstSeenStagedId` fixture delete/inserts `pending_syncs` — an invariant-2 locked table — through the PostgREST admin client with no advisory lock (`deep-link-walker.spec.ts:136-168`, a pre-existing latent violation this milestone's walker rework inherits). PostgREST cannot hold a multi-statement `pg_advisory_xact_lock` transaction, so that fixture MOVES into the locked seed extension — with its `drive_file_id` RENAMED to `seed-fixture:walker-first-seen` (joining the sorted four-id lock set, the `seed-fixture:%` cleanup sweep, base `db:seed` cleanup, and capture isolation; the old unprefixed id must not survive anywhere — the close-out grep includes it), delete+insert with the existing fixed staged UUID; the walker's `firstSeenStagedId` becomes a pure lookup that loud-throws when absent. A structural pin asserts the walker spec + its helpers contain NO `.insert(`/`.update(`/`.delete(` builder call on any invariant-2 locked table (`shows`, `crew_members`, `crew_member_auth`, `pending_syncs`, `pending_ingestions`) — per-test mutations stay legal only for non-locked tables.
- **Row 4 (legend):** the walker-only extension seeds one show with `last_sync_status = 'pending_review'` (maps to bucket `review` — `lib/admin/syncStatus.ts:29-30`).
...
- **Row 5 (archived):** `sourceRoute` is `/admin?bucket=archived`; the walker-only extension seeds ≥1 archived show.
