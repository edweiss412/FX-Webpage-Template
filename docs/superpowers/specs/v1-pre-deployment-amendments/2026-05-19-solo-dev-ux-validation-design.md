...
**Sibling specs:**
- Master crew-pages spec: [`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md`](../2026-04-30-fxav-crew-pages-v1.md)
- M11 user-facing docs: [`docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md`](./2026-05-12-user-facing-docs-design.md)
- **M11.5 crew auth pivot** (auth model for M12, canonical for v1 on every §5.2 / §7.2 / §7.2.1–3 / §9.2 surface that touched signed links): [`docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-23-crew-auth-pivot-show-link-picker.md`](./2026-05-23-crew-auth-pivot-show-link-picker.md). The M11.5 close-out delta-list at [`docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/handoffs/M11.5-delta-for-m12.md`](../../plans/2026-04-30-fxav-crew-pages-v1/handoffs/M11.5-delta-for-m12.md) is the authoritative diff source for the rebase.
...
| 5 | Picker crew — LEAD role | Fresh `/show/<slug>/<shareToken>/` (tokenized URL form per picker-pivot spec §3) followed by `<SignInOrSkipGate>` Mode A skip + picker pick of a `LEAD`-flagged identity | Full content visible including `shows_internal.financials`; no role-hiding |
| 6 | Picker crew — non-LEAD scope variants | Same tokenized URL form; picker pick of a non-`LEAD` identity (`A1`, `BO`, etc. — see §3.2 for role-variant inventory) | Role-filter sentinels active. **Multiple sub-variants exercised** — see §3.2 for the role-variant inventory. |
| 7 | Google-signed-in crew (fresh + return) | Google OAuth path on the tokenized show URL — distinct from persona 5/6 because the auth chain enters via the OAuth-claim / picker-bootstrap branch (Resolved Decision 17, picker-pivot spec §5). Validates `validateGoogleSession` for show-bound surfaces AND `validateGoogleIdentity` for `/me` (master spec line 2266 — separate validators). | Same crew-page surface, different session origin. Fresh OAuth and return OAuth sessions exercised separately because the first-sign-in chrome differs from the subsequent-visit chrome. |
...
**Persona 3 vs 2** is split because the UX changes shape (wizard vs steady state). **Personas 5 and 6** share the surface column for the tokenized show URL (`/show/<slug>/<shareToken>/`) but get separate matrix rows so role-hiding is exercised explicitly. **Persona 7** is split into fresh vs return OAuth sub-cases — the first-sign-in chrome (consent screen, fresh session establishment) differs from return-visit chrome and exercises different code paths. **Persona 8** is a distinct surface (`/me`) that uses a different validator (`validateGoogleIdentity`, not `validateGoogleSession`) per master spec line 2266; the audit allowlist X.3 explicitly enforces the separation, so this surface must be exercised independently.
...
| Target selection (R9 amendment + R11 correction + R13 commit 30 J3-claim-email addition; R21 commit 46 — defer per-command env-var map to §9.1.2; R27 commit 58 F28 Option D refactor — own-enumeration replaced by cross-reference to §9.1.2) | Script requires the prod-equivalent Supabase target — NOT local. **Required env vars: per the canonical CLI command-by-command env-var contract at §9.1.2 (R21 commit 44 F20 amendment + R27 commit 58 F28 Option D — §9.1.2 is the single source of truth; do NOT inline-restate the literal env-var names here).** The reseed row in the §9.1.2 table enumerates the exact env-var set this CLI requires; missing any aborts. (R27 Option D rationale: F10-class hit 5 rounds of same-vector recurrence — R12 F10 + R14 F13/F14 + R20 F20 + R24 F25 + R26 F28 — each round revealed a new syntactic form not covered by prior per-syntactic-form regex defenses. R27 ships structural exclusivity: only §9.1.2 + the `.env.local.example` config template carry own-enumerations; every other surface cross-references §9.1.2.) <!-- not-subject-to-meta: R27 commit 58 Option D refactor — this §3.3 row is a cross-reference to §9.1.2, NOT an own-enumeration. The single `VALIDATION_SUPABASE_URL` mention below is in the localhost-rejection clause (a single-literal narrative citation, not a contract enumeration); the F20 R21 markdown-table-row guard predates the R27 structural-exclusivity contract and still expects every table row mentioning a canonical literal to either enumerate all 4 OR carry a subset marker. This waiver opts the row out of F20 since the R27 walker now provides the contract-level guarantee. --> If `VALIDATION_SUPABASE_URL` matches `localhost`, `127.0.0.1`, or `::1` the script aborts unless `--allow-local-override` is passed (intentional override gate; not used during M12 walks). Phase 0 sub-task adds all four to the dev's local `.env.local` AND extends `.env.local.example` with each as a documented placeholder. The Supabase project ref is stamped in `validation_state.seeded_supabase_project_ref` (added in §3.3.2 DDL) so check-seed can verify target consistency; check-seed predicate (k) (added R13 commit 30) fails if the J3-claim-email env var (named in §9.1.2 row) is still a placeholder domain at seed time.

**R13 amendment 2026-05-26 — J3 OAuth-walk fixture impossibility (F10 finding); R15 commit 35 — canonical rejected-domain set extension per F14 finding.** Per spec §1.5 "solo-dev IS the validation": J3 leg (c) requires the dev to sign in to Google AS the alias_5a_lead identity for combo R1 (the canonical LEAD combo for J3) to trigger `claim_oauth_identity`. The pre-R13 seed contract synthesized all fixture emails as `validation+<combo>-<alias>@example.com` (or the shorter `validation+5a@example.com` form per the original R-combo prose). Google OAuth cannot authenticate against placeholder/dev-only reserved domains — the load-bearing identity-exclusivity walk was unwalk-able as designed. **Canonical rejected-domain set (R15 commit 35 amendment — single source of truth, mirrored by plan §0.C.5 predicate (k) regex + plan §0.C.3 fixture-build TS guard + plan §0.C.4 mint RPC defense-in-depth):** `@example.com`, `@example.org`, `@example.net` (RFC 2606); `*.test`, `*.invalid`, `*.localhost`, bare `localhost` (RFC 6761); `*.local`, `dev.local` (mDNS RFC 6762 + project-conventional dev). Canonical regex (TS form): `/@(example\.com|example\.org|example\.net|[^@\s]+\.test|[^@\s]+\.invalid|localhost|[^@\s]+\.localhost|[^@\s]+\.local|dev\.local)$/i`. The R13 amendment parameterizes ONLY combo R1's `alias_5a_lead.email` via a new env var `VALIDATION_J3_CLAIM_EMAIL`; the reseed reads this env var at fixture-build time and writes it as the `crew_members.email` for R1's `alias_5a_lead` row only. Every other combo's `alias_5a_lead` (and every other alias in every combo, including R1) keeps the synthesized `validation+<combo>-<alias>@example.com` format. **Combo-isolation rationale:** the dev's Google account email is unique → `claim_oauth_identity(canonicalEmail)` stamps `claimed_via_oauth_at` only on R1's `alias_5a_lead` row → claims in J3 leg (c) don't poison the LEAD identity across other combos (R2–R8b + 6 SW-states). This preserves the multi-combo walk's identity-baseline assumption across non-J3-walked combos. Future J3 walks against other R-combos would require additional `VALIDATION_J3_CLAIM_EMAIL_<combo>` vars — out of scope for v1 since J3 only walks once per validation session against combo R1 per Task 1.6 convention.

**Why this is not a workaround:** the picker pivot's load-bearing identity contract is OAuth-claim identity-exclusivity (H8 two-reasons doc-guard). That contract is only testable end-to-end if a real Google account can authenticate against a fixture email. Per spec §1.5 the validation is performed by the solo dev against their own production-equivalent stack; the dev's personal email IS the validation identity. The env var makes the dev-as-fixture-identity contract explicit at config time (set once during Phase 0.A) rather than implicit at walk time (dev has to mentally reconcile fixture vs personal email). It's the simplest path that respects the solo-dev-validation framing without inventing a Google-OAuth-mockable domain.
...
| Verification command | `pnpm validation:check-seed` returns exit 0 if (under `--combo all` dispatch) `last_seed_date = today` AND `combos_materialized` covers what the next walk needs; under `--combo <single>` dispatch the equivalent gate is `combos_seeded_dates[<single>] = today` (predicate (b'), R55 amendment) and `last_seed_date` is NOT consulted. Per R57 commit 95 F49 amendment, `last_seed_date IS NULL` is treated as stale under `--combo all` dispatch (mint RPC never writes the column; only `validation_finalize_all_atomic` does, so NULL = finalizer-never-ran). Returns exit 1 otherwise. The dev runs this at the start of every walk session per §3.3 step 5 above. |
...
**R13 commit 30 amendment — combo R1's `alias_5a_lead.email` exception; R15 commit 35 — canonical rejected-domain set extension.** The synthesized `validation+<combo>-<alias>@example.com` (or shorter `validation+5a@example.com`) format applies to every alias in every combo EXCEPT **combo R1's `alias_5a_lead`** — that one row's `email` field reads from env var `VALIDATION_J3_CLAIM_EMAIL` (the dev's real Google account email). Rationale: J3 leg (c) (06-phase1-matrix-walk.md Task 1.6 leg c) walks the OAuth-claim identity-exclusivity contract by signing the dev's real Google account in via `claim_oauth_identity`; Google OAuth cannot authenticate against placeholder/dev-only reserved domains (the **canonical rejected set** — `@example.com` / `.org` / `.net` per RFC 2606; `*.test` / `*.invalid` / `*.localhost` / bare `localhost` per RFC 6761; `*.local` / `dev.local` per mDNS RFC 6762 + project-conventional), so the synthesized email is unusable for this one load-bearing fixture row. See the §3.3 step 5 R13-amendment paragraph for the canonical regex source; predicate (k) in §3.3.2 below mirrors the same rejected set. Per spec §1.5 "solo-dev IS the validation" the dev's personal email IS the alias_5a_lead identity for combo R1. **Combo-isolation:** the dev's email is unique → `claim_oauth_identity(canonicalEmail)` stamps `claimed_via_oauth_at` only on R1's `alias_5a_lead` row → claims don't poison the LEAD identity across R2–R8b + 6 SW-states. See the R13-amendment paragraph below the §3.3 step 5 Target-selection row for full rationale. The reseed's `mint_validation_fixture_atomic` RPC receives the env var value via the TypeScript script's fixture-payload assembly (canonicalized via `lib/email/canonicalize.ts` BEFORE the RPC write per AGENTS.md invariant 3); the RPC writes it as-is. check-seed predicate (k) fails if the env var is still a placeholder reserved domain at seed time.

**Pre-M11.5 historical note.** Earlier R22/R23 spec rounds added `alias_5a_lead_for_revoke` (10th) + `alias_5a_lead_for_query_compromise` (11th) to isolate the J3 signed-link revoke + query-token-compromise legs from the baseline alias. The 2026-05-26 picker-pivot rebase retires both — the picker pivot's destructive admin actions (share-token rotation + picker-epoch reset) are inherently fixture-clean (rotation invalidates ALL devices for the show; epoch reset invalidates ALL cookies for the show), so there is no surgical-revocation poisoning vector to defend against. The two J3-isolation aliases are deleted from the seed contract; J3's OAuth-claim leg (c) reuses the baseline `alias_5a_lead` + `alias_6a_a1` (the `claimed_via_oauth_at` stamp is structurally reversible by reseed: the mint RPC's `ON CONFLICT (show_id, name) DO UPDATE SET ..., claimed_via_oauth_at = NULL` clause **preserves the stable `crew_members.id` while resetting `claimed_via_oauth_at` to NULL** — see the canonical Cleanup contract below at the `crew_members.claimed_via_oauth_at` bullet for the full row-stability contract that J3's multi-step walk relies on; `--combo all` after an OAuth-claim walk restores the baseline deterministically without rotating `crew_members.id`, so devices holding stale cookies remain valid and only the `claimed_via_oauth_at` predicate flips back to NULL, per R13 commit 31 F11 amendment + R23 commit 49 F23 amendment).
...
- For every R-combo fixture: write **9** `crew_members` rows with `email`, `name`, `role_flags`, and the implicit canonical-email derivation handled by the existing email-canonicalization helper (`lib/email/canonicalize.ts` is the only function permitted to touch raw emails before they enter the system — AGENTS.md invariant 3). **Full-replace semantics per §3.3.2 R17 commit 39 F16 amendment (R21 commit 46 lockstep peer-fix):** `mint_validation_fixture_atomic` DELETEs `crew_members` rows for the validation show whose `name` is NOT in the incoming payload's `crewMembers[].name` array, then UPSERTs the canonical 9 rows per payload. The DELETE-before-UPSERT ordering inside the per-show advisory-lock transaction ensures concurrent picker reads either see the pre-RPC roster or the post-RPC roster, never a transient empty roster. Predicate (m) of check-seed (see §3.3.2) is the structural guard — it fires when ANY validation show carries an orphan `crew_members` row not enumerated in `validation_state.alias_map[combo]`, catching the F16 failure mode where the DELETE block is omitted or scoped wrong. See §3.3.2 "Singleton write semantics" step 1 for the canonical RPC body.
- For every SW-state fixture: 1 `crew_members` row (LEAD only); same full-replace semantics apply (the DELETE keeps only the single LEAD alias enumerated for the SW-state).
- The `show_share_tokens.share_token` row is created by a **dual-source sentinel** (R21 commit 45 F21 amendment — aligned to §3.3.2 R19 commit 43 self-heal contract): **initial INSERT path** — the `shows_create_share_token_after_insert` AFTER INSERT trigger (per migration `20260523000002_show_share_tokens.sql`) fires when the `shows` row is first inserted and writes the share_token row via `INSERT INTO show_share_tokens (show_id) VALUES (NEW.id) ON CONFLICT (show_id) DO NOTHING`; **UPSERT update-path / self-heal path** — when a reseed re-mints a show that already exists, the AFTER INSERT trigger does NOT re-fire (it only fires on INSERT, not on UPSERT update-path); to close the gap where a manually-cleaned-up share-token row would otherwise stay missing across reseeds, `mint_validation_fixture_atomic` self-heals via `INSERT INTO public.show_share_tokens (show_id) VALUES (v_show_id) ON CONFLICT (show_id) DO NOTHING` immediately after the shows UPSERT (R19 commit 43 F19 amendment — see §3.3.2 "Singleton write semantics" step 1 for the canonical RPC body). In the happy path (row already present), ON CONFLICT DO NOTHING is a no-op and preserves the existing share_token; in the corrupted path (row deleted out-of-band), the INSERT fires and the `share_token` column DEFAULT `encode(gen_random_bytes(32), 'hex')` populates a fresh 64-hex token. The dev's bookmarked URL stays valid across re-seeds in the happy path; `--combo all` does NOT rotate the share-token unless the dev explicitly does so via `RotateShareTokenButton`. Predicate (g) of check-seed (see §3.3.2) is the dual-source sentinel's CI/CD-time guard — it fires when ANY seeded show is missing its `show_share_tokens` row, which means EITHER the initial-INSERT trigger never fired OR the mint RPC's R19 self-heal block was omitted/malformed. **Manual `INSERT INTO public.show_share_tokens (show_id) SELECT ...` SQL backfill is NOT a normal repair path** — it papers over a contract bug rather than fixing it. If predicate (g) fires after a reseed, the implementer's first move is to verify the mint RPC body matches the R19 self-heal contract; the backfill SQL exists only as the M11.5 one-shot migration's bridge (validation shows that pre-existed the trigger) and should not be a recurring step in the M12 walk loop.
- `crew_members.claimed_via_oauth_at` is **explicitly RESET TO NULL on every reseed** (R13 commit 31 amendment) — the `mint_validation_fixture_atomic` RPC's `ON CONFLICT (show_id, name) DO UPDATE SET ...` clause includes `claimed_via_oauth_at = NULL` alongside the email/role/flags/restrictions sets. This restores the bypass-pickable baseline on every reseed regardless of whether prior J3 leg (c) walks stamped the row. (Pre-R13 framing: the RPC preserved the row through ON CONFLICT and did NOT touch claimed_via_oauth_at — F11 finding — leaving stamped rows poisoned across reseeds. Fixed in R13 commit 31.) J3 leg (c)'s OAuth-claim walk stamps `claimed_via_oauth_at = now()` via the live `claim_oauth_identity` RPC during the walk; the next reseed restores NULL automatically.
...
- **`crew_members.claimed_via_oauth_at`** — stamped by `claim_oauth_identity` during J3 leg (c). The reseed RPC explicitly resets this to `NULL` on every reseed via the `ON CONFLICT DO UPDATE SET ..., claimed_via_oauth_at = NULL` clause (R13 commit 31 F11 amendment). This preserves the row's stable `crew_members.id` (the J3 walk's identity-claim contract relies on row stability across the walk's multi-step procedure) while structurally restoring the bypass-pickable baseline on the next reseed. Devices that previously matched the claimed row see `claimed_via_oauth_at IS NULL` again and become bypass-pickable. **Devices holding stale cookies remain valid** (the `crew_members.id` is unchanged by reseed; only `claimed_via_oauth_at` resets); the resolver's claim arm at `lib/auth/picker/resolvePickerSelection.ts:110-120` requires `crewClaimRow.claimed_via_oauth_at !== null`, so reset rows fall through to `resolved` cleanly. check-seed predicate (l) (R13 amendment) verifies the discipline held post-reseed.
...
**check-seed predicates.** Beyond the 5 predicates in §3.3.2, check-seed fails additionally if (f) for any alias in `alias_map`, `crew_members` is missing the matching row OR has `email IS NULL` OR has the row but it's no longer eligible (e.g., `archived = true` on the show), OR (g) the show's `show_share_tokens.share_token` row is missing — **dual-source sentinel per R21 commit 45 F21 amendment**: predicate (g) fires when EITHER the `shows_create_share_token_after_insert` AFTER INSERT trigger never fired on initial INSERT OR the `mint_validation_fixture_atomic` R19 commit 43 self-heal block (`INSERT INTO show_share_tokens ... ON CONFLICT DO NOTHING` after the shows UPSERT) was omitted/malformed at the UPSERT update-path. Predicates (h)+ from the retired `revoked_links` contract are deleted.
...
CREATE TABLE IF NOT EXISTS public.validation_state (
  ...
  last_seed_date                   date NULL,                             -- R57 commit 95 F49 amendment — NULL until validation_finalize_all_atomic stamps. Per R55 Option (b) + R57 F49 fix, the mint RPC's initial INSERT (first singleton creation) MUST NOT write this column; only the finalizer writes it. Predicate (b) treats NULL as stale ("last_seed_date IS NULL OR last_seed_date != $VALIDATION_TODAY_ISO"). Prior NOT NULL framing (pre-R57) allowed mint RPC's initial INSERT to stamp a fresh all-combos completion date on single-combo reseed, bypassing the F47 CAS contract scoped to the finalizer (F49 INSERT-side bypass). Documented in spec §3.3.2 singleton write semantics paragraph below and mirrored in plan 02 DDL.
...
1. **`mint_validation_fixture_atomic(p_combo, p_fixture_payload)`** — invoked per-combo by `validation:reseed`. Acquires `pg_advisory_xact_lock(hashtext('show:' || drive_file_id))`, then within the SAME transaction: UPSERTs `shows` (the `shows_create_share_token_after_insert` trigger auto-creates the `show_share_tokens` row on first insert; ON CONFLICT no-op for re-seeds preserves the existing share-token), **self-heals `show_share_tokens` via `INSERT INTO public.show_share_tokens (show_id) VALUES (v_show_id) ON CONFLICT (show_id) DO NOTHING` after the shows UPSERT (R19 commit 43 F19 amendment — closes the trigger-bypass-on-UPSERT-update-path gap: the AFTER INSERT trigger doesn't re-fire on UPSERT update-path, so a manually-cleaned-up share-token row would otherwise stay missing across reseeds, leaving smoke 6 / J3 unwalkable; the ON CONFLICT DO NOTHING clause preserves the existing token in the happy path while restoring a fresh 64-hex token via the column DEFAULT when the row is absent)**, **DELETEs `crew_members` rows for the validation show whose `name` is NOT in the incoming payload's `crewMembers[].name` array — full-replace semantics per R17 commit 39 F16 amendment** (the picker reads `crew_members` directly via `loadShowCrew` / `resolve_show_by_slug_and_token`; without the DELETE, stale rows from prior fixture revisions or manual probes survive and surface in the picker even though `alias_map` no longer enumerates them, expanding the walk's test surface beyond the canonical fixture), UPSERTs `crew_members` per payload (DELETE-before-UPSERT ordering ensures the roster is never transiently empty as observed by any other reader — the advisory lock holds the whole transaction; concurrent picker reads either see the pre-RPC roster or the post-RPC roster), merges the alias-map slice into `validation_state.alias_map`, AND stamps `combos_seeded_dates[p_combo] = $validationTodayIso` for the per-combo gate (see predicate (i) below). **Does NOT touch `last_seed_date` on EITHER the initial INSERT side OR the ON CONFLICT DO UPDATE side (R57 commit 95 F49 amendment — both write paths must exclude this column; the initial INSERT omits it from the column-list entirely so the column defaults to NULL per R57 nullable DDL).** The pre-R57 mint RPC body stamped `last_seed_date = v_validation_today_iso::date` on the INSERT side which falsified the "finalizer-exclusive" contract on first singleton creation (F49 INSERT-side bypass — R56); R57 closes the bypass by omitting `last_seed_date` from both INSERT lists entirely.
...
The `validation:check-seed` command reads exactly the `key = 'validation_seed'` row and fails if (a) zero rows, (b) **(R55 commit 94 F48 amendment — `--combo all` only; R57 commit 95 F49 amendment — NULL handling)** when invoked as `check-seed --combo all`, `last_seed_date IS NULL OR last_seed_date != $VALIDATION_TODAY_ISO`. **NULL means `validation_finalize_all_atomic` has never run** — the initial validation_state singleton creation is performed by the mint RPC and (per R57 F49 fix) deliberately omits `last_seed_date` from its INSERT column-list, leaving it NULL until the finalizer stamps it. Treating NULL as stale closes the F49 INSERT-side bypass where the pre-R57 mint RPC stamped `last_seed_date = $validationTodayIso` on first singleton creation, falsely advancing the all-combos completion stamp without the finalizer ever running (and bypassing the F47 CAS contract that scopes exclusively to the finalizer). **Scope:** `last_seed_date` is the "all-combos completion stamp" — written exclusively by `validation_finalize_all_atomic` after every per-combo mint in a `--combo all` reseed succeeds. Predicate (b) therefore only applies to `--combo all` check-seed invocations; single-combo invocations route to predicate (b') below to avoid the F48 semantic gap where a fresh single-combo reseed cannot stamp the all-combos-scoped `last_seed_date` and predicate (b) would falsely fire on a fresh/next-day stack. (R27 commit 56 F26 amendment — TZ-pin alignment with predicate (i); the prior `current_date` framing read the Postgres server clock and reintroduced the UTC-vs-local-midnight skew class the rest of §3.3.2 explicitly defends against. `$VALIDATION_TODAY_ISO` is the canonical UTC `YYYY-MM-DD` value the script computes, NOT Postgres `current_date`. The DB-side `current_date` is reserved for the bounded-skew sanity check inside the mint RPC per the R11 F9 fix at plan 03 line 242), **(b') (R55 commit 94 F48 amendment — single-combo freshness)** when invoked as `check-seed --combo <single>` (any non-`all` combo enum value, e.g., `--combo R1`), `combos_seeded_dates[<single>] != $VALIDATION_TODAY_ISO`. Reads the per-combo stamp written by `mint_validation_fixture_atomic` (which does NOT touch `last_seed_date`). Catches the F48 failure mode where smoke 6's prerequisite `reseed --combo R1` → `check-seed --combo R1` would otherwise block on stale `last_seed_date` from an earlier day's all-combos run (or a never-run finalizer). Diagnostic: "validation_state.combos_seeded_dates[<single>] = <stamp> != $VALIDATION_TODAY_ISO — re-run `pnpm validation:reseed --combo <single>` to refresh the per-combo stamp." Note: predicate (i) below ALSO compares `combos_seeded_dates[combo]` to `$VALIDATION_TODAY_ISO` but its scope is the partial-`--combo all` failure mode (every combo in the requested set must be fresh); predicate (b') is the single-combo dispatch surface and overlaps in behavior but not in intent. (c) `combos_materialized` doesn't cover the combos needed by the next walk (16 combos for `--combo all`), (d) `seeded_supabase_project_ref != $VALIDATION_SUPABASE_PROJECT_REF` (R9 amendment — target-consistency), (e) `alias_map` does not satisfy the §3.3 alias-map storage predicate (see §3.3 for the canonical count — currently **9 alias entries per R-combo × 10 R-combos + 1 alias per SW-state × 6 = 96 total leaves** post-2026-05-26 picker-pivot rebase; predicate cross-references §3.3 rather than restating the value, so future count drift updates only §3.3), OR (i) for ANY combo in the requested set, `combos_seeded_dates[combo] != $VALIDATION_TODAY_ISO` — catches the partial-`--combo all` failure mode where some combos succeeded on day X and others stamped day Y (UTC midnight crossed mid-run) OR some combos succeeded and others failed (the finalizer never ran). check-seed accepts the date as an env var or CLI flag; defaults to `new Date().toISOString().slice(0, 10)` (TZ-pinned UTC date; the structural defense that audits this discipline — `tests/cross-cutting/validation-tooling-tz-pin.test.ts` — is planned for Phase 0.C authoring per plan-tree `DEFERRED.md` entry `M12-PHASE0C-TZ-PIN-METATEST`. R11 audit 2026-05-26 verified the file was NOT actually committed by the R5 amendment; reframed in R11 commit 28). **(k) (R13 commit 30 amendment — J3-claim-email guard; R15 commit 35 — canonical rejected-domain set extension per F14 finding)** `VALIDATION_J3_CLAIM_EMAIL` is unset OR matches any placeholder/dev-only reserved domain in the **canonical rejected set** (single source of truth — see §3.3 step 5 R13-amendment paragraph for the regex form; mirrored in plan §0.C.5 predicate (k) + plan §0.C.3 fixture-build TS guard + plan §0.C.4 mint RPC defense-in-depth): `@example.com` / `@example.org` / `@example.net` (RFC 2606); `*.test` / `*.invalid` / `*.localhost` / bare `localhost` (RFC 6761); `*.local` / `dev.local` (mDNS RFC 6762 + project-conventional dev). The J3 OAuth-claim walk would be unwalk-able because Google OAuth cannot authenticate against any of these. **(l) (R13 commit 31 amendment — baseline-claim guard)** For any baseline picker alias enumerated in §3.2 (the 9 role-variant aliases × the seeded combos), `crew_members.claimed_via_oauth_at IS NOT NULL` after a fresh `--combo all` reseed — catches the F11-class failure mode where the mint RPC's UPSERT `SET` clause drifts and stops resetting the claim state. Diagnostic: "crew_members row for <combo>.<alias> has claimed_via_oauth_at = <timestamp> after reseed — mint RPC SET clause missing `claimed_via_oauth_at = NULL`; re-check the migration body against the R13 commit 31 contract." See §3.3 for the canonical alias-map contract. **(m) (R17 commit 39 amendment — full-replace orphan guard for F16 finding; R19 commit 42 F18 case-normalization fix)** For every seeded validation show, the materialized `crew_members` roster for that show MUST equal the canonical fixture roster enumerated in `validation_state.alias_map[combo]`. Concretely: for each combo C in `combos_materialized`, the C-show is resolved via `drive_file_id = 'validation_' || C` — **canonical case is UPPERCASE combo enum verbatim** (the mint RPC writes `'validation_' || p_combo` with no `lower()` coercion; combo enum values `R1`/`R7b`/`SW-POST_SHOW` produce `validation_R1`/`validation_R7b`/`validation_SW-POST_SHOW`). Every `crew_members.name` for the C-show MUST appear in the canonical fixture body for combo C — orphan rows from prior fixture revisions or manual SQL probes that the mint RPC's full-replace DELETE-before-UPSERT (per "Singleton write semantics" above) should have removed but didn't, are flagged. Diagnostic: "validation show <C> has orphan crew_members row(s) <names> not enumerated in validation_state.alias_map[<C>] — mint_validation_fixture_atomic full-replace DELETE-before-UPSERT did not fire OR a manual write landed a stale row; re-run `pnpm validation:reseed --combo <C>` to clear." **(n) (R27 commit 57 F27 amendment — shows baseline-eligibility guard for F16-class same-vector recurrence at SET-clause-column scope)** For every seeded validation show, `archived = false AND published = true` after a fresh `--combo all` reseed. Catches the F16-class failure mode where the mint RPC's ON CONFLICT UPDATE SET clause omits `archived = false` and/or `published = true`, leaving a manually-archived or manually-unpublished validation show stranded across reseeds (check-seed predicate (f) treats archived rows as not-yet-eligible; the M11.5 picker route short-circuits unpublished shows). Diagnostic: "validation show <C> has archived=<a> published=<p> after reseed — mint RPC ON CONFLICT UPDATE SET clause missing `archived = false` and/or `published = true`; re-check the migration body against the R27 commit 57 contract at plan 03 line 284." Plan-spec'd regression at plan 03 Task 0.C.5 Step 5 — fixture-set the validation show `archived = true` (or `published = false`) BEFORE reseed; run reseed; assert post-reseed `archived = false AND published = true` AND check-seed predicate (n) PASS. (Per F16-class same-vector recurrence: R16 F16 crew_members orphan + R18 F19 share-token trigger-bypass + R26 F27 SET-clause-column completeness — each round revealed a new sub-shape; R27 audit at SET-clause-column scope enumerated every `public.shows` column the mint RPC writes via INSERT and verified each is in UPDATE SET or carries documented exception — see plan 03 line 284 inline audit table for the per-column outcome.)
...
4. **Master spec AC-2.5 amendment** — `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` **line 3536** (verified live 2026-05-26): the per-table list grows to include `validation_state`; the **21 tables × 4 verbs = 84 assertions** literal updates to **22 tables × 4 verbs = 88 assertions** (prose track). Add a one-line cross-reference footnote: *"Count reflects the §4.3 nominal prose baseline; live `ADMIN_TABLES.length = 18` per §4.3 footnote (M11.5 picker-pivot filter drops 4)."* — the cross-reference makes the count math auditable for future reviewers.
...
| Axis | Policy | Detail |
|---|---|---|
| **Surface × Persona** | FULL (within applicability — see below) | Every surface × every persona that can reach it. Anonymous reaches only auth-error surfaces; admin reaches admin surfaces; picker crew (personas 5 + 6) reach the tokenized show URL surfaces; Google-signed-in crew (personas 7 + 8) reach the tokenized show URL via the OAuth-claim / picker-bootstrap branch (persona 7) and the `/me` cross-show surface (persona 8). Out-of-domain combinations (e.g., anonymous × admin dashboard) are dispositioned EXCLUDED in MATRIX-INVENTORY.md. |
...
| **Real-device-vs-emulated** | SMOKE-SAMPLE | Real iPhone only for the curated subset named in §3.1 (Right Now, schedule, share-link + picker first-contact via `<SignInOrSkipGate>` Mode A, sign-in, share-token-rotated reload surfacing `PICKER_SHOW_UNAVAILABLE`, epoch-reset reload surfacing `PICKER_EPOCH_STALE_BANNER`, `/me`) for personas 5/6/7/8. Other cells emulate. |
...
The "curated subset" for real-iPhone is enumerated at plan-writing time; defaults are the Right Now card, the schedule tile, share-link + picker first-contact (tokenized URL → `<SignInOrSkipGate>` Mode A skip → picker pick → `_ShowBody`), sign-in flow, share-token-rotated reload (`PICKER_SHOW_UNAVAILABLE`), epoch-reset reload (`PICKER_EPOCH_STALE_BANNER` cascade), and the `/me` cross-show list.
...
The canonical source of "every documented surface" is the master spec (`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md`) + the M11 spec (`docs/superpowers/specs/v1-pre-deployment-amendments/2026-05-12-user-facing-docs-design.md`). This spec does NOT pre-enumerate every row, because the source has hundreds of references and would rot; the plan-time derivation is authoritative — and the derivation itself is structurally required (§4.1.1), not delegated.
...
|---|---|---|
| Master spec heading inventory | Every `##` and `###` heading in `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` that names a UI surface | One row per heading naming a surface |
...
| **E. Cross-cutting affordances** | Every `?` tooltip / "Learn more →" link from §9.0.1 surface affordance matrix (M11 §5.6). Every catalog-driven error message rendered through `messageFor()` — **both admin-facing AND crew-facing** (an earlier draft restricted this band to `/admin/*` only; corrected per Codex R1 P0 — crew-facing catalog-driven messages like `PICKER_SHOW_UNAVAILABLE`, `PICKER_EPOCH_STALE_BANNER`, "not on crew list", and rate-limit copy are equally in scope). AlertBanner row rendering for each non-info-severity admin catalog code. |
| **F. Report-pipeline surfaces (M8)** | Master spec §13.1 enumerates 4 report entry points (admin parse-panel button, preview/banner button, crew footer "Something looks wrong?" modal, and the §13 admin surfaces); each is a surface in the matrix. Submission outcome surfaces also walked: success confirmation, in-flight idempotency (`IDEMPOTENCY_IN_FLIGHT`), rate-limit hit (429 for admin and crew). **R14 amendment for materializability (R15 corrected; R31 commit a5ed46f per-outcome producer-map ratification; R33 commit 68 spec propagation per §13.2.3 plan-supersedes-spec model):** the deep-failure outcomes (GitHub-lookup-inconclusive 502, `REPORT_HORIZON_EXPIRED` 410, `REPORT_ORPHANED_LOST_LEASE`) require GitHub API faults / lease-timing races not deterministically reproducible against a live prod-equivalent stack without a fault-injection harness. Each is dispositioned in MATRIX-INVENTORY.md as either:<br/><br/>**(a) INCLUDED-via-harness** — Phase 0 sub-task adds a fault-injection harness `scripts/validation-report-fixtures.ts` that materializes each named failure state via the **per-outcome producer-state map** (R31 ratification; canonical source at handoff §9 R31 §A). The harness writes to THREE producer tables (NOT just `reports`) depending on the outcome: <br/>&nbsp;&nbsp;• `reports` for `success-admin`, `success-crew`, `in-flight`, `lease-expired`, `horizon-expired` (and optionally as a stub row for `lookup-inconclusive` / `orphaned-lost-lease`) — R47 commit 86 F43 amendment: canonical enum values per R43 commit 81 F40 split; pre-R47 wording was `success` (admin) / `success` (crew); <br/>&nbsp;&nbsp;• `report_rate_limits` for `rate-limit-admin`, `rate-limit-crew`; <br/>&nbsp;&nbsp;• `admin_alerts` for `lookup-inconclusive`, `orphaned-lost-lease` (primary observable rendering surface is `AlertBanner`, not `ReportModal`). <br/>The producer table chosen for each outcome matches the live production code's writer (per `lib/reports/submit.ts` + `lib/reports/rateLimit.ts` per outcome) — the harness MUST seed the same table the live consumer reads. Cleanup order (canonical, FK-safe + audit-trail-safe): `admin_alerts` → `report_rate_limits` → `reports`. **R15+R31 amendment — feedback_inbox is NOT a target table:** an earlier draft listed `feedback_inbox` as a target, but `feedback_inbox` is BACKLOG-only (`BL-PUSH-NOTIFICATIONS` notification-design-memo), not in v1 schema. The harness targets ONLY the three v1 producer tables enumerated above (`reports` + `report_rate_limits` + `admin_alerts` — all v1 admin-only tables per master spec §4.3); the producer-set is NOT singular (the pre-R31 spec wording calling `reports` "the only v1 admin-only table in this domain" was producer-state mismatch and is retired). If a future milestone adds `feedback_inbox` to v1, the harness contract is extended in that milestone's plan.<br/><br/>**(b) EXCLUDED-rely-on-structural** — for outcomes where the UI rendering is identical to a simpler outcome already covered, the row is dispositioned EXCLUDED with a cite to the structural test that already pins the contract (e.g., `tests/report/orphaned-lease.test.ts`). The plan picks per-row at plan-writing time. Default is (a) — harness-materialized — for any outcome whose UI state is not otherwise covered. |
...
Fresh browser profile, deployed production URL (`*.vercel.app`, no custom domain). Sign in via Google. Land on `/admin`. From `/admin`, follow the "Take the tour" link into `/help`. Read `/help/getting-started` and `/help/daily-rhythm`. Use only the /help docs as the map — *do not navigate by dev memory*. Drop a fixture sheet into the watched folder. See cron pick it up (or wait the cron interval). See first-seen auto-publish per master-spec amendment 9. Open the crew preview link, see crew page render. Confirm the per-show `CurrentShareLinkPanel` displays the canonical share URL (`/show/<slug>/<shareToken>/` form) ready for out-of-band distribution.

### 5.2 J2 — Pending-sync triage

Edit a published sheet to trigger MI staging events. Pick MI-6 (crew shrinkage) and MI-11 (email change) as the two highest-stakes classes — separate stagings, not bundled. Wait for cron + push-debounce window. Open dashboard; see AlertBanner + staged-review card. Open `/admin/show/[slug]`, drill into staged change. Exercise the **Apply** path on one staging AND the **Discard** path on the other. Confirm Apply propagates to the crew page and to existing picker cookies for this show (Realtime broadcast on `show:<showId>:invalidation` fires; iPhone holding a valid post-pick cookie reloads to the post-Apply state). Confirm Discard leaves prior state intact.
...
| 2 | **The OAuth claim** — on the iPhone, sign in via Google using a Google account whose email matches `alias_5a_lead`'s `crew_members.email` (the dev controls the fixture email; this is the bypass-pick-then-claim scenario). The OAuth flow: iPhone → `/auth/sign-in` → Google consent → `/auth/callback/route.ts` → invokes `claim_oauth_identity(email)` RPC which stamps `claimed_via_oauth_at = now()` on every matching `crew_members` row (under per-show advisory locks); callback DOES NOT mint a picker cookie (R41-R6 lazy-mint). The iPhone redirects to `next` (the tokenized show URL). | iPhone's page-route resolver at `resolveShowPageAccess.ts:170-212`: `validateGoogleSession` returns `success` (Google session matches `alias_5a_lead`'s crew row for this show); cookie's `id` matches `google.viewer.crewMemberId`; `crewClaimRow.claimed_via_oauth_at !== null`; cookie's `t` (from leg (b) step 3) is pre-claim, so `entry.t <= claimEpochMillis`. **Per `resolveShowPageAccess.ts:204-208`, the resolver returns `needs_picker_bootstrap`** (the validateGoogleSession-success branch fires BEFORE `resolvePickerSelection`; `claimed_after_pick` at `resolvePickerSelection.ts:110-120` is NOT reachable from this path because the page-route bootstrap arm short-circuits first). Page redirects to `/api/auth/picker-bootstrap?next=<URL>&t=<intentToken>`; bootstrap mints a NEW picker cookie with `t = result.mint_safe_t_millis` (DB-side, strictly greater than `claimed_via_oauth_at`); 302 back to the URL; `_ShowBody` renders directly. **The user never sees the picker interstitial; the visible signal that the claim succeeded is the `OAUTH_IDENTITY_CLAIMED` admin alert** (P-R26 email-bearing context — verified at step 5). User-perceived flow: "signed in → bootstrap auto-redirect → page renders" per Resolved Decision 17. |
| 3 | **Cross-device claim propagation (`claimed_after_pick` observation).** On the **desktop** (still holding the pre-claim `alias_5a_lead` cookie from step 1; **no active Google session on this desktop browser**): reload the SAME URL. | Desktop's page-route resolver: `validateGoogleSession` returns plain `continue` (no Google session for this show) → does NOT match the GOOGLE_NO_CREW_MATCH branch or the success branch → falls through to `resolvePickerSelection` at line 212. Picker resolver: cookie has `id: <alias_5a_lead.id>`; `crewRow.claimed_via_oauth_at !== null` (claim from step 2 is global); cookie's `t` from step 1 < `claim_epoch_millis` → **per `resolvePickerSelection.ts:110-120`, the claim arm fires and returns `identity_invalidated` with `reason: 'claimed_after_pick'`** (the session-email arm at line 122 is unreachable because `sessionEmail` is null without a Google session). Route handler maps to `PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER`. The dev confirms: `alias_5a_lead`'s row is visually disabled in the desktop's picker (OAuth claim is global; every device sees this identity as claimed — picker-pivot Resolved Decision 15). **Pick `alias_6a_a1` (a non-LEAD, OAuth-UNCLAIMED identity) instead** → `selectIdentity` writes a fresh cookie entry for `alias_6a_a1.id`; `_ShowBody` renders as A1 viewer. Desktop's cookie now holds the UNCLAIMED alternate identity for step 4. |
| 4 | **Shared-device identity-leak vector defense (`no_auth/google_mismatch` Mode B).** On the desktop (cookie now holds `alias_6a_a1.id` from step 3), sign in to Google as a "Bob" account whose email matches NEITHER `alias_5a_lead`'s NOR `alias_6a_a1`'s `crew_members.email` (any non-fixture Gmail address works; this is the "Bob picked up Alice's device" composition). After OAuth callback redirects back to the tokenized URL: resolver fires. | Desktop's page-route resolver: `validateGoogleSession(req, {showId})` checks Bob's email against `crew_members` for this show. **Bob has no matching crew row → returns `continue` with `code: 'GOOGLE_NO_CREW_MATCH'`**. Per `resolveShowPageAccess.ts:176-178`, the resolver returns `no_auth` with `reason: 'google_mismatch'` (TERMINAL — does NOT fall through to the cookie/picker path; the shared-device identity-leak vector defense ratified in P-R27 closes here). The dev sees `<SignInOrSkipGate>` Mode B "signed in as someone else" copy with a "sign out and try again" affordance, NOT the picker. `resolvePickerSelection` is NEVER called on this path; `alias_6a_a1`'s cookie is irrelevant to the outcome (the desktop's cookie data does not leak into Bob's session view). **Sign out of Google on the desktop → reload** → no Google session → `validateGoogleSession` returns plain `continue` → falls through to `resolvePickerSelection` → cookie has `alias_6a_a1.id`, `claimed_via_oauth_at IS NULL`, `sessionEmail` is null → `resolved` arm → `_ShowBody` renders as A1 viewer. Confirms the shared-device-leak vector is blocked at the page-route's TERMINAL Mode-B short-circuit (the closing disclosure paragraph below explains why this is NOT `session_mismatch` at the picker resolver). |
| 5 | **Verify the admin-alerts trail.** Open the desktop's admin session, check `/admin` AlertBanner. | Expect `OAUTH_IDENTITY_CLAIMED` row from step 2 with the canonical-email hash context per P-R26 email-bearing alert codes (3 email-bearing + 3 email-less alert codes per the picker-pivot R41 matrix). |

**`session_mismatch` reachability disclosure (R8 amendment 2026-05-26).** The `identity_invalidated` wire arm carries two reasons — `claimed_after_pick` AND `session_mismatch` per P-R29/P-R30 Fix-1. This leg's page-route walks exercise `claimed_after_pick` directly (step 3 above), but `session_mismatch` (at `lib/auth/picker/resolvePickerSelection.ts:122-143`) is **structurally unreachable from the page-route**. The page-route at `resolveShowPageAccess.ts:174-212` calls `validateGoogleSession` BEFORE `resolvePickerSelection`, and: (i) the `GOOGLE_NO_CREW_MATCH` short-circuit at lines 176-178 forecloses any session-email mismatch where the Google session's email does not match a crew row for this show — the page returns `no_auth/google_mismatch` Mode B (TERMINAL, step 4 above); (ii) the success branch at lines 179-209 returns `needs_picker_bootstrap` for any cookie-mismatch / unclaimed-row / cookie-pre-dates-claim sub-state (lines 188, 197, 204-208), so a Google session that DOES match a crew row never reaches `resolvePickerSelection`'s session-email arm either; (iii) the only fall-through to `resolvePickerSelection` at line 212 is when no Google session exists — and in that branch, the picker resolver's `sessionEmail` (line 63-68, read from `auth_email_canonical` RPC) is null without a Supabase Auth session, so the `if (sessionEmail)` gate at line 122 is skipped. `session_mismatch` is reachable **only via API-route callers** that bypass `validateGoogleSession`: `app/api/show/[slug]/version/route.ts:82`, `app/api/realtime/subscriber-token/route.ts:97`, `app/api/report/route.ts:118`, and `lib/auth/picker/validatePickerAssetSession.ts:37`. The H8 two-reasons doc-guard contract (`tests/cross-cutting/identity-invalidated-two-reasons-doc-guard.test.ts`) is satisfied as a **documentation contract** — the spec body mentions both reason strings at the §3 preamble + this disclosure paragraph + step 3 (`claimed_after_pick` from desktop no-session reload). The two-reasons contract is a documentation-completeness invariant, not a J3-walk-exercise invariant.

This leg exercises: (i) `OAUTH_IDENTITY_CLAIMED` emission per P-R8 Fix-3 / P-R20 Fix-2 isolation (step 2); (ii) the H8 doc-guard's two-reasons contract as a documentation invariant — `claimed_after_pick` exercised at step 3 (desktop, no Google session, claim-arm fires); `session_mismatch` API-route-only reachability documented in the closing disclosure paragraph above; (iii) the picker's visually-disabled-row rendering for OAuth-claimed identities (step 3); (iv) the lazy-mint picker-bootstrap path (step 2 — Resolved Decision 17, login skips the picker via the `needs_picker_bootstrap` → `/api/auth/picker-bootstrap` → fresh-cookie chain); (v) the cross-device claim-propagation surface (step 3 confirms the claim from step 2 is globally visible without per-device action — `alias_5a_lead`'s row is disabled on the desktop even though the claim happened on the iPhone); (vi) the shared-device identity-leak vector defense via `no_auth/google_mismatch` Mode B (step 4 — Bob's Google session triggers the page-route's TERMINAL `no_auth/google_mismatch` short-circuit per P-R27, NOT `session_mismatch` at the picker resolver per the closing disclosure paragraph).
...
A fix to a band-E catalog-driven affordance (e.g., `messageFor('PICKER_SHOW_UNAVAILABLE').dougFacing` copy change) propagates to every consumer surface in band B or band F. The re-exercise scope therefore includes:
...
| `lib/auth/*` change (validateGoogleSession, validateGoogleIdentity, requireAdmin, requireAdminIdentity, `resolveShowPageAccess`, `resolvePickerSelection`) | List every route file under `app/` that imports the changed validator or resolver. | Every listed route gets re-walked. |
...
| 0.C | Author `scripts/validation-reseed.ts` + `validation:check-seed` + `validation:resolve-alias`. Run reseed --combo all + check-seed against the prod-equivalent stack. | 1–2 days | Yes — fixtures must exist before smoke 6. |
| 0.D | **DELETED (2026-05-26 picker-pivot amendment).** Originally: author `scripts/validation-mint-link.ts` + `validation:revoke-link` wrapping the M9.5 `signLinkJwt` + `revoked_links` surfaces. Those surfaces were retired at M11.5 G3 cutover (`20260523000099_cutover_drop_m9_5.sql`); the picker-pivot's destructive admin actions (`RotateShareTokenButton` + `ResetPickerEpochButton` on the M11.5 admin UI) are now the canonical share-link interface and need no CLI parity. No work in this row. | — | No |
| 0.E | Author `scripts/validation-report-fixtures.ts`. | 0.5 day | **R24: BLOCKING Phase 1 unless MATRIX-INVENTORY.md pre-dispositions every "deep" report-pipeline outcome (lookup-inconclusive, lease-expired, horizon-expired, orphaned-lost-lease) as `EXCLUDED-rely-on-structural` per §4.2 band F. Default is INCLUDED-via-harness, so by default this task IS blocking. To unblock without authoring the harness, the dev MUST commit a MATRIX-INVENTORY.md with every deep-outcome row dispositioned EXCLUDED + cite to the structural test that pins each contract.** Add Phase 0 smoke test 7 (new): run `pnpm validation:report-fixtures --outcome lookup-inconclusive --alert-code inconclusive` (R47 commit 85 F42 amendment — explicit selector pins the canonical `REPORT_LOOKUP_INCONCLUSIVE` surface per `lookupAlertCode` default branch at `lib/reports/submit.ts:202-208`; R43 commit 81 default is `bot-login-missing`) and verify the report-failure UI row renders. If the dev chooses the EXCLUDED-rely-on-structural path, smoke 7 is skipped. |
...
| **CI gates** | The eight canonical CI gates from `.github/workflows/x-audits.yml` (per master spec X.* lineage) WERE required-blocking; per DEFERRED.md §X6-D-1 (2026-05-20), branch protection on `main` was deleted and both `verify-branch-protection` + `verify-branch-protection-status` are guarded `if: false` in the workflow. The eight gates STILL run on every PR + push and surface red checks in the GitHub UI; they no longer block merges until X6-D-1 reopens. The eight gates are enumerated in §9.1.1 below. M12.1 (2026-05-27) added gate #8 `x6-pg-cron-pivot`. |
...
The "eight CI gates" referenced in §9.1 are concrete jobs in `.github/workflows/x-audits.yml`. Each WAS independently required-blocking on the branch protection rule; per DEFERRED.md §X6-D-1, branch protection on `main` is deleted (2026-05-20). Phase 0 no longer verifies branch-protection settings; it verifies each gate is wired in the workflow and fires correctly. M12.1 added gate #8 `x6-pg-cron-pivot` (2026-05-27):
...
| 7 | `verify-branch-protection-status` | (X6-D-1 dormant) PR check that WOULD read the latest privileged `verify-branch-protection` artifact and assert it ran recently AND succeeded — DORMANT per DEFERRED.md §X6-D-1, currently `if: false`. | PR (dormant) |
| 8 | `x6-pg-cron-pivot` | M12.1 pg_cron pivot doc-guard + no-vercel-cron + M12-plan-pg-cron-pivot-amendment | PR + main |
...
Phase 0 sub-task — per X6-D-1, branch protection on `main` is DELETED and not part of Phase 0 verification. Confirm each of the eight gate jobs is wired in `.github/workflows/x-audits.yml` and runs on PR + push. When X6-D-1 reopens, this sub-task is reactivated to re-PUT branch protection with all eight gate names per the master spec AC-X.6 inventory + the BRANCH_PROTECTION_DRIFT resolution prose.

### 9.1.2 Validation tooling reference (R17 amendment; rebased 2026-05-26 for picker pivot; R21 commit 44 F20 amendment — per-row env-var explicitness)

Every validation command introduced by M12 is enumerated below with its full contract. Other sections of this spec reference these commands by name; this table is the source of truth for CLI args, env vars, target DB, output, idempotency, and exit codes. Implementers of the M12 plan use this section as the man-page for the tooling. **The canonical CLI command-by-command env-var map is this table** — spec §3.3 step 5 R13-amendment paragraph + plan §0.A.5 `.env.local.example` template defer to this table for the per-command env-var contract; do NOT inherit env-var sets across rows by shorthand.

<!-- canonical-env-var-source: keep — §9.1.2 is the SOLE spec-side source of truth for VALIDATION_* canonical env-var literals per R27 commit 58 F10-class Option D refactor. Every other M12 doc surface that needs to name canonical env vars MUST cross-reference §9.1.2 (e.g., "per the canonical CLI env-var contract at §9.1.2") rather than re-listing the literal names. The structural-exclusivity walker at tests/cross-cutting/reseed-clears-oauth-claim-doc-guard.test.ts enforces this at CI time; this marker explicitly opts §9.1.2 INTO own-enumeration. Defense-in-depth: the walker matches the literal phrase "canonical-env-var-source: keep" to whitelist this section + the .env.local.example template block at 01-phase0-infra.md. -->
...
**R21 commit 44 F20 amendment — per-row env-var enumeration.** R20 surfaced that the prior table listed `VALIDATION_SUPABASE_URL` + `VALIDATION_SUPABASE_SECRET_KEY` + `VALIDATION_SUPABASE_PROJECT_REF` for the reseed row and used "Same three env vars" shorthand for the other three rows — omitting `VALIDATION_J3_CLAIM_EMAIL` (R13 commit 30) from every row's contract. An implementer following the table could ship reseed + check-seed without setting `VALIDATION_J3_CLAIM_EMAIL`, leaving R1.alias_5a_lead with a placeholder email at fixture-build time → predicate (k) DB-side check fires OR (worse) the env var is silently absent at TS-side fixture-build and the script throws. This was an F10-class same-vector recurrence (3rd round: R12 F10 + R14 F13/F14 + R20 F20). The table below now enumerates every env var per row explicitly; the "Same three env vars" shorthand is retired. Rows that do NOT need `VALIDATION_J3_CLAIM_EMAIL` (read-only commands that don't touch J3 OAuth state) are explicitly enumerated as `3 vars` with a reason; rows that DO need it are explicitly enumerated as `4 vars`. The structural defense extension at `tests/cross-cutting/reseed-clears-oauth-claim-doc-guard.test.ts` (R21 commit 47) asserts at CI time that every §-numbered table mentioning `VALIDATION_` literal lists all 4 canonical env vars OR carries an explicit `not-subject-to-meta: <reason>` comment.

| Command | Script path | CLI args | Required env vars | Target DB | Stdout contract | Stderr / exit codes | Idempotency |
|---|---|---|---|---|---|---|---|
| `pnpm validation:reseed` | `scripts/validation-reseed.ts` | `[--combo <R1\|...\|R8b\|SW-PRE_TRAVEL\|...\|SW-POST_SHOW\|all>] [--allow-local-override]` | **4 vars (R21 F20 — explicit):** `VALIDATION_SUPABASE_URL`, `VALIDATION_SUPABASE_SECRET_KEY`, `VALIDATION_SUPABASE_PROJECT_REF`, `VALIDATION_J3_CLAIM_EMAIL` (the dev's REAL Google email — used at TS-side fixture-build to write combo R1's `alias_5a_lead.email` per spec §3.3 step 5 R13-amendment paragraph; script aborts with predicate-(k)-shaped diagnostic if unset OR matches the canonical rejected-domain set) | Prod-equivalent Supabase (rejects localhost unless `--allow-local-override`) | Final line: `seeded <N> combos at <ISO timestamp>` | Exit 0 on success; exit 1 on env error / target validation / DB write failure. Stderr lines start with `[validation-reseed] `. | **`--combo <single>` is UPSERT-ONLY:** only the target combo's fixtures/crew_members/alias_map entries are touched; other combos preserved. **`--combo all` is full-replace.** Re-running with same args on same day = no-op; different date updates per-fixture date columns + bumps `last_seed_date`. |
| `pnpm validation:check-seed` | `scripts/validation-check-seed.ts` | `[--combo <single combo or 'all'>]` | **4 vars (R21 F20 — explicit):** `VALIDATION_SUPABASE_URL`, `VALIDATION_SUPABASE_SECRET_KEY`, `VALIDATION_SUPABASE_PROJECT_REF`, `VALIDATION_J3_CLAIM_EMAIL` (predicate (k) — see §3.3.2 — fails if env var is unset OR matches a placeholder/dev-only reserved domain in the canonical rejected set; the DB-side variant of predicate (k) also checks combo R1's `alias_5a_lead.email` row value against the same regex, but the env-var-side check fires first and produces a clearer diagnostic so the var is required even though check-seed is read-only) | Same | Exit-0 prints `OK: seed matches today (combos: ...)`; exit-1 prints diagnostic enumerating failed predicate (zero rows / stale date / missing combos / project_ref mismatch / missing aliases / show_share_tokens row missing / R1.alias_5a_lead placeholder-email DB or env / baseline-claim guard / orphan-roster guard) | Exit 0 = ready to walk; exit 1 = block walks until reseed runs. | Pure read; idempotent |
| `pnpm validation:resolve-alias` | `scripts/validation-resolve-alias.ts` | `<combo> <alias>` (positional; e.g. `R7b alias_5a_lead`) | **3 vars (R21 F20 — explicit; J3-claim-email NOT required):** `VALIDATION_SUPABASE_URL`, `VALIDATION_SUPABASE_SECRET_KEY`, `VALIDATION_SUPABASE_PROJECT_REF`. **Reason J3-claim-email is omitted:** this is a pure-read alias→UUID lookup; it does NOT touch J3 OAuth state, does NOT mint fixtures, and does NOT exercise predicate (k). Running with a missing/placeholder `VALIDATION_J3_CLAIM_EMAIL` is safe — the resolve-alias path never reads the var. | Same | Stdout = bare UUID on success | Exit 0 + UUID printed; exit 1 + diagnostic if combo or alias missing from alias_map | Pure read |
| `pnpm validation:report-fixtures` | `scripts/validation-report-fixtures.ts` | `--outcome <success-admin\|success-crew\|in-flight\|rate-limit-admin\|rate-limit-crew\|lookup-inconclusive\|lease-expired\|horizon-expired\|orphaned-lost-lease>` (R43 commit 81 F40 amendment — `success` split into `success-admin` + `success-crew` to expose the two distinct row-shapes pre-R43 conflated under one enum value: admin row carries `github_issue_url` in the response body per `lib/reports/submit.ts:179-186`, crew row omits it; outcome count is 9 post-R43, was 8 pre-R43) <br/>`[--alert-code <bot-login-missing\|duplicate-live-matches\|open-orphan-label\|inconclusive>]` (R43 commit 81 F40 amendment — variant selector for the 4 distinct `admin_alerts.code` values `lookup-inconclusive` can produce per `lookupAlertCode` at `lib/reports/submit.ts:202-208`: `BOT_LOGIN_MISSING`→`GITHUB_BOT_LOGIN_MISSING`, `DUPLICATE_LIVE_MATCHES`→`REPORT_DUPLICATE_LIVE_MATCHES`, `OPEN_ISSUE_WITH_ORPHAN_LABEL`→`REPORT_OPEN_ORPHAN_LABEL`, default→`REPORT_LOOKUP_INCONCLUSIVE`; default selector value is `bot-login-missing` matching the pre-R43 fixture context `code:'BOT_LOGIN_MISSING'`; IGNORED for all other 8 outcomes — silently accepted as no-op so wrapper scripts can pass uniformly) <br/>`[--combo <R1\|...\|R8b>]` (R41 commit 79 F38 amendment — REQUIRED for `--outcome rate-limit-crew`; resolves to the fixture `crew_member_id` UUID via `validation:resolve-alias <combo> alias_5a_lead` at harness start; IGNORED for all other 8 outcomes which use synthetic identities OR the env-var-sourced admin email — passing `--combo` to a non-crew outcome is silently accepted as a no-op so wrapper scripts can pass it uniformly) <br/>`[--cleanup [--include-admin-email <email>] [--include-crew-id <uuid>]]` (R33 commit 69 + R39 commit 76 F36 amendment) <br/>`[--force-overwrite-snapshot]` (R43 commit 80 F39 amendment — explicit escape hatch for the refuse-existing-snapshot guard; valid ONLY with `--outcome rate-limit-admin` OR `--outcome rate-limit-crew`; passing with any other outcome OR alongside `--cleanup` exits 1; surfaces the force-overwrite warning to stderr; rewrites the existing snapshot file with the currently-seeded state — loses the original pre-seed `prior_count`) | **4 vars (R21 F20 + R33 commit 68 — explicit; J3-claim-email NOT required, replaced by ADMIN_EMAIL for the rate-limit-admin outcome):** `VALIDATION_SUPABASE_URL`, `VALIDATION_SUPABASE_SECRET_KEY`, `VALIDATION_SUPABASE_PROJECT_REF`, `VALIDATION_ADMIN_EMAIL` (the dev's REAL admin email — used to seed `report_rate_limits.identity` for the `--outcome rate-limit-admin` path so the production quota deny path is actually exercised on the next admin POST; live `enforceQuota` canonicalizes the admin identity via `lib/email/canonicalize.ts:76`, so the harness MUST seed the canonical form too). **Reason J3-claim-email is omitted:** report-pipeline outcomes are independent of J3 OAuth identity — the 3-table producer set (`reports` / `report_rate_limits` / `admin_alerts`) does not reference picker rosters, alias maps, OR `VALIDATION_J3_CLAIM_EMAIL`. Running with a missing/placeholder J3 var is safe — the report-fixtures path never reads it. The ADMIN_EMAIL var is required only for the `rate-limit-admin` outcome. **R39 commit 76 F36 identity-shape amendment (count updated R43 F40).** Post-R43, the 9 outcomes split into 2 real-identity outcomes (rate-limit-admin uses canonical admin email; rate-limit-crew uses a raw fixture `crew_members.id` UUID resolved via `pnpm validation:resolve-alias <combo> alias_5a_lead`) + 7 synthetic-default outcomes (success-admin / success-crew / in-flight / lookup-inconclusive / lease-expired / horizon-expired / orphaned-lost-lease — using `validation:m12-fixture-<outcome>:<uuid>` synthetic identities via `context->>'validation_tag'` for safe-by-default cleanup; the R43 F40 split of pre-R43 `success` into `success-admin` + `success-crew` raised the synthetic-default count from 6 to 7). The pre-R39 wording "other 7 outcomes use `validation:m12-fixture-<outcome>:<uuid>` synthetic identities" is **RETIRED** — it conflated the rate-limit-crew outcome with the synthetic-default set, contradicting live `lib/reports/submit.ts:168` (`reporterFor` writes `identity=auth.crewMemberId` raw UUID for crew quota; synthetic prefix cannot intercept production quota path) and contradicting plan 04:70/107 which already named the real fixture crew_member_id. | **Per-outcome producer table (R31 commit a5ed46f producer-map ratification — canonical source at handoff §9 R31 §A; outcome count updated to 9 per R43 F40):** writes to `reports` (success-admin / success-crew / in-flight / lease-expired / horizon-expired; optional stub for lookup-inconclusive / orphaned-lost-lease), `report_rate_limits` (rate-limit-admin / rate-limit-crew), AND `admin_alerts` (lookup-inconclusive / orphaned-lost-lease — primary observable surface via `AlertBanner`; lookup-inconclusive's `admin_alerts.code` value is selected via the R43 F40 `--alert-code` selector per `lookupAlertCode` at `lib/reports/submit.ts:202-208`). Service role for all writes; row shapes match the live production writers per `lib/reports/submit.ts` + `lib/reports/rateLimit.ts`. NOT a singular-`reports`-only target — pre-R31 wording retired. | Stdout = `materialized <outcome> report row <id>` where `<id>` is the polymorphic producer-row identifier: `reports.id` (uuid) for reports-producing outcomes; `admin_alerts.id` (uuid) for admin_alerts-primary outcomes; `(kind, identity, hour_bucket)` tuple for rate-limit outcomes. The leading word `report` is preserved across outcomes for stdout-parser stability — the table varies by outcome, the stdout shape does not. | Exit 0 on success; exit 1 if outcome name unknown OR DB write fails OR (for rate-limit-admin) `VALIDATION_ADMIN_EMAIL` unset/empty OR (for rate-limit-crew, R41 commit 79 F38) `--combo <combo>` flag unset/empty OR the combo's `alias_5a_lead` resolves to no `crew_member_id` (stderr: `--outcome rate-limit-crew requires --combo <combo> to resolve a fixture crew_member_id via validation:resolve-alias <combo> alias_5a_lead`) OR (for rate-limit-{admin,crew}, R43 commit 80 F39) the snapshot file at `.validation-state/rate-limit-{admin,crew}-snapshot.json` already exists AND `--force-overwrite-snapshot` is NOT passed (stderr: `snapshot file already present at <path>; run cleanup first OR use --force-overwrite-snapshot to re-snapshot the existing seeded state (loses original pre-seed prior_count)`) OR `--force-overwrite-snapshot` is passed alongside any non-rate-limit outcome OR alongside `--cleanup` (stderr names the flag as valid only with `--outcome rate-limit-{admin,crew}` at seed time) OR (for lookup-inconclusive, R43 commit 81 F40) `--alert-code <value>` is passed with a value not in the canonical 4-variant set `{bot-login-missing, duplicate-live-matches, open-orphan-label, inconclusive}` (stderr names the canonical set) | Each invocation inserts a new row in the appropriate producer table. **Cleanup contract:** `--cleanup` deletes validation-tagged rows in canonical order — `admin_alerts` (where `context->>'validation_tag' LIKE 'm12-fixture-%'`) first → `report_rate_limits` (where `identity LIKE 'validation:m12-fixture-%'`) second → `reports` (where `context->>'validation_tag' LIKE 'm12-fixture-%'`) last. Default `--cleanup` is conservative: does NOT touch real admin-email rows AND does NOT touch real crew-UUID rows. **`--cleanup --include-admin-email <email>` snapshot+restore contract (R37 commit 74 propagation of R35 commit 73 F34 plan-side amendment; canonical source at plan `04-phase0-tooling-report.md` F34 cleanup safety contract block).** The rate-limit-admin cleanup is **non-destructive of prod state** — it MUST NOT issue a blanket `WHERE kind='admin' AND identity=canonicalize(<email>)` predicate spanning all `hour_bucket` values (the `report_rate_limits` primary key is `(kind, identity, hour_bucket)` per `supabase/migrations/20260501001000_internal_and_admin.sql:333`; a blanket predicate would erase live production admin rate-limit state in unrelated hours). The lifecycle is: **(1) Snapshot before seed.** Before UPSERTing the rate-limit-admin row the harness `SELECT count FROM report_rate_limits WHERE kind='admin' AND identity=canonicalize($VALIDATION_ADMIN_EMAIL) AND hour_bucket=date_trunc('hour', now())` and persists `(snapshot_prior_count, recorded_hour_bucket, email_canonical_form)` across the CLI-invocation boundary (separate processes — snapshot must survive seed → exercise → cleanup). If no row exists pre-seed, `snapshot_prior_count = NULL`. **(2) Seed at the exact recorded bucket.** UPSERT — `INSERT ... ON CONFLICT (kind, identity, hour_bucket) DO UPDATE SET count = 11` — so the seed is idempotent and the bucket coordinates are pinned. **(3) Cleanup branches at the EXACT recorded bucket (never cross-hour).** `snapshot_prior_count IS NULL` branch issues a DELETE scoped to `hour_bucket=<recorded_hour_bucket>` only; `snapshot_prior_count IS NOT NULL` branch issues an UPDATE restoring the prior count scoped to `hour_bucket=<recorded_hour_bucket>` only. Both branches carry the explicit `AND hour_bucket=<recorded>` clause — cross-hour deletion is structurally impossible. **(4) Snapshot persistence (R41 commit 78 F37 amendment — file-backed only).** The sole supported storage strategy that survives the inter-process boundary is file-backed at `.validation-state/rate-limit-admin-snapshot.json` (gitignored — `.validation-state/` is in repo-root `.gitignore` per R41 commit 78). `validation_state` CANNOT be used as a snapshot backend because the table is defined with `CHECK (key = 'validation_seed')` singleton constraint per spec §3.3.2 DDL — any other key fails the CHECK at INSERT. The pre-R41 wording allowing `validation_state` row as an alternative is **RETIRED**. **(5) Defense-in-depth refusal absent snapshot.** Cleanup invoked WITHOUT a snapshot record (orphaned cleanup after a crash, lost snapshot file/row) MUST refuse with stderr `no rate-limit-admin snapshot found; cleanup aborted to avoid touching prod buckets` (exit 1). **(6) Emergency-recovery escape hatch.** `--force-cleanup-without-snapshot --hour-bucket <ISO timestamp> --kind admin` is the explicit emergency flag for the case where the dev knows the recorded bucket value but the snapshot was lost — DELETEs only the exact named bucket (still never cross-hour). **(7) Refuse-existing-snapshot guard at seed time (R43 commit 80 F39 amendment — F34-class round 3 structural fix).** F39 (R42 finding) surfaced that the pre-R43 contract had no guard against duplicate seed invocations: a second `pnpm validation:report-fixtures --outcome rate-limit-admin` before cleanup would silently overwrite the snapshot file with the already-seeded count=11 (the pre-seed prior count was lost; cleanup restored the wrong state). The repair: the harness MUST refuse to seed if `.validation-state/rate-limit-admin-snapshot.json` already exists — exit 1 with stderr `snapshot file already present at .validation-state/rate-limit-admin-snapshot.json; run cleanup first OR use --force-overwrite-snapshot to re-snapshot the existing seeded state (loses original pre-seed prior_count)`. The `--force-overwrite-snapshot` flag is the explicit crash-recovery escape hatch (surfaces stderr warning `--force-overwrite-snapshot: rewriting existing snapshot at .validation-state/rate-limit-admin-snapshot.json from previous seeded state; original pre-seed prior_count is lost`). Cleanup (steps 3 + 6 above) `unlink`s the snapshot file on success so the next seed invocation starts clean — close-on-success completes the round-trip semantics. The pre-R37 wording omitting the exact-bucket clause is **RETIRED** — it described destructive cross-hour deletion. **`--cleanup --include-crew-id <uuid>` snapshot+restore contract (R39 commit 76 F36 amendment — parallels the admin contract above; canonical source at plan `04-phase0-tooling-report.md` F36 cleanup safety contract block).** The rate-limit-crew cleanup is **non-destructive of prod state** — F36 was F34-class round 2: R35 patched the admin destructive-cleanup surface but missed the parallel crew surface (same shape, same data-loss class). A pre-R39 cleanup of the form covering kind='crew' identity=<uuid> spanning all hour_bucket values would have erased live production crew rate-limit state for that fixture crew member in unrelated hours. The crew lifecycle is structurally identical to the admin lifecycle: **(1) Snapshot before seed (crew).** Harness `SELECT count FROM report_rate_limits WHERE kind='crew' AND identity=<fixture_crew_member_id_uuid> AND hour_bucket=date_trunc('hour', now())` and persists `(snapshot_prior_count_crew, recorded_hour_bucket_crew, crew_member_id_uuid)` across the CLI-invocation boundary (NO canonicalization — crew identity is a raw UUID per `lib/reports/submit.ts:168` and `lib/reports/rateLimit.ts:76`). **(2) Seed at the exact recorded bucket (crew).** UPSERT at the recorded bucket with count=4 (crew limit is 3 per `lib/reports/rateLimit.ts:54`). **(3) Cleanup branches at the EXACT recorded bucket (crew, never cross-hour).** Snapshot-NULL branch is an exact-bucket DELETE; snapshot-present branch is an exact-bucket UPDATE restoring the prior count. Both branches structurally carry the `AND hour_bucket=<recorded>` clause. **(4) Snapshot persistence (crew — R41 commit 78 F37 amendment — file-backed only).** Same single strategy as admin: file-backed at `.validation-state/rate-limit-crew-snapshot.json` (gitignored). `validation_state` is unavailable as a backend per the same `CHECK (key = 'validation_seed')` singleton-constraint reason. **(5) Defense-in-depth refusal absent snapshot (crew).** Refuses with `no rate-limit-crew snapshot found; cleanup aborted to avoid touching prod buckets` (exit 1). **(6) Emergency-recovery escape hatch (crew).** `--force-cleanup-without-snapshot --hour-bucket <ISO> --kind crew` (the `--kind` flag is mandatory on the force escape hatch so an admin-snapshot-loss accident cannot silently land a crew-cleanup DELETE). Combined invocation `--cleanup --include-admin-email <email> --include-crew-id <uuid>` is supported and cleans both real-identity surfaces in a single command. **(7) Refuse-existing-snapshot guard at seed time (crew — R43 commit 80 F39 amendment — F34-class round 3 structural fix; parallels the admin F39 guard).** Same shape, same data-loss class: a second `pnpm validation:report-fixtures --outcome rate-limit-crew --combo <combo>` before cleanup would silently overwrite the snapshot file with the already-seeded count=4 OR — worse — clobber the first crew UUID's snapshot with a second `--combo` resolving to a different crew_member_id. The repair: the harness MUST refuse to seed if `.validation-state/rate-limit-crew-snapshot.json` already exists (file-presence-based, NOT identity-based, so any second crew seed of any combo refuses) — exit 1 with stderr `snapshot file already present at .validation-state/rate-limit-crew-snapshot.json; run cleanup first OR use --force-overwrite-snapshot to re-snapshot the existing seeded state (loses original pre-seed prior_count)`. The `--force-overwrite-snapshot` flag is the same explicit crash-recovery escape hatch as admin (warns about lost pre-seed prior_count). Cleanup `unlink`s the crew snapshot file on success so the next seed invocation starts clean. |
...
**Auth contract:** the Vercel route handlers continue to use `app/api/cron/_auth.ts` `rejectUnauthorizedCron` (unchanged from M12 R70 HEAD). `net.http_get()` (pg_net installs its functions in the `net` schema, NOT `pg_net`) includes the header `{ "Authorization": "Bearer <CRON_SECRET>" }` where `<CRON_SECRET>` is sourced inside the cron job body via the Supabase Vault decrypted-secrets view, keyed by the `fxav_cron_secret` name. Vault is preferred over a Postgres GUC because (a) encrypted at rest, (b) rotation does not require re-deploying a migration, (c) isolates secret value from `pg_settings` visibility.

**Time zone:** all 7 schedules are interpreted in UTC. pg_cron's default timezone is the cluster setting; Supabase clusters are UTC. Vercel Cron was also UTC. Equivalence is preserved.

**Env var contract delta to §9.1.2:** the `CRON_SECRET` env var (already documented in `_auth.ts:4`) MUST be set in Vercel Production scope AND mirrored into the Vault entry `fxav_cron_secret` in the validation Supabase project. The two values MUST match byte-for-byte. Wired in Phase 0.A.4.5 (Vault populate via Dashboard UI) + Phase 0.A.5 (Vercel env var).

**Smoke 3 observability path** is amended per §9.2 below.
...
**Smoke test 7 (conditional, R24; R47 commit 85 F42 amendment — explicit `--alert-code inconclusive`).** If Band F deep report outcomes default to INCLUDED-via-harness, run `pnpm validation:report-fixtures --outcome lookup-inconclusive --alert-code inconclusive` and verify the report-failure UI row renders correctly. The explicit `--alert-code inconclusive` selector pins the canonical `REPORT_LOOKUP_INCONCLUSIVE` surface per `lookupAlertCode` at `lib/reports/submit.ts:202-208` (default branch); the R43 commit 81 F40 default selector is `bot-login-missing` which would produce `GITHUB_BOT_LOGIN_MISSING` — see plan 04 F41 alignment + R45 commit 84 propagation. Skipped only when MATRIX-INVENTORY.md pre-dispositions every deep outcome as EXCLUDED-rely-on-structural.
...
| Build-vs-runtime gate explicitness | Applies | Phase 0 §9 names the build target (Vercel `*.vercel.app` production deployment, no custom domain); §9.2 names the six runtime smoke tests that gate Phase 1 (R15 added the mint-redeem round-trip as smoke #6). The eight CI gates (§9.1.1) are PR-time + main-branch build-time gates (X6-D-1 dormancy applies to the merge-blocking layer only; the gates still run continuously); the alert path is a runtime path. R6 amendment: stale "preview build" wording corrected to "production deployment" in this row. M12.1 (2026-05-27): added gate #8 `x6-pg-cron-pivot`. |
...
**Posture.** This is a structural amendment, not a review round. It rebases the spec (drafted 2026-05-19 against the pre-M11.5 per-crew signed-link auth model) onto the picker-pivot world ratified at the M11.5 close-out (`b4b2c38`, 2026-05-25). The amendment session was authored by an Opus 4.7 implementer per the orchestrator's scope ratification, with the user's `(d)` decision on J3 baked in. The full milestone handoff lives at [`docs/superpowers/plans/2026-04-30-fxav-crew-pages-v1/handoffs/M12-solo-dev-ux-validation.md`](../../plans/2026-04-30-fxav-crew-pages-v1/handoffs/M12-solo-dev-ux-validation.md) (project convention: single milestone handoff in the master plan's handoffs dir, mirroring `M11.5-crew-auth-pivot.md`; round-by-round audit lives inline in that doc's Convergence log). Post-amendment, the spec proceeds to fresh cross-CLI adversarial review at R6 (= amendment R1); the R1–R25 rounds in §15.1–§15.25 are pre-rebase audit trail and remain readable as historical context.
