...
**R13 commit 30 amendment — combo R1's `alias_5a_lead.email` exception.** The synthesized `validation+<combo>-<alias>@example.com` (or shorter `validation+5a@example.com`) format applies to every alias in every combo EXCEPT **combo R1's `alias_5a_lead`** — that one row's `email` field reads from env var `VALIDATION_J3_CLAIM_EMAIL` (the dev's real Google account email). Rationale: J3 leg (c) (06-phase1-matrix-walk.md Task 1.6 leg c) walks the OAuth-claim identity-exclusivity contract by signing the dev's real Google account in via `claim_oauth_identity`; Google OAuth cannot authenticate against the RFC 2606 reserved domains (`example.com` / `example.org` / `example.net`), so the synthesized email is unusable for this one load-bearing fixture row. Per spec §1.5 "solo-dev IS the validation" the dev's personal email IS the alias_5a_lead identity for combo R1. **Combo-isolation:** the dev's email is unique → `claim_oauth_identity(canonicalEmail)` stamps `claimed_via_oauth_at` only on R1's `alias_5a_lead` row → claims don't poison the LEAD identity across R2–R8b + 6 SW-states. See the R13-amendment paragraph below the §3.3 step 5 Target-selection row for full rationale. The reseed's `mint_validation_fixture_atomic` RPC receives the env var value via the TypeScript script's fixture-payload assembly (canonicalized via `lib/email/canonicalize.ts` BEFORE the RPC write per AGENTS.md invariant 3); the RPC writes it as-is. check-seed predicate (k) fails if the env var is still a placeholder reserved domain at seed time.
