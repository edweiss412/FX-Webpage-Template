...
| Target selection (R9 amendment + R11 correction + R13 commit 30 J3-claim-email addition; R21 commit 46 — defer per-command env-var map to §9.1.2) | Script requires the prod-equivalent Supabase target — NOT local. Reads **four** required env vars (aligned with repo convention from `.env.local.example:5` which uses `SUPABASE_SECRET_KEY` not `SERVICE_KEY`): **`VALIDATION_SUPABASE_URL`** + **`VALIDATION_SUPABASE_SECRET_KEY`** + **`VALIDATION_SUPABASE_PROJECT_REF`** + **`VALIDATION_J3_CLAIM_EMAIL`** (R13 amendment — the dev's REAL Google account email used for J3 leg (c) OAuth-claim walk; see R13-amendment paragraph below this row). **Canonical per-CLI env-var contract:** §9.1.2 (R21 commit 44 F20 amendment) is the authoritative command-by-command env-var map; this row describes the reseed CLI's contract, but the table at §9.1.2 enumerates each command's exact env-var set explicitly (no "Same N env vars" shorthand). Plan §0.A.5 `.env.local.example` template defers to §9.1.2 for the per-command map. All four are mandatory; missing any aborts. If `VALIDATION_SUPABASE_URL` matches `localhost`, `127.0.0.1`, or `::1` the script aborts unless `--allow-local-override` is passed (intentional override gate; not used during M12 walks). Phase 0 sub-task adds all four to the dev's local `.env.local` AND extends `.env.local.example` with each as a documented placeholder. The Supabase project ref is stamped in `validation_state.seeded_supabase_project_ref` (added in §3.3.2 DDL) so check-seed can verify target consistency (`check-seed` fails if `seeded_supabase_project_ref != $VALIDATION_SUPABASE_PROJECT_REF`); check-seed predicate (k) (added R13 commit 30) fails if `VALIDATION_J3_CLAIM_EMAIL` is still a placeholder example.com / example.org / example.net domain at seed time.
