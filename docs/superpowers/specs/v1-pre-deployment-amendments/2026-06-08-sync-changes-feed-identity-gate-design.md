...
**Guard conditions:** Approve/Reject are idempotent against a stale hold (if it already released via sheet reconciliation, the action no-ops with a typed "already resolved" result via `lib/messages`). Approve uses `base_modified_time` to detect a disposition the sheet has since moved past: if the live sheet `modifiedTime` no longer matches, Approve returns a typed "changed — re-review" result rather than applying a stale disposition. The feed-shown disposition and the applied disposition are guaranteed identical (both are `proposed_value`). Concurrent sync + approve are serialized by the per-show advisory lock.

**Collision & swap handling (finding F10 — multiple open holds).** A show can have *N* independent `mi11_pending` holds; their `held_value` rows stay pinned to the old emails, so a single Approve can collide with `crew_members_show_email_unique` (`supabase/migrations/20260501000000_initial_public_schema.sql:49`). The canonical case is a **two-crew email swap** (Alice a→b, Bob b→a — both trip MI-11, both pinned to old emails); approving either alone tries to write an email the other still owns. Rule: when Approve's `proposed_value` (email or rename name) would collide with a live row, **classify the colliding owner**:
  - **The owner is itself an open hold whose disposition would *vacate* that value** → the holds form a **collision group**, approved **atomically in one lock-taking RPC** with a swap-safe write sequence — park one row's email to **NULL** (the index is partial `WHERE email IS NOT NULL`, so NULLs never collide), reassign the freed targets, then settle — because the partial unique index is **not** deferrable and a naive single-statement swap fails mid-statement. The feed renders grouped holds with a single **[Approve both]** action.
  - **The owner is a genuine duplicate** (no vacating hold) → Approve returns a typed `EMAIL_WOULD_DUPLICATE` conflict; the feed shows "would duplicate `<email>` — fix the sheet" **instead of** an Approve button (never a button that always fails).
  Tested by the two-crew-swap case (§9).
...
- **Two-crew email swap (F10):** Alice a→b, Bob b→a → two `mi11_pending` holds; the feed groups them with a single [Approve both]; atomic swap-safe approve satisfies `crew_members_show_email_unique`; a genuine duplicate (no vacating sibling hold) yields a typed `EMAIL_WOULD_DUPLICATE` + no Approve button (catches "two un-approvable items / unique-index violation").
