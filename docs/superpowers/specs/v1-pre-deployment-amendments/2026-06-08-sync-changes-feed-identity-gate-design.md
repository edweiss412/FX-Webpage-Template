...
| `held_value` | jsonb not null | the prior crew row — used for its **identity** (old email + old name, which the apply pins each sync per §4.2; non-identity fields are NOT pinned, finding F17) and as the full row to **retain on a removal** or **restore on Reject/undo** |
...
- **`crew_email` hold on crew "Alice" — an IDENTITY-level hold, not a full-row freeze (finding F17).** Only Alice's **identity** (email, name, existence) is held; her **non-identity fields auto-apply** like everyone else's, preserving the core contract that only MI-11 identity movement is gated. Concretely:
  - **(a) Identity pin + delete-suppression:** Alice's name is **excluded from `deleteCrewMembersNotIn`** (`lib/sync/applyParseResult.ts:53`; `lib/sync/runScheduledCronSync.ts:1104`) so a later sheet edit cannot drop her. On upsert, her **non-identity fields (phone, role, restrictions, flight_info, …) follow the latest sheet row and auto-apply each sync** — only her **identity is substituted**: email pinned to `held_value.email` (old), and the name pinned to the old name (a pending rename's new name is suppressed). Approval is the only moment the *identity* moves; routine field edits never wait.
  - **(b) Identity re-target:** a further **identity** change (a new email, or a rename) updates `proposed_value` + `base_modified_time` (§4.3). Non-identity field changes are **not** gated and do not touch `proposed_value`.
...
- **Identity-only hold (F17):** while Alice's email is held, a later sheet edit to **Alice's own** phone / role / restrictions / flight_info **auto-applies** (is NOT frozen); only her email stays pinned until Approve (catches "MI-11 hold freezes the whole crew row, staling non-identity fields").
