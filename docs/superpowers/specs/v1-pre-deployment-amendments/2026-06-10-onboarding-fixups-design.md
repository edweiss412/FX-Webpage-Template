...
| D-2 | Existing-show wizard applies flow through the **same machinery as the live Apply path**: the hold-aware `runPhase2` core (feed entries via `writeAutoApplyChanges` + MI-11 hold semantics) that cron auto-apply uses, with wizard reviewer choices satisfying the identity gate the same way reviewer choices do on that path. No second apply variant. **Clarified post plan-R10:** the legacy dashboard whole-parse staged-Apply surface for live rows was RETIRED at the sync-changes-feed cutover (P2-F7 comment, `lib/sync/applyStaged.ts:1345-1353`) and remains deliberately fail-closed with no feed writes — it is NOT the parity target and is not extended. Parity regressions compare wizard existing-show applies to the cron auto-apply semantics. |
...
- Drive/Storage I/O placement (Phase D stays SQL-only; the full apply is pure SQL). **Bounded staleness window, ratified + self-healing (R23 finding 1 disposition):** master spec 2583(vi) ratifies Phase D as "ONE short transaction with NO Drive/Storage I/O"; the mandatory Drive head re-verify lives in Phase B (B.2.pre, master spec 2581). A Doug-edit landing between Phase B and Phase D means Phase D applies the operator-reviewed staged parse, then the next cron pass re-syncs (Drive modifiedTime > applied watermark → the watermark gate fires), correcting children/feed and re-evaluating holds in place (sync_holds re-evaluation is part of the hold-aware apply contract). Adding Drive I/O to Phase D would both violate the ratified contract and remain TOCTOU. Required regression: stage → Doug-edit → Phase D applies staged content → next cron pass converges to the newest revision, with feed/hold state consistent.
...
- **One-time purge** of the current 18+18 synthetic rows rides the F2 migration, keyed to the EXACT 18 `wizard_session_id` values captured from the validation DB (enumerated in the migration; no-op in any environment where they don't exist). An earlier `drive_file_id like 'drive-%'` prefix boundary was rejected (R23 finding 2): Drive IDs are opaque external identifiers, so a prefix is not a safety proof.
...
| NEW structural guard | A meta-test asserting no file outside an explicit allowlist issues `insert into public.shows` / child-table snapshot-replacement SQL (the "second copy" tripwire that would have caught this bug at introduction). Allowlist pinned by path+symbol (R22 finding 1): the shared apply module AND the canonical first-seen insert inside `lib/sync/runScheduledCronSync.ts` (`PostgresPipelineTx.applyShowSnapshot`'s `insertFirstSeenShowWithSlugRetry` call site) — which §3.1 deliberately retains. Walks `app/api/**` + `lib/**`, not a lexical file list; any NEW writer (e.g., a resurrected bespoke wizard insert) fails. |
