...
- **Source-scoped pending-ingestion cleanup (R10 finding 1).** The shared core's `ApplyTx` contract includes `deleteLivePendingIngestion(driveFileId)` (`lib/sync/applyParseResult.ts:41`), which `applyParseResult` calls unconditionally — correct for live-partition callers (cron/dashboard), WRONG for wizard-scoped applies: a coexisting live `pending_ingestions` row (`wizard_session_id IS NULL`) for the same `drive_file_id` would be silently erased from the dashboard's needs-attention partition by a wizard action. The extraction makes this cleanup an injected, source-scoped operation: live callers keep the current delete; wizard callers (Phase B first-seen AND Phase D shadow) pass a no-op — the wizard's own row lifecycle is already handled by Phase B's `pending_syncs` DELETE and the manifest machinery, and wizard-partition `pending_ingestions` rows are managed by the retry/defer routes, never by the apply core. Required regression: a live `pending_ingestions` row and a wizard staging row sharing a `drive_file_id`; wizard finalize completes; the live row survives.
     ...
     where s.last_seen_modified_time is not null
       and (
         ...
         (select not (sa.parse_result_summary ? 'crewCount')
                 ...
                 and (prev_pass is null or sa.applied_at > prev_pass)
