...
| F5 retry-route hardening | manifest, deferred_ingestions, pending_ingestions | existing JS-side `withPostgresSyncPipelineLock` (retry/route.ts:73); if option (b) RPC is chosen, the RPC acquires NO advisory lock — the JS wrapper stays the single holder, pinned in `advisoryLockRpcDeadlock.test.ts` |
