...
2. **Page consumption:** `authenticated` ⇒ `redirect("/auth/sign-in?next=/admin")` — the sign-in page's session-present branch resolves admin → `/admin`, non-admin → `/me` (D-2; one extra 302 on a rare path, zero duplicated logic). `anonymous` ⇒ render the landing. `infra_error` ⇒ `console.error("[root-landing] session probe infra fault:", message)` **then render the landing** — the ratified fail-open UI (D-1/§7.3) with an operator-observable signal (Vercel function logs); the fault is a discriminable typed result end-to-end and only the RENDER decision converges with anonymous. A signed-in user during an Auth outage sees the landing card whose CTA leads into sign-in, which surfaces the cataloged `ADMIN_SESSION_LOOKUP_FAILED` block — for THROWN faults today (`app/auth/sign-in/page.tsx:107-112`) and, with this milestone's §4.1.5 alignment, for RETURNED non-missing faults as well.
...
5. **Sign-in returned-error alignment (R3 amendment, small in-scope change to `app/auth/sign-in/page.tsx`):** the sign-in already-authenticated guard currently forces `ADMIN_SESSION_LOOKUP_FAILED` only when `getUser` THROWS (`:107-112`); a RETURNED non-missing error silently falls through to the bare CTA (`:113-116` region) — the same R2 classification gap at the surface where the user actually acts. This milestone adds the one-branch alignment: returned error with `isAuthSessionMissingError(error)` false ⇒ `forcedErrorCode = "ADMIN_SESSION_LOOKUP_FAILED"` (returned missing-session errors keep today's fall-through-to-CTA behavior). This makes the §4.1.2 outage-visibility claim true end-to-end and closes the class at both surfaces. Existing sign-in tests must stay green except any that pinned the silent fall-through for non-missing returned errors (none expected; verify at plan time — if one exists, it pinned the gap, not a contract).
...
Marketing content, SEO beyond existing metadata, per-session deep links beyond sign-in's resolution, crew "find my show" search, screenshot baseline, changes to `/me`. (`/auth/sign-in` gets exactly the one-branch §4.1.5 alignment and nothing else.)
...
2b. **Returned-non-missing chain (R3)** — unit-level: sign-in page with `getUser` mocked to RETURN a non-missing error (status-500 AuthApiError shape) renders the `sign-in-error-block` with `ADMIN_SESSION_LOOKUP_FAILED` copy (and does NOT redirect); root page with the same mock renders the landing + `console.error`. *Catches: the outage being user-hidden at either surface (the R3 finding).*
