...
This is the v1 admin-scoped predicate. Live `lib/messages/adminSurface.ts:23 (`INFO_SEVERITY_CODES` filter; originally cited `components/admin/AlertBanner.tsx:57`, but the banner no longer exists — retired by the bell notification center, which reads the same catalog-derived predicate from `lib/messages/adminSurface.ts`)` treats only `severity === "info"` as excluded from the alert banner; unset severity is rendered as warning-equivalent. The narrowed predicate covers warning + unset-severity entries AND restricts to admin-facing rows.
...
2. **Catalog meta-test** (`tests/messages/_metaErrorCatalogDocs.test.ts`) — r8 simplified
   ...
   - Predicate covers both `severity === "warning"` and entries with unset severity (per `lib/messages/adminSurface.ts:23 (`INFO_SEVERITY_CODES` filter; originally cited `components/admin/AlertBanner.tsx:57`, but the banner no longer exists — retired by the bell notification center, which reads the same catalog-derived predicate from `lib/messages/adminSurface.ts`)` default-warning rule)
...
| `severity` is OPTIONAL on catalog entries; many user-visible codes omit it (e.g. `LEAKED_LINK_DETECTED`) and are rendered as warning-equivalent | `lib/messages/catalog.ts:64` (drifted from `:46-53` as catalog grew during M9.5/M10); default-warning rule at `lib/messages/adminSurface.ts:23 (`INFO_SEVERITY_CODES` filter; originally cited `components/admin/AlertBanner.tsx:57`, but the banner no longer exists — retired by the bell notification center, which reads the same catalog-derived predicate from `lib/messages/adminSurface.ts`)` | ✅ |
...
| **AC-11.28** | The `messageFor` signature is unchanged. Existing call sites (e.g., `app/admin/layout.tsx`, `components/admin/BellPanel.tsx`) continue to compile and behave identically. Only the return type widens. |
...
| **AC-11.33** | Screenshot harness suppresses Supabase Realtime push during capture (per §3.6.2) via a no-op `WebSocket` init script (`scripts/capture-core.ts:25`; extracted from help-screenshots.ts by the 2026-07-26 gallery-screenshot-capture spec §3 item 1). **r13 amendment (Phase I Codex R3):** the original wording named "stable cookies persist across captures via `globalSetup` `storageState` reuse" — the shipped harness uses a paired setup-project (per r12 amendment on AC-11.27) plus per-capture sign-in instead. `tests/e2e/screenshots-help-setup.ts` only seeds the DB; `scripts/help-screenshots.ts:96` calls `signInAs(page, adminFixture)` inside each capture and creates a fresh browser context per entry/theme (`scripts/help-screenshots.ts:133`). The "stable cookies across captures" property is provided not by `storageState` reuse but by the deterministic `signInAs(adminFixture)` reusing the same fixture identity on every capture, plus `ENABLE_TEST_AUTH=true` bypassing CSRF nonce churn for the test session. The Realtime-suppression half of this AC continues to apply to the WebSocket init-script path. See §3.6.2 row "CSRF / session-cookie volatility" for the same amendment. |
