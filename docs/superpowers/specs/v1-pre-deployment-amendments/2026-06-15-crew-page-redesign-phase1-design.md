...
14. **Transport visibility gate preserved.** Travel ground transport keeps `transportTileVisible` (driver PII / vehicle / plate / parking / assignments shown only to admin, the assigned driver, or named crew). The Gear A/V/L gate→emphasis flip (D-5) does **not** extend to transport — that is a privacy gate, not a scope-relevance gate.
...
17. **Transport visibility gate** (Travel) — `transportTileVisible` matrix: admin → ground transport visible; assigned driver (`viewerName` matches the leg) → visible; crew named on a transport leg → visible; **unassigned crew → the Travel ground-transport block is omitted** (no driver name/phone, vehicle, plate, parking, or assignments in the DOM). Also: a `transportation` tile-error for unassigned crew shows neither data nor degraded UI. _Catches: leaking driver PII / assignments to every crew member — a trust-boundary regression from dropping the gate._
