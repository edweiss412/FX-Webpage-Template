...
- **RightNowHero owns the live clock + state derivation (reused from `RightNowCard`; NOT a precomputed prop).** It takes `{ context }` only; it seeds `useState(() => initialNow)` from the server's `nowDate()` (SSR + screenshot determinism via `X-Screenshot-Frozen-Now`), runs the 60s minute-tick + `visibilitychange` refresh, and **re-derives `selectRightNowState(now, context.dates, context.dateRestriction)` per tick** — so the hydrated hero reclassifies across minute/day boundaries (Today → off-day → show-day rollover) instead of freezing the SSR state. It carries `RightNowCard`'s `lastGood`/`morph-to-last-good` + `prefersReducedMotion` machinery (§4.16). Effectively **`RightNowHero` IS `RightNowCard` with re-skinned body slots**; it does **not** accept a precomputed `state`.
...
- **Today-highlight uses the show timezone.** The Schedule "today" pin and every today-comparison use `todayIsoInShowTimezone` / `resolveShowTimezone(venue.timezone)` (`ScheduleTile:170`) — consistent with the hero's `nowDate()` (§4.14); a day-boundary correctness contract, not wall-clock.
