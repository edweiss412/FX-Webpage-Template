...
- **`tileErrors` observability is section-independent (the alert bridge).** `getShowForViewer` (`:335-505`) only *populates* `tileErrors[key]`. Today the persistent `admin_alerts` row is guaranteed by the **always-rendered** `notes-tile` catch-all (`_ShowBody.tsx:408-419`), which re-throws on `hotel`/`rooms`/`contacts` **ungated** (+ `transportation` gated) through `TileServerFallback`. Because `CrewShell` renders only the **active** section, that guarantee would be lost — a `hotel`/`rooms`/`contacts` failure whose block sits in an unopened section would never alert. So `CrewShell` mounts a **section-independent `<TileErrorAlertBridge>`** (always rendered; renders nothing) that performs a **single** upsert of a **distinct, NEW** admin-alert code **`TILE_PROJECTION_FETCH_FAILED`** (projection/data-fetch failures — semantically separate from a tile *render* throw, which keeps `TILE_SERVER_RENDER_FAILED`) via the shared `upsertAdminAlert(...)`, context `{ sheet_name (=show.title), tileId (=crew:alert-bridge), message, failedKeys, signature }` with `signature = { viewerVersionToken, failedKeys (sorted) }`. The bridge is the **sole** producer of this new code (§9 registers it in `_metaAdminAlertCatalog`); `admin_alerts.code` is unconstrained `text`, so **no migration** — only the §12.4 three-lockstep (§5). Which keys count, per the effective current contract:
  ...
  **A distinct code is the robust fix** for three problems a *shared* `TILE_SERVER_RENDER_FAILED` created: (1) **coexistence** — projection-fetch vs render-throw failures now occupy **separate** `(show_id, code)` unresolved rows (`admin_alerts_one_unresolved_idx`), so neither clobbers the other's `context`; (2) **aggregation** — one bridge upsert with `failedKeys` keeps every failed domain (its row coalesces only with itself); (3) **idempotency** (below). Best-effort, Supabase-call-boundary discipline; the **sole** source of projection-`tileErrors` alerts (no double-upsert with the visual fallback below).
  - **Idempotent per failure signature (reliable — its own row).** Section navigation re-renders `CrewShell` (server `router.push`), so a naive upsert would bump `occurrence_count` once per tab tap. The bridge reads **its own** unresolved `TILE_PROJECTION_FETCH_FAILED` row (best-effort, `{ data, error }` destructured) and **skips the upsert when the stored `context.signature` equals the current `{ viewerVersionToken, sorted failedKeys }`**, writing a fresh signature only on change. Because **no other writer touches this code's row** (render throws use the separate `TILE_SERVER_RENDER_FAILED`), the signature is never clobbered — so `occurrence_count` tracks distinct failure occurrences, not tab visits. (The dedup read is a new Supabase read boundary — registered in the infra meta-test, fail-quiet: a read fault falls through to the upsert.)
...
No raw error codes in crew UI (invariant 5). **One new §12.4 admin-alert code — `TILE_PROJECTION_FETCH_FAILED`** (the bridge, §4.13) — lands the three-lockstep in one commit (master-spec §12.4 prose at `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` + `pnpm gen:spec-codes` + a `lib/messages/catalog.ts` row with an admin-facing `helpfulContext` / `<sheet-name>` copy), passes the `x1-catalog-parity` gate, and adds a `_metaAdminAlertCatalog` producer row (§9). **No DB migration** — `admin_alerts.code` is unconstrained `text`.

---

## 6. DB / migration matrices

**No table migration.** The only data addition is `dates.loadIn` written into the existing `shows.dates` **jsonb** column (`migrations/20260501000000_initial_public_schema.sql:12`) — jsonb is schemaless, so no DDL, no CHECK/enum change, no RLS/REVOKE change. Tier×domain, CHECK/enum migration, and apply-twice matrices are **N/A — declared**. The one new admin-alert **code** (`TILE_PROJECTION_FETCH_FAILED`, §4.13) is **catalog-level, not a DB change** — `admin_alerts.code` is unconstrained `text` (no enum/CHECK), so it lands via the §12.4 three-lockstep (§5), not a migration.
...
24. **`?s=` deep-link preserved through auth/picker redirects** (§4.1). **The bridge uses a distinct code `TILE_PROJECTION_FETCH_FAILED`** (§4.13), separate from render-throw `TILE_SERVER_RENDER_FAILED` — separate `(show_id, code)` rows resolve coexistence (no context clobber), aggregation (`failedKeys`), and idempotency (own-row `{viewerVersionToken, sorted failedKeys}` signature dedup; section re-renders do not inflate `occurrence_count`). New code = §12.4 three-lockstep, **no migration** (§5).
...
- **EXTEND `tests/messages/_metaAdminAlertCatalog.test.ts` + the §12.4 catalog** — register the **new** code `TILE_PROJECTION_FETCH_FAILED` (catalog completeness + the bridge as its **sole** producer, validating context keys `sheet_name`/`tileId`/`message`/`failedKeys`/`signature`). The §12.4 three-lockstep (master-spec prose + `gen:spec-codes` + `catalog.ts`, §5) lands in the same commit; `x1-catalog-parity` enforces it. The bridge **writes** via the existing `upsertAdminAlert` helper (a caller, boundary-compliant); its **idempotency read** (the dedup `SELECT` on `admin_alerts`, §4.13) IS a new Supabase read boundary → register it in the infra-contract meta-test (`tests/auth/_metaInfraContract.test.ts` or the analogous registry) with a `{ data, error }` destructure + a typed **fail-quiet** infra-fault result (read fault → fall through to the upsert; never crash the render).
- **Not touched (declared):** advisory-lock, DML-lockdown, sentinel registries beyond the `components/crew/` extension above — no new locks; **no table DDL** (the parser `dates.loadIn` lands in the existing `dates` jsonb, §6; the new alert code is catalog-level text, no migration).
