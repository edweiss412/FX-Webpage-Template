...
4. **Layout/transition/screenshots + close-out** — Playwright dimensions + nav + transition-audit; manifest entries + baselines (pinned docker); **tile migration (file-by-file, not a blanket delete — wp-20):** (a) KEEP/reuse the non-View modules — `components/tiles/OpeningReelVideo.tsx` (media player, reused by Gear's reel block), the `lib/` `load*` data helpers, and the `components/shared/` error infra; (b) migrate the tile test suite — tests targeting deleted `*Tile.tsx` View shells are replaced by the new section/primitive tests, tests for reused modules retargeted; (c) **only then** delete the obsolete `*Tile.tsx` View shells, `_ShowBody.tsx`, and `selectTodayTiles` once nothing imports them; impeccable dual-gate; adversarial review; real-CI; merge.
