...
- **`tileErrors` observability is section-independent (the alert bridge).** `getShowForViewer` (`:335-505`) only *populates* `tileErrors[key]`. Today the persistent `admin_alerts` row is guaranteed by the **always-rendered** `notes-tile` catch-all (`_ShowBody.tsx:408-419`), which re-throws on `hotel`/`rooms`/`contacts` **ungated** (+ `transportation` gated) through `TileServerFallback`. Because `CrewShell` renders only the **active** section, that guarantee would be lost — a `hotel`/`rooms`/`contacts` failure whose block sits in an unopened section would never alert. So `CrewShell` mounts a **section-independent `<TileErrorAlertBridge>`** (always rendered; renders nothing) that performs a **single** `TILE_SERVER_RENDER_FAILED` upsert via the shared `upsertAdminAlert(...)` helper, with the **same context contract** as `TileServerFallback` — `sheet_name` (=`show.title`), `tileId` (=`crew:alert-bridge`), `message` — **plus** `failedKeys` = the list of gated-visible failing keys. (It is a second registered producer of this code — §9 extends `_metaAdminAlertCatalog`.) Which keys count, per the effective current contract:
  ...
  **One** upsert, not per-key: `admin_alerts` coalesces on `(coalesce(show_id::text,''), code) WHERE resolved_at IS NULL` (`admin_alerts_one_unresolved_idx`, `supabase/migrations/20260501001000_internal_and_admin.sql:279-280`) and **replaces context on conflict**, so separate per-key upserts would collapse into a single last-writer-wins row that loses the other domains. The aggregated `failedKeys` payload keeps every failed domain in the one durable row. Best-effort, Supabase-call-boundary discipline; the **sole** source of projection-`tileErrors` alerts (no double-upsert with the visual fallback below).
  - **Idempotent per failure signature.** Section navigation re-renders `CrewShell` (server `router.push`), so a naive upsert would bump `occurrence_count` once per tab tap for the same unchanged failure. The bridge is keyed on `signature = { viewerVersionToken, failedKeys (sorted) }`: it first reads the show's current unresolved `TILE_SERVER_RENDER_FAILED` row (best-effort, `{ data, error }` destructured per the call-boundary discipline) and **skips the upsert when the stored `context.signature` already equals the current signature**, writing (and recording the new `signature`) only when it changes. `occurrence_count` then tracks distinct failure occurrences (data-version changes), not tab visits. A benign race under concurrent renders costs at most one extra increment. (This read makes the bridge a Supabase read boundary too — registered in the infra-contract meta-test, §9.)
...
- **Coexistence with render-throw alerts (coalescing is by design).** Both the bridge (projection `tileErrors`) and per-block `TileServerFallback` (render throws) write `TILE_SERVER_RENDER_FAILED`, which coalesces by `(show_id, code)` (`admin_alerts_one_unresolved_idx`). When they co-occur in one render: one unresolved row, `occurrence_count` = total upserts, `sheet_name` (the §12.4 interpolation key) always present, most-recent context wins. This matches the current multi-writer coalescing — the alert's purpose ("this show has tile failures") is served. The bridge's `failedKeys` enriches the projection case; richer **simultaneous** diagnostic capture (both `failedKeys` AND the thrown `tileId` in one row) would need an RPC context-merge or a distinct code — deferred (`BACKLOG.md` BL-TILE-ALERT-CONTEXT-MERGE).
...
24. **`?s=` deep-link preserved through auth/picker redirects** (§4.1), and **`TILE_SERVER_RENDER_FAILED` coalescing is intentional** (§4.13) — co-occurring projection + render failures collapse to one show-scoped row (`occurrence_count` + `sheet_name` preserved); richer simultaneous diagnostic context is BACKLOG (`BL-TILE-ALERT-CONTEXT-MERGE`), not a Phase-1 gap. The bridge is **idempotent per `{viewerVersionToken, sorted failedKeys}` signature** (§4.13) — section re-renders do not inflate `occurrence_count`.
...
29. **Mixed alert coalescing** (§4.13) — a projection `tileErrors` (bridge) AND a wrapped-block render throw in one render → exactly **one** unresolved `TILE_SERVER_RENDER_FAILED` row with `occurrence_count ≥ 2` and `sheet_name` present (interpolation intact). _Catches: a co-occurring incident raising zero alerts or losing the show-level signal._
...
- **EXTEND `tests/messages/_metaAdminAlertCatalog.test.ts`** — `TileErrorAlertBridge` is a **second** `TILE_SERVER_RENDER_FAILED` producer (§4.13). The registry currently maps the code to one path (`:252-254`, `TileServerFallback.tsx`); add the bridge as an additional registered write site and assert it supplies the required context keys (`sheet_name`/`tileId`/`message`, + `failedKeys`). The bridge **writes** via the existing `upsertAdminAlert` helper (already boundary-compliant — a caller, not a new boundary). Its **idempotency read** (the dedup `SELECT` on `admin_alerts`, §4.13) IS a new Supabase read boundary → register it in the infra-contract meta-test (`tests/auth/_metaInfraContract.test.ts` or the analogous registry) with a `{ data, error }` destructure + a typed infra-fault result that is **fail-quiet** (a read fault → fall through to the upsert; never crash the render).
- **Not touched (declared):** advisory-lock, DML-lockdown, sentinel registries beyond the `components/crew/` extension above — no new locks; **no table DDL** (the parser `dates.loadIn` lands in the existing `dates` jsonb, §6); **no new alert _code_** (reuses `TILE_SERVER_RENDER_FAILED`, only a second producer).
