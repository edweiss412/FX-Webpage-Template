...
- **`tileErrors` → re-throw contract (ported verbatim from `_ShowBody`).** `getShowForViewer` (`:335-505`) only *populates* `tileErrors[key]`; the persistent `admin_alerts` row comes from each block's `load()` **re-throwing** on its key (gated), so the throw is caught by that block's `WrappedTile`/`TileServerFallback` → `TILE_SERVER_RENDER_FAILED` upsert + fallback. The fallback IS what the viewer sees — **not** a hand-rolled degraded block. The per-key gates are **ported, not re-derived**:
  - `hotel` → re-throw iff `isAdmin` (`_ShowBody.tsx:189`) — Travel + Today "Tonight"
  - `contacts` → re-throw **ungated**, all viewers (`:224`) — Crew + Today "Need something"
  - `rooms` → re-throw iff the scope/anchor block is shown. The redesign shows scope to **all** (§4.5), so **any** viewer of the Gear scope block OR the Today/Schedule **KeyTimesStrip** gets the throw (broadened from the old A/V/L-only gate `:255-292` — the time anchors now depend on rooms, so a rooms fetch error stays **observable** instead of silently collapsing into "no times")
  - `transportation` → re-throw iff `isAdmin || transportTileVisible(...)` (`:309`, `:416`) — Travel + Venue parking
  - `financials` → re-throw iff `financialsVisible(flags, isAdmin)` (`:373`) — Budget
  Where the gate is false, the block silently omits (genuine absence).
...
17. **`CrewShell` ports `_ShowBody`'s cross-cutting contracts verbatim — not re-derived.** Three contracts live in `_ShowBody.tsx` today and must be copied line-for-line into `CrewShell`/the section blocks before `_ShowBody` is deleted (§10): (a) the per-key `tileErrors` re-throw gates (`:189` hotel/`isAdmin`, `:224` contacts/ungated, `:255-292` rooms, `:309`/`:416` transportation, `:373` financials — §4.13); (b) the Footer report props per viewer kind (`:509-535` — §4.1); (c) every visibility gate (wp-16). Paraphrasing any of these is where a contract silently drops — reviewers verify the ports are exact, and tests 16/17/18/19 pin each one.
...
16. **Section error containment** (§4.13) — for **each** `tileErrors` key (`rooms`/`contacts`/`hotel`/`transportation`/`financials`) with its per-key gate satisfied: the block **re-throws through `TileServerFallback`** → the `admin_alerts` `TILE_SERVER_RENDER_FAILED` upsert is attempted (mocked, asserted **per key** — not only an injected render throw) and the fallback renders; with the gate **not** satisfied → silent omission, **no upsert**. The **error** path is distinguishable from the **no-data** path (assert both). The `rooms` re-throw now fires for any Gear/KeyTimesStrip viewer (scope shown to all). Plus a render-throw injected into one wrapped block → `TileErrorBoundary` catches it. _Catches: a section rendering a degraded block without routing through the upsert (data-source failures visible only to whoever opens the section, dropping the persistent admin alert); rooms errors silently collapsing into "no times."_
