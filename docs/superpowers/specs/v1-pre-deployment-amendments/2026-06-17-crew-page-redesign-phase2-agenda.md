...
2. **Walk the day-TYPE banner (row 2) left→right.** Each non-empty cell **opens a day block**; the block's column span = `[this column, next non-empty banner column)`. `TRAVEL`/`SET` → skip. `DAY N` → a 6-col show block `[START, FINISH, TRT, TITLE, ROOM, AV]` (TITLE = block-start + 3).
