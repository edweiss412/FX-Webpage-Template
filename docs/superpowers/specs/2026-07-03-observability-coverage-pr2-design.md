...
- `finalize-cas/route.ts` `applyShadow`: identically, the retained/blocked rows carry their code in the collected `shadowResults`/`per_row` set; emit each `code !== "OK"` as **`log.warn`** at the finalize-cas POST-COMMIT flush (after its committing wrapper resolves) — NOT at the shadow-validation return sites (which are pre-commit).
...
- **Fail-open (mandatory for every S3 catch-side emission):** each new `log.error` inside an existing `catch` (or `logAdminOutcome` outside the tx) is best-effort — wrap as `try { await log.error(...) } catch {}` so a logger fault can NEVER replace the route's original error response/status. The success `logAdminOutcome` calls run after the committing wrapper and are similarly wrapped where the route lacks an outer try.
...
- **Fail-open (mandatory, all surfaces):** every NEW emission that could throw — `logAdminOutcome` and every catch-side `log.error`/`log.warn` — is best-effort: either already inside the route's outer try, or wrapped `try { await log.* } catch {}`. A logging fault must NEVER change a route's control flow, response body, or status code. This is the operative guarantee behind the "pure add, no behavior change" claim.
