...
- **Abuse guards (the endpoint persists rows the admin timeline reads, so it cannot be a wide-open write):**
  - **Same-origin only.** Reject (**403**, no write) unless `Sec-Fetch-Site === "same-origin"` OR (for browsers that omit `Sec-Fetch-Site`) the `Origin` header `=== NEXT_PUBLIC_SITE_ORIGIN`. NOT `same-site` (a sibling sub-domain should not write our timeline). This blocks the dominant real-world vector — a **drive-by cross-site request from a victim's browser** (the browser stamps `Sec-Fetch-Site: cross-site`, which we reject). It does NOT stop a determined non-browser client that hand-crafts/spoofs `Origin`/`Sec-Fetch-Site` — that is part of the accepted residual below.
  - **Content-type** must be `application/json` (else 400).
  - Size caps + the per-instance rate backstop (below).
  - **Accepted residual (documented, not a gap):** a determined attacker who spoofs the `Origin`/`Sec-Fetch-Site` headers (trivial outside a browser) OR scripts a real same-origin browser context can still inject a BOUNDED number of clearly-labeled `client.*` rows. This is low-impact: rows are capped, plainly sourced `client.*` (visually distinct on the technical diagnostics page), grant NO read access, and cannot touch crew/sync/admin state — NOT a privilege escalation. We accept it rather than gate crew error-reporting behind auth (which would lose the errors we most want — anonymous crew crashes). The same-origin check is a casual-abuse + drive-by filter, not an authentication boundary.
...
9. **boundary effects** — render each boundary with a thrown error ⇒ `Sentry.captureException` mock called once AND `reportClientError` mock called once with the right `area`; the fallback renders `getRequiredCrewFacing("PAGE_RENDER_FAILED")` (crew/global — `crewFacing` copy) or `getRequiredDougFacing("ADMIN_ROUTE_LOAD_FAILED")` (admin). Catches: a boundary that captures but doesn't mirror (or vice-versa), renders a raw code, OR reads the wrong audience field (Doug-facing is `null` for `PAGE_RENDER_FAILED` ⇒ `getRequiredDougFacing` would throw).
