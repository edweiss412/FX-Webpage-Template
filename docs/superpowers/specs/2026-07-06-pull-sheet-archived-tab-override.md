...
| D5 | Fingerprint | SHA-256 hex over the **entire reviewed block**: every cell that contributes to the rendered `headerPreview` (the show-identifying header lines the admin reads) **plus** the parsed pull-sheet item-bearing rows — normalized (whitespace/blank-row collapsed so cosmetic reformat is stable), computed in the exporter layer where the xlsx bytes are available. Covering the header (not just item rows) is mandatory: the admin's decision is "does this header belong to *this* show" — a header-only change (e.g. re-heading a prior show's gear) MUST change the fingerprint so accept is rejected / an active override auto-drops (Codex R5). |
...
- For every worksheet dropped by the `\bOLD\b` rule, before dropping, detect a **parseable pull-sheet block** — NOT a mere substring mention (Codex R3 finding 2). Run the same normalization + block split the inclusion path uses (`normalizePullSheetGrid` — name-gated `/PULL SHEET/i`, which "OLD PULL SHEET" matches — then `splitBlocks`), and require a block whose **header row satisfies the parser predicate: every cell contains `"PULL SHEET"`** (`pull-sheet.ts:60` `cells.every(c => c.toUpperCase().includes("PULL SHEET"))`) — the same all-cells test, not any-cell. Only then record an `ArchivedPullSheetTab`, with `headerPreview` (that block's first non-blank up-to-4 lines, `&#10;`/newlines flattened to `" / "`, capped 120 chars) and `fingerprint` computed over **the header-preview cells AND that block's item-bearing rows** (D5) — so anything the admin reads in S2/S4 is under the pin. A stale notes/contact/room OLD tab that merely mentions "pull sheet" in one cell produces **no** `ArchivedPullSheetTab`, no warning, no offer. Still drop the tab from `markdown` (unless it is the accepted tab — see next).
