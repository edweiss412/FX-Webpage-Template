...
| D5 | Fingerprint | SHA-256 hex over **all pull-sheet blocks that inclusion would emit from the tab** (Codex R6) — each block's header/preview cells AND its item-bearing rows, concatenated in stable order — normalized (whitespace/blank-row collapsed so cosmetic reformat is stable), computed in the exporter where the xlsx bytes are available. Two coverage mandates: (a) the header cells, not just item rows, are hashed so a header-only re-heading changes the fingerprint (Codex R5); (b) *every* emitted block, not just the first, is hashed so a change to any case in a multi-block tab changes it (Codex R6). Fingerprint unit ≡ emitted unit ≡ reviewed unit. |
| D6 | Un-skip granularity | The exporter emits **exactly the collected pull-sheet blocks** (the same set the fingerprint covers) from an accepted tab; all other blocks from it are discarded. Time-anchor path (`showDayTimeAnchors.ts`) and room/other-block emission stay unconditionally OLD-skipping. |
...
| D10 | Staged consistency | `pending_syncs.pull_sheet_override_applied jsonb` stores `overrideSnapshot(...)` = `{ tabName, fingerprint } | null` the staged parse was produced under; finalize refuses when it ≠ `overrideSnapshot(current pull_sheet_override)` (audit fields excluded from the compare, 5.8). Onboarding-only, not on `shows`. |
...
- `ArchivedPullSheetTab = { tabName: string; headerPreview: string; fingerprint: string; included: boolean }`.
- **Detection is independent of the include/drop decision and runs for EVERY OLD tab — including the accepted one** (Codex R6 finding 2). For each worksheet matched by the `\bOLD\b` rule, run the same normalization + block split the inclusion path uses (`normalizePullSheetGrid` — name-gated `/PULL SHEET/i`, which "OLD PULL SHEET" matches — then `splitBlocks`) and collect **every** block whose **header row satisfies the parser predicate: all cells contain `"PULL SHEET"`** (`pull-sheet.ts:60` `cells.every(c => c.toUpperCase().includes("PULL SHEET"))`) — the all-cells test, not any-cell. If ≥1 such block exists, record one `ArchivedPullSheetTab` for the tab. A stale notes/contact/room OLD tab that merely mentions "pull sheet" in one cell yields **no** block → no entry, no warning, no offer.
- **Fingerprint unit == emitted unit** (Codex R6 finding 1): the `fingerprint` is computed over **every pull-sheet block that inclusion would emit from this tab** — all such blocks' header/preview cells AND their item-bearing rows, concatenated in stable order (D5). One offer per tab, but the pin covers *all* cases/blocks emitted, so a change to any (even a non-first) block changes the fingerprint. `headerPreview` is the first block's up-to-4 header lines (`&#10;`/newlines flattened to `" / "`, capped 120 chars) — representative for display; the fingerprint is the authority.
...
- **Emission:** when `opts.includePullSheetFromTab === sheetName` for an OLD tab, that tab is **not** dropped, and exactly the collected pull-sheet blocks (the same set the fingerprint covers) are emitted; every non-pull-sheet block on it is discarded (D6). All other OLD tabs are dropped from `markdown` entirely.
...
For the `included: true` entry (the accepted tab), the sync layer compares its **returned current fingerprint** to the stored override's fingerprint: **match** → no warning; the pull sheet stays included (5.1). **Mismatch** (content changed, incl. header-only, 5.1/D5) → clear the override (write `null`), emit `PULL_SHEET_OVERRIDE_CONTENT_CHANGED` (forensic), and — because the include was based on a now-stale override — re-emit as an `included: false` offer (`PULL_SHEET_ON_ARCHIVED_TAB` + S4 re-confirm). The accepted tab therefore always returns comparable fingerprint metadata; the auto-drop path can never be defeated by the tab being un-dropped.
...
6. **Content-change auto-drop (incl. header-only)**: override set, tab item rows OR header-preview cells changed → override cleared, `PULL_SHEET_OVERRIDE_CONTENT_CHANGED` emitted, `PULL_SHEET_ON_ARCHIVED_TAB` re-fired, `pr.pullSheet` empty. Explicit header-only-change case (Codex R5). Catches: stale/re-headed gear silently publishing.
