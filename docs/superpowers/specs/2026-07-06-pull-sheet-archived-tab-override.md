...
- **Scope — why the deferred-apply gate is onboarding-only** (Codex R10 finding 1, premise-corrected): the ONLY stage-then-apply-later path is the **wizard** `pending_syncs` flow (Flow A + shadow Flow B). **Live-cron `pending_syncs` staging was retired (PF31, `supabase/migrations/20260608000004_retire_live_pending_syncs.sql`)** — the cron decision rule now **auto-applies inline under `hashtext('show:'||drive_file_id)` and NEVER inserts a live `pending_sync` (`wizard_session_id IS NULL`)**. So cron has no staged-parse-applied-later gap: it reads `shows.pull_sheet_override` pre-lock, re-reads under the lock (5.7), discards-and-reruns on content drift (5.2), and applies in the same locked tx. A **held** sync retains last-good and re-parses fresh on the next run (no stale deferred apply). The consistency gate is therefore path-general in principle (I4/I5) but instantiated only where deferred apply exists — the wizard flow. Any future path that reintroduces stage-then-apply MUST carry `pull_sheet_override_applied` and gate against its desired override.
...
- **I5 — No stale parse is ever staged OR inline-applied.** Two guards, both under the `show:` lock: (a) override-**row** drift vs the pre-lock snapshot → refuse-and-retry (5.7); (b) sheet-**content** drift under an unchanged override (`included:true` fingerprint mismatch) → discard-and-rerun, clear override, empty `pullSheet` with `applied=null` (5.2). Holds for the wizard deferred-apply flow (via the 5.8 gate) AND the cron inline-apply flow (5.7+5.2 in the same locked tx; no live `pending_syncs` staging exists — PF31). A changed OLD tab never yields a staged/published `pullSheet`.
