...
- **Fingerprint unit == emitted unit == reviewed unit** (Codex R6 finding 1 + R7 finding 1): `fingerprint` is computed over **every case region inclusion would emit from this tab** — each region's header/preview cells AND its `collectDataBlock` item rows, concatenated in stable order (D5). One offer per tab; the pin covers *all* cases (even non-first), so a change to any case's header or items changes the fingerprint. `headerPreview` is the first region's up-to-4 header lines (`&#10;`/newlines flattened to `" / "`, capped 120 chars) — representative for display; the fingerprint is authority.
...
- **Flow B — existing (already-live) show shadow path** (`app/api/admin/onboarding/finalize/route.ts` stages approved existing shows into `shows_pending_changes.payload` and deletes the `pending_syncs` row, then `finalize-cas` / `applyRescanDecisionUnderLock` applies the shadow via `parseShadowPayloadForApply`, `lib/onboarding/shadowPayload.ts:75`): because the `pending_syncs` row (and its `pull_sheet_override` + `pull_sheet_override_applied` columns) is **deleted** at staging, BOTH values MUST ride the shadow payload (Codex R7 finding 2). Specifically: (a) write `pull_sheet_override` AND `pull_sheet_override_applied` into `shows_pending_changes.payload` when the existing show is staged, (b) surface both via `parseShadowPayloadForApply` (add to `ParsedShadowPayloadForApply`), (c) at Phase-D apply, under the `show:` lock, run the 5.8 consistency gate using the **payload-carried** `pull_sheet_override_applied` vs the current durable override, and on match write `pull_sheet_override` to `shows`. Without the payload-carried applied snapshot, Flow B either skips the gate (stale gear publishes for a live show) or can never finalize (snapshot missing).
...
- **Finalize/apply gate** (under the `show:` lock): refuse to finalize a row whose applied snapshot ≠ `overrideSnapshot(current pull_sheet_override)`. **Flow A** reads `pull_sheet_override_applied` from the live `pending_syncs` row; **Flow B** reads it from the shadow payload (`shows_pending_changes.payload`), since the `pending_syncs` row is deleted at staging (5.5, Codex R7). Typed blocking outcome ("re-scan needed before publishing"), surfaced via lookup copy — never a silent apply of mismatched gear.
...
- **Staged parse can never publish out of sync with the override** (5.8) — `pull_sheet_override_applied` is compared to the current override at finalize under lock; divergence (incl. after a failed re-scan) refuses finalize rather than applying mismatched gear. Declarative gate, no compensation write. Flow B carries the applied snapshot in the shadow payload so the gate survives `pending_syncs` deletion (5.5/5.8).
...
8d. **Staged↔override consistency on failed re-scan (both flows)**: (a) Flow A accepted gear staged → revoke → re-scan fails → finalize REFUSED (row unfinalizable, not applying archived gear); (b) accept → re-scan fails → override set but staged parse lacks gear → finalize REFUSED until a successful re-scan reconverges `pull_sheet_override_applied`; (c) Flow B (existing live show): the gate uses the shadow-payload-carried applied snapshot after `pending_syncs` is deleted — a revoke-then-failed-rescan still REFUSES finalize (Codex R7-2). Catches: mixed-row publish across failed re-scan (Codex R2-2) on both flows.
