...
**Excluded scenarios are not rendered at all `[R2-3]`** — `isModalExpressible` (§3.2) filters `T2_SECTION_ABSENT`, `T2_OVERVIEW_ABSENT`, `T2_CREW_ROW_ABSENT` out of the switcher, so no misleading item ever appears. They are enumerated in a `SwitcherControls` footnote ("card-only structural probes, not modal-expressible: …") and the handoff, and the excluded set is pinned by a test — the boundary is visible, not hidden, and cannot silently drift.
...
- **NEW:** `attentionModalSwitcher.test.tsx (new)`, `attentionModalGallery.serverProps.test.ts (new)` (server passes only data; `structuredClone` per scenario; `isModalExpressible` excluded-set pin), `attentionModalFixture.test.ts (new)` (fixture shape + no functions), `tests/e2e/attention-modal-gallery.spec.ts (new)`.
