...
- Properties: `SECURITY INVOKER`; performs no writes (`STABLE`); acquires no advisory locks (invariant 2 untouched — single-holder rule has no new holder); `REVOKE EXECUTE FROM anon, authenticated` (called only by the admin server client; PostgREST surface stays closed, consistent with the DML-lockdown posture).
