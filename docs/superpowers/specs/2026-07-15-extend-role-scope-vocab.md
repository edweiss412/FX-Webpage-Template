...
- Stale provenance (`code: "stale"`, §8.3 — another admin mapped it, or a re-parse resolved the warning between load and submit; Codex R6 F2): distinct benign notice, NOT error styling and never the raw code: **"Looks like this was already taken care of. The page will show the latest next time it loads."** Tested as its own component branch.
...
**New code `ROLE_TOKEN_MAPPED`** (info-level app_event, emitted post-commit). Event context carries `{ token, grants, crewName }` from the overlay's `applied[]` entry (§6 — `memberName` maps to `crewName`) so the catalog's `<crew-name>`/`<token>` placeholders are renderable per event (Codex R3 F3); no other PII.

**Delta-gated emission (Codex R6 F3 — steady state must be silent; inputs pinned R7 F1):** the overlay re-applies on EVERY sync (flags are recomputed), so raw per-application emission would flood app_events on every cron pass of a stable show. Emission is gated on observable change. **Gate inputs are exclusively PRIOR-persisted state, never this parse's pre-overlay output** (a fresh parse always re-emits the warning pre-overlay, so using it would emit every sync — the exact failure the gate exists to prevent): (1) the member's prior persisted `role_flags` (the same prior-crew source `nonLeadRoleFlagChanges` diffs against, `phase2.ts:218`) and (2) the prior PERSISTED `parse_warnings` (previous `shows_internal` state on the cron/manual paths; the prior staged row on the wizard path; absent prior state ⇒ everything counts as new ⇒ emit). Emit for an `applied[]` entry only when (a) `grants` non-empty AND at least one granted flag is newly present vs input (1), or (b) `grants` empty (recognize-only) AND input (2) still contained the matching `(UNKNOWN_ROLE_TOKEN, roleToken)` entry for that member — the suppression is new. `phase2` therefore receives the prior persisted warnings alongside its existing prior-crew arg; the overlay itself stays pure and gate-free. A steady-state re-sync emits ZERO `ROLE_TOKEN_MAPPED` events. Test (§13 telemetry-carrier test extended): two consecutive syncs with an unchanged sheet — first emits per mapping (including a recognize-only mapping), second emits none of either kind.
