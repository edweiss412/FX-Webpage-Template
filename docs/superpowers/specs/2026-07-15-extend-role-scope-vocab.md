...
**Delta-gated emission (Codex R6 F3 — steady state must be silent):** the overlay re-applies on EVERY sync (flags are recomputed), so raw per-application emission would flood app_events on every cron pass of a stable show. Emission is therefore gated on observable change, computed at the phase2 seam where prior state is already available (prior crew for `nonLeadRoleFlagChanges`; prior persisted warnings): emit for an `applied[]` entry only when (a) `grants` non-empty AND at least one granted flag is newly present vs that member's PRIOR persisted `role_flags`, or (b) `grants` empty (recognize-only) AND the prior persisted warnings still contained the matching `(UNKNOWN_ROLE_TOKEN, roleToken)` entry for that member — i.e., the suppression is new. A steady-state re-sync emits ZERO `ROLE_TOKEN_MAPPED` events. Test (§13 telemetry-carrier test extended): two consecutive syncs with an unchanged sheet — first emits per mapping, second emits none.
