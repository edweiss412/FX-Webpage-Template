...
Result contract for `mapRoleToken` (live), exact union (Codex R8 F3): `{ ok: false, code: "stale" | "infra_error" | "show_not_found" | "validation_error" }` when nothing durable happened — and the component MUST branch `code: "stale"` to the §9 benign stale notice, never the generic error copy (own tested branch) · `{ ok: true, state: "applied" }` when the follow-up `runManualSyncForShow` reports `outcome === "applied"` · `{ ok: true, state: "apply_pending" }` when the mapping committed but the re-sync failed, threw (caught), or returned any non-applied outcome (§7; mirrors `_actions/useRaw.ts:155-170`). `mapRoleTokenStaged` has the same shape with the re-stage in place of the re-sync. Settings-page `update`/`delete` return plain ok/error — no attached re-sync, convergence is cron-driven (§7).
...
1. **Overlay** (§6, pure, gate-free): returns `applied[]` with `{ token, grants, memberIndex, memberName }`. `memberName` exists for the gate and change-feed correlation only — it never reaches telemetry.
2. **Gate (computed inside `phase2`):** emission-worthiness is decided per applied entry against PRIOR-PERSISTED state only, never this parse's pre-overlay output (a fresh parse always re-emits the warning pre-overlay; using it would emit every sync): (a) `grants` non-empty AND ≥1 granted flag newly present vs the member's prior persisted `role_flags` (same prior-crew source `nonLeadRoleFlagChanges` diffs, `phase2.ts:218`); or (b) `grants` empty (recognize-only) AND the prior persisted `parse_warnings` still contained the matching `(UNKNOWN_ROLE_TOKEN, roleToken)` for that member — the suppression is new. Absent prior state ⇒ everything is new ⇒ emit.
3. **Args:** `Phase2Args` gains TWO explicit fields: `roleTokenMappings?: RoleTokenMapping[]` (§6.2) and `priorParseWarnings?: ParseWarning[]`. Provenance per caller: cron + manual paths → previous `shows_internal.parse_warnings` (read where they already read prior show state, e.g. `runScheduledCronSync.ts:920` region); staged-apply/wizard path → the prior staged row's parse warnings (`applyStaged.ts:530` region). A caller that omits the field gets emit-everything-new semantics — acceptable only where no prior exists (first publish).
4. **Result:** `Phase2Result.appliedRoleMappings` carries POST-GATE, emission-ready entries only, shaped `{ token, grants, newMemberCount }` — **no member names (Codex R8 F2: crew names are PII; they already surface with names in the changes feed via `ROLE_FLAGS_NOTICE`, telemetry does not need them)**. Always present, `[]` when nothing gate-passing.
