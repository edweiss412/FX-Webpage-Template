...
> Build the graph whose nodes are **declarations** — functions, variables, parameters, class members — and whose edges are **references**. A test is environment-touching if any node reachable from its body's references contains a declared provenance anywhere in its own initializer or body. Cycle-safe fixed point.
...
**The vector is fenced in both directions from here.** A further finding on it is admissible only if it names a syntactic position where a declared provenance can appear that is **not** inside a declaration reachable through the reference graph. That is a closable challenge rather than an open enumeration, and it is the convergence criterion for this section. Conversely, if such a position exists, the rule above is wrong at its root and needs replacing, not extending.
...
**Consequence bound**, stated as what the analysis can prove rather than as what would be nice. Every test in an enrolled suite reaches one of exactly four states, and there is no fifth: it carries a premise; it carries an explicit exemption with a non-empty reason; it is classified environment-free by a **fixed point over the declaration-reference graph** (§3.3.2.1); or it is **reported as unclassifiable and reds the run**. Nothing within the repository graph is silently unclassified — including a spawn or a `process.env` read moved behind a local helper at any depth, or into a module-scope constant, a default parameter, or a class field. Those are the ordinary refactors the first three drafts of this bound let through one at a time, which is why the rule now keys on declarations and references rather than on a list of positions.
