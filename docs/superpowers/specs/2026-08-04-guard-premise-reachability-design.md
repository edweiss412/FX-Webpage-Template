...
#### 3.3.2.3 Every executable fixture, in one table

Spec R9 established that an executable fixture must name its file and its execution owner, and spec R12 found the rule stated in §3.1 and then not applied to the fixtures §3.3.2.2 demands — the same class-sweep failure as R11, on the rule R9 created. So the whole set lives here, in one place, rather than being described where each is introduced.

**An executable premise fixture must FAIL**, which is what makes it different from every other fixture in this spec: it cannot run as an ordinary test, because a suite containing a deliberately failing test is red. Each is therefore invoked as a **child vitest run**, and its non-zero exit is the assertion — the same shape `runSuite` already uses (`tests/mutation/source/runner.ts`).

| fixture | file, all under `tests/mutation/source/fixtures/ (new)` | owner | asserts | cadence |
| --- | --- | --- | --- | --- |
| `@/` alias resolves | `aliasImport.fixture.ts (new)` | `_metaOverlayConfigParity.test.ts (new)` | child run exits **0** | merge |
| `testTimeout` past 5 000 ms | `slowTest.fixture.ts (new)` | `guardSurfaces.gate.test.ts` | child run exits **0** | nightly (~5.2 s) |
| empty `it.each` producer beside a passing test | `emptyItEach.fixture.ts (new)` | `_metaPremiseContract.test.ts (new)` | child run exits **non-zero** | merge |
| empty `test.each` producer beside a passing test | `emptyTestEach.fixture.ts (new)` | same | child run exits **non-zero** | merge |
| empty `describe.each` producer beside a passing test | `emptyDescribeEach.fixture.ts (new)` | same | child run exits **non-zero** | merge |
| associated placement executes on an empty producer | `associatedPlacement.fixture.ts (new)` | same | child run exits **non-zero** | merge |

Every file is named `*.fixture.ts`, which `BASE_INCLUDE` (`vitest.projects.ts:34`) cannot match, so none is discovered by any default project and none can red the merge suite by existing. The wiring assertion of AC-3a covers all six: the default projects resolve **zero** files under the fixtures directory, and each owner executes the fixtures it claims. Without that second half, a fixture whose owner silently stopped invoking it is dark — which is this spec's subject, at the level of this spec's own tests.

The purely **static** fixtures — the provenance matrix of §3.3.2.1 and the placement rejections of AC-8d — are synthetic source strings parsed by the checker, never executed, so they need no runner and appear in no row above.
...
| All six executable fixtures are enumerated in ONE table with file, owner, expected exit and cadence (§3.3.2.3) | spec R12, the same class-sweep failure as R11 and on the rule R9 created; a fixture that must FAIL cannot be a discovered test, so its runner has to be named | an executable fixture in this spec absent from that table |
...
- **AC-3a** Every executable fixture names its file and its execution owner, and **all six are enumerated in one table** (§3.3.2.3) rather than described where each is introduced — R12 found the rule stated in §3.1 and not applied to §3.3.2.2's fixtures. A wiring meta-test proves the default vitest projects resolve **zero** files under `tests/mutation/source/fixtures/`, that each owner executes the fixtures it claims, and that `pnpm mutation:guards` executes the slow one. Without the second clause a fixture whose owner stopped invoking it is dark, which is this spec's subject applied to its own tests.
...
- **AC-8c** **Executable, not static.** For **each** construct in AC-8b, the named fixture of §3.3.2.3 — a producer returning `[]` beside a passing test — is invoked as a child vitest run by the premise meta-test of §3.3.2.3 and must exit **non-zero**, including the `describe.each` case, where zero registrations means zero nested tests. A deliberately failing fixture cannot be an ordinary discovered test, which is why the runner is named. Probed at spec R8 that `test.each([])` currently passes green — zero cases registered, callback never run — so this criterion is met by running it, never by classification.
