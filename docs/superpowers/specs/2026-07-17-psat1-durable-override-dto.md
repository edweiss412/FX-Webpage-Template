...
| F3 | `components/admin/showpage/PublishedReviewPage.tsx:188` renders `<ShowReviewSurface layout="page">` | Omit the prop → default `false` (published never shows S5). |
...
**Why this never collides with S4 (content-changed).** The content-changed / tab-missing path is driven by `discardAndRerun` (`lib/sync/pullSheetOverride.ts:164`), which invokes an injected `clearOverride` callback (`:173`) AND flags the re-detected preview entry `contentChangedSinceAccept = true` / `included:false` (`:175-181`). The actual durable `pull_sheet_override = null` write is deferred to the caller's **locked staging upsert within the same re-scan** — in the onboarding path the callback records intent (`lib/sync/runOnboardingScan.ts:1222-1225`) and the null column write lands in the locked upsert (`lib/sync/runOnboardingScan.ts:574-576`); `rescanWizardSheet` prepares before the show lock (`lib/onboarding/rescanWizardSheet.ts:143-147`) and writes under it (`:161-185`). The load-bearing property for THIS design is only that the durable-clear and the refreshed preview are produced by the **same successful re-scan** (both persisted under the one show lock, so a Step-3 reader never sees one without the other): after a successful content-changed re-scan the durable snapshot is `null` AND `previewSnapshot` is `null` (no `included` tab) → `overrideSnapshotsEqual(null, null) === true` → **not divergent** → S4 renders normally. The ONLY producer of divergence is PSAT-1's exact scenario: the accept/revoke RPC commits the durable override in its own tx, and the **separate** best-effort post-commit re-scan (which would refresh the preview) fails, so the durable write persists while the preview never refreshes. (Plan/tests must assert the S4 non-collision at the Step-3 read/render layer — via the §4.3 case (f) fixture — NOT by asserting which sync layer writes the null.)
...
4. **S5 vs S4 non-collision is structural** (§3.2): the content-changed re-scan produces the durable-clear AND the refreshed preview together under one show lock, so a Step-3 reader always sees `null==null` (not divergent) for content-changed. Assert this at the read/render layer via §4.3 case (f), NOT by pinning which sync layer writes the null (the null write is deferred to the locked staging upsert, `runOnboardingScan.ts:574-576`, not `discardAndRerun` directly).
