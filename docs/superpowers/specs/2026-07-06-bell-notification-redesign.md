...
| D3 | **"Mark all read"** button in header (accent text), shown when unread active rows exist. | None (opening the panel already advances the `/bell/open` watermark). | **Adopt, client-side, reusing the existing `/bell/read` path.** Clears all active unread markers optimistically and fires the existing per-row `/bell/read` POST for each still-unread active row. **No new API.** |
