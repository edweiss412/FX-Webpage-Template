...
|---|---|---|---|---|
| DI-1 | Desktop panel root | self | width === 420px, right edge ≤ viewport | `sm:w-[420px] sm:max-w-[calc(100vw-1.5rem)]`, `sm:right-0` |
...
| row collapsed → expanded | message `-webkit-line-clamp` 2→unset (instant, height is content-driven), caret rotate 0→90° `--duration-fast`, helpful disclosure appears instant |
| row expanded → collapsed | reverse; caret 90°→0° `--duration-fast` |
...
| **compound:** mark-all-read WHILE a row is mid-expand | independent — mark-all drives dot/tint opacity; expand drives clamp/caret. No shared property, no conflict |
...
1. **Update/extend (dot contract UNCHANGED):** amend `notifBell` / `bellPanel` / `bellPanelActions` / `bellPanelDeferrals` to add assertions for severity tone (`bell-sev-{id}` `data-tone` from health / info-code / default fixtures), `bell-mark-all-read` (visible only with unread active rows; click clears all active unread markers + fires `/bell/read` once per still-unread active row; hidden when none unread), the new `data-unread` row attribute, and header/empty/history restyle testids. The existing `bell-unread-dot-{id}` opacity-flip assertions stay as-is. `aria-modal="true"` assertion unchanged (true both modes).
