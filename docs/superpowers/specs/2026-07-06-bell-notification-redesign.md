...
3. **Layout e2e:** update `tests/e2e/bell-panel-layout.spec.ts` — replace the desktop `sm:items-center` vertically-centered assertion with the **anchored** geometry (panel top-right, below the bell; caret present; overlay transparent/no dim at sm+). **Add desktop-width assertions (R2):** at sm+ viewports the panel inner width === 420px (±TOL) and the panel right edge ≤ viewport width (no horizontal overflow); at a narrow desktop (~430px) the `calc(100vw-1.5rem)` cap holds (width ≤ viewport−24px). Mobile sheet (`items-end`, full-width ≤420) unchanged. Keep all fixed-parent BoundingRect + dot-slot/opacity assertions.
