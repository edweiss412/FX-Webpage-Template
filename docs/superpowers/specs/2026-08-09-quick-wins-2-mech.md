...
3. **Derived guard — an arrow-form ban over ALL declared `@theme` tokens.** New vitest guard under `tests/specLint/`, file "themeTokenArrowBan.test.ts" (created by this arc). Rationale: every `@theme` token generates its canonical utility by construction (that is what `@theme` does), so an arrow form `<utility>-(--<token>)` naming a DECLARED theme token is never necessary — banning the whole class needs no per-token lint-behavior knowledge, is derived (parse the `@theme` block for token names; no enumeration), covers the measured silent residue and every future token on arrival, and is merely redundant where the plugin already reports (the guard fires only on what reaches the tree). Arrow forms referencing NON-theme variables stay out of scope. Mechanics: walk every tracked `.ts`/`.tsx` under `app/`, `components/`, `lib/`; fail on any string-literal arrow use of a declared token, naming file:line per hit; comment-only lines excluded by the entry probe's convention.
...
**The fix, three parts (part 3 added by R1 F2's probe — hostname parsing alone does NOT constrain psql's effective target):**
...
2. **Loopback refusal, on every channel the URL can name a host.** The helper THROWS unless ALL of: the parsed hostname is loopback; the URL's query string carries no `host`/`hostaddr` parameter (or only loopback values) — the R1 probe demonstrated `?host=192.0.2.1` and `?hostaddr=192.0.2.3` steering psql away from a loopback hostname. The error names the resolved host, the channel that supplied it, and the escape hatch — a new explicit `LOCKED_FIXTURE_ALLOW_REMOTE=1` opt-in (aligned with the observe CLI's `--env` guardrail posture). It replaces today's misleading "run `pnpm db:seed`?" symptom for this failure shape.
3. **Scrubbed child environment.** The psql spawn receives the parent env with every `PG*` variable removed — the R1 probe demonstrated ambient `PGHOSTADDR=192.0.2.2` overriding a loopback URL. The connection string is self-contained (it embeds credentials), so nothing legitimate is lost; the opt-in restores the ambient env verbatim for the operator who explicitly asked for remote.
...
- **AC-M5** (§2.6): unit suite green incl. observed-RED on the refusal case; call-time resolution proven; the query-param (`?host=`/`?hostaddr=`) and `PG*`-env bypass channels each have a refusal case; BOTH in-class helpers consume the shared resolver; sweep table reconciled at implementation time.
