...
7. **§2.6's refusal covers URI/env target channels, not filesystem trust.** An operator who writes a malicious the libpq user service file (pg_service.conf in the home directory) AND passes the explicit opt-in is outside the threat model (accidental misconfiguration, not adversarial local users).
