...
3. **Derived guard — an arrow-form ban over declared `@theme` tokens, breakpoint namespace excluded.** New vitest guard under `tests/specLint/`, file "themeTokenArrowBan.test.ts" (created by this arc). Rationale: for the covered namespaces, the `@theme` token generates a canonical utility resolving the SAME token, so the arrow form is never necessary — the ban is derived (parse the `@theme` block for token names; no enumeration), covers the measured silent residue and every future token on arrival, and is merely redundant where the plugin already reports. **The `--breakpoint-*` namespace is EXCLUDED by rule (R2 probe):** against the installed Tailwind 4.2.4 compiler, `min-w-(--breakpoint-sm)` emits `var(--breakpoint-sm)` while `min-w-sm` resolves `--container-sm` — a different token — so there IS no token-preserving canonical form, the arrow form is the legitimate spelling, and a mistaken "canonicalization" would silently change the resolved value; the tree's three live instances (`--breakpoint-sm`/`-lg`/`-xl` arrows) are correct as written. Arrow forms referencing NON-theme variables stay out of scope. Mechanics: walk every tracked `.ts`/`.tsx` under `app/`, `components/`, `lib/`; fail on any string-literal arrow use of a covered declared token, naming file:line per hit; comment-only lines excluded by the entry probe's convention.
...
2. **Loopback refusal, on every channel the URL can name a target — the shared resolver ADOPTS `devCaptureStaged`'s existing stronger check as its floor.** `devCaptureStaged.ts` already rejects `service` and other authority-override URI parameters (drafting locator :54); that allowlist posture becomes the SHARED contract: the resolver throws unless the parsed hostname is loopback AND the query string carries no target-steering parameter — `host`, `hostaddr`, and `service` at minimum, with unknown target-class parameters refused by allowlist rather than admitted by denylist (the R1 probe steered via `?host=`/`?hostaddr=`; the R2 probe steered a loopback authority to `192.0.2.3` via `?service=probe`). Migrating `lockedCrewRestriction` onto the shared resolver must not REGRESS any of `devCaptureStaged`'s existing port/database/credential protections — the resolver's checks are a superset of both helpers' current checks. The error names the resolved target, the channel that supplied it, and the escape hatch — explicit `LOCKED_FIXTURE_ALLOW_REMOTE=1` (aligned with the observe CLI's `--env` guardrail posture). It replaces today's misleading "run `pnpm db:seed`?" symptom.
3. **Scrubbed child environment, with the default service file neutralized.** The psql spawn receives the parent env with every `PG*` variable removed (R1 probe: ambient `PGHOSTADDR=192.0.2.2` overrode a loopback URL) AND `PGSERVICEFILE` pointed at an empty/nonexistent path — scrubbing alone leaves `HOME` in place and libpq falls back to `~/.pg_service.conf`, the R2 probe's second `service` channel. The connection string is self-contained, so nothing legitimate is lost; the opt-in restores the ambient env verbatim.
...
6. **§2.2's guard does not judge `--breakpoint-*` arrow forms.** No token-preserving canonical utility exists for that namespace (R2 probe: `min-w-sm` resolves `--container-sm`, not `--breakpoint-sm`), so the arrow form is the correct spelling there and the guard deliberately leaves it alone — a namespace exclusion, stated here so a later reader does not "fix" the three legitimate instances.
7. **§2.6's refusal covers URI/env target channels, not filesystem trust.** An operator who writes a malicious `~/.pg_service.conf` AND passes the explicit opt-in is outside the threat model (accidental misconfiguration, not adversarial local users).
...
- **AC-M5** (§2.6): unit suite green incl. observed-RED on the refusal case; call-time resolution proven; the `?host=`/`?hostaddr=`/`?service=` query channels, the `PG*` env channel, and the default-service-file channel (`PGSERVICEFILE` neutralized) each have a refusal case; unknown target-class query params refused by allowlist; BOTH in-class helpers consume the shared resolver through EVERY dsn entry point (env default AND explicit dsn args), with `devCaptureStaged`'s pre-existing protections asserted as preserved; sweep reconciled at implementation time.
