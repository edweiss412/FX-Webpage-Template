...
**Consequence bound for this arc (narrowed R1 — read §3.2 before using it).** Every control **the derived cover can see** either carries an outline at ≥3:1 against its own fill and against every neutral ground it is rendered on, or is a named exemption with its ratio recorded in §6. There is no third state *within that population*. What the cover cannot see is enumerated with probes in §3.2 and filed — it is neither claimed as covered nor left unstated.

The qualifier is not hedging; it is the R1 repair. The bound as first written ("every in-scope control") was **false**, and the reviewer settled it by probe: the cover is a cover of interactive elements whose OWN className the scanner statically resolves, which is a strictly smaller set than "controls with a low-contrast outline". Widening the scanner to close that gap is the recognizer-growth anti-pattern AGENTS.md names — each widening is a bigger target for the next round — so this spec narrows the claim to what is true and files the residue instead.
...
**(a) Form fields are not in the scanner's element vocabulary at all.** `scanInteractiveElements` over `components/admin/BellPanel.tsx` returns rows tagged `a`, `button`, `div` and `summary` — and no `input`. The two number fields at `components/admin/BellPanel.tsx:836` and `components/admin/BellPanel.tsx:847` (`min-h-tap-min w-20 rounded-sm border border-border-strong bg-surface`) and the textarea at `components/admin/wizard/step3ReviewSections.tsx:4171` are invisible to the cover in both directions: the guard will never flag them and never exempt them.
...
The probe that settles the shape (run 2026-08-16, this session): `ScanElement` carries **no testid**, so the entry's suggested "file+testid" key is not available. What it does carry is `paths` — one class-string array per conditional branch of the element's `className`.

**The obvious predicate is underpowered, and R1 F2 proved it by mutant.** "Exempt iff some branch carries `border-accent-edge`" fails in both directions:

- *It does not pin what §6 records.* One ordinary edit — `PublishedToggle`'s OFF branch from `bg-surface-sunken` to `bg-surface` — leaves the element covered, still exempt, and the registry's file-path equality still satisfied, while the documented 1.43/1.75 exemption has silently become an unrecorded 1.59/1.60 treatment. The guard would stay green through exactly the corruption it exists to prevent.
- *Accent-edge does not imply switch track.* `components/admin/OnboardingWizard.tsx:238` carries `border-accent-edge bg-accent` on a step pill that is not a toggle. Today the cover never sees it, because the pill is a child of the `Link` at `components/admin/OnboardingWizard.tsx:251` (§3.2b) — so the predicate is safe only by an accident of what the scanner cannot see, which is not a property to build a guard on.

So the predicate pins the **whole recipe, both branches**, not one token:

> **The exemption is an ACCEPT-SET over branches, quantified universally.** Define the two accepted branch shapes: `ON` = tokens include both `border-accent-edge` and `bg-accent`; `OFF` = tokens include both `border-border-strong` and `bg-surface-sunken`. An element in the cover is exempt if and only if **every** one of its `paths` branches is an `ON` or an `OFF`, **and** at least one of each is present. Any branch that is neither — and any element missing either shape — is a failure.

**The universal quantifier is the R2 F1 repair and it is what terminates this vector.** The previous form was existential ("has an ON branch AND has an OFF branch"), and the reviewer broke it in one probe: append `["border-border-strong bg-surface"]` to any live track's `paths` and the element stays exempt, because the two required branches are still present — while the appended branch measures **1.59 light / 1.60 dark**, neither ≥3:1 nor recorded in §6. That is the consequence bound violated by a guard that is green.

An existential predicate is a **denylist wearing an allowlist's clothes**: it names what must be present and is therefore silent about everything else, so it accepts whatever it did not model. AGENTS.md's accept-set discipline for detector surfaces asks for the opposite — state what is ACCEPTED, keyed on structure, and reject or report everything outside it by name. The universal form is that accept-set, and it closes the vector structurally rather than per-instance: no third branch, of any spelling, can be added without reddening the guard. Adding another accepted shape is then a deliberate spec edit with its ratio recorded, which is exactly the friction this arc wants.

This also closes both R1 F2 holes. Moving an OFF fill to `bg-surface` produces a branch that is neither `ON` nor `OFF` and reds. A non-toggle carrying only an accent-edge branch has no `OFF` and is not exempt — so the predicate stops depending on the scanner's blind spot, which matters because `DESIGN.md:35` documents `accent-edge` as serving the "active step pill" and the "active show-day progress segment" as well as the toggle track. The token was never toggle-exclusive; the first predicate contradicted the token's own documented scope.

The registry does not disappear, it changes job: the suite also asserts the exempt set's **file paths equal a named registry**, now **five rows** (§3.1), so a sixth track is not silently absorbed — it reds once, and someone adds a row deliberately with the ruling cited. Derivation decides admissibility; the registry decides whether the population changed. Both, not either.

Note the registry's five rows and the cover's three visible tracks are not the same number, and must not be conflated: `AutoRefreshControl` and `DeveloperToggleButton` paint their tracks on nested children the cover cannot see (§3.2b), so they are registry rows WITHOUT a cover element. The suite asserts the cover's exempt set equals the registry's *cover-visible* subset, and asserts the other two rows exist in source by their recipe — two different checks, because they answer two different questions.
...
- **Both halves of the exemption, separately:** a fixture carrying the FULL recipe (an `border-accent-edge` + `bg-accent` branch AND a `border-border-strong` + `bg-surface-sunken` branch) is exempt; a fixture missing either branch is not. One case each, so the predicate cannot pass by always answering the same way.
- **The R2 F1 mutant, as a standing case — the accept-set's own proof.** A fixture carrying a valid `ON` branch AND a valid `OFF` branch AND a third branch `border-border-strong bg-surface` must NOT be exempt. Under the existential predicate it stayed exempt while that third branch measured 1.59/1.60, neither ≥3:1 nor recorded in §6. This is the case that proves the quantifier is universal; without it the suite cannot tell the two predicates apart.
- **The R1 F2 mutant, as a standing case.** A fixture identical to the switch recipe except that its OFF branch reads `border-border-strong bg-surface` must NOT be exempt. This is the reviewer's probe promoted into the suite: under the first draft's single-token predicate it stayed exempt while §6's recorded 1.43/1.75 silently became an unrecorded 1.59/1.60. A guard that cannot fail on the mutant that broke its predecessor has not learned anything.
- **Accent-edge alone is not a track.** A fixture with an `border-accent-edge bg-accent` branch and NO strong/sunken branch must NOT be exempt — the `components/admin/OnboardingWizard.tsx:238` shape (§3.2b), which is a step pill rather than a toggle.
...
**No state is added, removed, or re-timed.** The swap changes which colour a control's EXISTING treatment resolves to — not the set of states, nor any pair between them, nor any timing.
...
| --- | --- | --- |
| rest → hover | each control's existing treatment — `transition-colors duration-fast` on 19 of 21, instant on the two in the table above; hover changes the FILL, not the outline | No — the outline colour is constant across this pair, before and after |
