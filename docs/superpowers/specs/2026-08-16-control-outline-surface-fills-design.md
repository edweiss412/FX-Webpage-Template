...
**§6 is the single source for every SURVIVING SUB-3:1 ratio — the residue the arc guarantees to record — and §2's BINDING BOUND restates none of them.** It is not the only place a ratio appears, and claiming that would be its own false universal (whole-diff R10): §3's measurement tables and §4.2's per-site post-swap column are both current, and both are EVIDENCE for the predicate rather than claims about what survives. The rule is about the residue, which is the part a reader has to be able to find in one place. (§2.1's rejection rationales and the R1/R3 history below do quote measurements, as historical record of decisions already taken; they are not claims the arc guarantees, and every figure in them is §6's.) Three of the ten R1-R3 findings, plus R4 F1, were a universal in one section contradicting a measurement in another; the fix is not another carefully-worded universal but removing the duplicate. §2 says what the arc GUARANTEES (nothing sub-3:1 goes unrecorded); §6 holds the numbers. R4 F1 is the last instance: §2 claimed "every path this arc swaps measures ≥3:1", which is false for the `StagedPreviewBanner` link at **2.79 dark** on `warning-bg` — a value §4.4 and §6 had recorded correctly all along.
...
Transparent fill on the `bg-warning-bg` banner — a TINTED plate, and the one census row whose OWN FILL is that plate, so it is the only row whose outline is against `warning-bg` on BOTH edges and therefore the only one whose post-swap number does not clear 3:1 in both themes on either side. **Two further census rows stand ON a tinted plate while carrying their own `bg-surface` fill** (whole-diff R5; a third was raised in the same round and did not reproduce): `app/admin/settings/roles/RoleMappingRow.tsx:343` inside the `bg-warning-bg` confirm card at `app/admin/settings/roles/RoleMappingRow.tsx:330`, and `components/shared/ReportModal.tsx:622` inside the start-fresh warning plate at `components/shared/ReportModal.tsx:615`, both confirmed by nesting probe; a third, `components/admin/RoleRecognizeControl.tsx:225`, was raised in the same round and is NOT counted — its nearest enclosing fills are `bg-surface`/`bg-surface-sunken` at `components/admin/RoleRecognizeControl.tsx:93-94` and the cited assembly point carries no warning fill, so the claim did not reproduce. For the two confirmed, the outline's INNER edge is against `surface` and clears at 3.35/3.76; only the OUTER edge against the plate sits at 3.04 light / 2.79 dark. They join `BL-CONTROL-OUTLINE-ON-TINTED-PLATES` with the picker link, taking that entry to fourteen controls across thirteen sites. It swaps with the rest and both themes improve (1.44 → 3.04 light, 1.19 → 2.79 dark); the residual dark **2.79** is exactly the tinted-plate class already filed as `BL-CONTROL-OUTLINE-ON-TINTED-PLATES`, whose entry already records `warning-bg` at 2.79 dark. It files there, not here, and this arc adds no new entry for it — the site simply joins the eleven controls that entry already counts, and the entry's site list gains this row. The sibling `<span aria-current>` at `components/admin/StagedPreviewBanner.tsx:65` is non-interactive chrome: outside the census, keeps its token.
