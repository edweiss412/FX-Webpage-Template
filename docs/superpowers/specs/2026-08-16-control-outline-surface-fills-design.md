...
> Within the population the derived cover can see (§3.2 names what it cannot), **no element outside the switch-track registry retains a resting `border-border-strong` outline**, and every path this arc swaps measures ≥3:1 against its own fill and its ground. Any other token's treatment on those same elements is out of this arc's scope and is recorded in §6 with its ratio — never silently.
