...
> An element in the cover is exempt if and only if it has a branch whose tokens include BOTH `border-accent-edge` and `bg-accent`, AND a branch whose tokens include BOTH `border-border-strong` and `bg-surface-sunken`. That pair IS the documented switch recipe, and it is the pair whose ratios §6 records. Anything else in the cover is a failure.

This closes both holes with one change. The mutant above now reds: moving the OFF fill to `bg-surface` breaks the second conjunct. A non-toggle carrying only an accent-edge branch is not exempt, so the predicate stops depending on the scanner's blind spot. And it states the ruling's reason — the toggle's ON/OFF pair is a designed relationship, and the exemption is of that PAIR — rather than restating its outcome.
