...
- **An eager position is EVALUATED, and the walk stops at exactly ONE thing: a nested
  registration's INLINE SUITE BODY.** It does NOT stop at function-like nodes. A stop at
  `ts.isFunctionLike` stood here for two rounds on the reasoning that a function VALUE in an eager
  position is not invoked while Vitest collects. **That reasoning reads LEXICAL SHAPE and calls it
  execution**, and diff review r4 probed the difference: an IIFE, an invoked object method, a
  synchronously invoked callback, an invoked default-parameter initializer and a computed
  method/getter/setter name all sit inside a function-like node and all RUN, at `runtimeHookCalls=1`
  against `scanner=environment-free`. That is false certification, which the bound forbids, bought in
  exchange for the wrong attribution it permits. Nothing syntactic connects a function body to a call
  site, so the scanner cannot decide invocation and deciding it is the resolution R1 declines. The
  stop is DELETED; a `.each` datum, an IIFE and an uncalled helper all report, and the residue is
  limit L8.
- **The one stop that remains is narrow, and its justification ranges over everything it does.** A
  nested registration's inline suite body is skipped because `hookBodies` already walks it and
  attaches the hook to that suite's own tests, so descending there would name the OUTER eager
  position for a hook belonging to the INNER suite — a wrong attribution. It fires only on an
  argument `suiteBodyFunction` resolves to a body, of a call `registrarRoot` recognizes, at an index
  Vitest actually invokes (slot 0 excluded, because Vitest FORMATS a function-valued name rather than
  invoking it). It does NOT fire on a function value in general. Diff review r5 finding 1, which
  deleting the broad stop exposed, bracketed: all three eager categories read `environment-free` on
  `origin/main` and under this change once the narrow stop is in place. A hook written DIRECTLY in a
  datum, `describe.each([beforeEach(…)])`, still reports.
- **Eager positions** are the ones `hookBodies` already treats as eager for the nested case: the
  ...
  refuted.** `hookBodies` carries no function-like stop, and neither does this producer any more — the draft that
  claimed a divergence here was describing the stop that r4 deleted. A hook written inside a DEFERRED
  datum classifies `environment-touching` under `hookBodies` when it sits in an inline describe, and
  is REPORTED by this producer at file scope. Bracketed on both trees, that
...
--- cells that must REPORT   (bare identifier, function declaration, property
    ...
    registration-inside-a-function-value, hook in a function-valued .each
    datum, hook in a method-shorthand .each datum)                        all PASS
...
| **L4** | **A hook registered by a CALLED HELPER is invisible, at every position.** `describe(registerHook(), …)`, where `registerHook()` registers a `beforeEach` and returns a name, is classified `environment-free`. Neither producer fires, because both key on a syntactic `HOOK_REGISTRARS` call and there is none. | **PRE-EXISTING and UNIFORM, bracketed against the shipped baseline rather than asserted.** The same helper is equally invisible inside an inline `describe` body and as a plain file-scope statement — all three classify `environment-free` identically on `origin/main` and under this change. It is `hookBodies`' and the top-level seed's existing LEXICAL contract, which this change neither causes nor widens, and it is the same disposition the filing arc applied to its own four pre-existing gaps. Closing it means following hook registration through a call, which is the resolution R1 declines. Raised as spec review r2 finding 1, whose substance was right and whose attribution to this design was not. Diff review r4 re-raised the class with nine probed shapes and it BRACKETS the same way: an invoked object method, a synchronously invoked callback, an invoked default-parameter initializer and computed object/class method, getter and setter names all read `environment-free` on `origin/main` AND at this head, because the hook is not in the registration's argument subtree at all — it is in a separate declaration reached by CALL, which is this row. Two of that round's nine — an arrow IIFE and a function-expression IIFE — were genuinely in-argument, moved `environment-free` to `unclassifiable`, and were REPAIRED rather than bracketed; the difference is measured, not argued. |
...
| **L8** | Anything inside a FUNCTION VALUE — a registration for producer B, a hook for producer A — is reported whether or not that function is ever invoked — a `.each` datum and an uncalled helper are reported exactly as an invoked helper is. | The scanner is syntactic and nothing connects a function body to a call site, so the two are indistinguishable to it. The rejected alternative was suppressing on execution SHAPE, which diff review r3 showed silences the INVOKED case too — false certification, which the bound forbids, traded for a wrong attribution it permits. Diff review r4 then found the SAME suppression still standing in producer A, where a stop at `ts.isFunctionLike` silenced an IIFE and an invoked function expression, and it was deleted symmetrically. Both deletions are the same repair, and the r3 justification that the surviving site survived "for a different reason" was true of the `.each` datum case it was checked against and NOT of everything that site did. Reporting both is the conservative direction, and both reasons stay TRUE: producer A says whether the hook registers cannot be determined, producer B says IF the argument is the factory its hooks cannot be located. Zero live instances. |
...
| **hook inside a function value in an eager position — `describe.each([() => { beforeEach(…) }])(…)`** | the specified rule, whose collector stops at nothing but an inline suite body | an implementation stopping at `ts.isFunctionLike` inside the collector | **the function-like suppression in producer A.** It was a SILENT cell until diff review r4, which probed that the same stop also silences an IIFE, an invoked object method, a synchronously invoked callback, an invoked default-parameter initializer and a computed method/getter/setter name — hooks that RUN, at `runtimeHookCalls=1` against `scanner=environment-free`. Reading lexical shape and calling it execution is the false certification the bound forbids. Residue is L8, now covering both producers |
| **hook inside a METHOD-SHORTHAND function value — `describe.each([{ setup() { beforeEach(…) } }])(…)`** | the specified rule | the same suppression, reached through a different node kind | **the same suppression via a MethodDeclaration.** ONE cell for the whole function-like class rather than one per node kind: the deleted stop used TypeScript's own `isFunctionLike`, which covers methods, getters, accessors and constructors together, so a cell per kind would rebuild the enumeration that predicate existed to delete |
