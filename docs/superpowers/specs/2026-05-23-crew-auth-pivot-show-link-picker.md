...
- `app/show/[slug]/[shareToken]/page.tsx` (new, replaces `app/show/[slug]/page.tsx`) — auth resolution + delegation. **R41-R2 amendment (CRITICAL repair); R41-R6 intent-token contract.** Imports `resolveShowPageAccess`. When the helper returns `{ kind: 'needs_picker_bootstrap', intentToken }`, the page calls `redirect('/api/auth/picker-bootstrap?next=' + encodeURIComponent(currentURL) + '&t=' + encodeURIComponent(intentToken))` from `next/navigation`. **The `&t=<intentToken>` parameter is REQUIRED** — picker-bootstrap returns 403 without it (R41-R5 CSRF defense). The intent token is generated by `resolveShowPageAccess` at decision time as `HMAC-SHA256(JSON({slug, shareToken, exp: now+60s}), PICKER_COOKIE_SIGNING_KEY)`. Server Components cannot emit `Set-Cookie`; the Route Handler at the redirect target is the legal cookie mutator. Admin precedence + `requireAdmin()` defense-in-depth at the top of the chain (per the old file's docstring at lines 27-35) is preserved verbatim.
- `app/api/auth/picker-bootstrap/route.ts` (new, R41-R2 amendment; R41-R3 atomic claim-and-mint; R41-R5 intent-token CSRF; **R41-R6 — one-show write only, no fill-in-the-blanks**). Route Handler that legally mints `__Host-fxav_picker` for a Google-signed-in user. **Flow:** (1) read `next` AND `t` (intent token) from query params; validate `next` against the `validateNextParam.ts` allowlist; **(1.5) verify the intent token** — `t = base64url(payload) + '.' + base64url(HMAC-SHA256(payload, PICKER_COOKIE_SIGNING_KEY))` where `payload = JSON({slug, shareToken, exp})`. Decoder rejects: missing `t`, malformed format, expired (`exp < now`), HMAC mismatch, OR `{slug, shareToken}` not matching the `next` URL's parsed slug+token. **A failed intent check returns 403** (NOT 302). This prevents an `<img src="https://app/api/auth/picker-bootstrap?next=...">` CSRF — an attacker cannot generate a valid HMAC. The token's 60s expiry bounds replay; (2) `validateGoogleSession(req)` — no session → 302 to `next` with NO cookie set; (3) invoke `claim_oauth_identity(p_email = auth.users.email)` via the service-role client — stamps `claimed_via_oauth_at` on every matching row under per-show advisory locks AND returns `{ claimed_count, shows }`; (4) **one-show write contract (R41-R6 lost-update repair)**: parse `next` to extract the target `slug`/`shareToken` → resolve via `resolve_show_by_slug_and_token` → obtain `target_show_id`. Find the entry for `target_show_id` in `result.shows`. If present, read the request's picker envelope, modify ONLY that show's entry to `{ id: crew_member_id, e: picker_epoch, t: now }`, leave every other entry untouched, write the modified envelope via `cookies().set('__Host-fxav_picker', signEnvelope(envelope), PICKER_COOKIE_OPTIONS)`. If `target_show_id` is NOT in `result.shows` (user not on this show's roster OR omitted as ambiguous), write NO cookie. **The handler never touches entries for shows other than `target_show_id`** — even though `result.shows` may contain other shows the user is on, those entries get minted lazily on the user's next visit to each such show. This is the structural defense against cross-show lost-update; (5) 302 to `next`. **No redirect loop is possible**: (a) target show in result.shows + non-ambiguous → bootstrap mints the entry → page step 6 catches it; (b) ambiguous shows → `resolveShowPageAccess` returns `ambiguous_email` and the page never redirects in the first place; (c) non-matching shows → bootstrap doesn't mint AND the page's step 4(e) falls through to step 5. **This Route Handler IS in the cookie-mutator allowlist** alongside `selectIdentity`, `clearIdentity`, `cleanupStaleEntry`, and `/auth/callback/route.ts`; the §10.1 picker-cookie-contract meta-test is extended to include it. **Anti-overwrite contract**: the picker-bootstrap handler MUST NOT modify the entry for any show other than `target_show_id`. The §10.2 regression test asserts this: a pre-existing entry for Show-Y persists byte-identical across a picker-bootstrap call for Show-X.
...
// visit via /api/auth/picker-bootstrap. This removes the highest-frequency
// race surface (callback bulk-cookie-write racing selectIdentity).
...
Visual treatment for BOTH cases: row background `var(--muted)` (instead of `var(--card)`); name + chip text `color: var(--muted-foreground)`; lock icon (16px, `color: var(--muted-foreground)`) to the LEFT of the role chip with `aria-label="Sign in to use this identity"`. The `<button>` element is STILL tappable — form action submits to `/auth/sign-in?next=<tokenized URL>` instead of `selectIdentity`. Tap on a deactivated row redirects to OAuth flow, completing the user's intent. Data attributes for test discrimination: `data-claimed="true"` for OAuth-claimed rows; `data-ambiguous="true"` for ambiguous-email rows (a row CAN have both, and both attributes are emitted in that case).
...
- **`PICKER_IDENTITY_AMBIGUOUS` (R41-R6 Fix-3):** the submitted `crewMemberId` row's email appears on 2+ `crew_members` rows on this show. Crew-facing copy: "This name needs roster cleanup. Ask Doug to remove the duplicate." Operator-facing: `{ show_id, attempted_crew_member_id, observed_email, dup_count }`. On render: the Server Action returns a redirect to `/auth/sign-in?next=<encoded tokenized URL>` (same destination as `PICKER_IDENTITY_CLAIMED`) — the user CAN proceed via OAuth if they own the email; if both ambiguous rows are theirs (data entry duplicate), the OAuth path successfully claims both rows and the user is identified consistently. Cataloged in `lib/messages/catalog.ts`. Carries the `tamper` log-flag.
...
- **Deactivated row tap-redirect (R41 amendment, §7.2; R41-R6 expanded for ambiguity)** — picker rendered with a roster containing crew members in three states:
  ...
  Tests assert: (a) State A, B, C rows have `aria-disabled="false"` (still tappable) AND name/chip computed colors equal `var(--muted-foreground)`; (b) each carries the 16px lock icon with `aria-label="Sign in to use this identity"`; (c) each row's `<form>` action submits to `/auth/sign-in?next=<encoded tokenized URL>` (NOT `selectIdentity`); (d) submitting the form does NOT mint a picker cookie; (e) State D rows are fully active. **R41-R6 server-side enforcement test**: hand-craft a Server Action POST for a State B row's `crewMemberId` (the form's action attribute would otherwise redirect, but a malicious client could bypass that). Assert: the Server Action returns `PICKER_IDENTITY_AMBIGUOUS` cataloged response (redirect to `/auth/sign-in?next=...`); NO `Set-Cookie` for `__Host-fxav_picker`; structured log carries `tamper` flag. **R41-R6 ambiguous-NULL regression**: this is the specific edge case the reviewer flagged — a signed-in user whose email is duplicated on the roster, with `claim_oauth_identity` having failed (or never run, for legacy session), should NOT be able to bypass-pick an ambiguous row. Verified end-to-end via both the UI deactivation AND the server-side check.
