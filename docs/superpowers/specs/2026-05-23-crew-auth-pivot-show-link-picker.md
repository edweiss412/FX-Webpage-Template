...
15. **Google sign-in is restored as an OPTIONAL crew identity layer (R41 — reverses R14/R15's deletion of `/me` + `validateGoogleSession`).** The owner re-decided on 2026-05-23 after the share-token-credential design hardened: the strict reading of "crew never sign in" was over-applied by Codex's R14 finding. The corrected reading is: **show ACCESS doesn't require sign-in — anyone with `/show/<slug>/<share-token>` reaches the picker without auth — but sign-in remains available as an opt-in upgrade that adds (a) identity exclusivity in the picker and (b) cross-show discovery via `/me`.** Net consequences this spec carries:
    - **`/me` is preserved.** The page and `lib/data/listShowsForCrew.ts` survive. The listing now renders tokenized URLs (`/show/<slug>/<share-token>`) — the new SECURITY DEFINER RPC `my_share_tokens_for_email()` (per §5.3 below) returns `{ slug, share_token }[]` for shows whose `crew_members.email` matches `auth.email()` server-side. Crew who never sign in never use `/me`; crew who do sign in get one place that lists every show they're on. Doug's share-link workflow is unchanged — `/me` is a fallback discovery tool, not a replacement.
    ...
    - **The auth chain at `/show/<slug>/<share-token>` adds a Google-session arm AHEAD of the picker.** New ordering: archived → 404, then admin → render-as-admin, then **`validateGoogleSession` matching `crew_members.email` for THIS show → mint picker cookie + render show body (login skips the picker entirely)**, then picker cookie → render show body, then unpublished → 404, then the new "Sign in or skip" interstitial gate. The picker interstitial is reached only via the "Skip" path. Detailed flow in §4.1.
...
│        for this show, cookie already has an entry for this   │
│        show → fall through to step 6 (cookie path; typical   │
│        post-callback flow).                                  │
...
│     BEFORE any picker/gate render so neither exposes a       │
...
- `app/api/auth/picker-bootstrap/route.ts` (new, R41-R2 amendment; R41-R3 revised to atomic claim-and-mint; **R41-R4 hardened with intent-token CSRF defense + fill-in-the-blanks merge**). Route Handler that legally mints `__Host-fxav_picker` for a Google-signed-in user. **Flow:** (1) read `next` AND `t` (intent token) from query params; validate `next` against the `validateNextParam.ts` allowlist; **(1.5) verify the intent token** — the token is `HMAC-SHA256({slug, shareToken, exp}, pickerCookieSigningKey)` (same key as the cookie envelope; one secret, two purposes — verified explicitly in tests as a deliberate dual-use). Decoder rejects: missing `t`, expired (`exp < now`), HMAC mismatch, OR `{slug, shareToken}` not matching the `next` URL's parsed slug+token. **A failed intent check returns 403** (NOT 302). This prevents an `<img src="https://app/api/auth/picker-bootstrap?next=...">` CSRF — an attacker cannot generate a valid HMAC. The token's 60s expiry bounds replay; (2) `validateGoogleSession(req)` — no session → 302 to `next` with NO cookie set; (3) invoke `claim_oauth_identity(p_email = auth.users.email)` via the service-role client — stamps `claimed_via_oauth_at` on every matching row under per-show advisory locks AND returns `{ claimed_count, shows }`; (4) **fill-in-the-blanks merge** (R41-R4 lost-update mitigation): read the request's current picker envelope, then for each show in `result.shows`, ADD an entry ONLY IF the current envelope has no entry for that `show_id`. Existing entries (the user picked something via `selectIdentity` earlier) are NEVER overwritten — this preserves a user's explicit pick over the OAuth auto-resolve. The picker UX still renders the OAuth-claimed crew_member row as deactivated, so a user who picked A on a show where their actual identity is B sees their cookie keep A while B's row deactivates; that's the intended divergence (the cookie reflects the user's last explicit choice; the deactivation reflects identity claim); (5) write the merged envelope via `cookies().set('__Host-fxav_picker', signEnvelope(merged), PICKER_COOKIE_OPTIONS)`; (6) 302 to `next`. **If the show in `next`'s slug is NOT in `result.shows`** (Google-signed-in user not on this show's roster at all OR the show is in the ambiguous-email omitted set — but `resolveShowPageAccess` step (d) prevents the page-route redirect for ambiguous shows in the first place per R41-R4 Fix-1), the redirect proceeds with no new entry for that show — the page route then renders the SignInOrSkipGate naturally. **No redirect loop is possible**: (a) for non-ambiguous + matching shows, bootstrap mints the entry → page step 6 catches it; (b) for ambiguous shows, `resolveShowPageAccess` returns `ambiguous_email` and the page route never redirects in the first place; (c) for non-matching shows, bootstrap doesn't mint AND the page's step 4(e) returns to step 5 next hop (no re-redirect). **This Route Handler IS in the cookie-mutator allowlist** alongside `selectIdentity`, `clearIdentity`, `cleanupStaleEntry`, and `/auth/callback/route.ts`; the §10.1 picker-cookie-contract meta-test is extended to include it.
...
- `lib/auth/picker/resolveShowPageAccess.ts` (new, R41 Fix-3; R41-R3 revised to pure-resolver). **Page-route-only auth chain helper.** Called exclusively by `app/show/[slug]/[shareToken]/page.tsx`. Encapsulates the resolver chain: archived → admin precedence → Google-session-matching-crew-row → existing picker cookie → unpublished. Imports `validateGoogleSession`. **The helper is PURE — it never encodes cookies and never calls `cookies().set()`.** Returns a discriminated union: `{ kind: 'archived' }` | `{ kind: 'admin' }` | `{ kind: 'needs_picker_bootstrap' }` (Google session matches + no cookie entry yet — caller redirects to `/api/auth/picker-bootstrap`) | `{ kind: 'resolved', crewMemberId, source: 'cookie' | 'admin' }` | `{ kind: 'unpublished' }` | `{ kind: 'no_auth' }` (no admin, no Google, no cookie — caller renders gate or picker depending on `?gate=skip`) | `{ kind: 'epoch_stale' }` | `{ kind: 'removed_from_roster' }` | `{ kind: 'show_unavailable' }` | `{ kind: 'infra_error', code }`. The R41-R2 `justMinted` arm and the cookie-encoder import are DELETED — Server Components cannot mint cookies, and re-introducing the field would tempt an implementer to call `cookies().set()` from the page. **API consumers MUST NOT import this helper** — the no-jwt-surface meta-test asserts the only consumer is the show-page route handler. **A NEW static guard** (per R41-R3 reviewer recommendation) asserts `resolveShowPageAccess.ts` does NOT import the picker cookie encoder (`lib/auth/picker/cookieEnvelope.ts`'s `signEnvelope`/`encodeEnvelope` exports) NOR `cookies` from `next/headers` — preventing a future implementer from undoing the R41-R2 boundary.
...
**Single-mechanism contract (v1) — R41-R2 expanded mutator list:** the cookie is mutated **only** by the five legal cookie-mutator surfaces: (a) three crew-side Server Actions — `selectIdentity`, `clearIdentity`, `cleanupStaleEntry`; (b) two R41-R2 Route Handlers — `app/auth/callback/route.ts` AND `app/api/auth/picker-bootstrap/route.ts`. Each surface reads the request's current cookie, mutates the envelope in memory, and writes a fresh Set-Cookie with `Max-Age=7776000`. **Server Components CANNOT mutate cookies** — this is a Next App Router invariant; the page route MUST redirect to the Route Handler for any cookie mint that originates from a Server Component context. **`resetPickerEpoch` does NOT touch the cookie (R30 amendment)**. Middleware does NOT touch the cookie.

**R41-R4 — fill-in-the-blanks merge semantics for OAuth surfaces (lost-update mitigation):** the OAuth-driven surfaces (`/auth/callback`, `/api/auth/picker-bootstrap`) use **fill-in-the-blanks** semantics: they ADD an entry for a show ONLY IF no entry currently exists for that `show_id` in the request cookie. Existing entries (from `selectIdentity`) are NEVER overwritten. This bounds the R16 lost-update class: a stale OAuth-handler response with a pre-`selectIdentity` cookie snapshot can only DUPLICATE entries that already exist (no-op) — it cannot resurrect a deleted entry or revert a fresh selectIdentity write. **The remaining theoretical lost-update window** is two OAuth handlers firing concurrently (callback + picker-bootstrap, or two callbacks across tabs); both are foreground user-initiated events (sign-in, navigate-to-new-show) and the user-behavior model makes simultaneous firing vanishingly rare. Server Actions retain READ-MODIFY-WRITE semantics for the specific show they mutate (selectIdentity overwrites its own entry; clearIdentity removes it; cleanupStaleEntry compare-and-deletes). **Cross-mutator-class invariant**: OAuth handlers never modify entries Server Actions wrote; Server Actions only modify their own target show. The §10.2 concurrency-race tests cover (a) callback racing selectIdentity on the same show, (b) picker-bootstrap racing selectIdentity on a different show, (c) two callbacks firing concurrently from two tabs.

**Why not per-show cookies (R41-R4 considered alternative):** a per-show cookie name like `__Host-fxav_picker_<show_id>` would eliminate the lost-update class entirely (each cookie name is its own atomic unit). Rejected for v1 because: (a) browser per-domain cookie cap (180 in Chrome, 150 in Firefox) is reachable for users on 100+ shows (theoretical, not v1); (b) the manifest cookie still has the same write contention; (c) the foreground-user-initiated-only frequency makes the residual lost-update window acceptable. Tracked in BACKLOG for v2 if the lost-update class surfaces in production telemetry.
...
if (user?.email) {
  ...
  if (error) {
    ...
    // R41-R4 fill-in-the-blanks: read the existing envelope, ONLY add entries
    // for shows that don't currently have one. Never overwrite a user's
    // explicit selectIdentity pick. This bounds the R16 lost-update class
    // for concurrent callback + selectIdentity.
    const existing = decodeEnvelope(cookies().get(PICKER_COOKIE_NAME)?.value);
    const merged = fillInBlanksFromShows(existing, result?.shows ?? []);
    if (merged.selections && Object.keys(merged.selections).length > 0) {
      cookies().set(PICKER_COOKIE_NAME, signEnvelope(merged), PICKER_COOKIE_OPTIONS);
    }
  }
}
```
...
begin
  ...
  perform pg_advisory_xact_lock(hashtext('show:' || s.drive_file_id))
    from public.shows s
  ...
  if array_length(v_ambiguous_show_ids, 1) is not null then
    insert into public.admin_alerts (code, payload)
    select 'AMBIGUOUS_EMAIL_BINDING',
           jsonb_build_object('email', v_email, 'show_id', show_id)
...
| `/me` | signed-in | **R41 amendment: PRESERVED.** Lists shows where signed-in user's email matches a `crew_members` row on a published+not-archived show. R41 difference vs M11: the listing now emits **tokenized URLs** `/show/<slug>/<share-token>` via the new SECURITY DEFINER RPC `my_share_tokens_for_email()` (per §5.3 R41). Crew never NEED to use `/me` — Doug's per-show links work without sign-in — but `/me` is the optional cross-show discovery surface for crew who do sign in. The RPC uses `auth.email()` internally so a signed-in user can only enumerate THEIR OWN show tokens. |
...
- `SELECT id, show_id, claimed_via_oauth_at FROM crew_members WHERE id = $1`: 0 rows → reject with `PICKER_CREW_MEMBER_NOT_FOUND`; show_id mismatch → reject with `PICKER_CREW_MEMBER_WRONG_SHOW` (defense-in-depth against form-tampering); **`claimed_via_oauth_at IS NOT NULL` → reject with `PICKER_IDENTITY_CLAIMED` (R41 Fix-2)**. The selection MUST happen atomically inside `select_identity_atomic` (the SECURITY DEFINER RPC under the per-show advisory lock) so a concurrent `claim_oauth_identity` cannot stamp `claimed_via_oauth_at` between this read and the cookie-mint without serialization. The RPC's body must include the `claimed_via_oauth_at IS NULL` predicate in its row-selection step. **This is the structural defense against the R41-R1 HIGH finding** — without this check, a hand-crafted Server Action POST could mint a cookie for a claimed identity by tampering with the form payload. The deactivated-row UI in §7.2 is the user-facing layer; this is the server-side enforcement.
...
**Deactivated-row contract (R41 amendment).** Any row whose underlying `crew_members.claimed_via_oauth_at` is non-null renders as deactivated: row background `var(--muted)` (instead of `var(--card)`); name + chip text `color: var(--muted-foreground)`; an additional lock icon (16px, `color: var(--muted-foreground)`) appears to the LEFT of the role chip with `aria-label="Sign in to use this identity"`. The `<button>` element is STILL tappable — but the form action submits to `/auth/sign-in?next=<tokenized URL>` instead of `selectIdentity`. This means a tap on a deactivated row redirects to OAuth flow, completing the user's intent (which is to identify as that person — they just need to prove it). The deactivated row's `data-testid` carries an additional attribute `data-claimed="true"` for test discrimination.
...
- **No picker-bootstrap redirect loop (R41-R3 CRITICAL regression)** — fixture: user has a `crew_members` row on Show-X with `claimed_via_oauth_at IS NULL` (Doug added them post-sign-in; the OAuth callback never ran for this row). Request flow: (1) GET `/show/<slug-X>/<token-X>` with Google session + NO picker cookie entry for Show-X; (2) page route detects Google session + matching row + no cookie → returns 302 to `/api/auth/picker-bootstrap?next=/show/<slug-X>/<token-X>`; (3) follow the redirect; (4) picker-bootstrap invokes `claim_oauth_identity(email)` which stamps Show-X (NEW: rows with `claimed_via_oauth_at IS NULL` get stamped this round, NOT bypassed) and returns `{ claimed_count: 1, shows: [{ show_id: Show-X, crew_member_id: <id>, picker_epoch: <int> }] }`; (5) handler mints cookie entry for Show-X + 302 to next; (6) follow the redirect; (7) page route step 6 reads the cookie, returns `{ kind: 'resolved' }`, renders `_ShowBody`. **Assert no third redirect** (no loop). Negative-regression variant: same fixture but `claim_oauth_identity` simulated to throw infra fault on first call → handler still 302s to next without a cookie (the page renders the SignInOrSkipGate; user can retry sign-in to recover). The test also asserts the redirect chain depth is bounded at 2 hops (page → bootstrap → page) and never 3+.
...
- **Ambiguous-email handling (R41-R3 HIGH regression, R41-R4 expanded for SQL bug)** — DB-level test with TWO ambiguous shows in the SAME `claim_oauth_identity` invocation (regression against the R41-R4 SQL bug where `SELECT INTO` over a GROUP BY HAVING returned only one ambiguous group). Fixture: Show-X has 2 `crew_members` rows with email `e@x.com`; Show-Y also has 2 `crew_members` rows with the same email `e@x.com`; Show-Z has 1 row (clean). Test invokes `claim_oauth_identity('e@x.com')`. Assert: (a) the UPDATE stamps ALL FIVE rows (the IS NULL filter doesn't care about ambiguity); (b) the returned `shows` array contains Show-Z ONLY (Show-X and Show-Y excluded by HAVING COUNT(*) = 1); (c) **TWO `AMBIGUOUS_EMAIL_BINDING` rows exist in `admin_alerts`** — one for Show-X, one for Show-Y (regression: a buggy SQL would emit one or zero); (d) the callback handler mints a picker cookie entry for Show-Z ONLY; (e) a Google-signed-in user visiting `/show/<slug-X>/<token-X>` sees the SignInOrSkipGate → picker → both ambiguous rows render as deactivated → tapping either redirects to `/auth/sign-in?next=...` (which loops back; resolution requires admin action to deduplicate the roster — out of pivot scope). Single-ambiguous-show variant: only Show-X ambiguous (1 row case) — asserts exactly one alert + Show-Y/Z minted normally. Zero-ambiguous variant: all shows clean — asserts no alerts emitted.
- **`resolveShowPageAccess` is a pure resolver (R41-R3 HIGH regression)** — static analysis test. Assert: `lib/auth/picker/resolveShowPageAccess.ts` does NOT import (a) `signEnvelope` / `encodeEnvelope` / any other export from `lib/auth/picker/cookieEnvelope.ts`; (b) `cookies` from `next/headers`; (c) the picker-bootstrap route handler or its helpers. The helper's only side effects are SELECTs against the DB; it returns a pure discriminated union. The companion runtime test: invoke `resolveShowPageAccess` with each of the 10 documented chain outcomes and assert the response object matches the expected `kind` discriminator with no `Set-Cookie` header in the calling context (mocked Next request context).
- **Ambiguous-email avoids redirect loop (R41-R4 HIGH regression)** — fixture: Show-X has 2 `crew_members` rows with the same email `e@x.com`; Google session is signed in as `e@x.com`. Request to `/show/<slug-X>/<token-X>` with NO picker cookie. Tests assert: (a) `resolveShowPageAccess` returns `{ kind: 'ambiguous_email' }` (NOT `needs_picker_bootstrap`); (b) page route does NOT emit a 302 to `/api/auth/picker-bootstrap`; (c) page route renders the SignInOrSkipGate (no `?gate=skip`) OR the picker (with `?gate=skip`); (d) the picker, when rendered, shows BOTH ambiguous rows as deactivated (after `claimed_via_oauth_at` is non-null; or NULL but per the same UX); (e) `validateGoogleSession`'s pre-existing `AMBIGUOUS_EMAIL_BINDING` emission fires (one alert per render — rate-limited downstream is out of pivot scope); (f) tapping either deactivated row redirects to `/auth/sign-in?next=...` which loops back to the gate (user cannot recover without admin roster cleanup; this is the documented out-of-pivot-scope state). **Redirect-chain depth assertion**: the response chain from the initial GET is at most 1 hop (no redirect for ambiguous-email; the page renders directly).
...
- **Fill-in-the-blanks cookie merge (R41-R4 MEDIUM regression — R16 lost-update mitigation)** — three concurrency scenarios:
  - **Callback racing selectIdentity, different shows**: fixture: user has crew rows on Show-A and Show-B. T0: user picks crew row C-B on Show-B via `selectIdentity` (cookie at T0 was empty, post-write cookie has `{Show-B: C-B}`). T0': in parallel, `/auth/callback` handler started with the SAME T0 empty cookie snapshot, processed OAuth, and at T200 writes its merged envelope. Without fill-in-the-blanks, the callback's write `{Show-A: C-A, Show-B: C-B-oauth}` would overwrite `{Show-B: C-B}`. WITH fill-in-the-blanks, the callback reads current cookie at write-time, sees `{Show-B: C-B}`, and merges to `{Show-A: C-A, Show-B: C-B}` (preserving the selectIdentity pick for Show-B; only adding the new Show-A entry). Assert: post-T200 cookie has BOTH `{Show-A: C-A}` AND `{Show-B: C-B}` (NOT C-B-oauth). The selectIdentity write is preserved.
  - **Picker-bootstrap racing selectIdentity, same show**: fixture: user has Show-A only. T0: user picks crew row C-1 via selectIdentity. T0': in parallel, picker-bootstrap for Show-A started, OAuth resolves to crew row C-2 (different from C-1 — e.g., a wrong-pick attempt before sign-in). T200: bootstrap's fill-in-the-blanks merge reads `{Show-A: C-1}` from current cookie, sees Show-A entry exists, DOES NOT overwrite. Assert: post-T200 cookie still has `{Show-A: C-1}` (user's pick wins over OAuth auto-resolve). The deactivation contract still fires on the next picker render because `claimed_via_oauth_at` was stamped — but the cookie selection remains C-1, which is the user's explicit choice; if C-1 is also OAuth-claimed (its row's claimed_via_oauth_at is non-null), the next page-load will still render normally because the cookie's stamped `e` matches the current epoch (the picker re-render only matters on no_selection / epoch_stale / removed_from_roster outcomes — `resolved` doesn't re-evaluate ambiguity).
  - **Two callbacks racing across tabs**: two browser tabs both OAuth-redirect simultaneously. Both callback handlers fire `claim_oauth_identity` with locks; one wins, one queues. Both write fill-in-the-blanks envelopes. Assert: no lost rows in the final cookie (both writes are idempotent fill-in-the-blanks).
