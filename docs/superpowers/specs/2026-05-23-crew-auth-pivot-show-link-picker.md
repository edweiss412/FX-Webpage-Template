...
4. **Cookie shape: `__Host-fxav_picker` carrying an HMAC-signed versioned JSON envelope (R36 amendment; R41-R22 millisecond precision).** Inner wire form: `{ "v": 1, "selections": { "<show_id_uuid>": { "id": "<crew_member_id_uuid>", "e": <picker_epoch_int>, "t": <unix_millis_bigint> } } }`. **R41-R22 precision change**: `t` is unix MILLISECONDS (NOT seconds). Earlier drafts used seconds, but the resolver's claimed-after-pick comparison (cookie.t vs claim_epoch) at second-resolution allowed a same-second bypass-then-claim sequence to collapse the comparison and let an impersonation cookie pass. Millisecond precision combined with the R41-R22 strict-greater bootstrap-mint contract (below) makes the comparison fail-closed. The cookie's actual value is `<base64url(payload)>.<base64url(hmac_sha256(payload, secret))>`. **R41-R20 + R41-R22 mint-path contract** — two surfaces mint a valid cookie:
   ...
   - **`selectIdentity` Server Action** (bypass-pick path): uses `select_identity_atomic.observed_at_millis` (DB-side, inside the per-show advisory lock — R41-R18). The bypass cookie's `t` reflects the moment IS-NULL was observed inside the lock, BEFORE any subsequent `claim_oauth_identity` could stamp the same row. By PostgreSQL transaction-monotonicity over a shared lock, every later `claim_oauth_identity` transaction's `claimed_via_oauth_at` is strictly later than `observed_at_millis`.
   - **`/api/auth/picker-bootstrap` Route Handler** (lazy-mint path for Google-signed-in users): uses `claim_oauth_identity.mint_safe_t_millis` returned by the RPC. The RPC computes `mint_safe_t_millis = max(floor(extract(epoch from now()) * 1000)::bigint, (select max(extract(epoch from claimed_via_oauth_at) * 1000)::bigint from public.crew_members where email = v_email)) + 1` — guaranteed strictly greater than any `claimed_via_oauth_at` on the user's rows. Bootstrap stamps cookie.t = mint_safe_t_millis directly (NOT JS Date.now(), because app-server vs DB-server clock skew could otherwise make legitimate cookies fail the strict-greater resolver check).
...
begin
  ...
  -- R41-R22: compute mint_safe_t_millis for picker-bootstrap's cookie.t.
  -- Strictly greater than any claimed_via_oauth_at on the user's rows AND
  -- strictly greater than the current transaction's now() millis. Adding 1ms
  -- guarantees `cookie.t > claim_epoch_millis` at the resolver's strict check.
...
- `select_identity_atomic(p_show_id uuid, p_crew_member_id uuid)` SECURITY DEFINER RPC under the per-show advisory lock performs ALL of: (a) `SELECT id, show_id, claimed_via_oauth_at, email FROM crew_members WHERE id = p_crew_member_id`: 0 rows → reject with `PICKER_CREW_MEMBER_NOT_FOUND`; show_id mismatch → reject with `PICKER_CREW_MEMBER_WRONG_SHOW`; **`claimed_via_oauth_at IS NOT NULL` → reject with `PICKER_IDENTITY_CLAIMED` (R41 Fix-2)**; **same-show-email-count > 1 → reject with `PICKER_IDENTITY_AMBIGUOUS` (R41-R6 Fix-3; R41-R17)**; (b) on success, returns `{ ok: true, crew_member_id, picker_epoch, observed_at_millis: floor(extract(epoch from now()) * 1000)::bigint }`. **R41-R18 CRITICAL fix + R41-R22 millisecond precision — the `observed_at_millis` field is the DB-side transaction timestamp captured INSIDE the advisory lock at MILLISECOND resolution. The Server Action MUST use this value as the cookie entry's `t` field**, NOT JavaScript's `Date.now()`. Earlier `observed_at_seconds` (second precision) allowed a same-second bypass-then-claim to tie at the resolver's `>=` check (R41-R22 finding); millisecond precision combined with the `<=` invalidation rule (per §6.1 step 9) fails closed on the residual same-millisecond case. Bypass cookies have `t = observed_at_millis` < claim's `claim_at_millis` by transaction monotonicity over the shared lock (the next claim_oauth_identity transaction on this show begins AFTER this transaction commits and releases the lock). All three layers (UI deactivation, server-side check, locked-millisecond-timestamp) remain in place.
...
- **Cookie cap (per Resolved Decision 6)**: byte-budget at 3800 bytes encoded. On every `selectIdentity` write, the encoder evicts the entry with the lowest `t` (last-touch unix-seconds) until the encoded value fits the budget. **Cookie wire shape with timestamps**:
  ...
  ```
  `t` is a unix-MILLISECOND timestamp of last touch (R41-R22 — second precision was vulnerable to same-second bypass-then-claim ties). **`t` is updated ONLY by the FOUR cookie-mutator surfaces (R41-R20 corrected)**: the three crew-side Server Actions (`selectIdentity`, `clearIdentity`, `cleanupStaleEntry`) AND the `/api/auth/picker-bootstrap` Route Handler. `selectIdentity` stamps `t` from `select_identity_atomic.observed_at_millis` (DB-side, inside the per-show advisory lock per R41-R18). `picker-bootstrap` stamps `t` from `claim_oauth_identity.mint_safe_t_millis` (DB-side; computed as `greatest(now(), max(claimed_via_oauth_at)) + 1ms` per the R41-R22 §5.3 SQL — strictly greater than any extant `claimed_via_oauth_at` on the user's rows, so the resolver's `cookie.t > claim_epoch_millis` check accepts cleanly even when bootstrap and claim land within the same millisecond). `clearIdentity` removes the entry (no `t`); `cleanupStaleEntry` removes the entry (no `t`). Reads never emit `Set-Cookie` per the R16 contract; `resetPickerEpoch` never touches the cookie per the R30 contract; `/auth/callback` never touches the cookie per R41-R6. On write of an entry that pushes the encoded cookie past the byte-budget cap, the minimum-`t` entry is evicted first.
...
- **`select_identity_atomic` locked-timestamp regression (R41-R18 CRITICAL; R41-R22 millisecond precision + strict-greater)** — fixture: Mallory attempts a bypass pick for Alice's crew row; Alice's `claimed_via_oauth_at` is currently NULL. Lock-serialized transaction interleave: T0 — Mallory's `select_identity_atomic` acquires Show-X advisory lock; T1 — Mallory's RPC reads Alice's row, sees IS NULL, captures `observed_at_millis = floor(extract(epoch from now()) * 1000)::bigint = T1_millis`; T2 — Mallory's RPC commits, lock releases; T3 — Alice signs in via Google; `claim_oauth_identity` acquires Show-X lock, stamps Alice's row's `claimed_via_oauth_at` with the new transaction's `now()` (T3_millis > T2 > T1_millis by PostgreSQL transaction monotonicity over the lock). Tests assert: (a) Mallory's cookie's `t` field equals T1_millis (DB-side, inside-lock), NOT JS Date.now(); (b) `T1_millis < T3_millis` by lock serialization; (c) on Mallory's next show request, `resolvePickerSelection` computes `cookie.t (T1_millis) <= (extract(epoch from claimed_via_oauth_at) * 1000)::bigint (T3_millis)` → returns `identity_invalidated/claimed_after_pick`; (d) Mallory's cookie rejected (API → 401; page re-prompts with CLAIMED_AFTER_PICK banner). **R41-R22 same-millisecond regression**: orchestrate Mallory's select_identity_atomic and Alice's claim_oauth_identity to land in the SAME unix millisecond (use `pg_sleep(0)` and/or rapid burst — should be possible on a fast local DB). With the `<=` invalidation rule + millisecond precision, ties go to invalidation: `cookie.t (T_ms) <= claim_epoch (T_ms)` is true → Mallory still invalidated. Without the `<=` rule (regression to `<`), ties would incorrectly resolve and Mallory's cookie would survive. The test asserts the `<=` semantics by asserting Mallory's cookie is rejected even when T1_millis == T3_millis.
