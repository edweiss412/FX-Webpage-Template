...
| `shows.share_token` | `TEXT NOT NULL UNIQUE DEFAULT encode(gen_random_bytes(32), 'base64')` (R34 amendment) | High-entropy bearer secret (256 bits, base64-encoded, ~43 chars after the `=` padding strip). Generated server-side on `shows` insert. The crew URL `/show/<slug>/<share-token>` requires the token to match. Backfill for existing rows (from M9.5 dev data, if any) generates a fresh token via a one-time UPDATE. Token rotation is NOT in scope for v1 (no admin "rotate share token" affordance) — if a token is suspected leaked outside Doug's intended audience, Doug archives the show and creates a new one. The `UNIQUE` constraint ensures lookup is by token alone (no slug + token race on writes). |
...
- **Share-token entropy meta-test (R34)** — DB-level test asserts every row in `shows` has a `share_token` with length ≥ 32 chars (post base64-encode floor). New-row insert test asserts the column is populated by the default expression without explicit assignment.
