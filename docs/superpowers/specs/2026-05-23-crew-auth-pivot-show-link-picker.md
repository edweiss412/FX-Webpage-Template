...
begin
  ...
  select coalesce(array_agg(distinct cm.show_id), array[]::uuid[])
...
- **Ambiguous-email handling (R41-R3 HIGH regression)** — DB-level test. Fixture: Show-X has TWO `crew_members` rows with the same email (Doug entered the email twice in the sheet, or a sync mistake created a duplicate). Test invokes `claim_oauth_identity(email)`. Assert: (a) the UPDATE stamps BOTH rows (the IS NULL filter stamps all matching unstamped rows; we don't suppress the stamp); (b) the returned `shows` array does NOT contain Show-X (the GROUP BY + HAVING COUNT(*) = 1 excludes it); (c) an `AMBIGUOUS_EMAIL_BINDING` row exists in `admin_alerts` carrying `{ email, show_id: Show-X }`; (d) the callback handler does NOT mint a picker cookie entry for Show-X; (e) a Google-signed-in user visiting `/show/<slug-X>/<token-X>` after this state sees the SignInOrSkipGate → picker (because no cookie for Show-X) → both ambiguous rows render as deactivated (claimed_via_oauth_at non-null on both) → tapping either redirects to `/auth/sign-in?next=...`. The user CAN navigate to the show via Skip → picker; both rows are deactivated; neither can be picked. (Resolution requires admin action to deduplicate the roster — out of pivot scope.)
