...
15. **Google sign-in is restored as an OPTIONAL crew identity layer (R41 — reverses R14/R15's deletion of `/me` + `validateGoogleSession`).** The owner re-decided on 2026-05-23 after the share-token-credential design hardened: the strict reading of "crew never sign in" was over-applied by Codex's R14 finding. The corrected reading is: **show ACCESS doesn't require sign-in — anyone with `/show/<slug>/<share-token>` reaches the picker without auth — but sign-in remains available as an opt-in upgrade that adds (a) identity exclusivity in the picker and (b) cross-show discovery via `/me`.** Net consequences this spec carries:
    ...
    - **`validateGoogleSession.ts` is preserved.** Production callers post-pivot (full structural allowlist enforced by `no-jwt-surface` meta-test §10.1): `lib/auth/picker/resolvePickerSelection.ts` (step-4 auto-resolve), `app/show/[slug]/[shareToken]/page.tsx` (calls resolvePickerSelection), `app/me/page.tsx` (cross-show discovery), `app/auth/callback/route.ts` (claim-stamp hook), and the test directory. **`/api/report` does NOT keep its M11 Google-session arm**: a Google-signed-in crew user visits the show page first (R41 step-4 mints a picker cookie automatically), then any subsequent API call carries the cookie. Removing the redundant arm from `/api/report` (and the other API consumers) is the minimum-surface-area choice and matches the §6 routing table.
...
│                                                              │
│  4. else: validateGoogleSession(req) — R41 amendment.        │
│     If session exists AND its auth.users.email matches a row │
│     in crew_members for THIS show_id, AND the show is        │
│     published:                                               │
...
if (user?.email) {
  // Service-role client; idempotent COALESCE preserves the first-stamp date.
  await serviceRole.rpc('claim_oauth_identity', { p_email: user.email });
}
```
...
as $$
  update public.crew_members
     set claimed_via_oauth_at = coalesce(claimed_via_oauth_at, now())
...
**Why COALESCE on UPDATE:** preserves the FIRST stamp date across multiple sign-ins. Useful for audit ("when did Alice first claim her identity?"). Also makes the stamp idempotent — calling the hook repeatedly with the same email is a no-op.
...
**Cross-show scope:** the UPDATE has no `show_id` filter — every `crew_members` row matching the email stamps. One sign-in claims identity globally across every show the email is on. Consistent with Decision 15's "permanent global claim."

**Race posture:** if a crew member signs in WHILE another user is mid-pick on the same show (a race between the OAuth callback's UPDATE and the picker's SELECT-then-INSERT), the worst case is that the bypass-picker user's pick succeeds (because the claim row didn't exist when their picker rendered), and the just-signed-in user sees their own identity claimed on their next picker render (which is correct — they did claim it). The race window doesn't allow a malicious bypass pick AFTER the claim commits because the picker re-reads `claimed_via_oauth_at` on every render. Acceptable trade-off.
