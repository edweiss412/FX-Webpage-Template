...
15. **Google sign-in is restored as an OPTIONAL crew identity layer (R41 — reverses R14/R15's deletion of `/me` + `validateGoogleSession`).** The owner re-decided on 2026-05-23 after the share-token-credential design hardened: the strict reading of "crew never sign in" was over-applied by Codex's R14 finding. The corrected reading is: **show ACCESS doesn't require sign-in — anyone with `/show/<slug>/<share-token>` reaches the picker without auth — but sign-in remains available as an opt-in upgrade that adds (a) identity exclusivity in the picker and (b) cross-show discovery via `/me`.** Net consequences this spec carries:
    ...
    - **`validateGoogleSession.ts` is preserved.** Production callers post-pivot (full structural allowlist enforced by `no-jwt-surface` meta-test §10.1): `lib/auth/picker/resolveShowPageAccess.ts` (the new PAGE-ROUTE-ONLY helper that contains the Google-session auto-resolve step — see Fix-3 below), `app/me/page.tsx` (cross-show discovery), `app/auth/callback/route.ts` (claim-stamp hook), and the test directory. **`resolvePickerSelection.ts` does NOT import `validateGoogleSession`** — it is the cookie-only helper called from both the page route AND every API consumer (`/api/asset/{diagram,reel,agenda}`, `/api/realtime/subscriber-token`, `/api/show/[slug]/version`, `/api/report`); putting Google auto-resolve inside it would silently authenticate API calls without a picker cookie, contradicting the route table's "picker cookie OR admin" contract. The split is enforced by the no-jwt-surface meta-test's structural allowlist (R41-R1 Fix-3). **`/api/report` does NOT keep its M11 Google-session arm**: a Google-signed-in crew user visits the show page first (R41 step-4 mints a picker cookie automatically via `resolveShowPageAccess`), then any subsequent API call carries the cookie. Removing the redundant arm from `/api/report` (and the other API consumers) is the minimum-surface-area choice and matches the §6 routing table.
...
- `lib/auth/picker/resolveShowPageAccess.ts` (new, R41 Fix-3) — **page-route-only auth chain.** Called exclusively by `app/show/[slug]/[shareToken]/page.tsx`. Encapsulates the full chain: archived → admin precedence → Google-session-matching-crew-row → existing picker cookie → unpublished. Imports `validateGoogleSession`. When the Google-session arm hits, the helper mints a fresh picker cookie via the same envelope encoder that `selectIdentity` uses (so the API consumers see an indistinguishable cookie on subsequent requests) AND returns `{ kind: 'resolved', crewMemberId, justMinted: true }` so the page route can set the Set-Cookie header. **API consumers MUST NOT import this helper** — the no-jwt-surface meta-test asserts the only consumer is the show-page route handler.
...
- ~~`lib/auth/validateGoogleSession.ts`~~ **R41 amendment: PRESERVED.** Used by the new auth-callback claim-stamp hook (§4.8), the `/show/<slug>/<share-token>` route's R41 step-4 Google-session arm, `/me`, `/api/report`'s Google-session arm, `/auth/sign-out`. R14/R15's deletion is reverted.
