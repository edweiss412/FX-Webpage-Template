...
**Same-origin / CSRF contract for picker Server Actions (R32 amendment).** All three cookie-mutating Server Actions — `selectIdentity`, `clearIdentity`, `cleanupStaleEntry` — are unauthenticated state-changing endpoints (anyone with the show-link can invoke them, by design, to set their own picker selection). To prevent cross-site forged invocation (an attacker page POSTing to the Server Action endpoint to set a victim's identity to a chosen crew member), each action MUST validate that the request is same-origin. **Mechanism**: Next 16 Server Actions enforce same-origin by default via the framework's `x-action` header check + Origin/`x-forwarded-host` reconciliation; the pivot relies on this default but pins it explicitly via the `serverActions.allowedOrigins` config in `next.config.ts` (set to exactly the canonical production origin `https://crew.fxav.show` and the dev origin `http://localhost:3000`). The spec's plan task list includes a verification step that `serverActions.allowedOrigins` is configured and that a synthetic cross-origin POST attempt to each Server Action returns 403/400 BEFORE the cookie is mutated. **Tests**: each cookie-mutator gets a negative-regression test that POSTs from a different origin (simulated via the `Origin` header) and asserts (a) no Set-Cookie is emitted, (b) the response is a same-origin denial (Next's standard rejection shape), (c) the cookie's prior value is preserved.
