...
- **Cookie cap (per Resolved Decision 6)**: byte-budget at 3800 bytes encoded. On every `selectIdentity` write, the encoder evicts the entry with the lowest `t` (last-touch unix-milliseconds per §6.0) until the encoded value fits the budget. **Cookie wire shape with timestamps**:
  ...
  ```
  `t` is a unix-MILLISECOND timestamp (13-digit; example value `1779514142000` represents 2026-05-23 — note the 13 digits, NOT 10; seconds-scale example values are FORBIDDEN per §6.0). **`t` is updated ONLY by the FOUR cookie-mutator surfaces** (per §6.0): the three crew-side Server Actions AND `/api/auth/picker-bootstrap`. `selectIdentity` stamps `t` from `select_identity_atomic.observed_at_millis` (DB-side `clock_timestamp()` after lock — per §6.0.2). `picker-bootstrap` stamps `t` from `claim_oauth_identity.mint_safe_t_millis` (computed as `greatest(clock_timestamp_millis, max(claimed_via_oauth_at_millis)) + 1` per §6.0.2 — `clock_timestamp()` NOT `now()`; the `now()` form would predate lock acquisition under contention per the R41-R23 finding). `clearIdentity`/`cleanupStaleEntry` remove the entry (no `t`). Reads never emit `Set-Cookie`; `resetPickerEpoch`/`/auth/callback` never touch the cookie. See §6.0 for the canonical timestamp-defense contract.
...
- **Rotate share-token action (R39/R40 amendment; R41-R28 admin-gate defense-in-depth)** — `tests/auth/_metaInfraContract.test.ts` registers `rotateShareToken`; `tests/auth/advisoryLockRpcDeadlock.test.ts` asserts `rotate_show_share_token` acquires the lock at exactly one layer inside its SECURITY DEFINER body. Functional tests: (a) admin clicks Rotate → response carries new `share_token` matching `^[0-9a-f]{64}$` AND new `picker_epoch` integer; (b) the OLD URL `/show/<slug>/<old-token>` now returns 404; (c) the NEW URL succeeds; (d) **a picker cookie minted BEFORE rotation is rejected on every API consumer post-rotation** — the cookie's `e` ≠ new `picker_epoch` → 401 (R40 critical fix); (e) non-admin caller via the Server Action → 401/403 (JS-side requireAdmin gate); (f) **R41-R28 direct-RPC regression (NEW vector)**: invoke `rotate_show_share_token` directly via PostgREST as an authenticated non-admin user (bypassing the JS Server Action wrapper). Assert: (i) the RPC raises an error with SQLSTATE 42501 (insufficient_privilege) — the in-function `if not public.is_admin() then raise exception ... using errcode = '42501'` gate fires; (ii) `shows.share_token` is UNCHANGED post-call (byte-identical to pre-call); (iii) `shows.picker_epoch` is UNCHANGED; (iv) no `publish_show_invalidation` was triggered. Without the in-function gate, the JS-side requireAdmin would be the only barrier and any authenticated user could rotate any show's share-token via direct PostgREST call. This regression mirrors the reset_picker_epoch_atomic direct-RPC test pattern; the structural meta-test grep-asserts EVERY SECURITY DEFINER RPC that mutates `shows` includes the `public.is_admin()` gate (rotate_show_share_token, reset_picker_epoch_atomic).
