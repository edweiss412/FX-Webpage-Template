...
│     show's roster; they get the picker interstitial like     │
│     anyone else).                                            │
│                                                              │
│  5. else if !published:                                      │
...
- `app/api/auth/picker-bootstrap/route.ts` (new, R41-R2 amendment; R41-R3 revised to atomic claim-and-mint). Route Handler that legally mints `__Host-fxav_picker` for a Google-signed-in user. **Flow (R41-R3 redrives via the same `claim_oauth_identity` RPC the callback uses, so newly-added rows + callback-fault recovery + duplicate-email handling are uniform across both surfaces and DO NOT loop):** (1) read `next` from query params, validate against the same `validateNextParam.ts` allowlist as `/auth/sign-in`; (2) `validateGoogleSession(req)` — no session → 302 to `next` with NO cookie set (the page will render its non-Google fallback chain on the next hop); (3) invoke `claim_oauth_identity(p_email = auth.users.email)` via the service-role client — this stamps `claimed_via_oauth_at` on every matching row under per-show advisory locks AND returns the `{ claimed_count, shows }` set, identical to the callback's RPC call. The RPC handles both newly-added rows (stamps them this round) and infra-error retry semantics (idempotent — repeat-safe); (4) build/update the picker envelope from `result.shows` (one entry per show in the result), preserving any existing non-Google entries (i.e. selections made via `selectIdentity` for shows the user is not on remain untouched); (5) write the envelope via `cookies().set('__Host-fxav_picker', signEnvelope(envelope), PICKER_COOKIE_OPTIONS)`; (6) 302 to `next`. **If the show in `next`'s slug is NOT in `result.shows`** (Google-signed-in user not on this show's roster at all), the redirect proceeds with NO entry for that show in the envelope — the page route then renders the SignInOrSkipGate naturally on the follow-up hop. **No redirect loop is possible**: every invocation of picker-bootstrap that reaches step 3 stamps any unstamped matching rows AND populates the envelope with every show the user is on; the page route's step 4 condition (`Google session + matching row + no cookie entry for this show`) cannot persist across the round-trip because picker-bootstrap either (a) added the entry (page falls through to step 6 next hop), OR (b) the show isn't in `result.shows` and the page won't trigger another bootstrap (no matching crew row means step 4 falls through to step 5). **Duplicate-email handling**: if `result.shows` contains 0 or 1 rows per `show_id` (the RPC enforces this via GROUP BY + HAVING COUNT(*) = 1 — see §5.3 R41-R3 amendment), the envelope mints normally; shows with ambiguous email (2+ matching rows) are OMITTED from `result.shows` AND an `AMBIGUOUS_EMAIL_BINDING` admin alert is emitted from the RPC body (one alert per ambiguous show). **This Route Handler IS in the cookie-mutator allowlist** alongside `selectIdentity`, `clearIdentity`, `cleanupStaleEntry`, and `/auth/callback/route.ts`; the §10.1 picker-cookie-contract meta-test is extended to include it. The handler is rate-limited at the route level (DB write happens via the locked RPC; cookie write happens once per request; no user-controlled write beyond the validated `next` URL).
