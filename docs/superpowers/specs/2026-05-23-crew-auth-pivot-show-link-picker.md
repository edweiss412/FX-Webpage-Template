...
- `lib/cache/showSlugMap.ts` (new) — process-local `Map<string, string>` slug → show UUID lookup with Realtime-driven invalidation. Used by middleware to resolve slug routes (`/show/<slug>`, `/api/show/<slug>/version`) to UUID for cookie refresh. See §4.9 mechanism A for full contract.
...
2. Resolves the request's `show_id` UUID via one of two paths, depending on URL shape:
   ...
   - **Slug-in-path routes** (`/show/<slug>`, `/api/show/<slug>/version`): resolve via the project's `slugToShowId(slug)` helper backed by `lib/cache/showSlugMap.ts` (new). The helper maintains a `Map<slug, showId>` populated lazily on first request per Vercel function instance, refreshed via the existing Realtime invalidation channel (subscribes to `show:*:invalidation` and patches the map on every event). Cache-miss falls through to a single bound `SELECT id FROM shows WHERE slug = $1` and populates the cache. Lookup-failure (DB unreachable, slug not found) → middleware silently skips refresh; the request proceeds to the route handler which has its own auth chain.
...
**`lib/cache/showSlugMap.ts` contract (new).** Process-local in-memory `Map<string, string>` (slug → show UUID). API:
- `slugToShowId(slug: string): Promise<string | null>` — returns cached value, or runs the bound SELECT on miss, or returns `null` on lookup failure. Returns `null`, never throws.
- `invalidateSlug(slug: string): void` — clears one entry.
- Realtime subscriber registered at module-load time subscribes to `show:*:invalidation` (the existing channel) and calls `invalidateSlug` on any payload. **Bounded staleness**: cached entries are valid until invalidated; brand-new shows miss the cache and pay one SELECT round-trip. Stale slugs (e.g., a slug that was renamed in the sheet) get invalidated on the next sync write that touches `shows.slug`. Slug changes in the corpus are rare per the parent spec's slug-derivation rules.
...
- **Middleware refresh — slug routes (cache hit)** — request to `/show/<slug>` with a populated `showSlugMap` cache emits a refreshed cookie for the entry under the resolved UUID. Asserts the `slugToShowId` call observed a cached lookup, not a DB SELECT.
- **Middleware refresh — slug routes (cache miss)** — first request to `/show/<slug>` per process triggers a `SELECT id FROM shows WHERE slug = $1` and populates the cache, then refreshes the cookie. Subsequent requests are cache-hit.
- **Middleware refresh — slug lookup failure** — `showSlugMap` returns `null` (unknown slug OR DB unreachable). Middleware skips refresh; no Set-Cookie header; request proceeds to the route handler.
- **`showSlugMap` Realtime invalidation** — synthetic `show:*:invalidation` broadcast invalidates the corresponding slug entry; next access re-fetches.
