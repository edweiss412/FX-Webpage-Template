...
15. **`validateGoogleSession`'s crew-email path is removed entirely; crew Google sign-in is dead.** The pre-pivot chain (`lib/auth/resolveShowViewer.ts:65-193`) ran `validateLinkSession` then `validateGoogleSession`. Post-pivot, the chain is `isAdminSession` then `resolvePickerSelection` — there is no Google session arm for crew, and no path anywhere in the app derives a crew identity from a Google session. Rationale: the owner determination says "crew never sign in" (`PRODUCT.md:73`) and the brief's out-of-scope list explicitly retires "SSO / OAuth crew identity." Two structural consequences this spec adds:
    ...
    - `validateGoogleSession.ts` itself stays in the repo because `/auth/sign-out` calls one of its helpers (the Supabase Auth session clear), but no consumer derives crew identity from it. The `no-jwt-surface` meta-test in §10.1 enforces this by restricting `validateGoogleSession` imports to the explicit preserved-allowlist (`app/auth/sign-out/route.ts`, the tests directory). `/me` and `/api/me/**` ARE deleted, removing them from the allowlist too. The `kind: 'crew_google'` arm of the `ShowViewer` discriminated union (`resolveShowViewer.ts:48-52`) is removed in the same edit.
...
- `lib/cache/showSlugMap.ts` (new) — bounded per-request slug → show UUID lookup. **NO process-local cache** in v1 (an earlier draft proposed a Realtime-invalidated cache; rejected in R6 because the existing Realtime payload carries `{ show_id, version_token }` only, no slug, and module-level wildcard `show:*:invalidation` subscription isn't a documented contract). Used by middleware to resolve slug routes (`/show/<slug>`, `/api/show/<slug>/version`) to UUID for cookie refresh. See §4.9 mechanism A for full contract.
...
2. Resolves the request's `show_id` UUID via one of two paths, depending on URL shape:
   - **UUID-in-path routes** (`/api/asset/{diagram,reel,agenda}/<uuid>/...`): use the in-URL UUID directly. No lookup needed.
   - **Slug-in-path routes** (`/show/<slug>`, `/api/show/<slug>/version`): resolve via the `slugToShowId(slug)` helper that runs a single bound `SELECT id FROM shows WHERE slug = $1` per middleware invocation. **No process-local cache in v1** — the existing Realtime invalidation contract doesn't carry slug, so we can't safely cache without inventing a new payload schema. The `slug` column is `UNIQUE` indexed (per `20260501000000_initial_public_schema.sql:6`); the lookup is a sub-millisecond PK-equivalent on Vercel-class infrastructure. Per-request cost is acceptable for the refresh path's traffic profile. Lookup-failure (DB unreachable, slug not found) → middleware silently skips refresh; the request proceeds to the route handler which has its own auth chain. If profiling shows the lookup is a measurable hot spot in production, a follow-up adds a short-TTL (60s) in-process cache with NO Realtime invalidation — slug renames are rare enough that 60s staleness in the refresh layer is acceptable, and the resolver inside the route handlers always re-reads canonical state.
   - **Body-only routes** (`/api/realtime/subscriber-token`, `/api/report`): the show_id is in the request body, NOT in the URL. Middleware cannot parse the body without consuming the stream. For these routes, middleware does NOT refresh; refresh happens implicitly when the user next visits the page route. **Trade-off acknowledged**: a crew member who only ever calls the subscriber-token endpoint (impossible in practice — bridge mounts only AFTER page render) would not get a TTL bump. The page render that hosts the bridge IS in the refresh scope, so the cookie's `t` is bumped on page load.
3. If decode succeeds AND the cookie contains an entry for the resolved `show_id`, bump that entry's `t` to the current unix-seconds and re-emit the cookie with `Max-Age=7776000`.
4. Decode failures, missing-entry, and stale-epoch all pass through untouched — the middleware does NOT validate against the DB; that's the resolver's job in the Server Component. The middleware is a pure refresh, not an enforcement layer.

**`lib/cache/showSlugMap.ts` contract (new).** Despite the filename, **v1 ships with no cache** — the module exposes only the bound SELECT helper. API:
- `slugToShowId(slug: string): Promise<string | null>` — runs a single bound `SELECT id FROM shows WHERE slug = $1`. Returns `null` on lookup failure (DB error, unknown slug) — never throws. The file name retains "Map" / "cache" semantics so a future follow-up can layer a TTL cache here without renaming the import surface across the codebase.
...
- **Middleware refresh — UUID routes** — request to `/api/asset/diagram/<show-uuid>/...` with a cookie entry for that UUID emits a Set-Cookie response header with bumped `t` and refreshed `Max-Age`. Repeated for reel + agenda.
- **Middleware refresh — slug routes (lookup succeeds)** — request to `/show/<slug>` emits a refreshed cookie for the entry under the resolved UUID. Asserts a single DB SELECT was issued per request (no module-level cache in v1).
- **Middleware refresh — slug lookup failure** — `slugToShowId` returns `null` (unknown slug OR DB unreachable). Middleware skips refresh; no Set-Cookie header; request proceeds to the route handler.
- **Cross-route cookie consistency** — request to `/show/<other-slug>` does NOT touch the cookie's entry for this slug. Request without a cookie produces no Set-Cookie header.
