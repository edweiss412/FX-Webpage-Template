...
3. **No `from('shows').update(...)`, `.insert(...)`, `.delete(...)`, or `.upsert(...)` call appears anywhere in `app/**`, `lib/**`, `components/**` — no service-role exception.** Post-pivot, every `public.shows` mutation goes through a SECURITY DEFINER RPC invoked via `.rpc(...)`. The R10/R11 audit confirmed zero current direct-DML callers; this rule keeps the surface clean as the codebase evolves. If a future feature legitimately needs to mutate `shows` from app code, the contract is: add a new SECURITY DEFINER RPC + advisory-lock topology coverage in `tests/auth/advisoryLockRpcDeadlock.test.ts`, then call the RPC from app code — never directly `.from('shows').update(...)`.
4. The static grep ALSO bans the substring `picker_epoch` and `picker_epoch_bumped_at` from any source under `app/**`, `lib/**`, `components/**` EXCEPT (a) inside `.rpc('reset_picker_epoch_atomic', ...)` call sites, (b) inside test fixtures, (c) inside docs. Catches the scenario where someone writes a SECURITY DEFINER RPC that touches these columns without proper advisory-lock coverage.
