...
- `tests/e2e/published-review-modal.deeplink.spec.ts` — new test for the end-to-end invariant: `goto /admin?show=<slug>`, wait for a `role="dialog"` frame AND poll until `document.activeElement` sits inside the dialog panel (initial focus applied ⇒ the frame's effects — including the shell's Esc listener, installed in a passive effect at `ReviewModalShell.tsx:229-237` — have flushed; both frames focus their X post-fix; same effect-flush discipline as `published-review-modal.interactions.spec.ts:103-120`, R3 F1), then press Esc once; assert eventual: zero modal frames, `?show` stripped from the URL, body scroll unlocked, `[data-inert-root]` un-inerted. Post-fix this is deterministic from EITHER frame (skeleton navs at dismiss-commit; loaded modal's `handleClose` navs at exit-end). Pre-fix it fails whenever Esc lands in the fallback window; it is a real-browser wedge-regression test, not the primary red/green proof.
