...
- **Known-gap fixture (documents the residual, does not assert closure):** a merged-cell overflow that shifts the surname token (driver stored as `"Doug Larson Loadout"` — the adjacent column fused onto the name — viewer `"Doug Larson"`) → **not visible** under current `namesRefer` (verified live: the multi-token rule compares last tokens `"loadout"` vs `"larson"`, `nameMatch.ts:50-53`). This fixture is asserted as the *current* behavior and annotated as the `BL-TRANSPORT-ID-RESOLUTION` residual, so the test file itself records what 8.3 must fix rather than pretending the case is handled. (An earlier draft used `"DougLarsonHotelBallroom"`, which `namesRefer` actually matches TRUE via single-token prefix — so it is NOT a gap; the corrected fixture is a genuine miss.)
