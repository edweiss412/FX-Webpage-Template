...
**Primary (guided path) — THREE detection points, one destination.** A resolved crew viewer's id can fail to match at three progressively-later moments inside the `resolved` case; all three route to the **same** guided re-pick (`loadRoster` → sanitized roster → `<PickerInterstitial banner="PICKER_REMOVED_FROM_ROSTER_BANNER" …/>`), exactly as the existing `removed_from_roster` case does (`page.tsx:234-263`):

- **Point A — pre-projection throw (the race Codex round-1 flagged).** `getShowForViewer` performs its crew id+show lookup FIRST (`getShowForViewer.ts:287-301`, before building the roster projection at `:395`). If the crew row was deleted between `resolveShowPageAccess` returning `resolved` and this lookup, `:301` throws. Today `page.tsx:158-167` catches **all** `getShowForViewer` errors as `<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" />` — so this legit-but-raced crew member lands on a dead end, not the audit's guided re-pick. **Fix:** make **only** the `:301` crew-row-miss throw a distinct typed error `CrewMemberNotInShowError extends Error` whose `.message` stays the literal `"PICKER_CREW_MEMBER_WRONG_SHOW"` (back-compat, see below). `page.tsx`'s `resolved`-case catch discriminates: `if (err instanceof CrewMemberNotInShowError)` → guided re-pick; **else** → `<TerminalFailure … />` unchanged.
- **Point B — post-projection guard.** If the id survives `:301` but is absent from the full-roster projection built at `getShowForViewer.ts:395-401` (a race between those two reads), `getShowForViewer` returns a well-formed `crewMembers` array missing the id. The page already computes `const crew = data.crewMembers?.find((c) => c.id === result.crewMemberId)` for the identity chip (`page.tsx:177`). New guard: `Array.isArray(data.crewMembers) && crew === undefined` → guided re-pick (not `CrewShell`). Non-array (malformed) falls through to `CrewShell`'s `MalformedProjectionError` arm, unchanged.
...
- **8.2 Point A routing:** assert `page.tsx`'s `resolved`-case handling routes `CrewMemberNotInShowError` → picker re-pick (`PickerInterstitial` + `PICKER_REMOVED_FROM_ROSTER_BANNER`) and every other `getShowForViewer` error → `TerminalFailure`. Since `page.tsx` is a Server Component, the plan extracts the catch's decision into a pure, testable classifier (input: caught error; output: `"repick" | "terminal"`) and unit-tests that; a source-grep meta-assert confirms the `resolved` catch calls the classifier.
