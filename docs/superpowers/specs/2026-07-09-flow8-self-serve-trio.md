...
Because the re-pick render itself performs a **second async load** (`loadRoster`, which can fail on infra), the re-pick path is extracted into **one shared helper** — `renderPickerRepick({ showId, slug, shareToken, s })` — that encapsulates the exact fail-closed contract the existing `removed_from_roster` / `epoch_stale` arms already use (`page.tsx:237-262`): `try { roster = await loadRoster(showId) } catch { return <TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" retryHref={…} /> }`, then `return <PickerInterstitial roster={roster} banner="PICKER_REMOVED_FROM_ROSTER_BANNER" staleCleanupHint={null} s={allowlistedS} … />`. (`loadRoster` already applies `sanitizePickerRoster` per D6, so the helper does not re-sanitize.) Points A and B call this helper; the existing `removed_from_roster` case is refactored to call it too (single source of the loadRoster-failure contract, so the new paths cannot escape to Next's generic error boundary — invariant per `page.tsx:31` "every external data load … "). The helper NEVER re-throws: roster-load failure is contained to `TerminalFailure`, never Next's generic boundary.

- **Point A — pre-projection throw (the race Codex round-1 flagged).** `getShowForViewer` performs its crew id+show lookup FIRST (`getShowForViewer.ts:287-301`, before building the roster projection at `:395`). If the crew row was deleted between `resolveShowPageAccess` returning `resolved` and this lookup, `:301` throws. Today `page.tsx:158-167` catches **all** `getShowForViewer` errors as `<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" />` — so this legit-but-raced crew member lands on a dead end, not the audit's guided re-pick. **Fix:** make **only** the `:301` crew-row-miss throw a distinct typed error `CrewMemberNotInShowError extends Error` whose `.message` stays the literal `"PICKER_CREW_MEMBER_WRONG_SHOW"` (back-compat, see below). `page.tsx`'s `resolved`-case catch discriminates: `if (err instanceof CrewMemberNotInShowError)` → `return renderPickerRepick(...)`; **else** → `<TerminalFailure … />` unchanged.
...
- **Point C — render-layer backstop.** `resolveViewerContext` throws `UnmatchedViewerError` for the same unmatched-in-well-formed-array state; `_CrewShell.tsx:211-219` catches it → `<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" retryHref={/show/${slug}/${shareToken}} />`. Unreachable in the happy path (Points A+B route to re-pick before `CrewShell` renders); it exists purely so no future caller of the pure helper can reintroduce the fail-open. A retry re-hits `page.tsx` and re-enters Points A/B.
...
- **8.2 Point A routing:** assert `page.tsx`'s `resolved`-case handling routes `CrewMemberNotInShowError` → the `renderPickerRepick` path and every other `getShowForViewer` error → `TerminalFailure`. Since `page.tsx` is a Server Component, the plan extracts the catch's decision into a pure, testable classifier (input: caught error; output: `"repick" | "terminal"`) and unit-tests that; a source-grep meta-assert confirms the `resolved` catch calls the classifier + `renderPickerRepick`.
...
- **8.2 Point B:** unit/integration test that a well-formed `crewMembers` array missing the resolved id (id survived `:301` but absent from the `:395` projection) routes to the picker re-pick, not `CrewShell`, via the same extracted guard.
...
- **8.2 fail-open reachability:** three race-window detection points (A pre-projection `:301`, B post-projection `:395`, C render backstop) all route unmatched crew to the guided re-pick; `:317`/`:321` (show-race) deliberately stay on `TerminalFailure`. Do not relitigate "is this reachable" — fail-closed + guided-repick is correct posture either way.
