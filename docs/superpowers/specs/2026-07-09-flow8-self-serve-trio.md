...
**Durable tracking is a BLOCKING deliverable of THIS PR (Round-10).** Because this milestone leaves the audited transport failure mode open, the `BACKLOG.md` entry `BL-TRANSPORT-ID-RESOLUTION` (describing the residual "a hard driver-name mis-parse hides the driver's own itinerary" + its dependency on the 8.3 enrich spec + the known-gap fixture reference) MUST be **created in this branch** — it is a required plan task, not an aspiration. The residual is only "tracked" once that durable row exists outside this spec's prose; a reviewer may treat a missing `BACKLOG.md` row as an unmet deliverable. (Prior to this PR, `rg BL-TRANSPORT-ID-RESOLUTION BACKLOG.md` returns nothing — the plan's first 8.4 task adds it.)
