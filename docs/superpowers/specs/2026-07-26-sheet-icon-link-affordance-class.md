...
10. **Adoption catcher (Codex r1 finding 3; predicate corrected per r2 finding 1 — the exactly-one-file form could never go green, since sites C and the agenda link legitimately keep the phrase):** a structural containment guard walking every source-extension file in the repository (`.ts`/`.tsx`/`.js`/`.jsx`/`.mjs`/`.cjs`/`.mts`/`.cts`/`.mdx`; filesystem walk, not a named list — widened from `components/**/*.tsx` to the tsconfig compile surface by whole-diff r9, then to the full tree by r10, after include-glob-derived sets twice proved narrower than what Next compiles; only an enumerated set of repo-root artifact dirs stays out — node_modules, coverage, test-results, playwright-report, .git, .next, matched at depth zero only since r12 so a nested `app/coverage/` stays visible, and each skip lapsing whenever git tracks ANYTHING inside the dir (r13 — a committed `coverage/sheetAlias.tsx` is importable first-party source, so the name alone earns nothing); dot-directories are otherwise walked since r11 — plus the tsconfig-`exclude`d trees, with the exclude list VALIDATED live rather than trusted (r12): an entry the walk cannot soundly skip, meaning anything other than node_modules, `.next*`, or a tests/-rooted tree whose import channel the tests/-import ban closes, fails the guard loudly; the non-tests entries additionally lose their skip if git tracks files under them, r13) asserting **per-file occurrence counts of the literal `Open the source sheet` as set-equality**: components/admin/SheetIconLink.tsx = 2 (subject + fallback literals), `components/admin/wizard/Step3SheetCard.tsx` = 1 (§1.5 text link), `components/admin/wizard/step3ReviewSections.tsx` = 1 (§1.11 agenda text link — the header-block occurrences at `components/admin/wizard/step3ReviewSections.tsx:989-1000` die with the A swap), every other file = 0 — except test files quoting the phrase as assertion material, which pin per-file counts in the same set-equality (r9: the walk covers tests/, so a new quoting assertion legitimately bumps its own row). Count-pinning keeps the guard live even inside allowlisted files: a re-inlined anchor anywhere bumps a count and fails. An escape-spelled phrase in any string-like literal — cooked text containing the phrase while the raw spelling does not — is flagged outright (r11: the raw count shares the escape blindness the identifier prefilter had, and no sanctioned use is escape-spelled; substitution-split templates remain runtime construction, out of static scope). Consequence for D: the label-quoting comment at `Step3ReviewModal.tsx:27` is reworded during the D adoption (meaning kept, literal dropped — §4.3). Red until the last site adopts; lands with the final adoption task. **Call-site className contract (whole-diff r5 amendment; TS-AST since r7; DEFAULT-DENY since r8; walk, sanction, and prefilter amendments r9–r12):** the same guard file asserts, by TypeScript-AST parse of walked source files, that every reference to the `SheetIconLink` identifier outside the component's own file is one of exactly three sanctioned shapes — an un-renamed `import { SheetIconLink }` specifier; a JSX tag whose attributes carry no spread and whose `className`, when present, is a plain string literal with every token in the closed positional allowlist (responsive-prefixed non-negative margin 0-3 in half steps, and order utilities); or a `typeof SheetIconLink` type query (r9 — type space is erased at compile, no runtime alias comes out of it). ANY other reference (assignment, destructuring, parameter default, object property, default export, export clause, property access, …) fails, and so does any non-sanctioned touch of the component module path in ANY specifier-bearing form — import/export declarations, `import()`, `require()`, `import =` — with the specifier compared after stripping a resolvable extension and a trailing `/index` (r10). The parse gate uses contiguous-fragment prefilters only: a file is parsed if its raw text contains the identifier, ANY backslash (r11 — escapes normalize in the AST and every hiding spelling — `\u`/`\x`, identity escapes, line continuations — shares that single character; the r10 enumerated form list was shown incomplete), or — outside the tests/-import-exempt trees — the fragment `tests/`. Files under the repo-root `tests/` tree (root-anchored, r10 — a segment match would hand the carve to a real `app/tests/` route) relax the at-tag attribute contract ONLY; identifier and module-path rules bind everywhere. Complementing the carve, importing any module under `tests/` in any specifier form is denied EVERYWHERE outside `tests/` itself and the `scripts/` tooling tree — including the BARE root-relative spelling `"tests/…"` a tsconfig `baseUrl` would legalize (r13) — (r10 bound only the shipped trees `app/`/`components/`/`lib/`/root; r11 widened the ban after an unclassified tree — a root `fixtures/` bridge — was shown able to re-export a carved off-contract wrapper for a shipped consumer that mentions neither the identifier nor the module path). The resolver surface the shipped bundle honours is pinned closed (r11; AST-scanned r12): tsconfig `compilerOptions.paths` must equal exactly `{"@/*": ["./*"]}` with `baseUrl` absent (r13 — baseUrl legalizes bare root-relative specifiers outside every recognized shape), package.json must carry no `imports`, `exports`, or `browser` field (r13 — `exports` enables self-referencing package imports that remap to any file), and no first-party module REACHABLE from next.config.ts (BFS over relative/`@/` imports, r13 — a helper can hold the alias literal the entry file merely spreads) may contain a cooked spelling of `resolveAlias`/`alias` or a statically unclearable computed element access. The reachability collector is FAIL-CLOSED (r14): every specifier-bearing form is collected (import/export declarations, `import =`, `import()`, bare `require()`, and any `<expr>.require()` property call); dynamic specifiers, local specifiers that do not resolve to a scannable file (including directory `package.json#main` shapes), and `module`/`node:module` (`createRequire`) in config space are offenses outright, and a reached `.json` file is scanned for alias-spelled keys. Bare specifiers stay node_modules territory because package.json dependency fields are pinned free of `file:`/`link:`/`workspace:` protocols and pnpm-workspace.yaml declares no `packages:` list (r14) — a config file cannot mint a specifier the module rule does not recognize. Static alias creation therefore requires the identifier, the module path, or a resolver-level alias, and all three are denied, so the consumer-bypass class is closed by construction, not enumeration; runtime indirection (computed specifiers, `React.createElement`) is out of a static guard's reach and explicitly out of scope. Negative plants pin every form. **Whole-diff r15–r21 harden the same denial surfaces:** the require CALLEE is normalized — bare identifier, any property access named `require`, string-keyed element access, each behind runtime-transparent wrappers (parens, `as`, `satisfies`, non-null, angle-bracket assertions) — and so is the specifier ARGUMENT, which may be a no-substitution template (r15/r16); specifiers are compared after stripping a webpack resource query/fragment (r16); AMD `require([deps])`/`define(…, [deps])` array elements are specifiers too, and context modules (`require.context`, `import.meta.webpackContext`) are denied as a mechanism — pattern selection never spells a path — with the contiguous `context` fragment joining the parse-gate prefilters (r17). The non-package specifier lane is classified, not trusted: `data:`/`file:`/protocol/absolute/`#`-subpath specifiers are offenses in config space, and in the live scan an unmodeled (absolute/URL/`..`-escaping) specifier carrying a `tests/` segment falls to the tests/-import ban (r15). The resolver pin covers the full remap surface: one `REMAP_SPELLINGS` set — the alias keys, the `webpack` hook key itself, `fallback`, `plugins`, `modules`, `extensionAlias`, `descriptionFiles`, `NormalModuleReplacementPlugin`, the resolution-input redirects `tsconfigPath`, `externalDir`, `modularizeImports`, `root`, and the extension redirects `resolveExtensions` (turbopack REPLACES the candidate-extension list, so a crafted entry lands an innocent-looking specifier on the component file) with its webpack twin `extensions` (r22) and the MDX loader's `extension` option (r23 — `createMDX({ extension: /\.mdx?$/ })` would compile tracked `.md`, a tree the walk treats as prose, into importable/page source) — feeds both the code lane and the reachable-JSON key scan (r16/r18/r20/r22/r23); the compiled-PAGE surface is pinned by exact AST equality of `pageExtensions` to `["ts", "tsx", "mdx"]` — GRAPH-WIDE since r24: every reachable TS/JS module is parsed, exactly one such property may exist and it lives in the entry file, while a `pageExtensions` key in reachable JSON is an offense in the JSON lane (spread-fodder; the entry property is the one sanctioned definition site); r25 hardens the pin against non-PropertyAssignment spellings — the graph-wide count of raw `pageExtensions` TOKENS (identifiers and string literals, escapes cooked) must equal exactly one, the pinned property's own name, so shorthand properties, string-literal computed keys, getters, later dot/bracket assignments, and `delete` all fail without the guard modeling object semantics, and a NON-literal computed property name anywhere in the config graph is a statically-unclearable offense outright (the object-literal twin of the r12 computed-element-access rule) — plus a zero-tracked-`.md`-under-`app/` tripwire (r23); resolution APIs are specifier-bearing (r24 — `require.resolve`/`resolveWeak` in property or string-keyed element spelling on any require callee, and `import.meta.resolve`, surface their literal argument to the same module-path denial in the live scan and the config collector alike, closing the `require(require.resolve(lit))` one-hop composition); the tracked SYMLINK set is pinned by exact equality (r29 — a tracked extensionless symlink is a filesystem-level alias no lane reads; both current entries are documentation/fixture `.md` files); every tracked REGULAR file must carry a modeled extension — a walked source extension or a non-executable type no pinned-config bundler resolves as code without a denied rules/loader config — with extensionless files pinned to the exact dotfile set (r30 — closes the tracked extensionless/unknown-suffix bridge module the walk never reads) and string-literal binding spellings of the identifier (`import { "SheetIconLink" as X }` / `export { X as "SheetIconLink" }`) are denied in import/export-specifier positions (r29 — they produce no Identifier node; assertion prose quoting the name stays clean); the config graph's bare-import set is pinned to exactly `["@next/mdx", "@sentry/nextjs", "next"]` (r19); the tracked config entry is pinned to exactly `next.config.ts` (r21 — Next prefers a `.js`/`.mjs` entry when one exists); the tracked manifest set is pinned to exactly the root `package.json` (r17 — package scope is per-directory, and a nested manifest carries its own `imports`/`exports`/`browser`); and the pnpm resolution controls — root `resolutions`/`overrides`/`pnpm.*` keys, workspace top-level keys beyond `allowBuilds`, pnpmfile hooks (r20), and tracked npmrc files (r22 — an `.npmrc` `pnpmfile=`/`global-pnpmfile=` entry points the install-time hook at any filename the tracked-pnpmfile glob never sees) — are pinned absent. Pinned-dependency CONTENT is ratified supply-chain trust (lockfile + dependency review own upgrades); with the substitution channels closed, no first-party mechanism silently replaces those bytes. The className allowlist is horizontal-only — `m[lr]` margins plus order utilities (r17: the overlay's 44px vertical floor is exactly met, so any `mt-*`/`mb-*` shifts it a pixel outside a 44px row while rect-size assertions stay green) — and the guard pins the direct-adopter file set by equality (r16: consuming-context geometry is unverifiable in a static AST guard, so adoption itself is the reviewable event; the per-site rect-intersection e2e and the invariant-8 impeccable gate own the geometry, and a new adopter registers its row alongside both).
