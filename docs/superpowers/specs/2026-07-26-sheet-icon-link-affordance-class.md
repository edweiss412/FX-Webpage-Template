...
6. **`duration-fast` is retained on the new component.** At ratification the token emitted no CSS repo-wide (Tailwind v4 reads `--transition-duration-*`; this repo defined only `--duration-fast`) — a known, class-wide defect across ~89 files with its own fix shape, deliberately not repaired here. SINCE SUPERSEDED (r33 sync): PR #632 (`fix/duration-tokens-emit-no-css`, merged to main 2026-07-27 and merged forward into this branch) added the `--transition-duration-*` aliases, so `duration-fast` now emits real CSS and the retained class transitions colours over 120ms (0s under reduced motion). The DECISION this item ratifies — keep the token, parity with every sibling — is unchanged; only the "emits nothing" datum aged out.
...
| Pair | Treatment |
| --- | --- |
| idle ↔ hover | colour only, `transition-colors duration-fast` (§1.6, r33 sync: 120ms since PR #632's alias fix; 0s under reduced motion; was effectively instant while the token emitted nothing) |
