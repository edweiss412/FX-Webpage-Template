...
7. **Edge-complete bleed probes** (real browser, 1px-inside-the-neighbour convention per `section-header-layout.layout.spec.ts:822-833`). Two rules govern every probe (Codex r2 finding 2): the target is the nearest CONTENT box (count node, chip, pill — interactivity irrelevant, §5.1), and each probe carries a **saturation precondition** asserting the neighbour's link-side edge actually lies within the OLD overlay's reach in the red configuration — a probe whose neighbour sits beyond reach is vacuous and must fail loudly, not pass silently. Per site:
   - **A, below `sm`:** the link-side neighbour is the COUNT node when a count renders (`step3ReviewSections.tsx:970-976` — the count is the last box before the link), the heading otherwise. The existing matrix cells leave slack at short names, so a **saturated-name cell is added to `_sectionHeaderCellHarness.tsx`** (long name + count filling the row at 320/375); probe 1px inside the count box (red today: 12px reach vs 10px gap). Vertical: 1px inside the line-2 pill's top edge at the link's centre-x (pill on its own line — `step3ReviewSections.tsx:1014-1027`) → resolves to the pill; documents the 44px line-1 floor's containment.
   - **A, `sm`+:** pill-side (existing `tests/e2e/section-header-layout.layout.spec.ts:809-832` case, re-derived for the 10px reach); plus the no-pill saturated case, where the link's left neighbour is the count/heading directly.
   - **B and D (saturated title fixtures — the long-title precedent at `tests/e2e/step3-review-modal.layout.spec.ts:613-639`; a short title leaves the cluster beyond reach and the action-side probes vacuous):** title-side (1px inside the title box); subline 1px below the row at the link's centre-x → resolves to subline text; D's eyebrow 1px above; actions-side 1px inside the cluster's FIRST content box (D: state chip `Step3ReviewModal.tsx:443-465`; B: state pill `PublishedReviewModal.tsx:879-901`) at the link's centre-y.
   - Sides without any neighbouring box (A's right edge) are documented as such in the test.
