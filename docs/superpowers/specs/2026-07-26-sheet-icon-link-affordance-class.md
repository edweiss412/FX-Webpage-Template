...
10. **Adoption catcher (Codex r1 finding 3; predicate corrected per r2 finding 1 — the exactly-one-file form could never go green, since sites C and the agenda link legitimately keep the phrase):** a structural containment guard walking `components/**/*.tsx` (filesystem walk, not a named list) asserting **per-file occurrence counts of the literal `Open the source sheet` as set-equality**: components/admin/SheetIconLink.tsx = 2 (subject + fallback literals), `components/admin/wizard/Step3SheetCard.tsx` = 1 (§1.5 text link), `components/admin/wizard/step3ReviewSections.tsx` = 1 (§1.11 agenda text link — the header-block occurrences at `components/admin/wizard/step3ReviewSections.tsx:989-1000` die with the A swap), every other file = 0. Count-pinning keeps the guard live even inside allowlisted files: a re-inlined anchor anywhere bumps a count and fails. Consequence for D: the label-quoting comment at `Step3ReviewModal.tsx:27` is reworded during the D adoption (meaning kept, literal dropped — §4.3). Red until the last site adopts; lands with the final adoption task. **Call-site className contract (whole-diff r5 amendment; TS-AST form since r7):** the same guard file also asserts, by TypeScript-AST parse of every walked source file (`.js`/`.jsx` included — allowJs), that the ONLY binding of the component is an un-renamed `import { SheetIconLink }` (renamed specifiers, namespace imports, re-export renames, star re-exports, and variable aliases all fail — an alias defeats a name-keyed guard) and that every JSX usage carries no spread attribute and passes `className`, when present, as a plain string literal whose tokens ALL match the closed positional allowlist (responsive-prefixed non-negative margin 0-3 in half steps, and order utilities). Brace-expression classNames, negative margins, arbitrary values, and box-altering utilities fail by construction. Closes the consumer-bypass hole: colour/size/hit-area utilities smuggled through the positional prop would otherwise ship with no guard failing (the §7.6 set-equality covers only the component's own output). Negative plants pin every form.
