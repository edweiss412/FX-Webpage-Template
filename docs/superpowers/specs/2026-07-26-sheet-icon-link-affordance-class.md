...
10. **Adoption catcher (Codex r1 finding 3; predicate corrected per r2 finding 1 — the exactly-one-file form could never go green, since sites C and the agenda link legitimately keep the phrase):** a structural containment guard walking every source-extension file in the repository (`.ts`/`.tsx`/`.js`/`.jsx`/`.mjs`/`.cjs`/`.mts`/`.cts`/`.mdx`; filesystem walk, not a named list — widened from `components/**/*.tsx` to the tsconfig compile surface by whole-diff r9, then to the full tree by r10, after include-glob-derived sets twice proved narrower than what Next compiles; only dot-directories, non-compile artifact dirs, and the tsconfig-`exclude`d fixture trees stay out) asserting **per-file occurrence counts of the literal `Open the source sheet` as set-equality**: components/admin/SheetIconLink.tsx = 2 (subject + fallback literals), `components/admin/wizard/Step3SheetCard.tsx` = 1 (§1.5 text link), `components/admin/wizard/step3ReviewSections.tsx` = 1 (§1.11 agenda text link — the header-block occurrences at `components/admin/wizard/step3ReviewSections.tsx:989-1000` die with the A swap), every other file = 0 — except test files quoting the phrase as assertion material, which pin per-file counts in the same set-equality (r9: the walk covers tests/, so a new quoting assertion legitimately bumps its own row). Count-pinning keeps the guard live even inside allowlisted files: a re-inlined anchor anywhere bumps a count and fails. Consequence for D: the label-quoting comment at `Step3ReviewModal.tsx:27` is reworded during the D adoption (meaning kept, literal dropped — §4.3). Red until the last site adopts; lands with the final adoption task. **Call-site className contract (whole-diff r5 amendment; TS-AST since r7; DEFAULT-DENY since r8; walk, sanction, and prefilter amendments r9–r10):** the same guard file asserts, by TypeScript-AST parse of walked source files, that every reference to the `SheetIconLink` identifier outside the component's own file is one of exactly three sanctioned shapes — an un-renamed `import { SheetIconLink }` specifier; a JSX tag whose attributes carry no spread and whose `className`, when present, is a plain string literal with every token in the closed positional allowlist (responsive-prefixed non-negative margin 0-3 in half steps, and order utilities); or a `typeof SheetIconLink` type query (r9 — type space is erased at compile, no runtime alias comes out of it). ANY other reference (assignment, destructuring, parameter default, object property, default export, export clause, property access, …) fails, and so does any non-sanctioned touch of the component module path in ANY specifier-bearing form — import/export declarations, `import()`, `require()`, `import =` — with the specifier compared after stripping a resolvable extension and a trailing `/index` (r10). The parse gate uses contiguous-fragment prefilters only: a file is parsed if its raw text contains the identifier, a `\u`/`\x` escape spelling (escapes normalize in the AST; the raw-only gate was the r10 laundering hole), or — in shipped trees — the fragment `tests/`. Files under the repo-root `tests/` tree (root-anchored, r10 — a segment match would hand the carve to a real `app/tests/` route) relax the at-tag attribute contract ONLY; identifier and module-path rules bind everywhere. Complementing the carve, shipped runtime trees (`app/`, `components/`, `lib/`, root-level compiled files) must not import any module under `tests/` in any specifier form — otherwise a carved off-contract wrapper could reach a shipped consumer that mentions neither the identifier nor the module path (r10); tooling trees (`scripts/`) are exempt. Static alias creation requires the identifier or the module path, so the consumer-bypass class is closed by construction, not enumeration; runtime indirection (computed specifiers, `React.createElement`) is out of a static guard's reach and explicitly out of scope. Negative plants pin every form.
