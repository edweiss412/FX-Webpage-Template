...
1. **SET day** — its load-in / setup time(s) surfaced via the day-card **meta** (from `dates.loadIn`/`setupTime`, with `showStart`/`window` no-colon fallback), correct across all SET-cell shapes. (Rich multi-entry SET run-of-show — the "morning set + afternoon session" future case — is deferred; see §6.)
...
- **Write path:** `"strike"`/`"loadout"` set only by `deriveScheduleBookends`; all other entries (show-day time entries, AGENDA grid) leave `kind` absent (= agenda). The SET day has no entries (§6 meta-only). Producers only ever write the 3 enum values (or omit), so the runtime always matches the `AgendaEntryKind` type at the write boundary.
...
## 6. Derivation — SET day (meta-only; structural close of the SET-tokenization vector)

**Why meta-only (R9/R11/R12 same-vector recurrence → structural fix).** Three review rounds found title-association bugs when the SHOW-DAY tokenizer was applied to SET cells: bare clock (R9), no-colon range (R11), and **label-before-clock** (R12 — `"Load In: 7:00 PM Room Access: 8:30 PM"`, the live Redefining-FI fixture, tokenizes to a *mislabeled* `"7:00 PM Room Access"` and **drops** `8:30 PM`, because the show-day tokenizer reads the title from text *after* each clock). SET cells use a fundamentally different grammar (logistics labels, often label-*before*-clock) than show-day run-of-show (clock-then-title). Per the same-vector-recurrence rule, the fix is structural: **the SET day does NOT produce titled run-of-show entries at all.** This eliminates the entire title-assignment surface.

**What the SET day produces.** `parseScheduleTimes` reads the SET row's TIME cell only to obtain **time anchors** (never titled entries):

- Match a DATES data row whose label (col1, `clean`+`trim`) matches `/^SET\b/i` **or** `/TRAVEL\s*\/\s*SET/i` (combined row — Consultants; `classifyLabel`, `dates.ts:35`). 5-col shape required (`row.length >= 5`); v1 2-col SET rows have no TIME column → no SET ScheduleDay (faithful).
- Key by `normalizeDate(col3)` (= `dates.set`). Run the permissive tokenizer (`tokenize`, `scheduleTimes.ts:55`) on col4 but build a `ScheduleDay` with **`entries: []` always** — keep only `showStart` (first leading clock) and `window` (no-colon title-less range, `:144-160`). **No titled entries are ever created from a SET cell**, so the label-before-clock / mislabel / dropped-time class cannot occur.
- The authoritative SET times come from `dates.loadIn` / `dates.setupTime` (`dates.ts:218-224`), which `extractClockTimes` pulls **in document order regardless of label position** — for `"Load In: 7:00 PM Room Access: 8:30 PM"` → `loadIn="7:00 PM"`, `setupTime="8:30 PM"` (both correct, neither dropped); for `"11:00 AM LOAD IN"` → `loadIn="11:00 AM"`. The `showStart`/`window` on the SET ScheduleDay are the **no-colon fallback** (`dates.loadIn` is colon-required by design — `"AFTER 8PM"` → null, a ratified §4.2 contract we must NOT change).

**Guards / warnings:** Empty / no-time SET cell → SET ScheduleDay omitted (or showStart/window null). **No warning** — the SET parse is best-effort and must NOT reuse `SCHEDULE_TIME_UNPARSED` (its message hard-codes `"SHOW DAY … TIME cell"` and its anchor resolver only scans SHOW DAY rows — closes R6 medium). No regression: today the SET row yields no schedule entries; `dates.loadIn`/`setupTime` are untouched.

**Same-date collision policy (closes R10 medium), simplified by meta-only:**
- **SET date == a SHOW DAY date** (rare combined day): both rows resolve to the same ISO. Since the SET row contributes `entries:[]`, the SHOW DAY's `ScheduleDay` (with entries + showStart) must **win** — the merge keeps the entries-bearing day. (In `readShowDayTimeCells` the SET row precedes show days, so the show day's later write naturally overwrites; pin with a same-ISO SET+SHOW test asserting the show entries + showStart survive.)
- **AGENDA grid exists for the SET date**: ratified §4.3 D2 grid-wins (`index.ts:433-445`) is unchanged. The SET card meta still reads `dates.loadIn`/`setupTime` independently, so no SET time is lost.
- **`deriveScheduleBookends` only ever appends** (§7.1) — never replaces — so strike/load-out injection cannot drop entries on any date.

**Crew SET-card meta (`ScheduleSection.tsx` `isSetDay` branch, `:253-259`) — always shows the SET time:**
1. `dates.loadIn` present → `meta = "Load In {loadIn}"` (+ `" · Setup {setupTime}"` when `dates.setupTime` present). Covers all colon-clock shapes incl. label-before-clock (both times, correctly labeled by position).
2. Else `sd.window` → `meta = formatScheduleWindow(sd.window)` (no-colon range `"9AM–5PM"`, `agendaDisplay.ts:102`).
3. Else `sd.showStart` → `meta = "Load In {showStart}"` (no-colon single clock `"10AM"`).
4. Else → no meta (genuinely time-free SET cell).

All values routed through `resolveOptionalField` (sentinel-guard). **Guarantee:** whenever the SET cell has any time (colon clock(s), no-colon clock, or range), the SET card shows it; the second time is labeled "Setup" generically (e.g. Redefining's "Room Access 8:30 PM" surfaces as "Setup 8:30 PM" — correct time, generic label). The SET day has no `RunOfShowList` (entries always `[]`), so there is no meta-suppression branch and no double-print risk.

**Deferred (RD1 future):** rich **multi-entry SET run-of-show** with per-item labels (a true "morning set + afternoon *session*" rendered as ≥2 labeled rows) is **deferred to `DEFERRED.md`** — it needs a SET-specific label-before-clock tokenizer, which the corpus does not yet require (every live SET cell is load-in/setup logistics covered by the 2-time meta). Trigger to revisit: a real SET cell needs >2 distinct times or precise non-"Setup" labels for the second+ entry.
...
- SET-day meta (`isSetDay` branch) → the §6 precedence (`dates.loadIn`/`setupTime` → `sd.window` → `sd.showStart`). No suppression branch is needed: the SET day's `entries` are always `[]` (§6 meta-only), so `displayableEntries` is empty → the `RunOfShowList` never renders for the SET day and the meta always shows.
...
- **D14 — Same-date SET collisions have a defined policy** (§6, closes R10 medium). With meta-only SET (no SET entries), SET==show-day → the show-day `ScheduleDay` (entries+showStart) wins; SET-date agenda grid → grid wins (ratified §4.3 D2) with `dates.loadIn` meta fallback so no load-in is lost; `deriveScheduleBookends` only appends. Don't relitigate grid-wins (ratified).
- **D15 — SET day is meta-only; rich multi-entry SET run-of-show is DEFERRED** (§6, closes R9/R11/R12 same-vector recurrence). The SHOW-DAY tokenizer cannot title-associate SET cells (label-before-clock `"Load In: 7:00 PM Room Access: 8:30 PM"` mislabels + drops a time), so SET produces NO titled entries — only `showStart`/`window` anchors + the `dates.loadIn`/`setupTime` meta (correct for all corpus shapes). A SET-specific label-before-clock tokenizer for multi-entry SET run-of-show is deferred to `DEFERRED.md` (corpus doesn't need it). Don't re-propose tokenizing SET into titled entries.
...
| **Write path** | `deriveScheduleBookends` sets `"strike"`/`"loadout"`; show-day/grid entries leave it absent (agenda); SET day has no entries (§6). |
...
- `tests/parser/blocks/scheduleTimes.test.ts` (extend): SET row → SET `ScheduleDay` keyed by `dates.set` with **`entries: []`** and a `showStart` (or `window`) anchor only — **never a titled entry** (meta-only, §6). Assert the label-before-clock cell `"Load In: 7:00 PM Room Access: 8:30 PM"` produces **no** mislabeled entry (no `"7:00 PM Room Access"` entry), while `dates.loadIn`/`setupTime` (from `dates.ts`) = `"7:00 PM"`/`"8:30 PM"` (neither dropped). v1 2-col SET → no SET ScheduleDay. **Same-date collisions (R10 medium):** SET ISO == a SHOW DAY ISO → the show-day `ScheduleDay` (entries + showStart) **wins** (SET contributes no entries); SET date with an AGENDA grid entry → grid **wins** (ratified §4.3 D2) AND the SET DayCard still shows `dates.loadIn` (assert via the render/meta test).
...
- **SET card always shows its time across all SET-cell shapes (R9/R11/R12, meta-only):** `"11:00 AM LOAD IN"` → meta `"Load In 11:00 AM"`; **label-before-clock `"Load In: 7:00 PM Room Access: 8:30 PM"` → meta `"Load In 7:00 PM · Setup 8:30 PM"`** (both times, correct order, no dropped/mislabeled entry — the R12 fixture); `"9:00PM - LOAD IN / 10:00PM - SETUP"` → `"Load In 9:00PM · Setup 10:00PM"`; bare `"10:00 AM"` → `"Load In 10:00 AM"`; no-colon `"10AM"` → meta from `sd.showStart`; no-colon range `"9AM - 5PM"` → `"9AM–5PM"` (`sd.window`); time-free SET cell → no meta. The SET day never renders a `RunOfShowList` (entries `[]`). Negative-regression: drop the `sd.window`/`sd.showStart` fallbacks → the no-colon cases lose their time. Failure mode: a SET card with no/incorrect load-in time.
...
| `lib/parser/blocks/scheduleTimes.ts` | SET-row capture in `readShowDayTimeCells` → `ScheduleDay` with `entries:[]` + `showStart`/`window` anchors only (meta-only, §6); warning-free; export shared `extractFirstClock`. |
...
| `components/crew/sections/ScheduleSection.tsx` | SET-day meta from `dates.loadIn`/`setupTime` (+window/showStart fallback, §6); import `transportTileVisible`; route per-day entries through `scheduleEntriesForViewer`. |
