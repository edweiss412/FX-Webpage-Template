...
1. **SET day** — its load-in / setup time(s) surfaced via the day-card **meta**, read directly from `dates.loadIn`/`setupTime` (no SET tokenization; correct across all colon-clock SET-cell shapes incl. label-before-clock). (Rich multi-entry SET run-of-show — the "morning set + afternoon session" future case — is deferred; see §6.)
...
## 6. Derivation — SET day (meta from `dates`; NO SET tokenization)

**Structural decision (closes the R9/R11/R12/R13 SET vector).** Four review rounds found problems when SET cells were run through the SHOW-DAY tokenizer or carried no-colon anchors on a fragile `run_of_show` key: bare clock (R9), no-colon range (R11), label-before-clock mislabel/drop (R12 — `"Load In: 7:00 PM Room Access: 8:30 PM"`), and grid-overwrite of the no-colon anchor (R13). The convergent fix removes the entire surface: **the SET day performs NO tokenization and gets NO `run_of_show` key.** Its load-in/setup time is read straight from `dates.loadIn`/`dates.setupTime`, which live on the show object (grid-independent) and which `dates.ts` (`extractClockTimes`, `:218-224`) **already** extracts from the SET row in document order, **regardless of label position**.

- **No parser change for SET.** `parseScheduleTimes`/`readShowDayTimeCells` are **not** extended to SET rows; the SET day produces no `run_of_show` key. The only change is the crew render.
- **Crew SET-card meta** (`ScheduleSection.tsx` `isSetDay` branch, `:253-259`, replacing the current setupTime-only meta): `meta = "Load In {loadIn}"` + (`" · Setup {setupTime}"` when present), where `loadIn = resolveOptionalField(data.show.dates.loadIn)` and `setupTime = resolveOptionalField(data.show.dates.setupTime)`. Both null → no meta (today's behavior).
- **Coverage = the whole corpus.** Every live SET cell uses colon clocks, which `dates.loadIn`/`setupTime` capture correctly: FinTech `"11:00 AM LOAD IN"` → `Load In 11:00 AM`; Consultants `"9:00PM - LOAD IN / 10:00PM - SETUP"` → `Load In 9:00PM · Setup 10:00PM`; **Redefining `"Load In: 7:00 PM Room Access: 8:30 PM"` → `Load In 7:00 PM · Setup 8:30 PM`** (both times, correct order, no mislabel/drop — the R12 fixture, now handled with zero tokenizer involvement). East Coast v1 `"AFTER 8PM"` → `loadIn` null by the **ratified colon-requirement** (per-day-schedule spec §4.2 R12: `"AFTER 8PM"`→null) → no SET meta, exactly today's behavior.
- **No-colon SET times** (`"10AM"`, `"9AM - 5PM"` — NOT in the corpus) are intentionally not shown, **consistent with the ratified colon-requirement** (the very rule that drops `"AFTER 8PM"` to avoid a false `"8PM"`). This is the existing pre-feature behavior, not a regression — and it removes the grid-fragile no-colon anchor entirely (closes R13).
- **Grid / collision: structurally moot.** The SET load-in lives on `dates` (the show object), independent of `run_of_show` and the AGENDA grid — nothing can overwrite it. The SET day has no `run_of_show` key, so there is no SET-vs-grid or SET-vs-show-day key collision (closes R10/R13 for SET). (`deriveScheduleBookends` may still append a strike/load-out to the SET date's ScheduleDay if a room strikes / the van departs on the set day — rare; it creates the key fresh and appends, and never touches the `dates`-sourced SET meta.)
- The second SET time is labeled "Setup" generically (e.g. Redefining's "Room Access 8:30 PM" → "Setup 8:30 PM" — correct time, generic label), matching the existing `setupTime` semantics.

**Deferred (RD1 future):** rich **multi-entry SET run-of-show** with per-item labels (a true "morning set + afternoon *session*" rendered as ≥2 labeled rows) is **deferred to `DEFERRED.md`** — it needs a SET-specific label-before-clock tokenizer the corpus does not yet require (every live SET cell is load-in/setup logistics covered by the 2-time meta). Trigger to revisit: a real SET cell needs >2 distinct times or precise non-"Setup" labels.
...
- SET-day meta (`isSetDay` branch) = `"Load In {dates.loadIn}"` (+ `" · Setup {dates.setupTime}"`), per §6 — read from the show object, not `run_of_show`. No suppression branch: the SET day has no `run_of_show` key, so `displayableEntries(sd?.entries)` is empty and the `RunOfShowList` never renders for it.
...
**The crew data path drops synthetic entries on non-show aggregate days today.** `getShowForViewer.ts:669-695` gates the decoded `runOfShow` to keys in `showDaySet = new Set(show.dates.showDays)` (∩ the active viewer's `DateRestriction`). The SET day carries no `run_of_show` key (§6, meta-only), so it is unaffected — but a **synthetic strike/load-out entry on an aggregate-but-non-show day** (most concretely, a **Load Out from a Pick Up Venue dated on the travel-out day** — the van leaves the day after the last show) is keyed by `dates.travelOut` (or `travelIn`/`set`), which is **never** a member of `showDays`. So that entry would be filtered out **before** `data.runOfShow` reaches the crew components, shown in admin (Step-3 reads `parseResult.runOfShow` directly) yet **silently invisible to crew** despite landing on an aggregate day the crew renders a card for. (Strikes/load-out on show days — the entire surveyed corpus — already pass the existing filter; this widening is the robustness fix for the travel-day case.)
...
- **D14 — No SET key ⇒ no SET collision** (§6, closes R10/R13). The SET day produces no `run_of_show` key, so there is no SET-vs-grid or SET-vs-show-day key collision and the SET load-in (on `dates`) cannot be overwritten by the grid. Grid-wins (ratified §4.3 D2) is unchanged for show/grid days. `deriveScheduleBookends` only appends.
- **D15 — SET day is `dates`-sourced (no tokenization); rich multi-entry SET run-of-show is DEFERRED** (§6, closes R9/R11/R12/R13 same-vector recurrence). The SHOW-DAY tokenizer cannot title-associate SET cells (label-before-clock `"Load In: 7:00 PM Room Access: 8:30 PM"` mislabels + drops a time), so SET performs NO tokenization and gets NO `run_of_show` key — the SET meta reads `dates.loadIn`/`setupTime` (correct for all colon-clock corpus shapes). No-colon SET times are intentionally not shown (ratified colon-requirement). A SET-specific label-before-clock tokenizer for multi-entry SET run-of-show is deferred to `DEFERRED.md`. Don't re-propose tokenizing SET cells.
...
- `tests/parser/blocks/dates.test.ts` (extend) + `tests/parser/parseSheet*.test.ts`: the SET feature creates **NO `run_of_show` key** for `dates.set` (§6 — no SET tokenization). Assert `dates.loadIn`/`setupTime` correctly capture SET clocks across shapes, especially the label-before-clock R12 fixture `"Load In: 7:00 PM Room Access: 8:30 PM"` → `loadIn="7:00 PM"`, `setupTime="8:30 PM"` (both, correct order, neither dropped, no mislabel), and `"9:00PM - LOAD IN / 10:00PM - SETUP"` → `"9:00PM"`/`"10:00PM"`. Preserve the ratified `"AFTER 8PM"`→`null` colon-requirement case. (No SET-vs-grid / SET-vs-show-day key collision exists — SET has no key.)
...
- **SET card meta from `dates` (R9/R11/R12/R13, no tokenization):** crew `ScheduleSection` SET card → `"11:00 AM LOAD IN"` → meta `"Load In 11:00 AM"`; **label-before-clock `"Load In: 7:00 PM Room Access: 8:30 PM"` → `"Load In 7:00 PM · Setup 8:30 PM"`** (both, correct, no mislabel/drop — the R12 fixture); `"9:00PM - LOAD IN / 10:00PM - SETUP"` → `"Load In 9:00PM · Setup 10:00PM"`; `"AFTER 8PM"` (v1, no colon) → no meta (ratified). The SET day never renders a `RunOfShowList`. Negative-regression: revert the meta to the current setupTime-only form → the load-in disappears (pins the loadIn addition). Anti-tautology: assert against `data.show.dates.loadIn` (the source), not the rendered container. Failure mode: a SET card missing the load-in time.
...
| `components/crew/sections/ScheduleSection.tsx` | SET-day meta = `dates.loadIn` + `setupTime` (§6); import `transportTileVisible`; route per-day entries through `scheduleEntriesForViewer`. |
