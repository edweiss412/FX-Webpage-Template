...
2. Else → `meta` is composed from `loadIn = resolveOptionalField(data.show.dates.loadIn) ?? resolveOptionalField(sd?.showStart)` and `setup = resolveOptionalField(data.show.dates.setupTime)`: `"Load In {loadIn}"` (+ `" · Setup {setup}"` when present). `dates.loadIn` is the colon-form first clock (`dates.ts:218-224`); `sd.showStart` is the permissive-tokenizer fallback that also catches no-colon clocks the colon-required `extractClockTimes` misses (`"10AM"`). So **whenever any clock exists in the SET cell, the SET card shows the load-in time** — the bare-clock case no longer renders a timeless SET card.
3. Both null (truly no clock) → no meta (same as today).
...
- **Bare-clock SET card shows the time (R9 high #1):** a SET cell `"10:00 AM"` (untitled clock) → no displayable run-of-show entries, but the crew SET DayCard meta renders `"Load In 10:00 AM"` (from `dates.loadIn`); a no-colon `"10AM"` SET cell → meta from `sd.showStart`. A titled SET cell `"11:00 AM LOAD IN"` → entry rendered, meta suppressed. Negative-regression: drop the `?? sd?.showStart` fallback → the no-colon case loses its time (pins the fallback). Failure mode: a SET card with no visible load-in/setup time.
