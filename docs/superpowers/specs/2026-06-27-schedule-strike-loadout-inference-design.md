...
- Crew `ScheduleSection` test: SET day card shows the load-in/setup **meta** (no `RunOfShowList` on the SET day — entries `[]`); a **strike** entry shows on its day; **load-out** on the last day. Derive the expected day count from the fixture's aggregate days.
