...
1. **SET day** — its load-in / setup time(s) surfaced as **synthesized `"Load In"`/`"Setup"` run-of-show entries** (built from `dates.loadIn`/`setupTime` by `deriveScheduleBookends`, no SET-cell tokenization; correct across all colon-clock SET shapes incl. label-before-clock), rendered uniformly on admin Step-3 + crew Schedule/Today like strike/load-out. (Rich multi-entry SET run-of-show — the "morning set + afternoon session" future case — is deferred; see §6.)
...
- No change to the existing **"Daily call times"** `KeyTimesStrip` *content* (`resolveKeyTimes`, `lib/crew/resolveKeyTimes.ts`) — it keeps surfacing a single GS-room `strike` summary anchor and a `set` anchor; summary-strip ↔ per-day-detail overlap is intentional (§9.5, §11 D2). **One targeted exception (§9.8, closes R10 high):** `resolveKeyTimes`'s show-anchor *fallback* must skip synthetic entries so a teardown time is never reported as a show call time. This is a guard, not a content change.
...
| # | Decision | Value |
|---|---|---|
| RD1 | Bookends to add | **SET + Strike + Load-Out.** SET day surfaces its load-in/setup as **synthesized `"Load In"`/`"Setup"` run-of-show entries** (built from `dates.loadIn`/`setupTime` by `deriveScheduleBookends`, §6; up to two), rendered uniformly on admin Step-3 + crew Schedule/Today; the `isSetDay` "Setup" meta is suppressed when those entries exist. Correct across all colon-clock SET-cell shapes. Rich multi-entry SET *run-of-show* with cell-derived titles (the "morning set + afternoon session" future case) is **deferred** (§6 Deferred) after the tokenization-edge rounds — the 2-entry synthesis covers the live corpus (load-in/setup logistics). |
...
- **Write path:** `"strike"`/`"loadout"` set only by `deriveScheduleBookends`; all other entries (show-day time entries, AGENDA grid, and the synthesized SET `"Load In"`/`"Setup"` entries) leave `kind` absent (= agenda). Producers only ever write the 3 enum values (or omit), so the runtime always matches the `AgendaEntryKind` type at the write boundary.
...
## 6. Derivation — SET load-in/setup as SYNTHESIZED entries (from `dates`, via `deriveScheduleBookends`)

**Design (unifies SET with strike/load-out; closes R9–R14 for SET).** The SET day's load-in/setup time(s) are added to `run_of_show[dates.set]` as **synthesized entries by `deriveScheduleBookends`** (§7.1), sourced from `dates.loadIn`/`dates.setupTime` — **NOT tokenized from the SET cell.** This is the same machinery that synthesizes strike/load-out, so SET, Strike, and Load-Out are all derived uniformly from already-parsed fields and **all three render on every surface that consumes `run_of_show`** — admin Step-3 (`parseResult.runOfShow`), crew Schedule, and crew Today — closing the R14 surface gap (no per-surface bespoke SET meta).

Why this beats the alternatives the prior rounds explored:
- **No tokenizer** → no title-association bugs. `dates.loadIn`/`setupTime` (`dates.ts:218-224`, `extractClockTimes`) pull clocks in **document order regardless of label position**: `"Load In: 7:00 PM Room Access: 8:30 PM"` → `loadIn="7:00 PM"`, `setupTime="8:30 PM"` (R12 fixture — both, correct order, no mislabel/drop); `"11:00 AM LOAD IN"` → `"11:00 AM"`. Closes R9/R11/R12.
- **Appended, never overwrites** → a grid/collision on the SET date keeps both the grid entries AND the synthesized load-in. Closes R10/R13.

Synthesis (in `deriveScheduleBookends`, §7.1, alongside strike/load-out):
```
if dates.set:
  if presence(dates.loadIn):    appendEntry(ros, dates.set, { start: dates.loadIn,    title: "Load In" })   // kind absent ⇒ agenda
  if presence(dates.setupTime): appendEntry(ros, dates.set, { start: dates.setupTime, title: "Setup"  })
```
- **Fixed titles** "Load In"/"Setup" (synthesized, not read from the cell) — the second SET time is labeled "Setup" generically (Redefining's "Room Access 8:30 PM" → "Setup 8:30 PM"; correct time, generic label, matching existing `setupTime` semantics).
- **Kind absent** (= agenda): SET entries are plain run-of-show rows (no badge, not transport-gated — they're not `loadout`). They count toward the agenda cap (SET has ≤2 → never near it).
- **No-colon SET** (`"AFTER 8PM"`, `"9AM - 5PM"`) → `dates.loadIn` null (ratified colon-requirement, per-day-schedule spec §4.2 R12: `"AFTER 8PM"`→null to avoid a false `"8PM"`) → **no SET entry**. Not in corpus; matches existing pre-feature behavior; not a regression.
- The SET key reaches crew via §9.7 (aggregate-domain projection) and admin via direct `parseResult.runOfShow`. No warning (D11).

**Render:** the SET day renders its synthesized entries via `RunOfShowList` on **all three** surfaces (admin Step-3, crew Schedule, crew Today) through their existing per-day `run_of_show` rendering — no surface-specific SET code. Crew `ScheduleSection`'s `isSetDay` meta (currently `"Setup {setupTime}"`, `:253-259`) is **suppressed when the SET day has displayable entries** (avoid double-printing setupTime); when the SET day has no entries (no-colon), no meta (today's behavior).

**Deferred (RD1 future):** rich **multi-entry SET run-of-show** with per-item labels read from the SET cell (a true "morning set + afternoon *session*" with cell-derived titles, needing a SET-specific label-before-clock tokenizer) is **deferred to `DEFERRED.md`** — the 2-entry load-in/setup synthesis covers the live corpus (load-in/setup logistics). Trigger to revisit: a real SET cell needs >2 times or precise non-"Setup" labels.

(No change to admin — admin has no per-day meta; admin renders the titled entries via `ScheduleDayRow`, and a bare-clock SET day with only `showStart` shows the date header with no rows, which is acceptable on the admin preview.)
...
// ── SET load-in / setup (synthesized from dates; NO tokenization — §6) ──────────
// dates.loadIn/setupTime are colon-extracted (dates.ts), correct for all SET-cell
// shapes incl. label-before-clock. Appended (never overwrites a grid/collision day).
// kind absent ⇒ agenda (plain row, not transport-gated). No-colon SET → loadIn null → skip.
if dates.set:
...
No structural change to day iteration. Synthetic entries on `aggregateDays` dates light up via the existing per-day gate (`:286-287`), now routed through `scheduleEntriesForViewer` (§9.6). Edits:
- SET-day rendering: the SET day now carries synthesized `"Load In"`/`"Setup"` entries on `run_of_show[dates.set]` (§6), so the existing per-day `RunOfShowList` renders them like any day. The `isSetDay` meta (`:253-259`, currently `"Setup {setupTime}"`) is **suppressed when the SET day has displayable entries** (`displayableEntries(sd?.entries).length > 0`) to avoid double-printing setupTime; when the SET day has no entries (no-colon SET), no meta (today's behavior).
...
- **Today:** synthetic entries (incl. the SET load-in/setup when today is the set day, and strike/load-out) on today's card render via the same `RunOfShowList` (gated `isShowDay = aggregateDays(...).some(d.date===todayIso)`, `:197` — `aggregateDays` includes the set day, so a set-day "today" qualifies), routed through `scheduleEntriesForViewer(todays, { transportVisible })` — `transportVisible` is **already computed** at `TodaySection.tsx:281`, so the load-out gate (§9.6) reuses it (no new gate plumbing on Today). This gives SET/strike/load-out uniform Today coverage (closes R14 Today gap).
...
**The crew data path drops `run_of_show` keys that aren't show days today.** `getShowForViewer.ts:669-695` gates the decoded `runOfShow` to keys in `showDaySet = new Set(show.dates.showDays)` (∩ the active viewer's `DateRestriction`). The **synthesized SET entries are keyed by `dates.set`** (§6) — **never** a member of `showDays` — so without this change they would be filtered out before `data.runOfShow` reaches the crew components: the SET feature would be parsed/stored, shown in admin (Step-3 reads `parseResult.runOfShow` directly), yet **silently invisible to crew** — the §1 primary goal. (A Load-Out keyed on `dates.travelOut`, or a strike on `travelIn`/`set`, would be dropped for the same reason.) Strikes/load-out on show days — the entire surveyed corpus — already pass the existing filter; this widening is required for SET and the travel-day cases.
...
- `activeRestriction.kind === "none"` (or admin) → `allowed = aggregateSet = new Set(aggregateDays(show.dates).map(d => d.date))` (was `showDaySet`).
- `activeRestriction.kind === "explicit"` → `allowed = new Set(activeRestriction.days.filter(d => aggregateSet.has(d)))` (was `…filter(d => showDaySet.has(d))`). **Correction (R9 high #2):** `normalizeDateRestriction` (`lib/data/normalizeDateRestriction.ts:119`) only normalizes raw `M/D` tokens to ISO via a fallback year — it does **NOT** intersect with `showDays`. So `restriction.days` CAN include a SET/travel date if the operator listed it, and `ScheduleSection`'s explicit branch already shows such an aggregate day (`:171-175`: `allowedShowDays.has(d) || dateRestriction.days.includes(d)`). Filtering against `aggregateSet` (not `showDaySet`) makes the **data** match that **component** behavior: an explicit viewer whose restriction names the SET date sees the SET card (already) **and** its entries (now); one whose restriction names only show days sees only those. This is **not** new exposure — the viewer is already shown any day their restriction lists; we only stop dropping the run-of-show for SET/travel days they're already allowed to see. (The old `showDaySet` filter silently dropped SET entries for such viewers — a latent inconsistency this corrects.)
...
### 9.8 Anchor-selection guard — synthetic entries are never a show call time (closes R10 high)

`resolveKeyTimes` derives each day's "Daily call times" / Right-Now Show anchor from a candidate cascade `[day.showStart, day.window?.start, day.entries[0]?.start]` (`resolveKeyTimes.ts:135`), and `buildRightNowContext` consumes the same anchors. Appending a synthetic entry to a show day that has **no** agenda entries (e.g. GS strikes on a show day whose DATES TIME cell was empty) would make `day.entries[0]` the **strike** — so the teardown time would be reported as the day's **show start**. Fix: the `entries[*]` candidate must be the **first non-synthetic entry**, not `entries[0]`:

```ts
// was: day.entries[0]?.start
day.entries.find((e) => e.kind !== "strike" && e.kind !== "loadout")?.start
```

So a show day with only synthetic entries yields no entry-based anchor → it falls through to the existing room `show_time` fallback (or omits), exactly as a no-run-of-show day does today. This is the single change to `resolveKeyTimes`; it leaves `set`/`strike` summary anchors (`:106-117`) untouched and needs no `buildRightNowContext` edit (it reads `resolveKeyTimes`'s output).
...
- **D12 — Crew read-model projection is widened to the aggregate domain** (§9.7, closes R8 high). `getShowForViewer` previously dropped all non-`showDays` `runOfShow` keys, which would have made the SET day crew-invisible; it now keeps the `aggregateDays` domain (`travelIn/set/showDays/travelOut`). For **explicit** viewers the filter is `restriction.days ∩ aggregateSet` — `normalizeDateRestriction` does NOT pre-intersect `showDays`, so a restriction naming the SET/travel date now surfaces those entries, exactly matching what `ScheduleSection`'s explicit branch already renders as a card (R9 #2 corrected an earlier wrong "⊆ showDays" claim). `unknown_asterisk` still leaks nothing. Off-aggregate keys are still dropped (the §10 boundary). Parser `scheduleDateSet`, component `aggregateDays`, and this projection are ONE domain (reuse `aggregateDays`, don't re-derive).
- **D13 — `resolveKeyTimes`/Right-Now anchor selection skips synthetic entries** (§9.8, closes R10 high). A teardown (strike/load-out) time is never reported as a show call time; the only `resolveKeyTimes` change is `entries[0]` → first non-synthetic entry. Summary `set`/`strike` anchors unchanged.
- **D14 — SET entries are APPENDED, so no collision loss** (§6/§7.1, closes R10/R13). `deriveScheduleBookends` appends the synthesized SET Load In/Setup to `run_of_show[dates.set]` (running after the merge) — it never overwrites. A grid/collision on the SET date keeps both the grid entries and the appended load-in. Grid-wins (ratified §4.3 D2) is unchanged for show/grid days.
- **D15 — SET is `dates`-SYNTHESIZED (not tokenized); rich multi-entry SET run-of-show is DEFERRED** (§6, closes R9/R11/R12/R13/R14 same-vector recurrence). The SHOW-DAY tokenizer cannot title-associate SET cells (label-before-clock `"Load In: 7:00 PM Room Access: 8:30 PM"` mislabels + drops a time), so SET performs NO tokenization; instead `deriveScheduleBookends` synthesizes `"Load In"`/`"Setup"` entries from `dates.loadIn`/`setupTime` (colon-extracted, correct for all corpus shapes) — rendered uniformly on admin + crew Schedule/Today via `run_of_show` (R14 fix). No-colon SET times are intentionally not shown (ratified colon-requirement). A SET-specific label-before-clock tokenizer for multi-entry SET run-of-show with cell-derived titles is deferred to `DEFERRED.md`. Don't re-propose tokenizing SET cells.
- **D11 — No SET warning** (§6, closes R6 medium). Since SET performs no tokenization (D15), it emits no warning at all — in particular it never reuses `SCHEDULE_TIME_UNPARSED` (whose "SHOW DAY" copy + SHOW-DAY-only anchor resolver don't fit SET). The only new warning in this feature is `SCHEDULE_STRIKE_DATE_OFF_SCHEDULE` (§8).
...
| **Write path** | `deriveScheduleBookends` sets `"strike"`/`"loadout"`; show-day/grid entries and synthesized SET `"Load In"`/`"Setup"` entries leave `kind` absent (agenda). |
...
- `tests/parser/blocks/scheduleBookends.test.ts` (SET synthesis): `deriveScheduleBookends` appends `"Load In {dates.loadIn}"` + `"Setup {dates.setupTime}"` entries (kind absent) to `runOfShow[dates.set]`, sourced from `dates` (NOT tokenized). Label-before-clock R12 fixture: with `dates.loadIn="7:00 PM"`, `setupTime="8:30 PM"` → entries `"Load In 7:00 PM"`, `"Setup 8:30 PM"` (both, correct order, no mislabel/drop). `"AFTER 8PM"` → `dates.loadIn` null → **no SET entry** (ratified colon-requirement). **Append-not-overwrite:** a pre-existing grid `ScheduleDay` on `dates.set` keeps its grid entries AND gains the appended Load In/Setup (no collision/overwrite — closes R10/R13). Derive expected from `dates`, not hardcoded.
- **Anchor-selection guard (R10 high):** `tests/crew/resolveKeyTimes*.test.ts` — a show day whose `runOfShow` entry has **only** a `kind:"strike"`/`"loadout"` entry (no agenda, no showStart/window) → the Show anchor is **NOT** the strike/load-out time (falls back to room `show_time` or omits). Negative-regression: revert `find(non-synthetic)` to `entries[0]` → the teardown time wrongly becomes the show anchor. Mirror for `buildRightNowContext` (consumes the same anchors).
...
- Crew `ScheduleSection` test: the SET day renders its synthesized `"Load In"`/`"Setup"` entries via `RunOfShowList` (with the `isSetDay` "Setup" meta **suppressed** since entries exist); a **strike** entry shows on its day; **load-out** on the last day. Derive the expected day count from the fixture's aggregate days. (See the dedicated SET-render test below for the meta-suppression + all-three-surfaces assertions.)
...
- **Crew read-model projection (R8 high pin):** `tests/data/getShowForViewer*RunOfShow*.test.ts` (extend) — a stored `runOfShow` with a **SET-day key** (`dates.set`) and a **strike on a show day** → a `none`/admin viewer's `data.runOfShow` contains **both** (SET key no longer dropped); an **off-aggregate** key (date ∉ travelIn/set/showDays/travelOut) is **dropped**; `unknown_asterisk` → `null`. **Explicit (R9 high #2):** an explicit viewer whose `restriction.days` **includes** the SET date → SET key present; an explicit viewer whose `restriction.days` lists **only** show days → SET key absent (matches `ScheduleSection`'s explicit day-list). Negative-regression: revert the filter to `showDaySet` → the SET key disappears even for the `none` viewer (proves the widening is load-bearing for the primary goal). Derive the expected key set from the fixture's `dates`, not hardcoded.
- **SET renders on all three surfaces (R9–R14, synthesized entries):** the SET day's `"Load In"`/`"Setup"` entries render via `RunOfShowList` on crew `ScheduleSection`, crew `TodaySection` (when today is the set day), and admin `ScheduleBreakdown` — assert each. Crew `ScheduleSection` `isSetDay` meta is **suppressed** when SET entries exist (no double-print of setupTime — clone the SET card, remove the `RunOfShowList`, assert the standalone "Setup" meta is gone). Anti-tautology: assert the entries' `start` against `data.show.dates.loadIn`/`setupTime` (the source), not the container. Negative-regression: drop the SET synthesis step → the SET day loses its Load In/Setup entries on all three surfaces. Failure mode: SET absent from admin/Today (the R14 gap).
...
| `lib/parser/blocks/scheduleTimes.ts` | export shared `extractFirstClock` (§7.2). **No SET-row change** — SET is `dates`-sourced (§6). |
| `lib/parser/blocks/scheduleBookends.ts` | **new** — `deriveScheduleBookends` (synthesizes SET load-in/setup from `dates`, strikes from rooms, load-out from transport) + `parseRoomTimeCell` (reuses `extractFirstClock`). |
...
| `lib/crew/resolveKeyTimes.ts` | show-anchor `entries[*]` candidate (`:135`) skips synthetic (`kind` strike/loadout) entries — first non-synthetic entry only (§9.8). |
| `lib/crew/agendaDisplay.ts` | **new** `scheduleEntriesForViewer` helper (load-out transport gate; §9.6). |
| `components/crew/sections/ScheduleSection.tsx` | suppress `isSetDay` meta when SET has displayable entries (§6/§9.1); import `transportTileVisible`; route per-day entries through `scheduleEntriesForViewer`. |
