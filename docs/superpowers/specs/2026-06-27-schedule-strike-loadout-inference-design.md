...
| # | Decision | Value |
|---|---|---|
| RD1 | Bookends to add | **SET + Strike + Load-Out.** SET day surfaces its load-in/setup **time(s) via the day-card meta** (up to two: load-in + setup), correct across all SET-cell shapes. Rich multi-entry SET *run-of-show* (the "morning set + afternoon session" future case) is **deferred** (§6 Deferred) after 3 tokenization-edge rounds — the meta covers the live corpus (load-in/setup logistics). |
