...
**Same-date collision policy (closes R10 medium).** The SET day is keyed by `dates.set`. In the normal corpus, `dates.set` ∉ `showDays` and has no AGENDA-grid entry → its key is distinct and flows through untouched. Two collisions must be defined:
- **SET date == a SHOW DAY date** (rare combined set+show day): `parseScheduleTimes` builds one `ScheduleDay` per ISO and currently last-write-wins (`scheduleTimes.ts:191`). Change: when the SET row and a SHOW DAY row resolve to the **same ISO**, **concatenate** their entries (show-day entries first, then SET entries) and keep the **show-day** `showStart`/`window` — never silently drop one side. (Pin with a same-date SET+SHOW parser test.)
- **AGENDA grid exists for the SET date**: the ratified §4.3 D2 merge has the grid **win** for a date with ≥1 titled grid entry (`index.ts:433-445`) — the grid `ScheduleDay` replaces the DATES-derived one. This rule is unchanged and applies to the SET date too: an operator-maintained agenda grid for the set date is authoritative. **No load-in is lost:** the SET DayCard meta still renders `dates.loadIn`/`setupTime` (§6 mode-boundary fallback), which are extracted independently in `dates.ts` regardless of the grid. Document, don't special-case. (Pin with a same-date SET+grid parser test asserting grid entries win AND the SET meta still shows loadIn.)
- **`deriveScheduleBookends` only ever appends** (§7.1) — it never replaces a day's entries — so strike/load-out injection cannot drop SET or agenda entries on any date.

**Mode boundary (SET-day meta) — guarantees the SET time always shows (closes R9 high #1):** the tokenizer only emits an *entry* for a **titled** clock; a **bare untitled clock** SET cell (e.g. `"10:00 AM"`) yields `entries:[]` + `showStart` (`scheduleTimes.ts:164-180`), so `displayableEntries` is empty and the run-of-show list would render nothing. So the `isSetDay` branch in `ScheduleSection.tsx` (`:253-259`) computes meta with this precedence:
1. SET day has displayable run-of-show entries (titled — `"11:00 AM LOAD IN"`, or the morning-set/afternoon-session case) → render them via `RunOfShowList`; **suppress** the meta (entries carry the times; avoid double-print).
2. Else if the SET `ScheduleDay` has a **`window`** (a no-colon title-less range like `"9AM - 5PM"` → the tokenizer emits `window:{start,end}`, `entries:[]`, `showStart:null`, `scheduleTimes.ts:144-160`) → `meta = formatScheduleWindow(sd.window)` (e.g. `"9AM–5PM"`, the existing helper `agendaDisplay.ts:102`).
3. Else → `meta` composed from `loadIn = resolveOptionalField(data.show.dates.loadIn) ?? resolveOptionalField(sd?.showStart)` and `setup = resolveOptionalField(data.show.dates.setupTime)`: `"Load In {loadIn}"` (+ `" · Setup {setup}"` when present). `dates.loadIn` is the colon-form first clock (`dates.ts:218-224`); `sd.showStart` is the permissive-tokenizer fallback that catches no-colon single clocks (`"10AM"`).
4. Else (no entries, no window, no clock) → no meta (truly timeless SET cell, same as today).

**Guarantee:** the SET tokenizer's three non-empty outcomes — titled entries (1), bare window (2), bare clock→showStart (3) — are each covered, plus `dates.loadIn` (3). So **whenever the SET cell contains any time at all (titled, range, colon clock, or no-colon clock), the SET card shows it**; only a genuinely time-free SET cell renders without a time.
...
- **D14 — Same-date SET collisions have a defined policy** (§6, closes R10 medium). SET==show-day → concatenate (keep show `showStart`); SET-date agenda grid → grid wins (ratified §4.3 D2) with `dates.loadIn` meta fallback so no load-in is lost; `deriveScheduleBookends` only appends. Don't relitigate grid-wins (ratified).
...
- `tests/parser/blocks/scheduleTimes.test.ts` (extend): SET row tokenized → SET-day `ScheduleDay` keyed by `dates.set` with the load-in entry; combined `TRAVEL / SET` row tokenizes both clocks; v1 2-col SET → no SET ScheduleDay. **Same-date collisions (R10 medium):** SET date == a SHOW DAY date → entries **concatenated** (show-day + SET), show-day `showStart` kept, neither dropped; SET date with an AGENDA grid entry → grid entries **win** (ratified §4.3 D2) AND the SET DayCard still shows `dates.loadIn` (assert via the render/meta test).
...
- **SET card always shows its time across all tokenizer shapes (R9 high #1 + R11 medium):** titled `"11:00 AM LOAD IN"` → entry rendered, meta suppressed; bare clock `"10:00 AM"` → meta `"Load In 10:00 AM"` (`dates.loadIn`); no-colon clock `"10AM"` → meta from `sd.showStart`; **no-colon range `"9AM - 5PM"` → meta `"9AM–5PM"` (`sd.window`)**; truly time-free SET cell → no meta. Negative-regression: drop the `sd.window`/`sd.showStart` fallbacks → the range / no-colon cases lose their time (pins each fallback). Failure mode: a SET card with no visible load-in/setup time.
