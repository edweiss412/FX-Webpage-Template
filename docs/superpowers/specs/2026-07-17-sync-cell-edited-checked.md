...
| `StatusIndicator` renders dot + label for a bucket | `components/admin/StatusIndicator.tsx` |
...
`SyncCell` returns a block containing:
...
- **Line 2** — a muted meta line, rendered **only when `row.lastCheckedAt != null`** (see §4.3):
  ```
  <div class="mt-0.5 text-xs text-text-faint tabular-nums" data-testid="shows-sync-times-{slug}">
    Edited {formatRelative(row.lastSyncedAt, now)}
    <span aria-hidden="true"> · </span>
    Checked {formatRelative(row.lastCheckedAt, now)}
  </div>
  ```
  The middot separator is `aria-hidden` so a screen reader announces `Edited 3h ago Checked 2m ago` (two clauses, no stray "middot"). Word choice: **"Checked"** (matches the column's own migration comment "we successfully reached Drive and evaluated this show"). "Edited" for `last_synced_at` (last content change).
...
- **`row.lastCheckedAt == null`** → line 2 is **not rendered** at all. Rationale: a row exists in `shows` only because the cron first-saw it, which sets `last_checked_at` (`runScheduledCronSync.ts` first-seen path) and the migration backfilled all pre-existing rows (`20260717000000...:8`). So null is the theoretical brand-new-and-never-checked edge; in that state line 1 already reads "Not synced yet," which fully carries the meaning — a `Checked never` line would be redundant noise. This is the ONLY suppression condition.
- **`row.lastCheckedAt != null` but `row.lastSyncedAt == null`** → line 2 renders `Edited never · Checked {rel}`. `formatRelative(null, now)` returns `"never"` (`showDisplay.ts:93`). This is a real state: cron checked the sheet but never applied content (e.g. held for review before first apply).
- **`row.lastSyncStatus == null` AND `lastCheckedAt != null`** → line 1 "Not synced yet", line 2 `Edited never · Checked {rel}` renders. (Suppression is keyed on `lastCheckedAt`, not `lastSyncStatus`.)
...
- The `12rem` Sync **width** track is unchanged; `Edited 3h ago · Checked 2m ago` (~26 chars) fits within `12rem` (~192px) at `text-xs`; if a value is longer it wraps within the track (the track already permits wrapping per `ShowsTable.tsx:71-72`). No `whitespace-nowrap` on line 2.
...
5. **Middot separator is aria-hidden.** Assert the accessible text of line 2 has no literal "·" (query the aria-hidden span; or assert `textContent` split). *Catches:* SR announcing a stray middot.
...
Literals in this spec: `12rem` (existing Sync track width, cited `ShowsTable.tsx` — not changed by this spec), `768px` (existing mobile breakpoint, cited), `~26 chars`/`~192px` (fit estimate, illustrative). No new numeric constant is introduced into code. Row-count caps, durations, env-var counts: none touched.
