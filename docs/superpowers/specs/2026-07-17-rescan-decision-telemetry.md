...
- `logAdminOutcome` (`lib/log/logAdminOutcome.ts:8-51`): `AdminOutcome.extra?: Record<string, unknown>` spreads FIRST into `app_events.context` (`:38`, `...(o.extra ?? {})`), cannot override reserved fields. `extra` is NOT hashed but the whole record passes through `log.info` → `sanitizeContext` (AGENTS.md "Redaction posture": `app_events.context` IS redaction-guaranteed at write time). Wrapper swallows all throws (invariant 9).
...
`reviewCodes` = a **deduped, order-stable** union of every ACTIVE `isDirty` driver, so it is never empty on a demote:

1. `sentinelItems` invariants + `decisionItems` invariants: `[...sentinelItems, ...decisionItems].map(i => i.invariant)`.
...
The two synthetic tokens are defined as exported string constants (in `applyRescanDecisionUnderLock.ts`, co-located with the clause) so tests reference them by symbol. They are **diagnostic tokens in a `reviewCodes` array value**, NOT a `code:` field and NOT user-facing — invariant 5 (no raw codes in UI) does not apply (they never reach `mapResult`), and they are NOT §12.4 catalog codes (no `gen:spec-codes`/`catalog.ts` fan-out). Dedup via insertion-ordered `Set`.
...
| Demote via sentinel (`ONBOARDING_SCAN_REVIEW`/`FIRST_SEEN_REVIEW`) | `["ONBOARDING_SCAN_REVIEW", …]` | `true` |
...
**Completeness invariant (testable):** `demoted === true ⇒ reviewCodes.length > 0`. Proof: `isDirty` is true only if at least one of {`decisionItems`, `regressedGapClasses`, `PRIOR_PARSE_UNREADABLE` clause, `PRIOR_APPROVER_UNATTRIBUTABLE` clause} is active, and each contributes ≥1 token. (`sentinelItems` may add tokens but is not required for non-emptiness.)
...
- **Creates:** none.
- **Extends (executable):** `tests/onboarding/rescanDecision.test.ts` (regressedGapClasses), `tests/onboarding/applyRescanDecisionUnderLock.test.ts` (reviewCodes composition + completeness), `tests/api/rescanSheetRoute.test.ts` and/or `tests/api/admin/onboardingMutations-telemetry.test.ts` (emit carries the four `extra` fields).
- **Unaffected (must stay green):** `tests/log/_metaMutationSurfaceObservability.test.ts`, `tests/log/adminOutcomeBehavior.test.ts` — `SHEET_RESCANNED` registration + code-on-success-branch unchanged (still `code:"SHEET_RESCANNED"`; `extra` is additive).

## 9. Disagreement-loop preempts (do-not-relitigate)

- **Synthetic tokens are not §12.4 codes.** `PRIOR_PARSE_UNREADABLE`/`PRIOR_APPROVER_UNATTRIBUTABLE` live only inside a `reviewCodes` array value in telemetry `extra`; they are not `code:` literals, never reach UI, and require no catalog row. Cited: invariant 5 is user-visible-UI only; `mapResult` (route.ts:32-45) is untouched.
