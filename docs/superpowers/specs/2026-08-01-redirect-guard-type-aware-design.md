...
**Status:** R7 — whole-diff round-4 finding repaired · **Branch:** `test/redirect-guard-type-aware` · **Date:** 2026-08-01
...
- `FLAGGED_SPELLINGS` (19 rows) preserved with the compilable preamble, plus new positive rows R20–R82 (§6.1).
...
2. Updated `tests/cross-cutting/no-absolute-self-redirect.test.ts` (compilable fixtures; R20–R82, N1–N9, E1; memoized tree `describe` with JS sentinel and vacuous-walk floors).
