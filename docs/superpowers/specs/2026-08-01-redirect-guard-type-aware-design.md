...
2. **Reflection/eval.** `Reflect.get(NextResponse, k)`, `eval`. Distinct consequence bound from 1: these escape WITHOUT any receiver cast and cannot be pinned by a fixture (not statically expressible) — their coverage is that the spellings are rare, greppable, and loud in review (whole-diff r1 probes recorded both escaping).
