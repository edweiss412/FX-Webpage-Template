...
**Status:** R21 — whole-diff rounds 18–21 repaired (grammar edges, helper aliases, merge, corpus sync, wrapped inline helpers) · **Branch:** `test/redirect-guard-type-aware` · **Date:** 2026-08-01
...
**Global-carrier flows (whole-diff r14–r16):** `globalThis`/`window`/`self`/`global` carry the banned class one property down, so their VALUE flows are candidates too — decided by DESTINATION SHAPE (the contextual/asserted type exposes a callable `redirect`, bare or one carrier hop down), because these objects have pervasive legitimate uses and erasing polyfill casts must stay quiet (N16). Provenance is symbol-based (ambient node_modules declarations seed the set; shadowing locals never join — N17) and extends to a fixpoint through local aliases and single-file helper returns (R95–R96). Deeper environment-object indirection files to the §7 limit-2 evasion concession.
