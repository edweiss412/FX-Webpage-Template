...
**Status:** R28 — whole-diff round-28 repaired (legacy angle-bracket assertions at all five wrapper sites) · **Branch:** `test/redirect-guard-type-aware` · **Date:** 2026-08-01

**Whole-diff R28 disposition (2026-08-02):** one P1 — TypeScript's legacy `<T>x` TypeAssertionExpression was absent from every wrapper site. It now rides alongside `as`/`satisfies` at all five (position climb with carry-preservation, alias RHS, carrier-expression entry, both outer-destination walks). R107 pins `<any>globalThis` flows and `(<typeof pick>pick)()`; N20 pins angle-asserted receivers/new-callees quiet. The wrapper-kind axis is now grammar-complete: {parens, non-null, await, as, satisfies, angle-assertion} — the full set of TS transparent/cast wrappers.
...
**Whole-diff R18–R21 dispositions (2026-08-01):** r18 — conditional/logical (`?:`, `??`, `||`, `&&`) returns recurse; parenthesized/non-null callees unwrap; owned-return pruning extends to accessors, class expressions, constructors (R100; N18 rebuilt with a redirect-shaped sink); origin/main merged with both graduation sets layered. r19 — helper aliases join the carrier-function fixpoint at initializer and assignment sites. r20 — alias RHS unwraps transparent wrappers (parens/non-null/`as`/`satisfies`) on both alias kinds; R101/R101b pin each propagation path with exact counts; the importing mutant corpus gains the R94–R101 and N14–N18 rows. r21 — the function-like initializer branch classifies through the same RHS unwrap (`const pick = (() => globalThis)!` — R102), and the doc ranges below are synced through R102 (the r20 range sync had silently aborted; caught by the r21 review).

**Whole-diff R17 disposition (2026-08-01):** two P1 + three P2, all repaired in ONE comprehensive pass over the ENUMERABLE grammar instead of another increment (same-vector rule): carrier helpers now cover every single-file function-like form — FunctionDeclaration, FunctionExpression, ArrowFunction (block and expression bodies), async variants with `await` treated as transparent at both detection and flagging — and carrier aliases cover both declaration initializers and staged assignments (`let environment; environment = globalThis`), all symbol-keyed (R97–R99). Return scanning is scoped to returns OWNED by the helper (nested functions excluded — N18 pins the r17 false positive quiet). R95 is rebuilt as a reverse-ordered chain (use before every declaration; alias fed by a helper fed by a later alias) that only the fixpoint resolves, asserting `env2` by name. §5.5's stale R20–R93 range and the archive's unqualified global-carrier claim are synced. THE FENCE, restated as a completeness claim: this grammar — direct ambient references, local aliases (const/let/assignment), and single-file helper returns of any function-like form — is the WHOLE ratified category; environment objects stored in containers, class members, or crossing files are §7 limit-2 territory, and a new global-carrier finding must fall INSIDE the stated grammar to be admissible.

**Whole-diff R16 disposition (2026-08-01):** one P1 + three P2, all repaired. Global-carrier provenance is now SYMBOL-based: the four ambient names seed only when the identifier resolves to a node_modules declaration (a shadowing parameter or a local `const global = {…}` resolves elsewhere — N17 pins both quiet, closing the r16 false positives), and the fixpoint extends through local aliases AND single-file helper returns (function and arrow — R96), keyed by first-declaration nodes. R95 is strengthened to actually pin the fixpoint (the use sits above the declarations, and the assertion names `env2`). §5.1 gains the global-carrier paragraph below; the archive's broad claim is scoped by the same sentence. DEEPER environment-object indirection beyond aliases + single-file helper returns files to the §7 limit-2 evasion concession, like every other carrier family — ratified here to fence the subtree.

**Whole-diff R15 disposition (2026-08-01):** one P1 — `const environment = globalThis` lost carrier provenance, so the aliased flow escaped. Closed: global-carrier names extend through ordinary local aliases to a FIXPOINT (an alias-of-an-alias spells a tracked name at its own declaration), with the same destination-shape decision at every use; R95 pins the two-hop alias mutant. One P2 — the archive family summary now names require/import-equals/global-object carriers, and its probe-harness sentence reflects the split layout.

**Whole-diff R14 disposition (2026-08-01):** one P1 — `viaEnvironment(globalThis, url)` into a structurally-typed parameter destructuring `Response` escaped: the global objects are carriers (their types hold `Response`) but were untracked. Closed: `globalThis`/`window`/`self`/`global` value uses get a DESTINATION-typed, SHAPE-based decision — the naked flow flags when the contextual/asserted destination exposes a callable `redirect` (bare or one carrier hop down), which an erasing polyfill cast (`globalThis as Record<string, unknown>` — two legitimate tree sites false-flagged under the first own-type draft) does not. R94 pins the mutant; N16 pins ordinary global receiver/typeof uses quiet. One P2 — archive round count and family summary swept.
...
**Whole-diff R8 disposition (2026-08-01):** four findings. Finding 4 (REAL false positive — a local callable `interface Require` tripped the symbol-NAME check) is FIXED: the require branch now pins Node's declaration file (`isNodeRequireType`), with N12 pinning the local-interface case quiet; the repair's own probe also surfaced and fixed a latent order-dependence (bare-`require` fixtures resolved no `Require` symbol without another fixture having loaded @types/node — the fixture project now creates a types-anchor seed at construction). Findings 1–3 (nested re-export namespaces; require adapters/computed specifiers; literal-typed-variable import specifiers) are ratified to the DOCUMENTED EVASION CONCESSION rather than repaired: seven whole-diff rounds each moved the carrier boundary by one syntax, which is the unbounded-surface pattern the round-economy retrospective and this guard's own original BACKLOG entry both terminate by bounding the claim. The bound (new §7 limit 2, §3): the guard's threat model is INADVERTENT use; an author contrived enough to write adapter/nested-namespace/computed-specifier laundering can equally hand-roll the Location header, which has always been outside the claim (limit "hand-rolled Location", N4). The ninety-plus probed families remain pinned as regression floor, not as a completeness claim.

**Whole-diff R7 disposition (2026-08-01):** one RED finding — CommonJS carrier loads (`require("next/server")`, typed, untyped, or renamed) sat outside both the import-declaration and import-call detection. Closed (probe 12): a call whose CALLEE has the Node `Require` type and whose string specifier resolves to a module whose namespace carries is flagged at the require call (specifier verdicts cached per directory; unrelated modules stay quiet). Rows R90–R92 + N11. Also swept: the remaining stale ranges in the archive and test header now defer to §6.
...
| String-mediated dynamic access (eval-shaped) is a documented limit; receiver laundering, widened keys, and `Reflect.get` are CAUGHT (probe 7) | §7; E1 pins the eval shape; R68–R70 pin the flips |
| THREAT MODEL: inadvertent use is the claim; deliberate module-object laundering beyond the pinned §6 families is conceded alongside hand-rolled Location (whole-diff r8) | §3 admissibility, §7 limit 2 — escapes need a plausibly-inadvertent flow to be findings |
...
**Probe 12 — CommonJS require carriers closed (whole-diff r7 repair).** The callee's `Require` type identifies every require variant (incl. renamed `const r2 = require`); the string specifier resolves through the compiler host to the module whose namespace is then carrier-checked (all properties). Untyped, as-cast, and renamed variants flag; `require("react")` stays quiet.

**Probe 13 — Require-pin false-positive fix + fixture-project seed (whole-diff r8).** A local callable `interface Require` satisfied the symbol-NAME check (false finding); pinning to the @types/node declaration file distinguishes it. The probe also exposed that bare-`require` fixtures resolve NO `Require` symbol unless some fixture already pulled @types/node into the program — R90 failed under `-t` isolation — fixed by a types-anchor seed file created with the fixture project.
...
The guard's claim after this work: **in the walked roots, (1) any call whose callee statically resolves to `NextResponse.redirect` or `Response.redirect` is reported, and (2) any other reference to that method OR to the class object carrying it — extraction, storage, passing, adaptation, whole-object flow — is reported at the site where the method name or the class-object name is spelled.** Typed value flow BEYOND a flagged site needs no tracking: the extraction or object flow itself is already a finding (probes 4, 7). Convergence for review is this consequence bound plus the documented-limits section (§7) — NOT "no imaginable defeating input". A hypothetical escape is a finding only with (a) a live probe showing it escaping the CURRENT construction AND (b) a one-sentence argument that the flow is plausibly INADVERTENT — code a well-meaning author writes in a normal refactor. Deliberate-evasion constructions (adapters, nested re-export namespaces, computed specifiers, eval strings, or any flow more contrived than the pinned §6 families) file to the §7 evasion concession without a round: the guard's own hand-rolled-Location limit already concedes that a determined author bypasses it trivially, so closing exotic carrier channels adds no protection that concession has not already given up (finding-admissibility contract, AGENTS.md; ratified whole-diff r8).
...
function isBannedDecl(decl: Node | undefined): boolean {
  ...
  if (container !== "NextResponse" && container !== "Response") return false; // TypeLiteral's variable
  // Provenance pin (whole-diff r11): only the REAL classes ban; the
  // declaration must live under node_modules (next/dist, typescript/lib,
  // undici-types). An app-local class sharing the name is a domain model (N14).
...
- `auditTree()` — builds a fresh `Project` (same config; separate instance from the fixture project, so fixtures can never leak into the tree audit) with roots `app/**/*.{ts,tsx,js,jsx,mjs,cjs}`, `lib/**/*.{ts,tsx,js,jsx,mjs,cjs}`, `middleware.{ts,tsx,js}`, `proxy.{ts,tsx,js}`. Returns:
...
- `FLAGGED_SPELLINGS` (19 rows) preserved with the compilable preamble, plus new positive rows R20–R107 (§6.1).
...
| R90–R92 | CommonJS require carriers (whole-diff r7, probe 12) | untyped `require("next/server")`; `as typeof import(...)` cast; renamed require — flagged at the require call via the callee's `Require` type + resolved specifier |
...
| R94 | Global-object carrier into a structural parameter (whole-diff r14) | `viaEnvironment(globalThis, …)` destructuring `Response` — flagged at the naked global when the destination SHAPE exposes a callable `redirect` |
| R95 | Aliased global carrier, two hops (whole-diff r15; strengthened r16) | use ABOVE the declarations, assertion names `env2` — pins the fixpoint itself |
| R96 | Helper-return global carriers (whole-diff r16) | `function currentEnvironment() { return globalThis; }` and arrow twin — helper calls get the destination-shape decision |
| R97–R99 | Helper/alias grammar completion (whole-diff r17) | function-expression helper; async helper + `await`; staged-initialization alias — the complete single-file grammar |
...
| R106 | Awaited carriers (whole-diff r26) | async-helper promise awaited inline, via a resolved variable, and `await globalThis` |
| R107 | Legacy angle-bracket assertion carriers (whole-diff r28) | `<any>globalThis` flows and `(<typeof pick>pick)()` — fifth wrapper kind, same semantics as `as` |
...
| N20 | Angle-asserted non-extracting positions | carry-preserving `<typeof NextResponse>` climbs like `as` (r28) |
...
2. Updated `tests/cross-cutting/no-absolute-self-redirect.test.ts` (compilable fixtures; R20–R107, N1–N20, E1; memoized tree `describe` with JS sentinel and vacuous-walk floors).
