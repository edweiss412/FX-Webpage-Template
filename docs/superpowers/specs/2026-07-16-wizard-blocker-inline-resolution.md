...
**Reuse the lock-free core directly (mirroring finalize's inline auto-heal), NOT `rescanWizardSheet`.** `rescanWizardSheet` carries route-level semantics (its own `app_settings` session re-check + self-lock + checkpoint interaction) and self-locks the per-show key — nesting it under the route's lock would deadlock. Instead, rebuild follows the **finalize inline auto-heal precedent** (`app/api/admin/onboarding/finalize/route.ts:810-864`): acquire the per-show lock once, Drive-fetch + `prepareOnboardingFiles` for the single sheet (pre-lock, side-effect-free), then call the **lock-free** `applyRescanDecisionUnderLock(rescanTx, { …, isBlockerHeal: true })` directly on the held connection (`route.ts:859`). `isBlockerHeal: true` for the wizard rebuild (a finalize batch already ran; the publish path owns the manifest — the core must NOT restore it to `applied`, `route.ts:857`).

The core does NOT "rebuild a shadow that might still be corrupt" — it re-derives the sheet from Drive and **supersedes** the corrupt `shows_pending_changes` row into a fresh scan outcome (one of the seven `RescanDecisionOutcome`s). `postgres.js` commits on normal return (rolls back only on throw), so every typed outcome — success AND `hard_failed`/`needs_attention` — **commits**; the differentiator is whether that outcome deleted the corrupt shadow.
...
- **Cap consumption ⇔ the corrupt shadow row was deleted during THIS rebuild-initiated rescan.** The conditional increment `update onboarding_rebuild_attempts set attempts = attempts + 1 where … and attempts < $CAP` (upsert-if-absent) fires **in the same txn, immediately after each `shows_pending_changes` delete** (`applyRescanDecisionUnderLock:205` and `:258`), via a new optional `ApplyRescanDecisionDeps.onShadowDeleted` in-txn callback the route passes ONLY for cap-counted rebuilds (`ApplyRescanDecisionDeps` already exists — `applyRescanDecisionUnderLock.ts:56`; the plan adds the hook + invokes it at both delete sites). Shadow-RETAINED outcomes (`schema_missing`/`superseded`/`not_staged`) reach no delete → **no consume**, corrupt shadow intact, retry allowed. A throw rolls the whole txn back → no consume.
...
**Success-branch instrumentation (invariant 10).** The rebuild action mutates (shadow delete + restage + counter), so like unarchive it emits a **post-commit** breadcrumb `logAdminOutcome({ code: "ONBOARDING_BLOCKER_REBUILT", source: "api.admin.onboarding.resolveBlocker", driveFileId, wizardSessionId, showId, result: outcome.kind, extra: { corruptionReason, shadowDeleted } })` on every committed rebuild-initiated rescan (outside the lock txn). The gate-abort (`escalated`) and `not_currently_blocked` branches are non-mutating → no emit required. Both `resolve-blocker` breadcrumbs (`ONBOARDING_BLOCKER_UNARCHIVED`, `ONBOARDING_BLOCKER_REBUILT`) are registered in `AUDITABLE_MUTATIONS` with executable success-branch proofs (§6).
...
> **§3.7 topology (load-bearing):** rebuild is exactly ONE locked txn — the **route's own** `pg_advisory_xact_lock(hashtext('show:'||driveFileId))`, inside which it calls the lock-free `applyRescanDecisionUnderLock` (finalize inline-auto-heal pattern, `finalize/route.ts:859`). Gate-check + core re-derive + shadow delete + `onShadowDeleted` increment all inside it; `rescanWizardSheet`'s self-lock is NOT on this path (would nest). Pinned in `tests/auth/advisoryLockRpcDeadlock.test.ts`; a concurrent-double-submit test proves single consumption; an **outcome-enumeration test drives ALL seven `RescanDecisionOutcome`s and asserts `attempts` incremented ⇔ shadow deleted** (structural defense — the vector's comprehensive audit; §7).
...
| --- | --- | --- | --- | --- |
| `attempts` | new table `public.onboarding_rebuild_attempts (wizard_session_id uuid, drive_file_id text, attempts int not null default 0 check (attempts >= 0), escalation_logged boolean not null default false, updated_at timestamptz not null default now(), primary key (wizard_session_id, drive_file_id))` | `resolve-blocker` rebuild: conditional `update ... where attempts < CAP` **co-located with each `shows_pending_changes` delete** (`onShadowDeleted` in-txn hook) in the route's held lock/txn calling lock-free `applyRescanDecisionUnderLock` (consume ⇔ shadow deleted, R5 F1); the pre-restage gate READS only | (a) `resolve-blocker` pre-restage gate (`attempts >= CAP` → escalated); (b) `finalize-cas` sets `rebuildExhausted` per corrupt row | `attempts >= 1` (= CAP) ⇒ rebuild button suppressed, escalation shown |
...
- `rescanWizardSheet` — self-locks its mutation section (the standalone Step-3 re-scan path; NOT reused by rebuild).
- `finalize` inline auto-heal — holds the show lock, calls lock-free `applyRescanDecisionUnderLock` via `holdPort()` (`finalize/route.ts:810-864`). This is the pattern rebuild follows.
...
- `resolve-blocker / rebuild`: the route acquires the per-show lock ONCE and calls the lock-free `applyRescanDecisionUnderLock` directly (finalize inline pattern). The cap gate/increment, corrupt-shadow delete, and fresh `pending_syncs` restage all commit or roll back together in that one txn (§3.2). Single holder = the route; **never nested** (`rescanWizardSheet`'s self-lock is not on this path). Pinned by extending `tests/auth/advisoryLockRpcDeadlock.test.ts` + a concurrent-double-submit test.
...
- **2** (single-holder advisory lock): §3.7 — enumerated holders. **Wizard unarchive** route holds the per-show lock ONCE and calls the lock-free `_unarchive_show_apply` (it does NOT call `unarchive_show`, whose `is_admin()` gate the owner connection cannot satisfy). **Dashboard unarchive** remains the separate self-locking `unarchive_show` RPC path. **Rebuild** — the route holds the per-show lock ONCE and calls the **lock-free** `applyRescanDecisionUnderLock` directly (finalize inline-auto-heal pattern, `finalize/route.ts:859`), NOT `rescanWizardSheet` (which self-locks → would nest). Gate-check + core re-derive + shadow delete + `onShadowDeleted` increment all in the route's one txn. No path nests. Pinned in `advisoryLockRpcDeadlock.test.ts`.
...
- **10** (admin mutation instrumentation): `resolve-blocker` is an admin-gated mutating route under `app/api/admin/` → `AUDITABLE_MUTATIONS` registry rows (`tests/log/_auditableMutations.ts`) + executable success-branch proofs (`tests/log/adminOutcomeBehavior.test.ts`) for **BOTH** mutating branches: `ONBOARDING_BLOCKER_UNARCHIVED` (unarchive success) AND `ONBOARDING_BLOCKER_REBUILT` (every committed rebuild rescan — the rebuild path deletes/restages, so it cannot be silently dark). The `ONBOARDING_SHADOW_REBUILD_EXHAUSTED` escalation emits from **`finalize-cas`** (already an instrumented admin route) at exhaustion detection — its success-branch proof asserts the emit fires once when a corrupt row is re-observed with `attempts >= CAP` and flips `escalation_logged`. Static discovery meta-test (`_metaMutationSurfaceObservability.test.ts`) fails-by-default on the new `resolve-blocker` route until registered.
...
2. **Behavioral instrumentation:** sink-spy proving the success-branch emit fires only after the committed branch (invariant 10), for `ONBOARDING_BLOCKER_UNARCHIVED`, `ONBOARDING_BLOCKER_REBUILT` (every committed rebuild), and `ONBOARDING_SHADOW_REBUILD_EXHAUSTED`; non-mutating branches (`escalated`, `not_currently_blocked`) emit nothing.
...
- **Lock nesting** — rebuild is exactly ONE locked txn: the route's own advisory lock wrapping a direct call to the lock-free `applyRescanDecisionUnderLock` (finalize inline pattern, `finalize/route.ts:859`). Do NOT reuse `rescanWizardSheet` (self-locks → nests → M5 R20 deadlock class), and do NOT split into two txns.
