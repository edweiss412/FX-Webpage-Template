...
- **"sheets" label accuracy (R3 note):** `total`/`done` count items returned by `listFolder`, whose query filters to the Google Sheets MIME type (`lib/drive/list.ts:57`), so in production every counted item **is** a sheet and the "N of M sheets" / "Just read" copy is accurate. The `non_sheet` prepare branch (`runOnboardingScan.ts:911-913`) is reachable only via an injected test `listFolder`; such items still increment `done` and render their `name`, which is acceptable (tests assert counts, not the noun).
...
- `scanPreparedFiles` (`:804-869`): emit `deps.onProgress?.({ type:"staging" })` once before the per-file loop (`:815`). Its `deps` parameter is currently `Pick<RunOnboardingScanDeps, "runPhase1" | "withShowLock">` (`:808`) — **widen it to `Pick<RunOnboardingScanDeps, "runPhase1" | "withShowLock" | "onProgress">`** so `deps.onProgress` is in scope (R3 fix). The call site (`scanOnboardingPreparedFiles`/`runOnboardingScan` → `scanPreparedFiles(..., deps, ...)`, `:970,:986`) already forwards the full `deps`, so no call-site change is needed beyond the type widening.
