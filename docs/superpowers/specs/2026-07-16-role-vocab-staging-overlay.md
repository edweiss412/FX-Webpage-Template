...
**Infra-failure contract (adversarial R5 F1, invariant 9 — the finalize gate read is NOT the §3.1 best-effort loader; the two postures must never be conflated):** a fault on the per-row vocabulary SELECT (returned error or thrown) throws a typed infra error (`SyncInfraError` family, `lib/sync/perFileProcessor.ts` precedent). It is NEVER translated into an empty vocabulary — `[]` would falsely refuse every stamped row with the business code — and NEVER into `ROLE_MAPPINGS_OUTDATED_AT_FINALIZE`. The throw escapes to each route's existing unexpected-failure boundary (`finalize-cas/route.ts:924-934`): outer transaction ROLLS BACK (no row of the batch publishes partially-gated), `FINALIZE_CAS_UNEXPECTED_FAILURE` is logged with the error, and the client gets the typed 500 `ONBOARDING_FINALIZE_INTERNAL_ERROR`. Staged rows are preserved by the rollback; the admin retries finalize once the DB fault clears. Fail-closed in the correct direction: a transient outage blocks publication rather than publishing ungated or minting misleading business refusals.
...
15. **Gate infra-fault discrimination:** the per-row vocabulary SELECT faulting (injected returned-error AND thrown-error, invariant-9 both-paths) → the row is NOT published, the result is NOT `ROLE_MAPPINGS_OUTDATED_AT_FINALIZE` (assert the business code is absent), the typed infra error reaches the route boundary (500 `ONBOARDING_FINALIZE_INTERNAL_ERROR`), and the staged row survives for retry. Failure mode caught: an implementer reusing the §3.1 best-effort `[]` degrade at the gate, converting a DB blip into mass false refusals — the R5 F1 bug shape.
