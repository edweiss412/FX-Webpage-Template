...
Residual exposure (fully characterized): with the `FOR SHARE` serialization there is NO window in which a committed revoke/narrow precedes a publish of the stale grants. What remains is only the genuinely post-publish revoke — a settings mutation whose commit lands after the row's publish (either issued later, or issued mid-publish and blocked by the share lock until the publish committed). That class is unpreventable by any gate (the revoke postdates the publish) and its persistence characteristics are exactly the parent feature's existing convergence window, tracked as `BL-ROLE-VOCAB-MAPPING-CONVERGENCE` (§3.4).
