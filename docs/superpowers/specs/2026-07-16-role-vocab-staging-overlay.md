...
- **No finalize freshness gate for revoked/narrowed mappings (considered and rejected — do not relitigate).** A mapping deleted or narrowed between staging and finalize means the staged parse publishes with the staging-time grants baked in, converging on the next PROCESSED sync (§3.4 states the window precisely — it is NOT bounded by the next cron tick). This is NOT a new posture: the parent spec ratified exactly this convergence semantics for the settings mutation paths — "Settings-page `update`/`delete` return plain ok/error — no attached re-sync, convergence is cron-driven (§7)" and the §7 delete row promises reversion only "on each show's next sync" (parent spec `:167`) — and every already-published live show has the identical window today: after a settings delete, a published show's grants persist until that show's next processed sync, and cron does not process an unmodified sheet. `role_flags` are rebuilt from the fresh parse + current vocabulary on every processed sync, never accumulated, so when a sync does run, convergence is structural and downward-capable. Grantable flags gate crew-page tile visibility (parent spec §1: capability checkboxes gating scope tiles), a class the project treats as admin-correctable display scope, not a security boundary (M11.5 pivot: role filtering is UX, not security). The fail-closed alternative (persist a staging-time applied-mappings snapshot in `parse_result`, thread it through the Flow-B shadow payload like `useRawDecisions`, add a finalize refuse branch + cataloged code + rescan-heal path) would gate ONLY the wizard-publish instance of a window the settings paths leave open on every live show — security theater unless the whole-feature convergence question is solved first. That whole-feature question (should `role_token_mappings.updated_at` participate in the cron watermark, or should settings mutations fan out targeted re-syncs?) is filed as `BL-ROLE-VOCAB-MAPPING-CONVERGENCE` in `BACKLOG.md` as part of this PR's doc tasks (§5) — it is a parent-feature gap this spec surfaces but does not own. Escalation trigger: an actual mis-scoped-tile report from Doug, or the backlog item's design work.
