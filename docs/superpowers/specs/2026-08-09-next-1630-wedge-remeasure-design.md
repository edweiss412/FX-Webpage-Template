...
4. **Counting rule (flip-precise).** One SAMPLE = one executed repeat of the round-trip case; each sample performs TWO flips (`expectFlipLanded` for the OFF flip and the ON flip, tests/e2e/admin-lifecycle-transitions.spec.ts:430 and :474). A **wedged flip** = one `[wedge-recovery] <label>: tier=plain did not land, escalating` line — `expectFlipLanded` emits exactly one such line per flip that fails the plain wait (tests/e2e/admin-lifecycle-transitions.spec.ts:187), so this count can neither double-count a reload-tier wedge nor conflate the two flips. A **wedged sample** = a sample with ≥1 wedged flip. Landed-tier lines (`landed at tier=…`, :182) are recorded as color but are NOT the count. Logs via `gh run view --log`; trace artifacts retained 7 days (workflow:135-146).
5. **Run and sample validity.** A dispatch run is a VALID measurement run iff the transitions measurement step executed its repeats (the job can die earlier: the layout-spec step precedes it, workflow:109-110, and the job has a 35-minute ceiling, workflow:34). An invalid run (setup failure, layout-step failure, timeout before/inside the measurement step) is DISCARDED — noted with its run URL in the PR body — and replaced by a fresh dispatch. Within a valid run, a VALID sample = a repeat that either passed, or failed while showing ≥1 `[wedge-recovery]` line (it reached the subject; a repeat failing at the reload tier already emitted its plain-escalation line and counts as a wedged sample). A repeat that failed with NO `[wedge-recovery]` line never observed the subject (e.g. an unrelated harness flake): it is an invalid sample, noted in the PR body. Dispatch additional runs until ≥20 valid samples exist; the decision rule reads exactly the first 20 valid samples in dispatch order.

### §4.1 Decision rule (stated before measuring)

- **0 wedged samples in 20 valid samples (0 wedged flips in their 40 flips) → upstream fix confirmed.** Against the 7/10 baseline, Fisher exact one-sided p ≈ 5.9×10⁻⁵ (0/20 vs 7/10, sample-grained). The entry closes (§5 outcome A).
...
- Move the entry from BACKLOG.md to BACKLOG-archive.md with probe evidence: all valid dispatch run URLs, both vendored canary hashes (`3f0b9e61-20260317` wedged 7/10; `cbb046ab-20260731` wedged 0/20 samples, 0/40 flips), and the §4.1 decision rule. The archived body preserves the entry's two watch signals verbatim and notes the e2e recovery tiers remain in `expectFlipLanded` deliberately (self-documenting; zero cost on the healthy path).
...
- **Minor-version framework bump — the PR trigger matrix is NOT the full suite set (probed 2026-08-09).** Auto-running on a `package.json` + `pnpm-lock.yaml` diff: the unfiltered `pull_request` workflows (`crew-e2e.yml`, `lifecycle-layout-e2e.yml`, `quality.yml`, `section-header-visual.yml`, `standalone-e2e.yml`, `unit-suite.yml`, `x-audits.yml`) plus the two path-filtered workflows whose filters include the dependency files (`phantom-gap-e2e.yml`, `step3-live-bundle.yml`). **Dark on this diff** (path filters exclude both dependency files): `admin-layout-e2e.yml`, `help-affordances.yml`, `published-modal-e2e.yml`, `screenshots-drift.yml`, `mutation-harness.yml`, and `dev-gate-e2e.yml` (no `pull_request` trigger at all). All six have `workflow_dispatch`. The protocol therefore REQUIRES manual dispatch, on the branch, of the five Next-rendering-sensitive dark suites — `admin-layout-e2e.yml`, `help-affordances.yml`, `published-modal-e2e.yml`, `dev-gate-e2e.yml`, `screenshots-drift.yml` — each green (or its failure dispositioned) before merge. `mutation-harness.yml` is deliberately NOT dispatched: it exercises vitest guard suites, not browser rendering, and `unit-suite.yml` (auto-running, unfiltered) already compiles and runs the same test surface under the bumped Next.
...
- AC-2: All auto-triggered PR checks green on the branch head that merges, AND the five required dark-suite dispatches (§6 list) green or dispositioned on that head.
