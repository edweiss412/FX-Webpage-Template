...
- Admin preview-as (`viewer.kind === "admin_preview"`): `{ surface: "admin", surfaceIdScope: "admin-preview-card-${crewMemberId}", extraContext: { crewPreview: { crewMemberId, name, role } } }` — `name`/`role` resolved exactly as the footer does (`ctx.viewerName`, `ctx.viewerCrew?.role ?? null`, `_CrewShell.tsx:362-366`). The `crewMemberId` in the scope keeps each previewed viewer's draft + idempotency attempt separate (mirrors the footer's `admin-preview-footer-${slug}-${crewMemberId}`); without it, switching previewed viewers on the same card/show would resume a stale draft and mis-tag the submission's `crewPreview`.
