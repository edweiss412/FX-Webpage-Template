...
**Backlog:** `BL-EVENT-DETAILS-UNRENDERED` · audit finding **H3** (`docs/audits/info-tab-fidelity-audit-2026-06-29.md`)
...
4. **Sentinel handling differs by surface, intentionally — this is an EXISTING, TESTED contract.** The crew card hides sentinels (`shouldHideGenericOptional`, inherited from `KeyValueRows`) so the crew see only real values. The operator modal shows every **non-empty** known key as-parsed (gate on `value.length > 0` after coerce+trim) — **including** `TBD`/`N/A` — so the operator can verify the raw parsed state at the publish gate (a `TBD` tells the operator a cell parsed-but-unfilled, distinct from one that didn't parse). **DO NOT relitigate this asymmetry:** it predates this feature and is pinned by `tests/components/admin/wizard/Step3Review.test.tsx` — the existing event-details test asserts `keynote: "TBD"` renders, with the comment "shown as-parsed (review surface, not sentinel-hidden like the crew page)." Gating the modal on `shouldHideGenericOptional` would break that test and erase a deliberate review-surface affordance. (The loose `BL` wording "non-sentinel keys" is superseded by this actual tested contract.)
...
Replace the hard-coded keynote+reel push with an iteration over `EVENT_DETAILS_LABELS` (insertion order). **Coerce FIRST, then keep non-empty (review surface shows sentinels as-parsed — decision 4):** for each `key`, `const text = String(ed[key] ?? "").trim();` then `const value = key === "opening_reel" ? stripOpeningReelText(text).trim() : text;` then `if (value.length > 0) fields.push({ label, value });`. Gate on `value.length > 0` (NOT `shouldHideGenericOptional`, NOT `hasContent(ed[key])` against the RAW value — `hasContent` rejects non-strings, skipping a numeric value). `String(...)` coerces non-strings (a numeric value still shows); the `.trim()` means whitespace-only never inflates `count`; `TBD`/`N/A` ARE shown (the existing tested contract).
...
- A single sentinel value (e.g. `record: "N/A"`) → crew card: omitted (`KeyValueRows` / `shouldHideGenericOptional`); modal: SHOWN as-parsed ("Recording: N/A"), the review-surface contract (decision 4). Empty/whitespace → omitted on both.
...
4. **Modal renders all non-empty known TEXT keys as-parsed** (Step3SheetCard test): `event_details` with stage_size/podium/polling/keynote → `EventDetailsBreakdown` lists all of them (count matches), not just keynote+reel; **a sentinel (`led: "N/A"`, `keynote: "TBD"`) → SHOWN as-parsed (review surface, NOT hidden — the existing `Step3Review.test.tsx` contract)**; `diagrams` present → NOT listed (text-key scope); opening_reel URL stripped; a whitespace-only value → omitted; **a non-string value (number) → coerced+shown (no throw)**. Catches: the 2-of-19 gap; reel-strip regression; diagrams leak; whitespace count-inflation; admin/crew non-string divergence. Anti-tautology: assert against the breakdown's own list, scoped so a sibling section can't satisfy it.
