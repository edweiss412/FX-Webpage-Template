...
### 3.9 What the per-mutant child actually runs, and why it is not the whole suite

The score is only as honest as the run that produced it, so the two deliberate differences between a per-mutant child and a normal `unit-suite` run are stated here rather than left to be discovered from a config.

**`bail: 1` on the per-mutant child (`tests/mutation/source/mutantOverlay.config.ts`).** THE CONTRACT: the exit code is the entire VERDICT-BEARING signal the harness consumes. `childRun` returns `outcome.code` on a normal exit and otherwise THROWS `MutantRunInfraError` (timeout, or death by signal), so the two channels are disjoint: a verdict is an exit code, and an infra fault is an exception. `bail` acts only on the first channel, and only on its timing -- it changes WHEN a failing run stops, never WHETHER it failed. There is no input under which stopping earlier turns a non-zero exit into a zero one. A mutant the suite rejects still exits non-zero and is still KILLED; a SURVIVING mutant fails nothing, so bail never fires and it pays for its whole suite exactly as before. Both halves are pinned behaviorally in `tests/mutation/_metaOverlayConfigParity.test.ts` ("the per-mutant child bails on the first failure, and only on a failure"), by a fixture pair whose last case writes a marker only a non-bailed run can reach. A test asserting `cfg.test.bail === 1` would pass whether or not vitest honoured it, which is the failure worth catching. This is harness-wide: every enrolled surface gets both the saving and the contract.

**The thirty-two-form case does not run under mutation.** It builds thirty-two scratch roots at ~11 MB and cannot share them (the scanner caches parsed files by absolute path and never invalidates, §5.6), so it is ~15s of this file's ~45s and every mutant paid it. It is skipped from inside the body rather than with `it.skipIf`, because the coverage-map totality case matches lines starting with `it(` and the `it.skipIf(...)(` form would drop the case out of the map and silently stop requiring its `// covers:` line. The case still runs on every normal `unit-suite` run, so no CI coverage is lost; what it stops doing is contributing kills to the score. The resulting under-count is measured, not assumed, and is the documented limit in §6.

**Why both, rather than either.** They cover disjoint populations. `bail` reclaims the 236 KILLED mutants, which stop at their first failure. It can do nothing for the 14 SURVIVORS, which fail nothing and so pay every case; only removing the expensive case from the child reaches them. Measured on the pre-change run: three mutants on the tie-break at `controlOutlineResidue.ts:343` ground for 25, 57 and 125 minutes, 207 of that run's 335, because a late expensive case kept running under mutants already rejected. Together they bring the surface under the `source-shards` job cap of `timeout-minutes: 125`, which it exceeded on its own at 140 minutes of serial child time. The acceptance condition was drafted as "shards 0 AND 1 green under the cap" and was RE-STATED by the orchestrator on 2026-08-24, because the question it asks is whether the two levers fit the job: ALL FOUR shards completed inside the cap on run 32753100923 (81m, 114m, 58m, 85m), which answers it. Shard 1's failure there was an unaccepted survivor on `ledgerGit`, a surface this change does not touch, so it is a content failure and not a timing one. Stating acceptance over all four shards also keeps it partition-independent, which matters because merging main re-partitions every shard.

**A third change, and the defect that forced it: a contended suite manufactures FALSE KILLS.** Under the harness a suite that reds for ANY reason records its mutant as KILLED, so a case that crosses its timeout under I/O contention is indistinguishable from a mutant the suite rejected. This is not hypothetical here. Commit `6aa429064` raised the thirty-two-form case to 120s for exactly this reason and fixed the instance rather than the class: at least two more cases carry the same cost shape, and both were caught red on UNMUTATED source on 2026-08-24 within one session -- `key membership is by compiled declaration` timed out at 30s under load 27 and ran 11.4s at load 13, while `the key and the oracle read ONE selector rule` measured 56.3s outright. The scored run taken between those observations recorded `logical-connector:291:30:&&>||` and `integer-literal:294:41:0>1` as KILLED, contradicting their filed equivalence rows; re-probed against a GREEN baseline both SURVIVE. The rows were right and the record was wrong. The repair is a FILE-LEVEL `vi.setConfig({ testTimeout: 120_000, hookTimeout: 120_000 })`, not a per-case budget: three mechanical attempts to enumerate the at-risk subset disagreed with each other (a call-site count returned zero, because the helper is written once inside a `.map` over an N-element literal; a map-aware count missed two cases measurement caught; measurement missed one the structural count caught), and a budget that must be attached to the right subset is one refactor away from missing a case. It is inert on a passing test, since a failing assertion still fails immediately, so it can only prevent a spurious failure and never mask a real one. It also closes a latent red on `unit-suite`, a REQUIRED context.

**Why this matters to the score and not just to CI.** A false kill inflates the numerator and empties a filed equivalence, so the gate reports `stale-ledger-row` and invites exactly the wrong repair: deleting a correct row to make the gate green. That is the same error, in the same direction, as accepting a wall-clock ceiling kill as evidence. Both are refused here, and both are refused for the same reason -- a verdict is only evidence when the suite that produced it was green on unmutated source in the same conditions.

**Expected side effect, to be confirmed from the re-score record rather than assumed.** A killed mutant that stops at its first failure has far less opportunity to reach the wall-clock ceiling and be recorded as a `timeout` deciding child, which `gate.ts` itself calls "NOT evidence the suite rejected the mutant". The pre-change run carried three such cells. The re-score records 0, so the expectation held and the class is closed on this surface rather than merely predicted.
...
- **The score does not include the thirty-two-form case's kills.** That case is skipped in the per-mutant child (§3.9), so any mutant it ALONE would have killed is scored SURVIVED or is absent from the killed set. The size of that set is measured rather than assumed: the killed set under the shipped config, compared against the pre-change run's 236, is exactly the set of kills unique to the case. Measured: 0. The direction is conservative, since a lost kill lowers the score rather than inflating it, and the case's assertions still run on every `unit-suite` run. Re-file trigger: the `source-shards` job cap rising far enough to hold the case, or the case becoming roughly ten times cheaper, at which point it returns to the per-mutant child and the score is re-measured with it. The second is nearer than it reads. What forbids the cheaper form today is one line, `tests/styles/interactiveScanCore.ts:479`, a module-level `sourceCache` keyed by absolute path that `parse` reads and nothing ever invalidates, which is why thirty-two scratch roots cannot be one root reused. A fix making scratch-root reuse safe is in flight on a sibling arc as of 2026-08-24 and is sequenced after this PR, so the honest statement is that this limit expects to be re-opened rather than to stand: when reuse lands, re-include the case and re-measure, and the under-counted kills below come back with it.
