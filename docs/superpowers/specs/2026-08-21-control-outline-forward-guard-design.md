...
**Why both, rather than either.** They cover disjoint populations. `bail` reclaims the <<FILL:KILLED>> KILLED mutants, which stop at their first failure. It can do nothing for the <<FILL:SURVIVORS>> SURVIVORS, which fail nothing and so pay every case; only removing the expensive case from the child reaches them. Measured on the pre-change run: three mutants on the tie-break at `controlOutlineResidue.ts:343` ground for 25, 57 and 125 minutes, 207 of that run's 335, because a late expensive case kept running under mutants already rejected. Together they bring the surface under the `source-shards` job cap of `timeout-minutes: 125`, which it exceeded on its own at 140 minutes of serial child time, and which is the acceptance condition for this change: shards 0 AND 1 green under the cap in real CI.
