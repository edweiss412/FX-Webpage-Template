...
| `focus-state-chrome` | every weak token in the key carries a `focus:` or `focus-visible:` variant; the same render alternative of the LIVE element carries a `focus:ring-*` or `focus-visible:ring-*` token, read from the element's full class strings because ring tokens sit outside the paint projection by construction; reason states what carries the focus indication | none |
