...
**The acceptance floor, stated once and counted once.** FIVE source mutations that must RED (draft, R1, R2, R5, new control) and TWO control edits that must stay GREEN (a padding-only edit to a track; a token reorder of a track's recipe). Seven cases. Every later count in this spec refers back to this sentence. The shipped key (§3.3, the sorted paint projection) was then run on all seven, on the same scratch roots, 2026-08-21:
...
The seven cases ship in the suite as its acceptance floor (§5), so the table above is red-then-green under the shipped mechanism and not only under the prototype.
...
**Probe domain.** The live `app/` + `components/` corpus as scanned (362 elements at `00beaf19e`), plus the seven cases of §1.4.3's acceptance floor (five source mutations and two control edits), each applied as a source edit to a copy of `components/admin/PublishedToggle.tsx` or `components/admin/UnignoreButton.tsx` exactly as in §1.4.3. A probe input outside this domain, or more than one ordinary edit away from an input in it, files to §6, not to a finding.

**Closed convergence criterion.** The acceptance floor of §1.4.3 holds under the shipped suite (five source mutations red, two control edits green) and the unmutated corpus is green, with the suite's self-proofs (§5) passing; and the surface is enrolled in `tests/mutation/source/registry.ts` with its score and an empty unaccepted-survivor set stated in the round-1 diff brief. Never an open enumeration of spellings, tokens, or component shapes.
...
A token is weak when its UTILITY is exactly `border-border` or `border-border-strong`. The utility is the token after two strips, in this order: the variant chain (everything up to the last colon at zero bracket/paren depth, so `[&:hover]:`, `min-[480px]:` and `border-(--x)` survive), then ONE leading or trailing important marker `!` (the legacy `!border-border` form and the Tailwind v4 `border-border!` form). That is the contract `normalizeToken` in `tests/styles/_childlessGrowableScan.ts` already ships; the residue module replicates it rather than importing it, because that module's import graph is heavy and the residue module runs once per mutant under the harness, and the suite pins equality between the two on every token of the live residue plus the eight probe forms below (AC-13), so the copy cannot drift silently. Whole-token match only: `border-border-strong-x` and `border-borderline` are not matches. A token carrying both markers (`!border-border!`) is not Tailwind syntax and is outside the grammar (§6). Tokens are the whitespace-split pieces of each readable class string, the same split `tests/styles/_metaControlOutlineFill.test.ts` uses.
...
- **AC-2** The seven cases of §1.4.3's acceptance floor, applied to scratch copies of the corpus inside the suite: the five source mutations each produce set inequality with the new key's `file` asserted by equality, and the two control edits each produce set equality. Channel: seven `it` cases, one per case.
- **AC-3** The no-defect baseline for AC-2: the same scratch copy, unmutated, produces a residue equal to the live tree's. Channel: one `it` case, run before the seven.
...
- **AC-13** The token grammar agrees with the shipped normaliser: for every token of every live residue element and for the eight probe forms of §3.2, `utilityOf(token)` equals `normalizeToken(token)` from `tests/styles/_childlessGrowableScan.ts`; and each of the eight forms, substituted into a scratch copy of `UnignoreButton.tsx`, is residue. Channel: two `it` cases, the second with eight scratch roots.
...
- **PROBE DOMAIN:** the live `app/` + `components/` corpus as `scanInteractiveElements` reads it (362 elements at `00beaf19e`), and the seven cases of §1.4.3's acceptance floor (five source mutations, two control edits) applied to a copy of `components/admin/PublishedToggle.tsx` / `components/admin/UnignoreButton.tsx`. One ordinary edit away from one of those is admissible; a constructed component shape that occurs nowhere in the corpus is §6.
