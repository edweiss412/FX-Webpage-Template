...
- **Key membership is by compiled declaration, with two named exclusions.** `border-radius` and its corner forms are shape, not paint, and are outside the key; a boundary painted through `ring-*`, `outline-*`, `shadow-*` or a `divide-*` child rule declares no `border*`/`background*` property on the element's own rule and is outside the key and the oracle alike (probed, §3.3). `sr-only` (`border-width: 0`) is inside, because removing the outline is paint.
...
- **PROBE DOMAIN:** the live `app/` + `components/` corpus as `scanInteractiveElements` reads it (362 elements at `00beaf19e`), and the twelve cases of §1.4.3's acceptance floor (ten source mutations, two control edits) applied to a copy of `components/admin/PublishedToggle.tsx` / `components/admin/UnignoreButton.tsx`, plus the thirty-two forms of §3.2's two tables, the nine tokens of §3.3's probe, and the duplicate-key instance of §3.4. One ordinary edit away from one of those is admissible; a constructed component shape that occurs nowhere in the corpus is §6.
