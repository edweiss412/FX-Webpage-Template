...
An element is **residue** when any readable class string, on any render alternative, carries a **weak outline token**: a utility that BEGINS WITH the stem `border-border` (so `border-border`, `border-border-strong`, and either with an opacity modifier such as `/50`), under any variant chain (`focus:`, `max-sm:`, `hover:`, `aria-expanded:`, `before:`, …). One stem, named, and nothing after it consulted (§3.2); everything else is outside the accept-set (§6). `unresolved` does not exempt: an unresolved element whose readable strings carry a weak token is residue like any other, because the unread span can only add paint, never remove the token already seen.
...
**The acceptance floor, stated once and counted once.** SIX source mutations that must RED (draft, R1, R2, R5, new control, new control at `border-border-strong/50`) and TWO control edits that must stay GREEN (a padding-only edit to a track; a token reorder of a track's recipe). Eight cases. Every later count in this spec refers back to this sentence. The shipped key (§3.3, the sorted paint projection) was then run on the first seven, on the same scratch roots, 2026-08-21; the eighth, the opacity-modifier control spec round 3 brought, was run the same day on a fresh root under the stem predicate of §3.2, with the first seven re-run under it and unchanged:

```
[RED draft OFF fill->bg-surface]   RED   PublishedToggle.tsx  bg-accent border border-accent-edge || bg-surface border border-border-strong
[RED R1 third branch]              RED   PublishedToggle.tsx  … || bg-surface border border-border-strong || bg-surface-sunken border border-border-strong
[RED R2 tokens-include]            RED   PublishedToggle.tsx  … || bg-surface-sunken bg-warning-bg border border-border-strong
[RED R5 nested span]               RED   PublishedToggle.tsx  bg-surface border border-border-strong
[RED NEW control]                  RED   UnignoreButton.tsx   bg-surface border border-border-strong
[RED NEW control at /50]           RED   UnignoreButton.tsx   bg-surface border border-border-strong/50   (live residue=12, mutant residue=13; GREEN, a silent clear, under the exact-match predicate this round replaced)
[GREEN control: padding-only edit] GREEN
[GREEN control: R3 token reorder]  GREEN
...
The eight cases ship in the suite as its acceptance floor (§5), so the table above is red-then-green under the shipped mechanism and not only under the prototype.
...
**Probe domain.** The live `app/` + `components/` corpus as scanned (362 elements at `00beaf19e`), plus the eight cases of §1.4.3's acceptance floor (six source mutations and two control edits), each applied as a source edit to a copy of `components/admin/PublishedToggle.tsx` or `components/admin/UnignoreButton.tsx` exactly as in §1.4.3. A probe input outside this domain, or more than one ordinary edit away from an input in it, files to §6, not to a finding.

**Closed convergence criterion.** The acceptance floor of §1.4.3 holds under the shipped suite (six source mutations red, two control edits green) and the unmutated corpus is green, with the suite's self-proofs (§5) passing; and the surface is enrolled in `tests/mutation/source/registry.ts` with its score and an empty unaccepted-survivor set stated in the round-1 diff brief. Never an open enumeration of spellings, tokens, or component shapes.
...
- A new module `controlOutlineResidue` in `tests/styles/` (importable, no side effects at import, no `process.exit`): exports `isWeakOutlineToken(token)`, `paintProjection(path)`, `residueKey(element)`, `residueOf(rootDir)` (the derived population, as a `Map<key, count>` plus the element list for messages), `RESIDUE_CENSUS` (the rows), `RESIDUE_CATEGORIES`, `validateRow(row, element, css, ledgerText)` (the live element because the focus-ring check reads its full strings; the ledger text because a `backlogRef` must resolve to a `^## <id>` heading whose body names the row's file), and `recordedRatio(outlineToken, fillToken, css)` for the `switch-track` recomputation. The contrast arithmetic reuses the WCAG relative-luminance form already in `tests/styles/secondary-action-contrast.test.ts`; the plan decides whether to lift it into a shared helper or replicate the six lines, and says why.
...
A token is weak when its UTILITY BEGINS WITH the stem `border-border`. Nothing after the stem is read: `-strong`, an opacity modifier (`/50`, or the arbitrary alpha form /[.5] with the same compiled effect), or anything else. The predicate models no suffix, so there is no suffix a spelling can hide behind. The only Tailwind utilities carrying the stem are the two weak colours, `--color-border` and `--color-border-strong`, the only `--color-border*` declarations in `app/globals.css` (pinned by AC-15), with or without Tailwind's opacity modifier, which compiles `border-border-strong/50` to the same weak colour at half alpha (`color-mix(in oklab, var(--color-border-strong) 50%, transparent)`; spec round 3 brought the compiled output and the exact-match predicate's silent clear of it). A stem match on a token that does not compile (`border-border-strong-x`, `border-borderline`) is a red plus a row, the conservative direction (§6); it cannot clear anything. The utility is the token after two strips, in this order: the variant chain (everything up to the last colon at zero bracket/paren depth, so `[&:hover]:`, `min-[480px]:` and `border-(--x)` survive), then ONE leading or trailing important marker `!` (the legacy `!border-border` form and the Tailwind v4 `border-border!` form). That is the contract `normalizeToken` in `tests/styles/_childlessGrowableScan.ts` already ships; the residue module replicates it rather than importing it, because that module's import graph is heavy and the residue module runs once per mutant under the harness, and the suite pins equality between the two on every token of the live residue plus the sixteen probe forms below (AC-13), so the copy cannot drift silently. A token carrying both markers (`!border-border!`) is not Tailwind syntax and is outside the grammar (§6). Tokens are the whitespace-split pieces of each readable class string, the same split `tests/styles/_metaControlOutlineFill.test.ts` uses.

Probed 2026-08-21 on a scratch copy with `UnignoreButton.tsx`'s `border-text-faint` replaced by each form; every one is residue and enters the paint projection under the same normaliser:

```
!border-border             residue=true   border-border!             residue=true
!border-border-strong      residue=true   border-border-strong!      residue=true
focus:!border-border       residue=true   focus:border-border!       residue=true
sm:border-border-strong    residue=true   [&:hover]:border-border    residue=true
```

And the eight opacity-modifier forms, probed the same way after the stem repair (every one was `residue=false` under the exact-match predicate, the round-3 finding):

```
border-border/50           residue=true   border-border-strong/50    residue=true
focus:border-border-strong/50  residue=true   max-sm:border-border/50  residue=true
!border-border-strong/50   residue=true   border-border-strong/50!   residue=true
border-border-strong/[.5]  residue=true   sm:!border-border/25       residue=true
```

### 3.3 The paint projection (closed)

A render alternative's projection is the SORTED list of its tokens whose utility (after the same two strips as §3.2, so a leading `!` cannot hide a paint token from the key) begins with `border` or `bg-`; the token is kept in its raw spelling, opacity modifier included, so `bg-surface` and `bg-surface/50` are different keys: the projection already did for paint what §3.2 now does for the weak predicate. Sorted, because Tailwind resolves two conflicting utilities by their order in the generated stylesheet and never by their order in the class attribute, so a reorder of class tokens is semantically null and must not red the guard (the R3 control of §1.4.3). Everything else is outside the key by construction: padding, gap, rings, shadows, opacity, text colour, transitions. The accept-set is two prefixes, default-denied; a utility family outside it that affects effective paint is §6.
...
- **AC-2** The eight cases of §1.4.3's acceptance floor, applied to scratch copies of the corpus inside the suite: the six source mutations each produce set inequality with the new key's `file` asserted by equality, and the two control edits each produce set equality. Channel: eight `it` cases, one per case.
- **AC-3** The no-defect baseline for AC-2: the same scratch copy, unmutated, produces a residue equal to the live tree's. Channel: one `it` case, run before the eight.
- **AC-4** Every category bar is proven in BOTH directions. Refusals, by equality on the message: a `switch-track` row with three alternatives; a `switch-track` row with two fills on one alternative (the R2 shape); a `side-divider` row carrying a bare `border` beside `border-t`; a `focus-state-chrome` row with a bare weak token; a `responsive-skin-filed` row whose `backlogRef` resolves to nothing; a `responsive-skin-filed` row whose `backlogRef` resolves to an entry that does not name its file; a `filed-defect` row with a blank reason; a row with an unknown category. Acceptances, one per category, each one variable away from a refusal above: the three live tracks; AttentionMenu's divider with `last:border-b-0`; a skip link; ShareHub citing its own entry; a `filed-defect` row validated against a constructed ledger text whose entry names the row's file (the ledger is an input to `validateRow`, so the acceptance is attributable). Channel: eight refusal cases and five acceptance cases.
...
- **AC-11** Per-category counts: the seeded census holds exactly 3 / 5 / 2 / 2 / 0 rows by category, asserted against literals. Channel: one `it` case.
...
- **AC-13** The token grammar agrees with the shipped normaliser: for every token of every live residue element and for the sixteen probe forms of §3.2, `utilityOf(token)` equals `normalizeToken(token)` from `tests/styles/_childlessGrowableScan.ts`; and each of the sixteen forms, substituted into a scratch copy of `UnignoreButton.tsx`, is residue. Channel: two `it` cases, the second with sixteen scratch roots.
...
- **AC-15** The stem premise is pinned: the sorted list of `--color-<name>` declarations in `app/globals.css` whose name begins with `border`, the `-runtime` indirection excluded exactly as `colourNames` excludes it, equals `["border", "border-strong"]`. A third `border*` colour fails this pin and forces a re-decision of the stem in this spec instead of a silent widening of what the stem admits. Channel: one `it` case, equality on the array.
...
4. **Negative controls are expect-a-report, and every expect-clean case is paired.** The eight floor cases of AC-2 and the thirteen bar cases of AC-4 each assert a produced value by equality. Every case that expects an element NOT to be residue lives in a fixture that also contains an element that IS residue, one variable away, and asserts both, with a `premise` that the fixture produced at least two elements; a fixture the scanner fails to parse therefore reds the premise instead of passing the negative. No case in the suite passes because something was absent.
5. **Pre-dispatch mutants (four, per `docs/agents/writing-plans.md`, string-presence guards).** Recorded in the seeding commit, each as a paired fixture per item 4: (a) the weak-token predicate neutralised to `false` (never an excision, so the module stays collectable), expected to red the eight floor cases and the census equality; (b) a token one character short of the stem (`border-borde`) beside `border-border-strong/50`, expected NOT residue and residue respectively (the stem is a prefix test and this pins both of its edges); (c) the token present but not live: inside a comment, inside an attribute other than `className`, each beside a live `className` twin, expected NOT residue and residue; and behind a literal `false &&` branch, expected residue (the scanner reads both branches and the path model keeps the dead one; that is the conservative direction and is asserted, not assumed); (d) each discriminating parameter varied in a paired fixture: the variant chain present/absent, `unresolved` true/false, a `<div>` beside a `<button>` for the tag in and out of `isInScope`.
...
- **One stem only.** `border-border` is the stem of the two tokens the two rulings name and the two the swap arcs moved, `border-border` and `border-border-strong`, with or without an opacity modifier. A resting outline at a third weak token (`border-surface-raised`, `border-bg`, an arbitrary value `border-[#cfcdc7]`, a CSS-variable form `border-(--color-border-strong)`, `outline-*` or `ring-*` used as a boundary, `divide-*` on a parent) is outside the accept-set. Re-file trigger: a third token appearing on an interactive element in a census re-run, at which point it joins the grammar as a deliberate edit with its own seeded rows.
...
- **Important markers.** One leading or one trailing `!` is stripped by the normaliser (§3.2). A token with both, or a `!` inside a variant, is not Tailwind syntax; it is outside the grammar and files here.
- **Stem matches that do not compile.** `border-border-strong-x` or `border-borderline` carry the stem and are residue: the suite reds and asks for a row, and the author deletes the typo instead. A red on a token that paints nothing is the conservative direction and stays here rather than in the grammar; a predicate that declined to fire on a suffix it did not recognise is the exact-match predicate spec round 3 showed clearing `border-border-strong/50`.
...
- **PROBE DOMAIN:** the live `app/` + `components/` corpus as `scanInteractiveElements` reads it (362 elements at `00beaf19e`), and the eight cases of §1.4.3's acceptance floor (six source mutations, two control edits) applied to a copy of `components/admin/PublishedToggle.tsx` / `components/admin/UnignoreButton.tsx`, plus the sixteen token forms of §3.2 and the duplicate-key instance of §3.4. One ordinary edit away from one of those is admissible; a constructed component shape that occurs nowhere in the corpus is §6.
