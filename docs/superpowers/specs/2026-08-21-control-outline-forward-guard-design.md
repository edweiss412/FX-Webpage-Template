...
**The acceptance floor, stated once and counted once.** NINE source mutations that must RED (draft, R1, R2, R5, new control, and the four grammar mutants spec rounds 2 to 4 and the orchestrator's ruling of 2026-08-21 put on the floor: a new control at `!border-border-strong`, at `border-border-strong/50`, at `border-t-border-strong`, at `border-[#cfcdc7]`) and TWO control edits that must stay GREEN (a padding-only edit to a track; a token reorder of a track's recipe). Eleven cases. Every later count in this spec refers back to this sentence. The shipped key (§3.3, the sorted paint projection) was then run on all eleven under the oracle of §3.2, one fresh scratch root per case (§5.6), 2026-08-21, spec round 5; this run supersedes the grammar-predicate runs of rounds 1 to 4. The four lines marked `(cascade)` are the multi-token probe the ruling required and are not floor cases; the two `planted defect` lines are AC-16:
...
The eleven cases ship in the suite as its acceptance floor (§5), so the table above is red-then-green under the shipped mechanism and not only under the prototype.
...
The residual hazard is the one the batch named: a lazy reason row. The guard therefore demands a FORM, not prose, and validates it: Throughout the table, a weak token is a token `classify` (§3.2) marks weak on some side; the bars never spell.
...
**What the bound does not say, stated so it is not read into it.** It does not say the residue's effective paint is correct; it says the residue's paint tokens are frozen and reasoned. It does not say a control at a THIRD colour, one whose declaration references neither weak variable nor their values, is caught (§6); it does not reach paint Tailwind did not compile (§6). It does not reach an `<input type="text">`, a `<textarea>`, a `<select>`, or an outline painted on a non-interactive child (§1.1, `BL-CONTROL-OUTLINE-BEYOND-ELEMENT-COVER`).

**Probe domain.** The live `app/` + `components/` corpus as scanned (362 elements at `00beaf19e`), plus the eleven cases of §1.4.3's acceptance floor (nine source mutations and two control edits), each applied as a source edit to a copy of `components/admin/PublishedToggle.tsx` or `components/admin/UnignoreButton.tsx` exactly as in §1.4.3. A probe input outside this domain, or more than one ordinary edit away from an input in it, files to §6, not to a finding.

**Closed convergence criterion.** The acceptance floor of §1.4.3 holds under the shipped suite (nine source mutations red, two control edits green) and the unmutated corpus is green, with the suite's self-proofs (§5) passing; and the surface is enrolled in `tests/mutation/source/registry.ts` with its score and an empty unaccepted-survivor set stated in the round-1 diff brief. Never an open enumeration of spellings, tokens, or component shapes.
...
- **AC-2** The eleven cases of §1.4.3's acceptance floor, applied to scratch copies of the corpus inside the suite: the nine source mutations each produce set inequality with the new key's `file` asserted by equality, and the two control edits each produce set equality. Channel: eleven `it` cases, one per case.
- **AC-3** The no-defect baseline for AC-2: the same scratch copy, unmutated, produces a residue equal to the live tree's. Channel: one `it` case, run before the eleven.
...
- **AC-13** The projection's normaliser agrees with the shipped one, and the oracle classifies every spelling the grammar rounds raised: for every token of every live residue element, `utilityOf(token)` equals `normalizeToken(token)` from `tests/styles/_childlessGrowableScan.ts`; and each of the twenty weak spellings of §3.2's table, substituted into a scratch copy of `UnignoreButton.tsx`, is residue, while the two controls (`border-borde`, `border-text-faint/50`) are not. Channel: two `it` cases, the second with twenty-two scratch roots.
...
- **Weak means the two variables or their four values.** A declaration is weak when it references `--color-border` or `--color-border-strong` or carries one of their literal light or dark values; the arbitrary value and the CSS-variable form are therefore caught (§1.4.3). A resting outline at a THIRD colour (`border-surface-raised`, `border-bg`), or a boundary painted by a property the oracle does not read (`outline-*`, `ring-*`, `divide-*` on a parent), is outside the oracle's question by design: the rulings name two colours and one property, and a third colour or another property is a new ruling, not a spelling. Re-file trigger: a third colour appearing as a resting outline on an interactive element, at which point it joins `WEAK_COLOURS` as a deliberate edit with its own seeded rows.
...
- **Tokens that compile to nothing.** `group`, `peer`, a typo: a non-report, justified in §3.2 (a token that compiles to nothing paints nothing in production either). The suite never reds on them and never asks for a row.
...
- **PROBE DOMAIN:** the live `app/` + `components/` corpus as `scanInteractiveElements` reads it (362 elements at `00beaf19e`), and the eleven cases of §1.4.3's acceptance floor (nine source mutations, two control edits) applied to a copy of `components/admin/PublishedToggle.tsx` / `components/admin/UnignoreButton.tsx`, plus the twenty-two forms of §3.2's table and the duplicate-key instance of §3.4. One ordinary edit away from one of those is admissible; a constructed component shape that occurs nowhere in the corpus is §6.
