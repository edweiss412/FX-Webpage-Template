...
### 2.2a Client persistence framing (correction — Codex R1)
**Client telemetry is warn/error ONLY** — `clientLog` gates the transport on `level==="warn"||level==="error"` (`clientLog.ts:14`), the transport input type only accepts warn/error (`clientErrorTransport.ts:18`), and the route rejects any other level (`route.ts:91`). Both warn and error **always** persist regardless of code. So on the client path, `code` is purely for **categorization** (code-keyed dashboards / grouping) — it does **NOT** flip persistence (there are no client `info` rows to flip). The "info persists only with a code" rule applies only to SERVER info rows (e.g. S7's `DRIVE_WEBHOOK_RECEIVED`), never to this client surface.
...
- **Watch renewal failure** (`lib/drive/watch.ts`): **Log in `refreshWatchSubscriptions`, NOT at the shared chokepoint (Codex R1 HIGH).** `markWatchOrphanedWithTx` (`:384,397`) is called for `watch_create_failed` AND `activate_failed_after_watch_created` too — logging `DRIVE_WATCH_RENEWAL_FAILED` there would mislabel initial create/activate orphans (which are NOT renewals and already raise a `WATCH_CHANNEL_ORPHANED` admin_alert). The renewal-specific gap is that `refreshWatchSubscriptions` (`:425-428`) **discards** the `SubscribeResult.outcome`. Fix there: capture `const result = await subscribe(row.watchedFolderId)` and, when `result.outcome === "orphaned"`, `void log.warn("watch channel renewal failed", { source:"drive.watch", code:"DRIVE_WATCH_RENEWAL_FAILED", channelId: result.channelId, watchedFolderId: row.watchedFolderId })`. Only a renewal that orphaned emits the code; the loop's control flow (`refreshed.push`) is otherwise unchanged.
...
**File:** `lib/sync/runManualSyncForShow.ts`. **Logging-only.** `deleteLiveDeferral?.(driveFileId)` (`:425`) returns `void`, so the deferral kind is NOT knowable from the delete (Codex R1 MEDIUM). **Contract (concrete):** the tx interface already exposes `tx.readLiveDeferral(driveFileId): Promise<DeferredIngestionRow | null>` (`runScheduledCronSync.ts:131,953`; `DeferredIngestionRow.deferred_kind: "defer_until_modified" | "permanent_ignore"`). Inside the `withShowLock` callback (`:411-440`), capture the kind BEFORE the delete and hoist it to an outer-scope var (like `appliedShowId` in the retry route), then log POST-COMMIT (after `applyResult` resolves at `:441`) so a rolled-back tx never logs:
- Before `:425`: `const priorDeferral = await tx.readLiveDeferral?.(driveFileId); priorDeferralKind = priorDeferral?.deferred_kind;` (assign to an outer `let priorDeferralKind: string | undefined`), THEN `await tx.deleteLiveDeferral?.(driveFileId)`.
...
This makes visible the "reprocesses every cron run until a human re-defers" condition (Anomaly A). Code inside a dotted `log.warn` span → exempt. **Auto-restore is NOT done here** (BACKLOG).
