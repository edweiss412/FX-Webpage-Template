...
| `t2-share-batches` | active crew link (URL + Copy) and the multi-batch email note ("Your crew list needs N separate emails") with numbered mailto actions | `fixture: { share: { linkActive: true, crewEmails: 60 } }` — 60 generated distinct emails overflow `MAX_MAILTO_HREF_CHARS` so `buildCrewLinkMailtos` returns >1 batch (pinned by a unit test); `landing: "overview"` |
...
| `share.crewEmails < 1` or non-integer | Validation error |
