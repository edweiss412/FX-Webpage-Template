...
export type ScenarioChangeLogRow = {
  ...
  // Open-ended, matching the DB (no CHECK on change_kind; production also writes
  // e.g. "use_raw_stale") and the reader's `string` typing. Non-blank; undo gating
...
export type ScenarioFixture = {
  ...
  clientAbsent?: boolean; // true → snapshot client_label "" (NOT null - shows.client_label is NOT NULL in the DDL; the adapter maps the stored empty string to the absent UI state via str(...) || null, components/admin/review/publishedAdapter.ts:63-65)
  // Share-link surface: activates the crew link + email actions inside ShareHub.
  // linkActive threads a synthetic share token to the switcher's ShareTokenProvider
  // (a NEW per-scenario `shareToken` field on GallerySwitcherScenario, since the
  // token is provider-level, not a modal prop). crewEmails RESHAPES THE SNAPSHOT
  // ROSTER: applyFixture gives exactly N snapshot crew members an email (adding
  // generated members past the base 6 when N > the effective crew count), and the
  // modal's `crewEmails` prop is DERIVED from the reshaped snapshot rows the same
  // way the production loader derives it from publishedData.crewMembers
  // (app/admin/_showReviewModal.tsx:358-362) - never injected independently.
  // With enough fixed-length emails the mailto hrefs exceed MAX_MAILTO_HREF_CHARS
  // (1900, app/admin/show/[slug]/crewLinkMailto.ts:12) and ShareHub renders the
  // multi-batch note + numbered actions (components/admin/showpage/ShareHub.tsx:442-451).
...
- Else → `{ entries: shapeChangeFeed(toChangeLogRows(s), toHoldRows(s)), truncated: s.feedTruncated === true }`, where `toChangeLogRows` synthesizes ids (`${s.id}-log-${i}`) like `toHoldRows` does. Returns `null` (as today) exactly when the scenario has no holds AND no changeLog — `feedNull` plays NO role in this function (its entry-exclusivity guard means a `feedNull` scenario always lands in this absent-`null` arm anyway); the infra `feed: null` override is applied solely by `buildScenarioModalData`.
...
Guard: `feedNull: true` combined with `holds.length > 0`, a non-empty `changeLog`, or `feedTruncated === true` is a validation error (empty arrays and false/absent flags are legal — emptiness equals absence) — a null feed cannot carry entries, and holds would desync the changes-rail badge from the feed (Codex R1 P1 class from the switcher spec).

### 3.2 Lifecycle knobs

`buildScenarioModalData` (`lib/dev/buildScenarioModalData.ts:50`) threads `s.fixture` into both halves:

- Snapshot half — `buildGallerySnapshot(warnings, { anchors, fixture })`: `titleAbsent` → `show.title: ""`; `datesAbsent` → `dates: { travelIn: null, set: null, showDays: [], travelOut: null }`; `clientAbsent` → `client_label: ""` (storable empty; DDL is NOT NULL); `archived`/`published` mirrored into `show.archived`/`show.published`; `empty`/`volumes` per §3.3/§3.4.
- Data half — `buildGalleryModalData` overrides: `archived`, `published`, `finalizeOwned`, `isLive`, `lastSyncStatus`, `neverSynced` → `lastSyncedAt: null, lastCheckedAt: null`, `titleAbsent` → `title: null` (NOT `""` — the header falls back via `title || slug` but StatusStrip/ShareHub receive `title ?? slug`, so only null falls back in both, matching production's empty-to-null conversion at `app/admin/_showReviewModal.tsx:288`). (`openSheetHref` is always derived from the fixture drive_file_id — see the parity table; the R7-cut `sheetLinkAbsent` knob has no data-half mapping.)

Consistency is owned by ONE mapping function (`applyFixture`, unit-tested) so the snapshot and the modal props can never disagree.

**Derivation parity table (structural defense, added after four review rounds on this vector).** Every `GalleryModalData` field that production DERIVES from the snapshot must be derived the same way in the gallery — never independently injected. The full inventory, from the production loader (`app/admin/_showReviewModal.tsx`):

| Field | Production derivation | Gallery rule |
| --- | --- | --- |
| `crewEmails` | roster ≤ 500 ? email-bearing `publishedData.crewMembers` : `[]` (`app/admin/_showReviewModal.tsx:347-362`) | derived from the (reshaped) snapshot rows with the same cap + filter |
| `previewRoster` (inside `data`) | blanked to `[]` when the TOTAL roster exceeds 500 (`app/admin/_showReviewModal.tsx:347-358`) — crew-row action menus key off it | same over-cap transformation applied when `volumes.crew > 500` |
| `pickerCrew` | `archived ? [] : snapshot.crew_members → {id,name,role}` (`app/admin/_showReviewModal.tsx:374-379`) | SAME — the base fixture's hardcoded `pickerCrew: []` is a parity bug this feature fixes: every non-archived scenario derives pickerCrew from its snapshot crew, so `PickerResetControl` shows its real enabled-idle state for crew-bearing scenarios and "No crew to reset yet" only for genuinely crew-less ones (`t2-share-link`, `t2-nothing-parsed`, and archived scenarios) |
...
A unit test walks this table: for each row it builds a knob-bearing scenario and asserts the gallery value equals the production derivation applied to the gallery snapshot.
...
`empty` list empties the corresponding snapshot collections: `crew` → `crew_members: []`; `venue` → `venue: null`; `rooms` → `rooms: []`; `hotels` → `hotel_reservations: []`; `transport` → `transportation: []`; `contacts` → `contacts: []`; `billing` → `coi_status: null` (and no other billing sources); `agenda` → `agenda_links: []`. Empty-state copy verified in `components/admin/wizard/step3ReviewSections.tsx` at lines 1442 (crew), 1031 (venue), 2364 (hotels), 1128 (transport), 1779 (rooms), 1323 (billing), 939 (contacts). `agenda` is NOT an empty-copy branch: emptying `agenda_links` removes the Agenda SECTION entirely (`includesAgenda`, `components/admin/review/sectionInclusion.ts:27-29`). "No run-of-show parsed." (line 1739) comes via `datesAbsent` (bookend days derive from dates; `run_of_show` is already `{}` in the base fixture), and "No event details parsed." (line 1987) is inherited base-fixture emptiness (`event_details: null` absent the event anchor) — neither is an `empty` key.
...
- `diagramImages: 13` → diagrams anchor's `embeddedImages` gets 13 generated entries typed as full `PersistedEmbeddedImage` rows (`lib/parser/types.ts:332-348`): `objectId`, `mimeType` (allowed image MIME), `sheetTab`, `sheetsRevisionId`, `embeddedFingerprint`, `recovery_disposition`, plus the snapshot path field — the render guards (`lib/admin/stagedDiagramGuards.ts:11-29`) filter entries missing the string trio. HONESTY CAVEAT (review R9): the gallery is non-materialized with a synthetic show id, and the diagram asset route 410s on a non-UUID revision / missing manifest (`app/api/asset/diagram/[show]/[rev]/[key]/route.ts:143-145`), so every tile's `<img>` load FAILS and renders the "Preview unavailable" per-tile fallback — the scenario therefore demonstrates the GRID structure, the "+1 more" overflow note, AND the preview-unavailable tile state together, and is documented as such (real image bytes are reachable only through the materialize path on a real show). The unit test asserts 12 tiles + the note; the e2e asserts the fallback text on a tile. Requires `anchors.diagrams` (validation error otherwise).

The 99+ pill cap is DESCOPED: `validateScenario` rejects duplicate alert codes because `admin_alerts` has a partial unique index on (show_id, code) for open alerts (`lib/dev/attentionScenarios/validate.ts:187-191`), and `ATTENTION_ROUTES` holds ~45 codes total — so >99 open rows in one class is not a production-reachable state and the gallery must not fabricate it. Failed-keys "+N more" (>6, `components/admin/review/AttentionBanner.tsx:142`) is a `TILE_PROJECTION_FETCH_FAILED` context with 7 keys. The >2 crew under-row stack disclosure ("N more") needs 3 cards under ONE rendered member: only two crew-scoped autocorrect codes exist, and under-row routing requires an autocorrect subject matching a rendered member's canonical key (`lib/admin/sectionWarningModel.ts:119-136`) — plain `buildWarning` rows carry no such payload. The stack therefore composes 2 crew-scoped warnings that BOTH (a) carry a crew-routing `blockRef` (`blockRef.kind: "crew"` — routing is by `sectionForWarning`'s `KIND_TO_SECTION`, `lib/admin/step3SectionStatus.ts:22-36`; the autocorrect code alone does NOT route to Crew) and (b) carry `autocorrect.subject` naming one fixture member (the under-row index key, `lib/admin/sectionWarningModel.ts:119-136`), PLUS 1 crew-routed alert whose identity names the same member (`AMBIGUOUS_EMAIL_BINDING` with a matching `galleryIdentity`); a unit test asserts 3 cards stack under that member. All plain scenario data.
...
| `t2-finalizing` | "Finalizing…" chip on the ON toggle | `fixture: { finalizeOwned: true }`; `landing: "overview"` |
| `t2-publishing` | "Publishing…" chip — the distinct `published: false && finalizeOwned: true` production mode with its own copy (`components/admin/PublishedToggle.tsx:150-157` region) | `fixture: { published: false, finalizeOwned: true }`; `landing: "overview"` |
| `t2-share-link` | active crew link, URL + Copy, ZERO email actions — a published zero-crew show (link activity depends only on publish/archive/token, `components/admin/showpage/ShareHub.tsx:209-220`) | `fixture: { empty: ["crew"], share: { linkActive: true, crewEmails: 0 } }`; `landing: "mixed"` (empty-crew also renders "No crew parsed.") |
| `t2-share-single` | active crew link with the singular unnumbered "Email this link to crew" action (exactly 1 batch) | `fixture: { share: { linkActive: true, crewEmails: 3 } }` — 3 short generated emails fit one batch; `landing: "overview"` |
| `t2-share-batches` | active crew link and the multi-batch note ("Your crew list needs N separate emails") with numbered mailto actions | `fixture: { share: { linkActive: true, crewEmails: 60 } }` — 60 deterministic ~64-char emails: batching keys off encoded `mailto:` HREF LENGTH vs `MAX_MAILTO_HREF_CHARS` 1900 (`app/admin/show/[slug]/crewLinkMailto.ts:94-123`), not recipient count, so the generator fixes email length and a unit test pins `buildCrewLinkMailtos(...).length > 1`; `landing: "overview"` |
...
| `t2-diagram-images` | thumbnail grid structure + "+1 more" note + per-tile "Preview unavailable" fallback (see §3.4 honesty caveat — real bytes need materialize) | diagrams-anchored alert + `fixture: { volumes: { diagramImages: 13 } }` |
...
| `t2-all-ignored` | the all-ignored posture: zero ACTIVE warnings → "Nothing needs a look on this sheet" clean copy while the "Ignored (2)" disclosure still renders its muted cards (routing counts only active warnings, `lib/admin/routedWarnings.ts:44-55`; `components/admin/showpage/sectionWarningExtras.tsx:155-170`) | 2 warnings, both ignored via `ignoreWarningIndexes: [0, 1]` (non-blank distinct rawSnippets, same section); `landing: "warnings"` |
...
| `t2-warning-spread` | warning-pointer overflow past 3 named sections (`POINTER_NAME_CAP` 3, `components/admin/wizard/step3ReviewSections.tsx:708`). The section callout's "+N more in Parse warnings" was found STAGED-ONLY during review R5 (`ShowReviewSurface` gates `calloutEntries` on `isStaged(data)`, `components/admin/review/ShowReviewSurface.tsx:1040-1047`) — out of scope per §1.1 class 7 | warn-severity warnings routing (via `blockRef.kind`) to 5+ distinct sections (conditions pinned by a unit test against `components/admin/wizard/step3ReviewSections.tsx:707-717`); `landing: "mixed"` |
...
- `scenarioGroup` (`app/admin/dev/attention-gallery/buildSwitcherScenarios.ts:46`): when derived sections ∪ warning sections is empty AND `s.landing` is set, return `s.landing` (instead of `"baseline"`). Fixture-only scenarios thereby land in their real modal section group; `landing` is ignored when real sections exist. Warning-section routing gains production's rendered-section fallback: a warning whose `sectionForWarning` target is NOT rendered for this scenario's data routes to `"warnings"` (mirroring `warningsBySection`, `lib/admin/step3SectionStatus.ts:84-95`) — concretely, `empty: ["agenda"]` removes the Agenda section (the one data-dependent section), so an agenda-routed warning groups under `warnings`, not `mixed`. Alert-side sections get the SAME effective-placement treatment: an anchored item whose anchor is absent from the scenario's data is grouped under `overview`, mirroring the modal's unavailable-anchor redirect (`components/admin/showpage/PublishedReviewModal.tsx:367-392`) — today that corrects exactly one rendered scenario, `T2_ANCHOR_ABSENT` (a rooms-anchored alert deliberately built with no anchor, currently mislabeled `rooms` while its banner renders in Overview).
- `isModalVisible` (`buildSwitcherScenarios.ts:32`) gains a visible-state-carrier arm: a scenario is also visible when `feedNull === true`, `changeLog` is non-empty, or `fixture` is present with at least one knob — the clean-baseline arm (`alerts.length === 0 && holds.length === 0`) does NOT cover a scenario that combines a cut-only alert with one of these carriers, and without the new arm such a scenario would be mislabeled "cut" and dropped. `isModalExpressible` unchanged.
- Share-token threading: `GallerySwitcherScenario` gains `shareToken: string | null` (serializable). `partitionScenarios` sets it from `fixture.share?.linkActive` (a fixed synthetic token string); `AttentionModalSwitcher` passes `initialToken={current.shareToken ?? null}` to its `ShareTokenProvider` (currently hardcoded null at `components/admin/dev/AttentionModalSwitcher.tsx:159`) — and the PROVIDER itself gets `key={current.id}`: the provider preserves its current token when `initialToken` changes at the same epoch (`app/admin/show/[slug]/ShareTokenContext.tsx:44-69` documents that the caller must key it), so keying only the modal would leak token A into scenario B and an active token into null-token scenarios. A unit test exercises both transitions (active→null and the per-scenario remount).
...
| `changeLog` row with unknown `status`/`source` | Validation error (typed unions; validate re-checks at runtime for JS callers) |
| `changeLog` row `change_kind` blank | Validation error. NOT an enum check — `show_change_log.change_kind` is deliberately open-ended (production writes e.g. `use_raw_stale`, `lib/sync/changeLog/writeUseRawStaleChanges.ts`) and the reader types it as `string`; any non-blank string validates, and undo gating keys only off the crew-domain set |
| `changeLog` row `occurred_at` not parseable as a date, or non-null `acknowledged_at` not parseable | Validation error (the shaper's `toIso`/`sortKeyFromRaw` consume them; the hold validator's parseability check at `lib/dev/attentionScenarios/validate.ts:141-143` is the template) |
| `feedNull: true` + (`holds.length > 0` OR `changeLog` non-empty OR `feedTruncated === true`) | Validation error (§3.1). `holds: []` (mandatory on every scenario), `changeLog: []`, and `feedTruncated: false`/absent are all LEGAL beside `feedNull` — emptiness equals absence |
| `fixture: {}`, `empty: []`, `volumes: {}`, or any knob explicitly set to its base-default value (e.g. `archived: false`, `lastSyncStatus: "ok"`) | Validation error — no-op knobs are rejected so "fixture present" is always semantically effective; this keeps the `isModalVisible` carrier arm (§3.7) key-presence-simple without admitting cut-only scenarios that render nothing new |
| `fixture.volumes.crew: 0` | Validation error (positive integers only; use `empty` for zero) |
| `volumes` value equal to the base count (`crew: 6`, `rooms: 3`, `hotels: 2`) | Validation error (a no-op that would falsely activate the visibility carrier — same class as base-default knobs) |
...
| `empty` contains `"agenda"` + (`volumes.agenda` or `volumes.agendaLinks`) | Validation error (contradictory) |
| `volumes.agenda` + `volumes.agendaLinks` together | Validation error (both reshape the same `agenda_links` array with incompatible recipes — enrich-the-base vs replace-the-set) |
...
| `share.crewEmails >= 1` + effective TOTAL roster > 500 (e.g. `volumes.crew: 501`) | Validation error (production blanks ALL crew emails when the roster row count exceeds `CREW_ROSTER_READ_CAP` 500 regardless of how many carry addresses, `app/admin/_showReviewModal.tsx:347-362`) |
| `changeLog` longer than 50 rows | Validation error (the production reader's page limit — a longer feed cannot come from the modeled read path, `lib/sync/feed/readShowChangeFeed.ts` `DEFAULT_LIMIT`) |
...
- `scenarioGroup` landing fallback: fixture-only scenario + landing → landing; landing + real sections → real group; and the rendered-section fallback DISTINGUISHER — an agenda-routed warning combined with `empty: ["agenda"]` groups under `warnings`, a test the raw `sectionForWarning` routing would FAIL (it returns `agenda` without consulting the rendered registry).
...
Caps cited once in §2 (`CREW_CAP` 30 / `ROOMS_CAP` 20 / `HOTELS_CAP` 12 / `SCHEDULE_DAYS_CAP` 14 / `SCHEDULE_ENTRIES_CAP` 6) plus `PACK_LIST_CASES_CAP` 12 / `PACK_LIST_ITEMS_CAP` 8 / `POINTER_NAME_CAP` 3 (`components/admin/wizard/step3ReviewSections.tsx:152-163` and `components/admin/wizard/step3ReviewSections.tsx:708`). Roster volumes (crew 31 / rooms 21 / hotels 13 / schedule 15×8 agenda + 1 synthetic day / 13 diagram images / 13×9 pack list) each exceed their cap by construction. 36 new scenarios in §3.6 (3 changes-class, 8 lifecycle, 10 sync-posture, 1 minimal-header, 1 nothing-parsed, 9 volume/overflow (incl. roster-over-cap, multi-agenda, hotel-guest-stack, alert-deep-link), 1 diagram, 1 attention-extras, 2 ignored-warnings). 8 `empty` keys. 5 change-log statuses; `change_kind` open-ended (no finite cardinality — DB and reader both type it as string); 4 sources; 11 `t2-changelog-history` log rows + 1 hold = 12 feed entries.
...
**R8 (Codex via codex-guard, 2026-07-22): BLOCKING, 5 findings — all repaired.** F1: `t2-all-ignored` added (zero active warnings → clean copy + Ignored disclosure co-render). F2: stale `sheetLinkAbsent` data-half mapping deleted (R7 cut consistency). F3: matrix cardinality corrected to "exactly 11 rows". F4: empty-section evidence corrected (contacts line 939; agenda = section removal; event-details = inherited base emptiness). F5: `scenarioGroup` test suite gains the rendered-section-fallback distinguisher (agenda warning + empty agenda → `warnings` group).
...
**R5 (Codex via codex-guard, 2026-07-22): BLOCKING, 5 findings — all repaired.** F1: `t2-warning-spread` drops the "+N more in Parse warnings" callout claim (staged-only surface, class 7). F2: the attention-extras crew warnings must carry a crew-routing `blockRef.kind: "crew"` in addition to `autocorrect.subject` (the code alone does not route to Crew). F3: `share.crewEmails` now RESHAPES the snapshot roster and `crewEmails` derives from it via the production derivation; consistency guards added (share×empty-crew, share×volumes.crew). F4: no-op fixture knobs (base-default values, empty containers) are validation errors, keeping the visibility carrier arm semantically sound. F5: change_kind cardinality claim dropped (open-ended).

**R4 (Codex via codex-guard, 2026-07-22): BLOCKING, 5 findings — all repaired.** F1: `ShareTokenProvider` gets `key={current.id}` (provider preserves token across same-epoch `initialToken` changes; keying only the modal leaks tokens across scenarios). F2: `isModalVisible` gains a visible-state-carrier arm (feedNull / non-empty changeLog / non-empty fixture). F3: `t2-hold-dispositions` carries BOTH rename renderings (plain + folded) — 4 holds. F4: share roster split into zero-batch / single-batch / multi-batch scenarios; multi-batch guaranteed via fixed-length emails + pinned unit test (batching keys off HREF length, not count). F5: production-bound guards added — `changeLog` ≤ 50 rows, `share.crewEmails` ≤ 500.

**R3 (Codex via codex-guard, 2026-07-22): BLOCKING, 7 findings — all repaired.** F1: §3.2 data-half override corrected to `title: null` (the last stale `""`). F2: `t2-publishing` added (published:false ∧ finalizeOwned:true, "Publishing…" copy). F3: timestamp-parseability guards added for `occurred_at`/`acknowledged_at`. F4: ShareHub multi-batch state added — `share` knob + `shareToken` threading through `GallerySwitcherScenario` into the switcher's `ShareTokenProvider` + `t2-share-batches` (60 emails > MAX_MAILTO_HREF_CHARS batching). F5: `change_kind` opened to non-blank `string` (DB is open-ended; production writes `use_raw_stale`). F6: schedule-cap arithmetic restated per the renderer's index-then-exempt semantics ("…and 4 more days"). F7: `feedNull` exclusivity restated as emptiness-equals-absence.
