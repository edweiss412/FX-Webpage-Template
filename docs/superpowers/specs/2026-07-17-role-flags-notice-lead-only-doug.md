...
The shared writer diffs ALL applied role_flags changes (LEAD and non-LEAD), resolving the prior through the rename map so a same-sync rename+role co-change is caught (Round 2, Codex F4 — a renamed member's prior lives under the OLD name; `prevByName.get(next.name)` alone would miss it):

```ts
// writeRoleChangeLogRows(port, previousCrewMembers, appliedCrewMembers, renames, heldNames, occurredAt)
...
for (const next of appliedCrewMembers) {
  ...
  if (heldNames.has(next.name) || heldNames.has(priorName)) continue;   // held → excluded (feed entry from sync_holds)
  ...
  //   entity_ref = next.name (successor name — WHO),
  //   summary = `Crew member ${next.name} role assignment changed: ${fmt(prior.role_flags)} → ${fmt(next.role_flags)}` (WHAT),
  //   before_image = after_image = null (not undoable).
}
```

- `fmt` renders the flag array as a readable token list (`A1` / `A1, BO` / `none` for `[]`). Role tokens are crew-domain data (like the crew name already in `crew_added` summaries), NOT error/invariant codes — invariant 5 holds (no raw error code in `summary`/`change_kind`).
- **This SUBSUMES the MI-9 arm of the generic `field_changed` block in `writeAutoApplyChanges.ts`** (`:142-160`): remove `MI-9` from that arm (keep `MI-8`/`MI-8b`/`MI-8c`) so a LEAD change no longer emits the anonymous "A field changed" row — the shared writer emits its identifiable row instead (LEAD is a role_flags change, caught by the loop) PLUS the durable `LEAD_ROLE_APPLIED` event. No double-emit: role rows come ONLY from the shared writer; `writeAutoApplyChanges` keeps crew_added/removed/renamed + MI-8-financial/section/asset rows. Because the shared writer runs unconditionally in `runPhase2` (not gated by `feedPolicy`), removing the MI-9 arm does NOT lose the LEAD change-log row on the staged path — it gains it (staged previously wrote none).
- **Undo safety (verified):** `field_changed` is NOT in `UNDOABLE_CHANGE_KINDS` (`lib/sync/holds/types.ts:46` = `["crew_added","crew_removed","crew_renamed"]`) and `isCrewDomainChangeKind` (`readShowChangeFeed.ts:63-67`) is that same set, so a `field_changed` row with a non-null `entity_ref` is neither individually-undoable nor a supersession target (the PF19 flip, `phase2.ts:513-523`, targets crew-domain kinds). Setting `entity_ref` is purely a label.
- **Rename + role co-change → TWO rows:** `crew_renamed` (entity_ref = old name, from `writeAutoApplyChanges`) AND the `field_changed` role row (entity_ref = new name, from the shared writer). A LEAD rename+toggle additionally keeps its durable event (notice producer's `identityLinkRenames` mapping). Held renames emit neither (excluded). Genuinely-new crew emit no role row (a `crew_added`, not a *change*; new LEAD member still covered by notice + event).
- **Multiple changed members → multiple rows** (one per member) — intentional; the change log paginates.
- `roleFlagsEqual` is private to `phase2.ts`; if the shared writer lives in its own module, add a small **local** `roleFlagsSetEqual` helper (avoid a cross-module cycle). Reuses the SAME applied lists the notice producer diffs (`snapshot.previousCrewMembers`, `applyOutcome.appliedCrewMembers`, `phase2.ts:502-506`), so change-log and notice coverage stay consistent. `heldNames` (`phase2.ts:~495`) and `renames` (from `args.notableItems`) are already in `runPhase2` scope.
- **Emit-topology note:** the shared writer is a change-log WRITE inside the locked tx (like `writeAutoApplyChanges`), NOT a post-commit tail emit — so it does not touch invariant-10 post-commit telemetry ordering. It rides `runPhase2` itself, which is the single shared point every apply path funnels through.
...
There is **no residual**: every role change — LEAD or non-LEAD, auto-apply or staged — has a discrete, identifiable per-member audit row. The `sync_audit` record remains as the batch-level "Doug applied this" attribution but is no longer load-bearing for per-member role auditability. No `staged_apply` source, no CHECK migration.
...
2e. **The identifiable change-log row covers BOTH paths, written from the shared `runPhase2` point** (§2.4, §2.5) — role_flags changes auto-apply un-gated on every path, so the row is `source: 'auto_apply'` on both (accurate, no CHECK migration) and a staged non-LEAD role change gets a discrete identifiable row too. Do NOT relitigate to a `staged_apply` source, a CHECK migration, or a "sync_audit is sufficient" batch-level argument — the per-member row closes it.
