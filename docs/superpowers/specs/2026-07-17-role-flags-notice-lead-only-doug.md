...
**Scope: the AUTO-APPLY path only (Round 2, Codex F5).** This change-log row is written in `writeAutoApplyChanges`, which runs on the **auto-apply** path (cron/manual `processOneFile` → `runPhase2` with `notableItems`, `feedPolicy != none`). The **staged-apply** path (dashboard "Apply", `applyStaged.ts:1400` passes `feedPolicy: { kind: "none" }`) deliberately writes NO `auto_apply` feed rows — it is a **Doug-initiated** action recorded in the `staged_apply` sync-audit (`insertSyncAudit`, `auditSource: "staged_apply"`, `appliedByEmail`), and its held item writes its own `mi11_approve`/`mi11_reject` change-log row. A role_flags change riding a staged apply is therefore covered by that Doug-initiated, attributable sync-audit record, and — for a **LEAD** change — additionally by the durable `LEAD_ROLE_APPLIED` app_event, which rides the staged tail too (unchanged mi9 topology). See §2.5 for why this is complete, not a gap. The `show_change_log.source` CHECK (`20260608000001_show_change_log.sql:26` = `('auto_apply','mi11_approve','mi11_reject','undo')`) does not admit a `staged_apply` source, and the badge/`acknowledge_changes` count only `auto_apply` — so writing staged role rows into the auto-apply feed would require a CHECK migration AND would mislabel a reviewed apply as auto-applied; explicitly out of scope.
...
- **Rename + role change in the same sync (Round 2, Codex F4).** A renamed member's prior row lives under the OLD name and its `next` under the NEW name, so a naive `prevByName.get(next.name)` misses it and skipping renamed names would leave a rename+role co-change with NO identifiable role audit (the `crew_renamed` row records only name/email, and images aren't exposed). Therefore the loop MUST NOT skip renamed members — it resolves the prior through the rename mapping and still emits an identifiable role-change row **keyed to the successor (new) name**:
  ...
  A rename+role co-change thus produces TWO rows: the `crew_renamed` row (entity_ref = old name) AND this `field_changed` role row (entity_ref = new name). Consistent with "one identifiable change-log row for every AUTO-APPLIED role change," and it satisfies the same promise for a LEAD rename+toggle (which additionally keeps its durable event via the notice producer's `identityLinkRenames` mapping). Held renames emit neither row (excluded). Genuinely-new crew (no prior under either name) emit no role row here — a new member is a `crew_added`, not a role *change*; a new LEAD member is still covered by the notice + durable event.

### 2.5 Why staged-apply role changes need no auto-apply change-log row (Codex F5)

A role_flags change can ride a **staged apply** when the same sync also carries a held item (MI-11 email hold) that Doug approves via the dashboard. The audit posture is complete without an `auto_apply` change-log row:

- **LEAD (security-sensitive):** the durable, failure-visible `LEAD_ROLE_APPLIED` app_event fires on the staged tail (`applyStaged` → `emitLeadRoleApplied`, unchanged mi9 topology). A mistaken LEAD grant via staged apply is always recoverable via `observe events --code LEAD_ROLE_APPLIED`. This is the airtight path.
- **Non-LEAD (tile-only, no capability):** the change is part of a **Doug-initiated** `staged_apply` recorded in `sync_audit` (attributable to `appliedByEmail`, timestamped). Narrowing removes its bell alert — consistent with the owner's intent that non-LEAD changes not nag — and non-LEAD is not security-sensitive, so the initiated-apply record suffices. This does NOT leave it "dark": the apply is attributable and timestamped.

The residual difference vs the auto-apply path (no per-member `field_changed` row for a non-LEAD role change bundled into a staged apply) is intentional and bounded: the auto-apply change-log exists because cron auto-applies with no human in the loop; a staged apply always has Doug in the loop. Adding a `staged_apply`-sourced role row (CHECK migration + feed/badge semantics) is deferred to `BACKLOG.md` as a nicety, not a correctness gap.
...
- **Staged-apply coverage (§2.5, Codex F5):** a LEAD role change applied via the **staged-apply** path still emits `LEAD_ROLE_APPLIED` (assert on the `applyStaged` tail — the airtight staged audit). A non-LEAD role change via staged apply emits no bell alert, no `LEAD_ROLE_APPLIED`, and no `auto_apply` change-log row (it is recorded in the Doug-initiated `staged_apply` sync-audit) — assert the absence of a `ROLE_FLAGS_NOTICE` alert and the presence of the `staged_apply` sync-audit record. Failure mode caught: assuming the auto-apply change-log covers staged, or the staged tail dropping the LEAD durable event.
...
2e. **The identifiable change-log row is AUTO-APPLY-path only** (§2.4 scope, §2.5) — staged-apply role changes are a Doug-initiated `staged_apply` sync-audit + (LEAD) the durable event; the `show_change_log.source` CHECK doesn't admit `staged_apply` and the badge counts only `auto_apply`. Writing staged role rows is a deferred `BACKLOG.md` nicety, not a correctness gap. Do not demand staged change-log rows or a source-CHECK migration in this spec.
