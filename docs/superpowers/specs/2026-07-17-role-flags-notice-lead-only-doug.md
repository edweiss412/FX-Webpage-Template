...
**Owner decision (2026-07-17, post-ship review of #439):** Seeing the shipped `ROLE_FLAGS_NOTICE` bell alert live, the owner ratified: (a) the bell nudge should fire **only for LEAD gain/loss**, not routine non-LEAD department/scope swaps; (b) it is a **Doug-facing operator audit nudge**, not a developer/health concern, so it should be **dismissible**, render **amber (not red)**, and deep-link to the **sheet** (not dev telemetry). This **supersedes `mi9-lead-autoapply-fyi` §7.2** (which ratified `audience: "health"` for minimal fanout). The durable `LEAD_ROLE_APPLIED` app_event — the authoritative, non-losable dev/forensic record — is **unchanged**; this spec reduces feed noise and re-homes the operator nudge, it does not weaken the audit.
...
| `BellPanel` tone | `rowTone` isHealth short-circuit (`:129`) | not health → `severity: "info"` → **info tone (amber accent)**, not red |
...
### 2.4 Change-log coverage for non-LEAD role changes

So a non-LEAD role change stays auditable in the change log (matching the owner's "change-log only" intent), broaden the `field_changed` trigger in `writeAutoApplyChanges.ts` (`:143-152`) to ALSO fire when an existing (matched-by-name), **non-held, non-renamed** crew member has an applied `role_flags` delta — not only when an MI-8*/MI-9 invariant is present:

```ts
if (
  hasInvariant(args.triggeredItems, (i) => i.invariant === "MI-8" || ... || i.invariant === "MI-9") ||
  hasAppliedRoleFlagsChange(args)  // NEW: catches non-LEAD deltas that fire no invariant
) { rows.push({ changeKind: "field_changed", entityRef: null, summary: "A field changed on this sync", beforeImage: null, afterImage: null }); }
```

`hasAppliedRoleFlagsChange` iterates `nextCrewMembers`, skips held names and rename prior/added names, looks up the prior by name, and returns true on the first `role_flags` set-difference. Implementation notes:
- The block still pushes **at most one** `field_changed` row per sync (single `if`), so no double-emit when both a LEAD/MI-9 change and a financial MI-8 change occur — unchanged from today.
- It reuses the SAME applied lists the notice producer diffs — `previousCrewMembers: snapshot.previousCrewMembers`, `nextCrewMembers: applyOutcome.appliedCrewMembers` (`phase2.ts:502-506`) — so change-log coverage and notice coverage stay consistent (a folded LEAD change on a retained row is caught by both).
- `roleFlagsEqual` is private to `phase2.ts` and importing it here would create a cycle (phase2 imports writeAutoApplyChanges). Add a small **local** set-equality helper in `writeAutoApplyChanges.ts` instead.
- The row is notification-only (null before/after images), consistent with the existing architecture — role_flags changes (LEAD included) never carried structured change-log images; LEAD's structured detail lives in the `LEAD_ROLE_APPLIED` app_event. Non-LEAD gets the generic "go look" row; the live value is authoritative. This asymmetry (LEAD has a durable event, non-LEAD does not) is intentional — non-LEAD is not security-sensitive.

### 2.3 Copy tighten (LEAD-only framing) — §12.4 lockstep

`helpfulContext` (`catalog.ts:878-879`) currently enumerates non-LEAD examples ("a department swap (A1 → V1), an additive flag like BO") that no longer trigger the notice. Tighten to LEAD-only framing. `dougFacing` (`:874-875`) already reads correctly ("If the change included LEAD status … confirm it was intentional") — but since the notice is now LEAD-only, drop the conditional "If the change included" hedge and state it plainly. Proposed:

- `dougFacing`: "A crew member gained or lost **LEAD** status via a sheet edit — it was applied automatically (a sheet edit is deliberate). LEAD grants admin/ops/financials access, so confirm this was intentional; if it was a mistake, correct it in the sheet."
- `helpfulContext`: drop the department-swap / BO examples and the "no action needed" framing; state that this notice fires only for LEAD gain/loss (department/scope-only changes are not surfaced here), that LEAD auto-applies because a sheet edit is deliberate, and that a durable audit record also exists.

Because these are §12.4-catalog rows, the three-way lockstep applies (§4).
...
- **Non-LEAD change-log coverage (§2.4):** an existing crew member with a **non-LEAD** applied role_flags delta (A1 → V1) → **no** `ROLE_FLAGS_NOTICE` alert AND **no** `LEAD_ROLE_APPLIED` event, BUT a `field_changed` `show_change_log` row IS written. A held (MI-11) member's role delta → no change-log row (held-excluded). A LEAD change → still exactly one `field_changed` row (no double-emit alongside a co-occurring MI-8 financial change). Failure mode caught: narrowing leaving non-LEAD swaps with no history, or the new trigger double-emitting / emitting for held crew.
...
- **rowTone (if §5 included):** a notice-weight health code renders `data-tone="notice"` (amber), a degraded health code renders `data-tone="critical"` (red), an info non-health code renders `data-tone="info"`. Real-component/jsdom render assertion on `bell-sev-*` `data-tone`.
