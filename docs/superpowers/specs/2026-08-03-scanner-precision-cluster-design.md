...
**Measured (2026-08-03), two-pass:** **58 codes · 0 signalled · 44 capture-linked skips**
(2 FRAGMENT, 4 COPY, 4 FACTORY_BODY, 34 USE). Every skip passed both its local test and its
capture-link.
...
— all 58 codes resolve statically, and all 44 skips pass both their local test and their capture
...
| **P5-noio-caller — `definedIds`' own body** | Structural: the body touches no filesystem API, and the default reader is a single named one-liner reading exactly the path it was given. P5-trace watches the injected SPY and therefore cannot see a read that bypasses it — R7b compiled two such silent variants (a direct shadow read inside `definedIds`, and a default adapter reading an extra path per call). |
...
| --- | --- | --- |
| 1 | `bodyDefinedIds` must not know about files at all | **P5-noio** — its signature is `(text, opts)` AND `tests/docs/_ledgerMdast.ts` is structurally prohibited from importing `node:fs`/`node:path` or otherwise reading the filesystem. A signature alone does not stop a hidden internal read (R6b mutant `req1-hidden-read`). |
| 2b | Every PHYSICAL read flows one-to-one through the injected reader | **P5-noio-caller** |
| 2 | The file list is ONE named constant | **P5-trace** — the injected `read` is a spy, and the exact sequence of paths it is called with must equal the supplied `ledgers` list. Existence of `LEDGERS` does not stop a caller iterating `[...ledgers, ...SHADOW]` (R6b mutant `req2-hidden-list`). |
...
- **AC-B2** All TWELVE plants pass: P1–P8, P5-live, P5-sole, **P5-noio**, **P5-trace** (§4.2 / §4.2a). P5 is exercised against the exported
...
| B8 | HIGH (R6b) — §4.2a requirements 1-2 were discharged by signature and by the existence of `LEDGERS`, neither of which is executable; two compiled mutants (hidden internal read, shadow-extended list) keep every declared check green. | **Accepted.** Requirement 1 is now **P5-noio** (structural: no filesystem access from `_ledgerMdast.ts`) and requirement 2 is **P5-trace** (the injected `read` spy's exact call sequence must equal the supplied list). |
