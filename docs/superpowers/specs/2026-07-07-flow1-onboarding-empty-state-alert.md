...
- **§12.4 master-spec edit is TWO parts** (both required by `extract-spec-codes.ts`): the four-column table row AND the `helpfulContext` YAML appendix entry (`:3081+`). A non-null-`dougFacing` code missing its appendix entry throws at extraction (`:233`).
