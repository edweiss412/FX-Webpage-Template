...
- **Independent best-effort boundary (review R2 finding 2).** The existing `logAdminOutcome` call sits in its OWN `try/catch {/*best-effort*/}` (`route.ts:275-285`). The new alert emit gets a **separate, sibling `try/catch`** — NOT nested inside the `logAdminOutcome` try, and NOT sharing its catch. Sequencing (precise, review R7 finding): both emits run inside the `result.outcome === "completed"` block AFTER `runOnboardingScan` resolves (so all per-file DB txs are COMMITTED and no advisory lock is held — the invariant-10 "post-commit" property), and — like the existing `logAdminOutcome` — they run BEFORE the terminal `result` NDJSON message is enqueued (`route.ts:287-294`). They are NOT "fire-and-forget after streaming": the route currently `await`s `logAdminOutcome` before emitting the result, and the alert emit is a sibling `await` in its OWN `try/catch`, so both complete (or their throws are caught) before the result message. "Best-effort" here means only that a caught throw neither fails the request (no 500) nor blocks the result emit nor suppresses the sibling emit — NOT that they run asynchronously after the response. The test plan (§9) covers the cross-suppression failure mode explicitly.
...
| emit throws (infra) | Fail-quiet: wrapped in its OWN `try/catch { /* best-effort */ }`, identical to the adjacent `ONBOARDING_SCAN_COMPLETED` emit (`route.ts:283-285`). The scan's DB txs are already committed (emit is post-commit); a caught throw never fails the request (no 500) and never blocks the terminal `result` message. |
...
| §12.4 spec prose — TWO edits | In `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md`: (a) ADD the code's row to the §12.4 four-column table (`dougFacing`/`crewFacing`/`followUp`, near `:2946`); (b) ADD a `helpfulContext` entry for the code to the machine-parseable `<!-- §12.4 helpfulContext appendix -->` YAML block (`:3081+`). BOTH are required: `scripts/extract-spec-codes.ts:352-357` REQUIRES an appendix entry for any non-null-`dougFacing` code, pinned by `tests/cross-cutting/extract-spec-codes.test.ts:43-51` (review R7 finding). `crewFacing` is `—`/null (doug-only). |
...
- **§12.4 master-spec edit is TWO parts** (both required by `extract-spec-codes.ts`): the four-column table row AND the `helpfulContext` YAML appendix entry (`:3081+`). A non-null-`dougFacing` code missing its appendix entry fails extraction (`extract-spec-codes.ts:352-357`, pinned by `tests/cross-cutting/extract-spec-codes.test.ts:43-51`).
- **REGENERATE + COMMIT** two generated manifests in the PR: `lib/messages/__generated__/spec-codes.ts` (`pnpm gen:spec-codes`, x1 gate — parses BOTH the table and the appendix, deep-compares to `catalog.ts`) AND `lib/messages/__generated__/internal-code-enums.ts` (`pnpm gen:internal-code-enums`, x2 `no-raw-codes` gate — the route `code:` literal is extracted as `admin_alerts.code`). Both gates do exact-diff/parity checks and fail red if stale.
...
**Backend/catalog:** `app/api/admin/onboarding/scan/route.ts` (emit site), `lib/adminAlerts/upsertAdminAlert.ts` (`AdminAlertCode` union), `lib/adminAlerts/alertIdentityMap.ts` (`ALERT_IDENTITY_MAP` global entry), `lib/messages/catalog.ts`, `lib/messages/__generated__/spec-codes.ts` (generated, `gen:spec-codes`), `lib/messages/__generated__/internal-code-enums.ts` (generated, `gen:internal-code-enums`, x2 gate), `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md` (§12.4 table row **AND** helpfulContext YAML appendix entry).
