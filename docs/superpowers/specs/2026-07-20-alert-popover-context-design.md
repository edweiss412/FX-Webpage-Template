...
- **synthetic-input unit tests** call it on hand-built fixtures that exercise EVERY rule and branch independently — including the branches the empty ledger leaves un-exercised on the live catalog: a valid exemption (null context + well-formed row ⟹ no violation), an exempt-AND-authored code (⟹ rule-3 violation), a whitespace-only context and a whitespace-PADDED lead-in (⟹ rule-2 violations), an unknown exempt code, a `helpHref: null` exempt code, an empty-reason row, and duplicate rows (⟹ rule-4 violations).

This makes the gate TDD-natural (the synthetic tests are RED before `checkPopoverContextCoverage` exists) and proves each rule without relying on manual break/revert of the live catalog. The gate imports `HELP_ONLY_LEARN_MORE_LEAD_IN` and `renderEmphasis` from their shipped modules (no copy of the constant). It is structurally a sibling of `_metaShowScopedTemplates.test.ts` (`tests/messages/_metaShowScopedTemplates.test.ts:45-118`), whose defense-2 "valid as RENDERED output" and defense-3 "key set EQUALS declaring set" patterns this mirrors.
...
- **No interpolation step.** Popover strings contain NO `<placeholder>` tokens (unlike `dougFacing`), so `renderEmphasis` output does not depend on any params; rule 2 renders the trimmed string directly, with no param fixture involved.
...
<!-- spec-lint: ignore — new files created by this spec; not yet tracked -->
- **Create (new, untracked):** `tests/messages/popoverContextExemptions.ts` (`POPOVER_CONTEXT_EXEMPT = []`), `tests/messages/popoverContextCoverage.ts` (the pure `checkPopoverContextCoverage` checker), and `tests/messages/_metaPopoverContextCoverage.test.ts` (live meta-assertion + synthetic-input unit tests, the §4 gate).
