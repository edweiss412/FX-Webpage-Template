...
- **F4:** for each of the five panel surfaces: entering the confirm state moves `document.activeElement` to the cancel button (`waitFor`); cancel click returns focus to the re-mounted idle trigger; auto-revert with focus OUTSIDE the confirm row does NOT move focus (fake timers + focus planted on an external element); restyle assertion also checks the cancel lacks recipe tokens.
