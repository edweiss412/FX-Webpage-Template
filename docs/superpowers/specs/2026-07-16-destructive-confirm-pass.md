...
- **ReSync close path:** activating "Keep current version" (click or keyboard — both fire `onClick`) moves focus to the re-sync trigger (`admin-resync-button`, `ReSyncButton.tsx:135`) before the panel unmounts. Single-phase, no guard (activation itself proves the user is in the panel).
