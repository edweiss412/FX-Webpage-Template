...
- **C1 — Recipe.** Class set MUST include `bg-warning-text` and `text-warning-bg` and `font-semibold`, and MUST NOT include `bg-accent`, `bg-surface`, or `bg-bg`. Hover: `hover:opacity-90` (matches `MaintenanceResetButtons.tsx:298`). Existing contrast: light `#5c3f00`/`#fff3d6`, dark `#ffd68a`/`#3a2e14` (`app/globals.css` warning tokens) — both ≥7:1, no new token math.
- **C2 — Safe control.** Where a separate cancel/safe control exists in the confirm step, it keeps neutral `bg-bg` (+ `ring-offset-warning-bg` when inside an amber panel, per `ReSyncButton.tsx:172`). The safe and destructive controls MUST NOT share the same background token (FLOW4-5's defect class).
...
- G4: while any group's confirm is armed, other groups' buttons behave as today (`disabled={state.kind === "running"}` applies only while a bulk ignore is running; arming is per-group local state).
...
- **F2 (FLOW4-4 aggregate).** `confirmUndoAll` collects results: `const results = []; … results.push(await actions.undoFromDashboardAction(null, fd))`. After the loop, `const failed = results.filter(r => r && !r.ok).length`; store in new local state `bulkUndoFailed: number | null`. When `failed > 0`, render inside the group card (below the header, `role="alert"`): fixed copy `Couldn't undo {failed} of {total} changes. The rows still shown below were not undone.` — plain-language, no raw code (invariant 5), no new §12.4 entry (static copy, not a catalog code; same class as the strip's existing fixed infra sentence at `RecentAutoAppliedStrip.tsx:363-374`). State resets on next confirm open. The strip's existing revalidate self-heal is unchanged.
- **F3 (FLOW4-6 focus on close).** Before `setConfirming(false)` on BOTH completion paths (cancel already keeps focus on "Keep changes" — only `confirmUndoAll` completion needs it), move focus to the group disclosure toggle (`auto-applied-toggle-<showId>`, always mounted while the group renders) via a ref. If the group itself unmounts on revalidate (all changes undone), focus loss to body is accepted (the surface is gone; P3 residual, explicitly accepted).
- **F4 (C3 on the accent-panel family).** `RotateShareTokenButton`, `ResetPickerEpochButton`, `PickerResetControl`, `RevokeRowButton`, and `ResolveAlertButton` currently mount their confirm/cancel row with NO focus management — the idle trigger unmounts while focused, so keyboard focus is lost and a stray second Enter's target is undefined (verified: no `.focus()` call in any of the five; e.g. `ResolveAlertButton.tsx` has only the auto-revert timer refs). Each gains the `keepCurrentRef` pattern: a ref on the CANCEL button + an effect focusing it when the confirm state mounts. This closes the same stray-Enter vector the strip/ReSync panels already close. `ReSyncButton` (R8) already conforms (`keepCurrentRef`, `:76-79`).
...
New `tests/styles/_metaDestructiveConfirm.test.ts`, registry-style (same discipline as `tests/styles/_metaBgAccentInventory.test.ts`):

- **Scan:** walk `components/**` and `app/**` (`.tsx`, comments stripped — reuse `tests/styles/_classScanUtils.ts` `walk`/`stripComments`/`tokensOf`), find every className token-set containing BOTH `bg-warning-text` and `text-warning-bg` (the recipe signature).
- **Registry:** each hit must match a registered row `{ file, testid, kind: "morph" | "panel" }`. Unregistered hit → fail (`UNREGISTERED DESTRUCTIVE CONFIRM`). Registered row with no hit → fail (`STALE ROW`).
- **Recipe completeness:** each hit's token set must include `font-semibold` and must not include `bg-accent`/`bg-surface`/`bg-bg` (C1).
- **Panel focus rule:** for rows with `kind: "panel"`, the file must contain a ref-focus effect targeting a NON-recipe button (static assertion: the `ref={<safeRef>}` attribute sits on a button whose className lacks `bg-warning-text`). This is the static pin; per-component jsdom tests carry the behavioral proof.
- **Expected initial registry (15 rows):** panels — MaintenanceReset reset (`validation-reset-confirm`), Cleanup (`cleanup-abandoned-finalize-confirm-yes`), ReapStale (`reap-stale-sessions-confirm-yes`), strip Undo-all R1 (`auto-applied-undo-all-confirm-go-*`), Rotate R2, ResetEpoch R3, PickerReset R4, Revoke R5, ResolveAlert R6, ReSync R8; morphs — Archive R7 (`archive-show-confirm-button`), G1–G4. Prefix-wildcard testids allowed for per-entity ids (same convention as the bg-accent registry).
...
- **Restyle tests (R1–R8):** confirm-go carries `bg-warning-text`+`text-warning-bg`+`font-semibold`, not `bg-accent`/`bg-surface` — asserted on the rendered element by testid (existing test files extended; e.g. `RecentAutoAppliedStrip.test.tsx` TRACK-style class assertions).
- **F4:** for each of the five panel surfaces, entering the confirm state moves `document.activeElement` to the cancel button (`waitFor`).
- **F1:** on popover open, `document.activeElement` is the cancel button (`waitFor` — async focus rule per project memory).
- **F2:** mock `undoFromDashboardAction` to fail for a subset (`{ok:false, code:"UNDO_SUPERSEDED"}`); after confirm, the aggregate line renders with correct counts and `role="alert"`; zero failures → no line.
- **F3:** after successful bulk undo completes, `document.activeElement` is the group toggle.
