...
- **C1 — Recipe.** Class set MUST include `bg-warning-text` and `text-warning-bg` and `font-semibold`, and MUST NOT include `bg-accent`, `bg-surface`, or `bg-bg`. Hover (normative, deliberate): `hover:opacity-90` (matches `MaintenanceResetButtons.tsx:298`); the token set MUST NOT include any other `hover:bg-*` (e.g. `hover:bg-accent-hover`, `hover:bg-surface-sunken`, `hover:bg-warning-bg`). The hover token is PART OF the required recipe signature, not preferred styling — uniformity of the destructive affordance is the point, and the registry is the intended extension seam if a future variant genuinely needs a different hover (never matcher loosening). Enforced by both the meta-test and the rendered restyle tests (§8, §10). Existing contrast: light `#5c3f00`/`#fff3d6`, dark `#ffd68a`/`#3a2e14` (`app/globals.css` warning tokens) — both ≥7:1, no new token math.
...
| # | Surface | Kind | Change |
|---|---------|------|--------|
| R1 | `RecentAutoAppliedStrip.tsx` confirm-go — the button with testid `auto-applied-undo-all-confirm-go-<showId>` inside the confirm panel | panel | `border border-border-strong bg-surface … text-text-strong` → recipe (C1). "Keep changes" (`auto-applied-undo-all-cancel-*`) unchanged (already C2-compliant). The idle trigger `auto-applied-undo-all-<showId>` stays unchanged (§11 trigger exclusion). Closes FLOW4-5. |
...
- **F4 (C3+C5 on the accent-panel family).** `RotateShareTokenButton`, `ResetPickerEpochButton`, `PickerResetControl`, `RevokeRowButton`, and `ResolveAlertButton` currently mount their confirm/cancel row with NO focus management — the idle trigger unmounts while focused, so keyboard focus is lost and a stray second Enter's target is undefined (verified: no `.focus()` call in any of the five; e.g. `ResolveAlertButton.tsx` has only the auto-revert timer refs). Each gains:
  ...
  - **Close (C5), two-phase (R6 review):** the guard is evaluated BEFORE the state flip, the focus move happens AFTER the trigger remounts. In the cancel handler / auto-revert timer callback, while the confirm row is still mounted: `const shouldRestore = containerRef.current?.contains(document.activeElement) ?? false`; then flip to idle; when `shouldRestore`, an effect (or ref set consumed on the idle render) focuses the re-mounted trigger. An auto-revert firing while the user is focused elsewhere captures `shouldRestore === false` and MUST NOT steal focus.
  ...
  - `ReSyncButton` (R8) already conforms on open (`keepCurrentRef`, `:76-79`) but NOT on close (R5 review correction): "Keep current version" calls `setHeldShrink(null)` (`ReSyncButton.tsx:169`), unmounting the panel AND the focused safe button. ReSync joins the C5 close path, SINGLE-phase (its trigger stays mounted alongside the panel, unlike the trigger-swap surfaces): the cancel handler focuses the re-sync trigger button (`admin-resync-button`) FIRST, then calls `setHeldShrink(null)` — no guard needed beyond the click itself (a click on the cancel button means focus/activation is in the panel). No auto-revert case exists (persistent panel). Tested alongside F4.
...
- **Expected initial registry surfaces** (row counts deliberately NOT stated — the authoritative inventory is the fails-by-default walk's fail-first output at implementation time; a literal split adds a row, never loosens the matcher): panels — MaintenanceReset (`validation-reset-confirm`), Cleanup (`cleanup-abandoned-finalize-confirm-yes`), ReapStale (`reap-stale-sessions-confirm-yes`), strip Undo-all R1, Rotate R2, ResetEpoch R3, PickerReset R4, Revoke R5, ResolveAlert R6, ReSync R8; morphs — Archive R7 (compact/full ternary branches are separate literals), G1–G4.
...
- **F4:** for each of the five panel surfaces: entering the confirm state moves `document.activeElement` to the cancel button (`waitFor`); cancel activation returns focus to the re-mounted idle trigger; auto-revert with focus INSIDE the confirm row returns focus to the re-mounted idle trigger (fake timers); auto-revert with focus OUTSIDE the confirm row does NOT move focus (fake timers + focus planted on an external element). All five surfaces HAVE auto-revert timers (verified: `RotateShareTokenButton.tsx:28`, `ResetPickerEpochButton.tsx` `AUTO_REVERT_MS`, `PickerResetControl.tsx:27` + `:99-101`, `RevokeRowButton.tsx:37`, `ResolveAlertButton.tsx:64` + its timer), so the auto-revert cases apply uniformly.
- **ReSync close path:** activating "Keep current version" (click or keyboard — both fire `onClick`) moves focus to the re-sync trigger (`admin-resync-button`, `ReSyncButton.tsx:135`) before the panel unmounts. Single-phase, no guard — the proof is ACTIVATION of the cancel control (pointer click, Enter, or Space; all fire the button's `onClick`), which itself establishes the user is operating the panel.
- **F1:** on popover open, `document.activeElement` is the cancel button (`waitFor` — async focus rule per project memory).
- **F2:** mock `undoFromDashboardAction` to fail for a subset (`{ok:false, code:"UNDO_SUPERSEDED"}`); after confirm, the aggregate line renders with counts derived from the mocked failure set and `role="alert"`; zero failures → no line; re-opening the confirm clears a visible alert (open-clears model); outcome persists across collapse → re-expand of the group disclosure.
