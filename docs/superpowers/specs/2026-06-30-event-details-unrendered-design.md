...
**Typing (LOW-1):** declare the labels object so unknown keys can't be added without compiler help — e.g. `export const EVENT_DETAILS_LABELS = { … } as const satisfies Record<string, string>`, and type `CREW_TECH_SPEC_KEYS[number]` to be `keyof typeof EVENT_DETAILS_LABELS` (a compile-time assertion line: `const _crewKeysAreLabeled: readonly (keyof typeof EVENT_DETAILS_LABELS)[] = CREW_TECH_SPEC_KEYS;`). This makes "every crew key has a label" a compile error, not just a runtime test failure.
...
- **`_metaSentinelHidingContract` EXTENDED (Codex spec-R3).** Correction: this contract does NOT only walk `components/tiles/` — it also walks `CREW_DIRS = [components/crew/sections, components/crew/primitives]` (`tests/components/tiles/_metaSentinelHidingContract.test.ts:100-103`), so **GearSection.tsx IS walked**. GearSection stays compliant trivially: it already contains `shouldHideGenericOptional` (existing keynote/internet handling), and the new tech-spec rows render via `KeyValueRows` (a walked, compliant primitive that routes every row through `shouldHideGenericOptional`); the new keys are read in a loop over the `lib/crew/eventDetailsSpecs.ts` whitelist, so no new *unrouted literal* `event_details["…"]` appears in GearSection.tsx. **Action (forward-defense):** EXTEND `GENERIC_OPTIONAL_FIELDS` with the newly-surfaced crew tech-spec keys (`stage_size, podium_type, polling, led, scenic, gooseneck, digital_signage, test_pattern, fonts, equipment_storage, staff_office_room, record, virtual_speaker, virtual_audience` — `notes` is already covered by the `\.notes\b` pattern) so any FUTURE direct literal read of these in a walked component must route through `shouldHideGenericOptional`. The plan adds these patterns in the same commit as the card.
...
2. **Crew card renders real specs** (GearSection component test): fixture `event_details` with `stage_size`, `podium_type`, `polling` real → card shows those labels+values; `record: "N/A"` → that row absent (sentinel-hidden); a key already shown elsewhere (`power`) → NOT in the tech-specs card; **a non-string value (e.g. a number, simulating bad JSONB) → `String()`-coerced, no throw, hidden** (LOW-2 guard). Catches: missing render; sentinel leak; double-render; non-string crash.
