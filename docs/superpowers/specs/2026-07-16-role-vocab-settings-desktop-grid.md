...
1. **Component (jsdom, extend `tests/components/roleMappingSettingsRows.test.tsx`):**
   - Edit button carries `aria-label={COPY.EDIT_LABEL}` and renders BOTH `aria-hidden` label spans (`EDIT_LABEL` with `min-[760px]:hidden`, `EDIT_LABEL_SHORT` with `hidden min-[760px]:inline`) inside ONE button wired to the same handler; existing `getByRole("button", { name: COPY.EDIT_LABEL })` queries keep passing (aria-label supplies the constant accessible name — §4 contract).
