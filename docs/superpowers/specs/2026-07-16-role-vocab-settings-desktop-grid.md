...
`display:none` content is excluded from the accessible-name computation (ARIA acc-name spec step 2A; both Tailwind `hidden` and the active `min-[760px]:hidden` branch resolve to `display:none`). Therefore in a real browser the button's accessible name is EXACTLY `EDIT_LABEL` ("Edit what they see") below 760px and EXACTLY `EDIT_LABEL_SHORT` ("Edit") at ≥760px — never the concatenation. The e2e spec asserts this at both viewports via `getByRole("button", { name, exact: true })`. **jsdom caveat:** jsdom applies no Tailwind CSS, so BOTH spans are "visible" there and the accessible name IS the concatenation — component tests must therefore locate the button via `getByText(COPY.EDIT_LABEL)`/`closest("button")`-style queries (the existing suite already queries by text), never by exact role name.
...
2. **Real-browser layout (new `tests/e2e/roles-settings-layout.spec.ts`, desktop-chromium project):** seed 3 `role_token_mappings` rows via the service-role helper (`tests/e2e/helpers/supabaseAdmin.ts`) mirroring the mock triple (2 grants / financial grant / empty grants), `signInAs(ADMIN_FIXTURE)`, visit `/admin/settings/roles`. **Seed hygiene (global table):** in `beforeAll`, snapshot ALL existing `role_token_mappings` rows, delete them, insert the 3 fixture rows; in `afterAll`, delete the fixture rows and re-insert the snapshot verbatim — the table leaves the spec exactly as found, so the spec is order-independent and never destroys unrelated local/dev state (CI DBs are fresh; this protects local runs). Every helper call follows invariant-9 destructure-and-throw. Seed rows must satisfy the table CHECKs (`supabase/migrations/20260716000000_role_token_mappings.sql:4-18`): `token` uppercase/trimmed ≤64 chars, `grants ⊆ {A1,V1,L1,FINANCIALS}`, `decided_by` canonical lowercase email. Service-role writes are in-grant (`:27`).
   ...
   - **390×900:** stacked layout — chips container top ≥ token bottom, actions top ≥ chips bottom (vertical order proof); Edit button accessible name is EXACTLY `EDIT_LABEL` (`getByRole("button", { name: COPY.EDIT_LABEL, exact: true })` — proves the short-label span is `display:none` per §4's accessible-name contract). At 1280 the mirror assertion: accessible name EXACTLY `EDIT_LABEL_SHORT`.
