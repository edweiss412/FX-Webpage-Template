...
- **AC-10 (no in-flow growth, standalone).** The note node carries the absolute-positioning class contract (assert the class list names `absolute` and no in-flow display class), and `tests/e2e/appHealthIndicator.layout.spec.ts` passes UNMODIFIED — the admin action cluster's pinned geometry is the real-browser proof the wrapper added no width. All three consumers (§1.1 item 3 census) render the toggle with the note mechanism present.
