...
`docs/superpowers/specs/2026-08-25-fitwithinclip-key-node-matrix.probe.tsx.txt` — **`.txt` because a
`.tsx` under `docs/` is a UI-shaped file outside every scanned root, which
`tests/styles/_metaNewTabAnnouncement.test.ts:1056` exists to refuse, and it was right to refuse
mine.** Run it by copying into the admin unit-test directory under a test-file name; it has to be copied to
run either way. Its output, verbatim:
