...
- Insert immediately BEFORE `forbidden()` (which throws; anything after is unreachable): `await log.warn("admin access denied", { source: "auth.requireAdmin", code: "ADMIN_ACCESS_DENIED", actorHash: hashForLog(email) });`
...
|---|---|---|---|---|---|
| R1 | `app/api/admin/staged/[fileId]/apply/route.ts` | `SHOW_APPLIED` | **DIRECT** (lock inside `applyStaged`; `result` is post-commit in outer scope) — await emit before each committed-success return (`:168-199`) | `readAdminEmail()` → `admin.email` (already captured `:134`) | `driveFileId=fileId` (`:104`); `showId=result.showId` (applied) |
| R2 | `app/api/admin/show/staged/[stagedId]/apply/route.ts` | `SHOW_APPLIED` (shared, like `SHOW_FINALIZED` across finalize+cas) | **DIRECT** — before the sole success return (`:173-176`) | `deps.requireAdminIdentity()` → `admin.email` (captured `:140`) | `driveFileId` from `readDriveFileIdForStagedId` (`:154`); `showId=result.showId` (`:174`) |
| R3 | `app/api/admin/sync/[slug]/route.ts` | `SHOW_SYNCED_MANUAL` | **DIRECT** — before `:86` `return NextResponse.json({ ok:true, result })` | ⚠ currently `requireAdmin()` (void, `:57`) → **switch to `requireAdminIdentity()`** and capture email | `driveFileId=resolved.driveFileId` (`:68`); `showId=result.showId` when applied (`runScheduledCronSync.ts:190`) |
| R4 | `app/api/admin/pending-ingestions/[id]/retry/route.ts` | `PENDING_INGESTION_RETRIED` | **OUTCOME-REF** (success built inside `withRowTryLock` callback, pre-commit) — declare `let outcome: Omit<AdminOutcome,"code"> \| null = null` beside `appliedShowId` (`:337`), assign inside the callback on the applied path(s) (`:374/:397`), emit `if (outcome) await logAdminOutcome({ code:"PENDING_INGESTION_RETRIED", ...outcome })` AFTER the lock resolves (after the skip guard `:400`, beside `revalidateShow` `:405`) | ⚠ currently `await deps.requireAdminIdentity()` **return discarded** (`:321`) → **capture** `const admin = …` | `driveFileId` (`:330`); `showId=appliedShowId` |
...
Emit ONLY on the committed-mutation path. R4's non-applied outcomes (`parsed_pending_review`/`deferred`/`still_failed`) must NOT emit `PENDING_INGESTION_RETRIED` — only the `appliedShowId`-gated applied path (the clean mutation boundary), mirroring #218's "no log on idempotent re-poll" rule (`finalize/route.ts` leaves `outcome` null on no-op re-polls). R1–R3, R5 emit only in their committed-success branches, never on `skipped`/`409`/guard-fail returns.

### 6.3 `source` strings (mirror each route's existing `log source:` convention)

`api.admin.staged.apply` (R1), `api.admin.show.staged.apply` (R2), `api.admin.sync` (R3), `api.admin.pending-ingestions.retry` (R4), `api.admin.snapshot-rollback.repair` (R5).
