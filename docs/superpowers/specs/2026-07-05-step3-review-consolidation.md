...
|---|---|---|---|
| **Live** | session-provenance-joined show (§4.3) AND `published = true AND archived = false` — **OR** the existing-show branch (§4.3: `drive_file_id` match AND `wizard_created_session_id IS NULL` AND `published = true AND archived = false`) | accent/live | badge only (session-created); badge + existing-show checkbox for a pre-existing Live show re-touched this session |
...
- A manifest row with `created_show_id IS NULL` AND no matching pre-existing crew-visible show is never Live/Held — it falls to Ready/Needs-review from its manifest status. (A pre-existing *draft* or *archived* null-session show — `published = false`, or `archived = true` — fails the existing-show Live branch and, being null-session, is not Held either, so it falls to Ready.)
...
**Footer actions (redesigned copy + reduced set).** Three immediate, unambiguous actions, each named by object + effect. **Copy-routing scope note (LOW, R6):** invariant 5 (`lib/messages/lookup.ts`) governs **error/failure codes** (§12.4 catalog copy) rendered in the UI — NOT ordinary static action labels. These three button labels are plain UI copy authored inline (and `Re-scan this sheet` is `RescanSheetButton`'s existing hardcoded label at `:143`, reused verbatim). Any **failure code** surfaced inside the modal (apply/rescan/discard error) still routes through `lookup.ts`/`messageFor` (§9 invariant 5). No new §12.4 code is added.
...
- **`Ignore this sheet`** (secondary) — the wizard staged `discard` route with `kind: "permanent_ignore"`. **Semantics correction (HIGH, R3):** this writes a **session-scoped** `deferred_ingestions` row (`wizard_session_id = <session>`, `discardStaged.ts:295-325`) — it removes the show from **this setup run**, and is cleared when finalize completes (`finalize-cas/route.ts:633-639`). It does **NOT** appear in the dashboard **Ignored sheets** list (which reads only `wizard_session_id IS NULL`, `loadIgnoredSheets.ts:59`), so the spec makes **no** "restore via unignore" promise for this action. Copy: **"Ignore this sheet"** + subline **"Removed from this setup."** (No dashboard/unignore claim.) Purpose: let Doug exclude an unresolvable sheet so finalize can proceed. **Out of scope:** a durable, dashboard-visible ignore for these rows would require a new null-session ignore path (the existing durable routes reject `staged`+failure rows) — deliberately deferred to honor the "no mutation-API change" non-goal (§2).
