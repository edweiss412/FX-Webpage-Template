...
- **`Step3Row`** gains (all optional, coerced in `fetchStep3Data`): `stagedId?: string` (already selected at `OnboardingWizard.tsx:259/:326`, just thread it onto the row), `triggeredReviewItems?: TriggeredReviewItem[]` (add to the select + coerce via `parseTriggeredReviewItems`), `reviewItemsCorrupt?: boolean`, `allowedDiscardVariants?: DiscardKind[]`.
  - **`allowedDiscardVariants` MUST be server-computed from the row's MODE, NOT from the parsed review-items sentinel (HIGH, R10).** Today `StagedReviewCard` infers first-seen (→ all three variants: `try_again_next_sync` / `defer_until_modified` / `permanent_ignore`) vs existing-show (→ only `try_again_next_sync`, since `discardStaged` rejects defer/ignore on existing rows as `INVALID_REVIEWER_ACTION`) by scanning `triggered_review_items` for the `FIRST_SEEN_REVIEW`/`ONBOARDING_SCAN_REVIEW` sentinel (`StagedReviewCard.tsx:36-40`). A **corrupt** row has empty `triggeredReviewItems` (populated only when both validation levels pass), so that scan cannot detect first-seen — and since `try_again_next_sync` is dropped as a normal button, a corrupt row would be left with **no discard exit at all**. Fix: `fetchStep3Data` computes `allowedDiscardVariants` from the row's mode determined by `showExists(drive_file_id)` (the same `public.shows` read the Live/Held derivation already does), independent of review-items validity — existing-show → `["try_again_next_sync"]`; first-seen → `["try_again_next_sync","permanent_ignore"]` (defer dropped, §4.4). This guarantees the corrupt/no-parse recovery always has a valid durable discard the validator will accept.
...
- **Modal `Approve & apply` payload**: `{ stagedId: data.row.stagedId, reviewerChoicesVersion: 1, reviewerChoices }` where `reviewerChoices` are built from the tier-3 radio picks + the auto-bound single-action items (§4.4). `Ignore this sheet` maps to the `discard` route with the row's adaptive durable `kind` (§4.4: `permanent_ignore` where `allowedDiscardVariants` allows it, else `try_again_next_sync`; `StagedReviewCard.tsx:443-450`). `Re-scan this sheet` uses the separate `rescan-sheet` route (not the discard route). `defer_until_modified` is no longer surfaced (§4.4).
- **Corrupt guard**: `reviewItemsCorrupt === true` → suppress `Approve & apply`, offer discard, exactly as `StagedReviewCard.tsx:308-312` does today. **Guaranteed discard exit (HIGH, R10):** the discard control renders from the server-computed `allowedDiscardVariants` (above), never from the corrupt review-items — so a corrupt row ALWAYS has at least the universal `try_again_next_sync` exit (labeled per §4.4), plus `Re-scan this sheet`. `Ignore this sheet` (`permanent_ignore`) renders only when `allowedDiscardVariants` includes it (first-seen rows). A corrupt row is never stranded with only Re-scan.
...
- **`Ignore this sheet`** (secondary, **always present** — the guaranteed discard exit, R10) — the wizard staged `discard` route. Its `kind` is the row's best **durable** discard from server-computed `allowedDiscardVariants` (§4.3.1): **`permanent_ignore` where allowed** (first-seen rows), **else `try_again_next_sync`** (existing-show rows, where the validator rejects permanent_ignore as `INVALID_REVIEWER_ACTION`). Both remove the sheet from **this setup run** so finalize can proceed; `try_again_next_sync` additionally lets it re-stage on the next sync. **Semantics (HIGH, R3):** `permanent_ignore` writes a **session-scoped** `deferred_ingestions` row (`wizard_session_id = <session>`, `discardStaged.ts:295-325`), cleared when finalize completes (`finalize-cas/route.ts:633-639`); it does **NOT** appear in the dashboard **Ignored sheets** list (`wizard_session_id IS NULL`, `loadIgnoredSheets.ts:59`), so the spec makes **no** "restore via unignore" promise. Copy: **"Ignore this sheet"**; subline **"Removed from this setup."** for `permanent_ignore`, or **"Removed from this setup; returns if the sheet is edited again."** for `try_again_next_sync` (both honest, object-named). Purpose: let Doug clear an unresolvable/corrupt sheet so finalize proceeds. **Out of scope:** a durable, dashboard-visible ignore for `staged`+failure rows would require a new null-session ignore path — deliberately deferred to honor the "no mutation-API change" non-goal (§2).
...
**Row-dependent variants (R2 + R10 note).** `StagedReviewCard.tsx:36-40`: not every row allows every discard variant (existing-show rows reject defer/ignore). `Approve & apply` + `Re-scan this sheet` are universal; `Ignore this sheet` is **also** always rendered, but its underlying `kind` adapts to the row's `allowedDiscardVariants` (server-computed from mode, §4.3.1) — `permanent_ignore` for first-seen, `try_again_next_sync` for existing-show. `allowedDiscardVariants` is never empty (`try_again_next_sync` is universal), so the discard exit is always valid — the discard `kind` is derived from server mode, NOT from the (possibly-corrupt) review items.
...
- **Guaranteed discard exit for corrupt rows** (unit, HIGH R10): a corrupt row (empty `triggeredReviewItems`) still renders a valid `Ignore this sheet` discard whose `kind` comes from server-computed `allowedDiscardVariants` — `permanent_ignore` for a first-seen row, `try_again_next_sync` for an existing-show row (where permanent_ignore is rejected). Assert the discard kind is derived from mode, not the corrupt review items, and that it is NEVER absent. Failure mode caught: a corrupt existing-show row stranded with only `Re-scan` (no way to unblock finalize).
