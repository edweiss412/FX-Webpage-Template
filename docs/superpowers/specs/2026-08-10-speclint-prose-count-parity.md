...
The accept-set is deliberately keyed on structure: a doc names a script under `scripts/` by path or basename; that script contains a module-local `const <IDENT> = <integer>` with `IDENT` matching `/^EXPECTED_[A-Z0-9_]+$/`; the arm reads the declaration TEXTUALLY (never imports — the originating spec's own boundary). **Measured universe today: exactly one qualifying pair** — `EXPECTED_SITE_TOTAL = 37` (`scripts/verify-cn-operand-parity.mjs:80`), named by 7 docs — and **zero live mismatches**: the one present-tense cardinality (`docs/superpowers/plans/2026-08-07-classname-array-join-cn.md:21`, "All 37 sites") agrees with the constant; every disagreeing number in those docs (33, 36) sits in dated historical/review-round records the exclusions carve out. The arm compares present-tense prose cardinalities within the paragraph that names the script; the measured FP risk is nil at today's universe size and the arm's cost scales with the universe, not the corpus.
...
Corpus probe: **27 candidate groups** (near-identical repeated indented/fenced blocks within one doc, similarity ≥0.87, differing in numeric literals). Sampled groups split between benign near-repeats (distinct test commands that legitimately differ in a digit) and the motivating true-positive class (the wedge-remeasure filing's quoted disposition templates drifting a quantity). Ship posture: advisory, fires only within ONE doc, only on groups whose text differs SOLELY in numeric literals after whitespace normalization (the sampled benign groups differ in more than digits — command paths, flags — and fall out under that stricter equality); the 27-group sample is the implementing arc's fixture set.
