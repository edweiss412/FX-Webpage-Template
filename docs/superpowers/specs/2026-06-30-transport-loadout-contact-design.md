...
```
- **Idempotent (apply-twice safe):** `add column if not exists` + `drop constraint if exists`/`add` (per AGENTS.md CHECK/enum migration matrix). The dev clone mirrors the public table (`20260502000000_dev_schema_clone.sql:142-156`). The `add constraint` cannot fail on pre-existing non-canonical data: the three columns are **introduced in this same migration**, immediately before the CHECK, so on the first apply every `loadout_email` is `NULL` (CHECK passes trivially); on any re-apply the drop/add re-validates against data written **only** through `canonicalize()` (parser + cron, §2.3b), which is always canonical. There is no code path that writes a raw `loadout_email`, so the "partial prior deploy populated the column with a non-canonical value" scenario cannot arise.
...
### 2.3 Email-audit: two independent audits, both satisfied without a spec-prose edit
`loadout_email` matches `isEmailProperty` (`/_email$/`, `emailCanonicalization.ts:297-300`), so it is treated as an email column **by name** by BOTH x5 audits. Neither requires an `EMAIL_BOUNDARIES` / master-spec §17.2 prose entry:

**(a) Schema-check-body audit** (`auditEmailSchemaCheckSources`, `emailCanonicalization.ts:176-184`) — one-directional: it only flags a `*_email_canonical` CHECK whose **body** is non-canonical. A canonical CHECK mirroring `driver_email` passes; a persisted email column without a CHECK is NOT flagged. It scans only the three files in `defaultSchemaCheckSources()` (`:71-77`) — **add the new migration path to that list** so the new CHECK is actually scanned and confirmed canonical (belt-and-suspenders; omitting it means the CHECK is unscanned, not a failure). This is the ONLY email-audit **code** change.

**(b) Source-scan audit** (`auditLiveEmailCanonicalization` / `auditObjectWrite`, `emailCanonicalization.ts:341-366`, invoked live by the x5 test `email-canonicalization.test.ts:649`) — flags a write of an email-like column (matched at `:360` via `isEmailLikeDbColumn` → `isEmailProperty`) whose value expression is **not** wrapped in `canonicalize()`. It is satisfied by the **presence of the `canonicalize()` call**, not by prose. Our two write sites — the parser (`transport.ts`, `loadoutEmail = canonicalize(...)`) and the cron write (`runScheduledCronSync.ts`, `canonicalize(row.loadout_email)`) — both canonicalize, so both pass. Critically, both files **already** write `driver_email` (canonicalized), so they are already recognized LAYER:PATH boundaries — adding `loadout_email` to the same files creates **no new `+missing_in_plan:` boundary** (the file-level parity check, `email-canonicalization.test.ts:85-88`, stays green). Therefore `EMAIL_BOUNDARIES` stays byte-identical; no `gen:email-boundaries` drift, no §17.2 edit.

The seed write (`seed.ts`, `sqlNullableString(transportation.loadout_email)`) mirrors the existing `driver_email` seed write (`:406`) which is NOT wrapped in `canonicalize` and is green today — `seed.ts` is not in the source-scan file set; the value is already canonical (parsed via the parser boundary). Consistent, no new finding.
