...
1. **`humanizeDayList`** (humanize unit test file): `["2025-10-07","2025-10-09"] → "Oct 7 & 9"`; `[…07,…09,…11] → "Oct 7, 9 & 11"`; cross-month `["2025-10-30","2025-11-02"] → "Oct 30 & Nov 2"`; single `["2025-10-07"] → "Oct 7"`; `[] → null`; `["garbage","2025-10-07"] → "Oct 7"` (malformed skipped). Failure mode caught: the non-contiguous-collapse bug `humanizeDayRange` has.
2. **`partialAttendanceLabel`** (new unit test): explicit+humanize ISO → "Oct 7 & 9 only"; explicit+raw `["10/7","10/9"]` (humanize:false) → "10/7, 10/9 only"; `unknown_asterisk` → "Partial (dates TBD)" (both humanize modes); `none` → null; explicit `days:[]` → null; null restriction → null.
