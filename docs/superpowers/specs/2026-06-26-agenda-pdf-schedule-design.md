...
- `getAgendaChips(spreadsheetId: string): Promise<{ label: string; chipFileId: string | null }[]>` — Sheets `spreadsheets.get` with grid data + `chipRuns(chip(richLinkProperties(uri)))` + `formattedValue`, scanning the INFO tab; returns one element **per agenda-link row in grid row order**, `chipFileId` = `/d/<id>` from the value cell's chip `uri` (or `null` when that row's value is a plain URL/text, not a chip). Order preservation is the contract §4.5.1 depends on. Returns a discriminable infra fault (not `[]`) on a Sheets-API error, so "no agenda rows" and "couldn't read the sheet" are distinct.
