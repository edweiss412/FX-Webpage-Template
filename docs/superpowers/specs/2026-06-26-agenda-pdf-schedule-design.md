...
- `sessions ≥ 5`
- `% of detected time-anchor lines that normalized to a valid time ≥ 95%` — denominator is **time-anchor lines**, NOT sessions (Codex round-3 MED: "% of sessions with a time" is tautological since a session is only created from a parsed time). **A time-anchor line is one matching the modal clock font AND size (`timeFont`+`timeSize`), excluding day-headers** — pairing the font matters because in the real templates body/sponsor/intro paragraphs share the 14pt `timeSize` but use a *different* font; a size-only denominator wrongly tanks valid agendas (PCF 54% / FIT 72%) below 95% (impl discovery against the committed fixtures, 2026-06-26). A low ratio still means many clock-styled lines were *not* real clocks → mis-calibration → `'low'`.
...
**Ambiguous-first-clock guard (Codex round-2 MEDIUM).** A bare (no-meridiem) first clock with hour 7–11 is genuinely undecidable AM-vs-PM with no preceding context, yet §4.3.1 seeds it to AM and §4.3.2 never repairs a bare clock — so a show that *opens* in the evening could silently pass as morning. Therefore: **if the SHOW's first session has a time token with NO explicit `AM`/`PM` anywhere in it AND a leading hour in 7–11, the show is `confidence:'low'` (embed-only).** Scoped to the show's first session, NOT every day's first (impl discovery, 2026-06-26): a multi-day conference's later days legitimately open in the morning with bare clocks (FIT day-2 opens at a bare `7:45`, correctly seeded to AM by §4.3.1's 7–11→AM rule); gating per-day would wrongly suppress FIT. Only the genuinely context-free show opener is ambiguous. (FIT day-1 opens at `1:00` → seeded PM, not ambiguous; this gate rarely fires.)
...
| pdfjs throws / transient `infra_error` | a prior valid `extracted` may exist on the link | caught in `enrichAgenda`; the link is **left exactly as-is** — any pre-existing `extracted` is **preserved, never cleared** (a transient failure must not wipe stale-good data); scan continues. A test pins that a transient parse/Drive failure leaves an earlier high-confidence `extracted` intact (Codex round-5 LOW). |
