...
- **Unread semantics** *(amended per plan-review R3: compare against the activity timestamp, not `last_seen_at` alone, so the comparison uses the same value the read stamp carries)*: entry row is unread for viewer V iff no `admin_alert_reads` row for (`latest_row.id`, V) **or** `read_at < activity_at` where `activity_at = greatest(latest_row.raised_at, latest_row.last_seen_at)` (§3.2). A re-bump (`upsert_admin_alert` conflict arm sets `last_seen_at = now()`, `supabase/migrations/20260505000000_upsert_admin_alert.sql` conflict `do update set last_seen_at = now(), occurrence_count = +1`) therefore re-unreads; a resolve→re-raise mints a fresh row id (partial unique index `admin_alerts_one_unresolved_idx` on `(coalesce(show_id::text,''), code) WHERE resolved_at is null`, `supabase/migrations/20260501001000_internal_and_admin.sql:279`) and is unread by absence.
...
- Failures of `open`/`read` on the client are **fail-quiet** (badge/dot simply doesn't update; retried on next gesture) with server-side forensic logging via the established `logAdminOutcome` post-commit pattern (scanner-exempt via `tests/log/_metaAdminOutcomeContract.test.ts` — plan verifies registration shape).
...
- Returns **one meta row + zero-or-more entry rows** (discriminated by `is_meta boolean`). The meta row is always present — entry columns NULL, carrying `seen_through` (`now()` — the snapshot's transaction timestamp, §3.2), the `active_hit_cap` / `history_hit_cap` flags, and `viewer_opened_at` (the caller's `admin_bell_state.opened_at`, NULL if none) — so `/bell/open` stays snapshot-safe even when the feed is empty (R10 finding 1). Entry rows carry latest-row fields (`id, code, show_id, context, occurrence_count, raised_at, last_seen_at, resolved_at`), `slug` (join `shows`), `resolved_occurrence_sum`, `is_active`, and `viewer_read_at` (left-joined `admin_alert_reads.read_at` for `p_admin_email`). *(Amended per plan-review R4: viewer read/watermark state is folded into the SAME snapshot as the entries — a concurrent `/bell/open` from another tab can no longer make `unseenCount` disagree with the rendered snapshot, and the route layer performs no separate state reads, preserving the one-read contract with §3.3.)* `shapeBellEntries` splits on `is_meta` and treats a missing meta row as an infra error (fail-closed, never a silent default watermark).
...
1. **Catalog**: `BRANCH_PROTECTION_DRIFT` (`lib/messages/catalog.ts:1882`) and `BRANCH_PROTECTION_MONITOR_AUTH_FAILED` (`catalog.ts:1900`) flip `resolution: "manual"` → `"auto"`. Add both to `AUTO_RESOLVE_NOTES` (`lib/adminAlerts/audience.ts:69`), e.g. "Clears automatically the next time the branch-protection monitor verifies the settings." / "…the next time the monitor authenticates successfully."
