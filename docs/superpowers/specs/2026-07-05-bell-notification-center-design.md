...
Mutations flow only through server routes using the service-role client — **no client DML**. Per the PostgREST-DML-lockdown discipline (AGENTS.md "PostgREST DML lockdown for RPC-gated tables"; class-wide pattern from M9.5 R5/R6): `revoke insert, update, delete on <table> from anon, authenticated;` in the same migration that creates each table, plus RLS enabled with an admin-only SELECT policy (`is_admin()`, mirroring `admin_alerts`' `admin_only` policy at `supabase/rls_policies.sql:150`). A structural meta-test pins the REVOKE (§16).
...
| **New**: DML-lockdown meta-test rows | `admin_alert_reads`, `admin_bell_state` pinned REVOKEd (extend the existing postgrest-dml-lockdown suite) |
...
- **Route tests**: auth guards (non-admin 403/404 contract, config non-dev), `read` visibility fail-closed (non-dev marking a health alert id → 404, no row written), bounded-read registration, no-store headers, config 400-rejection bounds agree with the SQL CHECK ranges (derive both from one shared constant).
