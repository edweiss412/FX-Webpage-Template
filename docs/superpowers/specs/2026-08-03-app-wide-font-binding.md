...
- **The sign-out page's `system-ui, sans-serif` is defensible in a NARROW form.** Review R7 was right that the broad version — "error pages should not depend on a webfont" — is incoherent as a project principle, and self-serving here: this very change binds the webfont on `app/global-error.tsx`, which IS the app's fatal-error page. The honest, narrower claim: that page is a **minimal self-contained document with no stylesheet at all**, so an explicit system stack is a reasonable local choice for it, in a way that says nothing about error pages generally. It is a deliberate choice already in the code, not an oversight.
