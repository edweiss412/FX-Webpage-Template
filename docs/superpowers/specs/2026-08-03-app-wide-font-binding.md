...
A structural test that walks `app/` from the filesystem (not a lexical file list, so a NEW loader fails by default — the class-sweep discipline, `AGENTS.md`) and asserts the set of files importing from `next/font/` **is exactly `app/fonts.ts`**, and that the count of loader call sites matches.
