...
`DESIGN.md:133` commits the product to Inter loaded via `next/font/google`; that wiring was never done, so **only the crew tree rendered Inter while every other React root — admin, auth, help and the crash screen — rendered the system fallback**. This spec wires the ratified mechanism — at `app/fonts.ts`, shared by both of Next 16's roots — and pins the result with real-browser font-identity assertions.
...
| R2 | **`--font-sans` IS edited — it binds `var(--font-inter, "Inter", "Inter Fallback")`.** ~~The literal-`"Inter"` stack stays byte-identical.~~ | **Rewritten after review R8**, which was right that a governing row prescribing the opposite of the shipped design is not a harmless historical note. The original reasoning was half right: a BARE `var(--font-inter)` really would be invalid at computed-value time wherever the generated class is absent (every standalone harness), dropping `font-family` entirely rather than falling through. What it missed is that a `var()` FALLBACK LIST has neither problem, and that the literal-only stack skipped next/font's metric-matched face and so reflowed the swap window by ~10% on every React-root route. §2.5 carries the full reasoning; `DESIGN.md` §2.1's pinned stack is amended in lockstep. |
...
| R5 | **No product copy, label, or layout class changes.** Option (b) from the backlog entry (`whitespace-nowrap` + truncation on group titles) is **not** taken. | Once Inter loads app-wide, the wide-fallback path no longer exists for anything a React root renders, including the swap window (`Inter Fallback` is metric-size-adjusted). Adding an ellipsis to a real product label to satisfy a test harness inverts the dependency. See §5.1. |
...
- The class goes on `<html>` (`app/layout.tsx:57`), not `<body>`. **Precise reason, corrected after R1 and again after R16:** the generated class does NOT set `font-family` — it only defines `--font-inter`. Binding happens because `--font-sans` CONSUMES that token (§2.5), applied at `html` (`app/globals.css:659-663`), so the cascade names whatever family next/font generated. Nothing depends on that family's spelling; Next 16 happens to use the literal `Inter`, but the indirection is what binds. Proof that the class placement is not what binds: today the crew tree applies it to a nested `<div>` and `<html>` still resolves to Inter (§1.0). `<html>` is nonetheless the right host for the class, because it is the widest scope at which `--font-inter` can be exposed, and the token should not be narrower than the font it names.
...
4. Assert `<html>` exposes `--font-inter`. Asserted **separately and for its own reason**: it is the token every other assertion READS its expected family from, so if the loader silently stopped emitting `variable:` the rest would lose their oracle rather than fail informatively. Added after review R1 noted the spec promised an exposure it never tested.
...
**Concrete failure mode caught (anti-tautology, `docs/agents/writing-plans.md:14`):** assertion 2 fails today on both non-crew routes, because no Inter face is registered there and the forced-`"Inter"` probe falls back to the default serif metric. (The pre-correction measurement of that gap — 185.53 vs 167.14, 18.39px — was taken on the 404 shell; the real per-route numbers are recorded in the closeout from T1's own first RED run, not carried over from the probe.) The crew case's assertion 4 is also RED today, since the `.variable` class currently sits on a nested `<div>` rather than `<html>`. It is also upgrade-proof against a future Next that hashes the generated family name: the expected family is read from `--font-inter` rather than hardcoded, so it follows the same indirection `--font-sans` uses. An earlier version compared against the literal `"Inter"` and would have failed a working upgrade (review R15); and the `!== sans-serif` leg prevents a green read on a host that happens to ship Inter as a system font. **It does NOT use `document.fonts.check()`** (§1.0 finding 3: returns `true` where Inter is provably absent).
...
**Why the residual is acceptable rather than closed.** A second font loader is a mistake, not an attack. Mistakes are made in ordinary places, and there they are caught by this tripwire, with the runtime checks in §4.1 as an independent second look — those see any second loader whose faces differ in family, descriptor pair or tuple, ON THE THREE ROUTES THAT SUITE VISITS. A module reached only through some other route is outside both, which is the boundary §4.3's residual list states. A deliberately hidden byte-identical duplicate is caught by neither. Building module-graph resolution through the TypeScript and MDX compilers would close more of it, with its own failure modes, and is not justified by that risk. If it is ever wanted, it is its own piece of work.
...
- Consequence for the load site: the loader call lives in `app/fonts.ts`, exported, and BOTH roots import that one instance. A second `Inter()` call in `global-error.tsx` would emit a second `@font-face` set under the same family name. `DESIGN.md:133` NAMED `app/layout.tsx` as the load site and was amended in lockstep with this change to name `app/fonts.ts` — the same site hoisted one module so the second root can share it, which is a mechanical consequence of Next 16's two-root model rather than a departure.
