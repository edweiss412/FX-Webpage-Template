...
1. **Resolvers and derivations are IMPORTED, never re-typed — and they run on the detector side too.** A field the renderer unwraps, validates or derives before painting must be unwrapped, validated or derived identically here, or the two disagree about what "the same value" means. Three shipped bindings are called rather than reproduced: `resolveCurrentDiagrams`, `normalizeAgendaExtraction`, and `roomHasScope`; two more (`driftNote`, `agendaOverflowNotes`) moved to `lib/agenda/agendaPaint.ts` so both sides import one copy. The whole-diff review found the cost of the alternative: reading `embeddedImages` off the un-unwrapped wrapper collapsed EVERY wrapped show to one constant tuple, so no diagram edit could cue at all. Sections where the detector still reads WIDER than its renderer are over-cue only and are catalogued in `BL-FRESHNESS-PROJECTION-NARROWING`.
...
| `agenda` | per baseline item: `badge`, `href`, and the extraction AS PAINTED — `normalizeAgendaExtraction` first (the block runs it itself and renders nothing unless it returns a high-confidence extraction with days, `components/crew/AgendaScheduleBlock.tsx:73`), then per day `dayLabel` + `date`, per session `time` + `title` + `room` + `driftNote(drift)` + tracks. Plus `agendaOverflowNotes(block)` and whether `fullExtraction` is present. NOT `label` (the React key, `step3ReviewSections.tsx:3554`), and NOT `corrections` / `extractorVersion` / `sourceRevision`, which are painted nowhere. | `step3ReviewSections.tsx:4238` -> `AgendaItemRow` -> `components/crew/AgendaScheduleBlock.tsx` |
| | Nullable fields the block tests with `!== null` (`AgendaScheduleBlock.tsx:121`, `:165`, `:177`, `:195`) keep their presence bit through the leaf trim, because `normalizeAgendaExtraction` type-checks blanks but never collapses them — so `null` -> `" "` MOUNTS a paragraph and must cue. This is the one place the global `String(v ?? "").trim()` normalization does not describe the renderer. | `components/admin/review/sectionFreshness.ts` (`nullable`) |
...
| `rooms` | `rooms` (capped at `ROOMS_CAP`, plus the length) AND `rooms.filter(roomHasScope).length` over the UNCAPPED list, because that count is painted on the rail (`components/admin/wizard/step3ReviewSections.tsx:4260`) and a room past the cap gaining scope moves it. `diagrams` is resolved through `resolveCurrentDiagrams` FIRST — the adapter passes the persisted `{ current, pending }` jsonb through unchanged (`components/admin/review/publishedAdapter.ts:78`) and it is the renderer that unwraps (`components/admin/wizard/step3ReviewSections.tsx:3874`) — then narrowed to `snapshot_revision_id` (a path segment of every tile URL) + `linkedFolder` + the first `DIAGRAM_TILE_CAP` embedded images + both lengths (`linkedFolderItems` renders as a COUNT only, `components/admin/wizard/step3ReviewSections.tsx:3752`) | `components/admin/wizard/step3ReviewSections.tsx:4258-4271`, caps at `components/admin/wizard/step3ReviewSections.tsx:2018` and `components/admin/wizard/step3ReviewSections.tsx:3745` |
...
| Routed attention items | the attention items the modal buckets under this section, supplied by the caller as `attentionBySection` — every live item, not only the actionable ones (`components/admin/showpage/PublishedReviewModal.tsx:447` iterates `attentionItems`, the full prop — an earlier draft iterated the `live` subset and dropped exactly the clearing items this column exists to catch), because a clearing item paints a card too | The second review round found this by probe: attention items render inline card content in the crew, event, rooms and warnings bodies of `components/admin/wizard/step3ReviewSections.tsx`, so adding, editing or resolving one changes a card no own-field signature can see. The caller supplies the grouping because it already resolves the placement predicate through `resolveEffectiveSection` (`components/admin/showpage/PublishedReviewModal.tsx:415-416`); resolving it again inside the detector would be a second source of truth for where an item belongs. |
...
|---|---|---|---|
| `1600` | `SECTION_FRESHNESS_FLASH_MS` | `components/admin/review/sectionFreshness.ts (new)` | §4.2 timer, every `1600ms` duration in §4.5, tests N1 and N2 |
...
| N3 | each of the two `@keyframes` names is declared exactly once, and the count of `section-freshness-flash` occurrences in `app/globals.css` equals the count inside the normative block, so no stray rule exists elsewhere |
...
The edge rows duplicate ratios the share-link rows already assert (`tests/styles/status-token-contrast.test.ts:248-261`). They are restated on this surface's own row because DESIGN.md's note that the outline carries the dark-theme signal makes it non-decorative here too, and a future edit to the share-link rows must not silently remove this surface's floor.

**Two honest limits on what these rows prove**, both raised in round 2 and neither papered over:

- **The outward ground is an approximation.** The outline draws outside the card, so its outward neighbour is the transparent scroll container over the shell's own ground, not the card's `surface`. Measured against the real ground the edge clears 8.06:1 light and 9.39:1 dark, comfortably above the 3:1 floor, so the approximation is conservative rather than flattering. C4 keeps `surface` because that is the ground the outline shares its EDGE with at the moment the wash is still painted; the outward figure is recorded here.
- **The floors govern the opaque endpoints, not every frame of the fade.** Compositing the edge against its ground mid-fade necessarily lands below any static floor — at 25% alpha it measures 1.52:1 light and 1.64:1 dark — because that is what fading OUT means. The C-rows deliberately measure the endpoints: the cue carries no information (it is decoration over content that is fully legible on its own ground throughout), the reduced-motion branch never paints a partial frame at all, and a floor applied per-frame would forbid every fade in the system, including `SHARE_LINK_FLASH_MS`. Recorded so a later reviewer does not read the C-rows as a claim about the whole animation.
- **A relative timestamp crossing its own boundary does not cue.** `AttentionBanner` paints a relative time from `now` (`components/admin/review/AttentionBanner.tsx:203`), and the detector takes no `now` input, so a refresh that lands as "59 min ago" becomes "1h ago" changes a card without arming its cue. This is deliberate: `now` advances on its own with no refresh at all, so feeding it in would make the signature a function of wall-clock time and flash cards on refreshes where nothing was edited — trading a missed cue nobody is watching for a false cue on every card carrying a timestamp. The cue answers "did this content change", and a clock tick is not a content change.
