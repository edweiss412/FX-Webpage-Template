...
- Implementation re-runs the sweep: `rg 'text-accent[^-]' components/ app/` + `rg 'hover:text-accent(-hover)?\b'` — every hit migrated or exempted with reason.

**Structural pin:** new `tests/styles/_metaRawAccentText.test.ts` walks `components/**` and `app/**` (filesystem walk, fails-by-default on NEW files) and fails on any class string containing raw `text-accent` (word-boundary; `text-accent-on-bg`/`text-accent-text` excluded) or `hover:text-accent`/`hover:text-accent-hover`, with an explicit file+line+reason allowlist (empty at ship; the mechanism exists for future justified decorative uses).
