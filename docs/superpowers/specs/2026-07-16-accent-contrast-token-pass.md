...
**Structural pin:** new `tests/styles/_metaRawAccentText.test.ts` walks `components/**` and `app/**` (filesystem walk, fails-by-default on NEW files), TOKENIZES each class string on whitespace, and rejects a token iff its final utility segment (after stripping any variant-prefix chain, e.g. `md:hover:`, `focus:`, `data-[…]:`) is EXACTLY `text-accent` OR EXACTLY `text-accent-hover` — both hues are sub-AA as text in light mode, and neither has a legitimate text use in any variant chain. Exact-token match, not substring/word-boundary, so `text-accent-on-bg`, `hover:text-accent-on-bg`, and `text-accent-text` never match (amended 2026-07-16 plan R2: strictly stronger than the originally enumerated three tokens). The test includes fixture self-checks proving `hover:text-accent-on-bg` is ACCEPTED and `hover:text-accent-hover` is REJECTED (guards the matcher itself). Explicit file+line+reason allowlist (empty at ship; mechanism exists for future justified decorative uses).
...
The inventory below is GENERATED (2026-07-16, `rg -nP` exact-token variant-chain matcher over `components/**` + `app/**`, **59 hits**, reconciled against the disposition rows by `comm` diff — zero unaccounted) and lists EVERY occurrence individually — it is the seed content of the meta-row-11 registry. The scanner strips code comments before matching (5 of the 59 hits are JSDoc/comment mentions: `AccentButton.tsx:10,17,34`, `ReportButton.tsx:27`, `RightNowCard.tsx:617` — non-rendered, excluded; a fixture self-check proves comment text never registers).
