...
| parsed to `null` or to a non-object (number, string, array) | `json_not_an_object` |
...
Four tasks, each with an executable RED before implementation, per invariant 1. Every task states the concrete failure mode it catches, per the anti-tautology rule. Archiving the two ledger entries and clearing their in-progress markers is a **close-out step, not a task** — it is bookkeeping with no executable RED, and the plan does not pretend otherwise (R2 finding 3).
...
| Task | RED | Catches |
|---|---|---|
| 1 | A new suite, tests/auth/oauthRedirectInvalidTelemetry.test.ts — drive each of sites 1-2 through its real GET with a captured sink; assert exactly one record with `code === "OAUTH_REDIRECT_INVALID"`, `level === "warn"`, the site's `reason`, the unchanged response status, and — the §2.1 no-raw-input rule, asserted as an ACCEPT-SET for the same reason Task 4 uses one — that `Object.keys(record.context)` deep-equals `["reason"]` and `record.message` equals the exact expected literal, so the rejected `next` cannot surface under any field. | An emit added to one branch and forgotten on the sibling — the exact drip this arc exists to end. Fails today with zero records. |
...
| 4 | Same file — an **accept-set** case: with `GOOGLE_SERVICE_ACCOUNT_JSON` set to a sentinel-bearing value (once well-formed, once malformed), assert the record's shape EXACTLY — `Object.keys(record.context)` deep-equals `["reason"]`, `record.context.reason` is one of the four enum values, and `record.message` equals the exact expected literal. Content checks (no sentinel, no `private_key`) are retained as a backstop only. | The §2.2 secrets contract regressing. **A denylist was the R2 defect here:** asserting "no `error` key, no sentinel, no `private_key`" passes a parse message moved into `record.message`, passes one stored under any other context key, and passes a PARTIAL key fragment — three live leak channels the earlier draft did not model. An accept-set has no such gap: nothing derived from the env var can appear anywhere, because nothing beyond `reason` and a fixed message may appear at all. This is the accept-set discipline `docs/agents/spec-self-review.md` already mandates, applied where the earlier draft had authored a denylist. |

**Task 4 is a regression guard whose RED is mutation-induced, and this is stated rather than glossed.** Unlike Tasks 1-3 it has no natural failing state: before Task 3 the record array is empty so every negative assertion passes vacuously, and after Task 3 the shipped emit is already safe so they pass legitimately. A "RED" claimed on that basis would be exactly the tautology this project's rules exist to stop. Its RED is therefore **observed by mutation**, on the arc-c protocol: after Task 3 is green, run TWO mutants in the working tree, one per leak channel, observing each FAIL and reverting it: (a) attach `error` (the caught `JSON.parse` error) to the wizard emit; (b) interpolate a fragment of the raw env value into the emit's message string. Mutant (b) is precisely the one the earlier denylist draft would have passed, so running only (a) would leave the accept-set's central claim unproven. The mutant is never committed; the task record states both observations with their output. That is what makes the guard non-vacuous, and it is why finding the assertion shape (structural, not content-based) mattered — against this mutant the content-only assertions would have stayed green.
