...
| every other input (`slug`, `code`, `cards`, `bulk` internals, `groups` contents) | any type-legal value, incl. degenerate ones (`cards: null`, empty-string `slug`/`code`, `bulk.items: []`) | **Behavior-unchanged by this feature — pre-existing component contract, explicitly out of scope.** This table is NOT a totality claim over `Props`; it covers exactly the inputs whose render this feature changes (`itemCount`, `bulk` presence, `label`). Degenerate type-legal values render today's output byte-identically (e.g. `bulk.items: []` already renders "Ignore all 0" — unreachable from the production build site, whose bulk groups come from `groupIgnorableByCode`, which only emits ≥2-distinct-item groups, `lib/dataQuality/bulkIgnoreGroups.ts:8-10`). `itemCount` is the only field this feature adds, and it is required + `number` (its NaN/negative row above is the fail-visible bound); no runtime guards are added or removed anywhere. |
...
Compound: one compound state IS reachable — chip armed (`armedCode` set, 4s revert timer running, `BulkIgnoreControls.tsx:56-74`), then a per-card ignore (`DataQualityWarningControls`) triggers `router.refresh()` and the refreshed server props drop the group to N=1/`bulk: null`. The row is now suppressed while `armedCode` + timer survive the soft refresh (client state preserved, `BulkIgnoreControls.tsx:102-105`). This is NOT a new hazard: chip removal is gated on `bulk` presence (`BulkIgnoreControls.tsx:167`), not on the eyebrow row, so TODAY the same sequence already unmounts the chip (bare eyebrow remains) with `armedCode` surviving; if bulk eligibility returns within the 4s window the chip already re-renders armed today. This feature changes only whether the (chip-less) row is visible during that window — the armed-state lifecycle, the timer, and the re-render-armed edge are byte-identical before and after, and are pre-existing DQIGNORE-2/§4-G4 behavior out of this spec's scope. No state pair is added to the chip machinery. The transition-audit test (`tests/components/admin/bulkIgnoreControlsTransitionAudit.test.tsx:53-55`) pins the eyebrow className stable across the armed morph — that fixture is bulk-eligible (row kept), unaffected.
