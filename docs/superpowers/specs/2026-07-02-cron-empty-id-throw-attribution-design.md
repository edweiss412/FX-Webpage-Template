...
- **`runCronRoute` catch** (`lib/cron/withCronRunSummary.ts:22-64`): **inside** the existing `try { await log.error(...) } catch { /* swallow */ }` block (`:36-63`), select the attribution context robustly:
  ...
  ```
  `cronCtxFromALS` is a **total, no-throw** mapper: given a possibly-undefined store it returns `undefined` unless `store.cronPhase` is a string; otherwise it builds `{ phase, inFlightDriveFileId: cronInFlightDriveFileId ?? null, ...(typeof cronProcessedCount === "number" ? { processedBeforeThrow: cronProcessedCount } : {}) }` (conditional spread — no optional ever `undefined`; no `failures`/`folderId`). Emit `detail`/`driveFileId` exactly as today (the detail branch still fires only when `ctx` has `phase` / `failures.length` / `folderId` / numeric `processedBeforeThrow`), and add `source` **into the detail object** via conditional spread `...(attributionSource ? { source: attributionSource } : {})` — so `source` is present iff a real ctx was chosen and always names that ctx's true origin (never a spurious `"als-fallback"` when no ctx exists). **A present-but-malformed `err.syncRunContext` (no string `phase`) is REJECTED**, and the ALS fallback is used instead — a valid ALS attribution is never suppressed by a junk `syncRunContext`. The whole computation is inside the swallowed block, so a buggy mapper or malformed store can never mask the cron error; `throw err` at `:64` and all HTTP/error semantics are unchanged. `getRequestContext()` is in-scope (the catch runs inside `run()` under the ALS established at `:88-90`).
