...
- **GUARD the Drive chokepoint (change 1)** — reject an empty/blank `fileId` at both `drive.files.get` call sites with a typed `InvalidDriveFileIdError`. This (a) ensures an empty id **never reaches Google** (no opaque `File not found: .`), and (b) gives the rejection a stack pointing at the **actual caller frame**, so the next occurrence — including a detached one — **self-identifies** its call site. It does not prevent a detached rejection from still surfacing as a `threw`; it makes that `threw` diagnosable instead of anonymous.
...
| `agendaDrive.ts:114-118` — byte download `files.get({alt:'media'})` | `enrichAgenda.ts` after `enrichAgenda.ts:137 if (!link.fileId) continue;` | Yes — per-file loop; returns a discriminated union | change 1 (same guard, belt-and-suspenders) |

Shows-table rows are **not** in this table: they reach only `markMissingShow` (DB-only, `:2096-2110`).

## Design

Two runtime-only changes: no migration, no §12.4 catalog code, no UI, no advisory-lock topology change.

### 1. Empty-ID guard at BOTH Drive `files.get` chokepoints — `lib/drive/fetch.ts` + `lib/drive/agendaDrive.ts`
...
- Call `assertNonEmptyDriveFileId(params.fileId)` at the **top of `driveFilesGet`** (`fetch.ts:275`, before the retry thunk) and `assertNonEmptyDriveFileId(fileId)` at the **top of the `agendaDrive.ts:114` try** (before `drive.files.get`). Runs before any retry budget is spent.

Guard conditions: `fileId` may be `undefined`, `null`, `""`, or whitespace (`"   "`) — all throw. Any non-blank string is unchanged (guard no-op).

This is the **only** boundary between a caller-supplied id and Google; guarding it means an empty id **cannot reach the Drive client** from any source (reel, agenda, folder, direct, or a future/detached caller).
...
- **`runCronRoute` catch** (`lib/cron/withCronRunSummary.ts:22-64`): **inside** the existing `try { await log.error(...) } catch { /* swallow */ }` block (`:36-63`), compute `const ctx = err.syncRunContext ?? cronCtxFromALS(getRequestContext())`, where `cronCtxFromALS` is a **total, no-throw** mapper: given a possibly-undefined store, it returns `undefined` unless `cronPhase` is a string; otherwise it builds the object with **conditional spread** so no optional is ever assigned `undefined`:
  ```
  const ctx = { phase: store.cronPhase, inFlightDriveFileId: store.cronInFlightDriveFileId ?? null,
    ...(typeof store.cronProcessedCount === "number" ? { processedBeforeThrow: store.cronProcessedCount } : {}) };
  ```
  (no `failures`, no `folderId`). Emit `detail`/`driveFileId` exactly as today, and add `detail.source = err.syncRunContext ? "sync-body" : "als-fallback"` so every `threw` row self-identifies its attribution layer. Placing the whole computation **inside** the swallowed block guarantees a buggy mapper or malformed store can never mask the cron error; `throw err` at `:64` and all HTTP/error semantics are unchanged. `getRequestContext()` is in-scope (the catch runs inside `run()` under the ALS established at `:88-90`).

### 3. Tests (TDD)

- **Chokepoint guard — the boundary test (unit, both sites, DB-free integration):** with a mocked `drive.files.get`, call `driveFilesGet` / `fetchDriveFileMetadata` and the `agendaDrive` byte-download with `""`, `"  "`, `undefined`, `null` → each rejects with `InvalidDriveFileIdError` **and** `instanceof DriveFetchError`, **not** a Google 404, and **`drive.files.get` is NEVER called** (assert the mock recorded zero calls — proving the empty id cannot reach the Drive client). With a valid id → `drive.files.get` **is** called (mock records the args) and no guard error. This is the "only a valid id reaches the Drive client" boundary test, at the actual boundary. Concrete failure mode caught: an empty id silently reaching Google → opaque `File not found: .`.
...
- **Malformed-ALS no-throw (unit):** `runCronRoute` catch with a store whose cron fields are malformed (`cronPhase` a number, `cronProcessedCount` a string) → the original error still rethrows unchanged and exactly one summary row emits (fail-open: the mapper never throws out of the swallowed block).
