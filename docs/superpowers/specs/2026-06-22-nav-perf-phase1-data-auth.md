...
- **Gate `purgeAndRotateIfStale` off the hot path:** in `app/admin/page.tsx`, the function is a pure no-op when `pending_wizard_session_at IS NULL`. We need the settings row to know that — but `purgeAndRotateIfStale` is currently what *returns* `result.settings`. Resolution: read the settings row first (a single lightweight `app_settings` select the page already needs), and call `purgeAndRotateIfStale()` **only when `settings.pending_wizard_session_at !== null`**; otherwise skip the postgres.js tx entirely and use the already-read settings. This preserves behavior exactly (the function no-ops in that branch anyway) while avoiding the per-load connection+tx. **Advisory-lock note:** `purgeAndRotateIfStale` holds no `pg_advisory*` lock (it uses `app_settings` row-level `FOR UPDATE`/conditional UPDATE inside a tx, per `defaultWithTx`); gating the call changes *when* the tx opens, not lock topology. Confirm during implementation that no nested SECURITY DEFINER advisory lock is bypassed. **Expected ~300-900ms (incl. eliminating the postgres.js connect+tx on the common settled-dashboard path).**
...
- **`tests/admin/_metaInfraContract.test.ts`** — `fetchDashboardData` (L175), `fetchUnresolvedAlertCount` (L222), `loadNeedsAttentionCount` (L216) registered. Parallelizing `fetchDashboardData` must keep every supabase await inside `try/catch` (grep-shape L303-412) and the typed `infra_error` behavior. **Action:** **add a registry row for the new combined settings reader** (`lib/appSettings/getSettingsPageFlags.ts`) — it is a new service-role call site; register it (or carry an inline `// not-subject-to-meta:` waiver with reason — registry row preferred).
...
- **A2 gate:** with `pending_wizard_session_at = null`, assert `purgeAndRotateIfStale` is **not invoked** (spy) and the page still renders with the settings from the lightweight read; with non-null, assert it **is** invoked and behavior is unchanged.
- **B1/B2/B3:** (1) `getClaims` used, `getUser` not on the gate path; (2) three outcomes — valid admin claims + `is_admin=true` → `{email}`; no/empty claims → `redirectToSignIn`; `getClaims` error → `AdminInfraError`; `is_admin=false` → `forbidden`; (3) `React.cache` dedup: render a tree that calls the gate twice (layout-layer + page-layer) in one request and assert exactly one `getClaims` and one `is_admin` RPC fire (spy counts) — a non-cached impl fires two of each.
