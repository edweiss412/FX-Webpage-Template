...
| The success path's committed-refreshing window — `setArmed(false)` before the async `router.refresh()` resolves, letting the idle trigger remount briefly (`ArchiveShowButton.tsx:143-147`) — is EXISTING behavior shared verbatim with the legacy variants via the common `onResult`, and idempotency is owned server-side (`SHOW_ARCHIVED_IMMUTABLE` refusal, `ArchiveShowButton.tsx:50`). Documented in §4 for inventory accuracy; changing it is out of this restyle's scope. | `ArchiveShowButton.tsx:139-160` (shared `onResult`); m12.2-phase-b2 §2.2 lifecycle contract |
...
States of the row variant (R13 correction — the in-flight submit is its own visual state): **idle** (menu row), **armed-ready** (label header + prose + enabled Confirm/Cancel), **armed-pending** (Confirm shows "Archiving…", Confirm + Cancel disabled with `aria-busy`, host busy gate held true via `onBusyChange`), **committed-refreshing** (success committed server-side, `router.refresh()` in flight), **gone** (unmounted after the refreshed props land or on host swap). All pairs enumerated; every treatment is existing ratified behavior — this spec changes none of them, only the idle render.

| Pair | Treatment |
| --- | --- |
| idle → armed-ready (row click) | Instant swap; arming focuses Cancel (C3, `ArchiveShowButton.tsx:166-168`). Unchanged. |
| armed-ready → idle (Cancel) | Instant swap; focus restored to re-mounted trigger (C5, `ArchiveShowButton.tsx:172-177`). Unchanged. |
| armed-ready → armed-pending (Confirm tap) | `useFormStatus().pending` flips: label → "Archiving…", Confirm + Cancel `disabled`/`aria-busy`, host busy gate true (`onConfirmBusy`, `ArchiveShowButton.tsx:181-184`). The only ANIMATED property on the control: the disabled/hover opacity morph via the recipe's own `transition-opacity duration-fast` (`ArchiveShowButton.tsx:396`). Unchanged. |
| armed-pending → idle (action resolves to refusal / not-found / infra failure) | Instant swap via `onResult` (`ArchiveShowButton.tsx:143-160`): armed clears, the matching banner mounts as a wrapper sibling, busy gate releases. The `onResult` branch logic is shared with the legacy variants; the ROW-variant pending→idle-on-failure rendering gets a NEW test (§6 item 1a) — no existing row test pins it. Unchanged behavior. |
| armed-pending → committed-refreshing (success) | R14 accuracy fix — the previous draft compressed this into "gone": `onResult` clears `armed` BEFORE `router.refresh()` resolves (`ArchiveShowButton.tsx:143-147`, `setArmed(false)` then `refresh()`), so for the refresh interval the IDLE row remounts and the busy gate releases at settle (`ArchiveShowButton.tsx:378-383`; pinned by `ArchiveShowButton.test.tsx:184-210`). EXISTING behavior on every variant (the legacy branches share `onResult`), unchanged by this spec. A re-arm + resubmit inside the window is refused server-side as `SHOW_ARCHIVED_IMMUTABLE` (`KNOWN_REFUSAL_CODES`, `ArchiveShowButton.tsx:50`) and renders the refusal banner — the archive action, not the client, owns idempotency (§1.1). Instant; no animation. |
| committed-refreshing → gone | The refreshed server props land, the host swaps to the Archived presentation and closes the hub (existing hub-close contract, `tests/components/admin/showpage/shareHub.test.tsx:887-895`). Instant unmount, host-owned. Unchanged. |
| armed-pending → armed-ready | UNREACHABLE — `pending` only clears when the action settles, which always lands in `onResult` (idle) or unmount (gone). |
| idle → gone (host swaps to Unarchive / popover closes) | Instant unmount, host-owned. No cleanup needed (no timer in the row variant; busy false). Unchanged. |
| armed-ready → gone (popover dismissed while armed, not pending) | Instant unmount, host-owned — dismissal is only GATED while busy (`onBusyChange` level), so an armed-but-idle confirm may be dismissed. Existing hub busy-gate tests pin the gated case (`shareHub.test.tsx:830-860`). Unchanged. |
| idle hover/focus | `transition-colors duration-fast` on `hover:bg-surface-sunken` + tier-1 ring — the §4.1 recipe's own tokens, identical to rotate. |
| Compound: banner visible while any state toggles | Banners are wrapper siblings in every branch; no interaction. Unchanged. |
| Compound: host dismissal attempted during armed-pending | Blocked by the busy LEVEL until settle or the hub's 15s hang bound (`shareHub.test.tsx:952-980`). Unchanged. |

No timer in the row variant (ratified; §1.1). The legacy variants keep theirs.
...
6. **CI wiring (R11 adversarial finding — verified: no workflow invokes `admin-lifecycle-layout.spec.ts`; it is matched only by the `mobile-safari` project regex, and every e2e workflow runs an explicit spec list).** The implementation adds a workflow job — a new workflow file named lifecycle-layout-e2e.yml following the established shape (`crew-e2e.yml:105` is the mobile-safari template, and `crew-e2e.yml:24-34` its path-filter convention) — that runs `pnpm exec playwright test --project=mobile-safari tests/e2e/admin-lifecycle-layout.spec.ts`, with `workflow_dispatch:` enabled (repo close-out rule) and **NO `pull_request.paths` filter** — the job runs on every PR. Ratified R14 after the filter surface grew across four review rounds (R11 five paths → R12 dependency list → R13 `components/admin/**` + lockfiles → R14 `lib/**`/`app/api/test-auth/**`/`supabase/**`/CI bootstrap): the spec's true dependency graph is effectively the whole app plus the harness, so any enumerated filter re-opens the dark-path hole it was meant to close. Unfiltered is the structural end of the vector; the job's cost is bounded by the same concurrency-cancel shape the other e2e workflows use (`crew-e2e.yml:36-40`). Real-CI green on this job is part of the branch's close-out gate. **Workflow-coverage meta-test — CREATED by this branch** (R13; the CI-darkness vector hit three consecutive rounds, so the structural defense ships now per the structural-defense calibration rule): a new `tests/ci/` meta-test that parses every `.github/workflows/*.yml` `run:` block for Playwright invocations — BOTH direct `playwright test ...` commands AND pnpm script aliases, resolved through `package.json` `scripts` (R14: e.g. `modal-header-layout-e2e.yml` runs `pnpm test:e2e:modal-header`, whose spec list lives in `package.json`) — collects the transitively-invoked spec paths, walks `tests/e2e/*.spec.ts`, and FAILS for any spec file that a Playwright project `testMatch` matches but no workflow invokes; scanner self-tests cover direct, multiline, and package-script-mediated invocation shapes (the `_rowWrapperScan`-style tested-scanner pattern) — with an explicit `LOCAL_ONLY_ALLOWLIST` seeded with the currently-dark specs (each row carrying a reason or backlog ref, `admin-lifecycle-transitions.spec.ts` → `BL-E2E-LIFECYCLE-SPECS-CI-DARK`). New dark specs fail by default; the pre-existing darkness is inventoried, not silently blessed. Class-sweep note: wiring `admin-lifecycle-transitions.spec.ts` itself stays in that backlog entry (this spec wires only the file carrying its load-bearing assertions).
...
- **CREATES** the e2e workflow-coverage meta-test under `tests/ci/` (§6 item 6): Playwright-matched but workflow-uninvoked spec files fail by default, with a reasoned `LOCAL_ONLY_ALLOWLIST`.
