...
3. **New phone-height scrollport case** (R5 adversarial finding): a new test in the same spec at 390x700 — a height that forces the popover's `max-h-[min(70vh,32rem)]` scroller (`ShareHub.tsx:479`) to overflow — opens the hub, arms Archive, and asserts the armed `archive-show-confirm-button`'s bounding box lies fully inside the popover's visible scrollport (`popover.getBoundingClientRect()` intersected with viewport) after the arming click's `scrollIntoView` (`ShareHub.tsx:596-605`) settles, and that `scrollTop` moved when the confirm started below the fold. This covers the armed-spacing change (§1.1 exception) at exactly the venue-floor geometry the scroll handler exists for; no existing suite exercises it (the current sweep is width-only at height 1000).
