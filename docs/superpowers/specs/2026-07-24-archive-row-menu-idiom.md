...
3. **New phone-height scrollport case** (R5 adversarial finding; contract hardened R6): a new test in the same spec at 390x700 — a height that forces the popover's `max-h-[min(70vh,32rem)]` scroller (`ShareHub.tsx:479`) to overflow. Sequence, designed so the test cannot pass without the production `scrollIntoView` (`ShareHub.tsx:596-605`) doing the work:
   1. Open the hub; assert the OVERFLOW PRECONDITION on the popover: `scrollHeight > clientHeight`, and `scrollTop === 0` (fresh open, untouched scroller). If the precondition fails, the test fails — it must never silently degrade into a no-overflow pass.
   2. Arm via `locator.evaluate((el) => el.click())` on `archive-show-button` — a direct DOM click, NOT a Playwright actionability click, so no framework auto-scroll can be misattributed to the application handler (`reference: playwright actionability scrolls targets into view before pointer clicks`).
   3. After the confirm mounts and the handler's `requestAnimationFrame` settles, assert three things on the popover/confirm pair: (a) BELOW-FOLD PRECONDITION, reconstructed statically: the confirm's offset within the scroll content (`offsetTop + offsetHeight`, resolved against the popover as offset context) exceeds `clientHeight` — i.e. at the recorded `scrollTop === 0` origin the confirm could not have been visible; (b) `scrollTop > 0` — since step 2 ruled out framework scrolling, only the application's `scrollIntoView` can have moved it; (c) the confirm's bounding box lies fully inside the popover's visible scrollport (popover rect intersected with the viewport), within 0.5px.
