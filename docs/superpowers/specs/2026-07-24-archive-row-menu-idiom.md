...
6. **CI wiring (R11 adversarial finding — verified: no workflow invokes `admin-lifecycle-layout.spec.ts`; it is matched only by the `mobile-safari` project regex, and every e2e workflow runs an explicit spec list).** The implementation adds a workflow job — a new workflow file named lifecycle-layout-e2e.yml following the established shape (`crew-e2e.yml:105` is the mobile-safari template, and `crew-e2e.yml:24-34` its path-filter convention) — that runs `pnpm exec playwright test --project=mobile-safari tests/e2e/admin-lifecycle-layout.spec.ts`, with `workflow_dispatch:` enabled (repo close-out rule). Path filters (R12: cover the spec's real dependency surface, not just the diffed files, matching the crew-e2e convention of production surfaces + spec + helpers + config + self):
   - production surfaces the spec measures: `components/admin/ArchiveShowButton.tsx`, `components/admin/UnarchiveShowButton.tsx`, `components/admin/showpage/**` (ShareHub, StatusStrip, the modal surface), `app/admin/**` (dashboard + modal routes and the rotate/reset controls under `app/admin/show/[slug]/`), `app/globals.css`, `tailwind.config.*`
   - test dependency surface: `tests/e2e/admin-lifecycle-layout.spec.ts`, `tests/e2e/helpers/**`, `tests/db/_b2Helpers.ts`, `playwright.config.ts`
   - the workflow file itself (self-trigger path, .github/workflows/ + its own filename)

   Real-CI green on this job is part of the branch's close-out gate. The proposed "structural meta-test mapping every load-bearing e2e spec to a workflow trigger" is a repo-wide guard beyond this restyle's diff — folded into `BL-E2E-LIFECYCLE-SPECS-CI-DARK` (BACKLOG.md) as the structural fix for the whole class, per deferral discipline. Class-sweep note: `admin-lifecycle-transitions.spec.ts` is equally dark in CI; wiring it is likewise part of that backlog entry (this spec wires only the file carrying its load-bearing assertions).
