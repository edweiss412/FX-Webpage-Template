...
| armed-pending → gone (success) | `router.refresh()` re-renders the surface into the Archived presentation (`ArchiveShowButton.tsx:143-147`); `ConfirmButton`'s unmount cleanup clears a still-pending busy level so the host's dismissal gate can never stay wedged (`ArchiveShowButton.tsx:378-383`; busy release on settle pinned by `ArchiveShowButton.test.tsx:184-210`); the hub then closes across the Archive→Unarchive swap (existing hub-close contract, `tests/components/admin/showpage/shareHub.test.tsx:887-895`). Instant; no animation. Unchanged. |
...
6. **CI wiring (R11 adversarial finding — verified: no workflow invokes `admin-lifecycle-layout.spec.ts`; it is matched only by the `mobile-safari` project regex, and every e2e workflow runs an explicit spec list).** The implementation adds a workflow job — a new workflow file named lifecycle-layout-e2e.yml following the established shape (`crew-e2e.yml:105` is the mobile-safari template, and `crew-e2e.yml:24-34` its path-filter convention) — that runs `pnpm exec playwright test --project=mobile-safari tests/e2e/admin-lifecycle-layout.spec.ts`, with `workflow_dispatch:` enabled (repo close-out rule). Path filters (R12: cover the spec's real dependency surface, not just the diffed files, matching the crew-e2e convention of production surfaces + spec + helpers + config + self):
   - production surfaces the spec measures (R13: the dashboard/modal components its EXISTING assertions exercise, not just this diff's leaves): `components/admin/**`, `app/admin/**`, `app/globals.css`, `tailwind.config.*`
   - test + toolchain dependency surface: `tests/e2e/admin-lifecycle-layout.spec.ts`, `tests/e2e/helpers/**`, `tests/db/_b2Helpers.ts`, `playwright.config.ts`, `package.json`, `pnpm-lock.yaml`
   - the workflow file itself (self-trigger path, .github/workflows/ + its own filename)

   Real-CI green on this job is part of the branch's close-out gate. **Workflow-coverage meta-test — CREATED by this branch** (R13; the CI-darkness vector hit three consecutive rounds, so the structural defense ships now per the structural-defense calibration rule): a new `tests/ci/` meta-test that parses every `.github/workflows/*.yml` for `playwright test` invocations, collects the explicitly-invoked spec paths, walks `tests/e2e/*.spec.ts`, and FAILS for any spec file that a Playwright project `testMatch` matches but no workflow invokes — with an explicit `LOCAL_ONLY_ALLOWLIST` seeded with the currently-dark specs (each row carrying a reason or backlog ref, `admin-lifecycle-transitions.spec.ts` → `BL-E2E-LIFECYCLE-SPECS-CI-DARK`). New dark specs fail by default; the pre-existing darkness is inventoried, not silently blessed. Class-sweep note: wiring `admin-lifecycle-transitions.spec.ts` itself stays in that backlog entry (this spec wires only the file carrying its load-bearing assertions).
