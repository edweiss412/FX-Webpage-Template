...
3. **Fail-soft is the contract.** An unsplittable value renders the existing raw "Crew Wi-Fi" row byte-identically. Show-level accounting (spec R2 F1 — the 10 fixture SHOWS have raw + exporter MIRRORS whose values differ; the canonical per-show value is the richer mirror): 4 of 10 fixture shows split (Fixed Income, FinTech, Consultants, RIA), 3 are prose-only, 3 are empty; 2 of 4 live sheets split. Unchanged behavior for 8 of 14 SOURCES (the raw-family mirrors of Consultants/RIA are empty while their exporter mirrors split — mirror divergence is a fixture-family artifact, recorded, not a product state).
...
4. **On a rooms fetch failure the Venue room row is absent, identical to no-room** — deliberate: Venue adds information only from healthy data; the failure is surfaced by the rooms-consuming tiles' existing `SectionTileError`, so the state is signaled, just not twice. A synthesized `General Session` name is likewise suppressed, so v1 sheets show no room row** — their sheets never named the room; showing the placeholder would be noise. Files with the same response as limit 1 if a real venue room is ever literally named "General Session": the raw sheets can rename it, and the suppression is a one-line literal.
