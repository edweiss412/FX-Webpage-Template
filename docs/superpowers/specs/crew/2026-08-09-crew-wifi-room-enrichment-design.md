...
2. **Accept-set, not denylist (label vocabulary).** The splitter accepts EXACTLY the observed label vocabulary from the full-corpus probe (§4): `SSID` / `Network` (network-name labels) and `Code` / `PW` (password labels), `\b`-anchored, case-insensitive, colon-separated, line-oriented. Everything outside the accept-set renders as today's raw string — rejected-by-default, surfaced unchanged. `Password` as a label was NOT observed in any of 14 sources; if it ever appears, the raw fallback renders it verbatim (a conservative miss, §6.1) — widening the vocab then is a one-line change with its own probe.
...
- Tokenize into lines; within each line, match `\b(SSID|Network)\s*:\s*(\S[^\n]*?)\s*(?=\b(?:SSID|Network|Code|PW)\s*:|$)` and `\b(Code|PW)\s*:\s*(\S[^\n]*?)\s*(?=\b(?:SSID|Network|Code|PW)\s*:|$)` (case-insensitive) — the lookahead lets two pairs share one flattened line.
...
Room row: `const gsRoom = rooms.filter(r => r.kind === "gs").sort(compareRooms)[0]` — when present and `name` non-empty, `factRows.push({ k: "Room", v: gsRoom.name })` positioned before the connectivity rows. Guard conditions: zero rooms → no row; multiple gs rooms → first by `compareRooms` (stable order); name empty/whitespace → no row. Cap: single row; never a list (breakouts render in their own tiles).
...
| --- | --- | --- |
| Labeled pairs (SPLITS) | 2 fixtures + 2 live (same shows: Fixed Income 2025, FinTech 2026) | `Hardline from Encore\n\nSSID: Hyatt_Meeting\nCode: FITS2025` → ssid `Hyatt_Meeting`, password `FITS2025`, notes `Hardline from Encore` |
...
Calibration facts the design encodes: live cells are MULTI-LINE while fixture markdown flattened them (both shapes parse); observed label vocabulary is exactly `SSID`/`Network`/`Code`/`PW` (`Password` never appears); no internet value contains `/` (the rejected R2 rule would have been calibrated on nothing and endangered by `GRAND BALLROOM A/B`-class strings elsewhere); unanchored `code|pw` substrings are dangerous (`dress code` exists as an event key) — hence word-boundary plus colon anchoring and line orientation.
...
- **AC-1** `parseWifiValue` unit tests derived from the §4 corpus verbatim: both live multi-line values split (ssid/password/notes exact), the flattened fixture shapes split identically, all three prose-only values return `null`, empty returns `null`, password-only returns `null`.
...
- **AC-4** Room row: gs room name renders (`venue-room` testid); zero-rooms, empty-name, and multi-gs (first by `compareRooms`) cases covered; the redefining-fixed-income fixture drives the zero-case.
...
1. **Unobserved label spellings render raw.** `Password:`, `Passcode:`, `WPA:` etc. were never observed (14 sources); they fall to the raw fallback — surfaced verbatim, never silently wrong. Widening the accept-set is a one-line change gated on a future probe.
...
4. **The room row surfaces the general-session room only.** Breakout/additional rooms stay in their own tiles; a show whose crew works only breakouts sees no Venue room row (their room appears in room-scoped tiles). Accepted scope line.
