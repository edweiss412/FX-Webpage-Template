...
New `tests/db/crew-rpc-lifecycle-guard-meta.test.ts` (psql against `TEST_DATABASE_URL ?? DATABASE_URL ?? local`, mirroring `tests/db/b2-lifecycle-rpc-meta.test.ts:5-15`; CI sets `TEST_DATABASE_URL`=validation, the complete catalog). Table set for all SQL below = `{crew_members, crew_member_auth, show_share_tokens}` plus the `shows.picker_epoch` column. Three predicates, all computed from the **live catalog**:
