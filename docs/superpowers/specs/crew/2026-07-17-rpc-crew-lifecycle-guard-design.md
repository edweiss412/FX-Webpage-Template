...
Universe = every `public` `SECURITY DEFINER` function that mutates `crew_members`, `show_share_tokens`, or `shows.picker_epoch` **directly OR by delegating to a private helper that does** (the delegating-wrapper arm — §5 Step B(b)), enumerated from `pg_catalog`, not source grep (`create or replace` + later `drop function` means later migrations win). Guard tokens: **A** = archived refusal, **P** = published refusal, **F** = finalize-owned refusal.

Enumerated from the **validation** catalog (`TEST_DATABASE_URL`, the CI meta-test target — a complete, parity-maintained catalog; the local dev DB is unreliable, see §5 note) using the **anchored** `DIRECT_MUTATOR` predicate (§5): a DML keyword *immediately on a target table* (`insert into|update|delete from  <target>`), NOT merely "references a target table AND contains a DML keyword" (that looser form wrongly matched `validation_finalize_all_atomic`, which `DELETE`s `public.shows` and only reads crew — Codex R4 F2). Grant surface = live `has_function_privilege` (`authed|anon|svc`).

| RPC | target mutation | grant | §5 bucket | disposition |
|---|---|---|---|---|
...
| `archive_show` | (via `_archive_show_core`) | `t\|f\|f` | EXEMPT (entry) | lifecycle-transition wrapper; delegates share/epoch mutation to its core; carries its own B2 guards |
...
**Deliberately NOT in the universe** (verified against the validation catalog — do not re-add): `publish_show`/`unpublish_show` and their cores `_publish_show_core`/`_unpublish_show_core` — the cores update only `shows.published`, never a target table (crew/share/`picker_epoch`), so neither the cores nor their wrappers are crew/share mutators. `validation_finalize_all_atomic` — `DELETE`s `public.shows` (cascade) + `UPDATE`s `validation_state`; no direct target DML. `set_field_override`/`_apply_override_live` — added in `20260707000000_admin_field_overrides.sql`, **removed** in `20260710000000_remove_admin_field_overrides.sql`; absent from the live catalog (a source-grep-only reviewer will "find" them — Codex R1/R4 both did — but they don't exist; the meta-test enumerates the live catalog, so they never appear).

Each row's disposition feeds the matching §5 registry (GUARDED / EXEMPT-entry / TRIGGER_MUTATORS / PRIVATE_HELPERS). The table is enumerated from the **live** `pg_catalog` of the **validation** project (`TEST_DATABASE_URL`) — this is load-bearing on two counts: (1) `create or replace` + later `drop function` means source-grep is actively misleading (see below); (2) the local dev DB on this workstation is a partially/non-monotonically applied catalog (the `supabase db reset` pg_cron brick, see the durable memory), so it is NOT a reliable enumeration source — the CI meta-test runs against `TEST_DATABASE_URL` (validation), which is kept complete by the validation-schema-parity discipline.

**The crew-auth table (and its RPCs) were removed in M9.5 — the target set is `crew_members` + `show_share_tokens`.** The M9.5-era crew-auth table AND every RPC that mutated it (`revoke_all_links_rpc`, `issue_new_link_rpc`, `revoke_leaked_link_atomic`, the bootstrap-nonce set) were **dropped in the M9.5 cutover** (`supabase/migrations/20260523000099_cutover_drop_m9_5.sql`, incl. `drop table … :26`). The table no longer exists on validation or local. It is therefore NOT part of the meta-test's target set — and its name is a forbidden legacy reference pinned by `tests/cross-cutting/no-m9-5-surfaces.test.ts` (which scans `app`/`lib`/`components`/`tests`), so the meta-test uses `crew_members|show_share_tokens` only. (Should the crew-auth surface ever return, it would arrive via a new table + migration that its own lockdown/meta-tests would gate.)
...
New `tests/db/crew-rpc-lifecycle-guard-meta.test.ts` (psql against `TEST_DATABASE_URL ?? DATABASE_URL ?? local`, mirroring `tests/db/b2-lifecycle-rpc-meta.test.ts:5-15`; CI sets `TEST_DATABASE_URL`=validation, the complete catalog). Table set for all SQL below = `{crew_members, show_share_tokens}` plus the `shows.picker_epoch` column (the M9.5 crew-auth table was dropped — see §4). Three predicates, all computed from the **live catalog**:

- `DIRECT_MUTATOR(f)` — `pg_get_functiondef(f)` contains a DML keyword **immediately on a target table**: it matches the POSIX ERE `(insert into|update|delete from)[[:space:]]+(only[[:space:]]+)?(public\.)?(crew_members|show_share_tokens)`, OR it references `picker_epoch` within an `update…shows`. This anchored form is deliberate (Codex R4 F2): the loose "references a target AND contains any DML keyword" wrongly matched `validation_finalize_all_atomic` (which `DELETE`s `shows` and only reads crew). **Use POSIX `[[:space:]]`, not `\s`** — a bare `\s` silently failed to match during authoring and would produce an empty universe (which the Step-D parity check would then catch as "every GUARDED fn missing from the universe", but the anchored POSIX form is the intended, non-tautological predicate).
- `CLIENT_REACHABLE(f)` — `has_function_privilege(role, f, 'EXECUTE')` is true for **any** of `{authenticated, anon, service_role}`. (PUBLIC-only grants also count as reachable.) This is the fix for R2 F1: `claim_oauth_identity`, `mint_validation_fixture_atomic` are `service_role`-granted (not `authenticated`) — verified live `f|f|t` (order authenticated|anon|service_role) — so an "authenticated-only" test would wrongly bucket them as private helpers.
...
**Step A — private mutating-helper registry (closes the delegation blind spot, R1 F4).** A wrapper can mutate crew/share tables *without inline DML* by delegating to a helper (e.g. `unarchive_show` → `_unarchive_show_apply` at `...20260718000001...:40-50`; `undo_change` → `_undo_tombstone`). Enumerate **helpers**: every function (definer OR invoker) where `DIRECT_MUTATOR(f)` AND NOT `CLIENT_REACHABLE(f)` AND NOT `IS_TRIGGER(f)` — i.e. reachable by no client role, so only via a definer call chain. Assert each enumerated helper is in the in-test `PRIVATE_HELPERS` registry `{name → reason}` — exactly `_archive_show_core`, `_unarchive_show_apply`, `_undo_tombstone` (all verified `f|f|f`, all anchored-`DIRECT_MUTATOR`). Note `_publish_show_core`/`_unpublish_show_core` are **not** here: they update only `shows.published`, so they are not `DIRECT_MUTATOR`s of a target table and never enter this enumeration. **Fails-by-default:** a NEW private target-mutating helper forces a registry row — the prompt to check its callers. `HELPER_NAMES` = the registry's key set, used by Step B.
...
**Step B — entry-point universe (direct + delegating).** Enumerate every `public` `prosecdef` function where NOT `IS_TRIGGER(f)` AND `CLIENT_REACHABLE(f)` AND (`DIRECT_MUTATOR(f)` OR body calls any `HELPER_NAMES` member via POSIX regex `\m(_archive_show_core|_unarchive_show_apply|_undo_tombstone)[[:space:]]*\(`). Arm (b) is the delegation fix — `unarchive_show` enters via its `_unarchive_show_apply` call though its own body has no share/crew DML. Verified live: the ONLY authed/service-role definers calling a `HELPER_NAMES` member are `archive_show`, `unarchive_show`, `undo_change` (and `undo_change` is already a `DIRECT_MUTATOR`). `publish_show`/`unpublish_show` call `_publish_show_core`/`_unpublish_show_core`, which are NOT in `HELPER_NAMES`, so they correctly do NOT enter the universe. (Scope note: `ENTRY_POINT` is limited to `prosecdef` — a reachable **invoker** function mutating crew/share tables directly would be a PostgREST-DML-lockdown bypass, covered by `rpc-service-role-revokes.test.ts` + the RLS/table-grant lockdown tests, not this meta-test; noted so the boundary isn't a silent gap.)
...
- `EXEMPT` — the client-reachable, non-guarded entry points, each `{name → reason}` (exactly five):
  - `claim_oauth_identity` (service-role; live crew redemption — must run whenever the link is live).
  - `mint_validation_fixture_atomic` (service-role; validation/test fixture seeding — inserts crew/share fixture rows).
  - `mi11_approve_hold` (authenticated; pre-publish onboarding hold-approval — operates in the held window).
  - **`archive_show`, `unarchive_show`** (authenticated `t|f|f`; **lifecycle-transition wrappers** — they enter Step B via arm (b) because they delegate their share/epoch mutation to `_archive_show_core`/`_unarchive_show_apply`. A lifecycle guard is inapplicable: each **is** the transition and already carries its own B2 lifecycle refusals — pinned by `b2-lifecycle-rpc-meta.test.ts`, `archive_show_rpc.test.ts`, `unarchive_show_rpc.test.ts`. `publish_show`/`unpublish_show` are NOT here — their cores don't touch a target table, so they are outside the universe entirely.)
...
Both are `create or replace function` follow-ons (no schema/table DDL, no CHECK/enum change) → idempotent by construction (re-apply replaces the body). Per the validation-schema-parity discipline: (1) apply the two guard migrations + run the new/changed DB tests; (2) `pnpm gen:schema-manifest` — **function bodies are not in the public column/table manifest**, so the manifest is unlikely to change; regen + commit if it does, else note "no manifest delta (function-body-only change)"; (3) apply both migrations surgically to validation `vzakgrxqwcalbmagufjh` (`supabase db query --linked` or `psql "$TEST_DATABASE_URL" -f`), then `notify pgrst, 'reload schema'`. The `validation-schema-parity` gate is a superset check on public columns/tables; function-body changes don't move it, but the surgical apply keeps validation's live RPC behavior in parity for the validation-targeted DB tests.

**DB target for TDD (local-DB caveat).** The new `crew-rpc-lifecycle-guard-meta.test.ts` and the reset/undo behavioral DB tests read `TEST_DATABASE_URL ?? DATABASE_URL ?? local`. Because this workstation's **local** DB is a partially-applied catalog (the `supabase db reset` pg_cron brick — durable memory), the meta-test's live enumeration is only trustworthy against a **complete** catalog. Plan-time decision: run the new DB tests against `TEST_DATABASE_URL`=validation (as CI does; apply the two guard migrations there first, per step 3), OR bring the local DB fully current via the psql-apply loop (skipping the pg_cron/vault migrations `20260527000001/2/3`, `20260602000005`) before running locally. Do NOT trust a green run against the stale local DB — enumerate/verify against validation.
...
- **EXEMPT entries are intentional, not overlooked.** `claim_oauth_identity` / `mint_validation_fixture_atomic` (service-role) / `mi11_approve_hold` / `archive_show` / `unarchive_show`, plus the `create_share_token_for_show` trigger, each carry a §4/§5 rationale. The meta-test pins them so they can't drift silently — a reviewer asking "why isn't claim_oauth_identity guarded" is answered by the registry reason.
...
- **The audit universe (§4) is `pg_catalog`-enumerated on purpose; source-grep is stale.** Functions that a migration-source grep "finds" but that are NOT in the live catalog (do not re-flag): the crew_member_auth link RPCs `revoke_all_links_rpc` / `issue_new_link_rpc` / `revoke_leaked_link_atomic` (dropped in the M9.5 cutover, `...20260523000099...:5-11`); and `set_field_override` / `_apply_override_live` (added in `20260707000000`, **removed** in `20260710000000`). Both classes were raised from stale source during review (R1/R4) and both are absent live. Verify any "missing RPC" claim against `pg_catalog`, not `supabase/migrations/**`.
- **`validation_finalize_all_atomic`, `publish_show`, `unpublish_show` are deliberately outside the universe.** `validation_finalize_all_atomic` `DELETE`s `shows` (no target-table DML); `publish_show`/`unpublish_show` delegate to `_publish_show_core`/`_unpublish_show_core`, which update only `shows.published`. None is a crew/share/`picker_epoch` mutator. The anchored `DIRECT_MUTATOR` predicate (§5) excludes the first; the `HELPER_NAMES`-restricted delegation arm excludes the latter two. A reviewer proposing to add them should verify against the anchored predicate.
- **The meta-test detects delegating wrappers (Step B arm b), not just inline-DML bodies.** A reviewer worried that a helper-delegating wrapper escapes classification should see §5 Steps A/B: private target-mutating helpers are registered and any authed/service-role definer calling one enters the universe.
- **The meta-test enumerates against `TEST_DATABASE_URL` (validation), not the local dev DB.** The workstation local DB is a partially/non-monotonically applied catalog (`supabase db reset` pg_cron brick) and is NOT a reliable enumeration source; the spec's registries were finalized against the validation catalog. CI runs the meta-test against validation, which the validation-schema-parity discipline keeps complete.
