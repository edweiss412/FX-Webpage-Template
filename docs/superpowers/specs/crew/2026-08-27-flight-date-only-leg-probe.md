...
`--validation` reads `crew_members.flight_info` from the validation project (`TEST_DATABASE_URL`, or `--db <url>`) and classifies it with the same code, one read-only SELECT. It reports **30 crew rows, 5 with a non-null `flight_info`, 10 segments, 10 populated, 0 date-only**, and the five itinerary texts are IDENTICAL to the corpus quoted above, compared verbatim.

The identity is the point, and the agreeing counts are not enough for it. This cross-check exists to rule out one thing: a show that reached the database without passing through the folder the probe walks. Swap one folder itinerary for one outside-folder itinerary and every count above still agrees, so the aggregates cannot see the case they were supposed to catch. `itinerariesMatchCorpus` compares the sorted texts instead, the committed run records both sides, and the suite asserts the comparison AND that it discriminates. What remains true is that this is the expected agreement rather than an independent measurement: the column is this same parser's output over these same sheets.

A missing connection string prints a skip and returns null; it never degrades to a zero, for the same reason a parse failure does not.
...
- **A shrinking denominator.** The suite asserts an unparseable sheet is an unmeasured row and not a clean zero, and that a sheet with crew and no flight data at all stays in the corpus. The second uses a committed fixture under `fixtures/shows/raw/`; the first uses a minimal inline document, because the corpus rule keys on the PRESENCE of a hard error rather than on which code was raised, and the repository has no committed fixture that hard-fails. The inline case raises `MI-1_VERSION_DETECTION_FAILED`, not the live row's `VERSION_AMBIGUOUS`; that the live sheet fails at all is asserted separately, from the committed run.
- **A hand-transcribed report.** Every cell of the table above is asserted equal to `2026-08-27-flight-date-only-leg-probe.json`, the probe's own committed output, so a hand-edited cell fails rather than drifts. The columns are read out of the table's own header rather than listed in the test, and a header the suite cannot map to a probe field is itself a failure, so a column added here arrives pinned or not at all. The verbatim itinerary block is compared the same way, against the texts the run recorded.
...
- It walks one Drive folder. Shows outside `fxav-test-shows` are outside the measurement. The validation cross-check is the only evidence there are none, and it is evidence about one deployment: the itineraries in validation's `crew_members` are exactly the folder's, so nothing reached that database from elsewhere. A show that never reached validation either is invisible to both sources.
- One sheet in the folder does not parse. `TRANSPORTATION DETAILS FOR CJ` is `VERSION_AMBIGUOUS`, and the run extracted zero crew from it — which is all the evidence establishes. Whether it holds no crew block, or holds one the parser never reached because version detection failed, is exactly what a hard error leaves unknown. It is reported as unmeasured rather than guessed at, and if it does carry crew with flight data, the corpus zero is a zero over seven sheets and not eight.
