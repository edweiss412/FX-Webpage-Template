...
- **AC-2b (close clears; second failure clears).** Demote → close → immediate re-open inside the exit window (the retained-instance shape probed at `tests/components/diagrams/gallery.failedItem.test.tsx:553`) → no chip (§2.1 clear 3). Demote → clamped-tier failure (the `galleryLightbox.zoomGate.test.tsx:697` two-stage scenario) → no chip once the placeholder shows (§2.1 clear 4).
