...
- **Clear, four conditions (R1 F1/F2 widened the original two):**
  ...
  3. **Dialog close, at the moment the close BEGINS — not exit completion and not unmount (R1 F1).** A re-open inside the 220ms exit window CANCELS the exit and retains the component instance (probed by the live suite: the lightbox region is the same DOM node and `onExitComplete` never fires, `tests/components/diagrams/gallery.failedItem.test.tsx:553` and line 615), so an unmount-keyed clear would carry the chip into the next dialog session, contradicting §4 limit 3. Keying on the close transition (`open` going false) clears state and timer before any re-open can observe them.
