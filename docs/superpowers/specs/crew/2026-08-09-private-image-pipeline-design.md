...
**Lightbox** (`components/diagrams/GalleryLightbox.tsx`). Active slide: original-URL tier via `pinOriginal: true` (zoom needs full resolution). Inactive slides: the standard clamping loader with `sizes="100vw"` — clamping (R1 F3, folded into the single loader behavior above after R2 F4) guarantees they select the largest variant (1024 on a full ladder) and never the original; only variant-less entries serve the original. R1 probe evidence: with `sizes="100vw"` and an above-max→original rule, Next's device-width candidates (`deviceSizes` up to 3840 per Next's default image config — upstream constant, probed live in R1) produce `original@1080w, original@1200w, …`, so a 360-430 CSS-px 3× phone would have selected the ORIGINAL — violating the §1.1 inactive-tier contract. **Both lightbox branches carry `placeholder="blur"` when `blurDataURL` is present (R1 F6)** — the §1 goal promises blur on both components, active and inactive slides alike. When intrinsic dims present, render with `width`/`height` (correct aspect reservation); else `fill` + `object-contain`, with a positioned-ancestor guarantee PER BRANCH (R1 F5, corrected R2 F3 — the two branches have DIFFERENT containing blocks): **inactive** `fill` images are direct children of the slide `figure` (`components/diagrams/GalleryLightbox.tsx:691`), which is currently non-positioned (`components/diagrams/GalleryLightbox.tsx:488`) and gains `relative`, so `fill` resolves against the slide, not the outer lightbox viewport (`components/diagrams/GalleryLightbox.tsx:432`); **active** `fill` images sit inside `react-zoom-pan-pinch`'s `TransformComponent` (`components/diagrams/GalleryLightbox.tsx:629`), whose wrapper is ALREADY `position: relative` and is the nearer containing block — the active invariant is image box === TransformComponent wrapper box, and the wrapper box is not the figure box (the figure carries horizontal padding, `components/diagrams/GalleryLightbox.tsx:488`). Both branches appear as separate Dimensional Invariants rows and both get real-browser geometry assertions (§9). The existing zoom/pan and Embla integration contracts are untouched — the migration swaps the element, not the gesture layer.
...
| lightbox slide `figure` (GalleryLightbox.tsx:488 — gains `relative`, R1 F5) | INACTIVE slide `Image` (no-dims fallback branch; direct child of the figure, `components/diagrams/GalleryLightbox.tsx:691`) | image box === figure content box, letterboxed content | `relative` on the `figure` + `fill` + `object-contain` — without the `relative`, `fill` resolves against the outer lightbox viewport (`components/diagrams/GalleryLightbox.tsx:432`) |
...
- **Caller suites** (extend `tests/sync/snapshotAssets.test.ts`, `tests/sync/assetRecovery.test.ts` — R2 F1): failure isolation lives HERE (corrupt sibling → its entry field-less, siblings unaffected, `snapshot_status` unchanged); `variantFailures` threaded to the surface's post-commit sink; the `DIAGRAM_VARIANT_GENERATION_FAILED` emit happens post-commit ONLY (rollback fixture → zero emits).
...
- **Real-browser layout assertion** (per writing-plans layout-dimensions rule): thumbnail image box === its `aspect-square` cell; lightbox INACTIVE no-dims image box === its `relative` slide `figure` content box; lightbox ACTIVE no-dims image box === its `TransformComponent` wrapper box (R2 F3 — both branches, different containing blocks); all via `getBoundingClientRect` within 0.5px — jsdom cannot verify `fill` geometry.
...
- AC-12 (R1 F10, revised R2 F3): lightbox active slide fetches the original tier, inactive slides fetch the clamped variant tier, both show blur when present, `onError` lands in the unavailable branch, and BOTH no-dims `fill` branches measure equal to their containing blocks in a real browser (inactive → the `relative` slide `figure`; active → the `TransformComponent` wrapper).
