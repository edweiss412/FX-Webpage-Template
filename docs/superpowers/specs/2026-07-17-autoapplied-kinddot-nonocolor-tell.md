...
**Non-removed kinds (unchanged):** `<span aria-hidden="true" className={`size-2 rounded-full ${KIND_PILL[k]?.dot ?? FALLBACK_PILL.dot}`} />`.
...
The per-row `KindPill` (`:91`) also renders a color-only `size-1.5 rounded-full ${pill.dot}` dot, but it sits **immediately beside its uppercase text label** (`REMOVED`) and the row's `DiffBlock` renders a `line-through` value (`:139-145`) — the removed kind there is already dual-channel (color + text + strikethrough). No 1.4.1 concern; left unchanged to keep the blast radius to the one color-only surface (`KindDotCluster`).
...
- **Unknown kind** (outside `KIND_ORDER`): collapses to the single neutral `__fallback__` disc (`:161-162`), never a minus-bar (only literal `crew_removed` gets the bar). Unchanged.
...
Because the marker dimensions are CSS-literal (not the fragile flex-`items-stretch` gotcha the real-browser rule targets), the invariant is pinned in **jsdom** (class assertions), matching the ratified rationale for AUTOAPPLIED-REDESIGN-1's real-browser deferral (grid/explicit dims = CSS invariant, disproportionate to spin a standalone Playwright harness). No new real-browser harness.
...
- **AUTOAPPLIED-REDESIGN-2** (singleton card-in-card) — ratified as matching the approved mock.
