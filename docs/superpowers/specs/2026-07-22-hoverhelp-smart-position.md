...
| R8 | **Pinch-zoom / visual-viewport posture (R1 F2):** positioning is computed against the LAYOUT viewport (and the host rect). Under pinch zoom the popover may sit partly outside the VISUAL viewport; the user pans, exactly as for every other layout-viewport-positioned element in the app. This is byte-identical to the shipped absolute-positioning behavior — the portal neither improves nor regresses it. `visualViewport`-aware positioning is filed as `BL-HOVERHELP-VISUAL-VIEWPORT` (§7) — **SUPERSEDED 2026-07-25 by `docs/superpowers/specs/2026-07-24-hoverhelp-visual-viewport.md`**, which is that successor. R8's text stands as the ratification that filed it | This spec, R2 round |
...
**Scenario D (hover-timer bridge across the portal, added round 4):** replicated HoverHelp's full open/close machinery (mouse-gated pointerenter/leave on the root, pointerenter/leave on the body, shared 120ms close timer) with the body portaled and positioned 6px below the trigger; real Playwright pointer moved trigger → body. Result: body enter cancelled the pending close in BOTH variants — popover still open 300ms after arrival (log: `open`, `open`, no timer close), in-flow and portal identical. The one carried-over continuous-interaction mechanism is therefore empirically validated. The remaining §4.5 mechanisms (`focusin` timer cancel, Tab bridge) are NEW, not carried behavior — they are specified here and proven by T7/T8, which is the correct evidence tier for new code (the spike rule targets UNDOCUMENTED CARRIED framework behavior).
...
- Provider site — ONE, covering every dialog whose subtree can reach a HoverHelp instance: `ReviewModalShell` (panel div :618, `panelRef` at :146) wraps its children in `<PopoverHostContext.Provider value={panelRef}>`. Swept: `ShowReviewSurface` is the only HoverHelp-bearing subtree mounted inside any `role="dialog"`, and every dialog that mounts it — `PublishedReviewModal`, `app/admin/_showReviewModal.tsx`, and `Step3ReviewModal` (composed on the shell: import at Step3ReviewModal.tsx:43, rendered at :356, `<ShowReviewSurface>` inside it at :609) — is a `ReviewModalShell` consumer. (A round-4 draft wrongly described Step3ReviewModal as its own dialog implementation; corrected against :356.) Any future dialog that hosts HoverHelp instances adds the same provider; the §6 T1 fixture pins the mechanism, and the plan's class-sweep re-runs the `role="dialog"` × HoverHelp-subtree cross-grep at implementation time.
...
- **Panel host** (containing block = the panel, which is `position: relative`, ReviewModalShell.tsx:618; absolute offsets resolve from the containing block's PADDING box): `left = vx − hostRect.left − host.clientLeft + host.scrollLeft`, `top = vy − hostRect.top − host.clientTop + host.scrollTop` (clientLeft/Top = border widths; the scroll terms place the node in the host's SCROLLED content coordinate space — for the modal panel they are 0 because its inner pane scrolls instead, but a SCROLLING host — e.g. the geometry harness's pane-host case — needs them or the popover drifts by the scroll offset on every reposition).
...
| Tab order INSIDE modals | link reached at the panel's end (after every intervening control) instead of adjacent to its trigger; multiple open popovers group their links at the end | ACCEPTED, enumerated: reachability is guaranteed by trap enumeration (the hard requirement); end-of-dialog supplementary content is an established dialog pattern; a panel-host Tab bridge was REJECTED because the trap's native panel listener and the bridge's React handler act on the same keypress with registration-order-dependent outcomes when the link is the panel's last focusable (analyzed round 4) — an ordering fragility worse than the order quirk it would fix. In practice at most one popover is open (any outside interaction closes others) |
| Screen-reader browse/reading order (both hosts) | body no longer follows the trigger in the accessibility tree | MITIGATED: the root wrapper gains `aria-owns={bodyId}`, re-adopting the body at the trigger's position in the accessibility tree for browse navigation (T2 asserts the attribute + relationship); the CONTENT was already announced at the trigger via `aria-describedby` (shipped M12.5 contract, unchanged) |
...
- Tab (no shift) while focus is on the LINK → `preventDefault`, close the popover, restore focus to the trigger. DECLARED consequence: forward navigation visits the trigger twice (trigger → link → trigger → next control) — one extra keypress, non-looping (the popover is closed on return, so the bridge is inert for the next Tab). Accepted as the price of adjacency; moving focus directly to "the control after the trigger" would require re-implementing tab-order enumeration, precisely the complexity this design avoids.
...
| trigger rect with zero area (zero width or height) | `hidden` — the step-1 gate requires POSITIVE-area overlap, so a zero-area trigger never places (T-PURE row asserts `hidden`) |
...
<!-- spec-lint: ignore — new files created by this spec; not yet tracked -->
- **T-PURE (unit — the positioning core, NEW `tests/lib/popover/position.test.ts`):** exhaustive decision-table over `computePopoverPlacement`: fits-below / fits-above-only / neither-side (shrink engaged, maxHeight === larger space) / anchor-gone / narrow-bounds width shrink WITH `wrappedHeightAt` returning a LARGER height that flips the vertical decision (the R2 F1 composite, pinned forever) / align-left+right at both bounds edges / clamp saturation / zero-area trigger → hidden / exact-fit equalities (`height0 === space(side)` both sides) / equal-space ties at every comparison (resolve to preferredSide, fits and shrink branches) / preferred-top symmetry rows mirroring every preferred-bottom row / partially-intersecting anchors (each edge) / `naturalSize.width === bounds.width` boundary / degenerate-bounds and non-finite-input rows → hidden (incl. non-finite `wrappedHeightAt` result) / trigger positively overlapping but vertically spanning bounds so BOTH spaces compute to exactly 0 → hidden / trigger touching a bounds edge with zero intersection area → hidden. Pure algebra — no DOM. Also the independent numeric pins (R2 F9): `expect(GAP).toBe(6)` and `expect(VIEWPORT_INSET).toBe(8)` as spec-literal assertions, so geometry tests may then use the exported constants without the two-sided-drift tautology.
...
- **T7 (e2e + unit — reposition lifecycle):** e2e: (i) pane scroll → offset from trigger preserved (±1px) next frame; (ii) window resize while open → repositioned; (iii) body content growth (harness knob) → ResizeObserver reposition; (iv) HOST resize (harness shrinks the panel width — R2 F5) → re-bounded, body ⊆ new B, AND a previously applied maxWidth is CLEARED when the host re-expands (R2 F2 restoration). Unit (jsdom, mocked rAF — R2 F11 + R5 F2): (u1) close with a frame pending → `cancelAnimationFrame` called with the stored id, no style write; (u2) unmount with a frame pending → same, no error; (u3) OPEN performs its measure-and-apply without requesting any animation frame (rAF spy not called by the open path); (u4) `schedule()` while closed → no frame requested; (u5) three synthetic scroll events in one tick → exactly ONE frame requested (coalescing), and after it runs a fourth event requests a NEW frame (id cleared post-run); (u6) TRIGGER ResizeObserver entry invokes reposition; (u7) after close and after unmount, scroll/resize listeners and all three ResizeObserver observations are detached (spy on removeEventListener/unobserve).
  - **T3h (e2e — discriminative metric case, R5 F3):** fixture pinned by TWO preconditions asserted in the test itself: `body.scrollHeight > spaceBelow` (the uncapped metric would NOT fit below) AND `body.getBoundingClientRect().height ≤ spaceBelow` (the capped border-box metric DOES fit) → popover stays on the preferred bottom side with NO inline maxHeight (a `scrollHeight`-measuring implementation would wrongly flip or shrink). T7 additionally asserts a previously applied inline `maxHeight` is CLEARED when space re-expands (symmetric with the maxWidth restoration case).
