...
- `ReviewModalShell` wraps its children in `<PopoverHostContext.Provider value={panelRef}>` (it already holds `panelRef` for the trap and transitions). Any future dialog that hosts HoverHelp instances does the same.
