...
- The title is an `<h1>`, not a styled `<p>`. For a no-JS visitor the routed content never renders (§1.0), so within the route segment this notice is all there is, and a screen-reader user navigating by heading reaches a landmark that says what state the page is in. It is not always the document's *only* heading — a persistent layout outside the suspense boundary still renders, and `app/help/layout.tsx` keeps a sidebar whose links are `<h3>` elements (`app/help/_components/Sidebar.tsx:56`). An `<h1>` above those is a correct hierarchy, not a collision. The type scale matches the app's other dead-end screens — `ShowUnavailable.tsx:33-38`, `app/admin/layout.tsx:95-96` and `app/admin/error.tsx:37` all pair a `text-2xl` title with `text-base` body — so the notice reads in the same register as every other 'this page cannot show you anything' state rather than as a caption. (The earlier draft used a `<p>` and justified it as "should not invite interaction"; heading semantics do not imply interactivity, and the two undifferentiated paragraphs that produced were a real a11y defect.)
...
```html
<div data-testid="probe"><noscript><style>[data-loading-shell-content]{display:none}</style><div data-testid="loading-nojs-notice" class="rounded-lg border border-border bg-surface p-4"><h1 class="text-base font-semibold text-text-strong">JavaScript is required</h1><p class="mt-1 text-sm text-text-subtle">This page needs JavaScript to load. Turn it on in your browser settings, then reload.</p></div></noscript><div data-loading-shell-content=""><p role="status" class="sr-only">Loading your dashboard…</p><div data-testid="child"></div></div></div>
```
...
| 8 | The notice element's `class` attribute contains every token class from §2: `rounded-lg`, `border`, `border-border`, `bg-surface`, `p-4`. Membership, not string equality, so class order is not pinned. | A classless or differently-styled card. Assertions 1-7 all pass on a bare `<div>` with the right copy, discarding the visual treatment the user chose from three rendered options. |
| 9 | The heading's class list contains `text-base`, `font-semibold`, `text-text-strong`; the body paragraph's contains `mt-1`, `text-sm`, `text-text-subtle`. | Token drift on the two text elements — the same failure class as 8, one level down. Named separately because a fix to the card's classes does not imply a fix to these. |
| 10 | The heading and the body paragraph are both `contains`-descendants of the `loading-nojs-notice` element, not merely present somewhere in `noscriptInner`. | An empty padded card rendered beside loose copy. Assertion 5 alone passes on that, because it searches the whole `noscriptInner` document. |
| 11 | The serialized `outer` markup contains the literal `data-loading-shell-content=""`. | Regression to the bare JSX attribute, which serializes to `="true"`. §2 pins the empty-string form deliberately; nothing in assertions 1-10 notices the difference because the attribute *selector* matches either. |
...
**Two `describe` blocks, each with its own `test.use` — never a file-scoped one.** A file-scoped `test.use({ javaScriptEnabled: false })` applies to every case in the file, including the JS-on control, which would then silently test the same configuration twice and prove nothing about the JS-on path. Note that the JS-on control is not a race: with JavaScript enabled the browser never parses `<noscript>` contents at all, so the count is 0 from first paint onward and stays 0 — asserting at commit is simply the earliest and strictest moment to read it. The structure is:
...
| 4 | JS on | `page.goto("/admin", { waitUntil: "commit" })`, then **immediately** assert `getByTestId("admin-dashboard-loading")` is attached and `getByTestId("loading-nojs-notice")` has count 0. | The notice rendering unconditionally. This must be measured **while the fallback is still on screen**, not after the dashboard settles: once the page settles, the whole `loading.tsx` subtree is gone, so an unconditional notice that was visible for the entire load still reports count 0 at the end. Asserting at commit — the fallback attached, nothing hydrated — is the only moment where the count is evidence. No settle signal is needed or referenced, which also removes the flake surface and the question of whether `/admin` resolves to dashboard, onboarding, or an error branch. |
