...
| 15 | Both the root and the wrapper are `DIV` elements, and every assertion above runs for the **no-`testId`** variant too. | Element-type substitution and prop-keyed mutations. Swapping either div for a `<dialog>` keeps all attribute assertions true while UA styling hides the JS-on fallback; a class applied only when `testId` is absent leaves the probe render and the `/admin` e2e byte-identical while hiding `/me`, which passes no `testId` (`app/me/loading.tsx:11`). Verified against both. |
