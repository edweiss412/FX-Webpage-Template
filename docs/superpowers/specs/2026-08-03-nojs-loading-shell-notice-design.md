...
| 4 | **`<noscript>`-scoped `<style>` is the chosen hide mechanism, deliberately.** A browser with JavaScript enabled parses `<noscript>` contents only as raw text: it materializes none of the descendant elements and applies none of the CSS. So the rule reaches no JS-on visitor and the normal path pays nothing beyond the bytes on the wire, and it cannot affect the JS-on path. No client-side detection, no hydration branch, no `useEffect`. | §2 |
...
- **The notice carries its own gutter and width cap** (`mx-auto w-full max-w-2xl px-4 py-8 sm:px-8`), which is not optional polish. Only three `loading.tsx` files put horizontal padding below `data-loading-shell-content` (`app/show/[slug]/[shareToken]/loading.tsx:38`, `app/admin/show/[slug]/preview/[crewId]/loading.tsx:33`, `app/admin/settings/admins/loading.tsx:12`); all six admin fallbacks inherit it from `app/admin/layout.tsx:191`, and `/me` and `/help` pad from a parent of `LoadingShell`. So the notice gets **no** horizontal padding on the mobile-primary crew route, and the gutter double-applies on the rest. Dispositioned as a P3 in the plan's §12.
- `rounded-md` and `p-tile-pad` rather than `rounded-lg`/`p-4`: DESIGN.md §4 reserves the 16px radius for modal and dialog surfaces. Census across `app/` and `components/` `.tsx` at `548a81c93`, each figure the line count of the bare `rg -o` for that token: `rounded-md` 157, `rounded-lg` 9 — and 3 of those 9 are popover surfaces carrying `shadow-popover` (`components/admin/FinalizeButton.tsx:1060`, `components/admin/CleanupAbandonedFinalizeButton.tsx:169`, `components/admin/BellPanel.tsx:1251`). `p-tile-pad` (20px) is the house card padding, 81 uses on the same measure. Structural twin: `components/admin/RecentAutoAppliedStrip.tsx:676`.
- The title is an `<h1>`, not a styled `<p>`. For a no-JS visitor the routed content never renders (§1.0), so within the route segment this notice is all there is, and a screen-reader user navigating by heading reaches one that says what state the page is in. It is not always the document's *only* heading — a persistent layout outside the suspense boundary still renders, and `app/help/layout.tsx` keeps a sidebar whose links are `<h3>` elements (`app/help/_components/Sidebar.tsx:56`). An `<h1>` above those is a correct hierarchy, not a collision. The type scale matches the app's other dead-end screens — `app/admin/layout.tsx:95-96` pairs a `text-2xl` title with a `text-base` body for exactly this kind of state — so the notice reads in the same register as every other 'this page cannot show you anything' state rather than as a caption. (The earlier draft used a `<p>` and justified it as "should not invite interaction"; heading semantics do not imply interactivity, and the two undifferentiated paragraphs that produced were a real a11y defect.)
...
`// @vitest-environment jsdom`, asserting on `renderToStaticMarkup(<LoadingShell {...idProp} label="Loading your dashboard…"><div data-testid="child" /></LoadingShell>)`, where `idProp` is `{ testId: "probe" }` for one variant and `{}` for the other — the `describe` is parameterized over both, so every assertion below runs twice (see the note under the table). The file renders on the server but runs under the jsdom environment, because jsdom is what supplies `DOMParser` (§7.1 assertions) — `node` has no DOM parser, and an earlier draft that specified `node` was not executable. Re-run against a throwaway patch carrying the final `<h1>` markup, the `testId="probe"` variant returns, verbatim (the omitted-prop variant is identical but for the absent `data-testid` on the root):
...
Each is parsed with `DOMParser` (so the test file runs under `// @vitest-environment jsdom` while still rendering via `react-dom/server` — jsdom supplies the parser, not the renderer), and the helper returns both parsed `Document`s. It also returns the undivided `html` string, for the one assertion that is about serialization rather than structure and that no `Document` can answer: assertion 12 checks how the wrapper attribute was written, not what it parses to. Everything else uses real DOM containment.
...
| 12 | The undivided `html` string returned by the helper contains the literal `data-loading-shell-content=""`. Against the string, not either `Document` — a parsed DOM normalizes the two forms apart from its `getAttribute` value, so serialization is only observable before parsing. | Regression to the bare JSX attribute, which serializes to `="true"`; the attribute *selector* matches either way. |
...
| 14 | The wrapper's parent is the shell root itself, that root's parent is `<body>`, and the root's attribute set is exactly `["data-testid"]` — or exactly `[]` in the omitted-prop variant, since `data-testid={undefined}` renders no attribute at all. The expectation is conditional on the variant; the point is that the set is CLOSED either way. | Any hiding ANCESTOR. Blacklisting mechanisms loses — `hidden`, then `display:none`, then `visibility:hidden`, then `opacity:0`, then `clip`/`height:0` each defeated the previous round's list. This pins the chain's SHAPE instead, so any inserted element or any class/style on the root fails regardless of how it would hide the fallback. Verified against `opacity:0`, `sr-only`, and `height:0 overflow:hidden`. |
| 15 | Both the root and the wrapper are `DIV` elements. | Element-type substitution. Swapping either div for a `<dialog>` keeps all attribute assertions true while UA styling hides the JS-on fallback outright. |

**The `describe` is parameterized over both `testId` variants, so all fifteen run twice.** `testId` is optional and `app/me/loading.tsx:11` omits it, so a mutation keyed on the prop — `className={testId ? undefined : "hidden"}` on the wrapper — leaves the probe render and the `/admin` e2e byte-identical while hiding `/me`. The parameterization is on the block, not on individual cases, and that distinction is the finding: per-case coverage was the first fix, it reached two of the fifteen, and the mutant above escaped the other thirteen (cross-model review round 9 — 15/15 green, style guard green, `/me` wrapper rendered `class="hidden"`). On the block there is no per-assertion decision to get wrong and a new assertion is covered by construction.

### 7.2 Real-browser probe — tests/e2e/nojs-loading-notice.spec.ts (new file, so the path is unlinked)

jsdom cannot evaluate `<noscript>` semantics, so the behavioral claim needs a real browser. (Its CSS support is not the gap — jsdom does compute `display:none` from a stylesheet-backed rule. The gap is that scripting is always "enabled" there, so a `<noscript>`-scoped rule never applies, and React's client renderer leaves the subtree empty besides.) This test is also the regression guard that the deleted admin-banner spec used to provide, and the standing proof of §1.0.

**Two `describe` blocks, each with its own `test.use` — never a file-scoped one.** A file-scoped `test.use({ javaScriptEnabled: false })` applies to every case in the file, including the JS-on control, which would then silently test the same configuration twice and prove nothing about the JS-on path. The JS-on control is not a race in either direction: a JavaScript-enabled browser parses `<noscript>` contents only as raw text and materializes nothing from them, so the count is 0 from first paint onward and stays 0. Both reads below are therefore stable. The structure is:
...
**Registration is three steps, and the second is the whole point of the test.**
...
3. `tests/ci/_metaE2eWorkflowCoverage.test.ts` — a reasoned `LOCAL_ONLY_ALLOWLIST` row. The coverage scanner treats a path-filtered workflow as not PR-blocking-capable, so a spec invoked only from one must carry a row or the required Vitest suite fails outright. `PATH_GATED` is the reason constant its two siblings in the same job already use.
