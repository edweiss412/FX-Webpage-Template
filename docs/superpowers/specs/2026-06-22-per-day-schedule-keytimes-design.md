...
- **Forced re-sync is a RELEASE GATE with an ENFORCEABLE ARTIFACT (R4 finding 8 + R7 finding 12), not fire-and-forget:** parser changes affect only future parses, and the recovered DATES-column shows have no pre-existing stored per-day data (§3.2), so they stay degraded until re-synced. The deployment:
  ...
  2. Runs a **concrete verification artifact** — `scripts/verify-resync-scheduletimes.ts` (a `pnpm` script; reads each affected show's `shows_internal.run_of_show` via the validation/prod Supabase using the established `supabase db query --linked`/`TEST_DATABASE_URL` mechanism) that asserts, per show, the value decodes as `Record<iso, ScheduleDay>` with at least one of `showStart`/`window`/`entries` populated for that show's `showDays`. It emits an explicit per-show **PASS/FAIL table** and exits non-zero if any affected show is still legacy/null/empty.
