...
- Single titled token (`"4:15pm - Meeting Concludes"`) → `entries:[{start:"4:15pm", title:"Meeting Concludes"}]`, `showStart:"4:15pm"`, `window:null`.
...
`resolveKeyTimes(show, rooms)` → `resolveKeyTimes(show, rooms, runOfShow, visibleShowDays)` where `runOfShow` is the **already-date-gated** projection from `getShowForViewer` (§5.6) and **`visibleShowDays: string[]`** is the explicit set of ISO show days the viewer may see (`show.dates.showDays ∩ DateRestriction`, computed by each caller — `ScheduleSection` already derives this as `visibleDays`, `ScheduleSection.tsx:121-128`; `TodaySection`/`buildRightNowContext` derive it from their `dateRestriction`). **`resolveKeyTimes` iterates `visibleShowDays`, NEVER the raw `show.dates.showDays`** — this is the privacy fix for R3 finding 5 (see §5.6).
...
2. **The room `show_time` fallback** (§5.1) is gated by `resolveKeyTimes` iterating **`visibleShowDays` (`showDays ∩ DateRestriction`), never the raw calendar** — so a gated-out day can NOT re-leak via the room fallback (R3 finding 5). Gate 1 alone is insufficient *because* the fallback reads room data, not `runOfShow`; gate 2 closes that path. This is the §8 Day-1-only-viewer regression test.

`unknown_asterisk` leaks zero days (Schedule early return + Today Mode A; `visibleShowDays` is empty). Every free-text field routes through `shouldHideGenericOptional`/`resolveOptionalField` (`lib/crew/agendaDisplay.ts:26-31,43-45`).
...
- **Privacy — date-restricted viewer (R3 finding 5):** an **explicit Day-1-only** viewer on a **multi-day** show **with a GS room `show_time` present** → `resolveKeyTimes`/`ScheduleSection`/`TodaySection`/`buildRightNowContext` emit **NO Day-2 anchor** (and no Day-2 label/date) via the room fallback. Assert `shows` contains only Day 1; stash the `visibleShowDays` gate → Day 2 must reappear, proving the test pins the leak fix. Mirrors the existing date-restriction test shape (`ScheduleSection.tsx:121-128`).
...
| Persistence | `lib/sync/applyParseResult.ts` (persist predicate + `AGENDA_DAY_EMPTIED` reconcile — NO source merge), `lib/sync/enrichWithDrivePins.ts` (`ScheduleDay` passthrough typing), `lib/sync/runScheduledCronSync.ts` (passthrough, no SQL change) |
