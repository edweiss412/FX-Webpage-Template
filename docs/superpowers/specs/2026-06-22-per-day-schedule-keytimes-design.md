...
| D6 | Today scope | **Today-focused — applies to the SHOW anchors only.** Today's Key Times shows only TODAY's Show anchor (never the other show days' Show anchors); the full per-day Show breakdown lives in Schedule. **Set/Strike remain show-wide milestones on Today** (always rendered for assigned crew regardless of date — pre-existing ratified Phase-1 Today behavior, §13.8), suppressed only for `unknown_asterisk`. This resolves the R6 contradiction: "today-focused" never meant date-gating Set/Strike. |
...
A pure function invoked from `lib/parser/index.ts` after `parseDates` + `parseAgenda`, returning **`{ scheduleDays: Record<isoDate, ScheduleDay>, warnings: ParseWarning[] }`** (mirrors `ParseAgendaResult`, `lib/parser/blocks/agenda.ts:11-14`). `scheduleDays` is keyed by the same normalized ISO dates `parseDates` produces for `showDays`.

**Warning propagation contract (closes adversarial R8 finding 13).** `SCHEDULE_TIME_UNPARSED` (§9) is a load-bearing operator signal, NOT a local-only unit-test flag. `parseScheduleTimes` returns it in `warnings`, and `index.ts` merges them into the `ParseAggregator` **exactly as it does for `parseAgenda`** — `agg.warnings.push(...scheduleTimesResult.warnings)` alongside the existing `agg.warnings.push(...agendaResult.warnings)` (`lib/parser/index.ts:369`). This routes the warning to `ParsedSheet.warnings` → sync log → the §12.4 catalog surface. A `parseSheet`-level test (§8) proves a `GS: ... - 6:00 PM` end-only cell yields a `SCHEDULE_TIME_UNPARSED` that survives into `ParsedSheet.warnings`, not just the `scheduleTimes` unit test.

**Tokenizer (clock-boundary, not line-based):**
1. Find all clock tokens in the cell. Must tolerate observed variants: `7:15am`, `8:00 AM`, `4pm` (no colon), `5;30pm` (semicolon typo), `12:50pm`, leading/trailing space, and `GS:`/label prefixes. Normalize casing to uppercase AM/PM, collapse internal whitespace (mirrors `extractClockTime`'s normalization at `dates.ts:268-271`). **This permissive (no-colon/semicolon) tokenizer is SHOW-DAY-ONLY** — the SET-row `extractClockTimes` stays colon-required (§4.2, R12 finding 19); the two extractors are deliberately separate.
...
5. **`showStart` = the first clock ONLY when it is a LEADING START** — i.e. nothing precedes it in the cell except an optional short label token ending in `:` (e.g. `"GS:"`) and whitespace. A clock that is **preceded by an ellipsis/placeholder (`...`, `TBD`/`TBA`/`N/A`) or a leading separator with no start before it** is an END / unknown-start marker and is **NOT** promoted to `showStart`. In that case `showStart = null`, `window = null`, and the parser emits **`SCHEDULE_TIME_UNPARSED`** (§9); `resolveKeyTimes` then falls back to the room `show_time` for that day's anchor (or omits it). **No clock at all** → `showStart = null`, `window = null`, `entries = []`. **General `SCHEDULE_TIME_UNPARSED` rule (closes adversarial R9 finding 15):** the warning fires whenever a SHOW DAY TIME cell is **non-empty AND non-sentinel** yet yields **zero usable fields** (no `showStart`, no `window`, no `entries`) — this covers the ellipsis/end-only path AND the no-clock path (e.g. `"General Session TBD"`, `"see agenda"` produce no `ScheduleDay`, get dropped by the §7 confirmed predicate, and MUST still warn so operators + the re-sync verifier see them). A **purely empty or bare-sentinel** (`TBD`/`N/A`/`TBA`) cell emits nothing (intentional absence). This matches the §9 catalog contract exactly — no daylight between §4.1 and §9.
...
`extractClockTime` (`dates.ts:263-272`) → add `extractClockTimes(raw): string[]` returning **all** clock tokens in document order. **CRITICAL — the SET extractor stays COLON-REQUIRED (closes adversarial R12 finding 19):** `extractClockTimes` MUST keep `extractClockTime`'s exact `\d{1,2}:\d{2}(?:\s*[AaPp][Mm])?` regex (colon mandatory), only changing "first match" → "all matches." The no-colon (`4pm`) / semicolon (`5;30pm`) tolerance is **EXCLUSIVE to the SHOW DAY `parseScheduleTimes` tokenizer (§4.1)** and MUST NOT bleed into the SET row. This preserves the ratified existing behavior that coarse SET qualifiers like `"AFTER 8PM"` and `"LOAD IN"` (no colon) resolve to **null** — they are approximate, not precise call times, and §5.1 would otherwise compose+render `"Set 3/23 @ 8PM"`, silently converting vague source text into an exact crew-facing key time. `parseDates` set/travel_set cases (`dates.ts:207-221`) keep `loadIn = times[0]` (unchanged precedence: explicit `set` overrides `travel_set`) and add `dates.setupTime = times[1] ?? null` (free-text, e.g. `"10:00PM"` / `"8:30 PM"`). `dates.loadIn` semantics + precedence + rooms-independence are **unchanged** (`resolveKeyTimes.ts:54-59`; ratified Phase-1 change-1).

`ShowRow.dates` gains `setupTime?: string | null` (rides existing `shows.dates` jsonb, no migration; `dates` column has no CHECK, `supabase/migrations/20260501000000_initial_public_schema.sql:12`). **Display consumer:** the Set-day `DayCard` meta (`"Setup 10:00PM"`, §5.3) — so `setupTime` is not a zombie field (R9 finding 17).
...
- `parseAgenda` continues to produce its grid `Record<iso, AgendaEntry[]>`; `parseScheduleTimes` produces `{ scheduleDays: Record<iso, ScheduleDay>, warnings }` from DATES col4 (warnings merged into `agg` per §4.1).
...
- **`shows`** (D4/D8): for each day in **`visibleShowDays`** (NOT the raw calendar), push `{date, label, time}` where the per-day **anchor resolution precedence** is `time = runOfShow[date].showStart ?? runOfShow[date].window?.start ?? runOfShow[date].entries[0]?.start`. The `entries[0].start` fallback is load-bearing: it gives **legacy-wrapped** rows (entries-only, `showStart:null` per §3.2) and any titled day a correct per-day anchor (closes R2 finding 3). The room `show_time` fallback is **DATE-SAFE, not blanket (closes adversarial R10 finding 18).** The GS room `show_time` is a SINGLE show-wide value with its own date (e.g. `"5/13 @ 8:00 AM"`); applying it to a different show day mislabels that day. So a visible day absent from `runOfShow` falls back to the room anchor **ONLY when EITHER (a) the show has exactly ONE TOTAL show day** — `show.dates.showDays.length === 1`, the **RAW** count, NOT the post-`DateRestriction` visible count (closes adversarial R11 finding: a Day-2-only restricted viewer on a raw-multi-day show has exactly one *visible* day, so a visible-count rule would still cross-label) — **OR (b) the M/D date parsed from the room `show_time` string matches that day's ISO.** Otherwise that day's Show anchor is **OMITTED** — never a cross-day room value. (For a raw-multi-day show, rule (a) never applies regardless of restriction, so only a date-matched room value is ever used.) This is critical for **contentful-unparsed** days: Redefining-FI Day 2 (`"GS: ... - 6:00 PM"`) is intentionally absent from `runOfShow` (showStart=null + `SCHEDULE_TIME_UNPARSED`), so without this rule it would inherit the room's `5/13 @ 8:00 AM` (Day 1) value on the 5/14 row — a high-cost misdisplay. With the rule, 5/14 gets no anchor (the room date 5/13 ≠ 5/14, and there are 2 visible show days). Because the iteration domain is `visibleShowDays`, a gated-out day also never reaches the fallback (R3 finding 5). Sort ASC by date. Empty → omit `shows`.
**Per-day Show anchor — canonical decision table (single source; structural defense after the R10/R11 room-fallback vector).** For each visible show day `D` (after `unknown_asterisk → {}` short-circuits the whole strip), the anchor is the FIRST matching row:

| # | Condition | Anchor |
|---|---|---|
| 1 | `D ∈ runOfShow` and `showStart != null` | `showStart` |
| 2 | `D ∈ runOfShow` and `window != null` | `window.start` |
| 3 | `D ∈ runOfShow` and `entries.length > 0` | `entries[0].start` |
| 4 | `D ∉ runOfShow` (or rows 1–3 all null) AND `show.dates.showDays.length === 1` (RAW) | room `show_time` |
| 5 | `D ∉ runOfShow` (or rows 1–3 all null) AND M/D parsed from room `show_time` `=== D` | room `show_time` |
| 6 | otherwise | **OMIT** (no anchor for `D`) |

Rows 4/5 are the only room-fallback paths and are date-safe; row 4 uses the RAW total show-day count, never the post-restriction visible count. This table is the authoritative contract for the §8 fallback tests.
...
- **Set day → meta = `"Setup <dates.setupTime>"` when `dates.setupTime` is present** (the second/setup clock captured per D7/§4.2 — e.g. `"Setup 10:00PM"`); else no meta. This is `setupTime`'s display consumer (closes R9 finding 17 — load-in itself remains the Set Key-Time anchor; the setup time surfaces here). Sentinel-guarded via `shouldHideGenericOptional`.
- Other travel days → no meta.
...
- **Today** (D6, R6 contradiction resolved): filters `anchors.shows` to today's ISO date (`todayIsoInShowTimezone`) — Today shows ONLY today's Show anchor, never other show days'. **`set`/`strike` pass through SHOW-WIDE** (always rendered for assigned crew regardless of whether today is the set/strike day — §13.8, the pre-existing ratified Today behavior); there is **NO** "today-relevance" date predicate on Set/Strike. So: on a **show day** → Set + today's Show + Strike; on a **non-show day** (incl. set day / strike day / travel day) → Set + Strike, no Show anchor. `unknown_asterisk` → `resolveKeyTimes` returns `{}` → no strip (zero leak).
...
**All affected surfaces EXCEPT `RightNowHero` are synchronous Server Components** — `KeyTimesStrip`, `DayCard`, `ScheduleSection`, `TodaySection` carry no `'use client'`, no `framer-motion`, no `AnimatePresence`. The strip is fixed per server render; differences across viewers/days/show-counts are distinct renders, not in-page animated transitions. The only page-level motion is the route transition (M12.11), unchanged.

**`RightNowHero` IS a client component (R9 finding 16) with a live clock + state recovery** — it now selects the dated Show anchor from `showAnchors` by its client-computed show-tz `todayIso` (`RightNowHero.tsx:215`). Its client-state transitions MUST be inventoried + tested:
...
- **Forced re-sync is a RELEASE GATE with an ENFORCEABLE ARTIFACT (R4 finding 8 + R7 finding 12), not fire-and-forget:** parser changes affect only future parses, and the recovered DATES-column shows have no pre-existing stored per-day data (§3.2), so they stay degraded until re-synced. The deployment:
  1. Re-syncs the 7 live shows (+ VB/DRILL copies) via the existing retry/`requires_resync` path.
  2. Runs a **concrete verification artifact** — `scripts/verify-resync-scheduletimes.ts` (a `pnpm` script; reads each affected show's `shows_internal.run_of_show` via the validation/prod Supabase using the established `supabase db query --linked`/`TEST_DATABASE_URL` mechanism). **Per-show, per-ISO coverage (closes adversarial R8 finding 14 — NOT "≥1 day"):** the script builds an **expected per-show, per-show-day map** derived from the live/fixture DATES TIME cells (each recoverable show day → the field it should populate: `entries` for titled, `window` for bare-window, `showStart` for fragments). It **FAILS if ANY recoverable show day** lacks a decoded `ScheduleDay` with the expected populated field (so a 2-/3-day show with Day 1 recovered but Day 2 still legacy/null FAILS — not PASS). Deliberate end-only exceptions (e.g. Redefining-FI SHOW DAY 2 `GS: ... - 6:00 PM`) are encoded as **expected `SCHEDULE_TIME_UNPARSED`** entries, asserted separately, so they don't mask a real miss. It emits an explicit per-show/per-day **PASS/FAIL table** and exits non-zero on any miss.
  3. **Rollout is "complete" ONLY when the script is green.** Any FAIL show is recorded with a degraded flag (deploy checklist / `DEFERRED.md`) and is NOT silently considered fixed.
  `decodeRunOfShow` legacy-array tolerance (§3.2) prevents a hard break during the window but does not itself recover the new data — the artifact is what proves recovery.
...
- **Parser:** new `tests/parser/blocks/scheduleTimes.test.ts` — tokenizer, window-vs-list detection, every observed variant (`4pm`, `5;30pm`, `GS:` prefix, AM/PM casing), both flavors, empty/sentinel, single-token, `showStart` derivation. **Leading-start vs end-only (R1 finding 2):** `"GS: 8:00 AM -"` → `showStart:"8:00 AM"`; `"GS: ... - 6:00 PM"` (Redefining-FI SHOW DAY 2) → `showStart:null` + `SCHEDULE_TIME_UNPARSED`, asserting the trailing `6:00 PM` is NOT promoted to a start. **Terminal-event guard (R5 finding 9):** `"4:15pm - Meeting Concludes"` (Fixed Income Trading SHOW DAY 2 fixture) → entry kept, `showStart:null`, NO `SCHEDULE_TIME_UNPARSED`; and `"8:45am - General Session"` → `showStart:"8:45am"` (non-terminal single token still promotes). Negative-regression: stash the terminal guard → the meeting-concludes time must wrongly become `showStart`, proving the test pins it. **No-clock contentful cell (R9 finding 15):** `"General Session TBD"` / `"see agenda"` (non-empty, non-sentinel, zero clocks) → no `ScheduleDay`, **`SCHEDULE_TIME_UNPARSED` emitted**; a bare `"TBD"` cell → no warning (intentional absence). Assert at BOTH `scheduleTimes` unit + `parseSheet` (warning reaches `ParsedSheet.warnings`). Revise `tests/parser/blocks/dates.test.ts` loadIn suite (the suite that currently asserts SHOW DAY TIME fills nothing — must flip to assert capture + `setupTime`). **PRESERVE the colon-required SET semantics (R12 finding 19):** keep/assert the existing `"AFTER 8PM"` → null and `"LOAD IN"` → null SET-row cases under `extractClockTimes`; assert `extractClockTimes("9:00PM - LOAD IN 10:00PM - SETUP")` = `["9:00PM","10:00PM"]` (colon) but `extractClockTimes("AFTER 8PM")` = `[]` (no colon → no false load-in). Negative-regression: stash the colon constraint → `"AFTER 8PM"` wrongly yields `"8PM"` as `loadIn`. `tests/parser/parseAgenda.test.ts` — encode the D2 merge precedence (it currently pins parseAgenda as the sole runOfShow source and uses consultants as an all-`[]` fixture).
...
- **Warning propagation (R8 finding 13):** `tests/parser/parseSheet*.test.ts` — a fixture with a `GS: ... - 6:00 PM` end-only SHOW DAY cell yields a `SCHEDULE_TIME_UNPARSED` that reaches `ParsedSheet.warnings` (asserting the `parseScheduleTimes` `{warnings}` → `agg.warnings` merge in `index.ts`, NOT just the `scheduleTimes` unit test). Confirms the warning is on the sync-log/§12.4 path, not local-only.
...
- **Today day-matrix (R6 finding — pins the resolved contract):** for a multi-day show, render `TodaySection` with "today" = (a) set day, (b) show day 1, (c) show day 2, (d) strike day, (e) a travel/non-show-non-set-non-strike day. Assert: **Set + Strike render in ALL of a–e** (show-wide); the **Show anchor renders ONLY on show days and shows THAT day's** time (b→Day1, c→Day2), and is **absent** on a/d/e. Derive expected times from the fixture, not hardcoded. This makes the "today-focused = Show-anchor-only" contract un-ambiguous to the implementer.
- **Date-safe room fallback (R10 finding 18):** `resolveKeyTimes` test on a multi-day show where a visible show day is absent from `runOfShow` (contentful-unparsed, e.g. Redefining-FI Day 2 `5/14`) and the GS room `show_time` is `"5/13 @ 8:00 AM"` → assert **NO** Show anchor renders for `5/14` (the room's `5/13` value must NOT cross-label Day 2). Also: a **raw single-show-day** show with only a room `show_time` DOES get that anchor (rule (a)); and a multi-day day whose ISO matches the room date DOES (rule (b)). **Restricted-viewer variant (R11 finding):** a **Day-2-only** restricted viewer on Redefining-FI (raw 2 show days, so exactly one *visible* day) where Day 2 is absent from `runOfShow` and room `show_time` is dated `5/13` → assert NO anchor renders for `5/14` (rule (a) must key on the RAW count, so it does NOT fire; rule (b) date 5/13≠5/14 → omit). Negative-regression: stash the date-safe guard → the `5/13 @ 8:00 AM` anchor must wrongly appear on `5/14`.
- **`setupTime` display (R9 finding 17):** `ScheduleSection`/`DayCard` test — a show with `dates.setupTime` renders the Set-day `DayCard` meta `"Setup <time>"`; absent/sentinel `setupTime` → no meta. Proves the field reaches a user-visible surface.
- **`RightNowHero` client-state (R9 finding 16):** client-component test — with `showAnchors` carrying distinct Day-1/Day-2 anchors, simulate show-tz **midnight rollover** (Day 1 → Day 2) and a **recovery/last-good** cycle; assert the displayed call time re-selects to the current `todayIso`'s anchor and never freezes on the prior day's. Derive both anchors from the fixture.
...
No new boolean config flags or toggles introduced. Both new data fields have a COMPLETE storage → read → display lifecycle (no zombie fields, R9 finding 17):
- **`dates.setupTime`:** parser `extractClockTimes` (§4.2) → `shows.dates` jsonb write → `getShowForViewer` decode → **Set-day `DayCard` meta** (`"Setup 10:00PM"`, §5.3).
- **per-day `ScheduleDay` (`entries`/`showStart`/`window`):** parser merge (§4.3) → `shows_internal.run_of_show` jsonb → `getShowForViewer` date-gated read → `RunOfShowList` (entries) + `KeyTimeAnchors.shows` (showStart/window) + `DayCard` meta (window) (§5).
...
| UI | `components/crew/primitives/KeyTimesStrip.tsx`, `components/crew/primitives/DayCard.tsx` (meta wired), `components/crew/sections/ScheduleSection.tsx`, `components/crew/sections/TodaySection.tsx`, `components/right-now/buildRightNowContext.ts` (+ `showAnchors`), `components/crew/RightNowHero.tsx` (Day-2 anchor selection), `app/show/[slug]/[shareToken]/_CrewShell.tsx` (2nd `buildRightNowContext` caller — pass `runOfShow`), `app/admin/dev/source-link-dim/page.tsx` |
| Deploy/ops | `scripts/verify-resync-scheduletimes.ts` (re-sync release-gate artifact, §7); downgrade converter `ScheduleDay → AgendaEntry[]` (§14) |
| Tests | per §8 |
