...
- **Invariant 10 (mutation-surface observability):** every gated action's rejection branch emits a code-carrying `log.warn` (`SERVER_ACTION_ORIGIN_REJECTED` or `PICKER_ORIGIN_REJECTED`) — previously-dark refusal branches become instrumented. Existing per-surface instrumentation (success emits, `no-telemetry:` exemptions, `AUDITABLE_MUTATIONS` rows) is untouched: the gate adds a branch, not a surface, and `tests/log/_metaMutationSurfaceObservability.test.ts` keys on surfaces. **No registry row, exemption, or ledger entry changes** — probed, not reasoned: the refusal emit lives inside an imported helper, and `predicateFor` scans only the action's own body with `descend: false` for a non-route unit (`tests/log/_metaMutationSurfaceObservability.test.ts:61-64`) while `scanBody` recognizes direct `log.*` calls and not imported callees, so `codedLog` stays false for every exempt unit the gate touches and each keeps passing through its existing `no-telemetry:` exemption exactly as before. The walk also carries no unused-exemption rule. Emits fire outside any advisory-lock region; no secret is logged (the emit carries only the action name and source).
...
- **The three exempt bodies are FROZEN, not scanned** (added at diff review round 3). The round-3 finding was that an ordinary `cookies().set(...)` inserted into any exempt body leaves the unit accepted — both registries still say `read-only`, the `scanBody` tripwire stays clear, and reconciliation passes. Confirmed on all three rows. The repair is NOT another mutation verb: teaching the tripwire `cookies().set` invites `signOut` next, then a filesystem write, then a delegation, which is the non-terminating question §3.5 stopped asking. Instead the walk pins a whitespace-normalized digest of each exempt body. It decides nothing about mutation — it asks only whether the body is still the one whose read-only claim was ratified — so it cannot be defeated by a verb nobody modelled and needs no widening when someone invents a new way to write state. §7 already said the process for an exempt unit that grows a mutation is "review of that change"; the pin makes that executable rather than hopeful. A legitimate edit reds the walk until the digest is updated, and updating it is the moment the claim gets re-reviewed. Round 4 then found the frozen span was the wrong one: exempt units were excluded from the walk before `parametersAreInert` ever ran, and the digest covered only the body, so a call-valued DEFAULT PARAMETER executed ahead of the body while leaving the digest untouched — the same execution position the accept-sets pin for gated units. Both halves are repaired: the parameter precondition now applies to exempt units exactly as to gated ones (an exemption says the BODY only reads; it never said the signature may execute arbitrary code ahead of it), and the digest covers the parameter list as well as the body, so a signature edit re-triggers the read-only review. Pinned by eight fixtures — including one whose mutation is a verb no scanner models, one proving reformatting alone does not fire, one proving a missing pin fails rather than passing silently, and the round-4 pair — plus live-tree proof of every attack on all three real exempt units.
...
- **Discovery is not total, and the walk fails CLOSED on what it cannot classify** (added at diff review round 1). `collectSurfaceUnits` models specific export and action forms. The reviewer defeated the walk by probing forms it does not model, and the probe was reproduced independently against the live engine: a paren-wrapped module export, an export aliased through an intermediate binding, an ANONYMOUS inline action passed straight to a JSX `action={...}` prop, and an inline object method each return **zero** units. Next registers all of them, so a destructive action in any of those forms was invisible to the walk while `gated + exempted === discovered` still balanced — the unit was never counted on either side. The repair is NARROWING rather than grammar growth: teaching the shared invariant-10 engine eight more forms would grow a recognizer this arc does not own, and each widening is a bigger target for the next round. Instead the walk carries two deliberately dumb, TOTAL counters — every function-like body opening with the directive, and every exported VALUE name of a `"use server"` module — and any surplus over what discovery produced fails by name. The consequence bound is satisfied in the only form that closes: a Server Action is gated, or exempt, or **signalled**, never silently ungated. The residual is that discovery itself stays incomplete, which is the invariant-10 engine's contract to fix rather than this arc's; filed as `BL-SURFACE-DISCOVERY-UNNAMEABLE-ACTION-FORMS`. Round 2 then defeated the FIRST version of the tripwire three more ways, all of them totality bugs in counters that claimed to be total: an exported binding pattern (`export const { doIt } = bag`, and its array form) that an identifier-only reader called nothing; a directive PROLOGUE (`"use strict"; "use server";`) that a statement-zero check called nothing, though Next reads the whole prologue; and an inline action nested inside a file-level `"use server"` module, which the module branch skipped by `continue`ing before the inline counter ran. The repair made both counters total BY CONSTRUCTION rather than by enumeration — `ts.isFunctionLike` instead of a four-way kind check, the whole leading run of string-literal statements instead of index 0, a recursive walk of `BindingName` through both binding-pattern kinds, and both checks running for every file — so the next form TypeScript grows is covered without this file naming it. **Nine** escape families are pinned as fixture self-tests, each behind a premise asserting the engine still fails to discover it, so a future engine improvement reds the premise rather than silently voiding the fixture; four negative fixtures pin that the tripwire stays quiet on ordinary code. Each family was additionally proven against a REAL live-tree mutant, not only a fixture.
