...
**Class C** (`confirmUnpublishAction`) opens with the same `if` idiom, returning a call to a module-local, non-exported `rejectCrossOriginUnpublish` — the shipped `rejectCrossOrigin` shape (`lib/auth/picker/clearIdentity.ts:57`), which a `"use server"` module may hold because the restriction is on what it EXPORTS. That helper emits the `SERVER_ACTION_ORIGIN_REJECTED` `log.warn` and returns the action's own established refusal value `{ status: "neutral" }` — the value it already returns for a missing/blank field (`app/show/[slug]/unpublish/actions.ts:43`). A helper call rather than an inline object literal is not decoration: accept-set B admits exactly one expression shape in the return position (§3.6), because an unconstrained one executes on the refusal path. Neutral is the correct rendering: the confirm page's neutral state is deliberately non-committal (spec'd so CONSUMED never leaks), and no token is consumed because `prevalidateUnpublishBinding` is never reached. Note this action's authority is the emailed capability token, not a cookie, so the CSRF value of forging it is nil — it is gated anyway because it is destructive and the gate costs one line (uniform posture beats a per-action threat argument).

### 3.5 Classes E and F — the exemption sets

**The `read-only` set is DERIVED from the sibling guard, not re-declared.** The same three units are already carried as `kind: "read-only"` rows in the invariant-10 registry (`ADMIN_SURFACE_EXEMPTIONS` at `tests/log/mutationSurface/exemptions.ts:62`, whose `getStagedResult` row is `tests/log/mutationSurface/exemptions.ts:75`, `captureShowTelemetry` `tests/log/mutationSurface/exemptions.ts:79`, and `listFixtures` `tests/log/mutationSurface/exemptions.ts:80`). A second hand-maintained list of the same three units is a drift surface with no upside: the two walks would then be able to disagree about which units are read-only. So the origin walk reads `ADMIN_SURFACE_EXEMPTIONS`, filters to `kind === "read-only"` rows that resolve to a discovered non-route unit **whose `unit.admin` is true**, and that is its read-only set.

**The `admin` filter is load-bearing and the derivation delegates NO verification.** The sibling guard consults that registry only inside its `if (unit.admin)` branch (`tests/log/_metaMutationSurfaceObservability.test.ts:160-169`); a non-admin unit never reaches it and instead takes the coded-log / `no-telemetry` / ledger path (`tests/log/_metaMutationSurfaceObservability.test.ts:177-182`). So a `read-only` row added for a non-admin unit is verified by nobody there — and every destructive class-B and class-D unit passes the strict predicate and already carries a `no-telemetry:` comment, so such a row would sail through both walks while the unit mutates a cookie (probed: `cleanupStaleEntry`, `cleanupStaleEntryCore`, `selectIdentity`, `selectIdentityCore`, and the three `clearIdentity` units all report `strictReadOnlyPredicate=true`). The origin walk therefore (a) admits only `admin === true` units into the read-only set, and (b) **re-runs the strict predicate itself on every derived row** rather than trusting that the other guard ran it. The derivation settles which units are CLAIMED read-only, so the two lists cannot drift; it never outsources whether the claim is true.
...
- the `thenStatement` is a `ReturnStatement`, or a `Block` containing exactly one `ReturnStatement`, and **its returned expression is a `CallExpression` whose arguments are all `StringLiteral`s and whose callee is an `Identifier` that resolves to a verified refusal helper** (below).
...
**Verified refusal helper.** The callee identifier must resolve to one of exactly two shapes, and in both cases its body must be non-mutating under the same strict predicate §3.5 applies to exemptions:

1. a named import, under its exact export name and not locally rebound, from `@/lib/auth/picker/rejectCrossOriginPicker` — the shared class-B helper; or
2. a module-local, **non-exported** `FunctionDeclaration` in the same source file. This is the shipped pattern: `rejectCrossOrigin` at `lib/auth/picker/clearIdentity.ts:57` is exactly this, and it is why class D passes unchanged. It is also how class C refuses — a `"use server"` module may only *export* async functions, but a module-local helper is unconstrained, so `confirmUnpublishAction` gets a local `rejectCrossOriginUnpublish` returning `{ status: "neutral" }` rather than an inline object literal.

Requiring the refusal to be a call to a checked helper, rather than an arbitrary expression, is what closes the return position: `return await cleanupStaleEntryCoreImpl(input)` — whose callee `cleanupStaleEntryCoreImpl` is already reached from `cleanupStaleEntryCore` at `lib/auth/picker/cleanupStaleEntry.ts:58` and writes the cookie at `lib/auth/picker/cleanupStaleEntry.ts:89` — satisfies "an `if` that returns" but is a mutation on the refusal path.
...
**Registry hygiene** (the reconcile-your-own-counts lesson): every delegator row matches exactly one discovered unit, every derived read-only row resolves to one discovered unit, no orphans, no duplicates, and the shipped form of the test asserts `gated + exempted === discovered` so no unit can go unaccounted. During implementation the walk carries the `PENDING_GATE` term described in §5; the task that empties it also deletes it, so the reconciliation above is the form that merges.
...
| accept-set B, well-formed, imported helper | passes |
| accept-set B, well-formed, module-local non-exported helper | passes |
...
| accept-set B, returns an arbitrary call rather than a refusal helper | FAILS |
| accept-set B, refusal-helper argument is a call rather than a literal | FAILS |
| accept-set B, local refusal helper whose body mutates | FAILS |
...
3. **Class C** (`confirmUnpublishAction`): cross-site → returns `{ status: "neutral" }` via the module-local `rejectCrossOriginUnpublish`, `prevalidateUnpublishBinding` spy NOT called (no token touched), emit fired. Baseline: same-origin case reaches the pre-check.
...
| `app/show/[slug]/unpublish/actions.ts` | class-C gate line plus a module-local, non-exported `rejectCrossOriginUnpublish` refusal helper (§3.4) |
