...
4. **Op semantics:**
   - `upsert`: validate `override_value` (§7.4) incl. the name-collision guard; **capture the live/parsed value being overridden and persist it into `admin_overrides.sheet_value` in the SAME locked tx, BEFORE overwriting the live row** (see the durability note below); upsert `admin_overrides` (`active=true`, `sheet_value` set); then immediately apply `override_value` to the live row via the RPC immediate-apply helper (§7.3). No auth-table write (§5.2).
   - `revert`: restore `sheet_value` to the live row; delete the override row. No auth-table write.

   **sheet_value durability (R6):** `sheet_value` MUST be populated at create/upsert time from the value being overridden — NOT left to the next applied sync. Otherwise a **create-then-revert-before-any-sync** would restore `NULL` (or a stale value), destroying the very value Doug just overrode (a one-click-revert data-loss). Because the RPC captures the pre-override live value under the lock and writes it to `sheet_value` before the live-row UPDATE, revert always has the correct value immediately. (The sync path Stage B also refreshes `sheet_value` each applied sync — that keeps it current as the sheet changes; the RPC capture is what makes the *fresh, pre-sync* override revert-safe.) Applies to all domains (show/crew/hotel). Tested: create-then-revert-without-sync restores the original value for each of the 6 fields.
