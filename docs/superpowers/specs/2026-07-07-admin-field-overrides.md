...
3. **Two-part row-state CAS (both under the lock; either mismatch → `errcode 40001` → 409 stale_review):**
   ...
   **Hotels additionally** verify the `p_current_ordinal` row's current `hotel_name = p_expected_live_hotel_name` (§5.3) — the row-locator CAS (the row didn't move) — complementary to CAS-B (the value didn't change). Every successful mutation ends by `version = version + 1`.
4. **Op semantics:**
