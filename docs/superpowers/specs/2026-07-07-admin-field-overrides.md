...
- **hotel** (`target_missing` OR `name_conflict`): `target_missing` = the (name+disambiguator) `match_key` absent from the parsed hotel set; `name_conflict` = a `hotel_name` override whose output collides with another parsed reservation's name (R26, §5.3 runtime collision). No hold interaction; both planned in Stage A, **committed in Stage B** (applied path only).
- **show**: never stale.
