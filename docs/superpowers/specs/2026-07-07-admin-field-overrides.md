...
1.5. **Sheet-adopts-override convergence (R22 — MUST run before genuine-removal).** The primary correction workflow is: Doug overrides `Jon → John`, and a later sheet edit / parser fix makes the RAW parse itself emit `John`. Now the override's `parsedIdentity` (`Jon`) is **absent** from the next parse while the override's **output** (`John`) is **present** as a next parsed name — the sheet has *converged to* the correction. Without special handling, step 2 would treat `Jon` as a genuine removal (DELETE id X) and `John` as a new member (INSERT new id), **churning the picker-cookie id on exactly the workflow this feature exists to serve.** So, for each prev row with an active name override whose `parsedIdentity` is absent from the next parsed names but whose `displayName` **uniquely** equals a next parsed name **not claimed by any other prev parsedIdentity**: treat the override as **adopted** — **re-key that prev row's identity from `parsedIdentity` to `displayName`** (so it matches the next `John` row **by its existing id X**), and **RETIRE the override**: **DELETE the `admin_overrides` row** and set `crew_members.sheet_name = NULL` in the same locked tx. This is a **retire-on-convergence, NOT a stale deactivation** — the override_value already equals the newly-parsed value, so nothing is lost (distinct from watchpoint 3's anti-data-loss deactivation, which protects a correction whose target *vanished*; here the target was *achieved*). **No needs-attention signal** (convergence is a success, not a problem); it is recorded as a display-name change in the change-log, never as an inactive override. If `displayName` matches **>1** next parsed name, or the parsed `John` is **also** independently present alongside `Jon` (both in the next parse), this is NOT convergence — fall through to collision (step 1, a genuine second `John`) / normal rename.
2. **Match & write by id:**
   - `parsedName` in BOTH prev and next → `UPDATE crew_members SET name=displayName, role=… WHERE id = prevId` (id-keyed rename-in-place; covers override apply, override edit, override release, collision-release, and **convergence-adoption** uniformly — **id preserved**).
   - `parsedName` in next only (new member) → `INSERT` (new id).
   - `parsedName` in prev only (absent from next): if the member is **held/protected** (its raw `parsedName` ∈ the hold plan's protected set) → **retain** the row unchanged (no write, id preserved, override stays active — the R10 removal-hold case); else **genuine removal** → `DELETE WHERE id = prevId`, and its override deactivates with `deactivation_code='target_missing'` (§6). (A convergence-adopted row is already re-keyed by step 1.5, so it is never seen here as prev-only.)
3. `added` / `removed` for the change-log derive from parsedIdentity set-difference (prev vs next parsed names) **after** the step-1.5 re-keying, so a pure display rename **and a sheet-adopts-override convergence** are each neither an add nor a remove.
...
Tests (§13): id-keyed rename preserves id across override apply/edit/release; collision deactivates the override AND leaves the pre-conflict `crew_members.id` bound to its original parsed identity (never reassigned to the newly-parsed colliding member); removal-hold retains id; genuine removal deletes + deactivates; **sheet-adopts-override convergence (`Jon→John` override, next parse emits `John`) preserves `crew_members.id`, RETIRES the override (row deleted, `sheet_name=NULL`), emits NO needs-attention signal, and logs neither an add nor a remove — across two real sync runs (R22)**.
...
**Convergence-retire is NOT a deactivation (R22).** The one crew case that **deletes** an override on the sync path is the sheet-adopts-override convergence (§3.6 step 1.5): the raw parse itself now emits the override's output, so the override_value **equals** the newly-parsed value and the override is genuinely redundant. That override row is **deleted** (not `active=false`) inside the locked tx, `sheet_name` cleared, the `crew_members.id` preserved — and it produces **no** needs-attention signal and **no** `deactivation_code` (it is neither `target_missing` nor `name_conflict`). This is safe against watchpoint 3's anti-data-loss rule precisely because the target was *achieved*, not *lost* (value equality means nothing to restore). All other stale/conflict paths remain deactivate-not-delete.
...
5a. **Crew write = id-keyed parsed-identity reconciliation (§3.6), NOT name-keyed delete/upsert.** This is the structural resolution of the whole crew-name identity vector (R7/R10/R11): identity is tracked by parsed name (`match_key`), every rename is an `UPDATE … WHERE id=X`, and stale is decided post-hold. It provably prevents id churn (delete+reinsert), collision id-swap, and premature deactivation. It also preserves the id on **sheet-adopts-override convergence** (§3.6 step 1.5, R22): when the raw parse itself adopts the override value, the row is re-keyed in place and the redundant override is retired (deleted) — the primary correction workflow keeps its picker cookie. Holds still decide **disposition** on RAW parse first (§3.4 ordering); §3.6 does the write. Do not "simplify" back to name-keyed folding, defer-on-hold, or pre-hold stale planning — each reintroduces a closed bug. The id-keyed path activates only when a crew override is active (non-override shows keep the existing name-keyed apply).
