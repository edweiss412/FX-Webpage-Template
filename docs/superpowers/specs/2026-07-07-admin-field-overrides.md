...
2. **Match & write by id:**
   ...
   - `parsedName` in prev only (absent from next): if the member is **held/protected** (its raw `parsedName` ∈ the hold plan's protected set) → **retain** the row unchanged (no write, id preserved, override stays active — the R10 removal-hold case); else **genuine removal** → `DELETE WHERE id = prevId`, and its override deactivates **stale** (§6).
