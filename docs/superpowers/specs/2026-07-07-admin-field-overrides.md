...
- **Runtime name-collision (R26 — analog of the crew §3.6 step-1 collision).** The write-time guard (§7.4) blocks *creating* a `hotel_name` override whose value collides, but a *later sync* can introduce the collision — the sheet adds a real reservation whose parsed `hotel_name` equals an active `hotel_name` override's **output** (on a different reservation). Applying the override would then produce **two live rows with the same `hotel_name`**, which the immediate resolver (exactly-one-live-match, §5.3/§7.2) could never edit/revert — a stranded override. So the Stage-A transform, before applying a `hotel_name` override, checks whether its `override_value` equals any *other* parsed reservation's `hotel_name` in this sync; if so it **does NOT apply the override** and instead **deactivates it `deactivation_code='name_conflict'`** (§6, committed Stage B) → needs-attention → Doug re-points or discards. (`hotel_address` overrides never collide — address is not the resolver key.)
...
| hotel_name | empty/whitespace-only; `= match_key` (no-op); **or collides** — equals any *other* current live hotel name OR any *other* current parsed hotel name in the show (R26: would create two live rows with the same `hotel_name`, which the immediate resolver — exactly-one-live-match, §5.3 — could then never edit/revert, stranding the override). Mirrors the crew name-collision guard. | 200 chars |
...
6b. **`hotel_name` override values can't create an un-editable duplicate (R26).** A `hotel_name` override whose value equals another live/parsed hotel name would make the live set non-unique and strand the override (the exactly-one-live-match resolver would 409 every subsequent edit/revert). Closed two ways, mirroring the crew name-collision handling: (1) **write-time** — §7.4 rejects a colliding `hotel_name` value at create/edit; (2) **sync-time** — a later sync that introduces the collision deactivates the override `name_conflict` (§5.3/§6) → needs-attention → Doug re-points/discards. Do not relitigate as "hotel_name is free text" — it is the resolver key, so it carries the same collision contract as crew name.
