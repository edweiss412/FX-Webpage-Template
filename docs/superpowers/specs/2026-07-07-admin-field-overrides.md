...
3. **Two-part row-state CAS (both under the lock; either mismatch → `errcode 40001` → 409 stale_review):**
   - **CAS-A version (R15) — guards the override row.** Read the target's current `admin_overrides.version`; if ≠ `p_expected_version`, 409. Catches **any** concurrent override mutation (edit/revert/repoint/discard/sync-refresh) because every mutation **bumps `version`** (`sheet_value` is *preserved* on edit and cannot detect two stale pages editing the same override — R15). On **create**, `p_expected_version` is NULL and the RPC asserts **no _active_ override row exists** for the target (else 409). An **inactive** same-key row is NOT a 409 — create **reactivates it in place** (§7.2 item 4, R28), so the retained `active=false` stale row never collides with the unique key.
