...
- Immediate-apply RPC path — **hotel live-row resolver (R12 finding 2), analogous to the crew §7.6 resolver.** Hotel rows have unstable ids, so the RPC anchors on `(show_id, ordinal)` with a CAS on the **current live hotel name**, which — exactly like crew — is NOT always `match_key`: once a `hotel_name` override is active, the live row at that ordinal is named the **override value**, not the parsed `match_key`. So `currentLiveHotelName(reservation) =` the active `hotel_name` override's output if one exists, else `match_key`. The RPC verifies the ordinal row's current `hotel_name == p_expected_live_hotel_name` (passed by the loader, = `currentLiveHotelName`), then `UPDATE public.hotel_reservations SET <field> = $override WHERE show_id=$1 AND ordinal=$current_ordinal`. This makes a **`hotel_address` edit after a `hotel_name` override** correct (it anchors on the override name the row actually has, not the parsed name — no false 409, no wrong-row apply). A `hotel_name` and `hotel_address` override on the same reservation share the same `match_key` (parsed name) but resolve the live row through the sibling `hotel_name` override — the exact crew role/name pattern.
  - **Duplicate-name groups — the ordinal + name CAS is NOT sufficient; the RPC ALSO verifies the content disambiguator, fail-closed (R19).** For a same-name duplicate group, `p_expected_live_hotel_name` (the shared duplicate name) is **not unique**, so `(ordinal, name)` alone can silently retarget a sibling: if a concurrent sync reorders the group or inserts a same-name reservation ahead between UI-load and save, `p_current_ordinal` now addresses a **different** reservation whose `hotel_name` still equals `p_expected_live_hotel_name` — the name CAS passes and (for a `hotel_name` override) CAS-B compares the same duplicate value, so the wrong live row is mutated with no 409. To close this, the RPC **recomputes the §5.3 content disambiguator** (`check_in` [+ `confirmation_no` | first `names[]` entry]) **from the row at `p_current_ordinal`** — these are **non-overridable** content columns, so the disambiguator is stable across any `hotel_name` override — and requires it to equal the disambiguator embedded in `p_match_key`. **Fail-closed uniqueness:** the RPC additionally requires that **exactly one** live reservation in the show matches `(currentLiveHotelName + disambiguator)`; zero or >1 → **409 stale_review**, never a guessed apply. A **unique** hotel name carries no disambiguator in `p_match_key`, so this reduces to the plain name CAS. This is the RPC-path mirror of the sync-path fail-closed rule above — both refuse to apply to a guessed row when the duplicate group's composition changed.
  - If the CAS name mismatches (a sync reordered/renamed the row) OR the disambiguator/uniqueness check fails, the RPC returns 409 stale-review. Durable re-application is the sync transform (matched by parsed name + content disambiguator, fail-closed). Tests: hotel_address create/edit/revert while a `hotel_name` override is active; **duplicate-hotel-name reorder / insert-before / removal each fail-close (deactivate stale) rather than retarget the wrong reservation (R16)**; **the SAME reorder / insert-before races between UI-load and an immediate RPC save 409 (not wrong-row apply) for both `hotel_name` and `hotel_address` (R19 — RPC path, distinct from the sync fail-closed tests)**.
...
3. **Two-part row-state CAS (both under the lock; either mismatch → `errcode 40001` → 409 stale_review):**
   ...
   **Hotels additionally** verify the `p_current_ordinal` row's current `hotel_name = p_expected_live_hotel_name` (§5.3) — the row-locator CAS (the row didn't move) — complementary to CAS-B (the value didn't change). **For a duplicate-name group (`p_match_key` carries a content disambiguator) the name CAS is not unique, so the RPC ALSO recomputes the §5.3 disambiguator from the ordinal row's non-overridable `check_in`/`confirmation_no`/`names[0]` and requires it to equal `p_match_key`'s disambiguator AND that exactly one live reservation matches `(currentLiveHotelName + disambiguator)` — zero/>1 → 409 stale_review (R19 fail-closed; prevents a reorder/insert-before race from retargeting a sibling same-name row).** Every successful mutation ends by `version = version + 1`.
4. **Op semantics:**
...
If a live-row anchor matches **zero** rows when the UI expected one (a sync moved the row out from under an *active*-override op), the RPC raises CAS 409 `stale_review` (§7.2 item 3) rather than silently no-op'ing — the admin UI reloads instead of showing a stale value until the next sync. The `show` domain anchors on the shows PK (singleton, no ambiguity). The `hotel` domain anchors on `(show_id, ordinal)` **plus** the name CAS **plus** the content-disambiguator + fail-closed-uniqueness check for duplicate-name groups (§5.3/§7.2 item 3, R19) — ordinal alone is NOT trusted, because a duplicate-name reorder/insert-before can move the ordinal onto a sibling same-name row; that case 409s.
