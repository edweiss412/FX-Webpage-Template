...
**Hold + override on the same member (edge) — DO NOT defer (that reintroduces id churn).** A naive "skip the fold when a hold is open" would write the RAW held name `Jon` while the persisted live row is already `John` → `deleteCrewMembersNotIn` deletes `John` and re-inserts `Jon` with a **new id** → picker cookie breaks (the exact §3.1 failure, in the hold edge). So the override fold **still applies** even with an open hold; only the hold *disposition decision* is computed on raw parse. The reconciliation:
- Hold **disposition** (release / retarget / suppress) is decided by `planHoldAwareApply` on RAW parse — unchanged, correct.
- The override fold is then applied to the returned write list, AND the hold's `protectedNames` / `heldNames` (raw, e.g. `Jon`) are **mapped through the active name override** to their override output (`John`) when building `deleteKeepNames` and the `added`/`removed`/held filters. So the keep-set protects the **override-named** live row → **id stays stable** (assert `crew_members.id` unchanged across the hold+override sync).
- If a held raw name is **absent** from the new parse (a removal-suppression hold), its override's `match_key` is also absent → the override goes stale by §6 (deactivated), and the held raw row is protected under its raw name as usual. No fold, no churn.

Net: holds decide on raw truth; the override fold + id-preservation both still hold. Tested: open MI-11 hold + active crew-name override → hold release/retarget follows raw sheet AND `crew_members.id` is stable across the sync.
