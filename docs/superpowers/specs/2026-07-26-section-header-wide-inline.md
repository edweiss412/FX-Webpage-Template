...
2. **Reconciliation of the existing wide-mode assertions (R1 finding 1 — enumerated, not discovered during implementation).** Four existing regions currently assert the STACKED layout at 1280 and/or measure boxes that flatten at `sm`+; a correct implementation leaves them red unless they are retargeted in the SAME task that adds the new assertions:
   - `tests/e2e/section-header-layout.layout.spec.ts:386-439` (cell-shape measurement): `headerLine` (the icon's parent) has no box at `sm`+ (`display: contents`). Retarget: the measured "header row" box becomes `outer` (`headerLine.parentElement`) at EVERY width — at narrow, `outer` and `headerLine` have identical content widths (both `w-full`, no horizontal padding), so the narrow expectations are unchanged; the 44px line-height assertion at `sm`+ applies to `outer`.
   ...
   - `tests/e2e/admin-layout-dimensions.spec.ts:967-983` and `tests/e2e/admin-layout-dimensions.spec.ts:1058-1063` (§5 width-link chain): `headerLine→column` and `pillLine→column` links are narrow-only (both wrappers deliberately have no box at `sm`+). At `sm`+ the link becomes `outer→column`, and the pill-line link is replaced by an assertion that the pill participates in `outer`'s row (same `getBoundingClientRect` top-band as the heading). The pillLinesMeasured/pillLinesPresent equality keeps its shape with the `sm`+ variant measuring the pill span directly.
