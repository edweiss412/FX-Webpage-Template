...
Ingredient widths measured on the same run (worst case of each): status icon 28px, `gap-2.5` 10px, widest real section name "Sheet warnings" 116.4px, in-group `gap-1.5` 6px, count "(128)"-class ≈ 35px (measured 18.84px for "(1)"; +2 digits ≈ 16px), widest pill "Parsed with judgment" 142.06px, sheet glyph 20px:

```
28 + 10 + 116.4 + 6 + 35 + 10 + 142.06 + 10 + 20 = 377.5px  ≤  552px  (174px slack)
```

The inline row fits at every measured `sm`+ width with ≥174px of slack. No name wraps; the row is a single 44px line (`min-h-tap-min`, `--spacing-tap-min: 44px`, `app/globals.css:162`).
...
- **At `sm`+:** `line1` and `line2` flatten (`sm:contents`); `outer` becomes the flex row (`sm:flex-row sm:items-center sm:gap-2.5 sm:min-h-tap-min`); `group` left-aligns (`sm:justify-start`) and keeps `flex-1 min-w-0`, so it absorbs all slack and pushes the trailing items to the right edge — no pusher element, no `ml-auto` needed with a growable middle; the visual order is `icon, name+count, pill, glyph` (glyph carries a `sm:`-scoped order utility placing it last; the pill needs none — flattened source order already places it after the group).
...
| glyph `<a>` | corner of line 1 | rightmost item of the row (order utility) |
...
2. **15-cell matrix extension** (`section-header-layout.layout.spec.ts` + `_sectionHeaderCellHarness.tsx`): existing 320/375/430 cases keep their stacked expectations verbatim (decision 4 regression guard). New per-cell assertions at 640 and 1280:
   - row height 44px (±0.5) for clean AND pilled cells (the 72.8px expectation moves to narrow-only);
   - name occupies exactly 1 text line (`Range.getClientRects()` on the name's text node, #605 method);
   - pilled cells: pill vertical centre within the row (±0.5 of row centre), pill right of the name group, glyph right of the pill, glyph's right edge flush with the row's content-box right edge (±1);
   - linkless cells: computed `padding-right` of `group` is 0 at `sm`+, and the trailing cluster right edge is the pill (G-report/G-diagrams have no pill either — then the name group simply extends to the row edge);
   - name left-aligned: heading box left edge within icon-width+gap of the row's content-box left edge.
3. **Structural**: exactly one `<a>` (per linked cell) and at most one pill span in the header tree at 375 AND 1280 — pins decision 6 against a future twin-DOM regression.
4. **jsdom class assertions** (existing step3 suites): `outer` carries `sm:flex-row sm:items-center sm:gap-2.5 sm:min-h-tap-min`; wrappers carry `sm:contents`; `group` carries `sm:justify-start` and the linkless branch `sm:pr-0`; glyph carries its `sm:` order utility. Cheap tripwire so a class typo fails before the e2e tier.
5. **Phantom-gap regression:** the flattened wrappers must not introduce a phantom item; `scanForPhantomGaps` coverage on this modal already runs in `admin-layout-dimensions.spec.ts` (its `KNOWN_...` ledger is empty and stays empty).
6. **No new e2e spec file**: extensions land in the two existing specs above (no CI fan-out: both already run in the `mobile-safari`/`desktop-chromium` projects' `layout-dimensions`/`section-header` matchers — `section-header-layout.layout` runs where it runs today; verify with `--list` before relying on it).
