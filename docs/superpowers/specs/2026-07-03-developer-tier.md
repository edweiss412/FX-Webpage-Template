...
declare
  ...
  v_found record;
begin
  -- Gate on is_developer(), NOT is_admin(): only a developer may grant/revoke developer.
...
**Reap route fix (R3):** the catch (`route.ts:43-49`) returns 500-class when `code` is `ADMIN_SESSION_LOOKUP_FAILED` **or** `DEVELOPER_SESSION_LOOKUP_FAILED`, reusing the existing cataloged `ADMIN_SESSION_LOOKUP_FAILED` copy (generic "couldn't verify your session" 500; the admin/developer distinction is internal to `.code`, not user-facing). Non-developer still → `403 ADMIN_FORBIDDEN`.

**validationReset fix (R4):** `MaintenanceResetButtons` already converts action throws to inline copy — so an `AdminInfraError` from `requireAdmin()` is *already* swallowed to inline today; swapping to `requireDeveloper()` does not change that. The two actions **catch the gate infra fault and return a cataloged typed `{kind:'error', code}`** surfaced inline by `MaintenanceResetButtons`'s existing `result.kind === "error"` branch, rather than letting it throw into the client catch-all. This is a **documented, intentional exception to §3.4, scoped to this validation-env-only surface**: it is invariant-9-compliant (a discriminable typed result, never a silent/blank swallow) and consistent with the surface's existing inline-error UX. Reuse the cataloged code the `'error'` variant already renders — **no new §12.4 code**.

**Structural defense (shipped WITH the implementation, not deferred — per the structural-defense calibration for a recurring vector):** a meta-test `tests/auth/developerSurfaceErrorPosture.test.ts` holds a `DEVELOPER_GATED_SURFACES` registry (one row per surface above: `id`, `consumerKind`, `gate`, `declaredPosture ∈ {boundary-500 | route-500-json | inline-typed-exception}`). It asserts (a) **set-equality** between the registry and the developer-gated surfaces in the §6 matrix (a new developer-gated surface MUST register — no silent additions), and (b) every `inline-typed-exception` surface is **explicitly enumerated** (so a future client-`catch` consumer cannot silently downgrade a gate infra fault without declaring the exception here). This converts "find the next peer instance by hand" into a CI-time completeness gate.
...
- **Server action** (`app/admin/settings/admins/actions.ts`): add `setDeveloperAction(prev, formData)` gated by **`requireDeveloperIdentity()`** (outside the try, so its `DeveloperInfraError` propagates to the catalog 500 boundary — mirrors `addAdminAction` at `:76`). Wrap only the data call; `AdminEmailsInfraError` → `{ kind: "infra_error" }`. Extend `AdminEmailActionResult` with `{ kind: "self_developer_demote_forbidden"; email }` and `{ kind: "ok_developer"; email; isDeveloper }` (or reuse `ok`). `revalidatePath("/admin/settings")` + `/admin/settings/admins` on success.
...
2. **`tests/cross-cutting/auth-chain-audit.test.ts`** (EXTEND, via `lib/audit/trustDomains.ts` + `lib/audit/authPrimitives.ts`): add `"requireDeveloper"` to the `ChainStep` union (`trustDomains.ts:12`); change the `PROTECTED_ROUTES` rows for the dev routes + observability + reap route from `chain: ["requireAdmin"]` to `chain: ["requireDeveloper"]`; teach `authPrimitives.ts` to recognize `requireDeveloper`/`requireDeveloperIdentity` as valid first-line gates (validator recognition `~:135`, the `chain[0] === "requireAdmin"` precedence check `~:622`). This structurally catches a missed server action that stayed admin-accessible.
...
7. **`tests/auth/developerSurfaceErrorPosture.test.ts`** (NEW structural meta-test): the `DEVELOPER_GATED_SURFACES` registry + set-equality vs the §6 matrix + explicit enumeration of `inline-typed-exception` surfaces (§6.1 structural defense, Codex spec R4).
8. **New behavioral tests:** the **reap route error-mapping test** — force `requireDeveloperIdentity` to throw `DeveloperInfraError` → assert HTTP **500** (not 403 downgrade); force a confirmed-non-developer → assert 403 (§6.1, Codex spec R3); the **validationReset error-posture test** — force `requireDeveloper` to throw in `resetValidationDataAction` and `reseedValidationFixturesAction` → assert each **returns** the cataloged typed `{kind:'error',code}` (NOT an uncaught throw) AND `MaintenanceResetButtons` renders the cataloged inline copy, not a blank/swallowed state (§6.1, Codex spec R4); `set_admin_developer_rpc` (promote, demote-other, self-demote-refused, non-developer caller 42501, active-rows-only, revoke-clears-bit); `is_developer()` (email arm true; JWT arm true only when `role='admin'` AND `developer='true'`; **JWT `developer:true` WITHOUT `role='admin'` → FALSE** (Codex spec R1 HIGH); revoked row excluded); the toggle UI (visible only to developers, locked on self, outcome copy routed through `getDougFacing`); a Playwright e2e proving a normal-admin fixture sees none of the four surfaces and a developer fixture sees all four; a **bootstrap-invariant test** asserting the migration leaves ≥1 active developer even when the seed row pre-exists in a *revoked* state — i.e. applying the migration against a DB whose `edweiss412@gmail.com` row is revoked yields an active `is_developer=true` row and does NOT raise (Codex spec R2 HIGH).
...
**Any admin can revoke any admin** — including the last developer — because other-revoke is intentionally allowed (ratified amendment §5.5/§11; only *self*-revoke is refused, `20260621000000`). So a normal admin *could* revoke the last developer's admin row, which clears the developer bit (§4.5), leaving zero developers until a re-seed migration/manual DB edit. This is the **existing** admin posture, inherited unchanged — it is **not** a regression introduced here and we deliberately add no new protection. The self-demote refusal (§3.6) prevents a developer locking *themselves* out via the toggle; the last-developer-via-other-admin-revoke case is out of scope and consistent with how last-admin-via-other-revoke already behaves. Reviewers: do not relitigate — cite this section.
...
- **1** new structural meta-test (`developerSurfaceErrorPosture.test.ts`) + **2** extended (`_metaInfraContract`, `auth-chain-audit`) (§10).
