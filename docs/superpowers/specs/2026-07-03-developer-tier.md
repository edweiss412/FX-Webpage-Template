...
**validationReset fix (R4):** `MaintenanceResetButtons` already converts action throws to inline copy — so an `AdminInfraError` from `requireAdmin()` is *already* swallowed to inline today; swapping to `requireDeveloper()` does not change that. Each action places `await requireDeveloper()` as the **first side-effecting statement inside its top-level `try`** (nothing — no `destructiveResetAllowed()` env read, no Supabase/service-role client construction — precedes it), and the `catch` maps a `DeveloperInfraError` to a **cataloged typed `{kind:'error', code}`** surfaced inline by `MaintenanceResetButtons`'s existing `result.kind === "error"` branch, rather than letting it throw into the client catch-all. (Next control-flow digests / the non-developer `forbidden()` are re-thrown, mirroring `addAdminAction`.) This gate placement is exactly what the `developerGatingContract` meta-test asserts for `inline-typed-exception` actions (§6.1 item 1), so the R4 fix and the gate-coverage guard are consistent, not contradictory. This is a **documented, intentional exception to §3.4, scoped to this validation-env-only surface**: it is invariant-9-compliant (a discriminable typed result, never a silent/blank swallow) and consistent with the surface's existing inline-error UX. Reuse the cataloged code the `'error'` variant already renders — **no new §12.4 code**.
