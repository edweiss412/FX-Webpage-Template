...
### 6.1 Error-mapping parity for gate-swapped surfaces (Codex spec R3 HIGH)

Swapping a gate from `requireAdmin*` to `requireDeveloper*` changes the thrown infra-error type from `AdminInfraError` (`code = "ADMIN_SESSION_LOOKUP_FAILED"`) to `DeveloperInfraError` (`code = "DEVELOPER_SESSION_LOOKUP_FAILED"`). Any surface with **bespoke error-code-keyed mapping** must recognize the new code with the **same posture** as the admin one (infra → 500-class; confirmed-non-developer → 403), or an infra fault gets silently downgraded to a 403 authorization denial — violating §3.4 and hiding operator-actionable failures.

**Class sweep (done at spec time):** the only gate-swapped surface with bespoke code-keyed mapping is the **reap API route** (`app/api/admin/onboarding/reap-stale-sessions/route.ts:43-49` — `if (code === "ADMIN_SESSION_LOOKUP_FAILED") return 500; else return 403 ADMIN_FORBIDDEN`). The other swapped surfaces do **not** key on the code:
- The `/admin/dev` page + 2 render harnesses + observability page are **RSC** — a thrown `DeveloperInfraError` propagates to the admin error boundary (`app/admin/error.tsx` / `app/admin/settings/error.tsx`), which renders a generic cataloged 500 for *any* thrown error (verified: those boundaries do NOT branch on `ADMIN_SESSION_LOOKUP_FAILED`), so the posture is preserved with no change.
- The dev server actions + validationReset server actions likewise let the infra error propagate to the same generic-500 boundary (verified: `validationReset.ts:7,20` documents "AdminInfraError → cataloged 500 path", boundary-driven, not code-keyed).

**Fix for the reap route:** the catch must return a 500-class response when `code` is `ADMIN_SESSION_LOOKUP_FAILED` **or** `DEVELOPER_SESSION_LOOKUP_FAILED`. To avoid a second new §12.4 code, the 500 response **reuses the existing cataloged `ADMIN_SESSION_LOOKUP_FAILED`** copy (a generic "couldn't verify your session" 500 — the admin/developer distinction is internal to the thrown `.code`, not user-facing). Confirmed-non-developer still returns `403 ADMIN_FORBIDDEN` (generic forbidden copy). **Required test:** force `requireDeveloperIdentity` to throw `DeveloperInfraError` in the reap route and assert **status 500** (not 403); force a confirmed-non-developer and assert 403.
...
- **1** new §12.4 code (`SELF_DEVELOPER_DEMOTE_FORBIDDEN`); infra reuses `ADMIN_EMAIL_WRITE_FAILED` (§3.9 / §8).
