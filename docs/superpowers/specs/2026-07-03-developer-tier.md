...
1. **Server-action gate coverage (AST, the R5 gap).** Using `ts-morph` (already a repo dependency; template: `resolve-show-page-access-exhaustiveness.test.ts`), it walks each registered server-action file — `app/admin/dev/actions.ts`, `app/admin/settings/_actions/validationReset.ts`, and the new `setDeveloperAction` file — extracts **every exported `async` function** that is a server action (file-level or function-level `"use server"`), and asserts each one's **first-line gate is `requireDeveloper`/`requireDeveloperIdentity`** (not `requireAdmin*`). **Set-equality:** the set of exported server actions discovered by the AST walk MUST equal the registry's action rows — so adding a new action to `actions.ts` without registering + gating it fails CI. This is the guard `auth-chain-audit` cannot provide for non-route files.
