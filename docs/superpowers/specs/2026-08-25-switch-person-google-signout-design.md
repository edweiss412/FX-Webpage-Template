...
6. `clearIdentityCore` failure after a successful sign-out: the fault is injected by removing `PICKER_COOKIE_SIGNING_KEY`, which `signOutThisDevice` never reads and `clearIdentityCoreImpl` reads first (`lib/auth/picker/clearIdentity.ts:227`, throwing per `lib/env/pickerCookieSigningKey.ts:9`), so the sweep at `lib/auth/picker/clearIdentity.ts:184-188` cannot absorb the injection the way a `cookies()` rejection could (spec review round 2). Asserts `{ ok: false, code: "PICKER_RESOLVER_LOOKUP_FAILED" }`, `signOutResolved` present in the call log, and zero writes of the picker cookie. Catches an implementation that calls the core and ignores its `{ ok: false }`: with the key removed the core returns the failure code, and a body that reports `ok: true` fails the first assertion. Rule for every fault injection in this describe: the injected fault must be reachable ONLY by the stage under test (the key for the core; `throwOnCreate` for construction; the `signOut` mock for the two sign-out stages; a `set` that throws only for `sb-`-prefixed names for the sweep), so no earlier stage can produce the asserted code.
