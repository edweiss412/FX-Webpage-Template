...
| R5 | The utterance goes through `AdminAnnounceProvider`. No second live region is minted anywhere in the nav. | `DESIGN.md:983-985` (two sanctioned region shapes; branch-stability applies to the OWNER). The layout already wraps `<AdminNav>` in the provider at `app/admin/layout.tsx:205` and `app/admin/layout.tsx:221`. Enforced by `tests/components/_metaLiveRegionMounting.test.ts`. |
...
Both are `.catch`-wrapped, so **both always settle and neither ever rejects.**
That is a contract this design depends on, and §5 pins it.
...
`setCount` is called with a number at every commit site in both hooks
(`useBellBadge.ts:111`, `useBellBadge.ts:143`, `useBellBadge.ts:169`; `useNeedsAttentionBadge.ts:65`, `useNeedsAttentionBadge.ts:85`) except the
fail-quiet branch at `useNeedsAttentionBadge.ts:88`. So for the bell,
`typeof count === "number" || degraded` is a complete definiteness predicate
for the bell, computable in the component. For attention it is not, and the
missing half has to come from the promise.
...
1. **navArrivalAnnounce.ts** — a new pure module under `components/admin/nav/`, holding the copy builder.
   ...
   one place.
2. **`NotifBell.tsx`** — gains ONE optional prop, `onFirstSettled`, fired at
   most once per mount with the bell's first definite answer.
...
state the accessible name reads, so the two cannot diverge.

### 3.2 The join

Each half reports exactly once per mount, with a `number | null`. `null` means
"nothing announceable ever arrived from this half."

**Bell half** — computed inside `NotifBell` from `useBellBadge`'s outputs:

| Condition | Report |
|---|---|
| `typeof count === "number"` | `count` |
| `degraded === true` and `count === null` | `null` |
| `initialCount == null && countPromise == null` | `null`, immediately — nothing will ever arrive |
| otherwise | not yet; keep waiting |

`degraded === true` with a non-null `count` reports the count: the bell's
ratified posture is keep-last-known-and-mark-stale (`useBellBadge.ts:17-24`), so
a number is still the number the accessible name shows.

**Attention half** — computed inside `AdminNav`:

| Condition | Report |
|---|---|
| `typeof badgeCount === "number"` | `badgeCount` |
| `attentionCountPromise` settled to `kind !== "ok"` | `null` |
| `attentionCountPromise == null && initialBadgeCount == null` | `null`, immediately |
| otherwise | not yet |

The promise subscription exists for exactly one reason: `null` cannot be told
apart from pending in this hook's return (§2.2). It reads the promise for the
SETTLEMENT SIGNAL only, never for the announced number, which always comes from
`badgeCount`.

**The announcement fires on the first render in which BOTH halves have
reported.** Not on promise settlement, and this is deliberate: a definite
report requires the hook to have actually committed, so the effect can never
read a count that has not landed yet, and the design does not rest on React's
auto-batching of two `.then` callbacks.
...
exported from the new module navArrivalAnnounce.ts under `components/admin/nav/`. Both arguments are filtered by
...
**`NotifBell.onFirstSettled`** — new optional prop:

| Input | Behavior |
|---|---|
| absent (every existing caller) | no report is attempted; `NotifBell` behaves exactly as today |
| present | called at most once per mount, per the table in §3.2 |
...
The announcer has three states: **pending** (at least one half unreported),
**spoken** (both reported, message emitted), **settled-silent** (both reported,
...
Compound transitions: the only concurrent state this component can observe is
the bell panel opening mid-pending (`zeroNow()` at `useBellBadge.ts:138-144`).
That commits `count = 0`, which makes the bell half definite and reports `0`,
which drops out. So opening the bell during the pending window suppresses the
bell sentence and leaves the attention sentence intact. That is correct: the
operator is looking at the panel.
...
**Creates:** one structural guard, the new file tests/components/admin/nav/_metaNavArrivalReport.test.ts.

The class it closes: a future commit adds a fourth commit path to
`useBellBadge` or `useNeedsAttentionBadge`, or a fourth branch to the settle
predicate, and the announcer silently stops covering it. The guard derives its
population rather than listing it: it walks `components/admin/nav/` for every
`setCount(` call site in the two hook files and asserts the count matches the
commit-site inventory this spec enumerates in §2.2, so a NEW commit path fails
by default and forces the author back to this table. It is a fail-by-default
walk, not a name list.
...
`tests/components/_metaLiveRegionMounting.test.ts` already covers this surface
and stays green by construction: no `role="log"` or `role="status"` attribute is
added anywhere, and the announcement rides `UndoAnnounceContext`.
...
|---|---|---|
| Both badge promises always settle and never reject | `app/admin/layout.tsx:192-197`, the two `.catch` wrappers | The un-caught half never reports, the join stalls, and the surface goes silent. Silence is today's behavior, so the failure is a non-repair, not a regression |
...
| `useBellBadge` never commits `null` to `count` | `useBellBadge.ts:111`, `useBellBadge.ts:143`, `useBellBadge.ts:169` — all three take numbers | `typeof count === "number" \|\| degraded` stops being a complete definiteness predicate and the bell half could report late |
...
2. **A hung badge read is silence.** If a promise never settles (as opposed to
   settling with an `infra_error`), its half never reports and the whole
   utterance is withheld, including the other half's count. Both promises are
   `.catch`-wrapped so this needs a hang rather than a fault, and the outcome
...
3. **A demoted attention seed can withhold the utterance.** If a pathname change
   ...
   half never reports. Requires a navigation inside the pending window followed
   by a failing fetch.
...
- AC-9 `NotifBell` rendered without `onFirstSettled` behaves identically to
...
- AC-11 Opening the bell panel during the pending window suppresses the bell
  sentence and leaves the attention sentence intact.
