...
- **`AGENDA_UNAUTHENTICATED`** — HTTP **401 only** from the proxy. The proxy emits 401 via multiple live paths (R2 amendment-review finding 2 closure — broadened from initial "no credentials only" claim): (a) no credentials present (no link cookie, no Google session, no cross-show envelope) via `validateCrewAssetSession.ts:136`; (b) `validateLinkSession` recoverable failures that ultimately fall through to the same `unauthorized()` path — `SESSION_NOT_FOUND` (`validateLinkSession.ts:188`), `SESSION_ABSOLUTE_TIMEOUT` (`validateLinkSession.ts:204`), `SESSION_IDLE_TIMEOUT` (`validateLinkSession.ts:288`); (c) terminal failures with status 401 forwarded verbatim from the validator — `LINK_SESSION_KEY_ROTATED` (`validateLinkSession.ts:222`); and any analogous 401 outcomes from `validateGoogleSession`'s terminal-failure path (per `validateCrewAssetSession.ts:117-122`). All live 401 outcomes share the same crew-side recovery: reopen Doug's most recent share link to mint a fresh session. Distinct from 403 cross-show (covered by `AGENDA_GONE_FOR_CREW`).
...
```markdown
| `AGENDA_GONE_FOR_CREW` | `AgendaPdfViewer` (via the proxy at `app/api/asset/agenda/[show]/[id]/route.ts`) received HTTP **410 Gone** OR **403 Forbidden** — header-emitting failure paths only. Proxy 410 triggers (all observed BEFORE the streaming `new Response(stream, ...)` is constructed, so AgendaPdfViewer reliably sees the status): Drive `notFound` / `permissionDenied` / `trashed` / `mimeType !== 'application/pdf'` / pre-flight size > `MAX_AGENDA_BYTES` from Drive metadata / file not present in show's `agenda_links` / invalid Drive-file-ID format / show row missing / non-admin viewer on unpublished show / revoked-link-session 410 fallback (`lib/auth/validateCrewAssetSession.ts:108, 130-131`). Proxy 403 triggers: cross-show link session (`validateCrewAssetSession.ts:133-134`); viewer-show mismatch on Link or Google branch (`validateCrewAssetSession.ts:93, 115`). The two status families share a single crew-side recovery — reopen Doug's link or ask him to re-share — so they map to one catalog row. **Mid-stream `ByteLimitExceededError` is NOT a 410-observable trigger** (R2 amendment-review finding 1 closure) — the bounded-passthrough error fires AFTER `new Response(stream, ...)` returns headers, so the client sees an aborted response body, not an HTTP 410. That case routes through AgendaPdfViewer's existing 5xx / network-error retry-able copy. Distinct from a transient network failure (which retries via the standard 5xx path). | — | "This agenda isn't available anymore. Ask Doug for a fresh link." | Crew → message Doug |
| `AGENDA_UNAUTHENTICATED` | `AgendaPdfViewer` proxy returned HTTP **401 Unauthorized**. Live triggers (R2 amendment-review finding 2 closure — broadened from initial "no credentials only" claim): (a) no credentials present, no link cookie AND no Google session AND no cross-show envelope (`validateCrewAssetSession.ts:136`); (b) `validateLinkSession` recoverable 401 outcomes that fall through to `unauthorized()` — `SESSION_NOT_FOUND` (`validateLinkSession.ts:188`), `SESSION_ABSOLUTE_TIMEOUT` (`validateLinkSession.ts:204`), `SESSION_IDLE_TIMEOUT` (`validateLinkSession.ts:288`); (c) `validateLinkSession` terminal-failure with status 401 forwarded verbatim — `LINK_SESSION_KEY_ROTATED` (`validateLinkSession.ts:222`) via `validateCrewAssetSession.ts:95-100`; (d) `validateGoogleSession` terminal-failure with status 401 via `validateCrewAssetSession.ts:117-122`. All live 401 paths share the same crew-side recovery: reopen Doug's most recent signed link to mint a fresh session. Distinct from 403 cross-show (covered by `AGENDA_GONE_FOR_CREW`) and 410 revoked-link (also `AGENDA_GONE_FOR_CREW`). | — | "Your link to this agenda expired. Reopen Doug's latest message to view it." | Crew → reopen signed link |
```
...
8. ✅ Proxy taxonomy verified against `app/api/asset/agenda/[show]/[id]/route.ts:113-115, 211-289, 367-379, 533-557`, `lib/auth/validateCrewAssetSession.ts:30-136`, and `lib/auth/validateLinkSession.ts:188, 204, 222, 288` (R1 + R2 amendment-review repair). The 410 trigger list enumerates every HEADER-EMITTING 410 path (pre-Response construction); mid-stream `ByteLimitExceededError` is explicitly EXCLUDED because it fires after `new Response(stream, ...)` returns headers (R2 finding 1). The 401 trigger list enumerates all live 401 outcomes from the validator chain (R2 finding 2): no-credentials, recoverable session-expiry paths from `validateLinkSession`, terminal `LINK_SESSION_KEY_ROTATED`. The 403 cross-show paths map to `AGENDA_GONE_FOR_CREW`.
...
10. ✅ Mid-stream `ByteLimitExceededError` exclusion is explicit (R2 amendment-review finding 1): the trigger row text spells out why streaming-path byte-limit is NOT 410-observable, and Task 9.M7-D2's TDD checklist includes a negative-coverage test asserting that case routes through the 5xx retry-able copy instead of `AGENDA_GONE_FOR_CREW`.
