...
| `docs/superpowers/specs/2026-07-20-warning-card-copy-restore.md` reconciliation (R3 finding 3, completed per R4) | The document has ALREADY drifted: the live registry + `EXPECTED_TRIGGER_CONTEXT` hold 41 codes while the §4.2 table has 39 rows — `AGENDA_FILE_INACCESSIBLE` and `HOTEL_ADDRESS_SPLIT_AMBIGUOUS` are missing. In the same commit as the `ORPHANED_CREW_ROWS` row: (a) BACK-FILL those two §4.2 rows using the LIVE frozen copy verbatim (their `triggerContext` from `tests/messages/warningCardCopyRegistry.ts`, their inline copy from the catalog — the enforcement arm is the source for the back-fill, since the table lags it); (b) refresh every count/list claim to the post-addition totals of 42 (registry), 29 parser / 13 sync, adding the missing names to the named lists (`docs/superpowers/specs/2026-07-20-warning-card-copy-restore.md:44` and lines 39 ("20-code" → 21), 53, 60, 62, 64, 103, 104 of that document); (c) extend the per-code emitter inventory (line 168 of that document) with rows for both back-filled codes and `ORPHANED_CREW_ROWS`. After this, table row count === registry count === prose claims |
