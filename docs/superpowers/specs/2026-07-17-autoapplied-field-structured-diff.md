...
**Value bounding (storage + render):** each stored `from`/`to`/`note` string is **truncated at the writer** to **120 chars** with a trailing `…` when longer. The only user-sourced values are MI-8b COI status strings (typically short — "pending"/"received" — but not schema-bounded, so the cap still applies); `note`/`label` are writer constants. This bounds `after_image` size and, combined with the render wrapping contract (§6), prevents one field row from dominating or breaking Doug's mobile-first dashboard.
