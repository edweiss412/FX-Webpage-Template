...
1. **Fresh cron/push parse** — in-memory items, **new shape** (`prior`/`next` present).
2. **Choice-aware staged apply** — `applyStagedCore.ts:578` calls `choiceAwareFeedItems(args.triggeredReviewItems, choices)` and forwards the result as `notableItems` **only when `feedPolicy.kind === "choice_aware"`** (`:600-605`); those items are the **stored** `pending.triggeredReviewItems` JSON (`applyStaged.ts:1386`), possibly staged **before this deploy**. (Note: the **dashboard** staged-apply caller uses `feedPolicy: { kind: "none" }`, `applyStaged.ts:1397-1399`, so it does **not** reach the writer — the concern is the choice-aware path only.)

MI-8 **can** be in a staged set — it is tiered for the wizard review UI (`lib/admin/step3ReviewItemTiers.ts:65`) — so a pre-deploy staged MI-8 is `{ id, invariant: "MI-8", field }` with **no `prior`/`next`**, and the staging guard `isStructurallyValidReviewItem` (`lib/staging/reviewPayloadGuards.ts:69`) does **not** require them (MI-8 is absent from `REVIEW_ITEM_REQUIRED_STRING_FIELDS`, `:53`), so it is valid and can reach the writer via path 2.

**Compatibility rule:** the MI-8 entry builder reads `from = typeof item.prior === "string" ? item.prior : "(unknown)"` — handling both `null` and **absent** — and always sets `to = "(cleared)"`. This is **backward-JSON tolerance at the writer**, not a routing change. **Non-tightening rule:** MI-8 MUST NOT be added to `REVIEW_ITEM_REQUIRED_STRING_FIELDS` (requiring `prior`/`next` there would reject otherwise-valid staged rows and brick pending work). Verified by a direct writer test (§9); a full choice-aware integration test is out of scope (routing is unchanged — the writer's input tolerance is the unit under change).

### 3.6 Malformed stored-item validation + value bounding (writer, fail-safe)

Because stored items reaching the writer (§3.5 path 2) are guarded only for `id`/`invariant` (`reviewPayloadGuards.ts:53,74`), the structured writer MUST NOT assume any other field is present or well-typed. The prior generic writer never dereferenced `field`/`crew_name`/`prior`/`next`, so a corrupt/forward-schema stored item was harmless; the new writer dereferences them and must **fail safe, never `undefined`, never throw**. Per-entry runtime validation (at the writer, before building each entry):

- **MI-8:** `item.field` must be one of `po | proposal | invoice | invoiceNotes`; otherwise **skip this entry** (no partial/`undefined` label).
- **MI-9:** `item.crew_name` must be a non-empty string; otherwise **skip**.
- **MI-8c:** `item.mode` must be one of `collapse | ambiguous_format | halved | case_dropped`; otherwise **skip**.
- **MI-8b:** always valid (COI). Because `normalizeCoi` (`invariants.ts:77-81`) treats `null`, `""`, and whitespace-only as equivalent "none", the raw stored `prior`/`next` can be `""` or `"   "` on a genuine change (e.g. `"" → "received"`). The display normalizer is therefore **trim-aware**: `x = typeof x === "string" && x.trim() !== "" ? x.trim() : "(none)"` — never a blank From/To cell.
- **Universal (trim-aware sentinel):** every value placed in an entry is coerced through `typeof x === "string" && x.trim() !== "" ? x.trim() : <sentinel>` (sentinel = `"(none)"`/`"(unknown)"`/`"(cleared)"` per field) so the stored JSON never contains `undefined`, `null`, `""`, or whitespace-only in a taken branch — upholding the "no blank cell" guarantee (§7).

If **every** field-family item in a sync is malformed and skipped, `fieldChanges` is empty → **no row** (§3 predicate). This is an acceptable degradation for corrupt/tampered stored data (the apply itself is unaffected; the change is simply not re-surfaced in the acknowledge strip) and is strictly fail-closed — never a throw that bricks staged apply/finalize. Deliberately **not** resurrecting a generic-summary fallback for this pathological case (YAGNI; fresh-path items are always well-formed).

**Value bounding (storage + render):** each stored `from`/`to`/`note`/`label` string is **truncated at the writer** to **120 chars** with a trailing `…` when longer (financial fields like `invoiceNotes` can be long/unbroken). This bounds `after_image` size and, combined with the render wrapping contract (§6), prevents one field row from dominating or breaking Doug's mobile-first dashboard.
...
- **Malformed stored item (bad/absent `field`, `crew_name`, `mode`):** writer skips that entry (§3.6) — never `undefined`, never throws. If all field-family items are malformed → `fieldChanges` empty → no row (fail-closed; apply unaffected).
- **Over-long value (`invoiceNotes` etc.):** writer truncates each stored `from`/`to`/`note`/`label` to 120 chars + `…` (§3.6); reader wraps via `wrap-break-word` (§6) — no horizontal overflow on mobile.
- **Empty `fieldChanges`:** the writer writes **no `field_changed` row at all** (§3 defensive predicate) — never an empty structured row. Every well-formed field-family item the writer receives (MI-8/8b/8c/9) yields an entry, so for well-formed input this branch does not fire for any pass that fires today (behavior-preserving); it also covers the all-malformed pathological case.
...
- **Debounce regression (Codex R4):** a Phase-1 test asserting that a sync whose only field-family items are **MI-8/MI-8b with an unstable Drive `modifiedTime`** returns `defer` (`mi8DebounceReason`) and therefore **writes no `field_changed` row** (the writer is not reached). Pins that this feature does not weaken the debounce; if `tests/sync/phase1.test.ts` already covers the defer outcome, add the "no change-log row written" assertion. Failure mode caught: enriching the writer while accidentally bypassing the pre-Phase-2 defer, logging a field change for a sync that did not apply.
  - **Legacy MI-8 backward-JSON tolerance (§3.5):** at the writer, an MI-8 item `{ id, invariant:"MI-8", field:"po" }` with **no `prior`/`next`** (the pre-deploy staged shape that reaches the writer via the choice-aware path) → entry `{ label:"PO number", from:"(unknown)", to:"(cleared)", note:null }`; no `undefined` in the stored JSON, no throw. This is a direct writer unit test (routing unchanged; no new choice-aware integration test). Failure mode caught: a staged-before-deploy row emitting `undefined → (cleared)` or throwing on apply.
  - **Malformed stored items (§3.6):** MI-8 with `field` missing / not an allowed enum, MI-9 with `crew_name` missing / non-string, MI-8c with an unknown `mode` → each **skipped**, no `undefined` in `after_image.fieldChanges`, no throw; a sync of only-malformed field-family items → **no row written**. Failure mode caught: the structured writer dereferencing a corrupt stored item into an `undefined` label or bricking staged apply/finalize.
  - **Value bounding (§3.6):** a `from`/`note` value > 120 chars (and a long unbroken token) → stored value truncated to 120 + `…`. Failure mode caught: an unbounded `invoiceNotes` bloating `after_image` / breaking layout.
  - **MI-8b empty/whitespace COI (§3.6, Codex R7):** items with `prior` = `null`, `""`, and `"   "` (each vs a real `next`) → `from` = `"(none)"` (never blank); plus the symmetric `next`-empty case. Failure mode caught: a valid COI change rendering a blank From/To cell.
...
- **`tests/components/admin/RecentAutoAppliedStrip.test.tsx`** — `DiffBlock` renders `fields` entries (From→To + note lines); **renders ALL entries with no cap** — a `fieldChanges` of >8 MI-9 crew notes shows **every** crew name (none hidden behind a `"+N more"`; guards the §6 no-cap decision — failure mode: an applied crew name hidden before a whole-row Accept); `none` fallback still renders summary. **`StripRow`-level:** a `field_changed` row with `diff.kind === "fields"` renders **no "Crew member" label** (guards the §6 `isCrew` narrowing); a `crew_renamed`/`crew_added` row still renders it. **Long-value:** an entry whose `from`/`note` is a long unbroken string renders with `wrap-break-word` and does not overflow its container (assert the class / the truncated content). Failure modes caught: the `fields` variant tripping the diff-kind-based `isCrew` guard; a hidden overflow entry; an unbounded value breaking the mobile card.
...
7. **Legacy staged MI-8 → `"(unknown)"` writer tolerance, guard NOT tightened, routing unchanged** — §3.5; the choice-aware staged path (`applyStagedCore.ts:578`) can forward a pre-deploy MI-8 lacking `prior`/`next`; the writer tolerates it (backward-JSON), `REVIEW_ITEM_REQUIRED_STRING_FIELDS` deliberately unchanged, no staged-feed routing change. The dashboard `feedPolicy:"none"` path does not reach the writer.
...
11. **Writer fails safe on malformed stored items; per-value 120-char cap** — §3.6; the structured writer validates `field`/`crew_name`/`mode`, skips malformed entries (never `undefined`/throw), and truncates long values — hardening the new dereferences the generic writer never made.
12. **Trim-aware sentinels (no blank cell)** — §3.6; `null`/`""`/whitespace all map to `"(none)"`/`"(unknown)"` (MI-8b COI normalizes with trim, so raw empties reach the writer).
