...
- `deriveDiff` (`:83`) gains a `field_changed` branch that reads `after_image.fieldChanges` and **re-validates + re-bounds at read time** (defence-in-depth — the loader must not trust the stored payload, which could be a corrupt, manually-repaired, or future-bug service-role row):
  - Each entry must be an object with a non-empty string `label` and a valid `note` XOR (`from`+`to`); malformed entries are dropped.
  - **Read-side caps:** at most **50** entries (`READ_FIELDS_ENTRY_CAP`) and each string re-truncated to **120** chars. A payload exceeding the entry cap → return `{ kind: "none" }` (summary fallback) rather than rendering an unbounded list; the loader emits a `code:`-carrying `log.warn` flagging the over-cap row (invalid-payload signal) so it is observable.
  - If ≥1 valid bounded entry remains → `{ kind: "fields", entries }`; if none → `{ kind: "none" }` (summary — back-compat for pre-existing null-after_image rows and generic-fallback rows).
- The 50-entry read cap sits above any real sync (roster-bounded, §6) so it never hides a legitimately-applied change, but bounds the JSONB select + DOM work from a pathological row. Combined with the writer caps (§3.6), both the write and read sides are bounded.
...
- **`tests/admin/loadRecentAutoApplied.test.ts`** — `deriveDiff` field_changed branch: well-formed → `{kind:"fields"}`; absent/`[]`/malformed → `{kind:"none"}`; pre-existing null-after_image row → `{kind:"none"}`. **Read-side hardening (Codex R10):** a stored payload with **>50 entries** → `{kind:"none"}` (summary fallback) + an invalid-payload `log.warn` (assert the warn fired); an entry with a malformed shape among valid ones → that entry dropped, valid ones kept; a stored string >120 chars → re-truncated at read. Failure mode caught: a corrupt/future-bug service-role row forcing an unbounded JSONB select + DOM render on the dashboard.
...
13. **No hidden entries in a real sync; reader-side 50-entry corruption cap** — §5/§6; a whole-row Accept must not dismiss changes hidden behind an overflow collapse, so real syncs render in full (roster-bounded); the only cap is a read-side sanity guard (50, far above any real sync) that falls a *corrupt* row back to the summary — it never truncates a legitimate change.
