...
| both=banner | permitted. Two banners stack; rotate's persists until its confirm is re-armed, reset's clears on its own 5s timer, so the pair resolves to rotate-only without either clearing the other. Both are `role="status"`; they announce in DOM order |
