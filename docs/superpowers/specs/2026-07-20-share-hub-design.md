...
- Popover idioms (#499): backdrop close (`CrewRowActions.tsx:17-19,227-231` — fixed inset-0 z-20, click closes, no focus restore on backdrop), Escape `preventDefault+stopPropagation` then close-with-trigger-focus (`CrewRowActions.tsx:115-121,158-163`; shell ignores defaultPrevented `ReviewModalShell.tsx:238-243`), resolving = inert close paths (`CrewRowActions.tsx:148-157`).
...
- Open/close: click either trigger toggles. Backdrop (`fixed inset-0 z-20`, UserMenu idiom per `CrewRowActions.tsx:17-19`) closes without focus restore. Escape inside popover: `preventDefault+stopPropagation`, close, focus the last-used trigger (`ReviewModalShell.tsx:238-243` contract). While either child control is `resolving`: Escape/backdrop close is inert (mirror `CrewRowActions.tsx:148-157`).
