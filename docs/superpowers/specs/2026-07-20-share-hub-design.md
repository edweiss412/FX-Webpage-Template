...
| banner→idle (either) | instant. Rotate clears its banner (success or error) only when the confirm is re-armed (`setResult(null)`, `RotateShareTokenButton.tsx:125-128`). Reset clears a SUCCESS banner on its timer; a reset ERROR banner clears only on re-arm (`enterConfirm`'s `setOutcome(null)`, `PickerResetControl.tsx:110-112`) |
...
| rotate=resolving ∧ reset=confirm | symmetric to the above |
| both=banner, both SUCCESS | permitted. Two banners stack, neither cross-clearing. Reset's success auto-dismisses at `SUCCESS_DISMISS_MS` (`PickerResetControl.tsx:102-106`); rotate's success persists until its confirm is re-armed (`setResult(null)`, `RotateShareTokenButton.tsx:125-128`), so the pair resolves to rotate-only. Announcement: reset via its persistent sr-only `role="status"` region (`PickerResetControl.tsx:156`, the visible banner being `aria-hidden`); rotate via its visible `role="status"` (`:203`, `:217`) |
