...
| rotate success while a Copy 2s window is open | `onRotated` swaps the context token and the URL re-renders. The stale "Copied" label now clears IMMEDIATELY on the url change rather than waiting out its 2s timer, and a `writeText` still in flight when the rotate lands is discarded instead of re-announcing success (`share-link-chrome-backlog`, 2026-07-25). The old behaviour left the button asserting a clipboard that held the dead url. |
