...
**Class-sweep note (repo-wide `rg "View details"`):** every non-generated reference to the old label lives in the files above plus the source file (`step3ReviewSections.tsx:540,588,593`). No other component, page, or fixture references it. This list is the complete `View details` → new-label migration surface; TDD tasks update each cited line and a final `rg "View details"` (excluding `node_modules`) must return zero hits before close-out.
