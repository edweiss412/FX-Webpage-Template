...
| test ports | the three files containing `activatePending` | NORMATIVE seam (spec review R3 finding 3): the settings comparison is part of `activatePending`'s CONTRACT, so each port models it at its own level. `tests/drive/watch.test.ts` + `tests/drive/watchExpiration.test.ts` (in-memory fakes): fake gains a settable `settingsRow: { watched_folder_id: string | null } | null` field (default = `null`, i.e. row absent, which the §3.1 rule maps to PROCEED — behaviorally transparent for existing cases without per-case folder wiring, which the fake's constructor cannot derive; `null` = row absent because the four-row rule requires absent to be representable; amended per plan reviews R1 F2 and R5 F2) that its `activatePending` compares per the §3.1 rule and returns the new discriminated result. `tests/db/watchLifecycle.db.test.ts` (REAL `PostgresWatchTx` adapter): seeds the real `app_settings` `'default'` row to the fixture folder before each case and restores the prior row state after (premise guard: the seeded value must be reachable — assert the seed wrote 1 row). All three are type-errors until updated (deliberate) |
