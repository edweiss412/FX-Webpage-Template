...
6. **The authored-red branch verifies the DECLARATION, not the defect.** For `red-state=authored`, the arm checks a `red-target=` names a tracked file and in-range line (§4.3). Whether that production line is genuinely "absent or defective" is semantic judgment and stays with the plan-review citation pass (governing spec §1.2 first bullet). Mechanizing existence-of-declaration is this arm; mechanizing truth-of-claim is not.
7. **Marker grammar widens globally; requirements apply only inside red-contract regions.** The v2 fields (§4.2) are legal on any marker anywhere (one grammar, no per-region parse forks), but their PRESENCE is required only in regions opened with the `red-contract` attribute. Legacy enrolled plans (31 tracked plans, 222 markers at authoring time) relint exactly as before.
...
9. **Consequence bound (convergence criterion).** Every input is handled correctly OR signaled, never silently wrong: a marker the grammar cannot parse is a hard finding (fail-closed within the declared domain, mirroring the governing spec's §1.1 item 10 posture); a citation the intent tiers cannot classify falls through to silence only when it carries no same-line identifiers (nothing to compare — the sketch's own demotion rule). A conservative demote plus a surfaced advisory is a DOCUMENTED LIMIT, not a finding. Mechanical convergence for the two new lib modules is the mutation score plus an empty unaccepted-survivor set (§7).
...
For a tier-4 finding, the arm searches each identifier (same discipline) across the OTHER files this doc's citations resolved to (the `resolvedPaths` set `checkCitations` already accumulates, deduplicated, minus the cited file). Files where any identifier matches are listed in the finding detail, capped at 3 paths in doc order. This is the "found in `runOnboardingScan.ts`" line that turns a shrug into a fix; it is bounded (a doc cites tens of files, all already read through the resolver's cache) and never leaves the doc's own citation set (§1.2). On the measured corpus the wrong sites-list rows relocate to `lib/sync/runOnboardingScan.ts`, which the known-bad plan already cites elsewhere.

### 3.5 Report shape

Tier-3 message: `no same-line identifier found near <citation>` (unchanged text, so existing operator reflexes keep working); detail gains the enclosing name: `cited line: <first cited line, trimmed to 160> · enclosing: <name or "(none)">`. Tier-4 message: `same-line identifiers absent from <path>`; detail: `enclosing: <name or "(none)"> · identifiers: <list> · found in: <up to 3 relocation paths, or "no other file cited by this doc">`. Both stay `check: "citations"`, severity `advisory`, anchored at the citation span (docLine, column) like today.
...
- `red-target=` — a backtick-delimited FULL-PATH line-bearing citation (colon form of the governing spec §4; bare-filename shorthand is not legal here, since a marker line offers no anchor context). Named identifiers, spaces, and nested backticks are excluded by the same character rules as `red=`.
...
### 4.3 Static checks, red-contract regions only (all hard)

Run per owned, well-formed marker after the shipped cardinality/classification pass:

- `RED_STATE_MISSING` — marker has no `red-state=`.
- `RED_WHY_MISSING` — marker has no `why=` (the grammar already rejects an empty one as malformed; this code is for absence).
- `RED_TARGET_MISSING` — `red-state=authored` with no `red-target=`.
- `RED_TARGET_INVALID` — `red-target=` present but its citation is malformed, untracked, unreadable, or out of range — verified through the SAME resolver machinery as prose citations (`checkCitations` semantics; the marker citation does not enter `resolvedPaths` or the anchor list, so it cannot anchor prose shorthands). One code with the specific reason in `detail`, because the repair is identical in every case: fix the target.
- `RED_TARGET_RETIRED` — some tracked-path-shaped token in the marker's `red=` command (a token matching the governing spec's citation path rule whose text is a member of the tracked-file set) also appears as the object of a `git mv <path> …` or `git rm <path>` command anywhere in this plan's FENCED code. The red-then-green cycle cannot complete: the same command never passes after the move. Fenced-only, exact-token matching keeps this precise on the measured corpus (23 such lines exist).
...
New CLI flag, usage error unless the resolved kind is `plan`. When set, the adapter executes each `red-state=live` marker's `red=` command (in doc order, sequentially) via `sh -c`, cwd the repo root, 600-second per-command ceiling, and hands the core the outcomes:
...
lib/specLint/types.ts            # CommandRunner + exec-outcome types
```

- **Purity holds.** `lib/specLint/**` still imports no `node:fs` / `node:child_process` / `node:process` (`tests/specLint/_metaPureCore.test.ts` walks the directory and covers the new files by default). Execution follows the resolver pattern: the core exposes the list of live markers to run; the adapter owns the subprocess and passes outcomes back in; finding synthesis is pure and unit-testable with a fake runner.
...
- **Red-contract unit suite** (new `redContract` test file + `tests/specLint/taskContract.test.ts` extensions): region attribute recognized / bare form unchanged / malformed attribute rejected; every §4.3 code fired and not-fired (state present, why present, target present-and-valid, target each invalid reason, retired-target hit via fenced `git mv` and `git rm`, non-fenced mention NOT firing); v2 fields outside contract regions drawing nothing; v1 markers inside contract regions drawing exactly the presence codes; `RED_CONJUNCTION` on `&&` only; gate marker shapes (well-formed with/without probed, malformed prefix line); field-order and single-space strictness (a reordered marker is malformed).
- **Execution suite** (fake runner, pure core): live markers enumerated in doc order; exit-0 → `RED_ALREADY_GREEN` hard; 126/127 → `RED_EXEC_ERROR`; timeout token → `RED_EXEC_TIMEOUT`; other non-zero → silence; authored markers never handed to the runner (spy pins it); no runner injected (static invocation) → no execution findings.
- **CLI adapter tests** (`cli.test.ts` extensions, committed fixture docs): `--exec-red` on a spec → exit 2; on a plan with a fixture `red-state=live` marker whose command is `sh -c "exit 0"`-equivalent → exit 1 with `RED_ALREADY_GREEN` in the report; command exiting non-zero → exit 0; duplicate `--exec-red` → exit 2. Real subprocess, trivial commands only (no test may execute a heavy phase).
...
5. **`red-target` truth**: the target's absence-or-defectiveness on the live tree is not machine-checked (§1.1 item 6).
...
7. **Retired-target detection is exact-token, fenced-only**: a prose instruction ("delete the old suite") or a renamed path spelled differently escapes. Widening to prose recognition is the recognizer-over-prose shape this tool's design forbids.
8. **Execution observes exit codes, not reasons**: a `red=` failing for an unrelated reason (missing env, wrong cwd) reads as red. `RED_EXEC_ERROR` catches the unrunnable subset (126/127); the rest is the operator's transcript to read.
...
- AC-3: matching is word-boundary + dotted-segment everywhere the arm compares identifiers (window, enclosing, file, relocation) — one shared implementation, pinned by a test that fails on substring semantics in any one of the four.
- AC-4: every §4.3/§4.4/§4.5/§4.6 code fires and not-fires per §6; presence requirements bind only in `red-contract` regions; v1 plans relint byte-identically (a corpus regression test over ≥2 committed legacy fixture plans).
- AC-5: `--exec-red` executes only `red-state=live` markers, only on plans, with the §4.4 outcome mapping; the fake-runner spy proves authored markers never execute.
