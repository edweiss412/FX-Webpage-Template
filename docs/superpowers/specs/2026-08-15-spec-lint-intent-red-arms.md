...
- **Marker-line spans never enter the citation pass.** Today `parseDoc` extracts inline spans from every non-fenced line, so a `red-target=` field would be resolved, anchor-seeded, and relocation-fed as if it were prose — probed live during review round 1. The orchestrator (`lib/specLint/run.ts`) therefore computes an exclusion set and passes it to `checkCitations`, whose span loop skips those lines entirely: their spans are never candidates, never resolve, never anchor a shorthand, and never feed relocation. The set is scoped to where the red-contract grammar guarantees a replacement finding path, so no existing hard citation finding is silently removed (probed in review round 3: spec-kind marker/gate lines and no-region plan markers DO produce `CITATION_FILE_MISSING` today): (a) gate-shaped lines in PLAN-kind docs (the gate grammar owns them unconditionally in plans — well-formed, `GATE_MALFORMED`, or `GATE_CMD_EMPTY`); (b) marker-shaped lines in PLAN-kind docs with at least one well-formed task region (enrollment established — every such line is owned, orphaned, or malformed by the task-contract pass). Spec-kind docs and non-enrolled plans keep today's citation behavior on those lines unchanged; a v2 field placed there flows through the prose-citation pass as ordinary spans (documented limit — v2 fields belong in enrolled plans). `red-target=` verification lives solely in the `redContract` module through the same injected resolver. Legacy impact: none measured — every real v1 `red=` payload in the corpus contains whitespace (the only whitespace-free payloads are the documentation placeholders `cmd` and `x`), so no v1 span changes candidacy.
...
13. **V2 fields outside enrollment are prose spans**: in a spec or a non-enrolled plan, a `red-target=` span flows through the ordinary citation pass (§5). The red contract binds where it is declared; declaring it is one region attribute.
...
- AC-4: every §4.3/§4.4/§4.5/§4.6 code fires and not-fires per §6; presence requirements bind only in `red-contract` regions; v1 plans relint with byte-identical task-contract findings (a corpus regression test over at least 2 committed legacy fixture plans), and the §5 exclusion is proven scope-exact: excluded on enrolled-plan marker lines and plan gate lines, NOT excluded on spec-kind marker/gate lines or non-enrolled-plan marker lines (each keeping its hard citation finding).
