...
- signal-terminated (the child dies with no exit code — `spawnSync` reports `status: null` with a `signal`) or spawn failure → **advisory `RED_EXEC_ERROR`**, detail naming the signal or spawn error. A killed command proves nothing about redness; classifying it as observed red would be the silent corruption this arm exists to prevent, so the outcome domain is closed: exit code, timeout, signal, spawn-error — nothing else exists, and each maps to exactly one disposition above.
