...
- A refresh landing around a close must not resurrect the modal. Close is NOT an instant URL strip: under normal motion, `ReviewModalShell.requestClose` plays the exit animation and calls `onClose` at exit-END (~220 ms; `components/admin/review/ReviewModalShell.tsx:345,389`, #488) — only then does `useShowModalNav().close` strip `show` from the URL (`useShowModalNav.ts:30-36`). Three distinct cases, each pinned (§6.4):
  ...
  2. **Refresh lands after the visual close, before/around the URL commit** (amended at implementation, first gated e2e run): the app router SERIALIZES actions, so an in-flight (held) refresh queues the close `push` behind it — a refresh response strictly after the close COMMIT is therefore unconstructible under network hold, and in production the commit itself waits for the refresh to settle. The pinned case: the exit finishes and the shell hides client-side while the URL still carries `show` and the refresh is outstanding; when the refresh applies, the open-URL payload must not re-show the visually-closed shell, and the queued close commit must follow. (The refreshed tree for the eventually-committed closed URL has no modal slot — nothing to resurrect.)
...
4. **Close safety (three §3.2 cases):** (a) animated close, then release the held refresh DURING the exit window — exit completes, URL strips, modal stays gone; (b) release after the VISUAL close with the URL commit still pending (see amended §3.2 case 2 — the router's action serialization makes release-after-commit unconstructible) — no resurrection, the queued commit lands, dashboard intact; (c) reduced-motion close then release — same. In all cases assert final state: no modal panel, URL without `show`.
