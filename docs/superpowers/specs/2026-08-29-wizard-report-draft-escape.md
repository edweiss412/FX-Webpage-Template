...
Per-keystroke writes are chosen over a write-on-unmount effect. Unmount ordering is one more thing to be wrong about, and the value is capped at `REPORT_MESSAGE_MAX_CHARS`, so the write is trivial.
...
| Disabled submit carries no programmatic reason for being disabled | P3 | PRE-EXISTING, not introduced. Out of scope under the class-sweep rule, which covers instances of the shape a PR is repairing; this is a different shape on a control the diff does not otherwise change. Noted rather than filed. |
...
**Refuted during review, recorded so it is not re-derived.** The key `fxav-report-draft-wizard-${wizardSessionId}-${driveFileId}` looks delimiter-ambiguous, since both segments are joined by hyphens and the ids could in principle straddle the boundary. It is unreachable: `wizardSessionId` is a UUID and therefore exactly 36 characters, so the segment boundary is fixed and no `(session, file)` pair can collide with another. The same shape is inherited unchanged from `reportAttemptStorageKey` beside it.
...
- **Keys accumulate for the life of the tab**, one per (wizard session, drive file) with a non-empty draft, each bounded at `REPORT_MESSAGE_MAX_CHARS`. A wizard session covering many shows where the operator types into many report fields and sends none is the worst case, and it is bounded by the show count and cleared by closing the tab. No eviction policy ships.
- **Nothing announces the restore to a screen-reader user.** A sighted operator gets the changed trigger label; a screen-reader user learns of the restored draft only on reaching that control, whose accessible name carries it. Accepted as a limit on the audit's own recommendation rather than fixed: this is not a WCAG 4.1.3 violation, because the change is a programmatically determinable accessible name on a control and not an unfocused status message, and routing it through the existing `role="status"` region would fire an unrequested announcement on every modal open — including the no-draft case, which is far the more common one. The guarantee line is ambient for the same reason. Re-file trigger: a design decision to announce draft state generally, which would make this one instance of a pattern rather than a lone interruption.
