...
1. **Detector fixtures.** The matcher is a pure exported function tested against synthetic sources: one inline `NextResponse.redirect(new URL(p, request.url))`, one variable-assigned `const url = new URL(p, request.url); return NextResponse.redirect(url)`, one `req.url` spelling of each, and three negatives (`NextResponse.redirect(data.url)`, a `new URL` with an absolute base, and a bare `new URL(..., request.url)` never passed to a redirect). A detector that recognises neither form, or only the inline form, fails here rather than passing vacuously on a fixed tree.
