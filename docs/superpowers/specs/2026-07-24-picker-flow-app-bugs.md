...
1. **Detector fixtures.** The matcher is a pure exported function tested against the canonical ten-case fixture set defined below — six positives and four negatives. That set is the single source of truth for these counts; no other section restates them.
...
| tests/cross-cutting/no-absolute-self-redirect.test.ts (new) | The three §3.4 layers: the detector flags all six positives and none of the four negatives in the canonical ten-case fixture set defined in §3.4; no file under `app/` is flagged; the walk visited more than 50 files | A detector that recognises neither form or only the inline one; reintroduction of the shape in a new route; a broken glob passing vacuously |
...
| tests | per §6, plus two files §6 does not cover because they pin process rather than behavior: tests/cross-cutting/picker-flow-e2e-ci-wiring.test.ts (the CI wiring, spec reference, key shape, and path filter) and tests/docs/pickerFlowBacklogArchived.test.ts (the backlog move and the filed follow-up) |
