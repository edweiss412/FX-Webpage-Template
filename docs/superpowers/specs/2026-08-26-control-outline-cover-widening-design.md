...
**Why not one shared vocabulary.** The tap-target census would absorb 60+ text-entry elements into a
...
### 5.1 One bounded extension, forced by the row's own flagship site

JSX ancestry alone does not reach `components/admin/wizard/VenueMapTile.tsx:121`, the site the row
names first and calls the strongest claim to have belonged in the 21. Its span lives in
`const inner = (<>…</>)` at `components/admin/wizard/VenueMapTile.tsx:42` and is rendered as
`{inner}` inside the `<a>` at `components/admin/wizard/VenueMapTile.tsx:143`. Measured:
under ancestry alone the site is absent from the red.

So the walk follows one more edge, and exactly one: **a JSX child expression that is a bare identifier
resolving to a file-local variable whose initializer contains JSX.** Same file, no import hops, no
component invocation. This is the binding resolution the module already performs for classNames
(`tests/styles/interactiveScanCore.ts:770-774`) applied to a JSX child rather than to a class string.
Cost, measured: universe 627 to 636, and one residue site, the flagship one.

Re-entrancy is bounded by a name set per file, so `const a = <>{b}</>; const b = <>{a}</>` terminates.

### 5.2 What the mechanism cannot reach, stated as a limit and not as a gap

**Paint moved into a sibling component function is invisible, and stays invisible.**
`components/admin/settings/DeveloperToggleButton.tsx:93` paints a switch track inside
`function SwitchTrack`, rendered as `<SwitchTrack on={…} />` at
`components/admin/settings/DeveloperToggleButton.tsx:128` and
`components/admin/settings/DeveloperToggleButton.tsx:165`. A component
invocation is not a value reference; reaching it means inlining a component's JSX at its call sites,
which is recognizer growth of the shape AGENTS.md "Repair direction under same-axis recurrence"
declines.

This corrects `DESIGN.md:260-261`, which today says the last two of the five switch-track paths "paint
the track on a nested `<span>`, which is why an element-level census reported three". Those two paths
are not the same mechanism: `AutoRefreshControl.tsx:105` is a lexical `<span>` child of its button and
IS reached, while `DeveloperToggleButton.tsx:93` is a component boundary and is not. So the
switch-track count moves **3 to 4**, not 3 to 5, and §11 rewrites that sentence to say which mechanism
each path uses.

**Nothing gets worse.** The fifth path is invisible to the census today and is invisible after. It is
a switch track, ruled OUT, so the only consequence of the miss is the absence of a `RESIDUE_CENSUS`
row that today does not exist either.

**RE-FILE TRIGGER for both limits:** a control whose only resting outline is painted inside a sibling
component reaching `main` at `border-border` or `border-border-strong`, or the scanner gaining
component-level resolution for some other reason.
...
| a JSX descendant that is itself in scope | admitted as an element (unchanged) | admitted as an element, never twice |
| a JSX child `{ident}` bound to a file-local JSX-valued variable | not followed | followed once per name per file |
...
| a component invocation `<Foo />` | admitted only if `Foo` is in scope by the existing rules | unchanged; its body is never inlined |
...
no JSX contributes nothing. A cyclic pair of JSX-valued consts terminates on a per-file name set. The
existing resolver bounds (`MAX_RESOLVE_DEPTH` 6, `MAX_PATHS` 64, `MAX_IMPORT_HOPS` 3 at
`tests/styles/interactiveScanCore.ts:79-81`) are untouched and apply to a painted child's className
exactly as they apply to an element's.
...
| `paintedChildren` only | 636 | 24 | 22 |
| both | **695** | **37** | **32** |

Ten of those 37 are the ten already registered. **The reddening set is 27 elements**, and one further
...
Five more (`AddAdminForm` twice, `ShowsTable`, `MaterializeCard` three times, `EventFilters` twice)
appear in neither. That gap is the reason the sweep is derived.

### 6.2 Family B, fourteen elements
...
decorative label) keeps the treatment `DESIGN.md` §1.2a's scope paragraph already gives non-interactive
chrome by name: "tile and card edges, hover borders, focus-adjacent chrome, and the status-emphasis
outline on non-interactive chrome (the flagged pill, the judgment chip)".
...
is the guard that drives the sweep. Universe 362 to 695, residue elements 10 to 37 before the repairs
and 15 after, `RESIDUE_CENSUS.length` 10 to 15.

**Why 15 rows and not 15 plus duplicates.** The residue key is `(file, tag, sorted projections)` with
no line number (`tests/styles/controlOutlineResidue.ts:436`), so identical siblings share one key at
multiplicity two and need two rows. Measured: 27 new elements collapse to 22 new keys, and all five
collisions are among the SWAPPED sites, never the registered ones.
...
`components/admin/wizard/step3ReviewSections.tsx:1779` are one key at multiplicity three. 27 minus 5 is 22, which is the measured key count and the check that this
reading of the key is the guard's reading. The five registered elements hold five distinct keys, so
the census is 10 plus 5 rows. Per category: `switch-track` 3 to 4,
`side-divider` 5 unchanged, `focus-state-chrome` 2 unchanged, `inner-chrome` 0 to 4 (new category,
§8), and `responsive-skin-filed`, `filed-defect`, `literal-outline` all 0 unchanged. Intended: this is
the deliverable. `tests/styles/_metaControlOutlineResidue.test.ts:386` and
`tests/styles/_metaControlOutlineResidue.test.ts:392-394` move to the derived values.

The `switch-track` movement is the checkable prediction the brief set: `AutoRefreshControl.tsx:105` is
a lexical `<span>` child of its button and becomes visible; its row is registered against the
2026-08-16 ruling, with the same OFF-ring ratio the three existing rows record. §5.2 says why the
fifth path does not follow and 4 rather than 5 is the honest number.

**7.2 `tests/styles/tintedPlateOutline.test.ts:56` reads `{ textEntry: true, paintedChildren: true }`.** Derived
subject list 8 to 9, the new member being §6.3. Universe premise `>= 200` still holds at 695. The
`neutralFaintCount` rows of its registry count occurrences in files, not scanned elements, so none of
them moves. Intended: the plate cover reaching a text field is exactly what ruling (a) makes correct.
...
about the height of a text field, and admitting 60+ text-entry elements to a 44px floor census would
oblige a reason row per element for a question nobody has asked. A future arc that wants that census
...
**7.6 `tests/styles/_metaControlOutlineFill.test.ts:53` scans directly, and reads the DEFAULT.** Its
...
- **AC-4** A JSX child that is a bare identifier bound to a file-local JSX-valued variable is followed;
  a component invocation is not. Both directions asserted, the second against a fixture shaped like
...
- **AC-6** Every one of the 27 elements in §6.1 and §6.2 is either swapped to the token its ground
  requires or holds a `RESIDUE_CENSUS` row that passes its category bar.
- **AC-7** `RESIDUE_CENSUS.length` is 15; per-category counts are 4 / 5 / 2 / 4 / 0 / 0 / 0 for
...
Three swap targets animate their outline colour, so the swap changes what is tweened and the pairs
are enumerated. Every other target has no colour transition, which is stated rather than left blank.

| Element | States | Transition |
| --- | --- | --- |
| `components/admin/ShowRowActions.tsx:647` | closed to open | `transition-colors duration-fast`, unchanged; the tween now runs `text-faint` on the row ground to `text-faint` on `surface-sunken` instead of `border` to `border-strong` |
...
| `components/admin/telemetry/AutoRefreshControl.tsx:105` | OFF to ON and ON to OFF | `transition-colors`, unchanged, and the recipe is unchanged: the track is registered, not swapped |
| `components/admin/OnboardingWizard.tsx:258` | visited to done, active to done, done to active | no colour transition on the pill visual today, and none added. Instant, deliberate |
| every Family A field | rest to focus | the focus cue is a `focus-visible:ring-*`, not a border tween; unchanged in both directions |
| `components/admin/dev/SwitcherControls.tsx:119` | rest to hover and hover to rest | no `transition-colors` on this element today. The hover token moves from `accent` to `accent-on-bg` and the switch stays instant, matching its two neighbours at `components/admin/dev/SwitcherControls.tsx:29` and `components/admin/dev/SwitcherControls.tsx:145`, which are also instant |

**Compound case.** A menu trigger's open/closed tween can be interrupted by a re-render that swaps the
whole className string, because the two arms are a ternary rather than a variant. That is true today
and is unchanged by a colour swap; no arm is added or removed.
...
- **L1. Paint inside a sibling component function is invisible.** §5.2. One live instance,
  `components/admin/settings/DeveloperToggleButton.tsx:93`, which is a switch track and therefore ruled
  out anyway. Reaching it means inlining a component's JSX at its call sites. RE-FILE TRIGGER: a
  control whose only resting outline is painted inside a sibling component reaching `main` at
  `border-border` or `border-border-strong`.
- **L2. A JSX child reached through anything but a bare local identifier is not followed.** A `.map()`
  callback, a function call, a prop, an imported constant. Conservative in the safe direction: the
  child is simply not admitted, so nothing is cleared that was not already clear.
