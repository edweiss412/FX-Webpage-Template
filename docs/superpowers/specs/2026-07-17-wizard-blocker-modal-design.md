...
- **`BlockedRowResolver`** (non-rescannable codes) keeps `onResolved={() => void run.runLoop()}` — on resolve it transitions `state` to `running`, so the modal unmounts (state leaves the blocker set) and `Step3CompactTracking` takes over in the footer center.
