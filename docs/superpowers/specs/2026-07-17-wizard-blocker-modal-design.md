...
- ¹ **ENTERS:** scrim fades in via the EXISTING grounded keyframe `step3-details-scrim-in` (`app/globals.css:726-728,747-748` — opacity 0→1), applied as `motion-safe:animate-[step3-details-scrim-in_var(--duration-normal)_ease-out]`; panel rises on mobile / pops on desktop via `motion-safe:animate-[sheet-rise_220ms_cubic-bezier(0.25,1,0.5,1)]` (both keyframes already exist — no new `@keyframes`). `motion-reduce:animate-none` on BOTH scrim and panel → instant, at-rest (reduced-motion). No new `scrim-fade` keyframe is invented.
