...
| `scope-fences` lines per spec doc (both heading families, depth ≥ 2, structural lines excluded) | median 4 | max 160 |
...
1. The line is non-fenced, is not a table row (leading optional whitespace then `|`), is not a HEADING line (a heading is a label, not a claim sentence — every motivating escape is body prose; R3 F1 resolved the contract/instrument divergence in the instrument's favor, so the committed record measures the contract), and does not contain an ISO date (`\d{4}-\d{2}-\d{2}` — dated historical records are never compared; the prose-count-parity exclusion family, adopted whole).
...
**Layer-3 reconciliation (implementation).** The shipped arm's own run reproduced the 212 above, and the bounded pass found FOUR of them wrong structure class — a grouping comma is a word boundary, so `all 5,022 eligible data rows` matched "all 5" and cleared the value bound as a two-digit population. That is the §1.2 defect class, not a survivor to classify, and it was repaired by narrowing (a cardinal followed by a grouping comma or a decimal point is a fragment of a longer literal and draws silence). The committed layer-3 population after that repair was **208**. Whole-diff review R1 then found a SECOND defect of the same class — the arm evaluated only the FIRST accept-set match on a line, so a candidate-specific rejection suppressed a later qualifying claim and the line drew silence, which the consequence bound forbids. Live proof: `docs/superpowers/specs/ci/2026-08-16-modal-wait-boundary-helper-adoption-design.md:238`. The arm now tries every candidate and the first that clears every gate owns the line. Final committed layer-3 population: **209** over 454 docs. Record: `docs/superpowers/specs/probes/2026-08-17-prose-consistency-arms.survivors.txt`; classification: `docs/superpowers/specs/probes/2026-08-17-prose-consistency-arms.classification.md`.
...
- **`universal-claims`:** every non-fenced, non-table, non-heading line whose clause start matches the closed quantifier set — line start, or after a period, semicolon, or colon followed by a space; then an optional list-marker/bold prefix; then `Every|Each|All|Any|No|Never|Nothing` as a word. Exactly the instrument's measured recognizer; synonyms outside the set (e.g. "entire", "none of", "always") are a documented limit.
- **`scope-fences`:** every non-blank, non-fenced, non-structural line of a region owned by a heading of depth ≥ 2 matching `/out of scope|non-goals?/i` or `/clos(e-?out|eout)|graduation/i` (region = heading to next heading of ≤ the OPENING heading's depth; a matching heading NESTED inside an open region does not re-anchor it — R3 F2, probed: re-anchoring silently dropped 142 lines on a one-rename edit). The depth bound is an R1 repair, probe-backed: a depth-1 TITLE containing "close-out" owns its whole document (measured: 513 lines of one doc would have entered the group), and a title is a doc identity, not a fence region. Fenced lines, nested heading lines, thematic breaks, and table DELIMITER rows are EXCLUDED — fence content is code/example, a nested heading's own content lines are walked beneath it, and a `---` break or `| --- |` separator carries no claim; table CONTENT rows stay, since a fence claim can live in a table cell (R4, probe-backed: 85 thematic breaks + 16 delimiter rows were inside regions corpus-wide) (R2 F2: the contract adopts the instrument's measured semantics; the earlier "every non-blank line" wording described a population 190 lines at max that nothing had measured). Re-measured with both families at depth ≥ 2 and the structural exclusions: median 4, max 160 lines per doc (§3.1).
...
- **One advisory per line.** The arm evaluates EVERY accept-set candidate on a line and the first that clears every gate owns it, so a second QUALIFYING claim on the same line is not separately reported. The line is quoted in full, so the sweep sees both. This is a reporting bound, not a scanning one: stopping the SCAN at the first match let a rejected candidate mask a later qualifying one, which was a defect against the §1.2 consequence bound rather than a limit (whole-diff review R1), and it diverges from the instrument deliberately.
