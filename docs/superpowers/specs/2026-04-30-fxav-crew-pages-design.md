...
create table shows (
  ...
  published       boolean not null default true,    -- M11 batch-13 finding 2: row visibility gate for multi-batch wizard finalize. Default TRUE so existing live shows (rows created via live cron/push/manual Apply) are crew-visible immediately. Wizard-finalize-promoted rows are INSERTed with `published = false` during INTERIM batches (1..N-1 of a multi-batch finalize); only the FINAL batch's transaction atomically flips `published = true` for ALL rows promoted across this wizard session — in the SAME transaction as the §4.5 atomic-promotion CAS (`watched_folder_id = pending_folder_id`). This guarantees the "candidate folder is not yet the watched folder; staged rows are not yet public" invariant for batched promotion: a 250-sheet folder requiring 3 finalize calls keeps the first 200 promoted rows invisible to crew until the third call atomically publishes all 250 AND flips the watched folder. Crew read paths in §9.0.2 / Task 10.2 scope to `WHERE published = TRUE`; admin read paths (Task 10.6 dashboard, per-show parse panel) do NOT — admin needs to see in-flight finalize rows with a yellow "publishing…" badge. The `shows` RLS policy admitting non-admin session reads (matched against `crew_members.email`) ALSO carries `AND shows.published = TRUE` so a crew member whose email matches a `crew_members` row for an interim-batch show STILL gets denied. Live cron/push/manual Apply ALWAYS lands `published = true` directly (those paths target the watched folder; no interim state). The default TRUE on the column means existing rows in test fixtures and seed data continue to behave correctly without explicit migration.
...
create or replace function bump_last_changed_at_and_publish_invalidation()
  returns trigger
  language plpgsql
  security definer
...
begin
  ...
  perform pg_notify(
    ...
    json_build_object(
      ...
      'event',   'invalidate',
      'payload', json_build_object('show_id', new.show_id, 'version_token', public.viewer_version_token(new.show_id))
    )::text
  );
  return new;
end;
$$;
revoke all on function bump_last_changed_at_and_publish_invalidation() from public;
...
3. For each embedded object on the DIAGRAMS tab whose type is image-like, record `{ sheetTab: "DIAGRAMS", objectId, mimeType, alt?, snapshotPath: null }` in the `parseResult.diagrams.embeddedImages` array. The bytes themselves are **snapshotted into Supabase Storage at Phase 2 (auto-apply) or Apply (staged) time** — not at parse time. This preserves the live-vs-staged snapshot boundary: the crew page for the currently-approved snapshot serves images from the storage path baked at the last successful Apply, never from live Drive state. If Doug edits an image while a stage is pending, the live crew page continues to show the previously-approved bytes. Specifically:
   ...
   **All-or-nothing promotion with manifest verification (M7 batch-14 finding 2, HIGH — replaces the prior "partial canonical is fine" prose)**: the post-commit promoter / GC re-attempt MUST treat the canonical prefix as either fully populated or empty — a partial canonical gallery (some assets renamed, others missing) MUST NOT become live. Protocol:
   ...
   - **(P4) Promote** — run the **promote** state transition above. ONLY when this UPDATE returns 1 row, ALSO run the **atomic JSONB cutover** that advances the canonical revision: `UPDATE shows SET diagrams = jsonb_set(jsonb_set(diagrams, '{snapshot_revision_id}', diagrams->'pending_revision_id'), '{pending_revision_id}', 'null'::jsonb) WHERE id = $1 AND (diagrams->>'pending_revision_id')::uuid = $newRevId` in the same DB transaction as the promote-state-transition UPDATE. After the JSONB cutover commits, clear `promote_started_at` (set to NULL) and `promoted_at` is set; the canonical gallery is now authoritative. **The earlier `shows.pending_snapshot_path` clear is REMOVED (M7 batch-15 finding 1)** — that column no longer exists; `pending_revision_id` on the JSONB takes its place as the cutover staging slot, and the asset route never reads it.
   - **(P5) Rollback (any individual rename failure in P2 OR manifest mismatch in P3)** — for each successfully renamed `asset_key` in the running set, reverse-rename `<canonical_prefix><asset_key>` → `<temp_prefix><asset_key>`. Do NOT advance the JSONB cutover; do NOT set `promoted_at`. Then `UPDATE shows SET diagrams = jsonb_set(diagrams, '{pending_revision_id}', 'null'::jsonb) WHERE id = $1` to clear the staged-but-failed `pending_revision_id` (so a future Apply can mint a fresh `pending_revision_id` without colliding). Clear `promote_started_at` (set to NULL) and release the claim (heartbeat with `claim_expires_at = now()` so the next sweep can re-claim). The canonical prefix MUST be empty post-rollback so a subsequent GC observing it knows nothing was promoted, and `shows.diagrams.snapshot_revision_id` still points at the prior approved revision (which has kept serving uninterrupted throughout). **(P5-stuck) Reverse-rename failure contract (M7 batch-15 finding 3)** — if the reverse-rename ITSELF fails mid-flight (some assets reverse-renamed back to temp, then a subsequent reverse-rename returns 5xx / network failure), the row is left in a split-prefix state with assets straddling temp + canonical. In this case the promoter MUST: (a) NOT clear `promote_started_at` (the row stays non-reclaimable to the GC sweep); (b) NOT clear `pending_revision_id` (so the admin repair flow can identify which revision was being promoted); (c) NOT release the claim; (d) emit an `admin_alerts` row coded `PENDING_SNAPSHOT_ROLLBACK_STUCK` with `(row_id, snapshot_revision_id, assets_reverted[], assets_still_canonical[])` so Eric sees the stuck state. The reclaim-expired GC sweep refuses to act on rows with `promote_started_at IS NOT NULL`, so a stuck-rollback row is invisible to automated reclamation. Recovery is admin-only: the operator runs `/api/admin/snapshot-rollback/[id]/repair` (admin-authenticated POST) which LISTs both the canonical and temp prefixes, computes the union manifest, decides per-asset which prefix is authoritative based on the rename audit log, performs final reverse-renames of remaining canonical-side assets, then clears `promote_started_at`, clears `pending_revision_id` from the JSONB, and releases the claim. Once `promote_started_at` is NULL again, the GC sweep can route the row through (iv-a) RENAME-RETRY or (iv-b) DELETE-ORPHAN normally.
...
- **Cookie value is a JSON-encoded `{ token, show_id }` payload (URL-encoded into the `Set-Cookie` value), NOT the JWT.** `token` is the random opaque server-side session token; `show_id` is the UUID of the show this session was minted for. The `link_sessions` row lookup still happens by `token`, but the validator additionally checks that the cookie's decoded `show_id` matches both the URL's show AND the row's `show_id` column. Storing `show_id` IN the cookie payload is what enables the §7.2.2 step-4 cross-show check — the route compares `cookie.show_id === request.show_id` BEFORE the DB read so even a tampered or stale cookie cannot leak across shows. Sessions are stored in `link_sessions` keyed by the random `token`, mapping to `(show_id, crew_member_id, jwt_token_version, expires_at, last_active_at)`. Per-request authz still re-derives role from current `crew_members.role_flags`; the cookie is identity, not authorization.
- **A→B→A burn pattern is acceptable for v1.** When a user with a redeemed-link session for show A briefly navigates to show B's route, the show-A cookie's `show_id` will not match show-B's URL slug; the validator returns `{ kind: 'continue', clearCookie: true }`, the chain adapter clears the cookie, and the validator DELETEs the show-A `link_sessions` row (single canonical contract — see §7.2.2 step 4). The user must re-mint show A's session by re-opening the original signed link to access show A again. **Multi-show navigation is rare** (a single crew member usually works one show at a time); this burn cost is acceptable in v1. **Future versions (post-v1) MAY upgrade the cookie value to a JSON-encoded array of `{ token, show_id }` entries (cap N, evict oldest)** so that multi-show users do not pay the re-mint cost; the validator would scan the array for a matching `show_id`. The array upgrade is non-breaking on the wire (cookie name is unchanged) and is deferred until usage data justifies it.
- **Helper API (M5 batch-13 — `lib/auth/cookies.ts`):** `setSessionCookie(value, opts)` and `clearSessionCookie()` — neither helper takes a slug parameter; the cookie name is the literal `__Host-fxav_session`. The `value` parameter is the URL-encoded JSON payload `{ token, show_id }` (callers serialize via a shared `encodeSessionCookieValue({ token, show_id })` helper that lives alongside in `lib/auth/cookies.ts`; a paired `decodeSessionCookieValue(raw)` returns `{ token, show_id } | null` — null on JSON parse failure, missing fields, or fields of the wrong type, which the validator treats as a client-side parse fault per §7.2.2 step 1's tri-state classification). Banned-identifier audit (X.3): the literal substring `__Host-fxav_session` outside `lib/auth/cookies.ts` and `lib/auth/constants.ts` MUST fail the audit. The exact regex `/__Host-fxav_session/` (no slug suffix variant) covers all hand-rolled producers; the helpers remain the only legitimate producers.
...
create table pending_syncs (
  ...
  wizard_approved_by_email text,                       -- M11 batch-12 finding 1: admin email captured at step 5W (auth_email_canonical(session.user.email)). NULL until approved. Survives the deferred-until-finalize window so finalize-time sync_audit can attribute the approval to the operator who actually clicked Apply, not to whoever happened to click Finalize (which may be a different admin).
...
```
If the unresolved set is non-empty → `ONBOARDING_NOT_RESOLVED` 409. If empty → run the **DEFERRED-UNTIL-FINALIZE promotion sequence (M11 batch-11 finding 1; M11 batch-12 findings 1, 2, and 3 amendments; M11 batch-13 findings 1, 2, 3 amendments)**. **Phase A — pre-commit work (no DB transaction, no per-show locks; M11 batch-12 finding 3):** (A.1) `SELECT id, drive_file_id, parse_result, triggered_review_items, staged_modified_time, wizard_approved_by_email, wizard_approved_at, wizard_reviewer_choices, wizard_reviewer_choices_version, (SELECT head_revision_id FROM pending_syncs_drive_pins WHERE drive_file_id = ps.drive_file_id) AS phase_a_head_revision_id FROM pending_syncs ps WHERE wizard_session_id = $sessionId AND wizard_approved = TRUE ORDER BY drive_file_id` — deterministic alphabetical order for deadlock prevention in Phase B; every row carries the durable approval payload per §4.5 symmetry CHECK. **M11 batch-13 finding 3: any row whose `wizard_reviewer_choices_version` is NOT in the current code's supported set MUST abort Phase A immediately with `WIZARD_REVIEWER_CHOICES_VERSION_UNSUPPORTED` 409, demote that row's manifest back to `'staged'` in a separate follow-up transaction, and surface the offending `drive_file_id` to the wizard UI so the operator can re-Apply.** **M11 batch-13 finding 1: capture the per-row `binding.headRevisionId` observed during the §6.8.1 step-3 re-verify (Phase A.2) into `phaseAResults[id].binding.headRevisionId` — this is the immutable token Phase B will re-fetch and CAS-check immediately before each row's commit, closing the Phase-A→Phase-B TOCTOU window.** **M11 batch-13 finding 2: cursor-based multi-batch protocol replaces the original "409 ONBOARDING_BATCH_TOO_LARGE" cap.** The 100-row-per-call limit is preserved (Phase B advisory-lock loops longer than this risk hot-locking the live folder's cron pass), but the contract is no longer "fail and the operator must split the folder" — it is "process up to 100 rows per call, return a continuation cursor for the rest, defer the §4.5 watched_folder CAS until the FINAL batch." The full contract: (a) the SELECT applies `LIMIT 100` to the deterministic ordering by `drive_file_id`; (b) finalize accepts an optional `?after=<lastDriveFileId>` query parameter and adds `AND drive_file_id > $afterCursor` to the SELECT when present; (c) before issuing the SELECT, finalize counts total `wizard_approved = TRUE` rows for the session — if the post-`?after` count exceeds 100 by even 1, this call is an INTERIM batch; if exactly the LIMIT-bounded set covers the full remainder, this call is the FINAL batch. (d) **Interim batches** insert `shows` rows with `published = false` (the §6.8.1 step-list 4L INSERT/UPDATE branches accept a `publishVisibility` argument that finalize threads through; live cron/push/manual paths always pass `true`, wizard-finalize interim batches pass `false`, wizard-finalize FINAL batch passes `true`); they update `onboarding_scan_manifest` to `'applied'`; they DO NOT run the §4.5 atomic-promotion CAS (the watched_folder_id stays at its prior value — typically NULL on first onboarding); they return HTTP 200 with body `{ status: 'batch_complete', remaining_count: <N>, next_batch_token: '<lastDriveFileId-of-this-batch>' }`. (e) **Final batch** (`remaining_count` after this batch is 0) runs the same per-row Phase 2 work AND additionally executes a single `UPDATE shows SET published = TRUE WHERE drive_file_id IN (<every drive_file_id ever promoted under this wizard_session_id>)` (the union is computed by joining against `onboarding_scan_manifest WHERE wizard_session_id = $sessionId AND status = 'applied'` — the manifest is the durable record of every row ever promoted under this session) AND runs the §4.5 atomic-promotion CAS in the SAME transaction. (f) The wizard UI on receiving `batch_complete` automatically clicks "Promote next batch" passing the cursor (no manual operator action required for normal flows; the affordance is auto-driven for sessions where `remaining_count > 0`); the wizard UI surfaces a progress indicator ("Publishing 150 of 250 sheets…"). (g) Pre-final-batch shows rows are NOT publicly visible because the `published = FALSE` gate is enforced by the §4.3 RLS policy (`AND shows.published = TRUE` predicate added per Task 2.3) AND by every crew read path's WHERE-clause predicate (Task 10.2 / §9.0.2 / per-show data fetcher). The admin dashboard (Task 10.6) renders interim-batch rows with a yellow "Publishing…" badge so the operator sees progress. (h) If the wizard tab is closed mid-multi-batch (after batches 1..N-1 commit but before the final batch), the next finalize click resumes from the cursor — the partial state is durable: `shows` rows exist with `published = false`, manifest rows are `'applied'`, the watched_folder_id is still un-flipped. A subsequent batch picks up where the cursor left off (no `?after` defaults to "from the start of unpromoted rows" — which is unambiguous because previously-promoted rows have NO `pending_syncs` row remaining, so the SELECT naturally returns only un-promoted rows). The 100-row cap remains the per-call ceiling but is no longer a hard failure. (A.2) For each selected row in order: run the §6.8.1 step-3 mandatory Drive re-verify (parents check pinned to `pending_folder_id`); on any `STAGED_PARSE_SOURCE_GONE` / `_OUT_OF_SCOPE` / `STAGED_PARSE_SUPERSEDED`, capture the per-row failure code in memory and skip to A.3 (do NOT abort yet — Phase B does the abort under transaction). (A.3) Snapshot every approved row's diagrams + linked-folder assets to **Storage at the per-(session, drive_file_id) temporary prefix** `diagram-snapshots/_finalize-pending/<wizard_session_id>/<drive_file_id>/<asset_key>` (M7 batch-10 / M7 batch-11 finding #1; the temporary prefix is OUTSIDE the canonical `<show_id>/<snapshot_revision_id>/` keyspace so a finalize that aborts cannot leave half-uploaded blobs at a real revision prefix). Capture the resulting paths in memory keyed by `(drive_file_id, asset_key)`. **Phase B — commit transaction (short, all-or-nothing; per-row locks acquired in deterministic order; M11 batch-12 finding 2 manifest lifecycle):** open a single `withShowSyncTransaction`. (B.1) If Phase A.2 captured ANY per-row failure code, abort with that code now (ROLLBACK), then in a separate follow-up transaction UPDATE the offending manifest row(s) back to `'staged'` per the M11 batch-12 finding 2 manifest-lifecycle rule above, and return 409 to the operator. (B.2) For each row in the SAME alphabetical order Phase A used, acquire `pg_advisory_xact_lock(hashtext('show:' || $row.drive_file_id))` (blocking; deterministic alphabetical order prevents deadlocks across concurrent finalize clicks against the same session). **(B.2.pre) MANDATORY pre-commit Drive head re-verify (M11 batch-13 finding 1).** IMMEDIATELY after acquiring the per-row advisory lock and BEFORE running any §6.8.1 Phase-2 SQL, re-fetch `files.get(row.drive_file_id, fields='headRevisionId,modifiedTime,trashed,parents', supportsAllDrives: true)` from inside the lock. CAS-check the re-fetched `headRevisionId` against `phaseAResults[row.drive_file_id].binding.headRevisionId` (captured during Phase A.2). On mismatch → abort that row with `STAGED_PARSE_REVISION_RACE_DURING_FINALIZE` (§12.4 — new code). The Phase B transaction proceeds for the OTHER rows in this batch (the abort is per-row, not per-batch); the raced row's manifest STAYS `'staged'` (the wizard manifest UPDATE for THIS row is skipped — the manifest demotion happens inline because we abort the row before its `'applied'` write fires); finalize completes the unaffected rows. The operator must re-Apply the raced row before the next finalize click; the per-row failure list is returned in the response body so the wizard UI can surface which file(s) need re-Apply. **CAS check is REQUIRED — without it, mid-flight Drive edits between Phase A.2 (re-verify) and the per-row commit silently promote stale data.** ALSO re-check `trashed === false` AND `parents` still contains `pending_folder_id`; failure on either → emit the existing `STAGED_PARSE_SOURCE_GONE` / `STAGED_PARSE_SOURCE_OUT_OF_SCOPE` per §6.8.1 step 3 semantics with the same per-row abort behavior. Run the §6.8.1 step-list 4L → 5L → 6L Phase-2 + sync_audit + DELETE flow against `parse_result`, **passing `wizard_reviewer_choices` as the choices payload** AND **passing `wizard_approved_by_email` + `wizard_approved_at` as the sync_audit attribution fields**. The Phase 2 UPDATE on `shows.diagrams` references the snapshot paths from Phase A.3 — for each diagram entry, set `snapshotPath` to the canonical `<show_id>/<snapshot_revision_id>/<asset_key>` location. **(B.2.a) Storage move from temporary to canonical prefix (M11 batch-12 finding 3):** within the per-row sub-statement, after Phase 2 has determined the freshly-minted `snapshot_revision_id`, issue Storage move/copy operations from `_finalize-pending/<wizard_session_id>/<drive_file_id>/<asset_key>` to `shows/<show_id>/<snapshot_revision_id>/<asset_key>` for every captured asset; if any move fails, the per-row Phase 2 raises a Storage error and the whole transaction rolls back per Phase B abort semantics below. The same per-row sub-statement also UPDATEs `onboarding_scan_manifest SET status = 'applied' WHERE wizard_session_id = $sessionId AND drive_file_id = $row.drive_file_id` (manifest-lifecycle invariant — this UPDATE commits with the `shows` write, never before). (B.3) Run the §4.5 atomic promotion CAS (`watched_folder_id = pending_folder_id`, etc.). (B.4) Commit. **If ANY per-row Phase 2 raises an unexpected error inside Phase B, abort the entire transaction (ROLLBACK), then in a separate follow-up transaction UPDATE the offending manifest row to `'staged'` per the manifest-lifecycle rule, then run the M11 batch-12 finding 3 temp-prefix cleanup: DELETE every Storage object under `_finalize-pending/<wizard_session_id>/` so orphan temp blobs never accumulate. Return 409 with the per-row failure code.** **Phase C — post-commit (M11 batch-12 finding 3 cleanup of superseded temp prefixes):** after Phase B commits, call `subscribeToWatchedFolder(folderId)` AND issue a best-effort Storage DELETE under the `_finalize-pending/<wizard_session_id>/` prefix to remove any temp blobs left behind for sheets that were superseded during Phase A.2 re-verify. The watch channel registration is non-transactional — the folder ID is now durable in `watched_folder_id` so a webhook race can do no harm. The cleanup is best-effort: a Storage DELETE failure here logs a `sync_log` entry coded `finalize_temp_prefix_cleanup_failed` (with `payload = { wizard_session_id, error }`) but does NOT affect the success of the finalize itself; a Task 7.8 GC sweeper consumes any leftover `_finalize-pending/*` prefixes older than 24 hours as a backstop. The manifest is the SOLE finalize gate — row absence in `pending_*` tables is NOT a substitute (the `discard_retryable` Discard variant deletes pending_syncs with no deferral row, which row-absence inference would silently treat as resolved). **Resolving a `'live_row_conflict'` row requires the operator to (a) resolve the underlying live `pending_syncs` / `pending_ingestions` row from the dashboard, then (b) re-run the wizard so a fresh `runOnboardingScan` re-attempts the per-file UPSERT and either succeeds (manifest transitions to `'staged'` / `'hard_failed'` / etc.) or re-raises the conflict (manifest stays at `'live_row_conflict'`).**
...
| `CSRF_NONCE_EXPIRED` | M5 batch-15 finding 2: `/api/auth/redeem-link` POST decoded its `__Host-fxav_bootstrap_v` cookie but the matching `(nonce_hash, show_id)` entry was no longer present in the cookie array — the entry was age-evicted (older than 30s) by the bootstrap-render path, OR the corresponding `bootstrap_nonces` row was already DELETEd at age-eviction time. Distinct from `CSRF_DENIED` (which signals an attacker-forged or cross-site POST): this code signals "you raced too many tabs in <30s" — a benign UX condition. The wizard / sign-in surfaces SHOULD render a friendly retry message ("Please refresh and click the link again — your bootstrap window expired") instead of the generic CSRF failure. 403 response. | "Your sign-in link expired before you finished. Refresh the page and click the link again." | — | Doug → refresh and re-click the link |
...
| `BRANCH_PROTECTION_DRIFT` | X.1-X.6 batch-15 finding 1: the `scripts/verify-branch-protection.ts` workflow step queried GitHub branch protection / rulesets API for the `main` branch and found that one or more of the six required X.* status checks (`traceability-audit`, `x1-catalog-parity`, `x2-no-raw-codes`, `x3-trust-domain`, `x4-no-global-cursor`, `x5-rls-coverage`) is NOT in `required_status_checks.contexts`, OR `required_pull_request_reviews.required_approving_review_count < 1`, OR `enforce_admins != true`. The verification runs weekly (cron) AND on every workflow run; drift detected → workflow fails + admin alert emitted. Without this verification, the manual one-time branch-protection admin step can be skipped or later reverted, leaving the X.* suite advisory. | (admin log only — operator + Eric must restore branch protection) | — | Eric → restore branch protection per X.6 contract |
...
| `OAUTH_REDIRECT_INVALID` | M11 batch-11 finding 4 (canonical row, post-M9+M10 batch-12 dedup): Supabase Google OAuth callback's `redirect_to` parameter doesn't match the allow-list of valid post-sign-in destinations (the canonical site origin + `/show/<slug>`, `/admin`, or `/me` paths). Open-redirect defense — without this guard, an attacker could trick the auth round-trip into landing on a malicious origin while the session cookie is still being minted. **Implementation detail (preserved from the retired M5 batch-11 operator-log-only variant; regex fixed M5 batch-12; alternation extended M5 batch-14 finding 1):** `/auth/sign-in` and `/auth/callback` reject any `next` parameter that doesn't match `^\/(show|admin|me)(\/[^?#]*)?$` (rejects protocol-relative, absolute, or off-app targets); on rejection the route falls back to `/admin` and logs the offending value verbatim. | "Sign-in landed somewhere we don't recognize. Please click the original link from Doug again to start over." | "Sign-in landed somewhere we don't recognize. Please click the original link from Doug again to start over." | Crew → reopen the link; Eric if persistent |
...
OAUTH_REDIRECT_INVALID: "The Google OAuth callback's `redirect_to` parameter pointed somewhere outside the allowed list of post-sign-in destinations (the canonical site origin + `/show/<slug>` paths). Without this guard, an attacker could trick the round-trip into landing on a malicious origin while we were still minting your session cookie. Click the original link from Doug again."
...
- **M5 batch-15 finding 3 (malformed-cookie envelope tests) — RESOLVED:** Plan Task 5.2 step 1's malformed-cookie regression-test enumeration has been rewritten around the URL-encoded JSON envelope `{ token, show_id }` (M5 batch-13 wire format). Retired JWT-shape fixtures (`malformed-cookie-wrong-issuer`, `-wrong-audience`, `-expired-signature`, `-not-base64`, `-missing-claims`, `-legacy-format`, `-truncated`, `-decode-failure`) are removed; the corrected enumeration covers `decodeSessionCookieValue` failure modes (empty string, `%`-decode failure, `{}`, `[]`, `null`, top-level string, missing/null/non-UUID `token` or `show_id`, oversize > 256 bytes, control chars) PLUS the missing-cookie-entirely case (NOT a parse fault — `continue` without `clearCookie:true`). Each parse-fault case MUST resolve to `{ kind: 'continue', clearCookie: true }` per §7.2.2 step 1's tri-state classification. The `decodeSessionCookieValue(raw): { token: string, show_id: string } | null` helper contract is now spelled out in the plan with the 9-step short-circuit implementation order (length cap → `decodeURIComponent` → `JSON.parse` → object/array discriminator → strict UUID-typed `token` AND `show_id` → strict shape with no extra fields). Helper exported from `lib/auth/cookies.ts` (Task 5.4 + Task 5.7).
...
- **X.1-X.6 batch-15 finding 1 (verify-branch-protection script):** `scripts/verify-branch-protection.ts` queries GitHub branch-protection API, asserts all 6 X.* checks present + `enforce_admins=true` + `required_approving_review_count >= 1`; runs weekly cron + on every workflow run; emits `BRANCH_PROTECTION_DRIFT` alert on drift.
...
The §12.4 catalog rows (`WIZARD_FINALIZE_UNRESOLVED_ROWS`, `CSRF_NONCE_EXPIRED`, `PENDING_SNAPSHOT_PROMOTE_STUCK`, `PENDING_SNAPSHOT_ROLLBACK_STUCK`, `BRANCH_PROTECTION_DRIFT`) and the `pending_snapshot_uploads.promote_started_at` column ARE landed in this commit so the catalog stays consistent while plan-level work proceeds.
