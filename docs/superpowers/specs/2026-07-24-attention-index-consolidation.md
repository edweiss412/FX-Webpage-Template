...
The only needs-look codes with an *internal* action are four: `PARSE_ERROR_LAST_GOOD`, `RESYNC_QUALITY_REGRESSED` (`lib/adminAlerts/alertActions.ts:169-170`), `SHOW_UNPUBLISHED` (`lib/adminAlerts/alertActions.ts:171`), and `RESYNC_SHRINK_HELD` (`lib/adminAlerts/alertActions.ts:141-149`). Each must be shown chip-less. **As shipped, the `external` gate alone does that** — a chip requires `action.external`, so no internal action can produce one. The per-code analysis below is the DERIVATION that made the external-only rule safe to adopt; it is not a description of a runtime guard (see the note after the table).

| Code | Warnings section available | Warnings section unavailable |
| --- | --- | --- |
| `PARSE_ERROR_LAST_GOOD` | Notes channel intercepts before the card path (`lib/admin/parseAttentionNote.ts:23`, `lib/admin/sectionAttention.ts:116-121`) — no card, so no chip | Falls **through** to the card path and lands in Overview (`lib/admin/sectionAttention.ts:111-128`, pinned by `tests/admin/bucketAttention.test.ts:64-71`). Card is in Overview, action targets `#overview`, so a chip pointing there would have pointed at itself |
| `RESYNC_QUALITY_REGRESSED` | Same as above | Same as above |
| `SHOW_UNPUBLISHED` | Card in Overview, action `#overview` — would have pointed at itself | Unchanged |
| `RESYNC_SHRINK_HELD` | Card in Overview, action `#overview` — would have pointed at itself | Unchanged |

An earlier draft derived external-only from "the notes codes never render a card." **That premise is false in the warnings-unavailable state**, where the router deliberately falls through to Overview so nothing is dropped. The invariant survives because `resolveEffectiveSection` returns Overview in that state (`lib/admin/sectionAttention.ts:70-85`) and both note-code actions target Overview — so even in the reachable fallback state, an internal chip would have pointed at the card's own section. That is what made external-only safe to adopt as the single rule.
...
| `a.action` | internal, any target section | No chip — the chip requires `action.external` (§2.3) |
...
14. **Layout (real browser), across TWO harnesses.** Playwright assertions for every §5 invariant. The split is forced, not stylistic: the three row/panel invariants require an OPEN menu, and the static-render layout harness (`tests/e2e/published-review-modal.layout.spec.ts:102-181`) serves server-rendered HTML with no hydration, so no click-mounted subtree exists there to measure. Those three — each needs-you row's height ≥ 44px, the scroll region clipping at `max-h-96` with 12 needs-you rows, and a long title not widening the panel past `w-[min(400px,calc(100vw-32px))]` — belong on the live hydrated harness (`tests/e2e/attention-pill-focus.spec.ts`, which bundles `_pillFocusLiveEntry.tsx` and exposes `window.__setItems`). The pill's resolved tap band ≥ 44px is already asserted on the static harness (`tests/e2e/published-review-modal.layout.spec.ts:569-623`) and is re-verified there under the new pill string rather than duplicated.
