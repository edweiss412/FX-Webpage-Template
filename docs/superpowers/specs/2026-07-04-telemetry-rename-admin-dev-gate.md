...
- Update every `@/components/admin/observability/*` and `@/lib/admin/observabilityTypes` import across the tree to the new paths.
...
- **Acceptance:** after the rename, `git grep -n observability -- app/ components/ lib/ scripts/ tests/` returns ZERO matches except any explicitly-commented legacy-compat key. Add this grep as the final rename-verification step.
