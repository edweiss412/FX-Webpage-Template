...
5. **`tests/e2e/admin-phase2-surfaces.spec.ts` (`:34-37` global `a[href*='/admin/dev']` rejection; the `/admin/settings` case `:66-78`)** (Codex R3 MED) — narrow the assertion to ALLOW `/admin/dev/telemetry` (the moved Diagnostics link, visible to a developer) while still forbidding `/admin/dev`, `/admin/dev/source-link-dim`, `/admin/dev/telemetry-dim`.
6. **`playwright.config.ts` comments (`:10,13,99,194,196,225,267,291`)** — prose stating "artifact must not contain `app/admin/dev/*`" / "`/admin/dev` is 404 in prod" is now partially false (telemetry is the exception). Update the affected comments for accuracy (non-functional, but avoids misleading the next reader; carry as part of this task, not a separate one).
7. **`tests/auth/developerGatingContract.test.ts` (`:110` `observability-dim` registry row)** → `telemetry-dim`. The telemetry PAGE is covered by enforcement-3 PROTECTED_ROUTES (renamed §2.5); confirm no page-registry row needs adding (the dim harnesses are the registered dev-only pages; telemetry page's coverage is via PROTECTED_ROUTES).
...
- **Full audit-gate sweep at close-out:** x3-trust-domain (route rename + PROTECTED_ROUTES), developerGatingContract (enforcement 2 flip + telemetry-dim registry rename), advisoryLockRpcDeadlock (new migration), validation-schema-parity (Part-B migration applied to validation), x1 (no §12.4 change expected), quality/typecheck (import rename correctness), **build-artifact-gate** (prod artifact now expects `/admin/dev/telemetry` present + the others absent — §2.1a.3), **no-raw-codes** (telemetry now crawled — §2.1a.4), and the NEW **`dev-route-prod-classification`** structural guard (§2.1a) that pins the prod-vs-dev-only `/admin/dev/*` classification.
