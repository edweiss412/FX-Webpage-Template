...
3. For each `{ header, headerLine, admit }` in `boVenue` **where `admit === true`**:
   ...
   - `if (!roomHasContent(room)) continue;` (mirrors rooms.ts:1054/1116). Note `roomHasContent` counts `dimensions`/`floor` (rooms.ts:687-703), so a header with dims but empty BO fields IS kept as a name+dims room — the same contract as the DAY-range loop (rooms.ts:1112-1116); only a `&#10;`-evidenced header with neither dims nor any field value is dropped.
...
| Header valid, dims present, but its BO fields are all empty | **emits a room with name+dims only** — `roomHasContent` counts `dimensions`/`floor` as content (rooms.ts:687-703), so this is intentionally kept, **consistent with the DAY-range loop** which likewise emits a dims-only room (rooms.ts:1112-1116). Not a phantom: the BO anchor proves the room exists |
| Header valid via `&#10;`-evidence only (NO dims token) AND all BO fields empty | dropped by `!roomHasContent` (no dims, no fields → nothing to show) |
...
5. A second dims-only breakout `MERIDIAN&#10;40' x 30'` + BO block placed **immediately** below case 1's block with **no** blank separator → both `SALON ABCD` and `MERIDIAN` parse with their **own** fields (proves the `extraTerminators` fix for admitted→admitted adjacency, §3.2 step 2; without it SALON ABCD would steal MERIDIAN's fields).
6. A dims-only breakout `ORCHID&#10;50' x 40'` + `BO Setup | orchid-setup`, placed **immediately** (no blank) above a **rejected** `| PROJECTOR CART | 3' x 4' |` (`label|value`) + `| BO Setup | cart-setup |` → `ORCHID` parses with `setup === 'orchid-setup'` (NOT `cart-setup`), and `PROJECTOR CART` is **not** a room. Proves a REJECTED header (`admit=false`) still enters `extraTerm` and terminates the prior admitted block — a `filter(h=>h.admit)` `extraTerm` would leak `cart-setup` into ORCHID (field theft). Added at plan-review round 1 (Codex plan HIGH2).
...
1. **`findBoBlockVenueHeaders` unit** — assert on the **`admit` flag**, not raw presence (rejected headers ARE recorded, as terminators): a dims-only `SALON ABCD` cell above a BO block → one record with `admit === true`; a `label|value` asset pair directly above a BO block → one record with `admit === false` (present so it terminates the prior block, but not a room); a `RPAS BREAKOUT 2\nLASALLE B\n30' x 25'` ownership header → `admit === false` (substring gate); a DAY-range header → `admit === false`; a legit venue named `BO BALLROOM\n40' x 30'` above a BO block → `admit === true` (the recognized-field-label skip must NOT swallow it); two BO field rows (`BO Setup` + `BO Set Time`) under one header → a **single** record (dedup by resolved header line). Across all cases assert `filter(h => h.admit)` contains exactly the intended rooms. *Catches: asset fabrication, show-prefixed-BREAKOUT/DAY double-emit, over-broad BO-row skip (HIGH3), N-fold header duplication.*
2. **End-to-end parse of the synthetic fixture** — `parseSheet(fixture).rooms`: exactly one `SALON ABCD` breakout (dims `60' x 45'`, right fields); one `MERIDIAN` breakout with its **own** fields (admitted→admitted adjacency, no field theft); one `ORCHID` breakout with `setup === 'orchid-setup'` and `setup !== 'cart-setup'` (case 6 — a REJECTED asset header still terminated ORCHID's extraction); no `PROJECTION SCREEN`/`RISER`/`PROJECTOR CART` room; the `GRAND HALL` DAY breakout present exactly once. *Catches: the whole admit path + adjacent-header termination (admitted AND rejected next-header) + no-double-emit + asset exclusion, end to end.*
...
- `5` tests (§7). `6` synthetic-fixture cases (§6) — cases 1–5 plus case 6 (rejected-header-terminator regression, added at plan review). Guard table (§4) enumerates every abandon branch.
