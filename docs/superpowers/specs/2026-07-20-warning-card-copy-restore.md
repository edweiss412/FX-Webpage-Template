...
Membership was hand-audited from every ParseWarning emission site in `lib/`. A mechanical scan (§3.5.4) serves as a TRIPWIRE against future drift, not as proof of completeness. It matches, in either order, `severity: "warn"` (tolerating a trailing `as const`) and `code: "X"` separated by a run of NON-`}` characters (`[^}]{0,600}`) — the no-closing-brace constraint keeps both fields inside ONE object literal, so the scan cannot pair fields from neighboring emissions — plus multiline `warn("CODE"` ParseWarning-helper calls. The `as const` tolerance exists because the audit's own first-pass pattern missed `UNKNOWN_DAY_RESTRICTION` (`lib/parser/blocks/crew.ts:397-403`, `severity: "warn" as const`) for exactly that reason; the meta-test self-check pins that code so the tolerance can never silently regress. `log.warn` telemetry calls (e.g. `PICKER_SELECTION_RESET_INFRA_FAILED`, `lib/auth/picker/resetCrewMemberSelection.ts:32`) are NOT ParseWarnings and are excluded by requiring the object-literal/`warn()` ParseWarning shapes. Known residual blind spots, accepted and documented in the test header: spread/variable-supplied `code` or `severity`, aliased warning helpers, and producers outside `lib/` — the tripwire narrows the drift class, the registry remains the source of truth.
...
4. Drift tripwire (best-effort, NOT a completeness proof — see §3.1): the producer scan runs over `lib/` at test time; every discovered code must be in `WARNING_CARD_COPY_CODES` (or the test's explicit non-ParseWarning exclusion list with a reason). Self-checks pin all four matched shapes so pattern rot is loud, mirroring `tests/cross-cutting/codes.test.ts:131-135`: `STAGE_WORD_AUTOCORRECTED` (object-literal, `severity` before `code`, plain), `UNKNOWN_DAY_RESTRICTION` (object-literal, `severity: "warn" as const` — pins the `as const` tolerance), `VENUE_GEOCODE_UNRESOLVED` (object-literal, `code` before `severity`), and `AGENDA_PDF_UNREADABLE` (multiline `warn()` helper). Each self-check asserts the code is discovered by the scan, and the scan's per-match span is asserted to contain no `}` (single-literal proof). The test header documents the accepted blind spots from §3.1.
...
| 32 | PULL_SHEET_ON_ARCHIVED_TAB | We found a PULL SHEET on a tab that looks like an older copy, so we left it out to avoid mixing old gear in. If it really is this show's gear, this card's controls let you include it. | Appears when a PULL SHEET is found on a tab that looks like an older copy of the sheet, not its main tab. |
