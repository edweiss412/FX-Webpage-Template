...
Membership was derived by a three-shape producer scan over `lib/` (object-literal `severity: "warn"` within 100 chars of `code:` in either order; multiline `warn("CODE"` ParseWarning-helper calls) plus a hand audit of the misses (`UNKNOWN_DAY_RESTRICTION` at `lib/parser/blocks/crew.ts:399` sits outside the 100-char window). `log.warn` telemetry calls (e.g. `PICKER_SELECTION_RESET_INFRA_FAILED`, `lib/auth/picker/resetCrewMemberSelection.ts:32`) are NOT ParseWarnings and are excluded.
...
`HoverHelp` gains an opt-in prop `compactTrigger?: boolean` (default false — every existing caller unchanged). When true, the custom-trigger button drops `min-h-tap-min min-w-tap-min` for `relative size-[22px]` plus a transparent `before:absolute before:-inset-[11px]` overlay — the same overlay pattern the default trigger already uses (`HoverHelp.tsx:204`), preserving the 44px tap-target floor (22 + 11 + 11 = 44) with zero layout inflation. `CompactAlertHelp` passes `compactTrigger` and drops the inner span's own `size-[22px]` grid box in favor of the button owning the 22px box (glyph centered). The Drive-health badge and every other custom-trigger caller keep the 44px box path.

Result: the `?` sits on the title line; the message row returns to mock height. With the inline guidance line present the row is content-sized, not trigger-sized.
...
4. Fails-by-default discovery: the three-shape producer scan from §3.1 runs over `lib/` at test time; every discovered code must be in `WARNING_CARD_COPY_CODES` (or the test's explicit non-ParseWarning exclusion list with a reason). Self-check pins that the scan finds `STAGE_WORD_AUTOCORRECTED` (object-literal shape) and `AGENDA_PDF_UNREADABLE` (multiline `warn()` helper shape) so pattern rot is loud, mirroring `tests/cross-cutting/codes.test.ts:131-135`.
...
| 14 | PULL_SHEET_PARSE_PARTIAL | Some pull-sheet rows have a QTY we couldn't read (a word, or a range like '1-2'), so those rows show their original text. Use Report if you'd like the format supported. | Appears when a pull-sheet QTY cell isn't a plain number. |
| 15 | PULL_SHEET_UNKNOWN_VARIANT | We read this case's rows but couldn't identify the column layout, so we used the usual one. Check that quantities, item names, and categories look right. | Appears when a pull-sheet's columns don't match any layout we know for certain. |
...
| 23 | UNKNOWN_DAY_RESTRICTION | This crew member is marked day-restricted (`***`) but the sheet doesn't say which days, so their schedule shows 'days unconfirmed'. Add the days to the name cell, like `(6/24 and 6/26 ONLY)`. | Appears when a name carries the `***` marker but no days are listed. |
| 24 | UNKNOWN_FIELD | Your sheet has a row we didn't recognize; we kept it as-is and nothing on the crew page is affected. Report it if you want it handled; otherwise you can ignore this. | Appears when a row's label doesn't match anything we know how to show. |
| 25 | UNKNOWN_ROLE_TOKEN | One of this crew member's role labels isn't one we recognize, so we left it off their page instead of guessing. If the label is correct, you can add it right from this warning. | Appears when a role label in a crew cell isn't on the known-roles list. |
| 26 | UNKNOWN_SECTION_HEADER — title: **"Section we didn't recognize"** | A header in your sheet isn't a section we know, so the rows under it aren't shown on the crew page. Rename it to a standard section, or Report it if it should be supported. | Appears when a header row doesn't match any section we know. |
...
| 31 | AGENDA_SCHEDULE_TIME_ADJUSTED | We corrected at least one agenda session time that looked like a typo, like a morning session marked PM. Open the agenda to confirm, and fix the source cell if the original was wrong. | Appears when an agenda time only makes sense with its AM/PM flipped. |
| 32 | PULL_SHEET_ON_ARCHIVED_TAB | We found a PULL SHEET on a tab that looks archived, so we left it out to avoid mixing old gear in. Include it from the review panel if it really is this show's gear. | Appears when a PULL SHEET sits on an older or renamed tab instead of the sheet's current body. |
| 33 | PULL_SHEET_OVERRIDE_CONTENT_CHANGED | An archived-tab pull sheet you'd included has changed since you accepted it, so we set it back to skipped rather than publish gear you haven't reviewed. Re-check the tab and include it again if it's still right. | Appears when the contents of an included archived-tab pull sheet change. |
...
| Emphasis markers in copy | — | Both fields render through `renderEmphasis`; no literal `*`/`_` reaches the DOM (pinned by `tests/messages/_metaEmphasisRenderContract.test.ts`). |

## 6. Dimensional invariants / transition inventory

- No fixed-dimension parent is introduced or modified; the card is content-sized flex-column. No layout-dimensions task required. The one geometry claim — the compact trigger's 44px hit area from a 22px box + 11px overlay — is asserted in a real browser via getBoundingClientRect on the button box plus a click at the overlay corner (hit-target test, §7).
- No new visual states. The popover open/close transition (`transition-discrete` + `starting:opacity-0`, `HoverHelp.tsx` body classes) is untouched. Transition inventory: N/A — no state pairs added.
...
- Trigger geometry: real-browser assertion (existing step3/standalone Playwright harness family) that the trigger button's border box is 22×22 (±0.5px) and the card message row height with a one-line title + one-line guidance is < 60px; jsdom is not sufficient for either.
<!-- spec-lint: ignore — new file created by this spec; not yet tracked -->
...
1. Catalog + §12.4 lockstep commit: condensed `helpfulContext` × 39 into master spec §12.4 list + `pnpm gen:spec-codes` regen + `lib/messages/catalog.ts`; same commit adds `triggerContext` (catalog-only) and the three new titles.
2. Registry + meta-test commit (test-first: meta-test lands red against a stub registry, green once copy lands — ordering per plan).
3. Adapter + geometry commits (test-first).
...
- Any `admin_alerts` code, `dougFacing` copy, bell/AttentionBanner/HealthAlertsPanel surfaces (concurrent session).
- `longExplanation` and the help-pages copy it feeds (the helpHref error anchors) — unchanged this sweep.
- `HoverHelp` default-trigger callers and non-card custom-trigger callers (Drive-health badge).
