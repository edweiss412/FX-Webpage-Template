...
New file `tests/messages/warningCardCopyRegistry.ts` (test-tree module — nothing in `lib/` or `components/` reads it; it is the meta-test's expected-membership fixture plus the frozen expected-copy table):

```ts
export const WARNING_CARD_COPY_CODES: ReadonlySet<string> = new Set([...]) // the 39 below
export const EXPECTED_TRIGGER_CONTEXT: Readonly<Record<string, string>> = { ... } // §4.2 popover column, byte-for-byte
export const EXPECTED_TITLE_CHANGES: Readonly<Record<string, string>> = { ... } // rows 10, 21, 26, 36
```

Membership was hand-audited from every ParseWarning emission site in `lib/` (per-code emitter citations in §4.3). Four review rounds established that every STATIC discovery mechanism proposed for "new warn producer must have card copy" is unsound at this codebase's scale: severity-aware regexes cannot prove single-literal field pairing, and a severity-blind `codeProducerLiterals()` partition over `lib/parser|sync|drive` measures 42 excluded codes on today's tree (breadcrumb/status `code:` fields, info-severity warnings, non-warning structures) — an exclusion table that large rots immediately and cannot be meaningfully audited. Static discovery is therefore DESCOPED (three-round cap: descope the vector rather than patch it again). Drift protection is two-layer instead (§3.5.4): a BEHAVIORAL corpus oracle for parser-side codes, and the existing reviewed new-code checklist for the rest. A new warn code that slips both layers degrades to today's exact shipped behavior — title-only card, no guidance, no popover (§5 unknown-entry row) — never a crash or a raw code.
...
4. Corpus oracle (behavioral, parser-side): parse the committed fixture corpus (`fixtures/shows/raw/*.md`, the same corpus the existing corpus meta-tests walk — e.g. `tests/parser/sourceAnchorsCorpus.test.ts`), collect every emitted `severity: "warn"` `ParseWarning`, and assert each code ∈ `WARNING_CARD_COPY_CODES`. No regex, no exclusion table — the oracle observes real emissions, so a NEW parser warn code exercised by any fixture fails by default (scope stated precisely: fails-by-default for corpus-exercised parser codes; NOT a general producer scan). Pattern-rot guard: the OBSERVED corpus warn-code set is frozen as an explicit fixture (`EXPECTED_CORPUS_WARN_CODES` in the registry module, measured at implementation and recorded in the plan) and the oracle asserts SET EQUALITY — a parser change that starts or stops emitting any warn code on the corpus fails loudly and the fixture update is a reviewed diff. This preserves per-code coverage identity, not just a count. Sync/enrichment producers (12 of the 39) are not corpus-reachable; they are covered by layer two: this spec adds one line to the AGENTS.md §12.4 new-code checklist — "a new warn-severity ParseWarning code also gets a `WARNING_CARD_COPY_CODES` row + §4.2-style copy" — making registry membership part of the same reviewed lockstep that already governs every new code.
5. Copy-fidelity freeze (verbatim contract): the meta-test embeds a frozen expected-copy fixture — the 39 `triggerContext` strings and the four changed titles copied byte-for-byte from §4.2 — and asserts catalog equality. `helpfulContext` needs no freeze here: the §12.4 lockstep + x1 parity gate already pin it. Swapped, placeholder, or drifted popover/title copy now fails mechanically; the fixture is the spec table's enforcement arm, updated only by editing this spec. The test header documents the accepted blind spots from §3.1.
...
- Dimensional invariants: the card itself stays content-sized flex-column, but the compact trigger button IS a new fixed-dimension parent — `size-[22px]` with one child (the `?` glyph span). Parent→child relationship and guaranteeing classes, per the project rule: button (22×22, fixed) → glyph span, centered by `grid place-items-center` on the button. Rendered proof required (class presence is not sufficient): the browser assertion below checks the glyph's bounding-box center sits within ±1px of the button's center. No layout-dimensions task required. The compact trigger's invariants are named explicitly: glyph centering via `grid place-items-center` on the 22px button; title-line alignment per §3.4's vertical placement contract; 44px usable hit region via the `before:-inset-[11px]` overlay, asserted in a real browser with `document.elementFromPoint` probes at all four overlay corners (±21px from the button center) each resolving to the trigger button — which also catches ancestor clipping or overlap interception that a pure border-box measurement would miss.
...
1. Copy commit (test-first): author `tests/messages/_metaWarningCardCopy.test.ts` FIRST. The registry constant and the frozen expected-copy fixture are TEST INPUTS (fixture data, like any expected-value table), not production implementation — the only production surface under test is the catalog. The meta-test reads catalog entries through an untyped `Record<string, unknown>` view of `MESSAGE_CATALOG`, deliberately, so its first run against the untouched catalog fails BEHAVIORALLY — 39 missing `triggerContext` strings, three null titles, one stale title, over-cap `helpfulContext` rows, banned-word hits — not as a compile error on a property `MessageCatalogEntry` does not yet declare. That red run is executed and its output quoted in the task log (TDD invariant 1). Then, in the SAME commit, land the implementation: condensed `helpfulContext` × 39 into master spec §12.4 list + `pnpm gen:spec-codes` regen + `lib/messages/catalog.ts` (including the `MessageCatalogEntry.triggerContext` field declaration) + the four title changes + the AGENTS.md §12.4-checklist line ("a new warn-severity ParseWarning code also gets a `WARNING_CARD_COPY_CODES` row + §4.2-style copy") — the declared drift defense for the 12 corpus-unreachable sync codes ships in the same commit as the registry it defends — turning the meta-test green. One commit so the suite (including the x1 parity gate) is never red at a commit boundary.
