...
Membership was hand-audited from every ParseWarning emission site in `lib/`. A mechanical scan (§3.5.4) serves as a TRIPWIRE against future drift, not as proof of completeness: it matches object-literal emissions with `severity: "warn"` and `code: "X"` within 600 characters in either order, plus multiline `warn("CODE"` ParseWarning-helper calls. The 600-char window was sized so the widest known emission (`UNKNOWN_DAY_RESTRICTION`, `lib/parser/blocks/crew.ts:399`, whose fields span past 100 chars) matches — and the meta-test self-check pins exactly that code so the window can never silently shrink below the widest real shape. `log.warn` telemetry calls (e.g. `PICKER_SELECTION_RESET_INFRA_FAILED`, `lib/auth/picker/resetCrewMemberSelection.ts:32`) are NOT ParseWarnings and are excluded by requiring the object-literal/`warn()` ParseWarning shapes. Known residual blind spots, accepted and documented in the test header: spread/variable-supplied `code` or `severity`, aliased warning helpers, and producers outside `lib/` — the tripwire narrows the drift class, the registry remains the source of truth.
...
4. Drift tripwire (best-effort, NOT a completeness proof — see §3.1): the producer scan runs over `lib/` at test time; every discovered code must be in `WARNING_CARD_COPY_CODES` (or the test's explicit non-ParseWarning exclusion list with a reason). Self-checks pin all three matched shapes so pattern rot is loud, mirroring `tests/cross-cutting/codes.test.ts:131-135`: `STAGE_WORD_AUTOCORRECTED` (object-literal, `severity` before `code`), `UNKNOWN_DAY_RESTRICTION` (object-literal, `code` before `severity`, the widest-span emission — pins the 600-char window), and `AGENDA_PDF_UNREADABLE` (multiline `warn()` helper). The test header documents the accepted blind spots from §3.1.
...
| 32 | PULL_SHEET_ON_ARCHIVED_TAB | We found a PULL SHEET on a tab that looks like an older copy, so we left it out to avoid mixing old gear in. If it really is this show's gear, this card's controls let you include it. | Appears when a PULL SHEET is found on a tab other than the sheet's main tab. |
| 33 | PULL_SHEET_OVERRIDE_CONTENT_CHANGED | A pull sheet you'd chosen to include has changed since you last looked at it, so we set it back to left-out rather than publish gear you haven't seen. Check the tab, then include it again from this card if it's still right. | Appears when the contents of an included archived-tab pull sheet change. |
...
Control-reference rule: rows 14, 24, 25, 26, 32, 33 name a control that renders on that code's card (Report / Ignore / role-add / include). The plan MUST verify per code which controls actually render (`DataQualityWarningControls`, `RoleRecognizeControlBoundary`, `UseRawControlBoundary` all self-hide by code) and adjust any row whose named control does not render on its card.
