...
4. Corpus oracle (behavioral, parser-side): parse the committed fixture corpus (`fixtures/shows/raw/*.md`, the same corpus the existing corpus meta-tests walk — e.g. `tests/parser/sourceAnchorsCorpus.test.ts`), collect every emitted `severity: "warn"` `ParseWarning`, and assert each code ∈ `WARNING_CARD_COPY_CODES`. No regex, no exclusion table — the oracle observes real emissions, so a NEW parser warn code exercised by any fixture fails by default. Vacuity guard: assert the corpus emits ≥ 5 distinct registry codes (the exact floor measured at implementation; the plan records the observed corpus code set). Sync/enrichment producers (12 of the 39) are not corpus-reachable; they are covered by layer two: this spec adds one line to the AGENTS.md §12.4 new-code checklist — "a new warn-severity ParseWarning code also gets a `WARNING_CARD_COPY_CODES` row + §4.2-style copy" — making registry membership part of the same reviewed lockstep that already governs every new code.
