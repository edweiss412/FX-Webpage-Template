...
**State isolation (post-R3 amendment, ratified from plan-review R1 finding 2):** every state capture starts from a FRESH `page.goto` of the cell page — no state capture reuses the page of a previous one, so retained focus or hover cannot leak into the next capture's pixels under a single-state name. Because the served pages default to `data-theme="light"`, the requested theme is REAPPLIED after every per-state navigation, and the oracle additionally asserts `document.documentElement.getAttribute('data-theme')` equals the requested theme immediately before the capture — otherwise all 20 dark state names could silently record light pixels while every pseudo-state check passes (plan-review R3 finding 1). The single-state oracles are EXCLUSIVE: hover asserts `el.matches(':hover:not(:focus-visible)')`; focus asserts `el.matches(':focus-visible:not(:hover)')`; active asserts `el.matches(':active')` and `!el.matches(':focus-visible')` (a real press inherently hovers — the active baseline deliberately records the pressed-while-hovered rendering, which is the only reachable real-press state); the compound asserts `el.matches(':hover:focus-visible')`.

<!-- spec-lint: ignore — pseudo-class selector literals in matches() calls, not file citations -->
**State-reachability oracle (R1 finding 3):** before each state capture, the spec asserts the pseudo-state actually holds on the link via `link.evaluate(el => el.matches(sel))` with the EXCLUSIVE selectors defined in the isolation paragraph above (one per state; the component styles the FOCUS-VISIBLE state, `components/admin/wizard/step3ReviewSections.tsx:1002`, so plain focus passing while focus-visible is false would capture idle pixels under a focus-named baseline, R2 finding 4) — so a capture can never silently record the idle rendering under a state-named baseline. The same configuration-vacuity class this spec exists to close applies to its own captures.
...
- `snapshotPathTemplate: "{snapshotDir}/{testFileName}-snapshots/{arg}{ext}"` — drops Playwright's default platform suffix. `{snapshotDir}` (absolute, defaults to testDir), NOT `{testFileDir}`: the latter is testDir-RELATIVE and empty for a root-level spec, which resolved the whole template to filesystem root on the second gate run of PR #617 (post-approval measured amendment). The platform is constant by construction (next bullet), and a platform-suffixed name would silently create a parallel darwin baseline set on a misconfigured local run instead of failing.
