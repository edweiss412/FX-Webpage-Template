...
5. **Management:** a per-row **Developer toggle** in the Administrators section, rendered/usable **only by developers**, backed by a new `set_admin_developer_rpc` whose write authorization is a **table-backed actor check** (`exists(select 1 from admin_emails where email = auth_email_canonical() and revoked_at is null and is_developer)`) — **NOT `is_admin()`, and deliberately NOT `is_developer()`** (whose JWT arm must never authorize a membership mutation; §4.4 / §11 / R9). Re-checked under the advisory lock before the write.
...
4. **Mutation-RPC authorization guard (Codex spec R9+R10).** Scans the `set_admin_developer_rpc` definition in its migration and asserts its authorization path (a) **contains** the table-backed `exists(... from public.admin_emails ... and is_developer)` actor check (both the fast-reject and the post-lock re-check), and (b) does **NOT** call `public.is_developer()` anywhere in that RPC — so the JWT arm can never authorize a developer-bit mutation and the canonical §3.5 / §11 contract cannot silently regress.

This converts "find the next peer instance by hand" into a CI-time completeness gate over gate coverage (incl. non-route server actions), error posture, **and** the mutation-RPC authorization contract.
...
| **Write paths** | `set_admin_developer_rpc` (promote/demote; authorized by a **table-backed** actor `exists(admin_emails … is_developer)` check — NOT `is_developer()`, R9 — re-checked under the advisory lock); `revoke_admin_email_rpc` (forces `false` on revoke); migration seed (`edweiss412` → `true`). Direct table DML is REVOKE'd from `authenticated`. |
...
> **SUPERSEDED (2026-07-04, Part B — `2026-07-04-telemetry-rename-admin-dev-gate` §3.4):** admin-roster mutation is now **developer-only**. `upsert_admin_email_rpc` + `revoke_admin_email_rpc` (migration `20260704000000_admin_mgmt_requires_developer.sql`) require a **table-backed active developer** actor (pre-lock fast-reject + post-lock TOCTOU re-check, parity with `set_admin_developer_rpc`), and the `addAdminAction`/`revokeAdminAction` server actions gate on `requireDeveloperIdentity`. So a normal (non-developer) admin can **neither** add a rogue admin **nor** revoke any admin — the "any admin can revoke any admin" risk below is **closed** for non-developers. The residual (a *developer* other-revoking the last developer) stays out of scope, consistent with last-admin-via-other-revoke; total lockout is recovered by the migration bootstrap (`edweiss412` `is_developer=true`), and self-revoke stays unconditionally refused so a lone developer cannot self-lock-out. The paragraph below is retained for historical context (the pre-Part-B posture).
