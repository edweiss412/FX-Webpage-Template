...
Result contract for `mapRoleToken` (live), exact union (Codex R8 F3 + R9 F1): `{ ok: false, code: "stale" | "conflict" | "infra_error" | "show_not_found" | "validation_error" }` when nothing durable happened — and the component MUST branch `code: "stale"` and `code: "conflict"` to their §9 benign notices, never the generic error copy (own tested branches) · `{ ok: true, state: "applied" }` when the follow-up `runManualSyncForShow` reports `outcome === "applied"` · `{ ok: true, state: "apply_pending" }` when the mapping committed but the re-sync failed, threw (caught), or returned any non-applied outcome (§7; mirrors `_actions/useRaw.ts:155-170`). `mapRoleTokenStaged` has the same shape with the re-stage in place of the re-sync; its success-state rule (Codex R14 F1): `state: "applied"` means the re-stage COMPLETED and the refreshed staged parse reflects the mapping (warning consumed) — an assertion about the staged data only, never about any published show; a failed/thrown re-stage after the durable upsert → `"apply_pending"`. **AMENDMENT SUPERSEDED (2026-07-16, staging-overlay integration — spec `2026-07-16-role-vocab-staging-overlay.md`):** the wizard staging pipeline NOW runs the role-mapping overlay at the `prepareOnboardingFiles` chokepoint, so the staged `"applied"` branch is reachable: `state: "applied"` means the re-stage COMPLETED and the refreshed staged parse no longer carries the token's warning (the Codex R14 F1 rule, unchanged). A failed/thrown re-stage after the durable upsert still resolves `"apply_pending"`. (Correction to the superseded text: "the staged-apply path threads `roleTokenMappings`, tested" referred to the LIVE dashboard staged-apply, `lib/sync/applyStaged.ts:1376` — the wizard finalize path threads `[]` by design; preview==publish parity per the staging-overlay spec §2.) Settings-page `update`/`delete` return plain ok/error — no attached re-sync, convergence is cron-driven (§7). **Built-in guard scope (Codex R14 F3):** `isBuiltInRoleToken` gates the CREATE paths only. `updateRoleTokenMapping`/`deleteRoleTokenMapping` operate on an existing row's token as a historical fact and skip the guard — a dormant row whose token later becomes built-in stays editable and removable (it is inert either way, §7).
...
5. **Emission:** every `Phase2Result`-consuming commit surface (cron `runScheduledCronSync.ts`, manual `runManualSyncForShow.ts`, staged-apply `applyStaged.ts`/`applyStagedCore.ts`) emits ONE `ROLE_TOKEN_MAPPED` event per entry AFTER its transaction commits, outside the advisory-lock tx (invariant 10); a rolled-back sync emits nothing. Walker-style test: every consuming commit site emits or carries an inline exemption comment. **Wizard-staging exemption (2026-07-16, staging-overlay spec §6):** grants applied via the wizard STAGING overlay reach publication without a `ROLE_TOKEN_MAPPED` event — the wizard finalize path was never one of the emit surfaces above (same ratified family as the first-seen carve-out pinned at `runManualStageForFirstSeen.ts:101-103`), and the post-publish sync finds nothing new (delta gate, steady-state silent). Doug drives the wizard application from the staged preview, and the create action's `logAdminOutcome` (`ROLE_TOKEN_MAPPING_SET`, with `wizardSessionId` + `driveFileId`) is the durable forensic record.
...
- **Financials projection test** (§4.1): a FINANCIALS-only (non-LEAD, non-admin) viewer's `getShowForViewer` result includes `financials` data (read issued); a viewer with neither entitlement issues ZERO financials reads (existing contract preserved). **Superseded 2026-08-07:** the zero-read contract is retired — the read issues for every viewer and the value is discarded at the return boundary, so the test now pins issuance PLUS an undefined projected value (`docs/superpowers/specs/2026-08-07-projection-financials-viewer-independent-design.md`).
