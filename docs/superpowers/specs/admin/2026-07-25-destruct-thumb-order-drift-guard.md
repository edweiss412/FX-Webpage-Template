...
Scanned schedulers: `setTimeout`, `setInterval`, `requestIdleCallback`, `AbortSignal.timeout`. Aliasing any of them (`const t = setTimeout;`) is **banned outright** in a registry file rather than chased, since a rename hides the call from the census entirely. The comparator is key-order independent, and the alias detector has a self-check pinning that an ordinary `const t = setTimeout(cb, MS)` handle assignment does **not** trip it — getting that backwards flagged two real files on the first attempt.
