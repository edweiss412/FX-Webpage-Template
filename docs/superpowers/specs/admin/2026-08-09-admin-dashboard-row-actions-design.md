...
| --- | --- | --- | --- |
| Open | ✓ | ✓ | `<Link>` using the SAME `openHref(row.slug)` helper the row Link uses (`useShowModalNav` builds the parameter-preserving `/admin?show=...` modal URL; `/admin/show/[slug]` is only a legacy redirect — spec R2 F6) |
