...
6. **The existing `rowAction` slot POSITION is the mount point, but not via a function prop across the server boundary** (spec R2 F1: `Dashboard.tsx` is an async Server Component and `ShowsTable.tsx` is `"use client"` — a `rowAction={(row) => ...}` render function is a non-serializable prop and cannot cross that boundary). Mechanism: `ShowsTable` gains `showRowActions?: boolean`; when set, it imports and renders `<ShowRowActions row={row} />` itself as a SIBLING of the row `<Link>` — an interactive control inside an anchor is invalid HTML, per the comment at the site. **AMENDMENT (2026-08-10, impeccable critique P1, ratified in the plan's §12 and recorded here because the spec is canonical and only an amendment may supersede it):** the mount POSITION is the row's trailing seat, not the below-row `rowAction` bar. Reusing that bar gave every active row a full-width `border-t bg-surface-sunken` strip ~68px tall — taller than the row — to hold one 44px button, nearly doubling the table's height. The menu is now absolutely positioned into the trailing cell the chevron vacates (`data-testid` `shows-row-menu-<slug>`; the chevron yields because the menu's first item IS Open, so the row's "this opens" cue is not lost, and two glyphs there would be). The `rowAction` bar is untouched and still renders for its own callers at `shows-row-action-<slug>`. The `rowAction` function prop remains for CLIENT callers and is untouched; the Dashboard passes the boolean.
...
**Positioning/collision contract (spec R4 F1):** `ShowsTable`'s row container clips (`overflow-hidden` on the rows wrapper), so an absolutely-positioned in-row panel is unreachable on bottom/edge rows. The menu (and every expanding surface it hosts: submenu, Archive confirm, held/error panels) renders in a PORTAL to `document.body`, anchored to its trigger, flipping vertically when the anchor is in the lower viewport region so the panel stays fully on-screen; it closes on WINDOW scroll and on row unmount (the §3.5 compound rows). (Plan R3 F3 corrected the scroll target: the rows wrapper is `overflow-hidden` but height-unconstrained — excess rows scroll the DOCUMENT, not the table — so the dismiss trigger is window scroll; confirmed in the real browser at Task 7, where `window.scrollTo` moves the page and `document.scrollingElement.scrollHeight` exceeds `innerHeight` with the seeded fixture.) **Amendment (Task 7, real-browser):** the dismiss discriminates by EVENT TARGET — the document's own scroll dismisses, a scrolling ANCESTOR still re-places — so one capture-phase listener serves both and they cannot disagree about what happened. And every focus move inside the menu passes `preventScroll: true`: measured on the real dashboard, a plain `focus()` on the first menu item jumped the page to the top at the exact moment the admin opened a menu on a scrolled row, which the geometry premise then read as a containment failure. The plan names the concrete portal/anchor primitive after checking whether the repo already ships one (BellPanel / modal shells are candidates); absent a reusable one, a minimal anchored-portal is part of this arc.
...
| compound: server refresh re-renders row while menu open (background revalidate) | the menu STAYS OPEN — corrected at whole-diff R3 against the Next 16 `useRouter` docs (`node_modules/next/dist/docs/01-app/03-api-reference/04-functions/use-router.md`): `router.refresh()` merges the RSC payload WITHOUT losing React state, and `ShowsTable` keys rows by `row.id`, which does not change. The original row assumed a remount that does not happen. What the surface owes instead is ELIGIBILITY: if the refresh flips the row to unpublished, every ACTIONABLE surface closes — the submenu, the Archive confirm, the held decision — leaving Open only (AC-2), while READ-ONLY outcome banners stay visible, because hiding an answer the admin is owed is the silent-outcome defect and not a fix for it (the rule is executable at `tests/components/admin/rowActions/_metaOutcomeVisibility.test.tsx`; corrected at whole-diff R6, which caught this row still saying "and any failure banner") — asserted on a same-instance re-render, since a remount would not exercise it. |
...
None introduced. **AMENDED with §1.6 (2026-08-10):** the menu is no longer an auto-height block below the row Link. It is an absolutely-positioned sibling in the row's trailing seat (`inset-y-0` + `items-center`, deliberately NO transform — a `translate` there would make the seat the containing block for `position: fixed` DESCENDANTS and collapse the menu's full-viewport outside-click backdrop to the 44px button, which the impeccable audit caught), and the row Link reserves the gutter with `pe-14` so row content never runs under it. The row's own height is therefore UNCHANGED by this feature, which is the dimensional claim that matters; the menu itself is content-sized and portaled. No fixed-dimension parent gains flex children. Declared explicitly per the self-review rule.
