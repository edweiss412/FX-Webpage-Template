...
- **Desired next state** = for each member in the **post-hold** raw write list (`planHoldAwareApply` output, §3.4) plus each **held-retained** member, the member's **FULL parsed `crew_members` row** (`{ parsedName, email, phone, role_flags, date_restriction, stage_restriction, flight_info, last_changed_at, … }` — every mutable column the legacy upsert writes) with **only `name` and `role` replaced by their overrides**: `displayName` = active name override's output (if any, and not deactivated by collision) else `parsedName`; `finalRole` = active role override's value else parsed role. **All OTHER parsed columns pass through unchanged** (R29 — the id-keyed path replaces the legacy `upsertCrewMembers`, so it MUST carry every column that path wrote, or those fields would stop syncing on override-active shows). Identity/lifecycle columns (`id`, `claimed_via_oauth_at`, `selections_reset_at`) are **preserved from the live row**, never overwritten from the parse.
...
2. **Match & write by id:**
   - `parsedName` in BOTH prev and next → `UPDATE crew_members SET name=displayName, role=finalRole, email=<canonicalized parsed email>, phone=…, role_flags=…, date_restriction=…, stage_restriction=…, flight_info=…, last_changed_at=… WHERE id = prevId` (id-keyed rename-in-place, writing the **full parsed row** with only name/role overridden — R29; `id`/`claimed_via_oauth_at`/`selections_reset_at` preserved; email canonicalized via `lib/email/canonicalize.ts`, invariant 3). Covers override apply, override edit, override release, and collision-release uniformly — **id preserved**.
...
**Uniqueness-safe write order (R24 — mandatory; the set operations above are ORDERED, not free).** `crew_members` has `unique(show_id, name)`, so the id-keyed writes must be sequenced within the locked tx or they transiently violate it — e.g. a survivor still named `John` (active `Jon→John` override) while a fresh parsed `John` must be inserted, or a name-swap cycle (`A: Jon→John` and `B: John→Jon`). Collision resolution (step 1) guarantees the **final** display names are all distinct; the writes reach that state in **four phases**, each individually constraint-safe:
   ...
   4. **Assign finals:** `UPDATE crew_members SET name = <finalDisplayName>, role = <finalRole>, email = <canonicalized parsed email>, phone = …, role_flags = …, date_restriction = …, stage_restriction = …, flight_info = …, last_changed_at = …, sheet_name = <match_key when a name override is active, else NULL> WHERE id = <parkedId>` for each parked survivor — the **full parsed row with only name/role overridden** (R29), `id`/`claimed_via_oauth_at`/`selections_reset_at` preserved. For **name-unchanged** members (not parked), the same full-column `UPDATE … WHERE id` still runs to refresh `role`/`email`/`phone`/`flags`/`restrictions`/`flight_info`/`sheet_name`. Every final `name` is distinct and unoccupied, so each assignment succeeds — including arbitrary rename permutations/cycles.
...
- **Runtime name-collision (R26 — analog of the crew §3.6 step-1 collision; collision is over FINAL names, R27).** The write-time guard (§7.4) blocks *creating* a `hotel_name` override whose value collides, but a *later sync* can introduce the collision. The Stage-A transform computes each reservation's **FINAL** `hotel_name` = its own active `hotel_name` override's output if any, else its parsed name (exactly like crew `displayName`). Before applying a `hotel_name` override, it checks whether that reservation's FINAL name equals **another reservation's FINAL name** — NOT the other's raw parsed name (R27: comparing against raw parse falsely fires when reservation B is parsed `Hilton` but has its own active override to `Hyatt`; the finals `Hilton`/`Hyatt` are distinct, so A→`Hilton` is fine and MUST survive across syncs — the promised stable-parse multi-hotel-correction case). Only when two FINAL names genuinely coincide would the override produce **two live rows the immediate resolver (exactly-one-live-match, §5.3) can no longer uniquely edit/revert** — a stranded override; in that case the transform **does NOT apply the override-derived one** and **deactivates it `deactivation_code='name_conflict'`** (§6, committed Stage B) → needs-attention → Doug re-points/discards. Two legitimately-parsed same-name reservations (a real dup-name group, each keyed with a §5.3 disambiguator, neither overridden to collide) are NOT a conflict — that is the dup-name handling above. (`hotel_address` overrides never collide — address is not the resolver key.) **Sibling `hotel_address` override is NOT stranded by a name-override deactivation (R30):** deactivating a `hotel_name` override REVERTS that reservation's live name to its parsed name — which resolves the collision, so the reservation is uniquely addressable again. The sibling `hotel_address` override on the same reservation shares the `match_key` and resolves via `currentLiveHotelName` = the (now-inactive name override ⇒) parsed name; if that parsed name is itself a genuine dup-group member, the shared `match_key` carries the §5.3 disambiguator and still resolves to exactly one row. So the address override keeps working (or, if the parsed name is itself non-unique-and-non-disambiguable, it fail-closes via the same exactly-one gate — never silently mis-targets).
...
- **hotel** (`target_missing` OR `name_conflict`): `target_missing` = the (name+disambiguator) `match_key` absent from the parsed hotel set; `name_conflict` = a `hotel_name` override whose reservation's FINAL name coincides with **another reservation's FINAL `hotel_name`** (each reservation's own active override applied, NOT raw parsed — R27, §5.3 runtime collision). No hold interaction; both planned in Stage A, **committed in Stage B** (applied path only).
- **show**: never stale.
...
3. **Best-effort push (not the durable guarantee) — COARSE per-(show,code), row-stream is authoritative (R30):** post-commit (outside the lock), also emit an `admin_alert` via `upsertAdminAlert` (`lib/adminAlerts/upsertAdminAlert.ts:46`) with a new `AdminAlertCode` (`OVERRIDE_TARGET_MISSING`, or `OVERRIDE_NAME_CONFLICT` for the collision case, §10) so the realtime bell/NotifBell lights up immediately. **`upsert_admin_alert` dedups on `(coalesce(show_id::text,''), code) where resolved_at is null` (`20260618000000_upsert_admin_alert_failedkeys_merge.sql:47`) — so this bell is deliberately ONE unresolved alert PER (show, code), NOT per override row: multiple paused overrides of the same code on one show COLLAPSE into a single nudge.** That is acceptable because the **authoritative, per-override signal is the inactive-row stream (step 2)** — the bell is only a coarse "this show has ≥1 paused override" nudge. **Lifecycle (R30):** these two codes are **auto-resolve-only** (`lib/adminAlerts/resolveAdminAlert.ts:13`); the same post-commit path that runs after ANY override op or sync that clears a stale row (discard, repoint, create-reactivate, sync reactivation) **re-derives per (show, code): if the show has ZERO remaining `active=false` rows of that code, resolve the alert (`resolveAdminAlert`); otherwise leave it open** — so the bell can never linger with no backing inactive row, nor clear while paused rows remain. If any of these best-effort emits/resolves fails, the step-2 row stream is still correct (load-bearing). The alert is **not** routed via `INBOX_ROUTED_CODES` (avoids double-surfacing the same item as both a derived-stream row and a routed-alert row).
4. The needs-attention row renders its copy from the **durable `deactivation_code`** (not the alert), **domain-aware** — `target_missing` → "sheet no longer has «X»"; `name_conflict` → "clashes with a real crew member" (crew) / "clashes with another hotel's name" (hotel) — so the operator sees the correct reason even when the step-3 alert emit failed. It offers **re-point** (update `match_key` to a current identifier, reactivate → clears `deactivation_code`) and **discard** (delete the override row) — both via `set_field_override` RPC variants (§7).
...
3. **Two-part row-state CAS (both under the lock; either mismatch → `errcode 40001` → 409 stale_review):**
   - **CAS-A version (R15) — guards the override row.** Read the target's current `admin_overrides.version`; if ≠ `p_expected_version`, 409. Catches concurrent override MUTATIONS (edit/revert/repoint/discard) and a concurrent sync-side **deactivation/reactivation**, because each of those **bumps `version`**. It deliberately does NOT fire on a benign sync-side `sheet_value` refresh of a still-active override (R30 — that does not bump `version`), so a routine cron sync between an admin's UI-load and save does not false-409 the edit; `sheet_value` is display-only and independent of `override_value` (which is what the admin is changing). On **create**, `p_expected_version` is NULL and the RPC asserts **no _active_ override row exists** for the target (else 409). An **inactive** same-key row is NOT a 409 — create **reactivates it in place** (§7.2 item 4, R28), so the retained `active=false` stale row never collides with the unique key.
   ...
   **Hotels resolve the target row by the unconditional fail-closed unique match of §5.3 (R20), NOT by `p_current_ordinal`.** The RPC requires **exactly one** live reservation in the show matching `hotel_name = p_expected_live_hotel_name` (and, when `p_match_key` carries a disambiguator, the recomputed §5.3 disambiguator from that row's non-overridable booking columns `check_in`/`confirmation_no` (NOT `names[]`, R30)); **zero or >1 → 409 stale_review**, and it applies to that one row by id. The uniqueness gate is unconditional, so a name unique at load that gains a same-name sibling before save (insert/reorder) 409s rather than retargeting (R20 — closes the gap the R19 disambiguator-only check left). `p_current_ordinal` is advisory (observed ordinal), not the locator. This subsumes CAS-B's row-locator role for hotels; CAS-B still guards the field value. Every successful mutation ends by `version = version + 1`.
4. **Op semantics:**
   - `upsert` (**create** — no active override on this target): validate `override_value` (§7.4) incl. the name-collision guard; **capture the current live value (= the parsed/sheet value, since nothing is overridden yet) into `admin_overrides.sheet_value` in the SAME locked tx, BEFORE overwriting the live row**; **write `admin_overrides` as an UPSERT on the unique key `(show_id,domain,field,match_key)` — INSERT if no row exists, else REACTIVATE the retained inactive row (R28): `UPDATE … SET override_value=…, active=true, deactivation_code=NULL, sheet_value=<captured>, version=version+1 WHERE …` (`insert … on conflict (show_id,domain,field,match_key) do update`)**; apply `override_value` to the live row (§7.3). No auth-table write (§5.2). **Because stale rows are retained `active=false` (§6), a plain INSERT would hit the `admin_overrides_uniq` constraint when a previously-deactivated override for the same target reappears — the reactivate-on-conflict path is mandatory (R28); it is the natural "target came back, re-apply my correction" flow. Only an _active_ same-key row blocks create (→ 409, CAS-A).**
   ...
   **sheet_value invariant (R6/R7):** `admin_overrides.sheet_value` ALWAYS holds the **parsed/sheet** value, never an override value. It is captured **once at create** (when the live value still equals the sheet value) and thereafter refreshed **only from the parsed source** on each applied sync (Stage B). It is **never** overwritten on an *edit* (the live value there is the prior override — capturing it would poison revert, R7 finding). This makes revert correct at every moment, including: (a) create-then-revert-before-any-sync (captured-at-create value restored, R6); (b) edit-then-revert-before-any-sync (preserved sheet_value restored, not the discarded prior override, R7). `repoint` to a fresh target captures **that target's** current parsed value (it is un-overridden, so live == sheet). Applies to all 6 fields. Tested: create-then-revert AND edit-then-revert, both without any intervening sync, restore the true sheet value for every field.
   ...
   - **Repoint target-key collision (R29 — both active and inactive repoint).** Because `admin_overrides_uniq` spans **all** rows (not just active), `set match_key = p_new_match_key` would hit the constraint if a **retained** override row already exists at `(show_id, domain, field, p_new_match_key)`. Before the match_key change, the RPC inspects any existing row at the new key **other than the one being repointed**: an **ACTIVE** row there → **409 invalid-state** (the target already has its own active override — Doug edits/reverts that one instead); an **INACTIVE** (stale) row there → **DELETE it (supersede) in the same locked tx** (it is a discardable stale record — equivalent to discarding it), then set `match_key`. This makes repoint the same "reactivate/supersede on the unique key" discipline as create (R28). Tested: repoint into a target key holding (a) an active override → 409 mutating nothing; (b) an inactive override → the stale row is deleted and the repoint succeeds, exactly one row at the key.
...
| hotel_name | empty/whitespace-only; `= match_key` (no-op); **or collides** — equals any *other* reservation's **current FINAL `hotel_name`** in the show (= that reservation's own active `hotel_name` override output if any, else its live/parsed name) (R26/R27: would create two live rows with the same `hotel_name`, which the immediate resolver — exactly-one-live-match, §5.3 — could then never edit/revert, stranding the override). Compare against FINALs, NOT raw parsed names, so a reservation parsed the same but overridden to a *different* name is not a false collision. Mirrors the crew name-collision guard (which compares `displayName`s). | 200 chars |
...
| `repoint` (**inactive/stale** override) | name/role | old target vanished **by definition** | **do NOT require the old live row.** Validate: (a) the stored inactive override-row CAS, (b) the new `match_key` is a current parsed identifier with no live-value collision (§7.4), (c) **target-key collision (R29): an active override already at the new key → 409; an inactive one → delete/supersede it in-tx**. Then set `match_key`=new target, `active=true`, and apply. | apply to new target |
...
| `OVERRIDE_NAME_CONFLICT` | `admin_alerts` (new `AdminAlertCode`) | doug | "A name override you set now clashes with another row of the same name (a real crew member, or another hotel). We paused it and are showing the sheet's value — re-point it to a different name or discard it." (covers BOTH crew name and hotel_name collisions, R26; the precise per-domain reason is on the durable needs-attention row, §6 step 4) |
...
- `tests/overrides/deactivationReason.test.ts` — the durable `deactivation_code` (R12): a stale deactivation sets `'target_missing'` and a collision sets `'name_conflict'` **in the locked tx**, and needs-attention renders the correct reason **even when the post-commit `upsertAdminAlert` throws** (the reason is read from the column, not the alert). **Alert lifecycle coherence (R30): two paused overrides of the same code on one show COLLAPSE to a single unresolved `admin_alert` (per-(show,code) dedup) while BOTH surface as separate inactive-row needs-attention items; when the LAST paused row of that code clears (discard/repoint/reactivate) the post-commit path resolves the alert (no lingering bell), and it stays open while ≥1 paused row of that code remains — the row stream and bell never diverge at show granularity.**
...
| Apply transform (sync) | replace parseResult.show before applyShowSnapshot | **id-keyed parsed-identity reconciliation §3.6** (post-hold, `UPDATE…WHERE id` of the FULL parsed row — all mutable columns, only name/role overridden, email canonicalized, id/claimed_via_oauth_at preserved — R29; NOT name-keyed delete/upsert); also set `crew_members.sheet_name` | replace hotel rows before replace |
...
6a. **Hotel duplicate-name identity is fail-closed on BOTH paths — comprehensive (R16 sync / R17 first-create key / R19 RPC / R20 unconditional unique-match).** One contract, three enforcement points: (1) **sync path** — the (parsed name + content disambiguator) `match_key` must resolve to exactly one parsed reservation each sync; zero/>1 -> deactivate stale, never a guessed apply (§5.3). (2) **RPC immediate path** — the row is resolved by an **UNCONDITIONAL exactly-one-live-match** on `currentLiveHotelName` [+ recomputed disambiguator for same-name groups]; `p_current_ordinal` is advisory, NOT the locator (R20); zero/>1 live matches -> 409, never a guessed apply — so even a name UNIQUE at load that gains a same-name sibling before save 409s (§5.3/§7.2). (3) **admin loader** — even with no active override, a reservation in a same-name group is keyed with the SAME disambiguator (§8.2a, R17), so first-create cannot produce an ambiguous key. The disambiguator is computed only from **non-overridable booking** columns (`check_in`/`confirmation_no` — NOT the mutable `names[]` occupant list, R30), so it is stable across a `hotel_name` override AND across roster/rooming edits; if those two still don't uniquely identify a same-name reservation the key is fail-closed (deactivate + re-point), never guessed from occupant data. Do not relitigate any single path in isolation — the three points are the complete closure of the duplicate-hotel-name vector; a change to the disambiguator definition must update all three.
6b. **`hotel_name` override values can't create an un-editable duplicate (R26).** A `hotel_name` override whose value equals another live/parsed hotel name would make the live set non-unique and strand the override (the exactly-one-live-match resolver would 409 every subsequent edit/revert). Closed two ways, mirroring the crew name-collision handling: (1) **write-time** — §7.4 rejects a `hotel_name` value equal to another reservation's current FINAL name at create/edit; (2) **sync-time** — a later sync where two reservations' FINAL names coincide deactivates the override-derived one `name_conflict` (§5.3/§6) → needs-attention → Doug re-points/discards. **Collision is over FINAL names (each reservation's own override applied), NOT raw parsed names (R27)** — else a valid multi-hotel composition (A→Hilton, B parsed Hilton but →Hyatt) would false-deactivate every sync. Do not relitigate as "hotel_name is free text" — it is the resolver key, so it carries the same final-name collision contract as crew `displayName`.
...
11. **Create is an UPSERT on the unique key, reactivating a retained inactive row (R28).** Stale overrides are kept `active=false` (deactivate-not-delete, watchpoint 3), and `admin_overrides_uniq` spans ALL rows on `(show_id,domain,field,match_key)`. So a plain INSERT on create would hit the constraint when a previously-deactivated override for the same target reappears. Create therefore does `insert … on conflict (…) do update … set active=true, deactivation_code=null, version=version+1` — the "target came back, re-apply my correction" flow. Only an *active* same-key row blocks create (409, CAS-A). Do not relitigate as "create should just INSERT" — the retained-stale-row design requires the reactivate-on-conflict path. **Repoint applies the same discipline (R29): repointing onto a key that holds an active override → 409; onto one holding an inactive override → supersede (delete) it in-tx.**
12. **The id-keyed crew path writes the FULL parsed row, not just name/role (R29).** It replaces the legacy `upsertCrewMembers`, so it MUST carry every mutable column that path wrote (`email` canonicalized, `phone`, `role_flags`, `date_restriction`, `stage_restriction`, `flight_info`, `last_changed_at`) — overriding only `name`/`role` — while preserving identity/lifecycle columns (`id`, `claimed_via_oauth_at`, `selections_reset_at`). Dropping any column would silently stop syncing it on override-active shows. Do not relitigate as "the override path only touches name/role" — it owns the whole row write for those shows.
