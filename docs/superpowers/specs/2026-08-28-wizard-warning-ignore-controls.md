...
- **Advisory-lock holder topology (invariant 2, enumerated).** The hashkey is `hashtext('show:' || drive_file_id)`. Existing holders acquire at exactly one of two layers per path: (a) the JS wrapper `withShowLock` (`lib/sync/lockedShowTx.ts`) — callers include `useRawStaged.ts`, `lib/onboarding/rescanWizardSheet.ts`, `lib/onboarding/applyRescanDecisionUnderLock.ts`, `lib/sync/applyStaged.ts`, `lib/sync/runOnboardingScan.ts`, `lib/sync/retrySingleFile.ts`, `lib/sync/unpublishShow.ts`, `lib/sync/assetRecovery.ts`, `lib/showLifecycle/archiveShow.ts`, and the two show-page actions; (b) in-RPC `pg_advisory_xact_lock` inside specific SECURITY DEFINER functions, pinned by `tests/auth/advisoryLockRpcDeadlock.test.ts`. The new action acquires ONLY at layer (a) — the same layer as `useRawStaged` — and its in-lock SQL is plain `select`/`update` on `pending_syncs` through the lock tx, calling NO RPC that locks. The deadlock-guard registry gains the new action's row so the topology stays pinned.
