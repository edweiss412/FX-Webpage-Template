...
Reads `pr.transportation` (`TransportationRow|null`, `types.ts:186-196`). If null → "No transportation parsed." Rows: **Driver** (`driver_name`), **Driver phone** (`driver_phone`), **Driver email** (`driver_email`), **Vehicle** (`vehicle`), **License plate** (`license_plate`), **Color** (`color`), **Parking** (`parking`), **Notes** (`notes`). Then the schedule legs (`pr.transportation.schedule[]`, `TransportScheduleEntry`, `types.ts:180-185`): one row per leg = `${stage}` with `${date} ${time}` and `assigned_names` joined, each leg gated by `hasContent(stage)`. `count` = present field rows + legs.

### NEW: `ContactsBreakdown` (`-breakdown-contacts`)
Reads `pr.show.client_contact` (`ClientContact`, `types.ts:88-94`: `{ name, email, phone, officePhone, secondary? }`) + `pr.contacts` (`ContactRow[]`, `types.ts:198-205`, `kind: 'venue'|'in_house_av'`). One sub-block per person: a kind/role label (**Client contact** for `client_contact`; **In-house AV** for `kind==='in_house_av'`; **Venue contact** for `kind==='venue'`) + name, phone, email (office phone for client) rows. A person with no `hasContent` fields is omitted. `count` = number of rendered people.
...
3. **Contacts** (`-breakdown-contacts`): a `client_contact` (name+phone) + a `contacts` entry kind `in_house_av` (name+email) + a `venue` contact → all three sub-blocks render with correct kind labels; count === 3 derived; no contacts + no client → empty state. Failure mode: client contact omitted; kind mislabeled.
