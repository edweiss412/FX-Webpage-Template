...
**Client people:** render BOTH the primary `client_contact` AND `client_contact.secondary` (when present) — iterate `[client_contact, client_contact.secondary].filter(Boolean)`, each as a **Client contact** sub-block with name, phone, email, and **Office** (`officePhone`) rows. (Spec-R1: `secondary` is a parsed operator-relevant person; completeness requires rendering it, not dropping it.)
