...
2. **Discovery layer (the gap-closer)** — a repo scan: grep `lib/sync`, `lib/showLifecycle`, `app/api`, and the admin `_actions` for writes to a getShowForViewer-read table (`.from("<read-table>").{insert,update,upsert,delete}` and `.rpc("<writing-rpc>")`, plus the `tx.upsert*/replace*/delete*` apply-method names). Any file that writes a read-table but is NOT in the registry FAILS the test ("unregistered show-data write site — add a revalidateTag + registry row or an exemption"). This is what makes enumeration-completeness structural rather than reviewer-dependent — the exact class that burned R1–R3.
