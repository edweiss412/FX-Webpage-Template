...
**Structural chokepoint for ALL sync apply (Codex R1+R2 — same-vector fix).** Two review rounds found missed sync routes (manual admin sync `app/api/admin/sync/[slug]/route.ts:66`; pending-ingestion retry `.../retry/route.ts:357`; on top of cron + webhook). Rather than enumerate routes (error-prone — that's how both got missed), the single chokepoint is **`processOneFile`** (`lib/sync/runScheduledCronSync.ts`): cron (`runScheduledCronSync`), webhook (`runPushSyncForShow.ts:293`), manual sync (`runManualSyncForShow`), and retry (`runManualSyncForShow_unlocked`) ALL funnel through it, and it returns `outcome:"applied"; showId` (`runScheduledCronSync.ts:174-175`). Place ONE `revalidateTag(\`show-${showId}\`)` in `processOneFile` **after the apply transaction commits, when `outcome==="applied"`** — NOT in the cron-only tail at `:1813` (which other callers don't reach) and NOT via the injected publisher. This covers all four routes at once; the meta-test (§6) pins processOneFile as the registered site.
...
|---|---|---|---|---|---|
| **ALL sync apply** — cron + webhook + manual admin sync + pending-ingestion retry | `processOneFile` (`runScheduledCronSync.ts`, via `applyParseResult.ts:127-186`); callers: cron route, `runPushSyncForShow.ts:293`, `runManualSyncForShow`, `runManualSyncForShow_unlocked` (`app/api/admin/sync/[slug]/route.ts:66`, `app/api/admin/pending-ingestions/[id]/retry/route.ts:357`) | crew_members, hotel_reservations, rooms, transportation, contacts, shows_internal, shows | **`processOneFile` post-commit, `outcome==="applied"`** (single chokepoint) | DB triggers on crew_members (+ JS publisher when injected — prod cron does NOT) | ADD ONE revalidateTag(result.showId) in processOneFile's applied path |
| Onboarding finalize / staged apply | `app/api/admin/onboarding/{finalize,finalize-cas}/route.ts`, `.../staged/[…]/apply/route.ts` (uses `makeSyncPipelineTx`+`applyParseResult` — plan MUST verify whether these reach `processOneFile` or apply directly) | crew_members, shows, etc. | each onboarding apply route (if it bypasses processOneFile) | DB triggers | VERIFY path; ADD revalidateTag(show) if not covered by the processOneFile chokepoint |
