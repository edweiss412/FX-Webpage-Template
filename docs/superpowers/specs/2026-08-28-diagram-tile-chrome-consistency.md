...
| `tests/styles/controlOutlineScan.ts:126` and `tests/styles/controlOutlineScan.ts:139` | two line-keyed registry rows address `components/admin/wizard/step3ReviewSections.tsx:4455` and `components/admin/wizard/step3ReviewSections.tsx:4512`, which the replaced comment pushed down by sixteen lines | relocate to `components/admin/wizard/step3ReviewSections.tsx:4471` and `components/admin/wizard/step3ReviewSections.tsx:4528`, LOCATED by the `<button>` openers themselves as those rows' own comments instruct, never by adding a delta |
