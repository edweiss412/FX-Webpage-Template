...
- `tests/sync/dev-routing.test.ts:290` — feeds garbage markdown (`SYNTHETIC_NO_VERSION_MARKDOWN`, no version markers) to the real dev-tool action `parseAndStage` (`@/app/admin/dev/actions`) and asserts a fuller chain: `result.hardFailCodes` contains MI-1, `result.outcome === "hard_fail"`, staging kind `pending_ingestion`, and the `dev.pending_ingestions` row's `last_error_code === "MI-1_VERSION_DETECTION_FAILED"`. **However** this test (a) is **DB-gated** — it uses `admin.schema("dev")` against a real Supabase (`tests/e2e/helpers/supabaseAdmin`), so it is skipped in the fast DB-free unit shards that run on every push and is part of the env-dependent suite set; (b) routes through the heavyweight `parseAndStage` admin dev-tool + `dev.*` shadow schema, not the production parser boundary; and (c) asserts only the error **code** + DB row, never the parser's early-return **stub shape**.

### The genuine remaining gap

Two things remain unproven by a **fast, DB-free, hermetic** test at the parser boundary:

1. **The early-return stub CONTRACT.** Nothing asserts the full shape `parseSheet` returns for garbage: exactly ONE hardError (the MI-1 code, no VERSION_AMBIGUOUS), empty crew/rooms/hotel/contacts, `transportation: null`, `pullSheet: null`, `template_version: "v4"` placeholder, empty title, empty warnings, and — critically — **no throw**. `parseSheet.test.ts:318` checks only codes; `dev-routing.test.ts:290` checks only code + DB row; `buildMinimalParsedSheet` has no direct shape test. A regression that added a spurious warning, dropped the MI-1 code, or (per finding #17 / PR #339) let the path throw would slip past all three.

2. **The parser→invariants COMPOSITION without the DB / dev-tool layer.** The documented production seam is `parseSheet → enrichWithDrivePins → runInvariants → phase1` (`lib/parser/index.ts:7`). The only end-to-end garbage→hard_fail proof (`dev-routing.test.ts:290`) requires a live DB and goes through `parseAndStage`; it does **not** run in the fast unit shards and does **not** isolate the pure `parseSheet(garbage) → enrichWithDrivePins → runInvariants` composition. `mi.test.ts:126` proves `runInvariants` on a **hand-fed** hardError atop a valid body, never the real empty-crew/empty-rooms garbage stub.

Closing both in one fast, DB-free test guards the fail-closed contract that item (a) / finding #17 relies on: a regression in the early-return stub shape (or in how `classifyVersion` gates garbage) that stopped the sheet hard-failing would auto-apply an empty sheet over a live show — and the only existing e2e that would catch it is DB-gated and does not run on every push.
...
- **Positive control:** the composed-hard_fail test would fail if `parseSheet` stopped early-returning (crew/rooms would be non-empty for a real sheet — but these inputs have no data to parse, so the meaningful control is that a VALID sheet does NOT hard-fail on MI-1). Include one negative-control assertion: a real fixture (`fixtures/shows/exporter-xlsx/east-coast.md`) through the SAME `parseSheet → enrichWithDrivePins → runInvariants` chain yields `outcome !== "hard_fail"` OR (if it hard-fails for an unrelated reason) `failedCodes` does NOT contain `MI-1_VERSION_DETECTION_FAILED`. This proves the MI-1 hard-fail is caused by the garbage, not by the test harness/enrich/invariants always hard-failing.
...
- Negative control proves a valid fixture does NOT hard-fail on MI-1 through the same chain.
