...
No test exercises the **composed** production path end-to-end: real garbage markdown → real `parseSheet` (early-return stub) → real `enrichWithDrivePins` (the documented `parseSheet → enrichWithDrivePins → runInvariants → phase1` seam, `lib/parser/index.ts:7`) → real `runInvariants` → `hard_fail`. Every existing layer is proven in isolation with the seam hand-fed or mocked; the composition is unproven. A regression in the early-return stub shape (or in how `classifyVersion` gates garbage) could stop the sheet hard-failing and auto-apply an empty sheet over a live show — exactly the fail-closed contract item (a) / finding #17 relies on.
...
- **Out of scope:** widening `classifyVersion` acceptance (that is audit rec-1, not rec-6b); fuzz/property testing (rec-5, separate item); the sync-layer first-seen/existing-show routing (already covered end-to-end for the throw path by `tests/sync/parseSheetCallSiteGuard.test.ts` shipped in PR #339, and this spec deliberately does not duplicate the sync layer — it proves the parser→invariants composition, which is the layer the audit line scopes with "at `parseSheet`").
