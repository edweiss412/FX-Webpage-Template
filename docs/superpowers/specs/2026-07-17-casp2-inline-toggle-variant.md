...
10. **Real-browser 390px strip height** (`tests/e2e/*` Playwright, real browser — jsdom computes no layout). At 390px viewport, render the strip in ALL THREE inline states and assert the full §7 invariant (Codex R1 finding 1 — idle-vs-error alone was insufficient; a finalize-only popover rendered in-flow would have slipped through):
    ...
    - **(c) Horizontal containment (Codex R4 finding 1):** in the error-popover and finalize-popover states at 390px, assert the popover's `getBoundingClientRect()` has `left >= 0` and `right <= 390` (fully in viewport), AND the document has no horizontal overflow (`document.documentElement.scrollWidth <= document.documentElement.clientWidth`). *Catches:* a `max-w`/`right-0` popover projecting off-screen or forcing a horizontal scrollbar on Doug's phone. To exercise a far-left toggle position too, run this assertion with the strip's title at both the short fixture and `SHOWPAGE_LONG_TITLE` (which changes where the toggle wraps).
    - **Driving the states (interactive Playwright, not static markup):** the existing `_showPageLayoutHarness.tsx` uses `renderToStaticMarkup`, which cannot produce the S2/S3 error state (that requires a completed form submit) and forbidding a test-only forced-error render path (Codex R5 advisory) leaves no static way to reach it. So the layout task uses an **interactive** Playwright page: mount `PublishedToggle variant="inline"` (inside a minimal strip-width `relative` flex container matching the strip's padding, so the 390px geometry is faithful), then (S2) click the switch with a mocked-refusal `setPublished` to open the real error popover, and (S4) render with `finalizeOwned` to get the finalize popover. Idle (S1) is the pre-click state. This is the executable mechanism §8.10 requires — no static-harness variant for the error states.
    *Catches:* the exact CASP-2 defect — any popover or label that inflates the phone strip in any state.

    *(S5 is not a separate height case — its popover is the same `position:absolute` error popover as S2, so it contributes zero flow height like every other state.)*

11. **Transition-audit state enumeration** (`tests/components/admin/*`, jsdom — new, distinct from §8.8's motion-import pin; Codex R1 finding 2, corrected R2/R3): enumerate the §4.8 reachable states S1–S5. Assert: (a) each state produces the expected popover presence + `role` — **S1** idle → no `published-toggle-popover`; **S2** (mocked known refusal, submitted from the enabled switch) → popover with `role="alert"`; **S3** (mocked unknown-code, submitted) → popover with `role="alert"` + retry copy; **S4** (`finalizeOwned` true, switch disabled, no error) → popover WITHOUT `role="alert"` + the switch's `aria-describedby` matches the popover `id`. (b) The inline branch contains no `AnimatePresence` / motion import / `animate-[` (belt-and-suspenders with §8.8). (c) **S5 compound (error preserved across finalize flip):** render enabled, submit a mocked refusal (S2), `rerender` with `finalizeOwned=true`; assert the ERROR popover (`role="alert"`) still shows — not the finalize hint — and the switch is now `disabled`. This is the same assertion as §8.6, exercised here as the compound-transition pin the writing-plans transition-audit rule mandates. *Catches:* an unenumerated state (a finalize popover wrongly keeping `role="alert"`, a missing `aria-describedby`, or a preserved error dropped when finalize flips in).
