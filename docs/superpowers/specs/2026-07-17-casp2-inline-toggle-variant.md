...
- Positioning: `absolute` anchored to the container's **left** edge (`top-full`, `left-0`), `z-40` (above the strip's `z-30`), `mt-1`, `w-max`, `max-w-60` (15rem / 240px), and **`break-words`** (`overflow-wrap: break-word`). **Left-anchoring is deliberate (not `right-0`):** on a 390px phone the toggle flex-wraps to the left of its row (x≈16px, the strip's `px-4`); a `right-0` popover would extend *leftward* from the toggle's right edge and project off the left viewport edge, whereas `left-0` extends *rightward* (left edge ≈16px, right edge ≈16+240=256px < 374px = viewport−padding) and stays on-screen. **`max-w-60` + `break-words` is the load-bearing horizontal guarantee for ANY content (Codex plan-R3 finding 1):** the error popover's content (`ErrorExplainer`'s long `text-base` paragraph + `HelpAffordance`'s learn-more link) is materially different from the short finalize hint, but `max-w-60` hard-caps the box at 240px and `break-words` forces even long/unbreakable strings to wrap inside it — so the popover is never wider than 240px < 358px regardless of content, and content growth is purely vertical (height, which is out of flow and irrelevant to the strip). Because it is `position:absolute`, it never contributes to the strip's flow **height**. The in-viewport invariant is pinned two ways at 390px (§8.10c): the finalize popover's rect stays within `[0, viewportWidth]` with no document horizontal scroll (proves the anchor + cap in a real browser), AND a real-browser probe of the actual `ErrorExplainer`+`HelpAffordance` content in a `max-w-60 break-words` box measures ≤240px wide (proves the real error content respects the cap). If a class choice ever violates that, the §8.10c gate fails — the invariant is the contract.
