...
- Positioning: `absolute` anchored to the container's **left** edge (`top-full`, `left-0`), `z-40` (above the strip's `z-30`), `mt-1`, `w-max`, `max-w-60` (15rem / 240px). **Left-anchoring is deliberate (not `right-0`):** on a 390px phone the toggle flex-wraps to the left of its row (x≈16px, the strip's `px-4`); a `right-0` popover would extend *leftward* from the toggle's right edge and project off the left viewport edge, whereas `left-0` extends *rightward* (left edge ≈16px, right edge ≈16+240=256px < 374px = viewport−padding) and stays on-screen. `max-w-60` = 240px is already < the 358px available at 390px, so it never overflows on width alone. Because it is `position:absolute`, it never contributes to the strip's flow **height**. The load-bearing guarantee is the **in-viewport invariant**, pinned by a real-browser assertion (§8.10c): in every popover state at 390px the popover's bounding rect stays within `[0, viewportWidth]` AND the document shows no horizontal scroll (`scrollWidth <= clientWidth`). If a class choice ever violates that, the §8.10c gate fails and the anchor is adjusted — the invariant is the contract, the class is the mechanism.
...
10. **Real-browser 390px strip geometry** (`tests/e2e/*` Playwright, real browser — jsdom computes no layout). Uses a **static** harness in the existing `_showPageLayoutHarness.tsx` pattern (`renderToStaticMarkup` → Tailwind-compiled CSS → served over `node:http` → measured), NOT an interactive-hydration harness (avoids the client-bundle/mock-hydration complexity, which is disproportionate here). The **finalize popover (S4) is prop-driven** — passing `finalizeOwned={true}` renders the real popover in initial markup with NO test-only forced-error path (Codex R5 advisory satisfied: it's the real conditional output). The error popover (S2/S3) shares the S4 popover's **identical positioning + width classes** (only skin/`role`/content differ — pinned in jsdom by §8.11d), so the S4 geometry generalizes to the error popover; no interactive submit is needed for the geometry proof. Render these harness states at 390px:
    ...
    - **(c) Horizontal containment (Codex R4 finding 1):** in the finalize-popover state at 390px, assert the popover's `getBoundingClientRect()` has `left >= 0` and `right <= 390` (fully in viewport), AND the document has no horizontal overflow (`document.documentElement.scrollWidth <= document.documentElement.clientWidth`). Because the error popover shares the S4 positioning classes (§8.11d), this generalizes to it. Run with the strip title at BOTH the short fixture and `SHOWPAGE_LONG_TITLE` (which changes where the toggle wraps, exercising a different toggle x-position). *Catches:* a `max-w`/anchor choice projecting the popover off-screen or forcing a horizontal scrollbar on Doug's phone.
