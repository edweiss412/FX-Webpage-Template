...
- Positioning: `absolute` `top-full right-0`, `z-40` (above the strip's `z-30`), `mt-1`, `w-max`, `max-w-60` (15rem / 240px), and **`break-words`** (`overflow-wrap: break-word`). **Anchored to the sticky StatusStrip, not the toggle (real-browser-verified decision, §8.10c):** the inline container is intentionally NOT `relative`, so the popover's containing block is the nearest *positioned* ancestor — the `sticky` StatusStrip. `right-0`/`top-full` therefore anchor to the STRIP's right edge, just below it, and the popover stays in-viewport at 390px REGARDLESS of where the toggle flex-wraps. This matters because a toggle-anchored popover fails at both extremes the real browser exposed: a *short* title keeps the toggle center-right (x≈218 → a `left-0` popover runs off the right edge at 457px), while a *long* title truncates and wraps the toggle far-left (x≈32 → a `right-0` toggle-anchored popover runs off the left edge at −87px). Anchoring to the full-width strip sidesteps both. **`max-w-60` + `break-words` is the load-bearing horizontal guarantee for ANY content (Codex plan-R3 finding 1):** the error popover's content (`ErrorExplainer`'s long `text-base` paragraph + `HelpAffordance`'s learn-more link) is materially different from the short finalize hint, but `max-w-60` hard-caps the box at 240px and `break-words` forces even long/unbreakable strings to wrap inside it — so the popover is never wider than 240px < 358px regardless of content, and content growth is purely vertical (height, which is out of flow and irrelevant to the strip). Because it is `position:absolute`, it never contributes to the strip's flow **height**. The in-viewport invariant is pinned two ways at 390px (§8.10c): the finalize popover's rect stays within `[0, viewportWidth]` with no document horizontal scroll (proves the strip-anchor + cap in a real browser, at BOTH a short and long title), AND a real-browser probe of the actual `ErrorExplainer`+`HelpAffordance` content in a `max-w-60 break-words` box measures ≤240px wide (proves the real error content respects the cap). If a class/anchor choice ever violates that, the §8.10c gate fails — the invariant is the contract. **(Inline is a strip-only variant** — `StatusStrip` is the sole caller — so relying on the strip as the positioning context is sound; a `card` caller is unaffected.)
...
10. **Real-browser 390px strip geometry** (`tests/e2e/*` Playwright, real browser — jsdom computes no layout). Uses a **static** harness in the existing `_showPageLayoutHarness.tsx` pattern (`renderToStaticMarkup` → Tailwind-compiled CSS → served over `node:http` → measured), NOT an interactive-hydration harness (avoids the client-bundle/mock-hydration complexity, which is disproportionate here). The **finalize popover (S4) is prop-driven** — passing `finalizeOwned={true}` renders the real popover in initial markup with NO test-only forced-error path (Codex R5 advisory satisfied: it's the real conditional output). The error popover (S2/S3) shares the S4 popover's **identical positioning + width classes** (only skin/`role`/content differ — pinned in jsdom by §8.11d), so the S4 *container* geometry generalizes to the error popover; its differing *content* width is measured directly by the (d) probe. No interactive submit / hydration is needed. Render these harness states at 390px:
    ...
    - **(c) Horizontal containment (Codex R4 finding 1, R3 finding 1):** in the finalize-popover state at 390px, assert the popover's `getBoundingClientRect()` has `left >= 0` and `right <= 390` (fully in viewport), AND the document has no horizontal overflow (`document.documentElement.scrollWidth <= document.documentElement.clientWidth`). Run with the strip title at BOTH the short fixture and `SHOWPAGE_LONG_TITLE` (which changes where the toggle wraps, exercising a different toggle x-position). This proves the anchor + cap for the shared popover container.
    - **(d) Error-content probe (Codex R3 finding 1):** because the S2/S3 error popover renders materially different content (`ErrorExplainer` long paragraph + `HelpAffordance` link) than the short finalize hint, the harness ALSO emits a probe: the REAL `<ErrorExplainer code="PUBLISH_BLOCKED_PENDING_REVIEW" surface="admin" />` + `<HelpAffordance code="PUBLISH_BLOCKED_PENDING_REVIEW" />` inside a `max-w-60 break-words` box (the real components; the box carries the width-governing classes the class-equality test §8.11d pins on the actual popover). At 390px assert the probe's `getBoundingClientRect().width <= 240.5` and `right <= 390` — proving the real error content wraps within the cap and never overflows horizontally. *Catches:* an `ErrorExplainer`/`HelpAffordance` child that ignores the cap (e.g. an unbreakable wide token) — the exact content-geometry gap that measuring only the finalize hint would miss.
    *Catches:* the exact CASP-2 defect — any popover or label that inflates the phone strip.

    *(S2/S3/S5 error popovers share the S4 container geometry (§8.11d class-equality pin → same zero flow height + same anchor/cap), and their differing CONTENT width is measured directly by the (d) error-content probe. So the finalize measurement + the class-equality pin + the content probe together cover every popover state without an interactive-hydration harness.)*

11. **Transition-audit state enumeration** (`tests/components/admin/*`, jsdom — new, distinct from §8.8's motion-import pin; Codex R1 finding 2, corrected R2/R3): enumerate the §4.8 reachable states S1–S5. Assert: (a) each state produces the expected popover presence + `role` — **S1** idle → no `published-toggle-popover`; **S2** (mocked known refusal, submitted from the enabled switch) → popover with `role="alert"`; **S3** (mocked unknown-code, submitted) → popover with `role="alert"` + retry copy; **S4** (`finalizeOwned` true, switch disabled, no error) → popover WITHOUT `role="alert"` + the switch's `aria-describedby` matches the popover `id`. (b) The inline branch contains no `AnimatePresence` / motion import / `animate-[` (belt-and-suspenders with §8.8). (c) **S5 compound (error preserved across finalize flip):** render enabled, submit a mocked refusal (S2), `rerender` with `finalizeOwned=true`; assert the ERROR popover (`role="alert"`) still shows — not the finalize hint — and the switch is now `disabled`. This is the same assertion as §8.6, exercised here as the compound-transition pin the writing-plans transition-audit rule mandates. (d) **Popover positioning-class equality (load-bearing for §8.10's geometry generalization):** assert the error popover (S2) and the finalize popover (S4) render with the SAME positioning + width classes (`absolute`, `top-full`, `right-0`, `max-w-60`, `w-max`, `break-words`, `z-40`, `mt-1`) — extract the shared positioning class string to one constant used by both branches and assert both DOM nodes carry it; the ONLY permitted difference is the skin (`bg-warning-bg`/`border-border-strong` vs `bg-surface`/`border-border`), `role`, and content. *Catches:* an unenumerated state (a finalize popover wrongly keeping `role="alert"`, a missing `aria-describedby`, or a preserved error dropped when finalize flips in) AND a positioning drift between the two popover skins that would break the §8.10 real-browser geometry generalization.
