...
- **Tight fence matcher on the persist UPDATE (Codex plan-R1 HIGH).** A loose `/UPDATE…RETURNING true AS ok/` would false-green a regression that strips the lease-owner or active-session fence. The tx#2 UPDATE expectation MUST require the SQL text to contain the active-session `EXISTS (… app_settings … pending_wizard_session_id)` fence AND the lease-owned `EXISTS (… agenda_extract_leases … owner … expires_at > now())` guard AND `staged_id` AND `staged_modified_time`, and MUST value-bind the interpolated `wizardSessionId`/`driveFileId`/`stagedId`/`owner` (the fake receives `(strings, ...values)` postgres.js-style). Same value-bind rigor on the tx#1a claim INSERT (`owner`), the tx#2 show-lock (`driveFileId`), and the release DELETE (`owner`, so it is owner-scoped not blanket). The proof thus pins WHO/WHICH-SHOW, not merely statement shape.
...
12. **A4 persist-UPDATE matcher is fence-tight + value-bound (Codex plan-R1 HIGH, closed §4.2).** The tx#2 UPDATE expectation requires both `EXISTS` fences (active-session + lease-owned) + staged fences + value-binds owner/session/drive/staged. Do not propose loosening to a tail-only `RETURNING true AS ok` match.
