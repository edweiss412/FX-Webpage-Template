...
- **Active-section rule:** reading sections in registry order (§6.1), `active` = the LAST section whose `offsetTop` (relative to the scroll container) `≤ scrollTop + SCROLL_SPY_OFFSET_PX`. If none qualifies (scrolled above the first section), `active` = the first rendered section. Registry order is strictly increasing in `offsetTop`, so "last that qualifies" is unambiguous — no tie-break needed; sections taller than the viewport stay active until the NEXT section's top crosses the offset line.
...
- **Click override:** clicking a rail item / chip sets `active` to the target immediately and calls `scrollTo({ top: section.offsetTop − 8 })` (JS passes no `behavior`; CSS `motion-safe` `scroll-behavior: smooth` governs, §11 T6). Scroll events during the glide re-run the rule (C4 — intermediate items may flash; converges because the rule is position-deterministic).
...
| Warnings | Existing row anatomy (severity dot, title, context, "Open in Sheet ↗" deep link) inside the new panel; severity icon chip `bg-warning-bg text-warning-text` (warn) / `bg-info-bg text-text-subtle` (info). Affirmative empty state per §3.10. **Title derivation is hardened (invariant 5):** cataloged code (`isMessageCode(w.code)`) → `messageFor(w.code).title`; else if `w.message` is non-empty AND `w.message !== w.code` → `w.message`; else the generic human fallback `"A parse issue was recorded for this sheet."`. Rationale: persisted warnings exist whose `message` IS the raw code (`reelWarning` at `lib/sync/phase2.ts:231-233`, e.g. `OPENING_REEL_UNREADABLE`), and the per-show page already pins the no-raw-code rule (`tests/app/admin/perShowPage.test.tsx` asserts `OPENING_REEL_UNREADABLE` never renders). The helper is a small exported function (in `step3ReviewSections.tsx`) so the regression test targets it AND the rendered panel. |
...
- **Controlled (all production mounts):** `onToggleChecked` changes type to `(next: boolean) => Promise<boolean>`. `Step3Review` resolves it from the machinery it already has: the overlay/flush loop (`Step3Review.tsx:681-744`) tracks per-row settlement (`markSending`/`sendingRef`); the handler registers a per-`driveFileId` resolver that the flush resolves when THAT row's write settles — and the resolution is VALUE-SPECIFIC, not merely settlement-specific: `requestSetChecked(true)` resolves `true` only when the row's settled final state equals `true`. Concretely: refusal/`{ok:false}`/network failure on the final POST resolves `false` (matching `postApproval` semantics `Step3Review.tsx:768-790`); an overlay entry dropped as already-matching-server resolves to that matching value's equality with the request; and if a LATER coalesced intent changed the row's target (e.g. the card checkbox flipped it to `false` while the modal's request was in flight, §11 C7), the original `true` request resolves `false` even though the final write itself succeeded — the modal must never close-and-announce "Selected to publish" over a row whose settled state is unselected. Pinned by a concurrency test: modal publish pending, flip the shared state to `false` before flush settlement, assert the modal's promise resolves `false`, the modal stays open, and no success announcement fires. The card checkbox's click path calls the same function and deliberately ignores the promise (fire-and-forget, current UX preserved); only the modal awaits it.
