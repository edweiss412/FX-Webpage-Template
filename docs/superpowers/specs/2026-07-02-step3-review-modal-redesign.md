...
- **Controlled (all production mounts):** `onToggleChecked` changes type to `(next: boolean) => Promise<boolean>`. `Step3Review` resolves it from the machinery it already has: the overlay/flush loop (`Step3Review.tsx:681-744`) tracks per-row settlement (`markSending`/`sendingRef`); the handler registers a per-`driveFileId` resolver that the flush resolves when THAT row's write settles — with the outcome of the FINAL coalesced POST for the row (a refusal/`{ok:false}`/network failure resolves `false`, matching `postApproval` semantics `Step3Review.tsx:768-790`; an overlay entry dropped as already-matching-server resolves `true`). The card checkbox's click path calls the same function and deliberately ignores the promise (fire-and-forget, current UX preserved); only the modal awaits it.
