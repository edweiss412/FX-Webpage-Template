...
- **AC-4 (census total):** on the live corpus: `undisposed === []`, `ambiguous === []`, drift `=== []`, every rule matches ≥1 candidate; `d/skeleton-tolerant-click` no longer exists; `f/member-shape-U-frame` claims the deeplink site; `N_WAIT_SITES` reconciles with the new realtime row; `PINNED_EXEMPTIONS` holds exactly the one `/non-member/` entry and the inventory test passes without a length literal edit.
