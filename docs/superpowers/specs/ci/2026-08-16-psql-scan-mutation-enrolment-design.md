...
- **A kill that also proves a real defect** (the mutant exposes scanner behavior that is WRONG today — the ORIGINAL returns a wrong verdict on some input, demonstrated by probe, not merely untested) upgrades to a source fix + test in this arc if the fix is local, else files a ledger entry named in the row. The bar is a probe against the ORIGINAL: a fixture where only the MUTANT misbehaves is an ordinary §2.1 KILL, never a source change. Worked counterexample (spec review R2, probed): `relational-boundary:528:47:>>>=` — the BACKLOG entry's `token.length > 1` starter — admits bare `-` into the flag-cluster branch ONLY under the mutant; the original already classifies `["-", "-X"]` as positional-terminated (`false`). It dispositions as a plain KILL with that fixture, one token substitution from the existing `["dsn", "-X"] → false` suite case; no source churn.
