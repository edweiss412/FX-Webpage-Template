...
## What the LIVE tier caught that 169 in-process cases could not
...
| `AC-9.k1` — a CLI-shaped surface the runner cannot overlay | ABSENT | The core is an importable module with a referring in-process suite (169 cases), and both adapters are thin `main(argv, deps)` entries holding no decision. A CLI-shaped surface would score as if untested; this one is imported directly by its suite. |
