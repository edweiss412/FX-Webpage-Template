...
| `unclassifiable` | any other reported site or edge, with a reason a reviewer can check | 6 at BASE — the five non-literal dynamic-import edges named in §2.4, plus one path-shaped specifier that resolves to NO FILE (§3.11); every target resolves outside `tests/` to a non-connecting module (a help page, a Vite plugin fixture output, two walker targets, a parser block), which each row's reason states and a reviewer can check by reading the line |
...
- each accepted class has ≥ 1 live member (`guard-bound` 85 sites, `validation-env` 79,
...
### §3.11 What the implementation measured that §3.5 did not predict

§3.5 predicted seven disposition rows at BASE. The shipped census reports EIGHT, and the
difference is not a defect in either direction — it is a class the three committed probes
never ranged over. Recorded here rather than by silently editing the prediction, because a
number that moves without its reason is the next stale claim.

| what the live census reported | why §3.5 did not predict it | disposition |
| --- | --- | --- |
| a path-shaped specifier that resolves to NO FILE (`./state`, in `tests/cross-cutting/fixtures/no-global-cursor/bad-aliased.ts`) | `probe-loaders-options.mts` counted NON-LITERAL specifiers; a literal specifier whose target does not exist was never a measured class | the eighth row, kind `unclassifiable`. The fixture is read with `readFileSync` by `tests/cross-cutting/no-global-cursor.test.ts` and never imported |
| 50+ files naming the driver binding in a TYPE (`postgres.JSONValue`, `ReturnType<typeof postgres>`) | §2.2's `value-reference` rule says "any identifier reference that is not the callee of a call", and a type position satisfies that wording while reading no value | the classifier asks the compiler for the first `ts.isTypeNode` ancestor. Reporting them would be 50+ FALSE reports, which §0 makes a defect exactly as a missed file is |
| three edges to NON-SOURCE targets (a JSON baseline, two CSS fixtures) | the population is source files; §2.4 never said what an edge to a data file is | a DECIDED non-edge, keyed on the source-extension set the walk itself uses — one set, two consumers |
| five NodeNext specifiers, each naming the EMITTED extension of a TypeScript sibling (a `.js` suffix naming `tests/db/_resetRpcSource.ts`) | resolution is injected, and §2.4 states the census's half (which specifiers are path-shaped) without stating the resolver's | the gate's resolver tries the TypeScript siblings of a `.js` specifier. This is the RESOLVER's completeness, not the census's |
| two edges into `tests/db/__generated__/` | §1.1 describes the walk as skipping `__generated__`, following the destructive guard | the census walks it. A directory the corpus imports FROM cannot be excluded from the population without manufacturing a population-gap report for every edge into it |

All five are the same shape: the predicted BASE report set was derived from probes that
ranged over fewer classes than the shipped census decides. Each repair moved the census
toward reporting exactly what the corpus needs — the direction §0 makes a requirement in
BOTH directions, since a report the corpus does not need is as much a defect as a file the
census never names.
