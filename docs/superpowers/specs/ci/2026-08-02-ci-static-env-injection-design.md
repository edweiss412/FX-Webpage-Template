...
- **Governance equality (R3):** every row's `governs` equals the live derivation (`envPairGovernance` over the parsed workflows + `governanceViolations`, both pure so §3 S8 can feed doctored trees). Relocating a pair away from the claims it gates, or a pair silently gaining governance of new claims, reds with the mismatch named.
...
| **S8 governance-binding deletion** (R3 BLOCKING, live probe) | hygiene narrowed to pair-presence, or `governs` ignored | doctored-tree twins through the pure checker: a pair RELOCATED from its claiming job to a non-claiming site reds (declared governs no longer derivable); a pair silently GAINING governance reds; the faithful tree passes. Kills the R3 park-the-flag-elsewhere mutant |
