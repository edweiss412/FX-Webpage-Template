...
| **S8 governance-binding deletion** (R3 BLOCKING + R4 recognizer laundering + R5 duplicate substitution, live probes) | hygiene narrowed to pair-presence, `governs` ignored, the derivation's recognizer widened past the scan's, or the derivation made qualification-blind | doctored-tree twins through the pure checker: a pair RELOCATED from its claiming job to a non-claiming site reds; a pair silently GAINING governance reds; a pair parked on an `echo tests/e2e/…` prose step confers NO governance and the declaring row reds (R4); a pair parked on a PATH-GATED duplicate of the real invocation confers nothing and reds (R5 — governance is credited only at the scan's covered.add site); an ALIAS-resolved claim DOES confer governance; the faithful tree passes |
