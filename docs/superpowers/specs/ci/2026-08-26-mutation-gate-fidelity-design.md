...
```
# measured at 75b8f7a3e, this branch's base
surfaces = 52
...
**A faulted surface must still emit a run record.** `evaluateSurface` calls `emitRunRecord` only after `runSurface` returns, so today a throw writes nothing — which is exactly why run 32958581720 uploaded 50 records for 52 surfaces (§2.2). That collides with the workflow: the records upload sets `if-no-files-found: error`, licensed by the explicit premise that "every shard is assigned at least one surface and every evaluated surface emits a record, so zero files means the leg produced nothing." Leg 0 holds exactly **one** surface, so under an isolation change that left the record path alone, that surface faulting would empty the directory and red the upload with `no files found` — a second, misleading red saying nothing about the cause. That is a property of the partition rather than of one registry snapshot: re-derived after this branch absorbed main, the registry is 54 surfaces and leg 0 still holds exactly one. R1 therefore emits a record for a faulted surface too (`passed: false`, `score: 0`, `outcomes: []`, plus the fault), which keeps the upload premise true and puts the fault where the attribution artifacts and the rate-drift report can see it.
...
- **EXTENDS:** `tests/mutation/_metaSourceShardIntegrity.test.ts` — the workflow changes in §3.2 and §3.4 need assertions, and a workflow edit with no assertion is not TDD. Its EXISTING assertions are unaffected (step 0, the single `$GITHUB_ENV` writer, the upload names, and the step-level `if:` pins inside `budget`); the file gains cases, it does not lose any. **This corrects an earlier draft that listed the file as untouched**, which was true of the design as first written and false once the plan gave those workflow changes their tests.
- **UNTOUCHED, declared:** `_metaGuardSurfaceRegistry.test.ts`, `_metaSpawnDisposition.test.ts` — no new or renamed ceiling constant, no `millisPerBoot` bound change.
