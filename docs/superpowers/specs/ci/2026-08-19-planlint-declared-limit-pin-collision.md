...
| …whose header line itself carries a path (inline form) | 696 |
| …of those, carrying a ROOT-file path only (`BACKLOG.md`, `package.json`) | 54 |
...
The 40-plan difference is prose citation: plans naming `_metaPremiseContract.test.ts`, `_metaReviewRoundEconomy.test.ts` or `numerics.ts` as context, contracts to satisfy, or prior art. None of them edits the surface. The Files declaration is the plan's own declaration of what it touches, so it is the grain — declared, never inferred, the same principle the task-marker contract rests on (`docs/agents/spec-self-review.md`).
...
3. **MULTI-LINE ordinary string literals** are blanked too — a string whose start and end sit on
   different physical lines, which in TypeScript means a backslash-continued one. Round 5 found the peer
   the template rule missed: such a string can contain a physical line beginning
   `test("… documented limit …")`, and preserving it wholesale let fixture text read as a live pin.
   SINGLE-LINE string literals are LEFT INTACT and must be, because they carry the titles — and a title
   literal is single-line by §3.1 item 2, so nothing the arm needs is ever blanked.

**Diagnostics are taken from the RAW source, BEFORE any blanking, and the order is load-bearing**
(§3.4, §8 item 15). Stripping first removes the very text that makes a file unparseable — an
unterminated `/*` is consumed as a comment to EOF — so a strip-then-parse adapter sees a clean parse,
blanks the pin along with everything after the opener, and reports "no pins" with no unreadable
advisory: the exact silent fail-open the decline exists to prevent. Parse once on the raw text to decide
readability, then blank.
...
| §3.2 naming, trailing side | bare substring | a `.bak` sibling of a live entry |
| §3.2 naming, LEADING side | check only the character AFTER the match | a PREFIXED path (`archive/` + a live entry), which an after-only implementation wrongly reads as the enrolled surface |
...
| §4 adapter, PREPARATION | pass raw lines | AC-10 step 1, IN PROCESS over decoy-bearing suite text — the shipped CLI cannot serve this, since it never reads a fixture suite |
| §4 adapter, WIRING | never inject the table | AC-10 step 2, a real subprocess over a REAL enrolled surface, asserted by that surface's specific `(suitePath, title)` |
...
- **Unreadable suite (§3.4), BOTH channels.** A resolver returning `null` for a `suitePath` draws `DECLARED_LIMIT_PIN_SUITE_UNREADABLE`; SEPARATELY, a `suitePath` absent from `listTrackedFiles()` draws it too, asserted with a fake resolver whose `readFileLines` SUCCEEDS for that path and returns real pin-bearing text. That second case is the one a tracking-blind implementation passes silently, and it is the reason the arm cannot rest on `readFileLines` alone. **All THREE channels assert that the OTHER suite's pins still report**, so an implementation abandoning the whole surface on one bad suite fails. The parse channel needs this most and had it least: live enrolled surfaces genuinely have several suites — `reviewRoundCount` names `tests/reviewRounds/count.test.ts` AND `tests/docs/_metaReviewRoundEconomy.test.ts`, and the pins live in the second — so an implementation that `return`s from the SURFACE on a parse failure while correctly continuing for the other two channels passes every other fixture and silently suppresses a live pin. The fixture therefore pairs an unparseable first suite with a pin-bearing second and asserts both the advisory and the pin.
...
9. **A Files declaration written in an unmodeled shape is not read.** A plan that lists files in a table, a fenced block, or prose under a differently-worded heading draws nothing. Measured fence-aware, against the rule §3.2 actually states rather than an earlier draft of it: of 2567 `**Files:**` headers, 696 carry the paths inline and 1666 are immediately followed by an unordered run — the two modeled shapes — while 19 open an ordered run (declined, item 11) and 882 are followed by neither, a population that includes every blank-then-list header the no-blank-skip rule declines (item 14). Silence from this arm is not a certificate.
...
- `docs/superpowers/specs/ci/README.md`: one row for this spec. The PER-DIRECTORY index is the one `tests/docs/specsReadmeIndexParity.test.ts` enforces — it asserts each README indexes every doc beside it — and an earlier draft of this section named the root index, which that gate does not check for this file.
...
- **AC-8** — Enrolment: `declaredLimitPins` carries both declarations (registry row AND `EXPECTED_LEDGER_KINDS`), scores at or above 0.95 with an empty unaccepted-survivor set, and the purity meta-test passes. **`parse.ts` unmodified is proved by a DIFF, not by the meta-test**, which only rejects `node:fs`, `node:child_process` and `node:process` imports and cannot see an unrelated edit; typecheck cannot see one either. The evidence is `git diff --exit-code origin/main -- lib/specLint/parse.ts`, run at closeout.
...
- **AC-10** — The adapter boundary is proved in TWO steps, because the shipped CLI never reads a fixture suite: PREPARATION in process, over suite text carrying one live pin and one differently-titled decoy per §3.1 channel, asserting no decoy title survives into the prepared text; and WIRING by subprocess, over a fixture plan naming a REAL enrolled surface, asserting the advisory appears identified by that surface's specific `(suitePath, title)`. Neither step covers the other. Two findings prove the adapter did not prepare the suite text; zero prove it did not inject the surface table. Both directions asserted. This is the only acceptance criterion that covers the adapter boundary, and every other §6 case is satisfiable without it.
