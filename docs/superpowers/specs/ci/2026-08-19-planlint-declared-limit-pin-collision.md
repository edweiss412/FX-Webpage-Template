...
| §3.4 channels | honor `readFileLines` only | an untracked suite whose read SUCCEEDS; and an unparseable one |
...
- **CLI boundary proof (a real subprocess, and the ONLY proof that covers preparation).** Every other case in this section exercises the pure core with prepared lines supplied by the test, so a shipped adapter that passes RAW lines — or never injects the surface table at all — satisfies all of them and the whole round-3 false-advisory class survives where it actually lives. This case therefore runs the shipped `spec:lint` binary over a fixture plan and a fixture suite in which one pin is live and a second, identical but for being wrapped in `/* … */`, is not. The run must emit exactly one `DECLARED_LIMIT_PIN_UNNAMED`, naming the live pin. A run emitting two proves the adapter never prepared; a run emitting none proves it never injected. Both failure directions are asserted, because either alone is satisfiable by the other defect.
...
- **AC-10** — The shipped CLI, as a real subprocess, emits exactly one `DECLARED_LIMIT_PIN_UNNAMED` over a fixture pair holding one live pin and one identical pin inside a block comment. Two findings prove the adapter did not prepare the suite text; zero prove it did not inject the surface table. Both directions asserted. This is the only acceptance criterion that covers the adapter boundary, and every other §6 case is satisfiable without it.
