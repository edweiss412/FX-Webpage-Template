...
- **AC-2** — Files-block grain: an enrolled path inside a modeled Files block names its surface; the same path in prose outside every block names nothing; a `**Files:**` line inside a fence is inert. Proved by the extent suite, and by the corpus assertion of AC-6, whose corpus contains the prose-only plans of §2.5 (63 whole-document minus 25 Files-declaration at the §2 baseline).
