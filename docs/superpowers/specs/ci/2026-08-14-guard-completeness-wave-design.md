...
Every entry this wave resolves leaves the open queue with a durable archive record AND a `BACKLOG_GRADUATED` registry row in `tests/docs/_metaDeferralLedgerGraduation.test.ts` (row shape: entry id + per-entry `provenance` string the archived section must contain — this branch), in the same commit as its archive move; in-progress markers come off in the PR's last commit per invariant 12:
