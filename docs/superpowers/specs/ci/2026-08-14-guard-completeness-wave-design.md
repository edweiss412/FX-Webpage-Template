...
**Fault repair folded in (spec review R1 F2 — probe showed the documented "ENOBUFS is loud" claim FALSE for `prList`).** `prList` returns `[]` on `r.status !== 0 || !r.stdout` and never reads `r.error` (`scripts/lib/ledger-git.ts:154` region), so a spawn-level fault — ENOBUFS past Node's 1 MiB default, ETIMEDOUT at GH_MS, gh missing — reads as an EMPTY open-PR universe, and `resolveClaims` consumes it with no degraded marker (`scripts/lib/ledger-claims-core.ts:201`). A fault that silently shrinks the claim universe is a false "no collision" — precisely the defect class invariant 12 exists to stop, and a consequence-bound violation (silently wrong, no signal). Repair, scoped to the two touched modules:

- `prList` throws on `r.error` and on `r.status !== 0` (same posture as `localRefs`, whose own comment states the rationale: an enumeration failure is an UNKNOWN universe, not an empty one — `scripts/lib/ledger-git.ts:111-113`). A clean gh run with zero PRs still returns `[]` via its JSON parse.
- `resolveClaims` wraps the `git.prList()` call in try/catch and pushes a `pr-universe-unavailable: <message>` degraded marker (the established idiom at `scripts/lib/ledger-claims-core.ts:139`), so a gh-less machine degrades VISIBLY (exit-2 "check could not be trusted" semantics) instead of resolving claims against a universe that silently lost its PR leg.
...
1. **`prList` fault semantics change is deliberate and bounded** (§3.4): faults now throw and surface as a degraded marker; only a clean zero-PR gh run yields `[]`. The prior "overflow is loud" belief was FALSE (spec review R1 F2 probe: ENOBUFS lands in the `status !== 0` branch and returned `[]`), which is why this is a repair and not scope creep.
...
- **Entry B:** new seam test (recording fake spawn) asserting per-reader `timeout`/`maxBuffer` literals across all six spawn sites; prList fault cases (spawn error object, non-zero exit) assert a THROW, and a `resolveClaims` case asserts the `pr-universe-unavailable` degraded marker; existing `ledgerClaimsCheck` real-git suites green (the gh-fault case at its lines 1150-1170 updated to the new contract); `pnpm heavy pnpm mutation:guards` — `ledgerGit` at `{ equivalent: 6, "accepted-gap": 0 }`, zero unaccepted survivors, ledger siteIds reconciled after line shifts; spot-check one timeout and one maxBuffer mutant by hand (mutate-your-own-fix).
