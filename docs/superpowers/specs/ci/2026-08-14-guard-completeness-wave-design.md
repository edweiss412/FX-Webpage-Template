...
Discovery lives in `tests/db/_metaDestructiveDbTargetGuard.test.ts` (191 lines): a filesystem walk of `tests/` (lines 81-91) matched against destructive-spelling regexes — `EXECUTES_WIPE` (line 58), `ENABLES_WIPE_GATE` (line 63), `EXECUTES_PRUNE` (line 71) — over the union of JS-stripped and SQL-stripped views (lines 117-120), then `analyseDestructiveFile` per discovered file (lines 165-190). Discovered population today: 9 files, all verdict ok. The rejection corpus is `tests/db/destructiveFileAnalysis.test.ts` (397 lines): 2 accept cases, 26 labeled rejection blocks expanding to 32 mutant sources, all inline template strings (lines 25-30).
...
**Every DB execution in a discovered file must run on a client in the checked set.**
...
- **Execution site:** any identifier used as a tagged-template tag (`` sql`...` ``), and any property call on a client identifier (`.unsafe(...)`, `.begin(...)`, `.end()`, …). A tagged-template tag or property-call receiver that is not a checked client fails with a named reason. A `.begin`/transaction callback parameter of a checked client is itself checked.
...
Acquisition then stops mattering, exactly as the entry's acceptance frames it: a client obtained via `require()`, `import()`, `createRequire`, `getBuiltinModule`, a namespace import, or any route nobody has enumerated yet simply is not in the checked set, and its first execution site rejects. The acquisition rules at lines 117-163, lines 178-216, lines 227-263 are **deleted** — the module gets smaller, per the entry's own acceptance line. Destructive-statement spelling no longer matters INSIDE a discovered file either: all execution sites are checked, not only destructive-looking ones, which is strictly stronger than the current per-statement posture.
...
2. All 9 real discovered files still pass, including `resetValidationDataConcurrency.test.ts` through the factory summary.
3. A file acquiring a driver by a route not in the old rejection list is rejected because its client is not in the checked set — new fixtures prove it for at least: an unenumerated dynamic acquisition shape, and a factory the summary cannot classify.
4. The acquisition rules are deleted; net module line count decreases.
5. Discovery (`_metaDestructiveDbTargetGuard.test.ts`) is unchanged in this wave — see §2.5.
...
The comparator is extracted as a small exported helper in `pgCronSmokes.ts` (pure function: `(url: URL, mode, gucValue) => {ok} | {ok:false; reason}`) so its failure modes are unit-testable without a DB.
...
- **Entry A:** the existing 32-fixture rejection corpus re-asserted with per-fixture REASONS (anti-tautology: each fixture pins the reason class it now trips, so a fixture cannot pass by tripping an unrelated check); new accept fixture for the factory pattern (`newConn` shape); new reject fixtures for an unclassifiable factory and an unenumerated acquisition route; the 9 real discovered files pass; meta-test discovery unchanged (its own suite stays green).
- **Entry B:** new seam test (recording fake spawn) asserting per-reader `timeout`/`maxBuffer` literals, including the PINNED ABSENCE of `maxBuffer` on `localRefs`/`prList`; existing `ledgerClaimsCheck` real-git suites unchanged and green; `pnpm heavy pnpm mutation:guards` — `ledgerGit` at `{ equivalent: 6, "accepted-gap": 0 }`, zero unaccepted survivors, ledger siteIds reconciled after line shifts; spot-check one timeout and one maxBuffer mutant by hand (mutate-your-own-fix).
- **Entry C:** docs-only — `pnpm ledger:*` guard suites green after the archive move (`tests/docs/_metaLedgerInProgress.test.ts` archives-reject-in-flight rule: the marker comes off in the same commit that archives, per invariant 12).
- **Entry D:** comparator unit cases (accept + http-for-https + preview host + explicit port + foreign host + empty-GUC-local); census assertion wired for every job row in both modes; one sabotage integration case (re-baked `http://` command in a rolled-back tx goes red by name); existing 10 suite cases stay green; validation-mode run via the `x-audits.yml` path exercised in CI before merge.
...
- Widening `localRefs`/`prList` `maxBuffer` (§6.1 — behavior change this entry does not need).
- Mutation-registry enrolment of the wiring guard or the pg-cron suite (logic inline in test files; the registry cannot express them as shipped — restructuring is not this wave's work).
