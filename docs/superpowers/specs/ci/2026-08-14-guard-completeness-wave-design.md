...
- `prList` throws on `r.error`, on `r.status !== 0`, AND on every status-zero malformed-output class spec review R2 F2 enumerated by probe: empty stdout, invalid JSON, non-array JSON, and array rows missing a numeric `number` or string `headRefName`. `[]` is returned ONLY when gh exits 0 and the payload parses to a well-formed (possibly empty) row array (`scripts/lib/ledger-git.ts:154-170` today returns `[]` or a null-padded row for all of these).
