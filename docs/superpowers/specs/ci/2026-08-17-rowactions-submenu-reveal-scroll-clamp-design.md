...
The pins converge against a CLOSED two-family mutant set — the mutation-family-closure form. A reviewer-proposed third family is admissible only with a live escaping mutant against the shipped pins.
...
- **useFitWithinClip:** the family-A fitted→unclipped case already discriminates family B — a migration reading the previous fit through `getComputedStyle` retains the stale cap and fails it.

**Mutant validation at implementation (AC-7):** per site, TWO deliberate temporary mutants — (A) drop the null-branch application (where one exists), (B) re-order/skip the natural-measure so measurement runs capped — each observed red against the site's pins, then reverted; both observations recorded in the task's commit message. The pins are regression pins, not REDs — the pre-migration tree passes them by design (current behavior is correct); their red condition is a defective migration.
...
- **AC-7 (transition pins, R2 F2 / R3 F1):** the §5.4 pins exist per the family matrix (family A at its three sites; family B at all four sites), and each site's pins have been shown red against BOTH deliberate migration mutants that apply to it (drop-branch; capped measurement), recorded in the implementing commit.
