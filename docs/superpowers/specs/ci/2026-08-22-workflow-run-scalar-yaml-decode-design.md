...
| AC-10 | Every live claim that a quoted executable scalar is not read is SUPERSEDED, and the two declared-limit rows are re-pinned as HITS. | `pnpm exec tsx docs/superpowers/specs/ci/probes/2026-08-22-quoted-run-claim-sweep.mts`. It gates REPAIR, not its own existence — the earlier version asserted only that five known instances EXIST, which the entirely unrepaired tree satisfied. Each live hit must carry a supersession marker within twelve lines or sit in an EXEMPT entry with its reason. On the pre-implementation tree it exits 1 naming 26 unmarked claims. It also self-tests against six known instances and exits 2 on a miss. |
| AC-8 | This arc's diff to `scan.ts` touches ONLY the declarations it is allowed to touch. | `node docs/superpowers/specs/ci/probes/2026-08-22-seam-check.mjs` — an ALLOWLIST. Two denylist designs preceded it and review found a hole in each, plus a third in the derived version: a denylist accepts whatever it did not model, and the surface it must model is not enumerable by hand. The check now names what this arc MAY change and denies the rest of the file by default, so a construct nobody thought of is covered rather than exempt. Both hunk sides are read, each against its own revision. Validated retroactively against all four historical escapes. |
