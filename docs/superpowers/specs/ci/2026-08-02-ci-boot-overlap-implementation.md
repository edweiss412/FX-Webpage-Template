...
**Probe A — host, `pnpm rebuild` (Darwin arm64). END-STATE ONLY.** Mark a timestamp, run `pnpm rebuild`, then look for anything under `supabase/` newer than the mark and for any git-visible change under `supabase/`. This observes post-run state, so it cannot see a transient create-then-delete; only probe B's live watch does. Read it as "nothing was left behind", not "nothing happened":
...
**Its scope is the DECLARED executable surface, not the transitive closure — deliberately, and this is a documented limit rather than an oversight.** The guard pins everything the REPOSITORY declares about what runs at install time: the root lifecycle keys (including pnpm's own `pnpm:devPreinstall`), the `allowBuilds` map with its booleans, `pnpm-workspace.yaml`'s whole top-level key set (an allowlist of one, so any workspace setting anyone adds fails), the absence of a `package.json` `pnpm` object, the pnpm hook routes, the eight `simple-git-hooks` config paths, the pnpm version, and the resolved versions of the five packages whose scripts actually execute.

It does NOT pin the transitive dependencies those five scripts `require` at runtime — `@sentry/cli`'s `node-fetch`/`https-proxy-agent`/`progress`/`proxy-from-env`/`which`, `sharp`'s `semver`/`detect-libc`, `unrs-resolver`'s `napi-postinstall`, and whatever those in turn load. That set is unbounded and moves on every lockfile refresh. Pinning it has only two honest forms: the whole lockfile, which would turn every routine dependency bump into a spec re-audit and get the guard disabled within a month; or an arbitrary depth-1 subset, which gives the APPEARANCE of closure while the depth-2 hole stays open. Neither is better than saying plainly what this is: a tripwire on the surface the repo controls, whose job is to make a DECLARED change fail loudly and force a re-run of §3.2's probes.

The transitive residual is covered by a different mechanism, not by the tripwire: §7's accept gate is a real observation on the real runner, and every future PR re-runs the whole overlapped step on eight legs. A transitive helper that started writing under `supabase/` would break the boot or the install, on some fraction of those legs, on the PR that introduced it.
...
(h) **Install write-surface guard, corrected inventory.** EIGHT pins, all with failure messages naming §3.2 as the audit to re-run:
  ...
  - **`pnpm-workspace.yaml`'s TOP-LEVEL KEY SET equals exactly `["allowBuilds"]`.** This replaces a per-setting denylist, and it is the only formulation that closes the class. Four review rounds added one route at a time — `configDependencies`, then the legacy `onlyBuiltDependencies`/`onlyBuiltDependenciesFile`, then `dangerouslyAllowAllBuilds` (which permits EVERY dependency build script while the `allowBuilds` map compares equal), then `packages:` (which enrolls another workspace project whose own `preinstall`/`install`/`postinstall`/`prepare` then runs on every install). Each was a real hole and each was found by reading pnpm's source; a denylist has no reason to stop there, because pnpm 10.x has settings this spec has not read and pnpm 11 will have more. An ALLOWLIST of one key fails on any of them, including ones nobody has enumerated, and its failure message says exactly what to do: re-run §3.2's probes and re-audit before adding a workspace setting.
  - **`package.json` declares no `pnpm` object at all** — one assertion covering `pnpm.configDependencies`, `pnpm.onlyBuiltDependencies`, `pnpm.onlyBuiltDependenciesFile` and any future sibling, for the same reason and on the same allowlist logic. It has none today.
  - **The pnpm version itself**, `pnpm@10.33.2`, read from `package.json`'s `packageManager` field — the executor of everything above, and a repo declaration like the rest. A pnpm major or minor bump can change which hooks are loaded and which lifecycle names run, which is the same class of change as adding one.
  - **No pnpm HOOK file or hook config.** pnpm 10.33.2 `require`s its hook modules, so their top-level code runs during install, ahead of and independent of every npm lifecycle name — and, critically, INDEPENDENT of the two allowlist assertions above: a repo-root pnpmfile is loaded whether or not `pnpm-workspace.yaml` or `package.json` mentions it. The guard asserts no repo-root `.pnpmfile` with the `cjs` extension is tracked, and that no tracked `.npmrc` declares `pnpmfile` or `globalPnpmfile`. (The in-file routes — a configured `pnpmfile` path and `configDependencies` plugin pnpmfiles — are already closed by the key-set allowlist and the no-`pnpm`-object assertion.) Restored after review round 17 caught round 16's allowlist edit deleting this bullet: the allowlists close the DECLARED routes, and this closes the one that needs no declaration.
  - **No tracked `simple-git-hooks` CONFIG FILE.** 2.13.1's loader checks EIGHT repo-root paths — `.simple-git-hooks` and `simple-git-hooks`, each with the extensions cjs, js, mjs and json — BEFORE the `package.json` object this guard pins, and the JS forms are executed at load. A newly tracked one could perform a top-level write under `supabase/` while the lifecycle key, the `package.json` config, the `allowBuilds` map and every pinned version stayed identical. The guard asserts none of those eight paths exists.
...
legfix () {  # usage: legfix <RUN_ID> -> per-leg lines on stderr, one summary line on stdout
  ...
  if [ -z "$raw" ]; then
    # A SUCCESSFUL fetch with no matching rows. Piping an empty string through
    # printf emits one BLANK record, which awk would count as a leg — the zero-row
    # branch below would be unreachable and the summary would read "jobs=1 legs=1
    # median=0". Handled here instead.
    echo "FAIL: no unit-suite-db jobs reported at all" >&2
    echo "jobs=0 legs=0 median=NA max-fixed-overhead=NA"
    return 1
  fi
  printf '%s\n' "$raw" | awk -F'\t' '
      { jobs++
...
legwall () {  # usage: legwall <RUN_ID> -> "legs=N max-leg=S", NA, or TRANSPORT-FAIL
  ...
  if [ -z "$raw" ]; then
    echo "FAIL: no unit-suite-db job reported a wall clock" >&2
    echo "legs=0 max-leg=NA"
    return 1
  fi
...
**Validated, not just written down.** Six cases, run rather than reasoned about:
...
| A VALID run of a different workflow, so the fetch succeeds with no matching rows (run 30787331372) | `jobs=0 legs=0 median=NA max-fixed-overhead=NA`, exit 1 — the empty-response branch, added after a review round exercised the earlier form and got `jobs=1 legs=1 median=0` from `printf` emitting one blank record |
| An unreadable run id, so the fetch itself fails | `jobs=TRANSPORT-FAIL legs=TRANSPORT-FAIL median=TRANSPORT-FAIL max-fixed-overhead=TRANSPORT-FAIL`, exit 2 |
...
3. **Accept** if median fixed overhead drops by **≥8s** versus that baseline AND every leg is green. **Evidence readability is checked FIRST, and it outranks both other exits.** If any input is unreadable — either `legfix` exiting 2 (`TRANSPORT-FAIL`), `legwall` exiting 2, or a non-zero status from the boot-log fetch — there is no measurement, so neither accept nor revert can be evaluated and the procedure ESCALATES. A transport failure must never produce a revert: that would destroy a change on the strength of a rate limit.

With readable evidence, **revert item 1** when any §7.3 accept criterion fails on an otherwise-complete run (the median gate or the boot-log criterion), or when the combined step itself failed in any db leg. **Any OTHER non-`success` outcome is not the run's decision to make** — it is a scope question (is this regression this branch's? does it reproduce on `main`? should the branch merge while a required context is red?) and those are the user's calls under this project's autonomy contract, so the procedure stops and escalates with the evidence rather than choosing for them. The plan's T4 owns the exact exits; nothing in this section asks for per-cause diagnosis, `main` comparison or in-branch repair of an unrelated regression — that machinery was removed in review round 11 and is not re-introduced here. **A revert is TERMINAL for item 1 on this branch:** once it has happened, the accept path does not re-open on a later green run, the ≥8s gate is not re-applied, and `BL-CI-OVERLAP-BOOT-WITH-SETUP` does not graduate — there is no overlap left to measure, and runner noise on a reverted tree could otherwise "clear" a threshold for a change that is gone. A persistent failure elsewhere is NOT a revert either — reverting the overlap cannot fix a `unit-suite-nodb`, prerequisite or aggregator regression and would discard a change the evidence does not implicate; it takes the escalate exit like every other non-measurement outcome. The 8s floor is half the theoretical 16s: it demands a real effect while tolerating contention, and is far enough from zero that runner noise cannot manufacture it. A one-second difference is not a gain.
...
Theoretical upper-bound saving 16s (the install step's measured duration, 2026-07-20 §2); accept threshold ≥8s median fixed-overhead reduction (§7.3), deliberately half of it; measured main baseline over 8 legs on run 30783618781 (§7.1): leg-median fixed overhead 96s, worst-leg fixed overhead 112s, max leg wall clock 255s — so the accept figure against that baseline is a median ≤88s; `unit-suite-db` 8 legs, `unit-suite-nodb` 3 legs (§2); `allowBuilds` five keys, four enabled (§3.1); five lifecycle scripts executed by each of the two §3.2 probes; probe A observed no residual change under `supabase/` (end state only), probe B observed zero filesystem events of any kind (live `inotifywait`, the stronger claim); audited executed-code versions `@sentry/cli` 2.58.5, `esbuild` 0.28.0, `sharp` 0.34.5, `unrs-resolver` 1.11.1 and `simple-git-hooks` 2.13.1 — five, not four — plus pnpm 10.33.2 itself (§3.1, pinned by §5h; the transitive closure of those scripts is a documented limit, §3.4); one Linux-only ignored build candidate, `@parcel/watcher` 2.6.0 (§3.2); the setup composite consumed by 31 job steps across 17 workflows (§4); `toPass` timeout 15_000 ms in both item-2 blocks (§6.2), matching `openHub`; test-level timeout 240_000 ms, unchanged; install-failure report delay ~70s typical, hard-bounded only by `timeout-minutes: 20` (§1.1).
