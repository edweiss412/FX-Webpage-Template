...
(h) **Install write-surface guard, corrected inventory.** SEVEN pins, all with failure messages naming §3.2 as the audit to re-run:
  ...
  - **No OTHER declared route into the install's executable surface.** pnpm 10.33.2 reads build-allowance and plugin settings from more places than `pnpm-workspace.yaml`'s `allowBuilds`, and each of them can enlarge the set of packages permitted to run install scripts — or load a plugin pnpmfile — without touching the pinned map. All of them are REPO DECLARATIONS, so all of them are pinnable, and all are absent today (`package.json` has no `pnpm` object; `pnpm-workspace.yaml` holds only `allowBuilds`). The guard asserts each stays absent: `package.json`'s `pnpm.configDependencies`, `pnpm.onlyBuiltDependencies` and `pnpm.onlyBuiltDependenciesFile`; and `pnpm-workspace.yaml`'s `onlyBuiltDependencies` and `onlyBuiltDependenciesFile`. The legacy `onlyBuiltDependencies` forms matter specifically because pnpm APPENDS `allowBuilds: true` entries to an existing array rather than replacing it, so a legacy list would add executing packages while the `allowBuilds` map compared equal.
  ...
  - **No pnpm HOOK file or hook config.** pnpm 10.33.2 `require`s its hook modules, so their top-level code runs during install, ahead of and independent of every npm lifecycle name. Four routes reach one: the default repo-root pnpmfile (a `.pnpmfile` with the `cjs` extension), an explicitly configured `pnpmfile` path, a `globalPnpmfile`, and `configDependencies` plugin pnpmfiles. The guard asserts no repo-root `.pnpmfile` with the `cjs` extension is tracked, and that neither `pnpm-workspace.yaml` nor a tracked `.npmrc` declares `pnpmfile`, `globalPnpmfile` or `configDependencies`. Today the repo tracks none of them (`git ls-files | grep -iE 'pnpmfile|npmrc'` returns nothing, and `pnpm-workspace.yaml` holds only `allowBuilds`), which is exactly why pinning that emptiness is what forces a re-audit when it stops being true.
