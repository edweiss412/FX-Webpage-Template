...
| **The program's originally-sketched Phase-3 subject — per-worker Postgres databases / schema-per-worker / per-worker Supabase stacks — is KILLED, not deferred.** The mandatory spike (§2) shows it is unnecessary: 509 of the 770 residual serial files never touch the DB at all. Building per-worker DB infrastructure to parallelize files that do not use the DB would be pure cost. The remaining files (254 in the spike's accounting, 258 as shipped) stay serial — most are genuinely DB-bound; the rest are retained because they fail an eligibility rule (§2.6), not because DB-boundedness was proven for each | this spec §2, §2.4; program doc §2 explicitly required the spike to "prove or kill" this |
...
| The regeneration PROCEDURE has two modes (measure = full protocol, run manually today; a static `--check` for the future BL-CI-SERIAL-AUDIT-SCRIPT tool) over a membership-independent candidate population, and the partition proof reads resolved config arrays rather than a classifier function | this spec §3.4, §4b; round-2 findings 1-5 |
...
All 562 marker-free files, one scratch config, `fileParallelism: true`, every DB/Supabase endpoint pointed at a closed port (TCP port 9, the discard service). The terminal reporter summarised **514 passed / 48 failed**; the JSON reporter's set of DISTINCT failing paths is **46** (`db` 27, `reports` 11, `sync` 5, `cross-cutting` 2, `notify` 1) — these reach the DB **transitively** through helpers, which the marker scan cannot see. The 2-file discrepancy between the two counts is NOT resolved here and is deliberately not load-bearing: the candidate set was formed conservatively as 562 − 46 = 516 (the spike's historical figure; the shipped list is 509 after §2.6) and then independently RE-VALIDATED green three times (§2.3), so the proof of membership is the repeat runs, not this subtraction. This is also why membership is measured, not inferred (§1.1).

### 2.3 Probe B — is the verified set stable under concurrency?

The 516-file verified set (562 marker-free − 46 measured-DB-bound; this is the SPIKE's historical figure — the shipped list is 509 after the §2.6 rules) was run **four times** with the DB closed and `fileParallelism: true`. First run: 3 transient failures (concurrent with another vitest process finishing). Three consecutive clean repeats afterwards: **0 failures, 0 flakes** (4,920 tests, ~113–118s wall each). The transient trio is attributed to host contention, not the set; CI legs run one vitest process per runner.

### 2.4 Probe C — the residual serial set

The remaining **254 files** (the SPIKE's residual; the shipped residual is 258 after §2.6) (`db` 102, `onboarding` 37, `sync` 36, `notify` 15, `reports` 12, `cross-cutting` 9, `scripts` 8, `auth`/`admin`/`api` 6 each, `codexGuard` 4, `observe` 3, …) were run serially against the live local DB, minus the three `ENV_BOUND_EXCLUDES` files (251 executed): 235 passed / 15 skipped / 1 failed — the single failure is `validation-check-seed-content-coverage`, the known locally-degraded-DB class that reproduces at merge-base (measured during P2: 45 failures on origin/main config vs 4 on the P2 diff). Conclusion: the residual set is coherent as the serial project.
...
Implementation and whole-diff review surfaced several false-positive classes the §2.2 protocol did not exclude. Two reach the real Postgres in CI (self-skipping DB-gated tests; files referencing a local endpoint directly, on either the Postgres 54322 or the Supabase REST 54321 port); one is a filesystem race rather than a DB one (tests that write repo-tracked files read by parallel workers); the naming-convention rule is defense in depth. The eligibility rules are therefore, in order: (1) candidate population as §3.4; (2) green in all three repeats; (3) **actually EXECUTED assertions in every repeat** — a DB-gated test that self-skips when the DB is unreachable reports "passed" while proving nothing; (4) **no direct reference to a local Postgres endpoint** (port 54322, the LOCAL_TEST_DATABASE_URL variable, or a loopback/localhost Postgres address on port 5432) — such a file bypasses the closed-port redirection entirely; (5) **no DB naming-convention match** (`tests/db/**`, `*.db.test.ts`, `*Db.test.ts`, `*.realdb.test.ts`, `*-real-db.test.ts`) as defense in depth; (6) **does not write to a repo-tracked path** — such a test races parallel readers importing that file across workers (writers confined to mkdtemp/tmpdir stay eligible); (7) **no loopback Supabase REST reference on port 54321** — the same bypass class as rule 4 on a different port. Applied to the measured intersection of 563: rules 3/4/5/6/7 (plus the one already-glob-claimed entry) removed 1 + 26 + 21 + 6 = 54 from the 563-file three-repeat intersection, leaving **509**. Every number elsewhere in this document that predates this amendment (the ≈509 expectation) is superseded by 509.
...
1. **`vitest.projects.ts` gains `PARALLEL_EXTRA_FILES: readonly string[]`** — the 509 exact paths whose membership is fixed by the §2.6 rules (§2.3 supplies the raw three-repeat intersection; §2.6 filters it), sorted, deduplicated. `PARALLEL_TEST_GLOBS` (dir globs plus its exact-file entries, e.g. `tests/sample.test.ts`) is unchanged; **no mixed dir is added to it.**
...
4. **Regeneration procedure (normative; the SCRIPT itself is descoped per §1.0 — this subsection describes what a human, or the future BL-CI-SERIAL-AUDIT-SCRIPT tool, must do)** — two explicit modes, one measurement definition.
   ...
   - **Measure mode (default, manual/periodic, minutes):** runs the §2.2/§2.3 protocol over the whole candidate population — closed-port env, `fileParallelism: true`, repeated `--repeats N` (default 3) — and then applies the §2.6 eligibility rules to that intersection — rule 3 (executed assertions in every repeat), rule 4 (no direct local-Postgres reference), rule 5 (no DB naming-convention match), rule 6 (no write to a repo-tracked path), rule 7 (no loopback Supabase REST on 54321) — emitting only the survivors. Emitting the raw intersection would reproduce 563 entries, not the shipped 509. Marker-bearing files are NOT categorically excluded: markers were only the spike's search heuristic, and membership is decided solely by measured behavior (§1.1). A marker-bearing file that genuinely passes the protocol is eligible.
...
EXTENDS `tests/cross-cutting/vitest-projects-partition.test.ts` — it owns §4a, §4b0, §4b, §4e, §4f. CREATES `tests/cross-cutting/serialAudit.test.ts` — it owns the glob-matcher cases plus §4c (list integrity) and §4d (band). §4g is WITHDRAWN with the audit script (§1.0); no assertion lives in both files.
...
(d) **Anti-vacuity band with an UPPER bound:** `PARALLEL_EXTRA_FILES.length` in `[400, 600]` — the lower bound catches an emptied list, the upper catches a list that accidentally swallowed the serial set. The band is documented as intentionally re-tuned whenever a legitimate re-measurement moves the count (the manual procedure today; the future BL-CI-SERIAL-AUDIT-SCRIPT tool later).
...
(g) WITHDRAWN with the audit script (§1.0). Integrity is enforced by (c)/(d) against the live tree instead.
...
Residual serial post-P2: 770 files, 208 marker, 562 marker-free (§2.1); probe A terminal 514 passed / 48 failed vs 46 distinct failing paths, discrepancy explicitly non-load-bearing (§2.2); spike verified set 516, 4 runs, 3 clean repeats, 4,920 tests (§2.3); shipped list 509 after the §2.6 rules (563 intersection − 54, §2.6); shipped serial residual 258; spike residual 254 files of which 251 executed (3 env-bound excluded), 235 passed / 15 skipped / 1 known-degraded (§2.4); 19 mixed dirs (§2.5); per-leg serial 69 files / 73s, parallel 78 files / 29s, run 29720857479 leg 1 (§2.1); `PARALLEL_EXTRA_FILES` band [400, 600] (§4d); P2 baseline max leg 254s, skew 57s (§5.2); regression floor 300s (§5.3); skew gate 75s (§5.4).
