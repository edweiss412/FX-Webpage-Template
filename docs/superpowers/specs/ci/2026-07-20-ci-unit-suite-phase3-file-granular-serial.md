...
1. **`vitest.projects.ts` gains `PARALLEL_EXTRA_FILES: readonly string[]`** — the 516 spike-verified exact paths (§2.3), sorted, deduplicated. `PARALLEL_TEST_GLOBS` (dir globs plus its exact-file entries, e.g. `tests/sample.test.ts`) is unchanged; **no mixed dir is added to it.**
