...
applied to `stripCommentsForFile(source, file)` output (`tests/_shared/stripComments.ts`). The scanner does not merely detect — it EXTRACTS, per file, the ordered list of `{ kind: "from" | "rpc", literal }` sites (capture groups 1 and 3; group 2 is the quote character, backreferenced so quotes cannot mismatch; the optional `<...>` segment covers the SDK's explicit type arguments — `.rpc<T>("x")` and `.from<A, B>("x")` typecheck against `SupabaseClient` and must be visible, spec R4 F1), which Layer 2 reconciles against the registry. The quote class includes the backtick: a NO-SUBSTITUTION template literal (`` .from(`shows`) ``) is an ordinary string argument that Prettier leaves unchanged, so it must be visible to every layer (spec R2 F2).

**The literal body runs lazily to the BACKREFERENCED closing quote, so the only character it excludes is the one that opened it** (whole-diff R1: the draft's `[^"'`$]+` class excluded all three quote characters and `$` outright, whichever quote had opened the literal). That over-exclusion silently dropped five shapes of ordinary authoring that compile and that Prettier leaves alone — `.rpc("cost$center")`, `.from("crew's")`, `.rpc('say"hi')`, a backticked `` `cost$center` ``, and `.from("")` — and "silently" is exact: the site vanished from the extraction, so Layer 1 saw no call and Layer 2's ordered deep-equal still matched. That is a breach of §1.4's consequence bound, not a documented limit, and it is repaired rather than accepted. Substitution templates are excluded where they actually live: after the match, a backtick-quoted literal containing `${` is dropped as a dynamic name (§6.1). `${` — not the presence of a `$` — is what separates the two.

Two further shapes were silently dropped by the R1 repair itself and are fixed in the same class (whole-diff R2 F1, F2). **The escape alternative is `\\[\s\S]`, not `\\.`** — `.` is not dot-all, so a backslash LINE CONTINUATION inside a name (LF, CR, CRLF, U+2028, U+2029) ended the match and dropped the site. And **the substitution test removes escape PAIRS before looking for `${`**: `` `cost\${center}` `` evaluates to the static string `cost${center}`, so testing the raw text called a static name dynamic and dropped it; removing `\$` (leaving a harmless `{`) and `\\` (so a literal backslash cannot hide a live `${`) makes the test exact. `isSubstitutionTemplate()` is that rule, named, with planted positives and negatives on both sides.

**Nothing between the method name and its argument list is assumed adjacent** (whole-diff R3 F2, F3). Whitespace, the optional generic segment, and an optional `?.` may all sit there: `sb.from <Row>("x")`, `sb.rpc/* note *\/<Row>("x")` (a comment arrives at the scanner as blanks, since stripping preserves offsets) and `sb.rpc?.("x")` all typecheck against the installed `SupabaseClient`, and every one was silently invisible while the pattern required adjacency. The discriminator is unchanged — `Array.from (iterable)` and `Array.from?.(iterable)` still carry no string literal and still do not match.

The string-literal first-argument anchor is the discriminator: it matches every real Supabase builder/RPC call (all 17 current sites name their table/function as a string literal) and rejects the two known non-Supabase shapes in the corpus — `Array.from(` (`lib/data/normalizeDateRestriction.ts`, `const years = Array.from(`) takes no string literal, and the prose mention of `.from()` in the `adminEmails.ts` `wrapInfra` doc comment is stripped before scanning. Calibration: §4.
...
2. an inline `// not-subject-to-meta: <reason>` waiver — recognized ONLY when both hold (spec R3 F3, tightening the auth test's raw substring check): (a) the ORIGINAL source matches `/^[ \t]*\/\/ not-subject-to-meta: \S/m` — the marker OPENS its own line comment and the reason is non-empty — and (b) the marker is ABSENT from the comment-STRIPPED source — present-in-original plus absent-after-strip proves the marker lives in a comment, so the marker inside a string literal cannot waive. **The line anchor is load-bearing and was added by whole-diff R2 (F3):** comment-stripping erases a BLOCK comment exactly as thoroughly as a line comment, so absent-after-strip alone could not tell a real waiver from documentation quoting the directive — `/* // not-subject-to-meta: … */` and a JSDoc line reading ` * // not-subject-to-meta: …` both waived a live call site, silently. Writing about the convention in a JSDoc is ordinary authoring; the anchor makes it inert. A waiver written any other way (trailing on a code line, a doubled space, the block form) is simply unrecognized, so the file stays undischarged and Layer 1 names it — the signalled direction, pinned by self-tests. File-grain otherwise, and see the registry-precedence rule below.
...
| F13 | Site whose literal contains the OTHER quote characters, a bare `$`, an escaped quote, or is empty — `.rpc("cost$center")`, `.from("crew's")`, `.rpc('say"hi')`, `` .from(`cost$center`) ``, `.from("")` (whole-diff R1, demonstrated escaping the draft scanner) | §3.2 backreferenced-quote literal body + Layer 3 planted positives, one per shape. This family is a REPAIR, not an accepted limit: each shape was silently invisible, and §1.4 admits conservative misses only when they are signalled |

## §6 Documented limits (accepted, not findings)

1. **Dynamic call arguments are invisible.** `.from(tableVar)` / `.rpc(fnVar)`, substitution templates (`` .from(`${t}`) ``, `` .from(`a${t}b`) ``), and generic segments containing parentheses or nested angle brackets (e.g. `.rpc<Record<string, () => void>>("x")` — the `[^>()]*` class deliberately stops there) do not match the scanner; no-substitution template literals ARE matched (§3.2, R2 F2), **including ones containing a bare `$`** — only `${` marks a substitution, so this limit covers only genuinely dynamic names and never an ordinary literal that happens to contain a dollar sign (whole-diff R1, F13). No such site exists in `lib/data` (§4), the repo convention is literal table names, and the limit is pinned executable by Layer 3 negative self-tests. Worst case: a dynamically-named read escapes the guard — conservative miss, no silent corruption of anything the guard already covers.
