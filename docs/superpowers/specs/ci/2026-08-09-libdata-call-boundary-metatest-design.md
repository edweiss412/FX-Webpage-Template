...
`Array.from(iterable)`, `Array.from({ length: n })` and `sb.from(tableVar)` remain non-sites for the reason they always did: no static string name. `sb["from"]("x")` IS a site — element access with a literal name costs three lines here and would otherwise have been the next round's finding.
