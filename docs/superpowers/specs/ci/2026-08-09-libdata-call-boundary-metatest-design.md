...
| F29 | A `coveredBy` citation satisfied by a LARGER identifier — `$get` matched by `x$get`, `café` by `caféX`, `admin_emails` by `admin_emails_v2` (whole-diff R7 F1), and any of the seven IdentifierPart families a letter/digit approximation misses: `Mn`, `Mc`, non-underscore `Pc`, `Other_ID_Continue`, non-letter `ID_Start`, ZWNJ, ZWJ (whole-diff R8 F1) | §3.4 rule 3 bounds containment by ECMAScript IdentifierPart, taken from `\p{ID_Continue}` plus `$`/ZWNJ/ZWJ. Planted rejections for the ASCII cases with passing exact-match controls, plus one per Unicode family. Mutants restoring `\b` and restoring the letter/digit approximation red 2 and 1 tests respectively |
