...
| F29 | A `coveredBy` citation satisfied by a LARGER identifier — `$get` matched by `x$get`, `café` by `caféX`, `admin_emails` by `admin_emails_v2` (whole-diff R7 F1) | §3.4 rule 3 bounds containment by identifier characters, Unicode-aware, instead of `\b`; three planted rejections with two passing exact-match controls, plus the literal-operand case. A mutant restoring `\b` reds 2 tests |
