...
| F27 | `Array.from("abc")` classified as a boundary — a LOUD false positive on an innocent line (whole-diff R5 F4) | `isStandardLibraryFrom()` excludes exactly an `Array` receiver; planted negatives plus a `sb.from("abc")` positive proving the exclusion is by receiver. A mutant removing it reds them |
