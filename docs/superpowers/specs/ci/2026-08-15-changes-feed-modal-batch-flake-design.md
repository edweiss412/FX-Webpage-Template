...
- `openShowReviewModal(page: Page, slug: string): Promise<Locator>` with this contract:
  ...
  5. Boundary visible → push a test annotation `{ type: "infra-recovery", description: <run context: slug + attempt> }` via `test.info().annotations` so every recovery is visible in the JSON report and `--reporter=list` output, click `[data-testid="admin-route-error-retry"]` (the product's `reset()` affordance, `app/admin/error.tsx:41-50`), and wait up to 30s for the loaded modal ONLY.
  6. Modal visible after retry → return it. Boundary again or timeout → fail with an error message that names the boundary, the retry, and the `show_review_snapshot_failed` server signature to grep for.
  - Recovery bound is exactly 1 (ratified, §1.1).

Guard conditions: `slug` is the only input beyond `page`; its null/empty behavior is step 1. The helper never swallows the timeout path — a run where neither locator appears within 30s fails with Playwright's normal timeout naming the `or()` pair.

### 4.2 Logging fix: `lib/admin/readShowReviewSnapshot.ts`

Replace the raw-object context value at `lib/admin/readShowReviewSnapshot.ts:51` with explicit fields: `error: error.message, code: error.code, details: error.details, hint: error.hint` (the PostgREST error shape; message-style matches the loader's sibling sites, e.g. `app/admin/_showReviewModal.tsx:110`). The `catch` branch at `lib/admin/readShowReviewSnapshot.ts:60-63` already benefits from `serializeError`'s `instanceof Error` arm and is unchanged. No behavior change — same return values, same log level, same code path; log content only.
...
- **AC-1:** `openShowReviewModal` exists with the §4.1 contract. The empty-slug guard is cheaply assertable and gets a test (the plan chooses the mechanics — pure-function extraction under vitest, or a scoped playwright case; `test.info()` is playwright-runtime, so any annotation assertion must run under playwright). The recovery path's full behavior is exercised structurally (the helper is the ONLY way the spec reaches the modal) rather than by synthetic fault injection (§7, limit 2).
...
- **AC-3:** The fatal snapshot log line carries `message`/`code`/`details`/`hint` fields (assert by scoped test on the log call's context object, or by live-probe transcript in the PR).
...
- **AC-6:** Every `infra-recovery` annotation occurrence in those five runs is reported in the PR (count may be zero; the annotation existing in the report format is verified once in AC-1's scoped test).
