...
- **AC-6:** Every `infra-recovery` annotation occurrence in those five runs is reported in the PR, read from the job logs' §4.4 oracle print (count may be zero). The oracle's annotation-print duty itself gets a vitest case against a fixture report exercising the §4.4 read-location semantics: one test carrying TWO `infra-recovery` annotations in `tests[].annotations`, one carrying one annotation duplicated across `tests[].annotations` AND `results[].annotations` (asserting it prints once — the merged location only), and one with none; the collector returns exactly three rows (red first: the collector does not exist).
