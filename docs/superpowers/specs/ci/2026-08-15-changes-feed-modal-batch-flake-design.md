...
- `openShowReviewModal(page: Page, slug: string, opts?: { timeoutMs?: number }): Promise<Locator>` (per-wait timeout, default 30_000 — the diagnostic e2e case below passes a small value so its deliberate starve does not cost 30s) with this contract:
...
Guard conditions: `slug` and `opts.timeoutMs` are the only inputs beyond `page`; slug's null/empty behavior is step 1; a non-positive/NaN `timeoutMs` falls back to the default. The helper never swallows a timeout — every timeout path exits through step 7's enriched error.

**Step 7 is deterministically testable without fault injection.** A syntactically valid slug with NO `shows` row makes the loader hit its missing-show gate and `redirect("/admin")` (`app/admin/_showReviewModal.tsx:125`) — the dashboard renders with NEITHER the modal NOR the boundary, which is exactly the neither-locator starve. The layout spec therefore gains a diagnostic e2e case: call the helper with a nonexistent slug (e.g. `zz-e2e-no-such-show`) and a small `timeoutMs`, and assert the rejection message names both locators and the `show_review_snapshot_failed` signature. The assertion pins substrings present on BOTH step-7 exits (recovery attempted or not), so an incidental real 502 during this navigation still passes it.

### 4.2 Logging fix: `lib/admin/readShowReviewSnapshot.ts`

Replace the raw-object context value at `lib/admin/readShowReviewSnapshot.ts:51` with explicit fields: `error: error.message, pgrstCode: error.code, pgrstDetails: error.details, pgrstHint: error.hint` (the PostgREST error shape, `PostgrestError { details, hint, code } extends Error` per the pinned postgrest-js 2.105.1 type declarations; message-style matches the loader's sibling sites, e.g. `app/admin/_showReviewModal.tsx:110`). **Deliberately NOT the `code` log field:** that key is the §12.4 telemetry-code slot (`lib/log/types.ts:6`), and the existing suite pins this read path as stamping NO code ("Zero-new-codes constraint", `tests/admin/readShowReviewSnapshot.test.ts:97-114`) — the PostgREST SQLSTATE rides under `pgrst*` keys so the pin stays true. That existing returned-error test case is EXTENDED in the same change (red first, invariant 1): it currently proves nothing about the `error` context value; its fixture error gains `details` and `hint` values, and it gains assertions on ALL FOUR fields — `fields.error` is the message string (`"permission denied"`), `fields.pgrstCode` is `"42501"`, `fields.pgrstDetails` and `fields.pgrstHint` echo the fixture values — plus the unchanged `fields` has no `code` property. The defective production line is `lib/admin/readShowReviewSnapshot.ts:51` (raw object → `serializeError`'s non-`Error` arm → `String(...)` → `'[object Object]'`, `lib/log/serializeError.ts:8-10` via `lib/log/logger.ts:38` — the CI runtime object is a plain object, not `instanceof Error`, which is why the fatal path was unreadable). The `catch` branch at `lib/admin/readShowReviewSnapshot.ts:60-63` is unchanged. No behavior change — same return values, same log level, same code path; log content only.
...
- **AC-1:** `openShowReviewModal` exists with the §4.1 contract. The empty-slug guard is cheaply assertable and gets a test (the plan chooses the mechanics — pure-function extraction under vitest, or a scoped playwright case; `test.info()` is playwright-runtime, so any annotation assertion must run under playwright). Step 7 (enriched neither-locator error) is exercised by the §4.1 dead-slug diagnostic e2e case asserting the rejection message content. The boundary-recovery branch (steps 5-6) has NO deterministic test — that is §7 limit 2, stated there with its accepted survivor.
- **AC-2:** `tests/e2e/admin-changes-feed-layout.spec.ts` passes locally 8/8 — the 3 band cases plus the dead-slug diagnostic case, in both projects (`--project=mobile-safari --project=desktop-chromium`) — against a freshly seeded database using the helper.
...
- **AC-6:** Every `infra-recovery` annotation occurrence in those five runs is reported in the PR, read from the job logs' §4.4 oracle print (count may be zero). The print duty is pinned END-TO-END, not collector-only (a collector whose rows nobody prints keeps recoveries invisible — the script's only stdout path today is the floor summary, `scripts/check-app-e2e-executed.mjs:152-157`): a vitest case runs the script AS A CHILD PROCESS (`node scripts/check-app-e2e-executed.mjs --report <synthetic>`) against a synthetic report DERIVED from the live `REQUIRED` table (per row, generate that many first-attempt-passed tests, so the floor check passes without hardcoding counts) carrying the §4.4 annotation cases — one test with TWO `infra-recovery` annotations in `tests[].annotations`, one duplicated across `tests[].annotations` AND `results[].annotations` (printed once — merged location only), one with none — and asserts exit 0, one stdout line per recovery (three total), and the total-count line (red first: the print duty does not exist).
