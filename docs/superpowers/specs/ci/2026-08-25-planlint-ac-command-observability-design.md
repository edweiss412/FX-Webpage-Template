...
**Consequence.** Keying the arm on the header name is a recognizer over open English, which the row forbids and which this repo has measured as the losing move. Keying it on the enclosing heading is the same thing with 25 spellings instead of 35 — both at `300a9f937`, and 26 over 35 at `f2c21a187`, so the heading is no more stable than the header. Keying it on "the last column" is worse than either: it is a silent guess, and section 6.3 shows it producing 42 hard findings on six v1-era tables whose last column is a Notes column.
