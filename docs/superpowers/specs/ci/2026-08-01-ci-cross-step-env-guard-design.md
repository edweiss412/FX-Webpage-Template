...
- Inside the per-job loop of `scanWorkflowCoverage` (~line 262), thread `envPoisoned` across the step iteration. For EVERY step chunk (before the `run:`-presence early-continue, which today skips non-run steps entirely):
  ...
  2. Then bookkeeping: if the comment-stripped chunk matches the poison predicate → `envPoisoned = true`. If the chunk carries a `uses:` ref (quotes stripped — a quoted `"./x"` must not dodge the `./` test into the trusted-marketplace branch): a `./` ref poisons fail-closed when it is absent from `localActions`, cyclic (path-scoped seen-set), or its manifest is not `using: composite` (comment-stripped match — R1 escaping mutant #1); a resolvable composite manifest poisons when its comment-stripped text matches the predicate, and its own local `uses:` refs RECURSE under the same rules (R1 escaping mutant #2). Non-local `uses:` → no change.
...
| **F7 non-composite opaque action** (R1 escaping mutant #1, live probe: `census.classified=true, scanner.covered=true, runtime.testsRan=false`) | a KNOWN javascript/docker local action treated as modeled | `uses: ./js` present in the map with `using: node20` (no `runs.steps`) before an invocation → reds both layers |
