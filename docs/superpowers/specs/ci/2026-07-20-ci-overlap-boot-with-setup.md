...
**Date:** 2026-07-20
**Status:** DESIGN AUTHORITY. The lever it designs was implemented, MEASURED on real CI, and REVERTED — leg-median fixed overhead 102s with the overlap against a 96s main baseline (PR #670, runs 30796409070 and 30783618781, eight green legs each), against a gate that required an 8s reduction. It is a measured-negative lever now, not an unexplored one; `BACKLOG.md`'s `BL-CI-OVERLAP-BOOT-WITH-SETUP` carries the figures, the per-leg distributions and the likely cause. The implementation attempt and its accept gate are [`2026-08-02-ci-boot-overlap-implementation.md`](./2026-08-02-ci-boot-overlap-implementation.md). This document's §1.1 resolved scope, §3 single-step design and §5 accept gate stand as ratified after eight adversarial rounds; the §2.1 probe data is durable. Two things in it are SUPERSEDED by the implementation spec and must be read there instead: (1) §3 and §4h contain a KNOWN FACTUAL ERROR — the `allowBuilds` inventory is five keys, not one — corrected and re-audited empirically in that spec's §3; (2) the §2 step table describes the pre-split single-job topology, which no longer exists (the job split into `unit-suite-db` / `unit-suite-nodb` on 2026-07-20), reconciled in that spec's §2.
