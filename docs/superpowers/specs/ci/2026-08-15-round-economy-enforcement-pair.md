...
- The block spans from the `**Mechanizable:**` marker line (existing disposition regex family, `lib/reviewRounds/filing.ts:17`) through subsequent lines until the next line matching ANY bold-run field marker — `/^\s*\*\*[^*]+:\*\*/` — or the section end. A derived boundary rather than the three-name enumeration first drafted (spec R5 finding 2): the live corpus already carries `**Carry-forward:**`, `**Diagnosis of the residual:**`, and four sibling spellings, and an id cited in such a trailing field must not leak into the Mechanizable block and satisfy its duty; an enumerated closer re-opens with every new field spelling, the derived class closes them all. Multi-line candidate lists (the `docs/review-rounds/README.md` filing example) belong to the block — their lines start with list markers or prose, never with a `**<field>:**` bold-run at line start.
...
- `hasDecline`: the block contains a line matching `/^\s*(?:[-*]\s*|\d+\.\s*)?declined:\s*\S/im` — a STRUCTURAL declaration (line-start, optionally after a list marker), not a substring (spec R5 finding 1): the unanchored word-boundary substring form counted `not declined: no owner has accepted this` and a prose MENTION of the form (`use declined: <reason> only after a decision`) as dispositions, silently bypassing the parity duty. Both probes become fixtures asserting `hasDecline: false`.
...
- **Zero or one canonical `**Mechanizable:**` marker per section (spec R6 finding).** A section holding two or more marker lines has no defined aggregation — probed across both orderings, every singular projection (first, last, union) silently loses an untracked block in at least one ordering — so `parseFiling` additionally exposes the marker count and `checkCorpus` reports a duplicate as `filing_malformed` for NON-grandfathered filings ("nothing says which is the entry", the `duplicate_section` precedent at `lib/reviewRounds/corpus.ts`). Both orderings (tracked block then uncited block, and the reverse) are mandated fixtures; `mechanizable` itself models the FIRST marker's block, which is safe because the duplicate check blocks the filing before parity is consulted.
