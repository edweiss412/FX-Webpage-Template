...
4. **Per-line contract (spec R1 findings 1–2): each `GUARD SURFACE:` line declares ONE surface and carries that surface's own disposition on the SAME line.** A multi-surface arc writes one line per surface. Every line matching `/^\s*GUARD SURFACE:/` must have a remainder (text after the first colon) satisfying at least one arm:
   - **Score arm:** the remainder contains a `\d+\s*\/\s*\d+` fraction that is SEMANTICALLY VALID — parsed as `<killed>/<total>`, `total ≥ 1` and `killed ≤ total` (spec R3 finding: the shipped mutation authority rejects a `0/0` run as "nothing was tested" and rejects killed exceeding generated, `tests/mutation/source/gate.ts` `no-mutants` and `unaccounted-mutants` conditions; a declaration the gate could never have produced as green must not pass the dispatch check) — AND matches `/\b(0|no)\s+unaccepted\s+survivors?\b/i`, the EMPTY unaccepted-survivor set the AGENTS.md convergence-criterion bullet 4 requires ("82/84, 0 unaccepted survivors" satisfies it; "2 unaccepted survivors" is a declared non-converged surface and is rejected — dispatching on it is exactly what enrolment-precedes-review forbids). When several fractions appear in one remainder, the arm is satisfied if any semantically valid fraction appears alongside the zero-unaccepted phrase — the gate stays structural and does not decide which fraction is "the" score.
