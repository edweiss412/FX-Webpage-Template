...
4. Otherwise the same `scanText` must satisfy at least one arm:
   - **Score arm:** a line matching `/^\s*MUTATION SCORE:/m` whose remainder (text after the colon, same line) contains a `\d+\s*\/\s*\d+` fraction AND the token `survivor` (case-insensitive) — the score plus the unaccepted-survivor set statement, in whatever prose the author uses ("82/84, 0 unaccepted survivors" satisfies it).
   - **Cannot-express arm:** a line matching `/^\s*CANNOT-EXPRESS:/m` with a non-empty remainder — the honest re-disposition citing its probe (the step3 pattern, `docs/superpowers/specs/2026-08-09-quick-wins-2-mech.md` §1.1.4). Content of the citation is the author's; the gate checks presence, not prose quality.
5. Neither arm → `usageError` (exit 2), message naming both missing arms and the declared trigger, e.g.: `brief declares GUARD SURFACE: but a round-1 diff dispatch must state MUTATION SCORE: (<killed>/<total> plus the unaccepted-survivor set) or CANNOT-EXPRESS: (probe citation) — see AGENTS.md convergence-criterion bullet 4`.

Accept-set, stated positively: the gate ACCEPTS (a) briefs with no line-anchored `GUARD SURFACE:` marker outside fenced blocks, (b) declared briefs carrying a conforming score arm, (c) declared briefs carrying a non-empty cannot-express arm, (d) any brief at stage ≠ diff or round ≠ 1. Everything else at the trigger point is rejected by name (the exit-2 message). Markers are case-sensitive SHOUTY, matching the existing brief-marker family (`VERDICT:`, `FINDINGS:`, `PROBE DOMAIN:`).
...
<!-- spec-lint: ignore — new file created by this spec's implementation; not yet tracked -->
1. **Half A** — new `tests/codexGuard/guardSurfaceGate.test.ts` on the existing spawn harness (`tests/codexGuard/fixtures/fake-codex.mjs`): exit-2 with message naming both arms (declared, neither arm); score arm passes; cannot-express arm passes; round 2 exempt; `--stage plan` exempt; fenced `GUARD SURFACE:` does not trigger; fenced `MUTATION SCORE:` does not satisfy a live trigger; score line lacking the fraction or the `survivor` token still exits 2.
...
3. **Enrolment precedes review (dogfood).** Before the first diff dispatch: add a `reviewRoundFiling` row to `tests/mutation/source/registry.ts` (`sourcePath: lib/reviewRounds/filing.ts`, suites: `tests/reviewRounds/filing.test.ts` + the meta-test), run `pnpm mutation:guards`, and state the scores plus unaccepted-survivor sets for `reviewRoundFiling` and `reviewRoundCorpus` in the round-1 diff brief under `GUARD SURFACE:` / `MUTATION SCORE:`; `scripts/codex-guard.mjs` is declared under the same trigger with `CANNOT-EXPRESS:` citing §1.1.7's shape probe. The arc's own round-1 diff dispatch is the gate's first live customer.
...
**Consequence bound:** every dispatch and every filing is either accepted, or rejected with a message naming the missing element — never silently wrong; a conservative under-check that passes an undeclared or grandfathered artifact is a DOCUMENTED LIMIT (§5), not a finding. **Threat-model fence:** accidental authoring omissions by an ordinary contributor writing briefs and filings in this repo; adversarial evasion of either gate files to documented limits. **PROBE DOMAIN:** briefs — the committed brief files under `docs/review-rounds/` dispatch history and this arc's own dispatch briefs; filings — the live corpus `docs/review-rounds/**/*.md` (56 files at authoring) plus the meta-test's planted fixtures. **Score:** stated per §6.3 in the round-1 diff brief.
