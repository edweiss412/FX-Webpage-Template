...
- **Waived, 12, in three reason families:** mutation write timestamps (`lib/adminAlerts/resolveAdminAlert.ts:33` and `lib/adminAlerts/resolveAdminAlert.ts:60`), dependency-injection defaults on non-render paths (five sites in `lib/drive/watch.ts` (lines 956, 1044, 1172, 1389 and 1562) and `lib/sync/runManualSyncForShow.ts:297`), and observe read-path windows (`lib/observe/query/{events,failures,staged,syncLog}.ts`). The third family names BOTH of its execution paths, not just the CLI: whole-diff review round 1 found `app/admin/_devCaptureAction.ts` calling all four at `app/admin/_devCaptureAction.ts:124` onward. It is a `"use server"` action assembling a downloaded diagnostic bundle, so the site is still never awaited by a render and the waiver stands — but a reason that covers only one of two execution paths is the same false-declared-reason defect §1.4 diagnoses in the residue registry, so it is named rather than left implied.
...
- **9 sit in server components.** Of those, 4 are emptiness checks (`allHidden && !roomsFetchFailed` and its three siblings in `components/crew/sections/`), and 2 are already registered (`components/admin/Dashboard.tsx:674` and `components/admin/Dashboard.tsx:858`). Discounting those six by JUDGEMENT — the four emptiness checks are not fault branches, the two registered ones are not new — leaves **3** sites the fallback would newly surface, at a cost of 79 declared reasons; even restricted to server components its output runs 6 noise to 3 signal.

**That 3 is a judgement, and it is deliberately NOT the re-file trigger.** The discount above is a reading of what those six sites mean, and a reading cannot be asserted. The trigger in §2.2 ranges over the computable set with no exclusion applied, which is 7. Round 2 caught an earlier draft using 3 as the trigger, where it was tripped from the moment it was authored.
...
**The per-run table, recorded here because the artifacts do not survive.** `retention-days: 7` (`.github/workflows/screenshots-drift.yml:202`) and the evidence record itself is never committed, so within a week the runner associations below are unrecoverable from GitHub. An aggregate count cannot be re-analysed; this table can. `dash` is `dashboard-overview/light`, the identity that drifted on 2026-08-18; `crew` is `crew-preview-gear-mobile/light`, the identity whose two values are the branch-attribution evidence. `runnerArch` is `X64`, `runnerOs` is `Linux` and `cpuCount` is 4 on every row.

| run | trigger | runnerName | cpuModel | dash pixelSha256 | crew pixelSha256 |
|---|---|---|---|---|---|
| 32907197024 | `pull_request` | GitHub Actions 1000095944 | AMD EPYC 7763 64-Core Processor | `46d095ed19f5b439` | `cc36e78cf5411002` |
| 32907278297 | `pull_request` | GitHub Actions 1000095932 | Intel(R) Xeon(R) Platinum 8370C CPU @ 2.80GHz | `46d095ed19f5b439` | `cc36e78cf5411002` |
| 32909205619 | `pull_request` | GitHub Actions 1000096021 | Intel(R) Xeon(R) 6973P-C | `46d095ed19f5b439` | `bbbe3503e580fcfa` |
| 32911850181 | `pull_request` | GitHub Actions 1000096128 | AMD EPYC 7763 64-Core Processor | `46d095ed19f5b439` | `cc36e78cf5411002` |
| 32912521153 | `pull_request` | GitHub Actions 1000096147 | AMD EPYC 9V74 80-Core Processor | `46d095ed19f5b439` | `cc36e78cf5411002` |
| 32912963457 | `pull_request` | GitHub Actions 1000096191 | AMD EPYC 9V74 80-Core Processor | `46d095ed19f5b439` | `bbbe3503e580fcfa` |
| 32915671785 | `workflow_dispatch` | GitHub Actions 1000096347 | Intel(R) Xeon(R) 6973P-C | `46d095ed19f5b439` | `cc36e78cf5411002` |
| 32915681874 | `workflow_dispatch` | GitHub Actions 1000096387 | AMD EPYC 9V74 80-Core Processor | `46d095ed19f5b439` | `cc36e78cf5411002` |

Every `runnerName` is distinct. The two `bbbe3503e580fcfa` rows are the `feat/ui-polish-class-sweep` runs; `Intel(R) Xeon(R) 6973P-C` and `AMD EPYC 9V74` each appear on both sides of that split.
...
The limit is recorded in three places, with the live 719 / 79 / 70 / 9 numbers, the 3-site judgement marked as a judgement, and the re-file trigger at its computable value of 7: at the arm in `tests/help/_renderFaultScan.ts`, in the residue registry's docblock, and in the coverage limit of the prior design — `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md` section 8, the item beginning "Layer 1 covers branches that directly return JSX". **That item is cited by its opening words, not by a number, and round 4 is why.** Earlier drafts of this spec sent the update to §8.5 on the arc brief's say-so; §8.5 is the geometry-refusal ATTRIBUTION limit, and writing ternary-coverage numbers there would have contaminated one limit while leaving the other stale. Section 8's list is additionally labelled 1, 2, 3, 5, 4 in source order, so a bare section number is an unreliable address in this particular document.
...
**The decline makes four sentences in the prior design false, and correcting them is part of this repair rather than a separate concern.** That design asserts complete coverage in normative voice: "the marker scan covers every JSX-returning fault branch reachable from the manifest" (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:143`), "Every fault branch reachable from the manifest that **directly returns JSX** carries" the marker (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:233`), "Anything outside the accept-set is **reported by name**, never silently dropped" (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:245-246`), and "The residue is visible and named, never silently absent" (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:260`).

A ternary whose `whenTrue` is JSX directly returns JSX and is reachable from the manifest, and this spec ratifies that the arm goes on dropping it in silence. So all four sentences are false at HEAD, and would stay false with the row archived — a design read as settled while its own scanner contradicts it, which is the same defect §1.4 diagnoses in the registry. Recording the limit at section 8 while section 3 still claims total coverage would leave the contradiction one page apart.

Each of the four is therefore qualified to the arms that actually report — `IfStatement`, `CaseClause`, `CatchClause` — with the `ConditionalExpression` arm's silent drop named at the point of the claim and pointing at the coverage limit item. This is not new scope: it is the consequence bound of this spec's own convergence criterion applied to the document that owns the contract. Round 5 found it, and it is the last spec round this arc runs.
...
- **`unreached-ternary`** — a `ConditionalExpression` begins at the entry's line AND `classifyExpression` returns `null` for its condition. That null is exactly why the arm's bare `continue` drops it, so the cause names its own mechanism.
- **`unreached-no-ternary`** — no `ConditionalExpression` begins at the entry's line AND `scanCandidates()` reports no candidate at that line. This is the genuinely flag-shaped case: the guard site returns no JSX, so no arm makes it a candidate.

**Both conjuncts of the second cause are load-bearing, and round 3 is why.** An earlier draft derived it from the structural half alone — "no `ConditionalExpression` at this line" — and the reviewer refuted it with a live site: `components/admin/settings/AdministratorsSection.tsx:54` is an accepted `IfStatement` candidate returning marked JSX, with no conditional at that line, so the structural test alone would label a REACHED site unreached. A registry whose every entry claims to be unreached must establish unreachedness, not merely the absence of one shape. The candidate-absence half supplies it.

**Candidate-absence is safe HERE and was not safe alone, which is the distinction worth keeping.** Round 2 refuted absence used on its own: all four unclassified ternaries are absent from `scanCandidates()`, so absence accepted every one of them. Conjoined with "no ternary begins at this line", a ternary entry fails the structural half and cannot take this cause at all. The two causes are disjoint on that same conjunct, so one derivation function returns exactly one cause per site, and the suite asserts the declared cause EQUALS the derived one. A wrong relabel now fails, which is what "0 false causes" has to mean to be worth asserting.
...
The prose reasons at `tests/help/_metaRenderFaultMarking.test.ts` lines 69 to 72, line 78 and line 80 are preserved as written, with ONE citation corrected. The `OnboardingWizard` entry at line 80 cites `OnboardingWizard.tsx:818` as the line rendering `OperatorErrorBlock`; at HEAD line 818 is a JSX attribute-list close, the ternary opens at `components/admin/OnboardingWizard.tsx:803` and `<OperatorErrorBlock />` is at `components/admin/OnboardingWizard.tsx:828`. Round 3 found it by sweeping every live file-and-line citation in the registry and reports it as the only false one; this arc verified that sweep rather than re-deriving it. The claim the row makes — that a registry whose reasons are wrong is worse than one with gaps, because it is read as settled — applies to the registry's own citations, so leaving it would reproduce the defect while repairing it. The Dashboard reasons at lines 69 to 72 are untouched and re-fixing them is explicitly not a finding. What changes is that a future entry cannot be filed under a false cause without the suite failing, which is the defect the row is actually about — a registry read as settled while four of seven rows sat under the wrong heading.
...
**Re-file trigger, stated in the header.** An earlier draft made it "a record showing two `pixelSha256` values for one identity", which is impossible by construction: a record holds one entry per identity and `scripts/verify-capture-evidence.ts` rejects duplicates outright. Round 3 caught it against a real record. The trigger that is actually reachable is **the next failure of the `Check screenshot drift` STEP** — not any red run of this job, a distinction round 2 of the whole-diff review caught. Supabase startup, the capture, and the evidence verification are each independently failure-capable and none of them leaves a reproducing sample; only the drift step fails BECAUSE a captured identity's bytes differ from the committed baseline. The instrument uploads a record on both outcomes, so THAT run's record is the reproducing sample the comparison has been missing. On that event, compare it against the table in §1.5 and the row's question becomes testable for the first time.
...
3. **The 2026-08-18 drift stays unexplained, and the runner axis stays UNTESTED rather than cleared.** Sub-pixel rasterization variance is the named mechanism. Eight records show byte equality holding across four `cpuModel` values, but not one of them reproduces the drift, so they say nothing about the failing case. The comparison the row schedules needs a reproducing run. Re-file trigger: the next failure of the `Check screenshot drift` STEP, whose uploaded record is the reproducing sample this population lacks. Not any red run of the job — Supabase startup, the capture and the evidence verification are each independently failure-capable and none leaves such a sample. A single record can never show two values for one identity — one entry per identity, duplicates rejected by `scripts/verify-capture-evidence.ts` — so that phrasing, used in an earlier draft, named an event that cannot occur. No further mechanism is proposed and no repair is opened.
...
- **AC-4a.** The four normative coverage claims in the prior design (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md` at `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:143`, `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:233`, `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:245-246`, `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:260`) are qualified to the `IfStatement`, `CaseClause` and `CatchClause` arms, each naming the `ConditionalExpression` arm's silent drop. Verified by re-reading each of the four: none may still assert coverage of every JSX-returning fault branch.
- **AC-4.** The `ConditionalExpression` arm's decline is pinned by an assertion, and the arm, the registry docblock and the prior design's COVERAGE limit (`docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md` section 8, the item beginning "Layer 1 covers branches that directly return JSX") each carry the live numbers and the re-file trigger. Not the geometry-refusal limit in that same section, which is about attribution and is a different limit.
...
- **AC-7.** `.github/workflows/screenshots-drift.yml`'s header carries a dated paragraph stating the record count by trigger, the distinct `cpuModel` count, that no record reproduces the drift and therefore what the population cannot settle, that the pin is extended to nothing, and the re-file trigger in its reachable form. It must not claim the runner axis is cleared, and it cites §1.5's per-run table by path rather than restating aggregates the table already holds.
- **AC-8.** All three ledger rows are archived with their measured numbers, and no new `BL-`/`DEF-` row is filed. The screenshots row's archive entry cites §1.5's per-run table rather than restating aggregates: the run artifacts expire after seven days, so the tracked table is the only surviving association between `runnerName`, `cpuModel` and each identity's `pixelSha256`.
...
| `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md` | the coverage limit gains the live ternary numbers and the re-file trigger; the four normative coverage claims at `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:143`, `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:233`, `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:245-246` and `docs/superpowers/specs/ci/2026-08-22-screenshots-drift-degraded-render-design.md:260` are qualified to the reporting arms |
