...
The LIVE text of this bullet is `AGENTS.md` convergence-criterion bullet 4, amended 2026-08-22 with the jurisdiction clause (§7 L-11; `docs/superpowers/specs/ci/2026-08-22-mutation-score-jurisdiction-gap.md` §2.3): a score is complete over its declared operators applied to code that EXISTS, so the surviving-mutant requirement binds only where a declared operator reaches the code, and the brief's score arm carries its `OPERATORS:` tail. The quotation above is the 2026-08-04 original and is not updated.
...
| L-10 | An assertion reachable ONLY through a SPAWNED CHILD PROCESS decides nothing about a mutant, and QUALIFIES L-6: the failure mode here is silent, not loud | The runner overlays mutants in the PARENT's module registry. A child re-reads the surface FROM DISK, so it executes the unmutated file and passes under every mutant. A suite that is thorough end-to-end therefore buys ZERO score for whatever only it covers, while the gate still reports a number and the surface still clears its floor. Measured 2026-08-20 on `claimSweep`: `declarationRefusal`'s three guard conjuncts survived a full run despite a dedicated refusals suite, because that suite asserted through `spawnSync(tsx)`; `parseRepairSpans` had the same shape and no in-process assertion at all. Repaired there by adding in-process cases. CLASS SIZE, measured across the registry rather than estimated: 10 of the 69 suites named in `suitePaths` spawn a child (`spawnSync`/`execFileSync`/`execSync`/`spawn`), of which 3 are `claimSweep`'s and now assert in-process as well. The other 7 are NOT probed — spawning is not itself the defect, only an assertion that exists NOWHERE ELSE is. **RE-FILE TRIGGER:** a probe showing any of those 7 carrying part of its floor on a spawn-only assertion is the incident that moves this from a documented limit to a `BL-` row. |
