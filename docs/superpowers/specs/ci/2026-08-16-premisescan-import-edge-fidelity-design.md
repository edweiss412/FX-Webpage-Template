...
- **FILES two backlog rows** (Task 7 Step 1), neither of which exists yet and neither of which this spec claims exists: `BL-PREMISESCAN-UNPARSEABLE-MODULE-UNREACHABLE` (§4 limit 8) and `BL-PREMISESCAN-NESTED-HOOK-SIBLING-LEAK` (§4 limit 14, carrying §3.11 row A as its probe). A round-3 draft listed a third for the assignment-position dynamic import; §2.4b now CLOSES that case, so the arc fixes it rather than deferring it.
