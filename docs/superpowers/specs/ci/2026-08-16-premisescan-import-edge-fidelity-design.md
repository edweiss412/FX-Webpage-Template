...
**Those files are parsed as TypeScript today, and that is measured rather than assumed.** A probe fixture whose `.json` payload is TypeScript reaching `node:child_process` classifies `environment-touching` on the unrepaired tree, and a `.mdx` payload does the same (§3.6). So the guard is not defending against a hypothetical: `moduleFacts` currently runs `ts.createSourceFile` over every data file an in-repo edge resolves to, and it is only the contents of real JSON — which parses to something inert — that keeps the current behaviour harmless. That measurement is also what makes AC-9 discriminating, because it gives the criterion a fixture that is RED today.
...
- **AC-9 — non-language targets stay pure, on a fixture that is RED today.** A test importing a `.json` file classifies `environment-free`, under two conditions that together make the assertion discriminating. First, **the specifier carries the `.json` extension**: `resolveSpecifier`'s candidates are the specifier plus `.ts`, plus `.tsx`, plus an index file inside it, and finally the bare specifier itself, so a specifier written without the extension never reaches a `.json` file and is pure whether or not the guard exists. Second, **the fixture's `.json` content is TypeScript that reaches `node:child_process`** — because that file is genuinely parsed as TypeScript today: probe §3.6 measures such a fixture as `environment-touching` on the unrepaired tree, and the same probe measures a `.mdx` target the same way. A fixture holding real JSON is `environment-free` before and after and proves nothing. *Catches:* the false positive §2.4 exists to prevent, on the only input shape that actually reaches the guard and moves when it lands.
