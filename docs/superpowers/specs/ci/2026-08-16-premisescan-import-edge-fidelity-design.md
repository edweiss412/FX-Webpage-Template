...
**Half 1 — the cross-module lookup answers the wrong question.** `moduleScopeExtent(facts, name)` (the `moduleScopeExtent` helper, `tests/mutation/source/premiseScan.ts` on the merged tree `ad9638fa9`) reads `facts.extents.get(facts.sf).get(name)` — a lookup of a **local declaration** in a module, standing in for a lookup of an **export** of that module. Two independent falsehoods live in that substitution:
...
**Mutation status (convergence bullet 4).** `premiseScan` is **already enrolled** on the merged tree `ad9638fa9` (`tests/mutation/source/registry.ts`, `id: "premiseScan"`): `scoreFloor: 0.95`, operators `relational-boundary` / `equality-flip` / `integer-literal`, deciding suites `tests/mutation/source/premiseScan.test.ts` + `tests/mutation/_metaPremiseContract.test.ts`, accepted set of 4 rows (3 `equivalent`, 1 `accepted-gap`). Accepted `siteId`s are LINE-keyed, so this arc's edit shifts every row below its first hunk; the implementation branch **re-derives the accepted set via `enumerateSites` rather than hand-adjusting**, reruns `pnpm heavy pnpm mutation:guards`, and states the score plus the unaccepted-survivor set in its round-1 diff brief. **Re-keying is not the whole job.** One of those four rows — the `integer-literal` row accepted `equivalent` on the grounds that "`unresolved` is provably always empty… populated only where `factsFor` returns null" — has its premise FALSIFIED by §2.6, which populates that array from two ordinary constructs. Re-deriving a `siteId` moves a line; it never re-tests an argument. That row is RETIRED, not migrated (§5, AC-15), and `EXPECTED_LEDGER_KINDS.premiseScan` in `tests/mutation/source/expectedLedgerKinds.ts` moves with it (PR #834 lifted it out of the gate suite after this spec was drafted). From that point the diff-stage convergence criterion is that score with an empty unaccepted-survivor set — a "the guard does not pin what it claims" finding is admissible only with a surviving mutant from a declared operator at a named site.
...
7. **Zero live instances is not a reason to descope.** §3.3 measures zero occurrences of every repaired form inside the classifier's current domain, and that is the *point*: the repair was PREDICTED verdict-neutral (AC-1, since amended — the `lib/log` barrel falsified the prediction for sixteen tests) and exists so that the ordinary refactor which introduces the first instance does not silently blind the guard. The near-domain already holds 22 in-repo namespace imports, 40 in-repo default imports, and 117 named re-export specifiers across 41 statements plus one `export *` (§3.4 states both grains, because a specifier count is not reproducible against a statement count), any of which enters the live domain the day its suite is enrolled.
...
**Every form this arc repairs occurs zero times in the classifier's domain, before and after #827** — measured on the SPELLINGS. That prediction held for the spellings and FAILED for the thing they compose into: the `lib/log` barrel hid a real environment reach, and implementation measured sixteen live verdicts moving in two suites (AC-1 as amended). So this section's numbers are the pre-implementation measurement and are kept as such; the shipped counts are the amended AC-1's. The unclassifiable set stays empty of UNEXEMPTED tests; one enrolled test carries a reasoned exemption and the meta-test filters it. That is AC-1, and it is machine-checkable rather than argued.
...
All six round-3 claims re-probed against the re-pinned `4e40db2b3` tree, and again against the merged `ad9638fa9`; all six reproduced both times.
...
**Base re-pin (finding 5), now CLOSED by the merge.** PR #827's branch advanced from `ac9a40cd8` to `4e40db2b3` (843 → 1,000 lines) during review, then merged as `ad9638fa9` (1,273 lines) on 2026-08-16 at 22:07 CDT and was deleted from `origin`. **Every citation in this document is re-anchored to that merged tree**, which is fixed: `ENVIRONMENT_SOURCES` is `tests/mutation/source/premiseScan.ts:29`, the extension-aware `parse()` is `tests/mutation/source/premiseScan.ts:61`, `hookBodies` is `tests/mutation/source/premiseScan.ts:1113` and is still recursive, the default-import registration that stores the LOCAL name is `tests/mutation/source/premiseScan.ts:587`, and the cross-module lookup is `tests/mutation/source/premiseScan.ts:1001`. The re-pin churn recorded below is history, not a live hazard. Every behavioural row of §3.1, §3.2 and §3.11 was re-run against the new head and **every row is unchanged**; `hookBodies` is still recursive (`tests/mutation/source/premiseScan.ts:1113`), `ENVIRONMENT_SOURCES` is unchanged (`tests/mutation/source/premiseScan.ts:29`), and the assignment-position comment is still present (`tests/mutation/source/premiseScan.ts:687`). `parse()` now selects `ScriptKind.TSX` by extension (`tests/mutation/source/premiseScan.ts:61`), which retracts §4 limit 9.
...
9. **RETRACTED — no longer true of the target tree.** A round-2 draft said every module is parsed with a fixed `ts.ScriptKind.TS`, so `.tsx`/`.jsx` targets are mis-parsed. On the merged tree `moduleFacts`'s `parse()` selects the kind by extension — `path.endsWith(".tsx") ? ts.ScriptKind.TSX : ts.ScriptKind.TS` (`tests/mutation/source/premiseScan.ts:61`) — so the limit describes a tree that no longer exists. It is retracted rather than deleted so the numbering below stays stable and a reader of the round-2 text finds the correction. `.jsx` still parses as `.ts` under that ternary; that is a residual of the SAME shape, it is pre-existing, and §2.4 keeps `.jsx` analyzed to preserve today's measured behaviour (§3.9) rather than to improve it.
...
14. **A hook nested inside a `describe` already leaks to sibling nested `describe`s, and this arc does not fix it.** `hookBodies` collects recursively, so under a shared outer `describe` a spawning hook in branch A marks a pure test in sibling branch B `environment-touching` — probed as §3.11 row A. It is a FALSE POSITIVE and it is **pre-existing**: repairing it would move live verdicts and break AC-1's verdict-neutrality, which is what AC-1 as AMENDED asserts: exact-count equality against the ratified sixteen-test movement. Repairing the leak would move a SEVENTEENTH verdict, which is what the criterion forbids. This arc's own seed repair is non-recursive precisely so it does not widen the leak (§2.6 item 4). Filed as `BL-PREMISESCAN-NESTED-HOOK-SIBLING-LEAK` by Task 7 Step 1; that row does not exist yet and this spec does not claim it does.
...
- **EXTENDS** `tests/mutation/_metaPremiseContract.test.ts` — TWO `EXPECTED_ENV_TOUCHING` rows change, and they are the arc's headline: `sameOriginServerAction` 7 -> 10 and `fieldNearMissBaseline` 0 -> 13 (AC-1 as amended). A round-1 draft of this spec predicted no change; the barrel reach it did not model is why that was wrong.
...
- **EXTENDS** `tests/mutation/source/expectedLedgerKinds.ts:33` — PR #834 lifted this declaration out of the gate suite and sharded that suite into `tests/mutation/guardSurfaces.gates.test.ts` plus four shard suites (`tests/mutation/guardSurfaces.shard0.test.ts` through shard3). `EXPECTED_LEDGER_KINDS.premiseScan` was predicted to declare `{ equivalent: 3, "accepted-gap": 1 }` and SHIPS `{ equivalent: 2 }`; it is asserted with `toEqual`, so retiring a row reds it.
- **CREATES** no new meta-test. `_metaPremiseContract.test.ts` already walks the enrolled suites from the registry and already asserts the unclassifiable set is empty of unexempted tests (it filters reasoned `// no-premise:` exemptions; one enrolled test uses that).
...
- **AC-1 — AMENDED, and the amendment is the arc's headline finding.** As written this criterion asked for verdict-neutrality: every `EXPECTED_ENV_TOUCHING` value unchanged. **It does not hold, and that is a FINDING rather than a regression.** `lib/log/index.ts` re-exports `log` from `./logger`, and `logger.ts` is environment-touching on a DIRECT import — the barrel hid a real environment reach, so sixteen tests in two enrolled suites were reading free while genuinely touching environment state: `tests/auth/sameOriginServerAction.test.ts` 7 -> 10 and `tests/parser/fieldNearMissBaseline.test.ts` 0 -> 13. The user was asked and chose to write REAL PREMISES for the twelve that carried neither premise nor exemption.

  **The amended criterion is EXACT-COUNT equality against those declared numbers, not zero movement.** `pnpm vitest run tests/mutation/_metaPremiseContract.test.ts` passes, and the declared movement is exactly the sixteen Task 0 measured — so a LATER repair that moves one more verdict still reds, which is the discrimination the original criterion was for. The re-baseline command below is retained for that purpose and is expected to report `AC-1 FAIL` on the two ratified rows: it answers "did a declared count move", and here two deliberately did. Read it against the ratified pair, not as a gate. Written as `if … then … else … fi` rather than `&& … || …`, because the short-circuit form prints the failure message and still exits 0 when the failure branch is a group:
...
1. Spec and plan were authored against PR #827's branch and re-anchored to the MERGED tree `ad9638fa9` once it landed (this document; every citation in §0-§2 is to that tree, by line).
