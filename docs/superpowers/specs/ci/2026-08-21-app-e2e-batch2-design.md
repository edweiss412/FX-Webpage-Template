...
- **`telemetry-layout` and `published-show-attention` fault through requests no retry wrapper sees, and batch 3 owns their restoration.** Recorded here by orchestrator ruling of 2026-08-25 rather than as a ledger row (Eric's no-new-rows directive on the `fix/supabase-upstream-fault-class` arc). Both were dropped from batch 2 on a second red per spec, and the arc that measured the class found their runs name NO mechanism at all: `telemetry-layout` in runs 32571008405 and 32573475808 ends at the loading shell with the sidebar and log at zero rects and produces no diagnostic; `published-show-attention` in 32571008405 shows the ratified open-time recovery firing and the boundary persisting. That is the opposite of their sibling members, whose logs name `requireAdmin: is_admin RPC failed` and which PR #882 now absorbs at the transport, both functions being in `RETRYABLE_RPCS` (`lib/supabase/retryEligibility.ts:25-40`). So these two are neither absorbed nor attributable on current evidence, and the batch-3 question inherits them with their run ids rather than with a diagnosis. Re-file trigger: a post-#882 run of either spec that carries a `SUPABASE_UPSTREAM_FAULT` record naming the request, which is what the transport observer shipped by that arc exists to produce. Closing this without that record would mean choosing between a page-segment recovery and a bootstrap change on evidence that supports neither, which the same ruling retired as undecidable.
