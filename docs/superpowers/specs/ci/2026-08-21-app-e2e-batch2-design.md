...
Update `BL-E2E-APP-DEPENDENT-SPECS-CI-DARK` in `BACKLOG.md`: a batch-2 paragraph in the same shape as the batch-1 one (members, workflow, date, PR), the census table restated by the section-2 commands (UNSEEN 23 to 23 minus members; total 51, with the staged-preview note), the class-C bookkeeping from 7.3, and the deferred rows from section 10 linked. Entry stays OPEN while any `UNSEEN` row remains. The `**Status:** IN PROGRESS · **Branch:** ci/app-e2e-batch2` marker comes off in the ledger-closeout commit taken BEFORE whole-diff review (the ratified early-closeout ordering; a marker that reaches main names a branch the merge just deleted).
