...
- **CSS-generated text is outside the runtime crawl, and always was.** The AC-X.2 collector reads
  ...
  own text is still caught exactly as before. Re-file trigger: a raw §12.4 code reaching a user
  through generated content, or a probe showing a shipped surface rendering one that way. Closing it
  means reading `getComputedStyle(el, '::before').content` per element, which is a new capability with
  its own false-positive surface (icon fonts and layout glyphs), not a repair.
...
- **`telemetry-layout` and `published-show-attention` name consumers the retry wrapper now covers, and are still not diagnosed.** Recorded here by orchestrator ruling of 2026-08-25 rather than as a ledger row (Eric's no-new-rows directive on the `fix/supabase-upstream-fault-class` arc). Both were dropped from batch 2 on a second red for one spec, which is batch 1's threshold and the ruling's boundary (d). The arc that measured the class read all twelve of the row's job attempts under the extraction rule this spec's own probe record states (`docs/superpowers/specs/ci/probes/2026-08-24-admin-loader-502-clustering.md:164`), and both specs' runs DO name consumers: `telemetry-layout` in 32571008405 and 32573475808 names `admin_read_share_token`, `viewer_version_token`, `get_admin_show_review_snapshot` and `readfinalizeowned_b2`; `published-show-attention` in 32571008405 shares that run's consumers, and its own failure is the ratified open-time recovery firing and the boundary persisting. Every one of those functions is in `RETRYABLE_RPCS` (`lib/supabase/retryEligibility.ts:25-40`), so like every other member of this class they fall inside the bound PR #882 now absorbs.

  **That is association, not diagnosis, and the distinction is the whole point of recording it here.** A failed write returns without logging, so no logged consumer can be tied to a particular failed assertion; the same arc's measurement includes an attempt that carried two 502 events and went green. So these two specs are UNATTRIBUTABLE on current evidence in exactly the sense the main disposition uses, and a repair cannot be chosen for a failure whose mechanism nobody can name. Batch 3 inherits them with their run ids on that footing. Re-file trigger: a post-#882 run of either spec carrying a `SUPABASE_UPSTREAM_FAULT` record that names the request, which is what the transport observer shipped by that arc exists to produce.
