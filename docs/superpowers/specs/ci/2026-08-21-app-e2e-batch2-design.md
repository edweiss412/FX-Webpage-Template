...
- **`telemetry-layout` and `published-show-attention` fault through requests no retry wrapper sees, and batch 3 owns their restoration.** Recorded here by orchestrator ruling of 2026-08-25 rather than as a ledger row (Eric's no-new-rows directive on the `fix/supabase-upstream-fault-class` arc). Both were dropped from batch 2 on a second red per spec, and the arc that measured the class read every one of the row's twelve job attempts uniformly. Both of these name `admin_read_share_token returned error: An invalid response was received from the upstream server` — `telemetry-layout` in runs 32571008405 and 32573475808, `published-show-attention` in 32571008405, where the ratified open-time recovery fired and the boundary persisted. `admin_read_share_token` is in `RETRYABLE_RPCS` (`lib/supabase/retryEligibility.ts:27`), so like every other member of this class its named mechanism is inside the bound PR #882 now absorbs. What that does NOT establish is causation: a failed write returns without logging, so a named RPC line cannot be tied to a particular failure, and no repair can be chosen for a failure whose mechanism nobody can name. So these two are ATTRIBUTED to a now-absorbed family and still not DIAGNOSED, and the batch-3 question inherits them with their run ids on that footing. Re-file trigger: a post-#882 run of either spec that carries a `SUPABASE_UPSTREAM_FAULT` record naming the request, which is what the transport observer shipped by that arc exists to produce. Closing this without that record would mean choosing between a page-segment recovery and a bootstrap change on evidence that supports neither, which the same ruling retired as undecidable.
