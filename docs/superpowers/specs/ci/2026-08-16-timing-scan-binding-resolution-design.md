...
**Round 2 (same substitute path, same scrape) returned 6 findings and NEEDS-ATTENTION, and its first finding is against this session's own conduct: the tree moved under the review.** Three commits landed between the reviewer's first read and its last, because this session kept re-probing round-1 claims while round 2 was live. Two of its findings were repaired mid-run and neither repair has been reviewed by anyone — the freeze-the-worktree-during-review rule exists for exactly that, and eagerness to verify is what broke it. The other five were real and are repaired: Task 2 named the header rewrite's owner without INSTRUCTING it (which would have pushed the rewrite past Task 4 and re-staleified the accepted set whose score the diff brief declares), the four-mutant protocol was undefined for the two fires cases that assert an ABSENCE, the plan's EXTENDS list omitted `declPos`, this §3 probe table had lost its header row after six appends (six rows rendering as literal pipe text, P10's among them), and "Task 4 is the last edit to that file" named the wrong task — Task 3 is.

Round 2 also re-ran attack vector 1 and found the design's second and last silent-drop shape — form 2d's position identity, repaired mid-round by P13 — then reported no further input in the domain that drops a site or resolves into another binding's row.
...
**Two details the key depends on, both of which the draft got wrong by generalizing.** The offset is the DECLARATION NAME node's start **on both sides**: the resolver takes `ts.getNameOfDeclaration(decl) ?? decl` rather than special-casing `VariableDeclaration`, and `scanTimingSites` records `declPos` from the NAME node even where the site's `line` comes from elsewhere — form 2d (`class C { "ttlMs" = 17000 }`) pushes its site from the property NODE, and `getNameOfDeclaration` returns the string literal, so a node-based `declPos` and a name-based resolver key differently. Measured (§3, P13): the two starts COINCIDE on a bare property and diverge the moment it carries a modifier or decorator — the kind of agreement that holds in a fixture and breaks in the tree. The key's file half is the repo-relative POSIX path `TimingSite.file` carries, so the resolver normalizes `declaration.getSourceFile().fileName`, which is absolute, before comparing.
...
**The count is TREE-DEPENDENT, and the number that matters is the post-#827 one.** On `origin/fix/scanner-scope-totality` — the tree the implementation runs against — there are six `scanRepo(REPO_ROOT)` in `tests/docs/_metaInteractionTimingInventory.test.ts` plus one `scanRepo(process.cwd())` in `tests/docs/interactionTimingScan.test.ts`: SEVEN. On the pre-#827 tree this branch is based on, the same greps give three plus one: FOUR (round-3 finding 1, which measured the base while this spec quoted the landed branch — both counts are right for their tree, and only the labelling was missing). The budget in AC-10 holds on either: 7 × 369 ms worst case is 2.6 s against a ≤2 s target only if every program is a cold first call, which P6 measures it is not (367 ms once, then 167-214 ms). The synthetic-root calls build programs over a handful of files each (~10 ms, P11) and are not material.
...
| **P15** | What does `noResolve` actually cost? | A covered constant reached through a re-export module OUTSIDE the universe: `noResolve` yields zero declarations and the site reports, while the same program without it resolves. Round-1 finding, re-probed; §4 item 1 carries it as an accepted cost. |
| **P14** | Does the CompilerHost repair hold both properties at once? | Yes. Over entirely virtual paths, a `@/`-aliased cross-file import still resolves under `noResolve` and lands on the declaration name node, with no filesystem access — which is what makes `refPos` index the same bytes the scan read. |
| **P13** | Does form 2d's site position match what the resolver reads? | `getNameOfDeclaration` returns the string-literal name; its start equals the property node's start only while the property has no modifier or decorator. `declPos` is therefore recorded from the NAME node, not the site's line source. |
| **P12** | Does the shorthand form resolve through `getSymbolAtLocation`? | No — it returns the ShorthandPropertyAssignment's own symbol. `getShorthandAssignmentValueSymbol` returns the value binding. Round-1 finding, re-probed; §2.3 specifies the API on this evidence. |
...
| **P16** | Is AC-12's same-line neighbour RED today, or already green? | Already GREEN — `other` reports under today's name filter, which has no notion of lines. AC-12 is therefore a REGRESSION PIN and not one of Task 1's REDs; it reds only against the line-keyed design this spec rejected (P10). |
| **P17** | Do the declared eleven assertions pin the resolver's SHORTHAND branch? | No — and neither does the live tree: the live `{ duration }` did not kill the mutants that revert that branch to `getSymbolAtLocation`. Resolver region 27 mutants: 18 killed / 9 survived with eleven, 21 killed / 6 survived with a twelfth shorthand-resolves case. §5 therefore declares TWELVE. |
...
1. **A binding the program cannot REACH does not resolve, and that is broader than "a binding outside the universe".** Two cases, and they differ:
   ...
   - A COVERED constant reached through an intermediate re-export module outside the universe reports `unclassified` where today it is suppressed — a NEW residual on an ordinary import-path refactor, because `noResolve` never adds the intermediate file and the alias dies there (round-1 mechanism finding 2, re-probed here as P15). Absent from the tree today: all 17 live cross-file resolutions import the declaring module directly (§3 P2). The failure direction is a surfaced name someone dispositions, and the fix if it ever lands is an `EXPLICIT_INCLUDES` row for the barrel — not switching module resolution back on, which costs the 30× the pinned options buy.
...
- **CREATES:** twelve fixture assertions in `tests/docs/interactionTimingScan.test.ts` — FIVE fires (module-level shadow, inner-scope shadow, parameter shadow, aliased import, property-value shadow), SIX stays-quiet (the unshadowed peer, a legit local, a direct import, a barrel re-export, the live-shaped `ttlMs` pass-through, and a SHORTHAND-RESOLVES case), and ONE regression pin (the P10 same-line neighbour, which is GREEN on the current tree — probe P16 — and red only against the rejected line-keyed design, so it is an assertion this arc keeps, never evidence that Task 1 was driven by a failing test); plus a structural pin that the resolver's `paths` assumption matches `tsconfig.json`. The plan's task tables are the single source for that split.

  **The twelfth is measured, not preference (probe P17).** This inventory said ELEVEN until the mutation evidence came in: with eleven, the resolver's shorthand branch is unpinned — the mutants that revert it to `getSymbolAtLocation` survive, and the live `{ duration }` at `components/crew/CrewSectionTransition.tsx` does not kill them either, so the API choice P12 established by probe would ship untested. The shorthand-resolves case (`const duration = 300; const opts = { duration };`, asserting no residual) kills them. It widens no recognizer — `ShorthandPropertyAssignment` is already form 3c and already a declared `refPos` producer in §2.3 — so it pins an existing contract rather than adding one, inside §1.1 item 2's fence.
- **EXTENDS:** `scripts/scan-interaction-timings.ts` (`TimingSite.refPos` and `TimingSite.declPos`, the resolver, `scanRepo`'s resolution step, the header paragraph that documents the hole, and the exported alias-mapping constant AC-7 pins); `tests/mutation/source/registry.ts` (`interactionTimingScan` accepted-survivor set re-derived — its `siteId`s are line-keyed, so re-derivation is mandatory rather than optional).
...
- **AC-9 (header honesty).** The scanner header's TOTALITY IS PER-FORM paragraph — the one whose text wraps as "… with one / hole: an identifier delay resolves by NAME, not by binding …" and ends by naming `BL-TIMING-SCAN-NAME-VS-BINDING` as the carrier — is rewritten to the closed contract in the same commit that closes it. (The sentence spans a comment line wrap, so grep it with `rg -U` or by the phrase `resolves by NAME`.) **Verification is TWO greps, not one:** the absence of `resolves by NAME` AND the presence of the new contract text. An absence check alone is satisfied by deleting the paragraph, which is not a rewrite — the same absence-assertion defect round 2 repaired in the test protocol, unswept here until round-3 finding 5.
- **AC-10 (cost).** Two budgets, separately measured and separately falsifiable, both recorded in closeout with their before/after numbers: (a) the two suites run together gain **≤ 2 s** wall clock, the arithmetic being seven whole-repo programs at 167-369 ms each (§2.4); (b) `pnpm heavy pnpm mutation:guards` for this surface gains **≤ 25%** wall clock, since the harness pays one program per mutant and a fixed second-per-mutant budget would be a different claim at every mutant count. Breaching (a) OR (b) lands the §2.4 memo fallback with its own before/after measurement; neither is waived by the other passing. Both budgets need a BEFORE number recorded at Task 0 — the suite pair's wall clock and the `mutation:guards` wall clock, not only its score — or (b) has nothing to compare against and an implementation whose harness gains 60% while the suites gain 1.4 s passes every step and violates this criterion (round-3 finding 2).
- **AC-12 (declaration identity, not proximity).** A file declaring `const CLOSE_DELAY_MS = 220, other = readConfig();` and calling `setTimeout(fn, other)` reports `other` as `unclassified`. **This is a REGRESSION PIN, not a fires case:** it already passes on the current tree (probe P16), because the name filter has no notion of lines; it fails only against the line-keyed design this spec rejected (probe P10). It earns its place by stopping a future refactor from re-adopting a key that aliases — and it must not be counted as one of Task 1's REDs.
