...
`scanTimingSites` gains two additive positional fields on `TimingSite`, both absolute offsets into the file it was given: `refPos`, the reference identifier's start, recorded on an `unclassified` site whose delay argument or property value IS a bare identifier; and `declPos`, the declaration name node's start, recorded on every `named-constant` site so the covered set can be keyed by identity rather than by line (§2.1). Both are absent — not `undefined` — on the paths that have neither. That is what lets the resolver ask the checker about the exact node rather than re-finding it by name — re-finding by name inside the resolver would reintroduce the defect one layer down.
