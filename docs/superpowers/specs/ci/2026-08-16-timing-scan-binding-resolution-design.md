...
- **CREATES:** eleven fixture assertions in `tests/docs/interactionTimingScan.test.ts` — FIVE fires (module-level shadow, inner-scope shadow, parameter shadow, aliased import, property-value shadow), FIVE stays-quiet (the unshadowed peer, a legit local, a direct import, a barrel re-export, the live-shaped `ttlMs` pass-through), and ONE regression pin (the P10 same-line neighbour, which is GREEN on the current tree — probe P16 — and red only against the rejected line-keyed design, so it is an assertion this arc keeps, never evidence that Task 1 was driven by a failing test); plus a structural pin that the resolver's `paths` assumption matches `tsconfig.json`. The plan's task tables are the single source for that split.
