...
**53 member open sites across 17 spec files** — 38 direct-goto (§2.3) and 15 non-textual (§2.4). They are adopted by **50 edit locations**: three of the non-textual sites are the extra `published-review-modal.reopen.spec.ts` row clicks, all four of which reach one wrapper, so one edit discharges four sites. One non-member `/admin?show=` goto exists (`published-review-modal.deeplink.spec.ts:298`, §2.5) and is the guard's single ship-time exemption.
...
### 2.4 Members — non-textual opens (15 sites)
...
1. **Expose the wait-plus-recovery core AND a URL-taking entry point; adopt per navigation shape (CHOSEN).** `openShowReviewModal` keeps its exact contract and becomes a thin wrapper. `awaitReviewModalOrRecover` carries the ready-or-boundary wait plus the single recovery for callers that own their navigation (row clicks, legacy redirects); `openShowReviewModalAt` adds goto-with-options for callers that own their URL. Pros: every one of the 53 sites adopts without changing what it navigates or asserts; the parent's ratified contract is untouched; one recovery implementation, zero copies; and because no spec writes the naked navigation any more, the §4.4 guard becomes a flat per-site ban rather than a heuristic. Cons: three exports to keep coherent — mitigated by two of them delegating to the third.
2. **Adopt `openShowReviewModal` only where its signature already fits.** Leaves the URL-variant deeplink sites, the `domcontentloaded` layout sites, both legacy-redirect sites, and all fifteen §2.4 opens unhardened. Rejected: the sweep's disposition rule (AGENTS.md class-sweep) is repair-all-in-one-PR unless a named exception applies; "the signature doesn't fit" is a helper-shape problem this arc can fix, not an exception.
...
- **`awaitReviewModalOrRecover(page, opts?): Promise<Locator>`** — steps 3-7 of the parent §4.1 contract, verbatim semantics. `opts = { timeoutMs?, label?, readySelector? }`.
  ...
  - **Strictness contract (spec-review R2 finding 2).** `readySelector` may legitimately match TWO
    elements: during the streaming swap the Suspense skeleton and the loaded modal transiently
    coexist, which is exactly why the two `MODAL_ANY` callers spell their own wait
    `page.locator(MODAL_ANY).first()` today (`published-review-modal.deeplink.spec.ts:345`). Every
    locator operation in the core therefore runs on a `.first()`-scoped locator. The sweep is over
...
- **Shape N — navigation is not a goto the helper can own (`awaitReviewModalOrRecover` alone).** The open — legacy-route redirect, row click, keyboard activation, link click, or a variable route the loop already navigated — is unchanged; only the wait routes through the core. Sites: `published-review-modal.deeplink.spec.ts` lines 239 and 257 (after their `waitForURL`), `needs-attention-holds.spec.ts:335` (before the `mi11-*` assertions), `alert-action-links.spec.ts:345` (for `?show=` routes only, after the `admin-layout` wait and before the fragment waits), `published-review-modal.interactions.spec.ts` lines 255, 355, 506, `published-review-modal.realtime.spec.ts:789` and `published-review-modal.realtime.spec.ts:913` (`readySelector: MODAL_ANY`), `published-review-modal.reopen.spec.ts:65` (its local `awaitLoadedModal` delegates — ONE edit covering the four click sites at 85, 100, 116, 141), `published-review-modal.closeFreshness.spec.ts:54` (same, covering the click at 71), and `admin-lifecycle-transitions.spec.ts:175` (the reload tier's bare `expect(modal).toBeVisible({ timeout: 20_000 })` becomes `awaitReviewModalOrRecover(page, { timeoutMs: 20_000, label: 'reload:expectFlipLanded' })`; the `modal` locator the caller passed in stays valid and every later `modal.getByTestId("published-toggle")` assertion is untouched). **15 sites over 8 files, 12 edit locations.**

28 + 10 + 15 = 53 sites, discharged by 28 + 10 + 12 = **50 edit locations**. No site is in two shapes; a FILE may host two (`admin-lifecycle-transitions.spec.ts` has five Shape G gotos and one Shape N reload). Sites and edits differ only where a wait wrapper serves several causing clicks — `reopen` (4 sites, 1 edit) is the whole of the difference.
...
- **AC-1:** `awaitReviewModalOrRecover` and `openShowReviewModalAt` exist with the §4.1 contract; `openShowReviewModal` delegates through both; the §1.1 pins (three unit cases, dead-slug diagnostic, no top-level `@playwright/test` value import) pass unchanged. New unit cases cover `readySelector` (custom selector waited on and named in the error), `gotoOptions` pass-through, `label` fallback, and — the R2 finding-2 pin — a **two-match fixture** in which the ready selector matches both frames, asserting that neither the first wait nor the post-recovery re-wait raises a strict-mode violation and that the error text still names the boundary and `show_review_snapshot_failed`. The custom-selector case alone does NOT falsify that failure, which is why it is called out separately.
- **AC-2:** All 53 member open sites in §2.3 + §2.4 (re-derived at implementation via §2.1's five commands) engage the helper in their assigned shape (§4.2); downstream assertions unchanged — verified by targeted local playwright runs of all 17 adopted specs under `pnpm heavy` (non-interactive playwright is heavy by invocation shape), green against a freshly seeded database.
...
7. **Non-goto opens are adopted but not guarded.** §2.4's fifteen sites — clicks, keyboard activations, legacy redirects, and the reload — get the recovery, and nothing machine-checks that a NEW one adopts it. The guard sees `goto` lines only, by design. Accepted: the only mechanism that would cover it is the runtime fixture rejected as §3 approach 3, whose blast radius exceeds the value.
