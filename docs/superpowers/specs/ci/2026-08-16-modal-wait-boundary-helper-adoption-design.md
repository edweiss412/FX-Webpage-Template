...
- **The app-e2e oracle's stdout is pinned and stays byte-identical ON A GREEN RUN.** The §4.3 ordering rule adds output on the FAILING path (where there is no pin today and where the oracle currently prints nothing at all); the green-run byte sequence is unchanged, which is what the pin covers. `tests/ci/appE2eAnnotationPrint.test.ts` runs `scripts/check-app-e2e-executed.mjs` as a child process and asserts its print lines; the §4.3 collector extraction must keep that suite green without editing its assertions.
...
- **Print-before-gate ordering rule (spec-review R6).** `if: always()` gets the oracle RUN; it
  does not get the print REACHED. Both oracles gate by `process.exit(1)` placed ABOVE their
  reporting tail — `scripts/check-crew-e2e-executed.mjs:165` and, identically,
  `scripts/check-app-e2e-executed.mjs:189`, whose own recovery print at
  `scripts/check-app-e2e-executed.mjs:201-203` is therefore already unreachable on a floor
  shortfall. That is the SHIPPED parent-arc duty carrying the same defect the reviewer found in
  the crew oracle, so this is a class repair, not a one-site patch. The failure path is exactly
  the one §4.3 exists to close: a spec recovers, a downstream assertion then fails, the run's
  executed-count floor falls short, and the oracle exits before naming the recovery.

  The repair is ordering by construction rather than by remembering: in any oracle carrying the
  print duty, **the gating exit is the LAST statement of the function**, with the recovery print
  unconditionally above it. Concretely — compute failures; emit the `ok — …` line when there are
  none; print recoveries; THEN `process.exit(1)` if there were failures. A `finally` block is NOT
  the mechanism, because `process.exit()` does not run `finally`. This also preserves §1.1's
  byte-identical-stdout pin exactly: on a green run the `ok — …` line still precedes the
  `infra-recovery` lines, so `tests/ci/appE2eAnnotationPrint.test.ts` stays green with its
  assertions unedited.
...
- **AC-4:** The §4.3 wiring rule holds: every workflow step executing a member spec emits a JSON report and prints its recoveries; `check-app-e2e-executed.mjs` stdout is byte-identical under the collector extraction (`tests/ci/appE2eAnnotationPrint.test.ts` green unchanged); the new printer has a vitest case (child-process, synthetic report: prints rows from `tests[].annotations` only, exactly-once on the duplicated-location case, plus the total line — red first). **Plus a behavioral case per gating oracle** (`check-app-e2e-executed.mjs` and `check-crew-e2e-executed.mjs`), each run as a child process against a synthetic report that contains BOTH an `infra-recovery` annotation AND an executed-count floor shortfall, asserting that the annotation IS printed and the exit code is still 1. Spec-review R6 caught that neither existing AC-4 case covers this path — the thin-printer case exercises an always-zero process, and the app-oracle case exercises a successful run — so without these two the ordering rule above would be asserted nowhere.
