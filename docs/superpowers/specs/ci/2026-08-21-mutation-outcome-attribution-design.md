...
11. **A declared input that changes and CHANGES BACK inside one child's lifetime is not observable.** The stamps are taken around each run — before the first, around every run, and after the last — so an A→B→A edit spanning runs IS caught even though the endpoints agree. What no number of stamps taken outside a child can see is a file that becomes B and returns to A entirely between two stamps, i.e. within one child's execution window. The residue is bounded by that window rather than by the whole invocation, and it is a REPORTING gap, never a wrong report: the harness stays silent about it rather than certifying anything false.

    **The alternative — stamping continuously, or from inside the child — is declined.** It would put a filesystem watcher in the measurement path of the thing being measured, which changes the timing this arc exists to characterize.
