...
**The bound that actually holds is the execution context's.** Every entry point that reaches `snapshotAssets` is a serverless function: `app/api/cron/sync/route.ts:15` and the onboarding finalize/extract routes declare `maxDuration = 300`, and routes that declare nothing take the platform default, which is likewise minutes. The process is killed long before the gate; a live run's own objects are therefore at most single-digit minutes old, against a 24h gate — a margin of ~288×. This is what makes the deletion safe, and because it is a property of the DEPLOYMENT rather than of this module, it is pinned executably by `tests/sync/pendingOrphanAgeGatePremise.test.ts` (every route reaching `snapshotAssets` whose declared `maxDuration`, times a 24× safety factor, must still fit inside `PENDING_ORPHAN_MIN_AGE_MS`) and recorded as a standing dependency in §7 L8.
...
| L8 | The §4 age gate is safe only while a live `snapshotAssets` run cannot outlive `PENDING_ORPHAN_MIN_AGE_MS`. That bound comes from the EXECUTION CONTEXT (serverless `maxDuration`, minutes), not from any guard inside the run — the drive stall guard is idle-bounded, not total-bounded (§4). | PINNED, not assumed: `tests/sync/pendingOrphanAgeGatePremise.test.ts` walks the import graph from every `app/api/**/route.ts`, and any route that transitively reaches `lib/sync/snapshotAssets.ts` must keep `declared maxDuration × 24 ≤ PENDING_ORPHAN_MIN_AGE_MS`. Moving snapshot uploads into a long-running worker, or raising a reaching route's `maxDuration` toward the gate, reds that test — which is the signal to revisit the gate, not a reason to raise the constant. |
