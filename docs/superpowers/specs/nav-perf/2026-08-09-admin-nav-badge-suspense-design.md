...
| navigation, then seed resolves | pathname refetch committed (token bumped); the attention seed is DISCARDED by the token; a bell seed goes through `ingestPropValue`, whose post-zero/staleness rules decide (demote or discard) — in neither case is the stale value painted |
