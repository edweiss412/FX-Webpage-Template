...
| navigation, then seed resolves | pathname refetch committed → state not virgin → BOTH seeds DEMOTED to a fresh fetch (§3.2.1; an `infra_error` seed applies its posture immediately instead, §5.5) — in neither case is the stale value painted (R4 F1's interleaving, test required) |
...
- **AC-5** Every §3.2/§3.7 interleaving row has a test: seed-then-navigate, navigate-then-seed (non-virgin: BOTH demoted to a fresh fetch per §3.2.1, and a non-virgin `infra_error` seed applying its posture immediately per §5.5 — the R4 F1 stale-paint interleaving asserted for BOTH hooks), a synchronous initial value claiming the hook so a later seed cannot overwrite it (§5.7), zeroNow-before-seed, infra_error seed (bell degraded), hung seed (pathname refetch repopulates), refresh-promise-resolves-after-zeroNow (value demoted to fresh fetch, stale pre-open count never painted), AND the promise-supersession pair for BOTH hooks: P1-resolves-after-P2-arrives (ignored) and P2-hangs-after-P1-invalidated (pending persists, refetch repopulates). Existing pathname-refetch and post-zero-demotion tests keep passing unmodified.
...
6. **The last-known-good count is RETAINED while a newer read is in flight, and this is deliberate** (diff review R1 F1, refuted as a defect and recorded here so it is not re-derived). Two orderings were probed: (a) a seed commits 5, a newer promise arrives and HANGS — the badge keeps showing 5; (b) a seed commits, a newer promise demotes to fetch F2, a third promise arrives and hangs, and F2 then resolves and paints. Neither is a stale paint. In (a) no newer value exists — 5 was true as of the last completed read, and blanking to the pending shape on every layout re-render would flicker the badge empty on every `router.refresh` while telling the user strictly less. In (b) F2 is a LIVE client fetch issued after its seed resolved, so its result is the freshest thing the client can obtain; if the third promise later resolves it demotes to its own fetch, whose newer monotonic token supersedes F2's. The general rule: **arrival of a newer promise invalidates the older SUBSCRIPTION, not the committed STATE** — state is only ever replaced by a value, never by the absence of one. Every case repopulates on the next pathname change.
   ...
   **The same limit covers the passive-effect timing window** (diff review R4 F1, refuted by equivalence). React flushes passive effects — including an effect's cleanup — after paint, so there is a window in which a newer promise has committed but the older subscription's cleanup has not yet run, and a resolution landing inside it still paints. The OUTCOME is the case already ratified two paragraphs up: the badge holds a completed read's value while a newer read is in flight, and if that newer read hangs it stays until the next navigation. It is the only completed read the client has.

   A demote-on-supersession variant was implemented and REVERTED rather than shipped. Comparing the resolving promise against a render-time ref does close the window, but the branch is not reachable from the test harness — `flushSync` flushes pending passive effects before its own render, and inside `act` the render is deferred past the resolution, so every constructible ordering resolves either before the newer promise renders (where painting is correct) or after its cleanup ran (where the existing closure flag already handles it). Shipping a branch whose guard can only be written vacuously is worse than the documented limit it removes, and the limit is one this spec had already accepted on its own merits.

7. **A synchronous initial value CLAIMS the hook** (diff review R2 F1). A caller may pass both a synchronous count and a seed promise; `AdminNav` accepts both props even though the layout passes promises only. A synchronous count has already committed, and a seed minted by the same server render is not newer than it, so each hook starts claimed whenever its initial prop carries a real value — the seed then takes the non-virgin path. `null` is the pending shape and claims nothing, which is what keeps the streaming path working.
