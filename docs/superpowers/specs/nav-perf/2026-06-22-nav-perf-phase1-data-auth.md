...
- `getShowForViewer(showId, viewer): Promise<ShowForViewer>` L244. Ordered reads: crew identity L262-267 (drives `isLead` L280, `viewerName` L275); shows L283 (depends only on `showId`); crew roster L346; hotel_reservations L390; rooms L424; transportation L455; contacts L503; `shows_internal` run_of_show L530; `shows_internal` financials L603 (**gated on `isLead`** L601); `viewer_version_token` RPC L637. **Superseded 2026-08-07:** the conditional omission of the financials read is reversed — the read is unconditional and only the RETURNED value is entitlement-gated (see `docs/superpowers/specs/2026-08-07-projection-financials-viewer-independent-design.md` §3); wave parallelism is preserved and pinned by the non-LEAD wave-membership test.
...
**A1 · `lib/data/getShowForViewer.ts`.** Keep the crew-identity lookup (L262) and the `shows` validation (L283) sequenced first (they gate role + `showId` and carry the fail-closed throws). After the show row is validated and `isLead` derived, run the independent reads concurrently with one `Promise.all` wave: crew roster, hotel_reservations, rooms, transportation, contacts, run_of_show, `viewer_version_token` RPC, and — only when `isLead` — financials (build the array conditionally, or `Promise.all` an array that omits the financials promise when `!isLead`). **Superseded 2026-08-07:** the financials promise is now an unconditional wave member for every viewer; entitlement gates only the returned value (see `docs/superpowers/specs/2026-08-07-projection-financials-viewer-independent-design.md` §3). The parallelism property this bullet protects is unchanged and pinned by the non-LEAD wave-membership test. Each promise keeps its existing `try/catch` + `tileErrors[id]` (soft) or throw (hard, for the RPC) handling — the parallel wave wraps the same per-read handlers. Post-fetch in-memory filtering (hotel by `viewerName`, run_of_show by date restriction) is unchanged. **Expected ~200-500ms.**
...
**A3 · `app/admin/settings/page.tsx`.** Add one combined reader (e.g. `lib/appSettings/getSettingsPageFlags.ts`) that does a single `app_settings` multi-column select (`auto_publish_clean_first_seen, alert_on_sync_problems, daily_review_digest, alert_on_auto_publish`) via `createSupabaseServiceRoleClient()`, returning a typed `{ kind: 'value', ... } | { kind: 'infra_error' }` with **literal-`=== true`** per-flag mapping (fail-closed, matching the existing single getters). The settings page calls it once as the **happy path**; **on `{ kind: 'infra_error' }` it FALLS BACK to the four existing single-column getters run in parallel** (`Promise.all([getAutoPublishCleanFirstSeen(), getAlertOnSyncProblems(), getDailyReviewDigest(), getAlertOnAutoPublish()])`), mapping each to its toggle initial independently. **Rationale (Codex plan R9 MEDIUM):** the combined read changes the failure boundary — one drifted/renamed/erroring column fails the whole SELECT and would degrade all four toggles; this repo has a documented validation/schema-drift failure mode, so the fallback preserves the prior **per-toggle isolation** (an unaffected column still renders its real value). Keep the four single getters untouched (other callers, incl. sync, depend on them; they are also the fallback). `Promise.all` the three independent top-level loads on the page (the flags read, `fetchDriveConnectionHealth`, `fetchEmbeddedAdminEmails`). **The combined reader is a NEW Supabase call site → add an invariant-9 registry row (admin meta-test) — see §6. Expected ~60-200ms (happy path); fallback is still one parallel wave.**
...
In `requireAdmin`, after a valid email is derived, call `is_session_live()` and `is_admin()` **in parallel** (`Promise.all` — both are independent JWT-only reads). Handling is **ERROR-FIRST** (invariant 9): destructure both `{data,error}`, then (1) `sessionError` → `AdminInfraError`, (2) `adminError` → `AdminInfraError` — BOTH checked **before any data verdict**, so the `{ sessionLive:false, adminError:<infra> }` combination does NOT collapse into a benign redirect and hide an admin DB outage (Codex plan R6 HIGH); then (3) `is_session_live` data `!== true` → `redirectToSignIn()` (revoked/expired session = unauthenticated, precedence over forbidden), (4) `is_admin` data `!== true` → `forbidden()`. A thrown error from the `Promise.all` → `AdminInfraError`.
...
**B3 · `React.cache()` the resolution.** Extract the expensive, layer-independent resolution into a **no-arg** cached core: `const resolveAdminIdentity = cache(async (): Promise<AdminIdentity> => { create client → getClaims → email → Promise.all([is_session_live, is_admin]) → return {email} })`. `requireAdminIdentity(opts)` runs the layer-specific `maybeForceTestInfraFail(forceHeaders, layer)` hook **outside** the cache (it must fire per-layer), then `return resolveAdminIdentity()`. `requireAdmin(opts)` runs its own hooks then delegates as `requireAdminIdentity(opts)` — forwarding the layer so the delegated gate runs at the caller's layer (the prior no-opts delegation was a latent layer-drop; fixed per Codex whole-diff R1, with a test pinning that a layout `requireAdmin` is exempt under a page-scoped force). Because the core is no-arg, the layout gate (`{layer:"layout"}`) and the page gate (`{layer:"page"}`) share one resolution per request → one `getClaims` (local) + one parallel `is_session_live`+`is_admin` RPC pair instead of two of each.
...
- **`getShowForViewer` financials gating:** when `!isLead`, the financials promise must be omitted from the wave (not awaited-then-discarded) so non-LEAD viewers issue zero financials reads — preserving current behavior and access posture. **REVERSED 2026-08-07:** the omit-not-discard clause is explicitly reversed by the viewer-independent-probe arc — awaited-then-discarded is now the ratified contract (the read runs for every viewer so fetch failures are observable on any cache fill; the value is discarded at the return boundary inside `readFinancials`, so the access posture is unchanged). See `docs/superpowers/specs/2026-08-07-projection-financials-viewer-independent-design.md` §2.1/§3.
