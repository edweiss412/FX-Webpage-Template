...
| Preserved testids (enumerated by NAME — line numbers shift, names are the contract) | `recent-auto-applied-strip`, `auto-applied-error`, `auto-applied-overflow`, `auto-applied-group-${showId}`, `auto-applied-row-${id}`, `auto-applied-accept-all-${showId}`, `auto-applied-undo-all-${showId}`, `auto-applied-undo-all-confirm-${showId}`, `auto-applied-undo-all-cancel-${showId}`, `auto-applied-undo-all-confirm-go-${showId}` (all in `RecentAutoAppliedStrip.tsx`); `change-feed-accept` (`AcceptChangeButton.tsx`), `change-feed-undo` (`UndoChangeButton.tsx`) |
...
- **Diff derivation:** given injected rows with `before_image`/`after_image`, assert each produced `row.diff` exactly: `crew_renamed` → `{kind:"fromTo", from:"Jon Clark", to:"John Clark"}`; `crew_added` → `{kind:"single", caption:"Added", value:"Maria Chen"}`; `crew_removed` → `{kind:"single", caption:"Removed", value:"Devin Park"}`; `field_changed` & `crew_email_changed` → `{kind:"none"}`.
  ...
  - **MANDATORY select-projection assertion (defeats the pass-through-`select` tautology):** the existing fake client's `builder.select` is a pass-through that captures nothing (`tests/admin/loadRecentAutoApplied.test.ts:44`), so a diff test would pass even if the real `.select(...)` were never updated to pull `before_image`/`after_image` — production would then silently degrade every row to `diff:none`. The extended fake client MUST capture the `select` projection string, and the test MUST assert it contains both `before_image` and `after_image`. This binds the green test to the real column list, not just to fixture shape. (Without this the whole projection is untested.)
