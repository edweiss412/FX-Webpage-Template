...
- **Site cap** = `MAX_SITES_PER_OP` (default **8**), applied as *first-N in document order* deterministically. When a fixture has more sites than the cap, the harness **`log`s the dropped count** (never silently truncates — audit-discipline "no silent caps"). Exposed as a `droppedSites` count asserted-visible, not hidden.
- **Determinism:** site enumeration is a deterministic scan (top-to-bottom); the Damerau transposition uses a fixed rule (first transposable adjacent pair of distinct chars). No `Math.random`.
...
4. Total mutant count and per-operator `droppedSites` are surfaced (a summary `console`/`log` line or an asserted count), so coverage is legible and silent truncation is impossible.
