...
These pin the audit's §2 "credit where due" invariants. A cosmetic edit must be **fully invisible** — not merely data-preserving. The oracle requires **both**: (a) data payload deep-equal to baseline, **and** (b) `signalKeys(mutant)` multiset-equal to `signalKeys(baseline)`. A cosmetic mutation that preserves data but *adds a warning or hardError* (e.g. a reordered block that trips `UNKNOWN_SECTION_HEADER`, or trailing whitespace that emits a spurious warning) changes admin/user-visible parser state and therefore **FAILS** — payload-only equality is insufficient (Codex R2 HIGH). Any divergence in payload **or** signal multiset → FAIL.
...
- **Per-domain coverage floor (GATING, not logged) with classifier parity.** Each block is classified by its leading header token via an **explicit** `SECTION_DOMAIN_MAP: Record<string, Domain>` keyed on `normalizeHeader(...)` (`lib/parser/knownSections.ts` — the parser's own header normalizer). `Domain = crew | hotel | rooms | transportation | agenda | dates | event_details | venue | dress | contacts | other`. The concrete mapping (single-sourced against the block-parser headers pinned in `tests/parser/_metaKnownSectionsRegistry.test.ts`):
  - `crew` ← `CREW`, `TECH`
  - `hotel` ← `HOTEL`, `HOTEL RESERVATIONS`, `HOTEL STAYS`
  - `rooms` ← `GENERAL SESSION`, `BREAKOUT`, `ADDITIONAL ROOM`, `LUNCH ROOM`
  - `event_details` ← `EVENT DETAILS`, `GS DETAILS`, `DETAILS`
  - `transportation` ← `TRANSPORTATION`; `dates` ← `DATES`; `agenda` ← `AGENDA`, `AGENDA LINK`
  - `venue` ← `VENUE`, `VENUES`; `dress` ← `DRESS`; `contacts` ← `IN HOUSE AV`
  - `other` ← a block whose leading header is **not** a parser-known section header (genuine metadata/client rows).
  - **Classifier-parity gate (structural test):** iterate `KNOWN_SECTION_HEADERS` and assert **every** parser-known header is explicitly present in `SECTION_DOMAIN_MAP` and does **not** fall through to `other`. A parser-known header missing from the map → **FAIL**. This prevents the finding #3 drift class: a future block-parser alias cannot silently demote a risk-critical domain to `other` and disable its floor (Codex R2 MEDIUM).
  - **The floor:** for each **corrupting** operator, assert **every risk-critical domain present in the fixture receives ≥1 mutant** — risk-critical = `{crew, hotel, rooms, transportation, agenda, dates, event_details}`. A risk-critical domain that is present (≥1 block classified to it with ≥1 applicable site) but receives **zero** mutants → **FAIL**. Raising `MAX_SITES_PER_OP` or reordering fixtures can never quietly starve a high-risk section. Domains absent from a fixture are skipped (not every show has transportation). `venue`/`dress`/`contacts`/`other` are mapped (never `other`-by-accident for the first three) but **un-floored** (lower audit-tier risk); they still participate in round-robin so they are not zero-covered.
