...
export type KnownHole = {
  ...
  fingerprint: string;                         // stable hash of HOW the mutant diverges (see below)
...
**Behavior fingerprint (Codex R7 MEDIUM).** Keying only on `(siteId, kind)` would let a *regression inside an already-ledgered hole* pass green: if a parser change makes a known `SILENT_WRONG` site corrupt **more** fields (or drop more data) while staying the same verdict kind, `actual` still matches the ledger. Each alarm mutant therefore carries a **`fingerprint`** — a deterministic, stable summary of *how* it diverges from baseline: the **sorted set of changed payload JSON-paths** (path only, not volatile values — so cosmetic value churn doesn't thrash it) **plus the sorted signal-key delta** (`+H:CODE` / `-W:CODE` / `±R:block|key`), hashed. The fingerprint is order-stable and value-light so it flags *structural* behavior change (new fields corrupted, a signal newly lost) without breaking on every fixture text tweak.
