...
1. `pnpm vitest run tests/parser/mutationHarness.test.ts` is **green** with the committed ledger: the day-1 committed ledger equals the day-1 **alarm set `SILENT_WRONG ∪ SILENT_SIGNAL_LOSS`**, compared on `(siteId, kind)` (both alarm classes pinned, not just `SILENT_WRONG`).
