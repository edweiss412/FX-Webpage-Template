...
**Non-goals:**
- **Zero product-source change; test/CI-infra wiring permitted.** The harness proper is purely additive test infrastructure under `tests/parser/mutation/` (+ `tests/parser/mutationHarness.test.ts`). It fixes no audit finding; it *pins current reality* and *catches regressions*. Findings #1–#13 remain open holes, recorded in the ledger. Because the harness is a ~100s serial gate, it is placed OFF the fast unit path (AC-5) via three TEST/CI-infrastructure edits — `vitest.projects.ts` (exclusion list), `package.json` (audit script), `.github/workflows/x-audits.yml` (dedicated job). **No PRODUCT source is touched** — nothing under `lib/parser`, `app/`, `components/`, `supabase/`.
...
5. The harness runs in local `pnpm test` (discovered by the existing vitest config) and in CI as a **dedicated `x-audits.yml` job** — OFF the fast 2-leg `unit-suite` path. At ~100s serial it is a heavy gate; per the repo convention (traceability, x1–x6, postgrest-dml-lockdown all run as parallel x-audits jobs), it is excluded from unit-suite via `ENV_BOUND_EXCLUDES` (gated by `VITEST_EXCLUDE_ENV_BOUND=1`) and run in CI by the x-audits `mutation-harness` job via `vitest run <file>`. (Amended from the original "plain discovered file, no new workflow" after the CI-topology analysis showed an unweighted heavy file would overload a unit-suite shard leg — plan-R19/R21.)
