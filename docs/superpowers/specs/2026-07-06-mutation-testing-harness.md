...
**Non-goals:**
- **Zero product-source change; test-infra weighting permitted.** The harness proper is purely additive test infrastructure under `tests/parser/mutation/` (+ `tests/parser/mutationHarness.test.ts`). It fixes no audit finding; it *pins current reality* and *catches regressions*. Findings #1–#13 remain open holes, recorded in the ledger. Because the harness is a ~100s serial gate, two TEST-infrastructure files are edited so the weight-balanced sequencer sizes it correctly on the (required, merge-gating) unit-suite path — `lib/test/vitest.weights.ts` (register its measured weight) and `tests/cross-cutting/vitest-shard-balance.test.ts` (derive the hot-file pair from the top-2 weights). **No PRODUCT source is touched** — nothing under `lib/parser`, `app/`, `components/`, `supabase/` — and no new workflow.
...
- **Detection is EXHAUSTIVE — no cap, no sampling (plan-R2).** The §2 goal is "every fixture × operator × **site**." Every applicable site an operator enumerates is parsed and classified by the oracle; there is **no `MAX_SITES_PER_OP` cap and no selection/reservation limiter on detection**. A cap would let a silent-wrong parse in an un-parsed site ship undetected (defeating the whole harness), so it is rejected. Because the committed fixtures are small and static and `parseSheet` is pure/fast, exhaustive parsing is tractable. (An earlier draft capped detection at 12 with floor-first reservation + round-robin fill + `droppedSites`; that machinery is removed — with exhaustive detection the floor is satisfied by construction and nothing is dropped.)
...
5. The harness runs in local `pnpm test` and in CI as part of the **required `unit-suite` merge gate** — a normally-discovered, weight-balanced heavy vitest file (no new workflow). It is registered in `lib/test/vitest.weights.ts` with its measured wall-clock so the LPT shard sequencer balances it across the two matrix legs, and being inside the required `unit-suite` aggregator (which gates on all legs) makes it **merge-blocking**: a divergence from the ledger fails the PR, satisfying the CI-gating requirement. (Amended plan-R22: a dedicated `x-audits` job — briefly adopted in R19–R21 to keep it off the fast path — is NOT in the required-check inventory (`generate-traceability.ts` recognizes only `x1`–`x6`/`traceability-audit`/`verify-branch-protection-status`), so it would run but not block merge; the unit-suite path is merge-gating for free, and correct weighting is the actual fix for the shard-balance concern.)
