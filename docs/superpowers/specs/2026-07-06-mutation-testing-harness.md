...
- **Full signal-object diff — aligned exactly to `signalEq` (Codex R9 HIGH, R15 HIGH):** the reduced key-multiset delta is insufficient for the `signal_loss` class, where the payload diff is empty AND the key multiset can be unchanged while a *within-object* field drifted. The fingerprint's signal component must pin **every field `signalEq` compares** — otherwise a ledgered hole could change source-anchoring/debug context (`blockRef`, `rawSnippet`, `sourceCell`) while keeping the same fingerprint. It is therefore the redacted diff of the **complete** channel entries: for each `warnings[]`/`hardErrors[]`/`raw_unrecognized[]` entry gained/lost/changed, a digest of the entry's **canonical redacted JSON over ALL its fields** — `ParseWarning{severity, code, digest(message), digest(rawSnippet), blockRef{kind,index,iso,name}, digest(sourceCell)}`, `ParseError{code, digest(message), blockRef{kind}}`, `raw_unrecognized{block, key, digest(value)}` (`types.ts:4-22,385`). Free-text/PII fields are `digest`-ed (per below); structural fields (severity, code, blockRef.kind/index/iso/name, block, key) are kept verbatim. The signal-object diff is sorted and hashed. Any change `signalEq` would see — message, snippet, anchor, or block index — moves the fingerprint.
