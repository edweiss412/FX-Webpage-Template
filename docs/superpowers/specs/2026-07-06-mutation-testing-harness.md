...
- **Detection is EXHAUSTIVE — no cap, no sampling (plan-R2).** The §2 goal is "every fixture × operator × **site**." Every applicable site an operator enumerates is parsed and classified by the oracle; there is **no `MAX_SITES_PER_OP` cap and no selection/reservation limiter on detection**. A cap would let a silent-wrong parse in an un-parsed site ship undetected (defeating the whole harness), so it is rejected. The committed fixtures are small and static, but exhaustive enumeration across all 17 still yields **101,795 mutants at ~29.8 ms/parse ≈ 50 min serial** (measured, AC-4 / Task 8 Step 1); rather than compromise exhaustiveness, the harness is placed on a **dedicated nightly workflow** off the merge-gating fast path (§Non-goals, AC-5). (An earlier draft capped detection at 12 with floor-first reservation + round-robin fill + `droppedSites`; that machinery is removed — with exhaustive detection the floor is satisfied by construction and nothing is dropped.)
