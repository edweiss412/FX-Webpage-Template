...
**Non-goals:**
- **Zero product-source change; a dedicated scheduled workflow is added.** The harness proper is purely additive test infrastructure under `tests/parser/mutation/` (+ `tests/parser/mutationHarness.test.ts`). It fixes no audit finding; it *pins current reality* and *catches regressions*. Findings #1–#13 remain open holes, recorded in the ledger. **Runtime reality (measured, plan-R17 / Task 8 Step 1): the exhaustive corpus is 101,795 mutants at ~54 ms/`parseSheet`, ≈ 5,541 s (~92 min) serial** — 20× the 150 s ceiling a merge-gating `unit-suite` leg (20-min timeout) can absorb. Rather than sample (which would let a silent-wrong parse ship undetected, §4.3) or shard into ~20 fragile files, the harness runs as a **dedicated scheduled (nightly) GitHub Actions workflow** (`.github/workflows/mutation-harness.yml`, `schedule:` cron + `workflow_dispatch:`), **NOT** on the merge-gating `unit-suite` path (amends the R22 merge-gate decision; user-directed 2026-07-06). It is therefore **not merge-blocking** — a ledger divergence fails the nightly run and is triaged, not gating every PR. Because it leaves the fast merge path, **no `lib/test/vitest.weights.ts` weight row and no `tests/cross-cutting/vitest-shard-balance.test.ts` edit are needed** (those exist only to balance the heavy file across unit-suite legs). **No PRODUCT source is touched** — nothing under `lib/parser`, `app/`, `components/`, `supabase/`; the only non-`tests/` file added is the new workflow YAML.
...
A change of meaning is acceptable *if announced*. **Operator cardinality (canonical, used everywhere):** 8 operators — **7 corrupting + 1 cosmetic (§4.2)** — emitting **9 registered operator keys**, because `blank-row` (item 5) registers as both `blank-row:inject` and `blank-row:remove`. Every count-bearing reference (corpus totals, coverage summaries, `siteId` keys, audit tables, the operator-inventory gate) is keyed on the **9 registered keys**. The corrupting set = the **six audit-named domain-scoped operators below** PLUS **`section-reorder`** — originally specified cosmetic but **reclassified corrupting 2026-07-06** after the day-1 exhaustive run measured the parser to be order-SENSITIVE (58/486 swaps SILENT_WRONG + 24 SILENT_SIGNAL_LOSS — reordering source sections silently reorders the output payload arrays). It is domain-agnostic (`domains: []`, whole-document perturbation), so it is EXEMPT from the per-domain coverage floor / audit-agreement gate (§4.3), and its silent holes are pinned in the ledger like any corrupting operator. Only `trailing-whitespace` remains cosmetic. The six audit-named corrupting operators:
...
7. **`section-reorder`** (RECLASSIFIED CORRUPTING 2026-07-06, was cosmetic) — swap the order of two adjacent top-level blocks separated by blank lines. The original premise (block parsers scan the whole doc, so a reorder is output-identical) was **empirically disproven** by the day-1 run: the parser preserves SOURCE ORDER into its output arrays (`crewMembers`/`rooms`/`hotelReservations`/…), so 58/486 swaps changed the payload with NO warning (SILENT_WRONG) and 24 dropped a signal (SILENT_SIGNAL_LOSS). This order-sensitivity is exactly the silent fragility the harness pins, so section-reorder is now a **corrupting** operator: order-preserving swaps stay `ABSORBED`, warning-tripping swaps are `SIGNALED`, and the silent ones become ledger holes. `domains: []` (whole-document) → exempt from the per-domain floor/audit gates. Site = each adjacent block-pair swap.

### 4.2 Cosmetic-invariant bucket — oracle: FULL parse equivalence (payload **and** signals unchanged)

These pin the audit's §2 "credit where due" invariants. A cosmetic edit must be **fully invisible** — not merely data-preserving. The cosmetic oracle requires **complete parse equivalence**: (a) data payload deep-equal to baseline, **and** (b) the **full signal channels** deep-equal to baseline — the entire `warnings[]`, `hardErrors[]`, and `raw_unrecognized[]` arrays including every field (`raw_unrecognized`'s `value`, not just `block`/`key`). The reduced `signalKeys` **multiset is NOT used for the cosmetic bucket** — it discards `raw_unrecognized.value` and warning messages, so a cosmetic mutant that changed an admin-visible raw-unrecognized value while keeping the same block/key count would wrongly pass (Codex R3 MEDIUM). Cosmetic comparison is a straight `toEqual` over `{ payload, warnings, hardErrors, raw_unrecognized }`. Trailing whitespace that alters any signal field therefore **FAILS**. (`signalKeys` remains the reduced multiset used ONLY by the corrupting bucket's `newSignalFired`, §3.2.) **Only one cosmetic operator remains** (section-reorder was reclassified corrupting, §4.1 item 7):

8. **`trailing-whitespace`** — append trailing spaces to non-empty lines and/or trailing blank lines at EOF (authoring noise MUST be invisible). Measured day-1: 0 violations — genuinely cosmetic.

### 4.3 Per-operator guard conditions

- Every operator is a **pure, lazy generator** `(markdown) → Generator<Mutant>` — this is the canonical corpus-scale form; a `Mutant` is `{ md: string; siteId: string; bucket: 'corrupting' | 'cosmetic'; domains: Domain[] }`. A thin eager `[...gen]` array wrapper (`OPERATORS[op](md): Mutant[]`) is provided for **BOUNDED synthetic unit tests only**. **Every corpus-scale enumeration MUST go through the shared budget-guarded stream `boundedMutants(op, md)`** — which wraps the module-private raw generators in `guardStream(gen, MUTANT_BUDGET, …)`, throwing before yielding the (`MUTANT_BUDGET`+1)th mutant — never an eager array over a real fixture. This makes a fanout regression fail fast with O(1) heap instead of OOM/hang. The raw generators are NOT exported, so the guarded stream is the only corpus path (memory/streaming vector closure, plan-R24).
- **No applicable site → the generator yields nothing** (an empty stream / empty `[...]`), never a throw. A fixture with no ≥3-cell row yields no `merged-cell` mutants — that is correct, not an error.
- **Logical-section segmentation — header-anchored, NOT blank-line-delimited (Codex R10 HIGH).** Raw fixtures pack several logical sections into one blank-line-delimited pipe run, separated by **empty table rows** (`|  |  |`), *not* blank lines: `fixtures/shows/raw/2025-10-consultants-roundtable.md:62-78` is one run holding `DATES` (:62), a spacer row (:68), `CREW` (:69), a spacer (:76), and `DRESS` (:77) — headers in **col 0**. A blank-line-delimited "block" classified from its first row would label that whole run `dates`, so the CREW cell sites are never owned by `crew` and the crew floor is silently evaded. Segmentation is therefore **logical**, mirroring the parser's own header-anchored scanning:
  ...
  - **Row taxonomy — only DATA rows are mutable (Codex R12 HIGH).** Within a section, each row is exactly one of: **header row** (`resolveHeader(firstCell) != null`); **alignment/separator row** (every non-empty cell matches `^:?-{1,}:?$`, i.e. `| :---: | :--- |`); **empty spacer row** (all cells blank); or **data row** (everything else). The cell/row-shaped operators (`ref-sub`, `unicode-inject`, `merged-cell`, `blank-row:inject`, `column-shift`) enumerate sites **only over data rows / data-row cells** — header, alignment, and spacer rows are **excluded** so a floor can never be satisfied by mutating a Markdown delimiter instead of real exported sheet data. This exclusion is applied identically in operator generation, `floorEligible`, and the independent `applicabilityAudit`.
- **Site ownership.** Every *intra-section* operator site (`header-typo`, `ref-sub`, `unicode-inject`, `column-shift`, `blank-row:inject`, `merged-cell`) belongs to **exactly one** logical section and carries its domain. The one *boundary* operator, **`blank-row:remove`**, mutates a blank line between two adjacent pipe **runs** `(i, i+1)` and is a **boundary site** (Codex R6 MEDIUM): its `domainSet = { domain(last logical section of run i), domain(first logical section of run i+1) }`. For `floorEligible` it counts toward **each** adjacent risk-critical domain in `domainSet`; its `siteId` locus is the inter-run gap index (`X<gapIdx>`).
...
- **`SECTION_DOMAIN_MAP` — covers every current `KNOWN_SECTION_HEADERS` member** (`knownSections.ts:34-65`, 30 entries — exhaustive so the parity gate is satisfiable). `Domain = crew | hotel | rooms | transportation | agenda | dates | event_details | venue | dress | contacts | client | pull_sheet | documents | other`:
  ...
  - **Classifier-parity gate (structural test):** (a) iterate `KNOWN_SECTION_HEADERS` — assert every entry is a key of `SECTION_DOMAIN_MAP` and does **not** resolve to `other` (a future registry alias missing from the map → **FAIL**; the map is forced complete because the gate reads the live registry); (b) iterate `PREFIX_SECTION_FAMILIES` — assert a **suffixed** sample (`"<fam> GRAND BALLROOM"`) classifies to `rooms`, pinning the prefix-resolution path (Codex R4 HIGH); (c) **independent domain oracle** — a hand-authored `EXPECTED_HEADER_DOMAINS` table in its OWN data module (`tests/parser/mutation/expectedDomains.ts`, NOT co-located with `SECTION_DOMAIN_MAP` in the classifier implementation) pins each header's **intended domain**. The gate asserts (c1) the oracle COVERS the live `KNOWN_SECTION_HEADERS` registry — a new parser header forces an oracle row — and (c2) `SECTION_DOMAIN_MAP` AGREES with the oracle, so a wrong non-`other` domain (e.g. `DRESS`→`venue`) is caught by cross-mismatch between two SEPARATELY-authored surfaces, not self-reference. A **red-phase proof** (mis-map one header in `SECTION_DOMAIN_MAP` only → gate fails → revert) pins the gate live. (No external domain pin exists in the repo — the live `_metaKnownSectionsRegistry.test.ts` pins header *tokens*, not domains — so the anti-drift anchor is registry-coverage + the separate-module oracle, not lockstep with a nonexistent domain pin; Codex plan-R21 [high].)
...
- **Applicability is per-operator, and the floor is scoped to it (Codex R5 HIGH).** An operator's applicability is *narrower* than domain-presence: `merged-cell` needs a body row with ≥3 cells, `blank-row:inject` needs a block with ≥2 body rows, `header-typo` needs a recognizable header token. A risk-critical domain can be present yet have **zero applicable sites** for a given operator (a two-column `HOTEL` table has no ≥3-cell row). A floor phrased "every present domain gets a mutant for **every** operator" is therefore **unsatisfiable** and would force the gate to be weakened ad hoc. The floor is instead defined over **operator-applicable** domains only:
  ...
  - **`domains(site)` — one uniform helper covering intra-section AND boundary sites (Codex R7 HIGH).** To avoid "in a section classified D" language silently excluding the boundary operator, eligibility is defined over a single per-site domain function: `domains(site)` = `{ classifySection(logical section of the site's row) }` for an intra-section site, and `domainSet` = `{ classifySection(last section of run i), classifySection(first section of run i+1) }` for a `blank-row:remove` boundary site. Every rule below reads `domains(site)`, never "the section", so `blank-row:remove` is a first-class floor participant.
  - `floorEligible(O) = { D ∈ risk-critical : ∃ O-applicable site s with D ∈ domains(s) }`.
- **Detection is EXHAUSTIVE — no cap, no sampling (plan-R2).** The §2 goal is "every fixture × operator × **site**." Every applicable site an operator enumerates is parsed and classified by the oracle; there is **no `MAX_SITES_PER_OP` cap and no selection/reservation limiter on detection**. A cap would let a silent-wrong parse in an un-parsed site ship undetected (defeating the whole harness), so it is rejected. The committed fixtures are small and static, but exhaustive enumeration across all 17 still yields **101,795 mutants at ~54 ms/parse ≈ 92 min serial** (measured, AC-4 / Task 8 Step 1); rather than compromise exhaustiveness, the harness is placed on a **dedicated nightly workflow** off the merge-gating fast path (§Non-goals, AC-5). (An earlier draft capped detection at 12 with floor-first reservation + round-robin fill + `droppedSites`; that machinery is removed — with exhaustive detection the floor is satisfied by construction and nothing is dropped.)
- **Per-domain coverage floor (GATING), scoped to applicability.** For each **corrupting** operator `O`, assert every domain in `floorEligible(O)` has ≥1 **generated** mutant (a mutant `s` covers `D` when `D ∈ domains(s)`). With exhaustive generation this holds trivially; the assertion is the *guard* that catches an operator that stops enumerating a domain (paired with the independent audit below). A risk-critical domain that is **present but not** in `floorEligible(O)` (no O-applicable site — intra-block or boundary) is **surfaced** — recorded in a per-`(fixture, operator)` `skippedInapplicableDomains` list (logged, asserted-visible), so a domain is never *silently* excused: the reason (no applicable site) is explicit and auditable. Non-risk-critical domains (`venue, dress, contacts, client, pull_sheet, documents, other`) are un-floored but still generated.
- **Independent applicability audit (anti-tautology — Codex R9 HIGH, R13 MEDIUM).** `floorEligible(O)` and `skippedInapplicableDomains` are both derived from the operators' *own* site enumeration, so an operator **bug** (e.g. `ref-sub` stops matching hotel data cells) would silently demote a domain to "inapplicable" and keep the floor green. The audit (`tests/parser/mutation/applicabilityAudit.ts`) breaks this self-reference on **two** levels:
  - **Implementation-independent segmentation/classification (R13 MEDIUM).** The audit does **not** import the harness's `classify.ts`/`operators.ts` helpers — a bug in the shared segmenter/classifier/row-taxonomy would otherwise corrupt operator AND audit identically, leaving `floorEligible` green while a section is skipped. The audit re-derives its site inventory with its **own minimal** header-anchored scan (its own `resolveHeader`/row-taxonomy pass over raw markdown), so the two paths can only agree by both being correct.
  - **Hand-verified golden inventory.** Beyond agreement, a **committed golden table** of exact `(fixture, operator, risk-critical domain) → siteCount` for a representative subset (e.g. `ref-sub × rpas × hotel`, `merged-cell × consultants × rooms`, `column-shift × consultants × crew`) is asserted — hand-verified against the raw markdown at authoring time. This is a truly-external oracle: an operator that stops enumerating a domain fails the golden count even if its self-reported eligibility is internally consistent. Counts derive from fixture dimensions (anti-tautology "derive from fixtures").
  - **Negative controls:** breaking the shared segmentation/classification/row-taxonomy helper (e.g. force `resolveHeader → null`) makes the audit **fail** (proving it does not silently track the same bug); and an operator crippled to emit zero sites for a domain fails against both the inventory-agreement and the golden counts.
- **Determinism:** site enumeration is a deterministic scan (top-to-bottom within each section, sections in document order); the Damerau transposition uses a fixed rule (first transposable adjacent pair of distinct chars). No `Math.random`.
...
- **Full signal-object diff — aligned exactly to `signalEq` (Codex R9 HIGH, R15 HIGH):** the reduced key-multiset delta is insufficient for the `signal_loss` class, where the payload diff is empty AND the key multiset can be unchanged while a *within-object* field drifted. The fingerprint's signal component must pin **every field `signalEq` compares** — otherwise a ledgered hole could change source-anchoring/debug context (`blockRef`, `rawSnippet`, `sourceCell`) while keeping the same fingerprint. It is therefore an **order-sensitive** redacted digest of the **complete** channel entries. Because `signalEq` is a `toEqual` over the `warnings[]`/`hardErrors[]`/`raw_unrecognized[]` **arrays**, array order is part of the alarm condition; the fingerprint must be order-sensitive too or a pure reshuffle of a ledgered hole's entries would keep the same fingerprint while `signalEq` sees a change (Codex R16 HIGH). Each entry is keyed by its `(channel, arrayIndex)` and carries a digest of its **canonical redacted JSON over ALL its fields** — `ParseWarning{severity, code, digest(message), digest(rawSnippet), blockRef{kind,index,iso,name}, digest(sourceCell)}`, `ParseError{code, digest(message), blockRef{kind}}`, `raw_unrecognized{block, key, digest(value)}` (`types.ts:4-22,385`). Free-text/PII fields are `digest`-ed (per below); structural fields (severity, code, blockRef.kind/index/iso/name, block, key) are kept verbatim. The `(channel, arrayIndex) → entry-digest` map is canonicalized **by `(channel, arrayIndex)`** (order-preserving, so swapping two entries changes which content sits at which index) and hashed. Any change `signalEq` would see — a field value, an anchor, OR a reorder — moves the fingerprint.
...
Plus **one new CI workflow** (AC-5, user-directed 2026-07-06) so the ~50-min exhaustive harness runs nightly OFF the merge-gating fast path — NOT product source:

```
.github/workflows/mutation-harness.yml           # schedule: (nightly cron) + workflow_dispatch:; runs ONLY tests/parser/mutationHarness.test.ts; NOT in unit-suite; not a required check
```

Because the harness leaves the `unit-suite` fast path, the earlier `lib/test/vitest.weights.ts` weight row and `tests/cross-cutting/vitest-shard-balance.test.ts` HOT-pair edit (which existed only to balance it across unit-suite legs) are **NO LONGER made** — they would be dead weight-tuning for a file the sequencer never sees. No **product** source under `lib/parser`, `app/`, `components/`, `supabase/` is created or modified; the workflow YAML is CI infrastructure, not runtime product code. Confirmed by the diff being confined to `tests/parser/mutation/**` + the driver + the new workflow YAML + this spec + the plan.
...
- **Whole-document operator (`section-reorder` / `trailing-whitespace`) that cannot find two adjacent blocks / any line**: zero mutants, no failure.
...
2. Introducing a **synthetic regression** makes the harness **red** — proven by negative-control assertions in the plan's verification task covering: cosmetic signal-invariance, ledger bidirectionality (both directions), **within-hole fingerprint drift** for BOTH classes — a new-path payload change, a same-path-new-value change, AND (signal-loss class) a same-`block|key` `raw_unrecognized.value` / warning-message / warning `rawSnippet` / warning-or-hardError `blockRef` / `sourceCell` change — OR a pure **reorder** of two signal entries — with payload equal all shift the `fingerprint` → FAIL (fingerprint pins every field AND the array order `signalEq` compares); per-domain floor (`floorEligible`), **boundary-operator coverage** (a risk-critical block adjacent to a `blank-row:remove` site is reserved via `domains(s)`), **independent applicability audit** (an operator crippled to emit zero sites for a domain fails against the raw-markdown inventory + pinned nonzero counts, even though its self-reported eligibility stays consistent), **fixture registry-parity** (an unregistered committed fixture fails), `siteId` uniqueness, **`SILENT_SIGNAL_LOSS`** (a payload-unchanged signal removal is caught, not `ABSORBED`), and **inapplicable-domain surfacing** (a present-but-inapplicable domain lands in `skippedInapplicableDomains`, not a silent pass).
3. **Classifier fidelity:** a positive test proves suffixed room headers (`GENERAL SESSION GRAND BALLROOM`, `BREAKOUT 1 STATE A`, `ADDITIONAL ROOM 2`, `LUNCH ROOM SALON A`) classify to `rooms` (prefix-resolution path), and the parity gate covers all 30 `KNOWN_SECTION_HEADERS` entries. **Logical-section regression:** `fixtures/shows/raw/2025-10-consultants-roundtable.md`'s single pipe run holding `DATES`/`CREW`/`DRESS` (col-0 headers, spacer-separated) segments into **distinct** logical sections with domains `dates`, `crew`, `dress` — the CREW cell sites are `floorEligible` for `crew` (not absorbed into `dates`), AND that run produces a **`column-shift` mutant credited to `crew`** (a per-logical-section site), proving column-shift is not misattributed to `dates` (Codex R11). **Data-row-only regression (Codex R12):** for a section with a standard `| :---: | :---: |` alignment row, the reserved first `ref-sub`/`merged-cell` site is a **real data cell/row**, never the `:---:` delimiter or an empty spacer. **column-shift data-row gate (Codex R13):** a header/alignment/spacer-only section yields no `column-shift` site. **Audit-independence (Codex R13):** breaking the shared segmentation/classification helper makes `applicabilityAudit` **fail** (it does not import those helpers), and the committed golden inventory counts are met.
4. **Exhaustive coverage:** every applicable site is generated and parsed (no cap); a fixture with a **late** risk-critical section still yields its mutants for every corrupting operator, and the independent applicability audit's per-domain COUNT equals the operator's generated count (plan-R3), demonstrated by an explicit assertion.
5. The harness runs as a **dedicated scheduled (nightly) GitHub Actions workflow** (`.github/workflows/mutation-harness.yml`: `schedule:` cron + `workflow_dispatch:`), which invokes `pnpm vitest run tests/parser/mutationHarness.test.ts` on a runner whose leg timeout accommodates the measured ~92-min corpus. Because it is a ~92-min run (measured, AC-4 / Task 8 Step 1), it is **excluded from the default discovered suite** — added to the same exclusion surface `vitest.projects.ts` uses for env-bound tests (so a bare local `pnpm test` and the merge-gating `unit-suite` legs do NOT pick it up and blow their timeouts). It is therefore **NOT merge-blocking**: a ledger divergence fails the *nightly* run and is triaged manually, not on every PR. (Amends the R22 merge-gate decision, user-directed 2026-07-06: the exhaustive design's real runtime — 20× the 150 s unit-suite budget — makes a merge-gating placement infeasible without sampling or ~20-file sharding; nightly is the chosen trade-off, keeping full exhaustiveness at the cost of merge-gating. `workflow_dispatch:` lets close-out verify the workflow green on the PR branch before merge, per the "local-passes-CI-fails is its own bug class" discipline.)
6. Total mutant count (all exhaustively generated) + per-`(fixture, operator)` `skippedInapplicableDomains` are surfaced (a summary `console`/`log` line or asserted counts), so coverage is legible. The **applicability-scoped coverage floor** (§4.3) is asserted: for every corrupting operator `O`, each domain in `floorEligible(O)` has ≥1 generated mutant — a gating check paired with the independent audit; present-but-inapplicable risk-critical domains appear in `skippedInapplicableDomains` (never silently excused, never a floor failure). Proven by a negative-control that cripples an operator for a domain and shows the audit-agreement gate goes red.
7. Full `pnpm test`, `pnpm typecheck`, `pnpm lint`, `pnpm format:check` green before push.

## 10. Meta-test inventory

This milestone **creates** one structural harness (`tests/parser/mutationHarness.test.ts`) with three internal structural gates: (a) the bidirectional **known-holes ledger** (`knownHoles.ts`) — its own source of truth; (b) a **`siteId`-uniqueness** assertion over all generated mutants (guards ledger-undercount, Codex R2); (c) a **classifier-parity** gate asserting every member of `lib/parser/knownSections.ts`'s `KNOWN_SECTION_HEADERS` is explicitly mapped in `SECTION_DOMAIN_MAP` (guards coverage-floor drift, Codex R2). It **reads** `KNOWN_SECTION_HEADERS`/`PREFIX_SECTION_FAMILIES`/`normalizeHeader` from the parser registry (classifier resolves prefix families, not exact-only) but does not modify `_metaKnownSectionsRegistry.test.ts`. **Same-vector structural closure (scoped precisely):** the coverage/classifier/floor vector drew findings across Codex R1–R8. Per the AGENTS.md same-vector rule (a finding *after* a claimed closure means the closure was incomplete → deep-dive until structural), what is now closed **by construction** is the **detection/floor/oracle** contract: detection is **exhaustive** (every applicable site parsed, no cap — plan-R2), the floor is defined over **operator applicability** (`floorEligible(O)` + explicit applicability matrix, §4.3) via a uniform `domains(site)` helper (intra-section + boundary), cross-checked by an **implementation-independent applicability audit with per-domain COUNT equality** (plan-R3), present-but-inapplicable domains **surfaced**, full-signal oracle equality in both buckets, and a value-sensitive behavior fingerprint on the ledger. The floor assertion, `siteId`-uniqueness, `SILENT_SIGNAL_LOSS` verdict, and fingerprint diff are *guards* over a design that cannot silently under-cover. **What is explicitly NOT claimed closed:** the completeness of `KNOWN_SECTION_HEADERS` itself vs the parser's true recognized-header set — that is the pre-existing hand-maintained-registry limitation tracked by `BL-KNOWN-SECTIONS-WALKER` (§4.3 "Inherited limitation"), not something this test-only harness resolves. No advisory-lock (`pg_advisory*`) surface is touched → holder-topology declaration is **N/A**. No Supabase call boundary is added → `_metaInfraContract` registry is **N/A**. No new §12.4 code → catalog-parity gates **N/A**.
