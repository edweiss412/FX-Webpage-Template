...
- `useDevCapture(opts): { state, run, busyRef }` — the state machine (§7) and orchestration (capture → telemetry action → zip → download). `busyRef` (plan-review amendment #3, 2026-07-22): a synchronous lockout ref, true from `run()` entry to settle — host toggle handlers MUST gate on it rather than on the rendered busy state, which lags a React commit and leaves a same-tick window where a toggle click could reopen the share popover into frame. `opts` carries `{ target: () => HTMLElement | null, request: CaptureTelemetryRequest, clientSnapshot: () => unknown, filenameSeed: string, preCapture?: () => Promise<void> }` — `preCapture` runs INSIDE the busy window before rasterizing (amendment #2: ShareHub passes popover-close + two settle frames here).
...
- Plan-review amendment #3 (plan R5, 2026-07-22): §2.4 hook returns a synchronous `busyRef`; toggle lockout gates on the ref (rendered state lags a commit — same-tick race closed).
