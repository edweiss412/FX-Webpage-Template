...
### 6.1b The lock has its own executable proof, and a limit the repo imposes on it
...
Invariant 2 requires tests to assert the lock is HELD, so `copyShowLocked` and `deleteShowsLocked` take an injectable `SqlExecutor`; `tests/e2e/helpers/lockedShowCopy.unit.test.ts` drives the REAL functions, captures the SQL they emit, and compares it to the text it declares. That proves the transaction shape, the per-show key each caller chooses, and that `runLocked` forwards it, with no recognizer to escape: the comparison is against one string.

**Where the seam stops, and why that is the repo's decision rather than this arc's.** Three diff-review rounds pushed the boundary lower, each because a reviewer mutated the project-owned code beneath it: `SqlExecutor`, then a `Spawn` closure, then Node's own `execFileSync`. The last of those is precisely the shape `tests/cross-cutting/psqlStartupFileSuppression.test.ts` forbids — it requires psql to reach a literal `execFileSync` as a literal argv[0] so its scanner can read the flags and prove `-X` is present, and it reports zero indirections across the tree with a new one failing by default. Its concern is the same one that motivated wanting the seam lower.

So the seam stops at `SqlExecutor`, and `psqlExecutor` is covered by the live e2e run that drives it against a real database rather than by a unit test. **Documented limit, stated so nobody re-derives it:** no unit test here proves that `psqlExecutor` forwards its arguments unaltered, that Node's `execFileSync` behaves as documented, or what Postgres does with the text it receives. The first is eight lines with no branches under a guard that reads its flags; the other two are not ours.
...
| T1b | The SQL `copyShowLocked` and `deleteShowsLocked` actually emit, captured through an injected `SqlExecutor` and compared to the declared text (whitespace runs normalized) | Any change to the transaction at all, including the ten escapes two recognizer-shaped versions were walked through, and the three production joins a hand-composed assertion never touched: which key each caller picks, and whether `runLocked` forwards it. Below the seam is `psqlExecutor`, left to the live e2e run because the psql startup-file guard forbids reaching psql through an identifier (§6.1b). |
