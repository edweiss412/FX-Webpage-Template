...
|---|---|---|
| T1 | `openStep3Modal` returns with zero running animations on the panel subtree, and the panel is asserted to have had at least one animation | The settle is deleted, or a descendant gains an entrance animation the subtree walk misses. Fails before T2's barrier lands, passes after. |
