...
- `helpHref: "/help/errors#RESYNC_QUALITY_REGRESSED"` — the `/help/errors#` family, mirroring `RESYNC_SHRINK_HELD` (NOT `/help/admin/parse-warnings#`, which `_metaErrorCatalogDocs` restricts to `WARN_`/`PARSE_` codes).
...
15. **help/errors** — new `/help/errors#RESYNC_QUALITY_REGRESSED` anchor + `_families`/`_metaErrorCatalogDocs` check (`feedback_new_12_4_code_full_ci_touchpoints`): mirror `RESYNC_SHRINK_HELD`'s `/help/errors#` family. Parse-warnings anchors are `WARN_`/`PARSE_`-only.
