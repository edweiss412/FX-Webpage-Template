...
4. **Open alert exists (stored `baseline`):** compare `current` against the STORED `baseline`, not the immediate prior:
   - `isQualityRegression(baseline, current)` still true → **keep open**, but **only re-upsert if the regression payload materially changed** (see §6.4a — a 40→40 keep-open MUST be a true no-op, no `last_seen_at`/`occurrence_count` bump, no bell re-ping). When it did change (e.g. 40→80, a new class appeared, or a class deepened) re-upsert refreshing `breakdown`/`new_classes`/`worsened` but **preserve `baseline` unchanged** (do not let a further 40→80 step move the anchor). This single ping on genuinely-new information is intended.
...
- **`priorWarnings` NULL (untrustworthy baseline) → record-and-skip, do NOT alert (round-9 finding 2).** C is a *self-relative* re-sync regression detector; it is only meaningful against a trustworthy prior baseline. A NULL `prior.parse_warnings` means "unknown prior quality," NOT "known-clean prior." For an existing published show whose row predates the `parse_warnings` column (legacy) or otherwise lacks it, treating NULL as a zero baseline would falsely fire on the show's **intrinsic** warnings (the sheet always had them) and — worse — anchor `baseline` at all-zero, so the alert would never resolve until the sheet became *perfectly* clean rather than returning to its real prior quality. Therefore: **if `prior.parse_warnings` is NULL, C evaluation is skipped for this sync entirely** — no OPEN, no RESOLVE. The applied sync still writes `nextWarnings` (`applyParseResult.ts:206`), which becomes the trustworthy baseline for the *next* sync's comparison. This is a record-and-skip: the first sync bootstraps the baseline; the second sync onward can detect regression. Distinguish NULL (unknown → skip) from a present-but-empty `[]` array (known-clean prior → a real zero baseline, regression legitimately fires). **Required test (§6.7):** NULL prior + non-empty current warnings → no `RESYNC_QUALITY_REGRESSED` opened, and `nextWarnings` is persisted as the baseline for subsequent evaluation.
...
2. **NULL-baseline record-and-skip (finding 2, §6.5).** For a published show with `prior.parse_warnings = NULL` and a non-empty current parse, run an applied sync → assert NO `RESYNC_QUALITY_REGRESSED` row is opened AND `nextWarnings` is persisted (`applyParseResult.ts:206`). A second sync with the now-present prior + a genuine regression → assert C opens (baseline = the bootstrapped prior). This proves NULL≠zero-baseline and that intrinsic warnings don't false-fire.
