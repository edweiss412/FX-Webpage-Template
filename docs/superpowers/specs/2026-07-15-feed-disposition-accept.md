...
| `acknowledged_at` malformed/absent in row shape | treat as `null` (unacknowledged) — but `source`/`status` drive `acceptable`, so a malformed timestamp on an otherwise-acceptable row yields `acceptable: true`, matching the RPC (which checks the column IS NULL server-side; a stale render degrades to a 0-count accept, not an error) |
...
- Empty `showId` → typed `{ ok:false, code:"SYNC_INFRA_ERROR" }` refusal, `acknowledgeChanges` never called (mirrors `autoApplied.ts:51`).
...
- Mode boundaries: `acceptable` and `acknowledgedAt` are mutually exclusive by construction (predicate). Neither appears on gate rows (`action==='approve_reject'`), pending/rejected/undone/superseded rows, or non-`auto_apply` sources. Accept + Undo co-render on acceptable crew rows.
...
| Undo of an accepted row (B, crew kind still undoable) | allowed — undo flips status to `undone`; "Accepted" tag remains (historical fact). No animation. |
...
2. **Action units**: both new actions — empty-showId refusal (helper never called), delegation payload (showId + ids array), success → both `revalidatePath` targets, typed failure passthrough, `logAdminOutcome` on success branch only. Mirror existing `undoChangeAction` test shape.
