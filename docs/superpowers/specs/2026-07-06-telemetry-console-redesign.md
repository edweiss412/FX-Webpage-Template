...
- **Both SQL functions (DB) — `tests/db/telemetryConsoleReads.test.ts`, the deployment guard.** Integration test against `TEST_DATABASE_URL`.
  ...
  - `admin_alert_summary`: seed unresolved/resolved `admin_alerts` across health/degraded/notice codes (in-tx), assert `total` counts only unresolved health-code rows, `degraded` counts the degraded subset, `notice = total - degraded ≥ 0`, `degraded ≤ total` always.
  ...
  - **Runtime RPC smoke (Codex R4 F1) — the real PostgREST path, the deployment proof.** The direct-SQL tests above prove BEHAVIOR but bypass PostgREST; they can pass while `supabase.rpc(...)` still fails (schema cache not reloaded after apply, exposed named-param drift, service-role RPC resolution). So a separate smoke test uses the **real** `createSupabaseServiceRoleClient()` and calls `rpc("admin_event_stats_24h", { _now: <iso> })` AND `rpc("admin_alert_summary", { _health_codes: HEALTH_CODES, _degraded_codes: DEGRADED_HEALTH_CODES })` with the EXACT runtime param names the loaders use. Asserts `error === null` and the returned row has the expected keys with numeric/array types (SHAPE only, not exact counts — non-flaky against live data). This is the surface that proves the migration + `notify pgrst, 'reload schema'` actually reached the PostgREST the app talks to. It requires the service-role client env (`SUPABASE_URL` + `SUPABASE_SECRET_KEY`) to reach a PostgREST with the migration applied; the plan wires it into the db-touching CI job (which applies the migration and reloads the schema) and MUST run for real — a skipped/mocked smoke is not the proof (per "mocked-only tests invite tautological approve" + "local-passes-CI-fails is its own bug class"). If the loaders' param names ever drift from the SQL signatures, THIS test fails first.
...
- **Function deployment guard:** `validation-schema-parity` cannot see functions (R2-F1); functions produce no manifest delta. The §14 guard layers: existence (`to_regprocedure`) + `has_function_privilege('service_role',…)` TRUE (anon/authenticated/public FALSE) + deterministic behavior via **rollback-tx** direct SQL at a pinned historical window (R3-F1) + a **real `service_role` `.rpc()` PostgREST smoke** with exact runtime param names (R4-F1). The direct-SQL tests are behavioral proof; the RPC smoke is the runtime-path/deployment proof. Settled; do not re-flag the parity gate, "connecting role can execute," or "direct SQL is enough" as the proof.
