...
- **The DB coverage is split across TWO files to match the CI topology (Codex plan-R2 F1)** — the unit-suite job boots LOCAL supabase and has NO validation Supabase REST creds, so a fail-closed real-`rpc()` test cannot live in it:
  ...
  - `tests/db/telemetryConsoleReads.rpc.test.ts` — the **real `createSupabaseServiceRoleClient().rpc()` smoke + same-project guard + fail-closed**. **EXCLUDED from unit-suite** via `ENV_BOUND_EXCLUDES` (`vitest.projects.ts`), and run in a **dedicated x-audits validation-scoped job** with `TEST_DATABASE_URL` = `secrets.SUPABASE_TEST_DATABASE_URL`, `SUPABASE_URL` = `secrets.SUPABASE_URL`, `SUPABASE_SECRET_KEY` = `secrets.SUPABASE_SECRET_KEY` (all the validation project). This job is the CI-ENFORCED validation-deployment proof: a function missing from validation, a stale schema cache, or param drift fails it. (Plan Task 1B wires the exclude + the x-audits job + updates the shard/exclude topology meta-tests.)
...
- **DB-test / deployment-proof contract (SETTLED after R2-R5 on this vector — comprehensive closure, do not relitigate per-instance).** The full contract for the two functions:
  ...
  5. Runtime path / deployment proof: a real `createSupabaseServiceRoleClient().rpc(...)` smoke with exact param names, SHAPE-only asserts, **same-project guard** (TEST_DATABASE_URL ref == SUPABASE_URL ref; loopback branch for local) + **fail-closed** on absent/mismatched env (R4-F1, R5-F1). It lives in `tests/db/telemetryConsoleReads.rpc.test.ts`, is **EXCLUDED from unit-suite** (`ENV_BOUND_EXCLUDES`, since unit-suite has no validation REST creds), and runs in a **dedicated x-audits validation-scoped job** with the validation `SUPABASE_TEST_DATABASE_URL`/`SUPABASE_URL`/`SUPABASE_SECRET_KEY` secrets (Codex plan-R2 F1). The behavioral/existence/privilege file (`telemetryConsoleReads.test.ts`) runs in unit-suite against local. Plan Task 1B wires the exclude + job + topology meta-tests.
