...
- **Supabase call-boundary (invariant 9) — the structural guard is `tests/admin/_metaInfraContract.test.ts`, NOT the auth one.** `loadAlertSummary` + `loadTelemetryStats` are new Supabase readers under `lib/admin/`; the admin-domain call-boundary meta-test already carries an `infraRegistry` where the precedent readers `loadAppEvents` (`:328`), `loadCronHealth`, `fetchHealthRollup` (`:249`), `loadHealthAlerts` (`:255`), `loadBellFeed` (`:272`) are registered. **Both new loaders MUST be added as registry rows** (the invariant-9 "analogous registry-style meta-test for new surfaces" requirement; a bare behavioral test is NOT sufficient — a NEW admin Supabase reader must fail-by-registry until listed). Coverage shape per helper:
  - `loadAlertSummary` — reads `admin_alerts` via `count:"exact", head:true` probes (same shape as `fetchHealthRollup`). Registry row is fully harness-drivable: the shared throwing client's `throwOnFrom` / `throwOnConstruct` exercise the from-throw + construction-throw paths → assert `{ kind:"infra_error" }`. Grep-shape rule (`{ data/count, error }` destructure) applies (no `skipGrepShape`).
  - `loadTelemetryStats` — calls `supabase.rpc("admin_event_stats_24h", …)`. The shared mock's `rpc()` is NOT fn-keyed (it returns `{data:null,error:null}` and cannot be made to throw from the registry harness — same limitation noted for `loadBellFeed` at `:263-271`). So the registry row pins the **construction-throw** path here, and the **rpc-returned-`{error}`** + **rpc-throw** + **malformed/missing-row** paths are covered behaviorally in a dedicated `tests/admin/loadTelemetryStats.test.ts` (the `loadBellFeed` precedent). Because the helper uses `supabase.rpc(...)` rather than the `.from().select()` builder, set `skipGrepShape: true` on the row (like the named-client rows at `:315`/`:325`) and pin the destructure discipline (`const { data, error } = await supabase.rpc(...)`) in the behavioral test instead.
  Both helpers still get their happy-path + coercion unit tests. §12 invariant-9 row and §5.2/§4 error-path claims reference this same registry.
