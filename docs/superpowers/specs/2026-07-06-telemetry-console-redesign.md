...
- **Strict row validation before returning ok (Codex plan-R2 F2) — mirror `loadTelemetryStats`.** Coerce `total`/`degraded` via `Number(...)`; require both to be **finite non-negative integers** AND `degraded <= total` (the SQL guarantees this from one scan, but a drifted shape must not produce a negative `notice`). Any failure → `infra_error`. Tests cover: missing field, non-numeric, `NaN`/`Infinity`, negative, and `degraded > total`.
...
> **AMENDMENT (2026-07-06, real-CI): REST rpc() smoke dropped — infeasible with available secrets.** The real-`rpc()` deployment smoke below (and its references throughout §13/§14/§15) assumed `secrets.SUPABASE_URL` + `secrets.SUPABASE_SECRET_KEY` were the VALIDATION project's REST creds. On the first real Actions run they proved to be the LOCAL supabase pair (loopback URL), so the same-project guard tripped and the REST path cannot reach validation — no validation-issued apikey exists as a repo secret (only `SUPABASE_TEST_DATABASE_URL` reaches validation, via psql). The CI-enforced validation-deployment proof is therefore the **`tests/db/telemetryConsoleReads.test.ts`** (postgres.js existence + `has_function_privilege('service_role', …)` + rollback-tx behavioral) re-run against `TEST_DATABASE_URL`=validation in the `telemetry-rpc-smoke` x-audits job. `to_regprocedure` pins the function signature (param types + arity). The REST-only coverage (PostgREST schema-cache staleness / named-param drift) is not exercised in CI; `tests/db/telemetryConsoleReads.rpc.test.ts` was deleted. This is the "local-passes-CI-fails is its own bug class" gate working as intended.

- **The DB coverage is split across TWO files to match the CI topology (Codex plan-R2 F1)** — the unit-suite job boots LOCAL supabase and has NO validation Supabase REST creds, so a fail-closed real-`rpc()` test cannot live in it:
  - `tests/db/telemetryConsoleReads.test.ts` — **direct-SQL behavioral + existence + privilege**. Pure `postgres.js` (`TEST_DATABASE_URL ?? local`); needs no REST creds. Runs in **unit-suite against LOCAL** (the local bootstrap applies the migration) as an always-on regression guard, and also correct against validation.
  - `tests/db/telemetryConsoleReads.rpc.test.ts` — the **real `createSupabaseServiceRoleClient().rpc()` smoke + same-project guard + fail-closed**. **Gated behind `describe.skipIf(!process.env.RUN_VALIDATION_RPC_SMOKE)`** — the var is set ONLY by a **dedicated x-audits validation-scoped job** (with `TEST_DATABASE_URL`=`secrets.SUPABASE_TEST_DATABASE_URL`, `SUPABASE_URL`=`secrets.SUPABASE_URL`, `SUPABASE_SECRET_KEY`=`secrets.SUPABASE_SECRET_KEY`). It SKIPS under local `pnpm test` and unit-suite (var unset), so it never breaks the full-suite gate; when it DOES run (x-audits) it is fail-closed on missing/mismatched env. This job is the CI-ENFORCED validation-deployment proof: a function missing from validation, a stale schema cache, or param drift fails it. (Plan Task 1B; no `ENV_BOUND_EXCLUDES`/topology change needed — the gate var handles all non-x-audits runs.)
- **`tests/db/telemetryConsoleReads.test.ts` cases (unit-suite, local).** Integration test against `TEST_DATABASE_URL ?? local`.
...
- **`tests/db/telemetryConsoleReads.rpc.test.ts` — Runtime RPC smoke (Codex R4 F1 / R5 F1), the CI-enforced validation-deployment proof.** The direct-SQL tests above prove BEHAVIOR but bypass PostgREST; they can pass while `supabase.rpc(...)` still fails (schema cache not reloaded after apply, exposed named-param drift, service-role RPC resolution). So this file uses the **real** `createSupabaseServiceRoleClient()` and calls `rpc("admin_event_stats_24h", { _now: <iso> })` AND `rpc("admin_alert_summary", { _health_codes: HEALTH_CODES, _degraded_codes: DEGRADED_HEALTH_CODES })` with the EXACT runtime param names the loaders use, asserting `error === null` and the row has the expected keys/types (SHAPE only, not exact counts — non-flaky against live data).
...
- **DB-test / deployment-proof contract (SETTLED after R2-R5 on this vector — comprehensive closure, do not relitigate per-instance).** The full contract for the two functions:
  ...
  5. Runtime path / deployment proof: a real `createSupabaseServiceRoleClient().rpc(...)` smoke with exact param names, SHAPE-only asserts, **same-project guard** (TEST_DATABASE_URL ref == SUPABASE_URL ref; loopback branch for local) + **fail-closed** on absent/mismatched env (R4-F1, R5-F1). It lives in `tests/db/telemetryConsoleReads.rpc.test.ts`, **gated by `RUN_VALIDATION_RPC_SMOKE`** (set only by the x-audits job) so it SKIPS under local `pnpm test` + unit-suite and never breaks the full-suite gate (Codex plan-R3), and runs fail-closed in a **dedicated x-audits validation-scoped job** with the validation `SUPABASE_TEST_DATABASE_URL`/`SUPABASE_URL`/`SUPABASE_SECRET_KEY` secrets (Codex plan-R2 F1). The behavioral/existence/privilege file (`telemetryConsoleReads.test.ts`) runs in unit-suite against local. Plan Task 1B wires the job (no `ENV_BOUND_EXCLUDES`/topology change).
  Do not re-flag the parity gate, "connecting role can execute," "direct SQL is enough," an un-pinned window, an un-guarded smoke target, or "the smoke isn't CI-wired" — each is addressed above.
