...
- **Function deployment guard (Codex R2 F1) — `validation-schema-parity` is insufficient for function-only migrations.** The manifest/parity gate sees base-table columns only. The real deployment proof is `tests/db/telemetryConsoleReads.test.ts` (§14): it RPC-calls `admin_event_stats_24h` AND `admin_alert_summary` against `TEST_DATABASE_URL`, asserting each function exists, is executable by the connecting role, and returns the expected shape — FAILING if the migration never reached that DB. Runs in the db-touching CI job.
...
- **Both SQL functions (DB) — `tests/db/telemetryConsoleReads.test.ts`, the deployment guard:** integration test against `TEST_DATABASE_URL`. `admin_event_stats_24h`: seed app_events across hour boundaries + levels, assert `total`, per-level counts, `buckets.length===24`, `sum(buckets)===total`, current-hour bucket is index 23, out-of-window rows excluded, empty window → all-zero buckets + total 0. `admin_alert_summary`: seed unresolved/resolved `admin_alerts` across health/degraded/notice codes, assert `total` counts only unresolved health-code rows, `degraded` counts the degraded subset, `total - degraded` = notice ≥ 0, and a concurrent-style check (degraded ≤ total always). Both: assert the function EXISTS + is executable (a missing function fails the test → forces the validation apply). Derive expectations from seeded fixtures (anti-tautology; never hardcode).
...
- **Function deployment guard:** `validation-schema-parity` cannot see functions (R2-F1). The guard is the DB RPC-existence test (§14). Settled; do not re-flag the parity gate as the function's proof.
