...
- **Function deployment guard (Codex R2 F1 / R3 F2) — `validation-schema-parity` is insufficient for function-only migrations, and the guard must check the `service_role` grant explicitly.** The manifest/parity gate sees base-table columns only; **functions produce NO manifest delta** (no "must land in manifest" requirement — that would be impossible). The real deployment proof is `tests/db/telemetryConsoleReads.test.ts` (§14) against `TEST_DATABASE_URL`, which asserts for BOTH functions: (a) the function EXISTS (`to_regprocedure('public.admin_event_stats_24h(timestamptz)')` / `('public.admin_alert_summary(text[],text[])')` is not null); (b) **`has_function_privilege('service_role', '<signature>', 'EXECUTE')` is TRUE** (the app calls via `service_role`; a plain "connecting role can execute" check would pass as owner even with the grant missing — R3 F2); (c) `has_function_privilege('anon'|'authenticated'|'public', …, 'EXECUTE')` is FALSE (revoke honored); (d) the behavioral shape (§14). Missing function or missing grant → test FAILS → forces the validation apply. Runs in the db-touching CI job (has `TEST_DATABASE_URL`).
...
- **Function deployment guard:** `validation-schema-parity` cannot see functions (R2-F1); functions produce no manifest delta. The guard is the §14 DB test asserting existence + `has_function_privilege('service_role',…)` TRUE (anon/authenticated/public FALSE) against `TEST_DATABASE_URL`. Behavioral cases run in a **rollback transaction** at a pinned historical window (R3-F1) — no persistent writes, deterministic on shared validation data. Settled; do not re-flag the parity gate or "connecting role can execute" as the proof.
