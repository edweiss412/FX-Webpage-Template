...
**Ordering (preserved from today):** groups appear in the order their codes first appear in the existing active list. That list is `displayWarnings = [...dataQuality.digest, ...actionableItems]` (`page.tsx:432`) — the **digest** portion first, then the operator-actionable portion. The digest is NOT two fixed codes: it is every warning passing `isDataQualityWarning(w) && !OPERATOR_ACTIONABLE_ANCHORED.has(w.code)` (`page.tsx:391-392`), which spans the full data-gap taxonomy (`GAP_CLASSES`, ~30 codes, `lib/parser/dataGaps.ts:30`; `UNKNOWN_SECTION_HEADER` / `BLOCK_DISAPPEARED` are only two examples). So the active list may carry many distinct codes, each becoming its own eyebrow group. `groupActiveByCode` groups over WHATEVER codes are present — it must not special-case any code set. Within a group, warnings keep their existing stable order, and groups appear in first-code-occurrence order. Note: grouping is not a no-op on item order — an interleaved later card of an already-seen code is pulled UP into its group (ahead of intervening other-code cards). That collapse is intended (it is the whole point of grouping); "no re-sort" means no comparator/sort key is introduced and both intra-group and first-appearance order are stable, NOT that every card keeps its exact flat index.
...
| `BLOCK_DISAPPEARED` (no fingerprint → never ignorable) | Its own eyebrow group, labeled by its `DATA_GAP_CLASS_LABELS` value — `"removed section"` (`lib/parser/dataGaps.ts:33`), NEVER hardcoded copy; cards are Report-only; never bulk-eligible. The label comes from the same `bulkGroupLabel(code)` source as every other group. |
...
| All active warnings share one code | A single eyebrow group; it carries the "Ignore all N" chip ONLY if that code is bulk-eligible (≥2 distinct contents, `bulkIgnoreGroups.ts:7`) — a lone singleton or a non-ignorable code gets the eyebrow with no chip. Degrades to today's single-dominant-code case, now with an explicit type header. |
...
**New tests:**
- `groupActiveByCode` unit test: first-appearance order preserved (mixed input of ≥3 distinct codes drawn from BOTH the digest population and the operator-actionable set — NOT only the two historical digest examples); singletons get their own group; distinct codes never merged; interleaved same-code warnings collapse into one group preserving intra-group order; empty input → empty. The fixture deliberately includes a digest code beyond `UNKNOWN_SECTION_HEADER`/`BLOCK_DISAPPEARED` (e.g. `SECTION_HEADER_NO_FIELDS` or `VENUE_GEOCODE_UNRESOLVED`) to prove the helper is code-set-agnostic (Codex spec R1 finding 2).
- Grouped-render test (pins DQIGNORE-6): with a multi-code active fixture (≥1 bulk-eligible code, ≥1 singleton, and the non-ignorable `BLOCK_DISAPPEARED`), every active code renders an eyebrow with its plain-language label (never the raw code); a bulk-eligible code's eyebrow carries the "Ignore all N" chip and its N matches the group's distinct-content count; a singleton/non-ignorable code's eyebrow has NO chip; the partial-failure notice renders inside the acting group (below its cards), not at panel top. Anti-tautology: assert the chip count against the fixture's distinct-content count (derived from fixture, not hardcoded), scope the "raw code absent" assertion to the eyebrow subtree, and assert the `BLOCK_DISAPPEARED` eyebrow reads its `DATA_GAP_CLASS_LABELS` value (`"removed section"`) rather than any hardcoded string.
