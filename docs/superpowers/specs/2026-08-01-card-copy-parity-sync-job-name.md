...
- Canonical table: `docs/superpowers/specs/2026-07-20-warning-card-copy-restore.md` §4.2 (44 rows; row cells: `| n | CODE [— title: …] | helpfulContext | triggerContext |`). Four rows carry a `— title:` suffix in the code cell today (rows 10, 21, 26, 36) while the shipped title-freeze fixture `EXPECTED_TITLE_CHANGES` holds SIX codes — rows 29 (`AGENDA_PDF_UNREADABLE`) and 40 (`AGENDA_FILE_INACCESSIBLE`) were retitled by the `ada490e58` split without gaining doc markers. This branch adds both markers (§2.3-1a), making the suffixed set equal the fixture's six.
...
1. **§4.2 rows 12 and 29** in `2026-07-20-warning-card-copy-restore.md`: replace both copy cells with the shipped catalog strings (table above).
   1a. **Title-marker consistency (full class):** rows 29 AND 40 gain the retitle markers already shipped in `EXPECTED_TITLE_CHANGES` (`— title: **"No agenda schedule found"**` and `— title: **"Can't open the agenda file"**`, both from the `ada490e58` split) — documentation only, titles are asserted from `EXPECTED_TITLE_CHANGES`, not the doc. The suffixed-row set then equals the fixture's six codes. Stale count statements updated with dated UPDATE notes in the same doc: the line-~55 pseudocode comment "// rows 10, 21, 26, 36", the line-~105 "four changed titles", and the line-~166 "except rows 10, 21, 26 … and row 36" sentence (each becomes the six-code list; line ~210's "the four title changes" is a historical task narrative of the original 2026-07-20 sweep and accurately describes that commit — unchanged). The meta-test header's "four changed titles" (`tests/messages/_metaWarningCardCopy.test.ts:8`) is updated in the Part-A test commit.
...
4. **`tests/messages/_metaWarningCardCopy.test.ts`**: (a) update the header + inline comments that describe partial coverage / the HOTEL_GUEST_SPLIT_AMBIGUOUS divergence / the backlog ref; (b) in the doc-reading test, change the row-code extraction from `cells[2]?.trim() === code` to first-token match (`cells[2]?.trim().split(/\s/)[0] === code`) so the six `— title:` suffixed rows (post-1a) resolve; (c) add an assertion that `EXPECTED_HELPFUL_CONTEXT`'s key set equals `WARNING_CARD_COPY_CODES` — the freeze can no longer silently lose coverage when a code is added to the registry without a frozen row.
5. **Backlog**: graduate `BL-CARD-COPY-HELPFULCONTEXT-PARITY` per the full graduation convention: (a) move the entry to `BACKLOG-archive.md` with an archived section that names the resolving branch, (b) prepend the "Last reconciled" note in `BACKLOG.md`, (c) add a `{ id: "BL-CARD-COPY-HELPFULCONTEXT-PARITY", provenance: "feat/card-copy-parity-sync-job-names" }` row to `BACKLOG_GRADUATED` in `tests/docs/_metaDeferralLedgerGraduation.test.ts` — the guard proves archive presence + provenance only for registered ids; deleting from `BACKLOG.md` alone leaves it vacuously green. (Part B does the same for `BL-SYNC-JOB-FOUR-NAMES`.)
...
| 9 | `docs/alerts/admin-alert-system-explainer.html` — the maintained rendered catalog mirror (updated alongside catalog changes since `eb19566b7`). Four stale instances across two mirrored rows: lines 927 + 936 (WATCH_CHANNEL_ORPHANED: `data-text` search mirror + visible copy, "the scheduled sync") and lines 1051 + 1059 (SYNC_STALLED: `data-text` "automatic syncing hasn't run…" + visible "Automatic syncing hasn't run…"). | as at left | mirror the new catalog strings ("Auto sync"; the lowercase `data-text` mirror keeps its case convention: "auto sync") |

Unchanged (referent named correctly already): `components/admin/StagedReviewCard.tsx:90` badge `"Auto sync"`; the `tests/help/_uiLabelExceptions.ts:191` comment listing the badge set. Historical specs/plans/handoffs are records of their own time and are not rewritten. Out-of-scope verb phrases and internals: §1.1.
...
2. §4.2 rows 12 + 29 byte-match the shipped catalog strings; the doc-reading test resolves all 44 rows including the six `— title:` suffixed ones (rows 10, 21, 26, 29, 36, 40 after §2.3-1a).
3. Zero user-visible occurrences of "Sheet sync", "the scheduled sync", "an automatic sync", "the automatic sync", "automatic sync", or "Automatic syncing" as the job's **name** in `lib/messages/catalog.ts`, `lib/cron/runSummary.ts`, `components/`, `app/`, or `docs/alerts/admin-alert-system-explainer.html` (verified by the §3.2 sweep re-run at close-out; out-of-scope verb phrases per §1.1 remain).
4. `pnpm test:audit:x1-catalog-parity` green (regen included in the Part-B commit).
5. Both backlog entries graduated: entries moved to `BACKLOG-archive.md` with the resolving branch named, `BACKLOG_GRADUATED` registry rows added (§2.3-5); graduation meta-test green.
