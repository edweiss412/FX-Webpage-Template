...
2. **In `runScheduledCronSync`**: declare `let inFlightPhase: "resolve-folder" | "list-folder" | "list-live-shows" | "missing-shows" | "file-loop" | "finish" = "resolve-folder"` and `let inFlightDriveFileId: string | null = null` and `let resolvedFolderId: string | null = null` alongside `processed` (`2781`… note `processed` is declared *after* folder resolve, so declare the trackers at function top, right after `finishCompletedRun` at `2746`). Update `inFlightPhase` at each phase boundary; set `resolvedFolderId = folderId` at `2776`; set `inFlightDriveFileId = show.driveFileId` at the top of the missing-shows loop (`2793`) and `inFlightDriveFileId = file.driveFileId` at the top of the file loop (`2826`), clearing to `null` after each iteration completes. Wrap the body from folder-resolve (`2748`) through `return finishCompletedRun(...)` (`2861`) in `try { … } catch (err) { attach; throw err; }` where attach = `if (err && typeof err === "object") (err as { syncRunContext?: unknown }).syncRunContext = { phase: inFlightPhase, folderId: resolvedFolderId, inFlightDriveFileId, processedBeforeThrow: processed.length, failures: buildSyncFailureBreadcrumbs(processed) };`. **Rethrow unchanged** — HTTP/error semantics preserved (like `withCronRunSummary.ts:37`). Do NOT swallow: a lock/DB/heartbeat fault is genuinely fatal and must still propagate.
...
### Proposed change
- **Annotate, do not suppress or downgrade.** When `summarizeSync` returns `outcome: "partial"`, compute a stable **fingerprint** = the sorted set of `` `${driveFileId}|${code ?? outcome}` `` over `detail.failures` (post-cap; if `failuresTruncated`, append a `"+truncated"` sentinel to the fingerprint so a growing tail counts as changed). Compare against the **prior** `cron.sync CRON_RUN_SUMMARY` row's stored fingerprint. Attach `stateChanged: boolean` and `unchangedSinceRuns?: number` (increment from the prior row when unchanged, else omit) to `detail`.
...
- The prior-row read MUST follow invariant 9 (`loadCronHealth.ts` is the template): destructure `{ data, error }`; a returned `error` OR a thrown fault → **fail-open**: return the summary **unchanged** with `stateChanged: true` (conservative: treat unknown as changed so we never silently suppress a real change) and skip the fingerprint write. A logging/annotation fault must NEVER break the cron (mirrors `withCronRunSummary.ts:54-56`, "observability must never break the cron"). Wrap the whole annotate in try/catch → on any throw, return the input summary untouched.
...
- **S3:** (a) two consecutive runs with the SAME failing show → 2nd row has `stateChanged:false`, `unchangedSinceRuns:1`; (b) a run where the failing show CHANGES (different driveFileId or code) → `stateChanged:true`; (c) prior-row read returns `{error}` → summary returned unchanged with `stateChanged:true`, cron does not throw; (d) prior-row read **throws** → same fail-open. Catches: silent suppression of a real change; annotation breaking the cron.
