...
### Proposed change
1. **Extract the failures reducer** from `summarizeSync.ts:26-45` into a shared exported helper `buildSyncFailureBreadcrumbs(processed): Array<{driveFileId; outcome; code?}>` (new file `lib/cron/syncFailureBreadcrumbs.ts` or exported from `summarizeSync.ts`), preserving `MAX_FAILURE_BREADCRUMBS=25`, the `CONCURRENT_SYNC_SKIPPED` skip, and the conservative "unknown ⇒ failed" rule. `summarizeSync` calls it so the partial and throw paths **cannot drift** (project treats breadcrumb-shape drift as a finding class).
2. **In `runScheduledCronSync`** — **HOIST all state above the wrapped body.** Declare, at function top right after the `finishCompletedRun` definition (`2746`), BEFORE the first throwable phase: `let inFlightPhase: "resolve-folder" | "list-folder" | "list-live-shows" | "missing-shows" | "file-loop" | "finish" = "resolve-folder"`, `let inFlightDriveFileId: string | null = null`, `let resolvedFolderId: string | null = null`, **and move `const processed: RunScheduledCronSyncResult["processed"] = []` up here too** — it is currently declared at `2781`, *after* `listFolder` at `2780`, so a folder-resolve or list-folder throw would hit `processed.length` in the catch before `processed` exists (ReferenceError / TDZ). Hoisting is mandatory; the existing `2781` declaration is deleted. Update `inFlightPhase` at each phase boundary; set `resolvedFolderId = folderId` after folder resolve (`2776`).
   - **Per-iteration clearing via `finally` (required).** The missing-shows loop's skip branch uses `continue` (`2797-2808`), so clearing at the loop-body tail would leave a stale id. Wrap each missing-shows iteration body in `try { inFlightDriveFileId = show.driveFileId; … } finally { inFlightDriveFileId = null }`. Do the same for the file loop (`2826`): set `inFlightDriveFileId = file.driveFileId` and clear in a `finally` (the file loop already has an inner `try/catch` at `2827/2843`; add a `finally { inFlightDriveFileId = null }` to that same try). This guarantees the id reflects the *currently* in-flight file only, never a stale prior one, on every exit path (`continue`, caught-and-pushed, or re-throw).
   - **Wrap** the body from folder-resolve (`2748`) through `return finishCompletedRun(...)` (`2861`) in an OUTER `try { … } catch (err) { attach; throw err; }` where attach = `if (err && typeof err === "object") (err as { syncRunContext?: unknown }).syncRunContext = { phase: inFlightPhase, folderId: resolvedFolderId, inFlightDriveFileId, processedBeforeThrow: processed.length, failures: buildSyncFailureBreadcrumbs(processed) };`. **Rethrow unchanged** — HTTP/error semantics preserved (like `withCronRunSummary.ts:37`). Do NOT swallow and do NOT log here (the wrapper catch is the sole emitter — avoids double-log): a lock/DB/heartbeat fault is genuinely fatal and must still propagate.
...
### Proposed change
- **Annotate, do not suppress or downgrade.** When `summarizeSync` returns `outcome: "partial"`, compute a stable **fingerprint** = `` `${counts.failed}#` `` + the **sorted-then-joined** set of `` `${driveFileId}|${code ?? outcome}` `` over `detail.failures`. Prefixing `counts.failed` (the EXACT total, correct even when `failures[]` is capped at 25 — `summarizeSync.ts:46` sets `counts.failed`, `:57` sets `failuresTruncated`) ensures 26→27 failures register as **changed** even though the capped breadcrumb list is byte-identical; the sort makes it order-independent. For a heartbeat-only partial (no show failures, `detail.maintenanceFaults` present, `counts.failed===0`), the fingerprint is `` `0#heartbeat` `` so it is distinct from a show-failure partial. Compare against the **prior** `cron.sync CRON_RUN_SUMMARY` row's stored fingerprint. Attach `stateChanged: boolean` and `unchangedSinceRuns?: number` (increment from the prior row when unchanged, else omit) to `detail`.
- **Where:** the fingerprint compare needs one indexed DB read, so it happens in the **route** (which already does I/O), not in pure `summarizeSync`. Add `lib/cron/annotateSyncStateChange.ts` exporting `async function annotateSyncStateChange(summary: CronRunSummary): Promise<CronRunSummary>`; the route becomes `summary: await annotateSyncStateChange(summarizeSync(result))`. It: reads the latest `cron.sync` `CRON_RUN_SUMMARY` row's `context->detail->failuresFingerprint` (+ `unchangedSinceRuns`) via a `createSupabaseServiceRoleClient()` query mirroring `loadCronHealth.ts:38-47`; writes `detail.failuresFingerprint` (the current fingerprint string) so the next run can compare; sets `stateChanged`.
- Only annotates `outcome: "partial"`. `ok`/`infra` summaries pass through unchanged (no read).

### Flag lifecycle (`stateChanged` / `failuresFingerprint` / `unchangedSinceRuns`)
| field | storage | write path | read path | effect on output |
|---|---|---|---|---|
| `failuresFingerprint` | `app_events.context->detail` (jsonb) | `annotateSyncStateChange` (this run) | `annotateSyncStateChange` (next run's compare) | none directly; enables the compare |
...
- **S1:** (a) a throw injected in the missing-shows loop yields an error row whose `drive_file_id` **column** equals the in-flight show AND `detail.failures` equals `buildSyncFailureBreadcrumbs(processed)` over the successes-so-far; (b) a throw in folder-resolve yields `detail.phase: "resolve-folder"`, no `driveFileId`, no crash; (c) a **non-object** throw (`throw "boom"`) still logs `outcome:"threw"` with no `syncRunContext` read error; (d) the shared reducer produces byte-identical output for the partial and throw paths on the same `processed` (anti-drift). Catches: the exact zero-attribution incident.
- **S3:** (a) two consecutive runs with the SAME failing show → 2nd row has `stateChanged:false`, `unchangedSinceRuns:1`; (b) a run where the failing show CHANGES (different driveFileId or code) → `stateChanged:true`; (c) prior-row read returns `{error}` → returns the canonical fail-open shape (`detail.stateChanged:true`, no `failuresFingerprint`), cron does not throw; (d) prior-row read **throws** → identical canonical fail-open shape; (e) **26→27 failures** (capped `failures[]` byte-identical, only `counts.failed` differs) → `stateChanged:true` via the `counts.failed` fingerprint prefix. Catches: silent suppression of a real change (incl. beyond the breadcrumb cap); annotation breaking the cron.
