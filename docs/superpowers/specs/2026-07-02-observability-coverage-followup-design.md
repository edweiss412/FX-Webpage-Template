...
### Proposed change
- **Annotate, do not suppress or downgrade.** The **fingerprint is computed inside `summarizeSync`** — which already iterates the FULL uncapped `result.processed` (`summarizeSync.ts:27`) — NOT in `annotate` from the capped `failures[]`. Computing from the capped 25-row list would miss a composition change *beyond the cap* whenever `counts.failed` is stable (e.g. 30 failures, #27 flips A→B). On the `partial` path, `summarizeSync` sets `detail.failuresFingerprint` = the **sorted-then-joined** list of `` `${driveFileId}|${code ?? outcome}` `` over EVERY failed `processed` entry (uncapped — same FAILED/unknown classification it already uses at `summarizeSync.ts:34-44`, just without the `MAX_FAILURE_BREADCRUMBS` cutoff). This is sensitive to both count and composition at any depth. For a heartbeat-only partial (no show failures, `detail.maintenanceFaults` present, `failed===0`), the fingerprint is the sentinel `"heartbeat"`, distinct from any show-failure fingerprint.
