...
**`inferShowYear` (`_helpers.ts:126`) must widen too (Codex spec R3 finding 2).** It back-fills yearless hotel/transport dates by scanning for "the first `M/D/YY(YY)` date anywhere in the sheet." A sheet whose dates are ALL ISO or long-form (no slash date) would otherwise infer NO show year, breaking yearless date back-fill. Widen its scan to also recognize an ISO (`YYYY-MM-DD`) or long-form (`Month D, YYYY` / `D Month YYYY`) date and extract the 4-digit year from the first such hit — same conservative, self-delimiting patterns as the `extractAllDates` widening (no bare dash in the free scan). `scheduleBookends.ts:39` is a banner regex unaffected by these formats (out of scope, unchanged).
...
### Widened grammar (accept, in addition to today)
Replace the fixed `\d+\s*'\s*x` core with shared, exported FRAGMENTS (`DIMS_SEP`, `DIMS_OPERAND_UNIT`, `DIMS_OPERAND_BARE` — see companion-surface invariant 1 for exact definitions) that each site composes into the shape IT needs (partial-START for `roomHeaderNameShape`/`dimStart`; full 2-or-3-operand capture for `harvestSameNameHeaderDims`; `DIMS_SEP` also drives the dangling-separator cleanup). The accepted forms:
...
- `lib/parser/blocks/_helpers.ts` — `normalizeDate` widened (ISO + long-form + cell dash); `inferShowYear` free-scan widened to extract the year from a first ISO/long-form date too.
...
- `lib/parser/blocks/rooms.ts` — shared dims-token FRAGMENTS (operand + separator); `roomHeaderNameShape` + `dimStart` build a partial-START matcher from them, `harvestSameNameHeaderDims` builds a FULL 2-or-3-operand capture from the SAME fragments; dangling-separator cleanup widened to `[x×]`.

## Companion-surface invariants (must stay in lockstep — enumerate in plan)
1. **Dims token single-source via shared FRAGMENTS (Codex spec R2 finding 3, R3 finding 1):** the three sites have DIFFERENT shapes by necessity — `roomHeaderNameShape:134` and `splitRoomHeader dimStart:1486` need a partial START matcher (enough to locate where dims begin and slice the name off), while `harvestSameNameHeaderDims:1270` needs a FULL capture (the whole 2-or-3-operand token). They therefore CANNOT share one whole-token regex; instead they compose three exported sub-fragments:
   - `DIMS_SEP` = `[x×]` (the separator).
   ...
   - `DIMS_OPERAND_BARE` = `\d{2,3}` (with `\b` boundaries at the token edges) — a **unit-less** operand, gated to 2–3 digits so `2026 x 40` / `5 x 8` are NOT admitted.
   The dims token matches EITHER a unit-bearing chain (`DIMS_OPERAND_UNIT DIMS_SEP …`, any digit count, mixing bare/unit follow-operands allowed) OR a fully-bare chain (`\bDIMS_OPERAND_BARE DIMS_SEP DIMS_OPERAND_BARE\b`). The optional-unit single-operand framing is explicitly REJECTED (it would let a bare 4-digit through). A test asserts all three sites reference the shared fragments (no re-inlined `'\s*x` literal), the dangling-separator cleanup uses `DIMS_SEP`, and that bare `2026 x 40` does NOT parse while `2026' x 40'` does — so widening a fragment updates every site at once without opening the 4-digit-bare hole.
...
- **Dims:** `splitRoomHeader` + `roomHeaderNameShape` + `harvestSameNameHeaderDims` over each new unit/symbol variant (`50′×45′`, `50ft x 40ft`, `8' x 10'`) AND the bare-`50 x 40`/`120x80` (admit) vs bare-`5 x 8`/`2026 x 40`/`Box40x2` (reject) gate; a dangling `75′ × 37′ ×` → trailing `×` stripped; the shared-fragments invariant test (invariant 1).
