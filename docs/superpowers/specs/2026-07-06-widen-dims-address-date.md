...
- **Exporter coupling (companion surface):** `shouldPreserveNewlines` (`lib/drive/exportSheetToMarkdown.ts:59`) owns a SEPARATE inline copy at `:80`: `if (lines[1] && /^[A-Z][A-Za-z .'-]+,\s*[A-Z]{2}\s+\d{5}/.test(lines[1])) return false;` — a 2-line cell whose 2nd line looks like `City, ST 12345` is treated as an address and its newline PRESERVED (`&#10;`). This regex is decoupled from hotels.ts and must be widened in lockstep for the same postal shapes, else a Canadian hotel's 2-line address cell flattens differently.

### Widened grammar (accept, in addition to today)
1. **Extend the street-suffix vocabulary** in `STREET_ADDRESS_RE` with common Canadian / broader-North-American suffixes (additive to the closed set): `Crescent|Cres|Commons|Close|Gate|Grove|Mews|Quay|Wharf|Gardens|Gdns|Green|Bay|Landing|Crossing|Esplanade|Promenade|Concourse|Alley|Bend|Common`.
2. **Canadian postal tail** in BOTH `STREET_ADDRESS_ZIP_RE` (hotels.ts:264) AND the exporter copy (exportSheetToMarkdown.ts:80): accept `, <PROV> <A1A 1A1>` where the postal code is `[A-Za-z]\d[A-Za-z]\s?\d[A-Za-z]\d` in addition to the US `<ST> <5-digit ZIP>`. Province is the same 2-letter uppercase token slot.
...
### Current behavior (cited)
The dims token `\d+\s*'\s*x` (feet-mark `'` then literal `x`, case-insensitive) appears at THREE sites that MUST widen together:
- `roomHeaderNameShape` (`rooms.ts:129`, guard at `:134` `/^\d+\s*'\s*x/i`) — REJECTS a dims-leading string from being a room NAME.
- `splitRoomHeader` `dimStart` (`rooms.ts:1486`): `/(?:\b(?:TOTAL|APPROXIMATELY)\s*:?\s*|\bA\/B\s*:\s*)?\d+\s*'\s*x/i` — EXTRACTS dims (slice at :1490/:1500). Callers: :748/953/964/1136/1183/1243/1400.
- `firstHeaderDimsToken` (helper ~`rooms.ts:1258`) — EXTRACTS the header-harvested dims token.
Existing post-extraction cleanup (dangling trailing `x`, `rooms.ts` audit idx22) is preserved.

### Widened grammar (accept, in addition to today)
Replace the fixed `\d+\s*'\s*x` core with a shared dims-token pattern accepting:
- **Unit** (optional, on the first operand): `'` (ASCII prime) | `′` (U+2032) | `ft`/`FT` (word-boundaried).
- **Separator**: `x` | `X` | `×` (U+00D7).
- **Bare-number gate (the false-positive control):** a fully unit-less token `\d+\s*[x×]\s*\d+` is admitted ONLY when **both operands are ≥ 2 digits** (i.e. ≥ 10) — real room dimensions are always ≥ 10 ft, so `50 x 40` / `120x80` are admitted while a single-digit `5 x 8` (index-card / count / odds) is NOT. A unit-bearing token (`50' x`, `50′×`, `50ft x`) needs no digit-count gate (the unit disambiguates).

Effective accepted set (examples): `50' x 40'`, `50'x40'`, `50′×45′`, `50ft x 40ft`, `50 FT X 40`, `50 x 40`, `APPROXIMATELY 60' x 45'`, `TOTAL 120 x 80`. All three sites derive from ONE exported pattern/constant so they cannot drift.

### STAYS REJECTED (dims)
- Fully bare single-digit `5 x 8`, `3x4` (both operands < 2 digits, no unit) → NOT dims (ambiguous with counts/odds/cards).
- A separator with a missing operand (`50' x`, trailing) → the existing dangling-`x` cleanup still strips it (unchanged).
- `x`/`×` glued inside a word (`Box40`, `Matrix`) → the `\d+` before the separator and operand-after requirement prevents a match.
...
- `lib/parser/blocks/rooms.ts` — one shared dims-token constant; `roomHeaderNameShape` + `dimStart` + `firstHeaderDimsToken` derive from it.

## Companion-surface invariants (must stay in lockstep — enumerate in plan)
1. **Dims token single-source:** `roomHeaderNameShape`, `splitRoomHeader dimStart`, and `firstHeaderDimsToken` all consume ONE exported dims-token pattern. A test asserts the three sites reference the shared constant (no re-inlined literal).
...
- **Dims:** `splitRoomHeader` + `roomHeaderNameShape` + `firstHeaderDimsToken` over each new unit/symbol variant AND the bare-`50 x 40` (admit) vs bare-`5 x 8` (reject) gate; the single-source invariant test (invariant 1).
