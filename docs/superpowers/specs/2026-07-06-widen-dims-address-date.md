...
- Bare `NxN` glued to adjacent alphanumerics (`Box40x2`, `Room4x4`, `120x80B`, `SKU 40x20A`) → NOT matched (leading `\b` + every bare operand's trailing `\b`; a letter or extra digit abutting an operand denies its boundary — Codex spec R5).
- Bare chain with a ≥4-digit operand mid-token (`50 x 1200`, `50 x 40 x 1200`) → the over-length bare operand denies `\d{2,3}\b`, so the token is rejected (2-operand case) or truncated to the valid leading pair and the bad tail dropped (3-operand case) — never captured as a partial `120`.
...
1. **Dims token single-source via shared FRAGMENTS (Codex spec R2 finding 3, R3 finding 1, R4 finding — SEVEN sites, three shape-classes):** the sites have DIFFERENT shapes by necessity — Class A/B need a partial START matcher (locate/attest where dims begin), Class C needs a FULL capture (the whole 2-or-3-operand token). They CANNOT share one whole-token regex; instead they compose exported sub-fragments (regex-source strings, shared from one module so every site imports the same literal — no re-inlined `'\s*x`):
   ...
   - `DIMS_OPERAND_BARE` = `\d{2,3}` — a **unit-less** operand, gated to 2–3 digits so `2026 x 40` / `5 x 8` are NOT admitted.
   Two composed matchers built from those fragments:
   - `DIMS_START` (partial — "a dims token begins here") = `(?:\d+\s*(?:['′]|ft\b)\s*[x×]|\b\d{2,3}\s*[x×]\s*\d{2,3}\b)`. The **unit-bearing** branch needs only operand-unit + separator (the unit disambiguates, matching today's `\d+'\s*x` "one operand + `x`" behavior). The **fully-bare** branch requires the WHOLE `\b NN x NN \b` token (both 2–3-digit operands, word-boundaried) — so a lone `50 x` (in "50 x 4 rows"), `50 xylophones`, or `2026 x 40` never triggers a bare START.
   - `DIMS_OPERAND` (either kind, self-boundaried on the bare alt) = `(?:\d+\s*(?:['′]|ft\b)|\d{2,3}\b)`. The bare alt carries its OWN trailing `\b`; the unit alt self-terminates on `'`/`′`/`ft\b`. **Every bare operand — 1st, 2nd, AND 3rd — must carry this trailing `\b`** (Codex spec R5: without it a bare chain leaks a truncated operand — `50 x 1200`→`50 x 120`, `120x80B`→`120x80`, `SKU 40x20A`→`40x20`; with `\d{2,3}\b` the trailing digit/letter denies the boundary so the whole token is rejected, matching the ">999 ft needs a unit" rule).
   - `DIMS_FULL` (2-or-3-operand capture) = `(\bDIMS_OPERAND\s*[x×]\s*DIMS_OPERAND(?:\s*[x×]\s*DIMS_OPERAND)?)`, i.e. `(\b(?:\d+\s*(?:['′]|ft\b)|\d{2,3}\b)\s*[x×]\s*(?:\d+\s*(?:['′]|ft\b)|\d{2,3}\b)(?:\s*[x×]\s*(?:\d+\s*(?:['′]|ft\b)|\d{2,3}\b))?)`. The leading `\b` gates the first operand's left edge; each bare operand's trailing `\b` gates its right edge; the mandatory second operand means `2026 x 40` cannot match (4-digit first operand is not `\d{2,3}` at a boundary) while `2026' x 40'` matches via the unit branch, and `50 x 1200` fails entirely (its 2nd bare operand `1200` denies `\d{2,3}\b`).
