...
**Date-surface scope boundary (complete companion sweep — `grep -rnE '\d\{1,2\}/\d\{1,2\}' lib/parser/`).** The widening reaches EXACTLY three surfaces: `normalizeDate` (the canonical parser), `extractAllDates` (free-scan), and `inferShowYear` (slash-first). Every OTHER slash-only date matcher in the parser is an independent, context-scoped detection/splitting surface — NOT a general date parser, NOT routed through `normalizeDate` — and is retained AS-IS (behavior-preserving documented residual, no corpus evidence for widening): hotel check-in/out capture (`hotels.ts:635-636`) + reservation-group split delimiter (`hotels.ts:610`); `sessionDayLabel` day-label extract (`rooms.ts:352`); `travelFlights` full-cell `DATE_RE` (`travelFlights.ts:10`); transport "date-like stage" heuristic (`transport.ts:96`); personalization schedule day-restriction tokens (`personalization.ts:59/62`, `3/24 & 3/26 ONLY`). None of these change.
