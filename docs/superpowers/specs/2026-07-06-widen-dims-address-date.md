...
### Widened grammar (accept, in addition to today)
Replace the fixed `\d+\s*'\s*x` core with two shared, exported FRAGMENTS — `DIMS_OPERAND` (a number with optional unit) and `DIMS_SEP` (`[x×]`) — that each site composes into the shape IT needs (partial-START for `roomHeaderNameShape`/`dimStart`; full 2-or-3-operand capture for `harvestSameNameHeaderDims`; `DIMS_SEP` also drives the dangling-separator cleanup). The fragments accept:
...
1. **Dims token single-source via shared FRAGMENTS (Codex spec R2 finding 3):** the three sites have DIFFERENT shapes by necessity — `roomHeaderNameShape:134` and `splitRoomHeader dimStart:1486` need a partial START matcher (`<operand><sep>` — enough to locate where dims begin and slice the name off), while `harvestSameNameHeaderDims:1270` needs a FULL capture (`<operand><sep><operand>(<sep><operand>)?` — the whole 2-or-3-operand token). They therefore CANNOT share one whole-token regex; instead they compose two exported sub-fragments — `DIMS_OPERAND` (`\d+\s*(?:['′]|ft\b)?` for unit-bearing, plus the bare 2–3-digit form) and `DIMS_SEP` (`[x×]`). A test asserts all three sites reference the shared fragments (no re-inlined `'\s*x` literal) AND that the dangling-separator cleanup uses `DIMS_SEP` (`[x×]`), so widening a fragment updates every site at once.
