...
| 8 | `BRANCH_PROTECTION_DRIFT` | `Open branch settings` | `https://github.com/${repo}/settings/branches` | yes | `repo` splits on `/` into exactly two segments: owner matches `/^[A-Za-z0-9-]+$/` (GitHub owner charset — no dots), repo-name matches `/^[A-Za-z0-9_.-]+$/` AND is not `.` or `..` (dot segments URL-normalize away from the intended path). Else null |
...
| `context.repo` | no link | no link | no link | segment-guard fail (§4 #8: wrong segment count, bad charset, or `.`/`..` repo-name) → no link |
