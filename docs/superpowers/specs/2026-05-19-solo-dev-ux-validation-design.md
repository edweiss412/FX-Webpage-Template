...
| `pnpm validation:mint-link` | `scripts/validation-mint-link.ts` | `--combo <combo> --alias <alias> --expires-in <seconds-relative-to-now>` (negative = pre-expired) | Same three env vars + `VALIDATION_JWT_SIGNING_SECRET` (must equal Vercel **Production** scope's `JWT_SIGNING_SECRET`) | Reads `active_signing_key_id` from `app_settings`; reads crew_id from `validation_state.alias_map`; INSERTs a `link_sessions` row | Stdout = full URL `/show/<slug>/p#t=<jwt>` | Exit 0 + URL; exit 1 + diagnostic on signing-secret missing, active_signing_key_id unavailable, or alias unresolvable. | Each call mints a NEW link (not idempotent by design); revoke via revoke-link |
| `pnpm validation:revoke-link` | `scripts/validation-revoke-link.ts` | `<link_session_id>` (positional UUID) | Same three env vars | UPDATEs `link_sessions.revoked_at = now()` + INSERTs `revoked_links` row | Stdout = `revoked <link_session_id>` | Exit 0 on success; exit 1 if link_session not found | Idempotent: revoking already-revoked link is a no-op |
