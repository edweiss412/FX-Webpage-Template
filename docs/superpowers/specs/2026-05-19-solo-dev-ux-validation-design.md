...
| 2 | On the iPhone (which previously held a valid post-rotation cookie from leg (a) step 3): reload the SAME URL. | Resolver compares cookie's `e` to the new `shows.picker_epoch` → `identity_invalidated` with `reason: 'session_mismatch'` per P-R29/P-R30 Fix-1 (the cookie's identity is no longer valid under the new epoch; this is the two-reasons doc-guard `session_mismatch` arm, NOT `claimed_after_pick`). Picker re-renders with `PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER` + auto-submitting `<StaleCleanupAutoSubmit>` (compare-and-delete the stale entry). |
...
| 2 | **The OAuth claim** — on the iPhone, sign in via Google using a Google account whose email matches `alias_5a_lead`'s `crew_members.email` (the dev controls the fixture email; this is the bypass-pick-then-claim scenario). The OAuth flow: iPhone → `/auth/sign-in` → Google consent → `/auth/callback/route.ts` → invokes `claim_oauth_identity(email)` RPC which stamps `claimed_via_oauth_at = now()` on every matching crew_members row (under per-show advisory locks); callback DOES NOT mint a picker cookie (R41-R6 — cookies mint lazily on next show visit via picker-bootstrap). The iPhone redirects to `next` (the tokenized show URL). | iPhone's page-route resolver: cookie has `id: <alias_5a_lead.id>`; Google session matches `alias_5a_lead`'s email; `claimed_via_oauth_at IS NOT NULL`; but cookie's `t` is from leg (b) step 3 (pre-claim) so `cookie.t < claim_epoch_millis`. Resolver returns `identity_invalidated` with `reason: 'claimed_after_pick'` per P-R29/P-R30 Fix-1; picker re-renders with `PICKER_IDENTITY_CLAIMED_AFTER_PICK_BANNER`. **BUT** — the picker now shows `alias_5a_lead`'s row as **visually disabled** (per picker-pivot Resolved Decision 15 + §7 visual contract — `claimed_via_oauth_at` is non-null). Tapping the disabled row redirects to `/auth/sign-in?next=<URL>` rather than minting a cookie. |
| 3 | On the iPhone: tap the (disabled) `alias_5a_lead` row OR navigate back to the sign-in flow OR (the typical-user path) just reload the URL — the Google session is still active. | The page-route's auth chain step 4 detects: Google session matches `alias_5a_lead`'s row, `claimed_via_oauth_at IS NOT NULL`, cookie either absent OR mismatched. The resolver returns `needs_picker_bootstrap` with an intent token; page redirects to `/api/auth/picker-bootstrap?next=<URL>&t=<intentToken>`. Bootstrap mints a NEW picker cookie with `t = result.mint_safe_t_millis` (DB-side, strictly greater than `claimed_via_oauth_at`); 302 back to the URL; `_ShowBody` renders. iPhone is now "signed in, picker skipped" per Resolved Decision 17. |
| 4 | **Switch to the desktop** (still holding the pre-claim cookie from step 1). Reload the same tokenized URL. | Desktop's resolver detects: cookie has `id: <alias_5a_lead.id>`, no Google session on this desktop browser, AND the canonical email check (`auth_email_canonical()` consistency) finds the cookie's crew_members row has been claimed by another session → `identity_invalidated` with `reason: 'session_mismatch'` (the OTHER reason in the two-reasons doc-guard contract). Picker re-renders with banner + `<StaleCleanupAutoSubmit>`. The dev confirms: `alias_5a_lead`'s row is now visually disabled in the desktop's picker too (the OAuth claim is global — every device sees this identity as claimed). Pick `alias_6a_a1` (a non-LEAD, OAuth-unclaimed identity) instead → `_ShowBody` renders as A1 viewer. |

This leg exercises: (i) `OAUTH_IDENTITY_CLAIMED` emission per P-R8 Fix-3 / P-R20 Fix-2 isolation; (ii) the H8 doc-guard's two-reasons contract by hitting BOTH `claimed_after_pick` (iPhone step 2) AND `session_mismatch` (desktop step 4); (iii) the picker's visually-disabled-row rendering for OAuth-claimed identities; (iv) the lazy-mint picker-bootstrap path (Resolved Decision 17 — login skips the picker via the bootstrap redirect chain).
