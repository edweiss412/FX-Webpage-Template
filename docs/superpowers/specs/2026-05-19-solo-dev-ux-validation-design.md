...
|---|---|---|---|---|
| 1 | 5a (`["LEAD"]`) | R1 (`none`/`none`/set day) | Baseline LEAD + no restriction; sanity check | Right Now: show-wide set-day state. Audio scope tile: hidden (no A1 flag). Pack-list: visible (set day, no stage restriction). Financials (shows_internal): visible (LEAD). |
| 2 | 5b (`["LEAD","A1"]`) | R2 (`explicit` today included) | Compound LEAD + audio scope tile with date restriction matching | Right Now: show-wide set-day state (today in assigned days, no off-day override). Audio scope tile: VISIBLE. Pack-list: visible. Financials: visible. |
| 3 | 5c (`["BO","LEAD"]`) | R7a (`none`/`["Load In","Set"]`/set day → visible) | Compound LEAD + backstage with set-day-only stage restriction; pack-list visible | Right Now: set-day state. Audio/video/lighting scope tiles: hidden. Pack-list: VISIBLE (today within stage restriction). Financials: visible. |
| 4 | 6a (`["A1"]`) | R3 (`explicit` today excluded) | Audio scope tile + off-day → no scope tile shown | Right Now: `viewer_off_day` copy (per master spec §8 line 2413). Audio scope tile: HIDDEN (date filter overrides — off-day suppresses all scope tiles). Pack-list: hidden. Financials: hidden (non-LEAD). |
| 5 | 6b (`["V1"]`) | R4 (`unknown_asterisk`) | Video scope + asterisk-unconfirmed; check both don't double-trigger | Right Now: `viewer_unconfirmed` copy (master spec §8 line 2411 — takes precedence over show-wide state). Video scope tile: HIDDEN (unconfirmed overrides). Pack-list: hidden. Financials: hidden. |
| 6 | 6c (`["L1"]`) | R5 (pre-show day) | Lighting scope + pre-show state | Right Now: `viewer_off_day_pre` copy with "first day: X" countdown (master spec §8 line 2414). Lighting tile: HIDDEN until first assigned day. Pack-list: hidden. Financials: hidden. |
| 7 | 6d (`["BO"]`) | R6 (post-show day) | No-scope crew + after-last-day state | Right Now: `viewer_after_last_day` copy (master spec §8 line 2412). No scope tiles render (no A1/V1/L1 + post-show). Pack-list: hidden. Financials: hidden. |
...
| 11 (R10 amendment) | 5a (`["LEAD"]`) | R8b (`none`/`["Load Out","Strike"]`/set day → pack-list hidden) | Pure LEAD + opposite-day stage restriction; confirms financials still render but pack-list hidden when stage filter excludes today | Right Now: set-day show-wide state. No scope tiles (no A1/V1/L1 flag). Pack-list: HIDDEN (stage filter excludes set day). Financials: VISIBLE (LEAD; pack-list hidden does not gate financials). |
...
| Revoked link (J3 revoked-link path) | `URL=$(pnpm -s validation:mint-link --combo R1 --alias alias_5a_lead --expires-in 900 \| jq -r .url)` then `pnpm validation:revoke-link "$URL"` (R18: revoke-link now accepts URL or UUID; mint-link emits JSON per §9.1.2) | Link redemption returns 401 with "not on crew list" surface per master spec §7 |
...
| `pnpm validation:mint-link` | `scripts/validation-mint-link.ts` | `--combo <combo> --alias <alias> --expires-in <seconds-relative-to-now>` (negative = pre-expired; positive = future expiry; required — no default) | Same three env vars + `VALIDATION_JWT_SIGNING_SECRET` (must equal Vercel **Production** scope's `JWT_SIGNING_SECRET`) | Reads `active_signing_key_id` from `app_settings`; reads crew_id from `validation_state.alias_map`; INSERTs a `link_sessions` row | **Structured JSON on stdout (R18 amendment — was bare URL): `{"link_session_id": "<uuid>", "url": "/show/<slug>/p#t=<jwt>", "expires_at": "<ISO>"}`** so revoke-link can consume the id without DB spelunking. | Exit 0 + JSON; exit 1 + diagnostic on signing-secret missing, active_signing_key_id unavailable, or alias unresolvable. | Each call mints a NEW link (not idempotent by design); revoke via revoke-link |
| `pnpm validation:revoke-link` | `scripts/validation-revoke-link.ts` | **R18 amendment: `<link_session_id\|url>` (positional). Accepts either the bare UUID OR the full URL emitted by mint-link (script extracts the JWT, decodes the `jti`/session-id claim, and resolves the underlying `link_sessions.id`).** | Same three env vars | UPDATEs `link_sessions.revoked_at = now()` + INSERTs `revoked_links` row | Stdout = `revoked <link_session_id>` | Exit 0 on success; exit 1 if link_session not found OR url parse fails | Idempotent: revoking already-revoked link is a no-op |
| `pnpm validation:report-fixtures` | `scripts/validation-report-fixtures.ts` | `--outcome <success\|in-flight\|rate-limit-admin\|rate-limit-crew\|lookup-inconclusive\|horizon-expired\|orphaned-lost-lease>` | Same three env vars | INSERTs / UPDATEs the `reports` table directly via service role to materialize each outcome's row shape per master spec §13.2.3 | Stdout = `materialized <outcome> report row <id>` | Exit 0 on success; exit 1 if outcome name unknown or DB write fails | Each invocation inserts a new row; clean-up via separate `--cleanup` flag (idempotent delete-by-tag) |
