...
| `pnpm validation:mint-link` | `scripts/validation-mint-link.ts` | `--combo <combo> --alias <alias> --expires-in <seconds-relative-to-now>` (negative = pre-expired; positive = future expiry; required — no default) | Same three env vars + `VALIDATION_JWT_SIGNING_SECRET` (must equal Vercel **Production** scope's `JWT_SIGNING_SECRET`) | Reads `active_signing_key_id` from `app_settings`; reads crew_id from `validation_state.alias_map`; INSERTs a `link_sessions` row | **Structured JSON on stdout (R19 corrected from R18 — aligned with live schema): `{"token": "<link_sessions.token PK>", "url": "/show/<slug>/p#t=<jwt>", "expires_at": "<ISO>", "show_id": "<uuid>", "crew_name": "<name>", "jwt_token_version": <int>}`** — these fields are the keys revoke-link needs to INSERT into `revoked_links` (PK = `(show_id, crew_name, token_version)`). | Exit 0 + JSON; exit 1 + diagnostic on signing-secret missing, active_signing_key_id unavailable, or alias unresolvable. | Each call mints a NEW link (not idempotent by design); revoke via revoke-link |
| `pnpm validation:revoke-link` | `scripts/validation-revoke-link.ts` | **R19 amendment: `<token\|url>` (positional). Accepts either the bare `link_sessions.token` value OR the full URL emitted by mint-link (script extracts the `#t=<jwt>` fragment → jwt is opaque token, look up `link_sessions WHERE token = <jwt>` → resolve `(show_id, crew_name, jwt_token_version)`).** R19 corrected — earlier R18 wording referenced nonexistent JWT `jti` claim and `link_sessions.id`/`revoked_at`. Live schema: `link_sessions` PK is `token text`; revocation is INSERT into `revoked_links` (PK `(show_id, crew_name, token_version)`). | Same three env vars | INSERTs `revoked_links (show_id, crew_name, token_version, revoked_at, revoked_reason)` matching the link's (show_id, crew_name, jwt_token_version). Does NOT touch link_sessions (per live schema — there's no revoked_at column there; validateLinkSession checks revoked_links to deny). | Stdout = `revoked <token>` | Exit 0 on success; exit 1 if token not found in link_sessions OR url parse fails | Idempotent: revoking already-revoked (token_version, show_id, crew_name) tuple succeeds as no-op via `ON CONFLICT DO NOTHING` |
