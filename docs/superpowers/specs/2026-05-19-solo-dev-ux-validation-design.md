...
- For every R-combo fixture: 9 `crew_member_auth` rows (one per crew_member, keyed by `(show_id, crew_name)`).
...
**check-seed validates crew_member_auth completeness.** Beyond the 5 predicates listed in §3.3.2, check-seed fails additionally if (f) for any alias in alias_map, `crew_member_auth` is missing the matching row, OR (g) `current_token_version` is unset/null.
...
| Revoked link (J3 revoked-link path) | `URL=$(pnpm -s validation:mint-link --combo R1 --alias alias_5a_lead --expires-in 900 \| jq -r .url)` then `pnpm validation:revoke-link "$URL"` (R19 corrected from R18: revoke-link parses the `#t=<token>` fragment to look up `link_sessions.token`, resolves `(show_id, crew_name, jwt_token_version)`, and INSERTs into `revoked_links`. mint-link emits structured JSON per §9.1.2 with `token` / `url` / `show_id` / `crew_name` / `jwt_token_version` so the revocation can also be scripted via the JSON fields directly if `jq` is unavailable.) | Link redemption returns 401 with "not on crew list" surface per master spec §7 |
| Valid baseline (control) | `pnpm validation:mint-link --combo R1 --alias alias_5a_lead --expires-in 900` (R18: --expires-in required per §9.1.2 — no default TTL) | Link redemption renders crew page |
...
| Aspect | Contract |
|---|---|
| Required env var (R21 corrected) | `VALIDATION_JWT_SIGNING_SECRET` (distinct from local `JWT_SIGNING_SECRET` to prevent accidental signing with wrong key). **R16 amendment: must equal the Vercel project's `JWT_SIGNING_SECRET` value scoped to the Vercel "Production" environment (not "Preview" or "Development") for the exact project backing the `*.vercel.app` production deployment.** Vercel env vars can differ across the three environment scopes; the validation tooling only works against the Production scope's value. Phase 0 sub-task: dev copies the Vercel **Production-environment** env var into local `.env.local` AND `.env.local.example` documents `VALIDATION_JWT_SIGNING_SECRET` with the inline note "must equal Vercel Production-scope JWT_SIGNING_SECRET". **R21: implementation contract for signLinkJwt's env-var indirection.** Live `lib/auth/jwt.ts:42` reads `process.env.JWT_SIGNING_SECRET`, not the VALIDATION_ variant. The validation-mint-link.ts script MUST handle this indirection in one of two safe ways: (a) at process start, assert that `process.env.JWT_SIGNING_SECRET` is undefined OR equals VALIDATION_JWT_SIGNING_SECRET, then set `process.env.JWT_SIGNING_SECRET = VALIDATION_JWT_SIGNING_SECRET` for the duration of the process; OR (b) spawn signLinkJwt in a child process with `env: { JWT_SIGNING_SECRET: VALIDATION_JWT_SIGNING_SECRET }`. Mismatched local-and-validation secrets abort with diagnostic. Plan picks the safer option; a Phase 0 sub-test exercises this env-mapping by intentionally mis-setting and confirming abort. |
