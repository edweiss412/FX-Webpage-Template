...
**check-seed validates crew_member_auth completeness.** Beyond the 5 predicates listed in §3.3.2, check-seed fails additionally if (f) for any alias in alias_map, `crew_member_auth` is missing the matching row, OR (g) `current_token_version` is unset/null, OR **(h) R22: `revoked_links` has a row matching the baseline `alias_5a_lead`'s (show_id, crew_name, current_token_version) tagged with `revoked_reason LIKE 'validation:%'`** — this would mean a prior J3 walk's revocation was not cleaned up by re-seed, poisoning the baseline.
...
**Expired-link fixture contract (R14 amendment, R21 corrected against two-token architecture).** "Waiting for TTL" is not viable — the live signed-link JWT has a 90-day default expiry. mint-link emits a fresh JWT (NOT a link_sessions row — see §9.1.2; link_sessions rows are created only at redemption with a fresh opaque token). The companion `pnpm validation:mint-link --combo <combo> --alias <alias> --expires-in <seconds-relative>` command (negative values mint already-expired JWTs):

| Scenario | Command | Expected outcome |
|---|---|---|
| Already-expired link (J3 expired-link path) | `pnpm validation:mint-link --combo R1 --alias alias_5a_lead --expires-in -3600` (1 hour past — R16 widened from -60s to absorb clock skew) | Link redemption returns 401 with `LINK_EXPIRED` surface. |
| Revoked link (J3 revoked-link path) | **Use dedicated `alias_5a_lead_for_revoke` (NOT the baseline alias_5a_lead — R22: revoke is surgical to (show_id, crew_name, token_version) and would poison the baseline control if both use the same alias).** `URL=$(pnpm -s validation:mint-link --combo R1 --alias alias_5a_lead_for_revoke --expires-in 900 \| jq -r .url)` then `pnpm validation:revoke-link "$URL"`. **R20/R22 corrected contract:** revoke-link verifies + decodes the JWT directly (no link_sessions lookup — link_sessions rows don't exist before redemption). The JWT payload supplies `{show_id, crew_name, jwt_token_version}` (no `token` field — that field referenced the now-removed link_sessions.token misunderstanding). revoke-link INSERTs into `revoked_links (show_id, crew_name, token_version, revoked_at, revoked_reason)` directly. | Link redemption returns 401 with "not on crew list" surface per master spec §7 |
| Valid baseline (control) | `pnpm validation:mint-link --combo R1 --alias alias_5a_lead --expires-in 900` (R18: --expires-in required per §9.1.2 — no default TTL). **R22: baseline uses `alias_5a_lead`, the un-revoked counterpart of `alias_5a_lead_for_revoke`. Re-seed clears `revoked_links` rows tagged with `revoked_reason LIKE 'validation:%'` so baseline stays valid across re-runs.** | Link redemption renders crew page |

The mint-link and revoke-link commands are plan-time deliverables added to the validation tooling alongside the re-seed script. **R21 amendment:** `check-seed` does NOT pre-verify expired/revoked link_session rows (per the two-token architecture, link_sessions rows are only created at redemption — they don't pre-exist). Instead, the dev mints the JWTs and revokes via the JWT payload at the moment of J3's walk; the revoked_links INSERT is the durable artifact, NOT a pre-seeded link_sessions row. check-seed verifies the FIXTURE prerequisites (crew_members, crew_member_auth, alias_map) are present so mint-link can produce JWTs successfully.
...
| Aspect | Contract |
|---|---|
| Required env var (R21 + R22 corrected) | `VALIDATION_JWT_SIGNING_SECRET` (distinct from local `JWT_SIGNING_SECRET`). Must equal Vercel **Production** scope's `JWT_SIGNING_SECRET`. Phase 0 dev copies the Production-scope env var into local `.env.local`; `.env.local.example` documents it. **R21/R22 implementation contract — THREE env vars indirected for signLinkJwt's process.** signLinkJwt reads `process.env.JWT_SIGNING_SECRET` (lib/auth/jwt.ts:42) AND `createSupabaseServiceRoleClient()` reads `process.env.SUPABASE_URL` + `process.env.SUPABASE_SECRET_KEY` (R22 finding — the active_signing_key_id lookup goes through the regular service-role client, NOT the VALIDATION_-prefixed env vars). The validation-mint-link.ts script MUST therefore map ALL THREE in lockstep into the signing process: (a) at process start, assert each of `process.env.{JWT_SIGNING_SECRET, SUPABASE_URL, SUPABASE_SECRET_KEY}` is undefined OR matches the VALIDATION_ variant, then set the three from `VALIDATION_JWT_SIGNING_SECRET`, `VALIDATION_SUPABASE_URL`, `VALIDATION_SUPABASE_SECRET_KEY` for the process; OR (b) spawn signLinkJwt in a child process with all three mapped via the child's `env`. **Mismatched secrets/URLs/keys abort with diagnostic** ("local SUPABASE_URL differs from validation; refusing to sign"). Phase 0 sub-test exercises this by intentionally setting `process.env.SUPABASE_URL` to a different value and confirming abort. |
...
| Phase 0 smoke test 6 (new — see §9.2 as canonical source) | Mint a valid link locally via `pnpm validation:mint-link --combo R1 --alias alias_5a_lead --expires-in 900` (positive 15-min TTL — R16 widened from 60s; absorbs local-vs-Vercel clock skew + human delay + share-flow friction). Open the URL on dev's iPhone against the Vercel `*.vercel.app` URL within the 15-min window. Confirm crew page renders. **R17 amendment: this row defers to §9.2 smoke 6 as the canonical source for TTL, prerequisite (reseed+check-seed --combo R1), and failure-isolation procedure. The 900s value here matches §9.2.** |
...
### 9.1.2 Validation tooling reference (R17 amendment — single authoritative source)
...
| `pnpm validation:mint-link` | `scripts/validation-mint-link.ts` | `--combo <combo> --alias <alias> --expires-in <seconds-relative-to-now>` (negative = pre-expired; positive = future expiry; required — no default) | Same three env vars + `VALIDATION_JWT_SIGNING_SECRET` (must equal Vercel **Production** scope's `JWT_SIGNING_SECRET`) | Reads `active_signing_key_id` from `app_settings`; reads crew identity (show slug, crew name, current token_version) from prod-equivalent Supabase via the alias→crew_id→crew_members lookup. **Calls `signLinkJwt(...)` directly** (does NOT pre-insert `link_sessions` — R20 corrected; the live flow only creates the session row at redemption time via `POST /api/auth/redeem-link` calling `mint_link_session_if_active_kid_matches` with a fresh `randomUUID()` opaque token). | **Structured JSON on stdout (R20 corrected): `{"url": "/show/<slug>/p#t=<jwt>", "expires_at": "<ISO>", "show_id": "<uuid>", "crew_name": "<name>", "jwt_token_version": <int>, "signing_kid": "<active_signing_key_id>"}`** — JWT payload fields the revoke-link tool uses to INSERT `revoked_links` directly. No `link_sessions.token` to emit because no session row has been created (it gets created on redemption with a different opaque token). | Exit 0 + JSON; exit 1 + diagnostic on signing-secret missing, active_signing_key_id unavailable, or alias unresolvable. | Each call mints a NEW JWT (not idempotent by design); revoke via revoke-link |
| `pnpm validation:revoke-link` | `scripts/validation-revoke-link.ts` | **R20 amendment: `<url\|jwt>` (positional). Accepts either the full URL emitted by mint-link OR the bare JWT. Script verifies + decodes the JWT (using `VALIDATION_JWT_SIGNING_SECRET` + active kid) to obtain `{show_id, crew_name, jwt_token_version}` from the payload.** R19 wrongly assumed the fragment token equals `link_sessions.token`; the live flow uses TWO distinct tokens — the JWT (issued by sign) and the opaque session token (issued by redemption). The dev's revoke command operates BEFORE any redemption, so there is no `link_sessions` row yet; revoke uses the JWT payload fields directly. | Same three env vars + `VALIDATION_JWT_SIGNING_SECRET` (needed to verify the JWT) | INSERTs `revoked_links (show_id, crew_name, token_version, revoked_at, revoked_reason)` matching the JWT payload. Does NOT touch `link_sessions` (the live revocation contract is "revoked_links INSERT denies any link with matching token_version", checked by `validateLinkSession` on every request). | Stdout = `revoked <show_id> <crew_name> token_version=<n>` | Exit 0 on success; exit 1 if JWT verification fails OR url parse fails | Idempotent: re-revoking the same tuple is a no-op via `ON CONFLICT (show_id, crew_name, token_version) DO NOTHING` |
| `pnpm validation:report-fixtures` | `scripts/validation-report-fixtures.ts` | `--outcome <success\|in-flight\|rate-limit-admin\|rate-limit-crew\|lookup-inconclusive\|lease-expired\|horizon-expired\|orphaned-lost-lease>` (R20 amendment — `lease-expired` added; was missing from the enum despite §4.1.1 requiring one matrix row per outcome) | Same three env vars | INSERTs / UPDATEs the `reports` table directly via service role to materialize each outcome's row shape per master spec §13.2.3 | Stdout = `materialized <outcome> report row <id>` | Exit 0 on success; exit 1 if outcome name unknown or DB write fails | Each invocation inserts a new row; clean-up via separate `--cleanup` flag (idempotent delete-by-tag) |

**Sequencing note.** The canonical Phase 0 sequence is: reseed → check-seed → (smoke 1) admin sign-in → (smoke 2) signed-link iPhone smoke → (smoke 3) cron → (smoke 4) admin_alerts → (smoke 5) clock control → (smoke 6) mint-redeem round-trip per §9.2. The reference table above is the contract surface; §9.2 is the smoke-sequence-and-prerequisite source.
...
**Posture.** This is a structural amendment, not a review round. It rebases the spec (drafted 2026-05-19 against the pre-M11.5 per-crew signed-link auth model) onto the picker-pivot world ratified at the M11.5 close-out (`b4b2c38`, 2026-05-25). The amendment is its own dispatch — see [`docs/superpowers/plans/2026-05-19-solo-dev-ux-validation/handoffs/M12-amendment-dispatch-brief.md`](../plans/2026-05-19-solo-dev-ux-validation/handoffs/M12-amendment-dispatch-brief.md) — and was authored by an Opus 4.7 implementer session per the dispatch brief, with the user's `(d)` decision on J3 baked in. Post-amendment, the spec proceeds to fresh cross-CLI adversarial review at R6 (`handoffs/round-06.md` onward); the R1–R25 rounds in §15.1–§15.25 are pre-rebase audit trail and remain readable as historical context.
...
- Class-sweep on dispatch §5.4 vocabulary: pre-amendment baseline 187 hits across 12 files (71 in spec). Post-amendment target: zero outside §15 audit-trail + `handoffs/M12-*.md`; residual count reported in the final amendment commit.

**Why this is an amendment and not a 26th review round.** Per user R0 authorization carried forward from §15.25 (and the dispatch brief §2 do-not-relitigate item 23), the spec proceeded to plan-writing after R25 without formal APPROVE. The 26th adversarial round was structurally not the convergence path — the picker pivot ratified between R25 (2026-05-19) and M11.5 close-out (2026-05-25) changed the rebase target. The amendment is the resolution; review resumes at R6 in `handoffs/round-06.md` against the rebased spec + plan.
