...
| [`2026-08-26-control-outline-cover-widening-design.md`](./2026-08-26-control-outline-cover-widening-design.md) | Closes `BL-CONTROL-OUTLINE-BEYOND-ELEMENT-COVER`. Two rulings admit text-entry fields and painted child visuals to the control-outline rule; `scanInteractiveElements` gains a per-consumer `ScanOptions` axis, the outline guards opt in, and the resulting red names all 35 sites the sweep moves. Adds the `inner-chrome` residue category. |
...
| [`2026-08-28-diagram-tile-chrome-consistency.md`](./2026-08-28-diagram-tile-chrome-consistency.md) | Closes `BL-DIAGRAM-TILE-CHROME-CONSISTENCY`. One visual idiom HAD been drawn two ways: the crew gallery painted the tile box on the grid cell, the admin wizard tile painted it on the `<Image>`. Sweeps every bordered thumbnail, rules that the chrome belongs on the box-forming wrapper in every branch, and re-derives the control-outline spec's painted-child count from four to three in the same change. The admin tile now paints its box on the anchor; the image carries only `object-cover`. |
| [`2026-08-28-published-escape-consumed-claim.md`](./2026-08-28-published-escape-consumed-claim.md) | Escape on the published review modal can close the whole dialog when the attention panel is down for a frame, because the shell closes on any Escape without asking whether anything consumed it. Holds the claim in the modal so it outlives the panel, and makes the mark a classifier between a transient unmount and an intentional dismissal. Retires two of the row's three candidates on measured evidence. |
| [`2026-08-28-wizard-warning-ignore-controls.md`](./2026-08-28-wizard-warning-ignore-controls.md) | Mounts the published surface's per-warning Report + Ignore controls (and an Ignored (N) disclosure) in the onboarding wizard review modal's Sheet warnings panel, threads staged show identity + a server-derived active/ignored warning model through Step3Row, and switches every wizard warning count to the active partition. |
