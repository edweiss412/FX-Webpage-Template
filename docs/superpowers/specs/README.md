...
| [`2026-08-28-diagram-tile-chrome-consistency.md`](./2026-08-28-diagram-tile-chrome-consistency.md) | Closes `BL-DIAGRAM-TILE-CHROME-CONSISTENCY`. One visual idiom was drawn two ways: the crew gallery paints the tile box on the grid cell, the admin wizard tile paints it on the `<Image>`. Sweeps every bordered thumbnail, rules that the chrome belongs on the wrapper because that is the only element present in every render state, and re-derives the control-outline spec's painted-child count from four to three in the same change. |
