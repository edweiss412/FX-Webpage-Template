...
- `apply-pending` (per-show surface only) — the decision committed but the delegated re-sync failed (§9b failure semantics): the control shows `raw-active` (the preference IS saved) plus a non-error notice "Saved. The crew-visible values will update on the next successful sync." The wizard surface has no `apply-pending` state (its decision materializes at finalize, not via an immediate re-sync). The transition-audit task MUST enumerate the `pending → apply-pending` and `apply-pending → raw-active` (next successful sync) pairs in addition to the base pairs.
