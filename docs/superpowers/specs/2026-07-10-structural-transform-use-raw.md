...
`preference:"transform", applied:true` is never persisted — once an apply restores the transform value the row is garbage-collected (absent == transform default, §7). A toggle-OFF of an `apply-pending` (never-applied) raw decision hard-deletes immediately (entity rows were never overlaid — already transform); a toggle-OFF of an `applied` raw decision writes `preference:"transform", applied:false` (durable `clear-pending`) so a reload after a failed re-sync still shows the crew data is pending-revert, never silently flipping to `transform-active` while the rows are still raw.
...
resolution?: {
  ...
  parsed:
    ...
    | { kind: "dates"; dates: DateOrderFields };   // the M/D/Y interpretation (mdyIso-derived)
  replacement:
    ...
    | { kind: "dates"; dmyDates: DateOrderFields };                     // DMY reinterpretation (dmyIso-derived)
};
...
- The committed write (in EITHER direction — the `preference:"raw"` set OR the `preference:"transform"` pending-revert) is **NOT rolled back** on a step-(2) failure — rolling it back would silently discard the admin's intent. It stays stored with `applied:false` and reaches its target state on the **next successful sync** (scheduled cron re-sync or a manual retry), same as any decision the admin sets between syncs.
- The step-(2) re-sync failure surfaces through the **existing re-sync entry's outcome telemetry + error path** (unchanged by this feature — the re-sync entry already reports its own success/failure); the toggle action returns a typed result distinguishing "saved + applied" from "saved, apply pending (re-sync failed: <reason>)".
- The UI (§8) derives its state durably from `preference`+`applied` (§3), so on a step-(2) failure it renders `apply-pending` (toggle-on) or `clear-pending` (toggle-off) rather than a bare success — the admin is never misled into thinking the crew page already reflects the change. It never shows `raw-active`/`transform-active` (the settled states) until the overlay has actually run (`applied:true`, or the row garbage-collected).
...
| Overlay field rewrite | room row `name`/`dimensions`/`floor` | hotel_reservation `guests` | `result.dates.{travelIn,set,showDays,travelOut}` ONLY (clock/free-text fields untouched, §6) |
...
- **Durable `preference`+`applied` state machine, BOTH directions (R7/R8):** toggle-ON writes `{preference:"raw", applied:false}`; a successful overlay rewrites `applied:true`. Toggle-OFF of an `applied:true` row writes `{preference:"transform", applied:false}` (NOT a hard-delete); a successful overlay restores the transform and GARBAGE-COLLECTS the row. Toggle-OFF of an `applied:false` row hard-deletes immediately. Assert reload-safe state derivation (§8) for all four persisted shapes → `transform-active`/`apply-pending`/`raw-active`/`clear-pending`. **Toggle-OFF failure symmetry (R8 crux):** a step-(2) re-sync failure after toggling OFF an applied row leaves `{preference:"transform", applied:false}` intact → asserts durable `clear-pending` after reload, NEVER silently `transform-active` while entity rows are still raw. Same for toggle-ON failure → durable `apply-pending`.
