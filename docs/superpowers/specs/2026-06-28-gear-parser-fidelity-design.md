...
- **Gate its emission on the GEAR date-grid signature, not tab presence.** In the anchor builder (`lib/drive/sourceAnchors.ts`), `gear_scope` resolves to the GEAR tab **only if that tab's grid contains the date-grid signature** (a `Rental Dates` row — the same coarse signature `parseGearTab` keys on). A GEAR-titled tab without a date-grid (RIA-style inventory/schedule content) → **no `gear_scope` anchor**. This keeps "anchor exists" ⇔ "a GEAR date-grid was parsed," which is exactly when room scope is GEAR-derived.
...
- `tests/parser/sourceAnchorsCorpus.test.ts`: `gear_scope` anchor present for a GEAR-date-grid show, **absent** for a GEAR-tab-without-date-grid show. Plus a GearSection regression test: an INFO-inline-with-GEAR-tab show (RIA shape) keeps scope cards on the INFO `rooms` anchor.
