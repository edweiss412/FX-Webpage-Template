...
Add a **form-layout fallback** to `parseEventDetails`: when the classic-header path harvests zero keys (the verified consultants case), locate the form block by **anchoring** on known event-field labels — a contiguous run of 2-cell `| label | value |` rows in which ≥3 col0 labels normalize to a `CANONICAL_KEY_MAP` key (`event.ts:60`/`EVENT_LABEL_VOCAB:107`) — then **harvest the ENTIRE contiguous run**, not only the known labels. Known labels → their canonical key; **unknown labels → `toCanonicalKey(col0)`** (`event.ts:244`, the exact normalize-and-keep fallback the classic path already uses at `:201`). This recovers the full intake form, not just the 6 vocab hits — Codex R3 verified the consultants form carries `Backdrop`, `Power Requirements`, `Internet Requirements`, `Recording`, `Podium for General Session`, `Stage Size for General Session`, `Onsite Equipment Storage`, `Additional Notes`, most of which are NOT in `CANONICAL_KEY_MAP`. The run is bounded by its own contiguity (ends at a blank/non-2-cell row), so it does not inherit the classic path's premature blank-line break, and the room-header terminators (`event.ts:165`) are tightened to fire only on an **ALL-CAPS** room-header shape (`GENERAL SESSION GRAND BALLROOM A/B`), never the title-case form field `General Session Room Name`. The classic-header path stays byte-for-byte unchanged when it already produces keys (negative-regression pinned: East Coast/RPAS).
...
**Scope-card source links — target the GEAR tab for GEAR-derived data (Codex R3-M3).** For the modern GEAR-tab family the A/V/L scope is parsed from the GEAR tab, but the existing `gear-scope-*` cards link to the `rooms` region → the **INFO** tab (`buildSheetDeepLink.ts:134-136`), which does not contain that equipment — a wrong source link that defeats operator verification. Fix:
- Add a new region `gear_scope` to `REGION_ANCHOR_SPEC` + `REGION_IDS` (`buildSheetDeepLink.ts`): `{ tabs: ["GEAR"], strategy: "whole-tab" }`. The generic anchor builder (`lib/drive/sourceAnchors.ts:182` iterates all `REGION_IDS`, first present+allowlisted tab wins) then emits a `gear_scope` anchor **iff the GEAR tab is present** — no anchor for legacy INFO-inline shows. (`GEAR` is already in `SOURCE_LINK_ALLOWLIST`, `:1`.)
- `CARD_REGION_MAP` gains `gear-scope-scenic` / `gear-scope-other` → `rooms` (the legacy fallback target).
- `GearSection` selects the scope-card source region **once per render**: `const scopeRegion = data.sourceAnchors["gear_scope"] ? "gear_scope" : CARD_REGION_MAP[\`gear-scope-${d.id}\`]`. GEAR-tab shows → all five scope cards link to the GEAR tab; legacy INFO-inline shows → the precise `rooms` (INFO row-label) anchor, unchanged. (Show-level is correct because GEAR-tab and INFO-inline scope are mutually exclusive per the verified fixtures.)
- `tests/parser/sourceAnchorsCorpus.test.ts` gains a `gear_scope` expectation (anchor present → GEAR tab when a GEAR tab exists; absent otherwise).
