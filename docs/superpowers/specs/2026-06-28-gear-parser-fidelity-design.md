...
1. **Per-item closed allow-list (highest priority).** Match the item text against the tight closed keyword allow-lists below. If matched → that discipline, **regardless of the active package bucket.**
   - `audio`: SPEAKER, CONSOLE, MICROPHONE, MIC, SNAKE, ANTENNA, QU32, QU24, AB168, KLA, K8, K10, GOOSNECK/GOOSENECK, AUDIO
...
2. **Active package bucket (fallback for allow-list misses only).** A row whose text exactly matches a closed **package header** sets the active bucket: `SOUND SYSTEM PACKAGE` → `audio`; `STAGE LIGHTING PACKAGE` / `(LED )UPLIGHTING PACKAGE` → `lighting`. An item with NO allow-list match inherits the active bucket (so `CABLING`, `SPEAKER STANDS`, `MOUNTING HARDWARE` under `SOUND SYSTEM PACKAGE` → audio). The bucket resets to none at each room boundary.
3. **`other` (catch-all).** No allow-list match AND no active bucket → `other` (truss, ZOOM/PTZ backup, misc) — nothing dropped (D2).

**Why this order (not bucket-first):** in `exporter-xlsx/consultants.md:166-169` the rows `COUNTDOWN CLOCK`, `DLP DATA PROJECTOR-BARCO W8`, projector lens, and `6'X10' … PROJECTION SCREEN` appear *after* `SOUND SYSTEM PACKAGE` (`:155`) and *before* `STAGE LIGHTING PACKAGE` (`:170`). A bucket-first rule would file these video items under audio. Allow-list-first puts them in `video` correctly. (The package header itself, e.g. `SOUND SYSTEM PACKAGE`, is a bucket-setter and is NOT emitted as an item.)
...
**Merge semantics (the safety guarantee).** Every harvested field — classic AND form — is written through the existing **sentinel-aware `writeField`/`exactReal` precedence** (`event.ts:171-177`): a real value displaces a sentinel; a sentinel never clobbers a real value. Consequences: (a) classic-only shows (East Coast/RPAS) re-harvest their own DETAILS block idempotently → identical key set + values (negative-regression pinned); (b) mixed shows (fixed-income) get the form's real `Opening Sizzle Reel = No` displacing the classic sentinel `Opening Reel = TBD` (both keyed `opening_reel`); (c) the form harvest can only ADD keys or UPGRADE sentinels — it can never remove or downgrade a classic real value. The run is bounded by its own contiguity (ends at a blank/non-2-cell row), so it does not inherit the classic path's premature blank-line break, and the room-header terminators (`event.ts:165`) are tightened to fire only on an **ALL-CAPS** room-header shape (`GENERAL SESSION GRAND BALLROOM A/B`), never the title-case form field `General Session Room Name`.
...
- **`tests/parser/event.test.ts`** (or extend `unknownSection.test.ts`) — consultants form-layout → `event_details.keynote_requirements === "TBD"`, **`event_details.opening_reel` contains the `consultants:264` "Opening Sizzle Reel" value** (`Available if needed…`), and the **whole-block harvest** recovers the unknown-label fields too — pin concrete keys derived via `toCanonicalKey` for `Power Requirements`, `Internet Requirements`, `Backdrop`, `Recording`, `Podium for General Session`, `Stage Size for General Session`, `Onsite Equipment Storage`, `Additional Notes` (assert ≥10 keys total AND each named field present, so a vocab-only implementation can't pass). Assert the terminator no longer cuts at `General Session Room Name`. **`Opening Sizzle Reel` alias + mixed-sheet precedence (Codex R1+R4):** assert the **fixed-income exporter** (classic `Opening Reel | TBD` at `:83` AND form-block `Opening Sizzle Reel | No` at `:233`) resolves `event_details.opening_reel === "No"` (the unconditional form harvest + sentinel-aware merge upgrades the classic sentinel) AND that later form-only fields are present — this fails if the form harvest is gated on zero classic keys. **Negative-regression:** East Coast (13 keys, `keynote="NONE"`, `reel="YES - LOOP VIDEO"`) and RPAS (17 keys, `keynote="TBD"`, `reel="MAYBE"`) classic-header shows have **identical** key sets + values after the unconditional harvest (idempotent re-harvest; sentinel-aware merge can't downgrade). Failure mode: whole EVENT block drop (current bug) + `Opening Sizzle Reel` miss + zero-keys gate dropping mixed-sheet form data + form harvest clobbering classic real values.
...
- **The form-layout harvest runs UNCONDITIONALLY** (after the classic pass), not gated on zero classic keys — required for mixed sheets (fixed-income). Safety is the sentinel-aware `writeField` merge (real displaces sentinel, sentinel never clobbers real), which makes classic-only re-harvest idempotent (negative-regression pinned). Settled in R4-H1; do not re-propose the zero-keys gate.
