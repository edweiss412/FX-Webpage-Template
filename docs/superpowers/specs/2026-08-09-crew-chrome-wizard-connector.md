...
- Blank/whitespace name (partial picker data is a real state — R1 F6): `avatarColor` already falls back to slate (DESIGN.md §1.4); initials fall back to a neutral glyph; the trigger's accessible name falls back to "Crew member, account menu" (no leading comma artifact); the identity header shows the role alone with NO separator. Blank/whitespace ROLE: header shows the name alone, no separator, no empty row — matching the shipped separator contract's partial-data behavior (`tests/components/identityChipSrSeparator.test.tsx` :106 region), which the menu inherits rather than re-decides.
