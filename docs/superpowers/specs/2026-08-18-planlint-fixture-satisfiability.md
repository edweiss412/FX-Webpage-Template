...
Three branches, and the claim set they draw from is closed (§1.1 item 1). For each enrolled block, the core evaluates these in order and emits **exactly one** outcome:

1. **The block did not run.** Its file is absent from the report; or no report was produced (the runner did not complete, timed out, was signalled, or wrote unreadable JSON); or the splice directory collided (§4.2 step 1); or the file is present with an EMPTY `assertionResults` array → **`FIXTURE_PROBE_UNVERIFIED`** (advisory), detail naming which of those it was. The last case is the measured collection family — unresolvable import, transform error, no test suite, and the outside-the-globs trap (§2.3, §2.4) — and it is the report's own statement that no test case existed, not an interpretation of one.
2. **A premise failed.** Any failure message on the file carries the sentinel `premise not met:` → **`FIXTURE_UNSATISFIABLE`** (fail), detail naming each such premise description. This is the arm's only hard verdict and the defect it exists to catch: a stated premise about the constructed fixture did not hold against the live tree.
3. **Otherwise → no finding.** The block ran and no stated premise failed. The arm asserts nothing further about it.
...
| `FIXTURE_PROBE_UNVERIFIED` | advisory | §4.3 — the block did not run, with which case named |
...
- AC-3: under `--exec-red`, §4.3 assigns exactly one outcome to every combination of (report present or absent, entries none or some, per-assertion statuses, failure messages sentinel or non-sentinel, file status), and no input produces a certificate: the only hard code requires the sentinel, and the only advisory requires the block not to have run.
- AC-4: the §2.4 historical pair reproduces — the r4 two-column header draws `FIXTURE_UNSATISFIABLE`, the merged three-column header draws nothing. The §2.5, §2.6, §2.7 and §2.8 shapes each draw NO finding, asserted explicitly; the §2.3 and §2.4 collection shapes draw the advisory.
