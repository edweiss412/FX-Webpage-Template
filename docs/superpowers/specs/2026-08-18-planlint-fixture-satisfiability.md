...
4. Removes the directory in a `finally`, so an exception, a timeout, or a signal cannot leave a file under `tests/`. **A removal that itself fails is RAISED, never swallowed** — the adapter throws and the CLI's outer handler turns it into exit 2, naming the path. The surviving directory holds test files the repo's own `BASE_INCLUDE` glob collects on the next full run. Since step 1's name is fixed, the NEXT invocation would at least refuse loudly on it — but that is the next invocation's signal, not this one's, and the run whose removal failed is the only place that knows it failed. A best-effort cleanup would therefore break this step's own promise silently in the run that broke it, which is the one thing this arm may never do.
