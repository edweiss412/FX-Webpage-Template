...
- **The sentinel is tested BEFORE the did-not-run branch.** An empty `assertionResults` array is the signature of a block that never ran (§2.3, §2.4) AND of one that ran far enough to fail a premise. Testing emptiness first would report "the block did not run" and suppress the true verdict on exactly the input this arm exists to catch.
