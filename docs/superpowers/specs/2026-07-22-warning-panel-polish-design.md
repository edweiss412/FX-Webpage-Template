...
- **Layer 2 (evasion tripwire):** every `severity: "info"` occurrence in those trees must be attributable to a Layer-1-recognized literal (same object literal). An occurrence the scanner cannot attribute — a factory, a variable severity, a spread — FAILS the test with "extend the scanner or register the code," so helper-based emitters cannot silently bypass discovery. False negatives are loud, not silent.
...
| Seam present ↔ absent (change 4, rides L↔S / S↔C) | instant — Silent is a render-time fact |
...
8. **Tap target.** Real-browser (same Playwright spec as item 6): for each inline section button, assert the effective hit area spans >=44px in BOTH dimensions — clicks at the visual center offset +/-21px vertically AND +/-21px horizontally from center still hit the same target (elementFromPoint resolves to the button or its overlay), including on the shortest label; plus a disjointness probe at the midpoint between two adjacent names (hits at most one target). A jsdom class assertion is only the mechanical-gate backstop, not the proof.
