...
**Staged byte-identity for changes 1+3 (why §1.1 item 7 holds without a gate input).** The split is prop-driven, and exactly ONE caller passes `followUpCopy`: `sectionWarningExtras.tsx:199`, inside `renderSectionExtras`, which is published-only by construction (`isPublished` guard at `sectionWarningExtras.tsx:149`; the staged wizard passes no `renderSectionExtras` at all). The staged caller `StagedReviewCard.tsx:521` passes no `followUpCopy` — its `followUp` is null before and after this change, so `popoverBody` is the bare `context` on both sides and the staged card DOM is byte-identical. §8.1 pins this with a staged-shaped fixture (no `followUpCopy`): trigger `aria-describedby` resolution and popover DOM structurally equal to the pre-change shape.
...
**Mount point — NOT inside the panel body.** The Silent state unmounts the ENTIRE panel-card subtree: `ModalSectionChrome` renders `{children}` only when `hasBody` (`step3ReviewSections.tsx:792-815`), and Silent sets `suppressPanelCard` (`ShowReviewSurface.tsx:265-269`). A status node inside `WarningsBreakdown` would therefore unmount on clean→Silent (announcement dropped) and freshly mount on Silent→clean (no initial announcement) — exactly the conditional-mounting failure `BulkIgnoreControls.tsx:173-174` warns about. So the live region mounts in `ShowReviewSurface`'s warnings-section block, a sibling OUTSIDE the chrome-suppressible card subtree (adjacent to where `renderSectionExtras` is invoked, `ShowReviewSurface.tsx:1076`), rendered whenever the trim gate is on. `ShowReviewSurface` owns every input the sentence needs (`visibleWarningRows(data.warnings, true).length`, `routedWarnings.here` / `.elsewhere` — the same derivations at `ShowReviewSurface.tsx:265-269`). The sentence builder is an exported pure helper so the copy contract is unit-testable without the tree.

**The sentence must change on every transition an ignore/un-ignore can cause, so it is a pure function of the full count tuple `(listed, here, elsewhere)`** — not of `listed` alone (a routed warn row's ignore never changes the listed count, because published rows are info-only per `lib/admin/visibleWarningRows.ts:18-24`). Builder: concatenate the applicable parts, space-separated, never empty. Exact copy, both grammatical numbers:

| Part | n = 1 | n > 1 |
|------|-------|-------|
| listed > 0 | `1 warning listed.` | `{n} warnings listed.` |
| here > 0 | `1 warning needs a look below.` | `{n} warnings need a look below.` |
| elsewhere > 0 | `1 warning needs a look in its own section.` | `{n} warnings need a look in their own sections.` |
| all zero | `Nothing needs a look on this sheet.` | — |

Distinct count tuples produce distinct text, so ANY single ignore/un-ignore (which changes exactly one bucket count) changes the text and announces. The Silent state announces its card count rather than going empty — an earlier draft emitted `""` there, which made the un-ignore transition clean→Silent announce nothing. `role="status"` (implicit polite) announces only when the text CHANGES after mount; initial mount announces nothing.

**Guard conditions.** Gate off → span absent (staged byte-identity; the staged wizard mounts `ShowReviewSurface` without `renderSectionExtras`/`routedWarnings`, so the gate is false there — `ShowReviewSurface.tsx:250-251`). All-ignored vs no-warnings both land in the clean sentence — the same pair the visible copy deliberately does not distinguish (`step3ReviewSections.tsx:2572-2573`).
...
**Map maintenance contract (structural defense, ships in this bundle).** A NEW info-severity `ParseWarning` code must decide its actionability at introduction. A bare code-literal scan (the `dataGapsClassCompleteness.test.ts:164-199` shape) cannot carry severity — restricted even to today's two emitter files it false-positives four warn codes (`UNKNOWN_STAGE_RESTRICTION`, `ROLE_TOKEN_AUTOCORRECTED`, `UNKNOWN_ROLE_TOKEN`, `FIELD_LABEL_AUTOCORRECTED` all appear as literals in `lib/parser/personalization.ts` / `lib/parser/blocks/venue.ts`). The registry test is therefore a **two-layer fail-closed scan** over `lib/parser/**` and `lib/sync/**` source text (AST-based, per the repo's existing AST-guard precedent for code stamps):

- **Layer 1 (discovery):** find every object literal that contains BOTH a `severity: "info"` property and a `code: "<LITERAL>"` property; the discovered code set must EQUAL the map's key set. Today that discovers exactly `DAY_RESTRICTION_DOUBLE_LOCATION` and `TYPO_NORMALIZED`.
- **Layer 2 (evasion tripwire):** every occurrence of a `severity` PROPERTY KEY (any value form, not just the literal `"info"`) in those trees must have a literal `"warn"` or `"info"` value; a `severity` key with a non-literal value (identifier, call result, conditional, spread source) FAILS the test with "unanalyzable severity: extend the scanner or register the code." Every literal-`"info"` occurrence must additionally sit in an object literal carrying a literal `code` (Layer 1 attribution) or FAIL. So variable-severity emitters and factories that name a `severity` key anywhere are caught loudly; the residual boundary — an emitter that constructs the property key itself dynamically, never writing `severity` in source — is stated as out of the scanner's reach and covered by the type system alone (`ParseWarning.severity` is the closed union at `lib/parser/types.ts:49`). False negatives inside the scanner's syntactic reach are loud, not silent.
...
**Guard conditions.** `onJumpToSection` absent (standalone/legacy mounts that pass no chrome, or chrome without the callback) → names render as plain bold text, not buttons (progressive: sentence still informative). Empty `activeWarningsBySection` minus `warnings` while `elsewhere > 0` cannot occur (both derive from the same loop, `routedWarnings.ts:48-55`); the builder still guards: zero derivable names → fall back to today's exact sentence. Label lookup miss for a section id → that id is skipped from naming and its count folds into the terminal "and N more." clause (the same clause the cap uses, applied even below the cap; e.g. 1 resolved + 1 missed → "A and 1 more."); all-miss → fallback sentence.
...
This surface ships no animations today and this bundle adds none; every entry below is **instant — no animation needed**, stated per pair as the inventory rule requires. The panel body has four states — List (L), Silent (S), Elsewhere-pointer (E), Clean (C), the explicit branches at `step3ReviewSections.tsx:2558-2588` — giving 4·3/2 = 6 pairs, every one reachable only through a server re-render (`router.refresh()`), never a client-side animation:

| Pair | Treatment |
|------|-----------|
| L ↔ S | instant — server re-render swaps body rows for suppressed card + cards below |
| L ↔ E | instant — rows swap for pointer sentence |
| L ↔ C | instant — rows swap for clean line |
| S ↔ E | instant — last here-card ignored/un-ignored |
| S ↔ C | instant — last routed card ignored/un-ignored |
| E ↔ C | instant — last elsewhere card ignored/un-ignored |
| Callout present ↔ suppressed (change 5, within L) | instant — same server re-render |
| Seam present ↔ absent (change 4, rides every pair entering or leaving S: L↔S, S↔E, S↔C) | instant — Silent is a render-time fact |
| Pointer linked-set content change (change 6, within E: a named section's last card ignored) | instant — sentence re-renders with the new set/cap |
...
| Live region text change (change 2, rides every pair above) | instant text swap; sr-only, no visual |

**Compound transitions:** one `router.refresh()` can change the live-region text, the seam, the callout, the pointer's named-section set, AND the body state in the SAME commit — they are one synchronous React render of new server props, so no ordering, tearing, or mid-transition interleaving is possible; each compound is as instant as its parts. No state can change while another is mid-transition because nothing transitions over time.
...
3. **Live region (change 2).** RTL: gate ON, render at a count tuple, re-render varying EACH bucket independently — `rows.length` change, `here` change (routed warn card ignored below), `elsewhere` change (routed warn card ignored in another section) — each produces a text change (jsdom asserts text content, not announcement — real announcement behavior is the pattern's contract per `BulkIgnoreControls.tsx:173-177`). Re-rendering with changed props IS the production path: `router.refresh()` (`DataQualityWarningControls.tsx:56`) delivers new server props to the same mounted tree. The mount test renders through the REAL suppression path — `ShowReviewSurface` (or the section block that owns the span), not a bare `WarningsBreakdown` — and asserts the SAME status node instance survives clean→Silent and Silent→clean (the panel-card subtree unmounts at `step3ReviewSections.tsx:792-815`; the span must not live inside it). Non-empty across all states including Silent; un-ignore clean→Silent changes text (the `""`-in-Silent bug this spec's first draft had). Gate OFF → absent entirely. Singular/plural boundary at n=1 per part, asserting the exact strings from the §3.2 copy table.
...
6. **Pointer links (change 6).** RTL with chrome callback spy, asserting the FULL rendered sentence string (textContent equality — punctuation, separators, terminal period all pinned, not fragment matches): 1 section → single name, no comma/"and"; 2 sections → "A and B."; 3 sections → "A, B, and C." with NO "more" suffix (cap boundary); 4 sections → "A, B, C, and 1 more." (first overflow); buttons fire the callback with the right `SectionId` in registry order; partial label-lookup miss → the resolved names render under the normal punctuation rules for their count and the missed count folds into the same terminal "and N more." clause even below the cap (exact pinned case: 1 resolved + 1 missed → "A and 1 more."); ALL labels miss → today's exact fallback sentence; callback absent → no buttons, bold text only. Real-browser (Playwright, existing published-modal e2e harness): FIRST assert the target section's container is NOT at the aligned scroll position (pre-click guard so "becomes visible" cannot pass vacuously), then tap, then assert the container reaches the aligned position (`getBoundingClientRect` delta within tolerance), using the harness's hydration gate (`waitForRowHydration`-class), never `networkidle` alone; detach-safe locators.
...
8. **Tap target.** Real-browser (same Playwright spec as item 6): for each inline section button, assert the effective hit area spans >=44px in BOTH dimensions — clicks at the visual center offset +/-21px vertically AND +/-21px horizontally from center still hit the same target (elementFromPoint resolves to the button or its overlay), including on the shortest label. Disjointness is proven geometrically, not by elementFromPoint (which returns one topmost element by definition and cannot detect overlap): compute each button's effective overlay rect from its `getBoundingClientRect()` center plus `getComputedStyle(el, "::before")` width/height, and assert numerically that adjacent overlay rects do not intersect. A jsdom class assertion is only the mechanical-gate backstop, not the proof.
...
No new persisted config fields. Every new boolean/optional surface, full lifecycle:

| Surface | Storage | Write path | Read path | Effect on output |
|---------|---------|-----------|-----------|------------------|
| `opts.seamless` (change 4) | none — per-call argument | `ShowReviewSurface.tsx:1076` call site (`s.id === "warnings" && suppressWarningsPanelCard`) | `renderSectionExtras` container class branch | drops `mt-3 border-t border-border pt-3` in the Silent state |
| `afterBodyText` (changes 1+3) | none — prop | `CompactAlertHelp` forwards `followUpCopy` | `HoverHelp` popover render + describedby ternary | second paragraph outside the description |
| `followUpCopy` (changes 1+3) | none — prop | `PerShowActionableWarnings` per-card | `CompactAlertHelp` | feeds `afterBodyText` |
| `onJumpToSection` (change 6) | none — context field | `ShowReviewSurface` supplies `handleNavClick` | pointer-sentence buttons | scroll-to-section on tap |

No column is empty for any row — no zombie surface.
