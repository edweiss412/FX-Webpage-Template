...
- Widen `GallerySwitcherScenario.tier` to `1 | 2 | 3` (`galleryModalTypes.ts:35`), `SwitcherControls` `tier` prop to match (`SwitcherControls.tsx:35`), and drop the now-wrong narrowing cast `tier: s.tier as 1 | 2` in `partitionScenarios` (`buildSwitcherScenarios.ts:61`). The tier chip renders "tier 3" with no other change.
...
| --- | --- | --- | --- |
| `T2_CLASS_MIX` | `t2-class-mix` | 3 alerts selected by the PILL's real predicates, not the routing ones: one with `actionable === true`, one deriving `clearingKind === "needs_look"`, one deriving `clearingKind === "self_heal"` (note `isAutoResolving` ≠ `isSelfHealing` — audience.ts:63 vs :102 — so a naive `pickCode("auto")` can still class as needs_look and collapse the pill to two segments). Selection helper classifies candidates by the DERIVED item's `actionable`/`clearingKind` (run each candidate through `deriveScenarioAttention` on a one-alert probe scenario) at runtime and throws if any class empties. The self-heal class is non-empty on today's catalog: e.g. `SYNC_STALLED` is `audience: "doug"` / `resolution: "auto"` (`lib/messages/catalog.ts:2367-2371`) and in `SELF_HEALING_CODE_LIST` (`lib/adminAlerts/audience.ts:73-77`), so it reaches the menu uncut. Pill renders all three segments ("to confirm" / "to review" / "monitoring", `components/admin/showpage/PublishedReviewModal.tsx:307-320` and `components/admin/showpage/PublishedReviewModal.tsx:760-802`). | Composite pill with all three segments live. |
...
| `t2-degraded-with-holds` grouping | degraded + 1 hold | Hold item derives `sectionId: "changes"` (`lib/admin/attentionItems.ts:309`) → group `changes`, NOT baseline — `degraded` never forces the baseline group; only an empty section set does. |
...
R1 (2026-07-22): cross-model dispatch died 6 times with zero reviewer output — codex-guard 3 attempts (`no_verdict`, `no_o_file` each) and inlined direct dispatch 3 attempts (prompt echoed, zero assistant tokens; cache-clear + stdin-pipe + spec-only-size variants all tried). Per the no_verdict ladder (AGENTS.md codex-guard contract + the #546 both-shapes-die precedent), this round is SELF-CERTIFIED: a structured adversarial self-review ran against the six focus vectors (grouping well-definedness, selection-helper guarantees, feedTruncated leaks, rich-fixture underspecification, tier-type consumers, guard-table completeness), with the two load-bearing claims re-verified against live code (hold `sectionId: "changes"` at `lib/admin/attentionItems.ts:309`; `SYNC_STALLED` doug-audience self-heal surviving the surface cut at `lib/messages/catalog.ts:2367-2371`). Findings repaired in this commit: (1) missing `tier: s.tier as 1 | 2` cast edit site enumerated; (2) degraded-with-holds grouping guard row added (degraded never forces baseline); (3) class-mix selection pinned to derived `actionable`/`clearingKind` with the surviving-self-heal citation. Cross-model review of the FULL IMPLEMENTATION DIFF at Stage 4 remains mandatory and will re-cover this spec's claims.
