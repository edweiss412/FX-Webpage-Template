...
**Acceptance predicate (must NOT be weaker than the existing #6 fallback).** Refactor the existing #6 guard (`index.ts:202-209`) into a shared helper `isAcceptableTitleCell(cell)` used by BOTH #0 and #6, so they can't drift. It rejects: empty; `isKnownNonTitle(cell)`; `cell.toUpperCase().startsWith("CLIENT")`; `startsWith("NO_HEADER")`; `cell === "\\#NUM\\!"`; `/^\\#/.test(cell)` (escaped error cells); **AND `KNOWN_SECTION_HEADERS.has(normalizeHeader(cell))`** (exact membership from `knownSections.ts` — catches `DOCUMENT FOLDER LINK`, `DETAILS`, `VENUE`, `DRESS`, `HOTEL`, etc.). **The section-header rejection lives in the SHARED helper, not just #0 (round-6 fix, Codex spec-R6)** — otherwise a duplicated `| DOCUMENT FOLDER LINK | … |` rejected by #0 would still be returned by the unguarded #6. Use **exact** `KNOWN_SECTION_HEADERS.has(normalizeHeader(cell))` for bare headers, NOT the prefix-tolerant `isKnownSectionHeader`, so a legitimate title that merely starts with a section-family word (e.g. "Breakout Innovation Summit") is not over-rejected. **PLUS a suffixed-room-header rejection DERIVED from `PREFIX_SECTION_FAMILIES` (round-7 fix tightened in round-8, Codex spec-R7/R8):** build the regex from the live family set so it can never drift from the registry —
```
const SUFFIXED_ROOM_HEADER_RE = new RegExp(
  '^(' + [...PREFIX_SECTION_FAMILIES].map(escapeRegExp).join('|') + ')\\b(\\s*\\d+)?\\s*[-–—]', 'i'
);
```
`PREFIX_SECTION_FAMILIES` (knownSections.ts) = {GENERAL SESSION, BREAKOUT, **BREAKOUTS**, ADDITIONAL ROOM, LUNCH ROOM, LUNCH SESSION}. A cell is a suffixed room header iff it starts with a family token, then an optional number, then a **separator** (`-`/`–`/`—`). This rejects `GENERAL SESSION - GRAND BALLROOM A/B`, `BREAKOUT 2 - SALON C`, `BREAKOUTS - SYNTHETIC ROOM`, `LUNCH SESSION - GRAND BALLROOM C`, `ADDITIONAL ROOM - X` — **including `BREAKOUTS`, which the earlier hardcoded regex missed**. The required-separator discriminator preserves "Breakout Innovation Summit" (family `BREAKOUT` + space + word, no number, **no separator** → not matched). Bare family headers (no suffix) are already caught by the exact `KNOWN_SECTION_HEADERS.has(...)` membership. Because both the guard and the structural test below are derived from the *same* `PREFIX_SECTION_FAMILIES`, they cannot disagree. So a no-banner sheet whose first row is a duplicated **section header** — bare (`| CLIENT | CLIENT |`, `| DOCUMENT FOLDER LINK | … |`, `| \#NUM\! | … |`) OR suffixed-room (`| GENERAL SESSION - GRAND BALLROOM A/B | … |`) — is rejected by BOTH #0 and #6 and falls through to the filename fallback (#7). Only after passing all guards is a duplicated `cells[0]` returned. This signature:
...
4. **Title banner** (`tests/parser/...`): exact assertions — consultants → `AII/III - Consultants Roundtable 2025`; fintech/fixed-income/rpas → proper-cased banners (not uppercase). Unchanged assertions: east-coast, ria, redefining-fi titles equal their current values (snapshot the current value first, assert preserved). Generic guards (parseSheet.test.ts:44/54) still pass. Negative: revert priority #0 → consultants title reverts to uppercase. **Duplicated-section-header negatives (Codex spec-R5/R6/R7):** inline no-banner fixtures whose first table row is `| CLIENT | CLIENT |`, `| DOCUMENT FOLDER LINK | DOCUMENT FOLDER LINK |`, `| \#NUM\! | \#NUM\! |`, **and a suffixed room header `| GENERAL SESSION - GRAND BALLROOM A/B | … |` (and `| BREAKOUT 2 - SALON C | … |`)** → assert the **final `show.title`** (via `parseSheet`, not just `#0` in isolation) is NOT that header — it falls through to the filename fallback (#7). This proves BOTH #0 and the shared #6 reject bare AND suffixed room headers. Positive: a title that merely starts with a section-family word but is NOT a room header ("Breakout Innovation Summit", duplicated banner) IS accepted (exact-membership + `BREAKOUT\s+\d` guard, so no over-rejection).
5. **Meta-test** (`_metaKnownSectionsRegistry`): `DRESS` registered (passes with the registry as-is).
6. **Structural title-guard test (class-level defense, Codex spec-R5/6/7/8 convergence):** `it.each` over **every** `KNOWN_SECTION_HEADERS` entry (bare) AND every `PREFIX_SECTION_FAMILIES` entry rendered with the **separator** suffix the derived guard keys on (`${fam} - SYNTHETIC ROOM` and `${fam} 2 - SYNTHETIC ROOM`, the latter covering the numbered-breakout shape) → build a minimal no-banner sheet whose first table row is that header **column-duplicated** → assert `parseSheet(...).show.title` is NEVER that header (falls through to filename fallback). Both the guard and this test import the SAME `PREFIX_SECTION_FAMILIES`/`KNOWN_SECTION_HEADERS` from `knownSections.ts`, so they cannot diverge and a future registry addition (e.g. a new `BREAKOUTS`-like entry) is automatically covered. Companion positive (must still pass): `Breakout Innovation Summit` (duplicated banner, no separator) IS accepted as the title.
