...
| Resolved head map has an **extra** name `ls-remote` does not | 2 | the deletion direction: a stale cached ref keeps declarations alive that can raise a false collision |
| Resolved head map is **missing** a name `ls-remote` advertises, with every shared OID matching | 2 | R13 finding 2: the remote-only direction, isolated. An implementation that validates intersection OIDs and rejects local extras but never checks for remote-only names passes every other membership row here while trusting a narrowed cache that omits the claimed branch — which is the configured-refspec case R10 finding 1 raised |
...
- Corpus regression bound: asserted against a **committed fixture corpus**, never against origin, so
  ...
  fixture required not to. Detection is **5 of 6** — R13 finding 1 caught the aggregate still reading 4 of 5 after the collision fixture made it six. Everything detects except the deep-quoted bare
