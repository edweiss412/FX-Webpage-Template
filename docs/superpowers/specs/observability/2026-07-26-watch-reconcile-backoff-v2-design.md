...
- **Settings copy amendment (2026-07-27, impeccable audit P1):** `DriveConnectionPanel` is a SERVER component - `toLocaleString` with an undefined locale renders the SERVER's timezone permanently, and `suppressHydrationWarning` is inert with no client render to correct it. The Settings sentence therefore renders the wait RELATIVE to the page's injected `now` (`Trying again in 15 min · ...`; past/unparseable/missing → `Trying again shortly`), via a module-local `formatWaitUntil(iso, now)`. The bell line keeps the absolute local time - its rows are client-fetched and format in the viewer's browser. The two surfaces deliberately differ in time phrasing because their rendering contexts differ; both keep the identical count clause.
