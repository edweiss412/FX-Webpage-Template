...
**Rendering (both surfaces, same sentence).** A `<p>` mirroring the bell auto-resolve note's shape (`components/admin/BellPanel.tsx:355-360`): a `data-testid`, plus the note's typography classes `wrap-break-word text-sm text-text-subtle`. The note's remaining class, `ml-auto`, is its OWN flex-row right-alignment and is deliberately NOT copied — the new line is a block-level sibling in the cell's column flow (spec R3 finding 5: the earlier "exact shape" claim omitted `ml-auto` and is corrected to this split). Rendered only when the ladder is actually in play:
...
6. Classes 11/12/18 (components + production data path): as specified in §3.6, including the never-render-error-fields scan and the anti-tautology clause (clone subtree, strip the alert's cataloged copy, derive expected time through the same formatter). Class 18 additionally carries three pins:
   - the `infra_error` → hidden mapping is asserted at the LOADER layer, where it lives (spec R3 finding 3): `bellFeed` with a failing state read → `watchState: null`, feed still renders; the Settings page read likewise → prop `null`, panel renders.
   - **the production opt-in is pinned end-to-end** (spec R3 finding 2 — a deps-injected spy replaces the whole function and cannot see the option): reconcile run with its DEFAULT subscribe binding and an injected failing `watchFolder` → a real `drive_watch_reconcile_state` row with `consecutive_failures = 1`; the Retry action likewise through its real call. Either test fails if the production call site drops `recordAttempt: true`.
6a. Class 19 re-derived — the deferred version keyed cases on `ReconcileOutcome` values this table no longer stores (spec R2 finding 3). The normative case list is the §3.6 render table over the COMPONENT contract (`WatchSurfaceState | null` — `infra_error` never reaches a component, it is mapped at the loader, class 18): `failed` + future `nextAttemptAt`; `failed` + past; `failed` + null; `succeeded`; state `null` — the last two assert the line ABSENT; plus the count-clause guards (0 omitted, 1 singular). Anti-tautology clause unchanged.
...
8. Class 20: `watch_backoff_ms` ↔ `BACKOFF_LADDER_MS` parity (real DB) at the SAME case set §3.3 names for the function contract — `n = 0, 1, 2, 3, 4, 5, 8, null` — against the independent literal table, with the `0`/`null` defensive-floor rows included (spec R3 finding 4 unified the two lists).
