...
**Rendering (both surfaces, same sentence).** A `<p>` with a `data-testid` and the bell auto-resolve note's typography classes `wrap-break-word text-sm text-text-subtle` (`components/admin/BellPanel.tsx:355-360`). Two of the note's own layout facts do NOT carry over (spec R3 finding 5, spec R4 finding 4): `ml-auto` (the note's right-alignment) is not copied, and the new line is NOT a block-flow sibling — both target parents are flex rows, so the `<p>` additionally carries **`w-full`**, which on a wrapping flex row forces it onto its own visual line. Rendered only when the ladder is actually in play:
