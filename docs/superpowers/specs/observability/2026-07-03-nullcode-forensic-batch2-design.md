...
**(b) Per-call anchored structural guard (closes Codex spec-R1/R2/R3 HIGH — pins each code to its EXACT intended call; class-closing for all 35).** The R2 strip-check proved "inside *some* log span"; it did not pin *which* span, leaving four modes open (wrong call in the same file, several codes crammed into one span, duplication, or placement in a `log.info`/`log.debug`/`logAdminOutcome` span instead of the intended `log.error`/`log.warn`). This guard closes all four by pinning each code to its intended call via a **message anchor + level**, and by a **global uniqueness** count.

Registry rows carry four fields: `NULLCODE_BATCH2_STAMPS: ReadonlyArray<{ file: string; code: string; level: "error" | "warn"; anchor: string }>` — `anchor` is a unique substring of the intended call's **message argument** (the verbatim message snippets captured during re-verification; e.g. `"app_events read returned error"`, `"wizard staged apply: unexpected failure"`). All 35 rows enumerated in the plan.

The test includes a small local `collectLogSpans(src): { level: string; text: string }[]` helper — a positive-extraction sibling of `stripLogEmissionCalls` (same balanced-paren + string/comment-aware tokenizer walk, but it **collects** each `log.<level>( … )` / `logAdminOutcome( … )` span's `{ level, text }` instead of deleting it). (The plan gives the full helper; it is ~25 lines mirroring `stripLogEmissionCalls`.) Then for **every** registry row, assert:

1. **Exactly one** span `S` in `collectLogSpans(src)` satisfies `S.level === row.level && S.text.includes(row.anchor) && S.text.includes('code: "<CODE>"')` (single- or double-quoted). This pins the code to the *intended* call (the anchor identifies it) at the *intended* level — a code in the wrong call lacks that call's anchor; a code in a `log.info`/`logAdminOutcome` span has the wrong `level`; several codes crammed into one span each fail their *own* anchor match.
2. **Global uniqueness:** the code literal `"<CODE>"` occurs **exactly once** in the whole file source (`src.split('"<CODE>"').length - 1 === 1`, plus the single-quote variant summing to 1) — catches duplication across two spans.

Together (1)+(2) prove each of the 35 codes sits in exactly one place: its intended `log.error`/`log.warn` call, matched by that call's own message. This uses the canonical span tokenizer (not fragile line/regex matching) and is robust to drift.

**Residual (fields-arg vs message-arg within the *right* call) — one site:** (1) pins the code to the correct call+level but can't distinguish the *fields* object from the *message* object when the message is itself an object literal. That is true for exactly one site — `selectIdentity.ts:56`, whose message arg is `JSON.stringify({…})`. For the other 34 the message arg is a plain string literal, so a `code:` object-property inside the pinned span can only be the fields arg. `selectIdentity` gets the decisive runtime test in §8.2.

Cross-guard: assert the `NULLCODE_BATCH2_STAMPS` code set == the 35 codes added to `NEW_FORENSIC_CODES` in (a) (no drift), all 35 distinct, and each `level` matches §3's table.
