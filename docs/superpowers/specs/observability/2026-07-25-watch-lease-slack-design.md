...
1. **Constants and the §2.1 ordering** (`tests/drive/**` + `tests/cron/**`, DB-free). `T_EXEC_BUDGET_MS` and `SAMPLING_PERIOD_MS` are pinned against the canonical registry `pg-cron-jobs.json` (`tests/cron/samplingPeriodParity.test.ts`), and against the LIVE `cron.job` row in `tests/cross-cutting/pg-cron-coverage.test.ts` wherever a database is reachable. **Not** parsed out of the migration SQL: an earlier draft did that, and whole-diff rounds R8-R17 each found another lexical corner where a hand-rolled scanner silently read the wrong value, so the scanner was deleted rather than widened a tenth time (see that test file's header for the full list). Two gaps are accepted and filed rather than papered over: `BL-PG-CRON-COVERAGE-UNRUN` (the live suite runs in no CI workflow) and `BL-CRON-REGISTRY-MIGRATION-PARITY` (no CI-running check ties a NEW migration to the registry). Also asserted: the orderings the heuristic depends on — `renewalLeadMs(WATCH_TTL_MS)` and `RENEWAL_MIN_LEAD_MS` both exceed `P + 2T`, which itself exceeds one bare `P`. **No phase sweep and no worst-case-margin assertion**: both were deleted with the guarantee they belonged to. `renewalLeadMs` has direct coverage for its proportional arm, its floor arm, and totality over NaN/Inf/negative/zero.
