...
<!-- spec-lint: not-ui — app/auth/callback/route.ts and app/auth/sign-out/route.ts are cited as log.* call sites only; no rendered UI surface changes -->
...
The non-`Error` branch collapses every plain object to the literal `"[object Object]"`. Supabase/PostgREST returned-errors are exactly that shape: `processResponse` in the installed postgrest-js package (version 2.105.1) builds the returned `error` as `JSON.parse(body)` (or hand-built `{ message, details, hint, code }` literals) and constructs a `PostgrestError` **class instance only on the `throwOnError` path** (its dist/index.cjs under node_modules, function `processResponse`: `error = JSON.parse(body)` … `if (error && _this2.shouldThrowOnError) throw new PostgrestError(error)`). Every **PostgREST** `{ data, error }` destructure in this repo therefore receives a plain object. Auth and Storage are the counterexample families (spec R2 F1, probed): `AuthApiError` and `StorageApiError` ARE `instanceof Error` with own enumerable status-class fields (probed live: Auth `["__isAuthError","name","status","code"]`; Storage `["__isStorageError","namespace","name","status","statusCode"]`), and live sites forward them to `log.*` (`app/auth/callback/route.ts:86` `getUser` returned error; `app/auth/sign-out/route.ts:96` `signOut` returned error) — they take the Error branch, whose own-enumerable-field expansion (§2.3) is what stops discarding their `status`/`code` today. Both families land in the blast surface (§2.5).
...
- `KEYS_MAX = 32` — per object, first `KEYS_MAX` own enumerable string keys in insertion order are kept; when any are dropped, one extra property `"~truncated": "<n> more keys"` is added. Each kept KEY is itself `.slice(0, STR_MAX)` (spec R3 F2 — an unbounded key defeats the total-size bound exactly like an unbounded value; probed with a 1,000,000-character key passing the prior text unsliced); two long keys identical through `STR_MAX` characters collide post-slice and last-write-win (documented limit 9's shape).
...
Accumulator objects are created with `Object.create(null)` (spec R3 F3): a plain `{}` accumulator loses an own `"__proto__"` key to the inherited setter — `JSON.parse('{"__proto__":{...}}')` carries one, the assignment silently sets the accumulator's prototype instead of a property, and the diagnostic vanishes without a marker (probed live: own-key count 0 after assignment on `{}`, `["__proto__"]` with the value intact and JSON-round-tripping on a null-prototype object). With a null-prototype accumulator, `"__proto__"` is an ordinary captured key, redacted and sliced like any other. Object-literal spreads elsewhere in the pipeline (`defaultSink`'s `compact`, the Error-branch triple merge) are safe as-is — spread uses CreateDataProperty semantics and never invokes the setter.
...
The `Error` branch becomes: bounded own-enumerable-field capture (same §2.2 mechanics, same budget) spread first, then `name`, `message` (sliced `STR_MAX`), `stack` (sliced `STACK_MAX`) written last so the protocol triple deterministically wins any collision (PostgrestError has an own enumerable `name`, probed §0). When `"cause" in error` and `error.cause !== undefined`, add `cause: <recursive serialization at child depth>` — `cause` is an own **non-enumerable** property on V8 `new Error(msg, { cause })`, so the enumerable spread alone would miss it, and `cause` is where Node fetch/system errors carry `ECONNREFUSED`-class detail. Same treatment for the other standard own **non-enumerable** payload field (spec R2 F2): when `Array.isArray((error as AggregateError).errors)`, add `errors: <recursive serialization of the array at child depth>` (bounded by `ITEMS_MAX`/`NODES_MAX` like any array) — `AggregateError.errors` is own non-enumerable in V8 (probed: `Object.keys` is `[]`, descriptor `enumerable: false`), so the enumerable spread misses it and the defining inner diagnostics would vanish without a marker. The standard-special-field class is closed by derivation for this runtime: Node 20's built-in `Error` own non-enumerable payload fields are exactly `cause` and `errors` (`SuppressedError` does not exist in this runtime); a future standard field files to documented limits until added. This closes the sibling loss: a Node `fs` error's `code`/`errno`/`path`, a `throwOnError` PostgrestError's `details`/`hint`/`code`, and an `AggregateError`'s inner errors all survive today's discard.
...
- **`app_events.context.error` payload shape** — was `string | {name,message,stack}`; becomes `SerializedError` (§2.1). This includes the Error-branch expansion for Auth/Storage returned-errors (§0): their persisted shape gains the probed own-enumerable field sets (§0: Auth `__isAuthError`/`name`/`status`/`code`; Storage `__isStorageError`/`namespace`/`name`/`status`/`statusCode`). `app_events.context` is unconstrained jsonb (`supabase/migrations/20260629000002_app_events.sql:14`, `context jsonb not null default '{}'`). Probed consumers: `rg "context\.error|context\['error'\]" lib/observe/ app/admin/` → zero matches; `rg "'error'|\"error\"" lib/observe/query/` → zero matches. The observe CLI renders context generically. No mechanical consumer breaks; human readers get strictly more.
...
- **AC-6** Error branch: `new TypeError("boom")` keeps the `{name, message, stack}` triple (stack sliced `STACK_MAX`); an Error with own enumerable `code`/`details` carries them; `new Error("x", { cause })` carries the recursively serialized `cause`; `new AggregateError([inner], "batch")` carries the recursively serialized `errors` array; an own enumerable `name` collision loses to the protocol `name`.
...
9. **Key-collision edge:** two distinct email keys redacting to the same `[email-redacted]` key, or two long keys identical through `STR_MAX` characters slicing to the same key, last-write-win; a signaled shape (the marker key is visible), never silent leakage.
