...
14. **Class B mock returns are real union members, typed to the dependency signature (Codex plan-R4 MEDIUM, closed in plan Tasks 5-8).** B1 applied carries `syncAuditId`/`derivedSideEffects`; B2 applied carries `parseWarnings`; B3 `not_stuck` carries `snapshotRevisionId`; B1/B4 failures are `not_found` (not a fictional `outcome:"x"`). Mocks are typed to the exported signatures so drift fails `typecheck`. Do not propose partial/ad-hoc mock shapes.
