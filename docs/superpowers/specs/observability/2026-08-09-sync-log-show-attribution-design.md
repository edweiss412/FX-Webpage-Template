...
| **Pruning `sync_log` discards nothing anyone is keeping.** `sync_audit` is the durable record of what was applied to a show (`supabase/migrations/20260501001000_internal_and_admin.sql:204-217`) and is **not** pruned by this change. Master spec `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:986` describes `sync_log` as "per-attempt and not surfaced as an alert"; `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:182` describes its rows as single-attempt outcomes distinct from show-level status. | Master spec `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:182`, `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:986`. |
...
| **Duration stays scoped to the `logSync` helper.** The other writers carry NULL by design and are enumerated in §3.3.1. Extending timing to them is out of scope, not an oversight. | This spec, §3.3.1 and §7. |

---

## 2. Goal and acceptance posture

**Goal.** Every `sync_log` row that *can* name its show does; every row written through the `logSync` helper records how long the attempt took (§3.3.1 lists the writers that carry NULL by design); per-show and per-file lookups are indexed; the table stops growing without bound.
...
**What `duration_ms` measures, stated precisely (R9 F3).** It is the **runner** boundary, not the whole operator-perceived attempt. For the first-seen retry path the route completes `prepareFirstSeenStage` — sheet export, parse, Drive enrichment (`app/api/admin/pending-ingestions/[id]/retry/route.ts:155`) — *before* invoking the timed runner at `app/api/admin/pending-ingestions/[id]/retry/route.ts:480`, so a slow export followed by a fast apply records only the fast tail. An injected-delta assertion inside the runner still passes, so the testing posture cannot detect the shortfall; that is why this is documented rather than asserted away.

This is a DOCUMENTED LIMIT, not a defect to fix here: moving capture ahead of preparation means threading a start through the route layer, which is plumbing this change does not otherwise touch. Readers of `duration_ms` should treat it as "time in the sync runner", and the column is not a latency metric for the operator's click.
...
Shape and naming mirror the established convention for this exact access pattern: `sync_audit_show_id_idx on public.sync_audit (show_id, applied_at desc)` (`supabase/migrations/20260501001000_internal_and_admin.sql:218`), `reports_show_id_idx ... (show_id, created_at desc)` (`supabase/migrations/20260501001000_internal_and_admin.sql:323`), and `app_events_show_id_idx on public.app_events (show_id, occurred_at desc)` (`supabase/migrations/20260629000002_app_events.sql:19`). `if not exists` follows the newer-migration convention (`supabase/migrations/20260629000002_app_events.sql:19-21`).
...
The structural guard that would enforce §3.2's rule at CI time is **not part of this change**. It is filed as `BL-SYNC-LOG-ATTRIBUTION-METATEST` with its design work preserved **and two definitional questions still open** — the signature-keyed accept-set is undecidable in both readings, and the three markers are not disjoint in the current tree. Those are the first work on that entry, not settled design.
...
So this change ships a new `syncLogRepairSites` suite under `tests/sync/` (a deliverable, not a citation): a **fixed list** of the fifteen sites this PR repairs, asserting each passes a `drive_file_id` or installs a sink. It is deliberately an enumeration, and that is not a relapse — the anti-enumeration lesson of this arc applies to **covers**, which claim completeness and rot when a site is added. This claims nothing about completeness. It pins fifteen specific repairs made in this diff, which is exactly what a regression test is for, and it is decidable with no accept-set, no marker taxonomy, and no writer derivation — the three things whose definitions would not converge (R14 F2, F3).
...
| Inline CHECK | N/A — no CHECK changes. `sync_log_drive_file_id_nonblank` (`supabase/migrations/20260702120200_drive_file_id_nonblank.sql:69-70`) is unaffected; nothing here writes `drive_file_id`. |
| RLS / grants | N/A — unchanged. `admin_only` policy at `supabase/migrations/20260501002000_rls_policies.sql:67-70`; the Aug-3 lockdown revoked only INSERT/UPDATE/DELETE (`supabase/migrations/20260803000000_lockdown_admin_only_tables.sql:46`), leaving SELECT granted. |
...
**The staged-apply surface is the same class (R10 F1).** `lib/sync/applyStaged.ts:1468-1498` imports and invokes `emitSuccessfulPhase2Tail`, so the descoped guard's writer-reachability rule would have classified it as an entry point — and on a live first-seen review the tail receives only `upsertAdminAlert` (`lib/sync/applyStaged.ts:1485-1493`): no sink, no start, no clock. **Two** live-capable callers reach that tail without tail dependencies — `app/api/admin/staged/[fileId]/apply/route.ts:152` (can select `"live"`, omits the argument) and `app/api/admin/show/staged/[stagedId]/apply/route.ts:164` (fixes `"live"`, passes `{}`). Those two take the gap marker.

**The wizard route does NOT (R11 F1 / R12 F1).** `app/api/admin/onboarding/staged/[wizardSessionId]/[driveFileId]/apply/route.ts:153-171` fixes `sourceScope: "wizard"`, and `applyStaged` routes that branch to `applyWizardWithDriveReverify` (`lib/sync/applyStaged.ts:2047-2066`); the writer-reaching tail is confined to the live path (`lib/sync/applyStaged.ts:1944-1945`, `lib/sync/applyStaged.ts:1467-1498`). No sync attempt happens there, so a gap marker would falsely record `BL-MANUAL-SYNC-UNEMITTED` against a path that never attempts a sync. `firstPublishedTailDeps` occurs only inside `lib/sync/applyStaged.ts` — declared at `lib/sync/applyStaged.ts:369` and `lib/sync/applyStaged.ts:950`, threaded at `lib/sync/applyStaged.ts:986-988`, consumed at `lib/sync/applyStaged.ts:1485` — with **no production injection anywhere**.

That is a first-seen apply whose attempt is unemitted — the same defect as the manual paths, on a third surface, and it falls under the same ratified decision rather than being new scope. `BL-MANUAL-SYNC-UNEMITTED` is extended to name it.

**These sites carry an explicit marker, not silence.** Each gets `// sync-log-emission-gap: BL-MANUAL-SYNC-UNEMITTED` at the call, and the filed guard is specified to accept that marker as a third disposition alongside "attributes" and "run-level". The distinction matters: a run-level marker asserts *no show is knowable*, which would be false here — a staged apply knows its show perfectly well. This marker asserts *the attempt is not emitted at all, and the gap is filed*. Conflating them would launder a filed defect into a design claim.
