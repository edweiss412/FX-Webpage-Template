...
**Accept-set, keyed on type rather than spelling:** an exported function is a sync entry point iff it transitively reaches **any derived writer** (§3.6's writer set). Naming `processOneFile`/`processOneFile_unlocked` was too narrow (R9 F1): `runManualStageForFirstSeen` reaches neither — it calls `emitSuccessfulPhase2Tail` directly (`lib/sync/runManualStageForFirstSeen.ts:147`) — so the per-call rule would have missed the production call at `app/api/admin/pending-ingestions/[id]/retry/route.ts:488`, contradicting §6.2's promise that manual paths receive a sink.
