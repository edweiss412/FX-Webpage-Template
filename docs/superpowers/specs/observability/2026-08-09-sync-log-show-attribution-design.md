...
**Accept-set, keyed on type rather than spelling:** an exported function is a sync entry point iff it transitively reaches **any derived writer** (§3.6's writer set).
...
The consequence is handled at the site rather than in the recognizer: a call the rule admits but that cannot produce an attributable attempt carries `// run-level-sync-log: <reason>` with the reason stating why. That marker's meaning is therefore **"this call site produces no attributable sync attempt"** — of which "run-level, no file in scope" is the common case and "fixed to a scope that never reaches the writer branch" is another. It is deliberately NOT the emission-gap marker: nothing is missing here, so pointing at a filing would be false. Naming `processOneFile`/`processOneFile_unlocked` was too narrow (R9 F1): `runManualStageForFirstSeen` reaches neither — it calls `emitSuccessfulPhase2Tail` directly (`lib/sync/runManualStageForFirstSeen.ts:147`) — so the per-call rule would have missed the production call at `app/api/admin/pending-ingestions/[id]/retry/route.ts:488`, contradicting §6.2's promise that manual paths receive a sink.
