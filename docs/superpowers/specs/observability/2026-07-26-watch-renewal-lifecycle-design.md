...
Two changes, both small: `listGcCandidates` gains a deterministic `order by created_at` and a `limit` sized so a full pass of failures fits inside the cron window; and the GC loop checks the same run budget the renewal loop uses, stopping cleanly rather than being cut off mid-row. `expired` rows cost no Drive call at all (§3.1.4), so they are never the bottleneck — it is the retrying `superseded` rows that need the ordering.
