...
**Supersession alone does not satisfy AC-6.18 — a late activation defeats it.** Spec R2 finding 1: refresh or reconcile can commit an old-folder `pending` row and complete `files.watch`, then pause. Promotion updates only rows that are already `active` (the statement above), so it cannot see that pending row; afterwards `activatePending` promotes it to `active` without rechecking `app_settings` (`lib/drive/watch.ts:157-186`), the partial unique index is per folder so it permits the row, and the webhook ingress treats it as authoritative because it matches on `status = 'active'` alone (`app/api/drive/webhook/route.ts:79-90`). AC-6.18 fails under exactly the refresh/promotion concurrency this design is about.

Two changes close it, and both are needed:

1. **Promotion orphans the old folder's PENDING rows in the same transaction**, alongside superseding its active ones: `update … set status = 'orphaned' where status = 'pending' and watched_folder_id is distinct from <new id>`. `orphaned` rather than `superseded` because the row was never activated, which is precisely what `orphaned` already means on this table.
...
**Verified routing, and one second-order detail.** The throw from (2) is raised inside `activateWithTx`, which `subscribeToWatchedFolder` calls inside a `try` (`lib/drive/watch.ts:523`); the `catch` at `lib/drive/watch.ts:591` raises `WATCH_CHANNEL_ORPHANED` with `reason: "activate_failed_after_watch_created"`. The detail: when the row was orphaned by promotion's (1), that catch's `markOrphaned` matches **zero** rows, because it filters `status = 'pending'` (`lib/drive/watch.ts:188-198`) and the row is already `orphaned`. That is harmless and deliberate — the transition is idempotent and the ALERT, which is the actionable output, is raised either way. Stated here so it is not mistaken for a defect.
...
| `tests/drive/watch.test.ts` refresh call sites at `tests/drive/watch.test.ts:487`, `tests/drive/watch.test.ts:549`, `tests/drive/watch.test.ts:723`, `tests/drive/watch.test.ts:757`, `tests/drive/watch.test.ts:1360`, `tests/drive/watch.test.ts:1391` | **MUST be edited.** Each calls `refreshWatchSubscriptions` with due rows but injects neither `getActiveWatchedFolder` nor a module mock. Under §3.2 they would perform the REAL service-role settings read, making DB-free unit behaviour depend on ambient environment state — the same missing-injected-dependency class already identified for the real-DB harness, which §5 repaired only there (spec R2 finding 4). Every one gets an injected folder read. |
