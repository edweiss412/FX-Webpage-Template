...
| Folder read fails on a tick with **`due` EMPTY** → `failures` is empty AND `DRIVE_WATCH_FOLDER_READ_FAILED` is still emitted | §3.2.2's fourth row, which has no other coverage: an implementation that always records `'*'` passes every other failure row here and recreates the false 500 / `renewal_failing` defect. |
...
| **`lib/drive/watch.ts` does not import `lib/drive/fetch.ts`** — a static source assertion | §3.1.4's entire reason for the leaf is keeping `xlsx` out of the hourly cron, and NO behavioural test can observe that: importing the full helper from `fetch.ts` classifies all three 404 shapes identically and passes every other row in this table (spec R8 finding 3). Only an import-graph assertion discriminates. |
