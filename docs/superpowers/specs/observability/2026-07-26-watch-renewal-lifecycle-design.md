...
| | | **How 404 is detected:** googleapis surfaces the status inconsistently — `response.status`, a top-level `status`, or a numeric `code` (`lib/drive/fetch.ts:188-197` already reads several of these for its own classification). The check reads all three and treats only a definite 404 as "already gone"; anything unrecognised is a non-404 and therefore a retry. Defaulting the ambiguous case to retry is the safe direction: retrying an already-gone channel is a wasted call, whereas mis-reading a live failure as 404 abandons the channel permanently — the exact leak this table exists to prevent (spec R4 finding 5). |
