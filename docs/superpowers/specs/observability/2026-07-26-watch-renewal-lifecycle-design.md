...
| D6 | Folder-read failure posture | `infra_error` or a thrown read → renew **NOTHING**, because the lease already absorbs a transient read failure (six renewal opportunities before expiry) and fail-open is what unbounded the §3.2.4 residual. `no_folder_configured` → renew nothing either (§3.2.2). |
| D7 | How the folder-read fault is reported | A durable forensic emit **plus** a `failures` entry of `{folderId: '*', operation: 'folder_read'}`. `RefreshResult`'s TYPE is unchanged. The `'*'` sentinel is now correct precisely because D6 became fail-closed: it means "renewal state for every folder is unknown this cycle", and after a failed folder read nothing was attempted (§3.2.3). |
...
| `DRIVE_WATCH_FOLDER_READ_FAILED` | `warn` | the configured-folder read returned `infra_error` or threw, so the run renewed NOTHING and recorded a `'*'` failure | `errorMessage` (through `redactWatchError`), `dueCount` |
...
| | | **How 404 is detected: reuse `driveErrorStatus`, do not reimplement.** It is already exported (`lib/drive/fetch.ts:175`) and already reads every shape googleapis uses — `response.status`, a top-level `status`, or a numeric `code` (`lib/drive/fetch.ts:192-196`) — plus the gaxios-7 timeout codes and the undici double-wrap. `lib/drive/watch.ts` importing `lib/drive/fetch.ts` creates no cycle (fetch.ts does not import watch.ts). A third private copy would be the second duplication of this helper: `lib/sync/applyStaged.ts:833` already keeps one, which is a pre-existing condition this diff does not widen. Only a definite `404` counts as "already gone"; every other value, and `null`, is a non-404 and therefore a retry. Defaulting ambiguity to retry is the safe direction — retrying an already-gone channel wastes one call, whereas mis-reading a live failure as 404 abandons the channel permanently, which is the leak this table exists to prevent (spec R4 finding 5). |
...
| `{kind:'infra_error'}` **or** a thrown read | **none** | `DRIVE_WATCH_FOLDER_READ_FAILED` (warn) |
...
**The folder-read failure branch renews NOTHING, and that is a change from an earlier revision of this spec** (spec R5 finding 1, BLOCKING). That revision renewed every due row on a failed read, preserving today's behaviour — and that is precisely what unbounded the §3.2.4 residual: a raced old-folder row entering its renewal window during ANY folder-read failure would be renewed with a fresh 24h expiry, so repeated failures extend the exposure indefinitely rather than to one lease.
...
**8.3 "The failed folder read should get its own field on the returned result."** Rejected — but note the narrower claim: the fault IS reported in the returned result, as a `failures` entry using the existing `'*'` sentinel (§3.2.3). What is rejected is a NEW FIELD, because `RefreshResult` is deep-equality asserted in five test files and serialised into the cron response, so no field is additive there. The condition is reported as a durable emit instead. Reusing the `'*'` sentinel is NOT rejected — §3.2.3 requires it, because under the fail-closed branch nothing was attempted and that is exactly what the sentinel means.
...
**8.5 "Two `getActiveWatchedFolder` reads per tick can disagree."** Correct and accepted. Refresh now fails CLOSED and records a `'*'` failure, so a disagreement cannot silently renew a stale folder; reconcile performs its own read and records its own fault; the window is one tick. Coupling them re-creates the durable-transport problem R4 of the deferred design removed.
