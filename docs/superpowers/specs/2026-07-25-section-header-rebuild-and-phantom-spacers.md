...
| 2 | **The pill gets its own centered line; it does NOT stay inline.** A centered name and an inline pill cannot share a row: measured, the inline variant reintroduces a 2-line name at 320px (the original bug) and lands the name 48–60px off the row's axis. **Superseded at `sm`+** by `2026-07-26-section-header-wide-inline.md` (owner re-decision); stands below `sm`. | §3.1.4, measured. |
| 3 | **Flagged/judgment sections therefore cost 72.8px instead of 44px, at every width.** Accepted. Only sections that need attention pay it; every ordinary section stays a single 44px row. This is the trade the centered layout forces — not an oversight. **Superseded at `sm`+** by `2026-07-26-section-header-wide-inline.md` (owner re-decision: one 44px row at `sm`+); stands below `sm`. | §3.1.4. |
