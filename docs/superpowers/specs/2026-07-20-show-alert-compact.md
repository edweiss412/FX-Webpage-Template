...
- **CONSTRAINS** `tests/components/admin/class-sweep-now-utility.test.ts` — source-scans AttentionBanner and asserts it consumes the caller-supplied `now: Date` prop and NEVER reads the clock itself (`class-sweep-now-utility.test.ts:126-133`: matches `/\bnow\s*:\s*Date\b/`, forbids `Date.now(` and `new Date()`). The adapter must keep taking `now` as a prop and must not introduce a clock read while restructuring the footer time. No edit to the test is needed, but the constraint binds the implementation. Found by a class sweep of every `readFileSync`-based test referencing these surfaces; it was missing from the R1 repair.
- **CONSTRAINS** `tests/styles/status-token-contrast.test.ts` — pins AA contrast for text tokens on the `--color-warning-bg` wash, explicitly framed as the attention banner's pairing (status-token-contrast.test.ts:150-158). Any token used for the new micro-label or detail-band values on `bg-warning-bg` must already be covered there, or the band uses an already-pinned token. The band's chosen classes (`text-warning-text` family and `text-text` on the warning wash) are within the pinned set; no new token is introduced (R10), so no test edit is needed — but a future substitution would require one.
