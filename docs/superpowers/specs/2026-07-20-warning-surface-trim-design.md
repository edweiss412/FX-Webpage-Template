...
Reverse reachability is NOT uniform, which R3a finding 1 correctly caught in an earlier blanket claim. The three pairs among Silent, Elsewhere, and Clean reverse by un-ignoring, since all three are body-empty states distinguished only by the active warn counts. The three pairs involving List do NOT: List is entered and left by a change to the INFO rows in the body, and an un-ignored warn row is filtered out of that body, so no ignore-family action can produce List from a body-empty state. Every reverse direction carries the same instant treatment regardless.
...
**Two extraction modes, always both.** Identity equality answers "are the right rows here"; a row-ELEMENT count answers "are there any extra rows". R3b findings 2 and 9 showed identity alone is blind to an uninstrumented leak, so every body assertion carries both.
...
13. **Bell inclusion through the rendered panel.** Render `BellPanel` against a mocked feed response carrying both cut codes and assert both entries appear in its output. Excludes the case R3b finding 13 named, where the panel itself drops the codes with a predicate importing no forbidden symbol, which a builder-level assertion and a source scan both miss. The source scan over `BellPanel`, the feed route, and its transitive builder is retained as a second signal. §11 meta-test 3.
