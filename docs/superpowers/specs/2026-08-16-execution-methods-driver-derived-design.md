...
- **The shipped 10-member set does not change.** This arc is drift visibility, not set expansion or contraction. In particular cursor stays in the set although it is a result-modifier member, because removing it is a behavior change out of this arc's scope; and prepare / largeObject are NOT added (§4, documented limits). A finding proposing membership changes is a re-scope, not a finding on this design.
...
Four pieces, following the repo's existing generator pattern (`scripts/generate-admin-tables.ts` emitting the generated module at `lib/audit/admin-tables.generated.ts`; check-mode and header conventions from `scripts/generate-schema-manifest.ts:19-20`).
...
4. **Premise guard** (per the anti-tautology rule in `docs/agents/writing-plans.md` and the guard-premise rule filed as BL-GUARD-PREMISE-REACHABILITY): `POSTGRES_EXECUTION_CORE` contains "unsafe" and "file". If the derivation ever degrades to producing nothing (e.g. an upstream restructuring the walker cannot see), the arms above would pass vacuously against an empty core; this arm makes the degradation loud.
...
The derivation ACCEPTS, keyed on structure rather than spelling: a method-signature member of an interface declaration, named by identifier, whose declared return type annotation's head identifier is one of PendingQuery, PendingRequest, ListenRequest (→ execution core) or Parameter, ArrayParameter (→ parameter members). Everything outside that accept-set is not collected — and "not collected" is safe by direction: a member missing from `core` can only leave the composed set exactly as wide as the hand list, never wider, and the §2.4 premise guard plus version sentinel make a collapsed or stale core loud rather than silent.

## 4. Documented limits

Conservative-plus-loud or conservative-plus-bounded, none silent-and-wrong:

- **Type-alias or renamed indirection in upstream types** (a hypothetical alias of PendingQuery used as a return annotation): the head-identifier match does not resolve aliases, so such a member would drop out of `core`. Consequence: the composed set narrows toward the hand list, and the §2.4 premise guard (unsafe/file present) fires on any restructuring that touches the known members — the realistic form of this drift. The threat fence applies: upstream types are non-adversarial; an alias refactor upstream arrives together with a version bump, which the sentinel already flags for regeneration and review.
...
- **The prepare member (TransactionSql, driver types file line 727) and the largeObject member (Sql, line 715) are not in the set** — pre-existing shipped behavior, unchanged here. prepare submits two-phase-commit SQL and largeObject opens a large-object handle, but both return plain Promises, and prepare is a collision-prone name of exactly the kind the set's own doc comment excludes (`tests/db/_destructiveFileAnalysis.ts:525-540`). Rule 2 backstops destructive statements routed through them. Adding either is a membership change fenced out by §1.1.
...
- **Consequence bound** (finite, per §3): for every member of the finite set of method-signature nodes in the pinned postgres 3.4.9 driver types file, the member is either collected per the accept-set or deliberately not collected with the §4 disposition; a member whose non-collection worst case is "composed set stays exactly as wide as shipped, Rule 2 backstops, premise guard loud on known-member drift" is a DOCUMENTED LIMIT, not a finding.
