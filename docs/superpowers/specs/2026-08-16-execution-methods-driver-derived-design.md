...
- **The json and array members stay out of the execution set.** They return Parameter and ArrayParameter (driver types file lines 698 and 695), and they collide with Response and Object members that real destructive files call on non-clients. Fixture `(cb)` (`tests/db/destructiveFileAnalysis.test.ts:1166`) pins the json half behaviorally (its body exercises a fetch Response's json call only — the array mention is title-only, spec review R3 finding 3); §2.4's disjointness arm pins both structurally, and §2.5 adds the array behavioral case. Ratified at the shipped set's own doc comment (`tests/db/_destructiveFileAnalysis.ts:525-540`).
...
3. **Disjointness.** `POSTGRES_EXECUTION_CORE` and `POSTGRES_PARAMETER_MEMBERS` share no member, and `CLIENT_CAPABILITY_METHODS` and `POSTGRES_PARAMETER_MEMBERS` share no member — the structural half of the json/array exclusion. Fixture `(cb)` is the behavioral half for json only (R3 finding 3: its body never calls array); the array behavioral case is added per §2.5.
...
- a method signature inside a TYPE LITERAL (the toJSON shape, driver types file line 546) is not collected — the walk domain is interface declarations.
...
One addition to the ANALYZER's fixture corpus in `tests/db/destructiveFileAnalysis.test.ts` (R3 finding 3): a case in which a non-client object's `.array()` call appears in a discovered file that must still pass — the behavioral twin of fixture `(cb)`'s json case, closing the half that fixture's title promises but its body does not exercise.
...
- **Complete census of the not-collected remainder (discharges the §7 consequence bound; spec review R1 finding 2).** A TypeScript AST census over the installed driver types file (walk with `ts.createSourceFile`, collect every method-signature member with an identifier name, classify by return-type head — the probe script is one ordinary edit from the §2.1 derivation itself) counts **44 method-signature nodes**: 7 collected (5 core nodes across the 4 core names — file has two overloads — plus 2 parameter-member nodes), 10 nodes carrying the 6 hand-list names (begin ×2, savepoint ×2, cursor ×3, end, reserve, subscribe), and **27 remaining nodes**, dispositioned here as one class with three named judgment calls. Census output (2026-08-16):
  ...
  **Class disposition — result-object and handle members** (everything above except the toJSON node and the three client-interface members called out next): their receivers are call results or handle objects, never bare client identifiers, and Rule 1's property-call leg keys exclusively on an identifier receiver that is a checked client (`checkedReceiver`, `tests/db/_destructiveFileAnalysis.ts:423-424`) — membership would add nothing it can act on, while importing exactly the collision class fixture `(cb)` exists for (forEach, readable, close, read, write, and toJSON are everyday member names on arrays, streams, and Response objects). Rule 2 backstops any recognized destructive string routed through them.
  **The toJSON node (driver types file line 546)** is not an interface member at all: it sits in a type literal inside the JSONValue union, outside the §2.1 walk domain (interface declarations), so it is never a candidate — and as the canonical serialization hook it is exactly the collision-prone name class the shipped set's doc comment excludes (`tests/db/_destructiveFileAnalysis.ts:525-540`).
  **Named judgment calls — client-interface members excluded from the shipped set:** the prepare member (TransactionSql, driver types file line 727), the largeObject member (Sql, line 715), and the release member (ReservedSql, line 731). None of the three routes caller-supplied SQL: prepare takes a transaction NAME (submitting PREPARE TRANSACTION for work whose destructive statements were already executed — and therefore already checked — through other methods), release takes nothing and returns void, and both are collision-prone common names of the kind the doc comment excludes. largeObject is different in kind: it opens a large-object handle whose read/write/truncate operations carry NO SQL statement, so Rule 2 has nothing to recognize — it is an **unmodeled non-SQL channel**, the same documented-limit class as the REST wipe channel already recorded in the analyzer's header (`tests/db/_destructiveFileAnalysis.ts:52-53`). Probed 2026-08-16: no file under tests/, scripts/, lib/ or app/ references largeObject, and an unchecked client's bare largeObject() call in a discovered file is accepted today (shipped behavior, spec review R3 finding 1's probe). Adding any of the three is a membership change fenced out by §1.1; the re-open triggers cover a first live use of largeObject.
...
- AC-2: `EXECUTION_METHODS` in the analyzer is composed per §2.3 and equals the shipped 10 members; every existing case in `tests/db/destructiveFileAnalysis.test.ts` passes unmodified, including `(ca)` and `(cb)`, and the §2.5 array-on-a-non-client addition passes alongside them.
