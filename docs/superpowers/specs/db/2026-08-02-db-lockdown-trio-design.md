...
| The declared-count tripwire lives in the cross-cutting TEST, not in the generator. `scripts/generate-admin-tables.ts` is unmodified — do not add assertions to it. | §4.5; R7 finding 2. |
...
**Retained from earlier rounds, as cheap defense-in-depth, not as the guarantee — and it lives in the TEST, not the generator.** `scripts/generate-admin-tables.ts` is genuinely unmodified; the cross-cutting test reads §4.3's declared counts itself and compares them against the generated list's length. Stated explicitly because it is the one thing a plan could get wrong: **do not add an assertion to the generator.** The count tripwire against §4.3's own declared counts (`docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:641`, `docs/superpowers/specs/2026-04-30-fxav-crew-pages-v1.md:643`) still runs, because it costs one integer comparison and localizes a §4.3 edit to the moment it happens. If it disagrees with directions A-C, the catalog wins.
...
2. `pnpm gen:schema-manifest`. For **this** migration the diff is empty by construction, so there is nothing to commit; run it and record the no-op (see the evidence table below).
...
4. `validation-schema-parity` CI job green — necessary but **not** sufficient here: for a REVOKE-only migration both layers are vacuously green. See the evidence table below for what actually proves each step.
