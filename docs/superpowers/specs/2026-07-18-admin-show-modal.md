...
- **Auth round-trip:** two layers must cooperate, and BOTH change:
  1. `validateNextParam` currently strips ALL query params from `/admin` next-targets (`lib/auth/validateNextParam.ts:70-88`, pinned by `tests/auth/validate-next-param-show-section.test.ts:82-83`). It gains a minimal `/admin` query ALLOWLIST: `show` and `alert_id` (first value, `firstParam` normalization) survive; every other param still stripped (pre-existing `bucket` stripping unchanged — out of scope).
  2. `requireAdmin`'s sign-in redirect derives `next` from the `x-pathname` header, which carries PATH ONLY and falls back to `/admin` (`lib/auth/requireAdmin.ts:50-64`; absent-header fallback pinned at `tests/auth/requireAdmin.test.ts:141-151`) — the query would never reach the sanitizer. `requireAdmin` gains an optional `nextHint?: string` parameter: when provided it is sanitized via `validateNextParam` and takes precedence over the header path. `app/admin/page.tsx` builds the hint from its own normalized searchParams (`/admin?show=…&alert_id=…`) and passes it; all other `requireAdmin()` call sites are unchanged (optional param, no signature break).
  Legacy path-shaped links (`/admin/show/<slug>`) already round-trip today via path preservation and continue to (redirect page keeps `requireAdmin()` with no hint). Tests: sanitizer unit rows (allowed pair survives, junk stripped, non-`/admin` unaffected); `requireAdmin` rows (hint wins over header, hint sanitized, absent hint = today's behavior); request-level proof that signed-out `/admin?show=x&alert_id=y` reaches the OAuth start `next` with both params intact (`app/api/auth/google/start/route.ts:37-45` carries `nextOutcome.path`).
...
| requireAdmin next derivation pins | `lib/auth/requireAdmin.ts:50-64`, `tests/auth/requireAdmin.test.ts:141-151` | add `nextHint` param + test rows (hint precedence, hint sanitized, no-hint unchanged) |
