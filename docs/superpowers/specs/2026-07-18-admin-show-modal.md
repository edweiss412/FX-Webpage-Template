...
| D10 | Durable/emailed deep links keep the PATH form `/admin/show/<slug>` (auth round-trips it today, then 307 → modal); `/admin?show=` is the canonical in-app URL; the auth `next` pipeline is untouched. Signed-out cold-load of a `?show=` URL degrades to bare `/admin` (documented limitation, matches existing `/admin?bucket=` behavior). | The admin auth pipeline is path-only at every layer (layout gate has no searchParams access; `x-pathname` carries no query); threading query through it is auth-surface churn for one feature. |
...
- **Auth round-trip (D10 — durable deep links stay PATH-shaped; auth surface untouched):** the admin auth `next` pipeline is path-only end-to-end: the earliest gate is the admin LAYOUT (`app/admin/layout.tsx:71-80` → `requireAdminIdentity({ layer: "layout" })`, `lib/auth/requireAdmin.ts:279-291`), layouts cannot read searchParams, and `redirectToSignIn` derives `next` exclusively from the path-only `x-pathname` header with `/admin` fallback (`lib/auth/requireAdmin.ts:49-64`; `tests/auth/requireAdmin.test.ts:141-151`; `validateNextParam` strips `/admin` queries — `lib/auth/validateNextParam.ts:70-88`). Threading query params through that pipeline would require touching the auth surface at multiple layers for one feature — rejected. Instead:
  - **Durable/emailed deep links keep the PATH form `/admin/show/<slug>`**: signed-out, the PATH round-trips auth exactly as today (`x-pathname` → sanitizer → OAuth `next`), lands on the redirect page post-auth, and 307s into the modal. Query params on such a link (`?alert_id=`) do NOT survive a signed-out round-trip — `x-pathname` is path-only — exactly as today on the old page; signed-in, the redirect preserves them (§3 Legacy). Zero auth code changes; `validateNextParam`, `requireAdmin`, and their pinning tests are UNTOUCHED.
  - **`/admin?show=` is the canonical in-app/runtime URL** (row clicks, close/open, refresh, back). If a signed-out user cold-loads it, auth returns them to bare `/admin` (query stripped — same pre-existing degradation `/admin?bucket=archived` has today); documented limitation, not a regression.
  - Tests: e2e signed-out `/admin/show/<slug>` (path only) → sign-in → post-auth lands on the modal (proves the path round-trip); SIGNED-IN legacy `/admin/show/<slug>?alert_id=x` → 307 preserves `alert_id` → highlight + scroll (proves redirect passthrough); in-app `/admin?show=…&alert_id=…` covered by the deep-link spec (§8).
...
| `lib/notify/templates/digest.ts:16`, `lib/notify/templates/realtimeProblem.ts:93`, `lib/reports/submit.ts:312` | absolute `${origin}/admin/show/${slug}` | UNCHANGED (D10) — emailed links deliberately keep the path form; it survives the signed-out auth round-trip and 307s into the modal |
...
`app/admin/page.tsx` changes: searchParams type gains `show?: string | string[]; alert_id?: string | string[]`, normalized through a shared `firstParam()` helper (first element wins, empty string → absent — §6.2 guard table); in the non-wizard branch, after `DashboardWithHeader`, when `show` is present render `<Suspense fallback={<ShowReviewModalSkeleton/>}><ShowReviewModal slug={sp.show} alertId={sp.alert_id ?? null}/></Suspense>`. Dashboard paints immediately; modal streams. Skeleton = shell chrome + loading blocks (replaces the deleted `loading.tsx` treatment) inside an open, **content-non-interactive** modal frame so the open gesture has immediate feedback — close affordances (X / scrim / Esc / grab / drag) are live and navigate back via `useShowModalNav().close` (MODAL-SKELETON-CLOSE-1 amendment, `2026-07-19-modal-skeleton-close.md`); the loading blocks themselves stay non-interactive.

Mutations: every server action reachable from the modal must revalidate `/admin`, or the modal UI goes stale after the action (a server action only re-renders the current route if it revalidates it). Already covered: `archive.ts:38`, `unarchive.ts:42`, `setPublished.ts:37`, `feed.ts:183,219` (`acceptChangeAction`/`acceptAllAction`). **Must be ADDED** — `revalidatePath("/admin", "page")` in `mi11ApproveAction` (`feed.ts:75-76`), `mi11RejectAction` (`feed.ts:109`), and `undoChangeAction` (`feed.ts:141-142`), each with a test asserting the call. `useRaw.ts` needs nothing: `UseRawControlBoundary` calls `router.refresh()` on success (`components/admin/UseRawControlBoundary.tsx:81`), which re-fetches the CURRENT route's server payload — on `/admin?show=` that re-renders the modal loader, identical semantics to today's page. `roleToken.ts` ALSO needs nothing, deliberately: `RoleRecognizeControlBoundary` has NO `router.refresh()` by documented design — the saved card is client-local until a later navigation unmounts the control (§8.1 timing contract, `components/admin/RoleRecognizeControlBoundary.tsx:21-23`); adding `revalidatePath("/admin","page")` to `mapRoleToken` would unmount the saved card mid-interaction and BREAK that contract. In-modal behavior is identical to today's page (client-local card, durable on next open); the modal test suite transplants the existing boundary assertions unchanged. (`revalidateShow(id)` in both actions is the crew-page cache-tag bust — `lib/data/showCacheTag.ts:34-36` — not an admin-path revalidate.) `revalidatePath("/admin/show/[slug]")` calls stay harmless (route still exists as redirect) but are dropped where touched.
...
API: `{ open: boolean; onClose(): void; onDismissStart?(): void; labelledBy: string; dataAttrPrefix: "step3-review" | "review-modal"; testIdBase: string; initialFocusRef: RefObject<HTMLElement | null>; header: ReactNode; children: ReactNode; footer?: ReactNode }`. `onDismissStart` (MODAL-SKELETON-CLOSE-1 amendment): fires exactly once per open shell instance, at dismiss-commit (`beginDismiss`), before exit styles and before any exit-end `onClose` — both the `requestClose` and drag-past-threshold paths reach it through `beginDismiss`, whose idempotence guard makes it one-shot.

- **`initialFocusRef`:** the consumer owns its close button (rendered inside `header`) and its ref; the shell passes the ref to `useDialogFocus(panelRef, initialFocusRef)` (`lib/a11y/dialogFocus.ts:41-54`). Step3 keeps its exact `closeRef` wiring (`Step3ReviewModal.tsx:168-170,741-747`); the close-button initial-focus pin (`tests/components/admin/wizard/Step3ReviewModal.test.tsx:399-402`) passes unchanged.
- **`testIdBase`:** shell-owned nodes render `` `${testIdBase}-modal` `` (panel root), `` `${testIdBase}-backdrop` `` (scrim), `` `${testIdBase}-grab` `` (grab strip), and the header/footer WRAPPER regions `` `${testIdBase}-header` `` / `` `${testIdBase}-footer` `` — matching the ids Step3 renders today with `testIdBase = ` `` `wizard-step3-card-${dfid}-review` `` (`Step3ReviewModal.tsx:605,618,641,660,865`; asserted at `tests/components/admin/wizard/Step3ReviewModal.test.tsx:502-529`, `tests/e2e/step3-review-modal.layout.spec.ts:227-231`). All other testids (title, chip, close, resolution, publish, …) are consumer slot content, untouched. Published modal passes `testIdBase="published-show-review"`. NO `scrollerRef` and NO body wrapper — `children` mount DIRECTLY into the panel's flex column, exactly as Step3 mounts `ShowReviewSurface` today (`Step3ReviewModal.tsx:761-767`): the surface's own root (`*-review-main` testid/classes, `ShowReviewSurface.tsx:622-627`) IS the body element, and it owns the live scroller (`ShowReviewSurface.tsx:136-150,784-790`) with the consumer holding the ref. Step3's DOM is byte-identical after extraction; the e2e panel-height assertion against the `main` node (`tests/e2e/step3-review-modal.layout.spec.ts:227-235`) passes unchanged. Shell owns chrome ONLY — no title semantics (heading-safe h2 pattern stays in each consumer's header slot), no footer logic, no section knowledge. `Step3ReviewModal` refactors to consume it with `dataAttrPrefix="step3-review"`; its rendered DOM (attrs, classes, focus order, drag behavior) is unchanged and its full test suite (`tests/components/admin/wizard/Step3ReviewModal.test.tsx`, `Step3ReviewModalResolution.test.tsx`, `step3ReviewModal.transitions.test.tsx`, e2e `step3-review-modal.layout.spec.ts` / `.interactions.spec.ts`) passes WITHOUT edits. `RescanSheetButton` render sites stay inside `Step3ReviewModal.tsx` body content, keeping the freeze contract (`tests/components/admin/wizard/_metaStep3FreezeContract.test.ts:24-26`) green with zero changes.
...
- **Header slot:** heading-safe `<h2>` with ONLY the show title text (aria-labelledby target), adjacent separate 44px sheet deep-link icon anchor (pattern from `Step3ReviewModal.tsx:670-691`), close button. Below the title row: `<StatusStrip>` with a new `renderTitle?: boolean` prop (default `true`) — the modal passes `false`, suppressing StatusStrip's internal `<h1>` title AND its immediately-following divider (`StatusStrip.tsx:139-157` — no orphan leading separator; the strip then starts at the publish toggle) so the modal contains exactly one title node (the `<h2>`) and no `<h1>`; all other StatusStrip content (PublishedToggle, live badge, copy-link) unchanged (`StatusStrip.tsx:54-81` props). A11y test: dialog accessible name === show title via `aria-labelledby`; no `h1` inside `[data-review-modal-panel]`.
...
| skeleton open → closed (X/scrim/Esc/grab/drag) | same shell exit animation as the loaded modal (`requestClose`); server fallback: nav issued at dismiss-commit (`onDismissStart`), instant hide at exit-end; ShowsTable optimistic copy: the passed cancel at exit-end (MODAL-SKELETON-CLOSE-1, `2026-07-19-modal-skeleton-close.md`) |
...
- Panel internal column per mode (no footer in the published modal; real-browser assertions within 0.5px; Tailwind v4 does not default flex stretch):
  ...
  - "Body" in these equations = `ShowReviewSurface`'s root `main` node (mounted directly in the panel column — no intermediate wrapper), the same node the step3 layout spec measures (`tests/e2e/step3-review-modal.layout.spec.ts:227-235`); the surface's internal scroller fills it per its existing contract.
...
| Page behavioral suite | `tests/app/admin/perShowPage.test.tsx` (returned-error + thrown coverage cited by the infra-contract row; asserts `notFound()` for missing slug + `not_admin_or_missing` at `:245-259`) | retarget to the loader component; infra-error THROW assertions unchanged; the missing-slug and `not_admin_or_missing` assertions CHANGE from `notFound()` to `redirect("/admin")` (D8) |
...
| pageTransitions count map | `tests/components/admin/showpage/pageTransitions.test.tsx:118-123` | replace `PublishedReviewPage.tsx` row with `PublishedReviewModal.tsx`; bump the `StatusStrip.tsx` conditional count (pinned at 7) for the new `renderTitle` branch |
...
| Auth next-param pipeline (`validateNextParam`, `requireAdmin`, layout gate) | `lib/auth/validateNextParam.ts:70-88`, `lib/auth/requireAdmin.ts:49-64,279-291`, `app/admin/layout.tsx:71-80` | UNTOUCHED (D10 — durable deep links stay path-shaped; the path form round-trips already) |
