...
| D9 | **Client-side** open/close navigation (`useShowModalNav`, backed by `useSearchParams`) mutates ONLY the `show` / `alert_id` params and preserves all others. **Server-rendered** link producers cannot see current params (`Dashboard` passes no search-param context to rows — `components/admin/Dashboard.tsx:696-701`) and instead embed their own known context explicitly: active `ShowsTable` rows emit `/admin?show=<slug>`; `ArchivedShowRow` emits `/admin?bucket=archived&show=<slug>`; producers outside the dashboard (BellPanel, telemetry, inbox, alert builders) emit bare `/admin?show=<slug>` (+`alert_id` where applicable). | Archived-bucket context survives opening/closing a show without threading searchParams through server row producers. |
