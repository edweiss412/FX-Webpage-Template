...
- **Guard conditions (all fields nullable, `step3ReviewSections.tsx:1403-1430` `hasContent` guards unchanged):** name+phone+email → gaps 4/10; phone absent → name-to-email gap is 10px (4+6, slightly looser — accepted, the email chip's `mt-1.5` is unconditional); email absent → name-to-phone 4px, no trailing gap; name absent → the first chip tops the cell. No combination renders an empty chip.
...
2. **Upward-only bleed:** the title-link rect is DISJOINT from the meta line's rect (and, in flow, from everything below the text line — box bottom is flush with the text bottom, carried by zero bottom padding). New assertion; fails against the shipped symmetric recipe by ~8px.
3. **Bleed containment:** the title-link rect stays inside its card's rect (top edge lands on the card's inner border edge, carried by `-mt-5` = `p-tile-pad`).
...
8. **Chip visible edge:** the `tel:` chip's computed `background-color` differs from the cell's, and its computed `border-top-width` is 1px (new; carried by `bg-surface border border-border` — a class-string regression to the invisible form fails by name).
...
TDD in the existing e2e file; every assertion mounts the PRODUCTION route and drives it to the state that renders the control (the suite's own contract). Render premises per the suite's existing pattern — the demoted_rescan seed carries `client_label: "Gallery Client"` (`tests/e2e/helpers/devCaptureStaged.ts:446`), so the meta line renders and premise 2 below is satisfiable; the meta line `<p>` gains a production testid (`wizard-step3-card-${dfid}-meta-line`, `Step3SheetCard.tsx:539`) so the new assertion locates it durably.

- **RED (all runnable against the current tree — each fails for a stated production-line reason):**
  1. Site 5: meta-line disjointness fails (~8px overlap from the shipped `-my-2.5 py-2.5`).
  2. Sites 6/7: `≥9.5px` clearance fails (shipped gap is 6px); full-width fails (shipped links shrink-wrap); visible-edge fails (no background/border utilities on the shipped strings).
...
4. **`mt-1.5` on the email chip is unconditional** — a phone-less cell shows a 10px name-to-email gap where 4px would be ideal. Accepted (§2.3 guard conditions) over a conditional class.
...
- **AC-1:** Site-5 title link bleeds only upward — §3 invariants 1-4 green in a real browser at 390px and a ≥sm width, on the demoted_rescan production route.
