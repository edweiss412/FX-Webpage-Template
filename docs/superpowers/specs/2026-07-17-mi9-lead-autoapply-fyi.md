...
**Therefore, in addition to the feed nudge, emit a durable code-carrying `app_event` per LEAD-bit change** (post-commit, outside the advisory-lock tx — invariant 10): a `log.info({ code: "LEAD_ROLE_APPLIED", ... })` (forensic code; NOT a §12.4 user-facing code — must pass the forensic-code namespace scanner, and must NOT collide with reserved prefixes) carrying the show, `crew_name`, `prior_flags`, `new_flags`, and the direction (gained/lost LEAD). This is per-event, **non-coalescing**, durable, and queryable via `pnpm observe events --code LEAD_ROLE_APPLIED`. It is emitted for BOTH an existing-crew LEAD toggle AND a new-crew-with-LEAD grant. The context is redaction-safe (crew name + flag tokens — no email/phone/token). This record — not the coalescing alert — is the authoritative LEAD audit trail; the alert remains a best-effort feed heads-up.
