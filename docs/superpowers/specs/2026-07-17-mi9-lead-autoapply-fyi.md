...
**Guard conditions:** `prior_flags`/`new_flags` are always arrays; a genuinely-new crew member (no `previousByName` match, `:256`) is skipped — a LEAD grant on a brand-new crew member surfaces via `crew_added` (unchanged). The `roleFlagsEqual` no-op skip (`:258`) stays — an identical applied role set emits nothing. Held-identity crew (MI-11 email hold) whose role_flags still applied to the retained row ARE captured (that is the point of diffing the applied list).
