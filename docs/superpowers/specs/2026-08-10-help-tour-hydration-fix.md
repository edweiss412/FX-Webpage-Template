...
**Wiring acceptance bar (R1 F2):** the parent entry's promotion bar applies unchanged — **five consecutive green normal-dispatch runs** before the spec counts as wired (`BACKLOG.md:671`, adopting `docs/superpowers/specs/ci/2026-08-09-app-e2e-batch1-design.md` §6.1/AC-6). The PR's own `pull_request` runs count toward the five (re-trigger via push or `workflow_dispatch` as needed); the evidence (five run links) is recorded on the PR before merge. A flake inside the five restarts the count, and if the flake is in `help-pages.spec.ts` itself the promotion reverts to the allowlist row rather than shipping a flaky gate.
...
- **AC-6:** The five-consecutive-green wiring bar (§5) is met, with the five run links recorded on the PR before merge; the `REQUIRED` oracle row and parity test (`tests/cross-cutting/app-e2e-ci-wiring.test.ts:203`) are green; the parent entry `BL-E2E-APP-DEPENDENT-SPECS-CI-DARK` is restated per §5.5.
