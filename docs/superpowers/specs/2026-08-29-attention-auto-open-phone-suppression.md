...
**One obligation stays stated here, because it is a design constraint rather than a test detail.** §8 requires the panel's vertical anchor to remain the pill's WRAPPER, its `offsetParent`, never the pill. This is not a preference: the source records the pill-anchored variant being tried and measured, and it put the panel over the status strip and made the published toggle unclickable, with a monitoring row taking its pointer events (`components/admin/showpage/AttentionMenu.tsx:378-388`). That is this arc's own defect reached by a second route, and it passes every suppression, tap, containment and desktop assertion. The plan's table carries the assertion (P-2.8); this paragraph carries the requirement, so the two would have to be dropped independently for the constraint to go missing.
