...
| The panel's vertical anchor is the pill's WRAPPER (its `offsetParent`), not the pill | `const anchor = panel.offsetParent` in `measureAndApply`. **Measured in-arc 2026-08-29, correcting what this row used to claim:** `wrapper.bottom - pill.bottom` is **0.0** at 375x667, 640x800 and 1280x800 with the menu open, so on today's layout a `pillRef` re-anchor would NOT sit the panel higher. The source comment at `components/admin/showpage/AttentionMenu.tsx:383` says it did, and whatever layout that measured is not this one. | UNCHANGED as a design constraint, because `offsetParent` is what the placement math is written against. But the CONSEQUENCE claim is retired: no edge comparison can detect a re-anchor while the two edges coincide, so the guard against it is AC-TOGGLE-OPERABLE, which asserts the measured outcome directly. |
...
3. the pill's hit band is HIT-TESTED at 21px above and below its centre and returns the pill both times, per §8. Not a dimension sum: round 4 was right that a computed 48px band is satisfied by a pseudo-element carrying `pointer-events: none`, which keeps the geometry and takes the taps, while a centre click still succeeds and hides it. The sibling oracle already does it this way (`tests/e2e/wizard-attention-menu.spec.ts:170-175`). Newly load-bearing, because the pill is the only way to open the panel at this width;
...
### 9.3 Obligations: where they live, and the one this spec keeps

The obligations table lives in the PLAN, at `docs/superpowers/plans/2026-08-29-attention-auto-open-phone-suppression.md` §2, in the derived-cover form it was written in: one row per claim in this document, each naming the assertion that settles it, so a claim added later without an assertion shows up as a missing row rather than as a review finding.

**Ratified 2026-08-29 (bl-orch), at the spec stage's 4-round cap.** The evidence decided it: eleven findings across four rounds, none of them on the design, all on the account of how the design would be proven. Proof plumbing is plan-grade content, and reviewing it on the spec stage's round budget spent four rounds where the plan stage's budget is the one that should carry it. The table keeps its form in the move; only its home changes.

**One obligation stays stated here, because it is a design constraint rather than a test detail.** §8 requires the panel's vertical anchor to remain the pill's WRAPPER, its `offsetParent`, never the pill. The source records the pill-anchored variant being tried, and reports that it put the panel over the status strip and made the published toggle unclickable, with a monitoring row taking its pointer events (`components/admin/showpage/AttentionMenu.tsx:378-388`). **That consequence is not reproducible on today's layout**: the wrapper's bottom edge and the pill's are identical to the pixel at every width measured in-arc, so the two anchors are vertically indistinguishable. The constraint stands anyway, because `offsetParent` is the anchor the placement math is written against and changing it is a redesign rather than a refactor; what does NOT stand is the idea that comparing the panel's top against the wrapper's bottom can catch a violation. It cannot, and an acceptance criterion that tried to was retired in-arc for exactly that reason. That is this arc's own defect reached by a second route, and it passes every suppression, tap, containment and desktop assertion. The plan's table carries the assertion (P-2.8); this paragraph carries the requirement, so the two would have to be dropped independently for the constraint to go missing.
