...
| **compound: dismissal lands while the operator is scrolling** | the scroller carries `motion-safe:scroll-smooth` (`components/admin/review/ShowReviewSurface.tsx:1054`). See §3.7 DI-10 for what actually moves and AC-11 for what is asserted |
| **compound: dismissal lands while a section is expanding or collapsing** | independent subtrees, no shared animation driver, no layout dependency. **No interaction** |
...
|---|---|---|
| AC-1 | Real-browser (Playwright) `getBoundingClientRect` at **375x667** on the pill at a **three-segment** load, via a new harness page — no committed fixture renders all three segments, and the tallest existing render is two at 94.89px. Asserts the measured height and that width stays at the 112px cap. Expected values derived from the fixture's own counts, never hardcoded | The one case §2.2 could not measure. jsdom cannot compute it |
...
| AC-4 | `tests/e2e/published-review-modal.layout.spec.ts` green at both viewports, including `T-LAYOUT` and `T-TAP` | RISK-2, and the 44px floor at the new size |
...
| AC-14 | `tests/e2e/step3-review-modal.layout.spec.ts` is wired into the `desktop-chromium` project and green, giving W2 real-browser layout coverage it did not have | The spec matched no project regex and ran nowhere. Probed green (50 passed alongside its sibling) before adopting; §2.5 records the control run that showed the first red was a local env gap |
