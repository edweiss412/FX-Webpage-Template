...
The note announces through the shell's existing `AdminAnnounceProvider`, and the visible element is **`aria-hidden`**. `ReviewModalShell.tsx:647-655` already wraps the entire panel interior in that provider (`testId={`${testIdBase}-undo-status`}`, `label="Status updates in this dialog"`), consumed as `const { announce } = useContext(UndoAnnounceContext)` — existing precedents at `components/admin/RecentAutoAppliedStrip.tsx:353`, `components/admin/RecentAutoAppliedStrip.tsx:716`, and `components/admin/ReSyncButton.tsx:317`. This mirrors the CrewBreakdown split (a persistent `sr-only role="status"` announcer plus an `aria-hidden` visible banner, `components/admin/wizard/step3ReviewSections.tsx:1705-1733`) while adding no second region.
...
|---|---|---|
| DI-8 | scroller -> note, full content width | **The scroller is a flex container**, not a block: `flex min-w-0 flex-1 flex-col gap-6 overflow-y-auto p-tile-pad motion-safe:scroll-smooth` (`components/admin/review/ShowReviewSurface.tsx:1051-1055`). The note is therefore a **flex item in a `flex-col`**, and this project's Tailwind v4 does not default `.flex` to `align-items: stretch`, so full width is **not** inherited. The note carries an explicit `w-full`. An earlier draft claimed there was "no flex in the chain" and that claim was false |
| DI-9 | the note must never be a **zero-height in-flow child** | it renders with content and padding when present, and is **absent from the DOM** when not — never mounted-but-empty. `tests/e2e/admin-layout-dimensions.spec.ts:599` scans for zero-height in-flow pane children; `tests/e2e/helpers/phantomGap.ts:260` skips `position:absolute`, which is why the `sr-only`-when-idle pattern is safe elsewhere. This note takes the simpler route and unmounts |
...
| DI-11 | removing the note must not disturb other specs' settled geometry | the note mounts only when `sessionStorage` holds a non-empty draft for this exact key. Every e2e context starts with empty `sessionStorage`, so it never mounts in those runs unless a fixture deliberately seeds the key. `tests/e2e/admin-layout-dimensions.spec.ts:1209-1215` polls a crew card's `top` until it settles and would be disturbed by a node vanishing above it; AC-12 pins that no existing spec seeds the key |
...
|---|---|---|
| AC-1 | Real-browser `getBoundingClientRect` at **375x667** on the pill at a **three-segment** load, via a new harness page (no committed fixture renders all three; the tallest existing render is two segments at 94.89px). Oracle is **differential, not a pixel literal**: (a) `getComputedStyle(pill).fontSize` is `14px` at 375 and `12px` at 640, which proves the responsive pair actually applied; (b) the same fixture is taller at 375 than at 640, by a delta the test measures rather than asserts a literal for; (c) the pill's width equals the measured cap, proving the wrap is cap-driven and not content-driven; (d) the rendered height is an exact whole number of line boxes, taking the line-box height from a one-segment variant of the same fixture in the same run | The one case §2.2 could not measure. The four-part oracle is the point: an earlier AC-1 said "derived from fixture counts, never hardcoded", which is incoherent for a pixel height — counts fix the copy, not the font metrics — so it could only have been satisfied by comparing the height with itself, which passes at 12px |
| AC-2 | `tests/e2e/attention-autoopen-suppress.spec.ts` green at the three-segment load, with the toggle-interception assertion non-vacuous (it must still observe an interceptor when the menu IS open) | RISK-1: the taller hit band reaching the published toggle |
| AC-3 | At 375px, P2's "Alerts unavailable" **wraps** and is not clipped: full text present in the accessible name, and no `text-overflow` clip | The round-4 defect returning at the larger size |
| AC-4 | `tests/e2e/published-review-modal.layout.spec.ts` green at both viewports, including `T-TAP`; **and `T-LAYOUT`'s `header + main + footer === panel` equation additionally evaluated against AC-1's three-segment fixture**, not only the default page | RISK-2, which the earlier AC-4 did not actually reach: `T-LAYOUT` opens the default harness page, whose pill is the one-segment `2 issues` case at +3.50px. The risk is the +10.50px-and-taller case, so an implementation could pass AC-4 while failing exactly the thing AC-4 claimed to cover |
...
| AC-12 | `tests/components/_metaLiveRegionMounting.test.ts` green with its declared count for `Step3ReviewModal.tsx` unchanged at 1, and no existing e2e fixture seeds the draft key | A forbidden gated live region; and DI-11's cross-spec disturbance |
...
| AC-14 | `tests/e2e/step3-review-modal.layout.spec.ts` runs in **real CI**: added to both the `paths` watch list and the `playwright test` invocation in `.github/workflows/step3-live-bundle.yml` (which today names only the interactions, agenda and wizard-attention-menu specs, at `.github/workflows/step3-live-bundle.yml:19-20` and `.github/workflows/step3-live-bundle.yml:87`), in addition to the `desktop-chromium` project entry in `playwright.config.ts` | The project-regex wire alone gives **local discovery only**. Without the workflow edit the spec still never runs in CI, so "W2 has real-browser coverage" would be true only for someone who runs it by hand, and the load-bearing test could drift untouched |
| AC-16 | With the pane scrolled past the note, a reference element's **viewport-relative** `top` is unchanged across the dismissal | The scrolled compound §3.6 used to wave through. AC-11 asserts `scrollTop === 0`, which says nothing here; this asserts what the operator actually sees |
| AC-17 | Toggle a section while the note is live, then let the note dismiss: the pane's scroll and the reference element behave as AC-11 and AC-16 require | The expand/collapse compound. Both changes share one `flex-col gap-6 overflow-y-auto` scroller, so they share scroll-anchoring state — they are not the independent subtrees §3.6 once claimed |
