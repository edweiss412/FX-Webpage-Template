...
| operator reaches Report and **clears or submits** the draft inside 5000ms | the note **stays** until its timer expires, and stays TRUE, because its copy describes the restore that happened at open rather than the draft's current state (§3.4). Storage is emptied by `clearStoredDraftIfUnchanged` and the label reverts to "Write a report"; the note does not contradict either. **AC-18** |
...
| AC-2 | `tests/e2e/attention-autoopen-suppress.spec.ts` green at the three-segment load, with two premises asserted before the occlusion check runs: the pill's text contains **all three** segments (an issues count, a sheet-warnings count and a monitoring count), and the toggle-interception assertion is non-vacuous (it must still observe an interceptor when the menu IS open) | RISK-1. The segment premise is the load-bearing half: without it a broken item mapping or a dropped segment shrinks the pill to one line, and the occlusion assertion then passes on a load that never exercised the risk |
...
| AC-14 | `tests/e2e/step3-review-modal.layout.spec.ts` runs in **real CI**: added to both the `paths` watch list and the `playwright test` invocation in `.github/workflows/step3-live-bundle.yml` (before this arc that workflow named only the interactions, agenda and wizard-attention-menu specs; the layout spec is added to its watch list at `.github/workflows/step3-live-bundle.yml:21` and to its invocation at `.github/workflows/step3-live-bundle.yml:88`), in addition to the `desktop-chromium` project entry in `playwright.config.ts` | The project-regex wire alone gives **local discovery only**. Without the workflow edit the spec still never runs in CI, so "W2 has real-browser coverage" would be true only for someone who runs it by hand, and the load-bearing test could drift untouched |
