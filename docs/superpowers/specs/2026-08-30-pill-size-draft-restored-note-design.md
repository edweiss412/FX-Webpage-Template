...
**A spec found on the way. CORRECTED 2026-08-30 by `test/e2e-spec-discovery-wiring`, and the wiring it describes has been REVERTED.** This paragraph read: a dead spec, matching no project regex in `playwright.config.ts`, so it ran nowhere and proved nothing, leaving W2 with no real-browser layout coverage; this arc wires it into `desktop-chromium` (one alternation entry). Every part of that is wrong except the file name. `playwright.config.ts` is one of four configs that resolve specs here; `tests/e2e/standalone.config.ts` resolved this spec from 2026-07-03 and the unfiltered `standalone-e2e.yml` ran it on every PR, so W2 had real-browser coverage throughout. The `desktop-chromium` entry this arc added executed in no workflow and has been removed. Probed before adopting: the spec first failed locally on a missing `HASH_FOR_LOG_PEPPER` in the harness subprocess, and a control run showed the already-wired, CI-green `step3-review-modal.agenda.spec.ts` failing **identically**, so the red was a local environment gap and said nothing about the wiring. With the variable exported, both specs run green together: **50 passed**. The wiring stays.
...
**Pre-made fallback for AC-14, so it is not decided under pressure.** (CORRECTED 2026-08-30: the spec HAD been running in CI on every PR via the standalone config, so this was not the never-run-in-CI case it is described as. The caution below was still reasonable on its own terms.) Adding a 44-test spec believed never to have run in CI is exactly the local-passes-CI-fails class. If real CI reds or flakes on it, the workflow entry is reverted and the spec keeps only its `playwright.config.ts` project wire, with the CI gap recorded as a documented limit and a re-file trigger of "the next arc touching Step3ReviewModal layout". ~~The project wire itself stays either way~~ (REVERTED 2026-08-30: the project wire is removed too, because no workflow selects `desktop-chromium` for this spec and the standalone config already runs it on every PR): it was probed green (50 passed with its sibling) and it makes the spec runnable by hand, which is strictly better than the nothing it had.
