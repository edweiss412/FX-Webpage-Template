...
**Second: in eleven arcs, not one repair declared provenance.** All eleven repaired — that is what review is — and the repair SHAPES partition three ways, none of them a pointer to a command. Seven made the table an OUTPUT (`fix/shell-brace-cross-construct` via `weaker-walks.mts`, "The table is no longer written down; it is produced"; `fix/severityless-warning-filters` via one query producing every number; `fix/mutation-shard-budget-six`, "Every figure below is printed by commands"; `fix/mutation-shard-weight-seconds`, "Each repair replaced the number with the COMMAND"; plus `feat/speclint-ac-unclaimed-arm`, `docs/quick-wins-2-specs`, `fix/supabase-upstream-fault-class`). Two DELETED the table (`feat/review-modal-strip-dock`; `feat/speclint-red-reason-verification`, "both movement tables were DELETED instead of annotated"). Two kept the table and COMMITTED the command's raw output beside it (`fix/sync-log-show-id-duration`, `docs/sync-log-emit-guard-spec`). **Zero declared the table's provenance with a pointer.** That is a shared absence rather than a convergence, and it is evidence about what authors reach for rather than proof the absent option is wrong — the structural argument above is what carries that.
...
Other measurements at `8b4d521cac00`: this corpus's two shipped opt-in doc markers have **4 live uses between them** across 3425 tables; an executing arm reaches **at most 8 of 3425**, and at most 6 once two adjacent-but-not-producing commands are removed. Full disposition, documented limits and declinations in that spec §5-§9, including the one narrower candidate (row count versus live hit count) whose population is measured at two.
