...
**MEASURED AND DECLINED 2026-08-28** by `feat/speclint-table-provenance`. The row's narrowing was a `<!-- table: cmd=`…` -->` marker checked the way `gate:` markers are. Neither arm ships, and the reason is structural rather than budgetary: the corpus had already ratified, six days earlier, that naming a producing command is not a binding (`docs/superpowers/specs/ci/probes/README.md`, `## Stating a figure`), so a table can satisfy the existence check on every line and remain exactly as drifted. Supporting measurements at `8b4d521cac00`: the nearest precedent (`feat/planlint-ac-command-observability`, structurally the same opt-in table marker) cost **14 review rounds and 37 findings** to build, against a class that has cost **6 rounds** across all four namings; this corpus's two shipped opt-in doc markers have **4 live uses between them** across 3425 tables; and an executing arm reaches **8 of 3425 tables**, since only 98 sit within 40 lines of a shell fence, 11 of those commands are pure and repo-local, and 3 of the 11 read a machine-local path. Full disposition, documented limits and declinations in that spec §5-§9.
