...
**Second: the practice the row would formalize is already widespread among those arcs, and already failing.** Eight of the eleven relate their table to a named script, query, command or committed output, and `feat/speclint-ac-unclaimed-arm` places the producing command beside the table outright. An earlier version of this entry claimed zero of them used a pointer; that was tautological (true only if "pointer" meant the exact unshipped HTML syntax) and is withdrawn, per that arc's spec review round 4. The stronger reading of the same records is the one that stands: naming the producing command beside a table is common practice in this corpus, and it does not prevent the drift. The live instance is an arc doing exactly that — `docs/superpowers/plans/2026-07-20-show-scoped-alert-copy/00-plan.md:394` prints its command above the table and states 53 where the command's comment says 56 and where it returns 55 today. A marker is that same practice with a linter able to see the comment, and the comment is not what drifted. For the record the repairs partition seven OUTPUT, two DELETE, two COMMIT-the-output, each cited in that spec's §5.
