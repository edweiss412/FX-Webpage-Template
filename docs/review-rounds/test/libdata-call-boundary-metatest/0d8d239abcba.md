...
**Examined:** 21 counted rounds (R2–R22) across two scopes, scanner-logic and registry-fidelity, declaring 50 findings; 3 dispatches returned no verdict and are not counted. **R22 APPROVED with 0 findings.** The registry-fidelity scope APPROVED at R6 with 0 findings and was not dispatched again. Every finding was probed by the reviewer against the shipped guard, and every accepted one was repaired with a planted self-test plus a mutant proving the repair load-bearing — 53 mutants killed, one proven equivalent.
...
**R18 is where the arc stopped repairing and started dispositioning.** It showed that R17's new rule proved any USE of the result binding, not error inspection — `return result.data` passes, and the obscuring is name-global. Both are true, and neither was repaired: proving the error is actually inspected is a semantic question about control flow that a TEXT pin cannot answer, which is the division §6.3 has drawn since the spec was written. The rule was renamed to claim only what it establishes, and the residue ships as documented limit §6.7 with planted ACCEPTANCES so its cost is visible. **R19** then found four surviving mutants in the suite's own coverage — Layer 1's disk walk, its extension breadth, reconciliation's second direction, and stale-file detection were all claimed but unpinned — the same "does the guard pin what it claims" question turned on the guard's own tests.
...
**The last three rounds are the arc's own lesson, played once more.** R20 found that every waiver positive sat in the file's leading trivia, so the recursion through child nodes was untested. Fixing it, I reached for trailing trivia so a marker at the end of a code line would also be recognized — and that immediately reopened R5's JSX false waiver, caught one command later by a planted negative. I concluded "collect less" and shipped the narrower collection. R21 then showed that conclusion was half right: the same-line JSX case was hidden, but the MULTILINE case was wide open, and a fake marker among an element's children silently waived a file with a live unregistered read. The sound fix was never "collect less" — it was the move this arc had already made four times: ask the PARSE what is text, and discard comment ranges inside `JsxText`. R22 approved with nothing found.

**Infra:** Three of twenty-six dispatches returned no verdict, none of them a review outcome. R1's whole-diff dispatch was killed at its attempt timeout with its audit complete — its stderr held the probe that became the R1 finding, recovered by reading the stream rather than re-dispatching. R3's registry-fidelity dispatch ran a single 7166s attempt with Codex's command runner stalling on `/bin/pwd`, at a machine load average of 172 caused by concurrent runs in other worktrees; the same review completed in ~20 minutes once serialized. R18's first dispatch was interrupted by a machine-wide Codex usage limit and was re-run once a fresh account was available.
