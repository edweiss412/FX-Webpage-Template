...
Root-widening was rejected: it both over-selects (the current predicate over `["lib","app","components","scripts"]` pulls in 20 admin-alert / report / fixture codes, and nothing textual separates a ParseWarning's `code:` from an admin alert's in the same file) and under-selects (three of the four residue codes reach their emit site as a *factory argument* no `code:` regex can see). Replaced by a type-aware recognizer resolving four shapes through the TypeScript checker — literal, imported const, factory argument, union-typed parameter — with no roots list at all, so an emitter in an unanticipated directory is covered by construction.
