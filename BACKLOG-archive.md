...
**Status:** RESOLVED 2026-08-03 (`fix/parse-warning-code-recognizer`). The row's prescribed fix was **superseded on probe evidence**, and its **Effort: S** estimate corrected to **M**.

Root-widening was rejected: it both over-selects (the current predicate over `["lib","app","components","scripts"]` pulls in 20 admin-alert / report / fixture codes, and nothing textual separates a ParseWarning's `code:` from an admin alert's in the same file) and under-selects (three of the four residue codes reach their emit site as a _factory argument_ no `code:` regex can see). Replaced by a type-aware recognizer resolving four shapes through the TypeScript checker — literal, imported const, factory argument, union-typed parameter — with no roots list at all, so an emitter in an unanticipated directory is covered by construction.

**The row's premise was also half wrong.** It said the residue "can rot invisibly" because the union de-duplicates. The residue had NOT rotted: the gallery filtered on strict equality while the observe CLI used `.includes`, so the three multi-provenance codes were invisible to one reader and visible to the other. Probed: strict bucket 43, residue 4, overlap **0** — every residue row was load-bearing, and deleting them without also fixing the filter would have REMOVED a live gallery scenario. The filter change to `.includes` shipped as a required part of the work.

Result: manifest bucket 46 → 57, gallery 47 → 57. `VERSION_AMBIGUOUS` and `MI-1_VERSION_DETECTION_FAILED` correctly lost their `parse_warnings.code` attribution — both are `ParseError`-shaped. Guard at `tests/messages/_metaParseWarningCodeSites.test.ts` pins eight mutation families, including a golden 57-code snapshot, which is the only check that survives the collector itself narrowing (the generator and the guard share one recognizer, so every artifact-derived assertion moves with it).
