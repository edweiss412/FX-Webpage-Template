...
| `tests/e2e/helpers/rightNow.ts` (6 sites)                                                                  | "the compound tests … cover the recovery paths"; "the helper covers TIME-DRIVEN transitions"; "The audit suite documents which transitions …"; the `right-now.spec.ts` present-tense description; plus the two `requiresShowMutation` notes attributing the undriven `unknown`/`dateless` endpoints to the compound audits |
...
## BL-ANNOUNCE-REGION-UNMOUNT-CLASS — four more surfaces announce into a region their own success destroys — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`). Every conditionally-inserted `role="status"` region in `app/` and `components/` now uses one of two lawful shapes: mounted unconditionally with toggled TEXT, or announced through `UndoAnnounceContext` where the region's owner does not outlive the announcement. The entry was filed FROM a sweep, and a sweep's output is a list that rots — so the closure is `tests/components/_metaLiveRegionMounting.test.ts`, which WALKS the tree from disk. A new offending site fails there rather than waiting for the next person to run the same grep, and the walk found sites the filed list did not. `role="alert"` is deliberately out of scope: alerts ARE announced on insertion, so the conditional form is correct for them and forbidding it would be a change for symmetry rather than for a defect. Four of the first five surfaces repaired were NOT the shape the entry predicted once the surface was actually read, which is why each site was read before it was edited. The PENDING map is retained EMPTY: it is the mechanism that stops a new debt row from being added silently (a row fails the moment its file goes clean), not dead code. Cost side, recorded because it is the honest trade: three existing tests asserted the region was ABSENT once cleared, and each became mounted-AND-empty-AND-`sr-only` — strictly stronger, since absence could not distinguish a cleared status from a deleted one.

## BL-BULK-UNDO-ANNOUNCE-UNMOUNT — the bulk Undo-all announcement dies with its group — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`). Closed as part of the announce-family sweep above: the bulk Undo-all path announces through `UndoAnnounceContext`, whose provider region lives in the admin layout, so the announcement no longer dies with the group that triggered it. This is exactly the case shape 1 cannot serve — a region stable relative to a branch that itself unmounts is not stable — and it is why the class guard admits two shapes rather than one.

## BL-ADMIN-BADGE-CONTRAST-TOKEN — badge token pair + nav polish batch — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`). `--color-badge-count-runtime` / `--color-badge-count-text-runtime` ship in all three theme scopes with the `@theme` pair, guarded by `tests/styles/badge-count-contrast.test.ts`, which computes the WCAG ratio from the LIVE tokens rather than from a hardcoded expectation. **The entry's suggested value was wrong and was not used:** `#C25E00` measures 4.29:1, under the 4.5:1 AA floor for normal text. Shipped `#b85800` at 4.74:1. The guard carries a premise row asserting black-on-white computes to 21:1, so a broken luminance implementation cannot make the real assertion vacuous. The first implementation missed the `[data-theme="dark"]` scope and the meta-test caught it.

## BL-FRESHNESS-PROJECTION-NARROWING — seven freshness projections hash a wider model than their renderer paints, so they can over-cue — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`). All seven projections narrowed to call the SHIPPED renderer collapse rather than a re-typed copy of it — `isParseableUrl`, `stripOpeningReelText`, `partialAttendanceLabel`, `contactBlocks`, `hasContent`, `formatIsoDate`, `packItemLabel`. Contacts calls `contactBlocks` itself, so the drop-an-all-blank-row rule cannot be transcribed wrong or drift later. Every narrowing is backed by a probe in `tests/components/admin/review/sectionFreshnessProjection.probe.test.tsx` that renders the section twice through the shipped `step3Sections(...).render` and compares HTML byte for byte, plus a `D21`-`D27` no-cue row. **The entry's prescribed venue fix was wrong in the missed-cue direction and the probe caught it:** gating `googleLink` through `isParseableUrl` and stopping would have hidden that the section's eyebrow field COUNT counts the link whether or not it parses, so absent-to-unparseable moves a painted `(3)` to `(4)` that a gated href cannot see. The shipped projection keeps a presence bit ALONGSIDE the parsed href; `D21b` is the regression pin. That is the entry's own probe-before-tightening rule paying for itself on the first case it was applied to.

## BL-CREW-UNKNOWN-ASTERISK-TODAY-DATES — Today Tonight/Where date rows for date-restricted viewers — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`), ratified as SUPPRESS in the M-wave spec §1.1. The Today Tonight card no longer renders `Check in` / `Check out` for an `unknown_asterisk` viewer — a hotel check-in date IS the travel-in date, so this card was still telling viewers whose days are unconfirmed when the show runs, after the schedule, agenda and key-times strip had all stopped. The hotel NAME stays: the ratified decision is about dates, and dropping the whole card is a larger change nothing ratified. The Hotel row also loses its `span: 2` when alone, or the 2-up grid strands it. **The durable half is that the literal became a predicate.** The plan said to import the shipped `unknown_asterisk` predicate; there wasn't one — the comparison was written out at six sites, and this entry was the seventh site where somebody simply did not write it. `lib/crew/dateSuppression.ts` now owns the rule and the four date-suppression sites call it. Deliberately NOT swept onto `partialAttendanceLabel` (produces a LABEL, not a suppression) or the data-layer / live-now checks, which answer different questions about the same enum value; folding those in would make the predicate mean "equals this enum member", which is the drift it exists to prevent. Three tests, two of which exist so the first cannot be vacuous: an unrestricted viewer still gets both rows, and an `explicit` viewer still gets both rows — the assertion that fails if the gate is written as "any restriction", which would hide dates from the people who most need them.

## BL-CREW-UNKNOWN-ASTERISK-TRAVEL-LEAK — four date classes still reach a viewer whose days are unconfirmed — ARCHIVED 2026-08-07 (arc A, `feat/a11y-privacy-cluster`)
...
**A SECOND candidate is now RULED, on the same shape and for a related reason.** The fourth site — the roster's `partialAttendanceLabel` — is not a leak of the viewer's own schedule either, and it is not being gated. Owner ruling 2026-08-06 (arc A spec §1.1 item 1, Option A): **a peer's exact attendance days stay visible to every viewer, because coordination wins.** The label answers "who else is here, and when", which is the question the Show-crew section exists to answer; withholding it would degrade the coordination the roster is for in exchange for protecting a datum the show's own crew are expected to share. The header precedent above is the template: the marker's semantics are about the VIEWER's days, and a peer's days are not the viewer's schedule. Zero code changed at `components/crew/sections/CrewSection.tsx` (the `partialAttendanceLabel` call). The ruling is also carried in the `lib/crew/dateSuppression.ts` header's NOT-GATED list so a reader who finds the ungated site inherits the decision instead of re-filing it. Re-open trigger: the owner revisits the coordination-vs-privacy call.

The census found no further ungated viewer-specific date renderers, so the list above is believed complete.

**THAT COMPLETENESS CLAIM IS FALSE, and the correction is the entry's most useful residue.** The census enumerated fields whose PURPOSE is a date. It could not see the other way a date reaches the viewer: landing in a field that is not a date field at all. Cross-model diff review found three such classes during the fix, each with a server-render probe — the flight card's unvalidated remainder (`flightNo`, `airline`, per-segment `conf`, itinerary `confirmation`), the ground leg's author-typed `stage` and `time`, and a withheld raw row being read as evidence a date existed. All three are closed in the shipping diff and recorded as spec §4 limits 9, 10 and 11. The durable lesson: for a suppression rule, "which fields mean a date" is the wrong question; "which fields can CONTAIN one" is the right one, and the answer is every free-text field the parser fills.
...
**RESOLVED 2026-08-07** (arc A, `feat/a11y-privacy-cluster`). All four sites are dispositioned: three gated, one ruled.

**The three mechanical sites are gated off one `hideDates` computed once from `ctx.dateRestriction`,** the same `suppressesDates` predicate the four already-gated surfaces use. Ground-leg dates are withheld BEFORE the leg-retention check, so `dateIsPrimary` is false by construction and the `<time dateTime=…>` element never mounts — the attribute is itself the leak, and a fix that only suppressed the visible text would have shipped it anyway. The hotel `<dl>` stays unmounted, so a reservation renders name and its other non-date fields exactly as it does when both dates are null, mirroring the Tonight card. **The flight site turned out to be a BLOCK, not the one expression the entry cited:** five paths carry the same claim — the structured date, its `dateRaw` fallback arm (a raw M/D token leaks identically, and is what renders whenever ISO inference fails), the Today/Next chip, the upcoming-row HIGHLIGHT (the same "which flight is next" claim rendered as styling rather than text, driven by a separate index), and raw-fallback rows, which are withheld ENTIRELY because `seg.raw` is unparseable mixed text that cannot be split into date and non-date parts.

**The durable half is that suppression re-derives VISIBILITY, not just content.** Withholding rows without re-computing what mounts around them strands the worst possible artifact: a titled card wrapping nothing, which also suppresses the section's own empty state (a present card makes `allHidden` false). Every derivation that decides whether a row, card, or section renders — leg retention, the legs count, hotels presence, `showFlight` — now consumes the FILTERED sets, so an `unknown_asterisk` viewer never sees a blank row, an empty card, or section chrome wrapping nothing; each case falls back to the designed no-data state a viewer with no travel data receives. Blank and corrupt `assigned_names` are sanitized under suppression only, because `[""]` renders an empty "With " label — not visible content, and therefore not something that may keep a date-emptied leg alive.

**Two shapes are recorded as documented limits rather than papered over.** Raw-fallback flight rows lose their non-date content along with the date (a "Charter pending" note goes with the "3/22"); the text cannot be split safely and hiding is the conservative arm, with the fix being parser-side, not render-side. And the section's sheet-authored free-text fields — `transportNotes`, `hotelAddress`, hotel `stayRows` values — stay ungated: prose fields are not date fields, and a date an author types into one is editorial content the operator chose to write. Both are named in the `dateSuppression.ts` header, which is the surface inventory of record.

**Anti-vacuity, since a suppression test passes trivially against a card that never mounted:** the transport PII gate is satisfied in every fixture (the viewer is a resolved transport owner), each negative assertion is scoped to its own gated subtree because the legs and hotels blocks independently render `<time>` elements, expected date strings are derived from the fixture's own ISO values in all three rendered spellings, and four non-suppression twins pin the other viewer arms — `none`, `explicit` days, and the bare admin all still see every date, while `admin_preview` of an unknown-asterisk member takes the SUPPRESSED render like every other already-gated surface. A gate written as "any restriction" rather than the one kind passes every suppression case and fails the twins by name.

## BL-LIVE-REGION-AST-WALK-RESIDUE — four regions the new AST walk found, and one gate shape it still cannot see — ARCHIVED 2026-08-07 (arc A, `feat/a11y-privacy-cluster`)
...
**RESOLVED 2026-08-07** (arc A, `feat/a11y-privacy-cluster`). All four sites repaired, one commit each, and the PENDING map is empty again — by repair, not by deletion.

**What each repair actually needed, because none of the four was a mechanical toggle.** `MaterializeCard`'s gated node also owned the card's visual box, so hoisting it whole would have painted an empty bordered card at idle: the region hoisted UNSTYLED and the box moved inside it, result-scoped. `RevokeRowButton` got a second key-stable region beside the existing `arm-expiry-region` — deliberately not folded into that one, whose docblock contract is "set ONLY in the arm timer's callback"; the visible warning keeps its copy, single-sourced with the announcement, and drops the role it could never honour. `Step3ReviewModal`'s note was ALREADY mounted-and-toggled, which is why it read as fixed; the defect was one level up, in the four-arm chain its row belonged to, so the region hoisted above the chain with its text arm-scoped — load-bearing, since `publishState` has no arm-change reset and a region keyed on it alone would resurface stale failure copy in the other three arms.

**The `expanded` disclosure was the finding that changed shape under probing, and it is worth recording.** R1 dispositioned the step3 `-report-status` gate as an exemptible surface gate. R2 refuted that: the send is asynchronous, the operator can collapse the disclosure mid-flight, the outcome settles while the region is ABSENT, and re-expansion mounts region and settled text together — the class defect exactly, and three existing test cases already walked that path while pinning the silent miss as correct. The repair keeps the FORM mounted and gates only its contents, which puts the region above the disclosure without moving it out of the button row it shares; `contents` while collapsed keeps the collapsed section's flex flow unchanged, and the textarea and submit stay genuinely unmounted so the disclosure contract and draft preservation are untouched. Both collapsed-state pins moved WITH the behaviour rather than being deleted — the absence pin became visually-hidden-but-mounted, and the mid-flight case flipped from asserting the region was gone (the same statement as "this outcome cannot be announced") to asserting it pre-exists the settled text.

**The limit is re-homed, not dropped.** Cross-component gate blindness moves to the `PENDING` docblock in `tests/components/_metaLiveRegionMounting.test.ts`, which is the owning surface's limits record, with the live instance named: the agenda parsing region's own in-file guard was repaired, but the SECTION above it is gated by `includesAgenda` in `components/admin/review/sectionInclusion.ts`, so that component's first paint still cannot announce. The entry's accepted first-paint limit therefore survives its repair, deliberately and in writing.
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`), landing the ratified §12.4 amendment (M-wave spec §1.1.5) in the same commit, because shipping a spec claim ahead of the code that satisfies it is the exact defect this closure adds a guard against. `RESYNC_QUALITY_REGRESSED` now links to `#warnings`, the panel its own copy tells the reader to open, instead of `#overview` at the top of the page. **Two things were false before this and neither was detectable:** §12.4 said "No action link." while `lib/adminAlerts/alertActions.ts` had been building one the whole time, aimed at the wrong section. Nothing caught it because nothing READ that sentence — the x1 catalog-parity gate compares four fields and the description column, where the action-link claim lives, is not extracted at all, so `pnpm gen:spec-codes` produces a zero-byte diff for such an edit. The plan's predicted x1 RED is therefore unreachable, and that is recorded rather than worked around. `tests/cross-cutting/specActionLinkParity.test.ts` gives the sentence an executor in both directions and found two defects on its first run — the shipped mismatch, and the amendment text itself, which quoted the retired phrase and thereby re-asserted it. `#warnings` needed a DOM id, added to ONE section rather than `id={s.id}` for all: every registry id would otherwise become a page-wide anchor, including `overview`, which `OverviewSection` already owns.

## BL-HELP-UI-LABEL-CROSSWALK-EXACT-MATCH — tighten short action labels in the /help UI-label crosswalk — ARCHIVED 2026-08-05 (M-wave W-UI, `feat/m-wave-ui`)
...
**RESOLVED 2026-08-05** (M-wave W-UI, `feat/m-wave-ui`) for the class it names; the two instances it cites are a DOCUMENTED LIMIT with the residual filed. Labels of six characters or fewer (INCLUSIVE — `Viewer` is six) now match only at word boundaries in both crosswalk layers, through one shared matcher so the heuristic and registry halves cannot disagree about what a match is; longer labels keep the substring test, because a long label cannot collide with an identifier by accident but CAN legitimately be split across JSX children, and a guard that cries wolf gets exemptions added to silence it. Import statements also leave the haystack: a specifier is never rendered. **Both named instances survive, for two reasons the tier cannot fix, and the probe is the deliverable.** `Viewer` occurs as a bare word-bounded identifier in a TYPE ANNOTATION (`viewer: Viewer`), which no lexical narrowing of a whole-source haystack distinguishes from a button label — excluding identifiers wholesale would break every label that IS its component name. And they are GOOGLE DRIVE's controls ("click **Share** on that folder… Give it **Viewer** access"), so the copy is CORRECT and the plan's instruction to rewrite it would have made accurate documentation inaccurate; `_uiLabelExceptions.ts` cannot hold them either, since every row must cite a `DEFERRED.md` id and these are not deferrals. Both are pinned as an executable documented limit that FAILS the day the real fix lands. Residual filed as `BL-CROSSWALK-HAYSTACK-RENDERED-TEXT-ONLY` under deferral exception (c).

## BL-OPS-LOG-OAUTH-EMITS — the `OAUTH_REDIRECT_INVALID` branches emit no durable code — CLOSED 2026-08-07 (`feat/ops-log-code-emits`, Cluster E, SHIPPED)

**Status:** CLOSED · **Severity:** low · **Class:** OBSERVABILITY · **Effort:** S · **Filed:** 2026-08-06 (L-wave decomposition of `BL-OPS-LOG`)

`OAUTH_REDIRECT_INVALID` was never a persisted `code:` anywhere. It existed only as a `validateNextParam` return discriminant (`lib/auth/validateNextParam.ts:13`) and a §12.4 catalog copy row — so an operator hitting it left no durable trace, while its sibling `OAUTH_STATE_INVALID` DID have adjacent durable emits. That asymmetry was the whole entry.
...
- `app/auth/callback/route.ts` — `signInRedirect(request, "OAUTH_REDIRECT_INVALID", …)`
- `app/api/auth/google/start/route.ts` — same call shape
- `app/api/auth/picker-bootstrap/route.ts` — `htmlResponse("OAUTH_REDIRECT_INVALID", 403)`, three branches

**No registry obligation, and this was stated so implementation did not invent one:** all three routes are **GET** handlers, so none is inside invariant 10's mutating-method scope. The emits are an observability improvement; they create no `AUDITABLE_MUTATIONS` row and no guard obligation. Decomposition record: `BACKLOG-archive.md` § `BL-OPS-LOG`.

**Resolution: shipped.** Each branch emits `log.warn` with `code: "OAUTH_REDIRECT_INVALID"` and a `context.reason` naming the call site — six reason values across five sites. Spec `docs/superpowers/specs/2026-08-07-ops-log-code-emits.md`, plan `docs/superpowers/plans/2026-08-07-ops-log-code-emits.md`, behavioral coverage in `tests/auth/oauthRedirectInvalidTelemetry.test.ts`.

**The persisted-code fork, recorded so it is not re-derived.** The adjacent shipped emits on the callback route persist FORENSIC codes deliberately distinct from what the user receives — `OAUTH_EXCHANGE_THREW` / `OAUTH_EXCHANGE_REJECTED` against a user-facing `OAUTH_STATE_INVALID`, neither in the §12.4 catalog. That made the choice a genuine fork rather than an obvious copy, and the precedent does NOT generalize here: there the internal event is an upstream Auth API failure and the user-facing code is a translation of it, so a distinct code names a genuinely distinct proposition. Here the internal event and the user-facing code assert the SAME proposition — the redirect target was invalid — so persisting `OAUTH_REDIRECT_INVALID` is faithful, and it is what makes this entry's own complaint false going forward. Per-branch discrimination is preserved by `reason`, not surrendered.

**Site 5 emits two reasons, and the split is load-bearing.** The shipped guard is one condition with three disjuncts and its first, `!intent`, is not one cause but eight — `verifyPickerIntent` returns a bare `null` for an absent or malformed `t`, a signature mismatch, a decode or parse throw, an invalid payload shape, and **expiry** alike. Labelling all of that `mismatch` would be actively misleading in the most common benign case: an operator reading "mismatch" on a crew member who simply sat on the page past `exp` would investigate tampering that never happened. `bootstrap_intent_target_mismatch` is now asserted ONLY when an intent verified and then disagreed.

**Deliberate opacity left in place**, both recorded as documented limits rather than defects:

- **The `next` validator's internal cause** (spec §5.1). `validateNextParamDetailed` returns an identical opaque outcome from five distinct rejection paths. `reason` discriminates the CALL SITE, not which validation rule refused. Widening `ValidateNextParamOutcome` would change the return type of a security-sensitive primitive shared by every auth entry point and wants its own review of the allow-list semantics; not filed speculatively, per the ledger filing bar.
- **The eight `verifyPickerIntent` null causes** (spec §5.6), collapsed under `bootstrap_intent_unverified` for the same reason — it is a signature-verification primitive whose contract deserves its own review. The label claims only that verification failed, never why, and never asserts tampering.

**No emit carries the rejected `next` value** (spec §5.2): it is attacker-controlled text of unbounded length and `sanitizeContext` redacts emails rather than generally neutralizing adversarial input. Pinned by a whole-record accept-set over exactly the nine fields `persistAppEvent` writes, so a leak cannot hide in `context`, in `message`, or in a promoted column.

**The refusal is unchanged at every site** — status, redirect target, PKCE cookie clearing, and the cataloged 403 body — asserted in the same cases as the emits. Every emit is `try`/`catch`-wrapped and that wrapper is itself tested (`AC-7`): against a rejecting sink all six sites still produce their unchanged refusal, with the RED for each obtained by removing that site's wrapper and observing the refusal collapse into an unhandled rejection.

## BL-OPS-LOG-ONBOARDING-EMIT — `ONBOARDING_OPERATOR_ERROR` has no producer — CLOSED 2026-08-07 (`feat/ops-log-code-emits`, Cluster E, SHIPPED)

**Status:** CLOSED · **Severity:** low · **Class:** OBSERVABILITY · **Effort:** S · **Filed:** 2026-08-06 (L-wave decomposition of `BL-OPS-LOG`)

`ONBOARDING_OPERATOR_ERROR` was render-only. `components/admin/OnboardingWizard.tsx` called `messageFor("ONBOARDING_OPERATOR_ERROR")` to display the §12.4 catalog copy to Doug, and **no `log.*` emit existed anywhere for it** (re-verified 2026-08-06: the only non-catalog hits in the tree were that render site and the component's own header comment). So the operator saw the error and the system retained nothing about it.
...
**Resolution: shipped.** One `log.error` in the async server component's body, guarded by `!service.ok`, carrying `code: "ONBOARDING_OPERATOR_ERROR"` and a `reason` read from the resolver's result — never re-derived, so the emit cannot disagree with the branch that produced it. Coverage in `tests/components/admin/OnboardingWizard.test.tsx`.

**The resolver's `try` was NARROWED so `json_malformed` never labels a value that parsed.** The shipped `try` wrapped both the `JSON.parse` and the later `client_email` property read, so `GOOGLE_SERVICE_ACCOUNT_JSON=null` parsed SUCCESSFULLY, threw a `TypeError` on the read, and landed in the `catch`; naming that `json_malformed` would have asserted a parse failure that demonstrably did not happen. Probed, not assumed — `null` is the only value reaching the shipped catch with the parse having succeeded, since numbers, strings and arrays all yield `undefined` on the read without throwing. Four reasons now discriminate four operationally different causes: `env_missing`, `json_malformed`, `json_not_an_object`, `client_email_missing`.

The restructure is behavior-preserving: the ok/not-ok partition over every input is unchanged, only the label is new. The three shipped render assertions pass byte-identical, which is the executable form of the `impeccable-gate: N/A — no UI surface` determination.
...
**Resolution: IMPLEMENTED, wider than filed.** The nine named consts are `cn(...)`-wrapped, which puts them inside the reach of `better-tailwindcss/enforce-canonical-classes` — the lint rule is the cover, and `quality` runs `pnpm lint` as a required CI context, so drift inside any of them fails CI. A dedicated guard was written and later DELETED: pinning declarations by name required re-deriving the rule's own identifier resolution, which took six review rounds and would have ended in a checker-backed symbol graph inside a test (closeout §12.1g). The behavioral evidence the entry asked for was observed in both directions: a planted `min-h-[44px]` was SILENT under `pnpm lint` pre-wrap in all three files and REPORTED post-wrap in each. The entry's worked example was real — `THUMB_BASE`'s `h-5 w-5` became `size-5` the moment the rule could see it. The census then went well past the nine: the entry's `rg` shape misses one-line declarations, function-scoped consts, object VALUES and literal concatenations, so it was re-run as an AST pass, and the impeccable gate found even THAT under-inclusive (no `/opacity` modifier, no single-token values). Two rounds, 132 sites across 38 files (31 and 14, overlapping on 7 — counted from the commits rather than added up, after the cross-model review probed the arithmetic), ALL repaired in the same PR — deferral under class-sweep exception (c) was considered and rejected on measurement: the transform is mechanical, `tsc --noEmit` clean, and the whole class produced only four Tailwind drifts (`h-5 w-5`→`size-5`, `break-words`→`wrap-break-word`, `min-w-[8.5rem]`→`min-w-34`, `h-full w-full`→`size-full`). Runtime is unchanged on a checked fact, not an assumption: `cn` is exactly `parts.filter(Boolean).join(" ")`, tailwind-merge having been rejected by ratification precisely so a wrap cannot re-resolve conflicting classes.
...
## BL-SPECLINT-PROSE-COUNT-PARITY — numeric-sweep extension: prose cardinalities against executable declarations — ✅ RESOLVED (2026-08-15, `feat/speclint-prose-count-parity`)

**Resolved by:** `feat/speclint-prose-count-parity`. All three measured shapes ship as ADVISORY codes in `lib/specLint/numerics.ts` beside `NUMERIC_NOUN_MISMATCH`, surfaced through `pnpm spec:lint` like every other check: `SCRIPT_CONSTANT_PARITY` (a) reads a script's module-local `const EXPECTED_* = <integer>` TEXTUALLY and compares it against a same-line cardinality whose noun singularizes to the noun derived from the constant's identifier; `SIBLING_LIST_CARDINALITY` (b) compares an "N nouns:" claim against the sibling items of the list immediately beneath it, through the calibrated gate ladder; `TEMPLATE_QUANTITY_DRIFT` (c) compares digit quantities across near-identical repeated lines within one document, all-pairs at set-Jaccard ≥ 0.85 with numerals participating in tokenization. All three honor one three-part exclusion rule — fenced block, a dated qualifier phrase binding its NEAREST preceding number within 40 characters, and any line carrying an ISO date — so dated historical records are never compared, which was this entry's own boundary.

Cross-file reads go through the existing I/O boundary: `runLint` resolves the `scripts/` paths a document names (by path or basename) via the injected `FileResolver` and passes `{path → text}` into `checkNumerics`, so `lib/specLint/numerics.ts` still performs no I/O and `tests/specLint/_metaPureCore.test.ts` still holds.

**Measurement (`docs/superpowers/specs/probes/2026-08-11-speclint-corpus-scan.ts`, which drives the real CLI entry point once per corpus document and captures each report rather than trusting exit codes):** 1052 of 1053 corpus documents linted in ~11s — one tracked symlink the CLI refuses by design, named in the record. 0 `SCRIPT_CONSTANT_PARITY`, 174 `SIBLING_LIST_CARDINALITY`, 84 `TEMPLATE_QUANTITY_DRIFT`, every one advisory, no hard finding from any new code. Shape (b) started at 179 — the calibration's 190 tier-6 mismatches less the 11 that the contract's qualifier and ISO-dated-line exclusions remove, the divergence the spec predicted — and lost six more when whole-diff review R20's class sweep re-derived the sibling counter's list-extent reading; those six were false, and none entered. Shape (a) finding zero live mismatches is the expected result and not a dud: the two live qualifying occurrences both agree with `EXPECTED_SITE_TOTAL = 37`, and the arm is the tripwire that notices when they stop agreeing.

**Deliberately NOT covered — the mechanical arm's documented limit.** The wedge-remeasure anchor (quantity drift between a counting rule and its list-item disposition template) is NOT expressible boundedly: three candidate designs were probe-refuted, and the pair's line-level Jaccard is 0.0748. That instance stays covered by the RULE half of this entry's filing — the numeric sweep promoted into `docs/agents/spec-self-review.md` — and `tests/specLint/numerics.test.ts` pins it as an explicit NO-FLAG fixture so no later round re-proposes a refuted design. Cross-document template drift remains out of scope per the originating filing.

**Enrolled in the source-mutation registry** (`tests/mutation/source/registry.ts`, surface id `specLintNumerics`), so the arms' own coverage is machine-checked rather than argued: `pnpm mutation:guards` reports 520 mutants, 470 killed, score 1.0000 against a 0.9 floor with an EMPTY unaccepted-survivor set, and each of the 50 blessed survivors carries a written reachability or control-flow argument. There is no `accepted-gap` row on this surface.

**What the arms are held to, stated because the review rounds turned on it.** They read two grammars they do not parse, so their bound is the LIVE CORPUS rather than CommonMark and JavaScript: a false advisory is a defect when it fires on a tracked document for a reason the classification record does not already carry, and a constructed input no tracked doc contains is a documented limit. The diff stage ran 21 rounds before that was stated; the filing below is where it is written down.

Spec: `docs/superpowers/specs/2026-08-10-speclint-prose-count-parity.md` (cross-model APPROVED, round 8). Plan: `docs/superpowers/plans/2026-08-10-speclint-prose-count-parity.md` (round 6). Round-economy filing: `docs/review-rounds/feat/speclint-prose-count-parity/a7393880ae6d.md`.

**Original entry (filed 2026-08-09, preserved verbatim as history):**
