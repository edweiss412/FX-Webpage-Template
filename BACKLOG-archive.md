...
**Resolution:** the syntactic 19-spelling matcher in `tests/cross-cutting/no-absolute-self-redirect-audit.ts` is replaced by TWO-PRONG type-checker resolution over walked roots extended to `app/** + lib/** + root middleware`: prong 1 flags every call whose resolved signature's declaration is `redirect` on a container named `NextResponse`/`Response`; prong 2 flags every OTHER reference to that method — property/element access, binding elements, and destructuring-assignment members (via the vendored compiler's `getTypeOfAssignmentPattern`) — type-decided, with extraction never allow-listable. All four residual classes this entry filed (helper return, class field, re-export, dynamic dispatch) are caught, plus the families four adversarial rounds surfaced on the way: twelve typed value-flow shapes (structural properties, composites, `.call`/`.apply` adapters, `as any` VALUE laundering), ten literal-typed computed-key extraction shapes, union-typed keys, and eight destructuring-assignment forms — R1–R57 positives, N1–N7 negatives, E1–E2 escape pins in `tests/cross-cutting/no-absolute-self-redirect.test.ts`. Precise residual boundary, pinned AS BEHAVIOR by fixtures: receiver laundering (`(NextResponse as any).redirect`, E1), WIDENED non-literal-typed computed keys (E2), and reflection/eval (documented, not statically expressible); plain-JS modules are fenced out of the walked roots by a sentinel (tsconfig `include` is TS-only + `checkJs` off, so `tsc --noEmit` gives JS no backstop). Spec: docs/superpowers/specs/2026-08-01-redirect-guard-type-aware-design.md (spec APPROVE r4, plan APPROVE r3; probe harness committed alongside as `…-probe*.mjs`). Original entry below.
