...
*Scope, stated plainly — and narrowed twice during review, so read the claim as written.* This closes the wide-fallback path for **every Next-rendered surface with a React root** — every page of the product proper. Deliberately NOT "everything a user reads": the four auth error documents below are read when they appear. Two things it does NOT reach, both documented rather than quietly implied:
