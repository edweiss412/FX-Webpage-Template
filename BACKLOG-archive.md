...
**Layer 1 keys by name AND parameter type list.** Name-only was wrong in both directions (Codex R3 HIGH, with a probe): an added OVERLOAD passed because the name was already known, and `drop function name(uuid)` erased the whole name even though other overloads survived. Arity alone did not fix it — the probe's two overloads both take one argument. Type spellings are canonicalised on both sides, because the first type-list key raised nine false alarms on the real corpus (`timestamptz` vs `timestamp with time zone`, `int` vs `integer`, a schema-qualified composite), and noise a contributor cannot act on is how a tripwire dies. The first type-list key ALSO had a real miss, found in review (Codex R4 HIGH): it dropped each parameter's first word as though it were always a name, so an UNNAMED `time with time zone` and an unnamed `timestamp with time zone` both collapsed to `with time zone` and keyed identically — adding one as an overload of the other read as already-known. Same for the `without time zone` and `varying` pairs. The key now asks whether that first word heads a multiword TYPE before treating it as a name, and all three pairs are pinned as rows, alongside the converse (a named `p_at timestamptz` must still key on its type). Where it degrades NOW it does fail conservatively: an unmodelled alias, or a parameter literally named after a type, produces a false alarm that regenerating ends — not a collision that hides an overload. The exact comparison stays Layer 2's, against Postgres's own identity encoding.

**Loud at BOTH layers, which the first version was not.** Layer 1 (the DB-free tripwire) now parses `create function` names as well. Covering only Layer 2 left the exact hole this entry names: forget the regen AND the validation apply, and Layer 2 compares two equally stale function sets and agrees with itself (Codex R1 HIGH). Proven by planting a mutant — deleting `is_admin` from the manifest fails Layer 1 by name.
