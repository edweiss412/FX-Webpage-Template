...
**Layer 1 is NAME-KEYED, and that bound took five review rounds to get honest.** Successive attempts keyed on arity, then on a normalized parameter type list; each closed the case it was shown and left another (two one-argument overloads collide under arity; an unnamed multiword type collides under a naive type list; and no DDL-derived key sees a posture flip, a return-type change or a parameter rename at all). A DDL string is not an identity — only `pg_get_function_identity_arguments` is, and the DB-free layer cannot call it. A key that looked exact and was not is worse than one that is honestly coarse.

**So the division of labour is explicit, and it is the three layers TOGETHER that close this entry.** Layer 1 (DB-free, every run) catches a function NAME the manifest has never heard of — the #9 vector, a migration landing with the regen forgotten. Layer 3 (`unit-suite-db`, local stack) compares the committed manifest against a FRESH introspection byte-for-byte: it is the exact tier and catches everything Layer 1 cannot, because it compares Postgres's own encoding instead of guessing at it, so a stale manifest cannot land. Layer 2 (`x-audits`, validation) asserts validation is a superset of that manifest at the full signature tier. Separately none of them suffices, and Layer 1 was never the one that could.
