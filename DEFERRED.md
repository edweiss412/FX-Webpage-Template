...
## drive-export-timeout — companion untimed-`fetch` asset sites (2026-06-26)

### DXT-1 — [WILL-DO] AbortSignal stall-guard for the three asset-download `fetch()` sites

- **What:** the onboarding scan hang (a single sheet whose Drive xlsx export stalled with no time bound → the whole Step-2 scan wedged at "18 of 19" until the route's 300s maxDuration killed it) was fixed by bounding the **export** `fetch` in `lib/drive/fetch.ts` with an `AbortController` stall-guard (`DRIVE_EXPORT_TIMEOUT_MS`, surfaced as a transient `DriveFetchError(504)` so `withDriveRetry` retries it). A class-sweep found three **companion** raw `fetch()` calls that share the exact same untimed-stall failure mode — asset/snapshot content downloads — and were left out of that diff because they live on a different surface (the cron/apply asset pipeline) with different, fail-soft (`return null`) semantics:
...
- **Why these stall the same way:** `readBoundedWebStream` / `bytesFromWebStream` cap **bytes**, not **time** — a stalled or slow-trickle response body hangs the read indefinitely, exactly like the export did. The byte cap bounds memory, not wall-clock.
- **Why deferred (not fixed in the export diff):** the reported + reproduced bug is the onboarding **export** path; these three are a distinct surface in the cron/apply pipeline. They already fail-soft (`return null` on `!ok`/`!body`), so the correct guard wiring (abort → catch → preserve `return null` vs. let it throw into the caller's existing error handling) needs per-site judgement + its own tests, which would widen the export-hang diff into the cron path. Tracked here rather than bundled.
- **Concrete fix:** wrap each with the same `AbortController` + `setTimeout(...).unref()` stall guard (factor the helper out of `lib/drive/fetch.ts` if shared), aborting the body read too (one signal covers fetch + stream read), and on abort preserve each call site's existing fail-soft contract (return null / propagate per the caller). Add a per-site stall test mirroring `tests/drive/fetch.test.ts`'s `stallUntilAborted` pattern.
- **Trigger:** next time any of these three files / the cron-apply asset pipeline is touched, OR a dedicated follow-up — whichever comes first. The export fix is the template.
