...
### DXT-3 — [WILL-DO] Timeout for the off-onboarding-path cron/apply metadata reads

- **What:** four untimed metadata reads of the DXT-1 `files.get`/Sheets class, but off the onboarding scan path (cron/apply) — fix with the same per-call `{ timeout, retry: false }` + the `driveErrorStatus` `TimeoutError`→504 mapping DXT-1 established:
  - `lib/sync/verifyReelOnApply.ts:59` (`defaultDrive().getFileMetadata`) — a RAW `getDriveClient().files.get({…})` that is untimed AND not wrapped in `withDriveRetry`/`driveFilesGet` (needs BOTH; route through `driveFilesGet` or add both).
  - `lib/sync/runScheduledCronSync.ts:1617` (`listSpreadsheetSheets`) — `sheetsClient.spreadsheets.get({…})` (Sheets v4, untimed; different client built from `getDriveAuth()`).
  - `lib/sync/runScheduledCronSync.ts:1636` (`getSpreadsheetRevisionId`) — `drive.revisions.list({…})` (untimed).
  - `lib/sync/applyStaged.ts:917` (`defaultRetryEmbeddedRevisionAvailability`) — a RAW `getDriveClient().revisions.list({…})` on the apply path, untimed AND not retried.
- **Why deferred:** smaller + a different pattern (gaxios per-call timeout, like DXT-1) than the DXT-2 stream guard; split out to keep each diff reviewable. The Sheets `spreadsheets.get` is on a separate client and is not currently wrapped in `withDriveRetry`, so it needs either its own retry wrapper or a fail-fast-on-timeout decision.
- **Trigger:** next time `verifyReelOnApply` / the cron `defaultDriveClient` is touched, OR a dedicated follow-up. Keep `revisions.*` free of `supportsAllDrives` (`_sharedDriveSupportContract`); if a new enclosing fn wraps `.files.get`, add a `_scopeCheckContract` exemption row in the same diff.
