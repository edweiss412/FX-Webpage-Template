...
### DXT-1 — [WILL-DO] AbortSignal/timeout stall-guard for the remaining untimed Drive reads

- **What:** the onboarding scan hang (a single sheet whose Drive xlsx export stalled with no time bound → the whole Step-2 scan wedged at "18 of 19" until the route's 300s maxDuration killed it) was fixed by bounding the **export** `fetch` in `lib/drive/fetch.ts` with an `AbortController` stall-guard (`DRIVE_EXPORT_TIMEOUT_MS`, surfaced as a transient `DriveFetchError(504)` so `withDriveRetry` retries it). A class-sweep (extended by the adversarial review of that fix) found the following **companion** untimed Drive reads that share the exact same silent-stall failure mode and were left out of that diff:

  **Asset/snapshot content downloads** (cron/apply pipeline; fail-soft `return null`):
  ...
  **`drive.revisions.get` stream reads** (same two functions, second untimed read in each):
  - `lib/sync/assetRecovery.ts` `fetchLinkedRevisionBytes` — `drive.revisions.get(…, {responseType:'stream'})` → bounded-stream read
  - `lib/sync/defaultSnapshotAssetsForApply.ts` `fetchLinkedRevisionBytes` — same shape

  **`files.get` metadata reads on the onboarding HOT path** (same path the user's hang hit — `getDriveClient()` builds the gaxios client with no `timeout`, gaxios default `timeout:0`):
  - `lib/drive/fetch.ts` `driveFilesGet` / `fetchFileForExport` — the before-`get` AND after-`get` per sheet (plus up to two conditional enrich gets) → 4+ of the ~6 Drive calls per sheet. A silent socket stall on any of these hangs `prepareOne` identically to the export bug.

- **Why these stall the same way:** `readBoundedWebStream` / `bytesFromWebStream` cap **bytes**, not **time** — a stalled/slow-trickle body hangs the read indefinitely; and gaxios `files.get` with `timeout:0` has no wall-clock bound at all. The byte caps bound memory, not time.
- **Why deferred (not fixed in the export diff):** the reported + reproduced bug was specifically the **export render** (the heavy server-side xlsx synthesis — the live repro measured 20.8s there vs a rock-solid 165–255ms for every `files.get`), so the export was the right and sufficient fix for the incident. The asset/revision reads live on a distinct fail-soft cron/apply surface; bounding the `files.get` reads needs gaxios-timeout error-classification (a gaxios timeout throws `code:'ECONNABORTED'`/`ETIMEDOUT` with no numeric status, so `withDriveRetry`'s 5xx classifier would NOT retry it as-is). All need per-site judgement + tests that would widen the focused export-hang diff.
- **Concrete fix:** (a) asset/revision reads — wrap each with the same `AbortController` + `setTimeout(...).unref()` guard (factor the helper out of `lib/drive/fetch.ts`), aborting the body/stream read too, preserving each site's fail-soft contract; (b) `files.get` — either pass a per-call `timeout` into `drive.files.get({…, timeout})` and extend `withDriveRetry`/`driveErrorStatus` to treat `ETIMEDOUT`/`ECONNABORTED` as transient, **or** set a single gaxios client-level `timeout` in `getDriveClient()` (`lib/drive/client.ts`) as the comprehensive class-wide defense for ALL Drive API calls (cron + sync + onboarding) — note that touches a shared client used everywhere, so it needs its own blast-radius review + tests. Add per-site stall tests mirroring `tests/drive/fetch.test.ts`'s `stallUntilAborted` / `responseWithStallingBody` patterns.
- **Trigger:** next time any of these files / the cron-apply asset pipeline is touched, OR a dedicated follow-up — whichever comes first. The export fix (`fetchXlsxExportBytes`) is the template. The `files.get` hot-path gap is the highest-value piece since it is on the exact onboarding-scan path the original hang occurred on.
