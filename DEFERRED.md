...
- **What:** five untimed asset/revision reads on the cron/apply pipeline share the same silent-stall class (a byte cap bounds memory, not time, so a stalled/slow-trickle body or stream hangs forever):
  ...
  **Cron/apply-path metadata reads** (same untimed-`files.get`/Sheets class as DXT-1, but off the onboarding scan path — fix with the same per-call `{ timeout, retry: false }` + `driveErrorStatus` mapping DXT-1 established):
  - `lib/sync/verifyReelOnApply.ts:59` — a RAW `getDriveClient().files.get({…})` that is untimed AND not wrapped in `withDriveRetry`/`driveFilesGet` (so it needs BOTH the timeout and retry coverage; route it through `driveFilesGet` or add both).
  - `lib/sync/runScheduledCronSync.ts:1582` — `sheetsClient.spreadsheets.get({…})` (Sheets v4, untimed).
  - `lib/sync/runScheduledCronSync.ts:1607` — `drive.revisions.list({…})` (untimed).
