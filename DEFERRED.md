...
### CASP-1 — [P2→deferred] Three per-show HoverHelp tooltips orphaned by the section dissolution

- **What:** the affordance-matrix rows `help-affordance--per-show-crew--tooltip`, `help-affordance--per-show-sync-footer--tooltip`, and `help-affordance--per-show-data-quality--tooltip` no longer render on the consolidated surface — their host sections (standalone Crew section, Sync footer, flat Data-quality panel) were removed. The three testids sit in `DEFERRED_TESTIDS` (`app/help/_affordanceMatrix.ts:297-299`), exempting them from the affordance-matrix parity render-check while their `/help` target anchors stay resolvable. Recorded in `.superpowers/sdd/task-13-report.md` §Watchpoints (item 2) and the `DEFERRED_TESTIDS` code comment.
- **Why deferred:** whether each tooltip should be re-homed on the consolidated surface (un-defer: re-anchor the `HoverHelp` to its new home — crew-in-review-surface / StatusStrip sync badge / per-section warning control) or retired (drop the matrix row + the `/help` page section) is a UX/design decision that belongs with the impeccable dual-gate pass, not the Task 13 structural rebuild.
- **Trigger:** **Task 16** (the impeccable critique + audit gate on this same branch) — re-home-or-retire each of the three affordances. When Task 16 resolves a tooltip, it updates or removes both the matching `DEFERRED_TESTIDS` entry and this row (a re-homed affordance un-defers into a live matrix row; a retired one drops the row + help section).
