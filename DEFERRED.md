...
- **Resolved the test-seam gap:** the default-port bodies had ZERO unit coverage (the asset tests stub the drive port); the extracted helpers are now exercised directly with an injected stalling `fetch`/`drive` + tiny budget — including the key non-tautology test that a slow-but-PROGRESSING download survives past the idle budget (proving idle-reset, not total-time).

### DXT-3 — [WILL-DO] Timeout for the off-onboarding-path cron/apply metadata reads

- **What:** three untimed metadata reads of the DXT-1 `files.get`/Sheets class, but off the onboarding scan path (cron/apply) — fix with the same per-call `{ timeout, retry: false }` + the `driveErrorStatus` `TimeoutError`→504 mapping DXT-1 established:
  - `lib/sync/verifyReelOnApply.ts:59` — a RAW `getDriveClient().files.get({…})` that is untimed AND not wrapped in `withDriveRetry`/`driveFilesGet` (needs BOTH; route through `driveFilesGet` or add both).
  - `lib/sync/runScheduledCronSync.ts` `getSpreadsheetSheets` — `sheetsClient.spreadsheets.get({…})` (Sheets v4, untimed; different client built from `getDriveAuth()`).
  - `lib/sync/runScheduledCronSync.ts` `getSpreadsheetRevisionId` — `drive.revisions.list({…})` (untimed).
