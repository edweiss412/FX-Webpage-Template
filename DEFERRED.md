...
### DXT-1 — [✅ DONE] `files.get` metadata-read timeout on the onboarding hot path

- **What:** `getDriveClient()` built the gaxios client with no timeout (gaxios default unbounded), so the before-`get`/after-`get` issued around every sheet (4+ of ~6 Drive calls per sheet, on the SAME onboarding path the original hang hit) could stall `prepareOne` identically to the export bug.
- **Done (this PR):** `driveFilesGet` (`lib/drive/fetch.ts`) now passes a per-call `{ timeout: DRIVE_FILES_GET_TIMEOUT_MS (10s), retry: false }` to `drive.files.get`, and `driveErrorStatus` maps the timeout code → transient 504 so the already-wrapping `withDriveRetry` retries then throws a typed error (same bounded contract as the export guard). `retry: false` keeps `withDriveRetry` the single retry layer. Threaded via `DriveFetchOptions.metadataTimeoutMs` for test injection. Chose the **scoped C1** over a client-level `getDriveClient` timeout (C2) because C2 would also abort legitimate slow crew-page agenda/reel **stream** downloads sharing that client, and C2 is anyway incomplete (the cron path builds 2 clients from `getDriveAuth()`, bypassing `getDriveClient`). ~14 `getDriveClient()` call sites confirmed this.
...
### DXT-2 — [WILL-DO] AbortSignal/timeout stall-guard for the asset/revision stream reads

- **What:** five untimed asset/revision reads on the cron/apply pipeline share the same silent-stall class (a byte cap bounds memory, not time, so a stalled/slow-trickle body or stream hangs forever):

  **Asset/snapshot content downloads** (web `fetch` → `response.body` ReadableStream; fail-soft `return null`):
  - `lib/sync/assetRecovery.ts:675` — `fetch(entry.contentUrl, …)` → `readBoundedWebStream(response.body, MAX_RECOVERY_SINGLE_BYTES, …)`
  - `lib/sync/defaultSnapshotAssetsForApply.ts:58` — `fetch(entry.contentUrl, …)` → `readBoundedWebStream(response.body, MAX_SINGLE_ASSET_BYTES)`
  - `lib/sync/runScheduledCronSync.ts:1599` — `fetch(contentUrl, …)` → `bytesFromWebStream(response.body, 50 * 1024 * 1024)`

  **`drive.revisions.get` Node-stream reads** (second untimed read in each `fetchLinkedRevisionBytes`):
  - `lib/sync/assetRecovery.ts` `fetchLinkedRevisionBytes` — `drive.revisions.get(…, {responseType:'stream'})` → `readBoundedNodeStream`
  - `lib/sync/defaultSnapshotAssetsForApply.ts` `fetchLinkedRevisionBytes` — same shape

- **Why deferred:** distinct, harder shape than the export/`files.get` fixes — three hard constraints from the DXT investigation: (1) `tests/sync/_streamingHashContract.test.ts:30-34` FORBIDS `arrayBuffer()` in `assetRecovery.ts`/`defaultSnapshotAssetsForApply.ts` and requires the bounded-stream readers, so the guard must wrap the **streaming** read in place and CANNOT reuse `fetchXlsxExportBytes` (which uses `arrayBuffer()`); (2) these reads are **fail-soft `return null`**, so the guard must abort→return null (NOT throw like the export/`files.get` fixes) to avoid converting a degraded-asset case into an apply-aborting throw; (3) the 2 `drive.revisions.get` reads yield a **Node Readable** consumed by `for await` — the gaxios signal/timeout bounds the handshake but NOT an already-returned mid-trickle stream, so the guard must ALSO `stream.destroy(err)` to interrupt the in-progress read. Plus a TEST-SEAM gap: all five sites are exercised by ZERO unit tests today (the asset tests stub the drive port), so each needs a new injectable `fetch`/`drive` seam (or an extracted, directly-unit-tested timed-stream helper) before a stall test is possible. All reads run PRE-lock (outside `withShowLock`), so no advisory-lock-topology concern as long as they stay pre-lock (`_advisoryLockSingleHolderContract`).
- **Concrete fix:** factor a STREAM-shaped stall guard (one `AbortController` + `setTimeout(...).unref()` wrapping `fetch`/`revisions.get` + the bounded-stream read; on timeout also `stream.destroy(err)` for the Node branch) — NOT the `arrayBuffer`-based `fetchXlsxExportBytes`. Abort → `return null` per site to preserve fail-soft. Add a `fetch`/`drive` injection seam to the three default factories (`defaultRecover`, `makeSnapshotAssetsForApply`, `defaultDriveClient`) or extract the timed read into a directly-tested helper. Stall tests mirror `tests/drive/fetch.test.ts`'s `stallUntilAborted` / `responseWithStallingBody`.
- **Trigger:** next time the cron-apply asset pipeline is touched, OR a dedicated follow-up. Keep `revisions.*` free of `supportsAllDrives` (`_sharedDriveSupportContract`); if a new enclosing fn wraps `.files.get`, add a `_scopeCheckContract` exemption row in the same diff.
