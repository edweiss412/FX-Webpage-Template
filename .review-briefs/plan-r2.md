...
Round 1 returned BLOCKING with 7 findings. All seven were confirmed against the live tree and repaired
in `8406b0f9a`. Do not re-raise them; verify the repairs.

- **F1 + F2, the four invalid markers.** Accepted structurally rather than patched. The declared region
  now covers Tasks 1 to 6 only, and §3 states why each of the other four cannot satisfy a red-green
  cycle: two are regression pins, Task 9's guard disclaims checking whether the gate ran, and Task 10's
  defect is in a root-level file no marker can cite. What those four owe instead is written into each
  task as a planted defect that must be OBSERVED failing. Task 7 additionally gained the `testMatch`
  wiring step it needed to be able to go green at all, and its harness description is corrected: the
  production build is CI-only.
- **F3.** Task 6's marker is the tinted-plate opt-in alone; the contrast relation already holds at
  3.82 / 5.22 and ships as a pin under AC-10.
- **F4.** Task 5's STEP 1 stop condition is restated against what the command actually prints, 29 keys,
  with the 35-element enumeration coming from the probe. AC-9 is checked against the element artifact.
- **F5.** Task 5 gained STEP 2b: delete `FilterTextInput`'s `className` prop, and the planted
  caller-override assertion AC-9b named but no step produced. The three `MaterializeCard` citations are
  fixed.
- **F6 + F7.** §4.4 now carries the extractor rather than a hand count (four rows shift, not one), and
  §4.1's split is 2 / 5 / 8.

**F5's citation defect was mine and I swept its class.** A blind expander attached three
`MaterializeCard` line numbers to `ShowsTable.tsx`, and `spec:lint` passed them because those lines
exist. Every `path:line` in the plan was then checked against the line it actually resolves to, not
just the three named.
