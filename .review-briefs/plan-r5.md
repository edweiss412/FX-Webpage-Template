# Adversarial review — plan, round 5
...
Four rounds, sixteen findings, all confirmed against the live tree and repaired: `8406b0f9a`,
`edede588f`, `597681a35`, `0f7c73434`. The plan stage's round-economy filing is committed alongside the
spec stage's at `docs/review-rounds/fix/control-outline-cover/b30413cf5e51.md`. Round 4 confirmed the
other fifteen hold. Do not re-raise any of the sixteen; verify the repairs.

Round 4's one, since it is the freshest: AC-15's instrument keyed on `file:line:tag` while the task it
guards inserts a `cn` import above two of its own targets, so the correct repair would have come back
`<absent>` and stopped the task. The key is now the Nth element of that tag in that file, in walk
order. Its control is deliberately two-phase (capture, insert two lines, compare) because comparing a
shifted tree to itself passes under the broken key too: measured, 362 targets and 0 differences after a
two-line insertion above every element in `components/admin/telemetry/EventFilters.tsx`.

**This is round 5, past the stage's four-round cap.** The bound, the fence and the probe domain below
are unchanged and they are the whole criterion. Check the sixteen repairs against them. If you find
