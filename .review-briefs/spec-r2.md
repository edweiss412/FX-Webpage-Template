# Adversarial review — spec, round 2
...
Round 1 returned BLOCKING with 3 findings. All three were confirmed against the live tree and repaired
in `24471717d`. Do not re-raise them as new; verify the repairs instead.

- **P0, the declined component edge.** Accepted in full. The walk now resolves a capitalised tag inside
  an in-scope ancestor to its declaration, in the same file or one named-or-default import hop away,
  and reports its elements against the CALLEE's file and line. `CronRunSummaryCard` swaps; the six
  chrome sites register. Re-measured: universe 362 to 767, residue 10 to 45, reddening set 35 elements
  (13 Family A, 22 Family B), census 10 to 22, `switch-track` 3 to 5.
- **P1, the wizard pill transition.** Accepted. `components/admin/OnboardingWizard.tsx:166` does carry
  `transition-colors duration-fast`; §15 now enumerates all six state pairs of the four `pillState`
  arms and says which endpoints this diff moves.
- **P2, the two stale counts.** Accepted: 59, and eight.

Self-review added three things round 1 did not reach, and they are new material for you: the `accepted`
mutation rows are LINE-KEYED and this diff shifts them (§12), `millisPerBoot` feeds the shard partition
(§12), and the tinted-plate registry's `step3ReviewSections` count moves 4 to 9 (§7.2).
