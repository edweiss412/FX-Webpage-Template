# Adversarial review — spec, round 5
...
Four rounds, eight findings, all confirmed against the live tree and repaired: `24471717d`,
`8c4e67598`, `5a0d4c827`, `27f8d5a70`. The round-economy filing for this stage is committed at
`docs/review-rounds/fix/control-outline-cover/b30413cf5e51.md`. Do not re-raise any of the eight;
verify the repairs.

Round 4's two, because they are the freshest:

- **P0.** You were right that L7's safety claim was false. `isResidue` is `weakSides(...).length > 0`
  and `weakSides` reads only readable tokens, so `unresolved` does not preserve an element that carries
  no readable weak token, and `cn` merges nothing. The repair no longer merges a caller className into
  a strong base: `FilterTextInput` loses its `className` prop entirely and takes the one variation its
  two call sites differ by as a boolean, so every token on the element is a literal the resolver reads
  and no caller can contribute a `border-*` token. L7 now states what the guard is actually silent
  about.
- **P2.** Corrected to 9 clear and 50 unclassified, in both places, with the nine named.

**This round is past the stage's four-round cap.** The bound, the fence and the probe domain below are
unchanged and they are the whole criterion. Check the eight repairs against them. If you find nothing
admissible, say so plainly and APPROVE. Do NOT name a fresh attack axis or propose a new probe
