# Adversarial review — spec, round 4 (final round of this stage)
...
## Rounds 1 to 3, and where this stage now stands

Three rounds, six findings, all confirmed against the live tree and repaired: `24471717d`,
`8c4e67598`, `5a0d4c827`. Round 2 verified round 1's arithmetic and round 3 verified rounds 1 and 2's
("35 red elements, 23 swaps, 12 registrations, 29 new keys, 22 census rows, and the corrected 11/0/14
mutation inventory"). Do not re-raise any of the six; verify the repairs.

- **R1 P0**, the declined component edge: the walk now follows a capitalised tag one import hop, which
  also produced the row's own acceptance check (`switch-track` 3 to 5).
- **R1 P1/P2**: the wizard pill's six state pairs, and two counts.
- **R2 P1**, the accepted-row inventory: §12 now carries the extractor and its output rather than a
  hand count, after two hand counts disagreed with each other and with the registry.
- **R2 P2**: four stale counts, repaired by sweeping every numeric literal rather than the four named.
- **R3 P0**, `EventFilters`: the weak paint sat in a dead fallback while both call sites supplied their
  own, so repairing what the transcript named would have cleared the guard with the pixels still weak.
  §6.4 moves the recipe into the component so the token the guard reads is the token that paints, and
  sweeps the multi-alternative class (six such elements; five have only live render states).

**This is round 4, the cap for this stage.** The bound, the fence and the probe domain are stated
below and are unchanged. Your job is to check the six repairs against them and nothing wider: if you
find nothing admissible, say so plainly and APPROVE. Do not name a fresh attack axis; a re-scope past
the cap is the orchestrator's call, not a line in this brief.
