# Adversarial review — plan, round 4 (stage cap)
...
Three rounds, fifteen findings, all confirmed against the live tree and repaired: `8406b0f9a`,
`edede588f`, `597681a35`. Round 3 confirmed the round-1 and round-2 repairs hold ("the declared region
now has six valid markers, each `red-target` names the stated live defect, and each command can green
within its task. The 15-call-site disposition, `5 + 5 + 2 + 10 = 22`, the 4-to-9 `neutralFaintCount`,
and both verbatim inventories match the live tree and canonical spec"). Do not re-raise any of the
fifteen; verify the repairs.

Round 3's two, since they are the freshest:

- **AC-15's instrument.** You were right that it compared nothing and would have accepted anything, and
  right that its per-source-occurrence rule would have rejected the correct `EventFilters` repair. The
  comparison is now per ELEMENT through the scanner, and BOTH controls were run against the live tree
  and pasted: 362 targets with zero differences on an unchanged tree, and a planted `border` to
  `border-2` named with both sides.
- **§4.3's command.** Rewritten as a command that runs, with its real output (`code-only: 4`,
  `raw: 6`) and the reason `stripCommentsForFile` takes `(src, filePath)` in that order.

**This is round 4, the cap for this stage.** The bound, the fence and the probe domain below are
unchanged and they are the whole criterion. Check the fifteen repairs against them. If you find
nothing admissible, say so plainly and APPROVE. Do NOT name a fresh attack axis or propose a new probe
direction: a re-scope past the cap belongs to the orchestrator, not to this brief.
