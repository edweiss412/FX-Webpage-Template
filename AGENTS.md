...
6. **Commit per task.** Format: `feat(<area>): <summary>` or `test(<area>): ...`. Areas: `parser`, `db`, `sync`, `auth`, `crew-page`, `admin`, `report`, `onboarding`, `assets`, `infra`. Don't batch tasks into one commit.
