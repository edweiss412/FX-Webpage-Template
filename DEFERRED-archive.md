...
**What the number turned out to be, and who chose it.** Thirty seconds, and the decision this row
said nobody had made was made by declining to make it: the check-in never declares the retry hung
and never gives up. It changes the copy, offers Restart, and leaves the request running. That
sidesteps the objection above rather than answering it — a deadline too short kills a
slow-but-working 50MB fetch on the venue floor, which §3.1 ratified against — because there is no
deadline, only a check-in.

U-1 was then measured in a real browser (design spec §1.2, 2026-09-01) and answered CONTINUED:
removing the mid-fetch `<img>` does not abandon its request. So Restart re-arms the watchdog over a
fetch the browser never dropped, and the replacement costs nothing. The copy is pinned per phase so
no later edit can quietly promise a download that will not happen.
...
### HOVERHELP-CLAMP-CARET-1 — [P3] no caret under horizontal clamp; no blur-close while focus wanders — ✅ RESOLVED

_Archived 2026-07-24 by a PR-body-vs-ledger reconciliation sweep. This entry was added to the working queue in `0b05fc22c` (PR #549) and DELETED in `6ece7960e` (PR #553) instead of being graduated here, so its provenance was missing from the archive. Reconstructed verbatim from those two commits._

**Original entry (verbatim):** "From the impeccable critique of `hoverhelp-smart-position` (2026-07-22, Assessment A 34/40). Two folded P3s: (1) when the §4.2 horizontal clamp slides the popover body away from its trigger there is no caret/pointer affordance tying body to trigger — precisely in the collision cases the feature serves; (2) the popover stays open while keyboard focus wanders elsewhere (no blur-close) — pre-existing behavior, unchanged by the portal work, slightly amplified in modals where the body is no longer visually adjacent. Both cosmetic-tier; the dual-gate mandates P0/P1 only. The critique's P1 (modal Tab adjacency) was refuted by spec ratification (spec 2026-07-22-hoverhelp-smart-position:149) and is recorded in the critique snapshot, not here. **Un-defer trigger:** user reports losing track of which trigger an open popover belongs to (caret), or confusion from a popover lingering after tabbing away (blur-close)."

**Resolved:** both folded halves shipped in `feat/hoverhelp-caret-blur-close` (PR #553; spec `docs/superpowers/specs/2026-07-22-hoverhelp-caret-blur-close.md`, plan `docs/superpowers/plans/2026-07-22-hoverhelp-caret-blur-close.md`) — picked directly rather than by the trigger firing. (1) **Caret:** a step-5 extension of the pure core (`lib/popover/position.ts`) whose center tracks the trigger center, clamped into `[bodyLeft + 18, bodyRight - 18]` so it never rides a rounded corner — exactly the collision case the smart-position feature serves; new exports `CARET_WIDTH`, `CARET_HEIGHT`, `CARET_EDGE_INSET`, `CARET_INNER_OFFSET`, with body and caret sharing one host-conversion helper so they cannot drift. The caret retains `data-popover-side` through the exit fade (cleanup was stripping it and collapsing its borders mid-fade; T-A1 pins it). (2) **Blur-close:** one `onBlur` on the root wrapper, pair-scoped (never document-scoped), `relatedTarget === null` ignored, never steals focus back, and disabled for the modal-host + `learnMore` quadrant where the parent spec routes the link through the panel's Tab order. React `focusout` semantics across the portal were probed in real Chromium before drafting (spec §4.0, P1–P7). Coverage: T-C1..T-C8 pure-core, T-J1..T-J6 + T-A1..T-A6 + T-B1..T-B11 jsdom, and 26 real-browser Playwright cases on the standalone config.
...
1280×800 and 390×844 (`tests/e2e/attention-modal-gallery.spec.ts` geometry test).
Original deferral text follows.

**Original entry (verbatim): ATTN-GALLERY-CONTROLBAR-OVERLAP-1 — [P2] switcher control bar overlaps the modal's constant header**
...
## Modal-header StatusStrip chrome (2026-07-19)

_Archived 2026-07-24 by a PR-body-vs-ledger reconciliation sweep. The entry was added to the working queue in `6970a49b9` (PR #476, alongside `MODAL-CLOSE-EXIT-ANIM-1` and `MODAL-SKELETON-CLOSE-1` above) and DELETED in `9981089cb` (PR #480) instead of being graduated here, so its provenance was missing from the archive. Reconstructed verbatim from those two commits._

### MODAL-STRIP-CHROME-1 — [P2] StatusStrip kept page-context chrome (`sticky top-0 z-30 border-b shadow-tile px-4`) inside the modal header — ✅ RESOLVED

**Original entry (verbatim):** "Inside the shell's `<header>` (which carries its own `border-b border-border px-tile-pad`), the strip's sticky/z are inert and its border-b + shadow-tile stack a doubled seam above the header's own bottom border, plus doubled horizontal padding. Source-inferred (browser render skipped in critique); cosmetic. Fix needs a StatusStrip chrome variant prop, which bumps the pageTransitions conditional-count pin (`tests/components/admin/showpage/pageTransitions.test.tsx`) — deliberately not landed mid-close-out. **Un-defer trigger:** first visual QA pass on the shipped modal confirming the seam, or the next StatusStrip task — add a `chrome?: "page" | "modal-header"` prop dropping sticky/z/shadow/border/px in modal mode and update the count pin."

**Resolved:** shipped in PR #480 (`9981089cb`) in the fix shape the entry named. `StatusStrip` gained `chrome?: "page" | "modal-header"` (default `"page"` — page behavior unchanged); `PublishedReviewModal` passes `"modal-header"`, keeping only the layout classes (flex/wrap/gaps/alignment) and letting the header's flex column supply the row gap. The entry's predicted count-pin bump did NOT materialize: the variant is a `const` ternary OUTSIDE the JSX, so it is not a conditional MOUNT and the `pageTransitions` conditional-count pin for `StatusStrip` stays at **8** (an inline `className` ternary would have bumped it). Coverage: unit assertions that `"page"` keeps every page-context class, that `"modal-header"` drops all of them, and that the layout classes survive; plus a modal-level assertion that the strip rendered inside the panel carries no seam/shadow/sticky/padding.
...
### D4 — [P3] Real-browser 390px density check (strip) — ✅ RESOLVED (real-browser verified + fix)

- **What:** with the cap 8, the worst realistic case is Set + up to 8 show rows + Strike (≤10 rows) in the narrow "Daily call times" card (`ScheduleSection.tsx`, `stack`); plus the `layout="row"` overflow visual (impeccable audit P3).
- **Resolution:** verified in a real browser (headless Chromium) rendering the actual `KeyTimesStrip` component against the compiled Tailwind tokens at the target viewports:
  - **390px stack, 8 shows** (Set + 8 + Strike = 10 rows), dark **and** light: no page/strip horizontal overflow, every row inside the card (strip height ≈ 380px — scrolls naturally, no clip).
  - **390px stack, >8 shows fully expanded** (overflow `<details>` open): no horizontal overflow, rows contained (strip ≈ 498px).
  - **1000px row, >8 shows**: the measurement caught a real **28px strip h-overflow** (the full-width overflow block in the horizontal `divide-x` strip). **Fixed**: the row container now `min-[720px]:flex-wrap` and the overflow group `min-[720px]:basis-full`, so the disclosure drops to its own full-width line below the columns — re-measured clean (686/686, no overflow). Pinned by jsdom class assertions in `tests/components/crew/primitives.test.tsx` (row strip has `flex-wrap`; overflow group has `basis-full`).
- **Method:** standalone harness — `@tailwindcss/cli` compile of the strip's utilities + `renderToStaticMarkup` of the real component + Playwright `getBoundingClientRect` measurement (no seeded show needed). Not wired into CI (needs `@tailwindcss/cli`/esbuild at capture time); the durable regression guard is the class-assertion unit test above.
...
- **Resolution:** added a `--color-text-subtle` on `--color-surface` row to DESIGN.md §1.2 with the computed WCAG ratios (light 6.8:1 on `#FFFFFF`, dark 6.4:1 on `#16171C` — both clear AA body ≥4.5:1; eyebrow/meta only, never an action target). Computed via the standard WCAG relative-luminance formula (validated against the existing text/text-strong rows, which reproduce to within 0.1). **Reconcile (done):** the existing `text-subtle` on `--color-bg` figures were mistranscribed (light `7.8:1` → **6.5:1**, dark `6.4:1` → **6.8:1**; both still AA body) — corrected in §1.2 and the §1 palette-table note, with a §1.2 method note recording that the two `text-subtle` rows are standard-formula and the broader dark column carries a small (~0.3–0.4) historical offset (a full-table recompute is a separate, non-load-bearing cleanup — every row already clears its floor with margin).
...
- **✅ Measured 2026-07-17 — trigger does NOT fire; NOT implemented.** Read the boot-step log of a live green `unit-suite` run (#29619069104, main): the image pull ran `22:51:16.3` (first `Pulling`) → `22:52:13.2` (last `Pulled`) = **~57s**, UNDER the 60s gate. The remaining boot (container start + guarded migrations) was ~32s, for an 89s total boot = only ~14% of the 631s leg; the serial vitest run is ~83% (521s). So even a perfect cache barely moves the wall-clock, and the multi-GB save/load could net-negative (risk (b)). Re-trigger only if a future runner/image change pushes the isolated pull past 60s. The throughput win went to the 3rd shard (D14) instead.
...
- **✅ help-affordances half SHIPPED 2026-07-17 (this PR); screenshots-drift half STILL DEFERRED.** Resolved caveat (a): `playwright.config.ts:316` `:3004` webServer runs `pnpm build` with `NEXT_DIST_DIR=.next-screenshots-help` and `next.config` `distDir = NEXT_DIST_DIR ?? ".next"`, so the compiler cache is at `.next-screenshots-help/cache` — now restored via `actions/cache` in `help-affordances.yml` before the walker. Caveats (b) root-owned Docker cache and (c) byte-identity proof apply ONLY to `screenshots-drift` (runs the build inside the pinned Docker container and IS a byte-comparison gate); help-affordances runs on the HOST and is the DOM deep-link WALKER (not a pixel gate) and a NON-required check, so a warm/cold divergence can only fail its own assertions. **Screenshots-drift half remains deferred** with its original trigger: dispatch `screenshots-regen` to prove warm-build byte-identity + chown the cache dir back to the runner user before caching there.
- **✅ screenshots-drift half SHIPPED 2026-07-18 (`perf/ci-screenshots-drift-next-cache`, PR #466).** Both D11 halves now closed — but the root cause was NOT what the original deferral assumed. **The premise was obsolete:** the deferral was written under a webpack mental model, but **Next 16's default builder is Turbopack, which writes no reusable `<distDir>/cache`** unless the beta `experimental.turbopackFileSystemCacheForBuild` flag is set. Measured proof: the first cut cached a **299 KB no-op** (a same-scope `workflow_dispatch` A/B showed cache-miss 143s vs cache-hit 161s — zero speedup; the "~30s" estimate never existed under Turbopack). **The actual fix is two parts:**
  1. **`experimental.turbopackFileSystemCacheForBuild: true`** in `next.config.ts` (global — every `next build`, CI + Vercel, can now warm from a real Turbopack filesystem cache). This is what makes BOTH D11 cache steps effective — the help-affordances half (shipped 2026-07-17) was ALSO a 299 KB no-op until this flag activated it.
  2. **`screenshots-drift.yml` cache plumbing:** restore `.next-screenshots-help/cache` (key namespace `nextcache-screenshots`, distinct from help's `nextcache-help` — the screenshot build carries ADMIN_DEV_PANEL_ENABLED + fixtures) before the pinned Docker capture, then an `if: always()` `sudo chown -R "$(id -u):$(id -g)" .next-screenshots-help/cache` reclaims ownership from root (container builds as root; the `actions/cache` post-step saves as the runner user) so the save works even on a drift failure.
  - **Measured with the flag ON:** cache grew 299 KB → **356 MB** (real Turbopack cache), same-scope warm-hit capture **138s vs cold 162s ≈ 20–24s** saved. Caveat (c) **byte-identity proven with the flag ON**: warm-cache build produced **zero drift** on the byte-comparison gate (Turbopack build cache is content-keyed; the runner-bimodality LSB jitter class fixed in PR #22 is orthogonal). Accepted tradeoffs (owner-directed, ship-global): the flag is beta for prod builds (Vercel prod now uses it; only the screenshot gate re-proves byte-identity) and the 356 MB per-sha cache entries churn against GitHub's 10 GB budget (~28 entries → LRU eviction, self-managing).
...
- **✅ SHIPPED 2026-07-17 (this PR).** `.github/actions/setup` composite created (pnpm/action-setup → setup-node node20/cache pnpm → `pnpm install --frozen-lockfile`); all ~20 jobs across the 8 workflows now run `checkout@v4` + `- uses: ./.github/actions/setup` (checkout stays in the job — a local `./`-composite is only referenceable after checkout). Added `"packageManager": "pnpm@10.33.2"` to package.json as the single pnpm-version source (pnpm/action-setup reads it; verified `--frozen-lockfile` unaffected — the field is not part of the lockfile checksum). The #104/#105 supabase-cli pin gate was already satisfied (2.107.0). supabase-cli + host psql installs deliberately NOT moved into the composite (only some jobs need them; the 2 in-container psql installs can't move to a host step). The D11 help-affordances Next-cache is the "consumer beyond DRY."
...
- **✅ Trigger fired, but resolved by a 3rd shard — NOT curated buckets (2026-07-17, this PR).** Measured a live green 2-leg run (#29619069104, main): leg1 596s (9m56s), leg2 631s (10m31s) → `max >= 9m` fires. BUT the two legs were already **balanced** (vitest 503s vs 521s, 18s apart; legs 35s apart, well under the 2-min bar). So D14's premise — that the legs are IMBALANCED and curated duration buckets would help — was **false**: rebalancing two already-equal ~500s vitest halves cannot beat the ~500s floor. The >9m was a THROUGHPUT problem, not a balance problem. The correct lever was therefore a **3rd count-shard** (`matrix.shard: [1, 2, 3]`, `--shard=i/3`): vitest ~1024s total / 3 ≈ 341s + ~90s boot ≈ **7.5m/leg**, hitting the `<9m` stretch target. `vitest --shard` stays count-balanced and auto-rebalances as files are added, so NO curated-bucket partition meta-test is needed (the existing `tests/cross-cutting/unit-suite-shard-topology.test.ts` was updated TDD-first to pin `[1,2,3]` / denominator `/3`). Curated duration buckets remain available as a future contingency ONLY if a 3-leg run later shows genuine imbalance (`max >= 9m` AND legs differ `> 2min`) — at which point the original curated-bucket trigger (measured per-file durations + partition meta-test + own adversarial review) applies. Verify the 3-leg wall-clock on the first post-merge `unit-suite` run.
...
- **✅ Resolved (2026-07-17, `feat/cardreport-tap-targets`):** both affordances now expose ≥44×44px tap targets via a transparent out-of-flow `::before` overlay that grows 44px in ONE direction, chosen by a `hitDirection: "up" | "down"` prop (default `"up"`). The overlay is invisible to `getBoundingClientRect()`, so the existing row-height + affordance-box invariants in `tests/e2e/source-link-dimensional.spec.ts` stay green **unchanged** (the deferral's guess that the invariant would need flipping was wrong). The direction is context-aware: every `SectionCard`-hosted cluster grows UP (zero downward overhang → clears the interactive `KeyValue` tel/mailto rows below); the one bare `schedule-days` header (`ScheduleSection.tsx:253`) grows DOWN (zero upward overhang → clears the agenda above; the `DayCard` list below is non-interactive). Sibling non-overlap is achieved by widening the cluster `gap-2`→`gap-4` (16px) so the trigger's centered 44px overlay clears `SourceLink`'s box by ≥1px; `SourceLink`'s overlay is full-width (`before:inset-x-0`) so it never overflows horizontally. `CardHeaderActions` reflects `data-hit-direction` as a production-wiring seam pinned by `tests/components/crew/sourceLinkCoverage.test.tsx` (schedule-days="down", all others="up"), and a real-browser `elementFromPoint` hit-probe (`source-link-dimensional.spec.ts`) proves 44px reach per direction, zero interactive-neighbor bleed, and no sibling overlap. Impeccable v3 dual-gate on the diff: critique CLEAN (detector `[]`; Assessment A no P0/P1/P2), audit 20/20 — one residual P3 (the overlay intentionally captures pointer events; guarded by the e2e neighbor probes), accepted. Spec `docs/superpowers/specs/2026-07-17-cardreport-tap-targets.md`, plan `docs/superpowers/plans/2026-07-17-cardreport-tap-targets.md`.
...
Active data-quality warnings now render grouped by code (`lib/dataQuality/groupActiveByCode.ts`, first-appearance order); each bulk "Ignore all N" control is an inline chip on its group's eyebrow header row, bound to the cards it ignores (`components/admin/BulkIgnoreControls.tsx`, widened from the stacked-button list to own the grouped active list + shared arm/run/error state; rendered at `app/admin/show/[slug]/page.tsx`). Every active code carries a plain-language eyebrow label (catalog title else `DATA_GAP_CLASS_LABELS`, never the raw code — invariant 5); singleton + non-ignorable codes (e.g. `BLOCK_DISAPPEARED` → "removed section") get the eyebrow with no chip. The two-tap destructive recipe, single-armed-panel-wide invariant (§4 G4), per-fingerprint fan-out, and honest partial-failure copy (now rendered inside the acting group, below its cards) are all preserved. The "Ignored (N)" subsection stays flat. Spec/plan: `docs/superpowers/{specs,plans}/2026-07-17-dq-group-active-by-code*`.
...
### OBS-1 — [P4] impeccable dual-gate for the null-render `GlobalErrorListener` + its layout mount — ✅ RESOLVED 2026-07-17 (standing exemption, not a pending defer)
...
- **Resolution (permanent standing exemption):** re-verified 2026-07-17 — `GlobalErrorListener` still `return null` (`components/observe/GlobalErrorListener.tsx`) and the layout mount is still a bare `<GlobalErrorListener />` with no rendered output. A zero-pixel component has no visual surface the dual-gate can ever evaluate, so this is not "deferred work awaiting a trigger" — it is a **standing N/A** for as long as the component renders nothing. Closed as a permanent exemption; the trigger below re-opens it (as a genuine new gate) the moment that architecture changes.
- **Trigger (re-opens the gate):** if `GlobalErrorListener` ever gains rendered output (e.g. a visible error toast/banner), it becomes a real visual surface and MUST pass the impeccable dual-gate at that point.
...
### SFS-1 — [P3] dedicated stage-restricted-crew real-browser e2e — ✅ RESOLVED 2026-07-17 (branch `test/sfs-stage-crew-e2e-impeccable`)

- **✅ Resolution (2026-07-17):** the trigger effectively fired — `DayCard.tsx`/`ScheduleSection.tsx` changed several times post-defer (`CARDREPORT-1` hit-target, "Show Day N" label render, meridiem-casing meta), each riding the admin-viewer `crew-layout-dimensions.spec.ts` but leaving the stage-restricted-crew assertion still absent — so the dedicated e2e is now built. `tests/e2e/stage-restricted-crew-schedule.spec.ts` renders **as a specific stage-restricted crew member** via the picker-cookie path (a fresh `__Host-fxav_picker` selection for an unclaimed seeded crew row + NO Google session → `resolveShowPageAccess` returns `resolved`/source-cookie with NO picker-bootstrap host-flip; `auth_email_canonical()` returns null for anon, so `resolvePickerSelection` resolves an unclaimed row on a fresh epoch). It asserts (1) BEHAVIOR — a "Strike / Load Out ONLY" crew sees ONLY its 2 worked day cards (04-23 Show Day 2 via compound Strike, 04-24 Travel Out via Load Out); the 3 non-worked days are ABSENT (privacy boundary); an admin control renders the SAME show unrestricted (all 5 cards) as the anti-tautology proof of narrowing; (2) the mandatory `DayCard` real-browser dimensional invariant FOR THE STAGE-RESTRICTED RENDER — `[data-testid="day-card-date"]` = 50px + `self-stretch` vline fills the card content box at ≥720px (`getBoundingClientRect`; jsdom insufficient). `seedShowWithCrew` gained optional show-level `dates` + crew-level `stageRestriction`/`dateRestriction` (additive). Local-only real-browser gate (mobile-safari testMatch), like picker-flow / crew-layout-dimensions; 3/3 green. **The original deferral context is preserved below for the record.**
...
### SFS-2 — [P4] invariant-8 impeccable dual-gate — ✅ RESOLVED 2026-07-17 (dual-gate run, both pass clean; branch `test/sfs-stage-crew-e2e-impeccable`)

- **✅ Resolution (2026-07-17):** the invariant-8 dual-gate was run on the stage-filtered crew schedule surface and both commands PASS with zero P0/P1. **Critique 36/40 (Excellent)** — dual-agent (design-review + detector), AI-slop verdict "not slop", `detect.mjs` `[]`/exit-0 on both markup component files, no new P0/P1/P2/P3 introduced. **Audit 20/20 (Exceptional)** — a11y 4 / performance 4 / theming 4 / responsive 4 / anti-patterns 4, no new defect in any dimension. Both gates confirm what the deferral asserted: the four edits produce no new visual state (a strict subset of the already-gated date-restricted-crew render), so there was nothing for the gate to fault — running it makes that an attested PASS rather than a standing exemption. Real-browser evidence substituted for the live-overlay step: `tests/e2e/stage-restricted-crew-schedule.spec.ts` (SFS-1) renders this exact surface as a stage-restricted crew member and asserts the filtered DOM + DayCard dimensional invariant (green). Critique snapshot: `.impeccable/critique/2026-07-18T00-00-48Z__components-crew-sections-schedulesection-tsx.md`. Pre-existing/out-of-scope (not this diff's findings): accent-on-bg AA contrast (`BL-ACCENT-ON-BG-AA-CONTRAST`); DayCard set-tone inline `#caa53a`. **The original deferral context is preserved below for the record.**
...
- **Resolved (2026-07-17, branch `fix/devtier-toggle-help`, spec `docs/superpowers/specs/2026-07-17-devtier-toggle-help.md`):** extended the EXISTING Administrators-heading `HoverHelp` developer-branch (`AdministratorsSection.tsx:94`) — single-source, no net-new element — with: "The Developer toggle gives that admin the same access you have, including managing admins (add, revoke, re-add, promote) and the Telemetry, Maintenance, Diagnostics, and Developer tools areas." Developer-only by construction (the `viewerIsDeveloper` branch + the toggle itself are developer-gated), so only a viewer who can use the toggle sees it. Copy uses the real UI labels; "same access you have, including …" is deliberately non-exhaustive (comprehensive dev-surface inventory in the spec). Clause-level tests pin the blast radius (`AdministratorsSection-developer.test.tsx`). **invariant-8 dual-gate:** critique dual-agent AI-slop clean, detector `[]`, heuristics #2/#4/#10 = 4, #8 = 3, zero P0/P1; audit 19/20 (a11y 4 / perf 4 / theming 4 / responsive 3 / anti-patterns 4), responsive CONTAINED at 390px (`w-72 max-w-[80vw]`, wraps, no h-overflow), zero P0/P1. Two P3s deferred below.

### DEVTIER-HELP-1 — [P3→deferred] Mild lexical stem-crowding in the grant copy

- **What:** the resolved copy repeats the "developer" stem within one sentence ("…the same **developer** access you have … and the **Developer** tools areas"). Critique Assessment A P3, explicitly "style-only; do not block."
- **Why deferred:** cosmetic; the sentence is scannable and every clause is load-bearing. Rewording ("…the Telemetry, Maintenance, and Diagnostics areas, plus Developer tools") would re-churn the verbatim GRANT_COPY that the tests + spec + plan pin in lockstep, for zero user impact.
- **Trigger:** next copy pass on the Administrators help, if the string is edited for another reason anyway.
- **✅ RESOLVED (2026-07-17, branch `fix/devtier-help-polish`):** the trigger fired (user-requested copy pass). Dropped the redundant "developer" from "the same **developer** access you have" → "the same access you have" — the reader is the developer whose access is being cloned, so "you have" already anchors the grant to their own (well-defined) access; the removed word carried no meaning the sentence lost. Leaves two "Developer" stems, both fixed UI labels (the **Developer** toggle / the **Developer** tools area). Updated in lockstep: `AdministratorsSection.tsx:94`, the verbatim `GRANT_COPY` pin in `AdministratorsSection-developer.test.tsx`, the spec verbatim + prose quotes (`2026-07-17-devtier-toggle-help.md` §copy/§rationale), and the plan pins. The "anchors to the reading developer's own access, non-exhaustive `including …`" design rationale is unchanged (the anchor was always "you have", not the adjective).

### DEVTIER-HELP-2 — [P3→deferred, pre-existing] `HoverHelp` body has no `max-h`/`overflow-y`

- **What:** `HoverHelp` body (`HoverHelp.tsx:185`, `w-72 max-w-[80vw]`) has no `max-height` + internal scroll, so a long body low on a short viewport can extend past the fold. Audit P3.
- **Why deferred:** PRE-EXISTING to `HoverHelp` (not introduced by this copy diff — it is a shared property of every HoverHelp consumer). This heading sits near the top of the page, so the doubled body fits the 390px viewport; content never clips. Fixing it is a shared-`HoverHelp` change, out of this branch's scope.
- **Trigger:** a HoverHelp with a genuinely long body appears low on a short viewport, OR a general HoverHelp polish pass; add `max-h-[...] overflow-y-auto` to the body once, for all consumers.
- **✅ RESOLVED (2026-07-17, branch `fix/devtier-help-polish`):** the general-polish trigger fired. Added `max-h-[min(60vh,24rem)] overflow-y-auto` to the shared `HoverHelp` body (`HoverHelp.tsx:185`), once, for every consumer — the body now caps at 24rem (or 60vh on short viewports, whichever is smaller) and scrolls internally instead of extending past the fold. `overflow-y` only; `max-w-[80vw]` + text wrap already prevent x-overflow. No visual change for existing short tooltips (content under the cap never triggers the scrollbar), so no screenshot-baseline drift.
...
- **Why deferred (pre-existing + system-wide, not diff-introduced):** identical construction ships in `NotifyToggle.tsx:134` and `PublishedToggle.tsx:146` — this is the shared project toggle recipe, not introduced by this diff. It is the same token deficiency already filed as `BL-ACCENT-ON-BG-AA-CONTRAST` in `BACKLOG.md` and deferred by the Step-3-modal gate as STEP3MODAL-1. Mitigated: the toggle state is NEVER color-only (thumb `translate-x` + `aria-checked` + the visible "Developer" label), so the color-blind floor holds. Fixing it only inside this toggle would fork the brand accent on one surface.
...
- **✅ FULLY RESOLVED (branch `feat/bell-triage-severity-grouping`, 2026-07-17):** mark-all-read shipped earlier in the 2026-07-06 redesign; the remaining **triage grouping** now ships as threshold-gated **severity** sectioning (show-grouping + collapsible sections were considered and rejected in the shape pass — see spec §8). At `active.length >= 9` on an **active-complete** feed the active list re-sections into static `Critical → Warning → Notice` dividers (`bell-section-active-tier-{tone}`, reusing the existing eyebrow class), activityAt-DESC preserved within tiers, empty tiers omitted; below 9 or on an active-truncated feed it stays flat (fail-closed: grouped iff `feed.activeTruncated === false`). Threshold/tone/partition live in a new pure `lib/admin/bellTriage.ts`; the feed gained a derived `activeTruncated` (from `meta.active_hit_cap`, no RPC/DB change). Spec `docs/superpowers/specs/2026-07-17-bell-triage-severity-grouping.md`, plan `docs/superpowers/plans/2026-07-17-bell-triage-severity-grouping.md`. Twin `BL-BELLPANEL-ROWTONE-NOTICE-WEIGHT` closed in the same PR (see BACKLOG).
...
### PSAT-1 — [P2] Committed override + failed follow-up re-scan leaves Step-3 re-offering S2/S3 from the stale preview (recovery-UX loop) — ✅ RESOLVED 2026-07-17 (durable override DTO: `pending_syncs.pull_sheet_override` reduced via `coerceOverrideSnapshotFromRow` onto `Step3Row` → `SectionCore.pullSheetOverride`; `PackListBreakdown` derives state from the durable snapshot + preview via `overrideSnapshotsEqual`; on divergence renders the S5 re-scan recovery block instead of re-offering S2/S3; S5 Re-scan frozen during publish runs via `Step3RunStateContext`. Plan `docs/superpowers/plans/2026-07-17-psat1-durable-override-dto/`, spec `docs/superpowers/specs/2026-07-17-psat1-durable-override-dto.md`.)
...
### VCR-4 — [P3] Link-only degraded tile reuses the transient-loading visual — ✅ RESOLVED 2026-07-17 (venue-degraded-tile-glyph: terminal link-only tile now renders a centered `MapPin` "no preview" glyph empty-state instead of the transient `map` corner label; the standard/loading tile keeps `map`. VCR-4's own trigger sanctioned this label divergence — the **Trigger** line below. No BACKLOG row was ever filed for `BL-VENUE-DEGRADED-TILE-LABEL` — closed directly here.)

Source: invariant-8 impeccable v3 dual-gate on branch `fix/venue-card-vcr2-vcr3`. Critique: AI-slop NO, 0 P0/P1, touched-heuristic scores 3–4/4; detector 6 `broken-image` hits ALL false-positive (comment substrings + the ratified dynamic-proxy-`<img>` FP — the shipped `<img>` scanned clean). Audit: 20/20, 0 P0/P1/P2. Two P3 polish nits deferred (below); all gate-blocking findings clear.

- **What:** on the permanent link-only degraded tile (`VenueMapTile.tsx`, `query===""` + valid `mapHref`), the always-painted stripe base still carries the `map` mono label — visually identical to the transient loading state (geocodable query, `<img>` still fetching). No signal distinguishes "map coming" from "no map, ever."
- **Why deferred:** (1) P3 polish, not a defect — the whole tile is the Maps anchor and the Directions button makes intent obvious, so the state is actionable and non-broken. (2) The `map` label is a ratified original venue-card design element (spec 2026-07-06 §3.2); conditionally swapping/dropping it diverges the link-only tile from the standard tile and adds a branch to a deliberately-simple "always painted" layer — a design decision beyond this deferral's scope. (3) Near-zero trigger (link-only venue) and single admin user (Doug). Audit rated the branch 20/20 with no change required.
- **Trigger:** the next Stage-3 venue-card polish pass, OR a design decision to distinguish loading vs terminal-degraded map states (e.g. terminal label copy, or a distinct degraded treatment). Backlog: `BL-VENUE-DEGRADED-TILE-LABEL`.
...
### FLOW4-2/3-POLISH — [✅ RESOLVED 2026-07-17] At-a-glance glyph distinction on a sunlit venue-floor phone

- **Resolved (branch `feat/badge-glance-polish`, spec `docs/superpowers/specs/2026-07-17-badge-glance-polish.md`):** the two amber chips now use `size-4` glyphs (16px, was `size-3.5`/14px) with a `gap-2` inter-chip gap (8px, was `gap-1.5`/6px), sharpening the pre-count silhouette glance. The dimensional invariant `badgeHeight === glyphHeight` is preserved at the new 16px baseline (the 12px `leading-none` count box stays shorter than the 16px glyph); re-verified green by the real-browser `tests/e2e/dataQualityBadge.layout.spec.ts`. Parent spec §5.1/§5.2/§5.4 amended in lockstep; no hue change (§1 color-blind floor upheld); no `DESIGN.md` edit (the §1.3 two-glyph note records glyph identity, not size).
- **Original:** the two amber chips (`Users` vs `TriangleAlert`, both `size-3.5`, 6px inter-chip gap) relied on a fine silhouette difference for the pre-count glance. The impeccable v3 critique (Assessment A, P2) on the `feat/badge-affordance-a11y` diff noted that on Casey's sunlit venue-floor phone the two same-hue chips could momentarily conflate before the count is parsed. Distinction was already carried by silhouette + per-chip count + full `aria-label` (AAA contrast); this pass strengthens the at-a-glance channel.
...
- **✅ RESOLVED-BY-SUPERSESSION (2026-07-17):** a later change added a `flatten` path to `StripRow` — a singleton group now flattens the inner row card (no card-in-card), while multi-row groups keep per-row cards. The "singleton renders the full card-in-card" premise (and the "matches the mock = keep card-in-card" rationale) was superseded. Pinned green by `tests/components/admin/RecentAutoAppliedStrip.test.tsx` ("singleton group flattens the inner row card; multi-row keeps per-row cards"). (Verified live during the KINDDOT-1 ship; DEFERRED.md had drifted.)
...
- **✅ RESOLVED (PR #453, 2026-07-17):** shipped the structured per-field diff. `field_changed` rows now render a per-field list (MI-8/8b/8c/9) via `lib/sync/changeLog/fieldChanges.ts` (`buildFieldChangesRow` writer + `deriveFieldsDiff` reader) into `show_change_log.after_image.fieldChanges` — no migration (freeform jsonb), no `TriggeredReviewItem` widening (spec §4), no old financial value stored (MI-8 note-only). MI-9 role is enriched (existing-crew-only). All-malformed → a visible "Unavailable" marker row (never null `after_image`). Field name is the entry heading. Spec `docs/superpowers/specs/2026-07-17-autoapplied-field-structured-diff.md`; backlog `BL-AUTOAPPLIED-FIELD-STRUCTURED-DIFF` closed. (`crew_email_changed` still renders its summary sentence — separate kind, explicitly out of scope §8, not part of this deferral's resolution.)

## Structural-transform use-raw — whole-diff R4 deferral (2026-07-15)

### USE-RAW-1 — ✅ RESOLVED (2026-07-16, PR #399 `feat/use-raw-wizard-full-list`) — [P2→deferred] Wizard callout renders the use-raw toggle only for the first 3 recoverable warnings per section

**Resolution (2026-07-16):** the wizard `WarningsBreakdown` now mounts `UseRawControlBoundary` (and `RoleRecognizeControlBoundary`) on every in-scope recoverable warning uncapped when `wizardSessionId` is threaded (`components/admin/wizard/step3ReviewSections.tsx:96,596-611,1974-1981`), matching the per-show live page — so warnings 4+ get the toggle in the wizard. `SectionFlagCallout` is kept as a capped actionable preview; `stableWarningKeys` pins identity at both render sites. Spec `docs/superpowers/specs/2026-07-16-use-raw-wizard-full-list-toggle.md`; backlog `BL-USE-RAW-WIZARD-FULL-LIST-TOGGLE` marked SHIPPED. Three dual-gate follow-ups filed → USE-RAW-FULL-LIST-1/2/3.
...
- **✅ RESOLVED-BY-SUPERSESSION (2026-07-17):** a later change added `KindDotCluster` to the collapsed `GroupSection` header (`components/admin/RecentAutoAppliedStrip.tsx`, function-comment cites this entry), so the collapsed header now surfaces one dot per distinct change kind (incl. destructive "Removed") + an `aria-label` naming every kind, before expanding. The "header shows only showName + a bare count badge" premise no longer holds. Pinned green by `tests/components/admin/RecentAutoAppliedStrip.test.tsx` ("collapsed header shows a kind-dot cluster"). The KINDDOT-1 pass above then hardened the destructive dot with a non-color tell. (Verified live during the KINDDOT-1 ship; DEFERRED.md had drifted.)
...
- **✅ RESOLVED-BY-SUPERSESSION (2026-07-17):** `CollapsePanel` no longer mounts/unmounts — it is an always-mounted `role="region"` grid-item that height-morphs (grid `0fr`→`1fr`) and toggles `inert` when closed (`components/admin/CollapsePanel.tsx`; the strip's `GroupSection` panel comment at `RecentAutoAppliedStrip.tsx` reads "the panel region is always mounted (CollapsePanel height-morph)"). The "instant mount/unmount, chevron animates but content does not" premise no longer holds — the panel now animates. The sibling accordions this entry cited as the deferral reason (`IgnoredSheetsDisclosure`, `AddAdminDisclosure`) have ALSO migrated to the same `CollapsePanel`, so the whole named disclosure family now shares the height-morph idiom and `BL-DISCLOSURE-FAMILY-HEIGHT-MORPH` is itself complete for that family. (Verified live during the KINDDOT-1 ship; DEFERRED.md + BACKLOG.md had drifted — both reconciled here.)
...
### ROLE-VOCAB-1 — [P2→deferred] Settings "Roles you've added" renders the stacked mobile card at every viewport (mock's one-line desktop grid dropped) — ✅ RESOLVED (PR #402)
...
- **Resolved:** shipped in PR #402 (`feat/role-vocab-settings-desktop-grid`, spec `docs/superpowers/specs/2026-07-16-role-vocab-settings-desktop-grid.md`, commits `21819c1b5` + `11d6fdd9d`). `RoleMappingRow` now carries the single-DOM `min-[760px]:` grid branch (`150px | chips | meta | actions`, `contents` header dissolution, full-width `col-span-4` sub-rows), the Edit button gained the constant `aria-label` + dual responsive label spans, `EDIT_LABEL_SHORT` was re-added, and the page container bumped `max-w-2xl` → `max-w-3xl`. Real-browser layout spec `tests/e2e/roles-settings-layout.spec.ts` (registered in `playwright.config.ts` desktop-chromium `testMatch`) + extended `tests/components/roleMappingSettingsRows.test.tsx` (21 green) pin it; invariant-8 dual-gate ran on the diff.
...
### ACCENT-PASS-1 — ✅ RESOLVED (2026-07-17, PR TBD `feat/hero-segment-vibrancy`) RightNowHero active show-day segment restored to vibrant accent

- **What (original):** the hero's active progress segment changed `bg-accent` → `bg-accent-on-bg` (`#a65000`) so the `role="img"` show-day indicator clears WCAG 1.4.11 (3:1 vs bg AND vs the inactive `bg-border` segments; raw `#ff8c1a` = 2.23:1 vs bg, 1.83:1 vs inactive `bg-border`). Critique P2: this is the one surface PRODUCT.md reserves for expressive orange, and the sunlit crew glance loses brand vibrancy.
- **Resolution:** active segment now `border border-accent-edge bg-accent` — vibrant `#ff8c1a` fill restored, with the WCAG 1.4.11 3:1 boundary carried by the `border-accent-edge` stroke (the identical recipe already used by the toggles + wizard active pill). The edge clears 3:1 on every surface the segment can render on: light edge `#7a3d00` = 8.42:1 vs `bg-surface`, 7.18:1 vs `bg-stale-tint` (morph-to-last-good path), 3.61:1 vs the fill; dark edge `#ffa047` = 8.84:1 / 7.82:1, and the dark fill itself already clears 3:1. The inactive `bg-border` track is unchanged (its sub-3:1-vs-surface is the pre-existing ratified contract). `_metaBgAccentInventory` registers the new occurrence as `edge-treated`; DESIGN.md §1.2 accent-edge row lists the segment as a consumer. Spec `docs/superpowers/specs/2026-07-17-hero-segment-vibrancy.md` (Codex-APPROVED, 4 rounds); plan `docs/superpowers/plans/2026-07-17-hero-segment-vibrancy.md` (APPROVED, 3 rounds). Closed directly in DEFERRED — the `BL-HERO-SEGMENT-VIBRANCY` name referenced in the original deferral was aspirational and was never filed as a `BACKLOG.md` row (grep-confirmed).

### HERO-VIBRANCY-DIM-1 — [DEFERRED-AS-N/A] no real-browser dimension assertion for the segment border

- **What:** the ACCENT-PASS-1 active segment gains a 1px `border`. No real-browser `getBoundingClientRect` parity test is added.
- **Why N/A:** the segment's dimensions are CSS literals (`h-1.5`, `flex-1`, `rounded-pill`, `border`) and Tailwind's default `box-sizing: border-box` absorbs the 1px stroke inside the existing 6px/`flex-1` box — there is NO fixed-height-parent → flex-child stretch dependency that the border could break (the row height is content-driven, not a fixed parent children must stretch to fill). The project layout-dimensions rule mandates a real-browser assertion only for that stretch-dependent case. The one genuinely visual question (does a 1px stroke read as a crisp hairline on a 6px pill) is a screenshot judgment covered by the invariant-8 impeccable real-browser gate, not a dimensional invariant. Mirrors `KINDDOT-DIM-1`.

## Consolidated admin show page — orphaned help affordances (2026-07-16, Task 13)

Source: the consolidated-admin-show-page rebuild (spec `docs/superpowers/specs/2026-07-16-consolidated-admin-show-page.md`, Task 13). The old per-show page's standalone Crew section, Sync footer, and flat Data-quality panel were dissolved by design (spec §6): crew now renders inside the shared review surface, sync is a StatusStrip badge, and data-quality warnings moved per-section. The three `HoverHelp` affordances that anchored those sections lost their host elements. The commit `352ad8c8b` meta-test reconcile parked their testids in `DEFERRED_TESTIDS` (`app/help/_affordanceMatrix.ts:284`) with the help TARGETS still live; this row records the deferral in the deferral ledger too (deferral discipline).

### CASP-1 — [P2→deferred] Three per-show HoverHelp tooltips orphaned by the section dissolution — ✅ RESOLVED-RETIRED (Task 16 impeccable gate, 2026-07-16)

- **What:** the affordance-matrix rows `help-affordance--per-show-crew--tooltip`, `help-affordance--per-show-sync-footer--tooltip`, and `help-affordance--per-show-data-quality--tooltip` no longer render on the consolidated surface — their host sections (standalone Crew section, Sync footer, flat Data-quality panel) were removed.
- **Resolution (Task 16, all three RETIRED — no re-home):** the impeccable critique gate ruled that none of the three should be re-homed on the consolidated surface, so all three matrix rows AND their `DEFERRED_TESTIDS` entries were removed (`app/help/_affordanceMatrix.ts`; the shape meta-test now pins 18 concrete rows + a single-element `DEFERRED_TESTIDS` = only `preview-banner`). Per-tooltip rationale: **Crew** — the preview-as-crew help (`/help/admin/preview-as-crew`) is served by the still-deferred `preview-banner` affordance on the actual `/preview/[crewId]` route; the shared review surface is not a natural host (it is for reviewing crew data, not initiating preview-as) — this diverges from the critique's "re-home (trimmed)" suggestion with cause, recorded in the Task 16 report. **Sync-footer** — sync is now a compact `StatusStrip` `StatusIndicator` badge that already carries a text label; a hover "?" tooltip there would be a hover-only affordance, banned for Doug-on-the-floor (touch). **Data-quality** — warnings now render per-section with their own Report/Ignore copy, and the live sibling `help-affordance--per-show-alerts--tooltip` already links `/help/admin/parse-warnings` from the same Overview area; a second tooltip is redundant.
- **Residual (out of scope, separate docs cleanup):** the three `/help` TARGET pages stay live (resolvable anchors, some still linked by other affordances). Whether the now-unlinked `/help/admin/per-show-panel#sync-health` content is stale vs the rebuilt page is a documentation-accuracy question for a separate docs pass, not this structural retire (mirrors the D9 residual pattern).

### CASP-2 — ✅ RESOLVED (2026-07-17, branch `feat/casp2-inline-toggle-variant`) StatusStrip wraps the full-weight PublishedToggle card in a slim strip

- **Resolution:** shipped `variant: "card" | "inline"` on `components/admin/PublishedToggle.tsx` (default `card` — every existing caller byte-identical). `StatusStrip` renders `variant="inline"`: a compact "Published" label + switch only; refusals / generic errors / the finalize-disabled reason render in an absolutely-positioned popover anchored to the **sticky strip** (`right-0 top-full z-40 max-w-60 break-words`) — out of flow, so ZERO strip-height cost. Error `useState` + form action stay in `PublishedToggle` (no cross-island lift; B1 dispatch-safety preserved). The subline is dropped in inline mode → the Overview `#share-access` `admin-share-link-inactive` notice is the single source for the crew-link-off copy (folds in the coupled critique P2 duplicate-copy). Spec `docs/superpowers/specs/2026-07-17-casp2-inline-toggle-variant.md` (adversarial-review APPROVED, 5 rounds); plan `docs/superpowers/plans/2026-07-17-casp2-inline-toggle-variant.md` (APPROVED, 4 rounds). Real-browser 390px geometry gate (`tests/e2e/statusStripToggleLayout.spec.ts`) proved toggle-anchoring overflows and forced the strip-anchor.
- **Impeccable dual-gate (invariant 8):** critique 29/40, detector clean `[]`, contrast pairs PASS, no P0. P1/P2 dispositions:
  - **CASP2-1 (P1 — popover not dismissible) — NOT A DEFECT (ratified).** Brainstorming chose "role=alert + auto-persist until next toggle attempt" over "manual dismiss (X)"; it mirrors the card's error block exactly (also clears only on next submit). Adding a dismiss affordance would diverge inline from card and re-open the B1 error-render surface. Do NOT "fix" without revisiting the ratification.
  - **CASP2-2 (P1 — popover spatially disconnected from the switch) — ✅ RESOLVED (2026-07-17).** The strip-anchored popover was widened from a right-anchored `max-w-60` box (`right-0 w-max max-w-60`) to a **full-strip-width banner** (`inset-x-0`) spanning the sticky strip's padding box just below it. A full-width banner reads as belonging to the strip — its x-position is now invariant to where the toggle flex-wraps, so it can never disconnect to a phantom right edge (the pre-fix disconnection on long titles). This keeps the correctness-forced in-viewport invariant (a full-width banner is in-viewport BY CONSTRUCTION at 390px, where the rejected toggle-anchored box overflowed both extremes) while restoring Gestalt proximity — the two constraints stop competing. The calm (finalize) skin also moved to a sunken plate + strong border (`bg-surface-sunken border-border-strong`) so it separates from the strip's own `bg-surface` (impeccable critique P2 on this pass). Geometry pinned by `tests/e2e/statusStripToggleLayout.spec.ts` §8.10c (banner hugs strip edges, width > 300px, x-position identical short vs long title); class-set pinned by the `PublishedToggle.test.tsx` positioning-parity unit test (`inset-x-0`, no single-side anchor / width cap). Impeccable dual-gate re-run on the diff: critique no-slop + detector clean `[]`; audit 20/20, contrast AA both themes (light 6.09:1 / dark 6.94:1). `BL-CASP2-POPOVER-PROXIMITY` closed. (StatusStrip-surface polish in CASP2-4 / `BL-CASP2-STRIP-POLISH` stays open — those are pre-existing strip treatments, not this popover.)
  - **CASP2-3 (P2 — inline OFF drops the "nobody can open this show" subline) — BY DESIGN.** Spec §4.7: the Overview `#share-access` inactive notice ("The crew link is inactive while this show is unpublished.") is the single source; removing the strip subline is the explicit duplicate-copy fix, not a regression.
  - **CASP2-4 (P2/P3 — finalize popover persistent overlay; live+published two adjacent orange; alert-badge focus-ring lacks ring-offset) — items 2+3 ✅ RESOLVED (2026-07-17, branch `feat/casp2-strip-polish`); item 1 remains open.** **Item 2 (two-orange — switch-on `bg-accent` + Live dot `bg-status-live`, same hue) — RESOLVED** via approach A: a `hidden h-5 w-px sm:block` control/signal divider between the toggle cluster and the first status signal (`StatusStrip.tsx`, gated `!archived && (isLive || syncLabel != null || alertCount > 0)`), so the ON switch and the Live dot stop reading as one orange smear — **without** touching the global `--color-status-live: var(--color-accent)` DESIGN.md decision (re-hueing was rejected for global fan-out to the dashboard). Real-browser §8.10(e) pins the divider between toggle and live badge at ≥sm and `display:none` at 390px. (Impeccable critique accepted residual: at 390px the strip `flex-wrap`s and the 16px `gap-x-4` already separates the switch from the Live dot, so a vertical rule is intentionally suppressed there — a full-height stroke on a wrapping multi-line row produces worse orphaned-stroke artifacts than the mild reduced-by-gap competition; matches the title divider's own `hidden sm:block` call.) **Item 3 (alert-badge focus-ring lacks ring-offset) — RESOLVED** by appending `focus-visible:ring-offset-2 focus-visible:ring-offset-surface` to the strip alert badge, matching the switch (`PublishedToggle.tsx:228`). **Item 1 (finalize popover persistent overlay) — ✅ RESOLVED (2026-07-17, branch `feat/casp2-finalize-inflow`, PR #463).** The finalize hint no longer shares the absolute `POPOVER_POSITION` banner: that constant is now **error-skin-only**, and the finalize skin split off to an in-flow compact chip (`FINALIZE_CHIP` = `inline-flex shrink-0 items-center whitespace-nowrap rounded-sm border border-border bg-surface-sunken px-2 py-0.5 text-xs font-medium text-text-subtle`) rendered as a flex sibling of the switch — so it stays inside the sticky strip's own flow and can never overlay the rail content below the strip during the (longer-lived) finalize window. Visible compact label is mode-dependent (`published` → "Finalizing…", `!published` → "Publishing…", `aria-hidden`); the full sentence is `sr-only` and remains the switch's `aria-describedby` target (role-less, calm — NOT `role=alert`). Real-browser geometry rewritten in `tests/e2e/statusStripToggleLayout.spec.ts` (CI-1 containment, CI-1b strip-height-bound, CI-2/CI-3 compact in-viewport pill width < 200px). Impeccable dual-gate: critique AI-slop NO + detector clean `[]`; audit 20/20; contrast AA both themes (6.09:1 light / 6.94:1 dark). `BL-CASP2-STRIP-POLISH` closed — the last open CASP2 residual.
...
### DESTRUCT-1 — [P2→deferred] Armed morph label expansion reflows the hit target under the finger — ✅ RESOLVED (2026-07-17, branch `fix/destruct1-armed-reflow`)

- **What:** the three two-tap guards swap in a longer armed label, so the button can grow/wrap between tap 1 and tap 2 while a phone user's finger is already traveling. (The deferral originally named **four** guards; `RescanSheetButton` no longer arms — its G3 guard was withdrawn in PR #411 — so the live surface is three: BulkIgnoreControls, PendingPanelDiscardButtons, StagedReviewCard.) Unverified at 360-390px.
- **Why deferred:** needed a real-browser 360px reflow measurement + a design decision (reserve max size vs fixed-height armed state); the auto-revert + label change were the shipped mitigations. Not demonstrable in jsdom.
- **Resolution:** real-browser measurement at 360px (328px content) found only **`PendingPanelDiscardButtons`** relocates the hit-target — idle "Permanently ignore" rides flex line 1 at `x171 w147` (right of Defer), armed "Confirm stop tracking this sheet permanently" (`w328`) wraps to its own row below Defer (`x16`, 155px left + a line down). Fixed with `basis-full sm:basis-auto` stacking both discard buttons full-width `< sm`, so the ignore box is identical idle vs armed (recolor in place, zero reflow); side-by-side restored at `sm+`. No label edit (DESTRUCT-2 grammar untouched). `BulkIgnoreControls` (`x249→x209`, right edge pinned at 344) and `StagedReviewCard` (`x16→x16`, left edge pinned) measured **benign, no change** — both grow from a pinned edge that stays under the finger. Guarded by a jsdom class test + a real-browser layout spec with a negative-control panel (`tests/e2e/pendingDiscardReflow.layout.spec.ts`). Backlog `BL-DESTRUCT-ARMED-REFLOW` closed. Spec `docs/superpowers/specs/2026-07-17-destruct1-armed-reflow.md`.
...
### MOBILEPARITY-1 — [RESOLVED 2026-07-17, branch `fix/mobileparity-heading-fixes`] Strip section heading renders at body size as a page `<h2>`

- **What:** `RecentAutoAppliedStrip` kept `text-sm font-semibold text-text-strong` on its section heading at BOTH `headingLevel` values. On `/admin/needs-attention` that heading is a semantic `<h2>` but rendered at 14px, visually subordinate to the group cards beneath it.
- **Resolution:** heading bumped to `text-base` (16px) at BOTH mounts — `§D3`'s identical-classes-across-contexts invariant is preserved (not forked by level), so the "bump both" option was chosen over the level-fork. 16px stays below both parents at both mounts (page h1 24px, dashboard section h3 18px) — no hierarchy inversion; weight stays `font-semibold` per the app heading-weight convention. Class equality across mounts is now pinned by test (`RecentAutoAppliedStrip.test.tsx`), strengthening §D3 from prose to an executable invariant. `BL-MOBILEPARITY-STRIP-HEADING-SIZE` closed.

### MOBILEPARITY-2 — [RESOLVED 2026-07-17, branch `fix/mobileparity-heading-fixes`] Needs-attention inbox `<section>` carries only `aria-label`, no heading element

- **What:** the primary inbox `<section aria-label="Needs attention">` duplicated the page `<h1>` text and held no heading element, so a heading-navigation SR user reached the strip's secondary `<h2>` before any inbox heading.
- **Resolution:** the inbox `<section>` is now named BY the page `<h1>` via `aria-labelledby` (new optional `titleId` prop on `AdminPageHeader` stamps the id), dropping the duplicate `aria-label`. The region's accessible name is unchanged ("Needs attention", now sourced from the real heading); a heading-nav SR user reaches the inbox heading (the h1) before the strip's h2. Pinned by test (`needsAttentionPage.test.tsx`: `aria-labelledby == h1 id`, no `aria-label`).

## Auto-applied-strip polish batch — impeccable dual-gate deferrals (2026-07-17)

Source: invariant-8 impeccable v3 dual-gate on branch `feat/autoapplied-strip-polish` (spec `docs/superpowers/specs/2026-07-17-autoapplied-strip-polish.md`). **Delivered scope: COLLAPSE-1 (collapsed-header kind-dot cluster), COLLAPSE-2 (shared `CollapsePanel` height-morph across the strip + `IgnoredSheetsDisclosure` + `AddAdminDisclosure`), REDESIGN-2 (singleton card-flatten).** DESTRUCT-3 (all-success bulk-undo sr-only `role=status`) was part of the original batch but shipped first in sibling PR #422 (`fix/destruct-harmonize`); on merge this branch adopted #422's canonical persistent-region implementation (testid `auto-applied-bulk-undo-status-*`) and dropped its own duplicate — the impeccable P2 (a `role=status` that first appears already-populated is skipped by some SR/browser combos → persistent-empty region + text-swap) is satisfied by #422's identical pattern; this branch keeps only an added singular-copy test. MOBILEPARITY-1 (heading `text-sm`→`text-base`) also arrived via #422 and is inherited here. Verdict on the delivered COLLAPSE/REDESIGN surfaces: critique **37/40** (dual-agent, AI-slop CLEAN, cognitive load 1/8), audit **18/20** Strong (anti-patterns PASS), deterministic detector `[]` (0 findings), real-browser morph PASS. The entries below are the accepted/deferred P2/P3s.

### KINDDOT-1 — ✅ RESOLVED (PR #451, merged) Collapsed-header kind dots are a color-only glance channel; `crew_removed` (warn) vs `crew_renamed` (review) hues are near-identical
...
- **Trigger:** a shows-glance-legibility pass, OR a Doug report that the collapsed dots are hard to tell apart. Then give `crew_removed` a non-color differentiator (glyph/ring) or nudge the warn/review hues apart. Backlog: `BL-AUTOAPPLIED-KINDDOT-NONCOLOR-TELL` (was never filed as a row; closed directly here).
- **✅ RESOLVED (2026-07-17, branch `fix/autoapplied-kinddot-tell`):** the shows-glance-legibility pass fired (user-directed cluster ship). `KindDotCluster`'s destructive `crew_removed` marker is now a shape-distinct **centered minus-bar** (`flex size-2 items-center justify-center` wrapper around an `h-0.5 w-2 rounded-full bg-status-warn` bar) instead of a filled disc, so it's distinguishable from the near-identical `crew_renamed` review hue without relying on color; non-removed kinds stay filled `size-2 rounded-full` discs. Every shown marker gained a shape-independent `data-testid="auto-applied-kind-marker"`. `aria-label` (naming every kind incl. "Removed") + `aria-hidden` unchanged — AT text channel untouched. `crew_removed` is `KIND_ORDER[0]` + `MAX_DOTS=4`, so the destructive marker is proven never collapsed into the `+N` overflow (pinned by a new test). `KindPill` (per-row, dot + uppercase text + strikethrough = already dual-channel) left unchanged. Spec `docs/superpowers/specs/2026-07-17-autoapplied-kinddot-nonocolor-tell.md`, plan `docs/superpowers/plans/2026-07-17-autoapplied-kinddot-nonocolor-tell.md`; 3 new + 1 updated test in `tests/components/admin/RecentAutoAppliedStrip.test.tsx`.

### KINDDOT-DIM-1 — [N/A] Real-browser dimension assertion for the `size-2` minus-bar marker — DEFERRED-AS-N/A

- **What:** the project layout-dimensions rule mandates a real-browser `getBoundingClientRect` assertion for fixed-dimension parents with flex/grid children. The KINDDOT-1 minus-bar sits in a fixed `size-2` (8px) `flex items-center justify-center` parent.
- **Why N/A:** the rule targets children whose dimensions **depend on flex `align-items: stretch`** (Tailwind v4 does not default `.flex` to stretch). This marker's inner bar is **CSS-literal** (`h-0.5` 2px × `w-2` 8px), centered — no stretch dependency, no child-fills-parent relationship. It is the same class the rule's own ratified deferrals carve out (`DQ-1`, `OUX-1`, `AUTOAPPLIED-REDESIGN-1`). A standalone esbuild+Playwright harness for an 8px centered bar is disproportionate; the invariant is pinned in jsdom (class assertions on the marker + inner bar).
- **Trigger:** any change that moves the marker off literal dims onto a stretch-derived relationship. Backlog: none (N/A).
...
### COLLAPSE-REGION-1 — [✅ RESOLVED 2026-07-17] `CollapsePanel` emitted a `role=region` landmark per panel; many groups could proliferate landmarks

**Resolution (2026-07-17):** `CollapsePanel` now takes an optional `region?: boolean` (default `true`); when `false` it drops `role="region"` + `aria-label` (a bare `aria-label` is not surfaced on a generic element) while preserving the id/testid (aria-controls target), overflow-hidden clip, and inert-when-closed behavior. `RecentAutoAppliedStrip` group panels pass `region={false}` — the toggle above already names + controls each panel (`aria-expanded` + `aria-controls`, show-name as the accessible name), so the landmark added nothing at the up-to-`STRIP_RENDER_CAP=50` scale. The two singleton disclosures (IgnoredSheets, AddAdmin) keep the default `region=true`. Pinned by `tests/components/admin/CollapsePanel.test.tsx` (region-false drops role/label but keeps id/inert; default keeps the landmark). Note: `BL-COLLAPSEPANEL-REGION-OPTOUT` was cited here but was never actually filed in `BACKLOG.md`, so there was no row to close at resolution time. Written retroactively at its resolved state in `BACKLOG-archive.md` (2026-08-02) so the shipped-source citation at `components/admin/RecentAutoAppliedStrip.tsx:459` resolves.

- **What (original):** `CollapsePanel` set `role="region"` + `aria-label` on every disclosed panel. WAI-APG advises against many region landmarks (~>6); an admin with many recently-auto-applied show groups (up to `STRIP_RENDER_CAP=50`) plus ignored-sheets + add-admin could proliferate landmarks in the AT landmark list.
- **Why it was deferred:** the region is what lets the toggle's `aria-controls` point at a labeled region (spec §1.1). In practice the dashboard rarely shows more than a few groups, so the landmark count stayed small; the opt-out was a net-new API for a bounded, rarely-hit case — now shipped.

## Wizard use-raw full-list controls (2026-07-16)

### USE-RAW-FULL-LIST-1 — [P1→ratified] Callout + full-list both render live role controls; siblings diverge until navigation — ✅ RESOLVED
...
### DESTRUCT-4 — [backlog] the stacked pair put the irreversible action nearest the thumb — ✅ RESOLVED (2026-07-26, PR #604)

- **What:** `BL-DESTRUCT-STACK-THUMB-ORDER`. When `PendingPanelDiscardButtons` wrapped, the irreversible "Permanently ignore" landed BELOW the safe "Defer until modified", nearest a resting thumb. Filed 2026-07-17 as a mobile-stacking issue.
- **What measurement changed:** it was already live on **desktop**. The dashboard Needs-attention rail is `min-[1240px]:w-80` (320px) and the card carries `p-tile-pad`, leaving **278px** for a pair needing 315.95px — so at a 1280px viewport the buttons already wrapped into the wrong order. The original mobile framing understated it.
- **Resolution:** swap the DOM order so Ignore comes first, and delete `basis-full sm:basis-auto` so the row wraps on available width instead of being forced full-width below `sm`. A wrap then puts the safe control lower; where there is room, nothing wraps. Measured on the real component tree: 278px stacks with Ignore above (idle and armed); 316px is knife-edge; 398px and 858px stay on one line. D4's no-relocation guarantee becomes structural through TWO mechanisms — Ignore is the first flex item, fixing its origin within the row; and `IgnoreLabelStack` keeps every label variant mounted in one grid cell (inactive ones `visibility: hidden`) so Ignore reserves its widest variant and the island's width cannot change on arm. First-flex-item alone was insufficient (whole-diff R9 F1: a shorter armed label shrank the island and let it un-wrap beside `Retry now` at ~440px, moving the confirm target dx +107.2px / dy −52px between taps); a numeric `min-width` floor was insufficient too (R10 F1: enlarged text outgrows the floor and the asymmetry returns). Swept 125 rail widths 280–900px: zero movement, zero width drift, Defer never the upper control.
- **Also shipped:** armed label `"Confirm ignore"` with the consequence in the live region plus a visible line under the row; an `event.repeat` guard, because Chrome activates buttons on Enter _keydown_ which auto-repeats — holding Enter previously armed and confirmed ~30ms apart while Space (keyup) was immune; and `ARM_REVERT_MS` moved from 11 copy-pasted literals into `lib/admin/destructiveConfirm.ts`, pinned by T1 (one declaration) and T3 (value is 4s).
- **Rejected on measured evidence:** a viewport-keyed `sm:` fork (does not fix the 278px rail, where the defect actually lives) and a container-query fork (works, but its verification cost ran to nine review rounds without converging). Recorded so neither is re-proposed without new evidence.
- **Backlog rows closed:** `BL-DESTRUCT-STACK-THUMB-ORDER`, plus `BL-DESTRUCT-CONFIRM-COPY-HARMONIZE` and `BL-DESTRUCT-BULK-UNDO-SUCCESS-STATUS`, which had shipped on 2026-07-17 and were never closed. Spec `docs/superpowers/specs/admin/2026-07-25-destruct-thumb-order-drift-guard.md`.

### SHARELINK-CUE-VISIBILITY-1 — the crew-URL cue can fire above the fold — ✅ GRADUATED (2026-08-07, arc A, `feat/a11y-privacy-cluster`)

- **What:** impeccable critique P1 (2026-07-25, share-link-chrome-backlog). The URL block sits at the TOP of the share hub's scrolling popover and the rotate control is below it, so on a phone the operator has scrolled past the block by the time they tap Confirm — `admin-lifecycle-layout.spec.ts` already measured that popover overflowing at 390x560. The critique's proposed fix was the `scrollIntoView` idiom already in `ShareHub.tsx`, fired on the null-to-non-null `flash` edge.
- **Why it was deferred, and what changed:** the deferral re-read the severity down (the rotate control's own `role="status"` banner renders where the operator is already looking, so the OUTCOME was communicated on the local path; what was missed was the enhancement) and named the real cost — auto-scrolling a popover during a destructive confirm is a NEW MOTION SURFACE needing its own transition inventory and reduced-motion arm, which is more than a polish pass should take on unreviewed. **The user un-deferred it into arc A on 2026-08-06**, which is exactly the trigger the entry named: a pass that can own the scroll behaviour and its reduced-motion handling together. Both stated requirements were satisfied rather than waived — the arc's spec carries the transition inventory (3 state pairs both directions + 4 compound rows) and the implementation carries the reduced-motion arm.
- **What shipped:** an effect on the FLASH edge, not the token — `flash` is already the settled answer to "did a cue just fire", folding in the popover being open and the link being live — scheduling one rAF whose id is retained so the cleanup can cancel it. It re-fires on n to n+1, because a second rotation is a new cue and the operator may have scrolled away again.
- **Reduced motion scrolls INSTANTLY rather than not at all.** The cue's information ("the link changed, here it is") is the accessibility payload, so skipping the relocation would withhold it from exactly the users who asked for less motion; `behavior: "auto"` delivers it without animation. Read at event time via the repo's one-shot `matchMedia` idiom, because `usePrefersReducedMotion` returns `null` on first render and that is the wrong answer for an event-time decision. The flash HIGHLIGHT's separate no-cue-under-reduced-motion contract (DESIGN.md) is untouched — this is a new surface with its own arm, and both are now stated.
