...
**Repair shape, three tiers, smallest first.** (1) CLEANUP, and it is the cheapest and largest single win: give every scratch-creating suite a matching removal in a `finally` or `afterAll`. This addresses the accumulation limb, which the other two tiers do not touch at all, and it needs no cache work. (2) INTERIM, ships independently: a single-slot admission class for mutation-score runs so at most ONE generates churn at a time; ordinary suites keep the 2-slot class. Note the design trap — a second slot DIRECTORY is two independent semaphores and would RAISE total concurrency to three, so the class lock must be an additional lock taken alongside the ordinary slot, acquired class-first to fix a global lock order. The orchestrator staggering runs by hand is the manual form of this and is what ended the incident. (3) DURABLE: make the `sourceCache` invalidatable (key on content hash or mtime, or export an explicit invalidate) so one root serves many fixtures. The reduction is per-suite-run, not per-scored-run: a suite creating N roots drops to one, which is ~4x on controlOutlineScan and ~32x on the case 67471884a excluded, NOT the ~250x a per-mutant reading would suggest — separate mutants are separate processes and never shared a root to begin with.
