...
The Flow-8 audit item 8.4 (`docs/audits/e2e-real-world-variation-preparedness-2026-07-07.md` §Flow 8) asks that a transport name mis-parse cannot hide a driver's own itinerary. `transportTileVisible` (`lib/visibility/scopeTiles.ts:177-202`) matches assigned crew by **fuzzy name** (`namesRefer`, `lib/data/nameMatch.ts`), which closes the common variance (nickname / legal-name / case / trim / prefix) but NOT a **hard** mis-parse (garbled/merged-cell driver name sharing no surname token with the roster, e.g. a driver stored as `"DougLarsonHotelBallroom"` vs roster `"Doug Larson"`). In that case the driver silently does not see their own ground-transport block and there is no operator warning. The `flow8-self-serve-trio` milestone ships only a **defensive regression pin** over the existing fuzzy tolerance (`tests/visibility/transportTileVisible*.test.ts` known-gap fixture) — it does **NOT** close this failure mode.
