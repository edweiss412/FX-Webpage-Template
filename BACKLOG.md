...
**Why still open, and why nothing was built:** the defect is real and reproducible, and this entry's own promotion prerequisite is what remains unmet. The measured reach is zero, so a labeled treatment would be a renderer built for a case the corpus does not contain; ruled RECORD AND PARK 2026-08-27. The entry keeps its place in the queue with the number attached, so the next reader inherits the measurement instead of re-deriving it.
