...
**Sweep status (2026-07-24).** Every item below was re-verified against live code on this date, and citations that had rotted were corrected in place — several were badly stale (`AlertBanner.tsx` deleted, `PerShowAlertSection.tsx` deleted, a 9-code registry that is now 20, line numbers shifted). One item closed as obsolete. Working order for the rest: PR2 `BL-ADMIN-QUIET-LINK-AFFORDANCE-A11Y`, PR3 `BL-AGENDA-PERDAY-VIEWER-FILTER`, PR4 `BL-SCAN-SSE-BODY-NULL-CODE`, PR5 `BL-PICKER-TAMPER-ADMIN-ALERT`, PR6 `BL-ALERT-ACTION-LINKS-E2E`. `BL-HEALTH-RESOLVE-DB-LOCKDOWN` stays an accepted risk and `BL-STEP3-IMPECCABLE-LIVE-RENDER` stays unscheduled — both deliberately, not by omission.
...
**Status:** OPEN — PRODUCT POSTURE DECIDED 2026-07-24, implementation queued · **Severity:** low · **Class:** VISIBILITY SCOPE
...
- **Fail-open is mandatory.** Day-to-date matching is best-effort: `parseIsoFromDayLabel` (`lib/crew/agendaDayForToday.ts`) parses a date out of the day heading, with a positional fallback that requires `ext.days.length === showDays.length`. When neither resolves a day for this viewer, render every day expanded (today's behavior) — a failed match must never cost the viewer the agenda, and must never fold the day they actually work.
...
**New-tab-announcement half is still open, and the original path citations are stale.** `components/admin/PerShowAlertSection.tsx` no longer exists; the per-show alert action link now flows through the per-code registry `lib/adminAlerts/alertActions.ts`. Its three resolver call sites are `lib/admin/attentionItems.ts:307` (`resolveAlertAction`), `lib/admin/bellFeed.ts:133` (`resolveAlertActions`), and `components/admin/telemetry/HealthAlertsPanel.tsx:83` (`resolveAlertAction`) — reaching the screen through `review/AttentionBanner.tsx`, `BellPanel.tsx`, and `HealthAlertsPanel.tsx` respectively. The card shell itself is `components/admin/CompactAlertCard.tsx` (consumers: `NoteWarningCard.tsx:93`, `PerShowActionableWarnings.tsx:305`, `review/AttentionBanner.tsx:238`, `telemetry/HealthAlertsPanel.tsx:179`). `components/admin/showpage/StatusStrip.tsx` is NOT a consumer — it only carries the `#share-access` destination the registry links AT, and its sole textual match on `alertActions` is a comment at `:191`. So this is a wider family than the two surfaces the item named.

The defect: an external quiet link marks its `↗` `aria-hidden` (`PerShowActionableWarnings.tsx:283`) with no accessible-name suffix, so a screen reader hears "Open in Sheet" and never learns the link leaves the page. `rg "opens in a new tab" components/` returns exactly two sites — `wizard/Step3SheetCard.tsx:152` and `wizard/VenueMapTile.tsx:138` — both using an `aria-label` that names the destination and the behavior (`aria-label="Open the venue in Google Maps (opens in a new tab)"`), which is the established convention. `components/` holds **18 `target="_blank"` anchors across 12 files** (`grep -rc 'target="_blank"' components/`), so 16 of them carry no such announcement — including the 6 in `wizard/step3ReviewSections.tsx` and this item's own `PerShowActionableWarnings.tsx` link. Whether each of those 16 is a real defect or a deliberate omission (crew-facing `SourceLink`, an already-labelled parent) is the scoping question the fix has to answer per site.
...
**Harness inventory re-verified 2026-07-24 — the item's parenthetical was too absolute.** "Every Step-3 layout spec is a standalone static harness" is no longer true: `tests/e2e/_step3ReviewModalLiveEntry.tsx` mounts the REAL `<Step3ReviewModal>` tree with `react-dom/client` (esbuild-bundled, served over `node:http`) so drag, scroll-spy, and Tab traversal run against real component JS in a real browser, driven by `step3-review-modal.interactions.spec.ts`. What remains true — and is the actual gap — is that no harness boots the **app**: `step3-review-page.layout.spec.ts` states its own posture in its header ("STANDALONE static harness (no app boot / no Supabase / no seed)"), and the live entry stubs `AppRouterContext` and resolves `onRequestSetChecked` immediately rather than exercising a publish path. So the page shell is still transcribed markup, and there is still no seeded `/admin?step=3` render for impeccable to critique. Current surface files: `components/admin/wizard/Step3Review.tsx`, `Step3ReviewModal.tsx`, `step3ReviewSections.tsx`, `Step3ReviewWithFinalize.tsx`, `Step3SheetCard.tsx`.

**Fix (when prioritized):** stand up a minimal admin Step-3 seed (a reserved wizard session + a manifest with ≥1 clean, ≥1 needs-a-look, ≥1 demoted, ≥1 no-details, ≥1 blocking, ≥1 set-aside row), then run the impeccable v3 dual-gate against the live `/admin?step=3` render — including an explicit dark-mode warn-contrast check and the double-"Review" affordance on demoted RESCAN cards (close-out §12 finding 7).
