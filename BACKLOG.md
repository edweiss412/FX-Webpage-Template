...
`vitest.parallel-extra-files.ts` is generated by the manual procedure documented in `docs/superpowers/specs/ci/2026-07-20-ci-unit-suite-phase3-file-granular-serial.md` §3.4: derive the candidate population (BASE_INCLUDE minus vitest default excludes, nightly globs, env-bound files, and everything claimed by a `PARALLEL_TEST_GLOBS` entry), run it with `fileParallelism: true` against closed-port DB/Supabase endpoints three times, keep the intersection, then apply ALL of the spec §2.6 eligibility rules: drop files that executed no assertions in any repeat, that reference a local Postgres endpoint directly (54322 / LOCAL_TEST_DATABASE_URL / loopback:5432), that match a DB naming convention (tests/db/\*_, _.db.test.ts, _Db.test.ts, _.realdb.test.ts, \*-real-db.test.ts), that write to a repo-tracked path (mkdtemp/tmpdir-confined writers stay eligible), or that reference a loopback Supabase REST endpoint on 54321. Automating that as a committed script was DESCOPED from Phase 3 (spec §1.0) after five plan-review rounds on how to test-bind the tool's CLI seams — including a genuine `.mjs`-cannot-import-`.ts`-under-plain-node defect. Design input for whoever picks this up: put every decision in a pure, in-process-testable core and keep the entry point logic-free, or run it under `tsx` like the repo's other TypeScript scripts.
