...
and is statically skipped (`BL-RIGHTNOW-SECTION57-FIXTURE-INERT`). Only the NINE deletions move the
...
### BL-REPORT-MODAL-STATIC-IMPORT-SHIPS-CATALOG — every route with a report button downloads the whole message catalog

**Status:** OPEN · **Severity:** MEDIUM (page weight on a venue-floor phone; no behavior defect) · **Class:** client-bundle weight · **Filed:** 2026-08-09 (`feat/help-report-surface` impeccable audit, P1) · **Effort:** M

**Probed, not theorized.** The import chain is static and unconditional:

```
$ grep -n 'import { ReportModal' components/shared/ReportButton.tsx
34:import {
  ReportModal, ...
$ grep -n 'HelpAffordance' components/shared/ReportModal.tsx
37:import { HelpAffordance } from "@/components/admin/HelpAffordance";
$ grep -n 'MESSAGE_CATALOG' components/admin/HelpAffordance.tsx
41:import { MESSAGE_CATALOG, type MessageCode } from "@/lib/messages/catalog";
$ wc -l lib/messages/catalog.ts
4052 lib/messages/catalog.ts
```

`ReportButton` already mounts `ReportModal` only when open (`components/shared/ReportButton.tsx`), so
the modal is not RENDERED early — but a static import puts it, `HelpAffordance`, and the full catalog
in the route bundle regardless. `feat/help-report-surface` made `/help/errors` the newest instance:
the page server-renders every catalog entry as HTML and now also downloads that prose a second time
as JS to render one button. The crew footer and the admin surfaces have carried the same weight since
M8.

**Scope of a real fix.** `next/dynamic` the modal inside `ReportButton`, so the download defers to
first open. Every surface wins at once.

**Why deferred rather than repaired in the branch that found it** (disposition reason (c), AGENTS.md
class-sweep rule): a dynamic import changes the mount contract for all four existing report surfaces
— the modal would no longer appear synchronously on click — and the M8 suites that pin that contract
(`tests/components/report/**`, the crew-footer and admin e2e specs) assert synchronous appearance.
That is a hardened-surface redesign the help-surface PR does not otherwise touch, and the 2026-08-09
spec §1.1 item 9 fences existing-surface behavior as byte-identical.

**Trigger:** any page-weight budget on `/help/**` or the crew page, or the next milestone that
touches `ReportButton`'s mount path for another reason.

---

### BL-REPORT-CTA-REMOUNT-DROPS-FOCUS — a mid-open fragment change closes the report dialog with no cue and no focus target

**Status:** OPEN · **Severity:** MEDIUM (a11y; the draft survives but is unreachable without returning to the fragment) · **Class:** dialog a11y · **Filed:** 2026-08-09 (`feat/help-report-surface` impeccable critique P2 + audit P1) · **Effort:** M

**Probed, not theorized.** `app/help/errors/_components/HelpReportCta.tsx` keys `ReportButton` by the
live fragment, so a `hashchange` while the modal is open remounts the button and unmounts the modal.
`tests/help/helpReportCta.test.tsx` case 6 asserts exactly that unmount — it is the ratified
mechanism (2026-08-09 spec §2.1), not an accident: it is what stops a live attempt from being
re-pointed at another code. The unaddressed consequence is what happens after:

```
$ sed -n '68,77p' lib/a11y/dialogFocus.ts
...
The trigger IS the element the remount just removed, so focus lands on `document.body` and a screen
reader gets no dialog-closed cue. The realistic path is the phone back gesture between two fragment
history entries.

**Why deferred rather than repaired in the branch that found it** (disposition reason (a)): the two
candidate repairs are a product decision this PR cannot settle. Freezing the hash for the lifetime of
an open attempt removes the unmount entirely and keeps key/draft/`helpCode` co-variance — but it
contradicts the remount the spec ratified at R5 and the test case that pins it. Restoring focus to
the NEW trigger keeps the ratified mechanism but needs a rule for when focus may move during ordinary
reading (every jump-list click is a `hashchange`, and stealing focus to the page-foot CTA on each one
is worse than the defect). Pick one deliberately, with the spec amended in the same pass.

**Trigger:** the next `/help/errors` a11y pass, or any report from a screen-reader user.

---

### BL-REPORT-MODAL-NO-ESCAPE-HANDLER — the report dialog cannot be dismissed with Esc

**Status:** OPEN · **Severity:** LOW (Close is Tab-reachable, so no SC failure; breaks the APG dialog pattern) · **Class:** dialog a11y · **Filed:** 2026-08-09 (`feat/help-report-surface` impeccable audit, P2) · **Effort:** S

**Probed, not theorized.**

```
$ grep -c 'Escape' components/shared/ReportModal.tsx
0
$ sed -n '13,14p' lib/a11y/dialogFocus.ts
 *      to the last. Esc handling is the dialog's responsibility
 *      (typically already wired in the dialog component).
```

The focus helper explicitly delegates Esc to the dialog, and the dialog never wires it; the only key
handling in `ReportModal` is Cmd/Ctrl+Enter to submit. Pre-existing since M8 on all four report
surfaces; `feat/help-report-surface` extends the gap to a fifth.

**Why deferred rather than repaired in the branch that found it** (disposition reason (b)): the
2026-08-09 spec §1.1 item 9 ratifies that the four existing surfaces keep byte-identical behavior in
this PR, and adding a dismissal path is a behavior change to all of them. It is a one-place fix and
should ship as its own small PR with the modal's own suite as the net.

**Trigger:** the next PR that touches `ReportModal`'s keyboard handling, or any keyboard-a11y sweep.
