...
`tests/components/admin/sheetIconLinkContainment.test.ts` walks the repository and ingests GITIGNORED local directories. Nothing in the walk consults `.gitignore`, so any scratch tree a session leaves behind becomes input to a containment census that was only ever meant to range over tracked source.
