...
**Status:** IN PROGRESS · **Branch:** docs/mutation-ledger-accuracy · **Filed:** 2026-08-16 (found while shipping `chore/round-economy-enforcement-pair`, whose own enrolled surfaces passed) · **Re-scoped:** 2026-08-17 (`docs/mutation-ledger-accuracy`) · **Severity:** MEDIUM (a permanently red non-required gate trains every arc to read its verdict as noise, which is how the twelve-survivor catch of #786 nearly did not happen) · **Class:** CI gate fidelity · **Effort:** S-M

**Renamed away from a title carrying a count.** This entry shipped as `…-TWO-SURFACES`, and in the nine hours after it was filed the count went two, three, four — while the MEMBERS turned over completely, so that both originally-named failures are now retired and not one of the four live failures is one the original row mentioned. The count was the fastest-staling fact in the row, so the id no longer carries one. Nothing else cites the old id; this heading was its only occurrence.

**The live failure set**, read PER-ANNOTATION off run 31989590619 (head `c5518dfab`, the branch tip merged as #827 / `ad9638fa9`, so this is current main):
...
- Original failure 2, `tests/parser/mutationHarness.shard4.test.ts` / `blank-row:inject:2026-04-asset-mgmt-cfo-coo-waldorf:B4:L38..L45` — **retired.** `parser-shards (4)` is `success` on all three most recent runs (31984549526, 31984664534, 31989590619), as is every other parser leg and `parser-gates`. Not attributed to a specific merge here; whatever closed it is on main.

**Reachability: PROBED.** Every failure above is quoted from a CI annotation, not inferred. Items (1) and (4) additionally reproduce cross-branch: (1) appears identically on `test/execution-methods-driver-derived` (run 31984549526) and `chore/heavy-orphan-reaper` (run 31984664534), two unrelated heads, neither of which touches `lib/ci/shardBudget.ts`. Items (2) and (3) are settled more strongly than by observation — they are a static disagreement between two committed files, reproducible with no CI at all. Item (4) is so far observed on ONE run, because on both earlier runs its shard was killed by the timeout described below and therefore reported nothing; treat a second sighting as confirmation rather than as new.
...
- **A shard that hits its `timeout-minutes: 90` ceiling reports NOTHING** — no gate annotation for any surface it holds, including the ones that failed. This is not hypothetical: `source-shards-2` was cancelled at 90m17s on run 31984664534 and at ~90m18s on 31984549526, which is why (4) went unseen on both. A timed-out leg is not a green leg, and its budget record (`5410s`) is the timeout wall, not a measurement.
- **Leg NUMBER is not a stable name for a surface.** The partition is recomputed from the registry on every run (`sourceShardAssignment`, LPT over modelled child boots), so #827's registry edit moved `shardBudget` from leg 3 to leg 0 and `rowScanOpener` from leg 0 to leg 1 within one merge. Cite the SURFACE; re-derive the leg when one is actually needed.
...
**First scheduled step:** repair (2) and (3) together as the one class they are — a single edit to `tests/mutation/source/expectedLedgerKinds.ts`, since the registry rows are the reviewed artifact and the declarations are what drifted from them. Then re-key (4), which is mechanical. Then decide (1) on its merits: `lib/ci/shardBudget.ts:73` is a `budgetSeconds <= 0` boundary and `:118` is an integer inside a warning STRING, so both are plausibly equivalent-by-argument rather than uncovered — which makes them ledger rows with reasons, not new tests. Confirm the clean baseline with `workflow_dispatch` on main, not by reading a PR run.

## BL-MUTATION-HARNESS-PR-TRIGGER-FANOUT — the harness's path-filtered PR trigger spends fourteen runners per harness-touching PR, and they compete with that PR's own required checks

**Status:** OPEN. · **Filed:** 2026-08-17 (`docs/mutation-ledger-accuracy`, from the shipping arc of #834 — it existed only in that arc's handoff message until now) · **Severity:** MEDIUM (it does not fail anything; it delays the merge path for exactly the PRs least able to afford it) · **Class:** CI capacity · **Effort:** S
...
**Reachability: PROBED.** A harness-touching PR now enqueues 16 jobs, 14 of which are full test legs: `parser-shards` ×8, `parser-gates`, `source-shards` ×4, and `source-gates`, plus the two cheap coordinators `budget` and `notify`. Measured consequence on the shipping arc itself: those legs occupied the account's queue alongside that PR's own required checks, and the arc CANCELLED its own harness run to get its required checks moving. The legs are long as well as numerous — on run 31989590619 the four source legs took 891s, 2172s, 3073s and 3361s of job wall clock, and on two earlier runs one leg was cancelled at its 90-minute ceiling.
...
**Status:** OPEN. · **Filed:** 2026-08-17 (`docs/mutation-ledger-accuracy`, on behalf of #833, which reported it batch-wide and did not file it) · **Severity:** MEDIUM (a latent flake on a full local clone; CI is structurally immune) · **Class:** test durability · **Effort:** S
