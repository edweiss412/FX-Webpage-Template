...
`vitest.parallel-extra-files.ts` is generated by the manual procedure documented in `docs/superpowers/specs/ci/2026-07-20-ci-unit-suite-phase3-file-granular-serial.md` §3.4: derive the candidate population (BASE_INCLUDE minus vitest default excludes, nightly globs, env-bound files, and everything claimed by a `PARALLEL_TEST_GLOBS` entry), run it with `fileParallelism: true` against closed-port DB/Supabase endpoints three times, keep the intersection, then apply the spec §2.6 eligibility rules (drop files that executed no assertions, that reference a local Postgres endpoint directly, or that match the DB naming conventions). Automating that as a committed script was DESCOPED from Phase 3 (spec §1.0) after five plan-review rounds on how to test-bind the tool's CLI seams — including a genuine `.mjs`-cannot-import-`.ts`-under-plain-node defect. Design input for whoever picks this up: put every decision in a pure, in-process-testable core and keep the entry point logic-free, or run it under `tsx` like the repo's other TypeScript scripts.
