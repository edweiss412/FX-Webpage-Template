...
Last reconciled: 2026-07-31 (`feat/sheet-icon-link-affordance-class` whole-diff rounds r22–r37, over the 2026-07-27 three-parallel-pass merge base) — mainline #628 graduated three in-place-terminal entries (`BL-E2E-LIFECYCLE-INACTIVE-NOTICE-RETIRED` PR #615, `BL-HEADER-PROBE-RESIDUAL-VACUITY` PR #617 — its one live follow-up rides `BL-SECTION-HEADER-VISUAL-REQUIRED-CONTEXT` — and `BL-AGENDA-PERDAY-VIEWER-FILTER` PR #610), corrected `BL-HEADER-FONT-FALLBACK-WRAP`'s "no `next/font` import anywhere" claim (the crew show layout has imported Inter since 2026-05-03; the admin tree is what loads nothing), kept `BL-CI-STALE-BRANCH-PROTECTION-COMMENT` deliberately (sub-entry of a still-open parent; rationale in the entry), and hardened the graduation meta-test against terminal HEADINGS and SHIPPED status lines. `fix/duration-tokens-emit-no-css` graduated `BL-DURATION-TOKENS-EMIT-NO-CSS` (shipped, alias approach; residual gap filed as `BL-BARE-TRANSITION-NO-DURATION-CLASS`). `feat/ci-dark-descoped-guards` graduated `BL-CI-UNREGISTERED-SELF-CONTAINED-SPEC` and `BL-CI-ENV-DEPENDENT-CONFIG-NARROWING` (spec-registration detector + post-run baseline comparator). `feat/sheet-icon-link-affordance-class` independently graduated the same three plus its own closure `BL-HEADER-LINK-AFFORDANCE-CLASS`, widening the meta-test to heading-suffix and opening-line spellings (SUPERSEDED/DONE in the terminal set, anchored + case-insensitive). The merge deduplicated the double-graduated archive copies (mainline provenance kept), unioned both meta-test widenings, and reverted this branch's graduation of `BL-CI-STALE-BRANCH-PROTECTION-COMMENT` per #628's deliberate keep. 2026-07-31 (whole-diff r22–r26): the meta-test additionally sees through markdown container prefixes — bullet, ordered-item (one through nine digits per the CommonMark grammar), and blockquote markers, plus GFM task-list checkboxes riding a list marker — on Status/Filed/opening lines, in both the matchers and the field-line filters; marker-only lines no longer swallow the opening-line selection, and direct negations (`Not`/`Never` immediately before a terminal word) read as OPEN claims in the bold-field lane. r27 closes the surface-spelling class by normalizing each section before any lane runs — `__` folds to `**`, HTML comments (rendered as nothing) are deleted — and the negation/PARTIAL veto window crosses emphasis boundaries. r28 narrows normalization to render-faithful folds only (boundary-anchored `__` pairs, same-line comments — no cross-line deletion that could swallow claims behind code-fence literals) and ratifies the guard's threat model in its header: a drift tripwire over first-party plain-markdown prose; render-equivalent obfuscation (HTML wrappers, character references, links, tables, commented headings) is review's failure class, not this tripwire's. r29, inside that retained scope: italic label wrappers (`*Status:*` and friends) join the matchers and filters, and multiword negations/historical qualifiers (`Not fully`, `No longer`, `Previously`) read as open claims. r30: mixed-emphasis labels (up to three marks per side), CommonMark heading whitespace, and an anchored heading-checkmark lane; the r29 fence-blanking and inline-code stripping are REMOVED — each shipped its own CommonMark-semantics defects — and the regex-grammar boundary plus the mdast port are filed as `BL-LEDGER-GUARD-MDAST-REWRITE`. Same day (r37): `BL-HEADER-PILL-LINK-TOUCH-BUFFER` and `BL-HEADER-SUBBLOCK-HIERARCHY-WIDE` graduated — both resolved by `feat/sheet-icon-link-affordance-class` itself (asymmetric-overlay 2px pill buffer; linkless sub-block floor drop at every width). Prior: 2026-07-26 — `BL-CHILDLESS-GROWABLE-STATIC-GUARD` graduated on `feat/childless-growable-static-guard`; 2026-07-25 — the three phantom-gap items plus 7 terminal-status entries; 2026-07-24 — 30 entries.
...
## BL-BARE-TRANSITION-NO-DURATION-CLASS — bare `transition-*` sites sit outside the duration-token system
