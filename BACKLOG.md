...
**Root cause.** Scratch roots are built with `mkdtempSync(join(tmpdir(), ...))` per mutant child and destroyed after, because the scanner caches parsed files BY ABSOLUTE PATH and never invalidates — so a root cannot be reused across mutants (ctloutline measured this while splitting the thirty-two-form case: shared roots return stale parses). Event volume is therefore proportional to mutants × roots × files-per-root, and fseventsd is the unbounded amplifier.

**Repair shape, two tiers.** (1) DURABLE: make the scanner cache invalidatable (key by content hash or mtime, or expose an explicit invalidate), then reuse ONE scratch root set per run — event volume drops by the mutant count, ~250×. (2) INTERIM, ships first and independently: a dedicated single-slot semaphore class for mutation-score runs (`pnpm heavy --class mutation` or equivalent) so at most ONE score run generates churn at a time; ordinary suites keep the 2-slot class. The orchestrator staggering runs by hand is the manual form of (2) and is what actually ended the incident.
