...
**Status:** IN PROGRESS · **Branch:** fix/mutation-shard-ceiling-pin · **Filed:** 2026-08-16 (found while shipping `chore/round-economy-enforcement-pair`, whose own enrolled surfaces passed) · **Re-scoped:** 2026-08-16 (`docs/mutation-ledger-accuracy`) · **Severity:** MEDIUM (a permanently red non-required gate trains every arc to read its verdict as noise, which is how the twelve-survivor catch of #786 nearly did not happen) · **Class:** CI gate fidelity · **Effort:** S-M
...
**The consequence is not the red `budget` job; it is the leg that goes silent next — SUPERSEDED 2026-08-21 as to the margin, not as to the mechanism.** A leg that hits its job ceiling reports NO gate annotation for ANY surface it holds — the trap `BL-MUTATION-HARNESS-MAIN-RED` records, which has already masked a genuine surface failure once. The binding leg was at 5210 s against the then-5400 s ceiling: **190 s of margin**, against the ~1800 s that two nightlies of enrolment just added. The ceiling is now 7200 s and pinned at >= 2x the budget, so that specific margin is superseded; what is NOT superseded is the mechanism, since the pin restores visibility without changing a single leg's cost. The next comparable batch does not merely breach the budget again, it censors a quarter of the source gate — and a censored leg is indistinguishable from a leg that had nothing to report.
