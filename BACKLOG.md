...
## BL-INTERNAL-CODE-ENUM-SCAN-WIDEN — the parse-warning enum generator scans one directory, so ten live emitters are hand-listed

**Filed:** 2026-08-02 (retroactively; cited by `lib/dev/attentionScenarios/tier1.ts` and `docs/superpowers/specs/2026-07-20-attention-scenario-gallery-design.md:165` as if already filed, with no row anywhere). **Class:** generated-registry completeness. **Effort:** M (revised 2026-08-03; filed as S against a premise that did not hold).
...
The `tier1.ts` comment attributes the miss to the content regex alone. Verified 2026-08-02, that is only the second filter: the emitters live outside the `["lib/parser"]` root the scan ever opens, so the regex never runs on them. Widening the content heuristic without widening the directory list would change nothing.

**AMENDED 2026-08-03 (`chore/ledger-body-ids-enum-scan-widen`) — the remedy above does not work, and the row understated the problem.** Three probes, evidence in `docs/superpowers/specs/2026-08-03-ledger-body-defined-ids-and-enum-scan-widen-design.md` §B.2:

- **Widening the scan roots is not the fix.** Re-running the current predicate over wider roots absorbs ONE of the four named codes (`PULL_SHEET_OVERRIDE_CONTENT_CHANGED`) and pulls in eight codes from three unrelated namespaces — six admin-alert codes from `lib/sync/runScheduledCronSync.ts`, `ROLE_FLAGS_NOTICE`, `STAGED_PARSE_OUTDATED_AT_PHASE_D`. Widening to all of `lib` additionally feeds the generator its own consumer, `lib/dev/attentionScenarios/tier{2,3}.ts`.
- **The other three are missed for a reason no root change touches.** `AGENDA_SCHEDULE_LOW_CONFIDENCE` and `AGENDA_SCHEDULE_TIME_ADJUSTED` are emitted through a positional helper, `warn("CODE", message)` (`lib/sync/enrichAgenda.ts:45-47`, used at `:424`/`:432`), which a `code:`-property regex cannot see at any root. `PULL_SHEET_ON_ARCHIVED_TAB` was already generated. And the row's attribution of `AGENDA_SCHEDULE_LOW_CONFIDENCE` to `lib/agenda/extractAgendaSchedule.ts` is **wrong** — that file's only occurrence is a `code:` field inside a `log.warn(...)` call, correctly stripped; it emits no ParseWarning.
- **The residue was 4 rows against 10 real ones.** A type-aware census (ts-morph: object literals whose contextual type is `ParseWarning`, plus calls returning one) finds ten production codes the generator never reaches, across `lib/sync/enrichAgenda.ts`, `enrichWithDrivePins.ts`, `snapshotAssets.ts`, `applyStaged.ts`, `pullSheetOverride.ts`. Six MORE are generated only by luck: nothing sees their emit sites, but `lib/parser/dataGaps.ts` — a `{ code, label }` registry that happens to sit inside the scanned root — happens to name them.
- **A text prefilter on `ParseWarning` is unsound.** `lib/sync/enrichVenueGeocode.ts`, `enrichTransportAssignees.ts`, and `applyStaged.ts` build ParseWarning-typed literals without ever naming the type, so any guard keyed on "files that mention ParseWarning" is blind to three live emitters on day one.

**Shipped on that branch** (the provable half): `warningCodes()` now partitions on `source.split(",").includes(...)` rather than `===`, which was silently dropping every parse warning that shares a code with `pending_ingestions.last_error_code`; the residue is re-derived from the census; and a **dead-row ratchet** fails the moment a residue row starts being generated. That ratchet is what answers the row's real complaint — the list can no longer rot invisibly — and is why the residue can stay while the generator is fixed properly. Gallery coverage 47 → 56 scenarios.

**Work (corrected):** make the generator's `parse_warnings.code` pass recognize ParseWarning constructions by TYPE rather than by directory-plus-regex, since the live emit shapes are `code:` object properties, positional `warn("CODE", msg)` helper calls, AND object literals that never name the type. Then delete `EXTRA_WARNING_CODES` and add the completeness guard. Note before starting: the type-aware census costs ~382 s on an M-series host, so it needs a lane decision — the slow `x-audits` lane, not the default vitest run. Effort is **M**, not S.
