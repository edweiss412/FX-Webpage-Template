...
`lib/sheet-links/buildSheetDeepLink.ts:22` cannot detect this. It guards structure only (allowlisted title, numeric gid), so an R1 anchor is accepted and the "In sheet" link opens the old range instead of falling back to `#gid=0`. The mis-link persists until a later scan of that sheet produces a NON-EMPTY map — which is not guaranteed to happen. Below the watermark the cron path skips the file entirely (`lib/sync/perFileProcessor.ts:337`), and a workbook whose recognized regions were renamed or removed extracts SUCCESSFULLY to `{}` on every pass, so its old anchors persist indefinitely.
