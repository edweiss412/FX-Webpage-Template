...
The trend matters more than the peak here: accumulation is a claim about growth over time, and a peak is a single sampled instant. Both thirds sit near zero and the difference between them (0.10 backends) is far below the ~5 a single file reaches, so the series shows no growth — though with the mean this close to zero, that is better read as "no signal of accumulation" than as a precise rate. postgres.js backends were open in only 175 of 2256 samples, and no sample exceeded 5.
...
**What replaced it.** The measurement holds only while the isolation does. `tests/cross-cutting/db-test-connection-hygiene.test.ts` pins both settings from the **resolved runtime config**, not the authored one: `isolate` directly, and file parallelism via `maxWorkers === 1` (the worker config does not carry `fileParallelism`, and vitest applies a CLI `--fileParallelism` after project options — so a config-file check alone reads `false` while the run is concurrent). It also asserts the authored `serial.fileParallelism`, and scans `package.json` scripts, workflow YAML, and `scripts/` launchers for the committed spellings that disable either, including `VITEST_MAX_WORKERS` — the case the runtime checks cannot see, since a run that excludes this file also excludes its assertions. That scan is bounded by being a text scan: a flag assembled at runtime, a value from repo or org CI settings, or an alternate `--config` whose file sets `isolate: false` all pass it. Underneath sits an AST census floor of the 60 unclosed module-scope clients, with membership fixtures so the census definition is pinned and not just its count. Verified by 14 mutation injections — 13 turn the guard red, and a comment-only mention of the flags correctly stays green. That covers each assertion's main failure mode, not every way each could break: loss of the `__vitest_worker__` internal, a vitest change to how `fileParallelism: false` resolves, and the scan's blind spots below are not among them. If disabling isolation ever becomes desirable, the `makeTestSql` work above becomes necessary again — that is the real trigger, not a connection count.
