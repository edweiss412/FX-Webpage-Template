...
**New-tab-announcement half is still open, and the original path citations are stale.** `components/admin/PerShowAlertSection.tsx` no longer exists; the per-show alert action link now flows through the per-code registry `lib/adminAlerts/alertActions.ts`, consumed by `components/admin/showpage/StatusStrip.tsx` and `components/admin/telemetry/HealthAlertsPanel.tsx`, and rendered through `components/admin/CompactAlertCard.tsx` (consumers: `NoteWarningCard.tsx:93`, `PerShowActionableWarnings.tsx:305`, `review/AttentionBanner.tsx:238`, `telemetry/HealthAlertsPanel.tsx:179`). So this is a wider family than the two surfaces the item named.
...
Where the raw string still flows, and why that is in-policy: `lib/drive/watchEscalation.ts:155` reads `alert.context?.error_message` into the escalation **email** body sent to configured admin recipients, and the same string is persisted on `shows.last_error_message` for the observe CLI (`lib/observe/query/failures.ts`, PII-gated through `sanitizeIdentityString`). Invariant 5 governs user-visible UI copy; an operator escalation email and a developer-tier telemetry read are the debug-only affordance the original item proposed keeping. `lib/adminAlerts/alertIdentityMap.ts:118` still carries a stale comment referring to "the pre-existing `error_message` `<code>` block" — harmless, but it is the one remaining reference to the retired surface.
