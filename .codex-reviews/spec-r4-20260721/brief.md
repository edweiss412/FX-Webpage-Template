# Adversarial spec review R4 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY. Findings only; no patches. Do NOT invoke any nested review.
## CRITICAL: Do NOT use any tools. Reason ONLY from the text below; reply with prose.

## Fresh-eyes posture
R1 (15), R2 (9), R3 (6) already ran and were repaired; §12 summarizes. The train is CONVERGING. Your job: confirm convergence or identify what genuinely remains. Do NOT manufacture findings to justify the round — if the spec is implementable without ambiguity, return APPROVE. Enumerate ALL instances of any real finding class.

## DO NOT RELITIGATE (§1.1, settled across rounds)
1. Cut trailing instruction for 3 unfixable codes. 2. Keep for COLUMN/FIELD. 3. Instance copy replaces generic line. 4. Under-row placement unconditional. 5. Not forced into AttentionItem. 6. Nesting = PR#532 deferral successor. 7. Pure composer (autocorrectGuidance) — details only. 8. Duplicate same-name: all cards under first row. 9. Instance COPY is universal (staged too); only PLACEMENT + NESTING are published-only (R3 resolution). 10. Merged stack is a deterministic pure re-derivation: alerts (attention order) then warnings (model order), cap 2 visible (R3 resolution).

## VERIFIED CODE FACTS (given)
- canonicalCrewKey=name.trim().toLowerCase(); interpolate leaves unresolved placeholder literal.
- BulkIgnoreControls: chip iff `bulk` present; text "Ignore all N"; type in eyebrow; group.cards renders any node incl empty.
- extractRoleFlags: one call site (crew.ts:364), warnings consumed only via stampedRoleWarnings.
- CREW_CAP=30; 13 emit sites (STAGE1 ROLE1 SECTION1 COLUMN2 FIELD8).
- dougFacing: STAGE1270 ROLE1285 COLUMN1299 SECTION1412 FIELD1426; non-crew carry _<sheet-name>_, crew _<crew-name>_.
- PR#532 cut published Parse warnings panel to info rows; a warn-severity crew-scoped autocorrect warning renders in exactly ONE place.
- SectionId: venue,event,crew,contacts,schedule,agenda,hotels,transport,rooms,diagrams,packlist,billing,warnings,report.

## WHERE I WANT PRESSURE
A. Is anything still CONTRADICTORY or underspecified enough to block a plan author? A genuinely blocking ambiguity, not a wish for more prose.
B. §4 composer guard totality; §5-6 conservation across every mutation; §9 transition completeness; §10 anti-tautology (does any test only prove "a function ran"?).
C. Any stale cross-reference, count, or citation that survived the R3 self-consistency sweep.

## OUTPUT
[SEVERITY: BLOCKING|HIGH|MEDIUM|LOW] <title> / Location / Claim / Reality / Impact
