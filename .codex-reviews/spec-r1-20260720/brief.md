...
For each finding: `SEVERITY (P0/P1/P2/P3) — <one-line claim> — <section/line> — <why it fails, concretely>`.

Then a short "classes exhausted" note listing each finding class and confirming you enumerated all instances.

End with a final line exactly:
`VERDICT: APPROVE` or `VERDICT: NEEDS-ATTENTION` or `VERDICT: BLOCKING`

## pnpm spec:lint output (already run)

```
citations:
  ADVISORY CITATION_SYMBOL_UNMATCHED 104:210 no same-line identifier found near scripts/extract-internal-code-enums.ts:71-72
  ADVISORY CITATION_SYMBOL_UNMATCHED 198:82 no same-line identifier found near components/admin/ReSyncButton.tsx:243
summary: 0 hard, 2 advisory
```
