# Adversarial PLAN review R3 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY. Findings only. Do NOT use tools. Reason ONLY from the text below.

## Posture
Implementation PLAN over an APPROVED spec. Rounds: R1 10 findings (BLOCKING), R2 1 (BLOCKING, a scope clarification). Converging. Decide if this plan is now executable by a TDD implementer without a blocking ambiguity. If yes, APPROVE — do not invent findings. The spec design is settled; review the PLAN only.

## R2 RESOLUTION (settled)
The section `Ignored (N)` disclosure is section-level (sectionWarningExtras.tsx:138, gated on model.ignored.length), independent of the per-code active-group emission predicate. The active group (eyebrow+chip+fallback cards) emits iff fallbackCards≥1 OR activeN≥2; that predicate never touches the ignored disclosure.

## Judge on: TDD soundness; anti-tautology; sequencing (no forward deps); coverage of all 21 spec tests + 4 changes; strict-TS realism; meta-test/registry fan-out. Report only what genuinely blocks or materially weakens an implementer; note polish as LOW.

## KEY SPEC FACTS (given)
13 emit sites; ROLE subject stamped at crew.ts:367-373; instance line plain-text (no renderEmphasis); copy universal, placement/nesting published-only; deterministic stack (alerts then warnings, cap 2, native <details>); duplicate same-name → all under first row; blank subject → group fallback; bulk crew-scoped: cards=fallback, bulk.items counts all N; §8 five dimensional rows @ 375+1280px hydrated; ignored disclosure section-level.

## OUTPUT
[SEVERITY] <title> / Location / Claim / Reality / Impact
...
**Scope clarification (R2 BLOCKING):** the per-code ACTIVE group (eyebrow + `Ignore all N` chip + fallback `items`) is DISTINCT from the section's single `Ignored (N)` disclosure. That disclosure is section-level — `section-ignored-warnings-<id>`, rendered iff `model.ignored.length > 0` (`components/admin/showpage/sectionWarningExtras.tsx:138`), and the whole extras block renders iff `active.length > 0 OR ignored.length > 0` (`sectionWarningExtras.tsx:86`). It is NOT nested inside any per-code active group, so the active-group emission predicate cannot unmount it. At N=2→1 (test 5e): the code's ACTIVE group unmounts (predicate false), the survivor stays under its row, and the newly-ignored warning renders in the section-level ignored disclosure, which persists because `model.ignored` is now length 1.

**Impl:** for a crew-scoped code, the `activeGroups` entry's `bulk.items` counts ALL N active fingerprints (under-row + fallback), so the chip reads `Ignore all N`; `items` (cards slot) holds only the fallback subset from Task 5. The ACTIVE group is emitted iff `fallbackCards≥1 OR activeN≥2` — this governs ONLY the eyebrow+chip+cards, never the section ignored disclosure (above). No new `BulkIgnoreControls` mode — the existing optional-`bulk` / any-`cards` shape (`components/admin/BulkIgnoreControls.tsx:127-195`) already supports it. Fingerprint-set ignore semantics unchanged, so one click clears both placements and the ignored identities flow into the section disclosure.
