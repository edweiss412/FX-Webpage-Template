# Adversarial spec review R2 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY
Do not fix issues or propose patches. Surface findings only. Do NOT invoke any nested review command.

## CRITICAL EXECUTION CONSTRAINT
Do NOT use any tools. Do NOT read files or run commands. Reason ONLY from the text inlined below and reply with prose. Everything you need is here.

## Fresh-eyes posture
Treat this spec as if you have not seen it before. R1 already ran and its 15 findings were repaired; a summary is in §12. Do NOT simply re-check R1 items — hunt for what R1 missed and for defects the repairs introduced. Enumerate ALL instances of any finding class in THIS round (a vector dripped one-per-round is a review defect).

## Context
Admin published-show modal. Five `*_AUTOCORRECTED` parse-warning codes render generic per-code catalog copy. This spec makes the card state the real correction and, for the two crew-scoped codes, the crew member's name, and moves crew-scoped cards under their member's row.

## DO NOT RELITIGATE (ratified; §1.1 carries all with citations)
1. Cutting the trailing instruction for the 3 unfixable codes (STAGE_WORD, ROLE_TOKEN, SECTION_HEADER).
2. Keeping the fix instruction for COLUMN_HEADER / FIELD_LABEL.
3. Instance copy REPLACES the generic line.
4. Under-row placement unconditional, not gated on count.
5. ParseWarnings NOT forced into AttentionItem.
6. Nesting change is PR #532's named-deferral successor.
7. The §4 pivot from dougFacing-template-reuse to a pure composer (autocorrectGuidance) is a settled R1 outcome — challenge its DETAILS, not the decision to compose.
Arguing these decisions themselves is out of scope.

## VERIFIED CODE FACTS (given; do not re-derive)
- canonicalCrewKey(name) => name.trim().toLowerCase().
- interpolate leaves an unresolved placeholder literal on screen.
- BulkIgnoreControls: chip renders iff `bulk` present; chip text is "Ignore all N"; type context is in the eyebrow label, not the chip; `group.cards` renders any node incl. empty.
- extractRoleFlags: ONE call site (crew.ts:364); its .warnings consumed only via the stampedRoleWarnings map.
- CREW_CAP = 30.
- 13 emit sites: STAGE_WORD x1, ROLE_TOKEN x1, SECTION_HEADER x1, COLUMN_HEADER x2 (crew, transport), FIELD_LABEL x8 (client x2, event x2, ops, rooms, transport, venue).
- dougFacing line map: STAGE_WORD 1270, ROLE_TOKEN 1285, COLUMN_HEADER 1299, SECTION_HEADER 1412, FIELD_LABEL 1426. The 3 non-crew templates carry a _<sheet-name>_ slot; the 2 crew ones carry _<crew-name>_.
- PR #532 cut the published Parse warnings panel to info-severity rows only; a warn-severity crew-scoped autocorrect warning now renders in exactly ONE place.

## WHERE I WANT PRESSURE
A. §4 composer (autocorrectGuidance) — the pivot is new. Is the composition contract COMPLETE and unambiguous? Guard table total over all inputs? Any state producing a blank or malformed card? Is the plain-text render (no renderEmphasis) fully specified at the call site?
B. §5-6 placement + bulk-ignore — the active/ignored/fallback partition and the "cards=fallback subset, chip counts all N" rule. Can a warning still render twice, zero times, or get double-counted? Is the group-emission rule (emit iff fallbackCards≥1 OR N≥2) complete across ignore/resolve mutations?
C. §9 transitions + §10 tests — are the three disclosure states and compound cases complete? Does any listed test only prove "a function was called"? Is any defect class uncovered?
D. Internal contradictions between sections; any numeric/label disagreement; any unspecified guard.
...
If a section is sound, say so briefly. End with exactly one line:
