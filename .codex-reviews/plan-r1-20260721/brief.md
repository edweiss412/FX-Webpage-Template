# Adversarial PLAN review R1 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY. Findings only; no patches. Do NOT use tools. Reason ONLY from the text below.

## What this is
An implementation PLAN (12 TDD tasks) over an already-APPROVED spec (the spec passed Codex review R5). Review the PLAN's executability, not the spec's design. The spec is settled; do not relitigate its decisions.

## Judge the plan on:
1. **TDD soundness** — does each task have a failing test BEFORE impl? Is the test's failure mode concrete, or does it only prove "a function ran"?
2. **Anti-tautology** — any test that could pass against wrong code? Any oracle derived from the artifact under change (should be from fixture input)?
3. **Sequencing/dependencies** — is any task blocked by a later one? Are the universal (copy) vs published-only (placement) scopes honored?
4. **Coverage** — does the task set cover every one of the spec's 21 tests (1,2,3,3b,4,4b-4f,5,5b-5e,6,7,8,9,10,11,11b) and the four changes? Any spec requirement with no task?
5. **Snippet/typing realism** — any impl step that won't typecheck under strict tsconfig (noUncheckedIndexedAccess, exactOptionalPropertyTypes)? The `autocorrect` field is exactOptional.
6. **Meta-test / registry fan-out** — is the CREATED meta-test (tests/parser/_metaAutocorrectProducers) correctly scoped? Any registry the plan should touch but doesn't?

## KEY SPEC FACTS (given; the plan must honor these)
- 13 parser emit sites; ROLE_TOKEN subject stamped at crew.ts:367-373 (not at its emit site, which is pure).
- autocorrectGuidance is a pure composer; instance line renders as PLAIN TEXT (no renderEmphasis) for injection safety.
- Instance COPY is universal (staged too); PLACEMENT + NESTING published-only.
- Merged under-row stack = deterministic re-derivation (alerts then warnings, cap 2, native <details> for overflow).
- Duplicate same-name: all cards under first row. Blank subject → group fallback.
- Bulk-ignore crew-scoped: group.cards = fallback subset, bulk.items counts all N; no new component mode.
- §8 has FIVE dimensional rows; real-browser Playwright at 375+1280px, hydrated harness.

## OUTPUT
[SEVERITY: BLOCKING|HIGH|MEDIUM|LOW] <title> / Location (task) / Claim / Reality / Impact
If sound, say so and APPROVE. End with exactly: VERDICT: APPROVE | VERDICT: NEEDS-ATTENTION | VERDICT: BLOCKING
