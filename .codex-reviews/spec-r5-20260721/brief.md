# Adversarial spec review R5 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY. Findings only. Do NOT use tools. Reason ONLY from the text below.

## Posture
This is R5 of a converging train: R1 15 findings (BLOCKING), R2 9 (BLOCKING), R3 6 (BLOCKING), R4 4 (NEEDS-ATTENTION). §12 summarizes each round. The spec is close. Your job: decide if it is now implementable without a blocking ambiguity. If yes, return APPROVE — do NOT invent findings to justify a round; a clean spec earns a clean verdict. If a genuine BLOCKING/HIGH remains, name it precisely. LOW/MEDIUM polish that does not block a plan author should be noted as such, not used to withhold approval.

## DO NOT RELITIGATE (settled; §1.1 + round history)
1-6 as in §1.1. 7. Pure composer autocorrectGuidance. 8. Duplicate same-name: all cards under first row. 9. Instance COPY universal (staged too); PLACEMENT+NESTING published-only. 10. Merged stack = deterministic pure re-derivation: alerts (attention order) then warnings (model order), cap 2 visible; mutations re-run this, no incremental append rule.
...
- dougFacing: STAGE1270 ROLE1285 COLUMN1299 SECTION1412 FIELD1426.
- PR#532: published Parse warnings panel = info rows only.
- SectionId: venue,event,crew,contacts,schedule,agenda,hotels,transport,rooms,diagrams,packlist,billing,warnings,report.

## FOCUS (secondary — do not treat as a primary hunt list)
Confirm the R4 repairs are internally consistent: §9 added-item bullet vs §5.3; §4.3 normalize-before-compare order; test 3 independent oracle; §2.2 citations. Then a fresh full-spec pass for any remaining blocking ambiguity only.
