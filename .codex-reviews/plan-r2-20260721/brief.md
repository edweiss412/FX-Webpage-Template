# Adversarial PLAN review R2 (INLINED, no tools) — warning-card identity + placement

## Your role: REVIEWER ONLY. Findings only. Do NOT use tools. Reason ONLY from the text below.

## Posture
Implementation PLAN over an APPROVED spec. R1 had 10 findings (BLOCKING), all repaired; the plan is now on its second pass. Confirm executability or name what genuinely still blocks a TDD implementer. If sound, APPROVE — do not manufacture findings. The spec design is settled; review the PLAN only.

## Judge on: TDD soundness (failing test first, concrete failure mode); anti-tautology (no oracle from the artifact under change); sequencing (no forward deps — R1 fixed two); coverage of all 21 spec tests + 4 changes; strict-TS realism (noUncheckedIndexedAccess, exactOptionalPropertyTypes); meta-test/registry fan-out.

## R1 RESOLUTIONS (settled — challenge only if a repair is wrong)
- CREW_SCOPED_WARNING_CODES created in Task 2 (lib/parser/autocorrectCodes); Task 4 imports it.
- Task 4 map holds ALL active non-blank crew-scoped warnings; cap/fallback is Task 5's render concern.
- Task 5 owns the full placement partition + group-fallback filtering (conservation self-contained); Task 7 = bulk-chip count only.
- Task 6 asserts DOM ancestry (descendant of section body).
- Task 2 covers 13 producer SITES individually with runtime values.
- Task 8 explicit GuidanceResult union.
- Task 9 source-scan for AnimatePresence + behavioral.
- Task 10 enumerates 5 dimensional invariants.
- Task 3 hand-authored oracle table; Task 11 independent oracle.

## KEY SPEC FACTS (given)
13 emit sites; ROLE subject stamped at crew.ts:367-373; instance line renders plain-text (no renderEmphasis); copy universal, placement/nesting published-only; deterministic stack (alerts then warnings, cap 2, native <details>); duplicate same-name → all under first row; blank subject → group fallback; bulk crew-scoped: cards=fallback, bulk.items counts all N; §8 five dimensional rows @ 375+1280px hydrated.
