...
export const SPEC_CODES = {
  ...
  "AGENDA_BLOCK_UNRESOLVED": {
    ...
    "helpfulContext": "One run-of-show day couldn't be matched to a calendar date, so that day shows the standard schedule. Usually the AGENDA date banner is missing or shows an error (like #REF!). Other days are fine.",
  },
  "AGENDA_DAY_AMBIGUOUS": {
    ...
    "helpfulContext": "A run-of-show day only listed a weekday (like 'Wednesday') that matches two of the show's days, so we didn't guess and that day shows the standard schedule. Add the actual date to the AGENDA banner to fix it.",
  },
  "AGENDA_DAY_EMPTIED": {
    "crewFacing": null,
    ...
    "helpfulContext": "A run-of-show day you'd published before is now blank in the sheet, so that day went back to the standard schedule (we don't keep old content once it's removed). If you cleared it on purpose you're done; if not, put the rows back and it returns on the next sync.",
  },
  ...
  "BRANCH_PROTECTION_DRIFT": {
    "crewFacing": null,
    "dougFacing": "Branch protection on _<repo>_ no longer matches the X.6 contract. Restore the required checks and review settings before merging.",
  ...
  "BRANCH_PROTECTION_MONITOR_AUTH_FAILED": {
    "crewFacing": null,
    "dougFacing": "Branch-protection monitoring for _<repo>_ cannot authenticate with GitHub. Rotate the GH App token or PAT within 24 hours.",
  ...
  "EMAIL_DELIVERY_FAILED": {
    "crewFacing": null,
    "dougFacing": "A notification email for _<show-name>_ couldn't be sent. We'll keep retrying automatically; if it persists, the developer will check the email provider setup.",
  ...
  "PENDING_SNAPSHOT_PROMOTE_STUCK": {
    "crewFacing": null,
    "dougFacing": "A diagram snapshot promotion for _<show-name>_ has been stuck for more than 15 minutes. Eric needs to run the snapshot-promote repair tool before cleanup can finish.",
  ...
  "PENDING_SNAPSHOT_ROLLBACK_STUCK": {
    "crewFacing": null,
    "dougFacing": "A diagram snapshot rollback for _<sheet-name>_ stalled after moving some assets. Eric needs to run the snapshot-rollback repair tool before cleanup can finish.",
  ...
  "REPORT_DUPLICATE_LIVE_MATCHES": {
    "crewFacing": null,
    "dougFacing": "Multiple live GitHub issues match one report for _<show-name>_. Recovery is paused until Eric reviews the duplicates.",
  ...
  "REPORT_LEASE_THRASHING": {
    "crewFacing": null,
    "dougFacing": "Bug-report processing is thrashing on _<show-name>_ — retries are racing against leases. This usually means the lease window needs tuning.",
  ...
  "REPORT_LOOKUP_INCONCLUSIVE": {
    ...
    "dougFacing": "We couldn't confirm whether a report for _<show-name>_ went through. Try again in a few minutes.",
  ...
  "REPORT_OPEN_ORPHAN_LABEL": {
    "crewFacing": null,
    "dougFacing": "An open GitHub issue for _<show-name>_ carries the orphan-cleanup label. Eric needs to re-close it or remove the label.",
  ...
  "REPORT_ORPHANED_LOST_LEASE": {
    "crewFacing": null,
    "dougFacing": "A duplicate bug-report issue for _<show-name>_ was auto-closed during a retry race. Click through to verify it closed correctly. If this recurs, increase the lease window.",
  ...
  "STALE_ORPHAN_REPORT": {
    "crewFacing": null,
    "dougFacing": "A stale bug-report reservation for _<show-name>_ expired before it could create a GitHub issue. No action needed unless it repeats.",
  ...
  "UNKNOWN_FIELD": {
    ...
    "followUp": "Doug → optional Report",
    "helpfulContext": "We found a row that doesn't match any section we read (CLIENT, DATES, CREW, and so on). It isn't breaking anything — we kept the row exactly as it is. If it's something you want handled, use Report; otherwise you can ignore this.",
  },
  "UNKNOWN_ROLE_TOKEN": {
    "crewFacing": null,
  ...
  "WIZARD_SESSION_SUPERSEDED_RACE": {
    "crewFacing": null,
    "dougFacing": "A leftover wizard action (_<attempted-action>_) for _<file-name>_ was safely cancelled before it could change the new wizard's state. Continue in the active wizard tab.",
