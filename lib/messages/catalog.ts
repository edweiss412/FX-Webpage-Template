...
export const MESSAGE_CATALOG = {
  ...
  LEAKED_LINK_DETECTED: {
    ...
    title: "Signed link possibly leaked",
    longExplanation: "A signed crew link was opened with its token in the URL query string instead of the URL fragment, so it may have been written to a request log or referrer header. The affected link has been auto-revoked. Issue a new link for that crew member when you're ready.",
    helpHref: "/help/errors#LEAKED_LINK_DETECTED",
  },
  CSRF_DENIED: {
    code: "CSRF_DENIED",
    dougFacing: null,
  ...
  LINK_SESSION_KEY_ROTATED: {
    ...
    longExplanation: "While you were finishing sign-in, the secret key the app uses to authenticate link sessions was rotated. Your session was bound to the old key, so it was retired. Open your original signed link again to mint a fresh session.",
    helpHref: "/help/errors#LINK_SESSION_KEY_ROTATED",
  },
  ...
  CSRF_KEY_ROTATED: {
    ...
    title: "CSRF key rotated",
    longExplanation: "While you were using the app, the secret key used to verify CSRF tokens was rotated. The token your browser was holding was minted under the old key, so it was rejected. Refresh the page and click your link again to mint a fresh token.",
    helpHref: "/help/errors#CSRF_KEY_ROTATED",
  },
  BOOTSTRAP_GENERIC: {
  ...
  LEAKED_LINK_REVOCATION_FAILED: {
    ...
    title: "Leaked link couldn't be revoked",
    longExplanation: "We detected a possible link leak but the auto-revocation step failed, so the leaked link may still work until an operator clears the token version manually. Eric has been notified.",
    helpHref: "/help/errors#LEAKED_LINK_REVOCATION_FAILED",
  },
