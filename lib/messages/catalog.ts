...
export const MESSAGE_CATALOG = {
  ...
  AMBIGUOUS_EMAIL_BINDING: {
    ...
    longExplanation: "Two rows in the CREW block share the same email address, so we can't safely tell who is logging in. Usually one of the two emails is a typo or paste mistake. Fix the duplicate in the sheet and this alert will clear on the next sync.",
  ...
  SHEET_UNAVAILABLE: {
    ...
    longExplanation: "We expected to find this sheet in the watched folder but it isn't there anymore. Either it was moved, unshared, or deleted. Crew see the last good version on file; re-share or move the sheet back in to bring the show back.",
  ...
  WIZARD_REVIEWER_CHOICES_VERSION_UNSUPPORTED: {
    ...
    longExplanation: "We updated the format used to record your review choices between when you applied this sheet and when finalize ran. Rather than silently replay old-format choices, the wizard holds the sheet for re-review. Re-apply the affected sheet, then click Finalize again.",
  ...
  STAGED_PARSE_REVISION_RACE_DURING_FINALIZE: {
    ...
    longExplanation: "The sheet was edited again in Drive between when you clicked Apply and when finalize tried to commit. The other sheets in the batch were committed; only the raced sheet needs your attention. Re-review the new edit, click Apply, then click Finalize.",
  ...
  IDEMPOTENCY_IN_FLIGHT: {
    ...
    longExplanation: "Your previous report submission is still being processed. Clicking again will not create a duplicate but also will not speed things up. If the original doesn't go through within a minute, try once more.",
  ...
  WATCH_CHANNEL_ORPHANED: {
    ...
    longExplanation: "We tried to register a real-time push subscription with Google Drive and didn't get a confirmation back. The cron job keeps shows in sync on its normal schedule; edits just won't appear instantly until the subscription is reconciled.",
  ...
  WEBHOOK_TOKEN_INVALID: {
    ...
    longExplanation: "A push notification from Google Drive arrived carrying the wrong verification token. Usually this means a stale subscription is still firing or someone is probing the endpoint. The developer has been notified.",
  ...
  STAGED_PARSE_OUTDATED: {
    ...
    longExplanation: "The sheet was edited again after the staged version you were reviewing was captured. The staged version is no longer the most recent state, so we discarded it. A fresh staged parse will be ready in a few minutes.",
  ...
  STAGED_PARSE_SOURCE_GONE: {
    ...
    longExplanation: "Between staging and Apply, the source sheet was deleted, trashed, or unshared in Drive. Without a sheet to read, we can't apply the staged parse. Restore the sheet or re-share it and the next sync will produce a new staged parse.",
  ...
  STAGED_PARSE_SOURCE_OUT_OF_SCOPE: {
    ...
    longExplanation: "Between staging and Apply, the sheet was moved out of the watched folder. Anything outside the watched folder is invisible to the sync pipeline. Move the sheet back in and the next sync will produce a new staged parse.",
  ...
  REEL_DRIFTED: {
    ...
    longExplanation: "The opening-reel video was replaced or edited in Drive after the staged parse was reviewed. Crew see the text status only without the inline video until you save the sheet again to re-stage the new reel.",
  ...
  OPENING_REEL_PERMISSION_DENIED: {
    ...
    longExplanation: "Drive returned permission-denied when we tried to fetch the opening-reel video. The file used to be accessible but the share was revoked, the file was made private, or it was moved out of a shared drive. Re-share the video, or replace the link with one you do share.",
  ...
  ASSET_RECOVERY_REVISION_DRIFT: {
    ...
    longExplanation: "Asset recovery fetched and verified diagram bytes against an older snapshot revision, but a newer Apply landed before recovery could write those bytes. The recovery run aborts so old assets are not attached to the new approved revision; the next run retries against the latest version.",
  ...
  ASSET_RECOVERY_DRIFT_COOLDOWN: {
    ...
    longExplanation: "The previous asset recovery attempt raced with a newer Apply, so recovery is briefly backing off for this snapshot revision. This bounds retry storms while the show is changing frequently; we'll retry automatically after the cooldown.",
  ...
  DIAGRAMS_EMBEDDED_REVISIONS_UNAVAILABLE: {
    ...
    longExplanation: "Google Drive did not return a usable revision token for this sheet, so the app could not snapshot the embedded diagrams. The previous version is still live for crew. The developer has been notified. This is rare and usually clears on the next edit to the sheet.",
  ...
  STAGED_PARSE_RESTAGED_INLINE: {
    ...
    longExplanation: "The sheet changed after the original scan, so the wizard re-parsed it inside your current setup session. Any decisions you made on the prior version were dropped. Review the refreshed parse from the top.",
  ...
  "MI-1_VERSION_DETECTION_FAILED": {
    ...
    longExplanation: "We look for specific row markers in your show template to recognize it as a real show sheet (Contact Office row and MAIN/SECONDARY block for v4; Hotel Contact Info row for v2). None of those markers were found. If your template has changed intentionally, tell the developer.",
  ...
  "MI-3_NO_PARSEABLE_DATE": {
    ...
    longExplanation: "We look for show dates in the DATES block (Travel In, Set Day, Show Day 1) and couldn't find anything we could read as a calendar date. Use a familiar format like '6/24' or 'June 24' in the right cells.",
  ...
  "MI-5b_DUPLICATE_CREW_EMAIL": {
    ...
    title: "Two crew rows share an email",
    longExplanation: "Two rows in the CREW block share the same email address. Email is how we identify a crew member across shows, so duplicates would let one person see another's view. Give each crew row a distinct email.",
  ...
  NO_FOLDER_CONFIGURED: {
    ...
    longExplanation: "Cron ran before the setup wizard saved a watched Drive folder. That's expected during first setup; the dashboard shows the setup call to action instead of treating it as a show error. Run the setup wizard to choose a folder.",
  ...
  "MI-7_SECTION_SHRINKAGE": {
    ...
    longExplanation: "More than half of the rows in a named section disappeared since the last approved sync. Section collapses are usually accidental, often a half-finished paste. Confirm before applying.",
  ...
  "MI-8_FINANCIAL_FIELD_COLLAPSE": {
    ...
    longExplanation: "A financial field (PO#, Proposal $, Invoice, Invoice Notes, COI) that was previously filled in is now blank. We hold the change for review because financial blanks are usually accidental.",
  ...
  "MI-9_ROLE_FLAGS_DELTA": {
    ...
    longExplanation: "A crew member's LEAD status changed. LEAD grants admin / ops surface access including internal financials, so every LEAD toggle is held for review. Confirm the change is intentional before applying.",
  ...
  MI11_TARGET_MOVED: {
    ...
    longExplanation: "Before applying a queued email change we re-check the live sheet. If the sheet was edited after this change was queued, the change you're approving may no longer match what the sheet says — so we stop rather than apply a stale value. Re-open the show to review the latest version.",
  ...
  MI11_DRIVE_RECHECK_FAILED: {
    ...
    longExplanation: "Before applying a queued email change we ask Google Drive for the sheet's latest revision time. That check failed — usually a transient network or permissions hiccup. Nothing was changed. Try Approve again in a moment; if it keeps failing, check that the folder is still shared with the service account.",
  ...
  MI11_HOLD_ALREADY_RESOLVED: {
    ...
    longExplanation: "This pending email change was already resolved — either a later sync brought the sheet back in line on its own, or you (or another open tab) already approved or rejected it. There's nothing left to do here; refresh the show to see the current state.",
  ...
  IDENTITY_WOULD_COLLIDE: {
    ...
    longExplanation: "Applying this email change would give a crew member an email or name that another crew member already has, and that other row isn't part of the same swap — so we can't apply it without creating a duplicate. Two crew rows can't share an email. Fix the clash in the sheet (one of them is usually a typo or a stale row), then re-sync.",
  ...
  UNDO_SUPERSEDED: {
    ...
    longExplanation: "Undo only reverses the most recent change to a crew member. A newer sync has already changed this person again since the change you're trying to undo, so the saved 'before' value no longer matches what's live. Refresh to see the current state; if you still want the old value, edit the sheet directly.",
  ...
  UNDO_EMAIL_CLAIMED: {
    ...
    longExplanation: "Undoing this would restore an email address that now belongs to a different crew member, and two people can't share an email. Rather than undo, fix the email in the sheet — the next sync will reconcile it safely.",
  ...
  UNDO_NOT_FOUND: {
    ...
    longExplanation: "We couldn't find the change you tried to undo. It may have already been undone, or it's a notification-only change (like a section shrinking) that doesn't carry a saved 'before' value to restore. Refresh the feed and try again.",
  ...
  "MI-11_EMAIL_CHANGE": {
    ...
    longExplanation: "A crew member's email address changed. After applying, the new email will get sign-in access; their existing share-link will stop working until you issue a new one.",
  ...
  "MI-12_PROBABLE_RENAME": {
    ...
    longExplanation: "A row was removed and a new row added in the same sync, and they share the same email address. That usually means a name was edited (rename), not two unrelated changes. Confirm whether to treat as a rename.",
  ...
  "MI-13_NAME_AND_EMAIL_CHANGE": {
    ...
    longExplanation: "A row was removed and a new row added with both a different name AND a different email. We can't tell from the data whether this is the same person or two unrelated changes. Confirm before applying.",
  ...
  "MI-14_NO_EMAIL_RENAME": {
    ...
    longExplanation: "A row was removed and a new row added, both without emails. The names are similar enough that this might be a rename, but with no email to compare we can't be sure. Confirm whether to treat as a rename.",
  ...
  SHOW_UNPUBLISHED: {
    ...
    longExplanation: "You clicked Unpublish on a recently-published show. The show is archived, its share-token URL no longer resolves, and crew can no longer reach the page. Nothing is lost. Drag the sheet back into the watched folder when you're ready to publish again.",
  ...
  ONBOARDING_SCAN_REVIEW: {
    ...
    longExplanation: "The setup wizard's folder scan found this sheet. Review the parse before activating the folder, so you do not commit to data you have not seen. This is the wizard's version of first-seen review.",
  ...
  UNKNOWN_DAY_RESTRICTION: {
    ...
    longExplanation: "A crew member has the day-restriction flag but the sheet doesn't say which days. Add a parenthetical to their name like '(6/24 and 6/26 ONLY)'. Until you do, their schedule will show 'days unconfirmed.'",
  ...
  DAY_RESTRICTION_DOUBLE_LOCATION: {
    ...
    longExplanation: "Day-restriction parentheticals appear in both the crew member's name cell and role cell. We use the role-cell version because that's closer to the staffing assignment. Remove the duplicate so the schedule stays clear.",
  ...
  UNKNOWN_ROLE_TOKEN: {
    ...
    longExplanation: "A role cell contains a token outside the canonical role vocabulary. We ignore the unknown token so it does not grant accidental access. If the token is intentional, report it so the developer can add it to the role vocabulary.",
  ...
  PULL_SHEET_AMBIGUOUS_FORMAT: {
    ...
    longExplanation: "We found something that looks like a PULL SHEET block, but the columns don't match the expected format. Crew see the preserved raw text; structured packing data is degraded until the format is supported. Report it if you'd like the developer to handle it.",
  ...
  PULL_SHEET_UNKNOWN_VARIANT: {
    ...
    longExplanation: "A pull-sheet case had data rows, but none exposed a TRUE/FALSE packed flag in the supported variants. The parser defaults to Variant A so crew still see the list. Report it if quantities or packing columns look wrong.",
  ...
  DIAGRAMS_EMBEDDED_OBJECT_INACCESSIBLE: {
    ...
    longExplanation: "An image embedded in the DIAGRAMS tab returned an HTTP error when we tried to download it. Crew see a placeholder where it should be. Re-paste the image (Drive sometimes loses object permissions on the underlying blob), or report it if this keeps happening.",
  ...
  MISSING_REVIEWER_CHOICE: {
    ...
    longExplanation: "Every triggered review item needs a decision before Apply can run. Your submission was missing one. This usually means the form fell out of sync with what the server was tracking. Refresh the admin page so the panel re-renders against the current items, then submit your decisions again.",
  ...
  EXTRA_REVIEWER_CHOICE: {
    ...
    longExplanation: "Your Apply submission carried a decision for an item the server isn't tracking. This usually happens when the staged parse you were viewing was replaced between when the page loaded and when you clicked Apply. Refresh the admin page and re-submit your decisions.",
  ...
  DUPLICATE_REVIEWER_CHOICE: {
    ...
    longExplanation: "Your Apply submission carried two decisions for the same item. The form should normally prevent this; you've reached this code via a stale or duplicated form state. Refresh the admin page and re-submit cleanly.",
  ...
  INVALID_REVIEWER_ACTION: {
    ...
    longExplanation: "Each review item has a fixed list of valid decisions (apply / reject / rename / independent, depending on the item). Your submission carried an action that isn't in the allowed list. Refresh the admin page and re-submit using the form controls.",
  ...
  REPORT_RATE_LIMITED_ADMIN: {
    ...
    longExplanation: "To keep the developer's inbox under control, the admin report endpoint is capped at 10 reports per hour on a rolling window. Wait the indicated time, or message Eric directly if it's urgent.",
  ...
  ONBOARDING_FOLDER_INVALID_URL: {
    ...
    longExplanation: "The setup wizard expects a Google Drive folder URL like 'https://drive.google.com/drive/folders/<id>'. Either the URL you pasted isn't a folder URL, or it's malformed. Open the folder in Drive, copy the URL from the address bar, and paste it again.",
  ...
  ONBOARDING_FOLDER_NOT_SHARED: {
    ...
    longExplanation: "We tried to read your folder using the service account but Drive returned access-denied. Open the folder's share dialog, add the service-account email shown in the wizard, then click 'Try again'.",
  ...
  ONBOARDING_NOT_RESOLVED: {
    ...
    longExplanation: "Some sheets in your folder still need review before setup can finish. Open each unresolved sheet in the wizard, decide what to do with it (approve, defer, ignore), then click Finalize again.",
  ...
  WIZARD_FINALIZE_CHECKPOINT_MISSING: {
    ...
    longExplanation: "Setup's final-publish step was invoked before all sheets in the candidate folder finished publishing. Click 'Promote next batch' in the wizard until the progress indicator says 'All sheets published'; the final-publish step will fire automatically at that point.",
  ...
  WIZARD_FINALIZE_UNRESOLVED_ROWS: {
    ...
    longExplanation: "Setup cannot publish while the scan manifest still has unresolved rows. Resolve each staged, hard-failed, discard-retryable, or live-row-conflict item highlighted on the wizard screen, then click Publish again.",
  ...
  NETWORK_UNREACHABLE: {
    ...
    longExplanation: "The browser couldn't reach the server. Typically this means the device is offline, DNS is failing, a captive portal is blocking the request, or a browser extension is intercepting the call. Check connectivity and retry; the request never arrived, so there's no admin trail either.",
  ...
  PENDING_SNAPSHOT_PROMOTE_STUCK: {
    ...
    longExplanation: "A diagram snapshot promotion has been in the non-reclaimable promote-started state for more than 15 minutes. Eric needs to reconcile the temp and canonical prefixes before cleanup can continue.",
  ...
  PENDING_SNAPSHOT_ROLLBACK_STUCK: {
    ...
    longExplanation: "A diagram snapshot rollback failed midway, leaving assets split across temp and canonical prefixes. Eric needs to reconcile both prefixes and finish the rollback before cleanup can continue.",
  ...
  BRANCH_PROTECTION_MONITOR_AUTH_FAILED: {
    ...
    longExplanation: "The privileged branch-protection monitor could not authenticate to GitHub, so it cannot prove the merge gate is still enforcing the required checks. Rotate the GitHub App token or fallback PAT and confirm the scheduled job succeeds again.",
  ...
  PENDING_INGESTION_TRANSITIONED: {
    ...
    longExplanation: "While you were clicking, another browser tab acted on the same sheet and finished a fraction of a second before yours. The server stopped before doing anything to prevent your click from writing on top of the other resolution. Refresh the dashboard to see the latest state.",
  ...
  ADMIN_ALERT_NOT_FOUND: {
    ...
    longExplanation: "When you clicked Mark resolved, the server looked up that alert by id and either didn't find it (already resolved and cleaned up) or it belongs to a different show than the page you clicked from. Refresh the dashboard to see the current state.",
  ...
  ALERT_REQUIRES_SHOW_SCOPED_RESOLVE: {
    ...
    longExplanation: "Per-show alerts are resolved from the show's parse panel, not the global dashboard banner. We require the click-through so the resolution is recorded in the show's audit trail. The dashboard's redirect link will take you straight to the show's alert section.",
  ...
  STALE_DISCARD_REJECTED: {
    ...
    longExplanation: "A newer parse was staged for this sheet between when you opened the review and when you clicked Discard. Refresh the admin page to see the latest version before deciding.",
  ...
  GITHUB_BOT_LOGIN_MISSING: {
    ...
    longExplanation: "The bug-report recovery path needs to know the GitHub username of the bot account so it can find issues created by previous attempts. The GITHUB_BOT_LOGIN environment variable isn't set; configure it on the deployment and redeploy.",
  ...
  ADMIN_EMAIL_INVALID: {
    ...
    longExplanation: "The email you submitted didn't pass the standard email-format check. Re-enter it as a valid address.",
  ...
  ADMIN_EMAIL_RE_ADD_PROMPT: {
    ...
    longExplanation: "The email you submitted matches a previously revoked administrator. Confirm whether you'd like to re-add and restore admin access for that email.",
  ...
  ADMIN_FORBIDDEN: {
    ...
    longExplanation: "Your current admin session can't access this action. Sign in again and retry.",
  ...
  AGENDA_ASSET_LOOKUP_FAILED: {
    ...
    longExplanation: "We couldn't resolve or stream the linked agenda PDF for this show. Refresh and try again; if it keeps failing, the developer needs to investigate.",
  ...
  CLEANUP_REQUIRES_STALE_SESSION: {
    ...
    longExplanation: "Abandoned-finalize cleanup is guarded by a stale-session check and a finalize-recency check so it can't interrupt an active setup publish. This session isn't stale enough yet.",
  ...
  CONCURRENT_FINALIZE_IN_FLIGHT: {
    ...
    longExplanation: "Setup publishing is already running in another tab. Only one finalize worker can hold the wizard finalize lock for a session; wait for the other tab to finish.",
  ...
  DIAGRAM_ASSET_LOOKUP_FAILED: {
    ...
    longExplanation: "We couldn't resolve or stream the stored diagram for this show. Refresh and try again; if it keeps failing, the developer needs to investigate.",
  ...
  DRIVE_METADATA_MISSING: {
    ...
    longExplanation: "Google Drive didn't return the sheet revision metadata we need to sync safely. The sync engine requires a head revision id so markdown export, enrichment, and final apply all describe the same sheet revision. We'll retry on the next run.",
  ...
  EMBEDDED_ASSET_DRIFTED: {
    ...
    longExplanation: "Apply re-checks each embedded image's fingerprint against what was staged. This image's fingerprint changed after the staged parse was reviewed, so the prior approved content stays live. Save the sheet again to re-stage the new image.",
  ...
  FOLDER_NOT_FOUND: {
    ...
    longExplanation: "Google Drive returned missing, deleted, or trashed for the folder ID in the link. Confirm the folder still exists, that the URL points to the folder itself, and that it hasn't been moved to trash.",
  ...
  FOLDER_NOT_SHARED: {
    ...
    title: "Folder not shared with sync account",
    longExplanation: "The app reads show sheets through a Google service account. Share the Drive folder with that account using Viewer access, then click Verify again. If the folder is in a shared drive, make sure the service account can see that shared drive too.",
  ...
  INVALID_FOLDER_URL: {
    ...
    longExplanation: "The setup wizard needs the URL for a Google Drive folder, usually shaped like 'drive.google.com/drive/folders/<folder-id>'. Open the folder in Drive, copy the browser URL, and paste that full link.",
  ...
  LAST_ADMIN_LOCKOUT_REFUSED: {
    ...
    longExplanation: "Self-revoke of the only active administrator is refused to prevent admin lockout. Add another admin first, then revoke this one.",
  ...
  LINKED_FOLDER_OVERFLOW_TRUNCATED: {
    ...
    longExplanation: "The linked diagram folder has more images than one release can publish. Crew see the first 60 images; everything beyond that is truncated.",
  ...
  OPERATOR_ERROR_INCOMPLETE_FOLDER_METADATA: {
    ...
    longExplanation: "We reached Google Drive but the metadata response didn't include the fields needed to confirm the link is a readable folder. Usually transient; try again in a moment. If it repeats, the developer should inspect the Drive API response.",
  ...
  OPERATOR_ERROR_NOT_FOLDER: {
    ...
    longExplanation: "The setup wizard scans every sheet inside one folder. A direct Google Sheet link can't be used for setup because it doesn't tell the app which sibling sheets belong in the same onboarding run.",
  ...
  PENDING_SNAPSHOT_NOT_STUCK: {
    ...
    longExplanation: "The repair endpoint only accepts pending snapshot rows that started promotion and exceeded the repair threshold. This snapshot isn't in that state.",
  ...
  PENDING_SNAPSHOT_PROMOTE_IN_FLIGHT: {
    ...
    longExplanation: "Promotion repair is blocked until the promote-started threshold has elapsed. Check again in a few minutes.",
  ...
  REEL_ASSET_LOOKUP_FAILED: {
    ...
    longExplanation: "We couldn't resolve or stream the stored opening-reel revision for this show. Refresh and try again; if it keeps failing, the developer needs to investigate.",
  ...
  REPORT_DUPLICATE_LIVE_MATCHES: {
    ...
    longExplanation: "The bug-report recovery scan found more than one non-orphan GitHub issue with the same report marker. The system fails closed instead of choosing a winner; Eric needs to review the duplicates.",
  ...
  REPORT_HORIZON_EXPIRED: {
    ...
    longExplanation: "Bug-report retry recovery only runs within 24 hours of the original attempt. This attempt is older than that. If the issue still applies, file a fresh report.",
  ...
  REPORT_LOOKUP_INCONCLUSIVE: {
    ...
    longExplanation: "The bug-report recovery path couldn't conclusively list recent GitHub issues for this report, so it refused to create a duplicate issue. Try again in a few minutes.",
  ...
  REPORT_OPEN_ORPHAN_LABEL: {
    ...
    longExplanation: "Orphan cleanup should close issues with the 'not planned' state. Seeing the orphan label on an open issue means manual intervention happened or GitHub returned an unexpected state. Eric needs to review and either re-close the issue or remove the label.",
  ...
  REPORT_PIPELINE_FAILED: {
    ...
    longExplanation: "The report route caught a typed infrastructure failure from the report submission or reaper path and returned a cataloged 500 instead of crashing. Try again in a few minutes.",
  ...
  SHOW_BUSY_RETRY: {
    ...
    longExplanation: "Another sync is holding the per-show advisory lock for this show. Retry in a moment.",
  ...
  STAGED_PARSE_FAILED: {
    ...
    longExplanation: "The live first-seen retry path fetched the sheet but the parser couldn't convert it into a show payload. The previous approved version (if any) is still serving crew. Check the per-show parse panel for the specific underlying error.",
  ...
  STAGED_PARSE_OUTDATED_AT_PHASE_D: {
    ...
    longExplanation: "Finalize Phase D only promotes shadow changes when the live show hasn't advanced past the wizard-staged modified time. A live show changed after setup staged its publish changes, so this batch is held.",
  ...
  STALE_ORPHAN_REPORT: {
    ...
    longExplanation: "A bug-report reservation aged past the 24-hour recovery horizon with its processing lease expired and was deleted by the reaper. No user action is needed unless this repeats.",
  ...
  SYNC_FILE_FAILED: {
    ...
    longExplanation: "A per-file sync step failed and was isolated from the rest of the folder run. The other sheets continued normally.",
  ...
  SYNC_INFRA_ERROR: {
    ...
    longExplanation: "A database or Supabase boundary returned an unexpected error during the sync step. The rest of the folder kept running. The structured log payload preserves the original operation and error class so the developer can investigate.",
