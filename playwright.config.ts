  ...
  projects: [
    {
      ...
      testMatch:
        /(confirm-focus-probe|font-rendering-census|sample|crew-page|tap-target-inline-controls\.layout|crew-section-toggle|report-modal|canonical-class-dimensions|crew-layout-dimensions|admin-layout-dimensions|admin-nav-layout-dimensions|control-outline-dimensions\.layout|control-outline-contrast\.live|control-outline-pill\.route|theme-toggle|empty-state-reachability|admin-layout|admin-lifecycle-layout|admin-changes-feed-layout|admin-lifecycle-transitions|admin-parse-panel|sign-in-page|me-page|onboarding-wizard-step1|admin-phase2-surfaces|no-raw-codes|help-pages|notify-toggles|needs-attention-page|root-landing|forcedColors)\.spec\.ts/,
      use: {
        ...devices["iPhone 14"],
    ...
    },
    {
      ...
      testMatch:
        /(admin-upstream-retry|picker-flow|stage-restricted-crew-schedule|sample|font-binding|right-now-transitions|report-modal|crew-layout-dimensions|admin-layout-dimensions|admin-nav-layout-dimensions|source-link-dimensional|telemetry-layout|theme-toggle|theme-persistence-note|bell-panel-layout|admin-changes-feed-layout|admin-layout|admin-parse-panel|admin-route-boundaries|staged-preview|admin-settings-admins-refresh|roles-settings-layout|developer-tier|sign-in-page|me-page|notify-toggles|needs-attention-page|root-landing|published-review-modal\.interactions|published-review-modal\.deeplink|published-review-modal\.layout|published-review-modal\.prefetch|published-review-modal\.reopen|published-review-modal\.closeFreshness|published-review-modal\.realtime|published-review-modal\.crew-actions|step3-review-modal\.interactions|step3-review-modal\.agenda|wizard-attention-menu|warning-panel-polish|dev-capture|published-show-attention|alert-action-links|nojs-loading-notice|font-rendering-census|needs-attention-holds|rowactions-geometry|forcedColors)\.spec\.ts/,
      use: {
    ...
    },
    {
      ...
      // silently proves nothing. attention-modal-gallery needs the built
      ...
      testMatch: /(admin-dev|attention-modal-gallery)\.spec\.ts/,
      use: {
        ...devices["Desktop Chrome"],
        viewport: { width: 1280, height: 800 },
