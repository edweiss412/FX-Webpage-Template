...
**The centred section-header pattern (review panel, 2026-07-25).** When a header row must carry a name plus two or three affordances, do NOT let them compete on one line — the NAME is what yields, and it yields silently. The review panel's section header measured 3 lines at 375px and 5 lines / 124px of row height at 320px on a flagged section before this was fixed. The shape that holds:
...
Verified in a real browser across the 15 reachable (geometry-class x status) cells at 320/375/430/1280 (`tests/e2e/section-header-layout.layout.spec.ts`), asserting each cell's rendered identity BEFORE its geometry so copies of one fixture cannot pass as distinct cells. Spec: `docs/superpowers/specs/2026-07-24-dq-eyebrow-divider-and-confirm-bar-design.md`.
