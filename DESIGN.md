...
| `--color-accent-tint`               | `#FEEEDE`                          | `#2A1E10`                       | Warm low-chroma wash behind the bell panel's **info** severity icon-circle and the **active-count pill**. The info icon on it uses `--color-accent-on-bg` (graphical, 4.91:1; also clears the 4.5:1 text floor); the pill NUMBER stays `--color-text-strong` for hierarchy, not necessity. A quiet tint, never a CTA fill; the ≤10% accent-coverage cap is unaffected (a few-px circle + pill).                                              |
...
| `--color-accent-on-bg` icon on `--color-accent-tint`  | 4.91:1 | 8.03:1 | ≥3:1 graphical (icon) — bell info circle (clears 4.5:1 text too) |
