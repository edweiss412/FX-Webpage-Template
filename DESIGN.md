...
| `--color-focus-ring`                | `rgba(255, 140, 26, 0.55)`         | `rgba(255, 160, 71, 0.65)`      | Focus outline color for keyboard-visible focus. Always orange-derived. 2px ring (`focus-visible:ring-2`, no `ring-3` anywhere). Where a 2px offset is used it MUST carry a container-matched color (`ring-offset-<backdrop>`, never bare `ring-offset-2` — the un-themed white gap is a dark-mode defect). Popover surfaces follow the two-tier recipe: offset only on armed destructive confirm-go buttons (spec `2026-07-23-sharehub-focus-pass` §2, §15). |
...
**The centred section-header pattern (review panel, 2026-07-25).** When a header row must carry a name plus two or three affordances, do NOT let them compete on one line — the NAME is what yields, and it yields silently. The review panel's section header measured 2 lines at 375px and 5 lines / 124px of row height at 320px on a flagged section before this was fixed (the full per-viewport table is in the spec's §1). The shape that holds:

- **A column, not a row.** Line one is `[status icon | centred name+count | corner link]`; line two carries the status pill when there is one. The pill line emits NO wrapper when there is no pill — an always-rendered empty wrapper would charge the column's `gap` and recreate the §7a seam.
- **`justify-center` on the middle group replaces a pusher.** No `flex-1` spacer.
- **A linkless section pads by the link's footprint** (`pr-header-link-slot`, §10) so its name lands on the same axis as a section that has one: measured +4px either way, versus +19px without.
- **`w-full` + `items-stretch` are load-bearing**, because the column and its parent section are both `flex-col` and Tailwind v4 does not default `.flex` to `align-items: stretch`. Without them the lines shrink-wrap and every centring measurement is against the wrong box.
- **Icon-only affordances beat text ones in a crowded header.** Replacing a text "In sheet" link with the external-link glyph returned ~60px. Keep the 44px hit area with a `before:-inset-3` overlay on a 20px glyph rather than a 44px visual box — measured 8-29px better on centring — and keep the `aria-label`, so the visible words cost assistive tech nothing.

Verified in a real browser across the 15 reachable (geometry-class x status) cells at 320/375/430/1280 (`tests/e2e/section-header-layout.layout.spec.ts`), asserting each cell's rendered identity BEFORE its geometry so copies of one fixture cannot pass as distinct cells. Spec: `docs/superpowers/specs/2026-07-25-section-header-rebuild-and-phantom-spacers.md`.
...
2. **Two-tap confirm (morph or panel) + 3–4s auto-revert** — irreversible or work-destroying ops: permanent ignore, stop-showing-sheet, bulk ignore/undo, archive, token rotate, picker resets, admin revoke, alert dismiss, shrink acceptance. _(Re-scan was withdrawn from this tier — it is content-aware and preserves ratified decisions on a clean refresh; see `docs/superpowers/specs/2026-07-16-withdraw-rescan-guard.md`.)_
   ...
   **Row-idiom carve-out (owner-ratified 2026-07-20).** A tier-2 control rendered as a titled ROW inside a popover — where the consequence is prose the operator is meant to READ rather than a button label — dismisses via an explicit `Cancel` instead of the auto-revert. A timer shorter than the reading punishes only the operator who reads. This is the shape `RotateShareTokenButton` has always had; `ArchiveShowButton`'s row variant now matches it, and the two sit adjacent in the ShareHub popover. The rest of the tier is unchanged: two-tap, the recipe on the confirm-go, the focus rules above (which now APPLY to the archive row — a separate safe control means the morphing-button exemption no longer covers it), and a registry row. Morph variants outside a popover keep the 3–4s timer. See `docs/superpowers/specs/admin/2026-07-16-destructive-confirm-pass.md`, amendment of the same date.
