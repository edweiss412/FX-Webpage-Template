...
**The section freshness cue** draws the same brief `accent-tint` wash, holding to 45% then settling back to the card's resting `surface`, plus a 2px `accent-edge` OUTLINE rather than a ring: the card already owns a border that switches tone when the section is flagged, and its own tile shadow, so an outline is the only layout-neutral marker that composes with both. It draws outside the border box, which the content pane's `gap-6` leaves room for. Measured contrast, both themes: `text` on `accent-tint` 15.16:1 light / 13.03:1 dark and `text-subtle` on `accent-tint` 5.95:1 / 5.77:1 (AA 4.5:1); outline against `accent-tint` 7.41:1 / 8.03:1 and against `surface` 8.42:1 / 8.84:1 (non-text 3:1). Pinned in `tests/styles/status-token-contrast.test.ts`. This is the first surface to put ordinary body text on `accent-tint` rather than `text-strong` alone, which is why the two text rows are measured here and not inherited.
