...
export function EventFilters({ filters }: { filters: AppEventFilters }) {
  ...
  return (
    <div
      data-testid="event-filter-toolbar"
      className="flex flex-col gap-2.5 rounded-md border border-border bg-surface p-3 text-sm shadow-tile"
    >
        ...
        <div className="inline-flex items-stretch overflow-hidden rounded border border-border">
          {LEVELS.map((lvl, i) => (
            <button
              ...
              className={`inline-flex min-h-tap-min items-center px-3 ${i > 0 ? "border-l border-border" : ""} ${levels.has(lvl) ? "bg-text text-bg" : "text-text"}`}
              onClick={() => {
