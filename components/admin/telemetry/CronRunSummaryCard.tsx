...
export function CronRunSummaryCard({ event }: { event: AppEventRow }) {
  ...
  const outcome =
    typeof ctx.outcome === "string" && VALID_OUTCOMES.has(ctx.outcome) ? ctx.outcome : "unknown";
  const durationMs =
    typeof ctx.durationMs === "number" && Number.isFinite(ctx.durationMs) ? ctx.durationMs : null;
  const counts = isPlainObject(ctx.counts)
    ? Object.entries(ctx.counts).filter(([, n]) => typeof n === "number")
    : [];
  return (
    <div className="rounded-md border border-text-faint bg-surface-sunken p-tile-pad">
      <div className="flex flex-wrap items-center gap-2 text-sm">
        <span className="font-medium text-text">{jobLabel(event)}</span>
        <span className="rounded-pill bg-surface px-2 py-0.5 text-xs text-text-subtle">
          {outcome}
        </span>
        {durationMs != null && (
          <span
            data-testid="cron-summary-duration"
      ...
      {counts.length > 0 && (
        <dl
          data-testid="cron-summary-counts"
          className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3"
        >
          {counts.map(([k, n]) => (
            <KeyValue key={k} label={k} value={String(n)} tabular />
          ))}
        </dl>
      )}
    </div>
