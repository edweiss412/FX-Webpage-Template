...
export function HoverHelp({
  ...
  useLayoutEffect(() => {
    ...
    // (b2) Pinch-zoom pan does NOT fire window scroll (spec §3.1, measured), so
    // the visual viewport is its own event source. Gated on the ENGINE, never on
    // current dimensions: a viewport reporting zero size at open must still be
    // subscribed or its recovery resize can never arrive (spec R13).
