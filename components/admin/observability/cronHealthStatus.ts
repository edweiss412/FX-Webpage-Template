...
export type CronStatusVisual = {
  status: "live" | "positive" | "review" | "warn" | "idle";
  label: string;
};
...
export function effectiveCronStatus(row: CronHealthRow, now: Date): CronStatusVisual {
  ...
  switch (row.outcome) {
    case "ok":
      return { status: "positive", label: `OK · ${rel}` };
    case "partial":
      return { status: "review", label: `Issues · ${rel}` };
    case "infra":
    case "threw":
      return { status: "warn", label: `Failed · ${rel}` };
    default: {
      // malformed: row present but no parseable outcome → fall back to level
