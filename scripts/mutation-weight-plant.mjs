...
const DEFECTS = [
  ...
  [
    "requiredEnv accepts an empty value instead of refusing it",
    "driftCli.ts",
    'if (raw === undefined || raw.trim() === "")',
    "if (raw === undefined)",
    "tests/ci/rateDrift.test.ts",
  ],
  [
    "requiredCount coerces a malformed spelling instead of refusing it",
    "driftCli.ts",
  ...
  [
    "renderDrift reports only the actionable surfaces, dropping the rest",
    "driftCli.ts",
    "for (const d of report.drifted) {",
    "for (const d of report.drifted.filter((x) => x.actionable)) {",
    "tests/ci/rateDrift.test.ts",
  ],
  [
    "renderDrift folds declared-but-unmeasured in with the drifted",
    "driftCli.ts",
    "`declared but UNMEASURED (${String(report.unmeasured.length)}): ` +",
    "`drifted (${String(report.unmeasured.length)}): ` +",
    "tests/ci/rateDrift.test.ts",
  ],
  [
    "renderDrift omits measured-but-undeclared from the output",
    "driftCli.ts",
  ...
  [
    "renderDrift miscounts the actionable surfaces",
    "driftCli.ts",
    "const actionable = report.drifted.filter((d) => d.actionable).length;",
    "const actionable = report.drifted.length;",
    "tests/ci/rateDrift.test.ts",
  ],
  [
