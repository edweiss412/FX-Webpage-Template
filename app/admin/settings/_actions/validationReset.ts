 ...
 * "Reseed validation fixtures" buttons (developer-tier §6 + §6.1: Gate 1
 * swapped requireAdmin → requireDeveloper). Both actions share three gate layers:
 *
 *   1. requireDeveloper()        — session/auth gate (throws DeveloperInfraError on infra fault)
 ...
 *
 * INLINE-TYPED posture (§6.1): the developer gate is the FIRST op inside a
 * top-level try; a DeveloperInfraError is CAUGHT and mapped to the action's
 * typed FAILED code (VALIDATION_RESET_FAILED / VALIDATION_RESEED_FAILED,
 * returned — never thrown), so a validation-project DB outage renders the
 * button's typed denial rather than a 500 boundary. A confirmed non-developer
 * forbidden() digest is re-thrown to the Next boundary.
 *
 * not-subject-to-meta: these are destructive developer server actions, not
 * sync-pipeline read helpers. They destructure { data, error } per invariant 9;
 * RPC errors are mapped to typed codes returned to the caller. No `.from()` /
 * builder pattern is used — the single entry points are direct `.rpc()` calls
...
export async function resetValidationDataAction(): Promise<ValidationActionResult> {
  try {
    ...
    throw err; // Next forbidden() digest / unknown -> boundary (client -> generic denial)
  }
}
...
export async function reseedValidationFixturesAction(): Promise<ValidationActionResult> {
  try {
    ...
    if (err instanceof DeveloperInfraError) return { ok: false, code: "VALIDATION_RESEED_FAILED" };
    throw err; // Next forbidden() digest / unknown -> boundary (client -> generic denial)
  }
}
