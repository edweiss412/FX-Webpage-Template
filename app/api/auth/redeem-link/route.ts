...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  const hash = nonceHash(body.nonce);
  const bootstrapEntries = parseBootstrapCookie(
    parseCookie(request, BOOTSTRAP_COOKIE_NAME),
  );
  const cookieEntry = bootstrapEntries.find(
    (entry) => entry.nonce_hash === hash && entry.show_id === body.show_id,
  );
  ...
  let verified;
  try {
    verified = await verifyLinkJwt(body.token);
  } catch {
  ...
  if (verified.payload.showId !== body.show_id) {
    return jsonError(403, "CSRF_DENIED");
  }
  ...
  response.headers.append(
    "Set-Cookie",
    setSessionCookie(
      encodeSessionCookieValue({ token: opaqueToken, show_id: body.show_id }),
