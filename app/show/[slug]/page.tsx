...
// M10 §B Cluster I-5: tile / context / visibility imports moved to
// `_ShowBody.tsx` along with the render body so this page file is now
// only responsible for auth + data resolution + delegation.
...
export default async function ShowPage({ params }: PageProps) {
  ...
  // M10 §B Cluster I-5: render body extracted to `_ShowBody.tsx` so the
  // preview-as route can reuse the same tile cascade with `kind:
  // 'admin_preview'`. The extraction is structural — no behavior change.
  return <ShowBody slug={slug} showId={showId} viewer={viewer} data={data} />;
}
