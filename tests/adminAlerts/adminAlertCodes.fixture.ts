 ...
 * file). This fixture is a verbatim copy so both that test and any new
 * completeness meta-test (e.g. `_metaAlertIdentityMap.test.ts`) can import a
 * single array without one test reaching into another test file's module.
 *
 * Keep in lockstep with `tests/messages/_metaAdminAlertCatalog.test.ts` — if
 * a new admin_alerts.upsert code is registered there, add it here too.
 */
