...
/** 32 — sheet-data-quality gaps counted by summarizeDataGaps (from GAP_CLASSES). */
...
/** The full persisted-ParseWarning universe (51) — every code lands in exactly one bucket. */
...
describe("data-gap class completeness (drift guard)", () => {
  it("Layer 1 — the 4 buckets are pairwise disjoint and total 52 (32/7/2/11)", () => {
    expect(DATA_GAP_CODES.size).toBe(32);
    expect(BENIGN_WARN_CODES.size).toBe(7);
    expect(BENIGN_INFO_CODES.size).toBe(2);
    expect(ASSET_WARN_CODES.size).toBe(11);
    expect(ALL_PERSISTED_WARNING_CODES.size).toBe(52); // Set dedups → proves pairwise-disjoint
