...
const HAND_EXPECTED: Record<string, number> = {
  ...
  "header-typo|hotel": 1,
  "ref-sub|crew": 4,
  "ref-sub|hotel": 2, // non-empty data cells: 2+2 / 2
  "unicode-inject|crew": 4,
  "unicode-inject|hotel": 1, // ≥2-scalar cells; "3" excluded
  "merged-cell|crew": 4,
  "merged-cell|hotel": 2, // (row.length-1) per ≥3-cell row
  "column-shift|crew": 1,
  "column-shift|hotel": 1, // one per section
  "blank-row:inject|crew": 1, // (dataRows-1); hotel has 1 row → 0
  "blank-row:remove|crew": 1,
  "blank-row:remove|hotel": 1, // one boundary, both domains
};
...
describe("independent applicability audit (Codex R9/R13)", () => {
  ...
  it("every GOLDEN_INVENTORY count is present in the real fixture (sanity; EXACT pin is the excerpt test, plan-R7/R26)", () => {
    ...
    for (const g of GOLDEN_INVENTORY) {
      ...
      expect(
        auditSites(md).get(`${g.op}|${g.domain}`) ?? 0,
        `${g.fixture} ${g.op} ${g.domain}`,
      ).toBeGreaterThanOrEqual(g.count);
    }
  });
  it("every GOLDEN_INVENTORY row has CONCRETE, REAL, LOCALIZING provenance (plan-R10)", () => {
    for (const g of GOLDEN_INVENTORY) {
      ...
      expect(
        g.lines,
        `${g.fixture} ${g.op} ${g.domain} lines must be a concrete range like "40-58"`,
      ).toMatch(/^\d+-\d+$/);
      ...
      expect(
        ...
        `${g.fixture} lines ${g.lines} exceed file length ${all.length}`,
      ...
      expect(
        auditSites(excerpt).get(`${g.op}|${g.domain}`) ?? 0,
        `${g.fixture} ${g.op} ${g.domain} does not localize to lines ${g.lines}`,
      ).toBe(g.count);
    }
  });
  it("GOLDEN_INVENTORY is structurally non-vacuous (plan-R6)", () => {
    ...
    for (const op of CORRUPT)
      expect(ops.has(op), `golden inventory missing operator ${op}`).toBe(true);
    const has = (op: string, domain: string) =>
      GOLDEN_INVENTORY.some((g) => g.op === op && g.domain === domain && g.count >= 1);
...
describe("audit independence is EXECUTABLE (plan-R7/R8/R25)", () => {
  it("NO import/export-from/require/import() in applicabilityAudit.ts RESOLVES to a shared harness module", () => {
    ...
    const forbidden = new Set(
      ["rows", "classify", "operators"].map((m) => resolve(dir, `${m}.ts`)),
    );
    const specifiers = [
      ...src.matchAll(
