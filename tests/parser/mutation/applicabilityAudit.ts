...
function resolve(col0: string): string | null {
  ...
  for (const fam of PREFIX_SECTION_FAMILIES)
    if (n.startsWith(fam) && (n.length === fam.length || /[^A-Z0-9]/.test(n[fam.length] ?? " ")))
      return fam;
  return null;
}
...
function sections(md: string): Sec[] {
  ...
  for (const line of md.split("\n")) {
    ...
    if (cls === "header") {
      cur = {
        domain: DOMAIN_OF[resolve(cells[0]!)!] ?? "other",
        headerToken: (cells[0] ?? "").trim(),
        dataRows: [],
        runIndex,
      };
      out.push(cur);
      ...
      if (!cur) {
        cur = { domain: "other", headerToken: null, dataRows: [], runIndex };
        out.push(cur);
      }
      cur.dataRows.push(cells);
    }
  }
  return out;
}
...
export function auditSites(md: string): Map<string, number> {
  ...
  const bump = (op: string, domain: string, n = 1) =>
    m.set(`${op}|${domain}`, (m.get(`${op}|${domain}`) ?? 0) + n);
...
export function expectedSkipped(md: string, op: string): string[] {
  ...
  return [...auditPresentRiskCritical(md)]
    .filter((d) => (sites.get(`${op}|${d}`) ?? 0) === 0)
    .sort();
}
 ...
 */
export const GOLDEN_INVENTORY: Array<{
  fixture: string;
  op: string;
  domain: string;
  count: number;
  ...
  {
    fixture: "fixtures/shows/raw/2025-10-consultants-roundtable.md",
    op: "ref-sub",
    domain: "crew",
    count: 18,
    lines: "69-76",
  ...
  {
    fixture: "fixtures/shows/exporter-xlsx/rpas.md",
    op: "ref-sub",
    domain: "hotel",
    count: 52,
    lines: "43-57",
