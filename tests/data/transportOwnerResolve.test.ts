...
describe("resolveTransportOwners", () => {
  ...
  it("3. first-name-only 'Doug' resolves via namesRefer", () => {
    expect(resolveTransportOwners(tx({ driver_name: "Doug" }), [crew("doug", "Doug Larson")])).toEqual([
      "doug",
    ]);
  });
  ...
  it("5. leg assigned_names garble resolves", () => {
    expect(
      resolveTransportOwners(tx({ schedule: [leg(["Doug Larson Loadout"])] }), [crew("doug", "Doug Larson")]),
    ).toEqual(["doug"]);
  });
  ...
  it("10. slash-merged 'David Johnson / Doug Larson' resolves Doug", () => {
    expect(
      resolveTransportOwners(tx({ driver_name: "David Johnson / Doug Larson" }), [crew("doug", "Doug Larson")]),
    ).toEqual(["doug"]);
  });
  it("11. name-override + garble resolves via sheet_name alias, not current name", () => {
    ...
    expect(
      resolveTransportOwners(tx({ driver_name: "Doug Larson Loadout" }), [crew("doug", "Doug Newname")]),
    ).toEqual([]);
  });
