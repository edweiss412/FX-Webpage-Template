...
function makeTx() {
  ...
  const tx = {
    ...
    upsertShowsInternal: vi.fn(
      async (_id: string, payload: { run_of_show: Record<string, ScheduleDay> | null }) => {
...
function baseArgs(
  ...
  prior: Record<string, ScheduleDay> | null,
) {
  return {
    driveFileId: "f1",
    parseResult: {
...
describe("applyParseResult — ScheduleDay persist predicate (§7)", () => {
  it("bare-window day AND showStart-only day BOTH survive storage (NOT entries.length>0)", async () => {
    ...
    await applyParseResult(
      tx,
      baseArgs({ "2025-06-25": bareWindow, "2025-06-26": showStartOnly }, null),
    );
  ...
  it("fully-empty day (no entries/showStart/window) is dropped from storage", async () => {
    ...
    await applyParseResult(
      tx,
      baseArgs({ "2025-06-25": titled("7:15am"), "2025-06-26": fullyEmpty }, null),
    );
  ...
  it("AGENDA_DAY_EMPTIED fires ONLY when a prior-stored day becomes fully empty", async () => {
    ...
    const codes = (
      args as { parseResult: { warnings: { code: string; message: string }[] } }
