...
describe("first-seen auto-publish cron/retry parity contract", () => {
  test("retry passes the same Phase 2 args and post-Phase-2 tail shape as cron", () => {
    const cronSource = stripCommentsForFile(
      readFileSync(join(root, "lib/sync/runScheduledCronSync.ts"), "utf8"),
      "lib/sync/runScheduledCronSync.ts",
    );
    const retrySource = stripCommentsForFile(
      readFileSync(join(root, "lib/sync/runManualStageForFirstSeen.ts"), "utf8"),
