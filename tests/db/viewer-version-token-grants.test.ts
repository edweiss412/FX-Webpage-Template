...
describe("public.viewer_version_token(uuid) grants", () => {
  ...
  test("calling viewer_version_token with a synthetic uuid returns text", () => {
    ...
    // it returns '0:0:false' (the to_char of greatest(0, 0, 0), plus the
    // picker_epoch suffix, plus the published component's coalesce-to-'false'
    // arm from the published-toggle migration 20260701000000) — we just
    // assert it returns SOME compound text without erroring.
    const out = runPsql(`
      select public.viewer_version_token('00000000-0000-0000-0000-000000000001'::uuid);
    `);
    expect(out).toMatch(/^[0-9]+:0:false$/);
  });
});
