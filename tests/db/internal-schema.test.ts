  ...
  if (migrationExists) {
    ...
    test("is a fresh-schema create-only artifact for Task 2.2 tables", () => {
      ...
      for (const tableName of adminTables) {
        const createMatches = sql.match(
          new RegExp(String.raw`\bcreate\s+table\s+(?:public\.)?${tableName}\s*\(`, "gi"),
        );
        expect(
          createMatches,
      ...
      for (const publicTable of [
        ...
        expect(sql).not.toMatch(
          new RegExp(String.raw`\bcreate\s+table\s+(?:public\.)?${publicTable}\s*\(`, "i"),
        );
      }
    });
    ...
    test("creates auth and link tables with version, session, and nonce invariants", () => {
      ...
      expectColumn(
        linkSessions,
        "crew_member_id",
        /uuid\s+references\s+(?:public\.)?crew_members\(id\)\s+on\s+delete\s+set\s+null/,
      );
      expectColumn(linkSessions, "signing_key_id", /text\s+not\s+null/);
      expect(sql).toMatch(
        /create\s+index\s+link_sessions_crew_member_id_idx\s+on\s+(?:public\.)?link_sessions\s*\(\s*crew_member_id\s*\)\s*;/i,
      );
      ...
      expect(sql).toMatch(
        /create\s+index\s+bootstrap_nonces_issued_at_idx\s+on\s+(?:public\.)?bootstrap_nonces\s*\(\s*issued_at\s*\)\s*;/i,
      );
    });
    ...
    test("creates pending staging tables with live-vs-wizard partition indexes and approval checks", () => {
      ...
      expect(pendingSyncs).toMatch(
        /constraint\s+pending_syncs_wizard_approved_requires_session\s+check/i,
      );
      expect(pendingSyncs).toMatch(
        /constraint\s+pending_syncs_live_rows_have_no_approval_payload\s+check/i,
      );
      expect(pendingSyncs).toMatch(
        /constraint\s+pending_syncs_approved_requires_full_payload\s+check/i,
      );
    ...
    test("creates runtime settings, deferrals, alerts, watch channels, and report ledgers", () => {
      ...
      expect(sql).toMatch(
        /create\s+unique\s+index\s+deferred_ingestions_live_drive_file_idx\b[\s\S]*where\s+wizard_session_id\s+is\s+null\s*;/i,
      );
      expect(sql).toMatch(
        /create\s+unique\s+index\s+deferred_ingestions_session_drive_file_idx\b[\s\S]*where\s+wizard_session_id\s+is\s+not\s+null\s*;/i,
      );

      expect(sql).toMatch(
        /create\s+unique\s+index\s+admin_alerts_one_unresolved_idx\b[\s\S]*where\s+resolved_at\s+is\s+null\s*;/i,
      );
      expect(sql).toMatch(
        /create\s+index\s+admin_alerts_unresolved_recent_idx\b[\s\S]*where\s+resolved_at\s+is\s+null\s*;/i,
      );

      const watchChannels = tableBody(sql, "drive_watch_channels");
      expect(watchChannels).toMatch(
        /constraint\s+drive_watch_channels_status_check\s+check\s*\(\s*status\s+in\s*\(/i,
      );
      expect(watchChannels).toMatch(
        /constraint\s+drive_watch_channels_active_requires_drive_state\s+check/i,
      );
      expect(sql).toMatch(
        /create\s+unique\s+index\s+drive_watch_channels_one_active_per_folder_idx\b[\s\S]*where\s+status\s*=\s*'active'\s*;/i,
      );
      ...
      expectColumn(
        reports,
        "idempotency_key",
        /uuid\s+not\s+null\s+default\s+gen_random_uuid\(\)\s+unique/,
      );
    ...
    test("creates snapshot, cooldown, finalize, and shadow-change ledgers with required indexes", () => {
      ...
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_temp_prefix_key\s+unique\s*\(\s*temp_prefix\s*\)/i,
      );
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_snapshot_revision_id_key\s+unique\s*\(\s*snapshot_revision_id\s*\)/i,
      );
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_asset_count_check\s+check\s*\(\s*asset_count\s*>=\s*0\s*\)/i,
      );
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_claim_symmetry_check\s+check/i,
      );
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_delete_requires_claim_check\s+check/i,
      );
      expect(snapshotUploads).toMatch(
        /constraint\s+pending_snapshot_uploads_delete_invariant_check\s+check/i,
      );
      ...
      expect(revisionCooldowns).toMatch(
        /primary\s+key\s*\(\s*drive_file_id\s*,\s*raced_head_revision_id\s*\)/i,
      );

      const recoveryCooldowns = tableBody(sql, "recovery_drift_cooldowns");
      expect(recoveryCooldowns).toMatch(
        /primary\s+key\s*\(\s*show_id\s*,\s*preview_revision_id\s*\)/i,
      );
      ...
      expect(finalizeCheckpoints).toMatch(
        /check\s*\(\s*status\s+in\s*\(\s*'in_progress'\s*,\s*'all_batches_complete'\s*,\s*'final_cas_done'\s*\)\s*\)/i,
      );
    ...
    test("creates viewer version token functions and statement-level invalidation triggers", () => {
      expect(sql).toMatch(
        /create\s+or\s+replace\s+function\s+(?:public\.)?bump_last_changed_at\(\)/i,
      );
      expect(sql).toMatch(
        /create\s+or\s+replace\s+function\s+(?:public\.)?publish_show_invalidation_after_statement\(\)/i,
      );
      expect(sql).toMatch(
        /create\s+or\s+replace\s+function\s+(?:public\.)?viewer_version_token\(p_show_id\s+uuid\)/i,
      );
      expect(sql).toMatch(/set\s+search_path\s*=\s*public\s*,\s*pg_temp/i);
      expect(sql).toMatch(
        /revoke\s+all\s+on\s+function\s+(?:public\.)?bump_last_changed_at\(\)\s+from\s+public\s*;/i,
      );
      expect(sql).toMatch(
        /grant\s+execute\s+on\s+function\s+(?:public\.)?viewer_version_token\(uuid\)\s+to\s+authenticated\s*,\s*anon\s*,\s*service_role\s*;/i,
      );
