...
describe("CRON_JOBS parity with pg-cron registry", () => {
  test("CRON_JOBS maps 1:1 onto the 9 fxav_cron_% jobs", () => {
    const raw = JSON.parse(readFileSync(PG_CRON_JSON, "utf8")) as { jobs: Array<{ jobname: string }> };
    const pgNames = new Set(raw.jobs.map((j) => j.jobname).filter((n) => n.startsWith("fxav_cron_")));
