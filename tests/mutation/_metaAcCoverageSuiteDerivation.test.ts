/**
 * The `acCoverage` surface's `suitePaths` is DERIVED, and this is where that is
 * enforced.
 *
 * Plan round 1 finding 4 and round 2 finding 2 are both behind this file. The
 * registry row says the list is derived rather than typed; without a check, that
 * sentence is a promise. An enumeration is CORRECT WHEN WRITTEN and wrong the
 * moment a task adds a fifth suite, and the failure is silent in the direction
 * that looks green: a suite missing from `suitePaths` runs against no mutant,
 * buys ZERO score, and decides nothing, while the gate still reports a number.
 *
 * WHY THIS FILE IS NOT NAMED `acCoverage*.test.ts`, and why it is not under
 * `tests/specLint/`. It would then match its own glob and have to enrol itself,
 * and it decides nothing about the module — it reads a registry and a directory.
 * Same reasoning as its `claimSweep` sibling, which is the template.
 *
...
const surface = GUARD_SURFACES.find((s) => s.id === "acCoverage");

/** Files matching the glob the registry row declares. */
const globMembers = readdirSync(SPEC_LINT_TESTS)
  .filter((f) => f.startsWith("acCoverage") && f.endsWith(".test.ts"))
...
const directImporters = readdirSync(SPEC_LINT_TESTS)
  ...
  .filter((f) =>
    /from "(?:\.\.\/\.\.|@)\/lib\/specLint\/acCoverage"/.test(
...
describe("acCoverage suitePaths is derived, not typed", () => {
  it("PREMISE: the surface is enrolled and the glob is non-empty", () => {
    ...
    // the number of suites this arc shipped, so deleting one fails here rather
    // than quietly shrinking both sides of an equality.
    expect(surface).toBeDefined();
    expect(globMembers.length).toBeGreaterThanOrEqual(4);
    expect(directImporters.length).toBeGreaterThanOrEqual(3);
  });

  it("is EXACTLY the files matching tests/specLint/acCoverage*.test.ts", () => {
