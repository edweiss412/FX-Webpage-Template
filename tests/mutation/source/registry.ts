...
export const GUARD_SURFACES: GuardSurface[] = [
  ...
  },
  {
    ...
    accepted: [
      ...
      {
        siteId: "integer-literal:530:20:0>1",
        kind: "equivalent",
        reason:
          "`group[0]?.dir` to `group[1]?.dir` in clause B's detail message, across BOTH cases the site has. When the group holds two or more bases, every member shares a branch and `dir` is derived from that same branch (`dir: [CORPUS_DIR, ...segments.slice(0, -1)].join(\"/\")` at corpus.ts:131, `branch` from the identical slice at :125), so `group[1].dir === group[0].dir` and the string is byte-identical. When the group holds ONE base, `arcSum` equals that base's own counted rounds, so clause A's `n < ROUND_THRESHOLD` guard (corpus.ts:253) does not fire and clause A either reports it — adding `branch\u0000stage` to `clauseAReported` — or skips it because the arc already carries a section for the stage, which `filedStages` then holds. Both disjuncts make `marked` false at :517, and :412 drops every unmarked total before its detail is read; `scripts/review-economy.ts` never reads `detail` at all. So no marked total can have one base, and no multi-base total can see the change. PROBED over the live corpus: 214 totals produced, 83 with `bases === 1`, and ZERO marked even when `clauseAReported` is passed empty — the setting most favourable to marking. Probed again on constructed fixtures WITH A POSITIVE CONTROL, because a probe that cannot produce the effect proves nothing about its absence: a single-base 4-round arc gives `marked=false` both with and without a filing, while a two-base 2+2 arc gives `marked=true`. HONEST ORIGIN: e10570896 intended to KILL this one (\"the message's directory one\") and the kill did not land — that run reported 11 survivors and killed 7 of them, not 8. The assertion written for it (`_metaReviewRoundEconomy.test.ts:1320`) uses a three-base fixture, where the mutant is invisible by the argument above; its comment claiming otherwise is corrected in the same commit. No test can kill this site, which is what makes it equivalent rather than a gap. FALSIFIER IS EXECUTABLE: if it ever stops surviving, the gate reds on a stale ledger row.",
      },
    ],
  },
  {
