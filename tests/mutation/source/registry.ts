...
export const GUARD_SURFACES: GuardSurface[] = [
   ...
   */
  {
    ...
    accepted: [
      {
        siteId: "statement-removal:1196:9:continue;>(removed)",
        kind: "equivalent",
        reason:
          ...
          "(tests/db/_connectionCensus.ts:1237) only ever asks `reaches.get(f)?.has(input.file)` " +
      ...
      {
        siteId: "statement-removal:1228:13:grew = true;>(removed)",
        kind: "equivalent",
        reason:
          ...
          "(tests/db/_connectionCensus.ts:1217,1222), which signals. The converse does not " +
