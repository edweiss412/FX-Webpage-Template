...
export const ENV_KEY_ALLOWLIST: EnvKeyAllowlist = {
    ...
    values: [
      {
        ...
        governs: [
          ...
          "tests/e2e/admin-upstream-retry.spec.ts",
          ...
          "tests/e2e/forcedColors.spec.ts",
  ...
  PLAYWRIGHT_JSON_OUTPUT_NAME: {
    values: [
      ...
      },
      {
        ...
        // UNfiltered pull_request and its step names the specs below — the same shape as
        // the tap-target value above. The confirm-focus probe joined that step on
        // 2026-08-31, which is why this row names two rather than one.
        governs: [
  ...
  SHARD_BUDGET_SECONDS: {
    ...
    reason:
      ...
      "per-job timeout-minutes of 125, pinned by _metaSourceShardIntegrity at >= 2x this value plus " +
      "a 300 s reporting reserve -- the reserve is load-bearing, since at exactly 2x a leg has " +
      "no time left to upload elapsed.txt and is cancelled silently anyway -- so " +
