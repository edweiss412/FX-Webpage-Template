...
describe("codex-guard signals + spawn errors", () => {
  ...
  it("scenario 14: nonexistent CODEX_GUARD_BIN → exit 3, wrapper_error, spawn_error attempt", async () => {
    ...
    const res = await runGuard(run, [], {
      CODEX_GUARD_BIN: "/nonexistent/codex-binary",
      CODEX_GUARD_BIN_ARGS: "[]",
    });
  ...
  it("scenario 16 (+14b history pin): SIGTERM to wrapper → exit 3, interrupted, group dead incl. TERM-ignoring grandchild, attempts preserved", async () => {
    ...
    writeScenario(run, [
      {
        onCall: 1,
        actions: [{ type: "grandchild" }, { type: "stdout", text: "x" }, { type: "hang" }],
      },
    ]);
    const child = spawn(
      process.execPath,
    ...
    try {
      for (let i = 0; i < 100 && !existsSync(join(run.recordDir, "pid-1.txt")); i++)
        await sleep(50);
      ...
      for (let i = 0; i < 100 && !existsSync(join(run.recordDir, "grandchild-pid-1.txt")); i++)
        await sleep(50);
      ...
      expect(
        isDead(Number(readFileSync(join(run.recordDir, "grandchild-pid-1.txt"), "utf8"))),
      ).toBe(true);
    } finally {
      child.kill("SIGKILL");
    }
