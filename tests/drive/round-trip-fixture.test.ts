...
describe.skipIf(!hasLiveDriveConfig)("M6 real Drive fixture round trip", () => {
  test(
    "xlsx-synthesized markdown parses to the same structure as the canonical fixture",
    async () => {
      ...
      expect(parseSheet(freshMarkdown, fixtureName)).toEqual(parseSheet(fixtureMarkdown, fixtureName));
    },
    60_000,
  );
});
