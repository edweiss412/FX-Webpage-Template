 ...
 * finding 3, which mutation-confirmed a false green in the previous line-only
 * filter: block-commenting the live `timeout_milliseconds` declaration and adding
 * a new one still extracted the OLD value).
 *
 * Concretely, the naive "strip block comments, then line comments" order breaks
 * on THIS migration: line 6 documents a `cron` route path whose glob segment puts
 ...
 * (nestable, unlike C); single-quoted strings with `''` doubling; and
 * dollar-quoted bodies (`$$…$$`, `$tag$…$tag$`) — comment markers inside any
 * string form are DATA and are preserved.
 */
...
describe("renewalLeadMs is the single definition of the lead (§2.1)", () => {
  ...
  test("the heuristic threshold is ordered correctly against the renewal lead", async () => {
    ...
    // The lease we actually request becomes DUE with hours of life still on it —
    // that lead, not the heuristic, is where the safety comes from. Stated as a
    // due-threshold, not as "is renewed": this asserts where the predicate turns
    // true, and nothing here proves any row is examined or renewed at all
    // (whole-diff R8 finding 1).
    expect(
    ...
    // The floor alone already exceeds the heuristic threshold, so any lease whose
    // lead is governed by the floor becomes due while still clear of it. Again a
    // statement about the threshold, not about execution.
