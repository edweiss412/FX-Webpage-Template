...
function fakeTx(): WatchTx {
  ...
  return {
    insertPending: async () => {},
    activatePending: async () => ({ promoted: 1, abortedFolderMismatch: false }),
...
describe("files.watch expiration request (§3.1)", () => {
  test("requests WATCH_TTL_MS ahead of now, as a millisecond timestamp string", async () => {
    ...
    // mistake here would request an expiry ~46 years in the past. What Drive
    // does with that is undefined — its documented 1h default covers an OMITTED
    // expiration only, so the units cannot be left to a benign fallback and are
    // pinned here instead (whole-diff R9 finding 3).
