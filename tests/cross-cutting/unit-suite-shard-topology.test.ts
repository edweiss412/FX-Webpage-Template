...
// ── Boot/install overlap (spec docs/superpowers/specs/ci/2026-08-02-ci-boot-overlap-implementation.md §5)
//
// unit-suite-db backgrounds the Supabase bootstrap, runs `pnpm install` in the
// foreground, and `wait`s on the PID — one shell, one native wait. The whole
// correctness surface is the step's SHAPE plus the premise that the concurrent
// install and the concurrent bootstrap write to disjoint places (spec §3, probed
// three ways). These assertions pin both.
...
const dbSteps = (): Step[] => {
  const doc = parseYaml(YAML) as { jobs?: Record<string, { steps?: Step[] }> };
  return doc.jobs?.["unit-suite-db"]?.steps ?? [];
};
...
const combinedStep = (): Step => {
  const hits = dbSteps().filter((s) => runOf(s).includes("supabase-local-bootstrap.sh"));
  expect(hits.length, "exactly one unit-suite-db step must invoke the shared bootstrap").toBe(1);
  return hits[0]!;
};

const repoFile = (...parts: string[]) => readFileSync(join(process.cwd(), ...parts), "utf8");

/** Fails the audit with the instruction that matters, not just the mismatch. */
const RERUN =
  "the concurrent install's executable surface changed — re-run the write-surface probes in " +
  "docs/superpowers/specs/ci/2026-08-02-ci-boot-overlap-implementation.md §3.2 and update this pin";

describe("unit-suite-db boot/install overlap (spec 2026-08-02 §5)", () => {
  it("(a) steps run in the ratified order, ending boot+install < pnpm-sources < vitest", () => {
    const steps = dbSteps();
    const idxUses = (u: string) => steps.findIndex((s) => s.uses === u);
    const idxRun = (needle: string) => steps.findIndex((s) => runOf(s).includes(needle));
    // The design authority's §4a order, verbatim: the bootstrap needs the Supabase
    // CLI and psql on PATH, and the install needs pnpm + the warm store.
    const order: ReadonlyArray<readonly [string, number]> = [
      ...
      ["assert-pnpm-sources", idxUses("./.github/actions/assert-pnpm-sources")],
      ["vitest", idxRun("vitest run")],
    ];
    for (const [name, i] of order) expect(i, `unit-suite-db needs a ${name} step`).toBeGreaterThanOrEqual(0);
    const idxs = order.map(([, i]) => i);
    expect(
      idxs,
      `unit-suite-db step order is wrong: ${order.map(([n, i]) => `${n}@${i}`).join(", ")}`,
    ).toEqual([...idxs].sort((x, y) => x - y));
  });

  it("(b)+(g) the combined step's body EQUALS the canonical five lines", () => {
    // Equality, not a denylist. Rounds 5-7 of the design spec showed a denylist
    // cannot close the class: `pnpm install ... || kill "$boot_pid"` carries no
    // banned token yet masks the install failure whenever the final wait succeeds.
    expect(normalizedBody(combinedStep()), `the combined step's body must be exactly:\n${COMBINED_BODY}`).toBe(
      COMBINED_BODY,
    );
  });

  it("(c) fail-closed at BOTH the body and the step level", () => {
    const step = combinedStep();
    const body = normalizedBody(step);
    expect(body.split("\n").at(-1), "`wait` must be the step's last command").toBe('wait "$boot_pid"');
    for (const forbidden of [/\|\|\s*true/, /set \+e/, /\bexit 0\b/, /\btrap\b/]) {
      expect(forbidden.test(body), `the combined step's body must not match ${forbidden}`).toBe(false);
    }
    // Body equality alone does not pin failure semantics: an expression-valued
    // continue-on-error, a shell: override, an if:, or a working-directory: each
    // masks or re-points a byte-identical body.
    expect(
      Object.keys(step).sort(),
      "the combined step must carry ONLY name/run",
    ).toEqual(["name", "run"]);
  });

  it("(d) every composite prerequisite is mirrored, derived from the composite itself", () => {
    const composite = parseYaml(repoFile(".github", "actions", "setup", "action.yml")) as {
      runs?: { steps?: Step[] };
    };
    const cSteps = composite.runs?.steps ?? [];
    const usesSteps = cSteps.filter((s) => typeof s.uses === "string");
    const runSteps = cSteps.filter((s) => typeof s.run === "string");
    expect(usesSteps.length, "the setup composite must declare at least one uses: prerequisite").toBeGreaterThan(0);

    const steps = dbSteps();
    const combinedIdx = steps.findIndex((s) => runOf(s).includes("supabase-local-bootstrap.sh"));
    for (const cs of usesSteps) {
      const i = steps.findIndex((s) => s.uses === cs.uses);
      expect(i, `unit-suite-db must mirror the composite's \`${String(cs.uses)}\` step`).toBeGreaterThanOrEqual(0);
      expect(i < combinedIdx, `\`${String(cs.uses)}\` must precede the combined step`).toBe(true);
      // Whole-step equality, so dropping `cache: pnpm` or moving off node 20 fails.
      expect(steps[i]!, `\`${String(cs.uses)}\` must mirror the composite EXACTLY`).toEqual(cs);
    }
    // Exactly one run: step in the composite, and it is the install this job inlines.
  ...
  it("(e) the install runs exactly once, and the setup composite is not also used", () => {
    const steps = dbSteps();
    expect(steps.filter((s) => runOf(s).includes("pnpm install")).length).toBe(1);
    expect(steps.filter((s) => s.uses === "./.github/actions/setup").length).toBe(0);
  });

  it("(f) the shared bootstrap is invoked exactly once, backgrounded", () => {
    expect(normalizedBody(combinedStep())).toContain("bash scripts/ci/supabase-local-bootstrap.sh &");
  });

  it("(h) the concurrent install's DECLARED executable surface is pinned", () => {
    ...
    // The set `pnpm install` executes — NOT `pnpm rebuild`'s: preprepare/postprepare
    // run on install only, and pnpm:devPreinstall is pnpm's own root hook, invisible
    // to every npm lifecycle name.
    for (const k of [
      "preinstall",
      "install",
      "postinstall",
      ...
      expect(pkg.scripts?.[k], `package.json must declare no \`${k}\` script — ${RERUN}`).toBeUndefined();
    }
    expect(pkg.scripts?.prepare).toBe("simple-git-hooks");
    expect(pkg["simple-git-hooks"]).toEqual({ "pre-commit": "pnpm exec lint-staged" });
    // ALLOWLISTS, not denylists. Five review rounds each found one more pnpm setting
    // that enlarges the executing set while allowBuilds compares equal
    // (configDependencies, onlyBuiltDependencies, onlyBuiltDependenciesFile,
    // dangerouslyAllowAllBuilds, packages:). A denylist has no reason to stop.
    expect(pkg.pnpm, `package.json must declare no \`pnpm\` object — ${RERUN}`).toBeUndefined();
    expect(pkg.packageManager, `pnpm's own version selects which hooks run — ${RERUN}`).toBe("pnpm@10.33.2");

    const ws = parseYaml(repoFile("pnpm-workspace.yaml")) as Record<string, unknown>;
    expect(Object.keys(ws).sort(), `pnpm-workspace.yaml may declare ONLY allowBuilds — ${RERUN}`).toEqual([
      "allowBuilds",
    ]);
    expect(ws["allowBuilds"], `the allowBuilds inventory changed — ${RERUN}`).toEqual({
      "@sentry/cli": true,
      esbuild: true,
      sharp: true,
    ...
    // Hook files need no declaration to load, so the allowlists above cannot cover
    // them: pnpm requires a repo-root pnpmfile whether or not any config names it.
    for (const ext of ["cjs", "js", "mjs", "json"]) {
      expect(
        existsSync(join(process.cwd(), `.simple-git-hooks.${ext}`)),
        `a .simple-git-hooks.${ext} config would load BEFORE package.json's — ${RERUN}`,
      ).toBe(false);
      expect(
        existsSync(join(process.cwd(), `simple-git-hooks.${ext}`)),
        `a simple-git-hooks.${ext} config would load BEFORE package.json's — ${RERUN}`,
    ...
    // NAME, so the COMPLETE matching-version set is compared, never a first match.
    const lock = parseYaml(repoFile("pnpm-lock.yaml")) as { packages?: Record<string, unknown> };
    const keys = Object.keys(lock.packages ?? {});
    expect(keys.length, "the lockfile's packages map must be non-empty (anti-vacuity)").toBeGreaterThan(100);
    const versionsOf = (name: string) =>
      keys
        .map((k) => {
          const at = k.lastIndexOf("@"); // the scoped @sentry/cli key contains an earlier one
          return [k.slice(0, at), k.slice(at + 1)] as const;
        })
        .filter(([n]) => n === name)
        .map(([, v]) => v)
        .sort();
    for (const [name, expected] of [
      ...
      expect(versionsOf(name), `${name}'s audited version set changed — ${RERUN}`).toEqual([...expected]);
    }
  });
});
