...
describe("§8.1↔§9 consistency", () => {
  it("every tab in every region spec is in the allowlist", () => {
    for (const id of REGION_IDS)
      for (const tab of REGION_ANCHOR_SPEC[id].tabs) {
  ...
  it("covers all 14 canonical regions and alias targets resolve", () => {
    expect(REGION_IDS.length).toBe(14); // +dress (issue #207 deep-link fix)
    expect(REGION_IDS).toContain("gear_scope"); // gear-parser-fidelity Task 8
    for (const id of REGION_IDS) {
