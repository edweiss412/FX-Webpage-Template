...
function stepper(): string {
  ...
  const done = (n: number, label: string) => `
    ...
    <span data-testid="wizard-step-connector" aria-hidden="true" class="h-px max-w-confirm-box flex-1 rounded-full bg-border-strong"></span>`;
  ...
  return `<nav aria-label="Onboarding progress" data-testid="wizard-step-indicator" class="flex min-w-0 flex-1 items-center gap-2 sm:gap-3">
