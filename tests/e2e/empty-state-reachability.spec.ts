 ...
 *      line 2437).
 *
 * AUTH STATE — TODO migration: this spec is skipped pending the same
 * crew-as-LEAD auth-fixture migration tracked in
 * `tests/e2e/empty-state.spec.ts:83-87`. The `?crew=` query-param mock
 * was retired in Task 5.7 follow-up (Issue 4); rehydrating with
 * `signInAs(NON_ADMIN_CREW_FIXTURE)` requires a per-test crew row whose
 * email matches the fixture's authenticator. Once that migration lands
 * the `.skip` here is dropped and CI generates baselines on first run.
 *
...
async function snapshot(): Promise<Snapshot> {
  ...
  if (crewRes.error || !crewRes.data?.length) {
    throw new Error(`empty-state-reachability.spec: no crew rows`);
  }
  const lead = crewRes.data.find(
    (c) => Array.isArray(c.role_flags) && (c.role_flags as string[]).includes("LEAD"),
  );
  if (!lead) throw new Error(`empty-state-reachability.spec: no LEAD crew`);
...
test.describe.skip("crew page — §8.3 empty-state reachability (Task 9.3, AC-9.2)", () => {
  let s: Snapshot;
  ...
  test("category 1: required-field-missing — venue.name null → placeholder", async ({ page }) => {
    ...
    // Anti-tautology: assert the canonical placeholder copy is present.
    await expect(venueTile).toContainText(/Doug hasn't filled this in yet/i);
