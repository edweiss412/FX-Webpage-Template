...
describe("<TerminalFailure>", () => {
  test("renders cataloged crewFacing copy for the given code", () => {
    const { getByTestId } = render(<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" />);
  ...
  test("renders different copy for a different code (anti-tautology)", () => {
    const { getByTestId } = render(<TerminalFailure code="PICKER_EPOCH_STALE_BANNER" />);
  ...
  test("never renders the raw code in the rendered DOM (AGENTS.md invariant 5)", () => {
    const { container } = render(<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" />);
  ...
  test("renders a custom title override when provided", () => {
    ...
    expect(getByTestId("terminal-failure").textContent).not.toContain("this show");
  });

  test("renders a 'Try again' link when retryHref is provided (recovery affordance)", () => {
    const { getByTestId, queryByTestId } = render(
      <TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" retryHref="/show/sample-show/a" />,
    );
    ...
    cleanup();
    const { queryByTestId: q2 } = render(<TerminalFailure code="PICKER_RESOLVER_LOOKUP_FAILED" />);
  ...
  test("falls back to dougFacing when crewFacing is null", () => {
    ...
    const fallbackCode = (
      Object.entries(MESSAGE_CATALOG) as [
        keyof typeof MESSAGE_CATALOG,
        (typeof MESSAGE_CATALOG)[keyof typeof MESSAGE_CATALOG],
      ][]
    ).find(([, entry]) => entry.crewFacing === null && typeof entry.dougFacing === "string");
    if (!fallbackCode) {
    ...
    expect(getByTestId("terminal-failure").textContent).toContain(entry.dougFacing!);
  });
});
