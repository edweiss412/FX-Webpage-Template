...
describe("NeedsAttentionInbox", () => {
    ...
    const items: NeedsAttentionItem[] = [
      {
        ...
        id: "i1",
        driveFileId: "d1",
        driveFileName: "Bad Sheet",
        copy: GENERIC,
        activityAt: ONE_HR_AGO,
      },
      {
        variant: "first_seen",
        key: "sync:s1",
  ...
  it("two same-variant rows expose distinct accessible names (incl. null-title fallbacks)", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Open show Beta Show (beta-show)" }),
    ).toBeInTheDocument();
  ...
  it("two same-TITLE rows still expose distinct accessible names (unconditional discriminator)", () => {
    ...
    expect(
      screen.getByRole("link", { name: "Open show Gala (gala-2026-east)" }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("link", { name: "Open show Gala (gala-2026-west)" }),
    ).toBeInTheDocument();
    const names = screen.getAllByRole("link").map((l) => l.getAttribute("aria-label"));
    expect(new Set(names).size).toBe(names.length);
  });
      ...
      ingestions: [
        {
          ...
          lastErrorCode: "MI-2_EMPTY_TITLE",
