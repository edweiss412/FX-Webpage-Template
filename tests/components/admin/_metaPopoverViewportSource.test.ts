...
describe("popover placement consumers read the visible viewport, not the layout viewport", () => {
  it("discovers exactly the two known consumers (guards against a walker that finds nothing)", () => {
    const rels = consumers.map((f) => relative(REPO_ROOT, f));
    expect(rels).toContain("components/admin/HoverHelp.tsx");
    expect(rels).toContain("components/admin/showpage/ShareHub.tsx");
  });
  ...
  it("no file under components/ or app/ IMPORTS the placement core", () => {
    ...
    const direct = sourceFiles.filter((f) =>
      IMPORTS_CORE_PLACEMENT.test(stripComments(readFileSync(f, "utf8"))),
    );
