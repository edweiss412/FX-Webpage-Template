...
describe("AdminNav", () => {
  ...
  it("desktop top bar: exactly Dashboard + Settings links, NO needs-attention link (mobileOnly, spec D-2)", () => {
    ...
    // Exactly two inline nav links: every NAV-href link in the top bar is
    // dashboard or settings (the brand link also points at /admin, so count
    // text-bearing nav links instead of raw hrefs).
    const navLinks = topbar
      .getAllByRole("link")
      .filter((l) => /Dashboard|Settings|Needs attention/i.test(l.textContent ?? ""));
    expect(navLinks).toHaveLength(2);
  });

  it("bottom tab bar: three tabs with attention BETWEEN dashboard and settings", () => {
    render(<AdminNav email="d@e.com" alertCount={okAlerts} />);
