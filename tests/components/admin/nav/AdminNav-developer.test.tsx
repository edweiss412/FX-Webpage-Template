...
//
// developer-tier Task 15 (spec §6 row 8) — the observability "Activity" nav
...
// the developer-only Activity/observability destination — the visibility half
...
describe("AdminNav — developer-only nav items (Task 15)", () => {
  it("viewerIsDeveloper={false} → Activity (observability) is absent from desktop AND mobile", () => {
    ...
    expect(topbar.queryByRole("link", { name: /Activity/i })).toBeNull();
    // Mobile bottom tab bar: never renders the observability tab (it is
  ...
  it("viewerIsDeveloper={true} → Activity appears in the desktop nav (still desktopOnly on mobile)", () => {
    render(<AdminNav email="doug@example.com" alertCount={okAlerts} viewerIsDeveloper={true} />);

    const topbar = within(screen.getByTestId("admin-nav-topbar"));
    expect(topbar.getByRole("link", { name: /Activity/i })).toBeInTheDocument();
    // desktopOnly still holds: Activity is not added to the mobile tab bar.
    expect(screen.queryByTestId("admin-bottom-tab-observability")).toBeNull();
  ...
  it("default (viewerIsDeveloper omitted) → Activity hidden (safe default)", () => {
