...
describe("StatStrip", () => {
  it("renders 4 stats with tabular-nums; live dot only when liveCount>0", () => {
    const { rerender } = render(
      <StatStrip activeCount={5} liveCount={2} needReviewCount={3} crewTotal={40} statsScope="global" />,
    );
    for (const id of ["stat-value-active", "stat-value-live", "stat-value-review", "stat-value-crew"]) {
    ...
    rerender(
      <StatStrip activeCount={5} liveCount={0} needReviewCount={0} crewTotal={40} statsScope="global" />,
    );
  ...
  it("renders 0 (not blank) for zero counts", () => {
    render(
      <StatStrip activeCount={0} liveCount={0} needReviewCount={0} crewTotal={0} statsScope="global" />,
    );
  ...
  it("statsScope='shown' labels Live + Crew as not-global (degraded); global does not", () => {
    const { rerender } = render(
      <StatStrip activeCount={600} liveCount={3} needReviewCount={1} crewTotal={50} statsScope="shown" />,
    );
    expect(within(screen.getByTestId("stat-cell-live")).getByText(/shown shows/i)).toBeInTheDocument();
    expect(within(screen.getByTestId("stat-cell-crew")).getByText(/shown shows/i)).toBeInTheDocument();

    rerender(
      <StatStrip activeCount={5} liveCount={3} needReviewCount={1} crewTotal={50} statsScope="global" />,
    );
